...
### Disabling attribute protection

```python
crud_factory(SQLA, Domain, Create, Patch, strict_attrs=False)

```

### More control for model converting

Now you can control converting proccess

```python

class UserRepository(crud_factory(UserSQLA, UserDomain, UserCreate, UserPatch, strict_attrs=False)):
    def to_repr(self, object: SA) -> DM:
        object.extra_attr = get_attr()
