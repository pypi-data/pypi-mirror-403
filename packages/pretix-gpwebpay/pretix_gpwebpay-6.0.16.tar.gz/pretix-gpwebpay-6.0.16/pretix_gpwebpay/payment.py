...
class GPWebPay(BasePaymentProvider):
    ...
    def payment_control_render(self, request, payment: OrderPayment):
        ...
        """
        ws_info = (payment.info or {}).get('gpwebpay_ws', {}) if hasattr(payment, 'info') else {}
        sync_form = ""
    ...
    def sync_payment_from_ws(self, payment: OrderPayment) -> str:
        ...
        response_data = self._parse_ws_response(response_xml, 'paymentDetailResponse')
        ...
        payment.info = payment.info or {}
        payment.info['gpwebpay_ws'] = {
            'message_id': response_data.get('messageId'),
            'state': response_data.get('state'),
            'status': status,
