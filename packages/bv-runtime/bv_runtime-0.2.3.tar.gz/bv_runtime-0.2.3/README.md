...
The runtime reads the following environment variables (set automatically by the runner or `bv run`):

| Variable | Description |
|----------|-------------|
| `BV_SDK_RUN` | Must be `1` to enable runtime APIs. Set by `bv run`. |
| `BV_BASE_URL` | Orchestrator base URL (e.g., `https://cloud.botvelocity.com`). |
| `BV_ORCHESTRATOR_URL` | Alternative to `BV_BASE_URL` (legacy). |
| `BV_ROBOT_TOKEN` | Robot API token for authentication. Present in runner mode. |
| `BV_ROBOT_NAME` | Name of the executing robot. |
| `BV_MACHINE_NAME` | Name of the host machine. |
| `BV_JOB_EXECUTION_ID` | Unique ID for the current job execution. |
| `BV_TENANT_ID` | Tenant ID for multi-tenant scoping (optional). |
| `BV_FOLDER_ID` | Folder ID for folder-based scoping (optional). |
| `BV_PROJECT_TYPE` | Project type (`rpa` or `agent`). |
