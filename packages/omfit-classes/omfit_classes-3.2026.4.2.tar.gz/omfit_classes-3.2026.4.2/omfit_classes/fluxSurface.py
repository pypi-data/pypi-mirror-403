...
class fluxSurfaces(SortedDict):
    ...
    def calc_poloidal_angle(self, npts=None, method="equal_arc", nint=1024, levels=None, make_plot=False, ax=None, plot_kw=None):
        """Method to calculate the poloidal angle, $\theta$, around flux surfaces.

        There are four supported methods for calculating the poloidal angle,
        - 'equal_arc': \theta points are uniformly spaced along flux surfaces such that there is equal poloidal
                        arc-length between \theta[i] and \theta[i+1]
        - 'straight_line': the magnetic field becomes a straight line in (\theta, \phi) space (PEST coordinate).
        - 'hammada': \theta points are spaced such than an equal volume is enclosed between \theta[i] and \theta[i+1].
        - 'boozer': \theta points associated with a Jacobian with special differential properties.

        Example,
        >> theta = fluxSurfaces.calc_poloidal_angle(method="equal_arc")

        WARNING: this method sorts flux-surface arrays (R, Z, Bp, etc.) such that,
        - theta[0]: $\theta = -pi$ (HFS midplane).
        - theta[npts//2]: $\theta = 0$ (LFS midplane).
        - theta[-1]: $\theta = pi - \Delta\theta$ (~HFS midplane).

        :param npts: (int or None, optional) number of points in theta to use on every surface.
            if None, the number of flux surface points is not altered.
        :param method: (str, optional) one of: 'equal_arc', 'straight_line', 'hammada', 'boozer'.
        :param nint: (int, optional) number of temporary points to use for \theta integration.
            NOTE: High nint may be necessary to accurately integrate near the edge.
        :param levels: (list, optional) list of levels to compute \theta. If None, all levels are used.
        :param make_plot: (bool, optional)
        :param ax: (None or matplotlib.Axes, optional) Axes instance to be used for plot, if None a new one is created.
        :param plot_kw: (dict or None, optional) kwargs **passed to plot, color is ignored.
        :return theta: (list of arrays) containing the \theta array for each flux surface level.
        """
        if not self.quiet:
            printi("Calculating the poloidal angle...")
        ...
        if method == "equal_arc":
            ...
            options = ['equal_arc', 'straight_line', 'hammada', 'boozer']
            print(f"ERROR (fluxSurfaces.calc_poloidal_angle): unknown poloidal_angle method: {method}, must be one of: {options}")
            return

        if npts is not None:
            self.resample(npts=npts)

        if levels is None:
            levels = range(self.nc)
        else:
            # convert from 'levels' (e.g. psi_n or rhon values) to indices,
            levels = [np.argmin(abs(l - self["levels"])) for l in levels]
        ...
        for l in levels:
            ...
            if npts is None:
                ...
                _npts = npts
            R, Z = self['flux'][l]['R'], self['flux'][l]['Z']
            R0, Z0 = self["R0"], self["Z0"]
            theta_geo = np.arctan2(Z - Z0, R - R0)  # geometric \theta.

            # Sort all arrays by the geometric \theta,
            srt_inds = np.argsort(theta_geo)  # [-pi, ..., pi]
            theta_geo = theta_geo[srt_inds]
            # High-res (temporary) \theta grid for integration,
            ti = np.linspace(0, 2 * np.pi, nint, endpoint=False)
            for q in self["flux"][l]:
                quant = self["flux"][l][q]
                if isinstance(quant, (float, int)):
                    continue
                if len(quant) == n:
                    self['flux'][l][q] = np.interp(ti, theta_geo, quant[srt_inds], period=2 * np.pi)
            # new, high res values,
            R = self['flux'][l]['R']
            Z = self['flux'][l]['Z']
            dl = np.sqrt(np.ediff1d(R, to_begin=0.0) ** 2 + np.ediff1d(Z, to_begin=0.0) ** 2)
            Br = self['flux'][l]['Br']
            Bz = self['flux'][l]['Bz']
            F = self['flux'][l]['F']  # scalar

            signBp = signTheta * np.sign((Z - Z0) * Br - (R - R0) * Bz)
            Bp = signBp * np.sqrt(Br**2 + Bz**2)
            Bt = F / R
            B = np.sqrt(Bp**2 + Bt**2)
            # Given Bp, it's easier to go backwards to $|\nabla\Psi| = RB_p$
            mod_dpsi = np.abs(R * Bp)

            # Compute the Jacobian,
            J = (R**i) / (mod_dpsi**j * B**k)
            integrand_dlp = R / np.abs(J * mod_dpsi) * dl
            I = np.sum(integrand_dlp)  # Integral around the entire flux surface.
            s = 2 * np.pi / I  # Normalization constant to make theta \in (0, 2pi)
            theta = s * np.cumsum(integrand_dlp)  # Actual poloidal angle, high-res.

            # Re-sort and Interpolate onto final theta grid,
            srt_inds = np.argsort(theta)  # [0, ... 2pi]
            theta = theta[srt_inds]
            # Final, possibly lower-res \theta grid from [-pi, ... pi-eps]
            theta_final = signTheta * np.linspace(-np.pi, np.pi, _npts, endpoint=False)
            ...
            for q in self["flux"][l]:
                ...
                if isinstance(quant, (float, int)):
                    continue
                if len(quant) == nint:
                    quant = np.interp(theta_final, theta, quant[srt_inds], period=2 * np.pi)
                    self['flux'][l][q] = quant
            # Any other quantities which are arrays around the flux surface must be updated,
            # otherwise all this sorting will scramble them.
            self.dl[l] = np.interp(theta_final, theta, dl[srt_inds], period=2 * np.pi)

            self["flux"][l]["theta"] = theta_final
            all_theta += [theta_final]
        ...
        if make_plot:
            ...
            if ax is None:
                fig, ax = pyplot.subplots(1, 1, num="fluxSurfaces.calc_poloidal_angle")
            # use degrees here to be more readable, actual \theta is stored in radians.
            theta_to_plot = np.arange(-180, 180, 10)
            cmap = pyplot.get_cmap("rainbow")
            norm = Normalize(-180, 180)
            sm = ScalarMappable(cmap=cmap, norm=norm)
            pyplot.colorbar(sm, ax=ax, ticks=theta_to_plot[::3], label=r"$\theta$ [deg]")
            for t in theta_to_plot:
                Zt, Rt = [], []  # Z, R for fixed \theta.
                for l in levels:
                    theta = self['flux'][l]["theta"]  # local \theta for this level.
                    i = np.argmin(abs(theta - t * np.pi / 180))
                    Zt += [self['flux'][l]["Z"][i]]
                    Rt += [self['flux'][l]["R"][i]]
                ax.plot(Rt, Zt, '-', color=cmap(norm(t)), **plot_kw)
            ax.set_aspect("equal")
