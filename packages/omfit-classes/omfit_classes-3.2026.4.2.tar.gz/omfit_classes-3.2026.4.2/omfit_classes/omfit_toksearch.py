...
# CONSTANTS
LOGIN_NODE_LIMIT = 5  # max gb fetch on omega login node, set by policy
SERVER_MEM = {'omega': 120, 'saga': 120}  # TO DO: need to use remote_sysinfo to find this
...
executables = {
    'omega': 'sbatch %s',
    'saga': 'sbatch %s',
    'omega_batch': bash_script % (TOKSEARCH_VERSION),
    'saga_batch': bash_script % (TOKSEARCH_VERSION),
}

login_nodes = set(('omega-a', 'omega-b'))
...
def server_info():
    ...
    return {
        'start_memory_usage': getrusage(RUSAGE_SELF).ru_maxrss,
...
def WARN(size):
    message = "WARNING: It is estimated that you are requesting approx. %.2f GB of data. Are you sure you want to do this?" % (
        size / (1024**3)
    )
...
def NOTIFY(size):
    if SERVER_NAME in login_nodes:
        message = (
            "ERROR: You are trying to pull back approx. %.2f GB of data. This will put you beyond the allowed limit of %s GB of accumulated data on a login node. \nPlease reduce your query size, or use SLURM que or another machine to run OMFIT."
            % (size * 1e-9, LOGIN_NODE_LIMIT)
        )
    else:
        message = "ERROR: You are trying to pull back approx. %.2f GB of data. Which is beyond the available limit on your machine." % (
            size / (1024**3)
        )
    from omfit_classes.OMFITx import Dialog
...
def toksearchQueryRemote(
    ...
    **compute_kwargs,
    ...
    queue_args.setdefault('partition', 'short' if is_server(serverPicker, 'omega') else 'nodes')  # type of queue resource
    ...
    data_files, mem_usage, data_size, ret_code = remote_python(
        None,
