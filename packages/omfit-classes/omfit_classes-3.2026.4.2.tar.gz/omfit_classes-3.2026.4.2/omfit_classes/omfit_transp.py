...
class OMFITtranspData(SortedDict):
    ...
    def __init__(self, transp_output, data):
        ...
        # MDSplus
            ...
            # time needs to be treated special since in MDSplus 'TIME' is stored ad TIME1D and TIME2D
...
def next_available_TRANSP_runid(runid, project, increment=1, skiplist=[], server=None):
    """
    Function which checks what MDSplus tree entries are available

    :param runid: runid to start checking from (included)

    :param project: project [e.g. D3D, ITER, NSTX, ...] used to set the MDSplus TRANSP tree and server to be queried

    :param increment: positive / negative

    :param skiplist: list of runids to skip

    :param server: MDSplus TRANSP server to be queried [e.g. 'atlas.gat.com', or 'transpgrid.pppl.gov']
    ...
    # handle MDSplus server/treename
    if server == None:
...
def vint(d, dvol=None):
    ...
    :param d: OMFITtranspData object from the MDSplus TRANSP OUTPUTS.TWO_D tree.
...
def vder(d, dvol=None):
    ...
    :param d: OMFITtranspData object from the MDSplus TRANSP tree.
    :type d: OMFITtranspData
    :param dvol: OMFITtranspData object 'dvol' from the mds TRANSP tree.
...
def set_grid(d, zone='X', dvol=None):
    ...
    :type zone: str
    :param dvol: OMFITtranspData object 'dvol' from the MDSplus TRANSP tree.
            If None, will be taken from Data's MDStree.
