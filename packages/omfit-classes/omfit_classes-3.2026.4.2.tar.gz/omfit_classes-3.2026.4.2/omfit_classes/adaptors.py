...
class ONETWO_statefile_adaptor(OMFITinterfaceAdaptor):
    ...
    def definitions(self, key, data):
        ...
        if self.register(key, 'darea', 'Gradient of cross sectional area enclosed by each flux surface', 'm^2', 'rt'):
            return gradient(self['area'], axis=0)
        ...
        if self.register(
            key, ['Btot_out', 'total B field out', 'B magnitude out', 'btot_out'], 'B field magnitude at outboard midplane', 'T', 'r'
        ):
...
class ONETWO_MDS_adaptor(OMFITinterfaceAdaptor):
    ...
    def definitions(self, key, data):
        ...
        if self.register(key, ['psin', 'psi_n'], 'Normalized poloidal flux', '1', 'rt'):
            return (self['psi'] - np.min(self['psi'], axis=0)[np.newaxis, :]) / (
                np.max(self['psi'], axis=0)[np.newaxis, :] - np.min(self['psi'], axis=0)[np.newaxis, :]
            )
        ...
        if self.register(key, ['dvol', 'dV/drho', 'differential volume'], 'Differential volume', 'm^3', 'rt'):
            return gradient(self['volume'], axis=0) / gradient(self.get('rho', 'rt'), axis=0)

        if self.register(key, 'pprime', 'dp/dpsi', 'N/m^2', 'rt'):
            return gradient(self['pressure'], axis=0) / gradient(self['psi'], axis=0)
        ...
        if self.register(key, ['Wtot', 'energy'], 'Total energy', 'J/m^3', 'rt'):
            return data['EQU']['PRESS_KIN'].data() * 3 / 2.0
        ...
        if self.register(key, 'ion species', 'Ion species', '1', 'i'):
            return data['INPUTS']['NAMELIS1']['NAMEP'].data()[0][0].strip("'").split(',') + data['INPUTS']['NAMELIS1']['NAMEI'].data()[0][
                0
            ].strip("'").split(',')
        ...
        if self.register(
            key, ['Qe', 'total anomalous electron heat flux'], 'Total anomalous electron heat flux (Qconde + Qconve - Qx)', 'W/m^2', 'rt'
        ):
            return data['DIFF']['E_FLUXE'].data()

        if self.register(
            key, ['Qi', 'total anomalous ion heat flux'], 'Total anomalous ion heat flux (Qcondi + Qconvi + Qx)', 'W/m^2', 'rt'
        ):
            ...
            for dd in ['r_min', 'rho_m', 'psi', 'rho']:
                if self['__quantities__'][k]['raw_dimension'] in ['rt', 'irt', 'brt']:
                    if self.register(
                        key,
                        ['d(' + k + ')/d(' + dd + ')'],
                        'd/dr(' + self['__quantities__'][k]['description'] + ')',
                        self['__quantities__'][k]['units'] + '/' + self['__quantities__'][dd]['units'],
                        self['__quantities__'][k]['raw_dimension'],
                    ):
...
class ONETWO_statefiles_adaptor(OMFITinterfaceAdaptor):
    ...
    def definitions(self, key, data):
        ...
        def interp_npsi_vars_to_rho(val):
            ...
            rho_mhd = self.gather_timeslices(
                'rho_mhd_gridnpsi', data
            )  # rho_mhd and val had both been modified using flipud. I think this just reverses 1d arrays
        ...
        if self.register(key, 'a', 'Plasma midplane minor radius', 'm', 't'):
            result = interp_npsi_vars_to_rho(self.gather_timeslices('rminavnpsi', data))[
                -1, :
            ]  # -1 to select the end of the position array
            printd(' >> result = {:}'.format(result))
            return result
        ...
        if self.register(key, ['psin', 'psi_n'], 'Normalized poloidal flux', '1', 'rt'):
            posaxis = 0
            return (self['psi'] - self['psi'].min(axis=posaxis)[np.newaxis, :]) / (
                (self['psi'].max(axis=posaxis) - self['psi'].min(axis=posaxis))[np.newaxis, :]
            )
        ...
        if self.register(key, ['rout', 'r_out'], 'Geometric midplane outer major radius', 'm', 'rt'):
            return interp_npsi_vars_to_rho(
                self.gather_timeslices('rmajavnpsi', data) + self.gather_timeslices('rminavnpsi', data)
        ...
        if self.register(key, 'delta', 'Triangularity', '1', 'rt'):
            return interp_npsi_vars_to_rho(
                0.5 * self.gather_timeslices('triangnpsi_l', data) + 0.5 * self.gather_timeslices('triangnpsi_u', data)
            )
        ...
        if self.register(key, ['dvol', 'dV/drho', 'differential volume'], 'Differential volume', 'm^3', 'rt'):
            ...
            return gradient(self['volume'], axis=positionaxis) / gradient(self['rho'], axis=positionaxis)

        if self.register(key, 'pprime', 'dp/dpsi', 'N/m^2', 'rt'):
            positionaxis = 0
            return gradient(self['pressure'], axis=positionaxis) / gradient(self['psi'], axis=positionaxis)
        ...
        if self.register(
            key, ['Btot_out', 'total B field out', 'B magnitude out', 'btot_out'], 'B field magnitude at outboard midplane', 'T', 'rt'
        ):
...
class TRANSP_MDS_adaptor(OMFITinterfaceAdaptor):
    ...
    def definitions(self, key, data):
        ...
        if self.register(key, 'rho_m', 'Minor radius', 'm', 'rt'):
            return self.tr2D('RBOUN', data) / 100.0

        if self.register(key, 'a', 'Plasma midplane minor radius', 'm', 't'):
            return self['r_min'][-1, :]

        if self.register(key, 'r_min', 'Geometric midplane minor radius', 'm', 'rt'):
            return self.tr2D('RMNMP', data) / 100.0

        if self.register(key, ['r_geo', 'r_maj'], 'Geometric midplane major radius', 'm', 'rt'):
            return self.tr2D('RMJMP', data) / 100.0
        ...
        if self.register(
            key,
            ['j.B fsa', 'jboot.B', 'bootstrap current times B fsa'],
            'Flux surf avg of B . neoclassical bootstrap current density <Jtor R0/R>',
            'T*A/m^2',
            'rt',
        ):
        ...
        if self.register(key, ['Pth', 'thermal pressure'], 'Thermal pressure', 'N/m^2', 'rt'):
            return self['thermal energy'] * 2 / 3.0
        ...
        if self.register(key, 'ion species', 'Ion species', '1', 'i'):
            ...
            for s in [
                'C',
                'Ne',
            ]:  # List of impurity species to search for
        ...
        if self.register(key, ['resist'], 'Sauter neoclassical resistivity', 'Ohm m', 'rt'):
            return self.tr2D('ETA_SNC', data) / 100.0

        if self.register(key, ['spitzer'], 'Spitzer resistivity', 'Ohm m', 'rt'):
            return self.tr2D('ETA_SP', data) / 100.0
...
class pFile_adaptor(OMFITinterfaceAdaptor):
    ...
    def definitions(self, key, data):
        ...
        if self.register(key, 'Zeff', 'Effective impurity charge', 'amu', 'r'):
            ...
            nb = self['nb']
            Zi = data['N Z A']['Z']
            return (ni[1] * Zi[0] ** 2 + nb[0] * Zi[1] ** 2 + ni[0] * Zi[2] ** 2) / (  # impurity  # beam  # main
                ni[1] * Zi[0] + nb[0] * Zi[1] + ni[0] * Zi[2]
            )
