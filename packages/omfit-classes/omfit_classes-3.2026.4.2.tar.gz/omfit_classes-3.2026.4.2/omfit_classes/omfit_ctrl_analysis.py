...
class ControlAnalysis(OrderedDict):
    ...
    def make_guess(self, x, y, u, order, guess=None, variable_gain=False, modified_y=False):
        ...
        if dy == 0:
            y0range = [-np.inf, np.inf]
        else:
            y0range = [y.min() - dy, y.max() + dy]
        if du == 0:
            gain_range = [-np.inf, np.inf]
        else:
            gain_range = [-dy / du * 10.0, dy / du * 10.0]
        if dx == 0:
            lag_range = [0, np.inf]
            scale_range = [min_scale, np.inf]
        else:
            lag_range = [0, dx * 0.9]
            scale_range = [min_scale, dx * 0.9]
        if order > 0:
            g.add('y0', value=guess[0], min=y0range[0], max=y0range[1])
            g.add('gain', value=guess[1], min=gain_range[0], max=gain_range[1])
            g.add('lag', value=guess[2], min=lag_range[0], max=lag_range[1])
            g.add('scale', value=guess[3], min=scale_range[0], max=scale_range[1])
            if order > 1:
    ...
    def model_residual(self, par, x, u, y, y_err=None, order=None, just_model=False):
        ...
        if nx2 == 0:
            residual = residual.T
        model = copy.copy(residual)
        model[:] = np.NaN
        for i in range(max([1, nx2])):
            if nx2 == 0:
                sel = ~np.isnan(x) & ~np.isnan(y) & ~np.isnan(u)
                xi = x[sel]
                yi = y[sel]
                ui = u[sel]
            else:
                ...
                ui = u[sel, i]
            if order == 1:
                model_ = self.p_first_order(par, xi, ui)
            elif order == 2:
                model_ = self.p_second_order(par, xi, ui)
            elif order == 3:
                model_ = self.p_third_order(par, xi, ui)
            else:
                raise OMFITexception('order must be 1, 2, or 3')
            if not just_model:
                self['__scratch__'][f'yi_{i}'] = yi
                self['__scratch__'][f'model_{i}'] = model_
                self['__scratch__']['i'] = i
                self['__scratch__']['residual'] = residual
            residual[sel, i] += yi - model_
            model[sel, i] = model_
        if nx2 == 0:  # Collapse unused extra dimension if x, y, u were purely 1d
            residual = residual[:, 0]
            model = model[:, 0]

        self['__scratch__']['model'] = model
    ...
    def fit_response_model(self, x, y, u, y_err=None, order=2, guess=None, name=None, npts=None, method=None, **kw):
        ...
        out['npts'] = npts
        xnew = np.linspace(np.nanmin(x), np.nanmax(x), npts2)
        if len(np.shape(x)) > 1:
            nx2 = np.shape(x)[1]
        ...
        with warnings.catch_warnings():
            ...
            fit_out = out['fit_out'] = minimize(self.model_residual, g, args=(x, u, y, y_err, order), method=method, nan_policy=nan_policy)
        out['params'] = fit_out.params
        out['model_y'] = self.model_residual(fit_out.params, x, u, x * 0, y_err=None, order=order, just_model=True)
    ...
    def plot_fit_response(self, order=None, x=None, y=None, u=None, y_err=None, extra_data=None, split=None, **kw):
        ...
        axs[0, 0] = pyplot.subplot(rows, columns, 1)
        for i in range(1, columns * rows):
            axs[i // columns, i % columns] = pyplot.subplot(rows, columns, i + 1, sharex=axs[0, 0])
        if split:
            axu = axs[:, 0]
            axy = axs[:, int(cmd)]
        else:
