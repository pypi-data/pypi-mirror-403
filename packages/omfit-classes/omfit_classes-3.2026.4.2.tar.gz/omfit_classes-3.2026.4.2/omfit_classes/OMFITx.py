...
def ListEditor(
    ...
    if sys.platform.lower() == 'darwin':
        olx.configure(yscrollincrement=6)
        slx.configure(yscrollincrement=6)

        def mouse_wheel(event, what):
            if event.delta != 0:
                # respond to aqua wheel event
                what.yview('scroll', -1 * event.delta, 'units')
            else:
                # respond to X11 wheel event
                if event.num == 5:
        ...
        def mouse_wheel(event, what):
            # respond to Windows wheel event
            if event.delta < 0:
                what.yview('scroll', -1 * event.delta // 120, 'units')
            elif event.delta > 0:
                what.yview('scroll', -1 * event.delta // 120, 'units')
            return 'break'
...
echo TS=`which ts | grep -v "not found"`
echo '------------ENV------------'
env
echo '------------ENV------------'
'''.strip()
        ...
        for k in range(len(std_out)):
            if env is None:
                ...
                elif std_out[k].startswith('TS='):
                    OMFITaux['sysinfo'][id_]['TS'] = std_out[k].split('=')[1].strip()
...
def _remote_upsync(server, local, remote, tunnel=None, ignoreReturnCode=False, quiet=False):
    ...
    if not len(server):
        ...
        while len(upsync):
            ...
            printd(command, topic='sync_comms')
...
def _remote_downsync(server, remote, local, tunnel=None, ignoreReturnCode=False, quiet=False):
    ...
    if not len(server):
        remote = [re.sub(' ', r'\ ', x).rstrip('./') for x in remote]
        ...
        cp_dir = builtins.any(os.path.isdir(x) for x in remote)
        downsync = system_executable('cp') + f' -Rfp{"T" if cp_dir else ""}' + ' {} ' + local
        ...
        # Newer versions of Rsync are more strict about host:file needing the colon for remote files.
        # Pre-Sequoia MacOS versions do not support prefixing with ':' as they get converted to path separators.
        # Later MacOS version can use ':' with rsync as a path relative to the server as they appear to have switched to
        # openrsync, which has a slightly different interface even for the same rsync version and protocol number.
        # Externally sourced 3.X versions on MacOS require the colon for remote files like other systems do.
        # However, MacOS can not have ':' as part of their file name in any version.
        remote_files_with_colon = True
        remote_placeholder = "{} "

        rsync_command = system_executable('rsync', True)
        if platform.system() == 'Darwin' and rsync_command['progress'] != '--info=progress2':  # progress2 indicates updated rsync
            if int(os.uname().release.split('.')[0]) < 24:  # Using earlier than Sequoia
                remote_files_with_colon = False
                remote_placeholder = ":'{}' "

        downsync = (
            "{path} {progress} --copy-links --update --inplace --compress --recursive -e '".format(**rsync_command)
            + sshOptions(ssh_path=SERVER['localhost'].get('ssh_path', None))
            + controlmaster(username, server, port, server0, ssh_path=SERVER['localhost'].get('ssh_path', None))
            + " -q -p "
            + port
...
def mdsvalue(server, treename=None, shot=None, TDI=None):
    """
    :param server: MDSplus server to connect to

    :param treename: name of the MDSplus tree
...
class manage_job(object):
    def __init__(
        ...
        qstat_command=None,
        qstat_findStatus=r'(?i)\s+(?:running|[rqwecpd]{1,2})\s+',
        qdel_command=None,
    ):
    ...
    def identify_queuing_system(self):
        ...
        if self.qstat_command is None or self.qdel_command is None:
            ...
            if sysinfo['SQUEUE']:
                ...
                if self.qstat_command is None:
                    self.qstat_command = 'ts -l'
                if self.qdel_command is None:
                    self.qdel_command = 'ts -r'
