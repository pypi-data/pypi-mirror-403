...
class reading_data:
    ...
    def read_xlsx_with_pandas(self, xlsx_path: str, schema: T.StructType, sheet_name=0):
        ...
        # Caso a conversão via Arrow falhe, o Spark automaticamente
        # cai para o modo seguro na próxima chamada.


        try:
            self.log.warning(
                "Databricks Serverless may emit an internal configuration warning. "
                "This message is safe to ignore and does not affect execution."
            )
            string_schema = T.StructType([T.StructField(c, T.StringType(), True) for c in df.columns])
            sdf = self.spark.createDataFrame(df, schema=string_schema)

        except Exception as e:
            # Mensagem genérica para o usuário
            self.log.warning("Conversion could not be completed. Proceeding with a safe fallback.")

            # Log técnico para debug interno
            self.log.debug(f"Internal error while creating the spark DataFrame.: {e}")

            # Fallback seguro (Spark já desabilitou Arrow internamente)
