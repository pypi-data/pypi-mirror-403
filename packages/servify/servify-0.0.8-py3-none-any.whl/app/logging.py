...
class SuppressSparkConfigWarnings(logging.Filter):
    """
    Suprime avisos internos do Spark relacionados a
    'CONFIG_NOT_AVAILABLE', comuns no Databricks Serverless.
    Também registra uma mensagem informativa personalizada.
    """

    def __init__(self):
        super().__init__()
        self._already_logged = False  # evita repetir a mensagem

    def filter(self, record: logging.LogRecord) -> bool:
        msg = record.getMessage()

        if "CONFIG_NOT_AVAILABLE" in msg:
            if not self._already_logged:
                loguru_logger.info(
                    "Databricks Serverless emitted an internal configuration warning. "
                    "This message is safe to ignore and does not affect execution."
                )
                self._already_logged = True

            return False  # suprime o aviso original
...
class Logger:
    ...
    def _configure_logger(self) -> None:
        ...
        spark_internal_logger = logging.getLogger("py4j")
        spark_internal_logger.addFilter(SuppressSparkConfigWarnings())
