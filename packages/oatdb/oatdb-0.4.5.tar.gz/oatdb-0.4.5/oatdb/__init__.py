...
class FunctionCall(ABC):
    ...
    def _serialize_for_hash(obj):
        ...
        if isinstance(obj, FunctionCall):
            ...
            # Handle objects with to_json method (like validate_options)
            return obj.to_json()
...
class filter_assignments(FunctionCall):
    ...
    lower_geq: RefOrValue
    upper_leq: RefOrValue
    ...
    def json_output(self) -> list:
        earlier_builds = []
        if issubclass(self.lower_geq.__class__, FunctionCall):
            earlier_builds += self.lower_geq.json_output()
        
        if issubclass(self.upper_leq.__class__, FunctionCall):
            earlier_builds += self.upper_leq.json_output()
        ...
        return earlier_builds + [
            {
                "fn": self._fn(),
                "args": {
                    "lower_geq": self._ref_or_value(self.lower_geq),
                    "upper_leq": self._ref_or_value(self.upper_leq),
...
@dataclass(eq=False)
class get_ids_from_assignments(FunctionCall):

    assignments: RefOrValue

    def json_output(self) -> list:
        build_assignments = self.assignments.json_output() if issubclass(self.assignments.__class__, FunctionCall) else []
        return build_assignments + [
            {
                ...
                "args": {
                    "assignments": [
                        {
                            "id": self._ref_or_value(assignment_id),
                            "bound": self._ref_or_bound(assignment_bound)
                        }
                        for assignment_id, assignment_bound in self.assignments.items()
                    ]
                },
                "out": self.out
            }
...
class validate(FunctionCall):
    ...
    dag: RefOrValue
    options: Optional[validate_options] = field(default_factory=validate_options)
