...
Usage:
    ...
    python -m questmind.server --model mlx-community/Llama-3.2-3B-Instruct-4bit

    # Batched mode (for multiple concurrent users)
    python -m questmind.server --model mlx-community/Llama-3.2-3B-Instruct-4bit --continuous-batching

    # With MCP tools
    python -m questmind.server --model mlx-community/Qwen3-4B-4bit --mcp-config mcp.json
...
async def init_mcp(config_path: str):
    ...
    try:
        # Try to import MCP from vllm_mlx (if installed)
        try:
            from vllm_mlx.mcp import (MCPClientManager, ToolExecutor,
                                      load_mcp_config)
        except ImportError:
            logger.warning(
                "MCP not available. Install vllm-mlx with MCP support or "
                "disable MCP with --mcp-config=None"
            )
            return
