...
class GracefulShutdownHandler:
    ...
    def _signal_handler(self, signum: int, frame):
        ...
        if self.shutdown_requested:
            ...
            log_message(message_type="shutdown", message="Force shutdown requested")
            sys.exit(1)
        
        self.shutdown_requested = True
        signame = "SIGINT" if signum == signal.SIGINT else "SIGTERM"
        log_message(message_type="shutdown", message=f"Received {signame}, initiating graceful shutdown...")
...
def run_with_graceful_shutdown(server: EdisonMCP, **kwargs):
    ...
    try:
        ...
        log_message(message_type="startup", message="Server starting with graceful shutdown enabled")
        server.run(**kwargs)
    except KeyboardInterrupt:
        log_message(message_type="shutdown", message="Shutdown signal received, cleaning up...")
    except Exception as e:
        log_message(message_type="error", message=f"Server error: {str(e)}")
        raise
    finally:
        shutdown_handler.restore_handlers()
        log_message(message_type="shutdown", message="Server stopped")
