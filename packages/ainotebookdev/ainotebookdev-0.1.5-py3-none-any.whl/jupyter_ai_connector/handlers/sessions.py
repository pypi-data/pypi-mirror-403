...
class SessionUsageHandler(BaseProxyHandler):
    """Proxy session usage from orchestrator."""
    ...
    async def get(self, session_id: str):
        ...
        try:
            query = self.request.query
            url = f"{self.orchestrator_url}/v1/sessions/{session_id}/usage"
            if query:
                url = f"{url}?{query}"
            response = await self.http_client.get(
                url,
            ...
            logger.exception("Failed to load session usage")
            await self.handle_error(500, str(e), code="PROXY_ERROR")


class TurnUsageHandler(BaseProxyHandler):
    """Proxy turn usage from orchestrator."""
    ...
    async def get(self, turn_id: str):
        ...
        try:
            query = self.request.query
            url = f"{self.orchestrator_url}/v1/turns/{turn_id}/usage"
            if query:
                url = f"{url}?{query}"
            response = await self.http_client.get(
                url,
            ...
            logger.exception("Failed to load turn usage")
            await self.handle_error(500, str(e), code="PROXY_ERROR")
