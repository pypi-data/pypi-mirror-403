"""Parameter widget for configuring storage settings with status display"""
    ...
    def update_status(self, config):
        ...
        if storage_type == "local":
            ...
            self.example_label.setText(
                f"Example: {path}/plate_ABC123.csv"
            )
            ...
            if bucket:
                ...
                example_key = f"{prefix}/plate_ABC123.csv" if prefix else "plate_ABC123.csv"
                self.example_label.setText(
                    f"Example: s3://{bucket}/{example_key}"
                )
            else:
            ...
            if container and has_conn:
                self.status_label.setText(
                    ...
                    f"Prefix: {prefix or '(root)'}<br>"
                    f"✓ Connection string configured"
                )
                example_blob = f"{prefix}/plate_ABC123.csv" if prefix else "plate_ABC123.csv"
                self.example_label.setText(
                    f"Example: azure://{container}/{example_blob}"
                )
            else:
            ...
            if bucket:
                ...
                example_key = f"{prefix}/plate_ABC123.csv" if prefix else "plate_ABC123.csv"
                self.example_label.setText(
                    f"Example: gs://{bucket}/{example_key}"
                )
            else:
...
class StorageConfigWidget(QWidget):
    ...
    def load_current_config(self):
        ...
        self.azure_container.setText(config.get("AZURE_CONTAINER", "napari-data"))
    ...
    def save_config(self):
        ...
        lines = [
            "# Napari Plate Layout - Storage Configuration",
