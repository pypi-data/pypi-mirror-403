...
class PlateLayoutWidget(QWidget):
    ...
    def export_csv(self):
        """Export plate layout to configured storage backend"""
        try:
            filename = f"{self.exp_fields['barcode'].text()}/layout.csv"
            ...
            for well in natsorted(self.well_data):
                ...
                # Get experiment values in order
                exp_row_values = [exp_values.get(k, "") for k in exp_columns]
                
                # Get well-specific values in order
                well_values = [str(wd.values.get(k, "")) for k in self.well_cfg.keys()]
                
                # Combine all values in order
                row_values = exp_row_values + [well] + well_values
                rows.append(",".join(row_values))
            
            content = "\n".join(rows)
            
            # Write to storage
            result_path = self.storage.write(filename, content)
            display_path = self.storage.get_display_path(filename)
