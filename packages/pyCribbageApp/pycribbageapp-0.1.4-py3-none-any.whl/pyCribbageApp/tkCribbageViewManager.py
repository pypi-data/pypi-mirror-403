...
class tkCribbageViewManager(tkViewManager):
    ...
    def handle_end_game_event(self, info):
        ...
        if info.name_winner == self._player1_name:
            self._board_widget._player1_track['text']=f"{self._player1_name} - Winner"
            self._board_widget._player2_track['text']=f"{self._player2_name}"
        elif info.name_winner == self._player2_name:
            self._board_widget._player2_track['text']=f"{self._player2_name} - Winner"
...
class CribbageBoardWidget(ttk.Labelframe, Subject):
    ...
    def set_pegs_player1(self, lead=0, trail=61):
        ...
        :return None:
        """
        # Adjust for going around the board the second time, or for end of game, where lead may exceed what is needed to win.
        if lead > 60:
            # We are starting around the board for the second time
            lead -= 60
            if lead > 60:
                # We are finished our second time around the board, and lead is larger than what is needed to win.
                # So set it to a winning peg location of 61.
                lead = 61
        if trail > 60 and lead != 0:
            # The and above accounts for the situation at the beginning of a game, where the leading peg is
            # stationed in location 0 and the trailing peg is stationed in location 61.
            trail -= 60
