...
class IrmKmiApiClient:
    ...
    _cache_max_age = (
        60 * 60 * 2
    )  # Remove items from the cache if they have not been hit since 2 hours
    _cache = {}
    ...
    async def get_forecasts_coord(self, coord: Dict[str, float | int]) -> dict:
        ...
        _LOGGER.debug(
            f"Observation for {response.get('cityName', '')}: {response.get('obs', '{}')}"
        )
    ...
    async def get_image(self, url, params: Dict[str, str] | None = None) -> bytes:
        ...
        :raise: IrmKmiApiError when communication with the API fails
        """
        r: bytes = await self._api_wrapper(
            base_url=url, params={} if params is None else params
        )
        return r
    ...
    def _api_key(method_name: str) -> str:
        """Get API key."""
        return hashlib.md5(
            f"r9EnW374jkJ9acc;{method_name};{datetime.now().strftime('%d/%m/%Y')}".encode()
        ).hexdigest()
...
class IrmKmiApiClientHa(IrmKmiApiClient):
    ...
    def __init__(
        self, session: aiohttp.ClientSession, user_agent: str, cdt_map: dict
    ) -> None:
    ...
    def get_current_weather(self, tz: ZoneInfo) -> CurrentWeatherData:
        ...
        try:
            pressure = (
                float(now_hourly.get("pressure", None))
                if now_hourly is not None
                else None
            )
        ...
        try:
            wind_speed = (
                float(now_hourly.get("windSpeedKm", None))
                if now_hourly is not None
                else None
            )
        except (TypeError, ValueError):
        ...
        try:
            wind_gust_speed = (
                float(now_hourly.get("windPeakSpeedKm", None))
                if now_hourly is not None
                else None
            )
        except (TypeError, ValueError):
        ...
        try:
            dir_cardinal = (
                now_hourly.get("windDirectionText", {}).get("en")
                if now_hourly is not None
                else None
            )
            ...
            if (
                ...
                try:
                    current_weather["wind_bearing"] = (
                        float(self._api_data.get("obs", {}).get("windDirection")) + 180
                    ) % 360
                except (TypeError, ValueError):
                    pass
    ...
    def get_daily_forecast(self, tz: ZoneInfo, lang: str) -> List[IrmKmiForecast]:
        ...
        for idx, f in enumerate(data):
            ...
            if timestamp is not None:
                ...
                forecast_day = self._next_weekday(
                    forecast_day, WEEKDAYS.index(day_name)
                )
            ...
            forecast = IrmKmiForecast(
                ...
                condition=self._cdt_map.get(
                    (f.get("ww1", None), f.get("dayNight", None)), None
                ),
                condition_2=self._cdt_map.get(
                    (f.get("ww2", None), f.get("dayNight", None)), None
                ),
            ...
            if (
                forecast["native_templow"] is not None
                ...
                (forecast["native_templow"], forecast["native_temperature"]) = (
                    forecast["native_temperature"],
                    forecast["native_templow"],
                )
    ...
    def get_animation_data(
        self, tz: ZoneInfo, lang: str, style: str, dark_mode: bool
    ) -> RadarAnimationData:
        """
        ...
        localisation_layer_url = self._api_data.get("animation", {}).get(
            "localisationLayer"
        )
        ...
        if (
            ...
            or localisation_layer_url is None
            or not isinstance(animation_data, list)
        ):
            raise ValueError("Cannot create animation data")

        localisation = self._merge_url_and_params(
            localisation_layer_url,
            {"th": "d" if country == "NL" or not dark_mode else "n"},
        )
        images_from_api = [
            self._merge_url_and_params(
                frame.get("uri"), {"rs": STYLE_TO_PARAM_MAP[style]}
            )
            for frame in animation_data
            if frame is not None and frame.get("uri") is not None
        ]
    ...
    def get_warnings(self, lang: str) -> List[WarningData]:
        ...
        if (
            warning_data is None
            or not isinstance(warning_data, list)
            or len(warning_data) == 0
        ):
            return []
        ...
        for data in warning_data:
            ...
            result.append(
                WarningData(
                    ...
                    id=warning_id,
                    level=level,
                    friendly_name=data.get("warningType", {})
                    .get("name", {})
                    .get(lang, ""),
