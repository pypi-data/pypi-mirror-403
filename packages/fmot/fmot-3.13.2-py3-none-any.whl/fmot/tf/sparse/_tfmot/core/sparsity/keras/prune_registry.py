...
try:
    ...
    try:
        base_layer = (
            tf._keras_internal.engine.base_layer
    ...
    def supports(cls, layer):
        ...
        if layer.__class__ in cls._RNN_LAYERS:
            for cell in cls._get_rnn_cells(layer):
                if cell.__class__ not in cls._RNN_CELLS_WEIGHTS_MAP and not isinstance(
                    cell, prunable_layer.PrunableLayer
                ):
                    return False
            return True
            ...
            def get_prunable_weights_rnn_cell(cell):
                ...
                raise ValueError(
                    cls._RNN_CELL_ERROR_MSG.format(
                        layer.__class__,
                        cell.__class__,
                        cls._RNN_CELLS_WEIGHTS_MAP.keys(),
                    )
                )
