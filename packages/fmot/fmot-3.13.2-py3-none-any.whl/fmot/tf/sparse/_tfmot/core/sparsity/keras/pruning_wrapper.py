...
from fmot.tf.sparse._tfmot.core.keras import compat as tf_compat
from fmot.tf.sparse._tfmot.core.keras import metrics
from fmot.tf.sparse._tfmot.core.keras import utils
from fmot.tf.sparse._tfmot.core.sparsity.keras import prunable_layer
from fmot.tf.sparse._tfmot.core.sparsity.keras import prune_registry
from fmot.tf.sparse._tfmot.core.sparsity.keras import pruning_impl
from fmot.tf.sparse._tfmot.core.sparsity.keras import (
    pruning_schedule as pruning_sched,
)
from fmot.tf.sparse._tfmot.core.sparsity.keras.pruning_utils import (
    convert_to_tuple_of_two_int,
)
from fmot.tf.sparse._tfmot.core.keras.compat import keras
...
class PruneLowMagnitude(Wrapper):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if block_pooling_type not in ["AVG", "MAX"]:
            raise ValueError(
                "Unsupported pooling type '{}'. Should be 'AVG' or 'MAX'.".format(
                    block_pooling_type
                )
            )
    ...
    def from_config(cls, config):
        ...
        config["pruning_schedule"] = deserialize_keras_object(
            pruning_schedule, module_objects=globals(), custom_objects=custom_objects
        )
