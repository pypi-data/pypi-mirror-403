...
from fmot.tf.sparse._tfmot.core.sparsity.keras import pruning_wrapper
...
def prune_scope():
    ...
    ```
    """
    return custom_object_scope({"PruneLowMagnitude": pruning_wrapper.PruneLowMagnitude})
...
def prune_low_magnitude(
    ...
    **kwargs
    ...
    is_sequential_or_functional = isinstance(to_prune, keras.Model) and (
        isinstance(to_prune, keras.Sequential) or to_prune._is_graph_network
    )

    # A subclassed model is also a subclass of keras.layers.Layer.
    is_keras_layer = isinstance(to_prune, keras.layers.Layer) and not isinstance(
        to_prune, keras.Model
    )
...
def strip_pruning(model):
    ...
    try:
        ...
        return _strip_pruning_wrapper(model)
