...
from bdat.entities.patterns import CPDischargeQOCVEval
...
class CPDischargeQOCV(EvalPattern):
    ...
    def pattern(self, species: BatterySpecies) -> SteplistPattern:
        ...
        chargePower = make_range(
            [self.chargePower, (0, 1e9)],
            (0.95, 1.05),
        )
        dischargePower = make_range(
            [
                self.dischargePower,
        ...
        cutoffCurrent = make_range(
            [self.cutoffCurrent, (0, chargePower[1])], (0.95, 1.05)
        )
        relaxationTime = make_range([self.relaxationTime, (0, 1e9)], (0.95, 1.05))
        cpDuration = make_range([self.cpDuration], (0.95, 1.05), allowNone=True)

        self.chargeStep = CPProperties(
            power=chargePower, voltageEnd=eocVoltage, duration=cpDuration
        )
        ...
        self.dischargeStep = CPProperties(power=dischargePower, voltageEnd=eodVoltage)
    ...
    def eval(
        ...
        dchStep = next(match.get_matches(self.dischargeStep)).steps[0].asCP()
