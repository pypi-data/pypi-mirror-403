...
class MCAClient:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Non-blocking, logs warnings if outdated
        check_version(__version__, timeout=self.config.collector_timeout)
        ...
        # Start background refresh (if registry configured)
        if self.config.registry_url and self.config.refresh_interval_secs > 0:
    ...
    def thresholds(self) -> Dict[str, float]:
        ...
        Returns:
            Dictionary of threshold values from registry
        """
    ...
    def _start_background_refresh(self) -> None:
        """Start background thread for periodic registry refresh."""
        self._refresh_running = True
        self._refresh_thread = threading.Thread(
            target=self._refresh_loop,
            name="MCA-Registry-Refresh",
            daemon=True,
        )
        self._refresh_thread.start()
        logging.info(
            f"Started registry refresh thread (interval: {self.config.refresh_interval_secs}s)"
        )

    def _refresh_loop(self) -> None:
        """Background loop for refreshing registry config."""
        while self._refresh_running:
            time.sleep(self.config.refresh_interval_secs)
            ...
            try:
                ...
                new_config = self._registry_client.fetch_model_config(
                    model_id=self.config.model_id or self.config.service_name,
                    version=self.config.model_version,
                )
                ...
                # Check for identity field changes (warn only)
                self._check_identity_changes(new_config)

                # Update mutable fields
                self._thresholds = new_config.thresholds

                # Record telemetry
                if self._registry_telemetry:
                    self._registry_telemetry.record_success(latency)

                logging.debug(f"Registry refresh successful (latency: {latency:.3f}s)")

            except Exception as e:
                logging.warning(f"Registry refresh failed: {e}")
                if self._registry_telemetry:
                    self._registry_telemetry.record_error(type(e).__name__)

    def _check_identity_changes(self, new_config: ModelConfig) -> None:
        """Warn if identity fields changed (cannot be applied at runtime).

        Args:
            new_config: New configuration from registry
        """
        if self._registry_config:
            if new_config.service_name != self._registry_config.service_name:
                logging.warning(
                    f"service_name changed in registry "
                    f"({self._registry_config.service_name} -> {new_config.service_name}). "
                    "Restart required to apply."
                )
            if new_config.model_id != self._registry_config.model_id:
                logging.warning(
                    f"model_id changed in registry "
                    f"({self._registry_config.model_id} -> {new_config.model_id}). "
                    "Restart required to apply."
                )
            if new_config.model_version != self._registry_config.model_version:
                logging.warning(
                    f"model_version changed in registry "
                    f"({self._registry_config.model_version} -> {new_config.model_version}). "
                    "Restart required to apply."
                )
    ...
    def shutdown(self, timeout_millis: int = 30000):
        ...
        if hasattr(self, "_refresh_running"):
            ...
            if self._refresh_thread and self._refresh_thread.is_alive():
                ...
                logging.debug("Registry refresh thread stopped")
