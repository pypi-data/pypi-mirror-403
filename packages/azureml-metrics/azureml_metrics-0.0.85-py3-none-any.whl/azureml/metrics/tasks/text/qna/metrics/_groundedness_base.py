...
from azureml.metrics.tasks.text.qna.metrics._seq2seq_qa import Seq2SeqQAMetric

logger = logging.getLogger(__name__)


class GroundednessBase(Seq2SeqQAMetric):
    ...
    def __init__(self,
        ...
        self.llm_url_connector = LLMUrlConnector(self.metric_data.llm_params, self.metric_data.llm_api_batch_size)
    ...
    def check_kwargs(self, metric_name: str) -> Any:
        if metric_name == constants.Metric.GPTGroundedness:
            if self.metric_data.openai_params is None:
                logger.error("OpenAI params not provided.")
                return "nan"
            if self.metric_data.generated_contents is None:
            ...
            if self.metric_data.llm_params is None:
