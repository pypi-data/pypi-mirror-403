...
Although aiomadeavr is meant to be used as a library, the module can be run as a standalone application:
...
## Command Line Options
    ...
    python3 -m aiomadeavr              # Discovery + TUI (or CLI if Textual not installed)
    python3 -m aiomadeavr --ip 192.168.1.100  # Direct connection to specific IP
    python3 -m aiomadeavr --cli        # Force CLI mode (no Textual)
    python3 -m aiomadeavr -v           # Verbose output
    python3 -m aiomadeavr -d           # Debug output
