...
class Perspective:
    ...
    def map_iso_f(self, x: float, y: float, z: float) -> tuple[float, float]:
        ...
        iso_x = (xp - yp) * COS_30
        iso_y = -(((xp + yp) / 2) + zp)
        return float(iso_x), float(iso_y)

    def get_pixel_center(
        self, x: int, y: int, z: int, face_id: FaceId
    ) -> tuple[float, float]:
        if face_id in ("front", "back"):
            y_offset = y if face_id == "front" else y + 1
            return self.map_iso_f(x + 0.5, y_offset, z + 0.5)
        elif face_id in ("left", "right"):
            x_offset = x if face_id == "left" else x + 1
            return self.map_iso_f(x_offset, y + 0.5, z + 0.5)
        else:  # up/down
            z_offset = z if face_id == "down" else z + 1
            return self.map_iso_f(x + 0.5, y + 0.5, z_offset)

    def map_iso(self, x: int, y: int, z: int) -> tuple[int, int]:
        iso_x, iso_y = self.map_iso_f(float(x), float(y), float(z))
