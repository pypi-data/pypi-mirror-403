...
### File Management (view, upload, download, delete)

- View files: `client.view_files(ListFilesRequest(path="/"))` lists entries for a path.
- Upload file: `client.upload_file(file_path, folder_path="/", custom_file_name=...)` uploads and returns path/name plus error info.
- Download file (bytes): `client.download_file(DownloadFileRequest(path="/name.ext"))` returns raw bytes.
- Upload folder (dict): `client.upload_folder(DownloadFileRequest(folderPath="/abc/pqr"))` returns dict.
- Download file (base64): `client.download_file_base64(DownloadFileRequest(path="/app/files/name.ext"))` returns base64 content + metadata.
- Delete file: `client.delete_file(DeleteFileRequest(path="/name.ext"))` removes the target and returns status/error details.
...
import json, tempfile, uuid, base64

client = UnmeshedClient(client_config)

# List files
files_response = client.view_files(ListFilesRequest(path="/"))
print([f"{e.name} ({'dir' if e.folder else 'file'})" for e in files_response.entries])

# Upload a sample JSON file with a unique name
sample_name = f"sample_upload_{uuid.uuid4().hex}.json"
with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False) as tmp:
    json.dump({"hello": "world"}, tmp)
    tmp_path = tmp.name
upload_response = client.upload_file(file_path=tmp_path, folder_path="/", custom_file_name=sample_name)
print(upload_response.to_json())

# Download as bytes and read content
download_bytes = client.download_file(DownloadFileRequest(path=f"/{sample_name}"))
print(download_bytes.decode("utf-8"))

# Upload Folder
upload_response = client.download_file(UploadFolderRequest(folderPath="/abc/pqr"))
print(upload_response)

# Download as base64 (server expects /app/files/ prefix for this endpoint)
download_base64: DownloadFileBase64Response = client.download_file_base64(
    DownloadFileRequest(path=f"/app/files/{sample_name}")
)
decoded = base64.b64decode(download_base64.contentBase64 or "").decode("utf-8")
print(decoded)

# Delete the file
delete_response = client.delete_file(DeleteFileRequest(path=f"/{sample_name}"))
print(delete_response.to_json())
```
