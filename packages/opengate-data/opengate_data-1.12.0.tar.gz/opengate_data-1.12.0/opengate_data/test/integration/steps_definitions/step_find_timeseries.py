...
scenarios("timeseries/find_timeseries.feature")

@pytest.fixture
def api_ctx():
    return {"payload": None, "response": None, "dataset_identifier": None}

@given(parsers.parse('I find timeseries by name "{dataset_name}"'))
def step_find_dataset_by_name(builder_holder, dataset_name: str):
    builder_holder["instance"].with_name(dataset_name)
    time.sleep(2)

@given(parsers.parse('I want a timeseries name by environment variable "{env_name}"'))
