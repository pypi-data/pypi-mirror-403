...
def enable_sqlalchemy_tenant_isolation(
    ...
    *,
    ...
    Args:
        engine_or_session_factory: SQLAlchemy Engine、AsyncEngine、sessionmaker、async_sessionmaker 或 Session/AsyncSession 类
    ...
    # 同时注册同步和异步的事件监听器，以确保兼容性
    if AsyncSession is not None:
        # 异步 Session
        try:
            listeners = event.contains(AsyncSession, "before_flush", _before_flush_add_tenant_id_async)
            if not listeners:
                event.listen(AsyncSession, "before_flush", _before_flush_add_tenant_id_async)
    ...
    try:
        ...
        if AsyncSession is not None:
            # 异步 Session 的 do_orm_execute
            @event.listens_for(AsyncSession, "do_orm_execute", once=False)
            def _do_orm_execute_async(state: ORMExecuteState) -> None:
                """
                在执行异步 ORM 查询时自动添加租户过滤
                """
                tenant_id = get_tenant_id()
