...
class RenameFields(ModelTransformer):
    ...
    Renames both regular fields and computed fields.
    ...
    def __call__(self, context: VariantContext) -> VariantContext:
        if not isinstance(context.current_variant, DecomposedModel):
            raise ValueError("Rename transformer requires DecomposedModel, got built model")

        # Rename regular fields
        new_fields = {}
        ...
        new_computed = {}
        for old_name, dec in decorators.computed_fields.items():
            new_name = self._rename_func(old_name) or old_name
            new_computed[new_name] = dec
