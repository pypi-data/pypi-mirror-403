...
class FilterFields(ModelTransformer):
    ...
    Filters both regular fields and computed fields by name.
    ...
    def __init__(
        ...
        if sum(x is not None for x in [exclude, include_only, filter_func]) != 1:
            raise ValueError("Must provide one of: exclude, include_only, or filter_func")
    ...
    def __call__(self, context: VariantContext) -> VariantContext:
        if not isinstance(context.current_variant, DecomposedModel):
            raise ValueError("Filter transformer requires DecomposedModel, got built model")
        ...
        # Filter computed fields by name
        decorators = context.current_variant._pydantic_decorators
        new_computed = {}
        for name, dec in decorators.computed_fields.items():
            # For computed fields, apply the same filter logic using the ComputedFieldInfo
            if not self._filter_func(name, dec.info):  # type: ignore
