...
class ModifyFields(ModelTransformer):
    ...
    def __call__(self, context: VariantContext) -> VariantContext:
        ...
        # Also modify computed fields by name
        decorators = context.current_variant._pydantic_decorators
        new_computed = copy(decorators.computed_fields)

        for field_name, decorator in new_computed.items():
            # For computed fields, check if there are modifications for this name
            # We pass decorator.info (ComputedFieldInfo) instead of FieldInfo
            modifications = self.modify_func(field_name, decorator.info)  # type: ignore

            if modifications:
                # For computed fields, we can update the ComputedFieldInfo properties
                for key, value in modifications.items():
                    if key != "metadata_callback" and hasattr(decorator.info, key):
                        setattr(decorator.info, key, value)
