...
class TreeNavigator:
    ...
    def _handle_security_scan(self, tree: TreeNode, predicates: List) -> List[TreeNode]:
        ...
        print(f"DEBUG: Checking {len(all_nodes)} nodes for security patterns")
        
        for i, node in enumerate(all_nodes):
            content = self._get_node_content(node).lower()
            if i < 3:  # Debug first 3 nodes
                print(f"DEBUG: Node {i}: name='{node.name}', content_len={len(content)}")
                print(f"DEBUG: Content preview: {content[:200]}...")
            
            if any(pattern in content for pattern in security_patterns):
                print(f"DEBUG: Found pattern in node: {node.name}")
                results.append(node)
        
        # Debug: Show what we found
        print(f"DEBUG: Security scan found {len(results)} results")
        for i, result in enumerate(results[:3]):  # Show first 3
            print(f"DEBUG: Result {i}: name='{result.name}', type={getattr(result.node_type, 'value', 'unknown')}")
    ...
    def _is_function_node(self, node: TreeNode) -> bool:
        ...
        if hasattr(node, 'metadata') and node.metadata:
            return node.metadata.get('node_type') == 'function'
        return False
    
    def _is_class_node(self, node: TreeNode) -> bool:
        """Check if node represents a class"""
        if hasattr(node, 'metadata') and node.metadata:
            return node.metadata.get('node_type') == 'class'
        return False
