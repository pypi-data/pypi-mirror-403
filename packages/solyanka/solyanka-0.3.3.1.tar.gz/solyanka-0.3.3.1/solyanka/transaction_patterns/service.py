...
class PatternsService:
    ...
    def get_available_regions(self) -> Dict[str, List[str]]:
        """Return a mapping of country -> available regions from the schema.

        Parses schema.json to extract the region constraints defined per country.
        Returns e.g. {"Thailand": ["Bangkok", "Phuket", ...], "Vietnam": [...]}
        """
        import json

        schema_path = self.patterns_dir / "schema.json"
        ...
        for rule in all_of:
            ...
            country_prop = props.get("country", {})
            country_name = country_prop.get("const")

            if (
                country_name
                and "required" in if_clause
                and "country" in if_clause["required"]
            ):
                then_props = then_clause.get("properties", {})
                region_prop = then_props.get("region", {})
                region_enum = region_prop.get("enum", [])
                if region_enum:
                    regions_map[country_name] = region_enum
