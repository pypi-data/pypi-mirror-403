...
IInternalMailshare_VERSION = "5.5.3158"


class IInternalMailshare(BaseEndpoint):
    ...
    def allComplete(self):
        ...
        response = requests.get(self.base + postUri, params=queryParams, verify=False, headers={
                                'X-BM-ApiKey': self.apiKey, 'Accept': 'application/json', 'X-BM-ClientVersion': IInternalMailshare_VERSION}, data=__encoded__)
    ...
    def createMailshare(self, uid, mailshare):
        postUri = "/{uid}"
        __data__ = None
        __encoded__ = None
        postUri = postUri.replace("{uid}", uid)
