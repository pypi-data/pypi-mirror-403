...
IInternalAPIKeys_VERSION = "5.5.3158"


class IInternalAPIKeys(BaseEndpoint):
    ...
    def create(self, displayName):
        ...
        response = requests.put(self.base + postUri, params=queryParams, verify=False, headers={
                                'X-BM-ApiKey': self.apiKey, 'Accept': 'application/json', 'X-BM-ClientVersion': IInternalAPIKeys_VERSION}, data=__encoded__)
    ...
    def put(self, displayName, sid):
        postUri = "/_put"
        __data__ = None
        __encoded__ = None
        __data__ = serder.STRING.encode(sid)
