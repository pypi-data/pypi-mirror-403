...
IInternalAuthentication_VERSION = "5.5.3158"


class IInternalAuthentication(BaseEndpoint):
    def __init__(self, apiKey, url):
        self.url = url
        self.apiKey = apiKey
        self.base = url + '/auth'

    def backchannelLogout(self, contentStream):
        postUri = "/logout/backchannel"
        __data__ = None
        __encoded__ = None
        __data__ = serder.STREAM.encode(contentStream)
        __encoded__ = __data__
        queryParams = {}

        response = requests.post(self.base + postUri, params=queryParams, verify=False, headers={
                                 'X-BM-ApiKey': self.apiKey, 'Accept': 'application/json', 'X-BM-ClientVersion': IInternalAuthentication_VERSION}, data=__encoded__)
    ...
    def validateCredentials(self, login, origin, password):
        postUri = "/validatecredential"
        __data__ = None
        __encoded__ = None
        __data__ = serder.STRING.encode(password)
        __encoded__ = json.dumps(__data__)
