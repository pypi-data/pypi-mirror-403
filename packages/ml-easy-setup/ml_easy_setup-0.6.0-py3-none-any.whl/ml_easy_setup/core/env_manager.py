...
"""
    ...
    def generate_project_files(
        ...
        Args:
            ...
            use_pyproject: 是否使用 pyproject.toml (现代 Python 打包)
        """
        ...
        if use_pyproject:
            # 生成 pyproject.toml
            self._generate_pyproject(name, template_config, deps, dev_deps)
            # 仍然生成 requirements.txt 作为兼容性备份
    ...
    def _generate_pyproject(
        ...
        # 构建依赖列表 (按字母排序)
        dependencies = sorted(deps, key=lambda x: x.lower())

        # 构建 UV dependency groups
        dev_group = sorted(dev_deps, key=lambda x: x.lower())

        # 根据 template 类型添加可选依赖组
        optional_groups = {}
        template_type = template_config.get("type", "minimal")

        if template_type == "pytorch":
            optional_groups["cuda"] = [
                "torch>=2.0.0",
                "torchvision>=0.15.0",
                "torchaudio>=2.0.0",
            ]
        elif template_type == "tensorflow":
            optional_groups["cuda"] = [
                "tensorflow[and-cuda]>=2.13.0",
            ]

        # LLM 特定的可选依赖
        if template_config.get("flash_attention_supported"):
            optional_groups["flash-attn"] = [
                "flash-attn>=2.3.0; platform_system == 'Linux' and python_version >= '3.8'",
            ]
        ...
        lines = [
            ...
            f'name = "{project_name}"',
            f'version = "0.1.0"',
            f'description = "{template_config.get("description", "ML Project")}"',
            'readme = "README.md"',
        ...
        lines.extend([
            ...
            "",
            "# Metadata",
            f'generated-by = "ml-easy-setup {datetime.now().strftime("%Y.%m.%d")}"',
            "",
        ])
        ...
        console.print(f"   [dim]生成 pyproject.toml[/dim]")
