...
@click.group()
@click.version_option(version="0.7.0", prog_name="ml-easy-setup")
def main():
    """
...
@main.command()
@click.option("--port", "-p", type=int, default=8501, help="Web UI 端口")
@click.option("--host", "-h", type=str, default="localhost", help="Web UI 主机")
def webui(port: int, host: str):
    """
    启动图形化 Web 界面

    无需命令行，通过浏览器界面配置和创建 ML 项目。

    示例:
        mlsetup webui
        mlsetup webui --port 8080
        mlsetup webui --host 0.0.0.0
    """
    import sys
    import subprocess
    ...
    try:
        ...
        console.print("""
[bold red]✗ 未安装 streamlit[/bold red]

请先安装 Web UI 依赖:

[yellow]pip install ml-easy-setup[webui][/yellow]

或:

[yellow]pip install streamlit[/yellow]
""")
        raise click.ClickException("streamlit 未安装")

    # 获取 webui.py 路径
    from ml_easy_setup.webui import main as webui_main

    # 直接运行 streamlit
    console.print(f"""
[bold cyan]🚀 启动 ML Easy Setup Web UI[/bold cyan]

访问地址: [yellow]http://{host}:{port}[/yellow]
