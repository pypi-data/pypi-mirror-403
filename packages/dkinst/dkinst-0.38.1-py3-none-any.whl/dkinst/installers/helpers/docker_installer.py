...
def is_docker_installed():
    ...
    try:
        ...
        print("Docker command not found. Docker is not installed.")
        return False
...
def add_current_user_to_docker_group():
    ...
    if 'docker' in result.stdout:
        console.print(f"User {current_user} was added to the docker group.", style='green')
        return True
    else:
        console.print(f"User {current_user} was not added to the docker group. Try executing with sudo", style='red')
        return False
...
def install_docker_ubuntu(
    ...
    if rootless and permissions.is_admin():
        console.print('Rootless installation requires running the script without sudo.', style='red')
        sys.exit()

    if use_docker_installer:
        if not ubuntu_terminal.is_executable_exists('curl'):
            console.print('curl is not installed, installing...', style='yellow')
    ...
    if rootless:
        ...
        if not ubuntu_terminal.is_package_installed('uidmap'):
            console.print('uidmap is not installed, installing...', style='yellow')
            ubuntu_terminal.update_system_packages()
    ...
    if 'Hello from Docker!' in '\n'.join(result):
        ...
        console.print(
            ...
            'Please check the logs above for more information.', style='red')
        return 1
...
def _make_parser():
    ...
    parser.add_argument(
        '-id', '--install',
        action='store_true',
        help='Install the latest version using Docker installer (https://get.docker.com).'
    )
    parser.add_argument(
        '-idr', '--install-rootless',
        action='store_true',
        help='Install the latest version using docker installer in rootless mode (https://get.docker.com).'
    )
    parser.add_argument(
        '-im', '--install-manually-via-apt',
        action='store_true',
        help='Install the latest version using the APT repository.'
    )

    parser.add_argument(
        '-op', '--offline-preparation',
        action='store_true',
        help='Prepare an offline installation bundle to file: offline-bundle.zip.'
    )
    parser.add_argument(
        '-oi', '--offline-installation',
        action='store_true',
        help='Install Docker using an offline installation bundle: offline-bundle.zip'
    )

    parser.add_argument(
        '-au', '--add-user-to-docker-group',
        action='store_true',
        help='Add the current user to the docker group after installation (only for non-rootless installation).'
    )

    parser.add_argument(
        '-ii', '--is-installed',
        action='store_true',
        help='Check if docker command is available and if so, what version.'
    )
...
def main(
        install: bool = False,
        install_rootless: bool = False,
        install_manually_via_apt: bool = False,
        offline_preparation: bool = False,
        offline_installation: bool = False,
        add_user_to_docker_group: bool = False,
        is_installed: bool = False,
) -> int:
    """
    Download and install the latest version of Docker.

    :param install: bool, if True, the latest version using the Docker available installer will be installed.
    :param install_rootless: bool, if True, the latest version will be installed in rootless mode.
    :param install_manually_via_apt: bool, if True, the latest version will be installed using the APT repository.
    :param offline_preparation: bool, if True, an offline installation bundle will be prepared.
    :param offline_installation: bool, if True, Docker will be installed using an offline installation bundle.
    :param add_user_to_docker_group: bool, if True, the current user will be added to the docker group after installation.
        Only applicable for non-rootless installation.
    :param is_installed: bool, if True, the function will check if docker command is available and if so, what version.
    :return: int, 0 if successful, 1 if failed.
    """

    if (install + install_rootless + install_manually_via_apt + offline_preparation + offline_installation + is_installed) > 1:
        console.print('Please specify exactly one action: --install, --install-rootless, --install-manually-via-apt, '
                      '--offline-preparation, --offline-installation, or --is-installed.', style='red')
        return 1

    if install:
        return install_docker_ubuntu(
            use_docker_installer=True,
            rootless=False,
            add_current_user_to_docker_group_bool=add_user_to_docker_group
        )
    elif install_rootless:
        if add_user_to_docker_group:
            console.print('Cannot add user to docker group in rootless mode installation.', style='red')
            return 1
        return install_docker_ubuntu(
            use_docker_installer=True,
            rootless=True,
            add_current_user_to_docker_group_bool=False
        )
    elif install_manually_via_apt:
        return install_docker_ubuntu(
            use_docker_installer=False,
            rootless=False,
            add_current_user_to_docker_group_bool=add_user_to_docker_group
        )
    elif offline_preparation:
        prepare_offline_installation_bundle()
        return 0
    elif offline_installation:
        install_offline_installation_bundle()
        return 0
    elif is_installed:
        is_docker_installed()
