...
from orcakit_api.api.a2a import a2a_routes
from orcakit_api.api.assistants import assistants_routes
from orcakit_api.api.mcp import mcp_routes
from orcakit_api.channels.openai import openai_routes
from orcakit_api.channels.wework import wework_routes
from orcakit_api.api.meta import meta_info, meta_metrics
from orcakit_api.api.openapi import get_openapi_spec
from orcakit_api.api.runs import runs_routes
from orcakit_api.api.store import store_routes
from orcakit_api.api.threads import threads_routes
from orcakit_api.api.ui import ui_routes
from orcakit_api.auth.middleware import auth_middleware
from orcakit_api.config import HTTP_CONFIG, MIGRATIONS_PATH, MOUNT_PREFIX
from orcakit_api.graph import js_bg_tasks
from orcakit_api.js.base import is_js_path
from orcakit_api.validation import DOCS_HTML
from orcakit_runtime.database import connect, healthcheck
...
async def ok(request: Request, *, disabled: bool = False):
    ...
    if check_db:
        await healthcheck()
    if js_bg_tasks:
        from orcakit_api.js.remote import js_healthcheck
...
if HTTP_CONFIG:
    ...
    if not HTTP_CONFIG.get("disable_openai"):
        protected_routes.extend(openai_routes)
    if not HTTP_CONFIG.get("disable_wework"):
        protected_routes.extend(wework_routes)
else:
    protected_routes.extend(assistants_routes)
    protected_routes.extend(runs_routes)
    protected_routes.extend(threads_routes)
...
if "inmem" in MIGRATIONS_PATH:
    ...
    async def truncate(request: Request):
        from orcakit_runtime.checkpoint import Checkpointer
