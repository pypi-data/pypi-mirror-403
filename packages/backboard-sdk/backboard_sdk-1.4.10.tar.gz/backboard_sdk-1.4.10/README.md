...
- **Response parity**: SDK now surfaces `retrieved_files` / `memory_operation_id` for non-streaming responses and parses `metadata_` consistently.
- **System prompt support**: `create_assistant` / `update_assistant` accept `system_prompt` (with backward-compatible `description` fallback).
