...
import { clamp, cssPx, el, on, stop, storage, uid } from "./utils.js";
...
export class Widget {
    ...
    dock() {
        ...
        if (!targetDash) {
            ...
            const container = window.__dashboardContainer;
            if (container) {
    ...
    renderToolbar(buttons) {
        ...
        for (const btn of allButtons) {
            if (btn.visible && !btn.visible(this))
                continue;
            ...
            this.disposers.push(on(button, "click", (ev) => {
                stop(ev);
                btn.onClick(this);
                this.renderToolbar();
            }));
    ...
    setupInteractions() {
        ...
        // Burger menu
        this.disposers.push(on(this.burgerBtn, "click", (ev) => {
            stop(ev);
            this.showBurgerMenu();
        }));
        // Double-click para maximize/restore
        this.disposers.push(on(this.titleBar, "dblclick", (ev) => {
            const target = ev.target;
