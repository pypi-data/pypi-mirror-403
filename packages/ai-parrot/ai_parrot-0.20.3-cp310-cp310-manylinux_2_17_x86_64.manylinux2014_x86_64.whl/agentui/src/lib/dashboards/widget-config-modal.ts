 ...
 */
export function createShareTab(widget: Widget): ConfigTab {
    return {
        id: "share",
        label: "Share",
        icon: "🔗",
        render(container: HTMLElement) {
            ...
            Object.assign(urlLabel.style, { display: "block", marginBottom: "6px", fontSize: "13px", fontWeight: "500" });

            // Note: We need the widget ID. If widget doesn't have an ID property exposed conveniently we might need to cast or access via layout.
            // But Widget class usually has an ID if it's in a layout.
            // If not, we might need to rely on the fact that for sharing to work, it MUST have an ID.
            const widgetId = (widget as any).id || "unknown"; // Assuming widget has public or accessible id

            // If widget.id is missing from public interface, check widget.js source or add it.
            // Based on dashboard.ts: "widget = dash.layout.getWidget(widgetId)" implies it's tracked.

            const shareUrl = `${window.location.origin}/share/widgets/${widgetId}`;

            const urlInput = el("input", { type: "text", value: shareUrl, readonly: "readonly" }) as HTMLInputElement;
            ...
            Object.assign(copyBtn.style, {
                marginTop: "12px", padding: "10px 20px", borderRadius: "6px",
                border: "1px solid #ddd", background: "#fff", cursor: "pointer", fontWeight: "600"
            });
