...
import { el, on, stop, uid, cssPx, type Dispose } from "./utils";
...
import type { DashboardTabConfig, DashboardTabOptions, AnyPlacement } from "./types";

export { GridLayout, FreeLayout, DockLayout };

type LayoutEngine = GridLayout | FreeLayout | DockLayout;

// === Dashboard Tab (una pestaña individual) ===
export class DashboardTab {
  ...
  constructor(id: string, title: string, icon: string, opts: DashboardTabOptions) {
    this.id = id;
    this.title = title;
    this.icon = icon;
...
export class DashboardContainer {
  ...
  private readonly dashboards = new Map<string, DashboardTab>();
   ...
   */
  getDashboard(id: string): DashboardTab | undefined {
   ...
   */
  getActiveDashboard(): DashboardTab | undefined {
   ...
   */
  findWidget(widgetId: string): { widget: Widget; dashboard: DashboardTab } | null {
    for (const dash of this.dashboards.values()) {
   ...
   */
  createDashboard(options?: { title?: string; icon?: string }): DashboardTab {
   ...
   */
  addDashboard(tab: DashboardTabConfig, view: DashboardTabOptions = {}): DashboardTab {
    ...
    const dash = new DashboardTab(id, tab.title, tab.icon ?? "📊", view);
   ...
   */
  forEach(callback: (dashboard: DashboardTab, id: string) => void): void {
    this.dashboards.forEach((dash, id) => callback(dash, id));
  }

  // === Private Methods ===

  private createTabElement(id: string, tab: DashboardTabConfig, layoutMode: string = "grid"): HTMLElement {
  ...
  private showTabMenu(anchor: HTMLElement, id: string): void {
    ...
    const items = [
      ...
      ...(dash.layoutMode === "dock" ? [{
        ...
        action: async () => {
          const { showLayoutPicker } = await import("./dock-layout-picker");
      ...
      { divider: true },
      {
        label: "⚙️ Settings...",
        action: async () => {
          const { openDashboardSettings } = await import("./dashboard-settings-modal");
  ...
  private async openWidgetSelector(dash: DashboardTab): Promise<void> {
    const widgetTypes = [
    ...
    if (type === "iframe") {
      ...
      const { IFrameWidget } = await import("./iframe-widget");
      ...
      const { ImageWidget } = await import("./image-widget");
      ...
      const { YouTubeWidget } = await import("./youtube-widget");
      ...
      const { VimeoWidget } = await import("./vimeo-widget");
      widget = new VimeoWidget({ title, url });

    } else if (type === "vega") {
      const { VegaWidget } = await import("./vega-widget");
      widget = new VegaWidget({ title });

    } else if (type === "echarts") {
      const { EChartsWidget } = await import("./echarts-widget");
      widget = new EChartsWidget({ title });

    } else if (type === "leaflet") {
      const { LeafletWidget } = await import("./leaflet-widget");
      widget = new LeafletWidget({ title });

    } else if (type === "markdown") {
      const { MarkdownWidget } = await import("./markdown-widget");
      widget = new MarkdownWidget({ title });

    } else if (type === "html") {
      const { HTMLWidget } = await import("./html-widget");
