...
export class InputModal {
    ...
    constructor(private options: InputModalOptions) { }

    static async prompt(options: InputModalOptions): Promise<string | null> {
    ...
    private render(): void {
        ...
        this.disposers.push(
            on(this.input, "focus", () => {
                if (this.input) this.input.style.borderColor = "var(--primary, #3b82f6)";
            }),
            on(this.input, "blur", () => {
                if (this.input) this.input.style.borderColor = "var(--border, #ddd)";
            }),
            on(this.input, "keydown", (e: KeyboardEvent) => {
        ...
        this.disposers.push(
            on(cancelBtn, "mouseover", () => cancelBtn.style.background = "rgba(0,0,0,0.05)"),
            on(cancelBtn, "mouseout", () => cancelBtn.style.background = "transparent")
        );
        ...
        // Hover
        this.disposers.push(
            on(confirmBtn, "mouseover", () => confirmBtn.style.background = "var(--primary-hover, #2563eb)"),
            on(confirmBtn, "mouseout", () => confirmBtn.style.background = "var(--primary, #3b82f6)")
        );

        this.disposers.push(
            on(cancelBtn, "click", () => this.cancel()),
            on(confirmBtn, "click", () => this.confirm())
        );
    ...
    private cleanup(): void {
        if (this.modal) {
            ...
            const dialog = this.modal.querySelector(".input-modal") as HTMLElement;
