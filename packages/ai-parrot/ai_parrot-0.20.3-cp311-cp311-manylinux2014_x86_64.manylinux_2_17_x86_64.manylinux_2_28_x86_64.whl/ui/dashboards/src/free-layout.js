...
export class FreeLayout {
    ...
    addWidget(widget, position = {}) {
        ...
        widget.setDocked(this.dashboard, pos);
        if (!this.widgets.has(widget.id)) {
            ...
            bus.on("widget:minimized", ({ widget: w }) => {
                if (this.widgets.has(w.id)) {
                    ...
                    if (entry)
                        this.renderWidget(w, entry.position);
                }
            });
        }
    ...
    resizeWidget(widget, newSize) {
        ...
        widget.setPlacement(entry.position);
        this.saveState();
    }
    getWidget(widgetId) {
    ...
    beginDrag(widget, ev) {
        ...
        const onMove = (e) => this.handleDragMove(e, startX, startY, initialPos, entry, widget);
        const onUp = () => {
    ...
    handleDragMove(e, startX, startY, initialPos, entry, widget) {
...
//# sourceMappingURL=free-layout.js.map