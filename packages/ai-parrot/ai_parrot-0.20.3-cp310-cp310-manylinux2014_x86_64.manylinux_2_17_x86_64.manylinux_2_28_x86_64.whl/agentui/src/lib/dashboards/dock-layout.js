...
export class DockLayout {
    ...
    buildStructure(structure, parentEl) {
    ...
    createPane(id) {
        ...
        for (const item of menuItems) {
            if (item.divider) {
                ...
                const btn = el("button", { class: "dock-pane-dropdown-item" }, item.label);
                ...
                this.disposers.push(on(btn, "click", (ev) => { stop(ev); item.action(); dropdown.style.display = "none"; }), on(btn, "mouseenter", () => btn.style.background = "var(--surface, #1a1a2e)"), on(btn, "mouseleave", () => btn.style.background = "transparent"));
                dropdown.appendChild(btn);
            }
        }
        this.disposers.push(on(menuBtn, "click", (ev) => {
            stop(ev);
            dropdown.style.display = dropdown.style.display === "none" ? "block" : "none";
        }), on(document, "click", (ev) => {
            if (!ev.target.closest(".dock-pane-toolbar")) {
    ...
    createGutter(container, index, isHorizontal, sizes) {
    ...
    beginGutterResize(container, index, isHorizontal, sizes, ev) {
        ...
        const children = Array.from(container.children).filter(c => !c.classList.contains("dock-gutter"));
        const onMove = (e) => {
            ...
            if (child1)
                child1.style.flex = `0 0 ${newSize1}%`;
            if (child2)
                child2.style.flex = `0 0 ${newSize2}%`;
        };
        const onUp = () => {
    ...
    // === Widget Management ===
    addWidget(widget, placement = "center") {
        ...
        widget.setDocked(this.dashboard, { dockPosition: "center" });
        bus.emit("widget:added", { widget, dashboard: this.dashboard, placement: { dockPosition: "center" } });
    }
    addWidgetToPane(widget, pane, emitEvent = true) {
    ...
    renderPaneTabs(pane) {
        ...
        if (toolbar)
            pane.tabBar.appendChild(toolbar);
        ...
        for (const widgetId of pane.widgets) {
            ...
            this.disposers.push(on(tab, "click", (ev) => {
                if (ev.target.classList.contains("dock-tab-close")) {
                    widget.close();
                }
                else {
                    stop(ev);
            ...
            // Tab drag start
            this.disposers.push(on(tab, "pointerdown", (ev) => {
                if (ev.target.classList.contains("dock-tab-close"))
                    return;
    ...
    // === Drag and Drop ===
    setupPaneDropTarget(pane) {
        this.disposers.push(on(pane.el, "dragover", (ev) => {
            ev.preventDefault();
        }), on(pane.el, "pointerenter", () => {
            if (this.drag) {
                this.drag.targetPane = pane;
                const dropZone = pane.contentArea.querySelector(".dock-pane-dropzone");
                if (dropZone)
