...
export function boot(mount) {
    ...
    // ========================================
    const salesDash = container.addDashboard({ id: "sales", title: "Sales (Grid)", icon: "💰", closable: false }, { layoutMode: "grid", grid: { cols: 12, rows: 12 } });
    const revenueWidget = new Widget({
        ...
        onRefresh: async (w) => {
            ...
            w.setContent(`<div class="metric-card"><div class="metric-value">$${val.toLocaleString()}</div><div class="metric-label">Total Revenue</div><div class="metric-trend positive">↑ ${(Math.random() * 20).toFixed(1)}%</div></div>`);
        },
    });
    ...
    // ========================================
    const freeDash = container.addDashboard({ id: "analytics", title: "Analytics (Free)", icon: "📊", closable: true }, { layoutMode: "free", free: { snapToGrid: true, gridSize: 10 } });
    ...
    // ========================================
    const dockDash = container.addDashboard({ id: "monitor", title: "Monitoring (Dock)", icon: "🖥️", closable: true }, { layoutMode: "dock" });
    const cpuWidget = new Widget({
        ...
        onRefresh: async (w) => {
            ...
            w.setContent(`<div class="metric-card"><div class="metric-value">${val}%</div><div class="metric-label">Average Load</div><div class="progress-bar"><div class="progress-fill cpu" style="width: ${val}%"></div></div></div>`);
        },
    });
    // Pass { dockPosition: 'center' } to satisfy AnyPlacement type structure we defined
    dockDash.addWidget(cpuWidget, { dockPosition: "center" });
    ...
    dockDash.addWidget(memWidget, { dockPosition: "right" });
    ...
    dockDash.addWidget(netWidget, { dockPosition: "bottom" });
    ...
    dockDash.addWidget(alertsWidget, { dockPosition: "left" });
