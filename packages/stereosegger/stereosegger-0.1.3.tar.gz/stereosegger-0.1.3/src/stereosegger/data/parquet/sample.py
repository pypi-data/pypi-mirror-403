...
from scipy.spatial import cKDTree as KDTree, Rectangle
        ...
        def func(region):
            ...
            for tile in tiles:
                ...
                if pyg_data is not None:
                    ...
                    if pyg_data["tx", "belongs", "bd"].edge_index.numel() == 0 or not has_labels:
...
class STTile:
    ...
    def get_filtered_boundaries(self) -> pd.DataFrame:
        ...
        if geometry_col:
            ...
            if hasattr(boundaries, "sindex"):
                indices = boundaries.sindex.query(outset, predicate="intersects")
                return boundaries.iloc[indices]
    ...
    def get_filtered_transcripts(self) -> pd.DataFrame:
        ...
        if hasattr(self.dataset, "kdtree_tx") and self.dataset.kdtree_tx is not None:
            ...
            r = max(rx, ry)
            # query_ball_point with p=inf is a square/box query
            indices = self.dataset.kdtree_tx.query_ball_point([cx, cy], r, p=np.inf)
            ...
            mask &= self.dataset.transcripts[y_col].between(ymin, ymax)
