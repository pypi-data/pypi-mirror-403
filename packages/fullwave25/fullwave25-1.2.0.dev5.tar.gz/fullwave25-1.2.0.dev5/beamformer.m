...
for n = 1:nangles
    ...
    for ii = 1:length(lats)
        ...
        for jj = 1:length(deps)
            ...
            % Find indices of transducer elements within the active aperture
            % based on F-number constraints. Selects elements whose x-coordinates
            % are within fcen(2)/(2*fnumber) distance from the focal point's
            % x-coordinate (fcen(1)). This implements F-number based aperture
            % limiting for dynamic focusing in beamforming.
