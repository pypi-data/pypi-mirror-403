...
from matrixllm.cloud.api_client import CloudApiClient
from matrixllm.cloud.device_config import CloudDeviceCredentials, load_cloud_device_credentials, save_cloud_device_credentials
from matrixllm.node.agent import CloudDeviceConfig, NodeConfig, default_node_id, run_cloud_device, run_node
...
def join(
    ...
    """Join this machine to an MatrixLLM Local Control Plane (outbound-only)."""
...
def cloud_pair(
    cloud: str = typer.Option(..., "--cloud", help="MatrixLLM Cloud base URL (e.g., https://matrixllm.example)"),
    runtime_base_url: str = typer.Option("http://127.0.0.1:11434", "--runtime", help="Local runtime base URL"),
    model: str = typer.Option("", "--ensure-model", help="If set, ensure this chat model exists"),
    ...
    """
    Pair this PC with MatrixLLM Cloud:
      - POST /device/start
      - show user_code + verification_url
      - poll POST /device/poll until approved
      - save device_id + device_token to ~/.matrixllm/cloud_device.json
    """
    if not is_ollama_installed():
        install_ollama()
    ensure_ollama_server_running()
                ...
                if pol.status == "approved" and pol.approved:
                    ...
                    console.print(
                        Panel(
                            ...
                            f"Next: run [bold]matrixllm-node cloud-connect[/bold]",
...
def cloud_connect(
    ...
    """
    Connect this PC to MatrixLLM Cloud relay using saved device credentials
    ...
    if not (cloud_url and did and tok):
        console.print(
            ...
            "Run: [bold]matrixllm-node cloud-pair --cloud https://...[/bold]\n"
    ...
    console.print(
        Panel(
            ...
            f"[bold]Runtime:[/bold] {runtime_base_url}\n",
            title="☁️  MatrixLLM Cloud Device",
        )
    )
