...
from sgFPCMed.sgFPCMed import SGFPCMed
from unittest.mock import patch
import sgFPCMed.sgFPCMed as sgfPcmed
...
def test_initialization():
    ...
    """
    model = SGFPCMed(C=2, m=2.0, eta=2.0, max_iter=5, tol=0.1,
...
def test_modified_median_refine_all_violated(data):
    ...
    with patch.object(model, "_violates_hard_uniqueness", return_value=True), \
         patch.object(sgfPcmed, "candidate_score_weighted", fake_candidate_score_weighted):
...
def test_pipeline(data):
    ...
    parameter (m,eta) and the objective function value (J).
    """
    model = SGFPCMed(C=3, m=2.0, eta=2.0,  max_iter=10, use_obj_stop=False)
    model.fit(data)
    ...
    for row in model.membership():
        ...
        sum_u = sum(row)
        assert abs(sum_u - 1.0) < 1e-6  # memberships should sum to 1

    T = model.typicality()
    for i in range(model.C):
        sum_t = sum(T[k][i] for k in range(len(data)))
        assert abs(sum_t - 1.0) < 1e-6
...
def test_more_clusters_than_samples(data):
    ...
    """
    model = SGFPCMed(C=30, max_iter=1)
    with pytest.raises(ValueError):
        model.fit(data)
...
def test_enforce_unique_prototypes(data):
    ...
    """
    model = SGFPCMed(C=2, enforce_unique=True)
    model.fit(data)
