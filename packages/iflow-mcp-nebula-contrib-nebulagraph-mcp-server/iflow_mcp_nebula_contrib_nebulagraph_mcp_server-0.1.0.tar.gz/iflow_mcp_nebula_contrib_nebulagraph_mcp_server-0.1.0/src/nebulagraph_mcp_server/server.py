...
async def nebula_lifespan(server: FastMCP) -> AsyncIterator[NebulaContext]:
    ...
    # Check NebulaGraph version
    if os.environ.get("NEBULA_VERSION") != "v3":
        raise ValueError("NebulaGraph version must be v3")

    # Initialize the connection (non-blocking for testing)
    try:
        global_pool.init(
            [
                (
        ...
        logger.warning(f"NebulaGraph connection failed, running in degraded mode: {e!s}")
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"Error closing connection pool: {e!s}")
