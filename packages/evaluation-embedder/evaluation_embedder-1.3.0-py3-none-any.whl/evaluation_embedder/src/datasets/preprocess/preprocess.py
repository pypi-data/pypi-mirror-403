...
class ChunkTextPreprocessor(TextPreprocessor[ChunkTextPreprocessorSettings[HeuristicTokenCounterSettings]]):
    def __init__(
        self,
        config: ChunkTextPreprocessorSettings[HeuristicTokenCounterSettings],
    ) -> None:
        super().__init__(config)
        ...
        self.chunk_size_chars = self.config.max_tokens * self.config.token_counter.chars_per_token
        self.overlap_chars = self.config.overlap_tokens * self.config.token_counter.chars_per_token
...
class ProcedureDocumentTextPreprocessor(TextPreprocessor[ProcedureDocumentTextPreprocessorSettings]):
    ...
    def build_procedure_document(self, text: str) -> str:
        sections = self.parse_procedure(text)
        if not sections:
            return text  # 👈 important fallback
        blocks: List[str] = []
    ...
    def apply(self, ds: TextDataset[Any]) -> TextDataset[Any]:
        ...
        df = df.with_columns(
            pl.col("page_content")
            .map_elements(
                self.build_procedure_document,
                return_dtype=pl.String,  # 🔴 REQUIRED
            )
            .alias("page_content")
        )
        return ds.__class__.from_polars(df)
