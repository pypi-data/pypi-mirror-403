...
from .common import PROJECT_ROOT, REPO_ROOT
...
DEFAULT_AGENTS: dict[str, dict[str, str]] = {
    "claude": {"base_path": "~/.claude", "local_base_path": ".claude"},
...
class AgentConfig:
    """Resolved agent paths from settings."""
...
def _ensure_repo(
    ...
    if branch:
        try:
            ...
            _run_git(["-C", clone_root, "checkout", branch])
    elif cloned or not os.path.isdir(os.path.join(clone_root, ".git", "refs")):
...
def _resolve_agent_path(value: str, is_local: bool) -> str:
    expanded = os.path.expanduser(value)
    if is_local and not os.path.isabs(expanded):
...
def get_agents() -> dict[str, AgentConfig]:
    ...
    agents_value = settings.get("agents")
    if agents_value is None:
        agents_value = DEFAULT_AGENTS
    if not isinstance(agents_value, dict):
        raise RuntimeError("invalid settings: agents must be a mapping")

    agents: dict[str, AgentConfig] = {}
    for raw_name, raw_config in agents_value.items():
        if not isinstance(raw_name, str) or not raw_name:
            raise RuntimeError("invalid settings: agent names must be strings")
        if not isinstance(raw_config, dict):
            raise RuntimeError(f"invalid settings: agent config for {raw_name} must be a mapping")
        base_path = raw_config.get("base_path")
        if not isinstance(base_path, str) or not base_path:
            raise RuntimeError(f"invalid settings: agent {raw_name} is missing base_path")
        local_base_path = raw_config.get("local_base_path")
        if not isinstance(local_base_path, str) or not local_base_path:
            raise RuntimeError(
                f"invalid settings: agent {raw_name} local_base_path must be a string"
            )
        agents[raw_name] = AgentConfig(
            base_path=_resolve_agent_path(base_path, is_local=False),
            local_base_path=_resolve_agent_path(local_base_path, is_local=True),
        )
    return agents
