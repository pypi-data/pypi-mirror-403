...
from skget.common import CLAUDE_BASE, CODEX_BASE
...
class SkgetApp(App):
    ...
    def _show_action_view(self, skill_name: str, refresh: bool = False) -> None:
        ...
        has_global_claude_root = os.path.isdir(CLAUDE_BASE)
        has_global_codex_root = os.path.isdir(CODEX_BASE)
        has_global_claude = self._has_destination(destinations, "claude", "global")
        has_global_codex = self._has_destination(destinations, "codex", "global")
        has_local_claude = self._has_destination(destinations, "claude", "local")
        has_local_codex = self._has_destination(destinations, "codex", "local")
        ...
        self._append_scope_actions(
            actions,
            scope="global",
            has_claude_root=has_global_claude_root,
            has_codex_root=has_global_codex_root,
            has_claude=has_global_claude,
            has_codex=has_global_codex,
            scope_label="globally",
        )
        self._append_scope_actions(
            actions,
            scope="local",
            has_claude_root=True,
            has_codex_root=True,
            has_claude=has_local_claude,
            has_codex=has_local_codex,
    ...
    def _append_scope_actions(
        ...
        if has_claude_root and has_codex_root:
            if is_mixed:
                actions.append(
        ...
        if has_claude_root:
            if has_claude:
                actions.append(
                    (
                        f"uninstall_{scope}_claude",
                        f"Uninstall {scope_label} for Claude Code",
                    )
                )
            else:
                actions.append(
                    (
                        f"install_{scope}_claude",
                        f"Install {scope_label} for Claude Code",
                    )
                )
        ...
        if has_codex_root:
            if has_codex:
                actions.append(
                    (f"uninstall_{scope}_codex", f"Uninstall {scope_label} for Codex")
                )
            else:
                actions.append(
                    (f"install_{scope}_codex", f"Install {scope_label} for Codex")
                )
