...
def copy_to(v, owner_type, owner_id):
    if models.Video.objects.filter(owner_type=owner_type, owner_id=owner_id, url=v.url).exists():
        print(f'exists: {v.url}')
        return
    v.id = None
    v.owner_type_id = owner_type
    v.owner_id = owner_id
    v.save()
