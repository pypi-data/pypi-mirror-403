...
# For signing
from pysmbserver import smb, nmb, ntlm, uuid
from pysmbserver import smb3structs as smb2
from pysmbserver.spnego import SPNEGO_NegTokenInit, TypesMech, MechTypes, SPNEGO_NegTokenResp, ASN1_AID, \
    ASN1_SUPPORTED_MECH
from pysmbserver.nt_errors import STATUS_NO_MORE_FILES, STATUS_NETWORK_NAME_DELETED, STATUS_INVALID_PARAMETER, \
...
def outputToJohnFormat(challenge, username, domain, lmresponse, ntresponse):
    ...
    try:
        if len(ntresponse) > 24:
            ...
            hashed_username = username.decode('utf-16le')
            hashed_domain = domain.decode('utf-16le')
            hashed_challenge = hexlify(challenge).decode('latin-1')
            hashed_ntresponse = hexlify(ntresponse).decode('latin-1')

            ret_value = {
                'hash_string': f"{hashed_username}::{hashed_domain}:{hashed_challenge}:{hashed_ntresponse[:32]}:{hashed_ntresponse[32:]}",
                'hash_version': 'ntlmv2'
            }
        else:
            # NTLMv1
            hashed_username = username.decode('utf-16le')
            hashed_domain = domain.decode('utf-16le')
            hashed_lmresponse = hexlify(lmresponse).decode('latin-1')
            hashed_ntresponse = hexlify(ntresponse).decode('latin-1')
            hashed_challenge = hexlify(challenge).decode()

            ret_value = {
                'hash_string': f'{hashed_username}::{hashed_domain}:{hashed_lmresponse}:{hashed_ntresponse}:{hashed_challenge}',
                'hash_version': 'ntlm'
            }
    except:
        ...
        try:
            if len(ntresponse) > 24:
                # Extended Security - NTLMv2
                hashed_username = username.decode('latin-1')
                hashed_domain = domain.decode('latin-1')
                hashed_challenge = hexlify(challenge).decode('latin-1')
                hashed_ntresponse = hexlify(ntresponse).decode('latin-1')
                ...
                ret_value = {
                    'hash_string': f"{username}::{domain}:{hashed_lmresponse}:{hashed_ntresponse}:{hashed_challenge}",
                    'hash_version': 'ntlm'
                }
...
def openFile(path, fileName, accessMode, fileAttributes, openMode):
    ...
    try:
        ...
        LOG.error(f"openFile: {pathName}, {mode}")
        fid = 0
    ...
    try:
        ...
        if os.path.exists(pathName):
            ...
            if level == smb.SMB_QUERY_FILE_BASIC_INFO:
                ...
                LOG.error(f"Unknown level for query path info! 0x{level}")
                # UNSUPPORTED
        ...
        if recvPacket['Tid'] in connData['ConnectedShares']:
            ...
            if isInFileJail(path, fileName):
                ...
                if informationLevel == smb.SMB_SET_FILE_BASIC_INFO:
                    ...
                    smbServer.log(f"Unknown level for set path info! 0x{setPathInfoParameters['InformationLevel']}", logging.ERROR)
                    # UNSUPPORTED
                    errorCode = STATUS_NOT_SUPPORTED
            else:
        ...
        if recvPacket['Tid'] in connData['ConnectedShares']:
            if setFileInfoParameters['FID'] in connData['OpenedFiles']:
                ...
                if informationLevel == smb.SMB_SET_FILE_DISPOSITION_INFO:
                    ...
                    smbServer.log(f"Unknown level for set file info! 0x{setFileInfoParameters['InformationLevel']}", logging.ERROR)
                    # UNSUPPORTED
                    errorCode = STATUS_NOT_SUPPORTED
            else:
                errorCode = STATUS_NO_SUCH_FILE
...
class SMBCommands:
        ...
        if transParameters['ParameterCount'] != transParameters['TotalParameterCount']:
            ...
            if command in transCommands:
                ...
                try:
                    ...
                    smbServer.log(f"Transaction: ({command}, {e})", logging.ERROR)
                    errorCode = STATUS_ACCESS_DENIED
                    # raise
                ...
                if setup == b'' and parameters == b'' and data == b'':
                    ...
                    while remainingData > 0 or remainingParameters > 0:
                        ...
                        if len(data) > transParameters['MaxDataCount']:
                            ...
                            LOG.debug(f"Lowering answer from {len(data)} to {transParameters['MaxDataCount']}")
                ...
                smbServer.log(f"Unsupported Transact command {command}", logging.ERROR)
                respParameters = b''
                respData = b''
        ...
        if NTTransParameters['ParameterCount'] != NTTransParameters['TotalParameterCount']:
            ...
            if command in transCommands:
                ...
                try:
                    ...
                    smbServer.log(f"NTTransaction: (0x{command}, {e})", logging.ERROR)
                    errorCode = STATUS_ACCESS_DENIED
                    # raise
                ...
                if setup == b'' and parameters == b'' and data == b'':
                    ...
                    while remainingData > 0 or remainingParameters > 0:
                        ...
                        if len(data) > NTTransParameters['MaxDataCount']:
                            # Answer doesn't fit in this packet
                            LOG.debug(f"Lowering answer from {len(data)} to {NTTransParameters['MaxDataCount']}")
        ...
        if trans2Parameters['ParameterCount'] != trans2Parameters['TotalParameterCount']:
            ...
            if command in transCommands:
                ...
                try:
                    ...
                    smbServer.log(f"Transaction2: (0x{command}, {e})", logging.ERROR)
                    # import traceback
                    # traceback.print_exc()
                    raise
                ...
                if setup == b'' and parameters == b'' and data == b'':
                    ...
                    while remainingData > 0 or remainingParameters > 0:
                        ...
                        if len(data) > trans2Parameters['MaxDataCount']:
                            # Answer doesn't fit in this packet
                            LOG.debug(f"Lowering answer from {len(data)} to {trans2Parameters['MaxDataCount']}")
                ...
                smbServer.log(f"Unsupported Transact/2 command 0x{command}", logging.ERROR)
                respParameters = b''
                respData = b''
                errorCode = STATUS_NOT_IMPLEMENTED
        ...
        if recvPacket['Tid'] in connData['ConnectedShares']:
            if comClose['FID'] in connData['OpenedFiles']:
                ...
                try:
                    ...
                    smbServer.log(f"comClose {e}", logging.ERROR)
                    errorCode = STATUS_ACCESS_DENIED
                else:
        ...
        if recvPacket['Tid'] in connData['ConnectedShares']:
            if comWriteParameters['Fid'] in connData['OpenedFiles']:
                ...
                try:
                    ...
                    smbServer.log(f"smbComWrite: {e}", logging.ERROR)
                    errorCode = STATUS_ACCESS_DENIED
            else:
                errorCode = STATUS_INVALID_HANDLE
        else:
        ...
        if recvPacket['Tid'] in connData['ConnectedShares']:
            if comFlush['FID'] in connData['OpenedFiles']:
                ...
                try:
                    ...
                    smbServer.log(f"comFlush {e}", logging.ERROR)
                    errorCode = STATUS_ACCESS_DENIED
            else:
                errorCode = STATUS_INVALID_HANDLE
        else:
        ...
        if recvPacket['Tid'] in connData['ConnectedShares']:
            ...
            if not isInFileJail(path, fileName):
                ...
                try:
                    ...
                    smbServer.log(f"smbComCreateDirectory: {e}", logging.ERROR)
                    errorCode = STATUS_ACCESS_DENIED
        else:
            errorCode = STATUS_SMB_BAD_TID
        ...
        if recvPacket['Tid'] in connData['ConnectedShares']:
            ...
            if not isInFileJail(path, oldFileName) or not isInFileJail(path, newFileName):
                ...
                try:
                    ...
                    smbServer.log(f"smbComRename: {e}", logging.ERROR)
                    errorCode = STATUS_ACCESS_DENIED
        else:
            errorCode = STATUS_SMB_BAD_TID
        ...
        if recvPacket['Tid'] in connData['ConnectedShares']:
            ...
            if not isInFileJail(path, fileName):
                ...
                try:
                    ...
                    smbServer.log(f"smbComDelete: {e}", logging.ERROR)
                    errorCode = STATUS_ACCESS_DENIED
        else:
            errorCode = STATUS_SMB_BAD_TID
        ...
        if recvPacket['Tid'] in connData['ConnectedShares']:
            ...
            if not isInFileJail(path, fileName):
                ...
                try:
                    ...
                    smbServer.log(f"smbComDeleteDirectory: {e}", logging.ERROR)
        ...
        if recvPacket['Tid'] in connData['ConnectedShares']:
            if writeAndX['Fid'] in connData['OpenedFiles']:
                ...
                try:
                    ...
                    smbServer.log(f"smbComWriteAndx: {e}", logging.ERROR)
                    errorCode = STATUS_ACCESS_DENIED
            else:
                errorCode = STATUS_INVALID_HANDLE
        else:
        ...
        if recvPacket['Tid'] in connData['ConnectedShares']:
            if comReadParameters['Fid'] in connData['OpenedFiles']:
                ...
                try:
                    ...
                    smbServer.log(f"smbComRead: {e}", logging.ERROR)
                    errorCode = STATUS_ACCESS_DENIED
            else:
                errorCode = STATUS_INVALID_HANDLE
        else:
        ...
        if recvPacket['Tid'] in connData['ConnectedShares']:
            if readAndX['Fid'] in connData['OpenedFiles']:
                ...
                try:
                    ...
                    smbServer.log(f"smbComReadAndx: {e}", logging.ERROR)
                    errorCode = STATUS_ACCESS_DENIED
            else:
                errorCode = STATUS_INVALID_HANDLE
        else:
    ...
    def smbComTreeDisconnect(connId, smbServer, SMBCommand, recvPacket):
        ...
        if recvPacket['Tid'] in connData['ConnectedShares']:
            smbServer.log(f"Disconnecting Share({recvPacket['Tid']}:{connData['ConnectedShares'][recvPacket['Tid']]['shareName']})", logging.DEBUG)
        ...
        if recvPacket['Tid'] in connData['ConnectedShares']:
            if queryInformation2['Fid'] in connData['OpenedFiles']:
                ...
                try:
                    ...
                    smbServer.log(f"smbComQueryInformation2 {e}", logging.ERROR)
                    errorCode = STATUS_ACCESS_DENIED
        else:
            errorCode = STATUS_SMB_BAD_TID
                ...
                if mode & os.O_CREAT == os.O_CREAT:
                    if createOptions & smb.FILE_DIRECTORY_FILE == smb.FILE_DIRECTORY_FILE:
                        try:
                            ...
                            smbServer.log(f"NTCreateAndX: {pathName}, {mode}, {e}", logging.ERROR)
                            errorCode = STATUS_ACCESS_DENIED
    ...
    def smbComTreeConnectAndX(connId, smbServer, SMBCommand, recvPacket):
        ...
        if share is not None:
            ...
            smbServer.log(f"TreeConnectAndX not found {path}", logging.ERROR)
    ...
    def smbComSessionSetupAndX(connId, smbServer, SMBCommand, recvPacket):
        ...
        if SMBCommand['WordCount'] == 12:
            ...
            if struct.unpack('B', sessionSetupData['SecurityBlob'][0:1])[0] == ASN1_AID:
                ...
                if len(blob['MechTypes'][0]) > 0:
                    ...
                    if mechType != TypesMech['NTLMSSP - Microsoft NTLM Security Support Provider']:
                        ...
                        smbServer.log(f"Unsupported MechType '{mechStr}'", logging.DEBUG)
            ...
            if messageType == 0x01:
                ...
                smbServer.log(f"AUTHENTICATE_MESSAGE ({domain_name}\\{user_name},{host_name})", logging.DEBUG)
                ...
                if errorCode == STATUS_SUCCESS:
                    ...
                    smbServer.log(f"User {host_name}\\{user_name} authenticated successfully", logging.INFO)
                    ...
                    try:
                        ...
                        smbServer.log(ntlm_hash_data['hash_string'], logging.DEBUG)
                        if jtr_dump_path != '':
                            writeJohnOutputToFile(ntlm_hash_data['hash_string'], ntlm_hash_data['hash_version'],
                                                  jtr_dump_path)
                    except:
                        smbServer.log(f"Could not write NTLM Hashes to the specified JTR_Dump_Path {jtr_dump_path}", logging.ERROR)
                else:
                    respToken = SPNEGO_NegTokenResp()
                    respToken['NegState'] = b'\x02'
                    smbServer.log("Could not authenticate user!", logging.ERROR)
                if smbServer.auth_callback is not None:
                    try:
                        ...
                        LOG.warning(f"Could not call auth_callback: {e}")

            else:
                raise Exception(f"Unknown NTLMSSP MessageType {messageType}")
            ...
            smbServer.log(f"User {sessionSetupData['PrimaryDomain']}\\{sessionSetupData['Account']} authenticated successfully (basic)", logging.INFO)
            try:
                jtr_dump_path = smbServer.getJTRdumpPath()
                ntlm_hash_data = outputToJohnFormat(b'',
                                                    b(sessionSetupData['Account']),
                                                    b(sessionSetupData['PrimaryDomain']), sessionSetupData['AnsiPwd'],
                                                    sessionSetupData['UnicodePwd'])
                smbServer.log(ntlm_hash_data['hash_string'], logging.INFO)
                if jtr_dump_path != '':
    ...
    def smbComNegotiate(connId, smbServer, SMBCommand, recvPacket):
        ...
        try:
            ...
            smbServer.log(f"smbComNegotiate: {e}", logging.ERROR)
    ...
    def default(connId, smbServer, SMBCommand, recvPacket):
        ...
        smbServer.log(f"Not implemented command: 0x{recvPacket['Command']}", logging.DEBUG)
...
class SMB2Commands:
    ...
    def smb2TreeConnect(connId, smbServer, recvPacket):
        ...
        if share is not None:
            ...
            smbServer.log(f"Connecting Share({tid}:{path})", logging.INFO)
        else:
                ...
                if mode & os.O_CREAT == os.O_CREAT:
                    if createOptions & smb2.FILE_DIRECTORY_FILE == smb2.FILE_DIRECTORY_FILE:
                        try:
                            ...
                            smbServer.log(f"SMB2_CREATE: {pathName},{mode},{e}", logging.ERROR)
                            errorCode = STATUS_ACCESS_DENIED
        ...
        if recvPacket['TreeID'] in connData['ConnectedShares']:
            if fileID in connData['OpenedFiles']:
                ...
                try:
                    ...
                    smbServer.log(f"SMB2_CLOSE {e}", logging.ERROR)
                    errorCode = STATUS_INVALID_HANDLE
                else:
                    # Check if the file was marked for removal
        ...
        if recvPacket['TreeID'] in connData['ConnectedShares']:
            if fileID in connData['OpenedFiles']:
                ...
                if queryInfo['InfoType'] == smb2.SMB2_0_INFO_FILE:
                    ...
                    smbServer.log(f"queryInfo not supported ({queryInfo['InfoType']})", logging.ERROR)
        ...
        if recvPacket['TreeID'] in connData['ConnectedShares']:
            ...
            if fileID in connData['OpenedFiles']:
                ...
                if setInfo['InfoType'] == smb2.SMB2_0_INFO_FILE:
                    ...
                    if informationLevel == smb2.SMB2_FILE_DISPOSITION_INFO:
                        ...
                        try:
                            ...
                            smbServer.log(f"smb2SetInfo: {e}", logging.ERROR)
                            errorCode = STATUS_ACCESS_DENIED
                        ...
                        smbServer.log(f"Unknown level for set file info! 0x{informationLevel}", logging.ERROR)
                        # UNSUPPORTED
                        errorCode = STATUS_NOT_SUPPORTED
                ...
                #    setInfo = queryFsInformation('/', fileName, queryInfo['FileInfoClass'])
                    ...
                    smbServer.log(f"setInfo not supported ({setInfo['InfoType']})", logging.ERROR)
        ...
        if recvPacket['TreeID'] in connData['ConnectedShares']:
            if fileID in connData['OpenedFiles']:
                ...
                try:
                    ...
                    smbServer.log(f"SMB2_WRITE: {e}", logging.ERROR)
                    errorCode = STATUS_ACCESS_DENIED
            else:
                errorCode = STATUS_INVALID_HANDLE
        else:
        ...
        if recvPacket['TreeID'] in connData['ConnectedShares']:
            if fileID in connData['OpenedFiles']:
                ...
                try:
                    ...
                    smbServer.log(f"SMB2_READ: {e}", logging.ERROR)
                    errorCode = STATUS_ACCESS_DENIED
            else:
                errorCode = STATUS_INVALID_HANDLE
        else:
        ...
        if recvPacket['TreeID'] in connData['ConnectedShares']:
            if flushRequest['FileID'].getData() in connData['OpenedFiles']:
                ...
                try:
                    ...
                    smbServer.log(f"SMB2_FLUSH {e}", logging.ERROR)
                    errorCode = STATUS_ACCESS_DENIED
            else:
                errorCode = STATUS_INVALID_HANDLE
        else:
    ...
    def smb2TreeDisconnect(connId, smbServer, recvPacket):
        ...
        # Get the Tid associated
        if recvPacket['TreeID'] in connData['ConnectedShares']:
            smbServer.log(f"Disconnecting Share({recvPacket['TreeID']}:{connData['ConnectedShares'][recvPacket['TreeID']]['shareName']})", logging.DEBUG)
    ...
    def smb2Ioctl(connId, smbServer, recvPacket):
        ...
        if ioctlRequest['CtlCode'] in ioctls:
            ...
            smbServer.log(f"Ioctl not implemented command: 0x{ioctlRequest['CtlCode']}", logging.DEBUG)
    ...
    def fsctlPipeTransceive(connId, smbServer, ioctlRequest):
        ...
        if ioctlRequest['FileID'].getData() in connData['OpenedFiles']:
            ...
            try:
                ...
                smbServer.log(f"fsctlPipeTransceive: {e}", logging.ERROR)
                errorCode = STATUS_ACCESS_DENIED
        else:
            errorCode = STATUS_INVALID_DEVICE_REQUEST
...
class SMBSERVERHandler(socketserver.BaseRequestHandler):
    ...
    def handle(self):
        self.__SMB.log(f"Incoming connection ({self.__ip},{self.__port})")
        ...
        while True:
            try:
                ...
                if p.get_type() == nmb.NETBIOS_SESSION_REQUEST:
                    ...
                    self.__SMB.log(f"NetBIOS Session request ({self.__ip},{remote_name[1].strip()},{myname[1]})")
    ...
    def finish(self):
        ...
        self.__SMB.log(f"Closing down connection ({self.__ip},{self.__port})")
...
class SMBSERVER(socketserver.ThreadingMixIn, socketserver.TCPServer):
    ...
    def removeConnection(self, name):
        try:
            ...
            pass
        self.log(f"Remaining connections {list(self.__activeConnections.keys())}", logging.INFO)
    ...
    def processRequest(self, connId, data):
        ...
        try:
                            ...
                            if self.__SMB2Support is True:
                                if packet['Command'] == smb.SMB.SMB_COM_NEGOTIATE:
                                    try:
                                        ...
                                        import traceback
                                        traceback.print_exc()
                                        self.log(f"SMB2_NEGOTIATE: {e}", logging.ERROR)
            ...
            self.log(f"processRequest (0x{packet['Command']},{e})", logging.ERROR)
            raise
    ...
    def processConfigFile(self, configFile=None):
        ...
        if credentials_fname != "":
            ...
            while line:
                ...
                line = cred.readline()
            cred.close()
        self.log('Config file parsed', logging.DEBUG)
...
from pysmbserver.dcerpc.v5.rpcrt import DCERPCServer
from pysmbserver.dcerpc.v5.dtypes import NULL
from pysmbserver.dcerpc.v5.srvs import NetrShareEnum, NetrShareEnumResponse, SHARE_INFO_1, NetrServerGetInfo, \
    NetrServerGetInfoResponse, NetrShareGetInfo, NetrShareGetInfoResponse
from pysmbserver.dcerpc.v5.wkst import NetrWkstaGetInfo, NetrWkstaGetInfoResponse
from pysmbserver.system_errors import ERROR_INVALID_LEVEL
...
class WKSTServer(DCERPCServer):
    ...
    def NetrWkstaGetInfo(self, data):
        ...
        self.log(f"NetrWkstaGetInfo Level: {request['Level']}", logging.INFO)
...
class SRVSServer(DCERPCServer):
    ...
    def NetrShareGetInfo(self, data):
        ...
        self.log(f"NetrGetShareInfo Level: {request['Level']}", logging.DEBUG)
    ...
    def NetrServerGetInfo(self, data):
        ...
        self.log(f"NetrServerGetInfo Level: {request['Level']}", logging.INFO)
    ...
    def NetrShareEnum(self, data):
        ...
        self.log(f"NetrShareEnum Level: {request['InfoStruct']['Level']}", logging.DEBUG)
...
class SimpleSMBServer:
    ...
    def addCredential(self, name, uid=0, password=None, hashes=None, prompt_if_missing=True):
        """
        Adds user credentials to the SMB server.

        :param name: Username
        :param uid: User ID (default 0)
        :param password: Clear-text password (optional)
        :param hashes: LMHASH:NTHASH (optional)
        :param prompt_if_missing: If True and no password/hashes is provided, prompt for password
        """
        ...
        if password is None and hashes is None:
            if prompt_if_missing:
                ...
                raise ValueError("Password or NTLM hashes must be provided")
