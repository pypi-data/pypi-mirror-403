...
class CreateInterceptionRequest(BaseModel):
    """
    ...
    """ # noqa: E501
    type: StrictStr
    api_id: StrictStr = Field(description="Should only be provided if type=PRE_REQUEST or type=POST_REQUEST", alias="apiId")
    entity_name: StrictStr = Field(description="Should only be provided if type=ADAPT_ENTITY", alias="entityName")
    endpoint: StrictStr
    execution_order: Optional[StrictInt] = Field(default=None, description="If not provided, the default value will be taken from the environment variable: EXECUTIONORDER_DEFAULT. Subscription with the same executionOrder cannot exist simultaneously. If an interception is created or updated to have the same executionOrder as an existing interception (including existing ones with default executionOrder), It will be set with that executionOrder and the existing interception's executionOrder will be incremented. If that interception's new executionOrder clashes with another the other one will be incremented as well, and so forth.", alias="executionOrder")
    # BioT Code Start: Store custom attributes in a dedicated dict (safe default)
    additional_properties: Dict[str, Any] = Field(default_factory=dict)
    # BioT Code End
    __properties: ClassVar[List[str]] = ["type", "apiId", "entityName", "endpoint", "executionOrder"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['PRE_REQUEST', 'POST_REQUEST', 'ADAPT_ENTITY']):
            raise ValueError("must be one of enum values ('PRE_REQUEST', 'POST_REQUEST', 'ADAPT_ENTITY')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of CreateInterceptionRequest from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of CreateInterceptionRequest from a dict"""
        if obj is None:
            return None
