...
class SubscribeResponse(BaseModel):
    ...
    # BioT Code End
    __properties: ClassVar[List[str]] = ["entityTypeName", "actionName", "endpoint", "id"]

    @field_validator('entity_type_name')
    def entity_type_name_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['device', 'organization', 'organization-user', 'caregiver', 'patient', 'usage-session', 'generic-entity', 'command', 'device-alert', 'patient-alert', 'registration-code']):
            raise ValueError("must be one of enum values ('device', 'organization', 'organization-user', 'caregiver', 'patient', 'usage-session', 'generic-entity', 'command', 'device-alert', 'patient-alert', 'registration-code')")
        return value

    @field_validator('action_name')
    def action_name_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['_create', '_update', '_delete', '_list', '_command', '_generateCertificate', '_activateCertificate', '_revokeCertificate', '_operate', '_read', '_start', '_stop', '_remoteStart', '_remoteStop', '_remoteResume', '_remotePause', '_updateAdmin', '_updateEnabledState', '_login', '_logout', '_clear', '_manage', '_assign', '_saveMeasurement', '_monitor', '_view', '_saveMeasurementBulk', '_incomingSms']):
            raise ValueError("must be one of enum values ('_create', '_update', '_delete', '_list', '_command', '_generateCertificate', '_activateCertificate', '_revokeCertificate', '_operate', '_read', '_start', '_stop', '_remoteStart', '_remoteStop', '_remoteResume', '_remotePause', '_updateAdmin', '_updateEnabledState', '_login', '_logout', '_clear', '_manage', '_assign', '_saveMeasurement', '_monitor', '_view', '_saveMeasurementBulk', '_incomingSms')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of SubscribeResponse from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of SubscribeResponse from a dict"""
        if obj is None:
            return None
