...
def edit_data(data_file: Path = DATA_FILE):
    """Open the data file in the default editor for manual editing."""
    ensure_data_dir(data_file)
    
    # Read existing records
    records = read_all_records(data_file)
    
    # Create a temporary file with the data in editable JSON format
    ...
    try:
        # Get the editor from environment variable, default to nano
        ...
        # Validate each record has the required fields
        for i, record in enumerate(edited_records):
            if not isinstance(record, dict):
                print(f"Error: Record {i} is not a JSON object.")
                return 1
            required_fields = [
                'sleep_date', 'sleep_time', 'wake_date', 'wake_time',
                'difficulty_sleep', 'difficulty_wake', 'duration_hours'
            ]
            for field in required_fields:
                if field not in record:
                    print(f"Error: Record {i} is missing required field '{field}'.")
                    return 1
        
        # Backup original data
        backup_path = data_file.with_suffix('.jsonl.bak')
        if data_file.exists():
            data_file.rename(backup_path)
        
        # Write the validated edited records
        ensure_data_dir(data_file)
        with data_file.open('w', encoding='utf-8') as f:
            for record in edited_records:
                f.write(json.dumps(record, ensure_ascii=False) + '\n')
        
        print(f"Data saved successfully. ({len(edited_records)} records)")
        ...
        print("Error: Editor was not closed successfully.")
        return 1
    except json.JSONDecodeError as e:
        print(f"Error: Invalid JSON in edited file: {e}")
        return 1
...
def main(argv=None):
    ...
    group.add_argument(
        "--edit",
        action="store_true",
        help="Edit existing sleep data in your default terminal editor",
    )
    ...
    if args.json:
        ...
        return 0
    if args.edit:
        return edit_data(data_file=data_file)
