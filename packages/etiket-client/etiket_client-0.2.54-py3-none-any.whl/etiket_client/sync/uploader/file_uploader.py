            ...
            try:
                ...
                if response.status_code == 200 or response.status_code == 201:
                    ...
                    if 'Content-MD5' in response.headers:
                        ...
                        logger.info(f"No Content-MD5 or ETag from server for {file_raw_name}. Comparing recalculated local checksum with pre-upload local checksum.")
                    ...
                    success = False
                    try:
                        logger.warning('Failed to upload a file with path (%s).\nRAW JSON response :: %s', file_raw_name, response.json())
