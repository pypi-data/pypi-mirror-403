from etiket_client.sync.base.manifest_v1 import manifest_state_V1
from etiket_client.sync.base.manifest_v2 import manifest_state_V2
...
class manifest_manager:
    ...
    def __init__(self, name : str, root_path : Optional[Path] = None, current_manifest : Optional[Dict[str, float]] = None,  level : int = -1, is_NFS : bool = False, is_single_file : bool = False):
        ...
        if name in self.__manifest_contents:
            ...
            if level > 0:
                self.state = manifest_state_V1(root_path, current_manifest, level, is_NFS=is_NFS, is_single_file=is_single_file)
            else:
