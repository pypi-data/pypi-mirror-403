...
class manifest_state_V1:
    ...
    def __post_init__(self):
        self.file_watcher = get_observer(self.root_path, self.level, self.is_single_file, self.update_queue, self.is_NFS)
        print(self.root_path, self.level, self.is_single_file)
        new_manifest = manifest_get(self.root_path, self.level, self.is_single_file)
...
def get_observer(root_path : Path, level : int, is_single_file : bool, update_queue : Dict[str, float], is_NFS : bool) -> 'BaseObserverSubclassCallable':
    if is_NFS:
        observer = PollingObserver()
    else:
        observer = Observer()
    
    observer.schedule(manifest_v1_watcher(root_path, level, is_single_file, update_queue), root_path, recursive=True)
    observer.start()
    return observer

class manifest_v1_watcher(FileSystemEventHandler):
    def __init__(self, root_path : Path, level : int, is_single_file : bool, update_queue : Dict[str, float]):
        super().__init__()
    ...
    def add_update(self, path : Path):
        if self.is_single_file is False:
            ...
            if len(key_name.parts) == self.level:
                self.update_queue[str(key_name)] = os.stat(path).st_mtime

def manifest_get(base_directory : Path, level : int, is_single_file : bool = False) -> Dict[str, float]:
    manifest  = {}
    ...
    if is_single_file is True:
        for root, _, files in os.walk(base_directory):
            ...
            if current_depth == level-1:
                for file in files:
                    mod_time = os.stat(f"{root}/{file}").st_mtime
                    key_name = str((Path(root)/file).relative_to(base_directory))
