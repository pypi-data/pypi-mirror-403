...
class manifest_state_V2:
    ...
    def __post_init__(self):
        self.file_watcher = get_observer(self.root_path, self.update_queue, self.current_manifest, self.is_NFS)
        new_manifest = get_dataset_mod_times(self.root_path)
        enqueue_updates(self.update_queue, self.current_manifest, new_manifest)

    def get_last_change(self, identifier : str) -> float:
        manifest_path = Path.joinpath(self.root_path, *identifier.split('/'))
        out = list(get_dataset_mod_times(manifest_path).values())[0]
        return out

def enqueue_updates(queue : Dict[str, float], manifest : Dict[str, float], new_items : Dict[str, float]):
    '''
    ...
    for path, mod_time in new_items.items():
        if path not in manifest:
            ...
            if mod_time > manifest[path]:
                queue[path] = mod_time
                manifest[path] = mod_time

def get_observer(root_path : Path, update_queue : Dict[str, float], current_manifest : Dict[str, float], is_NFS : bool) -> 'BaseObserverSubclassCallable':
    if is_NFS is True:
        observer = PollingObserver()
    else:
        observer = Observer()
    observer.schedule(manifest_v2_watcher(root_path, update_queue, current_manifest), root_path, recursive=True)
    observer.start()
    return observer

class manifest_v2_watcher(FileSystemEventHandler):
    def __init__(self, root_path : Path, update_queue : Dict[str, float], current_manifest : Dict[str, float]):
        super().__init__()
        self.root_path = root_path
        self.update_queue = update_queue
        self.current_manifest = current_manifest # this should be added, as somethings event can be triggered, if nothing did happen ...
    ...
    def add_update(self, path : Path):
        ...
        while path != self.root_path:
            if (path / QH_DATASET_INFO_FILE).exists():
                ...
                if len(mod_times) > 0:
                    enqueue_updates(self.update_queue, self.current_manifest, {relative_path: mod_times[0]})
                return
            path = path.parent
    ...
    for info_file in dataset_info_file_locations:
        ...
        for file_path in dataset_dir.rglob('*'):
            if file_path.is_file() and not neglect_file(file_path):
                # mod_time = round(file_path.stat().st_mtime, 3)
