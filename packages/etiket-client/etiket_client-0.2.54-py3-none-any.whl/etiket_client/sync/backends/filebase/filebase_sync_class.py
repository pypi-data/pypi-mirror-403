...
from etiket_client.sync.backends.filebase.converters.base import FileConverterHelper
from etiket_client.sync.backends.filebase.manifest import Manifest, get_mtime_of_folder
from etiket_client.exceptions import NoConvertorException, SynchronizationErrorException

from datetime import datetime
from pathlib import Path

import os, pathlib, xarray, yaml, typing, re, logging, fnmatch
...
class FileBaseSync(SyncSourceFileBase):
    SyncAgentName = "FileBaseGeneric"
    ConfigDataClass = FileBaseConfigData
    ...
    @staticmethod
    def checkLiveDataset(configData: FileBaseConfigData, syncIdentifier: sync_item, maxPriority: bool) -> bool:
        return False
    
    @staticmethod
    def syncDatasetNormal(configData: FileBaseConfigData, syncIdentifier: sync_item):
        ...
        try :
            ...
            # if dataset exists and sync_time < mod_time of the info file, then do not update the dataset
            if manifest.sync_time is not None:
                st_mtime = Path(dataset_path / QH_DATASET_INFO_FILE).stat().st_mtime
                st_mdatetime = datetime.fromtimestamp(st_mtime)
                if st_mdatetime > manifest.sync_time or not dataset_exists(syncIdentifier):
                    create_or_update_dataset(configData, syncIdentifier, sync_info)
            else:
                create_or_update_dataset(configData, syncIdentifier, sync_info)
        except Exception as e:
            manifest.add_error(e)
            manifest.write(sync_time)
            raise e
    ...
    @staticmethod
    def syncDatasetLive(configData: FileBaseConfigData, syncIdentifier: sync_item):
        raise NotImplementedError("Live sync is not implemented for FileBaseSync")


def dataset_exists(syncIdentifier : sync_item) -> bool:
    return sync_utilities.dataset_exists(syncIdentifier)

def create_or_update_dataset(configData : FileBaseConfigData, syncIdentifier : sync_item, sync_info : dict):
    """
...
def upload_file(root : str, file : str, dataset_path : pathlib.Path,
                syncIdentifier : sync_item, manifest : Manifest,
