...
class Manifest:
    def __init__(self, dataset_path : Path, syncIdentifier : sync_item):
        """
        Initialize the Manifest object.

        Args:
            dataset_path (Path): The path to the dataset directory.
            syncIdentifier (sync_item): The sync item object.
        """
    ...
    def sync_time(self) -> datetime.datetime:
        if self.manifest.get('sync_time', None) is not None:
            return datetime.datetime.fromisoformat(self.manifest['sync_time'])
        return None
    ...
    def write(self, sync_time : Optional[datetime.datetime] = None):
        """
        Write the manifest data to the manifest file.
        """
        if sync_time is None:
            sync_time = datetime.datetime.now()
        self.manifest['sync_time'] = sync_time.isoformat()
