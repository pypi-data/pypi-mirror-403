...
qdrive_version = qdrive.__version__
if parse_version(qdrive_version) < Version("0.2.49"):
    raise RuntimeError(f"qdrive>=0.2.49 required, found {qdrive_version}, please update.")
...
def copy_dataset(dataset_src_uuid, scope_src_uuid, scope_dst_uuid):
    ...
    try :
        if ds.alt_uid is not None:
            dataset_read_by_alt_uid(ds.alt_uid, scope_dst_uuid)
        else:
    ...
    for file in ds:
        ...
        for version in file.version_ids:
            ...
            if file_read_server is None:
                print(f"CREATING FILE {file_version.name} - {file_version.version_id} ON REMOTE SERVER")
                fc = FileCreate(
                    name=file_version.name,
                    filename = file_version.filename,
                    uuid=file_uuid,
                    creator=file_version.creator,
                    collected=file_version.collected,
                    size=size_bytes,
                    type=file_version.type,
                    file_generator="",
...
uuids = [
    '90c7fc9474754691a989f061bf5a6752',
    'c73551c199074e329bc22be0ba74a533',
    '90faffbbaf064919882bf52a77b59024'
]

source_scope = get_scope_by_name('test scope QH')
target_scope = get_scope_by_name('scope_3')

for ds_uuid in uuids:
    copy_dataset(ds_uuid, source_scope.uuid, target_scope.uuid)