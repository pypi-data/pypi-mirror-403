"""
PHPInfo Analysis Submodule

This submodule analyzes phpinfo.php pages to extract detailed technology 
information including PHP version, web server, operating system, and loaded 
PHP extensions.

The module uses pattern matching to identify technologies based on pattern 
definitions from JSON file.

Functions:
    analyze: Entry point for analyzing phpinfo pages (called by SOURCES)
"""
...
def analyze(tech_info: Dict[str, Any], args: object, helpers: object) -> Dict[str, Any]:
    """
    Analyze phpinfo.php page content.
    ...
    if not patterns:
        if args.verbose:
            ptprint("No phpinfo patterns loaded from phpinfo.json", "INFO", not args.json, indent=8, colortext=True)
        return tech_info
    ...
    if unique_components:
        ...
        for component in unique_components:
            ...
            # If this is a PHP Extension (category_id 15), add indentation
            is_php_extension = component.get('category_id') == 15
            indent_prefix = "    " if is_php_extension else ""
            ...
            storage.add_to_storage(
                ...
                description=f"PHPInfo: {component['technology']}",
...
def _load_patterns(helpers: object, args: object) -> List[Dict[str, Any]]:
    ...
    """
    try:        
        definitions = helpers.load_definitions("subdefinitions/phpinfo.json")
        return definitions.get('patterns', []) if definitions else []
    except Exception as e:
        if args.verbose:
            ptprint(f"Error loading phpinfo.json: {str(e)}","ADDITIONS", not args.json, indent=8, colortext=True)
        return []
...
def _deduplicate_components(components: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    Deduplicate detected components, preferring header sources over others.
    Priority: header > title > environment > server_api > system_distro > module > system_generic
