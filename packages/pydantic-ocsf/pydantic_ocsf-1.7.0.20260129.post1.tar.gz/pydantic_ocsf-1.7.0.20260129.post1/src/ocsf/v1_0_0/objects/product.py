...
class Product(OCSFBaseModel):
    ...
    See: https://schema.ocsf.io/1.0.0/objects/product
    """
    ...
    lang: str | None = Field(
        default=None,
        description="The two letter lower case language codes, as defined by <a target='_blank' href='https://en.wikipedia.org/wiki/ISO_639-1'>ISO 639-1</a>. For example: <code>en</code> (English), <code>de</code> (German), or <code>fr</code> (French). [Recommended]",
    )
