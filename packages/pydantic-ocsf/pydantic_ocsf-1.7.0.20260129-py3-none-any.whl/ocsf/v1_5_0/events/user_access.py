"""User Access Management event class."""
...
class UserAccess(OCSFBaseModel):
    """User Access Management events report management updates to a user's privileges.

    OCSF Class UID: 5
    Category:

    See: https://schema.ocsf.io/1.5.0/classes/user_access
    """
    ...
    privileges: list[str] = Field(..., description="List of privileges assigned to a user.")
    ...
    user: User = Field(..., description="User to which privileges were assigned.")
    ...
    resources: list[ResourceDetails] | None = Field(
        default=None, description="Resources that the privileges give access to. [Recommended]"
    )
