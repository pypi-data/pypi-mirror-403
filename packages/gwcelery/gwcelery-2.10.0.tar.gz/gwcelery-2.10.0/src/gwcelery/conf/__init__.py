...
voevent_receiver_addresses = ['68.169.57.253:8099']
"""The VOEvent listener will connect to these addresses to receive VOEvents.
For options, see `GCN's list of available VOEvent servers
<https://gcn.gsfc.nasa.gov/voevent.html#tc2>`_. If this is an empty list,
then this will completely disable the listener."""
...
pipelines are gstlal, pycbc, mbta, spiir, aframe, and raven. The Burst BBH
pipeline is cwb. CBC EarlyWarning pipelines are gstlal, pycbc, mbta, and spiir.
CBC SSM pipelines are gstlal, mbta, and raven.  The Burst AllSky searches are
treated as one group with one trials factor. The Burst AllSky piplines are cwb,
mly, and raven."""
...
trials factor. CBC AllSky pipelines are gstlal, pycbc, mbta, spiir, and aframe.
...
llhoft_glob = '/dev/shm/kafka/{detector}_O3ReplayEWBNS/*.gwf'
"""File glob for playground low-latency h(t) frames. Currently points
to O3 Low Latency Early Warning Mock Data Channel.
...
"""Low-latency iDQ OK channel names for O3 replay. High bit indicates iDQ is
ok."""
...
low_latency_frame_types = {'H1': 'H1_O3ReplayEWBNS_llhoft',
                           'L1': 'L1_O3ReplayEWBNS_llhoft',
                           'V1': 'V1_O3ReplayEWBNS_llhoft'}
...
strain_channel_names = {'H1': 'H1:GDS-CALIB_STRAIN_EW_BNS_INJ_O3ReplayEWBNS',
                        'L1': 'L1:GDS-CALIB_STRAIN_EW_BNS_INJ_O3ReplayEWBNS',
                        'V1': 'V1:Hrec_hoft_16384Hz_EW_BNS_INJ_O3ReplayEWBNS'}
...
raven_ext_rates = {
    'GRB': 325 / (3600 * 24 * 365),    # 325 / yr
    'MDC': 325 / (3600 * 24 * 365),    # 325 / yr
    'SubGRB': 390 / (3600 * 24 * 365)  # 390 / yr
}
...
Fermi+Swift+SVOM), accounting for multiple detections of the same
astrophysical event. For more details on methodology, see:
https://dcc.ligo.org/T1900297 and https://dcc.ligo.org/LIGO-T2400246."""
