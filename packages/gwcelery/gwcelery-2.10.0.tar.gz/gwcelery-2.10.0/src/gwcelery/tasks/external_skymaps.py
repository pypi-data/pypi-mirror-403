...
def create_external_skymap(ra, dec, error, pipeline, notice_type=111):
    ...
    if pipeline == 'Fermi' and notice_type is not None:
        ...
        # Use generic cone method for Swift, etc.
        skymap = bayestar_adaptive_grid(from_cone, ra, dec, error, rounds=8)

    return skymap


def create_offset_external_skymap(ra, dec, error, pipeline, notice_type=111):
    """Creates an offset sky map by generating a sky map centered on the
    injected site and drawing on it to find a new center to generate the final
    sky map.
    ...
    """
    #  Create sky map from the initial center
    skymap = create_external_skymap(ra, dec, error, pipeline, notice_type)

    #  Get probabilities to draw new center from
    level, ipix = ah.uniq_to_level_ipix(skymap['UNIQ'])
    nsides = ah.level_to_nside(level)
    areas = ah.nside_to_pixel_area(nsides)

    prob = skymap['PROBDENSITY'] * areas.value
    prob /= np.sum(prob)

    #  Draw from probability to get index of new center
    new_center = np.random.choice(np.arange(len(skymap)), p=prob)

    #  Find coordinates of final center
    new_ra, new_dec = ah.healpix_to_lonlat(ipix[new_center],
                                           nsides[new_center],
                                           order='nested')

    #  Create final sky map from coordinates
    return create_external_skymap(
        new_ra.to(u.deg).value, new_dec.to(u.deg).value, error,
        pipeline, notice_type)
