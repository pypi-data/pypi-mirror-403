...
def mock_get_superevent(superevent_id):
    if superevent_id == 'S1':
        ...
        old_time_far = 1e-4
        old_space_far = 1e-6
    elif superevent_id == 'S6':
        old_time_far = -1e-4
        old_space_far = -1e-6
    elif superevent_id == 'S7':
        old_time_far = 1e-4
        old_space_far = float('inf')
    else:
        old_time_far = 1e-5
        old_space_far = None
...
@pytest.mark.parametrize(
    ...
    [[1e-4, None, 'S1', 'GRB', True],
     ...
     # Check that ~zero temporal FAR is handled correctly
     [1e-1000, None, 'S2', 'GRB', True],
     # Check that ~zero spatial FAR is handled correctly
     [1e-100, 1e-1000, 'S4', 'GRB', True],
     # Check that positive values are preferred over negative
     [1e-3, 1e-5, 'S6', 'GRB', True],
     # Check that negative values are not preferred over positive
     [-1e-7, -1e-10, 'S4', 'GRB', False],
     # Check that infinite spatial FAR doesn't update spatial FAR
     [1e-5, float('inf'), 'S4', 'GRB', False],
     # Check that infinite spatial FAR does update temporal FAR
     [1e-5, float('inf'), 'S2', 'GRB', True],
     # Check that SubGRBTargeted doesn't overwrite infinite spatial FAR GRB
     [1e-5, 1e-8, 'S7', 'SubGRBTargeted', False],
...
def test_update_superevent(monkeypatch,
    ...
    if new_space_far is not None and new_space_far == float('inf'):
        new_space_far = sys.float_info.max
    if result:
...
def mock_get_event(graceid):
    if 'E' in graceid:
        ...
        return {'em_type': (graceid.replace('S', 'E') if graceid != 'S0'
                            else None),
                'time_coinc_far': 1e-20 if gracedb != 'S0' else None,
                'space_coinc_far': 1e-20 if gracedb != 'S0' else None,
...
@pytest.mark.parametrize(
    ...
    # New event should always update if no previous event
    [['S0', None, 'E1', True],
     ...
     ['S6', 'E6', 'E1', True],
     # Same event graceid should overrwrite previous info
     ['S1', 'E1', 'E1', True],
     # Same event graceid should be ignored if no new spatial joint FAR
     ['S7', 'E7', 'E7', False]]
)
...
def test_update_superevent_w_emtype(mock_update_superevent,
    ...
    if ext_event_id_old:
        ...
        ext_event_old = {'labels': []}
    ext_event_new = mock_get_event(ext_event_id_new)
    time_coinc_far = 1e-10 if ext_event_old['labels'] else 1e-20
    space_coinc_far = (1e-10 if ext_event_old['labels'] else None if
                       ext_event_id_old == 'E7' else 1e-30)
