...
def test_handle_filter_other_svom_notice_types(mock_create_event,
                                               mock_get_events):
    text = read_binary(data, 'svom_catalog_gcn.xml')
    external_triggers.handle_grb_gcn(payload=text)
...
@pytest.mark.parametrize('search', ['AllSky', 'SSM', 'EarlyWarning'])
@patch('gwcelery.tasks.gracedb.get_superevent',
    ...
    if search == 'AllSky':
        ...
        mock_raven_coincidence_search.assert_has_calls([
            call('S180616h', alert['object'], group='CBC',
                 searches=['GRB'], se_searches=[]),
            call('S180616h', alert['object'], group='CBC',
                 pipelines=['Fermi'], searches=['SubGRB']),
            call('S180616h', alert['object'], group='CBC',
                 pipelines=['Swift'], searches=['SubGRB'])])
...
@pytest.mark.parametrize('time_diff,expected_countdown',
                         [(5, 0),    # Data already available (>3 sec diff)
                          (2, 1),    # Need to wait 1 second (3-2=1)
                          (0, 3)])   # Need to wait full 3 seconds (3-0=3)
@patch('gwcelery.tasks.detchar.check_vectors.si')
@patch('astropy.time.Time.now')
def test_launch_external_detchar_timing(mock_time_now, mock_check_vectors,
                                        time_diff, expected_countdown):
    """Test that _launch_external_detchar calculates correct countdown
    based on data availability timing."""
    from unittest.mock import Mock

    # Mock event with trigger duration
    event = {
        'graceid': 'E1234',
        'gpstime': 1000.0,
        'pipeline': 'Fermi',
        'extra_attributes': {
            'GRB': {'trigger_duration': 4.0}
        }
    }

    # Set up mock times
    # Event starts at gpstime=1000, ends at 1004 (start + duration)
    # Current time is end + time_diff
    mock_current_time = Mock()
    mock_current_time.gps = 1004.0 + time_diff
    mock_time_now.return_value = mock_current_time

    # Mock the check_vectors task chain with delay method
    mock_task = Mock()
    mock_check_vectors.return_value = mock_task

    # Run the function
    result = external_triggers._launch_external_detchar(event)

    # Verify check_vectors was called with correct parameters
    mock_check_vectors.assert_called_once_with(
        event, 'E1234', 1000.0, 1004.0
    )

    # Verify delay was called with expected countdown
    mock_task.delay.assert_called_once_with(countdown=expected_countdown)

    # Verify function returns the event
    assert result == event
