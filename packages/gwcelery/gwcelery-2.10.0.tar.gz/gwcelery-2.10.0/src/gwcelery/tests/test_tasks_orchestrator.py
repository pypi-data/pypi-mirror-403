...
@pytest.mark.parametrize(  # noqa: F811
    'filenames',
    [['bayestar.multiorder.fits,0', None, None, None],
     ['bayestar.multiorder.fits,0', 'em_bright.json,0', 'p_astro.json,0',
      None],
     ['bayestar.multiorder.fits,0', 'em_bright.json,0', 'p_astro.json,0',
      'mchirp_source.json,0'],
     ['bayestar.multiorder.fits,0', 'em_bright.json,0', 'p_astro.json,0',
      'mchirp_source_PE.json,0']])
def test_handle_superevent_update_alert(monkeypatch, filenames):
    ...
    monkeypatch.setattr('gwcelery.tasks.gracedb.create_tag.run',
                        mock_create_tag)

    # Make copy that won't be modified by update_alert
    filenames_copy = copy.deepcopy(filenames)

    # Run function under test
    orchestrator.update_alert(filenames, 'S1000')

    mock_alert.assert_called_with(filenames_copy[:-1], superevent, 'update')
    mchirp_filename = filenames_copy[-1]
    if mchirp_filename is not None:
        mock_create_tag.assert_has_calls([
            call(mchirp_filename, 'public', 'S1000'),
            call(mchirp_filename.replace('json', 'png'), 'public', 'S1000')
        ])
