...
.. note::
   GWCelery uses HTCondor DAGMan to manage its worker processes. You may see a
   DAGMan job itself in the queue along with the individual worker jobs. The
   DAGMan job coordinates the submission and monitoring of all worker processes.
...
#. Resubmit the deployment that was running with::
   ...
   This regenerates the DAG files and submits them to HTCondor.
