...
-  **Dependencies should be installable by pip.** Dependencies of tasks should
   ...
   2.  We use `uv`_ to make the precise versions of packages reproducible
       in our deployment. If you make changes to requirements.txt, then run
       ``uv lock`` and commit the changes to `uv.lock`_.
...
.. _`uv.lock`: https://git.ligo.org/emfollow/gwcelery/blob/main/uv.lock
...
uv-managed virtual environment by running this command::
