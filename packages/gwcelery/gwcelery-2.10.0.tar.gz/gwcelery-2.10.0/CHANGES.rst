...
2.10.0 "Selkie" (2026-01-29)
----------------------------

-   Submit pipeline to HTCondor as a dag instead of using direct job
    submission.

-   Block coincidence searches between SubGRBTargeted external events and GW
    searches that don't trigger preliminary alerts, which currently includes
    SSM and EarlyWarning events. This is due to these events not triggering
    These targeted searches listen to preliminary alerts to look at their data
    and these GW searches are missing crucial values in the config.

-   Update ligo-raven to 3.3.

-   Update acceptance checks with current LLAI reviewers, a new tracked
    package, references to playground instead of test, and better organization.
    
-   Remove all references to oLIB.

-   Catch instances where spatio-temporal FAR is infinite and map these to a
    large number instead to allow compatibility with GraceDB.

-   Limit number of Bilby jobs running concurrently on playground.

-   Update playground configuration for O3 Replay Early Warning MDC.

-   Point dev deployment at gracedb-cit.

2.9.2 "Wendigo" (25-11-14)
--------------------------

-   Disable alerts for non-MDC events on production.

2.9.1 "Wendigo" (2025-10-29)
----------------------------

-   Set ``expose_to_public=False`` for the playground configuration.
