...
@pytest.mark.parametrize(
    "pipeline",
    ['gstlal', 'pycbc', 'mbta', 'spiir', 'aframe']
)
...
def test_handle_cbc_superevent(
        mock_upload, mock_download, mock_filelist, pipeline):
    alert = {
        "data": {
            "file_version": 0,
    ...
    if pipeline == 'aframe':
        alert['object']['preferred_event_data']['extra_attributes'] = {}
    gwskynet.handle_cbc_superevent(alert)
    if pipeline == 'aframe':
        mock_filelist.assert_not_called()
...
def test_handle_extreme_high_FAR_cbc_superevent(mock_upload, mock_download):
    alert = {
        ...
        "object": {
            "preferred_event_data": {
                "group": "CBC",
                "pipeline": "match-filter-cbc-pipeline",
                "extra_attributes": {
                    'SingleInspiral': [
                        {"ifo": "H1",
                         "snr": 6.6},
