...
@pytest.mark.parametrize(
    ...
    [['CBC', 'gstlal', 'AllSky', False, 1.e-10, 'H1',  # complete significant
     ...
     ['CBC', 'gstlal', 'SSM', False, 1.e-10, 'H1,L1,V1',  # significant SSM
        ['SKYMAP_READY'], False, False],
     ['CBC', 'aframe', 'AllSky', False, 1.e-10, 'H1,L1',  # complete
        ['PASTRO_READY', 'SKYMAP_READY', 'EMBRIGHT_READY'], True, True],
     ['CBC', 'aframe', 'AllSky', False, 1.e-10, 'H1,L1',  # incomplete
        ['PASTRO_READY', 'SKYMAP_READY'], False, False]])
...
def test_parse_trigger_cbc_6():
    """
    This test pertains to events from the aframe pipeline.
    A trigger G000000 is uploaded. If the preferred
    event is significant and from a match filtering search, the
    preferred event is unchanged, otherwise it is based on FAR.
    """
    event_dictionary = {'graceid': 'G000000',
                        'gpstime': 1163905224.4332082,
                        'group': 'CBC',
    ...
    superevent_response = patch(
        'gwcelery.tasks.gracedb.get_superevent',
        return_value=SUPEREVENTS_NEIGHBOURS['S0039'])
    # superevent_response contains a preferred event from gstlal
    # that is significant, hence the update_superevent is not called.
    with superevent_response, \
            patch('gwcelery.tasks.gracedb.add_event_to_superevent') as p1, \
            patch('gwcelery.tasks.gracedb.update_superevent') as p2, \
            patch('gwcelery.tasks.gracedb.get_superevents',
    ...
    # mock the superevent content to have a cwb-bbh as a preferred event.
    # In this case, the preferred event is updated based on FAR
    cwb_bbh_preferred_event_data = SUPEREVENTS_NEIGHBOURS[
        'S0039']['preferred_event_data'].copy()
    cwb_bbh_preferred_event_data.update(
        {
            'group': 'burst',
            'pipeline': 'cwb',
            'search': 'bbh',
    ...
    cwb_bbh_superevent_content = SUPEREVENTS_NEIGHBOURS['S0039'].copy()
    cwb_bbh_superevent_content.update(
        {'preferred_event_data': cwb_bbh_preferred_event_data})
    superevent_response = patch(
        'gwcelery.tasks.gracedb.get_superevent',
        return_value=cwb_bbh_superevent_content
    )
    with superevent_response, \
            patch('gwcelery.tasks.gracedb.add_event_to_superevent') as p1, \
            patch('gwcelery.tasks.gracedb.update_superevent') as p2, \
        ...
        p2.assert_called_once_with(
            'S0039',
            t_0=1163905224.4332082,
            preferred_event='G000000'
        )
        p3.assert_called_once()

    # mock superevent similar as above except the aframe event is less
    # significant compared to the cwb-bbh event
    cwb_bbh_superevent_content.update({'far': 1e-30})
    cwb_bbh_superevent_content['preferred_event_data'].update({'far': 1e-30})
    superevent_response = patch(
        'gwcelery.tasks.gracedb.get_superevent',
        return_value=cwb_bbh_superevent_content)
    with superevent_response, \
    ...
    # finally test the instance when an aframe event is the preferred
    # event, and is replaced by a more significant cwb-bbh upload.
    aframe_preferred_event_data = cwb_bbh_preferred_event_data.copy()
    aframe_preferred_event_data.update(
        {
            'far': 1e-10,
            'gpstime': 1163905224.44,  # match superevent t_0
            'group': 'cbc',
            'pipeline': 'aframe',
            'search': 'allsky',
            'extra_attributes': {}
    ...
    aframe_superevent_content = cwb_bbh_superevent_content.copy()
    aframe_superevent_content['far'] = 1e-10  # same as above since preferred
    aframe_superevent_content.update(
        {'preferred_event_data': aframe_preferred_event_data}
    )
    superevent_response = patch(
        'gwcelery.tasks.gracedb.get_superevent',
        return_value=aframe_superevent_content
    )
    event_dictionary.update(  # update so that new event is CWB-BBH
        {
            'group': 'burst',
            'pipeline': 'cwb',
            'search': 'bbh',
...
def test_parse_trigger_burst_2():
    """New MLy trigger G000006 with gpstime lying partially in
...
def test_parse_trigger_burst_3():
    """New MLy trigger G000007, not present among superevents
    New superevent created.
