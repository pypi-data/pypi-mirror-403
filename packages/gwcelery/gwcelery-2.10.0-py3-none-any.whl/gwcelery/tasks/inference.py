    ...
    if pe_pipeline == 'bilby':
        ...
        if (
            ...
            if number_of_bbh_running > 3:
                ...
                return
        if app.conf['gracedb_host'] == 'gracedb-playground.ligo.org':
            # FIXME Remove or comment out once EW MDC is complete.
            # Count the number of bilby jobs and do not start if more than 10
            # are currently running. This is for the Early Warning MDC, which
            # has an extremely high rate of high SNR BNS injections. Assume the
            # job is running if a data dump pickle file exists under the run
            # directory.
            number_of_bilby_jobs_running = len(glob.glob(os.path.join(
                        pipeline_dir, "*/*/data/*_generation_data_dump.pickle"
            )))

            if number_of_bilby_jobs_running > 10:
                gracedb.upload.delay(
                    filecontents=None, filename=None, graceid=superevent_id,
                    message='Parameter estimation will not start because '
                            'there are too many '
                            f'({number_of_bilby_jobs_running}) jobs running '
                            'concurrently.',
                    tags='pe'
                )
                return
