    ...
    *   If other type of IGWN alert, pass to _handle_skymaps to decide whether
    ...
    if alert['alert_type'] == 'new':
        if alert['object'].get('group') == 'External':
            ...
            preliminary_alert_dict = \
                app.conf['preliminary_alert_far_threshold'][gw_group.lower()]
            search_in_preliminary_alert = \
                search.lower() in {
                                   key for key in preliminary_alert_dict if
                                   preliminary_alert_dict[key] > 0
                }
            ...
            ext_searches = []
            if CBC_like:
                ext_searches.append('SubGRB')
            if search_in_preliminary_alert:
                ext_searches.append('SubGRBTargeted')

            # launch standard GRB search
            # Depending on the given superevent, this could be a CBC (AllSky,
            # SSM, EarlyWarning) or Burst (AllSky, BBH)
            raven.coincidence_search(graceid, alert['object'],
            ...
            if ext_searches:
                for pipeline in ['Fermi', 'Swift']:
                    raven.coincidence_search(
                        ...
                        searches=ext_searches, pipelines=[pipeline])
    else:
