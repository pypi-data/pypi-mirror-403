...
def update_coinc_far(coinc_far_dict, superevent, ext_event):
    ...
    * A lower temporal joint FAR is preferred over a higher temporal joint
    ...
    # If infinite spatial FAR, map to max allowable float
    # This will be effectively infinite, still be preferable over a
    # non-existant spatial FAR, and still allow plotting to occur
    spacetime_far = coinc_far_dict.get('spatiotemporal_coinc_far')
    if spacetime_far is not None and float(spacetime_far) == float('inf'):
        coinc_far_dict['spatiotemporal_coinc_far'] = sys.float_info.max
    ...
    if superevent_latest['em_type']:
        ...
        #  If same event as before, assume it is an update and prefer new data
        if ext_event['graceid'] == emtype_event['graceid'] and \
                coinc_far_dict['spatiotemporal_coinc_far'] is not None:
            preferred_event_far = (ext_event, coinc_far_dict)
        #  If different events, perform comparison to choose which to use
        else:
            events_fars = [(emtype_event, coinc_far_old),
                           (ext_event, coinc_far_dict)]

            preferred_event_far = max(events_fars, key=keyfunc)
    #  If no previous preferred external event, always update
    else:
        preferred_event_far = (ext_event, coinc_far_dict)
...
def keyfunc(event_far):
    ...
    try:
        spacetime_rank = \
            1 / spacetime_far if spacetime_far is not None else -float('inf')
    except ZeroDivisionError:
        spacetime_rank = float('inf')
    temporal_far = coinc_far.get('temporal_coinc_far')
    try:
        temporal_rank = \
            1 / temporal_far if temporal_far is not None else -float('inf')
    except ZeroDivisionError:
        temporal_rank = float('inf')
