...
class HLinkSlot:
    ...
    def url_to_clipboard(self, event):
        # ------------------------------------------------------------------------------------------
        self._w.clipboard_clear()
        self._w.clipboard_append(self.URL)
        self._w.update()
        print(f"Opentea copied the link '{self.URL}' in your clipboard.")
    ...
    def _parse_styles(self, tag, attrs):
        ...
        if HTML.Style.BACKGROUD_COLOR in attrs[HTML.Attrs.STYLE].keys():
            self._stack_add(
                tag,
                WCfg.BACKGROUND,
                attrs[HTML.Attrs.STYLE][HTML.Style.BACKGROUD_COLOR],
            )
    ...
    def _w_tags_apply_all(self):
        ...
        for key, tag in self._w_tags.items():
            ...
            if tag[Bind.KEY][Bind.LINK]:
                ...
                self._w.tag_bind(
                    key, "<Button-2>", self.hlink_slots[-1].url_to_clipboard
                )
