    ...
    if isinstance(schema, dict):
        if "type" in schema:
            if schema["type"] == "object":
                ...
                msgerr = (
                    "Dead end. type " + str(schema["type"]) + " not implemented... "
                )
...
def recursive_infer_oneof(schema, path, update_data=None, only_in_schema=True):
    ...
    if isinstance(update_data, dict):
        ...
        logger.warning(f"No data path {path} (OneOf), using default from schema")
...
def recursive_infer_properties(schema, path, update_data=None, only_in_schema=True):
    ...
    for key in schema:
        if isinstance(update_data, dict):
            ...
            logger.warning(f"No data path {path}/{key}, using default from schema")
            out[key] = recursive_infer(
                schema[key], [*path, key], only_in_schema=only_in_schema
            )
        else:
            raise RuntimeError(
                "Path " + str_address([*path, key]) + ": update_data is not a dict"
            )
