...
class BaseFixturesManager:
    ...
    def _load_fixtures_recursive(self, current_path: Path, fixtures: list[dict[str, Any]], order_shift: int, depth: int = 0, max_depth: int = 10) -> None:
        """
        Recursively traverse directory tree to find and load fixtures.

        Supports two patterns:
        1. Flat files (CSV/JSON list) in folders named after classes
        2. JSON files with explicit class specification

        Args:
            current_path: Current directory being processed
            fixtures: List to accumulate fixture data
            order_shift: Order offset for fixture precedence
        ...
        Returns:
            None
        """
    def _load_class_folder_fixtures(self, class_dir: Path, fixtures: list[dict[str, Any]]) -> None:
        """
        Load fixtures from a class-named directory (Pattern A).

        Args:
            class_dir: Directory named after a class (e.g., 'Company/')
            fixtures: List to append fixture data

        Returns:
            None
        """
    def _load_explicit_class_fixture(self, json_file: Path, fixtures: list[dict[str, Any]]) -> None:
        '''
        Load fixture from JSON with explicit class specification (Pattern B).

        Args:
            json_file: JSON file with format {"ClassName": [...]}
            fixtures: List to append fixture data
