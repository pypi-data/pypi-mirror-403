...
def slogdet(a):
    r""" Sign and logarithm of determinant of matrix ``a``.
...
def eigvalsh(a, eigvec=False):
    r""" Eigenvalues of Hermitian matrix ``a``.
...
def eigh(a, eigvec=True, rcond=None):
    r""" Eigenvalues and eigenvectors of symmetric matrix ``a``.
...
def svd(a, compute_uv=True, rcond=None):
    r""" svd decomposition of matrix ``a`` containing |GVar|\s.
...
def lstsq(a, b, rcond=None, weighted=False, extrainfo=False):
    r""" Least-squares solution ``x`` to ``a @ x = b`` for |GVar|\s.
...
def solve(a, b):
    r""" Find ``x`` such that ``a @ x = b`` for matrix ``a``.
