...
async def handle_read_resource(uri: str) -> str:
    ...
    try:
        resource_map = {
            "stocxer://portfolio/summary": "/trading/funds",
            "stocxer://positions/current": "/trading/positions",
            "stocxer://orders/today": "/trading/orders",
...
async def handle_call_tool(name: str, arguments: dict) -> list[TextContent]:
    ...
    try:
        if name == "get_positions":
            async with httpx.AsyncClient() as client:
                response = await client.get(f"{API_BASE_URL}/trading/positions", headers=headers, timeout=30.0)
                return [TextContent(type="text", text=response.text)]
        
        elif name == "get_orders":
            async with httpx.AsyncClient() as client:
                response = await client.get(f"{API_BASE_URL}/trading/orders", headers=headers, timeout=30.0)
                return [TextContent(type="text", text=response.text)]
        
        elif name == "get_portfolio_summary":
            async with httpx.AsyncClient() as client:
                response = await client.get(f"{API_BASE_URL}/trading/funds", headers=headers, timeout=30.0)
                return [TextContent(type="text", text=response.text)]
