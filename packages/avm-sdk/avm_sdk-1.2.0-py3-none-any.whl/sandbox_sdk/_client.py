...
class SandboxSDK(SyncAPIClient):
    ...
    def __init__(
        ...
        *,
        ...
        """Construct a new synchronous SandboxSDK client instance.

        This automatically infers the `api_key` argument from the `SANDBOX_SDK_API_KEY` environment variable if it is not provided.
        """
        if api_key is None:
            api_key = os.environ.get("SANDBOX_SDK_API_KEY")
        if api_key is None:
            raise SandboxSDKError(
                "The api_key client option must be set either by passing api_key to the client or by setting the SANDBOX_SDK_API_KEY environment variable"
            )
        self.api_key = api_key

        if base_url is None:
            base_url = os.environ.get("SANDBOX_SDK_BASE_URL")
        if base_url is None:
...
class AsyncSandboxSDK(AsyncAPIClient):
    ...
    def __init__(
        ...
        *,
        ...
        """Construct a new async AsyncSandboxSDK client instance.
