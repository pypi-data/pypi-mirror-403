...
_PERLMUTTER_HOSTNAME = "perlmutter.nersc.gov"
...
class PerlmutterCloud(BaseCloud):
    ...
    def __init__(self):
        ...
        # A mapping from user names to Perlmutter Clients.
        self._clients = collections.OrderedDict()
        # A mapping from cluster names to Perlmutter Cluster instances.
        self._clusters = collections.OrderedDict()

        # Check if any users have open SSH tunnels to Perlmutter.
        for user in SlurmClient.get_active_users(_PERLMUTTER_HOSTNAME):
    ...
    def _get_or_create_client(self, cluster_info: _ClusterInfo) -> SlurmClient:
        ...
        if cluster_info.user not in self._clients:
            self._clients[cluster_info.user] = SlurmClient(
                cluster_info.user, _PERLMUTTER_HOSTNAME, cluster_info.name
            )
        return self._clients[cluster_info.user]

    def _get_or_create_cluster(self, name: str) -> PerlmutterCluster:
        ...
        Returns:
            PerlmutterCluster: The cluster instance.
        """
        if name not in self._clusters:
            cluster_info = self._parse_cluster_name(name)
            self._clusters[name] = PerlmutterCluster(
                name, self._get_or_create_client(cluster_info)
            )
        return self._clusters[name]

    def initialize_clusters(self, user) -> list[BaseCluster]:
        """Initializes clusters for the specified user for all Perlmutter queues.

        Args:
            user: The user to initialize clusters for.

        Returns:
            List[PerlmutterCluster]: The list of initialized clusters.
        """
        clusters = []
        for q in sorted({q.value for q in PerlmutterCluster.SupportedQueues}):
            name = f"{q}.{user}"
...
@register_cloud_builder("perlmutter")
def perlmutter_cloud_builder() -> PerlmutterCloud:
    """Builds a PerlmutterCloud instance."""
    return PerlmutterCloud()
