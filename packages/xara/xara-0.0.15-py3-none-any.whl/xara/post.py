...
class NodalAverage(NodeField):
    def __init__(self, model, response: CellField, form=None, component=None, method=None):
        """
        ...
        if isinstance(response, str):
            self._resp_func = lambda elem: model.eleResponse(elem, response)
        elif callable(response):
            self._resp_func = response
        else:
        ...
        try: #if hasattr(model, "cells"):
            output = self._call_meshio(model)
        except: #else:
            output = self._call_xara(model)
    ...
    def __call__(self, node):
        ...
        elif self._component in s:
            return s[self._component]

        elif self._component.lower() in {"j2", "svm"}:
            if self._form == "frame":
...
def FiberStress(model,
    ...
    return NodalAverage(shape.model, f, component=stress, form="frame")


class NodalStress(NodalAverage):
    def __init__(self, model, component, method=None, form=None):
        import xara
