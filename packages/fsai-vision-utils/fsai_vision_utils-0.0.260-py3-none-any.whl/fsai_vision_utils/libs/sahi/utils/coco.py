...
class CocoAnnotation:
    ...
    def from_coco_annotation_dict(cls, annotation_dict: Dict, category_name: Optional[str] = None):
        ...
        keypoints = annotation_dict.get("keypoints", None)
        num_keypoints = annotation_dict.get("num_keypoints", None)
        ...
        if (
            ...
            return cls(
                ...
                bbox=annotation_dict.get("bbox"),
                category_id=annotation_dict["category_id"],
                category_name=category_name,
                image_id=annotation_dict.get("image_id", None),
    ...
    def from_shapely_annotation(
        ...
        slice_bbox: Optional[List[int]] = None,
    ):
        if slice_bbox and keypoints:
            slice_xmin, slice_ymin, slice_xmax, slice_ymax = slice_bbox
            shifted_keypoints = []
            for i in range(0, len(keypoints), 3):
                x, y, v = keypoints[i:i+3]
                if v > 0:
                    # Check if keypoint is inside slice bounds
                    if (slice_xmin <= x <= slice_xmax and 
                        slice_ymin <= y <= slice_ymax):
                        shifted_keypoints.extend([x - slice_xmin, y - slice_ymin, v])
                    else:
                        # Keypoint is outside slice bounds, set visibility to 0
                        shifted_keypoints.extend([0, 0, 0])
                else:
                    shifted_keypoints.extend([0, 0, 0])
    ...
    def get_sliced_coco_annotation(self, slice_bbox: List[int]):
        ...
        return CocoAnnotation.from_shapely_annotation(
            shapely_annotation=intersection_shapely_annotation,
    ...
    for coco_image in images:
        ...
        if ignore_negative_samples and num_annotations == 0:
            ...
            for coco_annotation in coco_annotations:
                ...
                if coco_annotation._keypoints is not None:
                    out_annotation["keypoints"] = coco_annotation._keypoints
                if coco_annotation._num_keypoints is not None:
                    out_annotation["num_keypoints"] = coco_annotation._num_keypoints
