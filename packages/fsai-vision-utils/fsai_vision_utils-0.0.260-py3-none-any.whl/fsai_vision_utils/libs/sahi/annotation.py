...
class Keypoint:
    ...
    def get_shifted_keypoint(self, shift_x: float, shift_y: float):
        return Keypoint(self.x + shift_x, self.y + shift_y, self.v)

    def __repr__(self):
        return f"Keypoint: <x: {self.x}, y: {self.y}, v: {self.v}>"


class Keypoints:
    def __init__(self, keypoints: List[float]):
        if len(keypoints) % 3 != 0:
            raise ValueError("Keypoints list must be a multiple of 3.")
        self.keypoints = [
            Keypoint(keypoints[i], keypoints[i + 1], keypoints[i + 2])
    ...
    def __repr__(self):
        return f"Keypoints: <{[(kp.x, kp.y, kp.v) for kp in self.keypoints]}>"

    @property
    def confidence(self):
        """Returns the average visibility of the keypoints, normalized to [0,1]."""
        if not self.keypoints:
            return None
        # Normalize visibility values to [0,1] before averaging
        normalized_visibilities = [min(max(kp.v, 0), 1) for kp in self.keypoints]
        return sum(normalized_visibilities) / len(normalized_visibilities)

    def get_shifted_keypoints(self, shift_x: float, shift_y: float):
        return Keypoints(
            [
                coord
                for kp in self.keypoints
                for coord in kp.get_shifted_keypoint(shift_x, shift_y).__dict__.values()
            ]
        )
...
class Mask:
    ...
    def get_shifted_mask(self):
        ...
        for s in self.segmentation:
            xs = [
                min(self.shift_x + s[i], self.full_shape_width)
                for i in range(0, len(s) - 1, 2)
            ]
            ys = [
                min(self.shift_y + s[i], self.full_shape_height)
