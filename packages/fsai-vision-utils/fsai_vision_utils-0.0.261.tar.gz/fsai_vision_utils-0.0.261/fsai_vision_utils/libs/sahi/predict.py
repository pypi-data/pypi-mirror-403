...
from fsai_vision_utils.libs.sahi.postprocess.combine import KeypointAwarePostprocess
from fsai_vision_utils.libs.sahi.prediction import ObjectPrediction, PredictionResult
...
def get_sliced_prediction(
    ...
    keypoint_merge_strategy: str = "WEIGHTED",
    verbose: int = 1,
    ...
    Args:
        ...
        keypoint_merge_strategy: str
            Strategy to use when merging keypoints from overlapping detections:
            - "SIMPLE": Simple averaging of all keypoints
            - "WEIGHTED": Weighted averaging based on detection scores and box overlap
            - "CLOSEST_TO_BOX": Use keypoints from the detection whose box is closest to the merged box
            Default is "WEIGHTED".
        verbose: int
            0: no print
    ...
    if postprocess_type == "KEYPOINT_AWARE":
        postprocess = KeypointAwarePostprocess(
            match_threshold=postprocess_match_threshold,
            match_metric=postprocess_match_metric,
            class_agnostic=postprocess_class_agnostic,
            postprocess_subtype="GREEDYNMM",  # Use GREEDYNMM as default for box merging
            keypoint_merge_strategy=keypoint_merge_strategy, 
            # disable_merging=True,
        )
    else:
        postprocess = POSTPROCESS_NAME_TO_CLASS[postprocess_type](
            match_threshold=postprocess_match_threshold,
            match_metric=postprocess_match_metric,
            class_agnostic=postprocess_class_agnostic,
        )
