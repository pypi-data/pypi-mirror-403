...
from fsai_vision_utils.libs.sahi.utils.coco import (
    Coco,
    CocoAnnotation,
    CocoImage,
    create_coco_dict,
...
def process_coco_annotations(
    ...
    """Slices and filters COCO annotations and adjusts keypoints to the slice.

    Args:
        coco_annotation_list (List[CocoAnnotation]): Original annotations.
        slice_bbox (List[int]): Slice bounding box [x_min, y_min, x_max, y_max].
        min_area_ratio (float): Threshold ratio to keep sliced annotation.

    Returns:
        List[CocoAnnotation]: List of valid, sliced annotations.
    """
    sliced_coco_annotation_list = []

    for coco_annotation in coco_annotation_list:
        if not annotation_inside_slice(coco_annotation.json, slice_bbox):
            continue

        sliced_annotation = coco_annotation.get_sliced_coco_annotation(slice_bbox)

        # Filter based on area ratio
        if sliced_annotation.area / coco_annotation.area >= min_area_ratio:
            sliced_coco_annotation_list.append(sliced_annotation)
...
def shift_masks(
    masks: np.ndarray, offset: Sequence[int], full_shape: Sequence[int]
) -> np.ndarray:
