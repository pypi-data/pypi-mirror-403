...
FlowAgent natively supports the Model Context Protocol (MCP) for tool extensibility.

## What is MCP?

MCP (Model Context Protocol) is an open protocol that enables AI models to securely access external tools and data sources. FlowAgent can connect to any MCP-compatible server.
...
```python
from flowagent import MCPClient
...
```python
from flowagent import StandardAgent, flowagent
...
```python
from flowagent import MCPProvider, ToolRegistry

# Create provider from config
provider = MCPProvider.from_config("flowagent.yaml")
...
result = await executor.run_with_tools(
    messages=[...],
    tool_names=["filesystem:read_file", "github:create_issue"]
)
```
...
```python
from flowagent import MCPError, MCPConnectionError, MCPToolError
