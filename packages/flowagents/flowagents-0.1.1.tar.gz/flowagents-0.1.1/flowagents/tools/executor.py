"""
FlowAgents Tool Executor - Execute tools and handle tool calling loop

Note:
    Approval logic is handled at the Agent/Orchestrator level, not here.
    Use `requires_approval` in agent config (flowagents.yaml) to require
...
class ToolExecutor:
    ...
    async def run_with_tools(
        ...
        for iteration in range(max_iterations):
            ...
            # Handle flowagent LLMResponse format
            if hasattr(response, 'content'):
                # flowagent LLMResponse
    ...
    def _parse_tool_call(self, tool_call) -> ToolCall:
        ...
        # Handle flowagent ToolCall format
        if isinstance(tool_call, ToolCall):
            return tool_call

        # Handle flowagent LLMResponse.tool_calls format
