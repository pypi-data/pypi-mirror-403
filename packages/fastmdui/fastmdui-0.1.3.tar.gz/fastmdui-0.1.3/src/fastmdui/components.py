from fasthtml.common import ft_hx, Script
from fasthtml.toaster import js
...
def Dialog(
    *items,
    ...
    description: str = "",
    open: bool = False,
    close_on_overlay_click: bool = False,
    close_on_esc: bool = False,
    fullscreen: bool = False,
    stacked_actions: bool = False,
    **kwargs):
    ...
    if open:
        attrs["open"] = open
    if close_on_esc:
        attrs["close-on-esc"] = close_on_esc
    if close_on_overlay_click:
        attrs["close-on-overlay-click"] = close_on_overlay_click
    if fullscreen:
        attrs["fullscreen"] = fullscreen
    if stacked_actions:
        attrs["stacked-actions"] = stacked_actions
    
    js_script = Script(
        f"""
        const dialog = document.querySelector(".{kwargs.get('cls')}");
    ...
    return _mdui_component("mdui-dialog", *items, **attrs), js_script
...
def Collapse(
    ...
    **kw):
    ...
    return _mdui_component("mdui-collapse", *items, **attrs)

def CollapseItem(*items, **kw):
    return _mdui_component("mdui-collapse-item", *items, **kw)
