...
from rakel.flow.flow_manager import FlowManager
from rakel.flow.mochi_flow import MochiFlow

class RakelClient:
    def __init__(self, node_path: str = "npx", core_script: str = None):
        # . check
    ...
    async def connect(self):
        # . action
        # Tenta rodar via npx tsx se for o script TS, ou node direto se for JS
        args = [self.node_path, "tsx", self.core_script]
        if self.core_script.endswith(".js") or self.core_script.endswith(".mjs"):
            args = ["node", self.core_script]
    ...
    async def _read_output(self):
        # . action
        asyncio.create_task(self._read_errors()) # Escuta erros também
        ...
        while True:
            ...
            try:
                # Se começar com { assume que é JSON-RPC
                if line_str.startswith("{"):
                    msg = json.loads(line_str)
                    await self._handle_bridge_message(msg)
                else:
                    # Se não for JSON, é o QR Code ou log do Node. Mostra para o usuário!
                    print(line_str)
    ...
    async def _read_errors(self):
        while True:
            ...
            if not line: break
            logger.error(f"🔴 Motor Node.js Error: {line.decode().strip()}")
