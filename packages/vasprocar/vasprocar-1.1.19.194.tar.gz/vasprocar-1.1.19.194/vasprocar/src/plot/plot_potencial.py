...
for l in range(1,(3+1)):
    ...
    if (l == 1):
        ...
        plt.plot([min(X), max(X)], [Efermi, Efermi], color = 'blue', linestyle = '--', linewidth = 1.0)   # Highlighting the Fermi energy 
    ...
    if (l == 2):
        ...
        plt.plot([min(Y), max(Y)], [Efermi, Efermi], color = 'blue', linestyle = '--', linewidth = 1.0)    # Highlighting the Fermi energy 
    ...
    if (l == 3):
        ...
        plt.plot([min(Z), max(Z)], [Efermi, Efermi], color = 'blue', linestyle = '--', linewidth = 1.0)   # Highlighting the Fermi energy  
    ...
    if (save_png == 1): plt.savefig(dir_output + name + '.png', dpi = 600, bbox_inches='tight', pad_inches = 0)
    if (save_pdf == 1): plt.savefig(dir_output + name + '.pdf', dpi = 600, bbox_inches='tight', pad_inches = 0)
    if (save_svg == 1): plt.savefig(dir_output + name + '.svg', dpi = 600, bbox_inches='tight', pad_inches = 0)
    if (save_eps == 1): plt.savefig(dir_output + name + '.eps', dpi = 600, bbox_inches='tight', pad_inches = 0)
