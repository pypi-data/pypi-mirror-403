...
class TestAttach:
    async def test_should_attach_handler(self, feature: Feature, handler: UnobservableCommand):
        # Attach
    ...
    async def test_should_ignore_disabled_handler(self, feature: Feature, handler: UnobservableCommand):
        # Attach
    ...
    async def test_should_call_enabled_callback(self, feature: Feature, handler: UnobservableCommand):
        # Attach
    ...
    async def test_should_attach_return_annotation(self, feature: Feature, handler: UnobservableCommand):
        # Create handler
        def function() -> int:
        ...
        # Assert that the method returns the correct value
        assert isinstance(handler._handler, sila.UnobservableCommand)
        assert handler._handler.responses == {
            "response_0": sila.Element(
                identifier="Response0", display_name="Response 0", description="", data_type=sila.Integer
            )
        }

    async def test_should_attach_missing_return_annotation(self, feature: Feature, handler: UnobservableCommand):
        # Create handler
        def function():
            return unittest.mock.sentinel.response
    ...
    async def test_should_attach_annotated_return(self, feature: Feature, handler: UnobservableCommand):
        # Create handler
        ...
        # Assert that the method returns the correct value
        assert isinstance(handler._handler, sila.UnobservableCommand)
        assert issubclass(handler._handler.responses["response_0"].data_type, sila.Constrained)
        assert handler._handler.responses["response_0"].data_type.data_type == sila.Integer
        assert handler._handler.responses["response_0"].data_type.constraints == [
            sila.Unit("s", [sila.UnitComponent("Second")])
        ]

    async def test_should_attach_documented_error(self, feature: Feature, handler: UnobservableCommand):
        # Create handler
        def function() -> int:
            """
    ...
    async def test_should_recognize_duplicate_errors(self, feature: Feature, handler: UnobservableCommand):
        # Create handler
        def function() -> int:
            """
            A function that returns an integer.
...
class TestExecute:
    ...
    async def test_execute_synchronous_default_parameters(self, feature: Feature, handler: UnobservableCommand):
        ...
        # Assert that the method returns the correct value
        assert result == {"response_0": unittest.mock.sentinel.response}
    ...
    # Execute synchronous function with no return value.
    async def test_execute_synchronous_returns_any(self, feature: Feature, handler: UnobservableCommand):
        # Initialize the unobservable command
        callback = unittest.mock.Mock()
    ...
    async def test_execute_asynchronous_returns_none(self, feature: Feature, handler: UnobservableCommand):
        ...
        async def function(argument: str) -> None:
            await callback(argument=argument)
    ...
    # Execute asynchronous function with no return value.
    async def test_execute_asynchronous_returns_any(self, feature: Feature, handler: UnobservableCommand):
        # Initialize the unobservable command
        callback = unittest.mock.AsyncMock()
