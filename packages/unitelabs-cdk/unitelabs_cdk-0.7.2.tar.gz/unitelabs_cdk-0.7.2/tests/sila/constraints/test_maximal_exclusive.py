...
from unitelabs.cdk.sila.constraints import MaximalExclusive
...
class TestValidate:
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(10, sila.data_types.Integer(9), False, id="valid-int"),
            pytest.param(10, sila.data_types.Integer(10), True, id="invalid-int"),
            pytest.param(10.5, sila.data_types.Real(10.4), False, id="valid-float"),
            pytest.param(10.5, sila.data_types.Real(10.5), True, id="invalid-float"),
            pytest.param(datetime.date(2023, 1, 1), sila.data_types.Date(2022, 12, 31), False, id="valid-date"),
            ...
            pytest.param(
                ...
                sila.data_types.Timestamp(2023, 1, 1, 11, 0, 0, 0),
                False,
                id="valid-timestamp",
            ),
            pytest.param(
    ...
    async def test_validation(self, value, validation, error):
        constraint = MaximalExclusive(value)
        if not error:
    ...
    async def test_should_raise_type_error_unsupported_type(self):
        with pytest.raises(TypeError):
            MaximalExclusive(unittest.mock.Mock())
