...
class TestAttach:
    ...
    async def test_should_attach_missing_return_annotation(self, feature: Feature, handler: ObservableCommand):
        ...
        assert isinstance(handler._handler, sila.ObservableCommand)
        assert handler._handler.responses == {
            "response_0": sila.Element(identifier="Response0", display_name="Response 0", description="", data_type=Any)
        }
    ...
    async def test_should_attach_documented_error(self, feature: Feature, handler: ObservableCommand):
        # Create handler
        def function() -> int:
            """
    ...
    async def test_should_recognize_duplicate_errors(self, feature: Feature, handler: ObservableCommand):
        # Create handler
        def function() -> int:
            """
            A function that returns an integer.
...
class TestExecute:
    ...
    # Execute synchronous function with no return value.
    async def test_execute_synchronous_returns_any(self, feature: Feature, handler: ObservableCommand):
        # Initialize the observable command
        callback = unittest.mock.Mock()
    ...
    # Execute asynchronous function with no return value.
    async def test_execute_asynchronous_returns_any(self, feature: Feature, handler: ObservableCommand):
        # Initialize the observable command
        callback = unittest.mock.AsyncMock()
