...
class ChannelAttention(nn.Module):
    ...
    def __init__(
        ...
        self.group_wm = (
            self.group_dim / self.base_group_dim
        )  # Width multiplier for each group.
...
class DaViT(nn.Module):
    ...
    Args:
        ...
        patch_prenorm (tuple(bool)): Use pre-normalization or not in different stages. \
        ...
        base_num_heads (tuple(int)): Number of attention heads in different layers (base case for muP). \
            Default: (4, 8, 12, 16)
        num_groups (tuple(int)): Number of groups in channel attention in different layers. Default: (3, 6, 12, 24)
        base_num_groups (tuple(int)): Number of groups in channel attention in different layers (base case for muP).\
            Default: (3, 6, 12, 24)
        ...
        standparam (bool): Use standard parametrization or mu-parametrization. Default: True\
            (i.e., use standard paramerization)
    """
    ...
    def __init__(
        ...
        if standparam:
            ...
            self.head = MuReadout(
                self.embed_dims[-1], num_classes, readout_zero_init=True
            )  # Follow examples/ResNet/resnet.py in muP.
    ...
    def _custom_init_weights(self, m):
        ...
        # These initializations will overwrite the existing
        # inializations from the modules and adjusted by set_base_shapes().
    ...
    def from_state_dict(self, pretrained_dict, pretrained_layers=[], verbose=True):
        ...
        def stripped_key(x):
            return x[14:] if x.startswith("image_encoder.") else x
    ...
    def forward_features_map(self, x):
        ...
        x = self.norms(x).reshape(x.shape[0], dim, dim, x.shape[2])
...
def create_mup_encoder(config_encoder):
    def gen_config(config, wm):
        ...
        for name in ["DIM_EMBED", "NUM_HEADS", "NUM_GROUPS"]:
            ...
            new_values = [
                round(base_value * wm) for base_value in config["SPEC"][base_name]
            ]  # New value = base value * width multiplier.
            logger.info(
                f'config["SPEC"]["{name}"]: {new_config["SPEC"][name]} -> {new_values}'
            )
...
def image_encoder(config_encoder, verbose, **kwargs):
    ...
    if config_encoder["LOAD_PRETRAINED"]:
        model.from_pretrained(
            config_encoder["PRETRAINED"], config_encoder["PRETRAINED_LAYERS"], verbose
        )
