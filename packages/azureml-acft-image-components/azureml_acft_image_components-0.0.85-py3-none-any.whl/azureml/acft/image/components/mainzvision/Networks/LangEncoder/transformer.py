...
from .multiheadatt_mup import MuPMultiheadAttention
...
class ResidualAttentionBlock(nn.Module):
    def __init__(
        ...
        if standparam:
            ...
            self.attn = MuPMultiheadAttention(
                d_model, base_d_model, n_head, base_n_head, standparam=standparam
            )
...
class Transformer(nn.Module):
    ...
    ):  # key_padding_token = 0 for clip tokenizer, 1 for xlm-roberta-base):
        super().__init__()
    ...
    def custom_init_weights(self, use_original_init=True):
        ...
        custom_trunc_normal_(self.positional_embedding, std=0.02)
        # normal_(self.token_embedding, std=.02)
        custom_trunc_normal_(self.token_embedding.weight, std=0.02)
    ...
    def _custom_init_weights(self, m):
        ...
        if isinstance(m, (nn.Linear, nn.Conv2d)):
            ...
            nn.init.constant_(m.bias, 0)
            # Note: Actually reset_parameters() in nn.LayerNorm and
            # nn.BatchNorm2d will initialize weights as 1 and bias as 0.
    ...
    def load_pretrained(self, pretrained="", pretrained_layers=[], verbose=True):
        if os.path.isfile(pretrained):
            ...
            def stripped_key(x):
                return x[13:] if x.startswith("lang_encoder.") else x
    ...
    def forward(self, input_ids, attention_mask=None):
        ...
        key_padding_mask = (
            (input_ids == self.key_padding_token) if not self.autogressive else None
        )
...
def create_mup_encoder(config_encoder, tokenizer):
    def gen_config(config, wm):
        ...
        for name in ["WIDTH", "HEADS"]:
            ...
            logger.info(f'config["{name}"]: {new_config[name]} -> {new_values}')
            new_config[name] = new_values
        return new_config
    ...
    base_model = create_encoder(base_config, tokenizer)

    logger.info("=> Create delta model")
    delta_config = gen_config(config_encoder, wm=2.0)
    delta_model = create_encoder(delta_config, tokenizer)
...
def lang_encoder(config_encoder, tokenizer, verbose, **kwargs):
    ...
    if standparam:
        ...
        model = create_mup_encoder(config_encoder, tokenizer)
