...
def gather_tensors(tensor):
    ...
    *** Warning ***: torch.distributed.all_gather has no gradient.
    """
    tensors_gather = [
        torch.ones_like(tensor) for _ in range(int(os.environ["WORLD_SIZE"]))
    ]
...
def download_file(url, filepath):
    ...
    with requests.get(url, stream=True, allow_redirects=True, timeout=60) as r:
        if r.status_code > 200:
            raise RuntimeError(
                f"Failed in downloading from {url}, status code {r.status_code}."
            )
...
class DistributionGridFactory:
    ...
    @classmethod
    def get_distribution_grid(
        cls, expert_parallel_group_size, expert_parallel_replica_group_size, ddp_type
    ):
        """
        ...
        Args:
            ...
            ddp_type: distributed data parallel type.
            "DDP" of the recipe, only allow ddp_type is "MAINZ", "OSS" or "ShardedDDP".
        ...
        assert ddp_type in ["MAINZ", "OSS", "SHARDEDDDP"], (
            f'DistributionGrid Factory only support "DDP" is "MAINZ",'
            f' "OSS" or "ShardedDDP".'
        ...
        if cached_distributed_grid is not None:
            ...
            distributed_grid = DistributionGrid(
                expert_parallel_group_size=expert_parallel_group_size,
                expert_parallel_replica_group_size=expert_parallel_replica_group_size,
            )

            cls.distribution_grid_cache[
                expert_parallel_group_size, expert_parallel_replica_group_size
            ] = distributed_grid
...
def save_result(result, filename):
    ...
    if isinstance(result, torch.Tensor) and result.ndim in [3, 4]:
        ...
        im = Image.fromarray(
            result.clamp_(0, 255).permute(1, 2, 0).to(torch.uint8).numpy()
        )
