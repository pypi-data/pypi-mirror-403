...
class TSVFile(object):
    ...
    def __del__(self):
        ...
        if self._fp:
            ...
            if (
                self._blob_storage
                and "azcopy" in self.tsv_file
                and os.path.exists(self.tsv_file)
            ):
                try:
...
class CompositeTSVFile:
    ...
    def initialize(self):
        ...
        if self.initialized:
            return
        self.tsvs = [
            TSVFile(op.join(self.root, f), class_selector=self._class_selector)
        ...
        if self._class_selector and all(
            [tsv.get_class_boundaries() for tsv in self.tsvs]
        ):
            """
            ...
            for tsv in self.tsvs:
                ...
                for bound in boundaries:
                    self._class_boundaries.append(
                        (bound[0] + offset, bound[1] + offset)
                    )
                offset += len(tsv)
