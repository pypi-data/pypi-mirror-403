...
class ZipData(data.Dataset):
    ...
    def __init__(
        self, path, map_file, transform=None, target_transform=None, extensions=None
    ):
        self._path = path
        if not extensions:
        ...
        with open(map_file, "r") as f:
            for line in iter(f.readline, ""):
                ...
                if not line:
                    continue
                cls_idx = [la for la in line.split("\t") if la]
                if not cls_idx:
                    continue
                if len(cls_idx) < 2:
                    cls_idx = [la for la in line.split(" ") if la]
                ...
                ), "class: {} idx: {} previously had idx: {}".format(cls, idx, prev_idx)
        ...
        assert len(self), "No images found in: {} with map: {}".format(
            self._path, map_file
        )
