...
class ClassAwareTargetSizeSampler(Sampler):
    def __init__(
        self, dataset, target_size, num_replicas=None, rank=None, shuffle=True
    ):
        ...
        ), "Dataset size %d is smaller than target size %d." % (
            len(dataset),
            target_size,
        )
        ...
        assert (
            len(self.class_boundaries) == self.num_classes
        ), "Number of classes and boundaries not consistent!"
        ...
        if target_size > 0:
            ...
            while self.target_size < target_size:
                ...
                self.target_size = sum(
                    [min(self.num_samples_cls, s) for s in class_sizes]
                )
        else:
            # Sample all available data
            self.target_size = len(dataset)
            self.num_samples_cls = max(
                [(high - low + 1) for low, high in self.class_boundaries]
            )
        ...
        logger.info(
            "[ClassAwareTargetSizeSampler] sampled data size: %d" % self.total_size
        )
...
class ClassAwareDistributedSampler(Sampler):
    ...
    def __init__(
        self, dataset, num_replicas=None, rank=None, shuffle=True, num_samples_cls=1000
    ):
        super().__init__(dataset)
        if num_replicas is None:
            if not dist.is_available():
        ...
        self.num_samples = int(
            math.ceil(self.num_samples_cls * self.num_classes * 1.0 / self.num_replicas)
        )
        self.total_size = self.num_samples * self.num_replicas
        self.shuffle = shuffle
    ...
    def __iter__(self):
        ...
        data_iter = [
            CycleIter(range(bound[0], bound[1]), shuffle=self.shuffle)
            for bound in self.class_boundaries
        ]
...
class ClassAwareAverageSampler(ClassAwareDistributedSampler):
    ...
    def __init__(self, dataset, num_replicas=None, rank=None, shuffle=True):
        if hasattr(dataset, "get_class_boundaries"):
            ...
            logger.info(
                "[ClassAwareAverageSampler] Average class size: %d", average_size
            )
        else:
            raise RuntimeError("Dataset does not provide the info of class boundary!")
