...
class MaskingScheme(object):
    ...
    def masking(self, x, np_random_state, **kwargs):
        raise NotImplementedError()

    def bert_masking(
        self, x, word_idx, mask_pos, masking_prob, random_token_prob, np_random_state
    ):
        """
        replacing each mask position to a special token MASK
        """
        masked_x, target = np.copy(x), np.copy(x)
        rand = np_random_state.rand(max(word_idx) + 1)
        ...
        for i in range(len(x)):
            ...
            if widx in mask_pos:
                ...
                # masking_prob + random_token_prob (Eg: 0.9)
                    ...
                    # sample random token from vocab, avoid special tokens
                    replace_idx = np_random_state.randint(0, len(self.tokenizer))
                    while replace_idx in self.special_symbols:
                        replace_idx = np_random_state.randint(0, len(self.tokenizer))
                    masked_x[i] = replace_idx
            else:
    ...
    def _get_spm_word_idx(self, x):
        ...
        if min(word_idx) < 0:
            logger.warning(
                f"[WARNING] no spm start in sample: {self.tokenizer.convert_ids_to_tokens(x)}, word_idx = {word_idx}"
            )
            word_idx += 1
...
class RandomTokenMasking(MaskingScheme):
    def __init__(
        ...
        super().__init__(tokenizer, mask_idx, bpe_cont_marker, bpe_end_marker)
    ...
    def masking(self, x, np_random_state, **kwargs):
        ...
        masking_prob = kwargs.get("masking_prob", self.masking_prob)
        random_token_prob = kwargs.get("random_token_prob", self.random_token_prob)

        x_len = len(x)
        x_len_no_eos = (x_len - 1) if (x[-1] == self.eos_idx) else x_len
        mask_num = math.ceil(x_len_no_eos * masking_ratio)
        mask_pos = np_random_state.choice(x_len_no_eos, mask_num, replace=False)
...
class RandomWordMasking(MaskingScheme):
    ...
    def masking(self, x, np_random_state, **kwargs):
        ...
        num_words_no_eos = (num_words - 1) if (x[-1] == self.eos_idx) else num_words

        mask_num = math.ceil(num_words_no_eos * masking_ratio)
        mask_pos = np_random_state.choice(num_words_no_eos, mask_num, replace=False)
...
class SpanTokenMasking(MaskingScheme):
    ...
    def masking(self, x, np_random_state, **kwargs):
        ...
        while len(mask_pos) < mask_num:
            span_len = min(np_random_state.poisson(lam=span_len_lambda), mask_num)
            start = np_random_state.choice(max(x_len_no_eos - span_len, 1))
...
class TextInfilling(MaskingScheme):
    ...
    def masking(self, x, np_random_state, **kwargs):
        ...
        while len(mask) < mask_num:
            span_len = min(np_random_state.poisson(lam=span_len_lambda), mask_num)
            start = np_random_state.choice(max(num_words_no_eos - span_len, 1))
            if start in mask:
                continue
            spans.append([start, start])
