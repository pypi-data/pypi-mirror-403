...
def load_opt_command(args):
    parser = argparse.ArgumentParser(
        description="MainzTrain: Pretrain or fine-tune models for NLP tasks."
    )
    ...
    parser.add_argument(
        "--config_overrides",
        nargs="*",
        help='''Override parameters on config with a json style string, e.g.
        {"<PARAM_NAME_1>": <PARAM_VALUE_1>, "<PARAM_GROUP_2>.<PARAM_SUBGROUP_2>.<PARAM_2>": <PARAM_VALUE_2>}.
        A key with "." updates the object in the corresponding nested dict. Remember to escape " in command line.''',
    )
