...
class AttentionDataStruct(nn.Module):
    ...
    def reset(self, flags, task, extra):
        ...
        for key, values in self.p_duplication.items():
            for name in values:
                self.duplication_dict["{}_{}".format(key, name)] = self.p_duplication[
                    key
                ][name]
        ...
        if self.task_switch["spatial"] and (
            "memories_spatial" in self.flags and self.flags["memories_spatial"] == True
        ):
            self.spatial_memory["prev_batch_mask"] = extra["prev_mask"]

        if self.task_switch["grounding"] and (
            "grounding" in self.flags and self.flags["grounding"] == True
        ):
            self.output["predictions_caption"] = []

        if self.task_switch["spatial"] and (
            "spatial" in self.flags and self.flags["spatial"] == True
        ):
            ...
            self.output["predictions_mask"] = (
                []
                if "predictions_mask" not in self.output
                else self.output["predictions_mask"]
            )
            self.output["predictions_class"] = (
                []
                if "predictions_class" not in self.output
                else self.output["predictions_class"]
            )
            self.output["predictions_caption"] = (
                []
                if "predictions_caption" not in self.output
                else self.output["predictions_caption"]
            )

        # initialize cross_attn, whether the variable is used in cross attention
        for key, values in self.p_cross_attn.items():
            for name in values:
                self.cross_attn_dict["{}_{}".format(key, name)] = self.p_cross_attn[
                    key
                ][name]
        ...
        for key, values in self.p_self_attn.items():
            for name in values:
                self.self_attn_dict["{}_{}".format(key, name)] = self.p_self_attn[key][
                    name
                ]
    ...
    def set(self, name, _type, output=None, pos=None, var=None, sample_size=None):
        if var is not None:
            ...
            ), "Duplication variable {} is not initialized yet.".format(name)
    ...
    def set_results(self, results):
        for name in self.cross_attn_name:
            self.attn_variables[name].attn_mask = results["attn_mask"][
    ...
    def cross_attn_variables(
        self,
    ):
        cross_attn_name = [
            key
            for key, value in self.cross_attn_dict.items()
            if (value == True)
            and (key in self.attn_variables)
            and (
                (key not in self.flags)
                or (key in self.flags and self.flags[key] == True)
            )
        ]
        self.cross_attn_name = cross_attn_name

        output = torch.cat(
            [self.attn_variables[name].output for name in cross_attn_name]
        )
        pos_emb = torch.cat([self.attn_variables[name].pos for name in cross_attn_name])

        index = 0
        for name in cross_attn_name:
            self.query_index[name] = [
                index,
    ...
    def cross_attn_mask(self, size, num_heads):
        ...
        if "memories_spatial" in self.cross_attn_name:
            ...
            probs = torch.tensor([1.0 / repeat for i in range(c)])
    ...
    def self_attn(self, bs, num_heads):
        self_attn_name = [
            key
            for key, value in self.self_attn_dict.items()
            if len(value) > 0
            and key in self.attn_variables
            and (
                (key not in self.flags)
        ...
        output = torch.cat(
            [self.attn_variables[name].output for name in self_attn_name]
        )
        ...
        self_attn_mask = torch.ones(
            (bs, output.shape[0], output.shape[0]),
            dtype=torch.bool,
        ...
        for key in self.masking:
            if key in self_attn_name:
                self_attn_mask[
                    ...
                    self.query_index[key][0] : self.query_index[key][1],
                ][self.attn_variables[key].masking] = True
                self_attn_mask[
                    :,
                    self.query_index[key][0] : self.query_index[key][1],
                    self.query_index[key][0] : self.query_index[key][1],
        ...
        for key1, key2 in self_attn_pair:
            ...
            if key1 in self.masking:
                self_attn_mask[
                    ...
                    self.attn_variables[key1].masking
                ] = True  # HACK, not verified
        ...
        if "queries_spatial" in self_attn_name and "tokens_spatial" in self_attn_name:
            diag_mask = ~(
                torch.eye(self.extra["spatial_query_number"])
                .repeat_interleave(self.extra["sample_size"], dim=0)
                .repeat_interleave(self.extra["sample_size"], dim=1)
            ).bool()
            self_attn_mask[
                :,
                self.query_index["queries_spatial"][0] : self.query_index[
                    "queries_spatial"
                ][1],
                self.query_index["queries_spatial"][0] : self.query_index[
                    "queries_spatial"
            ...
            diag_index = torch.arange(
                self.extra["spatial_query_number"], device=indices.device
            ).repeat_interleave(self.extra["sample_size"], dim=0)[None, :, None]
            ...
            self_attn_mask[
                ...
                self.query_index["tokens_spatial"][0] : self.query_index[
                    "tokens_spatial"
                ][1],
            ] = diag_mask
        ...
        if "memory_indices" in self.extra:
            ...
            diag_index = torch.arange(
                self.extra["spatial_query_number"], device=memory_indices.device
            ).repeat_interleave(self.extra["sample_size"], dim=0)[None, :, None]
