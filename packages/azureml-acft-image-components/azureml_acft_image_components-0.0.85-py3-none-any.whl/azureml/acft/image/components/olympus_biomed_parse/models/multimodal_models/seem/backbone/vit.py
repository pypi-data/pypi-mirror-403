...
class SimpleFPN(nn.Module):
    def __init__(self, in_dim=768, out_dims=[128, 256, 512, 1024]):
        ...
        self.down_16 = nn.Sequential(
            nn.Conv2d(in_dim, out_dims[2], 1), nn.GroupNorm(1, out_dims[2]), nn.GELU()
        )
...
class D2ViT(ImageEncoderViT, Backbone):
    def __init__(
        self, size: str = "base", out_features: List = ["res2", "res3", "res4", "res5"]
    ) -> None:
        if size == "base":
            encoder_depth = 12
    ...
    def load_weights(self, pretrained_dict=None, pretrained_layers=[], verbose=True):
        ...
        pretrained_dict = (
            pretrained_dict["model"] if "model" in pretrained_dict else pretrained_dict
        )
        pretrained_dict = {
            k.replace("image_encoder.", ""): v for k, v in pretrained_dict.items()
        }
        pretrained_dict = {
...
#         vit.load_weights(
#             ckpt,
#             cfg["MODEL"]["BACKBONE"]["VIT"].get("PRETRAINED_LAYERS", ["*"]),
#             cfg["VERBOSE"],
#         )
