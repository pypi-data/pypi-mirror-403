...
from ....external.sam2.modeling.position_encoding import (
    PositionEmbeddingRandom,
)

from ....external.sam2.modeling.sam2_utils import (
    LayerNorm2d,
)
...
class PromptEncoder(nn.Module):
    def __init__(
        ...
        # mask_mid_chans can't be 0
        mask_mid_chans = max(mask_in_chans // 4, 1)

        self.mask_downscaling = nn.Sequential(
            nn.Conv2d(1, mask_mid_chans, kernel_size=2, stride=2),
            LayerNorm2d(mask_mid_chans),
            activation(),
            nn.Conv2d(mask_mid_chans, mask_in_chans, kernel_size=2, stride=2),
...
class TextPromptEncoder(PromptEncoder):
    ...
    def forward(
        ...
        if tokens is not None:
            encoder_hidden_states = self.text_encoder(tokens)[self.hidden_state_key]
