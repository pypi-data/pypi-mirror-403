...
class SaveMLflowModelCallback(TrainerCallback):
    ...
    def on_train_end(self, args: TrainingArguments, state: TrainerState, control: TrainerControl, **kwargs):
        ...
        model, _ = kwargs["model"], kwargs["tokenizer"]
        ...
        if state.is_world_process_zero:
            ...
            if isinstance(model, AzuremlStableDiffusionPipeline):
                ...
                raise LLMException._with_error(
                    AzureMLError.create(LLMInternalError, error=(f"Got unexpected model - {model}"))
                )
            ...
            mlflow.hftransformers.save_model(
                diffusion_model, self.mlflow_model_save_path, hf_conf=misc_conf, mlflow_model=mlflow_model
            )  # type: ignore
