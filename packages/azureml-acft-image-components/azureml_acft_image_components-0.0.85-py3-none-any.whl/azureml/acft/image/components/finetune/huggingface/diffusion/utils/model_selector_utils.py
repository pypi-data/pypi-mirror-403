...
def validate_diffusion_text_to_image_model_name(model_name: str) -> None:
    """Validate diffusion text to image model name"""
    if model_name not in STABLE_DIFFUSION_SUPPORTED_MODELS:
        raise ValidationException._with_error(AzureMLError.create(ModelNotSupported, ModelName=model_name))
...
def get_model_name_from_pytorch_model(model_path: str) -> str:
    ...
    try:
        ...
        raise LLMException._with_error(
            AzureMLError.create(LLMInternalError, error=(f"Failed to load {finetune_args_file}\n" f"{e}"))
        )
    ...
    if finetune_args and "model_name" in finetune_args:
        ...
        raise LLMException._with_error(
            AzureMLError.create(
                LLMInternalError,
                error=(f"model_name is missing in " f"{SaveFileConstants.FINETUNE_ARGS_SAVE_PATH} file"),
            )
        )
...
def get_model_name_from_mlflow_model(model_path: str) -> str:
    ...
    try:
        ...
        raise LLMException._with_error(
            AzureMLError.create(LLMInternalError, error=(f"Failed to load {mlflow_config_file}\n" f"{e}"))
        )
    ...
    try:
        ...
        raise LLMException._with_error(
            AzureMLError.create(LLMInternalError, error=("{Invalid mlflow config file}\n" f"{e}"))
        )
...
def convert_mlflow_model_to_pytorch_model(mlflow_model_path: Union[str, Path], download_dir: Path):
    ...
    try:
        ...
        shutil.copytree(Path(mlflow_model_path, "data/model"), download_dir, dirs_exist_ok=True)
        # copy LICENSE file
        ...
        raise LLMException._with_error(
            AzureMLError.create(LLMInternalError, error=("Failed to convert mlflow model to pytorch model.\n" f"{e}"))
        )
...
def model_selector(model_selector_args: Dict[str, Any]):
    ...
    if pytorch_model_path is not None:
        ...
        try:
            ...
            shutil.rmtree(download_dir, ignore_errors=True)
            raise LLMException._with_error(
                AzureMLError.create(LLMInternalError, error=("shutil copy failed.\n" f"{e}"))
            )
        ...
        raise LLMException._with_error(
            AzureMLError.create(
                LLMInternalError,
                error=(
                    "Please provide `huggingface_id` or pass valid model to `pytorch_model_path` or"
                    " `mlflow_model_path`"
                ),
            )
        )
