...
from post_for_me import PostForMe, AsyncPostForMe, DefaultAioHttpClient
from post_for_me._utils import is_dict
...
logging.getLogger("post_for_me").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[PostForMe]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with PostForMe(base_url=base_url, api_key=api_key, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncPostForMe]:
