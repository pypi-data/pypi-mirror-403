...
class SiteParser:
    ...
    async def parser(
        ...
        if sitemap:
            logger.info(f"🗺️ Используется sitemap: {sitemap}")
            urls = search_sitemap(
                url=sitemap,
                regex=sitemap_regex,
                limit=sitemap_limit,
            ...
            if not urls:
                logger.warning("⚠️ Sitemap не вернул ссылок")
        ...
        logger.info(f"⚙️ Обработка {total} ссылок (одновременно: {concurrency})")
        ...
        async def process_link(idx: int, link: str):
            ...
            async with semaphore:
                ...
                if not raw_text:
                    ...
                    logger.warning(f"⚠️ {position} ({current_percent}%) Пропущено: не удалось получить текст")
                    return link, ""
                ...
                if cleaned:
                    ...
                    logger.warning(f"⚠️ {position} ({current_percent}%) Пустой результат")
        ...
        if to_files:
            ...
            logger.info(f"📁 Сохраняю файлы в: {output_dir}")
        ...
        logger.info(
            f"🏁 Готово! Успешно: {success_count}/{total} ({int(success_count/total*100)}%) | "
            f"Время: {elapsed_time:.2f} сек"
        )
