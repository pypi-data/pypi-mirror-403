...
from .schemas import DialogCalAct, DialogCalendarCallback, highlight, superscript
...
class DialogCalendar(GenericCalendar):
    ...
    async def start_calendar(self, year: int = datetime.now().year, month: Optional[int] = None) -> InlineKeyboardMarkup:
        today = datetime.now()
        now_year = today.year
