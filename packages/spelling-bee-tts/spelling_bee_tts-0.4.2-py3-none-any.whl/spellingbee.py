...
class SpellingBeeApp(Gtk.Application):
    def __init__(self):
        ...
        self.voice_menu_button = None
        self.voice_popover = None
        self.voice_menu_listbox = None
    ...
    def do_activate(self):
        ...
        self.voice_popover = Gtk.Popover()
        self.voice_popover.set_position(Gtk.PositionType.RIGHT)
        self.voice_menu_listbox = Gtk.ListBox()
        self.voice_menu_listbox.set_selection_mode(Gtk.SelectionMode.NONE)
        self.voice_popover.set_child(self.voice_menu_listbox)
        self.refresh_voice_menu()
        self.voice_menu_button = Gtk.MenuButton()
        self.voice_menu_button.set_tooltip_text("Voice settings")
        self.voice_menu_button.set_popover(self.voice_popover)
        config_icon = Gtk.Image.new_from_icon_name("emblem-system-symbolic")
        self.voice_menu_button.set_child(config_icon)
        ...
        self.button_row.append(self.voice_menu_button)
    ...
    def get_voice_options(self, current_voice):
        options = [
        ...
        voice_ids = [voice_id for _label, voice_id in options]
        if current_voice and current_voice not in voice_ids:
            options.append((f"{current_voice} (custom)", current_voice))
        return options

    def refresh_voice_menu(self):
        if not self.voice_menu_listbox:
            return
        options = self.get_voice_options(self.edge_voice)
        child = self.voice_menu_listbox.get_first_child()
        while child:
            self.voice_menu_listbox.remove(child)
            child = self.voice_menu_listbox.get_first_child()
        for label, voice_id in options:
            if voice_id == self.edge_voice:
                label = f"{label} (current)"
            row = Gtk.ListBoxRow()
            button = Gtk.Button(label=label)
            button.set_hexpand(True)
            button.set_halign(Gtk.Align.FILL)
            button.connect("clicked", self.on_voice_menu_clicked, voice_id)
            row.set_child(button)
            self.voice_menu_listbox.append(row)

    def apply_saved_voice(self):
        if not self.settings:
            return
        saved_voice = self.settings.get("edge_voice")
        if saved_voice:
            self.edge_voice = saved_voice
        self.refresh_voice_menu()

    def on_voice_menu_clicked(self, _button, voice_id):
        if voice_id == self.edge_voice:
            return
        ...
        self.refresh_voice_menu()
        if self.voice_popover:
            self.voice_popover.popdown()
