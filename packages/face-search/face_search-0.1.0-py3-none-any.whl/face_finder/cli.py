...
from face_finder.detectors import DetectorFactory
...
def search(ref, threshold, top, output, preserve_structure, output_file, index):
    ...
    try:
        ...
        click.echo("   Run 'face-finder build' first.\n", err=True)
        sys.exit(1)
    except Exception as e:
        click.echo(f"❌ Error: {e}", err=True)
        sys.exit(1)
...
def sources(index):
    ...
    try:
        ...
        if not sources:
            ...
            click.echo("   Run 'face-finder build <dir>' to index with tracking.\n")
            return
...
def remove(source_path, index, yes):
    ...
    try:
        ...
        if source_path not in sources:
            ...
            click.echo("   Use 'face-finder sources' to list indexed sources.\n", err=True)
            sys.exit(1)
...
def gui(port, share):
    """Launch web GUI."""
    try:
        from face_finder.gui import launch_gui
        ...
        click.echo("   pip install face-finder[gui]", err=True)
        sys.exit(1)
