...
class OzoneMoveObjectsOperator(BaseOperator):
    ...
    the directory structure when moving files. Uses native S3 copy_object for
    efficient server-side copying with automatic fallback to download/upload
    for edge cases (empty files, unsupported configurations).
    ...
    :param overwrite: If True, delete existing destination objects before copying (default: True).
                      Important for Ozone FS mode where directories cannot be overwritten with files.
    ...
    def _copy_directory(self, client, _source_bucket: str, _source_key: str,
                        dest_bucket: str, dest_key: str) -> bool:
        """
        Create a directory in destination in Ozone FS mode.
        In Ozone FS, directories are marker objects with empty body and specific ContentType.
        We create them explicitly rather than copying to ensure proper directory semantics.
        """
        try:
            self.log.info(
                f"Creating directory: {dest_bucket}/{dest_key}"
            )

            # In Ozone FS, directories are empty objects with ContentType application/x-directory
            # Creating explicitly ensures proper directory marker rather than relying on copy
            client.put_object(
                Bucket=dest_bucket,
                Key=dest_key,
                Body=b"",  # Empty body for directory marker
                ContentType="application/x-directory",  # Explicit directory ContentType
            )
    ...
    def _get_content_type_from_extension(self, key: str) -> str:
        """
        Determine ContentType based on file extension.
        Returns a file-appropriate ContentType for Ozone FS.
        """
        import mimetypes
        content_type, _ = mimetypes.guess_type(key)
        if content_type:
            return content_type
        # Default to binary for unknown extensions - important: NOT directory type
        return "application/octet-stream"

    def _copy_file(self, client, source_bucket: str, source_key: str,
                   dest_bucket: str, dest_key: str) -> bool:
        """
        Copy a file from source to destination using native S3 copy_object.
        Uses server-side copy which is more efficient and preserves metadata properly.

        Falls back to download/upload approach if copy_object fails (e.g., for
        cross-region copies or Ozone configurations that don't fully support it).
        """
        try:
            # First, get object metadata to check for empty files
            try:
                head_response = client.head_object(
                    Bucket=source_bucket,
                    Key=source_key,
                )
                source_content_type = head_response.get("ContentType")
                source_content_length = head_response.get("ContentLength", 0)
                self.log.info(
                    f"Source metadata for {source_key}: "
                    f"ContentType={source_content_type}, ContentLength={source_content_length}"
                )
            except Exception as head_err:
                self.log.warning(f"Could not get head_object for {source_key}: {head_err}")
                source_content_type = None
                source_content_length = 0

            # Check if empty file - needs special handling in Ozone FS mode
            if source_content_length == 0:
                self.log.warning(
                    f"Empty file detected: {source_key}. "
                    f"In Ozone FS mode, empty files become directories. "
                    f"Using download/upload with placeholder content."
                )
                return self._copy_file_with_upload(
                    client, source_bucket, source_key, dest_bucket, dest_key
                )

            # Use native copy_object for server-side copy (more efficient)
            copy_source = {"Bucket": source_bucket, "Key": source_key}

            # Determine ContentType to use
            content_type = source_content_type
            if not content_type or content_type == "application/x-directory":
                content_type = self._get_content_type_from_extension(source_key)
                self.log.info(f"Using guessed ContentType: {content_type} for {dest_key}")

            try:
                self.log.info(
                    f"Using copy_object: {source_bucket}/{source_key} -> {dest_bucket}/{dest_key}"
                )
                client.copy_object(
                    ...
                    MetadataDirective="REPLACE",  # Use our ContentType instead of source
                )
                self.log.info(f"Successfully copied file with ContentType: {content_type}")
                return True
            except Exception as copy_err:
                self.log.warning(
                    f"copy_object failed for {source_key}: {copy_err}. "
                    f"Falling back to download/upload approach."
                )
                return self._copy_file_with_upload(
                    client, source_bucket, source_key, dest_bucket, dest_key
                )
    ...
    def _copy_file_with_upload(self, client, source_bucket: str, source_key: str,
                                dest_bucket: str, dest_key: str) -> bool:
        """
        Fallback method: Copy a file using download/upload approach.
        Used when copy_object is not available or for empty files that need placeholder content.
        """
        try:
            ...
            # OZONE FS WORKAROUND: Empty files (0 bytes) become directories in Ozone FS mode
            if len(file_content) == 0:
                self.log.warning(
                    f"Adding placeholder content to empty file: {source_key}"
                )
                file_content = b"\n"
            ...
            self.log.info(
                f"Uploading file ({len(file_content)} bytes) with ContentType: {content_type}"
            )
            ...
            self.log.error(f"Failed to upload file {source_key}: {str(e)}")
            return False
