...
class OzoneMoveObjectsOperator(BaseOperator):
    ...
    def _ensure_directory_exists(self, client, bucket: str, directory_key: str) -> None:
        """
        Ensure a directory exists in Ozone FS mode by creating a directory marker.
        """
        if not directory_key.endswith("/"):
        ...
        try:
            ...
            self.log.debug(f"Created directory marker: {bucket}/{directory_key}")
        except Exception as e:
            # Directory might already exist, which is fine
            self.log.debug(f"Directory marker creation result for {directory_key}: {str(e)}")
    ...
    def execute(self, context: Context) -> dict[str, Any]:
        ...
        # Filter out directory markers - we only want actual files
        file_keys = [key for key in source_keys if not self._is_directory_marker(key)]
        self.log.info(f"Found {len(file_keys)} files to move (excluding directory markers)")

        # Collect all unique parent directories that need to be created
        directories_to_create = set()
        for source_key in file_keys:
            dest_key = self._get_dest_key(source_key)
            parent_dirs = self._get_parent_directories(dest_key)
            directories_to_create.update(parent_dirs)

        # Create all parent directories first (sorted by depth to create parents before children)
        sorted_dirs = sorted(directories_to_create, key=lambda x: x.count("/"))
        self.log.info(f"Creating {len(sorted_dirs)} directory markers in destination...")
        for dir_key in sorted_dirs:
            self._ensure_directory_exists(client, self.dest_bucket, dir_key)
