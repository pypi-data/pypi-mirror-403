...
class OzoneMoveObjectsOperator(BaseOperator):
    ...
    def _copy_file(self, client, source_bucket: str, source_key: str,
        ...
        IMPORTANT: In Ozone FS mode, empty files (0 bytes) are treated as directories.
        This method handles empty files by adding a minimal placeholder content to
        ensure they remain as files in the destination.
        """
        try:
            # First, get object metadata using head_object for more reliable metadata
            try:
            ...
            # Add minimal placeholder content to preserve file type
            if len(file_content) == 0:
                self.log.warning(
                    f"Empty file detected: {source_key}. "
                    f"In Ozone FS mode, empty files become directories. "
                    f"Adding placeholder content to preserve file type."
                )
                # Add a single newline as placeholder - minimal impact on file
                file_content = b"\n"

            # Determine ContentType - priority:
            # 1. From head_object
            # 2. From get_object response
            # 3. Guess from file extension
            content_type = source_content_type
            if not content_type:
                content_type = response.get("ContentType")
            if not content_type or content_type == "application/x-directory":
                # If it's marked as directory but we know it's a file, override
            ...
            put_params = {
                ...
                "Key": dest_key,
                "Body": file_content,
                "ContentType": content_type,  # Always set ContentType for files
            }

            self.log.info(f"Uploading file ({len(file_content)} bytes) with ContentType: {content_type}")
