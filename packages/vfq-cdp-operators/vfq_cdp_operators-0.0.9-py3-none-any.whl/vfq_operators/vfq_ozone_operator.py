...
class OzoneMoveObjectsOperator(BaseOperator):
    ...
    def _copy_directory(self, client, source_bucket: str, source_key: str,
        ...
        Uses native copy_object for server-side copy with metadata preservation.
        """
        try:
            copy_source = {"Bucket": source_bucket, "Key": source_key}

            self.log.info(
                f"Using copy_object for directory: {source_bucket}/{source_key} -> {dest_bucket}/{dest_key}"
            )

            # Use copy_object to preserve all metadata including ContentType
            client.copy_object(
                CopySource=copy_source,
                Bucket=dest_bucket,
                Key=dest_key,
                MetadataDirective="COPY",  # Preserve source metadata for directories
            )
            self.log.info(f"Created directory: {dest_bucket}/{dest_key}")
            return True
        except Exception as copy_err:
            # Fallback to download/upload if copy_object fails
            self.log.warning(
                f"copy_object failed for directory {source_key}: {copy_err}. "
            ...
            try:
                ...
                self.log.info(f"Created directory (fallback): {dest_bucket}/{dest_key}")
                return True
