...
CoinsCounter: TypeAlias = tuple[int, ...]


@lru_cache(1024)
def coins_counter(
    amount: int, denominations: Sequence[int], denominations_count: int, /
) -> CoinsCounter:
    if not amount:
        return _zeros(len(denominations))
    if denominations_count == 1:
        return _one_coin_counter(amount, denominations[0]) + _zeros(
            len(denominations) - 1
        )
    ...
    for denomination_index, denomination in enumerate(
        ...
        if nearest_denomination_index < len(denominations) and (
            denominations[nearest_denomination_index] == difference
        ):
            return (
                (
                    (
                ...
                if nearest_denomination_index == denomination_index
                    ...
                    (
                        *_zeros(denomination_index),
                        1,
                        *_zeros(
                            nearest_denomination_index - denomination_index - 1
                        ),
                        1,
                        *_zeros(
                            len(denominations) - nearest_denomination_index - 1
                        ),
                    )
                )
            )
    ...
    def key(counter: CoinsCounter, /) -> tuple[int, int]:
        return (
            sum(
                count * denomination
                for count, denomination in zip(
                    counter, denominations, strict=False
                )
                if count
            ),
            sum(counter),
    ...
    return min(
        _coins_counters(amount, denominations, denominations_count), key=key
    )
...
def _coins_counters(
    ...
    if denominations_count == 1:
        yield (
            _one_coin_counter(amount, denominations[0])
        ...
        max_last_denomination_count, remainder = divmod(
            amount, last_denomination
        )
        has_remainder = bool(remainder)
        if has_remainder:
            for last_denomination_count in range(
                max_last_denomination_count, 0, -1
            ):
                counter = coins_counter(
                    remainder, denominations, last_denomination_index
                )
                yield (
                    *counter[:last_denomination_index],
                    counter[last_denomination_index] + last_denomination_count,
        ...
        yield from _coins_counters(
            amount, denominations, last_denomination_index
        )
...
_zeros: Callable[[int], CoinsCounter] = (0,).__mul__
