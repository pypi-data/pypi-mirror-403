...
from payi import Payi, AsyncPayi, DefaultAioHttpClient
from payi._utils import is_dict
...
logging.getLogger("payi").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[Payi]:
...
@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncPayi]:
