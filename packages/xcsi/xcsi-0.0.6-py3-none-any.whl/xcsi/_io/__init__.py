...
def csiread(file_name, fmt=None):
    from xcsi.csi import load
    if fmt == "xlsx" or file_name.endswith(".xlsx"):
        return load(file_name, fmt="xls")
    ...
    try:
        ...
        raise Exception(f"Could not read file {file_name} with any known encoding.")



def _load_from_filelike(data, delimiter=None, comment=None, quote=None):
    pass 


def read(fname, encoding=None, **kwds):
    ...
    try:
        ...
        if isinstance(fname, str):
            fh = _open(fname, 'rt', encoding=encoding)
            if encoding is None:
...
def _open(path, mode='r', destpath=os.curdir, encoding=None, newline=None):
    """
