...
class LinearStaticAnalysis:
    ...
    def run(self, asm, verbose=False):
        """
...
class ModalAnalysis:
    ...
    def run(self, asm, verbose=True):
        ...
        self._apply_loads(asm)
        model = asm.model
        model.constraints("Transformation") #"Auto")
        model.system("BandGeneral")
        model.numberer("RCM")
        model.eval("number")

        ndf = asm.model.systemSize()
        print(f"Model DOF: {ndf}")
        if self.max_num_modes is None or self.max_num_modes > ndf:
            self.max_num_modes = ndf - 2
