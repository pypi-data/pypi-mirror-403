...
class _LinkMaterial04(_LinkMaterial):
# _LinkMaterial04 - Damper (Exponential)
    def create(self, instance):
        ...
        names  = instance.names
        for dof in ["U1", "U2", "U3", "R1", "R2", "R3"]:
            damper = find_row(csi["LINK PROPERTY DEFINITIONS 04 - DAMPER"],
                              Link=self.name, DOF=dof)
            if damper is None:
                continue
            stiff = damper["TransKE"]
...
#   LinkType                                    TableName
    ...
    "Damper - Exponential" : _LinkMaterial04, # "LINK PROPERTY DEFINITIONS 04 - DAMPER",
    "Gap"                  : _LinkMaterial05, # "LINK PROPERTY DEFINITIONS 05 - GAP",
    "Hook"                 : _LinkMaterialNA, # "LINK PROPERTY DEFINITIONS 06 - HOOK",
...
def _create_link_materials(instance):
    ...
    for link in csi.get("LINK PROPERTY ASSIGNMENTS",[]):
        if "LinkType" in link:
            ...
            props = find_row(csi.get("LINK PROPERTY DEFINITIONS 01 - GENERAL",[]), Link=link["LinkProp"])
            if props is None:
                raise ValueError(f"Link property {link['LinkProp']} not found for link {link['Link']}")
            LinkType = props["LinkType"]

        if LinkType not in _link_tables:
            instance._log(UnimplementedInstance(f"Link.Prop.LinkType={LinkType}", link))
            continue
        
        if link["Link"] in links:
            continue

        cls = _link_tables[LinkType](link, instance)
...
def _create_single_links(instance, link_props):
    ...
    for link in csi.get("CONNECTIVITY - LINK",[]):
        ...
        if not _is_type(link, assign, "SingleJoint"):
            continue
        ...
        if properties := find_row(csi.get("LINK PROPERTY DEFINITIONS 01 - GENERAL",[]), Link=assign["LinkProp"]):
            if "Mass" in properties and properties["Mass"] != 0:
                model.mass(nodes[0], position=properties["Mass"]/2.0)
...
def _is_type(link, assign, link_type):
    if "LinkJoints" in assign:
        return assign["LinkJoints"] == link_type
    if link["JointI"] == link["JointJ"]:
        return "SingleJoint" == link_type
    else:
        return "TwoJoint" == link_type
...
def _create_double_links(instance, link_props):
    ...
    for link in csi.get("CONNECTIVITY - LINK",[]):
        ...
        if not _is_type(link, assign, "TwoJoint"):
            continue
        ...
        if properties := find_row(csi.get("LINK PROPERTY DEFINITIONS 01 - GENERAL",[]), Link=assign["LinkProp"]):
            if "Mass" in properties and properties["Mass"] != 0:
                model.mass(nodes[0], position=properties["Mass"]/2.0)
                model.mass(nodes[1], position=properties["Mass"]/2.0)
        #
        # Get mats and dofs
        #
        if assign["LinkProp"] not in link_props:
            ...
            continue
        if isinstance(link_props[assign["LinkProp"]], _LinkMaterialNA):
            instance._log(UnimplementedInstance(f"Link.Prop.LinkProp={assign['LinkProp']}", assign))
            continue
