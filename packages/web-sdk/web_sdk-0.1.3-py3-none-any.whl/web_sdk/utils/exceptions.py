...
from web_sdk.settings import settings
...
class ExceptionModelMeta(ABCMeta):
    ...
    def __new__(
        ...
        cls.__template__ = mcs.localize(cls.__template__)
    ...
    def localize(value: str):
        """Localize message."""
        if not settings.localize or not settings.localize_function:
            return value

        return settings.localize_function(value)
