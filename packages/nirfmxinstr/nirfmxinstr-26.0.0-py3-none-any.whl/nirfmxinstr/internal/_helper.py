...
def build_port_string(selector_string, port_name, device_name, channel_number):
    ...
    if not device_name:
        if not stripped_port_name:
            result_port_part = ""
        else:
            result_port_part = port_name_prefix + stripped_port_name
    else:
        composite_device = (
            device_name if ("/" in device_name) else f"{device_name}/{channel_number}"
        )
        if not stripped_port_name:
            port_suffix = ""
        else:
            port_suffix = f"/{stripped_port_name}"
        result_port_part = f"{port_name_prefix}{composite_device}{port_suffix}"

    if selector_string:
        if not result_port_part:
            return selector_string
        else:
            return f"{selector_string}/{result_port_part}"
    else:
        return result_port_part
