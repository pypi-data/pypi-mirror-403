#!/usr/bin/env python3
"""
云服务器 Dify MCP Server（多厂商）
提供4个Dify工作流调用工具，支持腾讯云、阿里云、华为云等多厂商云服务器：
1. cvm_price_query - 云服务器机型价格查询（多厂商）
2. cvm_purchase - 云服务器购买（多厂商）
...
def cvm_price_query(lan: str) -> str:
    ...
    功能说明：
    - 支持腾讯云CVM、阿里云ECS、华为云等多家云厂商的云服务器机型价格查询
    - 支持自然语言查询，如"查询阿里云ecs.g9ae.large价格"、"腾讯云S5.MEDIUM4在广州多少钱"、"北京地区8核16G的机型多少钱"等
    - 可以查询按量付费、包年包月等不同计费模式的价格
    - 支持查询不同地域、不同规格、不同厂商的机型价格对比
    
    使用场景：
    - 当用户需要了解某云厂商机型的价格时
    - 当用户需要对比不同厂商、机型或地域的价格时
    - 当用户进行成本预估或选型时
    
    :param lan: 自然语言查询内容，可指定厂商和机型，描述你想查询的价格信息
    :return: 查询结果，包含机型、地域、价格等详细信息
    
    示例输入：
    - "查询阿里云ecs.g9ae.large的价格"
    - "查询腾讯云S5.MEDIUM4机型在广州的价格"
    - "8核16G的机型有哪些，分别多少钱"
    - "对比腾讯云SA2和阿里云同规格机型的价格"
    """
...
def cvm_purchase(lan: str) -> str:
    ...
    功能说明：
    - 支持腾讯云CVM、阿里云ECS、华为云等多厂商云服务器的购买操作
    - 支持自然语言描述购买需求，如"购买一台腾讯云S5.MEDIUM4"、"在阿里云创建一台ecs.g9ae.large"、"在北京地区创建一台8核16G的实例"等
    - 可以指定厂商、地域、可用区、机型、镜像、网络配置等参数
    - 支持按量付费和包年包月等计费模式
    
    使用场景：
    - 当用户需要创建新的云服务器实例时
    - 当用户需要批量购买云服务器时
    - 当用户描述了具体的厂商和配置需求需要下单时
    
    注意事项：
    - 购买操作会产生费用，请确认用户已明确表达购买意图
    - 建议在购买前先使用价格查询工具确认费用
    
    :param lan: 自然语言描述的购买需求，可指定厂商，包含机型、地域、数量等信息
    :return: 购买操作结果，包含实例ID等信息
    
    示例输入：
    - "在腾讯云广州地区购买一台S5.MEDIUM4机型，系统盘50G"
    - "在阿里云创建一台ecs.g9ae.large实例"
...
def automation_test(lan: str) -> str:
    ...
    功能说明：
    - 执行腾讯云、阿里云、华为云等云服务器实例相关的自动化测试任务
    - 支持自然语言描述测试需求，如"测试S5机型的网络性能"、"对阿里云实例执行磁盘IO压测"等
    ...
    :param lan: 自然语言描述的测试需求，可指定厂商，包含测试对象、测试类型、测试参数等
    :return: 测试执行结果，包含测试报告、性能数据等信息
    
    示例输入：
    - "对实例i-xxx执行CPU压力测试"
    - "测试广州地区S5机型的网络延迟"
    - "对阿里云实例执行磁盘读写性能测试，持续5分钟"
    - "验证实例的内存是否正常"
    """
...
def shutdown_monitor(lan: str) -> str:
    """
    云服务器自动化关停机监控工作流（支持多厂商）
    
    功能说明：
    - 监控和管理腾讯云、阿里云、华为云等云服务器实例的关停机状态
    ...
    :param lan: 自然语言描述的监控需求，可指定厂商，包含监控对象、监控类型、告警设置等
