...
def _render_table_html(
    ...
    Args:
        ...
        type (str): The type of this GNNTable: node or edge to be concatenated with the tables name.
    ...
    table_name = f"{table.name} ({type})"
    table_str = format_name(table_name, format_table_name)
    ...
    html += "".join(f'<TR><TD ALIGN="LEFT" PORT="{col.name}">{format_col_str(col)}</TD></TR>' for col in table.columns)
        ...
        with engine.connect() as connection:
            indexes = {
                ...
                for key, value in connection.execute(
                    text(f"SELECT indexname, indexdef FROM pg_indexes WHERE tablename = '{table.name}'")
                )
            }
...
def create_schema_graph(
    ...
    if format_schema_name is not None and len(set(format_schema_name.keys()).difference(_accepted_keys)) > 0:
    ...
    if format_table_name is not None and len(set(format_table_name.keys()).difference(_accepted_keys)) > 0:
        ...
        graph.add_node(
            pydot.Node(
                ...
                label=_render_table_html(
                    table=table,
                    type=metadata.table_types[table.name],
                    engine=engine,
