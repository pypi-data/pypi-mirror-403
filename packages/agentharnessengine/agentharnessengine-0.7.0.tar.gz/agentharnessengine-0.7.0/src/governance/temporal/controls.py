...
These consume BehaviorBudget outputs produced by Governance Kernel.
They must not influence kernel state, failure, recovery, or governance.

Governance Kernel regulates *permission to act*.
Control primitives decide *how actions are executed* downstream.

These classes are NOT part of the core governance loop. They are utilities
for downstream systems that consume kernel output.
"""
