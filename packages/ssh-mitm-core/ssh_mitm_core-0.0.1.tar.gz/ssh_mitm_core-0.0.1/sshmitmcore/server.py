...
from sshmitmcore import __version__ as ssh_mitm_version
from sshmitmcore import project_metadata
from sshmitmcore.authentication import Authenticator, AuthenticatorRemote
from sshmitmcore.exceptions import KeyGenerationError, SessionStartError
from sshmitmcore.forwarders.scp import SCPBaseForwarder, SCPForwarder
from sshmitmcore.forwarders.sftp import SFTPHandlerBasePlugin, SFTPHandlerPlugin
from sshmitmcore.forwarders.ssh import SSHBaseForwarder, SSHForwarder
from sshmitmcore.forwarders.tunnel import (
    LocalPortForwardingForwarder,
    RemotePortForwardingForwarder,
)
from sshmitmcore.interfaces.server import BaseServerInterface, ServerInterface
from sshmitmcore.interfaces.sftp import (
    BaseSFTPServerInterface,
    SFTPProxyServerInterface,
)
from sshmitmcore.logger import Colors
from sshmitmcore.multisocket import create_server_sock
from sshmitmcore.session import Session
from sshmitmcore.sshkeys import SSHPubKey
...
class SSHProxyServer:
    ...
    def log_interfaces(self) -> None:
        """
        Logs all interface values, each in its own line, within a single debug call.
        """
        interfaces = {
            "ssh_interface": self.ssh_interface,
            "scp_interface": self.scp_interface,
            "sftp_handler": self.sftp_handler,
            "sftp_interface": self.sftp_interface,
            "server_tunnel_interface": self.server_tunnel_interface,
            "client_tunnel_interface": self.client_tunnel_interface,
            "authentication_interface": self.authentication_interface,
            "authenticator_class": self.authenticator_class,
            "session_class": self.session_class,
        }

        # Erstelle eine mehrzeilige Nachricht
        log_message = "\n".join(
            [
                f"{name}: {interface.__name__ if interface and hasattr(interface, '__name__') else interface}"
                for name, interface in interfaces.items()
            ]
        )

        # Logge alle Interfaces in einer mehrzeiligen Nachricht
        logging.debug(f"Interfaces:\n{log_message}")
