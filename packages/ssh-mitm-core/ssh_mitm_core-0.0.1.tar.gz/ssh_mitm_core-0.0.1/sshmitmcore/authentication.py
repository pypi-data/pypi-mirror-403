...
from sshmitmcore.clients.ssh import AuthenticationMethod, SSHClient
from sshmitmcore.exceptions import MissingHostException
from sshmitmcore.forwarders.agent import AgentProxy
...
class Authenticator(BaseModule):
    ...
    def __init__(self, session: "sshmitmcore.session.Session") -> None:
        """
        Initializes Authenticator instance.

        This class pass the authentication from the client to the server.

        :param session: an object of sshmitmcore.session.Session class to store session information.
        """
        super().__init__()
