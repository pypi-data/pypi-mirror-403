...
if TYPE_CHECKING:
    from sshmitmcore.moduleparser.modules import BaseModule
...
class ModuleError(BaseModuleError):
    ...
    def __str__(self) -> str:
        ...
        moduleclass_name = (
            self.moduleclass.__name__
            if isinstance(self.moduleclass, type)
            else (
                ", ".join(cls.__name__ for cls in self.moduleclass)
                if self.moduleclass
                else "Unknown"
            )
        )
        baseclass_name = (
            ...
            if isinstance(self.baseclass, type)
            else (
                ", ".join(cls.__name__ for cls in self.baseclass)
                if self.baseclass
                else "Unknown"
            )
        )
        return (
            f"ModuleError: {self.message}\n"
            f"  Module class: {moduleclass_name}\n"
            f"  Base class: {baseclass_name}"
        )
