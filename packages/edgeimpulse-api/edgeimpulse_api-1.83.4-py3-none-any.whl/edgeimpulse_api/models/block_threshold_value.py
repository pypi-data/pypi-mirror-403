...
BLOCKTHRESHOLDVALUE_ONE_OF_SCHEMAS = ["float", "str"]

class BlockThresholdValue(BaseModel):
    # data type: float
    oneof_schema_1_validator: Optional[float] = None
    # data type: str
    oneof_schema_2_validator: Optional[StrictStr] = None
    actual_instance: Any
    one_of_schemas: List[str] = Field(BLOCKTHRESHOLDVALUE_ONE_OF_SCHEMAS, const=True)
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into BlockThresholdValue with oneOf schemas: float, str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into BlockThresholdValue with oneOf schemas: float, str. Details: " + ", ".join(error_messages))
        else:
            return v

    @classmethod
    def from_dict(cls, obj: dict) -> BlockThresholdValue:
        return cls.from_json(json.dumps(obj))

    @classmethod
    def from_json(cls, json_str: str) -> BlockThresholdValue:
        """Returns the object represented by the json string"""
        instance = cls()
        error_messages = []
        match = 0

        if "BlockThresholdValue" == 'TestPretrainedModelRequestModelInfoInput':
        ...
        elif "BlockThresholdValue" == 'DeployPretrainedModelRequestModelInfoModel':
        ...
        elif "BlockThresholdValue" == 'DeployPretrainedModelRequestModelInfoInput':
            temp = DeployPretrainedModelInputOther.from_json(json_str)
            if temp.input_type == 'time-series':
                instance.actual_instance = DeployPretrainedModelInputTimeSeries.from_json(json_str)
                match = 1
