...
def export_stp(doc, out_path: str) -> None:
    """
    导出 STP/STEP 格式。
    调用底层 NCTI 命令：cmd_ncti_export_file
    参数 2 通常代表 STEP 格式 (根据 import 命令推断)
    """
    os.makedirs(os.path.dirname(out_path) or ".", exist_ok=True)

    # 尝试调用底层命令导出
    # 注意：这里假设导出命令与导入命令(cmd_ncti_import_file)对应
    # 如果底层DLL命令不同，请查阅 ncti_command.dll 的文档
    ret = doc.RunCommand("cmd_ncti_export_file", out_path, 2)

    # 简单的成功检查（假设返回非0或True为成功，具体视接口定义而定）
    # 如果 RunCommand 没有返回值，可以检查文件是否生成
