...
INNERENTITYID_ANY_OF_SCHEMAS = ["int", "str"]

class InnerEntityId(BaseModel):
    """
    Internal entity identifier (normalized integer ID). This is the ID used internally for lookups and joins.
    """
    ...
    def actual_instance_must_validate_anyof(cls, v):
        ...
        instance = InnerEntityId.model_construct()
        error_messages = []
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in InnerEntityId with anyOf schemas: int, str. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into InnerEntityId with anyOf schemas: int, str. Details: " + ", ".join(error_messages))
        else:
            return instance
