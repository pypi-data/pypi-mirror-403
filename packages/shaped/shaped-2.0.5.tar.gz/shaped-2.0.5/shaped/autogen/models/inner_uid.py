...
INNERUID_ANY_OF_SCHEMAS = ["int", "str"]

class InnerUid(BaseModel):
    """
    Internal user ID if a user context was provided. This can be either an integer (for factorized stores) or a string (for no-op stores).
    """
    ...
    def actual_instance_must_validate_anyof(cls, v):
        ...
        instance = InnerUid.model_construct()
        error_messages = []
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in InnerUid with anyOf schemas: int, str. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into InnerUid with anyOf schemas: int, str. Details: " + ", ".join(error_messages))
        else:
            return instance
