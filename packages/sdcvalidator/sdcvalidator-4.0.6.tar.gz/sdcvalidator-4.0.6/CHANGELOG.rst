...
`v4.0.6`_ (2025-01-24)
======================
**Two-Tier Validation Strategy: Structural Rejection vs. Semantic Quarantine**

* **SECURITY FIX**: Implements Two-Tier Validation to separate structural rejection from semantic quarantine
* **Tier 1 (Structural)**: Unknown elements, wrong nesting, cardinality violations now cause HARD REJECT
* **Tier 2 (Semantic)**: Type errors, pattern violations, range constraints still use ExceptionalValue quarantine
* **NEW**: ``SDC4StructuralValidationError`` exception raised for Tier 1 structural violations
* **NEW**: ``ErrorMapper.is_structural_error()`` method to classify errors as structural vs semantic
* **NEW**: ``SDC4Validator.validate_structure()`` method for structural-only validation
* **Breaking Change**: Documents with unknown/unexpected elements are now rejected instead of quarantined

**Why this matters:**

Previously, if a user submitted a payload with undefined elements (e.g., ``<MaliciousTag>...</MaliciousTag>``),
the validator would attempt to quarantine it with ``NA`` (Not Applicable) ExceptionalValue. This created
a potential vulnerability for Mass Assignment or Injection attacks.

Now, structural violations are immediately rejected, while semantic violations (bad values in valid elements)
continue to use the quarantine-and-tag pattern.

**Structural Errors (Tier 1 - REJECT):**
...
**Semantic Errors (Tier 2 - QUARANTINE):**

* Type conversion errors (e.g., "abc" in integer field)
* Pattern violations
* Enumeration violations
* Range constraint violations (minInclusive, maxInclusive)

**New API:**

.. code-block:: python

    from sdcvalidator.sdc4 import SDC4Validator, SDC4StructuralValidationError

    validator = SDC4Validator('schema.xsd')

    # Check structure before recovery (optional)
    structural_errors = validator.validate_structure('input.xml')
    if structural_errors:
        print("Document has structural issues")

    try:
        # validate_with_recovery now rejects structural errors
        recovered = validator.validate_with_recovery('input.xml')
    except SDC4StructuralValidationError as e:
        print(f"Rejected: {e.error_count} structural violations")
...
.. _v4.0.6: https://github.com/Axius-SDC/sdcvalidator/compare/v4.0.5...v4.0.6
