...
环境变量配置：
- HSR_DIFY_API_KEY: Dify 工作流 API 密钥，需在 MCP 配置文件中设置
"""
...
# 从环境变量读取默认 API Key
DEFAULT_API_KEY = os.environ.get("HSR_DIFY_API_KEY", "")

# 全量测试用例列表（26个）
ALL_TEST_NAMES = [
    "alexnet_baseline",           # 深度学习模型性能测试
    "contextswitch_baseline",      # 上下文切换性能测试
    "core_latency_baseline",       # 核心延迟测试
    "cyclictest_baseline",         # 实时性延迟测试
    "ffmpeg_baseline",             # 视频编解码性能测试
    "gemm_baseline",               # 矩阵乘法运算测试
    "hackbench_baseline",          # 进程调度性能测试
    "hrtimer_baseline",            # 高精度定时器测试
    "ipi_baseline",                # 处理器间中断（IPI）测试
    "linpack_baseline",            # 线性代数计算测试
    "lmbench_baseline",            # 内存延迟基准测试
    "mlc_baseline",                # 内存延迟和带宽测试
    "openssl_baseline",            # 加密算法性能测试
    "compress_baseline",           # 文件压缩解压测试
    "passmark_baseline",           # 综合性能测试套件
    "perf_bench_baseline",         # 性能基准测试
    "redis_single_baseline",        # Redis 数据库性能测试
    "speccpu2017_baseline",        # CPU 性能基准测试
    "specjbb_baseline",            # Java 业务性能测试
    "stream_baseline",             # 内存带宽测试
    "stress_ng_baseline",          # 系统压力测试
    "super_pi_baseline",           # 浮点计算精度测试
    "tensorflowbenchmarks_baseline", # TensorFlow 性能测试
    "unixbench_total_baseline",    # Unix 系统综合性能测试
    "unixbench_baseline",          # Unix 基准测试
    "vray_baseline"                # 渲染性能测试
]
...
class ReportCompare:
    ...
    def filter_by_test_names(self, md_content: str, test_names: Optional[list] = None) -> str:
        """
        根据测试用例名称过滤 Markdown 表格
        :param md_content: 原始 Markdown 表格
        :param test_names: 测试用例名称列表，如果为 None 或空列表则不过滤
        :return: 过滤后的 Markdown 表格
        """
        if not test_names or len(test_names) == 0:
            return md_content
        
        # 转换为小写集合以便快速查找（不区分大小写）
        test_names_set = {name.strip().lower() for name in test_names if name.strip()}
        ...
        # 组合结果
        return "\n".join([header_line, separator_line] + filtered_rows)
    ...
    def compare_reports(
        ...
        :param test_names: 测试用例名称列表（可选），如果指定则只返回这些测试用例的数据
        ...
        # 3. 如果指定了测试用例，先过滤
        if test_names:
            md_content = self.filter_by_test_names(md_content, test_names)
            logger.info(f"已过滤测试用例: {test_names}")
...
def compare_cvm_reports(
    ...
    API 密钥自动从环境变量 HSR_DIFY_API_KEY 读取，无需手动传入。
    ...
    - test_names: (可选) 测试用例名称过滤，多个测试用例用英文逗号分隔。
                  如果指定，则只返回这些测试用例的数据；如果不指定或为空字符串，则返回所有测试用例的数据。
                  
                  输入格式：用英文逗号分隔的测试用例名称字符串
                  例如: "alexnet_baseline,lmbench_baseline,mlc_baseline"
                  
                  全量测试用例列表（26个）：
                  1. alexnet_baseline - 深度学习模型性能测试
                  2. contextswitch_baseline - 上下文切换性能测试
                  3. core_latency_baseline - 核心延迟测试
                  4. cyclictest_baseline - 实时性延迟测试
                  5. ffmpeg_baseline - 视频编解码性能测试
                  6. gemm_baseline - 矩阵乘法运算测试
                  7. hackbench_baseline - 进程调度性能测试
                  8. hrtimer_baseline - 高精度定时器测试
                  9. ipi_baseline - 处理器间中断（IPI）测试
                  10. linpack_baseline - 线性代数计算测试
                  11. lmbench_baseline - 内存延迟基准测试
                  12. mlc_baseline - 内存延迟和带宽测试
                  13. openssl_baseline - 加密算法性能测试
                  14. compress_baseline - 文件压缩解压测试
                  15. passmark_baseline - 综合性能测试套件
                  16. perf_bench_baseline - 性能基准测试
                  17. redis_single_baseline - Redis 数据库性能测试
                  18. speccpu2017_baseline - CPU 性能基准测试
                  19. specjbb_baseline - Java 业务性能测试
                  20. stream_baseline - 内存带宽测试
                  21. stress_ng_baseline - 系统压力测试
                  22. super_pi_baseline - 浮点计算精度测试
                  23. tensorflowbenchmarks_baseline - TensorFlow 性能测试
                  24. unixbench_total_baseline - Unix 系统综合性能测试
                  25. unixbench_baseline - Unix 基准测试
                  26. vray_baseline - 渲染性能测试
                  
                  注意：如果指定的测试用例在报告中不存在，将自动忽略，不会返回该测试用例的数据。
    ...
    result = compare_cvm_reports(
        ids_a="10001,10002",
        ids_b="20001,20002",
        test_names="alexnet_baseline,lmbench_baseline,mlc_baseline"
    )
    ```
    
    输出示例：
    ```
    | test_name | metric | metric_unit | Report 10001 | Report 20001 | 差异% (20001 vs 10001) |
    |---|---|---|---|---|---|
    ...
    3. 如果某个指标值为 null 或无法计算，差异列将显示 "N/A"
    4. test_names 参数是可选的，不指定时返回所有测试用例的数据
    
    Args:
        ids_a: 机型 A 的报告 ID 字符串，多个 ID 用逗号分隔
        ids_b: 机型 B 的报告 ID 字符串，多个 ID 用逗号分隔
        output_path: 输出文件路径（可选，默认为空不保存文件）
        test_names: 测试用例名称过滤（可选，多个用逗号分隔，默认为空返回所有）
    
    Returns:
        Markdown 格式的对比表格字符串，包含百分比差异列
    """
    if not DEFAULT_API_KEY:
        return "错误: 环境变量 HSR_DIFY_API_KEY 未设置，请在 MCP 配置文件中配置"
    
    try:
        # 解析 test_names 参数
        test_names_list = None
        if test_names and test_names.strip():
            test_names_list = [name.strip() for name in test_names.split(',') if name.strip()]
        
        comparer = ReportCompare(api_key=DEFAULT_API_KEY)
...
def get_single_report(
    ...
    ```
    # 返回所有测试用例
    result = get_single_report(report_ids="10001")
    
    # 只返回指定的测试用例
    result = get_single_report(
        report_ids="10001",
        test_names="alexnet_baseline,lmbench_baseline,mlc_baseline"
    )
    ```
    ...
    try:
        ...
        if test_names and test_names.strip():
            test_names_list = [name.strip() for name in test_names.split(',') if name.strip()]
            if test_names_list:
                md_content = comparer.filter_by_test_names(md_content, test_names_list)
                logger.info(f"已过滤测试用例: {test_names_list}")
