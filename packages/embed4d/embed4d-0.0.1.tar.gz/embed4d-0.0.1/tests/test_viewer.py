...
def test_notebbok_viewer_with_file_returns_html_object(tmp_path):
    """Test notebbok_viewer returns IPython.display.IFrame object with file."""
    glb_file = tmp_path / "test.glb"
    ...
    with patch.dict(
        ...
        from embed4d import notebbok_viewer

        result = notebbok_viewer(str(glb_file))
...
def test_notebbok_viewer_without_file_returns_html_object():
    """Test notebbok_viewer returns IPython.display.IFrame object without file."""
    mock_iframe = MagicMock()
    mock_iframe_class = MagicMock(return_value=mock_iframe)
...
def test_notebbok_viewer_with_custom_height(tmp_path):
    """Test notebbok_viewer with custom height parameter."""
    glb_file = tmp_path / "test.glb"
    glb_file.write_bytes(b"test")
    ...
    with patch.dict(
        ...
        result = notebbok_viewer(str(glb_file), height=800)
...
def test_notebbok_viewer_raises_import_error_without_ipython():
    """Test notebbok_viewer raises ImportError when IPython is not available."""
    ...
    try:
        ...
        with patch("builtins.__import__", side_effect=mock_import):
            ...
            from embed4d.viewer import notebbok_viewer
...
def test_notebbok_viewer_includes_base64_data(tmp_path):
    """Test that notebbok_viewer correctly embeds base64 data in HTML."""
    glb_file = tmp_path / "test.glb"
...
def test_notebbok_viewer_with_nonexistent_file(tmp_path):
    """Test notebbok_viewer handles nonexistent file gracefully."""
    ...
    with patch.dict(
        ...
        result = notebbok_viewer(str(nonexistent_file))
...
def test_notebbok_viewer_html_structure(tmp_path):
    """Test that notebbok_viewer generates valid HTML structure."""
    glb_file = tmp_path / "test.glb"
    glb_file.write_bytes(b"test")
