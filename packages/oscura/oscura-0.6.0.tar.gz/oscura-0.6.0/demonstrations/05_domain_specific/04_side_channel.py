...
# SKIP_VALIDATION: Side-channel analysis module removed in v0.6 refactoring
...
class SideChannelDemo(BaseDemo):
    ...
    def generate_test_data(self) -> dict:
        ...
        """
        # Reduced dataset for faster execution in validation
        num_traces = 50  # Number of power traces (increased for better statistical power)
        samples_per_trace = 400  # Samples per trace (reduced for speed)
        ...
        timing_traces = self._generate_timing_traces(
            num_traces=15,  # Reduced from 128 for faster validation
            key_byte=known_key_byte,
            sample_rate=sample_rate,
        )
    ...
    def _generate_dpa_traces(
        ...
        for plaintext in plaintexts:
            ...
            sbox_out = self.AES_SBOX[intermediate]

            # Power consumption model: Hamming weight of S-box output
            hamming_weight = bin(sbox_out).count("1")

            # Generate power trace (very low noise for validation)
            trace = np.random.normal(0.0, 0.01, samples_per_trace)

            # Add power consumption at point of interest (very strong signal for validation)
            trace[poi - 5 : poi + 5] += hamming_weight * 0.25
    ...
    def _generate_cpa_traces(
        ...
        for plaintext in plaintexts:
            ...
            # More realistic power trace (very low noise for validation)
            trace = np.random.normal(1.0, 0.05, samples_per_trace)

            # S-box operation (very strong leak for validation)
            trace[poi - 10 : poi + 10] += hamming_weight * 0.3

            # Add clock activity (reduced to minimize interference)
            clock_freq = 10e6  # 10 MHz
            t = np.arange(samples_per_trace) / sample_rate
            trace += 0.01 * np.sin(2 * np.pi * clock_freq * t)
    ...
    def _demonstrate_cpa(
        ...
        for key_guess in range(256):
            ...
            for sample_idx in range(trace_matrix.shape[1]):
                ...
                # Pearson correlation (handle NaN from zero variance)
                try:
                    corr_matrix = np.corrcoef(hypothetical_power, measured_power)
                    correlation = corr_matrix[0, 1]
                    if not np.isnan(correlation):
                        correlations[sample_idx] = abs(correlation)
                except Exception:
                    # Catch all exceptions (e.g., insufficient variation, computation errors)
                    correlations[sample_idx] = 0.0
