...
Auth0 does a nice job with Quick Start guides for each of their server-side SDKs. I especially like the option to integrate with my app (est. 15 min) or explore the sample app (est. 2 min).
...
Algolia provides guides for common search scenerios like including multiple result types
...
Contentful surfaces code samples for each of their SDKs inside their API reference docs. I especially like the deep linking that takes you directly to the code sample in the language you want.
...
We discussed annotating code in our section on code hinting. These annotation can be used to build SDK reference docs in the format for that language.
