...
Coding style varies from language to language. Keep that in mind when applying the naming conventions discussed in previous section. For example, if you choose to follow Google’s Java style guidelines, class names are PascalCase, methods are camelCase and constants are CONSTANT_CASE. Code style goes beyond naming conventions, to offer best practices around source file structure, braces, indentation, whitespace and much more.

There may be several code styles to choose from in each language. Checkout the coding standards and code style checkers we use at APIMatic. You’ll need to decide which one best represents your developers.
...
- Ruby Style Guide (which inspired both Shopify and AirBnB guides)
...
- Prettier
- JavaScript Standard Style (there is one for TypeScript too)
