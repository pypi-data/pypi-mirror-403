...
@click.option(
    ...
    help="Service account name for Basic Auth (env: SERVICE_ACCOUNT_NAME)",
)
@click.option(
    "--password",
    envvar="SERVICE_ACCOUNT_TOKEN",
    help="Service account token for Basic Auth (env: SERVICE_ACCOUNT_TOKEN)",
)
@click.option("-v", "--verbose", is_flag=True, help="Enable verbose output")
@click.version_option(version=__version__, prog_name="fdpbundle")
@click.pass_context
def main(ctx, api_url, username, password, verbose):
    ...
    if not username:
        username = os.getenv("SERVICE_ACCOUNT_NAME", "")
    if not password:
        password = os.getenv("SERVICE_ACCOUNT_TOKEN", "")
