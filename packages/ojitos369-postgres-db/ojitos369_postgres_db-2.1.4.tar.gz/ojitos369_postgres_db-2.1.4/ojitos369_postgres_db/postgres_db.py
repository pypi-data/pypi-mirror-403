...
class ConexionPostgreSQL:
    def __init__(self, db_data, **kwargs):
        ...
        self.closed = False
        self.mode = "pd"
        self.parameter_indicator = None
        for k in kwargs:
            setattr(self, k, kwargs[k])
        if self.parameter_indicator:
            p = re.escape(self.parameter_indicator)
            self._regex_param = re.compile(rf'(?<!{p}){p}([A-Za-z_][A-Za-z0-9_]*)')
    ...
    def _replace(self, query: str) -> str:
        if not self._regex_param:
            return query
        return self._regex_param.sub(lambda m: f"%({m.group(1)})s", query)

    def _prep(self, query, *args, **kwargs):
        q = self._replace(query)
        p = self._extract_params(args, kwargs)
        self.query = q
        self.params = p if p is not None else {}
        return q, p
    ...
    def consulta(self, query, params=None):
        query, params = self._prep(query, params)
        self.query = query
