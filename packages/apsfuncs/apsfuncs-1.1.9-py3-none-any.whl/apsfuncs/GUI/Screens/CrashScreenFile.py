...
from apsfuncs.Toolbox.db_connections import get_db_connection, get_user_program_ids, write_crash_report_entry
        ...
        if not self.bb.offline_mode:
            ...
            try:
                ...
                try:
                    ...
                    write_crash_report_entry(conn=conn, user_id=user_id, program_id=program_id, log_file_path=remote_dst_file, user_comments=self.user_comment_field.toPlainText(), version=app_version_data["Version"])

                except Exception as db_e:
                    # Database logging of the crash report failed; the crash log has already been
                    # successfully copied to the remote repository, however the report should still be stored locally, as this will trigger another entry attempt next time the program is run
