...
This guide helps resolve common cuDNN-related errors when running MurmurAI on GPU. These issues typically occur when the system can't locate cuDNN libraries or finds conflicting versions.

## Unable to Load cuDNN Libraries

If you encounter the following error when running MurmurAI:

`Unable to load any of {libcudnn_cnn.so.9.1.0, libcudnn_cnn.so.9.1, libcudnn_cnn.so.9, libcudnn_cnn.so}`

This means the cuDNN libraries are installed (via murmurai dependencies) but aren't in a location where the system's dynamic linker can find them.
...
**Note:** This error is unlikely on a clean install. If it occurs anyway, [open an issue](https://github.com/namastexlabs/murmurai/issues). If you've modified system libraries or CUDA/cuDNN, the options above should help resolve most cases.
