...
if TYPE_CHECKING:
    from .protocols import SyncProvider
...
class TeamManager:
    ...
    def __init__(
        ...
        Args:
            ...
            sync_provider: Optional sync provider for remote persistence
        """
    ...
    async def create_team(
        ...
        if self._provider and hasattr(self._provider, "create_team"):
            await self._provider.create_team(team_id, team.to_dict())
    ...
    async def get_team(self, team_id: str) -> Optional[Team]:
        ...
        # Try provider
        if self._provider and hasattr(self._provider, "get_team"):
            team_data = await self._provider.get_team(team_id)
            if team_data:
                team = Team.from_dict(team_data)
                self._teams[team_id] = team
                return team
    ...
    async def get_team_by_invite(self, invite_code: str) -> Optional[Team]:
        ...
        # Try provider
        if self._provider and hasattr(self._provider, "find_team_by_invite"):
            team_id = await self._provider.find_team_by_invite(invite_code)
            if team_id:
    ...
    async def join_team(
        ...
        # Update provider
        if self._provider and hasattr(self._provider, "update_team"):
            await self._provider.update_team(team.team_id, team.to_dict())
    ...
    async def leave_team(self, team_id: str, user_id: str) -> bool:
        ...
        # Update provider
        if self._provider and hasattr(self._provider, "update_team"):
            await self._provider.update_team(team_id, team.to_dict())
    ...
    async def delete_team(self, team_id: str, user_id: str) -> bool:
        ...
        if self._provider and hasattr(self._provider, "delete_team"):
            await self._provider.delete_team(team_id)
    ...
    async def get_user_teams(self, user_id: str) -> list[Team]:
        ...
        # Also check provider for teams not cached locally
        if self._provider and hasattr(self._provider, "get_user_teams"):
            remote_teams = await self._provider.get_user_teams(user_id)
            for team_data in remote_teams:
                team = Team.from_dict(team_data)
                if team.team_id not in self._teams:
                    self._teams[team.team_id] = team
                    user_teams.append(team)
    ...
    async def add_session_to_team(
        ...
        # Update provider
        if self._provider and hasattr(self._provider, "add_session_to_team"):
            await self._provider.add_session_to_team(
                team_id, session_id, title=title
            )
    ...
    async def remove_session_from_team(
        ...
        # Update provider
        if self._provider and hasattr(self._provider, "remove_session_from_team"):
            await self._provider.remove_session_from_team(team_id, session_id)
    ...
    async def list_team_sessions(
        ...
        if self._provider and hasattr(self._provider, "list_team_sessions"):
            session_list = await self._provider.list_team_sessions(team_id)
            for session_data in session_list:
                sessions.append(TeamSessionInfo.from_dict(session_data))
        else:
            # Build from local data - need session manager for details
    ...
    def registry_manager(self) -> "RegistryManager":
        ...
        if self._registry_manager is None:
            ...
            self._registry_manager = RegistryManager(
                ...
                remote_provider=self._provider,
            )
