...
async def create_sync_provider():
    """Create and connect to the configured sync provider.

    Returns:
        Connected SyncProvider instance, or None if disabled/failed
    ...
    if not config.enabled:
        log.debug("Multiuser is disabled")
        return None

    # Validate config
    errors = config.validate()
    if errors:
        raise ValueError(f"Invalid multiuser config: {', '.join(errors)}")

    # Create provider based on config
    if config.provider == SyncProviderType.LOCAL:
        from .providers.local import LocalFileSyncProvider

        provider = LocalFileSyncProvider(
            storage_root=config.storage_root,
        )

    elif config.provider == SyncProviderType.FIREBASE:
        from .providers.firebase import FirebaseSyncProvider
    ...
    if not connected:
        log.error(f"Failed to connect to {config.provider} provider")
        return None

    log.info(f"Connected to {config.provider} sync provider")
    return provider
