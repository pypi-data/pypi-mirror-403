...
class SharedEventBroadcaster(EventHandler):
    ...
    def __init__(
        ...
        Args:
            ...
            sync_provider: Optional provider for remote sync
        """
    ...
    def handle(self, event: AgentEvent) -> None:
        ...
        # Sync to remote if provider configured
        if self._sync_provider:
            asyncio.create_task(self._sync_event(shared_event))
    ...
    async def _sync_event(self, event: SharedEvent) -> None:
        """Sync event to remote provider."""
        if not self._sync_provider:
            return

        try:
            await self._sync_provider.push_event(self._session_id, event)
        except Exception as e:
            log.warning(f"Failed to sync event to remote: {e}")
