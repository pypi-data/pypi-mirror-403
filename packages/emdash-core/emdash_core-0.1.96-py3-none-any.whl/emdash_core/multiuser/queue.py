...
class SyncedMessageQueue(SharedMessageQueue):
    """Message queue that syncs with a remote SyncProvider.

    This extends SharedMessageQueue to push/pull queue state
    to a remote backend for multi-machine synchronization.
    """

    def __init__(
        self,
        session_id: str,
        sync_provider: Any,  # SyncProvider, but avoid circular import
        storage_path: Optional[Path] = None,
        on_event: Optional[Callable[[str, dict], None]] = None,
    ):
        ...
        Args:
            session_id: Session this queue belongs to
            sync_provider: Provider for remote sync
    ...
    async def _persist(self) -> None:
        """Persist to both local storage and remote."""
        # Local persistence
        ...
        if self._sync_provider:
            try:
                await self._sync_provider.update_session_state(
                    self._session_id,
                    {"message_queue": [m.to_dict() for m in self._queue]},
                )
            except Exception as e:
                log.warning(f"Failed to sync queue to remote: {e}")

    async def sync_from_remote(self) -> None:
        """Pull queue state from remote provider."""
        if not self._sync_provider:
            return

        try:
            state = await self._sync_provider.get_session_state(self._session_id)
            self._queue = state.message_queue
            log.debug(f"Synced queue from remote: {len(self._queue)} messages")
        except Exception as e:
            log.warning(f"Failed to sync queue from remote: {e}")
