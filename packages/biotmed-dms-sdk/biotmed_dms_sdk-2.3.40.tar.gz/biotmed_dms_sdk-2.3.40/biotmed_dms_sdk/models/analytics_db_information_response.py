...
class AnalyticsDBInformationResponse(BaseModel):
    """
    ...
    """ # noqa: E501
    activation_state: Optional[StrictStr] = Field(default=None, description="The activation state of the analytics DB", alias="activationState")
    deployment_state: Optional[StrictStr] = Field(default=None, description="The deployment state of the analytics DB", alias="deploymentState")
    ...
    # BioT Code End
    __properties: ClassVar[List[str]] = ["activationState", "deploymentState", "lastModifiedTime", "creationTime"]

    @field_validator('activation_state')
    def activation_state_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['INACTIVE', 'INITIALIZING', 'STOPPING_INITIALIZATION', 'ACTIVE_AND_SYNC_ON']):
            raise ValueError("must be one of enum values ('INACTIVE', 'INITIALIZING', 'STOPPING_INITIALIZATION', 'ACTIVE_AND_SYNC_ON')")
        return value
    ...
    def deployment_state_validate_enum(cls, value):
        ...
        if value not in set(['NOT_DEPLOYED', 'DEPLOYED']):
            raise ValueError("must be one of enum values ('NOT_DEPLOYED', 'DEPLOYED')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of AnalyticsDBInformationResponse from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of AnalyticsDBInformationResponse from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "activationState": obj.get("activationState"),
