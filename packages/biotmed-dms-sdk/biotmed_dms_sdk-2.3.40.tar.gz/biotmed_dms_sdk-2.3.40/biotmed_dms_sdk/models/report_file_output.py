...
class ReportFileOutput(BaseModel):
    ...
    """ # noqa: E501
    files_location: Optional[Dict[str, ReportLocations]] = Field(default=None, description="An object that holds for each data type exported, A list containing the paths to it's files", alias="filesLocation")
    url_expiration_time: datetime = Field(description="A UTC date-time in the ISO-8601 format", alias="urlExpirationTime")
    # BioT Code Start: Store custom attributes in a dedicated dict (safe default)
    additional_properties: Dict[str, Any] = Field(default_factory=dict)
    # BioT Code End
    __properties: ClassVar[List[str]] = ["filesLocation", "urlExpirationTime"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of ReportFileOutput from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # override the default output from pydantic by calling `to_dict()` of each value in files_location (dict)
        _field_dict = {}
        if self.files_location:
            for _key_files_location in self.files_location:
                if self.files_location[_key_files_location]:
                    _field_dict[_key_files_location] = self.files_location[_key_files_location].to_dict()
            _dict['filesLocation'] = _field_dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of ReportFileOutput from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "filesLocation": dict(
                (_k, ReportLocations.from_dict(_v))
                for _k, _v in obj["filesLocation"].items()
            )
            if obj.get("filesLocation") is not None
            else None,
            "urlExpirationTime": obj.get("urlExpirationTime")
        })
