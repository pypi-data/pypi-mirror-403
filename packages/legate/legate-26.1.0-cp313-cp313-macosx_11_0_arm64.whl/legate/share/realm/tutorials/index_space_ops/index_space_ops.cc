...
void run_set_operations()
{
  ...
  for(size_t y = 0; y <= size; y++) {
    subspaces.push_back(IndexSpace<2>(Rect<2>(Point<2>(0, y), Point<2>(size, y))));
  }
  ...
  Event event1 = IndexSpace<2>::compute_union(subspaces, union_is, ProfilingRequestSet());

  IndexSpace<2> diffs_is;
  Event event2 = IndexSpace<2>::compute_difference(union_is, center_is, diffs_is,
                                                   ProfilingRequestSet(), event1);

  IndexSpace<2> isect_is;
  Event event3 = IndexSpace<2>::compute_intersection(diffs_is, union_is, isect_is,
                                                     ProfilingRequestSet(), event2);
  event3.wait();
