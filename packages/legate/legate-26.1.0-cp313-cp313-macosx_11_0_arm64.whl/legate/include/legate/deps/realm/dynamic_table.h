...
namespace Realm {
  ...
  struct DefaultElementConstructor {
    void construct(ET *storage, ID id, unsigned owner) const { storage->init(id, owner); }
  };
  ...
  public:
    ...
    ET *lookup_entry(IT index, int owner, ET **free_list_head = 0,
                     ET **free_list_tail = 0);

    void set_constructor(Constructor &_elem_ctor);

  protected:
    NodeBase *new_tree_node(int level, IT first_index, IT last_index, int owner,
