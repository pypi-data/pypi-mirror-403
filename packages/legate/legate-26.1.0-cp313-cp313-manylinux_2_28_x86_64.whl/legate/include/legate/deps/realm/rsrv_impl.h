...
namespace Realm {
  ...
  public:
    ...
    enum AcquireType
    {
      ...
      // Reservation::try_acquire() - grants immediately or returns an event for when a
      ...
      // used when the try_acquire is preconditioned on something else first, so we never
      // grant, but record a retry'er
      ACQUIRE_NONBLOCKING_PLACEHOLDER,
    };

    // creates an Event if needed to describe when reservation is granted
    Event acquire(unsigned new_mode, bool exclusive, AcquireType acquire_type,
