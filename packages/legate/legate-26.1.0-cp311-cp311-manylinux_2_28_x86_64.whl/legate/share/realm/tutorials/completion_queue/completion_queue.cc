               ...
               Processor p)
{
  ...
  if(pq.count() < 2) {
    log_app.warning("It is better to run this program with at least 2 CPU processors, "
                    "please specify it through -ll:cpu");
  }

  CompletionQueue completion_queue =
      CompletionQueue::create_completion_queue(TestConfig::num_tasks);
  ...
  if(pq.count() < 2) {
    ...
    if(machine.get_address_space_count() == 1) {
      ...
      // pick a remote processor for the CLEANCQ_TASK, this is used to demonstrate
      // get_nonempty_event on non-owner node
  ...
  for(int i = 0; i < TestConfig::num_tasks; i++) {
    ...
    Event e = worker_procs[i % worker_procs.size()].spawn(WORKER_TASK, &worker_task_args,
                                                          sizeof(WorkerTaskArgs));
    completion_queue.add_event(e);
  }
  ...
  Event cleancq_e =
      cleancq_proc.spawn(CLEANCQ_TASK, &cq_task_args, sizeof(CleanCQTaskArgs), nonempty);
...
int main(int argc, char **argv)
{
  ...
  Processor::register_task_by_kind(Processor::LOC_PROC, false /*!global*/, WORKER_TASK,
                                   CodeDescriptor(worker_task), ProfilingRequestSet(), 0,
                                   0)
      .wait();
  Processor::register_task_by_kind(Processor::LOC_PROC, false /*!global*/, CLEANCQ_TASK,
                                   CodeDescriptor(cleancq_task), ProfilingRequestSet(), 0,
                                   0)
      .wait();
