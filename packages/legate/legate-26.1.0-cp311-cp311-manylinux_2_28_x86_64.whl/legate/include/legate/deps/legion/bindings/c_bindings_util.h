...
#define ARRAY_ACCESSOR(DIM)                                           \
  typedef Legion::UnsafeFieldAccessor<                                \
...
#ifdef __ICC
// icpc complains about "error #858: type qualifier on return type is
// meaningless" but it's pretty annoying to get this macro to handle all the
// cases right
#pragma warning(push)
...
#define NEW_OPAQUE_WRAPPER(T_, T)                             \
  ...
  {                                                           \
    ...
    NEW_OPAQUE_WRAPPER(
        legion_domain_point_iterator_t, Domain::DomainPointIterator*);
#define RECT_ITERATOR(DIM)                       \
  NEW_OPAQUE_WRAPPER(                            \
      legion_rect_in_domain_iterator_##DIM##d_t, \
      RectInDomainIterator##DIM##D*);
    LEGION_FOREACH_N(RECT_ITERATOR)
#undef RECT_ITERATOR
    ...
    NEW_OPAQUE_WRAPPER(
        legion_multi_domain_point_coloring_t, MultiDomainPointColoring*);
...
#define BUFFER_CHAR(DIM) \
  NEW_OPAQUE_WRAPPER(    \
      legion_deferred_buffer_char_##DIM##d_t, DeferredBufferChar##DIM##D*);
    LEGION_FOREACH_N(BUFFER_CHAR)
#undef BUFFER_CHAR
...
#undef ACCESSOR_ARRAY
    ...
    NEW_OPAQUE_WRAPPER(
        legion_task_layout_constraint_set_t, TaskLayoutConstraintSet*);
    NEW_OPAQUE_WRAPPER(legion_map_task_input_t, Mapping::Mapper::MapTaskInput*);
    NEW_OPAQUE_WRAPPER(
        legion_map_task_output_t, Mapping::Mapper::MapTaskOutput*);
    NEW_OPAQUE_WRAPPER(
        legion_slice_task_output_t, Mapping::Mapper::SliceTaskOutput*);
    ...
    NEW_OPAQUE_WRAPPER(
        legion_point_transform_functor_t, PointTransformFunctor*);
    NEW_OPAQUE_WRAPPER(legion_task_variant_registrar_t, TaskVariantRegistrar*);
...
#define NEW_POINT_WRAPPER(DIM)                            \
  ...
  {                                                       \
    ...
    for (int i = 0; i < DIM; i++) t_.x[i] = t[i];         \
    return t_;                                            \
  }                                                       \
  static Point##DIM##D unwrap(legion_point_##DIM##d_t t_) \
  {                                                       \
    Point##DIM##D t;                                      \
    for (int i = 0; i < DIM; i++) t[i] = t_.x[i];         \
    return t;                                             \
  }
...
#define NEW_BLOCKIFY_WRAPPER(DIM)                              \
  ...
  {                                                            \
    ...
    struct Blockify {
    public:
      Blockify(Point<DIM, coord_t> b, Point<DIM, coord_t> o)
...
#define NEW_TRANSFORM_WRAPPER(D1, D2)                                     \
  ...
  {                                                                       \
    ...
    for (int i = 0; i < D1; i++)                                          \
      for (int j = 0; j < D2; j++) t_.trans[i][j] = t[i][j];              \
    return t_;                                                            \
  }                                                                       \
  ...
  {                                                                       \
    Transform##D1##x##D2 t;                                               \
    for (int i = 0; i < D1; i++)                                          \
      for (int j = 0; j < D2; j++) t[i][j] = t_.trans[i][j];              \
    return t;                                                             \
  }
...
#define NEW_AFFINE_TRANSFORM_WRAPPER(D1, D2)                           \
  ...
  static legion_affine_transform_##D1##x##D2##_t wrap(                 \
  ...
  static AffineTransform##D1##x##D2 unwrap(                            \
      legion_affine_transform_##D1##x##D2##_t t_)                      \
  {                                                                    \
    AffineTransform##D1##x##D2 t;                                      \
...
#undef NEW_AFFINE_TRANSFORM_WRAPPER
    ...
    static legion_domain_t wrap(Domain domain)
    {
      ...
      std::copy(
          domain.rect_data, domain.rect_data + 2 * LEGION_MAX_DIM,
          domain_.rect_data);
      return domain_;
    }
    ...
    static Domain unwrap(legion_domain_t domain_)
    {
      ...
      std::copy(
          domain_.rect_data, domain_.rect_data + 2 * LEGION_MAX_DIM,
          domain.rect_data);
      return domain;
    }
    ...
    static legion_domain_transform_t wrap(DomainTransform transform)
    {
      ...
      std::copy(
          transform.matrix, transform.matrix + LEGION_MAX_DIM * LEGION_MAX_DIM,
          transform_.matrix);
      return transform_;
    }
    ...
    static DomainTransform unwrap(legion_domain_transform_t transform_)
    {
      ...
      std::copy(
          transform_.matrix,
          transform_.matrix + LEGION_MAX_DIM * LEGION_MAX_DIM,
          transform.matrix);
      return transform;
    }

    static legion_domain_affine_transform_t wrap(
    ...
    static DomainAffineTransform unwrap(
        legion_domain_affine_transform_t transform_)
    {
      DomainAffineTransform transform;
    ...
    {
      LogicalRegion r(
          r_.tree_id, unwrap(r_.index_space), unwrap(r_.field_space));
      return r;
    }
    ...
    {
      LogicalPartition r(
          r_.tree_id, unwrap(r_.index_partition), unwrap(r_.field_space));
      return r;
    }
    ...
    static TaskConfigOptions unwrap(legion_task_config_options_t options_)
    {
      TaskConfigOptions options(
          options_.leaf, options_.inner, options_.idempotent);
      return options;
    }
    ...
    static legion_slice_task_input_t wrap(
    ...
    static legion_slice_task_input_t wrap_const(
    ...
    static Mapping::Mapper::SliceTaskInput unwrap(
        legion_slice_task_input_t& input_)
    {
      Mapping::Mapper::SliceTaskInput input;
  ...
  class CContext {
  public:
    CContext(Context _ctx) : ctx(_ctx) { }

    CContext(Context _ctx, const std::vector<PhysicalRegion>& _physical_regions)
      : ctx(_ctx), physical_regions(_physical_regions.size())
    {
    ...
    Context context(void) const { return ctx; }
    ...
    size_t num_regions(void) const { return physical_regions.size(); }
  protected:
    Context ctx;
