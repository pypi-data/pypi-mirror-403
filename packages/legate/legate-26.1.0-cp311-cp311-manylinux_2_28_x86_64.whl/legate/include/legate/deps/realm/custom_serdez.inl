...
namespace Realm {
  ...
  public:
    ...
    virtual CustomSerdezUntyped *clone(void) const { return new CustomSerdezWrapper<T>; }
  ...
  template <typename T>
  inline size_t CustomSerdezWrapper<T>::serialized_size(const void *field_ptr,
                                                        ptrdiff_t stride,
                                                        size_t count) const
  {
    size_t total = 0;
    for(size_t i = 0; i < count; i++) {
      total +=
          T::serialized_size(*static_cast<const typename T::FIELD_TYPE *>(field_ptr));
      field_ptr = static_cast<const char *>(field_ptr) + stride;
    }
    return total;
  }

  // serializes 'val' into the provided buffer - no size is provided (serialized_size will
  // be called first),
  //  but the function should return the bytes used
  template <typename T>
  inline size_t CustomSerdezWrapper<T>::serialize(const void *field_ptr,
                                                  void *buffer) const
  {
  ...
  template <typename T>
  inline size_t CustomSerdezWrapper<T>::serialize(const void *field_ptr, ptrdiff_t stride,
                                                  size_t count, void *buffer) const
  {
    size_t total = 0;
    for(size_t i = 0; i < count; i++) {
      size_t n =
          T::serialize(*static_cast<const typename T::FIELD_TYPE *>(field_ptr), buffer);
      field_ptr = static_cast<const char *>(field_ptr) + stride;
  ...
  // deserializes the provided buffer into 'val' - the existing contents of 'val' are
  // overwritten, but
  //  will have already been "destroyed" if this copy is overwriting previously valid data
  //  - this call should return the number of bytes consumed - note that the size of the
  //  buffer is not supplied (this means the serialization format must have some internal
  //  way of knowing how much to read)
  template <typename T>
  inline size_t CustomSerdezWrapper<T>::deserialize(void *field_ptr,
                                                    const void *buffer) const
  {
  ...
  template <typename T>
  inline size_t CustomSerdezWrapper<T>::deserialize(void *field_ptr, ptrdiff_t stride,
                                                    size_t count,
                                                    const void *buffer) const
  {
    size_t total = 0;
    for(size_t i = 0; i < count; i++) {
      size_t n =
          T::deserialize(*static_cast<typename T::FIELD_TYPE *>(field_ptr), buffer);
  ...
  template <typename T>
  inline void CustomSerdezWrapper<T>::destroy(void *field_ptr, ptrdiff_t stride,
                                              size_t count) const
  {
    for(size_t i = 0; i < count; i++) {
      T::destroy(*static_cast<typename T::FIELD_TYPE *>(field_ptr));
