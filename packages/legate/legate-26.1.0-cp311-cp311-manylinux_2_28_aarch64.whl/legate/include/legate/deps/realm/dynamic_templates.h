...
namespace Realm {
  namespace DynamicTemplates {
    ...
    struct TypeListElem {
      ...
      struct MaxSize {
        ...
        static constexpr size_t value =
            sizeof(_HEAD) < _TAIL::MaxSize::value ? _TAIL::MaxSize::value : sizeof(_HEAD);
      };
      ...
      struct MaxSizeType {
        typedef
            typename std::conditional<SIZE == sizeof(_HEAD), _HEAD,
                                      typename _TAIL::template MaxSizeType<SIZE>>::type
                TYPE;
      };
    ...
    //  integers.  For uniformity, it actually holds Int<N>'s and supports the same
    //  operations:
    ...
    //
    // and a family of dynamic operations:
    // d) TL::demux<TARGET>(i, args...) - calls TARGET::demux<Int<i>>(args...) if i is in
    // the list
    ...
    struct IntDemuxHelper {
      ...
      static void demux(int index, T1 arg1, T2 arg2)
      {
        if(index == (BASE + DELTA))
          ...
          IntDemuxHelper<TARGET, BASE, DELTA - 1>::template demux<T1, T2>(index, arg1,
                                                                          arg2);
      }
      ...
      static void demux(int index, T1 arg1, T2 arg2, T3 arg3)
      {
        if(index == (BASE + DELTA))
          ...
          IntDemuxHelper<TARGET, BASE, DELTA - 1>::template demux<T1, T2, T3>(index, arg1,
                                                                              arg2, arg3);
      }
      ...
      static void demux(int index, T1 arg1, T2 arg2, T3 arg3, T4 arg4)
      {
        if(index == (BASE + DELTA))
          ...
          IntDemuxHelper<TARGET, BASE, DELTA - 1>::template demux<T1, T2, T3, T4>(
    ...
    struct ListProduct2 {
      template <typename T1, typename T2>
      REALM_CUDA_HD static constexpr TagType encode_tag(void);
