...
class _LLMOptions:
    ...
    use_anthropic: NotGivenOr[bool]
    anthropic_caching: NotGivenOr[bool]
    anthropic_cache_ttl: NotGivenOr[str]
...
class LLM(llm.LLM):
    def __init__(
        ...
        *,
        ...
        use_anthropic: NotGivenOr[bool] = NOT_GIVEN,
        anthropic_caching: NotGivenOr[bool] = NOT_GIVEN,
        anthropic_cache_ttl: NotGivenOr[str] = NOT_GIVEN,
    ) -> None:
        """
        ...
        Args:
            ...
            use_anthropic (bool, optional): Whether to use Anthropic/Claude models via Vertex AI. Requires vertexai=True.
                If not provided, it attempts to read from the `USE_ANTHROPIC_MODELS` environment variable. Defaults to False.
            anthropic_caching (bool, optional): Enable Anthropic prompt caching to cache system messages for cost savings and reduced latency.
                Only applicable when use_anthropic=True. If not provided, it attempts to read from the `ANTHROPIC_CACHING` environment variable. Defaults to False.
                - First request: +25% (5m cache) or +100% (1h cache) cost on cache writes
                - Subsequent requests: -90% cost on cache reads
            anthropic_cache_ttl (str, optional): Cache duration for Anthropic prompt caching. Options: "5m" (default) or "1h".
                Only applicable when anthropic_caching=True. If not provided, defaults to "5m".
                Note: 1-hour TTL not supported for Claude 3.7 Sonnet, Claude 3.5 Sonnet, or Claude 3 Opus models.
        """  # noqa: E501
        super().__init__()
        ...
        # Determine if using Anthropic models
        use_anthropic_models = (
            use_anthropic
            if is_given(use_anthropic)
            else os.environ.get("USE_ANTHROPIC_MODELS", "0").lower() in ["true", "1"]
        )

        # Validate Anthropic usage
        if use_anthropic_models:
            if not use_vertexai:
                raise ValueError(
                    "use_anthropic requires vertexai=True. Anthropic models can only be accessed via Vertex AI."
                )

            # Warn about Gemini-specific parameters
            if is_given(thinking_config):
                logger.warning(
                    "thinking_config is Gemini-specific and will be ignored for Anthropic models"
                )
            if is_given(retrieval_config):
                logger.warning(
                    "retrieval_config is Gemini-specific and will be ignored for Anthropic models"
                )
            if is_given(safety_settings):
                logger.warning(
                    "safety_settings is Gemini-specific and will be ignored for Anthropic models"
                )
            if is_given(top_k):
                logger.warning(
                    "top_k has different semantics in Anthropic and will be ignored"
                )
            if is_given(automatic_function_calling_config):
                logger.warning(
                    "automatic_function_calling_config is Gemini-specific and will be ignored for Anthropic models"
                )

        # Handle Anthropic caching parameters
        use_anthropic_caching = (
            anthropic_caching
            if is_given(anthropic_caching)
            else os.environ.get("ANTHROPIC_CACHING", "0").lower() in ["true", "1"]
        )

        anthropic_ttl = (
            anthropic_cache_ttl
            if is_given(anthropic_cache_ttl)
            else os.environ.get("ANTHROPIC_CACHE_TTL", "5m")
        )

        # Validate cache TTL
        if is_given(anthropic_ttl) and anthropic_ttl not in ["5m", "1h"]:
            raise ValueError(
                f"anthropic_cache_ttl must be either '5m' or '1h', got: {anthropic_ttl}"
            )

        # Validate model compatibility with 1h TTL
        if is_given(anthropic_ttl) and anthropic_ttl == "1h" and use_anthropic_models:
            incompatible_models = [
                "claude-3.7-sonnet",
                "claude-3.5-sonnet",
                "claude-3-opus",
            ]
            if any(incomp in model.lower() for incomp in incompatible_models):
                logger.warning(
                    f"Model {model} does not support 1h cache TTL. "
                    "Falling back to default 5-minute TTL."
                )
                anthropic_ttl = "5m"
        ...
        self._opts = _LLMOptions(
            ...
            use_anthropic=use_anthropic_models,
            project=project,
            location=location,
            ...
            anthropic_caching=use_anthropic_caching,
            anthropic_cache_ttl=anthropic_ttl,
        )
        ...
        if use_anthropic_models:
            ...
            self._anthropic_client = AsyncAnthropicVertex(
                ...
                region=gcp_location or "us-central1",
            )
    ...
    def provider(self) -> str:
        ...
        elif self._client and self._client.vertexai:
            return "Vertex AI"
        else:
            return "Gemini"
...
class LLMStream(llm.LLMStream):
    ...
    async def _run_anthropic(self) -> None:
        """Run streaming with Anthropic client via Vertex AI."""
        retryable = True
        request_id = utils.shortuuid()

        try:
            # Convert chat context to Anthropic format
            messages_data, extra_data = self._chat_ctx.to_provider_format(format="anthropic")

            # Build parameters
            params: dict[str, Any] = {
                "model": self._model,
                "messages": messages_data,
                "max_tokens": self._llm._opts.max_output_tokens or 8192,
            }

            # Handle system messages with optional caching
            if extra_data.system_messages:
                if is_given(self._llm._opts.anthropic_caching) and self._llm._opts.anthropic_caching:
                    # Convert to list format with cache_control
                    system_blocks = [
                        {"type": "text", "text": msg}
                        for msg in extra_data.system_messages
                    ]

                    # Add cache_control to last block
                    cache_control = {"type": "ephemeral"}
                    cache_ttl = self._llm._opts.anthropic_cache_ttl
                    if is_given(cache_ttl) and cache_ttl == "1h":
                    ...
                    params["system"] = "\n".join(extra_data.system_messages)

            # Add temperature if specified
            if is_given(self._llm._opts.temperature):
                params["temperature"] = self._llm._opts.temperature

            # Add top_p if specified
            if is_given(self._llm._opts.top_p):
                params["top_p"] = self._llm._opts.top_p
            ...
            if self._tools:
                ...
                anthropic_tools = [
                    schema
                    for schema in tool_ctx.parse_function_tools("anthropic")
                ]
                if anthropic_tools:
                    params["tools"] = anthropic_tools

            # Stream with AsyncAnthropicVertex client
            async with self._anthropic_client.messages.stream(**params) as stream:
                ...
                # Track usage metrics including cache metrics
                if message.usage:
                    usage_data = {
                    ...
                    if hasattr(message.usage, "cache_read_input_tokens") and message.usage.cache_read_input_tokens:
                        usage_data["prompt_cached_tokens"] = message.usage.cache_read_input_tokens

                    self._event_ch.send_nowait(
                        llm.ChatChunk(
                            id=request_id,
                            usage=llm.CompletionUsage(**usage_data),
                        )
                    )

                # Handle tool calls in the final message
            ...
            raise APIConnectionError(
                f"anthropic llm: error generating content {str(e)}",
                retryable=retryable,
            ) from e
