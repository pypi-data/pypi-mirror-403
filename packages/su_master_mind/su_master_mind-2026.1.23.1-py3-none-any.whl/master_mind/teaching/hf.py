...
def _get_fallback_cache_dir() -> str:
    """Get fallback cache directory: $MASTER_MIND_DATA_PATH/huggingface/."""
    cache_path = _get_cache_path()
    if cache_path:
        cache_dir = cache_path / "huggingface"
        cache_dir.mkdir(parents=True, exist_ok=True)
        return str(cache_dir)
    # If no MASTER_MIND_DATA_PATH, use temp directory
    tmpdir = tempfile.gettempdir()
    user = getpass.getuser()
    cache_dir = Path(tmpdir) / "master-mind" / user
    cache_dir.mkdir(parents=True, exist_ok=True)
    return str(cache_dir)
...
def _warn_fallback(
    resource_type: str, resource_id: str, cache_dir: Optional[str] = None
):
    """Emit a warning when falling back to HuggingFace Hub."""
    if cache_dir:
        msg = (
            f"{resource_type} '{resource_id}' not found in local cache. "
            f"Downloading from HuggingFace Hub to {cache_dir}."
        )
    else:
        msg = (
            f"{resource_type} '{resource_id}' not found in local cache. "
...
def load_hf_dataset(
    ...
    **kwargs,
    ...
    _warn_fallback("Dataset", dataset_id, fallback_cache)
    if fallback_cache:
...
def load_hf_model(
    ...
    **kwargs,
    ...
    # If MASTER_MIND_DATA_PATH was set, use fallback cache dir to avoid HF cache issues
    fallback_cache = _get_fallback_cache_dir() if cache_path else None
    _warn_fallback("Model", model_id, fallback_cache)
    if fallback_cache:
        kwargs.setdefault("cache_dir", fallback_cache)
