...
def load_hf_dataset(
    ...
    **kwargs,
        ...
        if local_base.exists():
            if split:
                ...
                if _is_download_complete(split_path):
                    _log_cache_info(
                        f"Loading dataset '{dataset_id}' (split={split}) "
                        f"from local cache: {split_path}"
                    )
                ...
                if splits:
                    _log_cache_info(
                        f"Loading dataset '{dataset_id}' from local cache: {local_base}"
                    )
    ...
    _log_cache_info(
        f"Loading dataset '{dataset_id}' from HuggingFace Hub (no local cache)"
    )
    ...
    **kwargs,
    ...
    if cache_path:
        ...
        if _is_download_complete(local_path):
            _log_cache_info(
                f"Loading model '{model_id}' from local cache: {local_path}"
            )
...
def load_hf_tokenizer(
    ...
    **kwargs,
    ...
    if cache_path:
        ...
        if _is_download_complete(local_path):
            _log_cache_info(
                f"Loading tokenizer '{model_id}' from local cache: {local_path}"
            )
    ...
    if fallback_cache:
        _log_cache_info(
            f"Loading tokenizer '{model_id}' from HuggingFace Hub "
            f"with fallback cache: {fallback_cache}"
        )
        kwargs.setdefault("cache_dir", fallback_cache)
    else:
        _log_cache_info(
            f"Loading tokenizer '{model_id}' from HuggingFace Hub (no local cache)"
        )
...
def load_hf_processor(
    ...
    **kwargs,
    ...
    if cache_path:
        ...
        if _is_download_complete(local_path):
            _log_cache_info(
                f"Loading processor '{model_id}' from local cache: {local_path}"
            )
    ...
    if fallback_cache:
        _log_cache_info(
            f"Loading processor '{model_id}' from HuggingFace Hub "
            f"with fallback cache: {fallback_cache}"
        )
        kwargs.setdefault("cache_dir", fallback_cache)
    else:
        _log_cache_info(
            f"Loading processor '{model_id}' from HuggingFace Hub (no local cache)"
        )
