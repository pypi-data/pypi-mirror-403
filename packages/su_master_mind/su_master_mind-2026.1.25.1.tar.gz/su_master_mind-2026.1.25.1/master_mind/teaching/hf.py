...
def _log_cache_info(message: str):
    """Log cache information, adapting to notebook or terminal environment."""
...
def _warn_fallback(resource_type: str, resource_id: str):
    ...
    logger.info(msg, UserWarning, stacklevel=3)
        ...
        if local_base.exists():
            if split:
                ...
                if _is_download_complete(split_path):
                    _log_cache_info(f"Loading dataset '{dataset_id}' (split={split}) from local cache: {split_path}")
                ...
                if splits:
                    _log_cache_info(f"Loading dataset '{dataset_id}' from local cache: {local_base}")
                    return DatasetDict(splits)

    # Fall back to HuggingFace Hub
    _warn_fallback("Dataset", dataset_id)
    _log_cache_info(f"Loading dataset '{dataset_id}' from HuggingFace Hub (no local cache)")
    ...
    **kwargs,
    ...
    if cache_path:
        ...
        if _is_download_complete(local_path):
            _log_cache_info(f"Loading model '{model_id}' from local cache: {local_path}")
            return model_class.from_pretrained(str(local_path), **kwargs)

    # Fall back to HuggingFace Hub
    _warn_fallback("Model", model_id)
    _log_cache_info(f"Loading model '{model_id}' from HuggingFace Hub (no local cache)")
...
def load_hf_tokenizer(
    ...
    **kwargs,
    ...
    if cache_path:
        ...
        if _is_download_complete(local_path):
            _log_cache_info(f"Loading tokenizer '{model_id}' from local cache: {local_path}")
    ...
    _warn_fallback("Tokenizer", model_id)
    if fallback_cache:
        _log_cache_info(f"Loading tokenizer '{model_id}' from HuggingFace Hub with fallback cache: {fallback_cache}")
        kwargs.setdefault("cache_dir", fallback_cache)
    else:
        _log_cache_info(f"Loading tokenizer '{model_id}' from HuggingFace Hub (no local cache)")
...
def load_hf_processor(
    ...
    **kwargs,
    ...
    if cache_path:
        ...
        if _is_download_complete(local_path):
            _log_cache_info(f"Loading processor '{model_id}' from local cache: {local_path}")
            return processor_class.from_pretrained(str(local_path), **kwargs)

    # Fall back to HuggingFace Hub
    # If MASTER_MIND_DATA_PATH was set, use fallback cache dir to avoid HF cache issues
    fallback_cache = _get_hf_cache_dir() if cache_path else None
    _warn_fallback("Processor", model_id)
    if fallback_cache:
        _log_cache_info(f"Loading processor '{model_id}' from HuggingFace Hub with fallback cache: {fallback_cache}")
        kwargs.setdefault("cache_dir", fallback_cache)
    else:
        _log_cache_info(f"Loading processor '{model_id}' from HuggingFace Hub (no local cache)")
