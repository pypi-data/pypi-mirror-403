...
class TestMakeHfTransformersResources:
    ...
    def test_creates_resource_with_correct_key(self, make_fn, resource_type, default_class):
        """Test that resource has correct key."""
        resource = make_fn("bert-base-uncased")
        assert resource.key == "bert-base-uncased"

    def test_creates_resource_with_default_description(self, make_fn, resource_type, default_class):
    ...
    def test_creates_resource_with_custom_description(self, make_fn, resource_type, default_class):
    ...
    def test_download_uses_local_cache_when_complete(self, make_fn, resource_type, default_class, tmp_path):
        ...
        with patch.dict(os.environ, {ENV_VAR: str(tmp_path)}):
            with patch.dict("sys.modules", {"transformers": mock_transformers}):
                ...
                getattr(mock_transformers, default_class).from_pretrained.assert_not_called()

    def test_download_falls_back_to_hf_when_no_cache_path(self, make_fn, resource_type, default_class):
