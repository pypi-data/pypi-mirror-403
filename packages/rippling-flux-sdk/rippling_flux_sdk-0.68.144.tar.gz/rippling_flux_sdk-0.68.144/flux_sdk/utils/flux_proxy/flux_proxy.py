    ...
    def _start_session(self, settings: SessionSettings) -> Tuple[str, str]:
        ...
        resp: StartSessionResponse = self._svc(self.ctl_plane_addr).StartSession(
            ...
            request=StartSessionRequest(
                ...
                expires_at=_proto_time(datetime.now() + settings.duration),
            ),
...
class FluxProxySession(requests.Session):
    ...
    def send(self, request, **kwargs):
        ...
        if not self.auto_renew:
            log.warning("Flux proxy session auto-renewing is turned off")
            return resp

        # this header may be missing in cases where we return with an error.
        sess_hdr = resp.headers.get("X-Flux-Session")
        if not sess_hdr:
            log.warning("Flux proxy session header is missing")
            return resp
        ...
        # Check if expires_at field exists in session parameters
        if "expires_at" not in sess_params:
            log.warning("Flux proxy session header missing expires_at field")
            return resp
        ...
        try:
            ...
            log.warning(
                "Failed to parse expires_at from Flux proxy session header",
                extra={
                    "flux_proxy_session_id": self.session_id,
                    "expires_at_value": sess_params["expires_at"],
                    "error": str(e),
                }
            )
            return resp
