...
class TestCheckField(unittest.TestCase):
    def test_required_string_value(self) -> None:
        @dataclass
        class Object:
            required_string_value: Optional[str]
        ...
        # check empty values
        for value in [None, ""]:
            o.required_string_value = value  # type: ignore
            with self.assertRaises(ValueError):
                check_field(o, "required_string_value", str, required=True)

        # check type
        for value in [123, datetime.now(), {"hello": "world"}, ("foo", "bar", "baz")]:  # type: ignore[assignment]
            o.required_string_value = value  # type: ignore[assignment]
            with self.assertRaises(TypeError):
                check_field(o, "required_string_value", str, required=True)

    def test_optional_string_value(self) -> None:
        @dataclass
        class Object:
        ...
        for value in [123, datetime.now(), {"hello": "world"}, ("foo", "bar", "baz")]:
            o.optional_string_value = value  # type: ignore
            with self.assertRaises(TypeError):
                check_field(o, "optional_string_value", str)

    def test_required_union_type(self) -> None:
        @dataclass
        class Record:
            checkpoint: Checkpoint

        r = Record(checkpoint=None)  # type: ignore[arg-type]
        with self.assertRaises(ValueError):
            check_field(r, "checkpoint", Checkpoint, required=True)

        # valid types
        for value in ["hello world", 123, datetime.now()]:
            r.checkpoint = value  # type: ignore[assignment]
            check_field(r, "checkpoint", Checkpoint, required=True)
    ...
    def test_dict_strings(self) -> None:
        ...
        r = Record(references=None)  # type: ignore[arg-type]
        ...
        for value in ["hello world", 123, 3.14, True, {"hello": 123}]:
            r.references = value  # type: ignore
            with self.assertRaises(TypeError):
                check_field(r, "references", dict[str, str])
                check_field(r, "references", dict[str, str], required=True)
