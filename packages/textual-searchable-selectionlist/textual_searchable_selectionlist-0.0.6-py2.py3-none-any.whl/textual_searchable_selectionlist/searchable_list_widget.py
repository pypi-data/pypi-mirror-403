...
class _SearchInput(Input):
    """Input widget with customized key bindings for selection list use."""

    BINDINGS = [
        binding
        for binding in Input.BINDINGS
        if binding.key not in ("home,ctrl+a", "delete,ctrl+d", "ctrl+c,super+c")  # type: ignore
    ] + [
...
class SearchableListWidget(Widget, Generic[SelectionType]):
    ...
    BINDINGS = [
        ...
        Binding("left,right", "toggle_select", "Toggle select"),
    ...
    class Selected(Message):
        """Emitted when a selection is made via the widget."""

        bubble = True

        def __init__(
            self,
            sender: 'SearchableListWidget',
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.selected_callback: Callable[[list[SelectionType]], TextType | None] | None = (
            selected_callback
        )
    ...
    def compose(self) -> ComposeResult:
        with Vertical():
            ...
            yield _SearchInput(placeholder='Search...', id='search_input')
            ...
            yield Label('', id='selection_result')

    def action_toggle_select(self):
        if (
            self.selection_strategy is SelectionStrategy.MULTIPLE
            and self.selection_list_widget.has_focus
        ):
            option_index = self.selection_list_widget.highlighted
            if option_index is not None:
                option = self.selection_list_widget.get_option_at_index(option_index)
    ...
    def on_mount(self):
        ...
        if len(self.selections) == 1:
            self._emit_selected(
                [self.selection_list_widget.get_option_at_index(0).value], confirmed=False
            )
    ...
    @cached_property
    def result_label(self) -> Label:
        return self.query_one('#selection_result', Label)
    ...
    def on_key(self, event: Key):
        ...
        if event.key == 'enter':
            ...
            if self.multiple:
                # Return all selected
                self._emit_selected(self.selection_list_widget.selected, confirmed=True)
            else:
                ...
                self._emit_selected([value], confirmed=True)
        elif event.key == 'down':
    ...
    def on_selection_list_selection_toggled(self, event: SelectionList.SelectionToggled):
        """Emit selection event when a list item is toggled (mouse/space)."""
        if event.selection_list is not self.selection_list_widget:
            return
        selected = self.selection_list_widget.selected
        if selected:
            self._emit_selected(selected, confirmed=False)

    def on_selection_list_selection_highlighted(self, event: SelectionList.SelectionHighlighted):
        """Update result label on highlight changes."""
        if event.selection_list is not self.selection_list_widget:
            return
        if self.selected_callback:
            result = self.selected_callback([event.selection.value])
    ...
    def _emit_selected(self, selected: list[SelectionType], confirmed: bool):
        """Emit selection through callback and Textual message."""
        result: TextType | None = None
        if self.selected_callback:
            result = self.selected_callback(selected)
        if result is not None:
            self.result_label.update(result)
        self.post_message(self.Selected(self, selected, confirmed=confirmed))
