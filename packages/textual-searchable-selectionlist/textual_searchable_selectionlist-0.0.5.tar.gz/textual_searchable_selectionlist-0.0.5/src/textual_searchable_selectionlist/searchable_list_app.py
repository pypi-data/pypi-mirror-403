...
class SearchableListApp(App, Generic[SelectionType]):
    ...
    def compose(self) -> ComposeResult:
        ...
        yield SearchableListWidget[SelectionType](
            *self.selections,
            ...
            selected_callback=self.selected_callback,
    ...
    def on_searchable_list_widget_selected(self, event: SearchableListWidget.Selected):
        """Exit the app when a selection is made."""
        if not event.selected:
            return
        if self.selection_strategy.multiple() and not event.confirmed:
            return
        self.exit(event.selected)
