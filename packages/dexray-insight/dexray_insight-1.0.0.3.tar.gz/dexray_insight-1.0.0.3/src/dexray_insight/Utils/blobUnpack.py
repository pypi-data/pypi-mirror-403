...
# Output / Internal
FILE_ASSEMBLIES_JSON = ".unpacked_blobs/assemblies.json"
...
class AssemblyStore(object):
    ...
    def __init__(self, in_file_name, manifest_entries, primary=True):
        ...
        if version > ASSEMBLY_STORE_FORMAT_VERSION:
            raise Exception(
                "This version is higher than expected! Max = %d, got %d" % ASSEMBLY_STORE_FORMAT_VERSION, version
            )
        ...
        for assembly in self.assemblies_list:
            ...
            if assembly_header == COMPRESSED_DATA_MAGIC:
                assembly_data = self.decompress_lz4(
                    self.raw[assembly.data_offset : assembly.data_offset + assembly.data_size]
                )
            ...
            logging.debug("Extracting %s..." % entry.name)
...
def lz4_compress(file_data, desc_idx):
    ...
    packed = struct.pack("4sII", COMPRESSED_DATA_MAGIC, desc_idx, len(file_data))
    ...
    compressed_data = lz4.block.compress(file_data, mode="high_compression", store_size=False, compression=9)
...
def do_unpack(in_directory, in_arch, force):
    ...
    if force and os.path.isdir(".unpacked_blobs/"):
        shutil.rmtree(".unpacked_blobs/")

    # First check if all files exist.
    if os.path.isdir(".unpacked_blobs/"):
    ...
    if arch_assemblies:
        arch_assemblies_path = os.path.join(in_directory, ARCHITECTURE_MAP[in_arch])

        arch_assembly_store = AssemblyStore(arch_assemblies_path, manifest_entries, primary=False)
        json_data = arch_assembly_store.extract_all(json_data)

    # Save the large config out.
    with open(FILE_ASSEMBLIES_JSON, "w") as assembly_file:
...
def do_pack(in_json_config):
    ...
    json_data = None
    with open(in_json_config) as json_f:
    ...
    for assembly_store in json_data["stores"]:
        for store_name, store_data in list(assembly_store.items()):
            ...
            for assembly in json_data["assemblies"]:
                ...
                assemblies_blob_f.write(struct.pack("IIIIII", next_data_offset, data_size, 0, 0, 0, 0))
            ...
            for assembly in sorted(assembly_data, key=lambda d: d["hash32"]):
                ...
                mapping_id = (
                    assembly["blob_idx"] if assembly["store_id"] == 0 else store_zero_lec + assembly["blob_idx"]
                )

                # Write the hash32
                assemblies_blob_f.seek(next_hash32_offset)
                assemblies_blob_f.write(
                    struct.pack("4sIIII", hash32, 0, mapping_id, assembly["blob_idx"], assembly["store_id"])
                )
            ...
            for assembly in sorted(assembly_data, key=lambda d: d["hash64"]):
                ...
                assemblies_blob_f.write(
                    struct.pack("8sIII", hash64, mapping_id, assembly["blob_idx"], assembly["store_id"])
                )
...
def unpack_store(args):
    ...
    parser.add_argument(
        ...
        const=True,
        default=False,
        help="Force re-create .unpacked_blobs/ directory.",
    )
