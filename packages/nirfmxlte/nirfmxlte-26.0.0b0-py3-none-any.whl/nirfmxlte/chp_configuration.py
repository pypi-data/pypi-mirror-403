...
class ChpConfiguration(object):
    ...
    @_raise_if_disposed
    def configure_integration_bandwidth_type(self, selector_string, integration_bandwidth_type):
        r"""Configures the type of integration bandwidth (IBW), which selects one of the frequency ranges over which the CHP is
        measured.

        Refer to the `LTE Channel Power (CHP)
        <https://www.ni.com/docs/en-US/bundle/rfmx-lte/page/lte-channel-power.html>`_ topic for more information.
        ...
        Args:
            ...
            integration_bandwidth_type (enums.ChpIntegrationBandwidthType, int):
                This parameter specifies the integration bandwidth (IBW) type used to measure the power of the acquired signal.
                Integration bandwidth is the frequency interval over which the power in each frequency bin is added to measure the
                total power in that interval. The default value is **Signal Bandwidth**.
        ...
        try:
            self._session_function_lock.enter_read_lock()
            _helper.validate_not_none(selector_string, "selector_string")
            integration_bandwidth_type = (
                integration_bandwidth_type.value
                if type(integration_bandwidth_type) is enums.ChpIntegrationBandwidthType
                else integration_bandwidth_type
            )
            updated_selector_string = _helper.validate_and_update_selector_string(
                selector_string, self._signal_obj
            )
            error_code = self._interpreter.chp_configure_integration_bandwidth_type(
                updated_selector_string, integration_bandwidth_type
            )
        finally:
            self._session_function_lock.exit_read_lock()
    ...
    def configure_rbw_filter(self, selector_string, rbw_auto, rbw, rbw_filter_type):
        ...
        Args:
            ...
            rbw (float):
                ...
                when you set the **RBW Auto** parameter to **False**. This value is expressed in Hz.
