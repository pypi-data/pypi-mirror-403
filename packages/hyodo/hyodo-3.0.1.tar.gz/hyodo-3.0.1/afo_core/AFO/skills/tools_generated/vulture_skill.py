...
def run(repo_root: Path) -> dict:
    ...
    candidates = [repo_root / "artifacts/vulture.json", repo_root / "artifacts/vulture.txt"]
    ...
    if hit and hit.stat().st_size > 0:
        ...
        return asdict(ToolSkillResult("vulture", "truth", "healthy", 90, rel))

    # Fail Loud if no evidence
    return asdict(ToolSkillResult("vulture", "truth", "unhealthy", 10, "missing"))
