...
def run(repo_root: Path) -> dict:
    ...
    candidates = [repo_root / "artifacts/mattermost-health.json"]
    ...
    if hit and hit.stat().st_size > 0:
        ...
        return asdict(ToolSkillResult("mattermost", "goodness", "healthy", 90, rel))

    # Fail Loud if no evidence
    return asdict(ToolSkillResult("mattermost", "goodness", "unhealthy", 10, "missing"))
