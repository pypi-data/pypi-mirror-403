...
def run(repo_root: Path) -> dict:
    ...
    candidates = [repo_root / "artifacts/tflite-version.json"]
    ...
    if hit and hit.stat().st_size > 0:
        ...
        return asdict(ToolSkillResult("tflite", "truth", "healthy", 90, rel))

    # Fail Loud if no evidence
    return asdict(ToolSkillResult("tflite", "truth", "unhealthy", 10, "missing"))
