...
def run(repo_root: Path) -> dict:
    ...
    candidates = [repo_root / "artifacts/fairlearn-version.json"]
    ...
    if hit and hit.stat().st_size > 0:
        ...
        return asdict(ToolSkillResult("fairlearn", "goodness", "healthy", 90, rel))

    # Fail Loud if no evidence
    return asdict(ToolSkillResult("fairlearn", "goodness", "unhealthy", 10, "missing"))
