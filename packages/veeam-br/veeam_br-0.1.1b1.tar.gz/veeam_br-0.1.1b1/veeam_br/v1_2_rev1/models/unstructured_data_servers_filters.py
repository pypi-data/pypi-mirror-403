...
class UnstructuredDataServersFilters:
    ...
    Attributes:
        ...
        order_asc (Union[Unset, bool]): If `true`, sorts unstructured data servers in the ascending order by the
            `orderColumn` parameter.
        name_filter (Union[Unset, str]): Filters unstructured data servers by the `nameFilter` pattern. The pattern can
            match any unstructured data server parameter. To substitute one or more characters, use the asterisk (*)
