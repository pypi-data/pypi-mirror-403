...
class PkgSettings(BaseSettings):
    ...
    commit_fix_diff_suffixes: tuple[str, ...] = Field(default_factory=default_commit_diff_suffixes)
    commit_fix_prefixes: tuple[str, ...] = Field(default_factory=default_commit_fix_prefixes)
    dev_mode: bool = False
    ...
    def check_paths(self) -> Self:
        assert self.pkg_directory.exists(), f"Package directory does not exist: {self.pkg_directory}"
    ...
    def public_groups_path(self) -> Path:
        return self._with_dev_suffix(self.state_dir / self.PUBLIC_GROUPS_STORAGE_FILENAME)
    ...
    def parse_computed_public_groups(self, t: type[T]) -> T:
        from pkg_ext._internal.models import PublicGroups  # avoid dependency
...
def pkg_settings(
    ...
    *,
    ...
    return PkgSettings(
        ...
        skip_open_in_editor=skip_open_in_editor if skip_open_in_editor is not None else user_config.skip_open_in_editor,
        dev_mode=dev_mode,
        ...
        commit_fix_diff_suffixes=commit_fix_diff_suffixes or project_config.commit_diff_suffixes,
        ...
        keep_prerelease=keep_prerelease if keep_prerelease is not None else project_config.keep_prerelease,
