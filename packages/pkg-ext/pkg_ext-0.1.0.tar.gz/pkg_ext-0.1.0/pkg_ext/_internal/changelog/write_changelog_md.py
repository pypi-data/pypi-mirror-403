...
from pkg_ext._internal.models import PublicGroup
...
def read_changelog_section(changelog_content: str, old_version: str, new_version: str) -> str:
    old_version_header_regex = re.compile(_header_regex.pattern + old_version, re.MULTILINE)
    new_version_header_regex = re.compile(_header_regex.pattern + new_version, re.MULTILINE)
    if header_match := new_version_header_regex.search(changelog_content):
        section_end = -1
        if old_version and (old_match := old_version_header_regex.search(changelog_content)):
...
def _add_changelog_section(old_content: str, new_section: str, version: str) -> str:
    ...
    if insert_point := next(
        ...
        return old_content[:insert_point] + new_section + "\n\n" + old_content[insert_point:]
...
def _create_changelog_content(
    ...
    root_prefix = "#" * _get_section_header_level(ctx.settings.changelog_md, old_version)
    ...
    if pr_url := git_changes.pr_url:
        ...
        pr_part = ""
    add_section(header=f"{new_version} {date_filename()}{pr_part}", lines=[], header_level=0)
...
def write_changelog_md(ctx: pkg_ctx) -> Path:
    ...
    unreleased = [
        action for action in ctx.pr_changelog_actions() if as_changelog_line(action, ctx.git_changes.remote_url, ctx)
    ]
    if not unreleased:
        return path
    ...
    new_content = _add_changelog_section(path.read_text(), "\n".join(changelog_md), new_version)
