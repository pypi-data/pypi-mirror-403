...
def ref_id_format(original: str):
    if "." not in original:
        raise ValueError(f"A ref_id must use the form parent.child:function module format, got: {original}")
    return original
...
def is_root_identifier(value: str):
    if value.isidentifier():
        return value
    raise ValueError(f"invalid python identifier: {value}, must not use . and be valid module name")
