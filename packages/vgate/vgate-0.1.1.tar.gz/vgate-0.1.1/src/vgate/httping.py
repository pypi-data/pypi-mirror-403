...
def cors_middleware():
    ...
    falcon.CORSMiddleware(allow_origins='*', allow_credentials='*', expose_headers=cesr_headers())
...
def desiginput(value):
    ...
    for name, svfields in sid.items():
        ...
        siginputs.append(
            Inputage(
                name=name, fields=fields, created=created, expires=expires, nonce=nonce, alg=alg, keyid=keyid, context=context
            )
        )
    return siginputs
