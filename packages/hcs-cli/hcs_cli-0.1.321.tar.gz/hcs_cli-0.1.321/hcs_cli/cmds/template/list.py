...
def _format_template_table(data):
    for d in data:
        ...
        updatedAt = reportedStatus["updatedAt"]
        v = duration.stale(updatedAt)
        if duration.from_now(updatedAt).days >= 1:
            v = click.style(v, fg="bright_black")
        d["stale"] = v

        d["_powered_on_vms"] = reportedStatus.get("poweredOnVMs", 0)

        if reportedStatus["status"] == "PARTIALLY_PROVISIONED":
            reportedStatus["status"] = "*PP"
        ...
        util.colorize(reportedStatus, "errorVMs", lambda n: "red" if n > 0 else None)
