...
# Test integration (includes JSON serialization)
...
If you're experiencing issues with QR code display (broken image icon):

1. **Check the integration guides:**
   - `FRONTEND_INTEGRATION.md` - Frontend integration examples
   - `DEBUG_QR_ISSUES.md` - Common issues and solutions

2. **Run the test scripts:**
   ```bash
   python test_qr_generation.py  # Test QR generation
   python test_integration.py     # Test full integration
   ```

3. **Verify in browser console:**
   - Check Network tab for API response
   - Verify `qr_base64` is present and valid
...
4. **Use helper methods:**
   ...
   qr = client.create_payment_qr(provider_username=user_id)
   return qr.to_json_safe_dict()  # Validates before returning
   ```
