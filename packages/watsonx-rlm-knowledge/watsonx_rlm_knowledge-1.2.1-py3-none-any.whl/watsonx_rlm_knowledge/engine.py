...
- You must find ALL relevant information, not just the first match
- There may be MULTIPLE documents covering different aspects/types/variations
- Do NOT stop after finding one answer - keep searching for additional relevant content
- Always check if there are different categories, types, or methods being documented separately
- Your final answer should synthesize information from ALL relevant sources
...
FINAL ANSWER FORMAT (only when you have gathered ALL relevant information):
- FINAL(your complete answer here) - for direct answers
- FINAL_VAR(variable_name) - to return a variable you built

EXHAUSTIVE SEARCH STRATEGY (REQUIRED):

PHASE 1 - DISCOVERY: Find ALL potentially relevant documents
```repl
# Collect ALL matching files - use multiple patterns
all_relevant_files = set()

# Search by filename patterns
for pattern in ["*cancel*", "*bare*metal*", "*server*", "*remove*", "*delete*"]:
    files = knowledge.find_files(pattern)
    all_relevant_files.update(files)
    print(f"Pattern '{{pattern}}': {{len(files)}} files")

# Search by content - collect unique file paths
for term in ["cancel", "bare metal", "server", "terminate", "delete"]:
    matches = knowledge.search(term, max_matches=50)
    paths = set(m.path for m in matches)
    all_relevant_files.update(paths)
    print(f"Term '{{term}}': found in {{len(paths)}} files")

print(f"\\nTotal unique relevant files: {{len(all_relevant_files)}}")
relevant_files = list(all_relevant_files)
print(relevant_files[:30])
```

PHASE 2 - READ ALL RELEVANT DOCUMENTS: Don't stop at one!
```repl
# Read ALL relevant documents, not just the first one
all_content = {{}}
for f in relevant_files[:20]:  # Process up to 20 relevant files
    try:
        content = knowledge.read_full(f)
        all_content[f] = content
        print(f"Read {{f}}: {{len(content)}} chars")
    except:
        pass
print(f"\\nLoaded {{len(all_content)}} documents")
```

PHASE 3 - EXTRACT FROM EACH DOCUMENT: Use llm_query_batched for efficiency
```repl
# Extract relevant info from EACH document
query = "your specific question here"
prompts = [f"Extract information about [topic] from this document. Include ALL methods/types/variations mentioned:\\n\\n{{content}}"
           for content in all_content.values()]
extractions = llm_query_batched(prompts)

# Show what was found in each document
findings = {{}}
for (path, _), extraction in zip(all_content.items(), extractions):
    if extraction and "not found" not in extraction.lower() and "no information" not in extraction.lower():
        findings[path] = extraction
        print(f"\\n=== {{path}} ===\\n{{extraction[:500]}}")

print(f"\\nFound relevant info in {{len(findings)}} documents")
```

PHASE 4 - SYNTHESIZE: Combine ALL findings into comprehensive answer
```repl
# Aggregate ALL findings
all_findings = "\\n\\n---\\n\\n".join([f"From {{path}}:\\n{{info}}" for path, info in findings.items()])
final_answer = llm_query(f"Synthesize ALL the following information into a comprehensive answer. Include ALL different methods/types/variations found:\\n\\n{{all_findings}}")
print(final_answer)
```
...
- NEVER provide FINAL after reading just ONE document - there may be more!
- After finding something, ALWAYS ask "Are there other documents with related info?"
- Use find_files() with MULTIPLE patterns to cast a wide net
- Process ALL relevant documents with llm_query_batched()
- Your final answer must synthesize from ALL sources found
- Track progress: "Found X documents, processed Y so far"
"""
        ...
        for iteration in range(self.config.max_iterations):
            ...
            try:
                ...
                if final_content is not None:
                    ...
                    # Check if multiple files were discovered but answer seems incomplete
                    # Look for signs of multi-file discovery in observations
                    obs_text = " ".join(result.observations)
                    multiple_files_found = (
                        "unique relevant files" in obs_text.lower() or
                        "total unique" in obs_text.lower() or
                        re.search(r"(\d+)\s+files", obs_text) is not None
                    )

                    # If we found multiple files, check if we only processed one
                    files_processed = obs_text.lower().count("read ") + obs_text.lower().count("loaded ")
                    if multiple_files_found and files_processed < 3 and iteration < 8:
                        messages.append({"role": "assistant", "content": model_response})
                        messages.append({
                            "role": "user",
                            "content": (
                                "WAIT - You discovered multiple relevant files but appear to have only processed a few. "
                                "There may be additional information in other documents.\n\n"
                                "Before providing a final answer:\n"
                                "1. Go back and read the OTHER relevant files you found\n"
                                "2. Use llm_query_batched() to process multiple documents efficiently\n"
                                "3. Look for different types/categories/methods that might be documented separately\n\n"
                                "Output a ```repl``` code block to process the remaining documents."
                            )
                        })
                        continue
