...
- **Integrate with Quality Tools**
  ...
  - Use Crackerjack for comprehensive code quality checks
...
- **Use UV for Tool Execution**
  ...
  - Execute tools like pyright, ruff, and opera-cloud-mcp through UV: `uv run pyright`
...
- **Crackerjack Integration**
  ...
  - Run all quality checks: `uv run crackerjack`
  - Run specific tools: `uv run ruff check --fix`, `uv run mypy .`, `uv run bandit -r .`
  - Quality standards are automatically enforced
  - See [Crackerjack Integration](./CLAUDE.md#crackerjack-integration) in CLAUDE.md for details
...
- **Code Quality Validation**
  ...
  - The ultimate goal is to run `uv run crackerjack -t` without any errors
...
- **MCP Server Integration**
  ...
  - MCP tools: `execute_opera-cloud-mcp`, `get_job_progress`, `get_comprehensive_status`
...
- **Quality Standards Maintenance**
  ...
  - Test all documentation updates by running `uv run crackerjack -t`
  - Prioritize frequently occurring error patterns as **CRITICAL** standards

- **Self-Learning Protocol**

  - When encountering new quality check failures, extract the error pattern and add to standards
