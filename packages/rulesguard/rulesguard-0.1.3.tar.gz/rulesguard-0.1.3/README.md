...
This pattern affects configuration files that influence execution across AI assistants, IDE settings, and automation/workflow definitions. RulesGuard provides static analysis and policy enforcement to help identify and prevent these security risks.
...
jobs:
  scan:
    ...
    steps:
      ...
      - name: Run RulesGuard
        uses: ./.github/actions/rulesguard
        with:
          path: ${{ github.workspace }}
          fail_on: critical
          format: sarif
...
- name: Run RulesGuard
  uses: ./.github/actions/rulesguard
  with:
    version: '0.1.3'  # Use specific version (no "v" prefix)
```
...
The scan fails if findings meet or exceed the `--fail-on` threshold, protecting your repository.
