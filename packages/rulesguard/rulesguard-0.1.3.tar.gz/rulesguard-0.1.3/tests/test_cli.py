...
class TestCLIExitCodes:
    ...
    def test_exit_code_high_findings(self, malicious_file: Path) -> None:
        ...
        assert result.returncode == 1, "Should exit with code 1 when threshold is high and CRITICAL findings exist"
    ...
    def test_exit_code_medium_findings(self, malicious_file: Path) -> None:
        ...
        assert result.returncode == 1, "Should exit with code 1 when threshold is medium and CRITICAL findings exist"
    ...
    def test_exit_code_low_findings(self, malicious_file: Path) -> None:
        ...
        assert result.returncode == 1, "Should exit with code 1 when threshold is low and CRITICAL findings exist"
    ...
    def test_exit_code_json_format(self, malicious_file: Path) -> None:
        ...
        result = subprocess.run(
            [sys.executable, "-m", "rulesguard.cli", str(malicious_file), "--format", "json", "--fail-on", "critical"],
            capture_output=True,
            text=True,
        )
        assert result.returncode == 1, "Should exit with code 1 for JSON format with CRITICAL findings"

    def test_exit_code_sarif_format(self, malicious_file: Path) -> None:
        """Test that exit codes work with SARIF output format."""
        result = subprocess.run(
            [sys.executable, "-m", "rulesguard.cli", str(malicious_file), "--format", "sarif", "--fail-on", "critical"],
            capture_output=True,
            text=True,
        )
        assert result.returncode == 1, "Should exit with code 1 for SARIF format with CRITICAL findings"
