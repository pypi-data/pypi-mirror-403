...
- In @src/rulesguard/cli.py around lines 267 - 272, The scanner instance lacks an initialized baseline, causing scan_file to access a missing attribute; update the RulesGuardScanner.__init__ to initialize self.baseline (e.g., to an empty dict/list/object as appropriate for baseline operations) so scan_file and scan can safely reference it; ensure the initialization matches the expected type used by scan_file/scan and include any related attributes (e.g., baseline_path or baseline_loaded flag) if scan relies on them.

- In @src/rulesguard/models.py at line 381, The type annotation for the to_dict methods uses the lowercase built-in `any` instead of the typing `Any`; update the signature(s) of the to_dict methods (e.g., the to_dict function at line with "def to_dict(self) -> Dict[str, any]") to use `Any` (`Dict[str, Any]`) and ensure `Any` is imported from `typing` (apply the same fix as done for `BaselineFinding.to_dict`), repeating the change for the other to_dict occurrence around the to_dict at the later location.

- In @src/rulesguard/models.py at line 234, The type annotation uses lowercase `any` which is invalid; update the imports to include `Any` from typing (e.g., add Any to the existing typing import) and change the return type hints in the affected methods (e.g., `to_dict`) from `Dict[str, any]` to `Dict[str, Any]` (apply the same change for the other occurrence around the `to_dict` at the other location).

- In @src/rulesguard/scanner.py around lines 91 - 110, The method _is_in_baseline uses self.baseline but __init__ never sets it, causing AttributeError; initialize self.baseline in the class constructor (in __init__) to a sensible default (e.g., None or an empty baseline object) and ensure any code that uses config.baseline_path or baseline.contains handles that default; update __init__ to set self.baseline = None (or an appropriate Baseline() instance) so _is_in_baseline and any baseline filtering logic safely reference the attribute.

- In @src/rulesguard/scanner.py around lines 201 - 206, The code mutates shared state by assigning ScannerConfig.load_ignore_file(scan_root) to self.config.ignore_patterns inside scan_path; instead keep ignore_patterns local (e.g., ignore_patterns = ScannerConfig.load_ignore_file(scan_root)) and do not assign to self.config, then pass that local ignore_patterns into the file-filtering logic or into should_scan_file (update should_scan_file signature to accept an ignore_patterns param) so patterns are isolated per scan; alternatively, if you must store on the instance, set self.config.ignore_patterns temporarily at the start of scan_path and restore the previous value at the end to avoid accumulation across calls.

- In @tests/test_baseline.py around lines 183 - 205, The test test_baseline_exit_code_only_considers_new_findings currently only asserts len(result2.findings) == 0 but its name/docstring promise exit-code behavior; either rename the function to reflect it duplicates test_baseline_suppresses_findings, or implement exit-code verification by invoking the CLI: use click.testing.CliRunner to call the CLI entrypoint (e.g., rulesguard.cli.cli) — run a scan first and assert result.exit_code != 0, generate a baseline (invoke the baseline generation command and write baseline.json), then run the scan again passing --baseline and assert result.exit_code == 0; update imports in the test accordingly and keep the original file/test logic for creating the test.cursorrules sample and baseline file.# 🛡️  RulesGuard
...
```yaml
- uses: NOTTIBOY137/RulesGuard/.github/actions/rulesguard@v0.1.4
  with:
...
Fail CI based on severity using `fail_on` / `--fail-on`. The action captures exit codes without failing the workflow step, giving you full control over CI behavior.
...
- name: Run RulesGuard with baseline
  uses: NOTTIBOY137/RulesGuard/.github/actions/rulesguard@v0.1.3
  with:
    path: ${{ github.workspace }}
...
- `path` (default: `${{ github.workspace }}`): Path to scan
- `fail_on` (default: `critical`): Exit with non-zero code if findings >= LEVEL (`none`, `low`, `medium`, `high`, `critical`)
- `format` (default: `sarif`): Output format (`console`, `json`, `sarif`)
- `output` (default: `rulesguard-results.sarif`): Output file path for JSON/SARIF export
- `version` (optional): RulesGuard version to install (empty = latest from PyPI)
...
- name: Run RulesGuard
  uses: NOTTIBOY137/RulesGuard/.github/actions/rulesguard@v0.1.3
  with:
    version: '0.1.4'  # Use specific version (no "v" prefix)
```
...
- `tests/fixtures/**` - Test fixtures (may contain intentional malicious patterns)
- `.cursor/**` - Cursor IDE metadata
- `.venv/**` and `venv/**` - Python virtual environments
- `dist/**` - Distribution/build directories
- `__pycache__`, `*.pyc` - Python cache files
- `.git`, `node_modules`, `.pytest_cache` - Common development directories
