...
class RulesGuardScanner:
    ...
    def scan_path(self, path: Path) -> ScanSummary:
        ...
        if scan_root.exists():
            ...
            if ignore_patterns:
                self.config.ignore_patterns = ignore_patterns

        if path.is_file():
            # Scan single file
            if self.config.should_scan_file(path, scan_root=path.parent):
            ...
            for file_path in path.glob(pattern):
                if file_path.is_file() and self.config.should_scan_file(file_path, scan_root=path):
