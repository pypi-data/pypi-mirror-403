...
class RulesGuardScanner:
    ...
    def _initialize_detectors(self) -> None:
        ...
        if self.config.detectors:
            ...
            self.detectors = [
                d for d in all_detectors if d.name.lower().replace("detector", "") in enabled_names
            ]
        else:
            # Use all detectors
    ...
    def scan_path(self, path: Path) -> ScanSummary:
        ...
        if path.is_file():
            # Scan single file
            if self.config.should_scan_file(
                path, scan_root=path.parent, ignore_patterns=ignore_patterns
            ):
            ...
            for file_path in path.glob(pattern):
                if file_path.is_file() and self.config.should_scan_file(
                    file_path, scan_root=path, ignore_patterns=ignore_patterns
                ):
