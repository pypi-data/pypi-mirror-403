...
class SARIFReporter:
    ...
    def __init__(self, tool_name: str = "RulesGuard", tool_version: str = __version__) -> None:
    ...
    def _finding_to_sarif(self, finding: Finding) -> Dict[str, Any]:
        ...
        # SECURITY: Resolve to absolute path before converting to URI
        # This prevents crashes with relative paths and ensures consistent URIs
        file_path = Path(finding.file_path)
        if not file_path.is_absolute():
            # If path is relative, resolve it relative to current working directory
            # This handles cases where relative paths are provided
            try:
                file_path = file_path.resolve()
            except (OSError, ValueError):
                # If resolution fails, try to make it absolute using cwd
