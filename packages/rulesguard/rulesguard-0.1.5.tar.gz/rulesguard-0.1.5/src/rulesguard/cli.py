...
# Create the group
@click.group()
@click.version_option(version="0.1.5", package_name="rulesguard")
def cli() -> None:
...
@click.option("--exclude", "-e", multiple=True, type=click.Path(path_type=Path), help="Paths to exclude from scanning")
@click.option("--detector", "-d", multiple=True, help="Detectors to enable (unicode, pattern, entropy). Default: all")
@click.option("--max-size", type=int, default=10 * 1024 * 1024, help="Maximum file size to scan in bytes (default: 10MB)")
@click.option("--output", "-o", type=click.Path(path_type=Path), help="Output file path for JSON/SARIF export")
@click.option("--format", "-f", type=click.Choice(["console", "json", "sarif"], case_sensitive=False), default="console", help="Output format (default: console)")
@click.option("--fail-on", type=click.Choice(["none", "low", "medium", "high", "critical"], case_sensitive=False), default="critical", help="Exit with non-zero code if findings >= LEVEL (default: critical)")
@click.option("--recursive/--no-recursive", default=True, help="Scan directories recursively (default: True)")
@click.option("--verbose", "-v", is_flag=True, help="Enable verbose logging")
@click.option("--baseline", type=click.Path(path_type=Path), help="Path to baseline file to suppress known findings")
def scan(
    paths: tuple,
    exclude: tuple,
    detector: tuple,
...
@cli.command()
@click.option("--with-baseline", is_flag=True, help="Generate baseline file from current scan")
@click.option("--with-pre-commit", is_flag=True, help="Generate .pre-commit-config.yaml")
def init(with_baseline: bool, with_pre_commit: bool) -> None:
    ...
    try:
        ...
        workflow_template = templates_pkg / "rulesguard.yml"
    ...
    if with_baseline:
        ...
        console.print(f"[green]Created {baseline_file} with {len(baseline_obj.findings)} findings[/green]")
...
@click.option("--output", "-o", type=click.Path(path_type=Path), default=".rulesguard.baseline.json", help="Output path for baseline file (default: .rulesguard.baseline.json)")
@click.option("--detector", "-d", multiple=True, help="Detectors to enable (unicode, pattern, entropy). Default: all")
@click.option("--max-size", type=int, default=10 * 1024 * 1024, help="Maximum file size to scan in bytes (default: 10MB)")
