...
- `.rulesguardignore` - Ignore patterns file
...
Optional flags:
- `--mode [audit|gate]` - Workflow mode (default: audit)
- `--fail-on LEVEL` - Fail threshold for gate mode: none, low, medium, high, critical (default: critical)
- `--with-baseline` - Generate baseline from current scan and include in workflow
- `--with-pre-commit` - Generate pre-commit hook configuration
...
- `baseline` (optional): Path to baseline file to suppress known findings
