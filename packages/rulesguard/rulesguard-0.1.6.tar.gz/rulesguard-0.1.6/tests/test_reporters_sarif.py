...
class TestSARIFReporter:
    ...
    def test_sarif_export_absolute_path(
        self, reporter: SARIFReporter, sample_finding_absolute: Finding, tmp_path: Path
    ) -> None:
    ...
    def test_sarif_version_consistency(self, reporter: SARIFReporter) -> None:
        ...
        assert (
            reporter.tool_version == __version__
        ), f"SARIF version {reporter.tool_version} should match package version {__version__}"

    def test_sarif_export_from_relative_scan_path(
        self, reporter: SARIFReporter, tmp_path: Path
    ) -> None:
        ...
        if run["results"]:
            ...
            assert uri.startswith(
                "file://"
            ), f"URI should be absolute even from relative path, got: {uri}"
