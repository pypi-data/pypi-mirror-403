...
class RecipientsTest(BaseTestLive):
    ...
    def test_create_recipient_sca_context(self):
        john = BaseTestLive.get_john_sca_owner()
        recipient = RecipientsTest.get_new_recipient_obj()
        recipient.sca_context = 'USER_PRESENT'
        created = Recipient(**recipient.create(john.id))
        self.assertIsNotNone(created.pending_user_action)
