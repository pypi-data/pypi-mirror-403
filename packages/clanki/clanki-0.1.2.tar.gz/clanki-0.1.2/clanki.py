#!/usr/bin/env python3
"""Standalone promotional screen for LIGHT terminals - Clanki logo with shimmer animation."""
...
# Darker base for text on light background
TEXT_BASE_COLOR = (80, 80, 80)
...
def render_frame(shimmer_pos: int) -> list[str]:
    """Render a single frame of the logo with shimmer effect (for light bg)."""
    lines = []
    ...
    for y, row in enumerate(LOGO_DATA):
        ...
        for x, (char, r, g, b) in enumerate(row):
            # For light background: darken overall but boost blue saturation
            # Check if this pixel has blue tint (b > r and b > g)
            if b > r and b > g:
                # Blue pixel - make it a rich dark blue
                r = int(r * 0.2)
                g = int(g * 0.3)
                b = int(b * 0.7)
            else:
                # Gray/white pixel - make it darker gray
                avg = (r + g + b) // 3
                darkness = int(avg * 0.35)
                r, g, b = darkness, darkness, darkness
