...
def data_write_save(simname,excel_file,start,total,space,fsnumber,start_df,start_sum_df,p_r_df,i_r_df,s_r_df, \
    ...
    with pd.ExcelWriter(excel_file, engine='openpyxl', mode='a',if_sheet_exists='overlay') as writer:
        ...
        dashboard_name = "0_"+simname+"_result_supply_points_loading.xlsx"
...
def nps_update(target,start_df,supply_list):
    ...
    for index, feeder in enumerate(supply_list):
        ...
        if pd.notna(pd.to_numeric(target.loc[feeder, '01 Minute'], errors='coerce')):
            nps_df.loc[feeder, '01 Minute'] = target.loc[feeder, '01 Minute'] / start_df.iloc[index, 5]
        if pd.notna(pd.to_numeric(target.loc[feeder, '10 Minutes'], errors='coerce')):
            nps_df.loc[feeder, '10 Minutes'] = target.loc[feeder, '10 Minutes'] / start_df.iloc[index, 5]
        if pd.notna(pd.to_numeric(target.loc[feeder, '30 Minutes'], errors='coerce')):
            nps_df.loc[feeder, '30 Minutes'] = target.loc[feeder, '30 Minutes'] / start_df.iloc[index, 5]
...
if __name__ == "__main__":
    ...
    text_input = "AC_01_Power"  # Adjust as needed
