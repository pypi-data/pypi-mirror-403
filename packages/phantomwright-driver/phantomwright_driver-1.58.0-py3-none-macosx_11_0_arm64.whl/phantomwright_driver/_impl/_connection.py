...
import phantomwright_driver._impl._impl_to_api_mapping
from phantomwright_driver._impl._errors import TargetClosedError, rewrite_error
from phantomwright_driver._impl._greenlets import EventGreenlet
from phantomwright_driver._impl._helper import Error, ParsedMessagePayload, parse_error
from phantomwright_driver._impl._transport import Transport
...
def _extract_stack_trace_information_from_stack(
    ...
    playwright_module_path = str(Path(phantomwright_driver.__file__).parents[0])
    last_internal_api_name = ""
    api_name = ""
    parsed_frames: List[StackFrame] = []
    for frame in st:
        if phantomwright_driver._impl._impl_to_api_mapping.__file__ == frame.filename:
            continue
