...
def ask_gemini[T](
    ...
    *,
    ...
    """Call Gemini API with automatic retry on rate limit errors. Returns RawCallResult.
        ...
        try:
            ...
            if VERBOSE:
                ...
                if len(error_str) <= 300:
                    print(f"[Gemini Retry] Error details: {error_str}")

            time.sleep(wait_time)
            total_tpm_wait += wait_time
            tpm_retries += 1

    raise RuntimeError("ask_gemini exhausted retry loop")
