...
class TravelingSalesmanEnv(Environment[TravelingSalesmanState]):
    ...
    def _random_positions(
        ...
        indices = jax.random.choice(
            rng_key, self.width * self.height, (count,), replace=replace
        )
    ...
    def reset(self, rng_key: jax.Array) -> tuple[TravelingSalesmanState, TimeStep]:
        ...
        agent_pos = self._random_positions(
            pos_key, self._config.num_agents, replace=False, pad=True
        )

        state = TravelingSalesmanState(
            agents_pos=agent_pos,
            flag_available=jnp.full(
                (self.num_agents, self._config.num_flags), True, dtype=jnp.bool_
            ),
            map=map,
    ...
    def step(
        ...
        def _step_agent(
            ...
            def on_found_flag(flag_available, flag_index):
                ...
                flag_available = jax.lax.cond(
                    is_all_taken,
                    lambda _: jnp.full(
                        (self._config.num_flags,), True, dtype=jnp.bool_
                    ),
                    lambda avail: avail,
                    flag_available,
                )
        ...
        new_position, rewards, new_flags = _step_agent(
            state.agents_pos, action, state.flag_available
        )
