...
class KingHillEnv(Environment[KingHillState]):
    ...
    def _generate_tiles(self, rng_key):
        ...
        noise = (
            generate_perlin_noise_2d(
                (self.width, self.height), (8, 8), rng_key=wall_key
            )
            > 0.20
        )
        noise = noise.at[:, 0].set(
            False
        )  # clear the starting edges so agents are not stuck in the walls
    ...
    def reset(self, rng_key: jax.Array) -> tuple[KingHillState, TimeStep]:
        ...
        # place objective
        control_point_x, control_point_y = choose_positions_in_rect(
            tiles,
            self._config.num_flags,
            pos_key,
            0,
            self.padded_height // 2 - 5,
            self.padded_width,
            11,
        )  # jnp.array([[self.padded_width // 2, self.padded_height // 2]], jnp.int32)
        ...
        xs = (
            jnp.arange(team_size, dtype=jnp.int32)
            + (self.width // 2 - self.num_agents // 4)
            + self.pad_width
        )
        ...
        state = KingHillState(
            ...
            control_point_team=jnp.zeros(
                (self._config.num_flags,), jnp.int8
            ),  # team index that controls the point
            tiles=tiles,
            time=jnp.int32(0),
    ...
    def _calculate_movement(
        ...
        proposed_position = jnp.where(
            (action < 4)[:, None],
            state.agents_pos + GW.DIRECTIONS[action],
            state.agents_pos,
        )
        target_tile = state.tiles[proposed_position[:, 0], proposed_position[:, 1]]
        not_blocked_by_tile = jnp.logical_and(
            target_tile != GW.TILE_WALL, target_tile != GW.TILE_DESTRUCTIBLE_WALL
        )

        ordered_proposed_position = proposed_position[move_order]
        movement_markers = movement_markers.at[
            ordered_proposed_position[:, 0], ordered_proposed_position[:, 1]
        ].set(move_order)
        movement_markers = movement_markers.at[
            state.agents_pos[:, 0], state.agents_pos[:, 1]
        ].set(idx)

        target_move = movement_markers[proposed_position[:, 0], proposed_position[:, 1]]
        proposed_position = jnp.where(
            jnp.logical_and(
                jnp.logical_or(target_move == idx, target_move == -1),
                not_blocked_by_tile,
            )[:, None],
            proposed_position,
            state.agents_pos,
    ...
    def _calculate_directions(
        self, state: KingHillState, action: jax.Array
    ) -> jax.Array:
        return jnp.astype(
            jnp.where(action < 4, action, state.agents_direction), jnp.int8
        )
    ...
    def _calculate_attacks(
        self, state: KingHillState, action: jax.Array, agent_targets: jax.Array
    ):
        # temp
        # todo create constants for the agent types, melee = 0, ranged = 1
        ranged_attack_mask = jnp.logical_and(
            action == GW.PRIMARY_ACTION, state.agents_types == 1
        )
        ...
        state = state._replace(
            ...
            arrows_pos=jnp.where(
                arrow_attack[:, None], agent_targets, state.arrows_pos
            ),
            arrows_direction=jnp.where(
                arrow_attack, state.agents_direction, state.arrows_direction
            ),
            arrows_timeouts=jnp.where(
                arrow_attack, self._config.arrow_timeout, state.arrows_timeouts
            ),
        )
        # temp
        ...
        melee_attack_mask = jnp.logical_and(
            action == GW.PRIMARY_ACTION, state.agents_types == 0
        )

        target_pos = jnp.concatenate(
            (melee_target_pos, arrow_target_pos, state.arrows_pos), axis=0
        )
        attack_mask = jnp.concatenate(
            (melee_attack_mask, state.arrows_mask, state.arrows_mask), axis=0
        )
        ...
        agents_health = (
            state.agents_health
            - damage_map[state.agents_pos[:, 0], state.agents_pos[:, 1]]
        )
        killed_mask = agents_health <= 0

        # reset health on death
        agents_health = jnp.where(
            killed_mask, self._agent_type_health[state.agents_types], agents_health
        )

        # respawn
        state = state._replace(
            agents_pos=jnp.where(
                killed_mask[:, None], state.agents_start_pos, state.agents_pos
            ),
            agents_health=agents_health,
            agents_timeouts=jnp.where(
                melee_attack_mask, self._config.melee_timeout, state.agents_timeouts
            ),
        )
    ...
    def _calculate_arrows(self, state: KingHillState) -> KingHillState:
        ...
        arrow_reset = jnp.logical_or(
            arrow_reset, target_tile == GW.TILE_DESTRUCTIBLE_WALL
        )
    ...
    def _calculate_digs(
        ...
        execute_dig = jnp.logical_and(
            state.tiles[agent_targets[:, 0], agent_targets[:, 1]]
            == GW.TILE_DESTRUCTIBLE_WALL,
            action == GW.DIG_ACTION,
        )

        tiles = state.tiles.at[agent_targets[:, 0], agent_targets[:, 1]].set(
            jnp.where(execute_dig, GW.TILE_EMPTY, target_tile)
        )

        state = state._replace(
            tiles=tiles,
            agents_timeouts=jnp.where(
                execute_dig, self._config.dig_timeout, state.agents_timeouts
            ),
        )
        return state
    ...
    def step(
        ...
        action = jnp.where(
            state.agents_timeouts > 0, GW.STAY, action
        )  # this does change the next observation, should probably be done with a action mask
    ...
    def encode_observations(self, state: KingHillState, actions, rewards) -> TimeStep:
