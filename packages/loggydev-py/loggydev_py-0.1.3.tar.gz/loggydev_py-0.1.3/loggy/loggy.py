...
class Loggy:
    ...
    def __init__(
        ...
        Args:
            ...
            tracer: Optional tracer instance for automatic trace context injection.
                When provided, logs will automatically include traceId and spanId
                from the current active span. Must have get_current_context() method.
        """
    ...
    def _queue_log(self, entry: LogEntry) -> None:
        ...
        # Inject trace context if tracer is configured
        if self._tracer is not None:
            try:
                ctx = self._tracer.get_current_context()
                if ctx:
                    trace_id = ctx.get("trace_id") or ctx.get("traceId")
                    span_id = ctx.get("span_id") or ctx.get("spanId")
                    if trace_id or span_id:
                        if entry.metadata is None:
                            entry.metadata = {}
                        if trace_id:
                            entry.metadata["traceId"] = trace_id
                        if span_id:
                            entry.metadata["spanId"] = span_id
            except Exception:
                pass  # Don't fail logging if tracer has issues
    ...
    def flush(self) -> None:
        ...
        try:
            ...
            if not response.ok:
                ...
                # Provide helpful error messages without crashing the app
                status = response.status_code
                if status in (401, 403):
                    print(
                        f"[Loggy] Authentication failed ({status}). Please check your token is valid.\n"
                        f"  → Get your token at https://loggy.dev/settings\n"
                        f"  → Make sure you're using the correct project token",
                        file=sys.stderr
                    )
                elif status == 429:
                    print(
                        "[Loggy] Rate limit exceeded. Logs are being queued and will retry.\n"
                        "  → Consider upgrading your plan for higher limits",
                        file=sys.stderr
                    )
                elif status >= 500:
                    print(
                        f"[Loggy] Server error ({status}). Logs are queued and will retry automatically.",
                        file=sys.stderr
                    )

        except requests.exceptions.ConnectionError:
            # Put logs back in buffer for retry on network errors
            with self._buffer_lock:
                self._log_buffer = logs_to_send + self._log_buffer
            print(
                "[Loggy] Network error: Unable to reach Loggy servers.\n"
                "  → Logs are queued locally and will retry when connection is restored.\n"
            ...
            with self._buffer_lock:
                self._log_buffer = logs_to_send + self._log_buffer
            print(
                "[Loggy] Request timed out. Logs are queued and will retry.",
                file=sys.stderr
            )
        except Exception:
            # Put logs back in buffer for retry - silent for other errors
            with self._buffer_lock:
                self._log_buffer = logs_to_send + self._log_buffer
