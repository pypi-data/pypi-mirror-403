...
#### 1.1.8 (11/12/2025)
- Add new cmaps `369`.
- `bp.contourf()` new argument `cax` to specify the colorbar axis. For example, `cax=[0.85, 0.1, 0.03, 0.8]` means the colorbar axis is placed at the right side of the figure with the specified position and size.

#### 1.1.9 (25/01/2026)
- Add `add_axes_edgeradius()` function to set rounded corners for axes.
- Add `edgeradius` argument in `bp.colorbar()` to set rounded corners for colorbars.
- Add `hatchcolor` argument in `bp.contourf()` to set the color of hatches.
