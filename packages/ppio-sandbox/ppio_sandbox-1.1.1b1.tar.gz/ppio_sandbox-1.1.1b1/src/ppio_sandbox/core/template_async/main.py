...
from ppio_sandbox.core.api import AsyncApiClient
...
class AsyncTemplate:
    """
    Async Template API for listing and managing sandbox templates.
    ...
    templates = await AsyncTemplate.list(template_type="snapshot_template", page=1, limit=10)
    print(f"Total: {templates.total}, Page: {templates.page}/{templates.total_pages}")
    
    # Delete a template
    await AsyncTemplate.delete(template_id="my-template-id")
    ```
    """
    ...
    async def list(
        ...
        **opts: Unpack[ApiParams],
        ...
        # With pagination
        templates = await AsyncTemplate.list(page=2, limit=50)
        ```
        """
        ...
        async with AsyncApiClient(config, limits=AsyncTemplate._limits) as api_client:
