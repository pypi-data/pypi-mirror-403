...
class ApiParams(TypedDict, total=False):
    ...
    """PPIO API Key to use for authentication, defaults to `PPIO_API_KEY` environment variable."""

    domain: Optional[str]
    """PPIO domain to use for authentication, defaults to `PPIO_DOMAIN` environment variable."""

    debug: Optional[bool]
    """Whether to use debug mode, defaults to `PPIO_DEBUG` environment variable."""
...
class ConnectionConfig:
    ...
    @staticmethod
    def _domain():
        return os.getenv("PPIO_DOMAIN") or "sandbox.ppio.cn"

    @staticmethod
    def _debug():
        return os.getenv("PPIO_DEBUG", "false").lower() == "true"
    ...
    @staticmethod
    def _access_token():
        return os.getenv("PPIO_ACCESS_TOKEN")
    ...
    def __init__(
        ...
        self.headers["User-Agent"] = f"ppio-sandbox-python-sdk/{package_version}"
    ...
    def sandbox_headers(self):
        """
        # We need this separate as we use the same header for PPIO access token to API and envd access token to sandbox.
        """
        return {
            **self.headers,
