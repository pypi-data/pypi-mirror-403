...
def validate():
    filename = "irreversible_trust_review.md"

    if not os.path.exists(filename):
        return False, (
            "irreversible_trust_review.md is missing. "
            "A written trust sustainability assessment is required."
        )
    ...
    # 2. Trust fragility focus
    if not any(term in lowered for term in TRUST_FRAGILITY_TERMS):
        return False, (
            "Trust fragility or legitimacy concerns are not clearly articulated. "
            "The analysis should explain why trust does not persist independently."
        )
    ...
    # 5. Forbidden trust-repair framing
    for phrase in FORBIDDEN_PHRASES:
        if phrase in lowered:
            return False, (
                "Trust recovery or justification mechanisms detected. "
                "This lab evaluates assessment and decision-making, not repair plans."
            )

    return True, (
        "Irreversible trust assessment completed successfully. "
