...
def validate():
    filename = "change_management_boundary_review.md"

    if not os.path.exists(filename):
        return False, (
            "change_management_boundary_review.md is missing. "
            "A written change-management boundary definition is required."
        )
    ...
    # 2. Explicit disallowed change zones
    if not any(m in lowered for m in DISALLOWED_MARKERS):
        return False, (
            "Explicit AI-disallowed change management boundaries are missing. "
    ...
    if not any(m in lowered for m in SUPPORT_MARKERS):
        return False, (
            ...
            "from change authority or execution."
        )
    ...
    return True, (
        "Change management boundaries defined successfully. "
