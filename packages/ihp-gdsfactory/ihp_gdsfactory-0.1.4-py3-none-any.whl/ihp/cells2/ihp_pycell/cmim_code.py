...
class cmim(DloGen):
    ...
    def genLayout(self):
        ...
        bottomMetalBBox = Box(
            -self.techparams["Mim_c"],
            -self.techparams["Mim_c"],
            self.w + self.techparams["Mim_c"],
            self.l + self.techparams["Mim_c"],
        )
    ...
    def createPins(self):
        ...
        self.addPin(
            "MINUS", "MINUS", self.bottomMetal.getBBox(), Layer("Metal5", "pin")
        )
    ...
    def generateVias(self):
        ...
        # how many vias?
        xanz = (self.w - cont_over - cont_over + cont_dist) // (
            cont_size + cont_dist
        ) + self.epsilon
        # width for vias
        ...
        yanz = (self.l - cont_over - cont_over + cont_dist) // (
            cont_size + cont_dist
        ) + self.epsilon
        ...
        while ycont_cnt + cont_size + cont_over <= self.l + self.epsilon:
            ...
            while xcont_cnt + cont_size + cont_over <= self.w + self.epsilon:
                via = Box(
                    xcont_cnt, ycont_cnt, xcont_cnt + cont_size, ycont_cnt + cont_size
                )
                Rect(Layer("Vmim"), via)
