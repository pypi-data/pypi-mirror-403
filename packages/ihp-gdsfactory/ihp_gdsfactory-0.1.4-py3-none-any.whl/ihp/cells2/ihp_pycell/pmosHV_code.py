...
class pmosHV(DloGen):
    ...
    def genLayout(self):
        ...
        smallw_gatpoly_cont_dist = (
            cont_Activ_overRec + self.techparams["Gat_d"]
        )  # for w < contActMin -> poly dogbone sep. to gate
        ...
        xanz = math.floor(
            (w - 2 * cont_Activ_overRec + cont_dist) / (cont_size + cont_dist)
            + self.epsilon
        )
        w1 = (
        ...
        diff_cont_offset = GridFix(
            (w - 2 * cont_Activ_overRec - ncont * cont_size - (ncont - 1) * cont_dist)
            / 2
        )
        ...
        dbCreateRect(
            ...
            metall_layer,
            Box(
                xcont_beg - cont_metall_over, yMet1, xcont_end + cont_metall_over, yMet2
            ),
        )
        ...
        for i in range(1, int(ng) + 1):
            ...
            dbCreateRect(
                ...
                poly_layer,
                Box(
                    xpoly_beg, ypoly_beg + diffoffset, xpoly_end, ypoly_end + diffoffset
                ),
            )

            ihpAddThermalMosLayer(
                self,
                Box(
                    xpoly_beg, ypoly_beg + diffoffset, xpoly_end, ypoly_end + diffoffset
                ),
            ...
            if i == 1:
                dbCreateLabel(
                    ...
                    text_layer,
                    Point(
                        (xpoly_beg + xpoly_end) / 2,
                        (ypoly_beg + ypoly_end) / 2 + diffoffset,
                    ),
                    "pmosHV",
                    "centerCenter",
                    "R90",
        ...
        dbCreateRect(
            self,
            pdiff_layer,
            Box(xdiff_beg, ydiff_beg + diffoffset, xdiff_end, ydiff_end + diffoffset),
        )
        dbCreateRect(
            self,
            pdiffx_layer,
        ...
        if (nwell_pActiv_over + diffoffset - nwell_offset) > (
