...
class npn13G2V(DloGen):
    ...
    def genLayout(self):
        ...
        inner = dbCreateRect(
            self, l_pSD, Box(0.9, 0.9, (7.74 - 0.9) + ((Nx - 1) * 2.34), 6.2 - 0.9 + le)
        )
        ...
        outer = dbCreateRect(
            self, l_act, Box(0.2, 0.2, 7.54 + ((Nx - 1) * 2.34), 6.0 + le)
        )
        inner = dbCreateRect(
            self, l_act, Box(0.7, 0.7, (7.54 - 0.5) + ((Nx - 1) * 2.34), 6.0 - 0.5 + le)
        )
        dbLayerXor(l_act, outer, inner)
        dbDeleteObject(outer)
        dbDeleteObject(inner)

        pcLabelText = f"Ae={int(Nx):d}*{le:.2f}*{we:.2f}"
        pcLabelHeight = 0.35
