...
class bondpad(DloGen):
    ...
    def defineParamSpecs(self, specs):
        ...
        specs(
            ...
            "octagon",
            "Shape",
            ChoiceConstraint(["octagon", "square", "circle"]),
        )
        ...
        specs(
            ...
            bottomMetal,
            "BottomMetal",
            ChoiceConstraint(["1", "2", "3", "4", "5", "TM1"]),
        )
        specs(
            "addFillerEx",
            addFillerEx,
            "Metal Filler Exclusion",
            ChoiceConstraint(["nil", "t"]),
        )
        specs("passEncl", "2.1u", "Passiv enclosure in TM2")

        specs(
            "padType",
            techparams["bondpad_padType"],
            "Pad type",
            ChoiceConstraint(["bondpad", "probepad"]),
        )
    ...
    def genLayout(self):
        ...
        if shape == "octagon":
            ...
            if stack:
                ...
                orady = rady - ooff
                ooff = tog(min(oradx, orady) * (1 - 1 / (sqrt(2) + 1)))
                poly2 = bondpadOctagonRingPoints(
                    radx, rady + 0.005, offset, oradx, orady, ooff
                )
            ...
            if shape == "square":
                if addFillerEx:
                    ...
                    for noFiller in noFillerStack:
                        dbCreateRect(
                            self,
                            Layer(noFiller, "nofill"),
                            Box(-oradx, -orady, oradx, orady),
                        )
                ...
                if addFillerEx:
                    oradx = radx + noFillerEnc
                    orady = rady + noFillerEnc
                    id = bondpadStretchedCircle(
                        self, Layer(noFillerStack[0], "nofill"), oradx, orady, grid
                    )
                    # id~>??
                ...
                if stack:
                    if fill:
                        ...
                        oradx = radx - ooff
                        orady = rady - ooff
                        idmask = bondpadStretchedCircle(
                            self, maskLayer, oradx, orady, grid
                        )
                    ...
                    for metal in range(bottomMetal, metalNumber):
                        ...
                        if fill:
                            ...
                            oradx = radx - stripeWidth
                            orady = rady - stripeWidth
                            id2 = bondpadStretchedCircle(
                                self, pcLayer, oradx, orady, grid
                            )
                        ...
                        if radx > rady:
                            ...
                            for i in range(int(n / 4)):
                                ...
                                y = tog(rad * sin(w / 2 + i * w) - vs / 2)
                                item = dbCreateRect(
                                    self, pcLayer, Box(x, y, x + vs, y + vs)
                                )
                                dbCopyShape(item, Point(0, 0), "MX")
                                dbCopyShape(item, Point(0, 0), "MY")
                                dbCopyShape(item, Point(0, 0), "R180")
