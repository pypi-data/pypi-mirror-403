...
class npn13G2L(DloGen):
    ...
    def genLayout(self):
        ...
        id = dbCreateRect(
            self,
            Layer("EmWind", "drawing"),
            Box(
                emWindOrigin_x, emWindOrigin_y, emWindOrigin_x + we, emWindOrigin_y + le
            ),
        )
        groupId = list()
        groupId.append(id)

        id = ihpAddThermalBjtLayer(
            self,
            Box(
                (emWindOrigin_x - 0.05),
                (emWindOrigin_y - 0.05),
                (emWindOrigin_x + we + 0.05),
                (emWindOrigin_y + le + 0.05),
            ),
            True,
            Cell,
        )
        ...
        outer = dbCreateRect(
            ...
            Layer("Activ", "drawing"),
            Box(
                emWindOrigin_x - Activ_enc_hori,
                emWindOrigin_y - Activ_enc_vert,
                emWindOrigin_x + we + Activ_enc_hori,
                emWindOrigin_y + le + Activ_enc_vert,
            ),
        )
        inner = dbCreateRect(
            self,
            Layer("Activ", "mask"),
            Box(
                emWindOrigin_x - 0.705,
                emWindOrigin_y - Activ_enc_vert,
                emWindOrigin_x - Emi_Metal1_enc_hori,
                emWindOrigin_y + le + Activ_enc_vert,
            ),
        )
        masks.add(inner)
        inner1 = dbCreateRect(
            ...
            Layer("Activ", "mask"),
            Box(
                emWindOrigin_x + we + 0.705,
                emWindOrigin_y - Activ_enc_vert,
                emWindOrigin_x + we + Emi_Metal1_enc_hori,
                emWindOrigin_y + le + Activ_enc_vert,
            ),
        )
        masks.add(inner1)
        ...
        # Draw contacts & Via
        id = dbCreateRect(
            self, Layer("Via1", "drawing"), Box(3.805, 3, 3.995, 3.2 + le)
        )
        groupId.append(id)
        id = dbCreateRect(
            self, Layer("Cont", "drawing"), Box(2.68, 2.95, 2.84, 3.25 + le)
        )
        groupId.append(id)
        id = dbCreateRect(
            self, Layer("Cont", "drawing"), Box(3.82, 2.95, 3.98, 3.25 + le)
        )
        groupId.append(id)
        id = dbCreateRect(
            self, Layer("Cont", "drawing"), Box(4.96, 2.95, 5.12, 3.25 + le)
        )
        groupId.append(id)
        ...
        id = dbCreateRect(
            ...
            Box(
                emWindOrigin_x - Col_Metal1_distance,
                2.82,
                emWindOrigin_x - Col_Metal1_distance - Col_Metal1_width,
                4.1 + le,
            ),
        )
        groupId.append(id)
        id = dbCreateRect(
            ...
            Layer("Metal1", "drawing"),
            Box(
                emWindOrigin_x + we + Col_Metal1_distance,
                2.82,
                emWindOrigin_x + we + Col_Metal1_distance + Col_Metal1_width,
                4.1 + le,
            ),
        )
        groupId.append(id)
        ...
        id = dbCreateRect(
            ...
            Layer("Metal1", "drawing"),
            Box(
                emWindOrigin_x - Bas_Metal1_distance,
                2.1,
                emWindOrigin_x - Bas_Metal1_distance - Bas_Metal1_width,
                3.38 + le,
            ),
        )
        groupId.append(id)
        id = dbCreateRect(
            ...
            Layer("Metal1", "drawing"),
            Box(
                emWindOrigin_x + we + Bas_Metal1_distance,
                2.1,
                emWindOrigin_x + we + Bas_Metal1_distance + Bas_Metal1_width,
                3.38 + le,
            ),
        )
        groupId.append(id)
        ...
        id = dbCreateRect(
            ...
            Box(
                emWindOrigin_x - Emi_Metal1_enc_hori,
                emWindOrigin_y - Emi_Metal1_enc_vert,
                emWindOrigin_x + we + Emi_Metal1_enc_hori,
                emWindOrigin_y + le + Emi_Metal1_enc_vert,
            ),
        )
        groupId.append(id)
        id = dbCreateRect(
            ...
            Box(
                emWindOrigin_x - Col_Metal1_distance - Col_Metal1_width,
                2.9,
                emWindOrigin_x + Col_Metal1_distance + Col_Metal1_width + we,
                3.3 + le,
            ),
        )
        id.emi = True
        ...
        dbCreateRect(
            self,
            Layer("TRANS", "drawing"),
            Box(0.9, 0.9, 6.9 + ((Nx - 1) * 2.8), 5.3 + le),
        )

        pcLayer = "pSD"
        outer = dbCreateRect(
            self, Layer("pSD", "drawing"), Box(0, 0, 7.8 + ((Nx - 1) * 2.8), 6.2 + le)
        )
        inner = dbCreateRect(
            self,
            Layer("pSD", "drawing"),
            Box(0.9, 0.9, 7.8 - 0.9 + ((Nx - 1) * 2.8), 6.2 - 0.9 + le),
        )
        ...
        outer = dbCreateRect(
            ...
            Box(0.2, 0.2, 7.6 + ((Nx - 1) * 2.8), 6.0 + le),
        )
        inner = dbCreateRect(
            self,
            Layer(pcLayer, pcPurpose),
            Box(0.7, 0.7, (7.6 - 0.5) + ((Nx - 1) * 2.8), 6.0 - 0.5 + le),
        )
        dbLayerXor(pcLayer, outer, inner)
        dbDeleteObject(outer)
        dbDeleteObject(inner)
        ...
        if self.Nx > 1:
            id = dbCreateRect(
                self, Layer("Metal1", "drawing"), Box(4.415, 1.45, 6.185, 2.1)
            )
            id.base = True
