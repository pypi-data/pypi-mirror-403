...
class ntap1(DloGen):
    ...
    def defineParamSpecs(self, specs):
        ...
        specs(
            ...
            "R,A",
            "Calculate",
            ChoiceConstraint(["R,A", "w,A", "l,A", "w,l,A", "w,l,R", "w,R", "l,R"]),
        )
        specs("R", r, "R")
        specs("w", defW, "Width")
        specs("l", defL, "Length")
    ...
    def genLayout(self):
        ...
        ndiff_over = self.techparams[
            "NW_e"
        ]  # Minimum NWell enclosure of NWell tie surrounded entirely by NWell in N+Activ1
        ...
        # checking for min w/l -> should be defined elsewhere
        ########################################
        ...
        bBox = DrawContArray(
            self, cont_layer, Box(0, 0, w, l), cont_size, cont_dist, cont_diff_over
        )
        ...
        dbCreateRect(
            ...
            Box(
                bBox.left,
                bBox.bottom - cont_metal_endcap,
                bBox.right,
                bBox.top + cont_metal_endcap,
            ),
        )
        dbCreateRect(
            self,
        ...
        dbCreateRect(
            self,
            well_layer,
            Box(-ndiff_over, -ndiff_over, w + ndiff_over, l + ndiff_over),
        )
        dbCreateRect(
            self,
            bulay_layer,
