...
class Box:
    ...
    def destroy(self):
        if not self.box._destroyed():
            ...
            cni.shape.Shape.getCell().shapes(self.__rect.getShape().layer).erase(
                self.__rect.getShape()
            )
            self.box._destroy()
        else:
            pya.Logger.warn("Box.destroy: already destroyed!")
    ...
    def moveBy(self, dx: float, dy: float) -> None:
        ...
        shape = (
            cni.shape.Shape.getCell().shapes(self.__rect._shape.layer).insert(movedBox)
        )
        self.destroy()
    ...
    def transform(self, transform: Transform) -> None:
        ...
        shape = (
            cni.shape.Shape.getCell()
            .shapes(self.__rect._shape.layer)
            .insert(transformedBox)
        )
        self.destroy()
        self.__rect._shape = shape
