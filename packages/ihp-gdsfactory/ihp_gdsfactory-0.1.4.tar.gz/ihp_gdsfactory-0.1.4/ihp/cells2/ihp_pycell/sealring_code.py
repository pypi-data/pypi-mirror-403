...
class sealring(DloGen):
    ...
    def defineParamSpecs(cls, specs):
        ...
        specs(
            "edgeBox",
            edgeBox,
            "EdgeSeal.boundary box away from the outer EdgeSeal.drawing",
        )
    ...
    def genLayout(self):
        ...
        corner_startx = (
            0 + corner_end - (corner_end - corner_width * (corner_steps + 1))
        )
        corner_starty = 0  # start at the bottom right
        metal_startx = (
            corner_end - (corner_end - maxMetalWidth * (corner_steps + 1)) + metalOffset
        )
        edgeBox_startx = 0
        edgeBox_starty = 0
        ...
        # Passiv
        layerobj = dbCreateRect(
            self,
            Layer("Passiv", "drawing"),
            Box(
                corner_startx + edgeBox,
                corner_starty + edgeBox,
                corner_end + edgeBox,
                corner_width + edgeBox,
            ),
        )
        item_list.append(layerobj)
        layerobj = generateCorner(
            self,
            corner_startx + edgeBox,
            corner_starty + edgeBox,
            corner_width,
        ...
        for layer in vias:
            ...
            via_startx = (
                corner_end
                - (corner_end - maxMetalWidth * (corner_steps + 1))
                + metalOffset
                - maxMetalWidth / 2
                - 0.1
            )
            layerobj = dbCreateRect(
                ...
                layer,
                Box(
                    via_startx, viaOffset, via_startx + viaWidth, viaOffset + viaLength
                ),
            )
            ...
            for cnt in range(1, corner_steps + 1):
                layerobj = dbCopyShape(
                    viaGroupId,
                    Point(
                        2 * viaOffset + viaLength * cnt + viaWidth - 0.1,
                        -viaLength * (cnt - 1),
                    ),
                    "R90",
                )
                ...
                layerobj = dbCopyShape(
                    viaGroupId,
                    Point(-maxMetalWidth * (cnt - 1), maxMetalWidth * (cnt - 1) - 0.1),
                    "R0",
                )
                cons(item_list, layerobj)
            ...
            layerobj = dbCreateRect(
                ...
                layer,
                Box(
                    via_startx, viaOffset - 0.1, corner_end, viaOffset - 0.1 + viaWidth
                ),
            )
            cons(item_list, layerobj)
            layerobj = dbCreateRect(
                ...
                Box(
                    viaOffset - 0.1,
                    corner_end - maxMetalWidth / 2 - 0.1,
                    viaOffset - 0.1 + viaWidth,
                    corner_end,
                ),
            )
            cons(item_list, layerobj)
        ...
        dbCreateRect(
            self,
            Layer("Passiv", "drawing"),
            Box(edgeBox, corner_end, corner_width + edgeBox, w - corner_end),
        )
        dbCreateRect(
            self,
            Layer("Passiv", "drawing"),
            Box(corner_end, edgeBox, l - corner_end, corner_width + edgeBox),
        )
        dbCreateRect(
            self,
            Layer("Passiv", "drawing"),
            Box(l - edgeBox, corner_end, l - corner_width - edgeBox, w - corner_end),
        )
        dbCreateRect(
            self,
            Layer("Passiv", "drawing"),
            Box(corner_end, w - edgeBox, l - corner_end, w - corner_width - edgeBox),
        )
        ...
        for layer in layers:
            dbCreateRect(
                ...
                Layer(layer, "drawing"),
                Box(
                    metalOffset, corner_end, metalOffset + corner_width, w - corner_end
                ),
            )
            dbCreateRect(
                self,
                Layer(layer, "drawing"),
                Box(
                    corner_end, metalOffset, l - corner_end, metalOffset + corner_width
                ),
            )
            dbCreateRect(
                self,
        ...
        dbCreateRect(
            self,
            Layer("EdgeSeal", "boundary"),
            Box(edgeBox_startx, edgeBox_starty, l, w),
        )

        # Creating text label w/ area for device registration
        sealringArea = (l * w) / 1e12  # mm2
        pcLabelText = f"Device registration size: x={l:.1f} um ; y={w:.1f} um\nCalculated area: {sealringArea:.1e} sq mm"
