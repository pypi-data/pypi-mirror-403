...
class rppd(DloGen):
    ...
    def genLayout(self):
        ...
        if (
            zerop(contactpush) == 1
            and self.ps - 2 * contoverlay <= psmin
            and onep(self.b) == 1
        ):
            asymcont = True
        else:
        ...
        dbCreateRect(
            ...
            Box(
                xpos1,
                ypos1 + (contactpush + li_salblock) * dir,
                xpos2,
                ypos2 + (contactpush + poly_cont_len + li_salblock) * dir,
            ),
        )
        # **************************************************************
        ...
        if contactpush == 0.0:
            dbCreateRect(
                ...
                Box(
                    xpos1 - psdover,
                    ypos2 + contactpush * dir,
                    xpos2 + psdover,
                    ypos2 + (li_salblock + psdover + poly_cont_len + contactpush) * dir,
                ),
            )
        else:
            dbCreateRect(
                ...
                Box(
                    xpos1 - psdover,
                    ypos2 + (contactpush - salover + psdover) * dir,
                    xpos2 + psdover,
                    ypos2 + (li_salblock + psdover + poly_cont_len + contactpush) * dir,
                ),
            )
        ...
        if drawbar:
            dbCreateRect(
                ...
                locintlayer,
                Box(
                    xpos1 + contbar_poly_over,
                    ypos2 + (contactpush + li_salblock + li_poly_over) * dir,
                    xpos2 - contbar_poly_over,
                    ypos2 + (contactpush + consize + li_salblock + li_poly_over) * dir,
                ),
            )
        else:
            for i in range(int(ncont)):
                dbCreateRect(
                    ...
                    locintlayer,
                    Box(
                        xpos1 + polyover + distr + i * distc,
                        ypos2 + (contactpush + li_salblock + li_poly_over) * dir,
                        xpos1 + polyover + distr + i * distc + consize,
                        ypos2
                        + (contactpush + consize + li_salblock + li_poly_over) * dir,
                    ),
                )
        ...
        dbCreateRect(
            ...
            metlayer,
            Box(
                xpos1 + contbar_poly_over - endcap,
                ypos1,
        ...
        if contactpush > 0:  # Contacts are out of Resitorsquare->draw rectangle
            ...
            dbCreateRect(
                ...
                Box(
                    xpos1 - salover,
                    ypos1 - contactpush,
                    xpos1 + stripes * (self.w + self.ps) - self.ps + salover,
                    ypos1 + self.l + self.w + salover,
                ),
            )
            ...
            dbCreateRect(
                ...
                Box(
                    xpos1 - psdover,
                    ypos1 - contactpush + salover - psdover,
                    xpos1 + stripes * (self.w + self.ps) - self.ps + psdover,
            ...
            if lcor > 0:
                if oddp(stripes):
                    ...
                    dbCreateRect(
                        ...
                        Box(
                            xpos1
                            + stripes * (self.w + self.ps)
                            - self.ps
                            - self.w
                            - salover,
                            ypos1 + self.l + self.w + salover,
                            xpos1 + stripes * (self.w + self.ps) - self.ps + salover,
                            ypos1 + self.l + self.w + salover + lcor,
                        ),
                    )
                    dbCreateRect(
                        self,
                ...
                if oddp(stripes):  # odd stripesnumber
                    dbCreateRect(
                        ...
                        Box(
                            xpos1 - salover,
                            ypos2,
                            xpos1 + (stripes - 1) * (self.w + self.ps) - ps + salover,
                            ypos2 + self.w + salover,
                        ),
                    )
                    dbCreateRect(
                        self,
                    ...
                    dbCreateRect(
                        ...
                        sallayer,
                        Box(
                            xpos1 + self.w + self.ps - salover,
                            ypos1,
                            xpos1 + stripes * (self.w + self.ps) - self.ps + salover,
                            ypos1 - self.w - salover,
                        ),
                    )
                    dbCreateRect(
                        self,
                    ...
                    dbCreateRect(
                        ...
                        psdlayer,
                        Box(
                            xpos1 + self.w + self.ps - psdover,
                            ypos1,
                            xpos1 + stripes * (self.w + self.ps) - self.ps + psdover,
                            ypos1 - self.w - psdover,
                        ),
                    )
        ...
        for j in range(int(stripes)):
            ...
            if i == 1:  # first part has to be longer
                dbCreateRect(
                    self,
                    bodypolylayer,
                    Box(xpos1, ypos1 - contactpush * dir, xpos2, ypos2),
                )
                ihpAddThermalResLayer(
                    self,
                    Box(xpos1, ypos1 - contactpush * dir, xpos2, ypos2),
                    True,
                    Cell,
                ...
                if i == stripes:  # Last Stripe has to be longer
                    dbCreateRect(
                        self,
                        bodypolylayer,
                        Box(xpos1, ypos1, xpos2, ypos2 + (contactpush + lcor) * dir),
                    )
                    ihpAddThermalResLayer(
                        self,
                        Box(xpos1, ypos1, xpos2, ypos2 + (contactpush + lcor) * dir),
                    ...
                    ihpAddThermalResLayer(
                        self, Box(xpos1, ypos1, xpos2, ypos2), True, Cell
                    )
        ...
        # contact area (Top)
        # **************************************************************
        if asymcont:  # contact has to be out of symmetric
            xpos1 = xpos1
        ...
        dbCreateRect(
            ...
            Box(
                xpos1,
                ypos2 + (lcor + contactpush) * dir,
                xpos2,
                ypos2 + (lcor + contactpush + li_salblock) * dir,
            ),
        )
        # cont
        dbCreateRect(
            ...
            Box(
                ...
                ypos2 + (lcor + contactpush + li_salblock) * dir,
                xpos2,
                ypos2 + (lcor + contactpush + poly_cont_len + li_salblock) * dir,
            ),
        )
        # psd
        dbCreateRect(
            ...
            Box(
                ...
                xpos2 + psdover,
                ypos2
                + (lcor + contactpush + li_salblock + psdover + poly_cont_len) * dir,
            ),
        )
        ...
        if drawbar:  # can only be in internal code
            dbCreateRect(
                ...
                Box(
                    xpos1 + contbar_poly_over,
                    ypos2 + (contactpush + li_salblock + li_poly_over + lcor) * dir,
                    xpos2 - contbar_poly_over,
                    ypos2
                    + (contactpush + consize + li_salblock + li_poly_over + lcor) * dir,
                ),
            )
        else:
            for i in range(ncont):
                dbCreateRect(
                    ...
                    Box(
                        ...
                        xpos1 + polyover + distr + i * distc + consize,
                        ypos2
                        + (lcor + contactpush + consize + li_salblock + li_poly_over)
                        * dir,
                    ),
                )
        # **************************************************************
        ...
        dbCreateRect(
            ...
            Box(
                xpos1 + contbar_poly_over - endcap,
                ypos2
                + (contactpush + li_salblock + li_poly_over - metover + lcor) * dir,
                xpos2 - contbar_poly_over + endcap,
                ypos2
                + (contactpush + consize + li_salblock + li_poly_over + metover + lcor)
                * dir,
            ),
        )
            ...
            if stripes > 1:
                dbCreateRect(
                    ...
                    Box(
                        self.w + psdover,
                        0,
                        self.w + self.ps - psdover,
                        -li_salblock - poly_cont_len - psdover,
                    ),
                )
            ...
            if stripes > 2:
                ...
                dbCreateRect(
                    ...
                    Box(
                        ...
                        ypos2,
                        xpos1 + self.w + self.ps - psdover,
                        ypos2 + dir * (li_salblock + poly_cont_len + psdover),
                    ),
                )
        ...
        resistance = CbResCalc(
            "R", 0, lcalc * 1e-6, self.w * 1e-6, self.b, self.ps * 1e-6, Cell
        )
        labeltext = f"rppd r={resistance:.3f}"
