...
def LayerGridFix(layerId, value):
    ...
    fix(
        value * igrid + SG13_EPSILON
    ) * grid  # always use "nice" numbers, as 1/grid may be irrational
...
def inductor_minD(w, s, nr, grid):
    ...
    if nr == 1:
        ...
        dmin = GridFix(
            (GridFix(w / sqrt2 + s / 2) + GridFix(s * 0.4143) + 0.02 + w)
            * 2
            * (1 + sqrt2)
            + 0.01
        )
        ...
        dmin = GridFix(
            ((GridFix(w / sqrt2 + s / 2) + GridFix(s * 0.4143)) * 2 + 2 * s + 4 * w)
            * (1 + sqrt2)
        )
...
def resCalc(self, cell):
    ...
    weff = self.w + lwd
    r = (
        self.l / weff * (self.b + 1) * rspec
        + (2.0 / kappa * weff + self.ps) * self.b / weff * rspec
        + 2.0 / self.w * rzspec
    )
...
def CbResCalc(calc, r, l, w, b, ps, cell):
    ...
    rspec = Numeric(
        techparams[cell + suffix + "_rspec"]
    )  # specific body res. per sq. (float)
    rkspec = Numeric(techparams[cell + "_rkspec"])  # res. per single contact (float)
    rzspec = (
        Numeric(techparams[cell + "_rzspec"]) * 1e6
    )  # transition res. per um width between contact area and body (float)
    lwd = (
        Numeric(techparams[cell + suffix + "_lwd"]) * 1e6
    )  # line width delta [um] (both edges, positive value adds to w)
    kappa = Numeric(techparams[cell + "_kappa"])
    poly_over_cont = techparams["Cnt_d"]  # strcat(cell '_poly_over_cont'))
    cont_size = techparams[
        "Cnt_a"
    ...
    if LeQp3(
        w, minW, "1u", techparams["epsilon1"]
    ):  # 6.8.03 GG: wmin -> minW,HS: Function'LeQp' 28.9.2004
    ...
    if calc == "R":
        ...
        result = (
            (weff * r - b * (2.0 / kappa * weff + ps) * rspec - 2.0 * weff / w * rzspec)
            / (rspec * (b + 1))
            * 1.0e-6
        )  # in [m]
    elif calc == "w":
...
def CbResCurrent(w, eps, cell):  # w must be float in [m], i is given as a string
    ...
    ncont = fix(
        (w * 1.0e6 - 2.0 * poly_over_cont + cont_space + eps) / cont_dist
    )  # max. nr. of contacts across resistor width
    if ncont < 1:
        ncont = 1
...
def CbCapCalc(calc, c, l, w, cell):
    ...
    caspec = (
        Numeric(techparams[cell + "_caspec"]) * 1e-12
    )  # specific cap per sq. [um] (float)
    cpspec = (
        Numeric(techparams[cell + "_cpspec"]) * 1e-6
    )  # specific cap. per [um] perimeter (float)
    ...
    if calc == "C":
        ...
        weff = w + lwd
        result = (
            (c - 2.0 * weff * cpspec) / (caspec * weff + 2.0 * cpspec) - lwd
        ) * 1.0e-6
    elif calc == "w":
        leff = l + lwd
        result = (
            (c - 2.0 * leff * cpspec) / (caspec * leff + 2.0 * cpspec) - lwd
        ) * 1.0e-6
    elif calc == "lw":
        result = (
            -2.0 * cpspec / caspec
            + sqrt(4.0 * cpspec * cpspec / (caspec * caspec) + c / caspec)
            - lwd
        ) * 1.0e-6
...
def CbTapCalc(calc, r, l, w, cell):
    ...
    raspec = (
        Numeric(techparams[cell + "_raspec"]) * 1.0e12
    )  # specific res per sq. [um] (float)
    rpspec = (
        Numeric(techparams[cell + "_rpspec"]) * 1.0e6
    )  # specific res. per [um] perimeter (float)
    ...
    if calc == "R":
        ...
        result = (
            (raspec * rpspec - r * raspec * 2.0 * w)
            / (r * raspec * 2.0 + r * rpspec * w)
            * 1.0e-6
        )
    elif calc == "w":
        result = (
            (raspec * rpspec - r * raspec * 2.0 * l)
            / (r * raspec * 2.0 + r * rpspec * l)
            * 1.0e-6
        )
    elif calc == "wl":
        result = (
            (
                -4.0 * r * raspec
                + sqrt(
                    16.0 * r * r * raspec * raspec + 4.0 * r * rpspec * rpspec * raspec
                )
            )
            / (2.0 * r * rpspec)
        ) * 1.0e-6
...
def CbDiodeCalc(calc, a, l, w, cell):
    ...
    if calc != "w" and calc != "wl":
        if w < minW:
            print(f"w {w} too small\n")
    if calc != "l" and calc != "wl":
        if l < minL:
            print(f"l {l} too small\n")
