...
class SVaricap(DloGen):
    ...
    def genLayout(self):
        ...
        if nr_psd > 1:
            x_psd = GridFix(
                (2 * NX * l + (2 * NX - 1) * gateS - (nr_psd - 1) * 10) / 2 + 0.61
            )
        ...
        for pcIndexX in range(NX):
            dbCreateRect(
                self,
                gate,
                Box(x1 + pcIndexX * pcStepX, y1, x1 + l + pcIndexX * pcStepX, y1 + w),
            )
            dbCreateRect(
                self,
                gate,
                Box(
                    x1 + l + gateS + pcIndexX * pcStepX,
                    y1 - gateS,
                    x1 - gateS + (pcIndexX + 1) * pcStepX,
                    y1 - gateS + w,
                ),
            )
            ...
            MetalCont(
                self,
                GridFix(x1 + l / 2) + pcIndexX * pcStepX,
                y1 + 0.08,
                GridFix(x1 + l / 2) + pcIndexX * pcStepX,
                y1 + w - 0.01,
                met1,
            ...
            MetalCont(
                self,
                x1 - gateS - GridFix(l / 2) + (pcIndexX + 1) * pcStepX,
                y1 - gateS + 0.01,
                x1 - gateS - GridFix(l / 2) + (pcIndexX + 1) * pcStepX,
                y1 - gateS + w - 0.08,
                met1,
            ...
            dbCreateRect(
                ...
                met1,
                Box(
                    x1 + GridFix((l - metW) / 2) + pcIndexX * pcStepX,
                    y1 + w - 0.01,
                    x1 + GridFix((l + metW) / 2) + pcIndexX * pcStepX,
                    y1 + w + 0.12,
                ),
            ...
            dbCreateRect(
                ...
                met1,
                Box(
                    x1 - gateS - GridFix((l + metW) / 2) + (pcIndexX + 1) * pcStepX,
                    y1 - gateS + 0.01,
                    x1 - gateS - GridFix((l - metW) / 2) + (pcIndexX + 1) * pcStepX,
                    y1 - gateS - 0.12,
                ),
        ...
        for pcIndexX in range(int(nr_psd)):
            dbCreateRect(
                ...
                Box(
                    x_psd + pcIndexX * 10,
                    y1 + w + 0.5 - gateOactiv,
                    x_psd + 0.24 + pcIndexX * 10,
                    y1 + w + 0.5 - gateOactiv + 0.76,
                ),
            )
            dbCreateRect(
                self,
                ...
                Box(
                    x_psd + pcIndexX * 10,
                    y1 - gateS - 0.5 + gateOactiv,
                    x_psd + 0.24 + pcIndexX * 10,
                    y1 - gateS - 0.5 + gateOactiv - 0.76,
                ),
            )
            dbCreateRect(
                self,
                psd,
                Box(
                    x_psd - 0.1 + pcIndexX * 10,
                    y1 + w + 0.6 - gateOactiv,
                    x_psd + 0.34 + pcIndexX * 10,
                    y1 + w + 0.6 - gateOactiv + 0.76,
                ),
            )
            dbCreateRect(
                self,
                ...
                Box(
                    x_psd - 0.1 + pcIndexX * 10,
                    y1 - gateS - 0.6 + gateOactiv,
                    x_psd + 0.34 + pcIndexX * 10,
                    y1 - gateS - 0.6 + gateOactiv - 0.76,
                ),
            )

        dbCreateRect(
            self, gate, Box(x1, y1 + w, x1 - gateS + NX * pcStepX, y1 + w + 0.5)
        )
        dbCreateRect(
            self, gate, Box(x1, y1 - gateS, x1 - gateS + NX * pcStepX, y1 - gateS - 0.5)
        )
        ...
        dbCreateRect(
            ...
            Box(
                x1 - 0.73,
                y1 - gateS - 0.5 + gateOnwell,
                x1 - gateS + NX * pcStepX + 0.57,
                y1 + w + 0.5 - gateOnwell,
            ),
        )
        dbCreateRect(
            ...
            Box(
                x1 - 0.49,
                y1 - gateS - 0.5 + gateOnbulay,
                x1 - gateS + NX * pcStepX + 0.33,
                y1 + w + 0.5 - gateOnbulay,
            ),
        )
        dbCreateLabel(
            self,
            text,
            Point(x1 - 0.49, y1 - gateS - 0.5 + gateOnbulay + gateOactiv),
            "SVaricap",
            "centerLeft",
            "R0",
            Font.EURO_STYLE,
