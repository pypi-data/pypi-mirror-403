...
class dpantenna(DloGen):
    ...
    def defineParamSpecs(self, specs):
        ...
        specs(
            "a",
            eng_string(
                CbDiodeCalc("a", 0, Numeric(defL), Numeric(defW), "dantenna"), 3
            ),
            "Device area",
        )
        specs(
            "p",
            eng_string(
                CbDiodeCalc("p", 0, Numeric(defL), Numeric(defW), "dantenna"), 3
            ),
            "Device perimeter",
        )
    ...
    def genLayout(self):
        ...
        if addRecLayer == "t":
            dbCreateRect(
                self,
                diods_layer,
                Box(-diods_over, -diods_over, w + diods_over, l + diods_over),
            )
