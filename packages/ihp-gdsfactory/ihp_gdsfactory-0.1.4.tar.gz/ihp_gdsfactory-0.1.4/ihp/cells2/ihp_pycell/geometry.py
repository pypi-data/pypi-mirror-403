...
from cni.dlo import *
from cni.geo import fgAnd, fgNot, fgXor
...
def dbLayerInside(self, layer, idlist, id):
    ...
    for item in idlist:
        if type(item) == Rect:
            if (
                pnts.containsPoint(Point(item.left, item.bottom))
                and pnts.containsPoint(Point(item.left, item.top))
                and pnts.containsPoint(Point(item.right, item.bottom))
                and pnts.containsPoint(Point(item.right, item.top))
            ):
                it = item.clone()
                it.layer = layer
                mlist.append(it)
...
def dbLayerOutside(self, layer, idlist, id):
    ...
    if type(id) == Rect:
        pnts = PointList(
            [
                Point(id.left, id.bottom),
                Point(id.left, id.top),
                Point(id.right, id.bottom),
                Point(id.right, id.top),
            ]
        )
    else:
        pnts = id.getPoints()
...
def ihpBuildCont(
    ...
    bBox = Box(
        pathpnts.left - (width1 / 2),
        pathpnts.bottom,
        pathpnts.right + (width1 / 2),
        pathpnts.top,
    )
    ...
    if layer3 and layer2 and layer1 and (layer1 != layer2):
        ...
        pin_fig = dbCreateRect(
            self,
            layer2,
            Box(
                bBox.left + layer2_enc,
                bBox.bottom + layer2_enc,
                bBox.right - layer2_enc,
                bBox.top - layer2_enc,
            ),
        )
        ...
        if pinlabel:
            if hor:
                MkPin(
                    ...
                    Box(
                        (pathpnts[0].x + offset),
                        pathpnts[0].y - width3 / 2,
                        (pathpnts[0].x + offset) + width3,
                        pathpnts[0].y + width3 / 2,
                    ),
                    layer2,
                    pinlabel,
                ...
                MkPin(
                    ...
                    0,
                    Box(
                        pathpnts[0].x - width3 / 2,
                        (pathpnts[0].y + offset),
                        pathpnts[0].x + width3 / 2,
                        (pathpnts[0].y + offset) + length3,
                    ),
                    layer2,
                    pinlabel,
                    layer2,
...
def MetalCont(
        ...
        if _nrect > 1:  # calculate new space and do a loop
            ...
            while _yy + srect_l <= y_top - srect_o + 0.0001:
                id = dbCreateRect(
                    self,
                    subRectLayer,
                    Box(tog(_yl), tog(_yy), tog(_xr), tog(_yy + srect_l)),
                )
                mlist.append(id)
            ...
            if _nrect == 1:  # center a single rect
                ...
                id = dbCreateRect(
                    self,
                    subRectLayer,
                    Box(tog(_yl), tog(_ymb), tog(_xr), tog(_ymb + srect_l)),
                )
                mlist.append(id)
            ...
            if _nrect > 1:  # calculate new space and do a loop
                ...
                while _xx + srect_l <= x_right - srect_o + 0.0001:
                    id = dbCreateRect(
                        self,
                        subRectLayer,
                        Box(tog(_xx), tog(_yb), tog(_xx + srect_l), tog(_yt)),
                    )
                    mlist.append(id)
                ...
                if _nrect == 1:  # center a single rect
                    ...
                    id = dbCreateRect(
                        self,
                        subRectLayer,
                        Box(tog(_xml), tog(_yb), tog(_xml + srect_l), tog(_yt)),
                    )
                    mlist.append(id)
    ...
    if pathLayer != "":
        id = dbCreateRect(
            self, pathLayer, Box(tog(x_left), tog(y_bot), tog(x_right), tog(y_top))
        )
        mlist.append(id)
    ...
    for j in range(int(yanz)):
        for i in range(int(xanz)):
            dbCreateRect(
                ...
                layer,
                Box(
                    x1 + xoff + over + (size + space) * i,
                    y1 + yoff + over + (size + space) * j,
                    x1 + xoff + over + (size + space) * i + size,
                    y1 + yoff + over + (size + space) * j + size,
                ),
            )
        ...
        for j in range(int(ny)):
            ...
            mlist.append(
                dbCreateRect(
                    self,
                    contLayer,
                    Box(xl + tog(x), yl + tog(y), xl + tog(x + ws), yl + tog(y + ws)),
                )
            )
            y = y + ws + dsy
...
def DrawContRowMid(self, layer, x0, y0, x1, y1, size, space, drawMid, GRID, EPSILON):
    ...
    if nonzero(dx) and nonzero(dy):
        delta = (
            int(space * (1.0 - 1.0 / math.sqrt(2.0) - 0.01) * IGRID - EPSILON) * GRID
        )
        if dx > 0.0:
            dx = dx - delta
        else:
...
def generateCorner(
    ...
    for cnt in range(corner_steps):
        rect = dbCreateRect(
            ...
            Layer(layer, "drawing"),
            Box(
                corner_startx - corner_width * (cnt + 1),
                corner_starty + offset + (corner_length - corner_width) * cnt,
                corner_startx - corner_width * cnt,
                corner_starty
                + corner_length
                + offset
                + (corner_length - corner_width) * cnt,
            ),
        )
        cons(item_list, rect)
    ...
    rect = dbCreateRect(
        ...
        Box(
            ...
            corner_starty + offset + (corner_length - corner_width) * corner_steps,
            corner_startx - corner_width * corner_steps,
            corner_end,
        ),
    )
    cons(item_list, rect)
...
# ***********************************************************************************************************************
def ihpCreatedExtendedShape(
    g_cvId, g_layerId, g_shapeId, g_extend=None, g_options=None
):
    options = g_options
    if g_options == None:
        ...
        theShapes = cons(
            ...
            pylist(
                ihpCreateExtendedShapeByPoints(
                    g_cvId, g_layerId, shapeId, g_extend, g_options
                )
            ),
        )
...
# ihpCreateExtendedShapeByPoints
# **************************************************************************************************************
def ihpCreateExtendedShapeByPoints(
    g_cvId, g_layerId, g_shapeId, g_extend, g_drawOptions
):
    points = PointList()
    ...
    if (
        ihpIsRectOpt(0, optionList) == 1
        and ihpIsRectOpt(1, optionList) == 1
        and ihpIsRectOpt(2, optionList) == 1
        and ihpIsRectOpt(3, optionList) == 1
    ):
...
def ihpSurroundShapeWithRing(g_cvId, g_layerId, g_shapeId, g_extend, g_ringWidth):
    ...
    shape1 = ihpCreatedExtendedShape(
        g_cvId, g_layerId, g_shapeId, Box(top, bottom, left, right)
    )
    shape2 = ihpCreatedExtendedShape(
        g_cvId,
        g_layerId,
        g_shapeId,
        Box(
...
# **************************************************************************************************************
def ihpCreateContactArrayInShape(
    g_cvId,
    g_layer,
    g_shapeId,
    g_baseShape,
    ...
    for i in range(fix(listlen(g_shapeId))):
        ...
        center = pylist(
            (nth(0, bbox) + left + nth(2, bbox) - right) * 0.5,
            (nth(0, bbox) + bottom + nth(3, bbox) - top) * 0.5,
        )
        ...
        if columns > 0 and rows > 0:
            contacts = ihpPerformContactPlacementInShape(
                g_cvId,
                g_layer,
                shape,
                pylist(columns, rows),
...
# **************************************************************************************************************
def ihpPerformContactPlacementInShape(
    g_cvId, g_layer, g_shapeId, g_table, g_dimentions
):
    contactList = list()
    ...
    for i in range(fix(listlen(rows - 1))):
        for j in range(fix(listlen(columns - 1))):
            ...
            contactId = dbCreateRect(
                cvid, layer, Box(xOrig, yOrig, (xOrig + width), (yOrig + height))
            )
