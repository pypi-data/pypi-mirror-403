...
class rfmosfet_base(DloGen):
    ...
    def defineParamSpecs(cls, specs):
        ...
        specs(
            ...
            "Yes",
            "Guard ring",
            ChoiceConstraint(["Yes", "No", "U", "Top+Bottom"]),
        )
    ...
    def genLayout(self):
        ...
        mlist.append(
            dbCreateRect(self, gatpoly, Box(-dgatx - wgat, u, -dgatx, hact - u))
        )
        mlist.append(
            dbCreateRect(self, gatpoly, Box(W + dgatx, u, W + dgatx + wgat, hact - u))
        )
        ...
        # Source pin
        id = dbCreateRect(
            self, Layer("Metal1", "pin"), Box(0.05, 0.015, W - 0.05, ec - 0.05 - dce)
        )
        ...
        id = dbCreateRect(
            self,
            Layer("Metal1", "pin"),
            Box(0.05, 0.015 + (dc + L) * 1, W - 0.05, ec - 0.05 + (dc + L) * 1 - dce),
        )
        MkPin(self, "D", 0, id.bbox, id.layer)
        dbCreateLabel(
            self,
            text,
            Point(W / 2, ec / 2 + (dc + L) * 1),
            "D",
            "centerCenter",
            "R0",
            Font.EURO_STYLE,
        ...
        if gat_ring == "Yes":
            id = dbCreateRect(
                self,
                met1,
                Box(-dgatx - wgat, -dgaty - wgat, -dgatx, hact + dgaty + wgat),
            )
            mlist = ulist[Rect]()
            mlist.append(id)
            id = dbCreateRect(
                self,
                met1,
                Box(W + dgatx, -dgaty - wgat, W + dgatx + wgat, hact + dgaty + wgat),
            )
            mlist.append(id)
            id = dbCreateRect(self, met1, Box(-dgatx, -dgaty - wgat, W + dgatx, -dgaty))
            mlist.append(id)
            id = dbCreateRect(
                self, met1, Box(-dgatx, hact + dgaty, W + dgatx, hact + dgaty + wgat)
            )
            mlist.append(id)
        ...
        id = dbCreateRect(
            self, Layer("Metal1", "pin"), Box(p1_x - 0.1, p1_y, p2_x + 0.1, p2_y)
        )
        MkPin(self, "G", 0, id.bbox, id.layer)
        dbCreateLabel(
            self,
            text,
            Point((p1_x + p2_x) / 2, (p1_y + p2_y) / 2),
            "G",
            "centerCenter",
            "R0",
            Font.EURO_STYLE,
        ...
        if guard_ring != "No":
            ...
            id = dbCreateRect(
                self,
                Layer("Metal1", "pin"),
                Box(p1_x, p1_y - wguard / 4, p2_x, p2_y + wguard / 4),
            )
