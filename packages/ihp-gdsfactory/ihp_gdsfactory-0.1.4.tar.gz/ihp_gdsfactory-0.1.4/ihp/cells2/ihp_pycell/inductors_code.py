...
class inductors(DloGen):
    ...
    def genLayout(self):
        ...
        if (nr_r == 2) or (nr_r == 1):
            if type3:
                x1_via = (
                    s + w / 2 + 0.5 + (w - nr_vias * 0.9 - (nr_vias - 1) * 1.06 - 1) / 2
                )
            else:
                x1_via = (
                    s / 2 + 0.5 + (w - nr_vias * 0.9 - (nr_vias - 1) * 1.06 - 1) / 2
                )

        else:
            x1_via = (
                x_via + s + w + 0.5 + (w - nr_vias * 0.9 - (nr_vias - 1) * 1.06 - 1) / 2
            )
        ...
        if nr_r != 1:
            for pcIndex1 in range(int(nr_vias)):
                for pcIndex2 in range(int(nr_vias)):
                    dbCreateRect(
                        self, TV2, Box(x1_via, y1_via, x1_via + 0.9, y1_via + 0.9)
                    )
                    dbCreateRect(
                        self, TV2, Box(-x1_via, y1_via, -(x1_via + 0.9), y1_via + 0.9)
                    )
        ...
        for pcIndexX in range(nr_r):
                ...
                if (nr_r == 2) or (nr_r == 1):
                    if type3:
                        polyPoints1 = PointList(
                            [
                                ...
                                Point(s + w / 2, y2 + w),
                                Point(lat_sm / 2, y2 + w),
                                Point(d / 2, y2 + w + cateta_sm),
                                Point(d / 2, y2 + w + cateta_sm + lat_sm),
                                Point(lat_sm / 2, y2 + w + d),
                                Point(x_via, y2 + w + d),
                                Point(x_via, y2 + w * 2 + d),
                                Point(lat_sm / 2 + d1_via_cross, y2 + d + 2 * w),
                                Point(
                                    (d + 2 * w) / 2,
                                    y2 + w + cateta_sm + lat_sm + d1_via_cross,
                                ),
                                Point(
                                    (d + 2 * w) / 2, y2 + w + cateta_sm - d1_via_cross
                                ),
                                Point(lat_sm / 2 + d1_via_cross, y2),
                            ]
                        )
                        polyPoints2 = PointList(
                            [
                                Point(-s - w / 2, y2),
                                Point(-s - w / 2, y2 + w),
                                Point(-lat_sm / 2, y2 + w),
                    ...
                    polyPoints1 = PointList(
                        [
                            ...
                            Point((d + 2 * w) / 2, y2 + w + cateta_sm - d1_via_cross),
                            Point(lat_sm / 2 + d1_via_cross, y2),
                        ]
                    )
                    polyPoints2 = PointList(
            ...
            if type3 and pcIndexX == nr_r - 1:
                x_mid = d / 2 + w / 2
                y_mid = (
                    ((y2 + w + cateta_sm + lat_sm) - (y2 + w + cateta_sm)) / 2
                    + y2
                    + w
                    + cateta_sm
                )
            ...
            if evenp(pcIndexX):
                ...
                #                        dbCreateLabel(self, Layer('TEXT', 'drawing'), Point(0, y2+w+w/2+d-w/3), 'r='+r, 'centerCenter', 'R0', Font.EURO_STYLE, w/6)
                    ...
                    polyPoints3 = PointList(
                        [
                            Point(x_via + w, y2 + w + d),
                            Point(x_via + w, y2 + w * 2 + d),
                            Point(x_cross, y2 + w * 2 + d),
                            Point(
                                x_cross - (w + s + 2 * grid),
                                y2 + w * 3 + s + d + 2 * grid,
                            ),
                            Point(-x_via - w, y2 + w * 3 + s + d + 2 * grid),
                            Point(-x_via - w, y2 + w * 2 + s + d + 2 * grid),
                            Point(-x_cross, y2 + w * 2 + s + d + 2 * grid),
                            Point(w + s + 2 * grid - x_cross, y2 + w + d),
                        ]
                    )
                    ...
                    polyPoints3 = PointList(
                        [
                            ...
                            Point(x_via, y2 + w * 3 + s + d + 2 * grid),
                            Point(x_via, y2 + w * 2 + s + d + 2 * grid),
                            Point(x_cross + 3 * grid, y2 + w * 2 + s + d + 2 * grid),
                            Point(x_cross - (w + s - grid), y2 + w + d),
                        ]
                    )
                    dbCreatePolygon(self, TM2, polyPoints3)
                    x1_via = (
                        x_via + 0.5 + (w - nr_vias * 0.9 - (nr_vias - 1) * 1.06 - 1) / 2
                    )
                    y1_via = (
                        y2
                        + w
                    ...
                    for pcIndex1 in range(int(nr_vias)):
                        for pcIndex2 in range(int(nr_vias)):
                            dbCreateRect(
                                self,
                                TV2,
                                Box(x1_via, y1_via, x1_via + 0.9, y1_via + 0.9),
                            )
                            dbCreateRect(
                                self,
                                TV2,
                ...
                if pcIndexX != 0:
                    polyPoints3 = PointList(
                        [
                            Point(x_via, y2),
                            Point(x_via, y2 + w),
                            Point(x_cross + grid, y2 + w),
                            Point(x_cross - (w + s) + grid, y2 + w * 2 + s),
                            Point(-x_via, y2 + w * 2 + s),
                            Point(-x_via, y2 + w + s),
                            Point(-x_cross, y2 + w + s),
                            Point(-x_cross + w + s, y2),
                        ]
                    )
                    dbCreatePolygon(self, TM2, polyPoints3)
                    polyPoints3 = PointList(
                        [
                            ...
                            Point(-x_cross - grid, y2 + w),
                            Point(-x_cross + w + s - grid, y2 + w * 2 + s),
                            Point(x_via + w + grid, y2 + w * 2 + s),
                            Point(x_via + w + grid, y2 + w + s),
                            Point(x_cross, y2 + w + s),
                            Point(x_cross - (w + s), y2),
                        ]
                    )
        ...
        pathPoints1 = PointList(
            [Point(x1, -1), Point(x1, nr_r * w + (nr_r - 1) * s + 30)]
        )
        pathPoints2 = PointList(
            [Point(-x1, -1), Point(-x1, nr_r * w + (nr_r - 1) * s + 30)]
        )
        if type2 and (nr_r == 1):
        ...
        polyPoints1 = PointList(
            [
                Point(lat_sm / 2, y2),
                Point(d / 2, y2 + cateta_sm),
                Point(d / 2, y2 + cateta_sm + lat_sm),
                Point(lat_sm / 2, y2 + d),
                Point(-lat_sm / 2, y2 + d),
                Point(-d / 2, y2 + cateta_sm + lat_sm),
                Point(-d / 2, y2 + cateta_sm),
        ...
        dbCreateLabel(
            self,
            Layer("TEXT", "drawing"),
            Point(x_left_edge, y2 + (d - lat_sm) / 4 + lat_sm),
            pcLabelText,
            "leftCenter",
            "R0",
            Font.EURO_STYLE,
