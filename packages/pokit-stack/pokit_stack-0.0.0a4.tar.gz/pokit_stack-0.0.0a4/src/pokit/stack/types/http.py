...
Documentation in this file derived from `MDN Web Docs`_ by Mozilla Contributors,
...
class HTTPMethod(str, Enum):
    ...
    US/docs/Web/HTTP/Reference/Methods/GET>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Methods/HEAD>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Methods/POST>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Methods/PUT>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Methods/DELETE>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Methods/CONNECT>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Methods/OPTIONS>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Methods/TRACE>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Methods/PATCH>`_
    """
...
class HTTPStatus(int, Enum):
    ...
    """
    This code is sent in response to an `Upgrade`_ request header from the client and
    indicates the
    protocol the server is switching to.

    .. _Upgrade: https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Upgrade
    ...
    """
    This code was used in `WebDAV`_ contexts to indicate that a request has been
    received by the
    ...
    EARLY_HINTS_103 = 103
    """
    This status code is primarily intended to be used with the `Link`_ header, letting
    the user
    agent start `preloading`_ resources while the server prepares a response or
    `preconnect`_ to an
    ...
    .. _preconnect: https://developer.mozilla.org/en-
    US/docs/Web/HTML/Reference/Attributes/rel/preconnect
    .. _preloading: https://developer.mozilla.org/en-
    US/docs/Web/HTML/Reference/Attributes/rel/preload
    ...
    OK_200 = 200
    """
    The request succeeded. The result and meaning of "success" depends on the HTTP
    method:

    - `GET`_: The resource has been fetched and transmitted in the message body.
    - `HEAD`_: Representation headers are included in the response without any message
    body.
    - `PUT`_ or `POST`_: The resource describing the result of the action is transmitted
    in the message body.
    ...
    there is no way
    in HTTP to later send an asynchronous response indicating the outcome of the
    request. It is
    ...
    available from the
    origin server, but is collected from a local or a third-party copy. This is mostly
    used for
    mirrors or backups of another resource. Except for that specific case, the `200 OK`_
    response is
    ...
    """
    This response code is used in response to a `range request`_ when the client has
    requested a
    part or parts of a resource.

    .. _range request: https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Guides/Range_requests
    ...
    """
    The server has fulfilled a `GET`_ request for the resource, and the response is a
    representation
    ...
    """
    In `agent-driven content negotiation`_, the request has more than one possible
    response and the
    user agent or user should choose one of them. There is no standardized way for
    clients to
    ...
    temporarily.
    Further changes in the URI might be made in the future, so the same URI should be
    used by the
    ...
    response
    must be accessed by a proxy. It has been deprecated due to security concerns
    regarding in-band
    configuration of a proxy.

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Status#305_use_proxy>`_
    """

    UNUSED_306 = 306
    """
    This response code is no longer used; but is reserved. It was used in a previous
    version of the
    HTTP/1.1 specification.

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Status#306_unused>`_
    """
    ...
    another URI
    with the same method that was used in the prior request. This has the same semantics
    as the
    ``302 Found`` response code, with the exception that the user agent must not change
    the HTTP
    method used: if a `POST`_ was used in the first request, a ``POST`` must be used in
    the
    redirected request.
    ...
    the
    `Location`_ response header. This has the same semantics as the ``301 Moved
    Permanently`` HTTP
    response code, with the exception that the user agent must not change the HTTP
    method used: if a
    `POST`_ was used in the first request, a ``POST`` must be used in the second
    request.

    .. _Location: https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Location
    .. _POST: https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Methods/POST
    ...
    """
    The initial purpose of this code was for digital payment systems, however this
    status code is
    ...
    so the
    server is refusing to give the requested resource. Unlike ``401 Unauthorized``, the
    client's
    ...
    resource itself
    does not exist. Servers may also send this response instead of ``403 Forbidden`` to
    hide the
    existence of a resource from an unauthorized client. This response code is probably
    the most
    ...
    """
    The `request method`_ is known by the server but is not supported by the target
    resource. For
    example, an API may not allow ``DELETE`` on a resource, or the ``TRACE`` method
    entirely.

    .. _request method: https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Methods
    ...
    negotiation`_, doesn't find any content that conforms to the criteria given by the
    user agent.
    ...
    previous request
    by the client. It means that the server would like to shut down this unused
    connection. This
    ...
    In
    `WebDAV`_ remote web authoring, ``409`` responses are errors sent to the client so
    that a user
    ...
    server, with
    no forwarding address. Clients are expected to remove their caches and links to the
    resource.
    The HTTP specification intends this status code to be used for "limited-time,
    promotional
    services". APIs should not feel compelled to indicate resources that have been
    deleted with this
    status code.
    ...
    """
    Server rejected the request because the `Content-Length`_ header field is not
    defined and the
    server requires it.

    .. _Content-Length: https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Content-Length
    ...
    """
    In `conditional requests`_, the client has indicated preconditions in its headers
    which the
    server does not meet.

    .. _conditional requests: https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Guides/Conditional_requests
    ...
    .. _Retry-After: https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Retry-After
    ...
    """
    The `ranges`_ specified by the ``Range`` header field in the request cannot be
    fulfilled. It's
    ...
    """
    This response code means the expectation indicated by the `Expect`_ request header
    field cannot
    be met by the server.

    .. _Expect: https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Expect
    ...
    can be sent by
    a server that is not configured to produce responses for the combination of scheme
    and authority
    ...
    willing to do
    so after the client upgrades to a different protocol. The server sends an `Upgrade`_
    header in a
    ...
    """
    The origin server requires the request to be `conditional`_. This response is
    intended to
    prevent the 'lost update' problem, where a client `GET`_s a resource's state,
    modifies it and
    `PUT`_s it back to the server, when meanwhile a third party has modified the state
    on the
    server, leading to a conflict.

    .. _conditional: https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Guides/Conditional_requests
    .. _GET: https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Methods/GET
    .. _PUT: https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Methods/PUT
    ...
    methods that
    servers are required to support (and therefore must not return this code) are `GET`_
    and
    `HEAD`_.
    ...
    down for
    maintenance or that is overloaded. Note that together with this response, a user-
    friendly page
    explaining the problem should be sent. This response should be used for temporary
    conditions and
    the `Retry-After`_ HTTP header should, if possible, contain the estimated time
    before the
    ...
    chosen variant
    is configured to engage in content negotiation itself, which results in circular
    references when
    creating responses.
    ...
    """
    The client request declares an HTTP Extension (`RFC 2774`_) that should be used to
    process the
...
class HTTPHeader(str, Enum):
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Accept>`_
    """

    ACCEPT_CH = "accept-ch"
    """
    Servers can advertise support for Client Hints using the ``Accept-CH`` header field
    or
    an equivalent HTML ``<meta>`` element with `http-equiv`_ attribute.

    .. _http-equiv: https://developer.mozilla.org/en-
    US/docs/Web/HTML/Reference/Elements/meta/http-equiv

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Accept-CH>`_
    """
    ...
    .. _compression algorithm: https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Guides/Compression

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Accept-Encoding>`_
    """
    ...
    always pay attention not to override an explicit user choice (like selecting a
    language
    from a dropdown).

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Accept-Language>`_
    """

    ACCEPT_PATCH = "accept-patch"
    """
    A request content negotiation response header that advertises which `media type`_
    the
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Accept-Patch>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Accept-Post>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Accept-Ranges>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Access-Control-Allow-Credentials>`_
    """

    ACCESS_CONTROL_ALLOW_HEADERS = "access-control-allow-headers"
    """
    Used in response to a `preflight request`_ to indicate which HTTP headers can be
    used
    when making the actual request.

    .. _preflight request: https://developer.mozilla.org/en-
    US/docs/Glossary/Preflight_request

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Access-Control-Allow-Headers>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Access-Control-Allow-Methods>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Access-Control-Allow-Origin>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Access-Control-Expose-Headers>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Access-Control-Max-Age>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Access-Control-Request-Headers>`_
    """

    ACCESS_CONTROL_REQUEST_METHOD = "access-control-request-method"
    """
    Used when issuing a preflight request to let the server know which `HTTP method`_
    will
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Access-Control-Request-Method>`_
    """
    ...
    activate an existing permission for secure access and retry the request with
    cookies, or
    load a resource with cookie access if it already has an activated permission.

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Activate-Storage-Access>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Age>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Allow>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Alt-
    Svc>`_
    """
    ...
    """
    Used to indicate that the response corresponding to the current request is eligible
    to
    take part in attribution reporting, by registering either an attribution source or
    trigger.

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Attribution-Reporting-Eligible>`_
    """

    ATTRIBUTION_REPORTING_REGISTER_SOURCE = "attribution-reporting-register-source"
    """
    Included as part of a response to a request that included an ``Attribution-
    Reporting-
    Eligible`` header, this is used to register an attribution source.

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Attribution-Reporting-Register-Source>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Attribution-Reporting-Register-Trigger>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Authorization>`_
    """

    AVAILABLE_DICTIONARY = "available-dictionary"
    """
    A browser can use this request header to indicate the best dictionary it has
    available
    for the server to use for compression.

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Available-Dictionary>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Cache-Control>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Clear-Site-Data>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Connection>`_
    """
    ...
    .. _Content-Encoding: https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Content-Encoding
    .. _Content-Range: https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Content-Range

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Content-Digest>`_
    """

    CONTENT_DISPOSITION = "content-disposition"
    """
    Indicates if the resource transmitted should be displayed inline (default behavior
    without the header), or if it should be handled like a download and the browser
    should
    present a "Save As" dialog.

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Content-Disposition>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Content-Encoding>`_
    """

    CONTENT_LANGUAGE = "content-language"
    """
    Describes the human language(s) intended for the audience, so that it allows a user
    to
    differentiate according to the users' own preferred language.

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Content-Language>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Content-Length>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Content-Location>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Content-Range>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Content-Security-Policy>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Content-Security-Policy-Report-Only>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Content-Type>`_
    """
    ...
    .. _Set-Cookie: https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Set-Cookie

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Cookie>`_
    """
    ...
    .. _Accept-CH: https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Accept-CH
    .. _critical client hints: https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Guides/Client_hints#critical_client_hints

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Critical-CH>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Cross-Origin-Embedder-Policy>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Cross-Origin-Opener-Policy>`_
    """
    ...
    .. _CORP explainer article: https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Guides/Cross-Origin_Resource_Policy

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Cross-Origin-Resource-Policy>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Date>`_
    """
    ...
    .. _Sec-CH-Device-Memory: https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Sec-CH-Device-Memory

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Device-Memory>`_
    """

    DICTIONARY_ID = "dictionary-id"
    """
    Used when a browser already has a dictionary available for a resource and the server
    provided an ``id`` for the dictionary in the ``Use-As-Dictionary`` header. Requests
    for
    resources that can use the dictionary have an ``Available-Dictionary`` header and
    the
    server-provided dictionary ``id`` in the ``Dictionary-ID`` header.

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Dictionary-ID>`_
    """
    ...
    .. _Sec-GPC: https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Sec-GPC
    .. _navigator.globalPrivacyControl: https://developer.mozilla.org/en-
    US/docs/Web/API/Navigator/globalPrivacyControl

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/DNT>`_
    """

    DOWNLINK = "downlink"
    """
    Approximate bandwidth of the client's connection to the server, in Mbps. This is
    part of
    the `Network Information API`_ .

    .. _Network Information API: https://developer.mozilla.org/en-
    US/docs/Web/API/Network_Information_API

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Downlink>`_
    """
    ...
    .. _Sec-CH-DPR: https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Sec-CH-DPR

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/DPR>`_
    """

    ECT = "ect"
    """
    The `effective connection type`_ ("network profile") that best matches the
    connection's
    latency and bandwidth. This is part of the `Network Information API`_ .

    .. _effective connection type: https://developer.mozilla.org/en-
    US/docs/Glossary/Effective_connection_type
    .. _Network Information API: https://developer.mozilla.org/en-
    US/docs/Web/API/Network_Information_API

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/ECT>`_
    """
    ...
    .. _If-Match: https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/If-Match
    .. _If-None-Match: https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/If-None-Match

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/ETag>`_
    """

    EXPECT = "expect"
    """
    Indicates expectations that need to be fulfilled by the server to properly handle
    the
    request.

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Expect>`_
    """

    EXPECT_CT = "expect-ct"
    """
    Lets sites opt in to reporting and enforcement of `Certificate Transparency`_ to
    detect
    use of misissued certificates for that site.

    .. _Certificate Transparency: https://developer.mozilla.org/en-
    US/docs/Web/Security/Defenses/Certificate_Transparency

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Expect-CT>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Expires>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Forwarded>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/From>`_
    """

    HOST = "host"
    """
    Specifies the domain name of the server (for virtual hosting), and (optionally) the
    TCP
    port number on which the server is listening.

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Host>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/If-
    Match>`_
    """
    ...
    has
    been modified after the given date. This is used to transmit data only when the
    cache is
    out of date.
    ...
    """
    Makes the request conditional, and applies the method only if the stored resource
    doesn't match any of the given ETags. This is used to update caches (for safe
    requests),
    ...
    of
    a specific range with previous ones, or to implement an optimistic concurrency
    control
    ...
    .. _Subresource Integrity: https://developer.mozilla.org/en-
    US/docs/Web/Security/Defenses/Subresource_Integrity

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Integrity-Policy>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Integrity-Policy-Report-Only>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Keep-
    Alive>`_
    """
    ...
    environments. Conditional requests using `If-Modified-Since`_ and `If-Unmodified-
    Since`_
    ...
    .. _If-Modified-Since: https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/If-Modified-Since
    .. _If-Unmodified-Since: https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/If-Unmodified-Since

    `MDN Docs <https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Last-
    Modified>`_
    """
    ...
    .. _<link>: https://developer.mozilla.org/en-
    US/docs/Web/HTML/Reference/Elements/link

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Link>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Location>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Max-
    Forwards>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/NEL>`_
    """

    NO_VARY_SEARCH = "no-vary-search"
    """
    Specifies a set of rules that define how a URL's query parameters will affect cache
    matching. These rules dictate whether the same URL with different URL parameters
    should
    be saved as separate browser cache entries.

    `MDN Docs <https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/No-
    Vary-Search>`_
    """

    OBSERVE_BROWSING_TOPICS = "observe-browsing-topics"
    """
    Response header used to mark topics of interest inferred from a calling site's URL
    as
    observed in the response to a request generated by a `feature that enables the
    Topics
    API`_ .

    .. _feature that enables the Topics API: https://developer.mozilla.org/en-
    US/docs/Web/API/Topics_API/Using#what_api_features_enable_the_topics_api

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Observe-Browsing-Topics>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Origin>`_
    """

    ORIGIN_AGENT_CLUSTER = "origin-agent-cluster"
    """
    Response header used to indicate that the associated `Document`_ should be placed in
    an
    origin-keyed `agent cluster`_ . This isolation allows user agents to allocate
    implementation-specific resources for agent clusters, such as processes or threads,
    more
    efficiently.
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Origin-Agent-Cluster>`_
    """

    PERMISSIONS_POLICY = "permissions-policy"
    """
    Provides a mechanism to allow and deny the use of browser features in a website's
    own
    frame, and in `<iframe>`_ s that it embeds.

    .. _<iframe>: https://developer.mozilla.org/en-
    US/docs/Web/HTML/Reference/Elements/iframe

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Permissions-Policy>`_
    """
    ...
    request-
    response chain. Used for backwards compatibility with HTTP/1.0 caches where the
    ``Cache-
    Control`` header is not yet present.

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Pragma>`_
    """

    PREFER = "prefer"
    """
    Indicates preferences for specific server behaviors during request processing. For
    example, it can request minimal response content ( ``return=minimal`` ) or
    asynchronous
    processing ( ``respond-async`` ). The server processes the request normally if the
    header is unsupported.

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Prefer>`_
    """

    PREFERENCE_APPLIED = "preference-applied"
    """
    Informs the client which preferences specified in the ``Prefer`` header were applied
    by
    the server. It is a response-only header providing transparency about preference
    handling.

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Preference-Applied>`_
    """

    PRIORITY = "priority"
    """
    Provides a hint from about the priority of a particular resource request on a
    particular
    connection. The value can be sent in a request to indicate the client priority, or
    in a
    response if the server chooses to reprioritize the request.

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Priority>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Proxy-Authenticate>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Proxy-Authorization>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Range>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Referer>`_
    """

    REFERRER_POLICY = "referrer-policy"
    """
    Governs which referrer information sent in the `Referer`_ header should be included
    with
    requests made.

    .. _Referer: https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Referer

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Referrer-Policy>`_
    """

    REFRESH = "refresh"
    """
    Directs the browser to reload the page or redirect to another. Takes the same value
    as
    the ``meta`` element with `http-equiv="refresh"`_ .

    .. _http-equiv="refresh": https://developer.mozilla.org/en-
    US/docs/Web/HTML/Reference/Elements/meta/http-equiv

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Refresh>`_
    """

    REPORT_TO = "report-to"
    """
    Response header used to specify server endpoints where the browser should send
    warning
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Report-To>`_
    """
    ...
    receive errors such as CSP violation reports, `Cross-Origin-Opener-Policy`_ reports,
    or
    other generic violations.

    .. _Cross-Origin-Opener-Policy: https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Cross-Origin-Opener-Policy

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Reporting-Endpoints>`_
    """
    ...
    .. _digest: https://developer.mozilla.org/en-US/docs/Glossary/Hash_function
    .. _Content-Digest: https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Content-Digest
    .. _Content-Encoding: https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Content-Encoding
    .. _Content-Range: https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Content-Range

    `MDN Docs <https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Repr-
    Digest>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Retry-After>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/RTT>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Save-
    Data>`_
    """
    ...
    associated request, which are used by an ad tech platform to choose a personalized
    ad to
    display.

    `MDN Docs <https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Sec-
    Browsing-Topics>`_
    """

    SEC_CH_DEVICE_MEMORY = "sec-ch-device-memory"
    """
    Approximate amount of available client RAM memory. This is part of the `Device
    Memory
    API`_ .

    .. _Device Memory API: https://developer.mozilla.org/en-
    US/docs/Web/API/Device_Memory_API
    ...
    `MDN Docs <https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Sec-
    CH-Prefers-Reduced-Transparency>`_
    """
    ...
    """
    Indicates the request's destination. It is a Structured Header whose value is a
    token
    with possible values ``audio`` , ``audioworklet`` , ``document`` , ``embed`` ,
    ``empty``
    , ``font`` , ``image`` , ``manifest`` , ``object`` , ``paintworklet`` , ``report`` ,
    ``script`` , ``serviceworker`` , ``sharedworker`` , ``style`` , ``track`` ,
    ``video`` ,
    ``worker`` , and ``xslt`` .
    ...
    token with possible values ``cors`` , ``navigate`` , ``no-cors`` , ``same-origin`` ,
    and
    ``websocket`` .
    ...
    """
    Indicates the relationship between a request initiator's origin and its target's
    origin.
    ...
    """
    Indicates the "storage access status" for the current fetch context, which will be
    one
    ...
    retry the request, or to load a resource with access to its third-party cookies if
    the
    status is ``active`` .

    `MDN Docs <https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Sec-
    Fetch-Storage-Access>`_
    """

    SEC_FETCH_USER = "sec-fetch-user"
    """
    Indicates whether or not a navigation request was triggered by user activation. It
    is a
    Structured Header whose value is a boolean so possible values are ``?0`` for false
    and
    ``?1`` for true.
    ...
    """
    Indicates the purpose of the request, when the purpose is something other than
    immediate
    use by the user-agent. The header currently has one possible value, ``prefetch`` ,
    which
    ...
    """
    In requests, this header indicates the WebSocket extensions supported by the client
    in
    preferred order. In responses, it indicates the extension selected by the server
    from
    the client's preferences.

    `MDN Docs <https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Sec-
    WebSocket-Extensions>`_
    """

    SEC_WEBSOCKET_KEY = "sec-websocket-key"
    """
    Request header containing a key that verifies that the client explicitly intends to
    open
    a ``WebSocket`` .
    ...
    preferred order. In responses, it indicates the sub-protocol selected by the server
    from
    the client's preferences.

    `MDN Docs <https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Sec-
    WebSocket-Protocol>`_
    """

    SEC_WEBSOCKET_VERSION = "sec-websocket-version"
    """
    In requests, this header indicates the version of the WebSocket protocol used by the
    client. In responses, it is sent only if the requested protocol version is not
    supported
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Server>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Server-Timing>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Service-Worker>`_
    """

    SERVICE_WORKER_ALLOWED = "service-worker-allowed"
    """
    Used to remove the `path restriction`_ by including this header `in the response of
    the
    Service Worker script`_ .

    .. _path restriction: https://developer.mozilla.org/en-US/docs/Web/API/Service_Worke
    r_API/Using_Service_Workers#why_is_my_service_worker_failing_to_register
    .. _in the response of the Service Worker script:
    https://w3c.github.io/ServiceWorker/#service-worker-script-response

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Service-Worker-Allowed>`_
    """
    ...
    worker boot. The value, which is set with
    `NavigationPreloadManager.setHeaderValue()`_ ,
    can be used to inform a server that a different resource should be returned than in
    a
    normal ``fetch()`` operation.

    .. _fetch(): https://developer.mozilla.org/en-US/docs/Web/API/Window/fetch
    .. _NavigationPreloadManager.setHeaderValue(): https://developer.mozilla.org/en-
    US/docs/Web/API/NavigationPreloadManager/setHeaderValue

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Service-Worker-Navigation-Preload>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Set-
    Cookie>`_
    """

    SOURCEMAP = "sourcemap"
    """
    Links to a `source map`_ so that debuggers can step through original source code
    instead
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/SourceMap>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Strict-Transport-Security>`_
    """
    ...
    .. _Resource Timing API: https://developer.mozilla.org/en-
    US/docs/Web/API/Performance_API/Resource_timing

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Timing-Allow-Origin>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Trailer>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Transfer-Encoding>`_
    """
    ...
    client/server
    connection to a different protocol (over the same transport protocol). For example,
    it
    can be used by a client to upgrade a connection from HTTP 1.1 to HTTP 2.0, or an
    HTTP or
    HTTPS connection into a WebSocket.

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Upgrade>`_
    """
    ...
    .. _upgrade-insecure-requests: https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Content-Security-Policy/upgrade-insecure-requests

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Upgrade-Insecure-Requests>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Use-
    As-Dictionary>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/User-
    Agent>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Vary>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Via>`_
    """
    ...
    .. _Sec-CH-Viewport-Width: https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Sec-CH-Viewport-Width

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Viewport-Width>`_
    """
    ...
    .. _Content-Digest: https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Content-Digest
    .. _Want-Repr-Digest: https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Want-Repr-Digest

    `MDN Docs <https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Want-
    Content-Digest>`_
    """
    ...
    .. _Repr-Digest: https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Repr-Digest
    .. _Want-Content-Digest: https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Want-Content-Digest
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Warning>`_
    """
    ...
    .. _Sec-CH-Width: https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Sec-CH-Width

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Width>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/WWW-
    Authenticate>`_
    """
    ...
    .. _Content-Type: https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/Content-Type

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/X-Content-Type-Options>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/X-DNS-Prefetch-Control>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/X-Forwarded-For>`_
    """

    X_FORWARDED_HOST = "x-forwarded-host"
    """
    Identifies the original host requested that a client used to connect to your proxy
    or
    load balancer.

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/X-Forwarded-Host>`_
    """

    X_FORWARDED_PROTO = "x-forwarded-proto"
    """
    Identifies the protocol (HTTP or HTTPS) that a client used to connect to your proxy
    or
    load balancer.

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/X-Forwarded-Proto>`_
    """
    ...
    .. _<frame>: https://developer.mozilla.org/en-
    US/docs/Web/HTML/Reference/Elements/frame
    .. _<iframe>: https://developer.mozilla.org/en-
    US/docs/Web/HTML/Reference/Elements/iframe
    .. _<embed>: https://developer.mozilla.org/en-
    US/docs/Web/HTML/Reference/Elements/embed
    .. _<object>: https://developer.mozilla.org/en-
    US/docs/Web/HTML/Reference/Elements/object

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/X-Frame-Options>`_
    """
    ...
    restricted due to the `Same-Origin Policy`_ . The ``X-Permitted-Cross-Domain-
    Policies``
    header overrides such policy files so that clients still block unwanted requests.

    .. _Same-Origin Policy: https://developer.mozilla.org/en-
    US/docs/Web/Security/Defenses/Same-origin_policy

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/X-Permitted-Cross-Domain-Policies>`_
    """
    ...
    about
    them while not providing any usefulness to the application or its visitors. Unset
    this
    header to avoid exposing potential vulnerabilities.

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/X-Powered-By>`_
    """

    X_ROBOTS_TAG = "x-robots-tag"
    """
    The `X-Robots-Tag`_ HTTP header is used to indicate how a web page is to be indexed
    within public search engine results. The header is equivalent to `<meta
    name="robots">`_
    elements.

    .. _X-Robots-Tag: https://developers.google.com/search/docs/crawling-
    indexing/robots-meta-tag
    .. _<meta name="robots">: https://developer.mozilla.org/en-
    US/docs/Web/HTML/Reference/Elements/meta/name/robots

    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/X-Robots-Tag>`_
    """
    ...
    `MDN Docs <https://developer.mozilla.org/en-
    US/docs/Web/HTTP/Reference/Headers/X-XSS-Protection>`_
    """
