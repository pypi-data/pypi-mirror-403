...
MCP_TOOLS = [
    # Playwright MCP can be added later if needed
]
...
def create_client(
    project_dir: Path, model: str, resume_session_id: str | None = None
) -> ClaudeSDKClient:
    """
    ...
    options = ClaudeAgentOptions(
        model=model,
        system_prompt="You are an expert software testing engineer creating comprehensive integration tests for EvalVault Web UI. Follow TDD, Hexagonal Architecture, and YAGNI principles.",
        allowed_tools=[
            *BUILTIN_TOOLS,
            *MCP_TOOLS,
        ],
        mcp_servers={
            # MCP servers can be configured here if needed
        },
        hooks={
            "PreToolUse": [
