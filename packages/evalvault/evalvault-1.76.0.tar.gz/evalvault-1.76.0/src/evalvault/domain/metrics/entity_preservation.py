"""Entity preservation metric for summarization in insurance domain."""
...
class EntityPreservation:
    """Measure whether critical entities from contexts appear in the summary."""
    ...
    def __init__(self, *, keywords: Iterable[str] | None = None) -> None:
        self._extra_keywords = tuple(keywords or ())

    def score(self, answer: str, contexts: list[str]) -> float:
        """Return preservation ratio based on context entities."""
        if not contexts:
            return 0.0
        ...
        preserved = context_entities.intersection(summary_entities)
        return len(preserved) / len(context_entities)
    ...
    def _extract_keyword_entities(self, text: str) -> set[str]:
        ...
        for keyword in self._extra_keywords:
            key = keyword.strip()
            if not key:
                continue
            if key in text or key.lower() in lower:
                entities.add(f"kw:{key.lower()}")
    ...
    def _normalize_number(raw: str | None) -> str | None:
        ...
        try:
            ...
            return None
        return EntityPreservation._format_decimal(value)
