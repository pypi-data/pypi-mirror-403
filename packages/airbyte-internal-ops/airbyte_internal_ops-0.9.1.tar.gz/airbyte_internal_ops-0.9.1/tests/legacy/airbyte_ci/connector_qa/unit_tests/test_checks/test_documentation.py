...
from airbyte_ops_mcp._legacy.airbyte_ci.connector_qa.checks.documentation import (
    documentation,
)
from airbyte_ops_mcp._legacy.airbyte_ci.connector_qa.models import CheckStatus
...
class TestCheckMigrationGuide:
    def test_passed_when_no_breaking_changes(self, mocker):
        ...
        assert (
            "No breaking changes found. A migration guide is not required"
            in result.message
        )

    def test_fail_when_migration_guide_file_path_does_not_exists(
    ...
    def test_fail_when_migration_guide_file_does_not_start_with_correct_header(
        self, mocker, tmp_path
    ):
        # Arrange
        connector = mocker.Mock(
        ...
        assert result.status == CheckStatus.FAILED
        assert (
            "Migration guide file for test-connector does not start with the correct header"
            in result.message
        )
        assert "Expected '# Test Connector Migration Guide', got ''" in result.message

    def test_fail_when_migration_guide_file_has_missing_version_headings(
        self, mocker, tmp_path
    ):
        # Arrange
        connector = mocker.Mock(
        ...
        assert result.status == CheckStatus.FAILED
        assert (
            f"Migration guide file for {connector.name_from_metadata} has incorrect version headings"
            in result.message
        )
        assert (
            "Expected headings: ['## Upgrading to 2.0.0', '## Upgrading to 1.0.0']"
            in result.message
        )

    def test_fail_when_migration_guide_file_has_invalid_version_headings(
        self, mocker, tmp_path
    ):
        # Arrange
        connector = mocker.Mock(
    ...
    def test_fail_when_migration_guide_file_has_ascending_version_headings(
        self, mocker, tmp_path
    ):
        # Arrange
        connector = mocker.Mock(
    ...
    def test_fail_when_migration_guide_file_has_incorrect_version_headings(
        self, mocker, tmp_path
    ):
        # Arrange
        connector = mocker.Mock(
    ...
    def test_pass_when_migration_guide_file_has_correct_version_headings(
        self, mocker, tmp_path
    ):
        # Arrange
        connector = mocker.Mock(
...
class TestCheckDocumentationExists:
    def test_fail_when_documentation_file_path_is_none(self, mocker):
        # Arrange
        connector = mocker.Mock(
            technical_name="test-connector", documentation_file_path=None
        )
        ...
        assert result.status == CheckStatus.FAILED
        assert (
            "User facing documentation file is missing. Please create it"
            in result.message
        )
    ...
    def test_pass_when_documentation_file_path_exists(self, mocker, tmp_path):
        ...
        assert result.status == CheckStatus.PASSED
        assert (
            f"User facing documentation file {documentation_file_path} exists"
            in result.message
        )
...
class TestCheckDocumentationContent:
    def test_fail_when_documentation_file_path_does_not_exists(self, mocker, tmp_path):
        ...
        assert result.status == CheckStatus.FAILED
        assert (
            "Could not check documentation structure as the documentation file is missing"
            in result.message
        )
    ...
    def test_fail_when_documentation_file_has_invalid_links(
        self, connector_with_invalid_links_in_documentation
    ):
        # Act
        result = documentation.CheckDocumentationLinks()._run(
            connector_with_invalid_links_in_documentation
        )
    ...
    def test_fail_when_documentation_file_has_missing_headers(
        self, connector_with_invalid_documentation
    ):
        # Act
        result = documentation.CheckDocumentationHeadersOrder()._run(
        ...
        assert result.status == CheckStatus.FAILED
        assert (
            "Documentation headers ordering/naming doesn't follow guidelines:"
            in result.message
        )
        assert (
            "Actual Heading: 'For Airbyte Cloud:'. Expected Heading: 'Setup guide'"
            in result.message
        )
    ...
    def test_fail_when_documentation_file_not_have_all_required_fields_in_prerequisites_section_content(
        ...
        assert result.status == CheckStatus.FAILED
        assert (
            "Missing descriptions for required spec fields: github repositories"
            in result.message
        )
    ...
    def test_fail_when_documentation_file_has_invalid_source_section_content(
        ...
        # Act
        result = documentation.CheckSourceSectionContent()._run(
            connector_with_invalid_documentation
        )
        ...
        assert result.status == CheckStatus.FAILED
        assert (
            "Connector GitHub section content does not follow standard template:"
            in result.message
        )
        assert (
    ...
    def test_success_when_documentation_file_has_invalid_for_airbyte_cloud_section_content(
        self, connector_with_invalid_documentation
    ):
        # We now allow for more lenient matching of the expected content. Leaving the old test here for reference.
        # Act
        result = documentation.CheckForAirbyteCloudSectionContent()._run(
            connector_with_invalid_documentation
        )
    ...
    def test_fail_when_documentation_file_has_invalid_for_airbyte_open_section_content(
        ...
        # We now allow for more lenient matching of the expected content. Leaving the old test here for reference.
        # Act
        result = documentation.CheckForAirbyteOpenSectionContent()._run(
            connector_with_invalid_documentation
        )
    ...
    def test_fail_when_documentation_file_has_invalid_supported_sync_modes_section_content(
        ...
        # We now allow for more lenient matching of the expected content. Leaving the old test here for reference.
        # Act
        result = documentation.CheckSupportedSyncModesSectionContent()._run(
            connector_with_invalid_documentation
        )
    ...
    def test_fail_when_documentation_file_has_invalid_tutorials_section_content(
        ...
        # We now allow for more lenient matching of the expected content. Leaving the old test here for reference.
        # Act
        result = documentation.CheckTutorialsSectionContent()._run(
            connector_with_invalid_documentation
        )
    ...
    def test_fail_when_documentation_file_has_invalid_changelog_section_content(
        ...
        # Act
        result = documentation.CheckChangelogSectionContent()._run(
            connector_with_invalid_documentation
        )
        ...
        assert result.status == CheckStatus.FAILED
        assert (
            "Connector Changelog section content does not follow standard template:"
            in result.message
        )
        assert (
    ...
    def test_pass_when_documentation_file_has_correct_headers(
    ...
    def test_pass_when_documentation_file_has_correct_prerequisites_section_content(
        ...
        assert result.status == CheckStatus.PASSED
        assert (
            "All required fields from spec are present in the connector documentation"
            in result.message
        )
...
class TestCheckChangelogEntry:
    def test_fail_when_documentation_file_path_does_not_exists(self, mocker, tmp_path):
        ...
        assert result.status == CheckStatus.FAILED
        assert (
            "Could not check changelog entry as the documentation file is missing. Please create it."
            in result.message
        )
    ...
    def test_fail_when_documentation_file_doesnt_have_changelog_section(
        self, mocker, tmp_path
    ):
        # Arrange
        documentation_file_path = tmp_path / "documentation.md"
        ...
        assert result.status == CheckStatus.FAILED
        assert (
            "Connector documentation is missing a 'Changelog' section" in result.message
        )

    def test_fail_when_documentation_file_doesnt_have_changelog_entry(
        self, mocker, tmp_path
    ):
        # Arrange
        documentation_file_path = tmp_path / "documentation.md"
        ...
        assert result.status == CheckStatus.FAILED
        assert (
            "Connectors must have a changelog entry for each version: changelog entry for version 1.0.0"
            in result.message
        )
