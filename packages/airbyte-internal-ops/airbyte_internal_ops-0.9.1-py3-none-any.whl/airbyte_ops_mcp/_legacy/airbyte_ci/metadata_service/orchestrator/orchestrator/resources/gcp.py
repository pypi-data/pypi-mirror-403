...
from ..config import get_public_url_for_gcs_file
...
class ContentTypeAwareGCSFileManager(GCSFileManager):
    ...
    def write(
        self, file_obj, mode="wb", ext=None, key: Optional[str] = None
    ) -> PublicGCSFileHandle:
        """
    ...
    def delete_by_key(
        self, key: str, ext: Optional[str] = None
    ) -> Optional[PublicGCSFileHandle]:
        gcs_key = self.get_full_key(key + (("." + ext) if ext is not None else ""))
        bucket_obj = self._client.bucket(self._gcs_bucket)
        blob = bucket_obj.blob(gcs_key)
...
def gcs_file_blob(resource_context: InitResourceContext) -> storage.Blob:
    ...
    resource_context.log.info(
        f"retrieving gcs file blob {gcs_file_path} in bucket: {gcs_bucket}"
    )
...
def gcs_directory_blobs(
    resource_context: InitResourceContext,
) -> Union[List[storage.Blob], storage.Blob]:
    """
    ...
    resource_context.log.info(
        f"retrieving gcs file blobs for prefix: {prefix}, match_regex: {match_regex}, in bucket: {gcs_bucket}"
    )

    gcs_file_blobs = bucket.list_blobs(prefix=prefix)
    if match_regex:
        gcs_file_blobs = [
            blob for blob in gcs_file_blobs if re.match(match_regex, blob.name)
        ]

    if sort_key:
        gcs_file_blobs = sorted(
            gcs_file_blobs, key=lambda x: getattr(x, sort_key), reverse=reverse_sort
        )
