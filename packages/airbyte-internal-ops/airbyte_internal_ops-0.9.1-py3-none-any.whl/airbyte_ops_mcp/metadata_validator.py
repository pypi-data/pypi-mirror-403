...
Validator = Callable[
    [ConnectorMetadataDefinitionV0, ValidatorOptions], ValidationResult
]
...
def validate_metadata_images_in_dockerhub(
    ...
    oss_docker_image = get(
        metadata_definition_dict,
        "data.registryOverrides.oss.dockerRepository",
        base_docker_image,
    )
    oss_docker_version = get(
        metadata_definition_dict,
        "data.registryOverrides.oss.dockerImageTag",
        base_docker_version,
    )

    cloud_docker_image = get(
        metadata_definition_dict,
        "data.registryOverrides.cloud.dockerRepository",
        base_docker_image,
    )
    cloud_docker_version = get(
        metadata_definition_dict,
        "data.registryOverrides.cloud.dockerImageTag",
        base_docker_version,
    )

    normalization_docker_image = get(
        metadata_definition_dict,
        "data.normalizationConfig.normalizationRepository",
        None,
    )
    normalization_docker_version = get(
        metadata_definition_dict, "data.normalizationConfig.normalizationTag", None
    )

    breaking_changes = get(
        metadata_definition_dict, "data.releases.breakingChanges", None
    )
    ...
    if not validator_opts.prerelease_tag:
        possible_docker_images.extend(
            [(base_docker_image, version) for version in breaking_change_versions]
        )
...
def is_major_version(version: str) -> bool:
    ...
    return (
        ...
        and semver_version.patch == 0
        and semver_version.prerelease is None
    )
...
def validate_pypi_only_for_python(
    ...
    if "language:python" not in tags and "language:low-code" not in tags:
        return (
            False,
            "If pypi publishing is enabled, the connector must have a python language tag.",
        )
...
def check_is_dev_version(version: str) -> bool:
    ...
    return (
        parsed_version.prerelease is not None and "rc" not in parsed_version.prerelease
    )
...
def validate_rc_suffix_and_rollout_configuration(
    ...
    if docker_image_tag is None:
        return False, "The dockerImageTag field is not set."
    try:
        is_major_release_candidate_version = check_is_major_release_candidate_version(
            docker_image_tag
        )
        ...
        return (
            False,
            f"The dockerImageTag field is not a valid semver version: {docker_image_tag}.",
        )
