...
from untappd_scraper.beer import Beer, checkin_details, extract_checkin_elements
...
def test_real_checkin_purchased_and_location(checkin_purchased_html: HTML) -> None:
    """Use the preloaded `checkin_purchased_html` fixture (avoid file IO in tests)."""
    doc = checkin_purchased_html
    el = doc.find(".item", first=True)

    result = checkin_details(el)

    assert result.location == "New Brighton Hotel"
    assert result.location_id == 152916
    assert result.purchased_at == "The Taproom"
    assert result.purchased_id == 12500353
    assert result.serving == "Draft"
    assert result.user_rating == 4.1
    assert result.beer_label_url == "https://assets.untappd.com/site/beer_logos/beer-3484719_ce851_sm.jpeg"
    assert result.user_name == "mw1414"
