...
def test_user_lists_to_summary(sample_list: WebUserList) -> None:
    result = sample_list.to_summary()

    assert isinstance(result, WebUserListSummary)
    assert result.name == sample_list.name
