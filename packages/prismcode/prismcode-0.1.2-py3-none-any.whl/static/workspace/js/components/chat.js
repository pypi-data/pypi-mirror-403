...
export class PrismChat extends PrismComponent {
  constructor() {
    ...
    this.userScrolledAt = 0;         // Timestamp when user last scrolled up
  ...
  handleScroll() {
    ...
    // If user scrolled up AT ALL (more than 100px from bottom), disable immediately
    // No dead zone - user intent is clear
  ...
  onToolDone(data) {
    ...
    if (lastTool) {
      ...
      } else if (data.name === 'focus' || data.name === 'unfocus') {
        const resultEl = lastTool.querySelector('.tool-result');
  ...
  scrollDuringStream() {
    ...
    // Don't scroll if user recently took control (within 1.5 seconds)
    if (this.userScrolledAt && Date.now() - this.userScrolledAt < 1500) {
      return;
    }
  ...
  loadHistory(history) {
    ...
    for (const msg of history) {
      if (msg.role === 'user') this.addMessage('user', msg.content);
        ...
        } else if (msg.tool_name === 'edit_file') {
          // For reloaded history, we get line counts instead of full content
