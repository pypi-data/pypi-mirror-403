...
from .history import SessionHistory, list_sessions, get_histories_dir
...
from .context_management.tokens import ModelProfile, CharCounter
from .context_management.strategies.rolling_gist import RollingGist
from .signella import Signella
from .project import Project
from .project_manager import ProjectManager
from .filesystem import set_thread_filesystem, clear_thread_filesystem
...
class Agent:
    ...
    def __init__(
        ...
        self.history.metadata["model"] = model

        # New ground truth history manager
        history_path = _get_history_path(self.history.session_id)
        ...
        self.model_profile = ModelProfile.from_name(model)
        # Use 3.3 chars/token for more accurate counting (Claude averages ~3.3-3.5 for code)
        self.counter = CharCounter(chars_per_token=3.3)
        # Use 85% utilization to leave buffer for response and slight miscounting
        self.context_budget = self.model_profile.budget(utilization=0.85)
        ...
        if session_id and history_path.exists():
            ...
            # Consolidation only happens before sending messages (in _preflight_check).
            # This keeps session loading fast and responsive.
