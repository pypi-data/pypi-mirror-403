...
Mobius is an AI coding assistant that streams responses and executes file operations through tool calls. It wraps LLM API calls (via LiteLLM) with a real-time web interface.

- **Web UI** (`run_web.py`) - Flask + SocketIO real-time interface with session management, tool animations, and streaming responses

Think of it as a local Claude Code-style interface with dependency-aware context management.
...
│              Sessions, Streaming, Tool Display              │
└─────────────────────────┬───────────────────────────────────┘
                          │
┌─────────────────────────▼───────────────────────────────────┐
│                    Agent (core/agent.py)                    │
...
└───────────┬─────────────────────────────────┬───────────────┘
            │                                 │
┌───────────▼───────────┐       ┌─────────────▼─────────────┐
│   Tools (tools/*.py)  │       │  History Manager          │
│  • File operations    │       │  (core/context_management)│
│  • Prism dependency   │       │  • Ground truth           │
│  • Focus management   │       │  • Working history        │
│  • Bash execution     │       │  • Projections            │
└───────────────────────┘       └───────────────────────────┘
```
...
| File | Purpose |
|------|---------|
| `run_web.py` | Flask + SocketIO server - streaming, sessions, slash commands, tool display |
| `templates/index.html` | Main HTML template - sidebar, chat area, modals |
| `static/js/app.js` | MobiusApp class - SocketIO client, markdown, diff viewer, tool animations |
| `static/css/style.css` | Dark theme styles |
| `static/css/highlight.css` | Code syntax highlighting (GitHub dark) |

### Configuration

| File | Purpose |
|------|---------|
| `config.py` | Central config: tool list, system prompt, model, slash commands |
| `settings.py` | User preferences (theme, show_diff) persisted to `settings.json` |

## 4. Data Flow

```
Browser (app.js)                         Server (run_web.py)
      │                                         │
      │──── send_message ──────────────────────▶│
      │                                         │
      │                                    ┌────▼────┐
      ...
      │                                    └────┬────┘
      │                                         │
      │◀──── agent_delta (text chunks) ────────│
      │◀──── tool_start ───────────────────────│
      │◀──── tool_done ────────────────────────│──┐
      │◀──── agent_done ───────────────────────│  │ Loop until
      │                                         │  │ no more tools
      │                                    ┌────┴──▼──┐
      │                                    │  Execute │
      │                                    │   Tool   │
...
**Tool Animations (Web UI)**:
- Streaming: shimmer effect while receiving arguments
- Executing: pulsing glow while running  
- Done: green dot indicator
...
|-------|-----------|---------|
| `send_message` | Client → Server | User sends message |
| `cancel` | Client → Server | Stop generation |
| `agent_start` | Server → Client | Generation starting |
| `agent_delta` | Server → Client | Streaming text chunk |
| `agent_done` | Server → Client | Generation complete |
| `tool_progress` | Server → Client | Tool args streaming |
| `tool_start` | Server → Client | Tool execution starting |
| `tool_done` | Server → Client | Tool result ready |
| `focused_files_updated` | Server → Client | Focus bar update |

### Key Features

- **Stop Button / Escape** - Cancel ongoing generation
- **Message Queuing** - Type next message while agent working
- **Auto-scroll** - Smart scroll that doesn't fight user
- **Tool Animations** - Shimmer (streaming) → glow (executing) → dot (done)
- **Diff Viewer** - Inline before/after for file edits
- **Focus Bar** - Expandable list of focused files with line counts
- **Session Sidebar** - Load/create sessions, toggle diff mode

### CSS Design

- Dark theme with CSS variables
- Accent color: `--accent: #ff6b2b`
- No frameworks, efficient selectors
