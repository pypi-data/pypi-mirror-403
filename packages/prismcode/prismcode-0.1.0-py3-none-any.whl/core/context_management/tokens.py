...
class ModelProfile:
    ...
    def counter(self, use_tiktoken: bool = True) -> TokenCounter:
        """
        Get a token counter for this model.

        Args:
            use_tiktoken: If True, use accurate tiktoken counter.
                         If False (or tiktoken unavailable), use CharCounter.
        """
        if use_tiktoken:
            try:
                return TiktokenCounter(self.tokenizer)
            except ImportError:
                pass
        return CharCounter()

    def cached_counter(self, use_tiktoken: bool = True, maxsize: int = 1024) -> CachedCounter:
        """Get a cached token counter for this model."""
        return CachedCounter(self.counter(use_tiktoken), maxsize=maxsize)
