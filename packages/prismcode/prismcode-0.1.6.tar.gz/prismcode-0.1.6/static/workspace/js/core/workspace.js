...
const DEFAULT_LAYOUT = {
  sidebar: {
    ...
    tabs: [
      ...
      { id: 'focus', icon: '◉', title: 'Focus', component: 'focus', config: {} },
...
export class Workspace {
  ...
  render() {
      ...
      <div class="workspace ${this.layout.right?.collapsed === false ? 'right-open' : ''}" data-project="${this.currentProject}">
        ...
        <header class="toolbar">
          ...
          <div class="toolbar-actions">
            <button class="toolbar-btn" id="toggle-sidebar" title="Toggle Sidebar (Ctrl+B)">☰</button>
            <button class="toolbar-btn" id="toggle-bottom" title="Toggle Terminal (Ctrl+\`)">⌨</button>
            <button class="toolbar-btn" id="toggle-right" title="Toggle Focus Panel">◉</button>
          </div>
        </header>
        ...
        <footer class="statusbar">
          <div class="statusbar-item"><span class="statusbar-dot online"></span> Connected</div>
          <div class="statusbar-item" id="status-model">claude-opus</div>
          <div class="toolbar-spacer"></div>
          <div class="statusbar-item" id="status-session">Session: ---</div>
        </footer>
      </div>
    `;
    ...
    if (window.socket) {
      ...
      window.socket.on('agent_start', (data) => this.setTabProcessing(data.session_id, true));
      window.socket.on('agent_complete', (data) => this.setTabProcessing(data.session_id, false));
      window.socket.on('agent_cancelled', (data) => this.setTabProcessing(data.session_id, false));
      window.socket.on('agent_error', (data) => this.setTabProcessing(data.session_id, false));
  ...
  setTabProcessing(sessionId, isProcessing) {
    ...
    let tab = tabId ? this.layout.main.tabs.find(t => t.id === tabId) : null;
  ...
  async loadTokenStats() {
    try {
      ...
      const url = sessionId 
        ? `/api/context-stats?session_id=${sessionId}`
        : '/api/context-stats';
