...
# Custom Install command to download JDK after wheel installation
from setuptools.command.install import install

class InstallWithJDK(install):
    """Custom install command that downloads JDK after installation."""
    
    def run(self):
        # First, run the standard install process
        install.run(self)
        
        # Then download and extract JDK to the installed package location
        print("\n" + "="*70)
        print("Downloading and extracting JDK to installed package location...")
        print("="*70)
        try:
            download_and_extract_jdk()
            print("JDK successfully downloaded and installed!")
        except Exception as e:
            print(f"\nWarning: JDK download failed: {e}")
            print("The package will fall back to system Java when available.")
            print("You can manually download JDK later by running:")
            print("  python -c 'from eubi_bridge.utils.jvm_manager import download_jdk; download_jdk()'")
        print("="*70 + "\n")
...
setuptools.setup(
    ...
    cmdclass={'install': InstallWithJDK},
)
