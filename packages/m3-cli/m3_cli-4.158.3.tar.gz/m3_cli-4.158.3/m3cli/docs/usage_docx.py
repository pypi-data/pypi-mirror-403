...
def generate_docx(
    ...
    try:
        convert_md_to_docx(md_path, result_docx_file)
        _LOG.info(
    ...
    if not no_page_numbers:
        _LOG.info(
            f"Adding page numbers (format: {page_number_format}, align: {page_number_align})..."
        )
        try:
            add_page_numbers(
                docx_path=result_docx_file,
...
parser = argparse.ArgumentParser(
    description='Generate M3 CLI User Guide in DOCX format',
    ...
    epilog="""
        ...
        Page Header Examples:
          --header-text "M3 CLI – User Guide"
          --header-text "M3 CLI – User Guide" --header-align right
        
        Last Page Examples:
          --changelog-url "https://github.com/Maestro-Cloud-Control/m3-cli/blob/main/CHANGELOG.md"
          --changelog-path "C:\\path\\to\\CHANGELOG.md"
          --cli-version "m3, version 4.158.0"
          --no-last-page  (to skip adding the last page)
    """,
)
parser.add_argument(
...
parser.add_argument(
    ...
    help='Text for page header (e.g., "M3 CLI – User Guide")'
)
parser.add_argument(
