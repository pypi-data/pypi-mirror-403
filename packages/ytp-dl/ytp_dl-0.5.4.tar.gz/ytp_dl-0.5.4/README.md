...
* 🔒 **Privacy First** — Rotates Mullvad connection per download (fresh relay/IP) without leaving the host disconnected
...
| `YTPDL_JOB_BASE_DIR`        | Job workspace parent directory (auto-cleaned)                    | `/var/tmp/ytpdl_jobs`           |
| `YTPDL_STALE_JOB_TTL_S`     | Cleanup threshold for abandoned job directories (seconds)        | `21600` (6 hours)               |
| `YTPDL_MULLVAD_CMD_TIMEOUT_S` | Timeout for Mullvad CLI commands (seconds)                     | `30`                            |
| `YTPDL_MULLVAD_CONNECT_WAIT_S` | How long to wait for Mullvad to reach Connected (seconds)      | `25`                            |
| `YTPDL_YTDLP_TIMEOUT_S`     | Hung-process safety timeout for yt-dlp/ffmpeg pipeline (seconds) | `7200` (2 hours)                |
...
* ✅ Installs `ytp-dl==0.5.4` into the virtualenv
...
#   - Installs ytp-dl==0.5.4 + gunicorn + gevent in that venv
#   - Creates a simple systemd service ytp-dl-api.service on port 5000
#
# Mullvad connection rotation is handled per-job by downloader.py.
...
pip install "ytp-dl==0.5.4" gunicorn gevent
deactivate
...
# Use temp-friendly workspace; job dirs are cleaned up automatically.
YTPDL_JOB_BASE_DIR=/var/tmp/ytpdl_jobs
YTPDL_STALE_JOB_TTL_S=21600

# Timeouts (hung-process safety nets)
YTPDL_MULLVAD_CMD_TIMEOUT_S=30
YTPDL_MULLVAD_CONNECT_WAIT_S=25
YTPDL_YTDLP_TIMEOUT_S=7200
EOF
...
# NOTE: timeout=0 disables worker timeout; keep it high instead of infinite.
ExecStart=${VENV_DIR}/bin/gunicorn -k gevent -w 1 \\
  --worker-connections 200 --timeout 7200 --graceful-timeout 30 --keep-alive 20 \\
