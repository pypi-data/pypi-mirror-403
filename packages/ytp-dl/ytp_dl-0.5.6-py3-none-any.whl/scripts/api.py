...
def handle_download():
    ...
    try:
        ...
        # IMPORTANT:
        # - Do NOT release the semaphore before streaming finishes.
        # - Otherwise, multiple jobs can pile up and job dirs won't be deleted until
        #   each client finishes (or disconnects).
        def _cleanup() -> None:
            try:
                if job_dir:
        ...
        # If we error before creating a response, cleanup immediately.
        if job_dir:
            shutil.rmtree(job_dir, ignore_errors=True)
        _release_once()
