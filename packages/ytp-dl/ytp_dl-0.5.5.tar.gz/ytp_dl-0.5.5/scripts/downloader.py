...
YTDLP_TIMEOUT_S = int(os.environ.get("YTPDL_YTDLP_TIMEOUT_S", "7200"))  # hung-process safety net
...
def _run_argv_capture(argv: List[str], timeout_s: Optional[int] = None) -> Tuple[int, str]:
    try:
        ...
        # stdout may be str or bytes depending on Python version / invocation
        out = e.stdout or ""
        if isinstance(out, bytes):
            try:
                ...
                out = ""
        return 124, (out or "") + f"\n[ytp-dl] TIMEOUT after {timeout_s}s\n"
...
def mullvad_set_location(location: Optional[str] = None) -> None:
    if not _mullvad_present():
        return
...
def mullvad_reconnect_wait() -> None:
    """
    Rotate to a fresh Mullvad relay/IP without leaving the host in a disconnected state.
    This avoids "block when disconnected" behavior that can make port 5000 unreachable.

    Tries (in order):
      - mullvad reconnect --wait  (newer CLI)
      - mullvad reconnect         (then poll status)
      - mullvad connect           (then poll status)
    """
    if not _mullvad_present():
        return

    # Prefer: mullvad reconnect --wait
    rc, _ = _run_argv_capture(["mullvad", "reconnect", "--wait"], timeout_s=max(5, MULLVAD_CONNECT_WAIT_S + 5))
    if rc == 0:
        return

    # Fallback: reconnect (no --wait) then poll
    _run_argv(["mullvad", "reconnect"], check=False, timeout_s=MULLVAD_CMD_TIMEOUT_S)
    if mullvad_wait_connected(timeout=MULLVAD_CONNECT_WAIT_S):
        return

    # Last resort: connect then poll
    ...
    if not mullvad_wait_connected(timeout=MULLVAD_CONNECT_WAIT_S):
        raise RuntimeError("Mullvad reconnect failed (did not reach Connected state)")


def mullvad_rotate_ip(location: Optional[str] = None) -> None:
    require_mullvad_login()
    mullvad_set_location(location)
    mullvad_reconnect_wait()
...
def _extract_final_path(stdout: str, out_dir: str) -> Optional[str]:
    ...
    # Prefer existing candidate (reverse traversal)
...
def download_video(
    ...
    # Fresh relay/IP per job without disconnecting afterwards.
    # (Disconnect can trigger Mullvad lockdown/firewall and make the VPS unreachable.)
