...
from .cost_calculator import calculator
from .optimizer import optimizer
from .bigquery_client import get_bigquery_client, BigQueryClient
...
⚠️ _Connect to BigQuery for precise estimation (--project)_
...
async def list_tables(dataset: str | None = None) -> str:
    ...
    try:
        ...
        if not isinstance(client, BigQueryClient):
            return "❌ Connect to BigQuery to list tables. Provide --project."
...
async def describe_table(table_name: str) -> str:
    ...
    try:
        ...
        if not isinstance(client, BigQueryClient):
            return "❌ Connect to BigQuery to describe tables. Provide --project."
...
```
costlens-mcp --allow-execute --project YOUR_PROJECT
```
"""
        ...
        if not isinstance(client, BigQueryClient):
            return "❌ Connect to BigQuery to execute queries. Provide --project."
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  # Estimation only (safe mode) - uses ADC if available
  costlens-mcp --project my-project
  
  # With explicit key file
  costlens-mcp --project my-project --key-file key.json
  
  # With execution enabled
  costlens-mcp --project my-project --allow-execute
  
  # With cost limit
  costlens-mcp --project my-project --allow-execute --max-cost 5.0
  
Authentication:
  CostLens supports Application Default Credentials (ADC).
  If you've run `gcloud auth application-default login`, 
  you don't need --key-file.
"""
    )
    ...
    parser.add_argument('--key-file', help='Service account key file path (optional with ADC)', required=False)
