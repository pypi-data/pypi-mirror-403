...
from ReProver.prover.evaluate import evaluate
...
class TopkAccuracy(Metric):
    ...
    def update(self, batch_preds: List[List[str]], batch_gt: List[str]):
        ...
        for preds, gt in zip(batch_preds, batch_gt):
            ...
            self.correct += int(gt in preds[: self.k])
...
class RetrievalAugmentedGenerator(pl.LightningModule):
    ...
    def load(
        ...
        return cast(
            "RetrievalAugmentedGenerator",
            load_checkpoint(cls, ckpt_path, device, freeze),
        )
    ...
    def on_fit_start(self) -> None:
        ...
        if self.retriever is not None:
            self.retriever.load_corpus(cast(Any, self.trainer).datamodule.corpus)
    ...
    def validation_step(self, batch: Dict[str, Any], _) -> None:
        ...
        if self.logger is not None and hasattr(self.logger, "log_text"):
            getattr(self.logger, "log_text")(
                "preds_val", ["tactics"], [[msg]], step=self.global_step
            )
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        if self.retriever is not None:
            ...
            self.retriever.reindex_corpus(
                cast(Any, self.trainer).datamodule.eval_batch_size
            )
            assert self.retriever.corpus_embeddings is not None
            pickle.dump(
                IndexedCorpus(
            ...
            acc = evaluate(
                data_path=cast(Any, self.trainer).datamodule.data_path,
