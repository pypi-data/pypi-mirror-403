...
def checkCompatible(fileSEGYs, verbose=False):
    ...
    f0 = open(fileSEGYs[0])
    x0r, y0r, z0r, dxr, dyr, dzr, nxr, nyr, nzr, thetar, thetaDr = (
        getGridCharacteristics(f0)
    )
    ...
    for iseg in range(1, nfileSEGY):
        ...
        if (
            ...
            if nx == nxr and ny == nyr and nz == nzr:
                print(
                    "However, the count of grid nodes being similar, the difference is considered as minor."
                )
                print(
                    "The SEGYS are considered as compatible: the information of SEGY#1 is used"
                )
                return True
...
def create3DGrid(
    fileSEGYs, dblabel, topName=None, botName=None, limitZ=None, verbose=False
):
    """
    ...
    dbsegy3D = gl.DbGrid.create(
        nx=nx, x0=x0, dx=dx, angles=angles, flagAddCoordinates=False
    )
    ...
    for iseg in range(nfileSEGY):
        ...
        matred = mat[int(limitX[0]) : int(limitX[1]), :, :][
            :, int(limitY[0]) : int(limitY[1]), :
        ][:, :, int(limitZ[0]) : int(limitZ[1])]
