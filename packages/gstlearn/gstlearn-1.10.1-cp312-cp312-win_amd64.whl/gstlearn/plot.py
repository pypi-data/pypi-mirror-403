...
try:
    ...
    msg = (
        "Python dependency 'matplotlib' not found.\n"
...
def _getFirstElement(tab):
    ...
    """
    if (
        hasattr(tab, "__getitem__") and len(tab) > 0
    ):  # Check that `tab` is subscriptable and not empty
        first = tab[0]
        if (
            hasattr(first, "__getitem__") and len(first) > 0
        ):  # Check that `tab[0]` is also subscriptable
            return first[0]
        return first
...
def _getGridVariable(
    ...
    if shading == "nearest":
        ...
        print(
            "The argument shading should be either 'nearest' for cells centered on (x,y)"
    ...
    if label is None:
        if flagLabelDir:
            if vario.isDefinedForGrid():
                ...
                angles = gl.GeometryHelper.rotationGetAngles(
                    vario.getCodirs(idir), True
                )
...
def varmod(vario=None, model=None, ivar=-1, jvar=-1, *args, **kwargs):
    ...
    **kwargs : arguments passed to _ax_varmod.
    ...
    return _ax_varmod(
        axs, vario=vario, model=model, ivar=ivar, jvar=jvar, *args, **kwargs
    )
...
def _ax_varmod(
    ...
    asCov=False,
    flagDrawVariance=True,
    varioLinestyle="dashed",
    modelLinestyle="solid",
    varColor="black",
    varLinestyle="dotted",
    envColor="black",
    envLinestyle="dotted",
            ...
            for idirUtil in ndirUtil:
                ...
                if vario is not None:
                    __ax_varioElem(
                        ...
                        varColor=varColor,
                        varLinestyle=varLinestyle,
                        flagLabelDir=flagLabelDir,
    ...
    **kwargs,
    ...
    for i in range(modelobj.getNCov()):
        if modelobj.getCovName(i) == "Nugget Effect":
            istart = (
                1  # do not plot the first lag (h=0) for nugget effect (discontinuity)
            )
...
def symbol(db, nameColor=None, nameSize=None, *args, **kwargs):
    ...
    *args, **kwargs : arguments passed to _ax_symbol
    """
    ax = _getNewAxes()
    return _ax_symbol(ax, db, nameColor=nameColor, nameSize=nameSize, *args, **kwargs)
    ...
    **kwargs : arguments passed to matplotllib.pyplot.scatter
    ...
    if nameSize is not None:
        ...
        if flagCst:
            ...
            if sizvmax > sizvmin:
                sizval = np.where(
                    (sizval >= sizvmin) & (sizval <= sizvmax),
                    np.interp(sizval, [sizvmin, sizvmax], [sizmin, sizmax]),
                    s,
                )
    else:
        sizval = np.full(nb, s)
...
def _ax_gradient(
    ax, db, nameCoorX=None, nameCoorY=None, useSel=True, posX=0, posY=1, **kwargs
):
    """
    Construct a layer for plotting the gradient information of a data base
...
def _ax_polygon(
    ...
    **kwargs: arguments passed to matplotlib.fill
    ...
    for ipol in range(npol):
        ...
        ax.fill(x, y, facecolor=facecolor_local, edgecolor=edgecolor_local, **kwargs)
...
def _ax_sortedCurve(ax, tabx, taby, color="black", flagLegend=False, *args, **kwargs):
...
def _ax_curve(
    ax, data1, data2=None, icas=1, color0="black", flagLegend=False, **kwargs
):
    """
    Plotting the curve of an array (argument 'data1')
...
def _ax_multiSegments(
    ax, center, data, color="black", flagLegend=False, label="segments", *args, **kwargs
):
    """
...
def _ax_faults(ax, faults, color="black", flagLegend=False, label="segments", **kwargs):
    color = kwargs.setdefault("color", color)
    label = kwargs.setdefault("label", label)
...
def _ax_neigh(
    ax, neighobj, grid=None, node=0, flagCell=False, flagZoom=False, **kwargs
):
...
def _ax_neighWeights(
    ...
    **kwargs,
    ...
    if flagWeights:
        for i in range(nech):
            ax.annotate(
                round(100.0 * res.wgt.getValue(i, 0), 2),
                (X[i], Y[i]),
...
def sectionFromPoints(
    ...
    **kwargs,
    ...
    ax = _getNewAxes()
    return _ax_sectionFromPoints(
        ax, trace, points, ptype, colors, disc, flagUp, radius, *args, **kwargs
    )


def _ax_sectionFromPoints(
    ax,
    trace,
    points,
    ptype=gl.ELoc.Z,
...
def plot(object, name1=None, name2=None, ranks=None, **kwargs):
    ...
    if filetype == "Db":
        ...
        filetype == "DbMeshTurbo"
        or filetype == "DbMeshStandard"
        or filetype == "Mesh"
        or filetype == "MeshETurbo"
    ):
