...
def sliderFloat(title="", value=0.0, mini=0.0, maxi=100.0, step=1.0, eventhandler=None):
    object = ipw.FloatSlider(
        min=mini,
        max=maxi,
        step=step,
        value=value,
...
def text(title="", value="", placeholder=""):
    object = ipw.Text(
        value=value, placeholder=placeholder, description=title, disabled=False
    )
    return object
...
class WModel(ipw.VBox):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.dropType = dropDown(
            title="Type",
            options=models.keys(),
            value=defvalue,
            eventhandler=self.__update_value,
        )
        self.sliderRangeX = sliderInt(
            title="RangeX",
            value=defrange,
            mini=1,
            maxi=50,
        ...
        self.sliderAngle = sliderInt(
            title="Angle", value=0, mini=0, maxi=180, eventhandler=self.__update_value
        )
        self.sliderSill = sliderInt(
            title="Sill",
            value=defsill,
            mini=0,
            maxi=100,
        ...
        self.hb1 = ipw.HBox([self.sliderRangeX, self.sliderRangeY, self.sliderParam])
        self.hb2 = ipw.HBox((self.sliderAngle, self.sliderSill))
        self.changeCallback = changeCallback
        self.models = models
        self.monmodel = gl.Model.createFromParam(
            type=models[defvalue], range=defrange, sill=defsill, param=defparam
        )

        #        self.__update_value()
        self.observe(self.__update_children, names="value")

        super().__init__(children=[self.dropType, self.hb1, self.hb2], **kwargs)

    def __toModel(self):
        self.monmodel = gl.Model.createFromParam(
            type=self.models[self.dropType.value],
    ...
    def __toValue(self, myModel):
        ...
        self.value = "{},{},{},{},{},{}".format(
            type, rangeX, rangeY, param, angle, sill
        )
