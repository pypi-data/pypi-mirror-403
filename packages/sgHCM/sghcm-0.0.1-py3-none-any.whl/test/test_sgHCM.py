...
from sgHCM.sgHCM import SGHCM
from unittest.mock import patch
import sgHCM.sgHCM as sgHCM
...
def test_initialization():
    ...
    """
    model = SGHCM(C=2, max_iter=5, tol=0.1,
...
def test_modified_median_refine_all_violated(data):
    ...
    with patch.object(model, "_violates_hard_uniqueness", return_value=True), \
         patch.object(sgHCM, "candidate_score_weighted", fake_candidate_score_weighted):
...
def test_enforce_unique_prototypes(data):
    ...
    """
    model = SGHCM(C=2, enforce_unique=True)
    model.fit(data)
