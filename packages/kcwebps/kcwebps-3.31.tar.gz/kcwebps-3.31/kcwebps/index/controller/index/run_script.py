import kcwsqlite,os,time,hashlib,sys,kcwcache,subprocess
def md5(strs):
    m = hashlib.md5()
...
if __name__ == "__main__":
    ...
    if daemon:
        while True:
            # os.system(cmd)
            process = subprocess.Popen(cmds,cwd=data['paths'],stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
            while True:
                # 读取一行输出
