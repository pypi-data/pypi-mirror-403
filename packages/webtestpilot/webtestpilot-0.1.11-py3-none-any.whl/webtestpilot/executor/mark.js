window.markWidgets = (
  ...
  const { doHighlightElements, focusHighlightIndex, viewportExpansion, targetX, targetY, maxNearby } = opts;
   ...
   */
  // Map of highlight index -> absolute XPath (serializable)
  const HIGHLIGHTED_ELEMENTS_MAP = {};
  // Map of highlight index -> array of bounding boxes {top,left,width,height}
  const BBOX_MAP = {};
  // Map of highlight index -> text content
  const TEXT_MAP = {};
  ...
  // Separate cache for absolute XPath strings to avoid mixing with the relative/stopAtBoundary cache
  const xpathAbsoluteCache = new WeakMap();
  ...
  /**
   * Returns an absolute XPath string for an element.
   * Uses a fast-path for elements with an `id` and caches results in `xpathAbsoluteCache`.
   */
  function getAbsoluteXPath(element) {
    if (!element || element.nodeType !== Node.ELEMENT_NODE) return "";

    if (xpathAbsoluteCache.has(element)) return xpathAbsoluteCache.get(element);

    // Fast-path: if element has an id attribute, use an id-based XPath which is unique and efficient
    try {
      const id = element.getAttribute && element.getAttribute('id');
      if (id) {
        const xpath = `//*[@id="${id}"]`;
        xpathAbsoluteCache.set(element, xpath);
        return xpath;
      }
    } catch (e) {
      // ignore and continue to full absolute build
    }
    ...
    const result = '/' + segments.join('/');
    xpathAbsoluteCache.set(element, result);
    return result;
  }
  ...
  function handleHighlighting(nodeData, node, parentIframe, isParentHighlighted) {
    ...
    if (shouldHighlight) {
      ...
      if (nodeData.isInViewport || viewportExpansion === -1) {
        ...
        const rects = getCachedClientRects(node) || [];
        let iframeOffset = { x: 0, y: 0 };
        if (parentIframe) {
        ...
        if (node.tagName.toLowerCase() === 'input' || node.tagName.toLowerCase() === 'textarea') {
           textContent = node.value || node.getAttribute('placeholder') || "";
        ...
        if (hasTarget) {
          const firstRect = rects && rects[0];
          let distance = Infinity;
          if (firstRect) {
            // Calculate distance to closest edge of the bounding box
            const rectLeft = firstRect.left + iframeOffset.x;
            const rectTop = firstRect.top + iframeOffset.y;
            const rectRight = rectLeft + firstRect.width;
            const rectBottom = rectTop + firstRect.height;
            
            // Find closest point on rectangle to target point
            const closestX = Math.max(rectLeft, Math.min(targetX, rectRight));
            const closestY = Math.max(rectTop, Math.min(targetY, rectBottom));
          ...
          HIGHLIGHTED_ELEMENTS_MAP[nodeData.highlightIndex] = getAbsoluteXPath(node);
          BBOX_MAP[nodeData.highlightIndex] = bboxes;
          TEXT_MAP[nodeData.highlightIndex] = textContent;
  ...
  function buildDomTree(node, parentIframe = null, isParentHighlighted = false) {
    ...
    if (node === document.body) {
      const nodeData = {
        tagName: 'body',
        attributes: {},
        xpath: getAbsoluteXPath(document.body),
        children: [],
      };
  ...
  // Draw a circle at target coordinates if provided
  // let container = document.getElementById(HIGHLIGHT_CONTAINER_ID);
  // if (!container) {
  //   container = document.createElement("div");
  //   container.id = HIGHLIGHT_CONTAINER_ID;
  ...
  // targetCircle.className = "playwright-target-circle";
  // targetCircle.style.position = "fixed";
  // targetCircle.style.width = "50px";
  // targetCircle.style.height = "50px";
  // targetCircle.style.borderRadius = "50%";
  // targetCircle.style.border = "3px solid #FF0000";
  // targetCircle.style.backgroundColor = "rgba(255, 0, 0, 0.1)";
  // targetCircle.style.left = `${targetX - 25}px`; // Center the circle on the coordinates
  // targetCircle.style.top = `${targetY - 25}px`;
  // targetCircle.style.pointerEvents = "none";
  // targetCircle.style.zIndex = "2147483648"; // Higher than highlights
  // container.appendChild(targetCircle);

  buildDomTree(document.body);

  if (hasTarget && nearbyCandidates.length) {
    nearbyCandidates.sort((a, b) => a.distance - b.distance);
    console.log("nearby:", nearbyCandidates);
    const selected = nearbyCandidates.slice(0, Math.max(1, 10));
    for (const cand of selected) {
      HIGHLIGHTED_ELEMENTS_MAP[cand.index] = cand.xpath;
      BBOX_MAP[cand.index] = cand.bboxes;
      TEXT_MAP[cand.index] = cand.text;
      if (doHighlightElements) {
        highlightElement(cand.element, cand.index, cand.parentIframe);
      }
    }
  }
  ...
  // Returns:
  // 1. highlightedMap: Map of highlight index -> XPath string
  // 2. bboxMap: Map of highlight index -> array of bounding boxes {top,left,width,height}
  // 3. textMap: Map of highlight index -> text content
  return { 
    highlightedMap: HIGHLIGHTED_ELEMENTS_MAP, 
    bboxMap: BBOX_MAP, 
    textMap: TEXT_MAP 
  };
};
