...
class PersonaStorage:
    ...
    def get_outputs_for_linking(self) -> list[dict[str, str]]:
        ...
        # Add session
        if self._session:
            description = f"Session {self._session.session_id}: {self._session.problem}"
        ...
        # Add responses
        for response_id, response in self._responses.items():
            description = f"Response from {response.persona_id} (round {response.round_number}): {response.content}"
