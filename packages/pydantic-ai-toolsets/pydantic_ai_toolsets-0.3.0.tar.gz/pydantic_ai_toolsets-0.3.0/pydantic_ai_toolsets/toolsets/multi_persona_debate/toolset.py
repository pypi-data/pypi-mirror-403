...
def create_persona_debate_toolset(
    ...
    *,
    ...
    def _get_next_hint() -> str:
        ...
        if uncritiqued:
            return f"Use critique_position to challenge [{uncritiqued[0].position_id}]."
    ...
    def _read_debate_state() -> str:
        ...
        if _storage.personas:
            ...
            for persona in _storage.personas.values():
                lines.append(
                    f"  [{persona.persona_id}] {persona.name} ({persona.persona_type})"
                )
        ...
        if positions_by_round:
            ...
            for round_num in sorted(positions_by_round.keys()):
                ...
                for position in positions:
                    ...
                    parent_str = (
                        f" (defends [{position.parent_position_id}])"
                    ...
                    lines.append(
                        ...
                        f"{position.position_id} {persona_name}{parent_str}{evidence_str}"
                    )
                    lines.append(f"      {position.content}")
                    if position.critiques_addressed:
                        lines.append(
                            f"      Addresses critiques: {', '.join(position.critiques_addressed)}"
                        )
                lines.append("")
        ...
        if _storage.critiques:
            ...
            for round_num in sorted(critiques_by_round.keys()):
                ...
                for critique in critiques:
                    ...
                    target_ref = (
                        f"[{critique.target_position_id}]"
                        if target
                        else f"[{critique.target_position_id}] (missing)"
                    )
                    lines.append(
                        f"    [{critique.critique_id}] {persona_name} critiques {target_ref}"
                    )
                    lines.append(f"      {critique.content}")
                    if critique.specific_points:
                        lines.append("      Points:")
                        for point in critique.specific_points:
        ...
        if _storage.agreements:
            ...
            for round_num in sorted(agreements_by_round.keys()):
                ...
                for agreement in agreements:
                    ...
                    target_ref = (
                        f"[{agreement.target_position_id}]"
                        if target
                        else f"[{agreement.target_position_id}] (missing)"
                    )
                    lines.append(
                        f"    [{agreement.agreement_id}] "
                        f"{persona_name} agrees with {target_ref}"
                    )
                    lines.append(f"      {agreement.content}")
                    if agreement.reasoning:
                        lines.append("      Reasoning:")
                        for reason in agreement.reasoning:
    ...
    async def propose_position(position: ProposePositionItem) -> str:
        ...
        result = (
            f"Position [{position_id}] proposed by {persona.name} "
    ...
    async def critique_position(critique: CritiquePositionItem) -> str:
        ...
        result = (
            f"Critique [{critique_id}] created by {persona.name} "
            f"targeting position [{critique.target_position_id}] "
            f"in round {session.current_round}"
        )
    ...
    async def agree_with_position(agreement: AgreeWithPositionItem) -> str:
        ...
        result = (
            f"Agreement [{agreement_id}] created by {persona.name} "
            f"agreeing with {target_name}'s position [{agreement.target_position_id}] "
            f"in round {session.current_round}"
        )
    ...
    async def defend_position(defense: DefendPositionItem) -> str:
        ...
        result = (
            f"Position [{position_id}] defended by {persona.name} "
        ...
        for persona_id, persona in _storage.personas.items():
            try:
                ...
                prompt = (
                    ...
                    f"**CRITICAL**: Always use persona_id='{persona_id}' and the exact position_id shown above in all tool calls!"
                )
                result = await agent.run(prompt)
                results.append(f"{persona.name} round {round_num}: {str(result)}")
