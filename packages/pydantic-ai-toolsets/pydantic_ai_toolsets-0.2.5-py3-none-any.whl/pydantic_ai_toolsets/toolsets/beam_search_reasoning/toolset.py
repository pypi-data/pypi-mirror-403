...
def create_beam_toolset(
    ...
    *,
    ...
    def _get_next_hint() -> str:
        ...
        if unscored:
            return f"Use score_candidate on [{unscored[0].candidate_id[:8]}...] to evaluate quality."
        ...
        if expandable:
            ...
            return f"Use expand_candidate on [{best.candidate_id[:8]}...] to generate next steps, then prune_beam."
    ...
    async def read_beam() -> str:
        ...
        if not _storage.candidates:
                ...
                for step in sorted(_storage.steps, key=lambda s: s.step_index):
                    ...
                    for cid in step.candidate_ids:
                        ...
                        if c:
                            ...
                            lines.append(f"    [{cid[:8]}] score={score}{term}")
                lines.append("")
            ...
            for depth in sorted(by_depth.keys()):
                ...
                for c in candidates[:5]:  # Limit display
                    score = f"{c.score:.0f}" if c.score is not None else "?"
                    term = " ⭐" if c.is_terminal else ""
                    parent = f" ←[{c.parent_id[:8]}]" if c.parent_id else " (root)"
                    lines.append(f"    [{c.candidate_id[:8]}] {score}{term}{parent}")
                    lines.append(f"      {c.content[:80]}{'...' if len(c.content) > 80 else ''}")
                if len(candidates) > 5:
    ...
    async def create_candidate(candidate: CreateCandidateItem) -> str:
        ...
        result = f"Created [{candidate_id[:8]}] at step 0"
    ...
    async def expand_candidate(expand: ExpandCandidateItem) -> str:
        ...
        result = f"Expanded [{expand.candidate_id[:8]}] → {len(expand.expansions)} candidates at step {next_step}"
    ...
    async def score_candidate(score: ScoreCandidateItem) -> str:
        ...
        if score.candidate_id not in _storage.candidates:
            available = ", ".join([c.candidate_id[:8] for c in list(_storage.candidates.values())[:5]])
            return f"Error: Candidate '{score.candidate_id[:8]}...' not found. Available: [{available}]. Call read_beam."

        candidate = _storage.candidates[score.candidate_id]
        candidate.score = score.score

        result = f"Scored [{score.candidate_id[:8]}]: {score.score:.0f}/100"
    ...
    async def prune_beam(prune: PruneBeamItem) -> str:
        ...
        if step is None:
            available_steps = ", ".join([str(s.step_index) for s in _storage.steps[:5]])
                ...
                if best_path is None:
                    ...
                    for i, cid in enumerate(best_path):
                        ...
                        if c:
                            score = f"{c.score:.0f}" if c.score is not None else "?"
                            term = " ⭐" if c.is_terminal else ""
                            lines.append(f"{i+1}. [{cid[:8]}] {score}{term}")
                            lines.append(f"   {c.content[:100]}{'...' if len(c.content) > 100 else ''}")
                    result = "\n".join(lines)
...
def get_beam_system_prompt(storage: BeamStorageProtocol | None = None) -> str:
    ...
    if scored_list:
        ...
        for c in scored_list[:3]:
            term = " ⭐" if c.is_terminal else ""
            lines.append(f"- [{c.candidate_id[:8]}] {c.score:.0f}/100{term}")
