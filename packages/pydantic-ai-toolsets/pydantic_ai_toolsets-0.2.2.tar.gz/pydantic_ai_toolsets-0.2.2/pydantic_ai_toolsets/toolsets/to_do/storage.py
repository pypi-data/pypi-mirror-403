...
class TodoStorage:
    ...
    def get_state_summary(self) -> str:
        ...
        if self._todos:
            ...
            lines.append(f"  Latest: {latest_todo.content[:100]}..." if len(latest_todo.content) > 100 else f"  Latest: {latest_todo.content} [{latest_todo.status}]")
        return "\n".join(lines)
    ...
    def get_outputs_for_linking(self) -> list[dict[str, str]]:
        ...
        for i, todo in enumerate(self._todos):
            item_id = str(i)
            description = f"Todo #{i+1} [{todo.status}]: {todo.content[:100]}..." if len(todo.content) > 100 else f"Todo #{i+1} [{todo.status}]: {todo.content}"
