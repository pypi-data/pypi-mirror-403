    ...
    *,
    ...
    def _get_next_hint() -> str:
        ...
        if unanswered:
            ...
            if can_ask_sub:
                return f"Use ask_sub_question or answer_question on [{can_ask_sub[0].question_id[:8]}...] (depth {can_ask_sub[0].depth})."
            else:
                return f"Use answer_question on [{unanswered[0].question_id[:8]}...] (depth {unanswered[0].depth}, max depth reached)."
        ...
        if answered_questions:
            main_q = main_questions[0]
            return f"All questions answered. Use compose_final_answer for main question [{main_q.question_id[:8]}...]."
    ...
    async def read_self_ask_state() -> str:
        ...
        for depth in sorted(questions_by_depth.keys()):
            ...
            for question in questions:
                ...
                lines.append(f"    {status_icon} [{question.question_id[:8]}...]{main_str} - {question.question_text}")
                if question.parent_question_id:
                    parent = _storage.questions.get(question.parent_question_id)
                    parent_ref = (
                        f"[{question.parent_question_id[:8]}...]"
                        if parent
                        else f"[{question.parent_question_id[:8]}...] (missing)"
                    )
        ...
        if _storage.answers:
            ...
            for answer in _storage.answers.values():
                ...
                question_ref = (
                    f"[{answer.question_id[:8]}...]"
                    if question
                    else f"[{answer.question_id[:8]}...] (missing)"
                )
                confidence_str = (
                ...
                lines.append(f"  Answer [{answer.answer_id[:8]}...] for question {question_ref}{confidence_str}{followup_str}:")
        ...
        if _storage.final_answers:
            ...
            for final_answer in _storage.final_answers.values():
                ...
                main_ref = (
                    f"[{final_answer.main_question_id[:8]}...]"
                    if main_q
                    else f"[{final_answer.main_question_id[:8]}...] (missing)"
                )
                complete_str = " ✓ COMPLETE" if final_answer.is_complete else ""
                lines.append(f"  Final Answer [{final_answer.final_answer_id[:8]}...] for {main_ref}{complete_str}:")
                lines.append(f"    {final_answer.final_answer_text}")
                if final_answer.composed_from_answers:
                    answer_refs = ", ".join([f"[{aid[:8]}...]" for aid in final_answer.composed_from_answers[:5]])
                    if len(final_answer.composed_from_answers) > 5:
                        answer_refs += f" ... and {len(final_answer.composed_from_answers) - 5} more"
        ...
        if main_questions:
            ...
            for main_q in main_questions:
                lines.append(f"  Main: [{main_q.question_id[:8]}...] - {main_q.question_text}")
                ...
                def _print_tree(question_id: str, prefix: str = "    ", depth: int = 0) -> None:
                    ...
                    for i, child in enumerate(children):
                        ...
                        lines.append(f"{prefix}{connector}{status_icon} [{child.question_id[:8]}...] (depth {child.depth}) - {child.question_text}")
                        if not is_last:
    ...
    async def ask_main_question(item: AskMainQuestionItem) -> str:
        ...
        if existing_main:
            ...
            return (
                f"Main question already exists: [{main_q.question_id[:8]}...] - {main_q.question_text}. "
    ...
    async def ask_sub_question(item: AskSubQuestionItem) -> str:
        ...
        if item.parent_question_id not in _storage.questions:
            available = ", ".join([q.question_id[:8] for q in list(_storage.questions.values())[:5]])
            return (
                f"Error: Parent question '{item.parent_question_id[:8]}...' not found. "
        ...
        if parent_question.depth >= MAX_DEPTH:
            return (
                f"Error: Cannot create sub-question. Parent question [{item.parent_question_id[:8]}...] "
        ...
        result = (
            ...
            f"from parent [{item.parent_question_id[:8]}...]{depth_warning}: {item.sub_question_text}"
        )
    ...
    async def answer_question(item: AnswerQuestionItem) -> str:
        ...
        if item.question_id not in _storage.questions:
            available = ", ".join([q.question_id[:8] for q in list(_storage.questions.values())[:5]])
            return (
                f"Error: Question '{item.question_id[:8]}...' not found. "
                f"Available: [{available}]. Call read_self_ask_state."
            )
        ...
        result = (
            f"Answered question [{item.question_id[:8]}...] "
            f"with answer [{answer_id[:8]}...]{confidence_str}{followup_str}"
        )
    ...
    async def compose_final_answer(item: ComposeFinalAnswerItem) -> str:
        ...
        if item.main_question_id not in _storage.questions:
            available = ", ".join([q.question_id[:8] for q in list(_storage.questions.values())[:5]])
            return (
                f"Error: Main question '{item.main_question_id[:8]}...' not found. "
                f"Available: [{available}]. Call read_self_ask_state."
            )
        ...
        if not main_question.is_main:
            return (
                f"Error: Question [{item.main_question_id[:8]}...] is not a main question. "
        ...
        if missing_answers:
            return (
                f"Error: Some answer IDs not found: {[aid[:8] + '...' for aid in missing_answers[:3]]}. "
        ...
        result = (
            ...
            f"[{item.main_question_id[:8]}...] using {len(item.answer_ids_used)} answer(s)"
        )
    ...
    async def get_final_answer() -> str:
        ...
        lines: list[str] = [f"Final Answer: [{final_answer.final_answer_id[:8]}...]"]
        ...
        if final_answer.composed_from_answers:
            ...
            for answer_id in final_answer.composed_from_answers:
                ...
                if answer and question:
                    lines.append(f"  [{answer_id[:8]}...] - Answer to: {question.question_text}")
                    lines.append(f"    {answer.answer_text}")
                else:
                    lines.append(f"  [{answer_id[:8]}...] - (missing)")
            lines.append("")
