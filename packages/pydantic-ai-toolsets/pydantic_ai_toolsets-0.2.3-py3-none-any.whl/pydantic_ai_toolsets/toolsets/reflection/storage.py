...
class ReflectionStorage:
    ...
    def get_outputs_for_linking(self) -> list[dict[str, str]]:
        ...
        # Add outputs
        for output_id, output in self._outputs.items():
            description = f"Output {output_id} (cycle {output.cycle}): {output.content[:100]}..." if len(output.content) > 100 else f"Output {output_id} (cycle {output.cycle}): {output.content}"
            if output.is_final:
                description += " [FINAL]"
            linkable_items.append({"id": output_id, "description": description})
        # Add critiques
        for critique_id, critique in self._critiques.items():
            description = f"Critique {critique_id} for output {critique.output_id}: {critique.content[:100]}..." if len(critique.content) > 100 else f"Critique {critique_id} for output {critique.output_id}: {critique.content}"
