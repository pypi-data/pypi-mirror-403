...
class PersonaDebateStorage:
    ...
    def get_outputs_for_linking(self) -> list[dict[str, str]]:
        ...
        # Add session
        if self._session:
            description = f"Debate Session {self._session.session_id}: {self._session.problem[:100]}..." if len(self._session.problem) > 100 else f"Debate Session {self._session.session_id}: {self._session.problem}"
        ...
        # Add positions
        for position_id, position in self._positions.items():
            description = f"Position from {position.persona_id} (round {position.round_number}): {position.content[:100]}..." if len(position.content) > 100 else f"Position from {position.persona_id} (round {position.round_number}): {position.content}"
            linkable_items.append({"id": position_id, "description": description})
        # Add critiques
        for critique_id, critique in self._critiques.items():
            description = f"Critique from {critique.persona_id} (round {critique.round_number}): {critique.content[:100]}..." if len(critique.content) > 100 else f"Critique from {critique.persona_id} (round {critique.round_number}): {critique.content}"
            linkable_items.append({"id": critique_id, "description": description})
        # Add agreements
        for agreement_id, agreement in self._agreements.items():
            description = f"Agreement between {', '.join(agreement.persona_ids)} (round {agreement.round_number}): {agreement.content[:100]}..." if len(agreement.content) > 100 else f"Agreement between {', '.join(agreement.persona_ids)} (round {agreement.round_number}): {agreement.content}"
