    ...
    *,
    ...
    async def read_mcts() -> str:
        ...
        if not _storage.nodes:
            ...
            def display_node(node: MCTSNode, indent: str = "") -> None:
                ...
                lines.append(
                    f"{indent}[{node.node_id[:8]}] visits={node.visits} "
                    f"wins={node.wins:.1f} rate={rate}{ucb}{term}"
                )
                lines.append(
                    f"{indent}  {node.content[:70]}{'...' if len(node.content) > 70 else ''}"
                )
    ...
    async def select_node(select: SelectNodeItem) -> str:
        ...
        if select.node_id:
            if select.node_id not in _storage.nodes:
                ...
                return f"Error: Node '{select.node_id[:8]}...' not found. Available: [{available}]. Call read_mcts."
            result = f"Selected [{select.node_id[:8]}]"
        else:
            ...
            result = f"Selected [{current.node_id[:8]}] via {path_str}"
    ...
    async def expand_node(expand: ExpandNodeItem) -> str:
        ...
        if expand.node_id not in _storage.nodes:
            ...
            if not _storage.nodes:
                ...
                available = ", ".join([n.node_id[:8] for n in list(_storage.nodes.values())[:5]])
                return f"Error: Node '{expand.node_id[:8]}...' not found. Available: [{available}]. Call read_mcts."
        ...
        result = f"Expanded [{expand.node_id[:8]}] → {len(expand.children)} children"
    ...
    async def simulate(sim: SimulateItem) -> str:
        ...
        if sim.node_id not in _storage.nodes:
            available = ", ".join([n.node_id[:8] for n in list(_storage.nodes.values())[:5]])
            return f"Error: Node '{sim.node_id[:8]}...' not found. Available: [{available}]. Call read_mcts."
        ...
        result = f"Simulated [{sim.node_id[:8]}] reward={sim.simulation_result:.2f}, updated {nodes_updated} nodes"
    ...
    async def backpropagate(backprop: BackpropagateItem) -> str:
        ...
        if backprop.node_id not in _storage.nodes:
            available = ", ".join([n.node_id[:8] for n in list(_storage.nodes.values())[:5]])
            return f"Error: Node '{backprop.node_id[:8]}...' not found. Available: [{available}]. Call read_mcts."
        ...
        result = f"Backpropagated from [{backprop.node_id[:8]}] reward={backprop.reward:.2f}, updated {nodes_updated} nodes"
    ...
    async def get_best_action() -> str:
        ...
        if not root:
            ...
            if best_child is None:
                ...
                lines = [
                    f"Best action: [{best_child.node_id[:8]}]",
                    f"  Visits: {best_child.visits}",
                    f"  Win rate: {rate:.2%}",
                    f"  Content: {best_child.content[:100]}{'...' if len(best_child.content) > 100 else ''}",
                ]
                ...
                for cid, child in children:
                    if child:
                        ...
                        lines.append(
                            f"  [{cid[:8]}] v={child.visits} rate={c_rate:.2%}{star}"
                        )
...
def get_mcts_system_prompt(storage: MCTSStorageProtocol | None = None) -> str:
    ...
    if root:
        ...
        if root.children_ids:
            ...
            for cid, child in children[:3]:
                if child:
                    rate = child.wins / child.visits if child.visits > 0 else 0
                    lines.append(f"- [{cid[:8]}] v={child.visits} rate={rate:.2%}")
