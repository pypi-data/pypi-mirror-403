...
class CoTStorage:
    ...
    def get_state_summary(self) -> str:
        ...
        if self._thoughts:
            lines.append(f"  Latest: {self._thoughts[-1].thought[:100]}..." if len(self._thoughts[-1].thought) > 100 else f"  Latest: {self._thoughts[-1].thought}")
        return "\n".join(lines)
    ...
    def get_outputs_for_linking(self) -> list[dict[str, str]]:
        ...
        for thought in self._thoughts:
            ...
            description = f"Thought #{thought.thought_number}: {thought.thought[:100]}..." if len(thought.thought) > 100 else f"Thought #{thought.thought_number}: {thought.thought}"
