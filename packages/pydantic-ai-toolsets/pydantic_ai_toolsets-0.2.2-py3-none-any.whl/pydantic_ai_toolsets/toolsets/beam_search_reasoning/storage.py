...
class BeamStorage:
    ...
    def get_state_summary(self) -> str:
        ...
        if self._candidates:
            ...
            lines.append(f"  Best candidate: {best_candidate.content[:100]}..." if len(best_candidate.content) > 100 else f"  Best candidate: {best_candidate.content}")
        return "\n".join(lines)
    ...
    def get_outputs_for_linking(self) -> list[dict[str, str]]:
        ...
        # Add candidates
        for candidate_id, candidate in self._candidates.items():
            description = f"Candidate {candidate_id} (depth {candidate.depth}): {candidate.content[:100]}..." if len(candidate.content) > 100 else f"Candidate {candidate_id} (depth {candidate.depth}): {candidate.content}"
