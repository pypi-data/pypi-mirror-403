...
class EndfieldClient:
    ...
    def _parse_response(self, data: dict, status_code: int = 200) -> SignInResult:
        ...
        if code == 0:
            ...
            # Parse reward - could be direct "reward" object or "awardIds" + "resourceInfoMap"
            reward_str = None
            if result_data.get("reward"):
                reward_str = self._format_reward(result_data["reward"])
            elif result_data.get("awardIds") and result_data.get("resourceInfoMap"):
                # New format: list of IDs and a map of details
                try:
                    rewards = []
                    resource_map = result_data["resourceInfoMap"]
                    for award in result_data["awardIds"]:
                        award_id = award.get("id")
                        if award_id and award_id in resource_map:
                            item = resource_map[award_id]
                    ...
                    logger.warning(f"Failed to parse complex reward structure: {e}")

            return SignInResult(
                success=True,
                message="Sign-in successful!",
                days_signed=days_signed,
                today_reward=reward_str,
            )
