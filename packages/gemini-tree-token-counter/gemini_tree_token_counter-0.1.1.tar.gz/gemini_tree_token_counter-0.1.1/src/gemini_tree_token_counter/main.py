...
def tokenize(text: str) -> list[str]:
    """
    Tokenize the text using the combined regex pattern.
    Uses finditer to ensure full match strings are returned even if capture groups exist.
    """
    if not text: return []
...
def get_token_count(text: str) -> int:
    ...
    # Use finditer to avoid re.findall behavior with capture groups
...
IGNORE_DIRS = {
    ...
    'venv', 'env', '.venv', 'coverage', '.next', '.ds_store'
}
IGNORE_EXTS = {
    '.png', '.jpg', '.jpeg', '.gif', '.ico', '.svg', '.pdf', '.zip', '.tar',
    '.gz', '.pyc', '.exe', '.dll', '.so', '.dylib', '.lock', '.class', '.bin',
    '.woff', '.woff2', '.ttf', '.eot'
}
IGNORE_FILES = {
...
def scan_directory(path):
    ...
    for entry in entries:
        ...
        if entry in IGNORE_DIRS or entry in IGNORE_FILES: continue
        if any(entry.endswith(ext) for ext in IGNORE_EXTS): continue
