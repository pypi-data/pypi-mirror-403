...
#ifndef MINDSPORE_CCSRC_PLUGIN_DEVICE_GPU_KERNEL_CUDA_IMPL_CUDA_CLASS_ERFINV_HELPER_H_
#define MINDSPORE_CCSRC_PLUGIN_DEVICE_GPU_KERNEL_CUDA_IMPL_CUDA_CLASS_ERFINV_HELPER_H_
#include <memory>
#include <string>
#include <vector>
#include "kernel/gpu/cuda_impl/cuda_class/helper_base.h"
#include "kernel/gpu/cuda_impl/cuda_ops/erfinv_impl.cuh"

namespace mindspore {
namespace cukernel {
template <typename T>
class ErfinvHelperGpuKernel : public GpuKernelHelperBase {
 public:
  explicit ErfinvHelperGpuKernel(const std::string &kernel_name, const uint32_t &device_id)
      : GpuKernelHelperBase(kernel_name, device_id) {
    is_null_input_ = false;
  }

  virtual ~ErfinvHelperGpuKernel() = default;
...
#endif  // MINDSPORE_CCSRC_PLUGIN_DEVICE_GPU_KERNEL_CUDA_IMPL_CUDA_CLASS_ERFINV_HELPER_H_
