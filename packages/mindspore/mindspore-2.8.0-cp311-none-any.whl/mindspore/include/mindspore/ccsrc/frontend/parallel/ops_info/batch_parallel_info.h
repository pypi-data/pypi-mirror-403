...
class LstsqV2Info : public BatchParallelInfo {
 public:
  LstsqV2Info(const std::string &name, const Shapes &inputs_shape, const Shapes &outputs_shape,
              const PrimitiveAttrs &attrs)
      : BatchParallelInfo(name, inputs_shape, outputs_shape, attrs, std::make_shared<BatchParallelCost>()) {}
  ~LstsqV2Info() override = default;
  void ReComputeBatchSplitFlagList() override;
};
}  // namespace parallel
}  // namespace mindspore
