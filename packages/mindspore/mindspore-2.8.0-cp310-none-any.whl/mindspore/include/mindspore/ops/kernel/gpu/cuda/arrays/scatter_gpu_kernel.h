...
#ifndef MINDSPORE_CCSRC_PLUGIN_DEVICE_GPU_KERNEL_ARRAYS_SCATTER_GPU_KERNEL_H
#define MINDSPORE_CCSRC_PLUGIN_DEVICE_GPU_KERNEL_ARRAYS_SCATTER_GPU_KERNEL_H
...
class ScatterGpuKernelMod : public NativeGpuKernelMod {
 public:
  ScatterGpuKernelMod() {}
  ~ScatterGpuKernelMod() { FreeResource(); }
 ...
 protected:
  using ScatterFunc = std::function<bool(ScatterGpuKernelMod *, const std::vector<kernel::KernelTensor *> &,
 ...
 private:
  ScatterFunc kernel_func_;
  Reduce type_{Reduce::REDUCE_NONE};
  std::vector<std::pair<KernelAttr, ScatterFunc>> func_list_;
...
#endif  // MINDSPORE_CCSRC_PLUGIN_DEVICE_GPU_KERNEL_ARRAYS_SCATTER_GPU_KERNEL_H
