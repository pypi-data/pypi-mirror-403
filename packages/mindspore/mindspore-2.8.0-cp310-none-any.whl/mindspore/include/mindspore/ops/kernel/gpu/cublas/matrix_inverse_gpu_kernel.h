...
class MatrixInverseGpuKernelMod : public NativeGpuKernelMod {
 ...
 private:
  ...
  template <typename T>
  void LaunchKernel_Cublas(const std::vector<kernel::KernelTensor *> &inputs,
                           const std::vector<kernel::KernelTensor *> &workspace,
                           const std::vector<kernel::KernelTensor *> &outputs, void *stream_ptr);
  template <typename T>
  void LaunchKernel_CuSolve(const std::vector<kernel::KernelTensor *> &inputs,
                            const std::vector<kernel::KernelTensor *> &workspace,
                            const std::vector<kernel::KernelTensor *> &outputs, void *stream_ptr);
  using MatrixInverseFunc = std::function<bool(MatrixInverseGpuKernelMod *, const std::vector<kernel::KernelTensor *> &,
