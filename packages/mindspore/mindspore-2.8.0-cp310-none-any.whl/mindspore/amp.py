...
def all_finite(inputs):
    ...
    Args:
        inputs (Union(tuple(Tensor), list(Tensor))): An iterable Tensor.
...
class LossScaler(ABC):
    ...
    Derived class needs to implement all of its methods. During training, `scale` and `unscale` are used
