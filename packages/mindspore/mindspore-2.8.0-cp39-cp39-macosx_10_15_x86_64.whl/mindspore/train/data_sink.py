...
def data_sink(fn, dataset, sink_size=1, jit_config=None, input_signature=None):
    ...
    Args:
        ...
        sink_size (int, optional): Control the amount of data in each sink. `sink_size` must be positive integer.
            Default: ``1`` .
        jit_config (JitConfig, optional): Controls the execution mode(Graph mode/PyNative mode) of the generated
            function, and Jit config for compile. Default: ``None`` , means running in PyNative mode.
        input_signature (Union[Tensor, List or Tuple of Tensors], optional): The Tensor which describes the input
            arguments. The shape and dtype of the Tensor will be supplied to this function. If input_signature is
            specified, each input to `fn` must be a `Tensor`. And the input parameters of `fn` cannot accept
