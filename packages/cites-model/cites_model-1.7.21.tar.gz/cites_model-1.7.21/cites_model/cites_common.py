...
class CertifiedLegacyEnum(BaseEnum):
    CERTIFIED_1 = '1'
    CERTIFIED_2 = '2'
    CERTIFIED_3 = '3'
    CERTIFIED_4 = '4'
...
class IssuingPurposeLegacyEnum(BaseEnum):
    PURPOSE_1 = '1'
    PURPOSE_2 = '2'
    PURPOSE_3 = '3'
    PURPOSE_EMPTY = ''
...
class ConditionsLegacyType(BaseModel):
    certified: Optional[List[Optional[CertifiedLegacyEnum]]] = Field(
        default=None,
        description='Tímto se potvrzuje, že uvedené exempláře (podmínka 1 až 8)',
        examples=['2', '5', '7'])
    issuing_purpose: Optional[List[Optional[IssuingPurposeLegacyEnum]]] = Field(
        default=None,
...
class WorkflowListType(BaseListType):
    ...
    def load_dds_st02(self, data):
        # žádost o stanovisko k výjimce
        form = data['@form']
        if form != 'request':
            return self
        self.entries = []
        date_modified = get_dict_value(data, 'Date_Modified')
        modified_by_system = 'CN=Administrátor/O=CITES'
        s0_date_created = get_dict_value_datetime(data, 'Date_Created')
        s0_creator = get_dict_value(data, 'From')
        s1_date_authorized = get_dict_value_datetime(data, 'Date_Authorized')
        s1_authorized_by = get_dict_value(data, 'authorized_by')
        # s1_authorized = get_dict_value_bool(data, 'Authorized')
        s2_date_accepted = get_dict_value_datetime(data, 'Date_Accepted')
        s2_accepted_by = get_dict_value(data, 'Accepted_By')
        # s2_accepted = get_dict_value_bool(data, 'Accepted')
        # s2_is_accepted = get_dict_value_bool(data, 'isAccepted')
        s3_date_in_process = get_dict_value_datetime(data, 'Date_inProcess')
        # s3_in_process = get_dict_value_bool(data, 'pending')
        s3_in_process_by = get_dict_value(data, 'inProcessBy')
        s4_date_finished = get_dict_value_datetime(data, 'Date_statementsIssued')
        s4_finished_by = get_dict_value(data, 'statementsIssuedBy')
        s9_date_rejected = get_dict_value_datetime(data, 'Date_Rejected')
        s9_rejected_by = get_dict_value(data, 'Rejected_By')
        s9_rejected = get_dict_value_bool(data, 'rejected')

        status = get_dict_value(data, 'status')
        if (status[0] == '4') and (s4_date_finished is None):
            s4_date_finished = date_modified
            s4_finished_by = modified_by_system
        ...
        if '@responses' in data:
            ...
            for item in responses:
                if item['@form'] == 'accept':
                    s2_date_accepted = get_dict_value_datetime(data, 'Date_Accepted')
                    s2_accepted_by = get_dict_value(data, 'Accepted_By')
                elif item['@form'] == 'reject':
                    s9_date_rejected = get_dict_value_datetime(data, 'Date_Rejected')
                    s9_rejected_by = get_dict_value(data, 'Rejected_By')

        if s0_date_created is not None:
            cn, ldap_name = parse_ldap_name(s0_creator)
            w0 = WorkflowCitesType(
                node_code='0 - Zapsáno',
                node_name='0 - Zapsáno',
                executor=UserType(dn=deepcopy(ldap_name), name=deepcopy(cn)),
                date_execution=s0_date_created,
                status_from=None,
                status_to="1 - Autorizováno",
            )
            status_out = w0.node_code
            self.entries.append(w0)
        if s1_date_authorized is not None:
            cn, ldap_name = parse_ldap_name(s1_authorized_by)
            w1 = WorkflowCitesType(
                node_code='1 - Autorizováno',
                node_name='1 - Autorizováno',
                executor=UserType(dn=deepcopy(ldap_name), name=deepcopy(cn)),
                date_execution=s1_date_authorized,
                status_from='0 - Zapsáno',
                status_to="2 - Akceptováno",
            )
            status_out = w1.node_code
            self.entries.append(w1)
        if s2_date_accepted is not None:
            cn, ldap_name = parse_ldap_name(s2_accepted_by)
            w2 = WorkflowCitesType(
                node_code="2 - Akceptováno",
                node_name="2 - Akceptováno",
                executor=UserType(dn=deepcopy(ldap_name), name=deepcopy(cn)),
                date_execution=s1_date_authorized,
                status_from='1 - Autorizováno',
                status_to="3 - V řešení",
            )
            status_out = w2.node_code
            self.entries.append(w2)
        if s3_date_in_process is not None:
            cn, ldap_name = parse_ldap_name(s3_in_process_by)
            w3 = WorkflowCitesType(
                node_code="3 - V řešení",
                node_name="3 - V řešení",
                executor=UserType(dn=deepcopy(ldap_name), name=deepcopy(cn)),
                date_execution=s1_date_authorized,
                status_from="2 - Akceptováno",
                status_to="3 - V řešení",
            )
            status_out = w3.node_code
            self.entries.append(w3)
        if s9_rejected is not None:
            cn, ldap_name = parse_ldap_name(s9_rejected_by)
            sf = "9 - Odmítnuto"
            st = None
            if status[0] != '9':
                sf = "3 - V řešení"
            ...
            w9 = WorkflowCitesType(
                node_code="9 - Odmítnuto",
                node_name="9 - Odmítnuto",
                executor=UserType(dn=deepcopy(ldap_name), name=deepcopy(cn)),
                date_execution=s9_date_rejected,
                status_from=sf,
                status_to=st,
            )
            status_out = w9.node_code
            self.entries.append(w9)
        if s4_date_finished is not None:
            cn, ldap_name = parse_ldap_name(s4_finished_by)
            w4 = WorkflowCitesType(
                node_code="4 - Zpracováno",
                node_name="4 - Zpracováno",
                executor=UserType(dn=deepcopy(ldap_name), name=deepcopy(cn)),
                date_execution=s1_date_authorized,
                status_from="3 - V řešení",
                status_to=None,
            )
            status_out = w4.node_code
    ...
    def load_dds(self, data):
        ...
        form = data['@form']
        if form != 'request':
            return self.load_dds_st02(data)
        self.entries = []
...
class DocumentCodeEnum(BaseEnum):
    ...
    DD14 = 'DD14'   # Potvrzení o výjimce z obchodování (v2)
    DD15 = 'DD15'   # Souhrnná žádost o RL/Potvrzení o výjimku
    DD64 = 'DD64'   # Historické Potvrzení o výjimce z obchodování (v1)
