...
class E2TTS(VoiceCloningTTSBase):
    ...
    def _load_model(self, load_path):
        ...
        # default, which may be read-only in some environments (e.g. sandboxed runs).
        # Point it at a writable directory under the model weights folder.
        try:
            cache_base = Path(load_path)
            if cache_base.is_file():
                cache_base = cache_base.parent
            cache_dir = cache_base / ".numba_cache"
            cache_dir.mkdir(parents=True, exist_ok=True)
            os.environ.setdefault("NUMBA_CACHE_DIR", str(cache_dir))
        except Exception:
            # If this fails, librosa/numba may still work; don't block model load here.
            pass
