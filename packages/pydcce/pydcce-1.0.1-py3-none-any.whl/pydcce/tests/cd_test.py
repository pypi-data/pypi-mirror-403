...
class CDTest:
    ...
    def _compute_rho_matrix(self) -> np.ndarray:
        ...
        # Handle missing values: fill with row means first
        for i in range(self.N):
            row = e[i, :]
            row_mean = np.nanmean(row)
            if np.isnan(row_mean):
                row_mean = 0
            e[i, np.isnan(row)] = row_mean
    ...
    def _cd_weighted(
        ...
        """
        # Pre-process data: handle missing values
        e_base = self.residuals.copy()
        for i in range(self.N):
            row = e_base[i, :]
            row_mean = np.nanmean(row)
            if np.isnan(row_mean):
                row_mean = 0
            e_base[i, np.isnan(row)] = row_mean
        ...
        for _ in range(reps):
            ...
            e_weighted = e_base * weights[:, np.newaxis].T
            
            # Demean weighted residuals
            e_weighted = e_weighted - np.mean(e_weighted, axis=1, keepdims=True)
