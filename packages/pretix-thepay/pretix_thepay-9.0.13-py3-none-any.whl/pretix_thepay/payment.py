...
class ThePay(BasePaymentProvider):
    ...
    def _create_payment(self, payment: OrderPayment, order: Order, merchant_id: str,
        ...
        try:
            ...
            payment.info_data['payment_uid'] = payment_uid
            if result.get('detail_url'):
                payment.info_data['detail_url'] = result.get('detail_url')
            if result.get('pay_url'):
                payment.info_data['pay_url'] = result.get('pay_url')
    ...
    def payment_refund_supported(self, payment: OrderPayment) -> bool:
        ...
        return bool(info.get('payment_uid'))
    ...
    def execute_refund(self, refund: OrderRefund):
        ...
        if not payment_uid:
            raise PaymentException(_('Missing The Pay payment UID.'))
        ...
        try:
            ...
            if not self._is_refund_visible(refund_info_before, refund_info_after, amount_int, str(reason)):
                raise PaymentException(_('Refund request was not confirmed by The Pay.'))
