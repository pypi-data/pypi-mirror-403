...
class Monthline(TushareJob):
    """
    描述:
    - 获取A股月线行情数据
    - 每月最后一个交易日更新, 本接口是未复权行情
    - 用户需要至少2000积分才可以调取
    - API文档: https://tushare.pro/document/2?doc_id=145
    - SCALE: CrossSection & Individual
    - TYPE: Partitioned
    - PAGINATE: 4500 rows / 1000 pages

    属性:
    - name: str, 作业名称 'monthline'。
    - key: str, 作业键 '/tushare/monthline'。
    - session: Session, Tushare会话对象。
    - source: TableInfo, 源表信息（Tushare原始格式）。
    - target: TableInfo, 目标表信息（转换后格式）。
    - params: Params, 查询参数。
        - ts_code: str, 可选, 股票代码
        - trade_date: str, 可选, 交易日期（YYYYMMDD）每月最后一个交易日
        - start_date: str, 可选, 开始日期（YYYYMMDD）
        - end_date: str, 可选, 结束日期（YYYYMMDD）
    - coolant: Coolant, 请求冷却控制。
    - paginate: Paginate, 分页控制（pagesize=4500, pagelimit=1000）。
    - retry: Retry, 重试策略。
    - cache: Cache, 缓存管理。

    方法:
    - run(): 执行作业，返回月线行情DataFrame。
    - _run(): 内部执行逻辑，处理日期参数。
    ...
    ```python
        ...
        from xfintech.data.source.tushare.stock.monthline import Monthline
