...
# Exported constants
NAME = "monthline"
KEY = "/tushare/monthline"
PAGINATE = {
    "pagesize": 4500,
    "pagelimit": 1000,
}
SOURCE = TableInfo(
    desc="A股月线行情数据（Tushare格式）",
    meta={
        "provider": PROVIDER,
        "source": SOURCE_NAME,
        "url": URL,
    ...
    columns=[
        ...
        ColumnInfo(name="open", kind=ColumnKind.FLOAT, desc="月开盘价"),
        ColumnInfo(name="high", kind=ColumnKind.FLOAT, desc="月最高价"),
        ColumnInfo(name="low", kind=ColumnKind.FLOAT, desc="月最低价"),
        ColumnInfo(name="close", kind=ColumnKind.FLOAT, desc="月收盘价"),
        ColumnInfo(name="pre_close", kind=ColumnKind.FLOAT, desc="上月收盘价"),
        ColumnInfo(name="change", kind=ColumnKind.FLOAT, desc="月涨跌额"),
        ColumnInfo(name="pct_chg", kind=ColumnKind.FLOAT, desc="月涨跌幅"),
        ColumnInfo(name="vol", kind=ColumnKind.FLOAT, desc="月成交量(手)"),
        ColumnInfo(name="amount", kind=ColumnKind.FLOAT, desc="月成交额(千元)"),
    ],
)
TARGET = TableInfo(
    desc="A股月线行情数据（xfintech格式）",
    meta={
        "key": KEY,
        "name": NAME,
        "type": "partitioned",
