...
class CustomApiClient(ApiClient):
    ...
    def forward_auth(self, headers):
        ...
        if 'x-api-key' in headers:
            return self.create_auth_settings('apiKeyAuth', headers.get('x-api-key'))
        
        if 'x-recaptcha-token' in headers:
            return self.create_auth_settings('captchaApiKey', headers.get('x-recaptcha-token'))
    ...
    def create_auth_settings(self, scheme, value):
        ...
        if 'apiKeyAuth' == scheme:
          conf = Configuration()
          conf.api_key['apiKeyAuth'] = value
          return conf.auth_settings()['apiKeyAuth']
        
        if 'captchaApiKey' == scheme:
          conf = Configuration()
          conf.api_key['captchaApiKey'] = value
          return conf.auth_settings()['captchaApiKey']
...
def create_manage_rfi_api(configurations=None, client_settings=None):
    """
    ...
    return ManageRfiApi(custom_api_client)
