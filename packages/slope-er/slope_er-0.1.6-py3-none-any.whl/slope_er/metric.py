...
def ser(input1, input2, boolflag=False):
  ...
  if (isinstance(input1, (float, int)) and isinstance(input2, (float, int))):
    if (0 <= input1 <= 1) and (0 <= input2 <= 1):
      ...
      raise ValueError("Float/integer inputs must be between 0 and 1.")
  # Case 2: input1 and input2 are 1D numpy arrays of the same length
    ...
    # New validation: Check if all values in NumPy arrays are either 0 or 1
    if not np.all(((input1 == 0) | (input1 == 1))):
      raise ValueError("All values in input1 (NumPy array) must be either 0 or 1.")
    if not np.all(((input2 == 0) | (input2 == 1))):
      raise ValueError("All values in input2 (NumPy array) must be either 0 or 1.")
