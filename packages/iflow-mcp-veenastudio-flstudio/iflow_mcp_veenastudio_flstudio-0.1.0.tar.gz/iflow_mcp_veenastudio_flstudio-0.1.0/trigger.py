...
# Try to open MIDI port, but allow running without it for testing
output_port = None
try:
    # Check if we're in a test environment (no MIDI hardware)
    if os.environ.get('MCP_TEST_MODE') == 'true':
        print("Running in test mode - MIDI port will be simulated")
        output_port = None
    else:
        output_port = mido.open_output('loopMIDI Port 2')
except Exception as e:
    print(f"Warning: Could not open MIDI port: {e}")
    print("Running in simulated mode - MIDI operations will be logged but not sent")
    output_port = None
...
def play():
    ...
    if output_port:
        ...
        print("Simulated: Sent Play command (note 60)")
...
def stop():
    ...
    if output_port:
        ...
        print("Simulated: Sent Stop command (note 61)")
...
def send_midi_note(note, velocity=1, duration=0.01):
    ...
    if output_port:
        ...
        print(f"Simulated: Sent MIDI note {note} (velocity={velocity})")
