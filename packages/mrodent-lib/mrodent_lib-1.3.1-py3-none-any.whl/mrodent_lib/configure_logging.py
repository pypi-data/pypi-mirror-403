...
def configure_logging(*args, maxBytes = 1024 * 1024):
    ...
    if len(args) == 1:
        if type(args[0]) is not str:
            raise Exception(f'configure_logging func was passed with arg which was not str: {args[0]}, type {type(args[0])}')
        logger_name = args[0]
    ...
    basic_logger.info(f'+++ PROJECT_NAME {PROJECT_NAME}')
    basic_logger.info(f'+++ logger_name {logger_name}')
...
typically something which will let the user know prominently that this configuration failed.        
    ...
    basic_logger.info(f'+++ log_file_path {log_file_path}')
...
EnhancedColourFormatter.formatTime() to add the milliseconds to this value as produced 
    ...
    if len(args) == 1:
        msg = """
Passing a parameter to configure_logging is deprecated: 
Instead, call configure_logging() with no parameters"""
