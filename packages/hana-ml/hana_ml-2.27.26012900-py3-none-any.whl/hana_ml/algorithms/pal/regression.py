...
class PolynomialRegression(PALBase):
    ...
    2   3  2.5
    3   4  3.5
    >>> pr.predict(data=df_predict, key='ID').collect()
       ID      VALUE
    0   1   6.157063
    1   2   8.401269
    2   3  15.668581
...
class GLM(PALBase):
    ...
    >>> glm.fit(data=df_train, key='ID', label='Y')

    Perform predict():
    >>> glm.predict(data=df_predict, key='ID').collect()
    ...
    def fit(self,# pylint: disable=too-many-arguments
        ...
        param_rows = [
            ...
            ('GROUP_RESPONSE', len(label) == 2 if isinstance(label, list) else None,
             None, None),
    ...
    def predict(self, # pylint: disable=too-many-arguments
        ...
        prediction_type : {'response', 'link'}, optional
            Specifies the type of predictors to output.

            - 'response' : output the direct response. It is the default setting.
            - 'link' : output the linear predictor, which is equivalent
              to direct response transformed by the corresponding link function.
        ...
        DataFrame
            ...
            The following five columns are only populated when ``solver`` is 'irls' in the training phase:
...
class ExponentialRegression(PALBase):
    ...
    def fit(self, data, key=None, features=None, label=None):
        ...
        Returns
        -------
        A fitted object of class "ExponentialRegression".
        """
        # pylint: disable=too-many-locals
...
class BiVariateNaturalLogarithmicRegression(PALBase):
    ...
    def fit(self, data, key=None, features=None, label=None):
        ...
        Returns
        -------
        A fitted object of class "BiVariateNaturalLogarithmicRegression".
        """
        # pylint: disable=too-many-locals
...
class CoxProportionalHazardModel(PALBase):
    ...
    Examples
    --------
    >>> cox = CoxProportionalHazardModel(significance_level= 0.05, calculate_hazard='yes', type_kind='risk')
    >>> cox.fit(data=df_train, key='ID', features=['X1', 'X2'], label='TIME')

    Perform predict():
    >>> cox.predict(data=df_predict, key='ID',features=['X1', 'X2']).collect()
