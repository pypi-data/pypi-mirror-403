...
flask_endpoint_to_restrict = {}


def get_token_from_request(request_token):
    return request_token.split(" ")[1]
...
def validate_roles_and_capabilities(token, end_point):
    if end_point in flask_endpoint_to_restrict:
        decoded_token = decode_jwt(token)

        exception_msg = "User does not have required {} for operation {}"
        expected_roles = flask_endpoint_to_restrict[end_point].get("roles", [])
        expected_capabilities = flask_endpoint_to_restrict[end_point].get("capabilities", [])
