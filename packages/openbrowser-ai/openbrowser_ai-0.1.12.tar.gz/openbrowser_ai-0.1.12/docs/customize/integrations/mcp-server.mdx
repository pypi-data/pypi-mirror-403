...
The MCP (Model Context Protocol) Server allows you to expose openbrowser's browser automation capabilities to AI assistants like Claude Desktop, Cline, and other MCP-compatible clients. This enables AI assistants to perform web automation tasks directly through openbrowser.
...
You can configure openbrowser through environment variables:

- `OPENAI_API_KEY` - Your OpenAI API key (required)
- `ANTHROPIC_API_KEY` - Your Anthropic API key (alternative to OpenAI)
- `OPENBROWSER_HEADLESS` - Set to `false` to show browser window
- `OPENBROWSER_DISABLE_SECURITY` - Set to `true` to disable browser security features
...
Claude will use the MCP server to execute these tasks through openbrowser.
