...
DEFAULT_JSON = Path(
    os.environ.get("LINKDB_JSON", _PROJECT_ROOT / "data" / "entries.json")
)
DEFAULT_DB = Path(os.environ.get("LINKDB_DB", _PROJECT_ROOT / "data" / "linkdb.db"))
...
LINK_INDEX = """
CREATE INDEX IF NOT EXISTS idx_link_entry_id ON link(entry_id)
"""
...
def init_db(db_path: Path) -> sqlite3.Connection:
    ...
    conn.execute(LINK_SCHEMA)
    conn.execute(LINK_INDEX)
    conn.commit()
    _migrate_db(conn)
    return conn
...
def _migrate_db(conn: sqlite3.Connection) -> None:
    ...
    # Ensure link table and index exist (for migration of older databases)
    cursor.execute(LINK_SCHEMA)
    cursor.execute(LINK_INDEX)
    conn.commit()
...
class Entry:
    ...
    links: list["Link"] = field(default_factory=list)  # Associated links
...
@dataclass
class Link:
    """A link associated with an entry (article, tutorial, video, etc.)."""
    ...
    VALID_TYPES = {"article", "tutorial", "video", "docs", "discussion"}
    ...
    @classmethod
    def validate_type(cls, link_type: Optional[str]) -> tuple[bool, Optional[str]]:
        """Validate link type. Returns (is_valid, error_message)."""
        if link_type is None:
            return True, None
        if link_type not in cls.VALID_TYPES:
            valid_types = ", ".join(sorted(cls.VALID_TYPES))
            return False, f"Invalid link_type '{link_type}'. Valid types: {valid_types}"
        return True, None
...
def _import_links_for_entry(
    ...
    """Import links for an entry. Returns count of links imported."""
    link_count = 0
    for link_data in links:
        link_url = link_data.get("url")
        if not link_url:
            errors.append(f"{entry_name}: link missing url")
            continue
        link_type = link_data.get("link_type")
        is_valid, err = Link.validate_type(link_type)
        if not is_valid:
            errors.append(f"{entry_name}: {err}")
            continue
        cursor.execute(
            """INSERT INTO link (entry_id, url, title, link_type, note)
               VALUES (?, ?, ?, ?, ?)""",
            (
                entry_id,
                link_url,
            ...
            if existing:
                if skip_duplicates:
                    ...
                    if not dry_run:
                        ...
                        # Delete existing links and re-import
                        cursor.execute("DELETE FROM link WHERE entry_id = ?", (entry_id,))
                        if links:
                            _import_links_for_entry(cursor, entry_id, links, errors, name)
                    imported += 1
            else:
                if not dry_run:
                    cursor.execute(
...
def export_to_json(
    ...
    with get_connection(db_path) as conn:
        ...
        # Fetch all links grouped by entry_id
        cursor.execute("SELECT * FROM link ORDER BY entry_id, id")
        link_rows = cursor.fetchall()
        links_by_entry: dict[int, list[dict[str, Any]]] = defaultdict(list)
        for link_row in link_rows:
            link = Link.from_row(link_row)
            links_by_entry[link.entry_id].append(link.to_dict())
    ...
    for row in rows:
        ...
        entry_id = row["id"]
        if entry_id in links_by_entry:
            entry["links"] = links_by_entry[entry_id]
...
def generate_readme(
    ...
    with get_connection(db_path) as conn:
        ...
        cursor.execute("SELECT * FROM link ORDER BY entry_id, id")
        link_rows = cursor.fetchall()
        links_by_entry: dict[int, list[Link]] = defaultdict(list)
        for link_row in link_rows:
            link = Link.from_row(link_row)
    ...
    for cat_name in sorted(groups.keys()):
        ...
        for row in sorted(groups[cat_name], key=lambda r: r["name"].lower()):
            ...
            # Add relevant links if present
            entry_id = row["id"]
            if entry_id in links_by_entry:
                section_lines.append("  - Relevant Links:\n")
                for link in links_by_entry[entry_id]:
                    link_title = link.title or link.url
                    type_tag = f" [{link.link_type}]" if link.link_type else ""
                    note_text = f" - {link.note}" if link.note else ""
                    section_lines.append(
                        f"    - [{link_title}]({link.url}){type_tag}{note_text}\n"
                    )
...
def _sync_links_to_json(
    conn: sqlite3.Connection,
    entry_id: int,
    entry_name: str,
    json_path: Optional[Path] = None,
) -> None:
    """Sync links for an entry from DB to JSON after CLI operations."""
    json_file = json_path or DEFAULT_JSON
    ...
    link_rows = cursor.fetchall()
    links = [Link.from_row(row).to_dict() for row in link_rows]
...
def cmd_add_link(args: argparse.Namespace) -> int:
    ...
    link_type = getattr(args, "type", None)
    note = getattr(args, "note", None)

    # Validate link_type
    is_valid, err = Link.validate_type(link_type)
    if not is_valid:
        cprint(f"Error: {err}", Color.RED)
        return 1
    ...
    with init_db(db_file) as conn:
        ...
        # Find entry by name
        cursor.execute("SELECT id FROM entry WHERE name = ?", (entry_name,))
        row = cursor.fetchone()
        if not row:
            cprint(f"Error: Entry '{entry_name}' not found", Color.RED)
            return 1
        ...
        cursor.execute(
            "SELECT id FROM link WHERE entry_id = ? AND url = ?",
            (entry_id, link_url),
        )
        existing = cursor.fetchone()
        if existing:
            cprint(f"Error: Link '{link_url}' already exists for entry '{entry_name}'", Color.RED)
            return 1
        ...
        cursor.execute(
            """INSERT INTO link (entry_id, url, title, link_type, note)
               VALUES (?, ?, ?, ?, ?)""",
            (entry_id, link_url, title, link_type, note),
        )
        conn.commit()

        # Sync to JSON
        _sync_links_to_json(conn, entry_id, entry_name, json_file)

    cprint(f"Added link to '{entry_name}'", Color.GREEN)
    return 0
...
def cmd_remove_link(args: argparse.Namespace) -> int:
    ...
    with init_db(db_file) as conn:
        ...
        existing = cursor.fetchone()
        if not existing:
            cprint(f"Error: Link '{link_url}' not found for entry '{entry_name}'", Color.RED)
            return 1

        # Delete link
        cursor.execute(
            "DELETE FROM link WHERE entry_id = ? AND url = ?",
            (entry_id, link_url),
        )
        conn.commit()
    ...
    cprint(f"Removed link from '{entry_name}'", Color.GREEN)
    return 0
...
def cmd_list_links(args: argparse.Namespace) -> int:
    ...
    db_file = Path(args.db) if args.db else DEFAULT_DB
    entry_name = getattr(args, "entry", None)
    ...
    with get_connection(db_file) as conn:
        ...
        if entry_name:
            ...
            if output_format == "json":
                links = [Link.from_row(row).to_dict(include_id=True) for row in link_rows]
                print(json.dumps({"entry": entry_name, "links": links}, indent=2))
            else:
                if not link_rows:
                    print(f"No links found for '{entry_name}'")
                else:
                    print(f"Links for '{entry_name}':")
                    for link_row in link_rows:
                        link = Link.from_row(link_row)
                        type_str = f" [{link.link_type}]" if link.link_type else ""
                        title_str = f" - {link.title}" if link.title else ""
                        note_str = f" ({link.note})" if link.note else ""
                        print(f"  {link.url}{type_str}{title_str}{note_str}")
        else:
            # List all links grouped by entry
            cursor.execute("""
                SELECT e.name, l.* FROM link l
                JOIN entry e ON l.entry_id = e.id
            ...
            if output_format == "json":
                ...
                for row in link_rows:
                    ...
                    links_by_entry[entry].append(link.to_dict(include_id=True))
                print(json.dumps(links_by_entry, indent=2))
            else:
                if not link_rows:
...
def main() -> int:
    ...
    # add-link
    p = subparsers.add_parser("add-link", help="Add a link to an entry")
    p.add_argument("entry", help="Entry name")
    p.add_argument("url", help="Link URL")
    p.add_argument("-t", "--title", help="Link title")
    p.add_argument(
        "--type",
        choices=sorted(Link.VALID_TYPES),
        help="Link type (article, tutorial, video, docs, discussion)",
    )
    p.add_argument("-n", "--note", help="Note about the link")
    p.add_argument("-j", "--json", help="Path to JSON file")
    p.add_argument("--db", help="Path to SQLite database")
    p.set_defaults(func=cmd_add_link)

    # remove-link
    p = subparsers.add_parser("remove-link", help="Remove a link from an entry")
    p.add_argument("entry", help="Entry name")
    p.add_argument("url", help="Link URL to remove")
    p.add_argument("-j", "--json", help="Path to JSON file")
    p.add_argument("--db", help="Path to SQLite database")
    p.set_defaults(func=cmd_remove_link)

    # list-links
    p = subparsers.add_parser("list-links", help="List links for entries")
    p.add_argument("entry", nargs="?", help="Entry name (omit for all entries)")
    p.add_argument(
        "-f",
        "--format",
        choices=["table", "json"],
