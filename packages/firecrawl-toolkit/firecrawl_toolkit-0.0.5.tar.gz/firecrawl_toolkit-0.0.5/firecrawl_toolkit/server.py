...
def main():
    ...
    # Helper to start a blocking mcp.run(...) in a daemon thread
    def _start_mcp_transport_in_thread(transport: str, host: str, port: int) -> threading.Thread:
        def _target():
            try:
                logger.info("启动 MCP transport=%s on %s:%s", transport, host, port)
                # This call is blocking and runs the transport server loop
                mcp.run(transport=transport, host=host, port=port)
            except Exception:
                logger.exception("MCP transport %s 在后台线程启动失败", transport)

        t = threading.Thread(target=_target, name=f"mcp-{transport}", daemon=True)
        t.start()
        return t
    ...
    try:
        ...
        logger.info("Firecrawl MCP 工具服务已启动（异步资源初始化完成）。")

        # Decide which transports to enable. Defaults: enable SSE and HTTP, keep STDIO enabled.
        enable_sse = _env_enabled("FIRECRAWL_MCP_ENABLE_SSE", True)
        enable_http = _env_enabled("FIRECRAWL_MCP_ENABLE_HTTP", True)

        sse_host = os.getenv("FIRECRAWL_MCP_SSE_HOST", "127.0.0.1")
        sse_port = int(os.getenv("FIRECRAWL_MCP_SSE_PORT", "8000"))
        http_host = os.getenv("FIRECRAWL_MCP_HTTP_HOST", "127.0.0.1")
        http_port = int(os.getenv("FIRECRAWL_MCP_HTTP_PORT", "8001"))

        # Start SSE / HTTP transports in background threads if enabled. Keep stdio in main thread.
        background_threads = []
        if enable_sse:
            try:
                background_threads.append(_start_mcp_transport_in_thread("sse", sse_host, sse_port))
            except Exception:
                logger.exception("尝试启动 SSE transport 失败")

        if enable_http:
            try:
                background_threads.append(_start_mcp_transport_in_thread("http", http_host, http_port))
            except Exception:
                logger.exception("尝试启动 HTTP transport 失败")

        logger.info("同时启用传输: stdio (主线程) %s, sse=%s(%s:%s), http=%s(%s:%s)",
                    "enabled", enable_sse, sse_host, sse_port, enable_http, http_host, http_port)

        # Now run the original stdio transport in the main thread to preserve backward compatibility.
        # Note: mcp.run() is blocking and will keep the process alive until interrupted.
        mcp.run()
