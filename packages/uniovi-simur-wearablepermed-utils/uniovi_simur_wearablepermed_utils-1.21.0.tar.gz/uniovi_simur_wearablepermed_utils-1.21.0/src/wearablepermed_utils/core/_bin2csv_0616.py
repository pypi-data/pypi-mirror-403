...
def read_one_package_raw_date(allFileDataBuff:bytearray, onePackageData, readFileQueue):
    ...
    while(1):
        ...
        if startRecnizOffset == -1:
            try:
                startRecnizOffset = allFileDataBuff.index(PACKAGE_HEARD_KEY)
            except:
                break

        try:
            endRecnizOffset = allFileDataBuff.index(PACKAGE_HEARD_KEY, 1)
            onePackageData.extend(allFileDataBuff[0:endRecnizOffset])

            debugInfo('endRecnizOffset:' + str(endRecnizOffset) + ' allFileDataBuff size:' + str(len(allFileDataBuff)))
...
def bin2csv(bin_file, csv_file):  # 建立一个任务线程类:  # 在启动线程后任务从这个函数里面开始执行
    ...
    try:
        ...
        (remarkes,) = sFileRemarkes.unpack(allFileDataBuff[0:sFileRemarkes.size])
    ...
    for j in range(0, headerPackeNum):
        ...
        # 检查CRC32
        calcCrc32 = binascii.crc32(onePackageData[len(recString)+4:])
        ...
        maxCount = max(rawDataSizeAcc, rawDataSizeGyro,rawDataSizeTemper, rawDataSizeHeart)
        ...
        timeAmongStep = ((itermEndTimeStamp - itermStartTimeStamp)*1000)/(maxCount)
        ...
        # 解析原始数据
        # Acc
        sGetRawData = struct.Struct(str(accByteSize)+'s'+str(GyroByteSize)+'s'+str(TemperByteSize)+'s'+str(heartByteSize)+'s')

        try:
            (accRawData, gyroRawData, temperRawData,heartRawData) = sGetRawData.unpack(onePackageData[0:])
        ...
        for i in range(0, rawDataSizeAcc):
            ...
            saveFileOnePackge[accIndex]['acc_x'] = calcAccGryro(saveFileOnePackge[accIndex]['acc_x'], accRange)
            saveFileOnePackge[accIndex]['acc_y'] = calcAccGryro(saveFileOnePackge[accIndex]['acc_y'], accRange)
            saveFileOnePackge[accIndex]['acc_z'] = calcAccGryro(saveFileOnePackge[accIndex]['acc_z'], accRange)
        ...
        for i in range(0, rawDataSizeGyro):
            ...
            saveFileOnePackge[gyroIndex]['gyr_x'] = calcAccGryro(saveFileOnePackge[gyroIndex]['gyr_x'], gyroRange)
            saveFileOnePackge[gyroIndex]['gyr_y'] = calcAccGryro(saveFileOnePackge[gyroIndex]['gyr_y'], gyroRange)
            saveFileOnePackge[gyroIndex]['gyr_z'] = calcAccGryro(saveFileOnePackge[gyroIndex]['gyr_z'], gyroRange)
        ...
        csv_write_dict(csv_file, csvFileHead[0], saveFileOnePackge)
