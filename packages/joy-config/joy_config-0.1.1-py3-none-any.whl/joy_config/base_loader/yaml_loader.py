...
def yaml_loader(yaml_path="config.yaml"):
    """
    加载YAML格式的配置文件，并将配置项作为返回对象的属性
    
    参数:
        yaml_path: YAML配置文件的路径，默认为当前目录下的config.yaml文件
        
    返回:
        一个包含配置项作为属性的YamlConfig对象
    """
    class YamlConfig:
        """
        存储YAML配置项的配置类
        ...
        def _load_yaml(self):
            ...
            try:
                if not os.path.exists(self._yaml_file):
                    logger.error(f"can't find the YAML file '{self._yaml_file}'")
                    return
                ...
                logger.error(f"invalid YAML format in '{self._yaml_file}': {e}")
