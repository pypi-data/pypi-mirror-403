...
async def run_webhook(
    ...
    if event == "customer_create":
        ...
        if not user:
            logging.info(f"Creating new user: {info['email']}")
            user = Profile(
            ...
            logging.info(f"User already exists: {info['email']}")
    ...
    # --------------------------------------------------------
        ...
        if not user:
            logging.info(f"User not found for subscription. Auto-creating: {info['email']}")
            user = Profile(
                email=info["email"], pabbly_customer_id=info["pabbly_customer_id"]
            )
            db.add(user)
        ...
        if not existing:
            logging.info(f"Creating new subscription: {info['sub_id']}")
            ...
            logging.info(f"Subscription already exists: {info['sub_id']}")
    ...
    # --------------------------------------------------------
        ...
        if sub_id and expiry:
            logging.info(f"Activating subscription {sub_id} until {expiry}")
    ...
    # --------------------------------------------------------
        ...
        sub_id = info.get("sub_id")
        logging.info(f"Cancelling subscription {sub_id}")
