...
def list_name(stub, resource_type):
    ...
    format_rows("NAME")
    res = sorted(
        list(stub_list_functions[resource_type](metadata_pb2.ListRequest())),
        key=lambda x: x.name,
    )
    for f in res:
        format_rows(f.name)
...
def list_name_status(stub, resource_type):
    stub_list_functions = {
        ...
        "user": stub.ListUsers,
        "feature_view": stub.ListFeatureViews,
    }
...
def list_name_variant_status(stub, resource_type):
    ...
    res = sorted(
        list(stub_list_functions[resource_type][0](metadata_pb2.ListRequest())),
        key=lambda x: x.name,
    )
    for f in res:
        for v in f.variants:
