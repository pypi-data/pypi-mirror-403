        ...
        if chord.bass:
            ...
            if not analysis.is_hybrid:
                ...
                if bass_pitch_class is not None:
                    # 1. 構成音に含まれるならそのスペルを採用 (転回形)
                    if bass_pitch_class in tones:
                        bass_fixed = tones[bass_pitch_class]
                    else:
                        # 2. 構成音でない場合 (Bass on non-chord tone)
                        # ダイアトニックスケール上の音と一致するなら、ダイアトニックなスペルに強制する
                        # (例: Key=AbでBass=D# -> Dist=7 -> V=Eb に直す)
                        # さらに、よくある借用和音のルート(bIII, bVI, bVII)もスペルを修正する
                        # (例: Key=AbでBass=B(dist=3) -> #IIではなくbIII(Cb) に直す)
                        dist = NoteSpeller.semitone_distance(chord.bass, key_tonic)
                        
                        target_dist = None
                        if dist in self.MAJOR_SCALE_STEPS:
                            target_dist = dist
                        elif dist in (3, 8, 10): # bIII, bVI, bVII
                            target_dist = dist
                        
                        if target_dist is not None:
                            # 距離から推奨される度数名を計算 (prefer_sharps=Falseでフラット系を優先)
                            # _calc_degree_base は dist=3 -> bIII, dist=8 -> bVI を返すはず
                            base, _ = self._calc_degree_base(target_dist, prefer_sharps=False)
                            bass_fixed = self._spell_degree_note(base, key_tonic)

                if bass_fixed is None:
