...
class TaxDependentRequest(BaseModel):
    """
    TaxDependentRequest
    """ # noqa: E501
    id: Optional[StrictStr] = Field(default=None, description="Unique identifier for the tax dependent")
    name: Annotated[str, Field(strict=True, max_length=100)] = Field(description="Name of the tax dependent")
    ptkp_relation_type: StrictStr = Field(description="Relationship type for PTKP calculation", alias="ptkpRelationType")
    date_of_birth: datetime = Field(description="Date of birth of the tax dependent", alias="dateOfBirth")
    job: Optional[Annotated[str, Field(strict=True, max_length=100)]] = Field(default=None, description="Job or occupation of the tax dependent")
    __properties: ClassVar[List[str]] = ["id", "name", "ptkpRelationType", "dateOfBirth", "job"]

    @field_validator('ptkp_relation_type')
    def ptkp_relation_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['CHILD', 'SPOUSE', 'PARENT']):
            raise ValueError("must be one of enum values ('CHILD', 'SPOUSE', 'PARENT')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "id": obj.get("id"),
            "name": obj.get("name"),
            "ptkpRelationType": obj.get("ptkpRelationType"),
