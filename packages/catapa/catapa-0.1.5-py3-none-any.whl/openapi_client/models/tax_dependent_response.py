...
class TaxDependentResponse(BaseModel):
    """
    TaxDependentResponse
    """ # noqa: E501
    name: Optional[StrictStr] = Field(default=None, description="Name of the tax dependent")
    ptkp_relation_type: Optional[StrictStr] = Field(default=None, description="Relationship type for PTKP calculation", alias="ptkpRelationType")
    date_of_birth: Optional[date] = Field(default=None, description="Date of birth of the tax dependent", alias="dateOfBirth")
    job: Optional[StrictStr] = Field(default=None, description="Job or occupation of the tax dependent")
    __properties: ClassVar[List[str]] = ["name", "ptkpRelationType", "dateOfBirth", "job"]
    ...
    def ptkp_relation_type_validate_enum(cls, value):
        ...
        if value not in set(['SPOUSE', 'CHILD', 'PARENT']):
            raise ValueError("must be one of enum values ('SPOUSE', 'CHILD', 'PARENT')")
        return value
