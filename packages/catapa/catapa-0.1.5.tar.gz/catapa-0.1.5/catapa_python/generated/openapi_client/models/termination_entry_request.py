...
class TerminationEntryRequest(BaseModel):
    """
    TerminationEntryRequest
    """ # noqa: E501
    termination_code: Optional[StrictStr] = Field(default=None, alias="terminationCode")
    payroll_method: StrictStr = Field(alias="payrollMethod")
    termination_reason_id: StrictStr = Field(alias="terminationReasonId")
    request_date: date = Field(description="YYYY-MM-DD", alias="requestDate")
    bpjs_manpower_end_date: date = Field(description="YYYY-MM-DD", alias="bpjsManpowerEndDate")
    service_period_by: Optional[StrictStr] = Field(default=None, alias="servicePeriodBy")
    pph21_membership_end_date: date = Field(description="YYYY-MM-DD", alias="pph21MembershipEndDate")
    bpjs_healthcare_end_date: date = Field(description="YYYY-MM-DD", alias="bpjsHealthcareEndDate")
    effective_date: date = Field(description="YYYY-MM-DD", alias="effectiveDate")
    rehire: StrictBool = Field(description="Deprecated field that no longer has any effect on terminated employee pages")
    __properties: ClassVar[List[str]] = ["terminationCode", "payrollMethod", "terminationReasonId", "requestDate", "bpjsManpowerEndDate", "servicePeriodBy", "pph21MembershipEndDate", "bpjsHealthcareEndDate", "effectiveDate", "rehire"]

    @field_validator('payroll_method')
    def payroll_method_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['FULL', 'PROPORTIONAL']):
            raise ValueError("must be one of enum values ('FULL', 'PROPORTIONAL')")
        return value

    @field_validator('service_period_by')
    def service_period_by_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['JOIN_DATE', 'START_DATE', 'PERMANENT_DATE']):
            raise ValueError("must be one of enum values ('JOIN_DATE', 'START_DATE', 'PERMANENT_DATE')")
        return value
