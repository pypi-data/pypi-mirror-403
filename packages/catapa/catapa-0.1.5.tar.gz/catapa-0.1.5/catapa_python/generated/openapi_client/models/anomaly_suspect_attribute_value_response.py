...
class AnomalySuspectAttributeValueResponse(BaseModel):
    """
    AnomalySuspectAttributeValueResponse
    """ # noqa: E501
    id: Optional[StrictStr] = Field(default=None, description="Unique identifier for the attribute value")
    attribute_value: Optional[StrictStr] = Field(default=None, description="Value of the attribute", alias="attributeValue")
    score: Optional[Union[StrictFloat, StrictInt]] = Field(default=None, description="Score associated with the attribute value")
    input_source: Optional[StrictStr] = Field(default=None, description="Source of the input data", alias="inputSource")
    anomaly_suspect_attribute: Optional[AnomalySuspectAttributeResponse] = Field(default=None, alias="anomalySuspectAttribute")
    child: Optional[List[AnomalySuspectAttributeValueChildResponse]] = Field(default=None, description="Child attribute values")
    __properties: ClassVar[List[str]] = ["id", "attributeValue", "score", "inputSource", "anomalySuspectAttribute", "child"]

    @field_validator('input_source')
    def input_source_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['USER', 'OCR']):
            raise ValueError("must be one of enum values ('USER', 'OCR')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of anomaly_suspect_attribute
        if self.anomaly_suspect_attribute:
            _dict['anomalySuspectAttribute'] = self.anomaly_suspect_attribute.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "score": obj.get("score"),
            "inputSource": obj.get("inputSource"),
            "anomalySuspectAttribute": AnomalySuspectAttributeResponse.from_dict(obj["anomalySuspectAttribute"]) if obj.get("anomalySuspectAttribute") is not None else None,
            "child": [AnomalySuspectAttributeValueChildResponse.from_dict(_item) for _item in obj["child"]] if obj.get("child") is not None else None
        })
        return _obj
