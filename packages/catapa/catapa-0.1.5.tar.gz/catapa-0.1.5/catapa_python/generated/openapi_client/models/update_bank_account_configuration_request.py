...
class UpdateBankAccountConfigurationRequest(BaseModel):
    """
    UpdateBankAccountConfigurationRequest
    """ # noqa: E501
    employee: Optional[IdRequest] = None
    ...
    bank_accounts: Optional[List[BankAccountConfigurationRequest]] = Field(default=None, alias="bankAccounts")
    __properties: ClassVar[List[str]] = ["employee", "effectiveDate", "paymentMethod", "distributionMethod", "bankAccounts"]

    @field_validator('payment_method')
    def payment_method_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['TRANSFER', 'CASH']):
            raise ValueError("must be one of enum values ('TRANSFER', 'CASH')")
        return value

    @field_validator('distribution_method')
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "employee": IdRequest.from_dict(obj["employee"]) if obj.get("employee") is not None else None,
            "effectiveDate": obj.get("effectiveDate"),
            "paymentMethod": obj.get("paymentMethod"),
            "distributionMethod": obj.get("distributionMethod"),
            "bankAccounts": [BankAccountConfigurationRequest.from_dict(_item) for _item in obj["bankAccounts"]] if obj.get("bankAccounts") is not None else None
        })
        return _obj
