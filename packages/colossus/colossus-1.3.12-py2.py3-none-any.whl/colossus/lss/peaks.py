...
def _peakCurvatureExact(nu, gamma):
	...
	args = nu, gamma
	norm, _ = scipy.integrate.quad(curvature_Npk, 0.0, np.inf, args, epsrel = 1E-10)
	integ, _ = scipy.integrate.quad(curvature_Npk_x, 0.0, np.inf, args, epsrel = 1E-10)
	xav = integ / norm
...
def powerSpectrumSlope(nu, z, slope_type = 'P', scale = 1.0, 
	...
	sigma_args: dict
		Arguments passed to the :func:`~cosmology.cosmology.Cosmology.sigma` function.
	deltac_args: dict
		Arguments passed to the :func:`collapseOverdensity` function. In this function, these
		arguments affect how peak height is converted to mass, and thus to a Lagrangian radius. 
	...
	M_L = massFromPeakHeight(nu, z, ps_args = ps_args, sigma_args = sigma_args, deltac_args = deltac_args)
	R_L = lagrangianR(M_L)
