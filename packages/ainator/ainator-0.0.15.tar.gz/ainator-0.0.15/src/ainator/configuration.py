...
class Configuration(dict):
    ...
    def model(self):
        model_str = os.getenv('AINATOR_MODEL')
        # Handle empty string environment variables
        if not model_str or not model_str.strip():
            model_str = self['model']
        return self.object_factory(model_str)

    @functools.cached_property
    def compression_model(self):
        compression_model_str = os.getenv(
            'AINATOR_COMPRESSION_MODEL',
            os.getenv('AINATOR_MODEL'),
        )
        # Handle empty string environment variables
        if not compression_model_str or not compression_model_str.strip():
            compression_model_str = self['compression_model']
        return self.object_factory(compression_model_str)
