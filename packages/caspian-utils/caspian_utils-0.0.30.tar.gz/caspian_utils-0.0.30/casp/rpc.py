    ...
    def check(self, func_key: str, ip: str, limit_str: str) -> tuple[bool, float]:
        ...
        try:
            ...
            if len(valid_history) >= limit_count:
                ...
                # Calculate time until the oldest request expires from the window
...
# ==== UPDATED DECORATOR WITH HIERARCHY ====
def rpc(require_auth: bool = False,
    ...
    def decorator(func):
        ...
        # 1. Determine which limit string to use
        final_limits = []

        # If explicit limits passed, they win
        if limits:
        ...
        RPC_META[key] = {
            ...
            'limits': final_limits,  # Stores the resolved limits
            'route': route
        }
...
async def _handle_rpc_request(request: Request, session: dict) -> Response:
    ...
    # ==== RATE LIMIT CHECK ====
    # This uses the 'final_limits' we calculated in the decorator
    ...
    if rpc_limits:
        ...
        for limit_rule in rpc_limits:
            ...
            if not is_allowed:
                # Pass the wait time in the error string so frontend can parse it
                return JSONResponse({
