...
class HurMultiModal:
    ...
    def __unique_training(self, dataset_path='', string='', precision=1.0, tokenizer='gpt-4', context_window=None, hurnet_initializer=True, hurnet_layer=False, hurnet_fit=False, end_tag=None, stream_dataset=False, validate=0.0, quantization=None, progress=True):
        try:
            ...
            max_current_precision = 0.0
            while feed_forward:
                Nx += 1
                self.__model.train()
                cumulative_current_accuracy, cumulative_index = 0.0, 0
                for input_batch, target_batch in train_loader:
                    ...
                    if progress:
                        if max_current_precision > 0: cumulative_current_accuracy = (max_current_precision+current_precision)/2
                        else: cumulative_current_accuracy = min(current_precision, precision-0.0002) / max(1, cumulative_index-Nx)
                        cumulative_current_accuracy = min(cumulative_current_accuracy, precision-0.0001)
                        progress_bar.set_postfix({'loss': f'{loss_item:.4f}', 'precision': f'{cumulative_current_accuracy:.4f}'})
                ...
                if current_precision >= precision or Nx >= self.EPOCHS:
                    ...
                    break
                if current_precision > max_current_precision: max_current_precision = current_precision
            if progress: 
