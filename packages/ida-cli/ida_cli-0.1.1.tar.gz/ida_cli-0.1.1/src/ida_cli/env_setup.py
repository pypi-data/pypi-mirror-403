...
def _is_valid_ida_dir(path: Path) -> bool:
    ...
    if not path.exists():
        return False
    # Must have idalib directory with python activation script
    idalib = path / "idalib"
...
def _find_mac_ida() -> Path | None:
    ...
    for pattern in patterns:
        for app in apps_dir.glob(pattern):
            # Check Contents/MacOS for idalib
            macos_dir = app / "Contents" / "MacOS"
            if _is_valid_ida_dir(macos_dir):
                found.append(macos_dir)

    # Sort by name (highest version first)
...
def find_ida_installation() -> Path | None:
    ...
    4. Glob search in /Applications for macOS .app bundles
    5. Glob search in home directory
    6. WSL Windows paths
    """
