...
def _timeit(fn, warmup, repeats):
    ...
    return {"median_s": med, "mean_s": mean, "cv": cv}
...
def main():
    ...
    if "numpy" in refs:
        add_backend("numpy", lambda: _impl_numpy(name, inputs, case))
    if "pandas" in refs:
        add_backend("pandas", lambda: _impl_pandas(name, inputs, case))
    if "scipy" in refs:
        add_backend("scipy", lambda: _impl_scipy(name, inputs, case))
    if with_python and "python" in refs:
        add_backend("py", lambda: _impl_py(name, inputs, case))
