...
@pytest.fixture
def driver() -> MatchDriverGreedy:
    driver = MatchDriverGreedy(4, 5)
...
def test_high_threshold(
    metric_23: np.ndarray[tuple[2, 3], np.dtype[float]], driver: MatchDriverGreedy
) -> None:
    """."""
...
def test_low_threshold(
    ...
    assert driver.reports == pytest.approx([1, 0, 9, 9])
    assert driver.targets == pytest.approx([2, 1, 9, 9])


def test_all_zeros(driver: MatchDriverGreedy) -> None:
    """."""
