"""
Alignment handling and parsing functions copied and modified from OpenFold codebase
...
class TemplateHit:
    ...
    # extra fields not stored in OpenFold implementation
    probability: float
    e_value: float
    score: float
    identities: float
...
def _parse_hhr_hit(detailed_lines: Sequence[str]) -> TemplateHit:
    ...
    (probability, e_value, score, aligned_cols, identities, similarity, sum_probs, template_neff) = [
...
# own additional functions below


def parse_a3m_from_file(file_path: str):
    """
    Read a3m format alignment from file

    Args:
        file_path: Path to a3m file that should be parsed

    """
    with open(file_path) as f:
        ali_str = f.read()
        
    return parse_a3m(ali_str)


def parse_hhr_from_file(file_path: str):
    """
    Read a3m format alignment from file
