...
from nimbleway_webit import Nimbleway, AsyncNimbleway, APIResponseValidationError
from nimbleway_webit._types import Omit
from nimbleway_webit._utils import asyncify
from nimbleway_webit._models import BaseModel, FinalRequestOptions
from nimbleway_webit._exceptions import APIStatusError, APITimeoutError, APIResponseValidationError
from nimbleway_webit._base_client import (
    DEFAULT_TIMEOUT,
...
class TestNimbleway:
            ...
            for frame in diff.traceback:
                if any(
                    ...
                    for fragment in [
                        ...
                        "nimbleway_webit/_legacy_response.py",
                        "nimbleway_webit/_response.py",
                        # pydantic.BaseModel.model_dump || pydantic.BaseModel.dict leak memory for some reason.
                        "nimbleway_webit/_compat.py",
    ...
    def test_validate_headers(self) -> None:
        ...
        with update_env(**{"WEBIT_API_KEY": Omit()}):
    ...
    def test_base_url_env(self) -> None:
        ...
        with update_env(NIMBLEWAY_BASE_URL="http://localhost:5000/from/env"):
            with pytest.raises(ValueError, match=r"you must pass base_url=None"):
                Nimbleway(api_key=api_key, _strict_response_validation=True, environment="production")

            client = Nimbleway(
                base_url=None, api_key=api_key, _strict_response_validation=True, environment="production"
            )
            assert str(client.base_url).startswith("https://api.webit.live")
    ...
    @mock.patch("nimbleway_webit._base_client.BaseClient._calculate_retry_timeout", _low_retry_timeout)
    @pytest.mark.respx(base_url=base_url)
    def test_retrying_timeout_errors_doesnt_leak(self, respx_mock: MockRouter, client: Nimbleway) -> None:
        respx_mock.post("/api/v2/crawl").mock(side_effect=httpx.TimeoutException("Test timeout error"))

        with pytest.raises(APITimeoutError):
            client.v2.crawl.with_streaming_response.crawl(url="https://example.com").__enter__()
    ...
    def test_retrying_status_errors_doesnt_leak(self, respx_mock: MockRouter, client: Nimbleway) -> None:
        respx_mock.post("/api/v2/crawl").mock(return_value=httpx.Response(500))
    ...
    def test_retries_taken(
        ...
        respx_mock.post("/api/v2/crawl").mock(side_effect=retry_handler)

        response = client.v2.crawl.with_raw_response.crawl(url="https://example.com")
...
class TestAsyncNimbleway:
    ...
    async def test_base_url_env(self) -> None:
        ...
        with update_env(NIMBLEWAY_BASE_URL="http://localhost:5000/from/env"):
            with pytest.raises(ValueError, match=r"you must pass base_url=None"):
                AsyncNimbleway(api_key=api_key, _strict_response_validation=True, environment="production")
    ...
    async def test_retrying_timeout_errors_doesnt_leak(
        ...
        with pytest.raises(APITimeoutError):
            await async_client.v2.crawl.with_streaming_response.crawl(url="https://example.com").__aenter__()
    ...
    async def test_retries_taken(
        ...
        response = await client.v2.crawl.with_raw_response.crawl(url="https://example.com")
    ...
    async def test_omit_retry_count_header(
        ...
        response = await client.v2.crawl.with_raw_response.crawl(
            url="https://example.com", extra_headers={"x-stainless-retry-count": Omit()}
        )
