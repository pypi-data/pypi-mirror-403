...
HistoryRow: TypeAlias = tuple[str, str]
...
def get_filtered_urls_chromium(
    ...
    filtered_urls: dict[str, HistoryRow] = {}
...
def update_last_viewed(files: list[pathlib.Path], filtered_urls: dict[str, HistoryRow]):
    for path in files:
...
def discard_existing_chars(
    files: list[pathlib.Path], filtered_urls: dict[str, HistoryRow]
):
    for path in files:
        if path.stem in filtered_urls:
