...
def main(log_level: str):
    ...
    log.add(core.get_log_path() / "logs.log", level="TRACE")
...
@chars.command("update", help="update the characters index with latest information.")
@click.pass_context
def chars_update(ctx: click.Context):
    import ast
    import json
    import random
    import time
    ...
    filtered_urls: dict[str, core.HistoryRow] = {}
        ...
        while pending:
            ...
            for future in as_completed(futures):
                ...
                if res.status_code == 429:
                    log.warning(f"rate limited, rescheduling {char_id=}")
                    log.info(f"sleeping for {SLEEP_RANGE[1]}")
                ...
                name: str | None = (  # type: ignore
                    data.get("character", {}).get("name")  # type: ignore
                    if isinstance(data, dict)
                    else None
                )
                log.info(f"saved {char_id=} {name=}")
...
def info():
    ...
    show_info("Log File", core.get_log_path() / "logs.log")
