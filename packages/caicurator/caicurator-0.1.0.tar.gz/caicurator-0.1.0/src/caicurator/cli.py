...
def update(ctx: click.Context):
    ...
    filtered_urls = core.get_filtered_urls(core.get_history_file_path())
...
def info():
    ...
    show_info("History File", core.get_history_file_path())

    con = core.get_history_file_sql_con(core.get_history_file_path())
    show_info(
        "Total URLs", con.cursor().execute("SELECT COUNT(*) FROM urls").fetchone()[0]
    )
    con.close()

    show_info(
        "Filtered URLs", len(core.get_filtered_urls(core.get_history_file_path()))
    )
