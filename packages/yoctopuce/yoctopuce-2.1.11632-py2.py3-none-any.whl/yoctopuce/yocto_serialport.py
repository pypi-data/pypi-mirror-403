...
class YSerialPort(YFunction):
    ...
    def snoopMessagesEx(self, maxWait, maxMsg):
        ...
        while idx < msglen:
            res.append(YSnoopingRecord(msgarr[idx].decode(YAPI.DefaultEncoding)))
            idx = idx + 1
    ...
    def _internalEventHandler(self, advstr):
        ...
        idx = 0
        while idx < msglen:
            self._eventCallback(self, YSnoopingRecord(msgarr[idx].decode(YAPI.DefaultEncoding)))
            idx = idx + 1
        return YAPI.SUCCESS
    ...
    def writeStxEtx(self, text):
        ...
        """
        # buff
        buff = bytearray("" + str(chr(2)) + "" + text + "" + str(chr(3)), YAPI.DefaultEncoding)
        # // send string using file upload
        return self._upload("txdata", buff)
