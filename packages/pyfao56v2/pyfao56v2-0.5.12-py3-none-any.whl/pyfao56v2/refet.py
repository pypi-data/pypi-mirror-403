...
def ascedaily(rfcrp,z,lat,doy,tmax,tmin,israd=float('NaN'),
              vapr=float('NaN'),tdew=float('NaN'),
              rhmax=float('NaN'),rhmin=float('NaN'),
              wndsp=float('NaN'),wndht=2.0,loctn='interior',
              clmt='dry'):
    ...
    israd : float, optional (but recommended)
        Incoming solar radiation (MJ m^-2 d^-1) (default = NaN)
    ...
    loctn : str, optional
        'interior' for locations away from large bodies of water (default)
        'coastal' for locations near large bodies of water
        'island' for locations on islands
    clmt : str, optional
        'dry' for dry climates (default)
        'humid' for humid climates
    ...
    """
    assert z is not None, 'Elevation (z) is required'
    assert lat is not None, 'Latitude (lat) is required'
    assert rfcrp in ['S','T'], f'rfcrp must be "S" or "T". Got {rfcrp}'
    assert loctn in ['interior','coastal','island'], f'loctn must be "interior", "coastal", or "island". Got {loctn}'
    assert clmt in ['dry','humid'], f'clmt must be "dry" or "humid". Got {clmt}'
    ...
    #ASCE (2005) Eq. 16
    if math.isnan(israd):
        if loctn in ['interior','coastal']:
            krs = 0.16 if loctn == 'interior' else 0.19
            israd = krs*(tmax-tmin)**0.5*ra # Eq. E.4
        else:
            israd = 0.7*ra-4 # FAO56, Eq. 51
        israd = min(israd, rso) # israd (rs) cannot exceed rso (p. 108, ASCE 2005 APPENDICES)
    albedo = 0.23
    rns = (1.0-albedo)*israd # Eq. 16
