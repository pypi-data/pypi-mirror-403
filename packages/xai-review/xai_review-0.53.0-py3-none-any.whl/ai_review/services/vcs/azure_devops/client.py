...
class AzureDevOpsVCSClient(VCSClientProtocol):
    ...
    async def delete_inline_comment(self, comment: ReviewCommentSchema) -> None:
        try:
            logger.info(f"Deleting inline comment comment_id={comment.id} in PR {self.pull_request_ref}")
            await self.http_client.pr.delete_thread(
                organization=self.organization,
                project=self.project,
                repository_id=self.repository_id,
            ...
            logger.info(f"Deleted inline comment comment_id={comment.id} in PR {self.pull_request_ref}")
        except Exception as error:
            logger.exception(
                f"Failed to delete inline comment comment_id={comment.id} in PR {self.pull_request_ref}: {error}"
            )
            raise
