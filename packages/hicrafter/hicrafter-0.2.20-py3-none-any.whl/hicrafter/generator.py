...
import glass.points        # Point catalog tools (e.g., galaxies)
import glass.shapes        # Galaxy shapes, ellipticity modeling
import glass.lensing       # Lensing effects: convergence, shear
...
class HIGenerator:
    def __init__(self, h=0.7, As=2e-9, Oc=0.25, Ob=0.05, ns=0.965,
        ...
        self.pars, self.cosmo = self._setup_cosmology()
    ...
    '''def _setup_cosmology(self):
        pars = camb.set_params(H0=100*self.h, omch2=self.Oc*self.h**2, ombh2=self.Ob*self.h**2, ns=self.ns, tau=0.055, r=0, As=2e-9)
        pars.NonLinear = camb.model.NonLinear_both
        pars.set_cosmology(H0=100*self.h, omch2=self.Oc*self.h**2, ombh2=self.Ob*self.h**2, tau=0.055)
        pars.InitPower.set_params(As=self.As, ns=self.ns)
        
        return pars, Cosmology.from_camb(pars)'''
    ...
    def _setup_cosmology(self):
        ...
        results = camb.get_results(pars)  # Get results with distance methods
        cosmo = Cosmology.from_camb(pars)  # Keep for other uses if needed
    
        return pars, results  # Return results instead of cosmo
