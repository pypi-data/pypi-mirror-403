...
fn main() {
    ...
    match compat::eigh(&symmetricmatrix.view(), UPLO::Lower) {
        Ok((eigenvals_with_vecs, eigenvecs)) => {
            ...
            println!(
                ...
                diff.iter().map(|x: &f64| x.abs()).fold(0.0, f64::max)
            );
            ...
            println!(
                ...
                orth_diff.iter().map(|x: &f64| x.abs()).fold(0.0, f64::max)
            );
        }
