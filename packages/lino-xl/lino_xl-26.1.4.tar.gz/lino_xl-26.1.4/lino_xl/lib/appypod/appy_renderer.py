...
class AppyRenderer(OriginalAppyRenderer):
    ...
    def __init__(self, ar, template, context, result, **kw):
        ...
        # self.ar.renderer = settings.SITE.plugins.bootstrap5.renderer
        ...
        context.update(sc=ar.get_user().site_config)
        if False:
    ...
    def story2odt(self, story, *args, **kw):
        for item in story:
            ...
            elif isinstance(item, ActionRequest) and issubclass(item.actor, AbstractTable):
