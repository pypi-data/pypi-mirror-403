...
class Couple(dd.Model):
    ...
    father = dd.ForeignKey('contacts.Person',
                           blank=True,
                           null=True,
                           related_name='couples_as_father')
    mother = dd.ForeignKey('contacts.Person',
                           blank=True,
                           null=True,
                           related_name='couples_as_mother')
    married = models.DateField(_("Married on"), blank=True, null=True)
    married_place = dd.ForeignKey('countries.Place',
                                  verbose_name=_("Married in"),
                                  blank=True,
                                  null=True)
...
class CoupleField(dd.VirtualField):
    ...
    def __init__(self, parents_name, my_attname, other_attname, verbose_name):
        ...
        rt = dd.ForeignKey('contacts.Person',
                           blank=True,
                           null=True,
...
class Child(dd.Model):
    ...
    def family(self, ar):
        ...
        if self.father:
            members.append([
                self.__class__.father.verbose_name, ': ',
                ar.obj2html(self.father)
            ])
        if self.mother:
            members.append([
                self.__class__.mother.verbose_name, ': ',
                ar.obj2html(self.mother)
            ])
    ...
    def father_choices(self):
        return contacts.Person.objects.filter(gender=dd.Genders.male).order_by(
            'last_name', 'first_name')

    @dd.chooser()
    def mother_choices(self):
        return contacts.Person.objects.filter(
            gender=dd.Genders.female).order_by('last_name', 'first_name')
