...
def objects(refs="PMO BNK"):  # welfare calls it with customized refs.
    ...
    for ref in refs.split():
        ...
        while date < end_date:
            ...
            ar = do_fill.create_request(master_instance=voucher)
            # select all rows:
            suggestions = sug_table.create_request(voucher)
            ar.selected_rows = list(suggestions)
            # print(f"20251129 suggested_rows {ar.selected_rows}")
            # run the action:
            ar.run()
            ...
            if voucher.items.count() == 0:
                # print(f"20251129 delete {voucher}")
                voucher.delete()
            else:
                # if ref == 'PMO':
                ...
                # print(f"20251129 register {voucher}")
                # voucher.full_clean()
                # voucher.save()
        ...
        # yield bs
        # suggestions = finan.SuggestionsByBankStatement.create_request(bs)
        # ba = suggestions.actor.get_action_by_name('do_fill')
        # ar = ba.create_request(master_instance=bs)
