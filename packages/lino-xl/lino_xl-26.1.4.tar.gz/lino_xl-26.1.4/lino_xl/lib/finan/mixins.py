...
from lino_xl.lib.accounting.utils import DC, ZERO, MAX_AMOUNT, myround
from lino_xl.lib.accounting.mixins import SequencedVoucherItem
from lino_xl.lib.accounting.mixins import ProjectRelated, Matching, LedgerRegistrable
...
class FinancialVoucher(Voucher, LedgerRegistrable, Certifiable):
    ...
    state = accounting.VoucherStates.field(default='draft')
    item_account = dd.ForeignKey('accounting.Account',
                                 verbose_name=_("Default account"),
                                 blank=True,
                                 null=True)
    item_remark = models.CharField(_("Your reference"),
                                   max_length=200,
                                   blank=True)
    ...
    def due2itemdict(self, due, **kwargs):
        ...
        if due.dc == self.journal.dc:
            kwargs.update(
                amount=-due.balance)  # 20201219 FinancialVoucher.due2itemdict
        else:
            kwargs.update(
                amount=due.balance)  # 20201219 FinancialVoucher.due2itemdict
        return kwargs
    ...
    def get_finan_movements(self):
        ...
        for i in self.items.order_by('seqno'):
            ...
            b = self.create_ledger_movement(
                i, (i.account or self.item_account, None), i.project, i.amount,
                **kw)
            amount += i.amount
...
class FinancialVoucherItem(SequencedVoucherItem, ProjectRelated, Matching):
    ...
    def collect_suggestions(self, ar, flt):
        suggestions = list(
            accounting.get_due_movements(self.voucher.journal.dc, flt))
    ...
    def full_clean(self, *args, **kwargs):
        ...
        if True:  # temporarily deactivated for data migration
            problems = list(FinancialVoucherItemChecker.check_instance(None, self))
            if len(problems):
        ...
        if obj.account_id and not obj.voucher.journal.preliminary:
            if obj.account.needs_partner:
                if not obj.partner_id:
                    yield (False,
                           _("Account {} needs a partner").format(obj.account))
            else:
                if obj.partner_id:
                    yield (
                        False,
                        _("Account {} cannot be used with a partner").format(
                            obj.account))
