...
class Address(AddressLocation):
    ...
    def before_ui_save(self, ar, cw):
        ...
        if (mi := self.partner) is None:
            return
        if self.__class__.objects.filter(partner=mi, primary=True).exists():
            self.primary = False
        else:
            self.primary = True
...
class AddressOwnerChecker(Checker):
    ...
    def get_checkdata_problems(self, ar, obj, fix=False):
        ...
        if ap != None and qs_marked_primary.count() != 0:
            ...
            ppaqs = ap.addresses_by_partner.filter(
                primary=True)  # parent's primary addresses
            if ppaqs.count() == 0:
                ...
                if mpa.can_sync_from(diffs):
                    ...
                    yield (True,
                           self.messages['remove_useless_addr'].format(ap))
                    if fix:
                        mpa.delete()
                else:
        ...
        if num_matching == 1:
            ...
            if len(addr.get_diffs(obj)):
                ...
                if fix:
                    ...
                    addr.save()
            if addr.primary:
                other_primaries = obj.addresses_by_partner.exclude(
                    id=addr.id).filter(primary=True)
        ...
        if num_primary == 0:
            if num_matching == 0:
                ...
                yield (False,
                       _("No primary address, but matching addresses exist."))
            return
        ...
        if addr and diffs:
            diffstext = [
                _("{0}:{1}->{2}").format(k, getattr(obj, k), v)
                for k, v in sorted(diffs.items())
            ]
            msg = _(
                "Primary address differs from owner address ({0}).").format(
                    ', '.join(diffstext))
            yield (False, msg)
