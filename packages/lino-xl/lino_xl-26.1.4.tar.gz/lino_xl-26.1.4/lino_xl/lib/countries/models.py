...
class Country(mixins.BabelNamed):
    ...
    def get_stdnum_module(self, name):
        """
        Return the named `stdnum` module for this country.

        `name` should be one of  {'vat', 'businessid',  'personalid',
        'postal_code'} as documented `here
        <https://arthurdejong.org/python-stdnum/doc/1.20/index.html#stdnum.get_cc_module
        >`__

        """
        return get_cc_module(self.isocode, name)
