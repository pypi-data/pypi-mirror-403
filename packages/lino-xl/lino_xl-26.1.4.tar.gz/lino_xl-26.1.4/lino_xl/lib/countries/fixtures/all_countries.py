    ...
    if USE_WIKIDATA:
        ...
        for coun in dom.documentElement.getElementsByTagName('coun:country'):
            ...
            for name in coun.getElementsByTagName('coun:name'):
                ...
                names[str(
                    name.attributes['lang'].value)] = name.firstChild.data

            kw = dd.babel_values('name', **names)
            iso2 = coun.getElementsByTagName(
                'coun:alpha2')[0].childNodes[0].data
            if Country.objects.filter(pk=iso2).count() > 0:
                logger.debug("ISO code %r already exists %s", iso2, coun)
                continue
            kw.update(
                isocode=iso2,
                iso3=coun.getElementsByTagName('coun:alpha3')
                [0].childNodes[0].data,
            )
            ...
            if kw['name']:
                ...
                logger.warning("%r : no name for default site language %s",
                               coun,
