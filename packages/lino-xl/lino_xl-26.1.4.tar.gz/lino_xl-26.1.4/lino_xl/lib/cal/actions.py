...
class UpdateGuests(dd.Action):
    ...
    def run_on_event(self, ar, obj):
        ...
        msg = _("Update presences for {} : "
                "{} created, {} unchanged, {} deleted.").format(obj, c, u, d)
        ar.debug(msg)
...
class UpdateAllGuests(UpdateGuests):
    ...
    def run_from_ui(self, ar, **kw):
        ...
        for obj in ar.selected_rows:
            qs = Event.objects.filter(**gfk2lookup(gfk, obj, state__in=states))
            ...
            ar.confirm(
                ok,
                _("Update presences for {} events: {}").format(
                    qs.count(), txt))
...
class RefuseGuestStates(dd.ChangeStateAction):
    ...
    def attach_to_workflow(self, wf, name):
        super().attach_to_workflow(wf, name)
        if isinstance(self.refuse_guest_states, str):
            self.refuse_guest_states = {
                GuestStates.get_by_name(x)
                for x in self.refuse_guest_states.split()
            }
    ...
    def before_execute(self, ar, obj):
        ...
        count = qs.count()
        if count > 0:
            guest_states = (" " + gettext("or") + " ").join(
                [str(s.text) for s in rgs])
            msg = gettext("Cannot mark as {state} because {count} "
