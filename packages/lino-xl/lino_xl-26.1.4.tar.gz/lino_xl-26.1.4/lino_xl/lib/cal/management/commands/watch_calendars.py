...
def receive(dbcal, calendar):
    ...
    if results:
        for comp in results:
            ...
            dblogger.info("Got calendar component <<<\n%s\n>>>",
                ...
                if isinstance(event, RecurringComponent):
                    ...
                    if location_name:
                        ...
                        if qs.count() == 0:
                            ...
                            if qs.count() > 1:
                                dblogger.warning(
                                    "Found more than 1 Place for location %r",
                                    location_name)
                    ...
                    raise Exception(
                        "comp.instance.vevent is a %s (expected VEvent)" %
                        type(event))
            else:
                raise Exception("Got unhandled component %s" %
    ...
    rs_deleted = qs.count()
    qs.delete()  # note: doesn't call delete methods of individual objects
    dblogger.info("--> Created %d, updated %d, deleted %s Events", count_new,
                  count_update, count_deleted)
    dblogger.info("--> Created %d, updated %d, deleted %s RecurrenceSets",
