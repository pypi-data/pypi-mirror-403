...
from lino_xl.lib.accounting.mixins import (Payable, PaymentRelated, ProjectRelated,
...
class AccountInvoice(BankAccount, PaymentRelated, Payable, Voucher, Matching,
                     ProjectRelated):
    ...
    def get_payable_sums_dict(self, ar=None):
        ...
        for i in self.items.order_by('seqno'):
            if i.amount:
                b = i.get_base_account(tt, ar)
                if b is None:
                    continue  # e.g. subtotal
