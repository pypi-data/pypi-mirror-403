...
TradingRule = rt.models.trading.TradingRule
...
class TimLoader(TimLoader):
    ...
    def load_par(self, row):
        ...
        if row.zahler.strip():
            ...
            if v:
                if isinstance(partner, Partner):
                    obj = partner.get_sales_rule()
                    obj. invoice_recipient = v
                    yield obj
                if course is not None:
                    course.partner = v
            else:
    ...
    def load_plp(self, row, **kw):
        ...
        try:
            ...
            return
        yield Enrolment(course=course,
                        pupil=person,
                        guest_role=role,
                        state='confirmed',
                        request_date=course.start_date)
    ...
    def load_ppr(self, row, **kw):
        ...
        try:
            ...
            dd.logger.warning("Cannot import PPR %s : no course ref %s", row,
                              idpar)
            return
    ...
    def load_msg(self, row, **kw):
        ...
        try:
            ...
            dd.logger.warning("Cannot import MSG %s : no course ref %s", row,
                              idpar)
            return
        # idpar = self.par_pk(row.idpar.strip())
        # if idpar is None:
    ...
    def objects(self):
        ...
        bulkdel(Guest, Event, TradingRule)
