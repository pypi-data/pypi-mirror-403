...
class StarObject(dd.Action):
    ...
    def run_from_ui(self, ar, **kw):
        ...
        ar.success(_("{0} is now starred.").format(obj), refresh_all=True)
...
class UnstarObject(dd.Action):
    ...
    def run_from_ui(self, ar, **kw):
        ...
        ar.success(_("{0} is no longer starred.").format(obj),
                   refresh_all=True)
    ...
    def delete_star(self, obj, ar):
        ...
        # ON cascade delete?
        master_star_qs = rt.models.stars.Star.for_obj(obj,
                                                      user=user,
                                                      master__isnull=True)
    ...
    if dd.is_installed("stars"):
        ...
        def disabled_fields(self, ar):
            ...
            if not user.is_authenticated:
                ...
                return s
            master_star_qs = Star.for_obj(self, user=user, master__isnull=True)
        ...
        def get_children_starrable(self):
            for model, fk, related in self.child_starrables:
                model = dd.resolve_model(model) if isinstance(
                    model, string_types) else model
