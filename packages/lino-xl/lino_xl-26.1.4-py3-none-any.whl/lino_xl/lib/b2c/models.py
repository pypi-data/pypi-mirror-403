...
class ImportStatements(dd.Action):
    ...
    def run_from_ui(self, ar):
        ...
        msg = msg.format(self.imported_files, self.new_statements,
    ...
    def import_file(self, ar, filename):
        ...
        for stmt in parser.parse(data_file.read()):
            ...
            key = dict(iban=iban)
            data = dict(owner_name=stmt.owner_name,
            ...
            try:
                ...
                dd.logger.warning("Found more than one account with IBAN %s",
                                  iban)
                failed_statements += 1
                continue
            ...
            data = dict(start_date=stmt.start_date,
            ...
            for mvmt in stmt.transactions:
                ...
                data = dict(
                    ...
                    remote_account=mvmt.remote_account_iban
                    or mvmt.remote_account_other or '',
                    ...
                    remote_owner_country_code=mvmt.remote_owner_country_code
                    or '',
                    txcd=mvmt.txcd,
                    txcd_issuer=mvmt.txcd_issuer)
                data.update(remote_owner_address=mvmt.remote_owner_address)
                ...
                try:
                    ...
                    dd.logger.warning("Failed to save transaction %s : %s", s,
                                      e)
                    break
        ...
        if failed_statements > 0:
            dd.logger.warning("%d statements were NOT imported from %s",
...
class Account(dd.Model):
    ...
    account_name = models.CharField(_("Account name"),
                                    max_length=70,
                                    blank=True)
    owner_name = models.CharField(_("Owner name"), max_length=70, blank=True)
    last_transaction = models.DateField(_('Last transaction'),
                                        null=True,
                                        blank=True)
