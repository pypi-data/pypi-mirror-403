...
class Coachable(ClientBase):
    ...
    def get_clients_coached_by(cls, user):
        qs = cls.objects.filter(coachings_by_client__user=user,
    ...
    def add_param_filter(cls, qs, lookup_prefix='', coached_by=None, **kwargs):
        if coached_by:
            qs = qs.filter(
                **{lookup_prefix + 'coachings_by_client__user': coached_by})
        return super(Coachable, cls).add_param_filter(qs, lookup_prefix,
                                                      **kwargs)
    ...
    def setup_auto_event(self, evt):
        d = evt.start_date
        coachings = self.get_coachings((d, d),
                                       type__does_integ=True,
                                       user=evt.user)
    ...
    def find_appointment(self, ar):
        ...
        for obj in self.coachings_by_client.all():
            sar = CalendarPanel.create_request(subst_user=obj.user,
                                               current_project=self.pk)
            elems += [ar.href_to_request(sar, obj.user.username), ' ']
        return E.div(*elems)


class ClientChecker(Checker):
    # Abstract base class for all checkers on client_model
    model = client_model

    def get_responsible_user(self, obj):
        return obj.get_primary_coach() or super().get_responsible_user(obj)
