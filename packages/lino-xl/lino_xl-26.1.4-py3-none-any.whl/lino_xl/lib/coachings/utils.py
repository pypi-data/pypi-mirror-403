...
def add_coachings_filter(qs, user, period, primary, coaching_type=None):
    assert period is None or len(period) == 2
    if not (user or period or primary or coaching_type):
        return qs
    flt = Q()
    if period:
    ...
    # assert coaching_type is not None
    if coaching_type:
        flt &= Q(coachings_by_client__type=coaching_type)
    qs = qs.filter(flt).distinct()
    # print("20251212 c", qs.query)
    return qs
