...
Account = dd.resolve_model('accounting.Account')
...
class TimLoader(TimLoader):
        ...
        for obj in super(TimLoader, self).load_par(row):
            ...
            if prt == "P":
                for user in self.get_users(row):
                    ...
                    # if Course.objects.filter(ref=row.idpar.strip()).exists():
                    #     return
                    therapy = Course(line=self.therapies,
                                     partner_id=obj.id,
                                     name=name,
                                     teacher=user,
                                     id=obj.id,
                ...
                dd.logger.warning("No coaching for non-client %s", obj)
    ...
    def load_usr(self, row, **kw):
        ...
        o = User(**kw)
        o.set_password(dd.plugins.users.demo_password)
        return o
    ...
    def load_plp(self, row, **kw):
        ...
        if plptype[0] in "01":
            if not plptype in self.linktypes:
                dd.logger.warning("Ignored PLP %s : Invalid type %s", row,
                                  plptype)
                return
            ...
            if p1 is None:
                dd.logger.debug("Ignored PLP %s : Invalid idpar1", row)
                return
            p2 = self.get_partner(Person, row.idpar2)
            if p2 is None:
                dd.logger.debug("Ignored PLP %s : Invalid idpar2", row)
                return
            ...
            if p1 is None:
                dd.logger.warning("Ignored PLP %s : Invalid idpar1", row)
                return
            p2 = self.get_partner(Client, row.idpar2)
            if p2 is None:
                dd.logger.warning("Ignored PLP %s : Invalid idpar2", row)
                return
            if plptype == "81":
            ...
            p1 = self.get_partner(Course, row.idpar1)
            if p1 is None:
                dd.logger.warning("Ignored PLP %s : no course for idpar1", row)
                return
            yield Enrolment(course=p1,
                            pupil=p2,
                            remark=str(role),
                            state='confirmed')
        elif plptype == "81-":
            return
        dd.logger.debug("Ignored PLP %s : invalid plptype", row)
    ...
    def finalize(self):
        dd.logger.info("Deleting %d obsolete partners",
                       len(self.obsolete_list))
        for (par1, idpar2) in self.obsolete_list:
                ...
                for obj in model.objects.filter(**{k: par1}):
                    ...
                    try:
                        ...
                        if delete:
                            ...
                            dd.logger.warning(
                                "Couldn't change obsolete %s to %s: %s", k,
                                par2, e)
            ...
            try:
                ...
                par1.full_clean()
                par1.save()
                dd.logger.warning("Failed to delete {} : {}".format(par1, e))
