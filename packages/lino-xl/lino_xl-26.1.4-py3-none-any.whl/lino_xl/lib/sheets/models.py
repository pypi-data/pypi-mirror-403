...
class Report(UserPlan, PeriodRange, Certifiable, Story):
    ...
    #         self.start_period = AP.get_or_create_from_date(dd.today())
    ...
    def get_outer_queryset(self, coll):
        ...
        sp = self.start_period or StoredPeriod.get_or_create_from_date(
            dd.today())
    ...
    def get_story(self, ar, header_level=None):
        self.check_period_range()
        yield AccountEntriesByReport.create_request(self, parent=ar)
        if dd.is_installed('ana'):
            yield AnaAccountEntriesByReport.create_request(self, parent=ar)
        for tt in TradeTypes.get_list_items():
            yield PartnerEntriesByReport.create_request(
                self, parent=ar, param_values=dict(trade_type=tt))
        yield BalanceEntriesByReport.create_request(self, parent=ar)
        yield ResultsEntriesByReport.create_request(self, parent=ar)
