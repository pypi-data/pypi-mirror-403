...
class BookingStates(ReservationStates):
    ...
    auto_update_calendar = models.BooleanField(_("Update calendar"),
                                               default=True)
...
def setup_rooms_workflow(sender=None, **kw):
    ...
    BookingStates.draft.add_transition(
        required_states='registered option cancelled', icon_name="pencil")
    ...
    BookingStates.registered.add_transition(
        required_states='draft option cancelled', icon_name="accept")
    BookingStates.cancelled.add_transition(
        required_states='draft option registered', icon_name='cross')
...
class Booking(ContactRelated, Reservation):
    ...
    event_type = dd.ForeignKey(
        ...
        blank=True,
        help_text=_(
            """The calendar entry type of the calendar entries to generate.""")
    )
...
class BookingDetail(dd.DetailLayout):
    ...
    # ~ start end freq
    main = "general trading.InvoiceItemsByGenerator"
    general = dd.Panel("""
