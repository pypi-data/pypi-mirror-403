...
def yaml2dict(data):
    ...
    if dd.plugins.beid.data_collector_dir:
        ...
        fn = os.path.join(dd.plugins.beid.data_collector_dir,
                          card_number + '.txt')
...
class BaseBeIdReadCardAction(dd.Action):
    ...
    def get_action_view_permission(self, actor, user_type):
        """Make invisible when :data:`urlhandler_prefix` is `None`."""
    ...
    def process_row(self, ar, obj, attrs):
        """Generate a confirmation that asks to update the given data row
