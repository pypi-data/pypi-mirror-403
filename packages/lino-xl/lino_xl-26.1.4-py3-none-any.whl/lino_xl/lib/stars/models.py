...
class Star(UserAuthored, Controllable):
    ...
    # )
    master = dd.ForeignKey(
        'self',
        verbose_name=_("Master_Star"),
        blank=True,
        null=True,
        related_name="children_stars",
    ...
    def master_owner(self, ar):
        if ar is None:
            return None
        return ar.obj2html(
            self.master.owner) if self.master is not None else ""
...
class Stars(dd.Table):
    ...
    parameters = dict(type=dd.ForeignKey(ContentType, blank=True, null=True),
                      inherited_stars=dd.YesNo.field(_("inherited stars"),
                                                     blank=True,
                                                     null=True))
    # params_layout = """"""
