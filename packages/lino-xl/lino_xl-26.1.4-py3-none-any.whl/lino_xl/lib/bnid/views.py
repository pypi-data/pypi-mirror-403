...
def read_card_data_from_file(fn):
    ...
    if 'success' not in rv:
        raise Warning(_("Got invalid card data {} from eidreader.").format(rv))

    data = AttrDict(rv)
    if not data.success:
        raise Warning(_("No card data found: {}").format(data.message))
    return data
...
def load_card_data(uuid):
    ...
    while True:
        try:
            ...
            count += 1
            if count > timeout:
                raise Warning(_("Abandoned after {} seconds").format(timeout))
