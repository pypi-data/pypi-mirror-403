...
TEST_MODE = not DATABRICKS_HOST or DATABRICKS_HOST == "none" or not CLIENT_ID or not CLIENT_SECRET

if TEST_MODE:
    logger.warning("Running in test mode - will return mock responses")
else:
...
class GenieClient:
    ...
    async def update_headers(self) -> None:
        ...
        if TEST_MODE:
            self.headers = {
                "Authorization": f"Bearer mock_test_token_for_mcp_testing",
    ...
    async def start_conversation(self, question: str) -> Dict[str, Any]:
        ...
        if TEST_MODE:
            logger.info(f"Test mode: Mock start_conversation for question: {question[:50]}...")
            return {
    ...
    async def send_message(self, conversation_id: str, message: str) -> Dict[str, Any]:
        ...
        if TEST_MODE:
            logger.info(f"Test mode: Mock send_message for conversation: {conversation_id}")
            return {"message_id": "test_message_id_789"}
    ...
    async def get_message(self, conversation_id: str, message_id: str) -> Dict[str, Any]:
        ...
        if TEST_MODE:
            logger.info(f"Test mode: Mock get_message for message: {message_id}")
            return {
    ...
    async def get_query_result(self, conversation_id: str, message_id: str, attachment_id: str) -> Dict[str, Any]:
        ...
        if TEST_MODE:
            logger.info(f"Test mode: Mock get_query_result")
            return {
                'data_array': [{'column1': 'value1', 'column2': 'value2'}],
                'schema': {'columns': [{'name': 'column1'}, {'name': 'column2'}]}
            }
    ...
    async def execute_query(self, conversation_id: str, message_id: str, attachment_id: str) -> Dict[str, Any]:
        ...
        if TEST_MODE:
            logger.info(f"Test mode: Mock execute_query")
