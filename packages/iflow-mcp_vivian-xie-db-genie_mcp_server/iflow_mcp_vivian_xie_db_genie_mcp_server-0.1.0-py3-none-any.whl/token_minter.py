...
class TokenMinter:
    ...
    def _refresh_token(self) -> None:
        ...
        # Check if running in test mode (no real credentials)
        if not self.host or self.host == "none" or not self.client_id or not self.client_secret:
            logger.warning("Running in test mode with no real credentials - using mock token")
            with self.lock:
                self.token = "mock_test_token_for_mcp_testing"
