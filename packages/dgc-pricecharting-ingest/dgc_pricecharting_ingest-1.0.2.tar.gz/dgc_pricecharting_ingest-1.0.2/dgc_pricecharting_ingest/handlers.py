...
def _load_manual_url_cache() -> dict[str, str]:
    ...
    try:
        resp = requests.get(f"{API_URL}/pricecharting-ingest-urls", timeout=5)
        if resp.status_code != 200:
            print(f"Manual URL API failed: {resp.status_code}")
            _manual_url_cache = {}
            return _manual_url_cache

        _manual_url_cache = resp.json()
        return _manual_url_cache

    except Exception:
        print("Failed to load manual URL cache")
