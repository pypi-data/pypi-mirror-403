...
class OrderJourneyVisualizer:
    ...
    def from_csv(
        ...
        **read_csv_kwargs,
        ...
        """
        Create an OrderJourneyVisualizer from a CSV file.
        ...
        Args:
            ...
            **read_csv_kwargs: Passed directly to pandas.read_csv
        """
    ...
    def _validate_dataframe(self):
        """
        Validate input dataframe schema and basic data sanity.
        """
        required_columns = {self.order_col, self.status_col, self.timestamp_col}
        ...
        try:
            ...
            raise ValueError(
                f"Failed to parse '{self.timestamp_col}' as datetime. "
                f"Original error: {e}"
            )
        self.df = self.df.sort_values([self.order_col, self.timestamp_col])

        # Check nulls
        null_counts = self.df[list(required_columns)].isnull().sum()
        if null_counts.any():
            raise ValueError(
    ...
    def build_graph(self):
        ...
        for (from_status, to_status), count in transitions.items():
            ...
            self.edges.append({
                ...
                'label': str(count),
                'full_label': f"{count} orders (~{round(avg_time, 1)}h)"
    ...
    def generate_html(self, output_file='order_journey.html'):
        ...
        if not self.nodes or not self.edges:
            raise RuntimeError(
                "Graph not built. Call build_graph() before generate_html()."
            )
        ...
        order_options_html = "".join(
                                        f'<option value="{order}">{order}</option>'
...
<body>
        ...
        <div class="container">
         ...
         <div class="top-strip">
            <div class="legend-panel">
                <h3>💡 How to Use</h3>
                <div class="legend-item">🔵 <strong>Nodes</strong> = Order statuses</div>
                <div class="legend-item">➡️ <strong>Edges</strong> = Transitions</div>
                <div class="legend-item">📊 <strong>Labels</strong> = Count + avg time</div>
                <div class="legend-item">🖱️ Drag, zoom, click for details</div>
            </div>
            ...
            <div class="upload-panel">
                ...
                <div class="hint">
                    Required columns:<br/>
                    <code>order_id</code>, <code>status</code>, <code>timestamp</code>
                </div>
            </div>
        </div>
        <div class="header">
            ...
            <div class="stats">
                <div class="stat-item">
                    <div class="stat-value" id="total-orders">__TOTAL_ORDERS__</div>
                    <div class="stat-label">Total Orders</div>
                </div>
                <div class="stat-item">
                    <div class="stat-value" id="total-transitions">__TOTAL_ORDERS__</div>
                    <div class="stat-label">Total Transitions</div>
                </div>
                <div class="stat-item">
                    <div class="stat-value" id="unique-statuses">__UNIQUE_STATUSES__</div>
    ...
    <script>
        ...
        const originalElements = __CY_ELEMENTS__;
        let currentElements = originalElements;
        const orderStatusMap = __ORDER_STATUS_MAP__;
        let cy;
        ...
        function initCytoscape(elements) {
                ...
                style: [
                    {
                        ...
                        style: {
                                ...
                                'font-size': '12px',
                                'width': 'mapData(count, 1, 100, 70, 140)',
                                'height': 'mapData(count, 1, 100, 70, 140)',
                                'font-weight': 'bold'
                        }
                    },
                    {
                    selector: 'edge',
                        style: {
                            'width': 'mapData(weight, 1, 100, 2, 8)',
                            'line-color': '#95a5a6',
            ...
            cy.on('mouseover', 'edge', function (evt) {
                ...
                if (!edge.data('originalLabel')) {
                    edge.data('originalLabel', edge.data('label'));
                }

                edge.style('label', edge.data('full_label'));
            });
            cy.on('mouseout', 'edge', function (evt) {
                const edge = evt.target;
                edge.style('label', edge.data('originalLabel'));
            });

        }
        
        // Initialize on page load
        initCytoscape(currentElements);
        ...
        function applyFilters() {
            ...
            if (csvModeActive) {
                alert("Filters are disabled when CSV data is loaded.");
                return;
            }
            ...
            // Filter elements
            let filteredElements = currentElements.filter(ele => {
        ...
        function resetFilters() {
            ...
            initCytoscape(currentElements);
            document.getElementById('unique-statuses').textContent = __UNIQUE_STATUSES__;
            document.getElementById('total-transitions').textContent = __TOTAL_TRANSITIONS__;
        }
        ...
        document.getElementById("csv-file").addEventListener("change", function (event) {
            ...
            function parseCsv(csvText) {
                    ...
                    const required = ["order_id", "status", "timestamp"];
                    ...
                    function buildGraphFromRows(rows) {
                        ...
                        Object.values(byOrder).forEach(events => {
                            ...
                            if (i < events.length - 1) {
                                const key = `${events[i].status}→${events[i + 1].status}`;
                                transitions[key] = (transitions[key] || 0) + 1;
                            }
                            }
                        });

                        return { transitions, nodeCounts };
                        }

                        function toCytoscapeElements(nodeCounts, transitions) {
                            const elements = [];

                            Object.entries(nodeCounts).forEach(([status, count]) => {
                                elements.push({
                                data: {
                                    id: status,
                                    label: status,
                            ...
                            Object.entries(transitions).forEach(([key, count], idx) => {
                            ...
                            function handleCsvUpload(csvText) {
                                csvModeActive = true; // 🔑 this is the missing piece

                            
                                document.getElementById("status-filter").disabled = true;
                                document.getElementById("order-filter").disabled = true;

                                try {
                                    const rows = parseCsv(csvText);
                                    validateCsvRows(rows);

                                    const { transitions, nodeCounts } = buildGraphFromRows(rows);
                                    const elements = toCytoscapeElements(nodeCounts, transitions);
                                    currentElements = elements;        // ← THIS WAS MISSING
                                    initCytoscape(currentElements);


                                    document.getElementById("unique-statuses").textContent =
                                    Object.keys(nodeCounts).length;
...
"""
        html_content = html_content.replace(
            "__CY_ELEMENTS__", json.dumps(cytoscape_elements)
        )
        html_content = html_content.replace(
            "__ORDER_STATUS_MAP__", json.dumps(order_status_map)
        )
        html_content = html_content.replace(
            "__TOTAL_ORDERS__", str(self.stats["total_orders"])
        )

        html_content = html_content.replace(
            "__TOTAL_TRANSITIONS__", str(self.stats["total_transitions"])
        )

        html_content = html_content.replace(
            "__UNIQUE_STATUSES__", str(self.stats["unique_statuses"])
        )


        html_content = html_content.replace(
            "__STATUS_OPTIONS__", status_options_html
        )

        html_content = html_content.replace(
            "__ORDER_OPTIONS__", order_options_html
        )
