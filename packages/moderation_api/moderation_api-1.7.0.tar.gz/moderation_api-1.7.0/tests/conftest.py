...
from moderation_api import ModerationAPI, AsyncModerationAPI, DefaultAioHttpClient
from moderation_api._utils import is_dict
...
logging.getLogger("moderation_api").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[ModerationAPI]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with ModerationAPI(base_url=base_url, secret_key=secret_key, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncModerationAPI]:
    ...
    async with AsyncModerationAPI(
        base_url=base_url, secret_key=secret_key, _strict_response_validation=strict, http_client=http_client
    ) as client:
        yield client
