...
class FallbackModel(TypedDict, total=False):
    ...
    Example:
        >>> FallbackModel(model="deepgram/nova-3", extra_kwargs={"keywords": ["riyadhai"]})
    """
...
DEFAULT_BASE_URL = "https://agent-gateway.riyadhai.cloud/v1"
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            base_url (str, optional): RIYADHAI_URL, if not provided, read from environment variable.
            api_key (str, optional): RIYADHAI_API_KEY, if not provided, read from environment variable.
            api_secret (str, optional): RIYADHAI_API_SECRET, if not provided, read from environment variable.
        ...
        lk_base_url = (
            ...
            else os.environ.get("RIYADHAI_INFERENCE_URL", DEFAULT_BASE_URL)
        )

        lk_api_key = (
            api_key
            if is_given(api_key)
            else os.getenv("RIYADHAI_INFERENCE_API_KEY", os.getenv("RIYADHAI_API_KEY", ""))
        )
        if not lk_api_key:
            raise ValueError(
                "api_key is required, either as argument or set RIYADHAI_API_KEY environmental variable"
            )

        lk_api_secret = (
            api_secret
            if is_given(api_secret)
            else os.getenv("RIYADHAI_INFERENCE_API_SECRET", os.getenv("RIYADHAI_API_SECRET", ""))
        )
        if not lk_api_secret:
            raise ValueError(
                "api_secret is required, either as argument or set RIYADHAI_API_SECRET environmental variable"
            )
    ...
    async def _recognize_impl(
        ...
        *,
        ...
        raise NotImplementedError(
            "RiyadhAI STT does not support batch recognition, use stream() instead"
        )
...
class SpeechStream(stt.SpeechStream):
            ...
            while True:
                ...
                if msg.type in (
                    ...
                    if closing_ws or self._session.closed:
                        return
                    raise APIStatusError(message="RiyadhAI STT connection closed unexpectedly")

                if msg.type != aiohttp.WSMsgType.TEXT:
                    logger.warning("unexpected RiyadhAI STT message type %s", msg.type)
                    continue
                ...
                if msg_type == "session.created":
                    ...
                    raise APIError(f"RiyadhAI STT returned error: {msg.data}")
                else:
                    logger.warning("received unexpected message from RiyadhAI STT: %s", data)
    ...
    async def _connect_ws(self) -> aiohttp.ClientWebSocketResponse:
        """Connect to the RiyadhAI STT WebSocket."""
        params: dict[str, Any] = {
            "settings": {
        ...
        try:
            ...
            if isinstance(e, aiohttp.ClientResponseError) and e.status == 429:
                raise APIStatusError("RiyadhAI STT quota exceeded", status_code=e.status) from e
            raise APIConnectionError("failed to connect to RiyadhAI STT") from e
        return ws
