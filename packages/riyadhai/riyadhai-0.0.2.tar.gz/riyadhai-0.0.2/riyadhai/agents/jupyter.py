...
from riyadhai import api
from riyadhai.rtc.jupyter import display_room
...
def run_app(server: AgentServer | WorkerOptions, *, jupyter_url: str | None = None) -> None:
    ...
    if isinstance(server, WorkerOptions):
        ...
        if IN_COLAB:
            ...
            if not jupyter_url:
                server.ws_url = userdata.get("RIYADHAI_URL")
                server.api_key = userdata.get("RIYADHAI_API_KEY")
                server.api_secret = userdata.get("RIYADHAI_API_SECRET")
        else:
            server.ws_url = server.ws_url or os.environ.get("RIYADHAI_URL", "")
            server.api_key = server.api_key or os.environ.get("RIYADHAI_API_KEY", "")
            server.api_secret = server.api_secret or os.environ.get("RIYADHAI_API_SECRET", "")

        if not jupyter_url and (not server.ws_url or not server.api_key or not server.api_secret):
            raise ValueError(
                "Failed to get RIYADHAI_URL, RIYADHAI_API_KEY, or RIYADHAI_API_SECRET from environment variables. "  # noqa: E501
    ...
    if jupyter_url:
        ...
        async def fetch_join_tokens(url: str) -> tuple[str, str, str]:
            async with aiohttp.ClientSession() as session:
                async with session.post(url) as response:
                    ...
                    return data["riyadhai_url"], data["user_token"], data["agent_token"]
        ...
        try:
            ...
            raise ValueError(
                ...
                "You can still use your own RIYADHAI_URL, RIYADHAI_API_KEY, and RIYADHAI_API_SECRET from environment variables instead."  # noqa: E501
            ) from None
    ...
    def _simulate_job() -> None:
        async def simulate_job() -> None:
            async with api.RiyadhAIAPI(ws_url, api_key, api_secret) as lk_api:
