...
class WarmTransferTask(AgentTask[WarmTransferResult]):
    def __init__(
        ...
        *,
        ...
        self._sip_trunk_id = (
            sip_trunk_id if is_given(sip_trunk_id) else os.getenv("RIYADHAI_SIP_OUTBOUND_TRUNK", "")
        )
        if not self._sip_trunk_id:
            raise ValueError(
                "`RIYADHAI_SIP_OUTBOUND_TRUNK` environment variable or `sip_trunk_id` argument must be set"
            )

        self._sip_number = (
            sip_number if is_given(sip_number) else os.getenv("RIYADHAI_SIP_NUMBER", "")
        )
