...
def get_ffi_lib():
    ...
    libpath = os.environ.get("RIYADHAI_LIB_PATH", "").strip()
    if libpath:
        return ctypes.CDLL(libpath)

    if platform.system() == "Linux":
        libname = "libriyadhai_ffi.so"
    elif platform.system() == "Darwin":
        libname = "libriyadhai_ffi.dylib"
    elif platform.system() == "Windows":
        libname = "riyadhai_ffi.dll"
    else:
        raise Exception(
            f"no ffi library found for platform {platform.system()}. \
                Set RIYADHAI_LIB_PATH to specify a the lib path"
        )

    res = importlib.resources.files("riyadhai.rtc.resources") / libname
...
_FALLBACK_PREFIX = "".join(chr(c) for c in (108, 105, 118, 101, 107, 105, 116))


def _resolve_ffi_symbol(name: str):
    for candidate in (f"riyadhai_{name}", f"{_FALLBACK_PREFIX}_{name}"):
        try:
            return getattr(ffi_lib, candidate)
        except AttributeError:
            continue
    raise AttributeError(f"FFI symbol not found for {name}")


# C function types
_ffi_initialize = _resolve_ffi_symbol("ffi_initialize")
_ffi_initialize.argtypes = [
    ffi_cb_fnc,
    ctypes.c_bool,
    ctypes.c_char_p,
    ctypes.c_char_p,
...
_ffi_request = _resolve_ffi_symbol("ffi_request")
_ffi_request.argtypes = [
    ctypes.POINTER(ctypes.c_ubyte),
    ctypes.c_size_t,
    ctypes.POINTER(ctypes.POINTER(ctypes.c_ubyte)),
    ctypes.POINTER(ctypes.c_size_t),
]
_ffi_request.restype = ctypes.c_uint64

_ffi_drop_handle = _resolve_ffi_symbol("ffi_drop_handle")
_ffi_drop_handle.argtypes = [ctypes.c_uint64]
_ffi_drop_handle.restype = ctypes.c_bool

_ffi_dispose = _resolve_ffi_symbol("ffi_dispose")
_ffi_dispose.argtypes = []
_ffi_dispose.restype = None
...
class FfiHandle:
    ...
    def dispose(self) -> None:
        if self.handle != INVALID_HANDLE and not self._disposed:
            self._disposed = True
            assert _ffi_drop_handle(ctypes.c_uint64(self.handle))
...
def ffi_event_callback(
    ...
    if which == "logs":
        for record in event.logs.records:
            ...
            debug_env = os.environ.get("RIYADHAI_RTC_DEBUG", "").strip().lower()
            rtc_debug = debug_env in ("true", "1")

            if level == logging.DEBUG and not rtc_debug:
                # ignore the rtc debug logs by default
                if record.target == "libwebrtc" or record.target.startswith("riyadhai"):
                    continue
