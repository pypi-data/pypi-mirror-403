...
class TextModeLogFilter(logging.Filter):
    ...
    def filter(self, record: logging.LogRecord) -> bool:
        if record.name != "riyadhai.agents":
            return True
...
def _build_cli(server: AgentServer) -> typer.Typer:
    ...
    def console(
        *,
        ...
        """
        Run a [bold]RiyadhAI Agents[/bold] in [yellow]console[/yellow] mode.
        """
        if list_devices:
    ...
    def start(
        *,
        ...
        url: Annotated[
            Optional[str],  # noqa: UP007
            typer.Option(
                help="The WebSocket URL of your RiyadhAI server or Cloud project.",
                envvar="RIYADHAI_URL",
            ),
        ] = None,
        api_key: Annotated[
            Optional[str],  # noqa: UP007
            typer.Option(
                help="API key for authenticating with your RiyadhAI server or Cloud project.",
                envvar="RIYADHAI_API_KEY",
            ),
        ] = None,
        api_secret: Annotated[
            Optional[str],  # noqa: UP007
            typer.Option(
                help="API secret for authenticating with your RiyadhAI server or Cloud project.",
                envvar="RIYADHAI_API_SECRET",
            ),
        ] = None,
        ...
        *,
        ...
        def _simulate_job() -> None:
            ...
            async def simulate_job() -> None:
                async with api.RiyadhAIAPI(url, api_key, api_secret) as lk_api:
