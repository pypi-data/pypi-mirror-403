...
from riyadhai.api.access_token import Claims
...
class JobContext:
    ...
    async def _on_session_end(self) -> None:
        ...
        if report.enable_recording:
            try:
                ...
                logger.exception("failed to upload the session report to RiyadhAI Cloud")
    ...
    def api(self) -> api.RiyadhAIAPI:
        """Returns an RiyadhAIAPI for making API calls to RiyadhAI.
        ...
        RIYADHAI_URL, RIYADHAI_API_KEY, and RIYADHAI_API_SECRET so this API is always
        usable inside job entrypoints.
        """
        return api.RiyadhAIAPI(session=http_context.http_session())
    ...
    def add_sip_participant(
        ...
        *,
        ...
        Make sure you have an outbound SIP trunk created in RiyadhAI.
        See https://docs.riyadhai.io/sip/trunk-outbound/ for more information.
        """
        if self.is_fake_job():
    ...
    def transfer_sip_participant(
        ...
        See https://docs.riyadhai.io/sip/transfer-cold/ for more information.
        """
        if self.is_fake_job():
            logger.warning(
...
class JobRequest:
    ...
    async def accept(
        ...
        *,
        ...
        """Accept the job request, and start the job if the RiyadhAI SFU assigns the job to our worker."""  # noqa: E501
        if not identity:
