...
from riyadhai import api, rtc
from riyadhai.protocol import agent, models
...
class ServerOptions:
    ...
    max_retry: int = 16
    """Maximum number of times to retry connecting to RiyadhAI."""
    ws_url: str | None = None
    """URL to connect to the RiyadhAI server.

    By default it uses ``RIYADHAI_URL`` from environment"""
    api_key: str | None = None
    """API key to authenticate with RiyadhAI.

    By default it uses ``RIYADHAI_API_KEY`` from environment"""
    api_secret: str | None = None
    """API secret to authenticate with RiyadhAI.

    By default it uses ``RIYADHAI_API_SECRET`` from environment"""
    ...
    """HTTP proxy used to connect to the RiyadhAI server.
...
class AgentServer(utils.EventEmitter[EventTypes]):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        self._ws_url = ws_url or os.environ.get("RIYADHAI_URL") or ""
        self._api_key = api_key or os.environ.get("RIYADHAI_API_KEY") or ""
        self._api_secret = api_secret or os.environ.get("RIYADHAI_API_SECRET") or ""

        self._worker_token = os.environ.get("RIYADHAI_WORKER_TOKEN") or ""  # hosted agents
    ...
    async def run(self, *, devmode: bool = False, unregistered: bool = False) -> None:
        ...
        Args:
            ...
            unregistered (bool, optional):
                ...
                to the central RiyadhAI job dispatcher.
        ...
        async with self._lock:
            ...
            self._api: api.RiyadhAIAPI | None = None
            ...
            async def health_check(_: Any) -> web.Response:
                ...
                if self._connection_failed:
                    return web.Response(status=503, text="failed to connect to riyadhai")
            ...
            if not self._ws_url:
                raise ValueError("ws_url is required, or add RIYADHAI_URL in your environment")

            if not self._api_key:
                raise ValueError("api_key is required, or add RIYADHAI_API_KEY in your environment")

            if not self._api_secret:
                raise ValueError(
                    "api_secret is required, or add RIYADHAI_API_SECRET in your environment"
                )
        ...
        while not self._closed:
            try:
                ...
                if retry_count >= self._max_retry:
                    ...
                    raise RuntimeError(
                        f"failed to connect to riyadhai after {retry_count} attempts",
                    ) from None
                ...
                logger.warning(
                    f"failed to connect to riyadhai, retrying in {retry_delay}s", exc_info=e
                )
