...
def _split_file_in_chunks(reader) -> Generator:
    """
    Build a generator for reading large files in chunks
    """
    b = reader(1024 * 1024)
    while b:
        yield b
...
def count_lines_in_file(file_buffer) -> int:
    """
    Count the number of lines in a file buffer

    :param file_buffer: The file buffer to count lines in,
    should have a raw attribute or a readlines method
    """
    if hasattr(file_buffer, "raw"):
        f_gen = _split_file_in_chunks(file_buffer.raw.read)
        return sum(buf.count(b"\n") for buf in f_gen)
    else:
        return sum(1 for line in file_buffer.readlines())
