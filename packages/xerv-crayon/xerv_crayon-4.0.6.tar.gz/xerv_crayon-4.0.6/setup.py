...
def check_binary(binary_name):
    try:
        ...
        # On some systems, the binary might be in a standard path not in PATH
        # but 'nvcc' is usually standard.
        return False
...
def compile_cuda_kernel(source_file):
    ...
    """
    # Ensure paths are absolute to avoid build temp dir confusion
    ...
    # Get Python Include Path (Critical for <Python.h>)
    py_include = sysconfig.get_path("include")
    
    print(f"[CRAYON BUILD] NVCC compiling: {source_file} -> {output_file}")
    print(f"[CRAYON INFO] Python Includes: {py_include}")
    
    # Flags: Use -Xcompiler -fPIC for Linux shared object compatibility
    flags = ["-O3", "-c"]
    ...
    # We allow nvcc to use its default architecture optimization (JIT/PTX)
    ...
    try:
        ...
        print(f"[!] NVCC Compilation Failed via subprocess: {e}")
        # We RE-RAISE to stop the build, as requested (no fail-safe)
    ...
    def build_extensions(self):
        ...
        if HAS_NVCC or FORCE_CUDA:
            ...
            if os.path.exists(cuda_src):
                ...
                if obj_path:
                    # Find the extension and link the object
                    for ext in self.extensions:
                        if ext.name == "crayon.c_ext.crayon_cuda":
                            # Remove .cu source, add .o object
