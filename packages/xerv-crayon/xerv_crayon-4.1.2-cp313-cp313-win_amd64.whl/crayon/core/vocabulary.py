...
class CrayonVocab:
    ...
    def load_profile(self, name_or_path: str):
        ...
        # Case B: Standard Profile Name (Look strictly in Package Resources)
        else:
            expected_dat = f"vocab_{name_or_path}.dat"
            
            # 1. Try relative lookup (standard installs)
            path = os.path.join(os.path.dirname(__file__), "..", "resources", "dat", expected_dat)
            
            if not os.path.exists(path):
                # 2. Try pkg_resources (virtualenvs / zip-safe)
                try:
                    import pkg_resources
                    path = pkg_resources.resource_filename('crayon', f'resources/dat/{expected_dat}')
                except Exception:
                    path = None

            if not path or not os.path.exists(path):
                raise FileNotFoundError(
                    f"Profile '{name_or_path}' not found locally. "
                    f"Crayon is in LOCAL-ONLY mode. "
                    f"Missing: {expected_dat} in crayon/resources/dat/"
                )
