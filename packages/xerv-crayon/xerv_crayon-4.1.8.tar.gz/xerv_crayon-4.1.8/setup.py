...
def log(msg):
    sys.stderr.write(f"[CRAYON SETUP] {msg}\n")
    sys.stderr.flush()
...
def compile_cuda_kernel(source_file, build_dir):
    try:
        ...
        # FLAGS: The "Hammer" Configuration
        # 1. -allow-unsupported-compiler: Fixes gcc/nvcc mismatch
        # 2. -std=c++17: Matches Python 3.12+
        # 3. Removed specific arch flags to rely on JIT/PTX if needed
        flags = [
            "-O3", "-c", "-std=c++17",
        ...
        # Run explicitly capturing output to stderr so user sees it
        ...
        if HAS_NVCC:
            ...
            if os.path.exists(cuda_src):
                ...
                for ext in self.extensions:
                 ...
                 log(f"Error: CUDA source missing at {cuda_src}")
