...
class CrayonVocab:
    ...
    def load_profile(self, name_or_path: str):
        ...
        Auto-downloads from Hugging Face if profile name is standard (e.g. 'lite', 'code').
        """
        # 1. Resolve Path
        path = None
        
        # Case A: Explicit File Path
        if os.path.exists(name_or_path):
            path = name_or_path
            
        # Case B: Standard Profile Name (Look in Cache -> Package -> Download)
        else:
            cache_dir = os.path.expanduser("~/.cache/xerv/crayon/profiles")
            expected_dat = f"vocab_{name_or_path}.dat"
            expected_json = f"vocab_{name_or_path}.json"
            
            # Check User Cache
            cache_path = os.path.join(cache_dir, expected_dat)
            if os.path.exists(cache_path):
                path = cache_path
            
            # Check Package Resources (Fallback)
            if not path:
                local_res = os.path.join(os.path.dirname(__file__), "..", "resources", "dat", expected_dat)
                if os.path.exists(local_res):
                    path = local_res
            
            # Check Download (Last Resort)
            if not path:
                print(f"[CRAYON] Profile '{name_or_path}' not found locally. Searching Hugging Face Hub...")
                try:
                    ...
                    path = hf_hub_download(
                        repo_id="Xerv/crayon-profiles", 
                        filename=expected_dat,
                        local_dir=cache_dir
                    )
                    
                    # Try Download JSON (Best Effort)
                    try:
                        hf_hub_download(
                            repo_id="Xerv/crayon-profiles",
                            filename=expected_json,
                            local_dir=cache_dir
                        )
                    except Exception:
                        pass # JSON optional for encoding, needed for decoding
                        
                    print(f"[CRAYON] Downloaded and cached to: {path}")
                    
                except ImportError:
                    print("[CRAYON] Hugging Face Hub not installed. Run `pip install huggingface_hub` to auto-download profiles.")
                    raise FileNotFoundError(f"Profile '{name_or_path}' not found and download failed.")
                except Exception as e:
                    raise FileNotFoundError(f"Could not download profile '{name_or_path}': {e}")

        if not path or not os.path.exists(path):
             raise FileNotFoundError(f"Profile resolve failed for: {name_or_path}")

        self.current_profile_path = path

        # --- 2. Load Decoder Mapping (Critical for SLMs) ---
        # We look for a .json file with the same base name
        json_path = os.path.splitext(path)[0] + ".json"
        if os.path.exists(json_path):
            import json
            with open(json_path, "r", encoding="utf-8") as f:
                # List[str] where index == token_id
                self._idx_to_str = json.load(f) 
        else:
            # print(f"[CRAYON] [!] Warning: Companion JSON not found for {path}. 'decode()' will be unavailable.")
            self._idx_to_str = []

        # --- 3. Load DAT Engine (Encoder) ---
