...
def add_version_list_command(subparsers):
    """Add the version-list subcommand"""
    version_list_examples = """
examples:
  # List RHEL versions (default: psql format)
  rhdl version-list

  # List RHEL versions in JSON format
  rhdl version-list --format json

  # List RHEL versions in TSV format
  rhdl version-list --format tsv

  # List RHEL versions in HTML format
  rhdl version-list --format html
"""
    version_list_parser = subparsers.add_parser(
        "version-list",
        help="List RHEL versions",
        description="List all the RHEL versions available.",
        epilog=version_list_examples + COPYRIGHT,
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    version_list_parser.add_argument(
        "--format",
        choices=["tsv", "json", "plain", "simple", "psql", "html"],
        default="psql",
        help="Output format (default: psql)",
    )
    version_list_parser.set_defaults(command="version-list")
...
def parse_arguments(arguments, cwd=None):
    ...
    add_version_list_command(subparsers)
