...
from autots.tools.shaping import drop_leading_na_block
...
eia_key = "jf4MmHXXEZnnC3htIZ75TjkbnRLnvXJGPIaUEIZy"  # EIA https://www.eia.gov/opendata/index.php
noaa_key = "eWGwqDRnzaaDcRsQSOgfXLTZVKQfnUrd"
...
forecast_length = 181  # number of periods to forecast ahead
...
]
{
    ...
    "PreprocessingRegression": 0.1,
    "PreprocessingExperts": 0.3,
}
transformer_list = "all"  #  "scalable"  # 'superfast'
...
ensemble = [
    ...
    'horizontal-min-5',
    'horizontal-profile',
    "mosaic-profile-unpredictability_adjusted-0-10",
    # 'horizontal-min-20',
    # 'horizontal-min-40',
    # "mosaic-mae-crosshair-0-20",
    # "mosaic-weighted-crosshair-0-40",
    "mosaic-weighted-median-0-5",
    ...
    "mosaic-weighted-median-profile",
    # "mosaic-profile",
...
if initial_training:
    ...
    gens = 1000
    generation_timeout = 900  # minutes
    models_to_validate = 0.15
else:
    gens = 0
...
if not csv_load:
    ...
    wikipedia_pages = ['all', 'Microsoft', "Procter_%26_Gamble", "YouTube", "United_States", "Bible", "List_of_presidents_of_the_United_States", "Periodic_table", "Chinese_New_Year", "William_Shakespeare"]
    df = load_live_daily(
        long=False,
        fred_key=fred_key,
        ...
        weather_stations=["USW00014771", "USW00094846", "USW00014925"],
        noaa_cdo_token=noaa_key,
        ...
        sleep_seconds=60, # 60 is slow but pretty reliable
    )
    ...
    # manually scale 'wiki_all' to millions to prevent too much skew of MAE
...
if not initial_training:
    if evolve:
        ...
        # remove slow model
        model.initial_template = model.initial_template[~((model.initial_template["Model"] == "MultivariateRegression") & (model.initial_template["ModelParameters"].str.contains("MLP")))]
        model.validate_import = model.validate_import[~((model.validate_import["Model"] == "MultivariateRegression") & (model.validate_import["ModelParameters"].str.contains("MLP")))]
        model.initial_template = model.initial_template[~((model.initial_template["Model"] == "DatepartRegression") & (model.initial_template["ModelParameters"].str.contains("MLP")))]
        model.validate_import = model.validate_import[~((model.validate_import["Model"] == "DatepartRegression") & (model.validate_import["ModelParameters"].str.contains("MLP")))]
        model.initial_template = model.initial_template[~((model.initial_template["TransformationParameters"].str.contains("KNN")))]
        model.validate_import = model.validate_import[~((model.validate_import["TransformationParameters"].str.contains("KNN")))]
        model.initial_template = model.initial_template[~((model.initial_template["TransformationParameters"].str.contains("ReconciliationTransformer")))]
        model.validate_import = model.validate_import[~((model.validate_import["TransformationParameters"].str.contains("ReconciliationTransformer")))]
...
if initial_training or evolve:
    model.export_template(
        ...
        focus_models=[
            "TVVAR", "Cassandra", "BallTreeMultivariateMotif", "MetricMotif",
            "MultivariateRegression", "BallTreeRegressionMotif", "ARDL", "VAR",
            "KalmanStateSpace", "VECM", "Theta", "SectionalMotif", "FBProphet",
            "SeasonalityMotif", "RRVAR", "DMD", "WindowRegression", "DatepartRegression",
        ],
        unpack_ensembles=True
    )
    model.export_template(
...
if model.best_model_ensemble == 2:
    modseries = model.best_model_params['series']
    firstkey = list(modseries.keys())[0]
    if isinstance(modseries[firstkey], str):
        mod_df = pd.DataFrame(modseries, index=[0])
    else:
        mod_df = pd.DataFrame.from_dict(model.best_model_params['series'])
    top_ids = pd.Series(mod_df.to_numpy().ravel()).value_counts().sort_values(ascending=False).to_frame()
