...
class AsyncTokenDAO:
    ...
    async def refresh(self, refresh_token: str) -> OperationResult[Token]:
        """Refresca un token de acceso usando un refresh token"""
        try:
            tokens_data = await self.client.a_refresh_token(refresh_token)
            token = Token(**tokens_data)
            return OperationResult(
                success=True,
                message="Tokens refrescado exitosamente",
                data=token
            )
        except Exception as e:
