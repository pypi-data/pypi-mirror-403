...
class SingleExecutionClock(TradingClock):
    ...
    def __iter__(self):
        if self.execute_on_period_end_bool:
            bts_minute = self.before_trading_start_minutes[-1]
            session = self.sessions[-1]
            minute = self.minutes_by_session[session][-1]
        else:
            bts_minute = self.before_trading_start_minutes[0]
            session = self.sessions[0]
            minute = self.minutes_by_session[session][0]
        ...
        yield minute, SimulationEvent.SESSION_END
