...
## Phase 1: Database Unification (Completed v0.5.3) ✅
- [x] Consolidate databases into single `data/kladml.sqlite`
- [x] Create SQLModel models matching platform schema:
    - [x] Dataset, Project, Family, Run
- [x] Abstract DB backend
- [x] Remove/archive legacy MLflow DB

## Phase 2: Registry System & Architecture Refactor (Completed v0.5.5) ✅
- [x] `ARCHITECTURE_REGISTRY`: Unified Models under `src/kladml/models/`
- [x] Renamed `BaseArchitecture` -> `BaseModel`
- [x] Renamed storage `models/` -> `registry/`
- [x] Refactored `callbacks.py` into modular package
- [ ] `PREPROCESSOR_REGISTRY`: Built-in transforms (Next)
- [ ] `EVALUATOR_REGISTRY`: Built-in evaluators (Next)
- [ ] CLI: `kladml list ...`

## Phase 3: Core CLI Commands (High Priority) ⭐
- [x] `kladml register preprocessor` (Implemented via `@register_component` decorator + aliases)
...
## Phase 4: Preprocessing Pipeline System (Completed v0.10.2) ✅
- [x] YAML pipeline definition format (`kladml data process`)
- [x] Preprocessor chaining (Standardized DataFrame I/O)
- [x] Schema validation between steps (Polars types)
- [x] CLI: `kladml data process --pipeline <path> --dataset <name>`
- [ ] Auto-split into train/val/test (Partially done via Splitter)
...
### Testing Strategy
- [x] Unit tests for each component (preprocessor, architecture, evaluator)
...
## Completed ✅
- [x] Typed Configuration (Pydantic v2)
- [x] Evaluator Hierarchy Structure
