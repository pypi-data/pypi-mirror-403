...
class db_organiser:
    ...
    def update_dataframe(self, fullpath=True):
        ...
        for db in db_list:
            ...
            if not fullpath:
                result_list.append(
                    [e_id, e_name, exp_date, exp_time, db_name, filesize]
                )
            else:
    ...
    def find_entry(
        ...
        if db_filename:
            ...
            return self.db.loc[
                (self.db["ethoscope_name"] == ethoscope_name)
                & (self.db["experiment_date"] == experiment_date)
                & (self.db["experiment_time"] == experiment_time)
            ]
    ...
    def add_files_to_metadata(self, metadata):
        ...
        """
        metadata[["db_filename", "filesize"]] = metadata.apply(
            self.find_db_file, axis=1, result_type="expand"
        )
        return metadata
...
class metadata_handler:
    ...
    def _create_summary(self):
        ...
        if "db_filename" in self.data:
            self.db_files = (
                self.data.groupby(
                    ["date", "machine_name", "db_filename", "filesize"], dropna=False
                )[["date", "machine_name", "db_filename", "filesize"]]
                .size()
                .to_frame("count")
                .reset_index()
            )
            ...
            self.db_files = (
                self.data.groupby(["date", "machine_name"])[["date", "machine_name"]]
                .size()
                .to_frame("count")
                .reset_index()
            )
...
class db_crawler:
    ...
    def populate_info(self):
        ...
        return {
            "projects": remove_empty(all_projects),
