    ...
    **Example Usage:**
    ...
    def _plot_ylayout(
        ...
        fig["layout"][axis].update(
            linecolor="black",
            type=ytype,
            title=dict(text=ylabel, font=dict(size=18, color="black")),
            rangemode="tozero",
            zeroline=False,
            ticks="outside",
            tickwidth=2,
    ...
    @staticmethod
    def _plot_meanbox(
        mean, median, q3, q1, x, colour, showlegend, name, xaxis, CI=True
    ):
    ...
    def _plot_boxpoints(y, x, colour, showlegend, name, xaxis, marker_size=None):
        ...
        trace_box = go.Box(
            ...
            x=x,
            xaxis=xaxis,
            line=dict(color="rgba(0,0,0,0)"),  # make the box transparent
            fillcolor="rgba(0,0,0,0)",  # make the box transparent
            marker=dict(color=colour, opacity=0.5, size=4),
            boxpoints="all",
            jitter=0.75,
            pointpos=0,
            width=0.9,
    ...
    def _plot_line(df, x_col, name, marker_col):
        ...
        trace = go.Scatter(
            ...
            mode="lines",
            name=name,
            line=dict(shape="spline", color=marker_col),
            fill="tonexty",
        )
    ...
    def heatmap(
        self, variable: str, t_column: str = "t", title="", lights_off: int | float = 12
    ):
        """
        ...
        **Example Usage:**
        ...
        fig = go.Figure(
            data=go.Heatmap(z=data, x=time_list, y=id, colorscale="Viridis")
        )
        ...
        fig.update_layout(
            ...
            xaxis=dict(
                ...
                linecolor="black",
                gridcolor="black",
                title=dict(text="ZT Time (Hours)", font=dict(size=18, color="black")),
                tick0=0,
                dtick=lights_off,
                ticks="outside",
                tickwidth=2,
    ...
    def plot_overtime(
        ...
        **Example Usage:**
        ...
        self._plot_xlayout(
            fig, xrange=False, t0=0, dtick=day_length / 4, xlabel="ZT (Hours)"
        )

        # Light-Dark annotaion bars
        bar_shapes, min_bar = circadian_bars(
            np.nanmin(min_t),
            np.nanmax(max_t),
            min_y=ymin,
            max_y=ymax,
    ...
    def plot_quantify(
        ...
        **Example Usage:**
        ...
        grouped_data, palette_dict, facet_labels, variable = (
            self._internal_plot_quantify(
                variable, facet_col, facet_arg, facet_labels, fun
            )
        )
        ...
        if facet_col:
            for lab in facet_labels:
                ...
                mean, median, q3, q1, zlist = self._zscore_bootstrap(
                    sub_df[plot_column].to_numpy(dtype=float), z_score=z_score
                )
            ...
            lab = " "
            mean, median, q3, q1, zlist = self._zscore_bootstrap(
                grouped_data[plot_column].to_numpy(dtype=float), z_score=z_score
            )
            ...
            fig.add_trace(
                self._plot_meanbox(
                    ...
                    q1=[q1],
                    x=[lab],
                    colour=list(palette_dict.values())[0],
                    showlegend=True,
                    name=lab,
                    xaxis="x",
                )
    ...
    def plot_compare_variables(
        ...
        **Example Usage:**
        ...
        grouped_data, palette_dict, facet_labels, variables = (
            self._internal_plot_quantify(
                variables, facet_col, facet_arg, facet_labels, fun
            )
        )
        ...
        for c, lab in enumerate(facet_labels):
            ...
            for c2, (var, secondary) in enumerate(zip(variables, bool_list)):
                ...
                mean, median, q3, q1, zlist = self._zscore_bootstrap(
                    sub_df[plot_column_list[c2]].to_numpy(dtype=float), z_score=z_score
                )
                ...
                if facet_col is None:
                    lab = facet_labels[
                        c
                    ]  # revert back for axis naming, not the most efficient \_O_O_/
        ...
        self._plot_ylayout(
            ...
            title=title,
            secondary=True,
            xdomain=f"x{axis_counter}",
            grid=grids,
        )

        fig["layout"].update(
            legend=dict(x=1.06, y=0.5)
        )  # shift the legend so it doesn't overlap
    ...
    def plot_day_night(
        ...
        grouped_data, palette_dict, facet_labels = self._internal_plot_day_night(
            variable,
            facet_col,
            facet_arg,
            facet_labels,
        ...
        palette_phase = {
            "light": "goldenrod",
            "dark": "black",
        }  # for use when no facet for extra style
        ...
        for c, phase in enumerate(["light", "dark"]):
            ...
            if facet_col:
                for lab in facet_labels:
                    ...
                    mean, median, q3, q1, zlist = self._zscore_bootstrap(
                        sub_sub_df[plot_column].to_numpy(dtype=float), z_score=z_score
                    )
    ...
    def plot_anticipation_score(
        ...
        Returns:
            ...
            data (pandas.DataFrame): DataFrame with grouped data based on the input parameters.
        """
        grouped_data, palette_dict, facet_labels = (
            self._internal_plot_anticipation_score(
                variable,
                facet_col,
                facet_arg,
                facet_labels,
        ...
        self._plot_ylayout(
            ...
            t0=0,
            dtick=20,
            ylabel="Anticipatory Phase Score",
            title=title,
            grid=grids,
        )
    ...
    def _actogram_plot(fig, data, mov, day, row, col):
        try:
            ...
            for i in range(max_days):
                ...
                y_list = np.append(
                    data[f"{mov}_mean"][data["day"] == i].tolist(),
                    data[f"{mov}_mean"][data["day"] == i + 1].tolist(),
                )
    ...
    def plot_actogram(
        ...
        return seaborn_df.plot_actogram(
            mov_variable=mov_variable,
    ...
    def plot_actogram_tile(
        ...
        return seaborn_df.plot_actogram_tile(
            mov_variable=mov_variable,
            labels=labels,
            bin_window=bin_window,
            day_length=day_length,
    ...
    def survival_plot(
        ...
        if repeat is True:
            if repeat not in self.meta.columns:
                raise KeyError(
                    f'Column "{repeat}" is not a metadata column, please check and add if you want repeat data'
                )
        ...
        if facet_col and facet_arg and repeat:
            data = self.xmv(facet_col, facet_arg).merge(
                self.meta[[facet_col, repeat]], left_index=True, right_index=True
            )
            sur_df = data.groupby(facet_col, group_keys=False).apply(
                partial(
                    self._time_alive,
                    facet_col=facet_col,
                    repeat=repeat,
            ...
            data = self.xmv(facet_col, facet_arg).merge(
                self.meta[[facet_col]], left_index=True, right_index=True
            )
            sur_df = data.groupby(facet_col, group_keys=False).apply(
                partial(
                    self._time_alive,
            ...
            data = self.copy(deep=True)
            sur_df = self._time_alive(
                df=data, facet_col=facet_col, repeat=repeat, t_column=t_column
            )
        ...
        self._plot_xlayout(
            fig, xrange=False, t0=0, dtick=day_length, xlabel="ZT (Hours)"
        )
        ...
        if facet_col is not None:
            ...
            for lab in facet_labels:
                upper, trace, lower, _, _ = self._generate_overtime_plot(
                    data=sur_df,
                    var="survived",
                    name=lab,
            ...
            upper, trace, lower, _, _ = self._generate_overtime_plot(
                ...
                var="survived",
                name=facet_labels[0],
                col=palette_dict[facet_labels[0]],
                avg_win=False,
                wrap=False,
                day_len=False,
                light_off=False,
    ...
    def plot_response_quantify(
        self,
        response_col: str = "has_responded",
        facet_col: None | str = None,
        facet_arg: None | str = None,
        facet_labels: None | str = None,
        z_score: bool = True,
        ...
        grouped_data, h_order, palette_dict = self._internal_plot_response_quantify(
            response_col, facet_col, facet_arg, facet_labels
        )
        ...
        for lab in h_order:
            ...
            mean, median, q3, q1, zlist = self._zscore_bootstrap(
                sub_df[plot_column].to_numpy(), z_score=zscore
            )
        ...
        if facet_col:
            ...
            grouped_data = grouped_data[
                [plot_column, f"{response_col}_std", facet_col]
            ].sort_values(facet_col)

        return fig, grouped_data

        facet_arg, facet_labels = self._check_lists(facet_col, facet_arg, facet_labels)

    def plot_habituation(
        self,
        plot_type: str,
        t_bin_hours: int = 1,
        response_col: str = "has_responded",
        interaction_id_col: str = "has_interacted",
        stim_count: bool = True,
        facet_col: None | str = None,
        facet_arg: None | str = None,
        facet_labels: None | str = None,
        x_limit: bool = False,
        ...
        seconday_label = {
            "time": "No. of stimulus (absolute)",
            "number": "% recieving stimulus",
        }

        # call the internal method to curate and analse data, see behavpy_draw
        grouped_data, h_order, palette_dict, x_max, plot_choice = (
            self._internal_plot_habituation(
                plot_type=plot_type,
                t_bin_hours=t_bin_hours,
        ...
        if stim_count is False:
            ...
            self._plot_ylayout(
                ...
                t0=0,
                dtick=False,
                ylabel=seconday_label[plot_type],
                title=title,
                secondary=True,
                xdomain="x1",
                grid=grids,
        ...
        self._plot_xlayout(
            fig, xrange=[0, x_max], t0=0, dtick=False, xlabel=plot_choice
        )
        ...
        for hue in h_order:
            ...
            upper, trace, lower = self._plot_line(
                df=sub_df, x_col=plot_choice, name=hue, marker_col=palette_dict[hue]
            )
            fig.add_trace(upper)
            fig.add_trace(trace)
            fig.add_trace(lower)
            ...
            if stim_count is True and "-True Stimulus" in hue:
                ...
                fig.add_trace(
                    go.Scatter(
                        ...
                        name=f"{hue} count",
                        line=dict(
                            dash="longdashdot", shape="spline", color=palette_dict[hue]
                        ),
                    ),
                    secondary_y=True,
                )
    ...
    def plot_response_over_activity(
        self,
        mov_df,
        activity: str,
        variable: str = "moving",
        ...
        # call the internal method to curate and analse data, see behavpy_draw
        grouped_data, h_order, palette_dict = self._internal_bout_activity(
            mov_df=mov_df,
            activity=activity,
            variable=variable,
            response_col=response_col,
        ...
        self._plot_xlayout(
            ...
            t0=0,
            dtick=1,
            xlabel=f"Consecutive minutes in behavioural bout ({activity})",
        )
        ...
        for hue in h_order:
            ...
            upper, trace, lower = self._plot_line(
                df=sub_df,
                x_col="previous_activity_count",
                name=hue,
                marker_col=palette_dict[hue],
            )
            fig.add_trace(upper)
            fig.add_trace(trace)
            fig.add_trace(lower)
    ...
    def plot_response_overtime(
        ...
        Notes:
            This function must be called on a behavpy dataframe that is populated with data loaded with the stimulus_response
                analysing function. Contain columns such as 'has_responded' and 'has_interacted'.
        """
        df, h_order, palette = self._internal_plot_response_overtime(
            t_bin_hours=t_bin_hours,
            response_col=response_col,
            interaction_id_col=interaction_id_col,
            facet_col=facet_col,
        ...
        return df.plot_overtime(
            variable="Response Rate",
            wrapped=wrapped,
            facet_col="new_facet",
            facet_arg=h_order,
            facet_labels=h_order,
    ...
    def plot_periodogram(
        ...
        self._plot_xlayout(
            fig,
            xrange=[min(self[period_var]), max(self[period_var])],
            tickvals=tick_6,
            ticktext=tick_6,
            xlabel="Period Frequency (Hours)",
        )
    ...
    def plot_wavelet(
        ...
        Returns:
            fig (plotly.figure.Figure): Figure object of the plot.
        """
        fun, avg_data = self._format_wavelet(
            mov_variable, sampling_rate, wavelet_type, t_col
        )
        ...
        self._plot_ylayout(
            fig,
            yrange=np.log2([2, 38]),
            tickvals=np.log2(yticks),
            ticktext=yticks,
            title=title,
            ylabel="Period Frequency (Hours)",
            grid=grids,
        )
    ...
    def plot_hmm_overtime(
        ...
        states_list, time_list = self._hmm_decode(
            df, hmm, t_bin, variable, func, t_column
        )
        ...
        for state_list, t in zip(states_list, time_list):
            tdf = hmm_pct_state(
                ...
                avg_window=int((avg_window * 60) / t_bin),
            )
            df = pd.concat([df, tdf], ignore_index=True)

        df.rename(
            columns=dict(zip([f"state_{c}" for c in range(0, len(labels))], labels)),
            inplace=True,
        )
        melt_df = df.melt("t")
        m = melt_df[["variable"]]
    ...
    def plot_hmm_split(
        ...
        if facet_col is not None:
            ...
            colours = [
                self._check_grey(name, colours[c])[1]
                for c, name in enumerate(facet_labels)
            ]  # change to grey if control
        ...
        ):  # if only 1 hmm with either facet_col or not, decode as whole for efficiency
            df = self.__class__(
                self._hmm_decode(
                    df, hmm, t_bin, variable, func, t_column, return_type="table"
                ),
                df.meta,
                check=True,
            )
        ...
        for c, (arg, n, h, b) in enumerate(
            ...
            if isinstance(hmm, list) is True:  # call the decode here if multiple HMMs
                states_list, time_list = self._hmm_decode(
                    sub_df, h, b, variable, func, t_column
                )
            else:
            ...
            for state_list, t in zip(states_list, time_list):
                temp_df = hmm_pct_state(
                    state_list, t, [0, 1, 2, 3], avg_window=int((avg_window * 60) / b)
                )
                analysed_df = pd.concat([analysed_df, temp_df], ignore_index=False)

            if wrapped is True:
                analysed_df["t"] = analysed_df["t"].map(
                    lambda t: t % (60 * 60 * day_length)
                )
            ...
            for i, (lab, row, col) in enumerate(zip(labels, row_list, col_list)):
                ...
                if facet_col is None:
                    upper, trace, lower = self._plot_line(
                        df=gb_df, x_col="t", name=n, marker_col=colours[i]
                    )
                else:
                    upper, trace, lower = self._plot_line(
                        df=gb_df, x_col="t", name=n, marker_col=colours[c]
                    )
    ...
    def plot_hmm_quantify(
        ...
        grouped_data, labels, colours, facet_col, facet_labels, palette_dict = (
            self._internal_plot_hmm_quantify(
                hmm,
                variable,
                labels,
                colours,
        ...
        if col_uniform:  # change the palette to be state labels rather than facet labe
            palette_dict = {
                name: self._check_grey(name, colours[c])[1]
        ...
        for c, state in enumerate(labels):
            ...
            if facet_col:
                for lab in facet_labels:
                    ...
                    try:
                        mean, median, q3, q1, zlist = self._zscore_bootstrap(
                            sub_sub_df[plot_column].to_numpy(dtype=float),
                            z_score=z_score,
                        )
                    except IndexError:
    ...
    def plot_hmm_quantify_length(
        ...
        """
        grouped_data, labels, colours, facet_col, facet_labels, palette_dict = (
            self._internal_plot_hmm_quantify_length(
                hmm,
                variable,
                labels,
                colours,
    ...
    def plot_hmm_quantify_length_min_max(
        self,
        hmm,
        variable: str = "moving",
        labels: list = None,
        ...
        grouped_data, labels, colours, facet_col, facet_labels, palette_dict = (
            self._internal_plot_hmm_quantify_length_min_max(
                hmm,
                variable,
                labels,
                colours,
        ...
        for c, state in enumerate(labels):
            ...
            if facet_col:
                ...
                mean, median, q3, q1, zlist = self._zscore_bootstrap(
                    sub_df[plot_column].to_numpy(dtype=float),
                    min_max=True,
                    z_score=False,
                )
    ...
    def plot_hmm_quantify_transition(
        self,
        hmm,
        variable: str = "moving",
        labels: list = None,
        ...
        grouped_data, labels, colours, facet_col, facet_labels, palette_dict = (
            self._internal_plot_hmm_quantify_transition(
                hmm,
                variable,
                labels,
                colours,
    ...
    def plot_hmm_raw(
        ...
        if stim_df is not None:
            assert isinstance(
                stim_df, self.__class__
            ), "The stim_df dataframe is not behavpy dataframe"

        if isinstance(hmm, list):
            num_plots = len(hmm)
            rand_flies = [
                np.random.permutation(list(set(self.meta.index)))[0]
            ] * num_plots
            h_list = hmm
        ...
        decoded = [
            self._hmm_decode(d, h, b, variable, func, t_column, return_type="table")
            .dropna()
            .set_index("id")
            for d, h, b in zip(df_list, h_list, b_list)
        ]
        ...
        self._plot_ylayout(
            fig, yrange=y_range, t0=0, dtick=False, ylabel="", title=title
        )
        ...
        for c, (df, b) in enumerate(zip(decoded, b_list)):
            ...
            min_t.append(
                int(
                    ...
                    * floor(df["bin"].min() / (day_length / lights_off))
                )
            )
            max_t.append(
                int(
                    (day_length / lights_off)
                    * floor(df["bin"].max() / (day_length / lights_off))
                )
            )
        ...
        fig.update_yaxes(
            title=dict(text="Predicted State", font=dict(size=18, color="black")),
            row=ceil(num_plots / 2),
            col=1,
        )
        fig.update_xaxes(
            title=dict(text="ZT Hours", font=dict(size=18, color="black")),
            row=num_plots,
            col=1,
        )
    ...
    def plot_hmm_response(
        ...
        if response_col not in self.columns.tolist():
            raise KeyError(
                f"The column you gave {response_col}, is not in the data. Check you have analysed the dataset with stimulus_response"
            )

        labels, colours = self._check_hmm_shape(hm=hmm, lab=labels, col=colours)
        facet_arg, facet_labels, hmm, t_bin = self._check_lists_hmm(
            facet_col, facet_arg, facet_labels, hmm, t_bin
        )

        if (
            isinstance(hmm, list) and facet_col is None
        ):  # End method is trying to have mutliple HMMs with no facet
            raise RuntimeError(
                "This method does not support multiple HMMs and no facet_col"
            )
    ...
    def plot_response_over_hmm_bouts(
        self,
        mov_df,
        hmm,
        variable: str = "moving",
        ...
        grouped_data, palette_dict, h_order = self._bouts_response(
            mov_df=mov_df,
            hmm=hmm,
            variable=variable,
            response_col=response_col,
        ...
        self._plot_xlayout(
            ...
            t0=0,
            dtick=t_bin / 60,
            xlabel="Consecutive minutes in state",
        )
    ...
    def make_tile(self, facet_tile, plot_fun, rows=None, cols=None):
        ...
        fig = make_subplots(
            rows=rows, cols=cols, shared_xaxes=True, subplot_titles=tile_list
        )
