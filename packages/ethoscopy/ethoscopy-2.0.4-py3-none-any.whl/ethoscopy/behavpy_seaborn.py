...
class behavpy_seaborn(behavpy_draw):
    ...
    **Example Usage:**
        ...
        **Example Usage:**
        ...
        if col_list is None:
            ...
            if len(col_list) != len(facet_labels):
                raise ValueError(
                    "Given col_list is of different length to the facet_arg list"
                )
        ...
        if figsize == (0, 0):
            figsize = (6 + 1 / 3 * len(x_ticks), 4 + 1 / 24 * len(x_ticks))
        ...
        if facet_col:
            ...
            )  # legend outside of area if faceted
        plt.title(title)
        ...
        bar_range, thickness = circadian_bars(
            t_min,
            t_max,
            min_y=ymin,
            max_y=ymax,
        ...
        for ax, var in zip(axes, variable):
            ...
            if facet_col:
                ...
                sns.stripplot(
                    data=grouped_data,
                    y=plot_column,
                    color=list(palette_dict.values())[0],
                    ax=ax,
                    alpha=0.5,
                    legend=False,
                )
    ...
    def plot_day_night(
        ...
        if facet_col:
            sns.stripplot(
                ...
                order=["light", "dark"],
                hue=facet_col,
                hue_order=facet_labels,
                ax=ax,
                palette=palette_dict,
                alpha=0.5,
                legend=False,
                dodge=0.8 - 0.8 / len(facet_labels),
            )
            sns.pointplot(
                data=grouped_data,
                x="phase",
            ...
            sns.stripplot(
                ...
                ax=ax,
                hue="phase",
                palette={"light": "gold", "dark": "darkgrey"},
                alpha=0.5,
                legend=False,
            )
            sns.pointplot(
    ...
    def plot_anticipation_score(
        ...
        if facet_col:
            sns.stripplot(
                data=grouped_data,
                x="phase",
                y="anticipation_score",
                order=["Lights On", "Lights Off"],
                ax=ax,
                hue=facet_col,
                hue_order=facet_labels,
                palette=palette_dict,
            ...
            sns.stripplot(
                ...
                ax=ax,
                hue="phase",
                palette={"Lights On": "gold", "Lights Off": "darkgrey"},
                alpha=0.5,
                legend=False,
            )
            sns.pointplot(
    ...
    def _internal_actogram(data, mov_variable, bin_window, t_column, facet_col):
        ...
        if facet_col:
            data = (
                data.groupby([f"{t_column}_bin", facet_col])
                .agg(
                    **{
    ...
    def plot_actogram(
        ...
        data, days = self._internal_actogram(
            data, mov_variable, bin_window, t_column, facet_col
        )
        ...
        if facet_col:
            ...
            for subplot in facet_arg:
                ...
                fig = self._plot_single_actogram(
                    dt, figsize, days, title, day_length, sub_title_size
                )
                plt.close()
            ...
            sub_title_size = 25
            return self._plot_single_actogram(
                data, figsize, days, title, day_length, sub_title_size
            )
    ...
    def plot_actogram_tile(
        ...
        # call the internal actogram augmentor
        data, days = self._internal_actogram(
            self, mov_variable, bin_window, t_column, facet_col="id"
        )
        ...
        for subplot, label in zip(facet_arg, title_list):
            ...
            subtitle = "%s" % (label)
            fig = self._plot_single_actogram(
                dt, figsize, days, subtitle, day_length, sub_title_size
            )
            plt.close()
    ...
    def survival_plot(
        ...
        # (0,0) means automatic size
        if figsize == (0, 0):
            figsize = (6 + 1 / 4 * len(x_ticks), 4 + 1 / 32 * len(x_ticks))
    ...
    def plot_habituation(
        ...
        for hue in h_order:
            ...
            ax.plot(
                sub_df[plot_choice], sub_df["mean"], label=hue, color=palette_dict[hue]
            )
            ax.fill_between(
                sub_df[plot_choice],
                sub_df["y_min"],
    ...
    def plot_response_over_activity(
        ...
        for hue in h_order:
            ...
            if len(sub_df) == 0:
                continue
            plt.plot(
                sub_df["previous_activity_count"],
                sub_df["mean"],
                label=hue,
                color=palette_dict[hue],
            )
    ...
    def plot_periodogram_tile(
        self, labels=None, find_peaks=False, title="", grids=False, figsize=(0, 0)
    ):
        """
    ...
    def plot_wavelet(
        ...
        CS = ax.contourf(
            t, per, power, levels=levels, cmap=self.attrs["sh_pal"], extend="min"
        )

        plt.colorbar(CS, label="Power")

        if figsize == (0, 0):
            figsize = (15 + (1 / 24 * len(t)), 12)
            fig.set_size_inches(figsize)
    ...
    def plot_hmm_split(
        ...
        for c, arg in enumerate(facet_arg):
            ...
            for state_list, t in zip(states_list, time_list):
                tdf = hmm_pct_state(
                    state_list, t, list(range(len(labels))), avg_window=5
                )
            ...
            states_df.rename(
                columns=dict(
                    zip([f"state_{c}" for c in range(0, len(labels))], labels)
                ),
                inplace=True,
            )
        ...
        for c, state in enumerate(labels):
            ...
            plot_m = pd.DataFrame(
                data={
                    "id": list(set(plot_df["facet_col"])),
                    "facet_col": list(set(plot_df["facet_col"])),
                }
            )
    ...
    def plot_hmm_quantify(
        ...
        grouped_data, labels, _, facet_col, facet_labels, palette_dict = (
    ...
    def plot_hmm_raw(
        ...
        for ax, data in enumerate(decoded):
            ...
            min_t.append(
                int(
                    (day_length / lights_off)
                    * floor(data["bin"].min() / (day_length / lights_off))
                )
            )
            max_t.append(
                int(
                    (day_length / lights_off)
                    * floor(data["bin"].max() / (day_length / lights_off))
                )
            )
        ...
        if np.max(max_t) - np.min(min_t) >= 24:
            x_ticks = np.arange(
                np.min(min_t), np.max(max_t) + lights_off, lights_off, dtype=int
            )
