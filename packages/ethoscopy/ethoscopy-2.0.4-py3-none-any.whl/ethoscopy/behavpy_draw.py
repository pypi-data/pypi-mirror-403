...
class behavpy_draw(behavpy_core):
    ...
    def _check_hmm_shape(self, hm, lab, col):
        ...
        if lab is not None and col is not None:
            if num_states == len(lab) and num_states == len(col):
                ...
                raise ValueError(
                    "The number of labels and colours does not match the number of states in the HMM"
                )
        ...
        if len(lab) != len(col):
            raise ValueError(
                "Internal check failed: There are more or less states than colours"
            )
    ...
    def _check_lists_hmm(self, f_col, f_arg, f_lab, h, b):
        ...
        if (
            f_col is not None and f_arg is not None and f_lab is not None
        ):  # is user provides all, just check for length match
            if len(f_arg) != len(f_lab):
                print(
                    "The facet labels don't match the length of the variables in the column. Using column variables names instead"
                )
                f_lab = [str(arg) for arg in f_arg]
            return f_arg, f_lab, h_list, b_list

        if (
            f_col is not None and f_arg is not None and f_lab is None
        ):  # if user provides a facet column and args but no labels
            f_lab = [str(arg) for arg in f_arg]
            return f_arg, f_lab, h_list, b_list

        if (
            f_col is not None and f_arg is None and f_lab is None
        ):  # if user provides a facet column but no args or labels
            f_arg = list(set(self.meta[f_col].tolist()))
            f_lab = [str(arg) for arg in f_arg]
            return f_arg, f_lab, h_list, b_list

    @staticmethod
    def _zscore_bootstrap(
        array: np.array,
        z_score: bool = True,
        second_array: np.array = None,
        min_max: bool = False,
    ):
        """
    ...
    def _get_colours(self, plot_list):
        ...
        if self.canvas == "plotly":
            if pl_len <= len(getattr(qualitative, self.attrs["sh_pal"])):
                ...
                raise IndexError(
                    "Too many sub groups to plot with the current colour palette (max is 48)"
                )
    ...
    def _adjust_colours(self, colour_list):
        ...
        def adjust_color_lighten(r, g, b, factor):
            ...
            """
            return [
                round(255 - (255 - r) * (1 - factor)),
                round(255 - (255 - g) * (1 - factor)),
                round(255 - (255 - b) * (1 - factor)),
            ]
    ...
    def _check_grey(name, col, response=False):
        ...
        if response is False:
            if (
                ...
                or "ctrl" in name.lower()
                or "spon. mov" in name.lower()
            ):
                col = "grey"
        else:
            if (
    ...
    def _generate_overtime_plot(
        self, data, name, col, var, avg_win, wrap, day_len, light_off, t_col
    ):
        """
        ...
        if avg_win:
            rolling_col = (
                data.groupby(data.index, sort=False)[var]
                .rolling(avg_win, min_periods=1)
                .mean()
        ...
        if self.canvas == "seaborn":
            ...
            upper, trace, lower = self._plot_line(
                df=gb_df, x_col=t_col, name=name, marker_col=col
            )
    ...
    def heatmap_dataset(self, variable, t_column):
        ...
        def align_data(data):
            ...
            df = data.merge(time_map, how="right", on="t_bin", copy=False).sort_values(
                by=["t_bin"]
            )
    ...
    def _hmm_response(
        ...
        if facet_col is None:  # decode the whole dataset
            mdata = self.__class__(
                self._hmm_decode(
                    mdata, hmm, t_bin, variable, func, t_column, return_type="table"
                ),
                mdata.meta,
                check=True,
            )
            ...
            if (
                isinstance(hmm, list) is False
            ):  # if only 1 hmm but is faceted, decode as whole for efficiency
                mdata = self.__class__(
                    self._hmm_decode(
                        mdata, hmm, t_bin, variable, func, t_column, return_type="table"
                    ),
                ...
                mdata = concat(
                    *[
                        ...
                        for arg, h, b in zip(facet_arg, hmm, t_bin)
                    ]
                )
        ...
        def alter_merge(response, mov, tb):
            ...
            """
            response["bin"] = response["interaction_t"].map(
                lambda t: tb * floor(t / tb)
            )
            ...
            merged["previous_state"] = np.where(
                merged["t_check"] > merged["bin"],
                merged["state"],
        ...
        if (
            isinstance(t_bin, list) is False
        ):  # if only 1 bin but is faceted, apply to whole df
            data = self.__class__(
                alter_merge(data, mdata, t_bin), data.meta, check=True
            )
        else:
            data = concat(
                *[
                    self.__class__(
                        alter_merge(
                            data.xmv(facet_col, arg), mdata.xmv(facet_col, arg), b
                        ),
                        data.meta,
                        check=True,
                    )
                    for arg, b in zip(facet_arg, t_bin)
                ]
            )

        grouped_data = data.groupby(
            [data.index, "previous_state", "has_interacted"]
        ).agg(**data_summary)
        ...
        if facet_col is not None:
            h_order = [
                ...
                for ty in ["Spon. Mov.", "True Stimulus"]
            ]
        ...
        palette_dict = {
            name: self._check_grey(name, palette[c], response=True)[1]
            for c, name in enumerate(h_order)
        }  # change to grey if control
    ...
    def _bouts_response(
        ...
        try:
            data = self.drop(columns=["moving"]).copy(
                deep=True
            )  # if 'moving' is in the response dataset then it messes up the merge
        except KeyError:
            data = self.copy(deep=True)
        ...
        if hmm is not False:
            ...
            mdata = self.__class__(
                self._hmm_decode(
                    mdata, hmm, t_bin, variable, func, t_col, return_type="table"
                ),
                mdata.meta,
                check=True,
            )
        ...
        merged["previous_activity_count"] = np.where(
            merged["t_check"] > merged[t_col],
            merged["activity_count"],
            merged["previous_activity_count"],
        )
        merged["previous_moving"] = np.where(
            merged["t_check"] > merged[t_col],
            merged["moving"],
            merged["previous_moving"],
        )
        merged = merged[merged["previous_activity_count"] <= x_limit]
        merged.dropna(
            subset=["previous_moving", "previous_activity_count"], inplace=True
        )
        merged["previous_activity_count"] = merged["previous_activity_count"].astype(
            int
        )

        # groupby the columns of interest, and find the mean and bootstrapped 95% CIs
        grouped_data = merged.groupby(
            ["previous_moving", "previous_activity_count", "has_interacted"]
        ).agg(**data_summary)
        grouped_data = grouped_data.reset_index()
        grouped_data[["y_max", "y_min"]] = pd.DataFrame(
            grouped_data["ci"].tolist(), index=grouped_data.index
        )
        ...
        grouped_data["label_col"] = (
            grouped_data["state"] + "-" + grouped_data["has_interacted"]
        )

        # create the order of plotting and double the colours to assign grey to false stimuli
        h_order = [
            f"{lab}-{ty}" for lab in labels for ty in ["Spon. Mov.", "True Stimulus"]
        ]
    ...
    def _internal_bout_activity(
        ...
        if activity not in activity_choice.keys():
            raise KeyError(
                f"activity argument must be one of {*activity_choice.keys(),}"
            )
        if activity == "both" and facet_col is not None:
            print(
                "When plotting both inactive and active runs you can not use facet_col. Reverted to None"
            )
        ...
        if facet_col and facet_arg:
            ...
            dfs = [
                rdata._bouts_response(
                    mov_df=mov_df.xmv(facet_col, arg),
                    hmm=False,
                    variable=variable,
                ...
                for arg, lab in zip(facet_arg, facet_labels)
            ]
            grouped_data = pd.concat(dfs)
        else:
            grouped_data = self._bouts_response(
                mov_df=mov_df,
                hmm=False,
                variable=variable,
                response_col=response_col,
            ...
            grouped_data["facet_col"] = grouped_data["previous_moving"].map(
                inverse_dict
            )
        ...
        if facet_col:
            palette = [
                x
                for xs in [[col, col] for col in self._get_colours(facet_labels)]
                for x in xs
            ]
            h_order = [
                f"{lab} {ty}"
            ...
            palette = [
                x
                for xs in [
                    [col, col] for col in self._get_colours(list(inverse_dict.values()))
                ]
                for x in xs
            ]
            h_order = [
                f"{lab} {ty}"
                for lab in list(inverse_dict.values())
                for ty in ["Spon. Mov.", "True Stimulus"]
            ]
        palette_dict = {
            name: self._check_grey(name, palette[c], response=True)[1]
        ...
        if isinstance(activity_choice[activity], int):
            grouped_data = grouped_data[
                grouped_data["previous_moving"] == activity_choice[activity]
            ]

        return grouped_data, h_order, palette_dict

    def _internal_plot_response_overtime(
        self,
        t_bin_hours,
        response_col,
        interaction_id_col,
        ...
        if (
            len(set(data[interaction_id_col])) == 1
        ):  # if only stimulus type in the dataset
            # get colours
            palette = self._get_colours(facet_labels)
            h_order = [
                f"{lab}-{ty}" for lab in facet_labels for ty in ["True Stimulus"]
            ]
            ...
            data1 = self.__class__(
                data[data[interaction_id_col] == 1].bin_time(
                    response_col,
                    (60 * 60) * t_bin_hours,
                    function=func,
                    t_column=t_column,
            ...
            data2 = data[data[interaction_id_col] == 2].bin_time(
                response_col, (60 * 60) * t_bin_hours, function=func, t_column=t_column
            )
        ...
        palette = [
            self._check_grey(name, palette[c], response=True)[1]
            for c, name in enumerate(h_order)
        ]  # change to grey if control

        grouped_data = (
            data.groupby([data.index, "t_bin"]).agg(**data_summary).reset_index(level=1)
        )
    ...
    def _internal_plot_habituation(
        self,
        plot_type,
        t_bin_hours,
        response_col,
        ...
        plot_choice = {
            "time": f"Hours {t_bin_hours} post first stimulus",
            "number": "Stimulus number post first",
        }
        ...
        def get_response(int_data, ptype, time_window_length, resp_col, t_col):
            ...
            if ptype == "time":
                ...
                int_data[plot_choice[plot_type]] = int_data[t_col].map(
                    lambda t: hour_secs * floor(t / hour_secs)
                )
                min_hour = int_data[plot_choice[plot_type]].min()
                int_data[plot_choice[plot_type]] = (
                    int_data[plot_choice[plot_type]] - min_hour
                ) / hour_secs
            ...
            # Sort the responses by time, assign int according to place in the list, return as dataframe
                ...
                return pd.DataFrame(
                    data={
                        "has_responded": int_data["has_responded"].tolist(),
                        plot_choice[plot_type]: int_data["n_stim"].tolist(),
                        "stim_count": [1] * len(int_data),
                    }
                ).set_index(plot_choice[plot_type])

        grouped_data = data.groupby([data.index, interaction_id_col]).apply(
            partial(
                get_response,
                ptype=plot_type,
                time_window_length=t_bin_hours,
                resp_col=response_col,
                t_col=t_column,
            ),
            include_groups=False,
        )
        grouped_data = self.__class__(
            grouped_data.reset_index().set_index("id"), data.meta, check=True
        )

        # reduce dataset to the maximum value of the True stimulus (reduces computation time)
        if x_limit is False:
            x_max = np.nanmax(
                grouped_data[grouped_data[interaction_id_col] == 1][
                    plot_choice[plot_type]
                ]
            )
        else:
            x_max = x_limit
        ...
        grouped_data[interaction_id_col] = grouped_data[interaction_id_col].map(
            map_dict
        )
        if facet_col:
            grouped_data = self.facet_merge(
                grouped_data, self.meta, facet_col, facet_arg, facet_labels
            )
            grouped_data[facet_col] = (
                grouped_data[facet_col].astype(str)
                + "-"
                + grouped_data[interaction_id_col]
            )
        else:
            facet_col = "stim_type"
        ...
        grouped_final = (
            grouped_data.groupby([facet_col, plot_choice[plot_type]])
            .agg(**data_summary)
            .reset_index(level=1)
        )
        grouped_final[["y_max", "y_min"]] = pd.DataFrame(
            grouped_final["ci"].tolist(), index=grouped_final.index
        )
    ...
    def _internal_plot_quantify(
        self, variable, facet_col, facet_arg, facet_labels, fun
    ):
        """
        ...
        for var in variable:
            data_summary.update(
                {f"{var}_{fun}": (var, fun), f"{var}_std": (var, "std")}
            )
        ...
        if facet_col:
            palette = self._get_colours(facet_labels)
            palette_dict = {
                name: self._check_grey(name, palette[c])[1]
            ...
            palette_dict = {
                name: self._check_grey(name, palette[c])[1]
                for c, name in enumerate(variable)
            }  # change to grey if control

        return grouped_data, palette_dict, facet_labels, variable

    def _internal_plot_response_quantify(
        self, response_col, facet_col, facet_arg, facet_labels
    ):
        """
        ...
        if response_col not in self.columns.tolist():
            raise KeyError(
                f"The column you gave {response_col}, is not in the data. Check you have analyed the dataset with stimulus_response"
            )
        ...
        if facet_col and facet_arg:
            ...
            grouped_data = (
                data.groupby([data.index, "has_interacted"])
                .agg(**data_summary)
                .reset_index(level=1)
                .merge(self.meta[[facet_col]], left_index=True, right_index=True)
            )
        ...
        # this applies in case we want to apply the specified information to ALL the data
            ...
            grouped_data = (
                self.groupby([self.index, "has_interacted"])
                .agg(**data_summary)
                .copy(deep=True)
                .reset_index()
            )
    ...
    def _internal_plot_day_night(
        ...
        data.add_day_phase(
            day_length=day_length, lights_off=lights_off, t_column=t_column
        )

        grouped_data = (
            data.groupby([data.index, "phase"], observed=True)
            .agg(**data_summary)
            .reset_index(1)
        )
    ...
    def _internal_plot_anticipation_score(
        self,
        variable,
        facet_col,
        facet_arg,
        ...
        grouped_data = data.anticipation_score(
            variable, day_length, lights_off, t_column
        )
    ...
    def _internal_plot_decoder(
        ...
        facet_arg, facet_labels, h_list, b_list = self._check_lists_hmm(
            facet_col, facet_arg, facet_labels, hmm, t_bin
        )
        ...
        if facet_col is None:  # decode the whole dataset
            if isinstance(hmm, list):
                ...
                decoded_data = self.__class__(
                    self._hmm_decode(
                        data, hmm, t_bin, variable, func, t_column, return_type="table"
                    ),
                    data.meta,
                    check=True,
                )
            ...
            if isinstance(hmm, list):
                decoded_data = concat(
                    *[
                        ...
                        for arg, h, b in zip(facet_arg, h_list, b_list)
                    ]
                )
    ...
    def _internal_plot_hmm_quantify(
        self,
        hmm,
        variable,
        labels,
        ...
        decoded_data, labels, colours, facet_col, facet_arg, facet_labels = (
            self._internal_plot_decoder(
                hmm,
                variable,
                labels,
                colours,
        ...
        grouped_data = decoded_data.groupby(
            [decoded_data.index, "state"], sort=False
        ).agg({"bin": "count"})
        grouped_data = grouped_data.join(
            decoded_data.groupby("id", sort=False).agg({"previous_state": "count"})
        )
        grouped_data["Fraction of time in each State"] = (
            grouped_data["bin"] / grouped_data["previous_state"]
        )
    ...
    def _internal_plot_hmm_quantify_length(
        self,
        hmm,
        variable,
        labels,
        ...
        states = decoded_data.groupby(decoded_data.index, sort=False)["state"].apply(
            list
        )
        df_lengths = []
        ...
        grouped_data.rename(
            columns={"mean_length": "Length of state bout (mins)"}, inplace=True
        )
    ...
    def _internal_plot_hmm_quantify_length_min_max(
        self,
        hmm,
        variable,
        labels,
        ...
        grouped_data = pd.concat(df_lengths)
        grouped_data.rename(
            columns={"length_adjusted": "Length of state bout (mins)"}, inplace=True
        )
        grouped_data.set_index("id", inplace=True)
    ...
    def _internal_plot_hmm_quantify_transition(
        self,
        hmm,
        variable,
        labels,
        ...
        df_list = []
        for state_list, id in zip(states, states.index):
            length = hmm_pct_transition(
                state_list, total_states=list(range(len(labels)))
            )
            length["id"] = [id] * len(length)
        ...
        grouped_data = (
            grouped_data.set_index("id").stack().reset_index().set_index("id")
        )
        grouped_data.rename(
            columns={"level_1": "state", 0: "Fraction of transitions into each state"},
            inplace=True,
        )
