...
class GitRepo:
    repo = None
    atlas_ignore_file = None
    atlas_ignore_spec = None
    atlas_ignore_ts = 0
    atlas_ignore_last_check = 0
    subtree_only = False
    ignore_file_cache = {}
    git_repo_error = None
    ...
    def __init__(
        ...
        if atlas_ignore_file:
            self.atlas_ignore_file = Path(atlas_ignore_file)

    def commit(self, fnames=None, context=None, message=None, atlas_edits=False, coder=None):
        """
        ...
        Args:
            ...
            atlas_edits (bool, optional): Whether the changes were made by Atlas. Defaults to False.
        ...
        Atlas generated the changes (`atlas_edits`).
        ...
        - atlas_edits=True: Changes were generated by Atlas (LLM).
        - atlas_edits=False: Commit is user-driven (e.g., /commit manually staged changes).
        ...
        Flags:
        - --attribute-author: Modify Author name to "User Name (atlas)".
        - --attribute-committer: Modify Committer name to "User Name (atlas)".
        - --attribute-co-authored-by: Add
          "Co-authored-by: atlas (<model>) <atlas@atlas.chat>" trailer to commit message.

        Behavior Summary:

        1. When atlas_edits = True (AI Changes):
        ...
        - AI edit with Co-authored-by + Explicit Author: Co-authored-by=True,
          --attribute-author -> Author=You(atlas), Committer=You, Trailer added
        - User commit (defaults): atlas_edits=False -> Author=You, Committer=You(atlas)
        - User commit with explicit no-committer: atlas_edits=False,
        ...
        prefix_commit_message = atlas_edits and (
        ...
        if atlas_edits and attribute_co_authored_by:
            model_name = "unknown-model"
            if coder and hasattr(coder, "main_model") and coder.main_model.name:
                model_name = coder.main_model.name
            commit_message_trailer = f"\n\nCo-authored-by: atlas ({model_name}) <atlas@atlas.chat>"

        # Determine if author/committer names should be modified
        # Author modification applies only to atlas edits.
        # It's used if effective_author is True AND
        # (co-authored-by is False OR author was explicitly set).
        use_attribute_author = (
            atlas_edits and effective_author and (not attribute_co_authored_by or author_explicit)
        )

        # Committer modification applies regardless of atlas_edits (based on tests).
        # It's used if effective_committer is True AND
        # (it's not an atlas edit with co-authored-by OR committer was explicitly set).
        use_attribute_committer = effective_committer and (
            not (atlas_edits and attribute_co_authored_by) or committer_explicit
        )
        ...
        if prefix_commit_message:
            commit_message = "atlas: " + commit_message
        ...
        committer_name = f"{original_user_name} (atlas)"
    ...
    def refresh_atlas_ignore(self):
        if not self.atlas_ignore_file:
            return

        current_time = time.time()
        if current_time - self.atlas_ignore_last_check < 1:
            return

        self.atlas_ignore_last_check = current_time

        if not self.atlas_ignore_file.is_file():
            return

        mtime = self.atlas_ignore_file.stat().st_mtime
        if mtime != self.atlas_ignore_ts:
            self.atlas_ignore_ts = mtime
            self.ignore_file_cache = {}
            lines = self.atlas_ignore_file.read_text().splitlines()
            self.atlas_ignore_spec = pathspec.PathSpec.from_lines(
    ...
    def ignored_file_raw(self, fname):
        ...
        if not self.atlas_ignore_file or not self.atlas_ignore_file.is_file():
            return False
        ...
        return self.atlas_ignore_spec.match_file(fname)
