...
def _flatten_errors(errors, parent_key=None):
    ...
    Input: {"email": ["Invalid"], "profile": {"age": ["Required"]}}
    Output: [
       {"field": "email", "message": "Invalid"},
       {"field": "profile.age", "message": "Required"}
    ]
    """
    flat_errors = []
...
def custom_exception_handler(exc, context):
    ...
    for item in raw_errors:
        formatted_errors.append({
            "code": code, # Barcha xatolar umumiy statusga ega bo'ladi (yoki logikani murakkablashtirish mumkin)
    ...
    # Agar hech qanday detail bo'lmasa (masalan 404 da data bo'sh bo'lishi mumkin)
