...
def _cleanup_impl():
    if context.akka_deployment or os.path.exists("lv.state.yml"):
        ...
        run_cli(f"hcs plan destroy -f {get_akka_plan_file_path()}", raise_on_error=False)
        context.akka_deployment = None

    trivial("Cleaning up test resources...")
    pool_ids = []
    ret = run_cli("hcs template list --search 'name $like lcm_test_' --ids", output_json=True, raise_on_error=False)
    if ret:
        pool_ids += ret
    ret = run_cli("hcs template list --search 'name $like lcm-verify-' --ids", output_json=True, raise_on_error=False)
    if ret:
        pool_ids += ret
    ret = run_cli("hcs template list --search 'name $like akka-lv-' --ids", output_json=True, raise_on_error=False)
    if ret:
        pool_ids += ret
    ...
    ret = run_cli(
        "hcs ims gold-pattern list --search 'providerType $eq akka' --ids", output_json=True, raise_on_error=False
    )
    if ret:
        for id in ret:
            run_cli(f"hcs ims gold-pattern delete -y {id}", raise_on_error=False)

    ret = run_cli("hcs edge list --search 'name $eq akka-lv'", output_json=True, raise_on_error=False)
    if ret:
        for id in ret:
            run_cli(f"hcs edge delete -y {id}", raise_on_error=False)
