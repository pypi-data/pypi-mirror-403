...
class AccessRequerimentToken(object):
    ...
    def sign_tra(self, private_key, certificate):
        tra_data = self._create_tra()
        pkey = private_key.encode('ascii')
        crt = certificate.encode('ascii')

        if sys.version_info[:2] >= (3, 10):
            try:
                key = load_pem_private_key(pkey, password=None)
            except Exception as e:
                raise ValueError("Formato de clave privada inválido") from e

            try:
                cert = load_pem_x509_certificate(crt)
            except Exception as e:
                raise ValueError("Formato de certificado inválido") from e

            try:
                signed_tra_data = base64.b64encode(pkcs7.PKCS7SignatureBuilder().set_data(
                    tra_data
                ).add_signer(
                ...
                return signed_tra_data
            except Exception as e:
                raise ValueError("Error firmando el TRA") from e

        else:
            smime = SMIME.SMIME()
            ks = BIO.MemoryBuffer(pkey)
            cs = BIO.MemoryBuffer(crt)
            bf = BIO.MemoryBuffer(tra_data)
