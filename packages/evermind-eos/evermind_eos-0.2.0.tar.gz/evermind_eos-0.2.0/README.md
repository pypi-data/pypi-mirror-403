...
client = EOS(
    api_key=os.environ.get("EOS_API_KEY"),  # This is the default and can be omitted
)
...
to add `EOS_API_KEY="My API Key"` to your `.env` file
so that your API Key is not stored in source control.

## Async usage

Simply import `AsyncEOS` instead of `EOS` and use `await` with each API call:
...
import os
import asyncio
from eos import DefaultAioHttpClient
from eos import AsyncEOS
...
When the library is unable to connect to the API (for example, due to network connection problems or a timeout), a subclass of `eos.APIConnectionError` is raised.

When the API returns a non-success status code (that is, 4xx or 5xx
response), a subclass of `eos.APIStatusError` is raised, containing `status_code` and `response` properties.

All errors inherit from `eos.APIError`.
...
except eos.APIConnectionError as e:
    print("The server could not be reached")
    print(e.__cause__)  # an underlying Exception, likely raised within httpx.
except eos.RateLimitError as e:
    print("A 429 status code was received; we should back off a bit.")
except eos.APIStatusError as e:
...
You can enable logging by setting the environment variable `EOS_LOG` to `info`.
...
```python
import httpx
from eos import EOS, DefaultHttpxClient

client = EOS(
    # Or use the `EOS_BASE_URL` env var
