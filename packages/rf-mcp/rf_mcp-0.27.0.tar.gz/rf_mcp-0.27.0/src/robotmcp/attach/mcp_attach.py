...
class McpAttach:
    ...
    def _run_keyword(
        ...
        try:
            ...
            # Provide enhanced error with context about available libraries
            error_msg = str(e)
            # Check if it's a "keyword not found" error
            if "No keyword with name" in error_msg:
                # Add hint about available libraries
                ctx = EXECUTION_CONTEXTS.current
                libs = []
                if ctx and getattr(ctx, "namespace", None):
                    libraries = ctx.namespace.libraries
                    if hasattr(libraries, "__iter__"):
                        libs = [
                            getattr(li, "name", getattr(li, "__class__", type(li)).__name__)
                            for li in libraries
                        ]
                if libs:
                    error_msg += f" Available libraries: {', '.join(libs)}"
