...
from qblox_instruments.scpi import Scpi, lookup_scpi_layer, scpi_error_check
...
class Cluster:
    ...
    def __init__(
        ...
        # Obtain IDN to identify instrument
        scpi = Scpi(self._transport, debug=debug)
        idn = scpi._get_idn()
        del scpi

        manufacturer = idn.split(",")[0]
        if manufacturer.lower() != "qblox":
            raise ConnectionError(f"Unsupported instrument manufacturer: {manufacturer}")
        self._cmm_dev_info = DeviceInfo.from_idn(idn)
        ...
        # Initialize SCPI layer
        scpi_version = self._cmm_dev_info.build.application_compat.scpi_version or (0, 1)
        scpi_layer = lookup_scpi_layer(model, scpi_version)
        if not scpi_layer:
            msg = "Unsupported instrument type/version detected: "
            msg += f"{self.instrument_type} {scpi_version[0]}.{scpi_version[1]}"
            raise ConnectionError(msg)
        self._scpi = scpi_layer(self._transport, debug=debug)
    ...
    def __getattr__(self, name: str) -> Any:
        if "_scpi" in self.__dict__ and hasattr(self._scpi, name):
            return getattr(self._scpi, name)
    ...
    def _get_scpi_commands(self) -> dict:
        ...
        cmds = self._scpi._get_scpi_commands()
    ...
    def get_idn(self) -> dict:
        ...
        """
        return DeviceInfo.from_idn(self._scpi._get_idn()).to_idn_dict()
    ...
    def get_system_status(self) -> SystemStatus:
        ...
        state = self._scpi._get_system_state()
    ...
    def _set_acq_scope_config(self, slot: int, config: dict) -> None:
        ...
        self._scpi._set_acq_scope_config(slot, config)
    ...
    def _get_acq_scope_config(self, slot: int) -> dict:
        ...
        return self._scpi._get_acq_scope_config(slot)
    ...
    def _set_io_channel_config(
        ...
        **_kwargs,
        ...
        if num_args == 2:  # Single channel
            ...
            return self._scpi._set_io_channel_config(slot, channel, config)
    ...
    def _set_output_normalized_amplitude(self, slot: int, channel: int, amplitude: float) -> None:
        ...
        self._scpi._set_output_normalized_amplitude(slot, channel, amplitude)
    ...
    def _get_output_normalized_amplitude(self, slot: int, channel: int) -> float:
        ...
        check_is_valid_type(self._is_qtm_type(slot) and self._is_eom_type(slot))
        return self._scpi._get_output_normalized_amplitude(slot, channel)
    ...
    def _set_io_pulse_output_offset(self, slot: int, channel: int, offset: float) -> None:
        ...
        check_is_valid_type(self._is_qtm_type(slot) and self._is_eom_type(slot))
        self._scpi._set_io_pulse_output_offset(slot, channel, offset)
    ...
    def _get_io_pulse_output_offset(self, slot: int, channel: int) -> float:
        ...
        check_is_valid_type(self._is_qtm_type(slot) and self._is_eom_type(slot))
        return self._scpi._get_io_pulse_output_offset(slot, channel)
    ...
    def _set_io_pulse_width_config(self, slot: int, channel: int, config: dict) -> None:
        ...
        check_is_valid_type(self._is_qtm_type(slot) and self._is_eom_type(slot))
        self._scpi._set_io_pulse_width(slot, channel, config)
    ...
    def _get_io_pulse_width_config(self, slot: int, channel: int) -> Any:
        ...
        check_is_valid_type(self._is_qtm_type(slot) and self._is_eom_type(slot))
        return self._scpi._get_io_pulse_width(slot, channel)
    ...
    def _get_io_channel_config(
        ...
        if channel is None:
            return self._scpi._get_io_output_config(slot)
        else:
            return self._scpi._get_io_channel_config(slot, channel)
    ...
    def _get_io_channel_status(self, slot: int, channel: int) -> dict:
        ...
        return self._scpi._get_io_channel_status(slot, channel)
    ...
    def _set_quad_config(self, slot: int, quad: int, config: dict) -> None:
        ...
        check_is_valid_type(self._is_qtm_type(slot))
        self._scpi._set_quad_config(slot, quad, config)
    ...
    def _get_quad_config(self, slot: int, quad: int) -> Any:
        ...
        check_is_valid_type(self._is_qtm_type(slot))
        return self._scpi._get_quad_config(slot, quad)
    ...
    def _set_sequencer_program(self, slot: int, sequencer: int, program: str) -> None:
        ...
        try:
            self._scpi._set_sequencer_program(slot, sequencer, program)
        except:
    ...
    def _set_sequencer_config(self, slot: int, sequencer: int, config: dict) -> None:
        ...
        check_sequencer_index(sequencer)
        self._scpi._set_sequencer_config(slot, sequencer, config)
    ...
    def _get_sequencer_config(self, slot: int, sequencer: int) -> dict:
        ...
        check_sequencer_index(sequencer)
        return self._scpi._get_sequencer_config(slot, sequencer)
    ...
    def _get_output_latency(self, slot: int, output: int) -> float:
        ...
        return self._scpi._get_output_latency(slot, output)
    ...
    def _set_pre_distortion_config(self, slot: int, config: dict) -> None:
        ...
        self._scpi._set_pre_distortion_config(slot, config)
    ...
    def _get_pre_distortion_config(self, slot: int) -> dict:
        ...
        return self._scpi._get_pre_distortion_config(slot)
    ...
    def _get_acq_data_and_convert(
        ...
        *,
        ...
        binned_acquisition_scaling = (
            BINNED_ACQUISITION_SCALING_QRC if self._is_qrc_type(slot) else 1
        )
        ...
        path0_data = numpy.where(
            valid, binned_acquisition_scaling * path0_raw / max_sample_value_binned**2, numpy.nan
        )
        path1_data = numpy.where(
            valid, binned_acquisition_scaling * path1_raw / max_sample_value_binned**2, numpy.nan
        )
    ...
    def _add_awg_waveform(
        ...
        """
        self._scpi._add_awg_waveform(slot, sequencer, name, len(waveform), False)
        self._scpi._set_awg_waveform_data(slot, sequencer, name, waveform)
        if index is not None:
            self._scpi._set_awg_waveform_index(slot, sequencer, name, index)
    ...
    def _delete_waveform(
        ...
        *,
        ...
        check_sequencer_index(sequencer)
        self._scpi._delete_awg_waveform(slot, sequencer, "all" if delete_all else name)
    ...
    def _add_acq_weight(
        ...
        """
        self._scpi._add_acq_weight(slot, sequencer, name, len(weight), False)
        self._scpi._set_acq_weight_data(slot, sequencer, name, weight)
        if index is not None:
            self._scpi._set_acq_weight_index(slot, sequencer, name, index)
    ...
    def _add_weights(self, slot: int, sequencer: int, weights: dict) -> None:
        """
        Add all weights in JSON compatible dictionary to the acquisition weight list
        of indexed sequencer.
    ...
    def _add_acq_acquisition(
        ...
        """
        self._scpi._add_acq_acquisition(slot, sequencer, name, num_bins)
        if index is not None:
            self._scpi._set_acq_acquisition_index(slot, sequencer, name, index)
    ...
    def _add_acquisitions(self, slot: int, sequencer: int, acquisitions: dict) -> None:
        """
        Add all waveforms and weights in JSON compatible dictionary to AWG waveform
        and acquisition weight lists of indexed sequencer.
    ...
    def get_acquisitions(self, slot: int, sequencer: int, *, as_numpy: bool = False) -> dict:
        ...
        divided by their respective integration lengths.
    ...
    def _set_voltage_instant(self, slot: int, io_channel: int, voltage: float) -> None:
        ...
        """
        check_is_valid_type(self._is_qsm_type(slot))
        return self._scpi._set_instant(slot, io_channel, voltage)
    ...
    def _set_voltage_wait(self, slot: int, io_channel: int, voltage: float) -> None:
        ...
        """
        check_is_valid_type(self._is_qsm_type(slot))
        return self._scpi._set_voltage_wait(slot, io_channel, voltage)
    ...
    def request_rampdown(self, slot: Optional[int]) -> None:
        ...
        for slot_idx in slots_to_update:
            self._scpi._request_rampdown(slot_idx)
    ...
    def clear_rampdown(self, slot: Optional[int]) -> None:
        ...
        for slot_idx in slots_to_update:
            self._scpi._clear_rampdown(slot_idx)

    # --------------------------------------------------------------------------
    @overload
    def _measure_current(self, slot: int, io_channels: int) -> float: ...
    @overload
    def _measure_current(self, slot: int, io_channels: Iterable[int]) -> list[float]: ...
    def _measure_current(
        self, slot: int, io_channels: Union[int, Iterable[int]]
        ...
        ----------
        slot : int
            The slot index of the module to measure.
        ...
        if isinstance(io_channels, int):
            ...
            single_input = False
            channels = list(io_channels)

        currents = self._scpi._measure_current(slot, channels)
        return currents[0] if single_input else currents
    ...
    def _get_slots_with_module_present(self) -> set[int]:
        ...
        """
        bitmask = self._scpi._get_modules_present()
    ...
    def _get_slots_with_module_connected(self) -> set[int]:
        ...
        """
        mods_info = self._scpi._get_mods_info()
    ...
    def _get_modules_present(self, slot: int) -> bool:
        ...
        """
        return self._is_module_present(slot, self._scpi._get_modules_present())
    ...
    def scpi_transaction(
        ...
        if self._debug in (DebugLevel.ERROR_CHECK, DebugLevel.VERSION_AND_ERROR_CHECK):
            try:
                yield conn, prefix
            except OSError:
                raise
            except Exception as error:
                conn._interface._check_error(error)
        else:
            yield conn, prefix
    ...
    def _check_error(self, error: Optional[Exception] = None) -> None:
        self._scpi._check_error(error)
