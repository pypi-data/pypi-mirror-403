...
class Command(BaseCommand):
    ...
    options = BaseCommand.options + [
        {
            ...
            'type': str,
            'default': 'sqlite',
            'help': 'The backend to use. Choices are "sqlite" (default) or "sql".'
        },
        {
            'name': '--backend-opts',
            'action': 'store',
    ...
    def run(self):
        ...
        if self.opts['backend'] == 'sql':
            ...
            # Default to a file-backed SQLite database under .checkmate/
            default_db = os.path.join(project_path, ".checkmate", "database.db")
            backend_opts = self.opts['backend_opts'] or f"sqlite:///{default_db}"
