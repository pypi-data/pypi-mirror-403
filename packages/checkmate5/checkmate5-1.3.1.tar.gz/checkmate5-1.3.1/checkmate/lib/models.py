...
class Project(BaseDocument):
    ...
    def get_disk_file_revisions(self, path=None, filters=None):
        """
        Build FileRevision objects from the filesystem.
        """
        from checkmate.management.helpers import get_files_list

        root_path = path or os.environ.get("CODE_DIR") or os.getcwd()
        file_paths = get_files_list(root_path)

        if filters:
            for filter_func in filters:
                allowed = set(filter_func([p.lstrip(os.sep) for p in file_paths]))
                file_paths = [p for p in file_paths if p.lstrip(os.sep) in allowed]
        ...
        for file_path in file_paths:
            ...
            content_hasher.add(file_content)

            file_revision = FileRevision({
                "path": rel_path,
                "sha": content_hasher.digest.hexdigest()
            })
            ...
            file_revision._file_content = lambda p=abs_path: _read_file(p)
