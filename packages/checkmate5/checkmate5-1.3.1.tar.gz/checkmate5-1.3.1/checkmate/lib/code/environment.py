...
class CodeEnvironment(object):
    ...
    def filter_file_revisions(self, file_revisions):
        ...
        ignore_patterns = []
        if hasattr(self.global_settings, "checkignore") and self.global_settings.checkignore:
            ignore_patterns.extend(parse_checkignore(self.global_settings.checkignore))
        if 'ignore' in self.project_settings:
            project_ignore = self.project_settings['ignore']
            if isinstance(project_ignore, str):
                ignore_patterns.extend(parse_checkignore(project_ignore))
            else:
                ignore_patterns.extend(project_ignore)

        if ignore_patterns:
            filters.append(lambda filenames: filter_filenames_by_checkignore(
                filenames, ignore_patterns))
    ...
    def analyze_file_revisions(self, file_revisions):
        ...
        for file_revision in tqdm(filtered_file_revisions):
            ...
            # Run analyzers matching file language
            primary_results = self.analyze_file_revision(
                file_revision,
                {
                    analyzer_name: analyzer_params
                    for analyzer_name, analyzer_params in list(self.analyzers.items())
                    if analyzer_params['language'] in (file_revision.language, "all")
                }
            )

            # Optionally run extra analyzers if present
            extra_results = {}
            for analyzer_name, analyzer_params in list(self.analyzers.items()):
                if analyzer_name in ("trojansource", "trufflehog3", "yara", "gptanalyzer", "privategptanalyzer"):
                    res = self.analyze_file_revision(file_revision, {analyzer_name: analyzer_params})
                    extra_results.update(res)

            file_revision.results = {**primary_results, **extra_results}
    ...
    def analyze(self, file_revisions, save_if_empty=False, snapshot=None):
        ...
        * After analysis, calculate the hash value based on path, SHA and dependencies
        ...
        elif not getattr(snapshot, "configuration", None):
            snapshot.configuration = self.project.configuration
        ...
        if not getattr(snapshot, "pk", None):
            snapshot.pk = uuid.uuid4().hex
        if not getattr(snapshot, "hash", None):
            hasher = Hasher()
            hasher.add(snapshot.configuration)
            for fr in snapshot.file_revisions:
                hasher.add(fr.hash)
            ...
            with self.project.backend.transaction():
                for analyzer_name, issues in list(issues_results.items()):
                    ...
                    for issue_dict in grouped_issues:
                        analyzer_meta = self.global_settings.analyzers.get(analyzer_name, {})
                        issues_data = analyzer_meta.get('issues_data', {}) if isinstance(analyzer_meta, dict) else {}
                        issue_meta = issues_data.get(issue_dict.get('code')) if isinstance(issues_data, dict) else None
                        if issue_meta:
                            if 'description' in issue_meta and 'data' not in issue_dict:
                                issue_dict['data'] = issue_meta['description']
                            if 'title' in issue_meta and 'title' not in issue_dict:
                                issue_dict['title'] = issue_meta['title']
                            if 'severity' in issue_meta and issue_dict.get('severity') in (None, 'Unknown'):
                                sev = issue_meta['severity']
                                sev_map = {'1': 'High', '2': 'Medium', '3': 'Low', '4': 'Info'}
                                issue_dict['severity'] = sev_map.get(str(sev), sev)
                        if 'data' not in issue_dict:
                            if 'description' in issue_dict:
                                issue_dict['data'] = issue_dict['description']
                            elif 'message' in issue_dict:
                                issue_dict['data'] = issue_dict['message']
                        if 'severity' not in issue_dict:
                            issue_dict['severity'] = 'Unknown'
                        if 'file' not in issue_dict:
                            issue_dict['file'] = file_revision.path
                        if 'line' not in issue_dict and issue_dict.get('occurrences'):
                            issue_dict['line'] = issue_dict['occurrences'][0].get('from_row')
                        ...
                        issue = None
                        try:
                            engine = getattr(self.project.backend, "engine", None)
                            if engine and engine.url and engine.url.database:
                                import sqlite3
                                conn = sqlite3.connect(engine.url.database)
                                cur = conn.cursor()
                                row = cur.execute(
                                    "select pk from issue where project = ? and fingerprint = ? and analyzer = ? and code = ? limit 1",
                                    (self.project.pk, issue_dict['fingerprint'], analyzer_name, issue_dict['code']),
                                ).fetchone()
                                conn.close()
                                if row and row[0]:
                                    issue = Issue({
                                        'hash': issue_dict['hash'],
                                        'analyzer': analyzer_name,
                                        'code': issue_dict['code'],
                                        'fingerprint': issue_dict['fingerprint'],
                                    })
                                    issue.pk = row[0]
                            ...
                            # we check if the issue already exists
                                ...
                                try:
                                    ...
                                    if IntegrityError and isinstance(exc, IntegrityError):
                                        try:
                                            issue = self.project.backend.get(
                                                Issue,
                                                {
                                                    'project': self.project,
                                                    'fingerprint': issue_dict['fingerprint'],
                                        ...
                                        if issue is None:
                                            try:
                                                issue_list = self.project.backend.filter(
                                                    Issue,
                                                    {
                                                        'project': self.project,
                                                        'fingerprint': issue_dict['fingerprint'],
                                        ...
                                        if issue is None:
                                            logger.warning(
                                                "Unable to resolve duplicate issue (fingerprint=%s, analyzer=%s, code=%s); skipping.",
                                                issue_dict.get('fingerprint'),
                                                analyzer_name,
                                                issue_dict.get('code'),
                                            )
                                            continue
                        ...
                        for occurrence in issue_dict['occurrences']:
                            occurrence['file'] = file_revision.path
                            occurrence['line'] = occurrence.get('from_row')
                            hasher = Hasher()
                            hasher.add(file_revision.hash)
