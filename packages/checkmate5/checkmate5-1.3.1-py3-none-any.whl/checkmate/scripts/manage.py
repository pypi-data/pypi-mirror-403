...
def main():
    # Allow running without git or CODE_DIR; emit warnings instead of aborting.
    if not os.path.isdir('.git'):
        sys.stderr.write("Warning: not a git repository; proceeding anyway.\n")
    if "CODE_DIR" not in os.environ:
        os.environ["CODE_DIR"] = os.getcwd()
        sys.stderr.write("Warning: CODE_DIR not set; defaulting to current working directory.\n")

    # Warn when running with stubbed checkmate.lib
    try:
        import checkmate.lib.code as _lib_code  # noqa: F401
        lib_doc = getattr(_lib_code, "__doc__", "") or ""
        if lib_doc.strip().startswith("Stub"):
            sys.stderr.write("Warning: running with stubbed checkmate.lib; analysis/diffs are non-functional.\n")
    except Exception:
        sys.stderr.write("Warning: checkmate.lib not fully available; functionality may be limited.\n")
    ...
    requires_project = getattr(CommandClass, "requires_valid_project", True)
