(() => {
  ...
  function MicroPlugin (Interface) {
    ...
    return class extends Interface {
      ...
      loadPlugin (name) {
        ...
        const plugin15 = Interface.plugins[name]
  ...
  for (const latin in latin_condensed) {
    const unicode = latin_condensed[latin] || ''
    for (let i = 0; i < unicode.length; i++) {
      const char = unicode.substring(i, i + 1)
  ...
  function * generator (code_points2) {
    for (const [code_point_min, code_point_max] of code_points2) {
      for (let i = code_point_min; i <= code_point_max; i++) {
        const composed = String.fromCharCode(i)
        const folded = asciifold(composed)
  ...
  const generateSets = (code_points2) => {
    ...
    for (const value of generator(code_points2)) {
  ...
  var generateMap = (code_points2) => {
    ...
    const unicode_map2 = {}
    const multi_char = []
    for (const folded in unicode_sets) {
      const set = unicode_sets[folded]
      if (set) {
  ...
  const sequencesToPattern = (sequences, all = true) => {
    const min_replacement = sequences.length > 1 ? 1 : 0
  ...
  const inSequences = (needle_seq, sequences) => {
    for (const seq of sequences) {
      ...
      const needle_parts = needle_seq.parts
      const filter = (part) => {
      ...
      const filtered = seq.parts.filter(filter)
  ...
  const Sequence = class _Sequence {
    parts
    substrs
    start
    end
    ...
    clone (position, last_piece) {
      const clone = new _Sequence()
      const parts = JSON.parse(JSON.stringify(this.parts))
      const last_part = parts.pop()
      for (const part of parts) {
        clone.add(part)
      }
      const last_substr = last_piece.substr.substring(0, position - last_part.start)
      const clone_last_len = last_substr.length
  ...
  const getPattern = (str) => {
    ...
    for (let i = 0; i < str.length; i++) {
      ...
      const match = substr.match(multi_char_reg)
      const char = str.substring(i, i + 1)
      const match_str = match ? match[0] : null
      let overlapping = []
      const added_types = /* @__PURE__ */ new Set()
      for (const sequence of sequences) {
        ...
        if (!last_piece || last_piece.length == 1 || last_piece.end <= i) {
          ...
          const clone = sequence.clone(i, last_piece)
          const len = match_str.length
      ...
      if (overlapping.length > 0) {
        ...
        for (const clone of overlapping) {
  ...
  const getAttrNesting = (obj, name) => {
    if (!obj) { return }
    let part; const names = name.split('.')
  ...
  const Sifter = class {
    ...
    _getScoreFunction (search) {
      const tokens = search.tokens; const token_count = tokens.length
      if (!token_count) {
        return function () {
          return 0
        }
      }
      const fields = search.options.fields; const weights = search.weights; const field_count = fields.length; const getAttrFn = search.getAttrFn
      if (!field_count) {
        return function () {
          return 1
        }
    ...
    _getSortFunction (search) {
      let implicit_score; let sort_flds = []
      const self = this; const options = search.options; const sort = !search.query && options.sort_empty ? options.sort_empty : options.sort
      ...
      return function (a, b) {
        let result, field
        for (const sort_fld of sort_flds) {
          field = sort_fld.field
          const multiplier = sort_fld.direction === 'desc' ? -1 : 1
    ...
    prepareSearch (query, optsUser) {
      const weights = {}
      const options = Object.assign({}, optsUser)
  ...
  const iterate2 = (object, callback) => {
    if (Array.isArray(object)) {
      object.forEach(callback)
    } else {
      for (const key in object) {
  ...
  const highlight = (element, regex) => {
    ...
    const highlightText = (node) => {
      ...
      if (match && node.data.length > 0) {
        ...
        const middlebit = node.splitText(match.index)
  ...
  function getSettings (input, settings_user) {
    const settings = Object.assign({}, defaults_default, settings_user)
    ...
    const init_select = () => {
      ...
      const addGroup = (optgroup) => {
        let id, optgroup_data
  ...
  let instance_i = 0
  const TomSelect = class extends MicroPlugin(MicroEvent) {
    constructor (input_arg, user_settings) {
      ...
      if (isHtmlString(settings.controlInput)) {
        ...
        const attrs = ['autocorrect', 'autocapitalize', 'autocomplete', 'spellcheck']
        iterate2(attrs, (attr) => {
    ...
    setup () {
      ...
      addEvent(dropdown, 'mouseenter', (e) => {
        const target_match = parentMatch(e.target, '[data-selectable]', dropdown)
      ...
      addEvent(control, 'click', (evt) => {
        const target_match = parentMatch(evt.target, '[data-ts-item]', control)
    ...
    refreshOptions (triggerDropdown = true) {
      let i, j, k, n, optgroup, optgroups, html, has_create_option, active_group
      let create
      const groups = {}
      const groups_order = []
      const self = this
      ...
      const getGroupFragment = (optgroup2, order) => {
        ...
        if (group_order_i !== void 0) {
          const order_group = groups_order[group_order_i]
          if (order_group !== void 0) {
            return [group_order_i, order_group.fragment]
          }
        }
        const group_fragment = document.createDocumentFragment()
      ...
      for (i = 0; i < n; i++) {
        ...
        for (j = 0, k = optgroups && optgroups.length; j < k; j++) {
          ...
          const self_optgroup = self.optgroups[optgroup]
      ...
      iterate2(groups_order, (group_order) => {
        const group_fragment = group_order.fragment
        const optgroup2 = group_order.optgroup
        if (!group_fragment || !group_fragment.children.length) { return }
        const group_heading = self.optgroups[optgroup2]
        if (group_heading !== void 0) {
          const group_options = document.createDocumentFragment()
          const header = self.render('optgroup_header', group_heading)
          append(group_options, header)
          append(group_options, group_fragment)
          const group_html = self.render('optgroup', { group: group_heading, options: group_options })
    ...
    updateOriginalInput (opts = {}) {
      ...
      if (self.is_select_tag) {
        const AddSelected = function (option_el, value, label2) {
          if (!option_el) {
    ...
    advanceSelection (direction, e) {
      let last_active; let adjacent; const self = this
  ...
  const addEvent2 = (target, type, callback, options) => {
    target.addEventListener(type, callback, options)
  }
  function plugin () {
  ...
  function plugin2 (userOptions) {
    ...
    const UpdateChecked = function UpdateChecked2 (checkbox, toCheck) {
      if (toCheck) {
    ...
    const UpdateCheckbox = function UpdateCheckbox2 (option) {
      setTimeout(() => {
        const checkbox = option.querySelector('input.' + cbOptions.className)
    ...
    self.hook('after', 'setupTemplates', () => {
      ...
      self.settings.render.option = (data, escape_html3) => {
        const rendered = getDom2(orig_render_option.call(self, data, escape_html3))
  ...
  function plugin3 (userOptions) {
    ...
    self.on('initialize', () => {
      const button = getDom3(options.html(options))
  ...
  const addEvent3 = (target, type, callback, options) => {
    target.addEventListener(type, callback, options)
  }
  ...
  const insertAfter = (referenceNode, newNode) => {
    let _referenceNode$parent;
    (_referenceNode$parent = referenceNode.parentNode) == null || _referenceNode$parent.insertBefore(newNode, referenceNode.nextSibling)
  }
  const insertBefore = (referenceNode, newNode) => {
    let _referenceNode$parent2;
  ...
  const preventDefault4 = (evt, stop = false) => {
    if (evt) {
      evt.preventDefault()
      if (stop) {
        evt.stopPropagation()
  ...
  function plugin5 (userOptions) {
    ...
    self.on('initialize', () => {
      const header = getDom5(options.html(options))
  ...
  const iterate4 = (object, callback) => {
    if (Array.isArray(object)) {
      object.forEach(callback)
    } else {
      for (const key in object) {
  ...
  const removeClasses2 = (elmts, ...classes) => {
    const norm_classes = classesArray2(classes)
  ...
  const KEY_ESC2 = 27
  const KEY_TAB2 = 9
  const preventDefault5 = (evt, stop = false) => {
    if (evt) {
      evt.preventDefault()
      if (stop) {
        evt.stopPropagation()
  ...
  const addEvent4 = (target, type, callback, options) => {
    target.addEventListener(type, callback, options)
  }
  const iterate5 = (object, callback) => {
    if (Array.isArray(object)) {
      object.forEach(callback)
    } else {
      for (const key in object) {
  ...
  const addClasses2 = (elmts, ...classes) => {
    const norm_classes = classesArray3(classes)
  ...
  const addEvent5 = (target, type, callback, options) => {
    target.addEventListener(type, callback, options)
  }
  function plugin8 () {
    const self = this
  ...
  const KEY_LEFT2 = 37
  const KEY_RIGHT2 = 39
  const parentMatch2 = (target, selector, wrapper) => {
    while (target && target.matches) {
      if (target.matches(selector)) {
        return target
      }
  ...
  const preventDefault6 = (evt, stop = false) => {
    if (evt) {
      evt.preventDefault()
      if (stop) {
        evt.stopPropagation()
  ...
  const addEvent6 = (target, type, callback, options) => {
    target.addEventListener(type, callback, options)
  }
  const getDom7 = (query) => {
    if (query.jquery) {
  ...
  function plugin12 (userOptions) {
    ...
    const html = '<a href="javascript:void(0)" class="' + options.className + '" tabindex="-1" title="' + escape_html2(options.title) + '">' + options.label + '</a>'
    self.hook('after', 'setupTemplates', () => {
      const orig_render_item = self.settings.render.item
      self.settings.render.item = (data, escape) => {
        const item = getDom7(orig_render_item.call(self, data, escape))
        const close_button = getDom7(html)
  ...
  const iterate6 = (object, callback) => {
    if (Array.isArray(object)) {
      object.forEach(callback)
    } else {
      for (const key in object) {
  ...
  const addClasses3 = (elmts, ...classes) => {
    const norm_classes = classesArray4(classes)
  ...
  const tom_select_complete_default = TomSelect
  ...
  tom_select_complete_default.define('checkbox_options', plugin2)
  ...
  if (typeof tom_select_complete_default === 'function') {
    ...
    window.TomSelect = tom_select_complete_default.default || tom_select_complete_default
  }
})()
