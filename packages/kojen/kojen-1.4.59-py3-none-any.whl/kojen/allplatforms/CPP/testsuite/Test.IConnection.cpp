...
/// 9) packets are offset in the middle of a stream ... not all perfect like above.
///    - fragmented header ...small bits...with noise inbetween
///    - fragmented header ...larger bits
...
MU_TEST(Scenario_8_RealDataBetweenGARBAGE_sync_to_stream)
{
    ...
    for (int j = 1; j < FRAGMENT_BUF_SIZE; ++j)
    {
        ...
        //    fixture.m_tx.SendData(&garbage_buffer[i], 1);
        //
        // Above is a nice test -> the moment j = 174, the last sent char is 173.
...
MU_TEST(Scenario_9_1_OffsetData_WithNoise_ByteForByte)
{
    MsgByteStreamHandlingGroup fixture;
    mu_assert(fixture.m_tx.HasMsgReceiver(),	  "Does not have a message receiver, but should have.");
    mu_assert(!fixture.m_tx.HasRawDataReceiver(), "Has a raw data receiver, but should not have.");
    ...
    uint8  noiseBuffer[10] = {5,7,3,9,4,7,1,0,9,23};
    for (uint16 cntNoise = 0; cntNoise < 3; cntNoise++)
    {
        fixture.m_tx.SendData(&noiseBuffer[cntNoise],1);
    }
    ...
    for (uint16 cntNoise = 0; cntNoise < 7; cntNoise++)
    {
        fixture.m_tx.SendData(&noiseBuffer[cntNoise],1);
    }
    ...
    for (uint16 cntNoise = 0; cntNoise < 5; cntNoise++)
    {
        fixture.m_tx.SendData(&noiseBuffer[cntNoise],1);
    }
    ...
    for (uint16 cntNoise = 0; cntNoise < 9; cntNoise++)
    {
        fixture.m_tx.SendData(&noiseBuffer[cntNoise],1);
    }
...
MU_TEST(Scenario_9_2_OffsetData_WithNoise_SmallFragments)
{
    MsgByteStreamHandlingGroup fixture;
    ...
    /// Scenario 2...greater than header, but less than message...
    {
        fixture.buffer[0] = 0x66;
        fixture.buffer[1] = 0xF6;
        fixture.buffer[2] = 0xA6;
        fixture.buffer[3] = 0xB6;
        fixture.buffer[4] = 0xE6;
        memcpy((void*)&fixture.buffer[5], &fixture.m_somecmdrsp, sizeof(sSomeCmdRsp));
        uint16 size_1 = 5 + sizeof(sMsgHeader) * 0.45;
        uint16 size_2 = (5 + sizeof(sMsgHeader)) - size_1;
        uint16 size_3 = sizeof(sSomeCmdRsp) - sizeof(sMsgHeader);
        fixture.m_tx.SendData(&fixture.buffer[0],size_1);
        fixture.m_tx.SendData(&fixture.buffer[size_1],size_2);
        fixture.m_tx.SendData(&fixture.buffer[size_1 + size_2],size_3);
        IsEqual(fixture.m_somecmdrsp, fixture.m_rx.m_rxsomecmdrsp);
    }
    {
        std::fill(fixture.buffer, fixture.buffer + sizeof(fixture.buffer), 0);
        fixture.buffer[0] = 0xFF;
        fixture.buffer[1] = 0xAA;
        fixture.buffer[2] = 0xFF;
        memcpy((void*)&fixture.buffer[3], &fixture.m_somereqrsp, sizeof(sSomeReqRsp));
        uint16 size_1 = 3 + 1;
        uint16 size_2 = 3 + sizeof(sMsgHeader) + 2;
        uint16 size_3 = 3 + sizeof(sSomeReqRsp) - size_2;
        fixture.m_tx.SendData(&fixture.buffer[0],size_1);
        fixture.m_tx.SendData(&fixture.buffer[size_1],size_2);
        fixture.m_tx.SendData(&fixture.buffer[size_1 + size_2],size_3);
        IsEqual(fixture.m_somereqrsp, fixture.m_rx.m_rxsomereqrsp);
...
MU_TEST_SUITE(MsgByteStreamHandling_Suite) {
    ...
    MU_RUN_TEST(Scenario_9_1_OffsetData_WithNoise_ByteForByte);
    MU_RUN_TEST(Scenario_9_2_OffsetData_WithNoise_SmallFragments);
}
