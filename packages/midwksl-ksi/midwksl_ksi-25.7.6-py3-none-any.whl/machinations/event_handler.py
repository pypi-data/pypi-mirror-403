...
class EventHandler:
    """
    Helper class to create an AvesTerra event_handler.
    ...
    def on_init(self):
        """
        This method is called between the av.initialize() and av.wait_event()
        Any exception raised in this method will prevent the event_handler from
        starting and will cleanly shutdown.
        This method should set self.outlet to the outlet that will be waited on.
        """
        pass
    ...
    def on_shutdown(self):
        """
        This method is called when the event_handler is shutting down. right before
        we call av.finalize().
        Typically, you want to call dao.stop(), close connection to db, etc.
        This is the method that should be implemented by the subclass.
        This method is called even if `on_init` raised an exception.
        This method is called before waiting for all the wait threads
        to return.
    ...
    def shutdown(self):
        """
        Stop the event_handler, or prevent it from starting if it hasn't started yet
        """
    ...
    def run(self, *args, **kwargs):
        """
        This method will block until the event_handler is stopped.
        There are several ways to stop the event_handler:
        ...
        ), "Event_Handler: Cannot run an event_handler that has been stopped"

        if self.outlet == NULL_ENTITY:
            av_log.fatal("Event_Handler: No outlet was set before calling `run()`. Aborting.")
            self.shutdown()
            return
        ...
        try:
            ...
            av_log.info(f"Event_Handler: Running with {self._thread_count} thread(s)...")
            ...
            av_log.fatal("Event_Handler: Received KeyboardInterrupt. Stopping.")
            self.shutdown()
        except BaseException as e:
            av_log.fatal(f"Event_Handler: Received {repr(e)}. Stopping.")
            self.shutdown()
