import logging
from .models import ModelManager, VLMAIModel
...
class DynamicAIManager:
    ...
    def load(self) -> None:
        ...
        for model_name in self.ai_model_names:
            ...
            if isinstance(model.model, VLMAIModel):
    ...
    def get_dynamic_video_ai_models(self, inputs: List[str], outputs: List[str]) -> List[ModelWrapper]:
        ...
        for model_idx, model in enumerate(self.models):
            ...
            log_name = getattr(model.client_config, 'model_id', None) or f"active_ai_model_{model_idx}"
    ...
    def __verify_models(self, models: List[VLMAIModel]) -> None:
        current_image_size: Optional[Union[Tuple[int, int], List[int]]] = None
        current_norm_config: Optional[Dict[str, Any]] = None
        
        for model_instance in models:
            if not isinstance(model_instance, VLMAIModel):
                raise ValueError(f"Error: Dynamic AI models must all be VLMAIModel instances! {model_instance} is not a VLMAIModel instance!")
