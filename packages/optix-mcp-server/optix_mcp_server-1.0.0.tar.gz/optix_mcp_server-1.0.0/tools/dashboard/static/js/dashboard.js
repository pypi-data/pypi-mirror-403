function dashboard() {
    return {
        ...
        async fetchReports() {
            const isInitialLoad = this.reports.length === 0;
            if (isInitialLoad) {
        ...
        async selectReport(filename) {
            ...
            try {
                ...
                this.reportContent = result.data.content;
                this.reportSections = this.extractSections(result.data.content);
        ...
        setupScrollObserver() {
            ...
            document.querySelectorAll('.report-content h1[id], .report-content h2[id]')
                .forEach(el => observer.observe(el));
        },

        get renderedReportHtml() {
            ...
            renderer.heading = function(data) {
                ...
                const slug = self.slugify(text);
                return `<h${depth} id="${slug}">${text}</h${depth}>`;
            };
            return marked.parse(this.reportContent, { renderer });
        },
