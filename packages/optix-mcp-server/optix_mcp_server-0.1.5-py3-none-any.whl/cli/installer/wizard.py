...
from cli.installer.env_writer import EnvWriter
...
class InstallationWizard:
    ...
    def _configure_single_agent(self, agent: AgentConfig) -> None:
        ...
        writer.merge_server_config(
            ...
            server_name="optix",
            config=agent.server_config,
        )

        self.ui.success(f"Configured {agent.label}: {config_path}")

    def _write_env_file(self) -> None:
        """Write .env file if expert analysis or dashboard is enabled."""
        if not self.expert_enabled and not self.dashboard_enabled:
            return

        env_writer = EnvWriter()
        env_path = env_writer.write(
            scope=self.scope,
        ...
        self.ui.success(f"Created environment file: {env_path}")
