...
| Ø<br>(qspice)  | DLL | `Øxxx «(1..99 input nodes)» «(0..99 output nodes)» «(0..99 common nodes)» value [TYPE parmkey=parmvalue [...]] [parmkey=parmvalue]...` (7) | holds model | yes (7) |
...
7. Several issues:
   * When using `save_netlist()`, you must specify the pin configuration of the `Ø` component, as spicelib is not equipped to read that configuration from the DLL. See the documentation of `QschEditor.save_netlist()`.
