...
Generated from API version: 0.1.18
...
class CleanerOptionsInput(BaseModel):
    ...
    base_url: str | None = Field(
        None, description="Base URL for resolving relative links (readability, refyne)"
    )
    extract_headings: bool | None = Field(
        None, description="Extract heading structure to frontmatter (refyne markdown)"
    )
    extract_images: bool | None = Field(
        None,
        description="Extract images to frontmatter with {{IMG_001}} placeholders (refyne markdown)",
    )
    images: bool = Field(True, description="Include images in output (trafilatura)")
    ...
    keep_selectors: list[str] | None = Field(
        None, description="CSS selectors for elements to always keep (refyne)"
    )
    links: bool = Field(True, description="Include links in output (trafilatura)")
    output: Literal["html", "text", "markdown"] = Field(
        "html", description="Output format (trafilatura, readability, refyne)"
    )
    preset: Literal["default", "minimal", "aggressive"] | None = Field(
        None, description="Refyne preset: default, minimal, or aggressive"
    )
    remove_selectors: list[str] | None = Field(
        None, description="CSS selectors for elements to remove (refyne)"
    )
    resolve_urls: bool | None = Field(
        None, description="Resolve relative URLs to absolute using base_url (refyne)"
    )
    tables: bool = Field(True, description="Include tables in output (trafilatura)")
...
class FallbackChainEntryInput(BaseModel):
    ...
    provider: Literal["openrouter", "anthropic", "openai", "ollama"] = Field(
...
class LLMConfigInput(BaseModel):
    ...
    provider: (
        Literal["anthropic", "openai", "openrouter", "ollama", "credits"] | None
...
class DebugCaptureEntry(BaseModel):
    ...
    raw_content: str | None = Field(
        None, description="Page content (for extract/crawl jobs)"
    )
    request: DebugCaptureLLMRequest = Field(..., description="LLM request metadata")
    response: DebugCaptureLLMResponse = Field(..., description="LLM response metadata")
