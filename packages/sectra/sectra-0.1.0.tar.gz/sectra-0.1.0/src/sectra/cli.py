...
def _do_request(ctx: Context, method: str, path: str, params: Optional[str], data: Optional[str]):
    try:
        ...
        if cached is not None:
            _print_response(_CachedResponse(cached), ctx.output)
            return
        resp = request(
            method=method,
            path=path,
...
@click.option("--params", help="Query params as JSON")
@click.pass_obj
def domains_search(ctx: Context, query, data_json, params):
    ...
    _do_request(ctx, "POST", "/domains/list", params, body)


@cli.command("domains-search-dsl")
@click.option("--query", required=True, help="DSL query string")
@click.pass_obj
def domains_search_dsl(ctx: Context, query):
    """Search domains (POST /domains/list-backup)."""
    _do_request(ctx, "POST", "/domains/list-backup", None, json.dumps({"query": query}))
...
@click.option("--data", "data_json", help="Raw JSON body (overrides --query)")
@click.pass_obj
def ips_search(ctx: Context, query, data_json):
    """Search IPs (POST /ips/list)."""
    if not data_json and not query:
        raise click.UsageError("Provide --query or --data")
    body = data_json or json.dumps({"query": query})
    _do_request(ctx, "POST", "/ips/list", None, body)
...
@click.option("--params", help="Query params as JSON")
@click.pass_obj
def sql_query(ctx: Context, query, data_json, params):
    """SQL API query (POST /query/scroll)."""
    if not data_json and not query:
        raise click.UsageError("Provide --query or --data")
    body = data_json or json.dumps({"query": query})
    _do_request(ctx, "POST", "/query/scroll", params, body)
