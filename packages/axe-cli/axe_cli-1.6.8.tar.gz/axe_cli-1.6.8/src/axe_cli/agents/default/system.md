...
The codebase is automatically indexed on startup using axe-dig, providing powerful semantic code search and analysis capabilities. When working with code, PREFER these tools over basic file operations:

## When to use Code Intelligence tools:

- **CodeSearch**: Use when searching for code by BEHAVIOR or PURPOSE (e.g., "find JWT validation", "authentication middleware")
  - Much better than Grep for finding code based on what it does
  - Returns semantic matches even without exact text matches
  
- **CodeContext**: Use when you need to understand a function/class WITHOUT reading entire files
  - 95% token reduction while preserving understanding
  - Includes cross-file dependencies and callers
...
- **Grep**: Exact string/regex searches, finding literal patterns in text
- **ReadFile**: Reading full file content when needed
