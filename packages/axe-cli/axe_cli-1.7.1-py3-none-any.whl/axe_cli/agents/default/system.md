...
- **CodeContext**: Use when you need to understand a specific function, class, or symbol WITHOUT reading entire files.
  ...
  - Includes cross-file dependencies (what it calls) and callers (who uses it).
...
- **Grep**: Use as a **FALLBACK** if CodeSearch fails, or for:
  - Exact string matches when you know the exact name.
  - Complex regex queries that semantic search might miss.
  - Finding literal patterns in text/config files.
