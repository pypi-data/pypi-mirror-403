...
from sgFCMed.sgFCMed import SGFCMed
from unittest.mock import patch
import sgFCMed.sgFCMed as sgfcmed
...
def test_initialization():
    ...
    """
    model = SGFCMed(C=2, m=2.0, max_iter=5, tol=1e-4,
...
def test_modified_median_refine_all_violated(data):
    ...
    with patch.object(model, "_violates_hard_uniqueness", return_value=True), \
         patch.object(sgfcmed, "candidate_score_weighted", fake_candidate_score_weighted):
...
def test_pipeline(data):
    ...
    parameter (m) and the objective function value (J).
    """
    model = SGFCMed(C=3, max_iter=5)
    model.fit(data)
...
def test_enforce_unique_prototypes(data):
    ...
    """
    model = SGFCMed(C=2, enforce_unique=True)
    model.fit(data)
