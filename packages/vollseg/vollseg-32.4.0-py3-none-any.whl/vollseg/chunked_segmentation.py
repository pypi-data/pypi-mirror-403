...
def segment_with_vollseg_chunked(
    ...
    min_overlap: Optional[Tuple[int, int, int]] = None,
    ...
    nms_thresh : float
        NMS threshold
    postprocess : bool
        If True, apply post-processing (hole filling, object removal).
        If False, just label the UNET result without post-processing.
        Default: False
    ...
    def vollseg_segment(chunk, **kwargs):
        ...
        # Instance labels ALWAYS at index 0
        labels = results[0]

        # Check if it's a boolean mask (UNET-only mode) or instance labels
        if labels.dtype == bool or labels.dtype == np.bool_:
            # Convert boolean mask to instance labels using connected components
            instance_labels, num_objects = scipy_label(labels)
            print(f"    Converted binary mask to {num_objects} instance labels")
            return instance_labels.astype(np.uint16)
        else:
            # Already instance labels (StarDist or hybrid mode)
