...
if TYPE_CHECKING:
    from ..models.flatten_query_params import FlattenQueryParams


T = TypeVar("T", bound="FlattenRequest")


@_attrs_define
class FlattenRequest:
    """Request to flatten nested artifact groups into a single group of standalone artifacts.

    Attributes:
        query (FlattenQueryParams): No specific parameters needed for flatten - just flattens the input artifact
        ...
        twin_artifact_id (None | Unset | UUID): The ID of a reference artifact, e.g. the right table in Deep Merge
            operation.
        task_type (Literal['flatten'] | Unset):  Default: 'flatten'.
        processing_mode (Literal['transform'] | Unset):  Default: 'transform'.
    """

    query: FlattenQueryParams
    ...
    task_type: Literal["flatten"] | Unset = "flatten"
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        ...
        d = dict(src_dict)
        query = FlattenQueryParams.from_dict(d.pop("query"))
        ...
        task_type = cast(Literal["flatten"] | Unset, d.pop("task_type", UNSET))
        if task_type != "flatten" and not isinstance(task_type, Unset):
            raise ValueError(f"task_type must match const 'flatten', got '{task_type}'")
        ...
        flatten_request.additional_properties = d
        return flatten_request
