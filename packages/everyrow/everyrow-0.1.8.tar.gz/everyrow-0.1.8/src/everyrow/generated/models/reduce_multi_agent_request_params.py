...
if TYPE_CHECKING:
    from ..models.multi_agent_query_params import MultiAgentQueryParams


T = TypeVar("T", bound="ReduceMultiAgentRequestParams")


@_attrs_define
class ReduceMultiAgentRequestParams:
    """Top-level request parameters for the agent service (REDUCE mode).

    Attributes:
        query (MultiAgentQueryParams):
        ...
        twin_artifact_id (None | Unset | UUID): The ID of a reference artifact, e.g. the right table in Deep Merge
            operation.
        task_type (Literal['multiagent'] | Unset):  Default: 'multiagent'.
        processing_mode (Literal['reduce'] | Unset):  Default: 'reduce'.
    """
    ...
    task_type: Literal["multiagent"] | Unset = "multiagent"
    processing_mode: Literal["reduce"] | Unset = "reduce"
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        ...
        d = dict(src_dict)
        query = MultiAgentQueryParams.from_dict(d.pop("query"))
        ...
        task_type = cast(Literal["multiagent"] | Unset, d.pop("task_type", UNSET))
        if task_type != "multiagent" and not isinstance(task_type, Unset):
            raise ValueError(f"task_type must match const 'multiagent', got '{task_type}'")

        processing_mode = cast(Literal["reduce"] | Unset, d.pop("processing_mode", UNSET))
        if processing_mode != "reduce" and not isinstance(processing_mode, Unset):
            raise ValueError(f"processing_mode must match const 'reduce', got '{processing_mode}'")

        reduce_multi_agent_request_params = cls(
            query=query,
        ...
        reduce_multi_agent_request_params.additional_properties = d
        return reduce_multi_agent_request_params
