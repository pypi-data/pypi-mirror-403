#!/usr/bin/env python
"""
Benchmark comparing satya vs Pydantic performance using Starlette.

This benchmark:
1. Sets up two Starlette applications - one using satya, one using Pydantic
...
try:
    ...
    print("pip install starlette uvicorn pydantic aiohttp matplotlib")
    exit(1)
...
# ======================= Starlette Apps =======================
# Helper function for satya models
...
# Pydantic Routes Handlers
async def pydantic_root(request):
    return JSONResponse({"message": "Pydantic Benchmark App"})

async def pydantic_create_item(request):
    data = await request.json()
    try:
        item = PydanticItem(**data)
        return JSONResponse(item.model_dump())
    except Exception as e:
        return JSONResponse({"error": str(e)}, status_code=400)

async def pydantic_get_item(request):
...
# Satya Routes Handlers
async def satya_root(request):
    return JSONResponse({"message": "Satya Benchmark App"})

async def satya_create_item(request):
    data = await request.json()
    try:
        item = SatyaItem(**data)
        return JSONResponse(serialize_satya(item))
    except Exception as e:
        return JSONResponse({"error": str(e)}, status_code=400)

async def satya_get_item(request):
    item_id = int(request.path_params["item_id"])
    item_size = request.query_params.get("item_size", "small")
...
# Create Starlette Apps
pydantic_routes = [
    Route("/", pydantic_root),
    Route("/items", pydantic_create_item, methods=["POST"]),
    Route("/items/{item_id:int}", pydantic_get_item),
]

satya_routes = [
    Route("/", satya_root),
    Route("/items", satya_create_item, methods=["POST"]),
    Route("/items/{item_id:int}", satya_get_item),
]

pydantic_app = Starlette(debug=False, routes=pydantic_routes)
satya_app = Starlette(debug=False, routes=satya_routes)
...
async def run_benchmark(framework: str, scenarios: List[Dict], operations: List[str], port: int):
    """Run the benchmark for a given framework (pydantic or satya)."""
    # Start the server
    host = "127.0.0.1"
    ...
    if framework == "pydantic":
        process = await asyncio.create_subprocess_exec(
            "uvicorn",
            "benchmarks.starlette_benchmark:pydantic_app",
            "--host", host,
            "--port", str(port),
            "--log-level", "error",
        ...
        process = await asyncio.create_subprocess_exec(
            "uvicorn",
            "benchmarks.starlette_benchmark:satya_app",
            "--host", host,
            "--port", str(port),
            "--log-level", "error",
            stdout=asyncio.subprocess.DEVNULL,
...
def generate_plots(results: List[BenchmarkResult], output_dir: str = "benchmarks/results"):
    ...
    for (scenario, operation), group_results in grouped_results.items():
        ...
        # Save the plot
        plt.tight_layout()
        plt.savefig(f"{output_dir}/starlette_{scenario}_{operation}_comparison.png")
        plt.close()
...
def generate_summary_plot(results: List[BenchmarkResult], output_dir: str):
    ...
    plt.tight_layout()
    plt.savefig(f"{output_dir}/starlette_summary_comparison.png")
    plt.close()
...
def save_results_to_file(results: List[BenchmarkResult], output_dir: str = "benchmarks/results"):
    ...
    # Save to JSON file
    with open(f"{output_dir}/starlette_benchmark_results.json", "w") as f:
        json.dump(result_dicts, f, indent=2)
    
    # Also save a summary CSV
    with open(f"{output_dir}/starlette_benchmark_summary.csv", "w") as f:
        # Write header
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Benchmark Starlette with satya vs Pydantic")
    ...
    pydantic_results = await run_benchmark(
        "pydantic", TEST_SCENARIOS, operations, 
        BENCHMARK_PORT_PYDANTIC
    )
    
    satya_results = await run_benchmark(
        "satya", TEST_SCENARIOS, operations, 
        BENCHMARK_PORT_SATYA
    )
