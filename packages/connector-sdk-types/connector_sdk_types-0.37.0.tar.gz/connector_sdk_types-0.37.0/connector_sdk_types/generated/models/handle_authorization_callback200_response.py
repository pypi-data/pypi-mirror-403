...
from connector_sdk_types.generated.models.handle_authorization_callback_response import HandleAuthorizationCallbackResponse
from typing import Union, Any, List, Set, TYPE_CHECKING, Optional, Dict
from typing_extensions import Literal, Self
from pydantic import Field

HANDLEAUTHORIZATIONCALLBACK200RESPONSE_ANY_OF_SCHEMAS = ["ErrorResponse", "HandleAuthorizationCallbackResponse"]
...
class HandleAuthorizationCallback200Response(BaseModel):
    ...
    # data type: HandleAuthorizationCallbackResponse
    ...
    def actual_instance_must_validate_anyof(cls, v):
        ...
        error_messages = []
        # validate data type: HandleAuthorizationCallbackResponse
        if not isinstance(v, HandleAuthorizationCallbackResponse):
            error_messages.append(f"Error! Input type `{type(v)}` is not `HandleAuthorizationCallbackResponse`")
        else:
            return v
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in HandleAuthorizationCallback200Response with anyOf schemas: ErrorResponse, HandleAuthorizationCallbackResponse. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into HandleAuthorizationCallback200Response with anyOf schemas: ErrorResponse, HandleAuthorizationCallbackResponse. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], ErrorResponse, HandleAuthorizationCallbackResponse]]:
