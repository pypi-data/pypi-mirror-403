...
class BasicAuthentication(BaseModel):
    """
    Basic authentication model. This is a model exclusively used by standard capabilities.
    """

    username: StrictStr = Field(description="The username for basic auth.")
    password: StrictStr = AnnotatedField(description="The password for basic auth.", secret=True)
    __properties: ClassVar[List[str]] = ["username", "password"]
    model_config = ConfigDict(
        populate_by_name=True,
        validate_assignment=True,
        protected_namespaces=(),
        json_schema_extra={"x-credential-type": "basic"},
    )
