...
class GetAccount(BaseModel):
    """
    Request parameters for retrieving a single account.
    """

    account_id: StrictStr = Field(
        description="Integration-specific identifier that uniquely identifies the user account.",
        json_schema_extra={"x-semantic": "account-id"},
    )
    custom_attributes: Optional[List[StrictStr]] = Field(
        default=None,
        description="Optional array of custom attribute identifiers whose values should be returned in the response.  If omitted, no custom attributes will be included.",
    )
    __properties: ClassVar[List[str]] = ["account_id", "custom_attributes"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of GetAccount from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of GetAccount from a dict"""
        if obj is None:
            return None
        if not isinstance(obj, dict):
            return cls.model_validate(obj)
        _obj = cls.model_validate(
            {"account_id": obj.get("account_id"), "custom_attributes": obj.get("custom_attributes")}
        )
        return _obj
