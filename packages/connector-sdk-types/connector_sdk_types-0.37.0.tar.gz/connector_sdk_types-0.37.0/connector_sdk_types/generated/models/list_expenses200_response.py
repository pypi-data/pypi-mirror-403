...
LISTEXPENSES200RESPONSE_ANY_OF_SCHEMAS = ["ErrorResponse", "ListExpensesResponse"]

class ListExpenses200Response(BaseModel):
    """
    ListExpenses200Response
    """

    # data type: ListExpensesResponse
    anyof_schema_1_validator: Optional[ListExpensesResponse] = None
    # data type: ErrorResponse
    anyof_schema_2_validator: Optional[ErrorResponse] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[ErrorResponse, ListExpensesResponse]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "ErrorResponse", "ListExpensesResponse" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = ListExpenses200Response.model_construct()
        error_messages = []
        # validate data type: ListExpensesResponse
        if not isinstance(v, ListExpensesResponse):
            error_messages.append(f"Error! Input type `{type(v)}` is not `ListExpensesResponse`")
        else:
            return v
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in ListExpenses200Response with anyOf schemas: ErrorResponse, ListExpensesResponse. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[ListExpensesResponse] = None
        try:
            instance.actual_instance = ListExpensesResponse.from_json(json_str)
            return instance
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into ListExpenses200Response with anyOf schemas: ErrorResponse, ListExpensesResponse. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], ErrorResponse, ListExpensesResponse]]:
