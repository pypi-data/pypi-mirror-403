...
class ValidatedCredentialConfig(BaseModel):
    """
    Result of credential validation containing validity and tenant information
    """ # noqa: E501
    valid: StrictBool = Field(description="Indicates whether the provided credentials are valid and active.  - true: Credentials are valid and can be used for API operations, no validation errors expected. - false: Credentials are invalid, expired, or revoked. It is expected that the connector returns at least one validation error.")
    validation_errors: Optional[List[StrictStr]] = Field(default=None, description="(Optional) List of errors that occurred during validation. This capability can return a list of validation errors, these should be displayed to a customer. An example of such an error is \"Access Token is missing required scopes, please update your OAuth app.\"")
    __properties: ClassVar[List[str]] = ["valid", "validation_errors"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of ValidatedCredentialConfig from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of ValidatedCredentialConfig from a dict"""
        if obj is None:
            return None
