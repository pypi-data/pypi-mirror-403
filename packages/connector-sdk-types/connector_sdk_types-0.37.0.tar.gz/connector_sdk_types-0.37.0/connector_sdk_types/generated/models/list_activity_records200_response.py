...
from connector_sdk_types.generated.models.list_activity_records_response import ListActivityRecordsResponse
from typing import Union, Any, List, Set, TYPE_CHECKING, Optional, Dict
from typing_extensions import Literal, Self
from pydantic import Field

LISTACTIVITYRECORDS200RESPONSE_ANY_OF_SCHEMAS = ["ErrorResponse", "ListActivityRecordsResponse"]

class ListActivityRecords200Response(BaseModel):
    """
    ListActivityRecords200Response
    """

    # data type: ListActivityRecordsResponse
    anyof_schema_1_validator: Optional[ListActivityRecordsResponse] = None
    # data type: ErrorResponse
    anyof_schema_2_validator: Optional[ErrorResponse] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[ErrorResponse, ListActivityRecordsResponse]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "ErrorResponse", "ListActivityRecordsResponse" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = ListActivityRecords200Response.model_construct()
        error_messages = []
        # validate data type: ListActivityRecordsResponse
        if not isinstance(v, ListActivityRecordsResponse):
            error_messages.append(f"Error! Input type `{type(v)}` is not `ListActivityRecordsResponse`")
        else:
            return v
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in ListActivityRecords200Response with anyOf schemas: ErrorResponse, ListActivityRecordsResponse. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[ListActivityRecordsResponse] = None
        try:
            instance.actual_instance = ListActivityRecordsResponse.from_json(json_str)
            return instance
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into ListActivityRecords200Response with anyOf schemas: ErrorResponse, ListActivityRecordsResponse. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], ErrorResponse, ListActivityRecordsResponse]]:
