...
class ListApplicationsEntitlements(BaseModel):
    """
    Request parameters for listing application entitlements.
    """

    custom_attributes: Optional[List[StrictStr]] = Field(
        default=None,
        description="Optional array of custom attribute names to include in the entitlement data. Each string in this array represents a specific custom attribute to retrieve. You can get these attributes from `list_custom_attributes_schema`",
    )
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of ListApplicationsEntitlements from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of ListApplicationsEntitlements from a dict"""
        if obj is None:
            return None
        if not isinstance(obj, dict):
            return cls.model_validate(obj)
        _obj = cls.model_validate({"custom_attributes": obj.get("custom_attributes")})
        return _obj
