...
class OauthCredentials(BaseModel):
    """
    OAuth credentials model.  Enough authentication material to enable a capability, e.g. List Accounts, for an OAuth-based connector.
    """ # noqa: E501
    access_token: StrictStr = Field(description="The token used for authenticating API requests, providing access to the API.")
    refresh_token: Optional[StrictStr] = Field(default=None, description="(Optional) A token used to refresh the access token, extending the session without re-authentication.")
    token_type: TokenType = Field(description="The type of token, usually \"bearer\", indicating how the token should be used.")
    state: Optional[StrictStr] = Field(default=None, description="(Optional) A state parameter for security validation, ensuring the response matches the request.")
    __properties: ClassVar[List[str]] = ["access_token", "refresh_token", "token_type", "state"]
