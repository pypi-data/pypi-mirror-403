...
class ActivityRecordTarget(BaseModel):
    """
    Represents the entity affected by an activity.  Currently only supports entitlement targets, but structured for easy future extension.
    """

    account_id: Optional[StrictStr] = Field(
        default=None,
        description="Account ID of the target",
        json_schema_extra={"x-semantic": "account-id"},
    )
    entitlement: Optional[ActivityRecordEntitlement] = Field(
        default=None, description="Entitlement that was affected by the activity"
    )
    __properties: ClassVar[List[str]] = ["account_id", "entitlement"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of ActivityRecordTarget from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of ActivityRecordTarget from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate(
            {
                ...
                "entitlement": ActivityRecordEntitlement.from_dict(obj["entitlement"])
