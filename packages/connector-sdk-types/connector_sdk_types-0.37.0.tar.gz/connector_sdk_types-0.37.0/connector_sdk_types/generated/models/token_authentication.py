...
class TokenAuthentication(BaseModel):
    """
    Token-based authentication model. This is a model exclusively used by standard capabilities.
    """

    token: StrictStr = AnnotatedField(description="The token for token-based auth.", secret=True)
    __properties: ClassVar[List[str]] = ["token"]
    model_config = ConfigDict(
        populate_by_name=True,
        validate_assignment=True,
        protected_namespaces=(),
        json_schema_extra={"x-credential-type": "token"},
    )
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of TokenAuthentication from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of TokenAuthentication from a dict"""
        if obj is None:
            return None
