...
class NoopEffect(BaseModel):
    """
    Effect indicating no operation was needed because the system already matched the desired state.
    """

    kind: StrictStr = Field(default="noop")
    reason: NoopEffectReason = Field(
        description='Required explanation of why no operation was performed.  This provides structured information about why an operation didn\'t modify the target system, such as "state_already_matches" or custom reason strings.'
    )
    ...
    def kind_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(["noop"]):
            raise ValueError("must be one of enum values ('noop')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of NoopEffect from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of NoopEffect from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate(
            {
                "kind": obj.get("kind") if obj.get("kind") is not None else "noop",
