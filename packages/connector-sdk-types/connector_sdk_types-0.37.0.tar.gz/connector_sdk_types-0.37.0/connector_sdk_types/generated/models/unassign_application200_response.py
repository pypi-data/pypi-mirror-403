...
from connector_sdk_types.generated.models.unassign_application_response import UnassignApplicationResponse
from typing import Union, Any, List, Set, TYPE_CHECKING, Optional, Dict
from typing_extensions import Literal, Self
from pydantic import Field

UNASSIGNAPPLICATION200RESPONSE_ANY_OF_SCHEMAS = ["ErrorResponse", "UnassignApplicationResponse"]

class UnassignApplication200Response(BaseModel):
    """
    UnassignApplication200Response
    """

    # data type: UnassignApplicationResponse
    anyof_schema_1_validator: Optional[UnassignApplicationResponse] = None
    # data type: ErrorResponse
    anyof_schema_2_validator: Optional[ErrorResponse] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[ErrorResponse, UnassignApplicationResponse]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "ErrorResponse", "UnassignApplicationResponse" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = UnassignApplication200Response.model_construct()
        error_messages = []
        # validate data type: UnassignApplicationResponse
        if not isinstance(v, UnassignApplicationResponse):
            error_messages.append(f"Error! Input type `{type(v)}` is not `UnassignApplicationResponse`")
        else:
            return v
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in UnassignApplication200Response with anyOf schemas: ErrorResponse, UnassignApplicationResponse. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[UnassignApplicationResponse] = None
        try:
            instance.actual_instance = UnassignApplicationResponse.from_json(json_str)
            return instance
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into UnassignApplication200Response with anyOf schemas: ErrorResponse, UnassignApplicationResponse. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], ErrorResponse, UnassignApplicationResponse]]:
