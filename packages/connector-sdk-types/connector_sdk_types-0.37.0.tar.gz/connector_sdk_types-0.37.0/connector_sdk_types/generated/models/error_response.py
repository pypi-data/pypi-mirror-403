...
class ErrorResponse(BaseModel):
    """
    Error details and metadata returned when an operation fails.
    """

    is_error: StrictBool = Field(description="Always true for error responses.", default=True)
    error: Error = Field(description="The error details.")
    raw_data: Optional[Dict[str, Any]] = Field(
        default=None, description="Raw data associated with the error, if any."
    )
    page: Optional[Page] = Field(default=None, description="Pagination information, if applicable.")
    __properties: ClassVar[List[str]] = ["is_error", "error", "raw_data", "page"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate(
            {
                "is_error": obj.get("is_error") if obj.get("is_error") is not None else True,
