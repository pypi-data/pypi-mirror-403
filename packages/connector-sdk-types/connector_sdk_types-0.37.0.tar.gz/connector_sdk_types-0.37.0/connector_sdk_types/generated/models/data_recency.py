...
class DataRecency(BaseModel):
    """
    Response model containing data recency information for standard capabilities.  This model maps each standard capability to an optional datetime indicating when the data for that capability was last updated or refreshed.
    """ # noqa: E501
    data: Dict[str, Optional[datetime]] = Field(description="Map of standard capability names to their data recency timestamps.  Each entry in this map represents a standard capability and its associated data recency information:  - **Key**: The name of a standard capability (e.g., \"list_accounts\", \"list_entitlements\") - **Value**: Datetime indicating when the data for this capability was last updated (may be null if recency information is not available)  ## Capability Coverage  The map should include entries for all standard capabilities that the connector supports and for which it can provide meaningful recency information.  ## Data Freshness Interpretation  - **non-null timestamp**: Data is current as of the specified time - **null value**: Connector cannot determine due to an unexpected error - **missing key**: If the a StandardCapabilityName doesn't have a key in the map, then the connector doesn't support surfacing data recency for that capability.  ## Common Use Cases  - **list_accounts**: When the account data was last synchronized - **list_entitlements**: When permission/role data was last updated - **list_updated_accounts**: When the change tracking data was last refreshed")
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of DataRecency from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of DataRecency from a dict"""
        if obj is None:
            return None
