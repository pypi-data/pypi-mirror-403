...
LISTRESOURCES200RESPONSE_ANY_OF_SCHEMAS = ["ErrorResponse", "ListResourcesResponse"]

class ListResources200Response(BaseModel):
    """
    ListResources200Response
    """

    # data type: ListResourcesResponse
    anyof_schema_1_validator: Optional[ListResourcesResponse] = None
    # data type: ErrorResponse
    anyof_schema_2_validator: Optional[ErrorResponse] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[ErrorResponse, ListResourcesResponse]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "ErrorResponse", "ListResourcesResponse" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = ListResources200Response.model_construct()
        error_messages = []
        # validate data type: ListResourcesResponse
        if not isinstance(v, ListResourcesResponse):
            error_messages.append(f"Error! Input type `{type(v)}` is not `ListResourcesResponse`")
        else:
            return v
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in ListResources200Response with anyOf schemas: ErrorResponse, ListResourcesResponse. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[ListResourcesResponse] = None
        try:
            instance.actual_instance = ListResourcesResponse.from_json(json_str)
            return instance
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into ListResources200Response with anyOf schemas: ErrorResponse, ListResourcesResponse. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], ErrorResponse, ListResourcesResponse]]:
