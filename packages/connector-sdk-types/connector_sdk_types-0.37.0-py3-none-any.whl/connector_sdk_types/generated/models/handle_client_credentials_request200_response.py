...
from connector_sdk_types.generated.models.handle_client_credentials_response import HandleClientCredentialsResponse
from typing import Union, Any, List, Set, TYPE_CHECKING, Optional, Dict
from typing_extensions import Literal, Self
from pydantic import Field

HANDLECLIENTCREDENTIALSREQUEST200RESPONSE_ANY_OF_SCHEMAS = ["ErrorResponse", "HandleClientCredentialsResponse"]
...
class HandleClientCredentialsRequest200Response(BaseModel):
    ...
    # data type: HandleClientCredentialsResponse
    ...
    def actual_instance_must_validate_anyof(cls, v):
        ...
        error_messages = []
        # validate data type: HandleClientCredentialsResponse
        if not isinstance(v, HandleClientCredentialsResponse):
            error_messages.append(f"Error! Input type `{type(v)}` is not `HandleClientCredentialsResponse`")
        else:
            return v
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in HandleClientCredentialsRequest200Response with anyOf schemas: ErrorResponse, HandleClientCredentialsResponse. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into HandleClientCredentialsRequest200Response with anyOf schemas: ErrorResponse, HandleClientCredentialsResponse. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], ErrorResponse, HandleClientCredentialsResponse]]:
