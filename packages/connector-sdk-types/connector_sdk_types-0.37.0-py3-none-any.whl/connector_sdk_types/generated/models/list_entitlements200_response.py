...
class ListEntitlements200Response(BaseModel):
    ...
    # data type: ListEntitlementsResponse
    ...
    def actual_instance_must_validate_anyof(cls, v):
        ...
        error_messages = []
        # validate data type: ListEntitlementsResponse
        if not isinstance(v, ListEntitlementsResponse):
            error_messages.append(f"Error! Input type `{type(v)}` is not `ListEntitlementsResponse`")
        else:
            return v
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in ListEntitlements200Response with anyOf schemas: ErrorResponse, ListEntitlementsResponse. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into ListEntitlements200Response with anyOf schemas: ErrorResponse, ListEntitlementsResponse. Details: " + ", ".join(error_messages))
        else:
            return instance
