...
class ExpenseFilters(BaseModel):
    """
    Filters to restrict an expense sync
    """ # noqa: E501
    transaction_date: Optional[TimeRange] = Field(default=None, description="Used to filter an expense sync to only expenses with a transaction_date that is within a specific time range")
    __properties: ClassVar[List[str]] = ["transaction_date"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of ExpenseFilters from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.transaction_date:
            _dict['transaction_date'] = self.transaction_date.to_dict()
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of ExpenseFilters from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "transaction_date": TimeRange.from_dict(obj["transaction_date"]) if obj.get("transaction_date") is not None else None
        })
        return _obj
