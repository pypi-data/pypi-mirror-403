...
import re
import json
from pydantic import BaseModel, ConfigDict, StrictStr, field_validator, Field
from typing import Any, ClassVar, Dict, List
from typing import Optional, Set
from typing_extensions import Self


class DeletedEffect(BaseModel):
    """
    Effect indicating an existing record was removed from the target system.
    """

    kind: StrictStr = Field(default="deleted")
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of DeletedEffect from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of DeletedEffect from a dict"""
        if obj is None:
            return None
        if not isinstance(obj, dict):
            return cls.model_validate(obj)
        _obj = cls.model_validate(
            {"kind": obj.get("kind") if obj.get("kind") is not None else "deleted"}
        )
        return _obj
