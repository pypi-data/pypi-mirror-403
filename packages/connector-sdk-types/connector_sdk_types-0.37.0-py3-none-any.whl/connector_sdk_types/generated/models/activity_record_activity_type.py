...
ACTIVITYRECORDACTIVITYTYPE_ANY_OF_SCHEMAS = ["ActivityType", "str"]

class ActivityRecordActivityType(BaseModel):
    """
    Type of activity performed (e.g., \"AssumeRole\", \"AccessFeature\"). For a given activity, prefer a standard ActivityType if one applies.
    """

    # data type: ActivityType
    anyof_schema_1_validator: Optional[ActivityType] = None
    # data type: str
    anyof_schema_2_validator: Optional[StrictStr] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[ActivityType, str]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "ActivityType", "str" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = ActivityRecordActivityType.model_construct()
        error_messages = []
        # validate data type: ActivityType
        if not isinstance(v, ActivityType):
            error_messages.append(f"Error! Input type `{type(v)}` is not `ActivityType`")
        else:
            return v
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in ActivityRecordActivityType with anyOf schemas: ActivityType, str. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[ActivityType] = None
        try:
            instance.actual_instance = ActivityType.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into ActivityRecordActivityType with anyOf schemas: ActivityType, str. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], ActivityType, str]]:
