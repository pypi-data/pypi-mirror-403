...
DOWNGRADELICENSE200RESPONSE_ANY_OF_SCHEMAS = ["DowngradeLicenseResponse", "ErrorResponse"]

class DowngradeLicense200Response(BaseModel):
    """
    DowngradeLicense200Response
    """

    # data type: DowngradeLicenseResponse
    anyof_schema_1_validator: Optional[DowngradeLicenseResponse] = None
    # data type: ErrorResponse
    anyof_schema_2_validator: Optional[ErrorResponse] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[DowngradeLicenseResponse, ErrorResponse]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "DowngradeLicenseResponse", "ErrorResponse" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = DowngradeLicense200Response.model_construct()
        error_messages = []
        # validate data type: DowngradeLicenseResponse
        if not isinstance(v, DowngradeLicenseResponse):
            error_messages.append(f"Error! Input type `{type(v)}` is not `DowngradeLicenseResponse`")
        else:
            return v
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in DowngradeLicense200Response with anyOf schemas: DowngradeLicenseResponse, ErrorResponse. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[DowngradeLicenseResponse] = None
        try:
            instance.actual_instance = DowngradeLicenseResponse.from_json(json_str)
            return instance
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into DowngradeLicense200Response with anyOf schemas: DowngradeLicenseResponse, ErrorResponse. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], DowngradeLicenseResponse, ErrorResponse]]:
