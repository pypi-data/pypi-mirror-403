...
class ActivityRecordActor(BaseModel):
    """
    Represents the entity that initiated an activity.  Currently only supports accounts (e.g., users or service accounts), but designed to be extended with other types in the future.
    """

    account_id: StrictStr = Field(
        description="Account ID of the actor", json_schema_extra={"x-semantic": "account-id"}
    )
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of ActivityRecordActor from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of ActivityRecordActor from a dict"""
        if obj is None:
            return None
