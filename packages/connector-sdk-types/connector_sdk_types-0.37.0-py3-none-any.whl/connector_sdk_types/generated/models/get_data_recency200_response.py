...
from connector_sdk_types.generated.models.get_data_recency_response import GetDataRecencyResponse
from typing import Union, Any, List, Set, TYPE_CHECKING, Optional, Dict
from typing_extensions import Literal, Self
from pydantic import Field

GETDATARECENCY200RESPONSE_ANY_OF_SCHEMAS = ["ErrorResponse", "GetDataRecencyResponse"]

class GetDataRecency200Response(BaseModel):
    """
    GetDataRecency200Response
    """

    # data type: GetDataRecencyResponse
    anyof_schema_1_validator: Optional[GetDataRecencyResponse] = None
    # data type: ErrorResponse
    anyof_schema_2_validator: Optional[ErrorResponse] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[ErrorResponse, GetDataRecencyResponse]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "ErrorResponse", "GetDataRecencyResponse" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = GetDataRecency200Response.model_construct()
        error_messages = []
        # validate data type: GetDataRecencyResponse
        if not isinstance(v, GetDataRecencyResponse):
            error_messages.append(f"Error! Input type `{type(v)}` is not `GetDataRecencyResponse`")
        else:
            return v
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in GetDataRecency200Response with anyOf schemas: ErrorResponse, GetDataRecencyResponse. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[GetDataRecencyResponse] = None
        try:
            instance.actual_instance = GetDataRecencyResponse.from_json(json_str)
            return instance
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into GetDataRecency200Response with anyOf schemas: ErrorResponse, GetDataRecencyResponse. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], ErrorResponse, GetDataRecencyResponse]]:
