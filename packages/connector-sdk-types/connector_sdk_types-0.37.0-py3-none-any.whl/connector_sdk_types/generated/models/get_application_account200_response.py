...
from connector_sdk_types.generated.models.get_application_account_response import GetApplicationAccountResponse
...
class GetApplicationAccount200Response(BaseModel):
    ...
    # data type: GetApplicationAccountResponse
    ...
    def actual_instance_must_validate_anyof(cls, v):
        ...
        error_messages = []
        # validate data type: GetApplicationAccountResponse
        if not isinstance(v, GetApplicationAccountResponse):
            error_messages.append(f"Error! Input type `{type(v)}` is not `GetApplicationAccountResponse`")
        else:
            return v
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in GetApplicationAccount200Response with anyOf schemas: ErrorResponse, GetApplicationAccountResponse. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into GetApplicationAccount200Response with anyOf schemas: ErrorResponse, GetApplicationAccountResponse. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], ErrorResponse, GetApplicationAccountResponse]]:
