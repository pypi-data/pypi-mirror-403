...
class AuthCredential(BaseModel):
    ...
    id: Optional[StrictStr] = Field(
        default=None, description="The ID of the authentication schema."
    )
    oauth: Optional[OAuthCredential] = Field(
        default=None, description="OAuth credentials, if using OAuth."
    )
    oauth_client_credentials: Optional[OAuthClientCredential] = Field(
        default=None,
        description="OAuth client credentials, if using OAuth Client Credentials flow.",
    )
    oauth1: Optional[OAuth1Credential] = Field(
        default=None, description="OAuth 1.0a credentials, if using OAuth 1.0a."
    )
    basic: Optional[BasicCredential] = Field(
        default=None, description="Basic auth credentials, if using basic auth."
    )
    token: Optional[TokenCredential] = Field(
        default=None, description="Token credentials, if using token-based auth."
    )
    jwt: Optional[JWTCredential] = Field(default=None, description="JWT credentials, if using JWT.")
    service_account: Optional[ServiceAccountCredential] = Field(
        default=None, description="Service account credentials, if using service account."
    )
    key_pair: Optional[KeyPairCredential] = Field(
        default=None,
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate(
            {
                "id": obj.get("id"),
                "oauth": OAuthCredential.from_dict(obj["oauth"])
                if obj.get("oauth") is not None
                else None,
                "oauth_client_credentials": OAuthClientCredential.from_dict(
                    obj["oauth_client_credentials"]
                )
                if obj.get("oauth_client_credentials") is not None
                else None,
                "oauth1": OAuth1Credential.from_dict(obj["oauth1"])
                if obj.get("oauth1") is not None
                else None,
                "basic": BasicCredential.from_dict(obj["basic"])
                if obj.get("basic") is not None
                else None,
                "token": TokenCredential.from_dict(obj["token"])
                if obj.get("token") is not None
                else None,
                "jwt": JWTCredential.from_dict(obj["jwt"]) if obj.get("jwt") is not None else None,
                "service_account": ServiceAccountCredential.from_dict(obj["service_account"])
