...
class Expense(BaseModel):
    """
    A representation of an expense or card transaction. The fields provided here are the most commonly occurring across all connectors. For a complete list of fields specific to your connector, please refer to the [info endpoint](#tag/Describe/operation/info).
    """ # noqa: E501
    id: StrictStr = Field(description="A unique, stable identifier for an expense in the source system. It is important that this does not change over time.")
    report_id: Optional[StrictStr] = Field(default=None, description="A unique, stable identifier for the report that this expense is a part of, if available")
    transaction_date: StrictStr = Field(description="The original date in which an employee paid the seller, in ISO 8601 format")
    payment_date: Optional[StrictStr] = Field(default=None, description="The date in which the company paid the expense, whether to the seller or the employee, in 8601 format")
    total_amount: Amount = Field(description="The total amount that the company owes for the expense")
    paid_amount: Optional[Amount] = Field(default=None, description="The total amount that the company has paid for the expense so far")
    seller: Vendor = Field(description="The company that sold the product represented by the expense")
    description: StrictStr = Field(description="A description of the purchase")
    submitter: SpendUser = Field(description="The employee that submitted the expense")
    type: ExpenseType = Field(description="The type of expense, either `reimbursement` or `card_transaction`")
    approval_status: ExpenseApprovalStatus = Field(description="The current state of the expense's approval status")
    payment_status: ExpensePaymentStatus = Field(description="The current state of the expense's payment status")
    url: Optional[StrictStr] = Field(default=None, description="A URL that directs the user to the expense in the source system")
    attributes: Optional[Dict[str, Any]] = Field(default=None, description="Attributes specific to the source system, but not to the tenant")
    custom_attributes: Optional[Dict[str, Any]] = Field(default=None, description="Attributes specific to the tenant")
    __properties: ClassVar[List[str]] = ["id", "report_id", "transaction_date", "payment_date", "total_amount", "paid_amount", "seller", "description", "submitter", "type", "approval_status", "payment_status", "url", "attributes", "custom_attributes"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of paid_amount
        if self.paid_amount:
            _dict['paid_amount'] = self.paid_amount.to_dict()
        ...
        if self.approval_status:
            _dict['approval_status'] = self.approval_status.to_dict()
        # override the default output from pydantic by calling `to_dict()` of payment_status
        if self.payment_status:
            _dict['payment_status'] = self.payment_status.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "total_amount": Amount.from_dict(obj["total_amount"]) if obj.get("total_amount") is not None else None,
            "paid_amount": Amount.from_dict(obj["paid_amount"]) if obj.get("paid_amount") is not None else None,
            "seller": Vendor.from_dict(obj["seller"]) if obj.get("seller") is not None else None,
            "description": obj.get("description"),
            "submitter": SpendUser.from_dict(obj["submitter"]) if obj.get("submitter") is not None else None,
            "type": obj.get("type"),
            "approval_status": ExpenseApprovalStatus.from_dict(obj["approval_status"]) if obj.get("approval_status") is not None else None,
            "payment_status": ExpensePaymentStatus.from_dict(obj["payment_status"]) if obj.get("payment_status") is not None else None,
            "url": obj.get("url"),
