...
def _get_jlink_script_content(ctx, net_name, debug_net):
    """
    Get base64-encoded J-Link script content for a debug net.

    Priority:
    1. Local .lager config [DEBUG] section (override)
    2. Script stored with net on box (default)

    Args:
        ctx: Click context
        net_name: Name of the debug net
        debug_net: Debug net configuration dict (may contain stored jlink_script)

    Returns:
        Base64-encoded script content, or None if no script configured
    """
    import base64
    from ....config import get_debug_script_for_net

    # Priority 1: Check local .lager config (override)
    ...
    # Priority 2: Use script stored with net on box
    stored_script = debug_net.get('jlink_script')
    if stored_script:
        return stored_script  # Already base64-encoded
