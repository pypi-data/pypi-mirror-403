...
Use `/telegram setup` in CLI to configure."""
    ...
    async def handler(message: str, *, _skip_broadcast: bool = False) -> AsyncIterator[dict]:
        """Process a message and stream events back.

        Args:
            message: User message to process
            _skip_broadcast: If True, skip broadcasting user message (used when
                processing multiuser messages on behalf of other users)
        ...
        # Check if message triggers agent (contains @agent or @emdash anywhere)
        import re
        message_lower = message.lower()
        triggers_agent = "@agent" in message_lower or "@emdash" in message_lower
        # Extract the message to send to agent (remove @agent/@emdash mention)
        agent_message = message
        if triggers_agent:
            # Remove @agent or @emdash from the message (case insensitive)
            agent_message = re.sub(r'@agent|@emdash', '', message, flags=re.IGNORECASE).strip()
            # If nothing left after removing trigger, use original message
            if not agent_message:
                agent_message = message

        # If in multiuser session as non-owner, send message to multiuser API
        if multiuser_session_id and multiuser_user_id and not multiuser_is_owner:
            async with httpx.AsyncClient(timeout=30.0) as client:
                try:
                    # Send message to shared session
                    msg_url = f"{multiuser_server_url}/api/multiuser/session/{multiuser_session_id}/message"
                    resp = await client.post(msg_url, json={
                        "user_id": multiuser_user_id,
                        "content": message,
                        "priority": 0,
                        "trigger_agent": triggers_agent,  # Only trigger agent if @agent/@emdash
                    })
                    resp.raise_for_status()

                    if triggers_agent:
                        # Message was queued for agent - the owner will process it
                        yield {
                            "type": "response",
                            "data": {"content": "*Sent to agent...*"}
                        }
                    else:
                        # Chat message sent - just clear processing state
                        yield {"type": "set_processing", "data": {"processing": False}}
                    return
                except httpx.HTTPStatusError as e:
                    yield {
                        "type": "error",
                        "data": {"message": f"Failed to send message: {e.response.status_code}"}
                    }
                    return
                except Exception as e:
                    yield {
        ...
        # If owner in multiuser session, broadcast our message to others first
        # Skip if _skip_broadcast is True (processing message on behalf of another user)
        if multiuser_session_id and multiuser_user_id and multiuser_is_owner and not _skip_broadcast:
            try:
                async with httpx.AsyncClient(timeout=10.0) as bc:
                    # Broadcast user message event
                    await bc.post(
                        f"{multiuser_server_url}/api/multiuser/session/{multiuser_session_id}/broadcast_event",
                        json={
                            "user_id": multiuser_user_id,
                            "event_type": "user_message",
                            "data": {"content": message, "display_name": "Owner"},
                        }
                    )
            except Exception:
                pass  # Silently fail broadcast

            # If this is just a chat message (no @agent/@emdash), don't process through agent
            if not triggers_agent:
                yield {"type": "set_processing", "data": {"processing": False}}
                return

        async with httpx.AsyncClient(timeout=None) as client:
            # Use agent_message (with @agent/@emdash stripped) if in multiuser session
            message_to_send = agent_message if multiuser_session_id else message
            payload = {
            ...
            # Track response content for multiuser broadcast
            response_content = ""

            # Events to broadcast to other participants in multiuser session
            broadcast_events = {"tool_start", "tool_result", "thinking", "chat_chunk", "chat_complete", "progress", "subagent_start", "subagent_end", "response", "partial_response"}

            # Helper to broadcast events to multiuser session
            async def broadcast_event(event_type: str, event_data: dict) -> None:
                if multiuser_session_id and multiuser_user_id and multiuser_is_owner:
                    import sys
                    print(f"[BROADCAST] event={event_type} user_id={multiuser_user_id!r}", file=sys.stderr)
                    try:
                        ...
                        pass  # Don't block on broadcast failures
            ...
            try:
                        ...
                        if line.startswith("event: "):
                            ...
                            if current_event:
                                try:
                                    ...
                                    # Capture response content for broadcast
                                    if current_event == "response":
                                        response_content = data.get("content", "")

                                    # Broadcast relevant events to multiuser participants
                                    # Always broadcast so joiners receive events via SSE
                                    # Owner's listener filters their own events by source_user_id
                                    if current_event in broadcast_events:
                                        await broadcast_event(current_event, data)
                ...
                # If owner in multiuser session, broadcast the response
                # Skip if _skip_broadcast is True (processing joiner's message - streaming events already broadcast)
                # Also skip since intermediate events (chat_chunk, chat_complete) are broadcast to participants
                # and they already receive the response via those streaming events
                if multiuser_session_id and multiuser_user_id and multiuser_is_owner and response_content and not _skip_broadcast:
                    # Only broadcast_response for owner's own messages when NOT streaming to participants
                    # Since we now broadcast streaming events, this is mostly a fallback
                    pass  # Streaming events already handle response broadcast
    ...
    async def approve_plan_mode(approved: bool = True, reply: str | None = None) -> AsyncIterator[dict]:
        """Approve or reject plan mode request and stream continuation.

        Args:
            approved: Whether the plan mode was approved
