...
export default function App({ bridgeMode = false, hasInputSupport = true }: AppProps) {
  ...
  // Ref to track typing indicator timeouts per user (for proper cleanup)
      ...
      switch (event.type) {
        ...
        case 'user_typing': {
          ...
          if (userId && userId !== state.multiuser.userId) {
            ...
            // Clear existing timeout for this user before setting a new one
            const existingTimeout = typingTimeoutsRef.current.get(userId);
            if (existingTimeout) {
              clearTimeout(existingTimeout);
            }
            // Set new timeout to auto-remove after 3 seconds of no typing events
            const newTimeout = setTimeout(() => {
              dispatch({ type: 'REMOVE_TYPING_USER', payload: userId });
              typingTimeoutsRef.current.delete(userId);
            }, 3000);
            typingTimeoutsRef.current.set(userId, newTimeout);
          }
          break;
        }
