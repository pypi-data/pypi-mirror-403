...
export function processEvent(
  ...
  switch (eventType) {
    ...
    case 'response': {
      ...
      const content = event.data.content || '';
      // Only add content if it wasn't already displayed via streaming
      // (hadStreaming is true when chat_chunk events were processed)
      if (content && !state.hadStreaming) {
        // This is a slash command response - add to log
        dispatch({
          type: 'ADD_LOG',
          payload: {
    ...
    case 'request_approval': {
      ...
      dispatch({ type: 'SET_APPROVAL', payload: true });
