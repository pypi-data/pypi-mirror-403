 ...
 */
export function handleApprove(dispatch: React.Dispatch<Action>): void {
  sendMessage({ type: 'plan_approval', data: { approved: true } });
  dispatch({ type: 'SET_APPROVAL', payload: false });
}

export function handleReject(dispatch: React.Dispatch<Action>): void {
  sendMessage({ type: 'plan_approval', data: { approved: false } });
  dispatch({ type: 'SET_APPROVAL', payload: false });
}

export function handleReply(message: string, dispatch: React.Dispatch<Action>): void {
  sendMessage({ type: 'plan_approval', data: { approved: false, reply: message } });
  dispatch({ type: 'SET_APPROVAL', payload: false });
}
