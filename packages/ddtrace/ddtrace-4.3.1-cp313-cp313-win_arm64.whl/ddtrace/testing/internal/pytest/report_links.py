...
def _build_test_commit_redirect_url(base_url, ci_tags, service, env):
    params = {
        "repo_url": ci_tags.get(GitTag.REPOSITORY_URL),
        "branch": ci_tags.get(GitTag.BRANCH),
        "commit_sha": ci_tags.get(GitTag.COMMIT_SHA),
        "service": service,
    }
...
def _build_test_runs_url(base_url, ci_tags):
    ci_job_name = ci_tags.get(CITag.JOB_NAME)
    ci_pipeline_id = ci_tags.get(CITag.PIPELINE_ID)
