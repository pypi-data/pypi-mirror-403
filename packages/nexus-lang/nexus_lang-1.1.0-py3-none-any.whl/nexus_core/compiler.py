...
""")
        ...
        # React blocks
        if blocks.get('react'):
            from nexus_core.frontend import FrontendCompiler, FrontendConfig
            config = FrontendConfig(framework='react')
            fc = FrontendCompiler('nexus_frontend')
            for i, code in enumerate(blocks.get('react', [])):
                result = fc.compile_react(code, config)
                artifacts.append(result)
                print(f"[NEXUS] React component generated")

        # Next.js blocks
        if blocks.get('nextjs'):
            from nexus_core.frontend import FrontendCompiler, FrontendConfig
            config = FrontendConfig(framework='nextjs')
            fc = FrontendCompiler('nexus_frontend')
            for i, code in enumerate(blocks.get('nextjs', [])):
                result = fc.compile_nextjs(code, config)
                artifacts.append(result)
                print(f"[NEXUS] Next.js project generated")
