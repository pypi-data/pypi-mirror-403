...
9. **Create GitHub Release**
   ...
   Use this template to ensure consistency and link back to full CHANGELOG:

   ```bash
   gh release create vX.Y.Z --title "vX.Y.Z - Brief Description" --notes "$(cat <<'EOF'
   ## What's New in vX.Y.Z

   [Copy the relevant section from CHANGELOG.md here]
   ...
   # With Redis support
   pip install bffauth[redis]==X.Y.Z
   ```

   **Full Changelog**: https://github.com/ShadNygren/bffauth/blob/main/CHANGELOG.md

   **Compare changes**: https://github.com/ShadNygren/bffauth/compare/vPREVIOUS...vX.Y.Z
   EOF
   )"
   ```

   **Important**: GitHub Releases should mirror the CHANGELOG.md entry for that version,
   plus include a link back to the full CHANGELOG for users who want complete history.
