...
from .domain import Batch, Task, Slice, News, User, PushConfig, SourceKeywordOffset, PushConfigKeyword
...
class Context:
    ...
    def must_get_task(self) -> Task:
        return self._must_get(ContextField.Task)

    def must_get_slice(self) -> Slice:
        return self._must_get(ContextField.Slice)

    def must_get_news(self) -> News:
        return self._must_get(ContextField.News)
    ...
    def with_task(self, task: Task) -> 'Context':
        return self._with(key=ContextField.Task, value=task)

    def with_slice(self, slice_obj: Slice) -> 'Context':
        return self._with(key=ContextField.Slice, value=slice_obj)

    def with_user(self, user: User) -> 'Context':
        return self._with(key=ContextField.User, value=user)

    def with_news(self, news: News) -> 'Context':
        return self._with(key=ContextField.News, value=news)
