...
class Dash(object):
    ...
    def __init__(self, log_way='./logbook', window=1, divisor=12, span=60, format=None, show=True, **kwargs):
        '''
        Args:
        >| log_way: a directory to store log files or an iterable
        ...
        >| format: a list of which the element is format and mode, e.g ['3f', RAW]
        >| show: whether to show in current process
        '''
        if isinstance(log_way, str):
            self.log_way = log_way
            if not os.path.exists(self.log_way):
                os.makedirs(self.log_way)
        elif isinstance(log_way, Iterable):
            self.log_way = None
            ...
            self.kwargs = kwargs
            self.iterable = log_way.__iter__()
            if not hasattr(log_way, '__len__'):
                self.length = kwargs.get('mpe', -1)
            else:
                self.length = len(log_way)
            self.cnt = 0
        else:
            raise TypeError('NEBULAE ERROR ៙ "log_way" must be either a path string or an iterator.')
        self.show = show
        self.window = window
    ...
    def _formatAsStr(self, stage, abbr, value, epoch, mile, mpe):
        ...
        if mode == RAW:
            ...
            if form(value, epoch, mile, mpe, stage):
                ...
                assert H<=96, 'NEBULAE ERROR ៙ the image height should not be greater than 64.'
    ...
    def __call__(self, entry, epoch, mile, mpe, stage='ITER', interv=1, duration=None, plot=True, wipe=False, flush=0,
        ...
        for abbr, value in entry.items():
            ...
            if self.log_way is None or self.format[abbr][1] in (IMAGE, INVIZ, CUSTOM):
                if flag_epoch_end:
        ...
        if flag_epoch_end:
            ...
            display = '| %d%s Epoch ⚘ %s Miles ₪ %.2fs/epoch | %s |%s' \
                    % (epoch, ordinal, mileage, (time.time() - self.time)*mpe / max(mpe - self.unk_miles, 1), stage, string_epoch)
    ...
    def log(self, gauge=True, tachograph=True, history='', subdir=''):
        if not self.show:
            return
        if subdir:
            log_way = os.path.join(self.log_way, subdir)
            os.makedirs(log_way, exist_ok=True)
        else:
            log_way = self.log_way
        if history:
        ...
        if gauge:
            ...
            for k in boards.keys():
                for i, b in enumerate(boards[k]):
                    ...
                    plt.savefig(os.path.join(log_way, '%s_%s_%.3g_mile_%d.jpg'
            ...
            for k in boards.keys():
                ...
                if len(boards[k]) > 0 and self.max_epoch > 0:
                    ...
                    plt.savefig(os.path.join(log_way, '%s_epoch_%d.jpg' % (k, self.max_epoch)))
                plt.close()
        if tachograph:
            for k in self.gauge_mile.keys():
                stage, metric = k.split(':')
                df = pd.DataFrame(data={'mile': self.trail_mile[stage], k: self.gauge_mile[k]})
                df.to_csv(os.path.join(log_way, '%s_%s_mile.csv'%(metric.replace('/', '-'), stage)), index=None)
                df = pd.DataFrame(data={'epoch': self.trail_epoch[stage], k: self.gauge_epoch[k]})
                df.to_csv(os.path.join(log_way, '%s_%s_epoch.csv' % (metric.replace('/', '-'), stage)), index=None)
...
if __name__ == "__main__": # for simple tests
    ...
    for i,n in Dash(enumerate(range(30)), format={'number': [lambda x:x[1], CUSTOM]}, epoch=2, mpe=30, stage='COUNT', interv=1, wipe=False):
        time.sleep(0.3)

    # in a complicated manner
    ld = Dash(format={'Acc': ['.1f', PERCENT]})
