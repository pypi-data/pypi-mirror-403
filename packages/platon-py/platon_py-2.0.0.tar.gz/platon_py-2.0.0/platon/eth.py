...
class BaseEth(Module):
    ...
    _get_BlockQuorumCert: Method[Callable[[List[_Hash32]], List[BlockQuorumCert]]] = Method(
        RPCEndpoint("platon_getBlockQuorumCertByHash"),
        mungers=[default_root_munger],
    )
...
class AsyncEth(BaseEth):
    ...
    async def get_block_quorum_cert(self, hashs: List[_Hash32]) -> List[BlockQuorumCert]:
        # types ignored b/c mypy conflict with BlockingEth properties
        return await self._get_BlockQuorumCert(hashs)  # type: ignore
...
class Eth(BaseEth, Module):
    ...
    def getBlockQuorumCert(self, hashs: List[_Hash32]) -> List[BlockQuorumCert]:
        return self._get_BlockQuorumCert(hashs)
