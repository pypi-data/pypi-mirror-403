...
class StructurePlotter:
    ...
    def __init__(self, res):
        ...
        # Cache Koordinaten
        self.xyz = np.array(
            [self.nodes["x[m]"], self.nodes["y[m]"], self.nodes["z[m]"]]
        ).T
    ...
    def _get_result_data(self, kind):
        ...
        if attr_name and hasattr(self.res, attr_name):
            ...
            print(
                f"Warnung: Ergebnis '{kind}' nicht gefunden (oder in 'res' noch nicht berechnet)."
            )
    ...
    # Änderung 1: Parameter 'show_local_z' hinzufügen
    def plot_diagram_interactive(self, kind="MY", scale_init=1.0, font_init=9, show_loads=True, flip_sign=False, show_local_z=False):
        """
        ...
        sl_labels = Slider(ax_sl_labels, 'Werte', 0, 5, valinit=0, valstep=1) 
        ...
        def draw(val):
            ...
            for i in range(len(self.na)):
                ...
                # ========================================================
                # NEU: Lokale z-Achse zeichnen
                # ========================================================
                if show_local_z:
                    # Mittelpunkt des Stabes
                    Pm = (Pa + Pe) / 2.0
                    # Pfeillänge: ca. 5% der Gesamtstrukturgröße
                    axis_len = span * 0.05
                    # Endpunkt des Pfeils in z-Richtung
                    Pz = Pm + ez * axis_len
                    
                    # Nutzung deiner existierenden _draw_arrow Hilfsfunktion
                    # Farbe 'cyan' hebt sich gut von Rot/Blau (Schnittgrößen) ab
                    self._draw_arrow(ax, actors, Pm, Pz, color="cyan", style="simple")
                # ========================================================
                ...
                # 1. Diagramm (Polygon + Kontur + Kamm)
                ...
                if n_labels > 0 and abs(Mi) + abs(Mj) > 1e-3:
                    if n_labels == 1:   steps = [0.5]
                    elif n_labels == 2: steps = [0.0, 1.0]
                    elif n_labels == 3: steps = [0.0, 0.5, 1.0]
                    else:               steps = np.linspace(0, 1, n_labels)
    ...
    def _draw_loads(self, ax, actor_list, scale):
        if not self.inp or not hasattr(self.inp, "NodalForces"):
            return
        ...
        for _, row in loads.iterrows():
            ...
            if is_moment:
                ...
                self._draw_arrow(
                    ax, actor_list, p_start, p_end, color="magenta", style="double"
                )
            else:
                p_end = pt
                p_start = pt - direction * base_len
                self._draw_arrow(
                    ax, actor_list, p_start, p_end, color="green", style="simple"
                )
    ...
    def _draw_arrow(self, ax, lst, p0, p1, color, style="simple"):
        ...
        if L < 1e-9:
            return
        (l,) = ax.plot(
            [p0[0], p1[0]], [p0[1], p1[1]], [p0[2], p1[2]], color=color, lw=2
        )
        lst.append(l)
        v = vec / L
