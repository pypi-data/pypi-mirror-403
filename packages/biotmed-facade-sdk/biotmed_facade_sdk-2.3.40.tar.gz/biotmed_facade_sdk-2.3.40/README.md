...
```bash
pip install biotmed_facade_sdk
```
...
configuration = biotmed_facade_sdk.Configuration(
...
# Create ApiClient
with biotmed_facade_sdk.ApiClient(configuration) as api_client:
    # Create API instances
    api = biotmed_facade_sdk.YourAPIClass(api_client)  # Replace with your actual API class name
    # Use the API...
```
...
create_request = biotmed_facade_sdk.CreateResourceRequest(  # Replace with your actual request model name
    _name="My Resource",
    _ownerOrganization=biotmed_facade_sdk.ReferenceAttributeUUIDIdRequest(
...
```python
create_request = biotmed_facade_sdk.CreateResourceRequest.model_validate({  # Replace with your actual request model name
    "_name": "My Resource",
    "_ownerOrganization": {
...
```python
from biotmed_facade_sdk import SearchRequestV2, FilterV2, Order
...
```python
from biotmed_facade_sdk import ApiException
...
try:
    ...
    if isinstance(error_response, biotmed_facade_sdk.ErrorResponse):
...
with biotmed_facade_sdk.ApiClient(configuration) as api_client:
    ...
    # Search example
    try:
        search_request = biotmed_facade_sdk.SearchRequestV2(
            filter={
                "_name": biotmed_facade_sdk.FilterV2(like="test")
            },
            limit=10,
            page=0
        )
