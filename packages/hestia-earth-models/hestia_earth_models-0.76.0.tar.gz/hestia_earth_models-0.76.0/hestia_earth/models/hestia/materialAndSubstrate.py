...
def _input(term_id: str, value: float, stats: dict) -> dict:
    node = _new_input(term=term_id + _ID_SUFFIX, model=MODEL, value=value)
    return node | stats
...
def _should_run_input(cycle: dict, input_node: dict) -> bool:
    ...
    term_id = term.get("@id") + (_ID_SUFFIX if has_depreciated_term else "")

    should_run = all([has_depreciated_term, has_valid_value, has_lifespan])
