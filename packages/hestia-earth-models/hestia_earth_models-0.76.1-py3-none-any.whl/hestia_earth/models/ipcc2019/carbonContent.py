...
LOOKUPS = ["C_CONTENT_AG_CROP_RESIDUE", "C_CONTENT_BG_CROP_RESIDUE"]
...
def _multiple_product_values(crop: dict, residue_id: str):
    ...
    return (
        (value * dm / 100 * yield_dm * (ratio or 1), c_content)
...
def _run_multiple_products(crop_residue_products: list, products: list):
    def run_product(product: dict):
        ...
        value = (
            sum([value * c_content for value, c_content in values]) / total
            if total > 0
            else None
        )
        prop = (
