...
def _emission(term_id: str, kwargs) -> dict:
    ...
    return emission | {"methodTier": TIER, **{k: kwargs.get(k) for k in other_keys}}


def _get_fuel_category(
    biomass_category: BiomassCategory, eco_climate_zone: EcoClimateZone
) -> FuelCategory:
    """
    ...
    """
    return _BIOMASS_CATEGORY_TO_FUEL_CATEGORY.get(biomass_category, {}).get(
        eco_climate_zone
    )
...
def _calc_burnt_fuel(
    area_converted: npt.NDArray, fuel_factor: npt.NDArray, frac_burnt: npt.NDArray
) -> npt.NDArray:
    """
    ...
    def accumulate_fuel_burnt(
        ...
        update_dict = (
            ...
            if area_converted == 0
                ...
                + _calc_burnt_fuel(area_converted, fuel_factor, frac_burnt)
            }
        )
