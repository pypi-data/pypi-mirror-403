...
def list_members(
    ...
    try:
        with httpx.Client() as client:
            response = client.get(
                f"{config.core_url}/orgs/{org_id}/members",
                headers={"Authorization": f"Bearer {tokens.access_token}"},
                timeout=10,
            )
    ...
    try:
        with httpx.Client() as client:
            response = client.patch(
                f"{config.core_url}/orgs/{org_id}/members/{user_id}",
                headers={"Authorization": f"Bearer {tokens.access_token}"},
                json={"role": role},
                timeout=10,
            )
...
def my_membership(
    ...
    try:
        with httpx.Client() as client:
            response = client.get(
                f"{config.core_url}/orgs/{org_id}/members/me",
                headers={"Authorization": f"Bearer {tokens.access_token}"},
                timeout=10,
            )
...
def create_invite(
    ...
    try:
        with httpx.Client() as client:
            if email:
                response = client.post(
                    f"{config.core_url}/orgs/{org_id}/invites/email",
                    headers={"Authorization": f"Bearer {tokens.access_token}"},
                ...
                response = client.post(
                    f"{config.core_url}/orgs/{org_id}/invites/link",
                    headers={"Authorization": f"Bearer {tokens.access_token}"},
                    json={
                        "role": role,
...
def list_invites(
    ...
    try:
        url = f"{config.core_url}/orgs/{org_id}/invites"
        if pending:
            url += "?status=pending"
...
def cancel_invite(
    ...
    try:
        with httpx.Client() as client:
            response = client.delete(
                f"{config.core_url}/orgs/{org_id}/invites/{invite_id}",
                headers={"Authorization": f"Bearer {tokens.access_token}"},
                timeout=10,
            )
...
def accept_invite(
    ...
    try:
        with httpx.Client() as client:
            response = client.post(
                f"{config.core_url}/invites/{token}/accept",
                headers={"Authorization": f"Bearer {tokens.access_token}"},
                timeout=10,
            )
