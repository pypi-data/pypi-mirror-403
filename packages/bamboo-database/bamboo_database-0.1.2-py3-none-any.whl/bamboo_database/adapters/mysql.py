...
class MySQLAdapter(DatabaseAdapter):
    ...
    def execute_script(self, sql: str) -> None:
        """Execute multiple SQL statements (a script).

        Uses multi=True to allow multiple statements. The iterator
        must be consumed to execute all statements.
        """
        self._ensure_connected()
        # multi=True returns an iterator that must be consumed
        for _ in self._cursor.execute(sql, multi=True):
            pass
    ...
    def placeholder(self) -> str:
        """Return the placeholder style for MySQL (%s)."""
        return "%s"
