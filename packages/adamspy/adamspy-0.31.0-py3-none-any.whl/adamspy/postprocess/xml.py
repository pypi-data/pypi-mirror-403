...
def _process_data_node(data_node, reqs_to_return: dict, time: list, req_ids: dict, t_max: float, t_min: float):
    for step_node in data_node:
        ...
        if 'time' in step_node.attrib:
            t_step = float(step_node.attrib['time'])
        elif step_data[1].strip():
            t_step = float(step_data[1])
        else:
            continue
...
def shrink_results(result_file, reqs_to_keep=None, t_min=None, t_max=None, new_result_file=None, in_place=False):
    ...
    root_ans = et.SubElement(root_res, 'Analysis', name=input_ans_node.attrib['name'], executionDate=input_ans_node.attrib['executionDate'],
                             Solver=input_ans_node.attrib['Solver'], script=input_ans_node.attrib['script'])
        ...
        if reqs is None or entity.attrib['name'] in reqs:
            ...
            for comp in entity:
                ...
                if reqs is None or req_comp in reqs[request]:
                    ...
                    # Create an entry in the `units` and `req_ids` dictionaries
                    units[request][req_comp] = comp.attrib.get('plotLabel', '')
