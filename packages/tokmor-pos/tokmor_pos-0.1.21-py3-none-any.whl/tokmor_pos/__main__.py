...
def main(argv: list[str] | None = None) -> int:
    ...
    p = argparse.ArgumentParser(prog="tokmor-pos", description="TokMor POS = ParSon axes (P/A/R/S/O/N) + derived EAR hint (blank allowed)")
    sub = p.add_subparsers(dest="cmd", required=True)

    p_ear = sub.add_parser("ear", help="TokMor POS = EAR hint (E/A/R + blank allowed)")
    p_ear.add_argument("--lang", required=True)
    p_ear.add_argument("--text", required=True)
    p_ear.add_argument("--e-min-conf", dest="e_min_conf", type=float, default=0.25, help="reserved (currently unused)")
