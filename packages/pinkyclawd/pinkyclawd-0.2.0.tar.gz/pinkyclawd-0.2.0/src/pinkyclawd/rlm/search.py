...
from pinkyclawd.config.storage import ContextBlock, get_storage
...
class SemanticSearcher:
    ...
    def __init__(self) -> None:
        ...
        self._similarity_threshold = SEMANTIC_SIMILARITY_THRESHOLD
    ...
    async def search(
        ...
        if not config.rlm.semantic_search.get("enabled", False):
            logger.debug("Semantic search disabled, using keyword search")
        ...
        try:
            return await self._semantic_search(query, blocks, limit)
    ...
    async def _semantic_search(
        ...
        from pinkyclawd.rlm.embedding import cosine_similarity, compute_content_hash
        ...
        # Get or generate embeddings for blocks
        results = []

        for block in blocks:
            # Create searchable content from block
            content = f"{block.task_description}\n{block.summary}\n{block.content}"
            content_hash = compute_content_hash(content)

            # Check for existing embedding
            embedding_data = storage.get_embedding(block.id)

            if embedding_data and embedding_data[2] == content_hash:
                # Use cached embedding
                block_embedding = embedding_data[0]
            else:
                # Generate new embedding
                block_embedding = await embedding_manager.embed(content)

                # Store for future use
                storage.save_embedding(
                    block_id=block.id,
                    vector=block_embedding,
                    model=embedding_manager.provider.model,
                    dimensions=len(block_embedding),
            ...
            if similarity >= self._similarity_threshold:
                results.append(
                    SearchResult(
                        ...
                        score=similarity,
                        match_type="semantic",
                        highlights=[],  # Could extract key phrases later
                    )
                )
    ...
    def search_sync(
        ...
        """
        Synchronous semantic search - falls back to keyword search.

        For async contexts, use search() instead.
        """
        return self._keyword_searcher.search(query, blocks, limit)
...
class ContextSearcher:
    ...
    def search(
        ...
        blocks = self._storage.get_context_blocks(session_id=session_id, limit=MAX_CONTEXT_BLOCKS)
        ...
        # Hybrid mode: combine keyword results with recency boost
        return self._hybrid_search(query, blocks, keyword_results, limit)
    ...
    def _hybrid_search(
        ...
        """
        Perform hybrid search combining keyword matching with recency.

        Synchronous version - combines:
        - Keyword relevance score
        - Recency boost for newer blocks
        - Task description bonus

        For semantic-enhanced hybrid search, use search_async().
        """
        ...
        weights = config.rlm.semantic_search.get(
            "hybrid_weights",
            {"keyword": 0.6, "recency": 0.3, "task_match": 0.1},
        )

        now = datetime.now()
        max_age_days = 30  # Blocks older than 30 days get no recency boost
        ...
        for result in keyword_results:
            ...
            # Base keyword score
            keyword_score = result.score * weights.get("keyword", 0.6)

            # Recency score (1.0 for recent, 0.0 for old)
            age = (now - block.created_at).days
            ...
            if block.task_description:
                ...
                task_lower = block.task_description.lower()
                if any(term in task_lower for term in query_lower.split()):
                    task_bonus = weights.get("task_match", 0.1)

            total_score = keyword_score + recency_score + task_bonus

            result_map[block_id] = SearchResult(
                block=block,
                score=total_score,
                match_type="hybrid",
        ...
        # Add blocks not found by keyword search but are very recent
        for block in blocks:
            if block.id not in result_map:
                age = (now - block.created_at).days
                if age <= 1:  # Last 24 hours
                    recency_score = weights.get("recency", 0.3) * 0.5
                    result_map[block.id] = SearchResult(
                        block=block,
                        score=recency_score,
                        match_type="hybrid",
                        highlights=[],
    ...
    async def search_semantic(
        ...
        Returns:
            Search results sorted by semantic similarity
        """
        blocks = self._storage.get_context_blocks(session_id=session_id, limit=MAX_CONTEXT_BLOCKS)
        ...
        return await self._semantic_searcher.search(query, blocks, limit)
    ...
    def get_by_task(self, task_id: str) -> ContextBlock | None:
        ...
        blocks = self._storage.get_context_blocks(limit=MAX_CONTEXT_BLOCKS)
        for block in blocks:
...
def get_searcher() -> ContextSearcher:
    ...
    Prefer using pinkyclawd.core.get_searcher() for consistency.
    """
