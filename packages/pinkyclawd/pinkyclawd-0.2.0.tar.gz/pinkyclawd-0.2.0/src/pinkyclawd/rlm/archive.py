...
from pinkyclawd.constants import POST_ARCHIVAL_TARGET_RATIO
...
class ContextArchiver:
    ...
    Now includes embedding generation for semantic search support
    and RLM-Graph ingestion for structural context retrieval.
    """
    ...
    def _get_graph_ingester(self):
        """Lazy load graph ingester."""
        if self._graph_ingester is None:
            try:
                from pinkyclawd.rlm.graph import get_graph_ingester
                self._graph_ingester = get_graph_ingester()
            except ImportError:
                logger.warning("Graph module not available")
        return self._graph_ingester
    ...
    def _archive_impl(
    ...
    ) -> tuple[ArchiveResult, ContextBlock | None]:
        """
        Core archival implementation (sync).
        ...
        Returns:
            Tuple of (ArchiveResult, ContextBlock or None)
        """
        if not messages:
            return (
                ArchiveResult(
        ...
        try:
            ...
            self._schedule_graph_ingestion(block, generate_embedding)
    ...
    def archive_messages(
        ...
        """
        Archive a set of messages (sync version).
        ...
        """
        result, block = self._archive_impl(
            session_id=session_id,
            messages=messages,
            task_id=task_id,
            task_description=task_description,
        ...
        # Schedule background embedding generation if enabled
        if result.success and block and generate_embedding:
    ...
    async def archive_messages_async(
        ...
        Args:
            ...
            task_description: Task description
            generate_embedding: Whether to generate embedding
            await_embeddings: Whether to await embedding generation (vs background)
    ...
    def _schedule_graph_ingestion(
        self,
        block: ContextBlock,
        generate_embeddings: bool = True,
    ) -> None:
        """Schedule graph ingestion for a context block."""
        config = get_config()
        if not getattr(config.rlm, "graph_enabled", True):
            return

        graph_ingester = self._get_graph_ingester()
        if not graph_ingester:
            return

        try:
            # Ingest synchronously (graph operations are fast)
            graph_ingester.ingest_context_block(block, generate_embeddings)
            logger.debug(f"Ingested block {block.id} into knowledge graph")
        except Exception as e:
            logger.warning(f"Failed to ingest block {block.id} into graph: {e}")
    ...
    def archive_oldest(
        ...
        if target_tokens is None:
            target_tokens = int(state.model_limit * POST_ARCHIVAL_TARGET_RATIO)
...
def get_archiver() -> ContextArchiver:
    ...
    Prefer using pinkyclawd.core.get_archiver() for consistency.
    """
