...
class ReturnCodeError(Exception):
    def __init__(self, message: str, code: int, stderr: str):
    ...
    def resolve_files(self, remote_path: str) -> Iterator[FileSpec]:
        for spec in self.files:
            ...
            if "*" in spec.remote:
                ...
                remote_pre, remote_post = (
                    spec.remote[:remote_index],
                    spec.remote[remote_index + 1 :],
                )
                local_pre, local_post = (
                    spec.local[:local_index],
                    spec.local[local_index + 1 :],
                )
        ...
        for filespec in self.dep.files:
            try:
                ...
                if "*" in filespec.remote:
                    ...
                    remote_pre, remote_post = (
                        filespec.remote[:remote_index],
                        filespec.remote[remote_index + 1 :],
                    )
                    local_pre, local_post = (
                        filespec.local[:local_index],
                        filespec.local[local_index + 1 :],
                    )
