"""
Configuration system for Haplophaser.
...
class OutputConfig(BaseModel):
    ...
    directory: Path = Field(default=Path("haplophaser_output"), description="Output directory")
    prefix: str = Field(default="haplophaser", description="Output file prefix")
...
class HaplophaserConfig(BaseModel):
    """Main configuration for Haplophaser analysis.
    ...
    Examples
    --------
    >>> config = HaplophaserConfig()  # Use defaults
    >>> config = HaplophaserConfig(window=WindowConfig(size=50000))
    ...
    def with_updates(self, **kwargs: Any) -> HaplophaserConfig:
        ...
        **kwargs
        ...
        return HaplophaserConfig.model_validate(current)
...
def load_config(path: Path | str) -> HaplophaserConfig:
    ...
    return HaplophaserConfig.model_validate(data)
...
def load_config_with_overrides(
    ...
    ... )
    """
    config = load_config(path) if path is not None else HaplophaserConfig()
...
def get_default_config() -> HaplophaserConfig:
    ...
    """
    return HaplophaserConfig()


# Example configuration template as YAML string
CONFIG_TEMPLATE = """\
# Haplophaser Configuration File
