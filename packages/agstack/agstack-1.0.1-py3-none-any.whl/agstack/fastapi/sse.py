...
try:
    from uvicorn.main import Server  # type: ignore[reportPrivateImportUsage]
...
Content = Union[str, bytes, dict, ServerSentEvent]  # noqa: UP007
SyncContentStream = Iterator[Content]
AsyncContentStream = AsyncIterable[Content]
ContentStream = Union[AsyncContentStream, SyncContentStream]  # noqa: UP007
    ...
    async def _ping(self, send: Send) -> None:
        ...
        while self.active:
            ...
            if self.ping_message_factory:
                assert isinstance(self.ping_message_factory, Callable)  # https://github.com/python/mypy/issues/6864
            ping = (
                ServerSentEvent(comment=f"ping - {datetime.now(UTC)}").encode()
                if self.ping_message_factory is None
                else ensure_bytes(self.ping_message_factory(), self.sep)
            )
            logger.debug(f"sse ping: {ping}")
            async with self._send_lock:
                if self.active:
