...
#define RING_BUFFER_SIZE     8192 /* bytes */
...
static uint16_t alloc_group(LHALH2Decoder *decoder)
{
	uint16_t result;
...
static void free_group(LHALH2Decoder *decoder, uint16_t group)
{
	--decoder->num_groups;
...
static void init_groups(LHALH2Decoder *decoder)
{
	unsigned int i;
...
static void init_tree(LHALH2Decoder *decoder)
{
...
static void fill_offset_range(LHALH2Decoder *decoder, uint8_t code,
...
static void init_offset_table(LHALH2Decoder *decoder)
{
...
static void init_ring_buffer(LHALH2Decoder *decoder)
{
	memset(decoder->ringbuf, ' ', RING_BUFFER_SIZE);
	decoder->ringbuf_pos = 0;
}

static int lha_lh2_init(void *data, LHADecoderCallback callback,
                        void *callback_data)
{
	LHALH2Decoder *decoder = data;
...
static uint16_t make_group_leader(LHALH2Decoder *decoder,
                                  uint16_t node_index)
{
	Node *node, *leader;
	uint16_t group;
...
static void increment_node_freq(LHALH2Decoder *decoder, uint16_t node_index)
{
	Node *node, *other;
...
static void reconstruct_tree(LHALH2Decoder *decoder)
{
	Node *leaf;
	unsigned int child;
	unsigned int freq;
...
static void increment_for_code(LHALH2Decoder *decoder, uint16_t code)
{
	uint16_t node_index;
...
static int read_code(LHALH2Decoder *decoder, uint16_t *result)
{
...
static int read_offset(LHALH2Decoder *decoder, unsigned int *result)
{
	unsigned int offset;
	int future, offset2;
	...
	offset2 = read_bits(&decoder->bit_stream_reader, 7);

	if (offset2 < 0) {
		return 0;
	}

	*result = (offset << 7) | (unsigned int) offset2;

	return 1;
}

static void output_byte(LHALH2Decoder *decoder, uint8_t *buf,
...
static size_t lha_lh2_read(void *data, uint8_t *buf)
{
	LHALH2Decoder *decoder = data;
	size_t result;
	uint16_t code;
...
const LHADecoderType lha_lh2_decoder = {
	lha_lh2_init,
	NULL,
	lha_lh2_read,
	sizeof(LHALH2Decoder),
	OUTPUT_BUFFER_SIZE,
	RING_BUFFER_SIZE
};
