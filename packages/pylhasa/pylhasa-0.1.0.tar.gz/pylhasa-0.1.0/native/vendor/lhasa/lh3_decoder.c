...
static uint16_t alloc_group(LHALH3Decoder *decoder)
{
	uint16_t result;
...
static void free_group(LHALH3Decoder *decoder, uint16_t group)
{
	--decoder->num_groups;
...
static void init_groups(LHALH3Decoder *decoder)
{
	unsigned int i;
...
static void init_tree(LHALH3Decoder *decoder)
{
...
static void fill_offset_range(LHALH3Decoder *decoder, uint8_t code,
...
static void init_offset_table(LHALH3Decoder *decoder)
{
...
static void init_ring_buffer(LHALH3Decoder *decoder)
{
	memset(decoder->ringbuf, ' ', RING_BUFFER_SIZE);
	decoder->ringbuf_pos = 0;
}

static int lha_lh3_init(void *data, LHADecoderCallback callback,
                        void *callback_data)
{
	LHALH3Decoder *decoder = data;
...
static uint16_t make_group_leader(LHALH3Decoder *decoder,
                                  uint16_t node_index)
{
	Node *node, *leader;
	uint16_t group;
...
static void increment_node_freq(LHALH3Decoder *decoder, uint16_t node_index)
{
	Node *node, *other;
...
static void reconstruct_tree(LHALH3Decoder *decoder)
{
	Node *leaf;
	unsigned int child;
	unsigned int freq;
...
static void increment_for_code(LHALH3Decoder *decoder, uint16_t code)
{
	uint16_t node_index;
...
static int read_code(LHALH3Decoder *decoder, uint16_t *result)
{
...
static int read_offset(LHALH3Decoder *decoder, unsigned int *result)
{
	unsigned int offset;
	int future, offset2;
...
static void output_byte(LHALH3Decoder *decoder, uint8_t *buf,
...
static size_t lha_lh3_read(void *data, uint8_t *buf)
{
	LHALH3Decoder *decoder = data;
	size_t result;
	uint16_t code;
...
const LHADecoderType lha_lh3_decoder = {
	lha_lh3_init,
	NULL,
	lha_lh3_read,
	sizeof(LHALH3Decoder),
	OUTPUT_BUFFER_SIZE,
	RING_BUFFER_SIZE
};
