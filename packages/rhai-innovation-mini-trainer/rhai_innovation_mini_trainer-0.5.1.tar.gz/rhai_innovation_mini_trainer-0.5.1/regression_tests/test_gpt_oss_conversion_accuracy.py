...
Adapted for mini_trainer from InstructLab training test.
"""
...
def test_conversion_accuracy():
    ...
    try:
        ...
        print("📥 Step 1: Loading ORIGINAL quantized model (ground truth)...")
        ...
        # Step 4: Apply our CORRECT conversion using mini_trainer utilities
        print("\\n🔄 Step 4: Applying Mini Trainer GPT-OSS quantization conversion...")
        ...
        output_dir = Path("test_converted_model_mini_trainer")
        ...
        try:
            ...
            # Update with quantization config using mini_trainer function
            update_config_for_quantized_format(config_path)
            print("   ✅ Saved config with Mini Trainer GPT-OSS quantization settings")
        ...
        try:
            ...
            print(f"\\n   📊 Mini Trainer converted model has {len(converted_tensors)} tensors total")
            
            # Show ALL converted expert parameters
            print("\\n   🔍 ALL Mini Trainer converted expert parameters:")
            conv_expert_params = {}
            ...
            print(f"\\n   📊 Mini Trainer converted model has quantized expert params in layers: {sorted(conv_layers_with_blocks)}")
            ...
            print(f"     Mini Trainer converted layers: {sorted(conv_layers)}")
            ...
            if orig_layers == conv_layers:
                print("     ✅ Layer sets match perfectly!")
            else:
                print("     ❌ Layer sets don't match - this explains the parameter mismatch!")
            ...
            print("\\n   📈 Mini Trainer Safetensors Comparison Summary:")
            ...
            print("\\n🧪 Step 7: Testing Mini Trainer converted model inference...")
            try:
                ...
                print("   ✅ Mini Trainer converted model loads successfully")
                test_inference(converted_model, tokenizer, "MINI_TRAINER_CONVERTED")
                del converted_model
                
            except Exception as load_e:
                print(f"   ❌ Failed to load Mini Trainer converted model for inference: {load_e}")
        ...
        print("\\n🧹 Mini Trainer GPT-OSS test completed. Check results above for conversion accuracy.")
        
    except Exception as e:
        print(f"❌ Mini Trainer GPT-OSS test failed: {e}")
        import traceback
        traceback.print_exc()
