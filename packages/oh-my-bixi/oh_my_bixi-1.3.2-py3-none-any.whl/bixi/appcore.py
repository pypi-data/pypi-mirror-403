...
class Workspace:
    ...
    To reload a workspace:
        Call `Workspace.restore_workspace(...)` to load metadata

    Note: 
        1. You are allowed to change prefix and name across different runs. Even if you change workspace
            location, the saved prefix and name does not change.
        2. Only id is guaranteed to be unique and persistent.
    """

    # ==== Mandatory Fields ========================================================
    prefix: str
    name: str
    # Be treated as a new workspace if None. private fields will be generated in __post_init__
    ...
    def __post_init__(self):
        ...
        if self.id is None:  # Be treated as a new workspace if None. private fields will be generated in __post_init__
        ...
        # Infer location dependent attributes around metadata filepath.
        # Note that these attributes are generated on-the-fly to adapt workspace from changing location
        ws_dirname = f"{self.id[:16]}_{self.name}"  # truncate the id to avoid too long directory name
        ws_rootpath = os.path.abspath(os.path.join(self.prefix, ws_dirname))
        metadata_filepath = os.path.join(ws_rootpath, self.WORKSPACE_METADATA_FILENAME)

        self._metadata_filepath = os.path.abspath(metadata_filepath)
        self._rootpath = os.path.abspath(ws_rootpath)
    ...
    # === Workspace Querying and Restoration ============================================
    @staticmethod
    def query_workspace_metadata(search_root: str = None, **requirements) -> Dict[str, dict]:
        ...
        Returns:
            A dict of workspace metadata dictionaries that satisfied the requirements.
            Key: workspace metadata file path
            Value: workspace metadata dictionary
        """
    ...
    @classmethod
    def restore_workspace(cls, ws_rootpath: str) -> 'Workspace':
        """Restore a workspace from the given root path."""
        metadata_filepath = os.path.join(ws_rootpath, Workspace.WORKSPACE_METADATA_FILENAME)
        if not os.path.isfile(metadata_filepath):
            raise FileNotFoundError(f"Workspace metadata file not found at {metadata_filepath}")

        with open(metadata_filepath, 'r') as f:
            metadata = yaml.safe_load(f)

        ws = cls(prefix=metadata['prefix'], name=metadata['name'], id=metadata['id'])
        ws._metadata = metadata  # attach loaded metadata (including protected keys)
...
class WorkspaceAppMixin(object):
    ...
    def workspace_init(self, *,
        ...
        This method is designed to be runnable in multi-process distributed setting AS LONG AS the user ensures
        the main process (rank 0) goes first before other processes.

        Args:
            rank: distributed rank for multi-processing usage. Default is None (no rank info).
            ws_dirpath: optional directory path for the workspace if you want to reuse an existing workspace.
                If provided, it will resuse the workspace at this path if possible. If both this and 
                is_create_workspace are provided:
                    1) If two workspace ID are same, main process will ignore this parameter and create workspace.
                    2) If two workspace ID are different, main process will raise error.
                Default is None.
                This can be useful when you want to setup workspace in non-primary processes in distributed
                settings. 
        ...
        Examples:
            ...
            with rank_zero_first(rank):
                app.workspace_init(rank=rank, ws_dirpath=shared_ws_path)
        """
        ...
        if ws_dirpath:
            ws_resume = Workspace.restore_workspace(ws_dirpath)
            
            if ws_resume.id != self.ws.id:
                # shall resume from existing workspace
                if is_create_workspace and is_main_process:
                    raise ValueError(f"Cannot create a new workspace (ID: {self.ws.id}) when resuming from workspace (ID: {ws_resume.id})")
                
                if self.ws.id != ws_resume.id:
                    self.ws = ws_resume
    ...
    def workspace_app_launch(
            ...
            ws_dirpath: Annotated[Optional[str], typer.Option(
                help="Path to an existing workspace directory to restore.",
            )] = None,
            ws_searchpath: Annotated[Optional[str], typer.Option(
                help="Root directory to search for workspace (used with name/prefix).",
            )] = None,
        ...
        1. If ws_dirpath is provided:
           - Use ws_dirpath to restore the workspace
           - If resume='new', raise error (cannot create new experiment with ws_dirpath)
        2. If ws_dirpath is not provided but name is provided:
           ...
           - Workspace search uses ws_searchpath (defaults to current working directory)
        3. If neither ws_dirpath nor name is provided:
        ...
        Raises:
            ValueError: If neither ws_dirpath nor name is provided
        ...
        if ws_dirpath is not None:
            # Case 1: Restore workspace by path
            if resume_policy == cls.ResumePolicy.NEW:
                raise RuntimeError("Cannot create new experiment when using ws_dirpath. Use 'name' parameter instead.")

            ws_dirpath = os.path.abspath(ws_dirpath)
            logging.info(f"Restoring workspace from path: {ws_dirpath}")
            ws = Workspace.restore_workspace(ws_dirpath)
            ...
            if resume_policy == cls.ResumePolicy.NEW:
                ...
                if len(matches) > 1:
                    raise RuntimeError(
                        f"Failed to resume workspace with name '{name}' and prefix '{prefix}': "
                        f"found {len(matches)} matches. Pass ws_dirpath to disambiguate."
                    )
                metadata_path = next(iter(matches.keys()))
                workspace_dirpath = os.path.dirname(metadata_path)
                logging.info(
                    f"Resuming workspace with name: {name}, prefix: {prefix} from {workspace_dirpath}"
                )
                ws = Workspace.restore_workspace(workspace_dirpath)
                ...
                if len(matches) == 0:
                    ...
                    workspace_dirpath = os.path.dirname(metadata_path)
                    logging.info(
                        f"Auto-resume found a unique workspace; resuming name: {name}, "
                        f"prefix: {prefix} from {workspace_dirpath}"
                    )
                    ws = Workspace.restore_workspace(workspace_dirpath)
                    logging.info(f"Successfully resumed workspace: {ws.rootpath}")
                else:
                    raise RuntimeError(
                        ...
                        "Pass ws_dirpath to disambiguate or use resume='new' to force creating a new workspace."
                    )
            ...
            # Case 3: Neither ws_dirpath nor name provided
            raise ValueError("Either 'ws_dirpath' or 'name' must be provided to identify the workspace")
