...
licCheckSleepTime = os.environ.get("PATME_LIC_CHECK_SLEEPTIME", 5)
numLicCheckRetries = os.environ.get("PATME_NUM_LIC_CHECKS", 12)
...
class FECaller(ResultLogFileChecker, ClusterCaller):
    ...
    def __init__(self, **kwargs):
        ...
        self._useNumberOfCores = femUsedCores
    ...
    def returnWhenLicenseAvailable(self, sleeptime=licCheckSleepTime):
        """doc"""
        # wait one minute
        numberOfTries = numLicCheckRetries
...
class AbaqusPythonCaller(AbaqusCaller):
    ...
    def __init__(self, **kwargs):
        ...
        useGui = kwargs.pop("useGui", False)  # only works if cae==True
        # eclipse adds some env variabels that do not work with abaqus
        os.environ.pop("PYTHONIOENCODING", None)
        ...
        if cae:
            ...
            runKeyword = "script" if useGui else "noGUI"
            callParams += ["cae", f"{runKeyword}=" + self.pythonScriptPath, "--"]
        else:
