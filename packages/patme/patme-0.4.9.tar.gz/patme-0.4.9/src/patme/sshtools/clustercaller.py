...
class ClusterCaller:
    ...
    def runRemote(self, copyFilesLocalToHost=True, jobName="job", **kwargs):
        ...
        if "wsl" not in self.clusterName:
            ...
            if self.docker_container is not None:
                ...
                remoteCmds[0] = (
                    f"docker run -w {docker_mnt_dir} -v {remoteRunDir}:{docker_mnt_dir} {self.docker_container} "
                )
        ...
        if "cara" in self.clusterName:
            ...
            wsl_cmd = ["wsl", "--cd", remoteRunDirAsWinPath] + command.split(" ")
            p = subprocess.run(wsl_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    def _transferInputToCluster(self, remoteRunDir, **kwargs):
        ...
        if "cara" in self.clusterName:
            ...
            cmd = ["wsl", "--cd", remoteRunDir, "tar", "-xf", tarFileName, "-C", ".", ";", "rm", tarFileName]
            subprocess.run(cmd)
