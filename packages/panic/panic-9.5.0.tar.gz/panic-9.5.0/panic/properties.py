...
try:
    db = PyTango.Database()
    if db:
        tango_host = str(db.get_db_host())+':'+str(db.get_db_port())
except:
    print('Unable to determine tango host')
...
SORT_ORDER = ('Active', '_State', 'Priority', 'Time', 'Error')
...
INFO_REQUESTS = ['SETTINGS', 'STATE', 'STATUS', 'VALUES', 'SNAP']
...
PANIC_PROPERTIES = {
    ...
    'ForbiddenActions':
        [PyTango.DevVarStringArray,
         "List of OS commands that alarms are forbidden to execute.",
         []],
    'StartupDelay':
        [PyTango.DevLong,
...
ALARM_CYCLE = {
    ...
    'PollingPeriod':
        [PyTango.DevFloat,
         "Period in SECONDS to poll formulas (including event-based)."
...
DEVICE_CONFIG = {
    ...
    'UseEvents':
        [PyTango.DevBoolean,
         "Subscribe to events in formulas (not reactive).",
         [False]],
}
