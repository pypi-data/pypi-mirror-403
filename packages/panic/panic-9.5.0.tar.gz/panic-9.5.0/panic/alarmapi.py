...
class Alarm(object):
    ...
    def get_state(self, force=False):
        ...
        if force or self._state is None:
            ...
            # THIS CHECKS MAY TRIGGER DEADLOCK IN A DEVICE SERVER
            # If Acknowledge/Disabled Arrays are not polled
            # if force:
            #     try:
            #         self.get_acknowledged(force=True)
            #     except:
            #         traceback.print_exc()
            #     try:
            #         self.get_enabled(force=True)
            #     except:
            #         traceback.print_exc()
...
class AlarmAPI(fandango.SingletonMap):
    ...
    def parse_attributes(self, formula, replace=True):
        ...
        return sorted('%s/%s' % (t[0],t[1]) for t in attributes)
