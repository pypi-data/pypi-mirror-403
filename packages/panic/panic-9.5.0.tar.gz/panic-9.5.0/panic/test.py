...
def check_alarms_vs_archiving(alarms=None):
  if alarms is None:
      alarms = []
  import PyTangoArchiving
  api = panic.api()
  alarms = alarms or [a for a in api if fandango.check_device(api[a].device)]
  rd = PyTangoArchiving.Reader()
  archs = rd.get_attributes()
  models = dict((a,api[a].get_attribute(full=1).lower()) for a in alarms)
  alarms = [a for a in alarms if models[a] in archs]
  olds = {}
  ...
  for a in sorted(alarms):
    af = models[a]
    v = fandango.tango.read_attribute(af)
