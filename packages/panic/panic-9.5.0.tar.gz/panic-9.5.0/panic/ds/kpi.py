...
def get_panic_report(api=None, timeout=3000., tries=3,
    ...
    for s, ss in api.servers.items():
        ...
        try:
            ...
            print('unable to parse {}'.format(admin))
            continue
...
def get_device_timeout(device, timeout, tries=1, exported=None,
    ...
    if device not in exported:
        if trace:
            print('{} is not exported!'.format(device))
        return -1
        ...
        for i in range(tries):
            ...
            try:
                ...
                if trace:
                    print('{} failed!: {}'.format(device, fd.excepts.exc2str(e)))
                # traceback.print_exc()
                t = END_OF_TIME
                break
...
def get_all_timeouts(api=None, devfilter='*', timeout=3000.):
    ...
    print('testing {:d} devices on {}'.format(len(devs), api.tango_host))
    times, failed, down = {}, [], []
    for d in devs:
        t = get_device_timeout(d, timeout, tries=3)
        print(d, t)
        times[d] = t
        down.append(d) if t < 0 else None
        failed.append(d) if t == END_OF_TIME else None

    print('\n{:d} devices were not exported: {}'.format(len(down), ','.join(down)))
    print('{:d} devices failed: {}'.format(len(failed), ','.join(failed)))
...
def get_alarms_variables(api=None, alarmfilter='*'):
    ...
    for d, v in api.devices.items():
        ...
        print('{} : {} attributes in {} alarms'.format(d, ss, len(v.alarms)))
    return attrs
...
def get_devices_eval_times(api=None, devfilter='*', timeout=10000.):
    ...
    times = {}
    print('{:d} devices'.format(len(devs)))
    for d in devs:
        try:
            ...
            if not quiet:
                if isSequence(r):
                    ...
                    for i, t in enumerate(r):
                        print('{}: {}'.format(i, fd.pprint(t)))  # obj2str(t)))
                    print('')
                else:
                    r = dict(r.items()) if hasattr(r, 'items') else None
                    print(fd.pprint(r))
            else:
                print('')
