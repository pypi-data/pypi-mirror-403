...
class FilterStack(SortedDict):
    ...
    def __init__(self, filters=None):
        ...
        if filters:
            ...
            elif isMapping(filters) and isMapping(first(filters.values())):
...
class AlarmView(EventListener):
    ...
    def __init__(self, name='AlarmView', filters={}, scope='*', api=None,
        ...
        self.verbose = verbose if not isNumber(verbose) else (
            {verbose > 3: 'DEBUG', 4 > verbose > 1: 'INFO', 
             verbose <= 1: 'WARNING'}[True])
        ...
        self.api = first(list(self.apis.values()))
        ...
        self.info('%d sources : %s ...'
                  % (len(self.alarms), fn.log.pformat(self.alarms)[:80]))

        N = len(self.alarms)

        ## How often should the ordered list be renewed?
        if len(self.alarms) > 150 and refresh < 6.:
    ...
    def __test__(*args):
        ...
        if opts:
            opts = dict(o.split('=') if '=' in o else (o, True)
                        for o in opts)
            opts.update((o, fn.str2type(v)) for o, v in opts.items())
    ...
    def apply_filters(self, *args, **filters):
        ...
        * hierarchy bottom
        ...
        try:
            ...
            self.info('AlarmView[{}].apply_filters({})'.format(
                len(self.api), repr(filters)))

            self.filtered = [a.tag for a in
                             filters.apply(self.api.values(), 
                             verbose=0)]
    ...
    def sort(self, sortkey=None, as_text=False, filtered=True, keep=True):
        ...
        * failed
        """
        try:
            self.info('AlarmView.sort(%s,%s): %d alarms in db' % (
                sortkey, as_text, len(self.alarms)))
            ...
            if as_text:
                kw = fn.isMapping(as_text) and as_text or {}
    ...
    def get_alarm_as_text(self, alarm=None, cols=None,
        ...
        try:
            for i, r in enumerate(cols):
                if s.strip(): s += sep
                v = getattr(alarm, r)
                args = [v() if fn.isCallable(v) else v]
    ...
    def update_sources(self):
        ...
        news = [ft.get_fqdn_name(self.api.devices[d].get_model())
                for d in devs]  # discard single attributes

        for o in olds:
            if o not in news:
                self.warning(o +' no longer in ' + str(news))
                self.warning('Removing source %s' % o)
                try:
                    self.remove_source(o)
                except:
                    self.warning('Error removing source %s: %s' % (o, traceback.format_exc()))
    ...
    def add_source(self, alarm):
        ...
        if s:
            self.info('%s already sourced as %s' % (alarm, s))
            return None
        else:
            alarm = self.get_model(alarm)
            ...
            ta.setLogLevel('ERROR') #self.getLogLevel())
    ...
    def remove_source(self, alarm):
        s = self.get_source(alarm)
        self.warning('removing Alarm Source: (%s, %s)' % (alarm,s))
        ...
        try:
            ...
            if not error:
                ...
                array = dict((l.split(splitter)[0].split('=')[-1], l)
                             for l in rvalue)
                self.debug('%s.rvalue = %s' % (src, fn.log.pformat(array)))
            else:
            ...
            if not len(alarms):
                self.debug('event with no alarms cant be processed!')
                return
            ...
            for a in alarms:
                ...
                if error:
                    ...
                    try:
                        ...
                        # %(av.tag,fn.time2str(av.active or 0)))
                    except:
            ...
            self.debug('event_hook() done in %.2e ms '
