...
def setup_logging(
    ...
    if log_file is None:
        try:
            ...
            log_file = str(log_dir / 'app.logs')
        except Exception as e:
            fallback_log_dir = Path('/tmp/logs')
            fallback_log_dir.mkdir(parents=True, exist_ok=True)
            log_file = str(fallback_log_dir / 'app.logs')
    ...
    logging.info(f"日志文件配置: 文件路径={log_file}, 最大字节={max_bytes}, 备份_计数={backup_count}")

    return log_file


__all__ = ['setup_logging', 'request_context', 'ContextFilter', 'APSchedulerFilter', 'JsonFormatter',
           'PlainTextFormatter']
