...
from nancal_langchain_base.error.exceptions import classify_error
from nancal_langchain_base.file.file import infer_file_category, File, FileOps
from nancal_langchain_base.messages.client import PromptBlock, PromptBlockContent, UploadFileBlockDetail, \
    ClientMessageContent, \
    ClientMessage, QueryDetail
from nancal_langchain_base.messages.server import ServerMessage, ServerMessageContent, MESSAGE_TYPE_ANSWER, \
    ToolRequestDetail, \
    MESSAGE_TYPE_TOOL_REQUEST, MESSAGE_TYPE_TOOL_RESPONSE, ToolResponseDetail, MESSAGE_TYPE_MESSAGE_START, \
    MessageStartDetail, MessageEndDetail, MESSAGE_END_CODE_SUCCESS, TokenCost, MESSAGE_TYPE_MESSAGE_END
...
def to_client_message(d: Dict[str, Any]) -> Tuple[ClientMessage, str]:
    prompt_list = d.get("content", {}).get("query", {}).get("prompt", []) or d.get('chat_messages')
