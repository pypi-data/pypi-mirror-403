"""
Step-by-step gym prompt for solving puzzles without traceback.
Used for gym mode where the model receives observations and takes actions one step at a time.
"""
...
def get_prompt(puzzle_data: Dict) -> Dict:
    """
    Generate the complete prompt dict for step-by-step gym mode (no traceback).
