...
impl Track {
    ...
    pub fn reduced(mut self) -> Self {
        self.points
            .dedup_by(|a, b| a.x_pos == b.x_pos && a.y_pos == b.y_pos && a.z_pos == b.z_pos);
        self
    }
}
...
impl FloatTrack {
    ...
    pub fn to_track(&self, extent: Extent3D, shape: Shape) -> Track {
        Track {
            points: self
                .points
                .iter()
                .map(|p| TrackPoint {
                    ...
                    y_pos: (((p.y_pos - extent.y0) / (extent.y1 - extent.y0) * shape.n_y as f32)
                        .round()) as i32,
                    z_pos: (((p.z_pos - extent.z0) / (extent.z1 - extent.z0) * shape.n_z as f32)
                        .round()) as i32,
        ...
        for end_idx in split_indices_array.iter().map(|&x| x as usize) {
            ...
            for m in start_idx..end_idx {
                ...
                new_track.points.push(FloatTrackPoint {
                    ...
                    z_pos: tracks_array[[m, 2]],
                    intensity: tracks_array[[m, 3]],
                });
            }
