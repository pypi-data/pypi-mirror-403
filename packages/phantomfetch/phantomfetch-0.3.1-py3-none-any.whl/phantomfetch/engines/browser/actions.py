...
async def execute_actions(
    ...
    Args:
        ...
        trace_context: Optional dictionary of attributes to add to spans (e.g. loop index)
    ...
    for action in actions:
        with tracer.start_as_current_span(
            ...
            # Inject propagated context (e.g., loop index)
            if trace_context:
                ...
                match action.action:
                    ...
                    case "loop":
                        if not action.selector or not action.actions:
                            ...
                            # Summary of child action types
                            child_types = list({a.action for a in action.actions})
                            span.set_attribute(
                                "phantomfetch.action.loop.child_types", str(child_types)
                            )
                            ...
                            for i, el in enumerate(elements):
                                ...
                                # Propagate loop index + existing context
                                ctx_update = trace_context.copy() if trace_context else {}
                                ctx_update["phantomfetch.action.loop_index"] = i
                                
                                sub_res = await execute_actions(
                                    el, action.actions, trace_context=ctx_update
                                )
                        ...
                        if condition_met:
                            if action.then_actions:
                                result.data = await execute_actions(
                                    ctx, action.then_actions, trace_context=trace_context
                                )
                        elif action.else_actions:
                            result.data = await execute_actions(
                                ctx, action.else_actions, trace_context=trace_context
                            )
                    ...
                    case "try":
                        ...
                        if not action.actions:
                            ...
                            try:
                                ...
                                sub_results = await execute_actions(
                                    ctx, action.actions, trace_context=trace_context
                                )
