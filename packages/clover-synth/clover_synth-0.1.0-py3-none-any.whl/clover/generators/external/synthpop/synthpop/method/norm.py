...
from ..method import Method, smooth
...
class NormMethod(Method):
    ...
    def fit(self, X_df, y_df):
        X_df, y_df = self.prepare_dfs(
            X_df=X_df, y_df=y_df, normalise_num_cols=True, one_hot_cat_cols=True
        )
        ...
        if self.proper:
            ...
            self.sigma = np.sqrt(
                np.sum(residuals**2) / np.random.chisquare(n_rows - n_cols)
            )
            # NOTE: I don't like the use of inv()
            V = inv(np.matmul(X.T, X))
            self.norm.coef_ += np.matmul(
                cholesky((V + V.T) / 2), np.random.normal(scale=self.sigma, size=n_cols)
            )
        else:
    ...
    def predict(self, X_test_df):
        X_test_df, _ = self.prepare_dfs(
            X_df=X_test_df, normalise_num_cols=True, one_hot_cat_cols=True, fit=False
        )
        n_test_rows = len(X_test_df)

        X_test = X_test_df.to_numpy()
        y_pred = self.norm.predict(X_test) + np.random.normal(
