...
from ..method import Method, proper, smooth
...
class CARTMethod(Method):
    ...
    def fit(self, X_df, y_df):
        ...
        X_df, y_df = self.prepare_dfs(
            X_df=X_df, y_df=y_df, normalise_num_cols=False, one_hot_cat_cols=True
        )
        if self.dtype in NUM_COLS_DTYPES:
        ...
        self.leaves_y_dict = (
            leaves_y_df.groupby("leaves").apply(lambda x: x.to_numpy()[:, -1]).to_dict()
        )

    def predict(self, X_test_df):
        X_test_df, _ = self.prepare_dfs(
            X_df=X_test_df, normalise_num_cols=False, one_hot_cat_cols=True, fit=False
        )
        ...
        for leaf, indices in leaves_pred_index_dict.items():
            y_pred[indices] = np.random.choice(
