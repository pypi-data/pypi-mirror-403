...
# global variables
from .. import NUM_COLS_DTYPES, CAT_COLS_DTYPES
...
class Processor:
    ...
    def preprocess(self, df, dtypes):
        for col in self.spop.visited_columns:
            ...
            col_nan_series = [(np.nan, col_nan_indices)] + list(
                cont_nan_indices.items()
            )
            ...
            if col in self.spop.numtocat:
                ...
                df.loc[col_not_nan_indices, col] = pd.cut(
                    df.loc[col_not_nan_indices, col],
                    self.spop.catgroups[col],
                    labels=range(self.spop.catgroups[col]),
                    include_lowest=True,
                )

                grouped = pd.DataFrame(
                    {
                        "grouped": df.loc[col_not_nan_indices, col],
                        "real": not_nan_values,
                    }
                ).groupby("grouped")
    ...
    def postprocess(self, synth_df):
        ...
        for col, processing_nan_col_dict in self.processing_dict[NAN_KEY].items():
            ...
            # NaNs in numerical columns
                ...
                for col_nan_flag, col_nan_value in processing_nan_col_dict[
                    "nan_flags"
                ].items():
                    nan_flag_indices = (
                        synth_df[processing_nan_col_dict["col_nan_name"]]
                        == col_nan_flag
                    )
                    synth_df.loc[nan_flag_indices, col] = col_nan_value
                synth_df.drop(
                    columns=processing_nan_col_dict["col_nan_name"], inplace=True
                )
