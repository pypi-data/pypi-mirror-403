...
from .method import CART_METHOD, METHODS_MAP, NA_METHODS
...
class Synthpop:
    ...
    def _fit(self, df):
        ...
        for col, visit_step in self.visit_sequence.sort_values().items():
            ...
            col_predictors = self.predictor_matrix_columns[
                self.predictor_matrix.loc[col].to_numpy() == 1
            ]
    ...
    def _generate(self):
        synth_df = pd.DataFrame(
            data=np.zeros([self.k, len(self.visit_sequence)]),
            columns=self.visit_sequence.index,
        )
        ...
        for col, visit_step in self.visit_sequence.sort_values().items():
            ...
            if (
                col in self.processor.processing_dict[NAN_KEY]
                and self.df_dtypes[col] in NUM_COLS_DTYPES
                and self.method[col] in NA_METHODS
            ):
                nan_indices = (
                    synth_df[
                        self.processor.processing_dict[NAN_KEY][col]["col_nan_name"]
                    ]
                    != 0
                )
