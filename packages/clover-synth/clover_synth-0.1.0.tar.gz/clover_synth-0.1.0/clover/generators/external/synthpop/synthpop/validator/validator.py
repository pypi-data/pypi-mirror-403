...
# global variables
from .. import NUM_COLS_DTYPES
from ..method import EMPTY_METHOD, SAMPLE_METHOD
from ..method import (
    DEFAULT_METHODS_MAP,
    INIT_METHODS_MAP,
    CONT_TO_CAT_METHODS_MAP,
)
from ..method import ALL_METHODS, INIT_METHODS, DEFAULT_METHODS, NA_METHODS
from ..processor import NAN_KEY
...
class Validator:
    ...
    def method_validator(self, step=None):
        ...
        if step == PROCESSOR_STEP:
            first_visited_col = self.spop.visit_sequence.index[
                self.spop.visit_sequence == 0
            ].values[0]
            ...
            if self.spop.method is None:
                ...
                for (
                    col,
                    visit_order,
                ) in self.spop.visit_sequence.sort_values().item():
        ...
        if step == FIT_STEP:
            for col in self.spop.method.index:
                if col in self.spop.numtocat:
                    self.spop.method[col] = CONT_TO_CAT_METHODS_MAP[
                        self.spop.method[col]
                    ]

                elif (
                    col in self.spop.processor.processing_dict[NAN_KEY]
                    and self.spop.df_dtypes[col] in NUM_COLS_DTYPES
                    and self.spop.method[col] in NA_METHODS
                ):
                    # TODO put in a function
                    nan_col_index = self.spop.method.index.get_loc(col)
                    index_list = self.spop.method.index.tolist()
                    index_list.insert(
                        nan_col_index, self.spop.processed_df_columns[nan_col_index]
                    )
                    self.spop.method = self.spop.method.reindex(
                        index_list,
                        fill_value=CONT_TO_CAT_METHODS_MAP[self.spop.method[col]],
                    )
    ...
    def visit_sequence_validator(self, step=None):
        if step == INIT_STEP:
            ...
            if isinstance(visit_sequence_type, list):
                ...
                assert len(set(self.spop.visit_sequence)) == len(
                    self.spop.visit_sequence
                )
                # validate all visits are either type int or type str
                assert all(
                    isinstance(col, int) for col in self.spop.visit_sequence
                ) or all(isinstance(col, str) for col in self.spop.visit_sequence)
        ...
        if step == PROCESSOR_STEP:
            if self.spop.visit_sequence is None:
                ...
                self.spop.visit_sequence = [
                    col.item() for col in np.arange(self.spop.n_df_columns)
                ]
            ...
            if isinstance(self.spop.visit_sequence[0], int):
                ...
                assert set(self.spop.visit_sequence).issubset(
                    set(np.arange(self.spop.n_df_columns))
                )
                # transform visit_sequence into a list of column names
                self.spop.visit_sequence = [
                    self.spop.df_columns[i] for i in self.spop.visit_sequence
                ]
            else:
            ...
            self.spop.visited_columns = [
                col for col in self.spop.df_columns if col in self.spop.visit_sequence
            ]
            self.spop.visit_sequence = pd.Series(
                [
                    self.spop.visit_sequence.index(col)
                    for col in self.spop.visited_columns
                ],
                index=self.spop.visited_columns,
            )
        ...
        if step == FIT_STEP:
            for col in self.spop.visit_sequence.index:
                if (
                    ...
                    self.spop.visit_sequence.loc[
                        self.spop.visit_sequence >= visit_step
                    ] += 1
                    ...
                    self.spop.visit_sequence = self.spop.visit_sequence.reindex(
                        index_list, fill_value=visit_step
                    )
    ...
    def predictor_matrix_validator(self, step=None):
        ...
        if step == PROCESSOR_STEP:
            ...
            self.spop.predictor_matrix = np.zeros(
                [len(self.spop.visit_sequence), len(self.spop.visit_sequence)],
                dtype=int,
            )
            self.spop.predictor_matrix = pd.DataFrame(
                self.spop.predictor_matrix,
                index=self.spop.visit_sequence.index,
                columns=self.spop.visit_sequence.index,
            )
            visited_columns = []
        ...
        if step == FIT_STEP:
            for col in self.spop.predictor_matrix:
                if (
                    ...
                    self.spop.predictor_matrix.insert(
                        nan_col_index,
                        self.spop.processed_df_columns[nan_col_index],
                        self.spop.predictor_matrix[col],
                    )
                    ...
                    self.spop.predictor_matrix = self.spop.predictor_matrix.reindex(
                        index_list, fill_value=0
                    )
                    self.spop.predictor_matrix.loc[
                        self.spop.processed_df_columns[nan_col_index]
                    ] = self.spop.predictor_matrix.loc[col]

                    self.spop.predictor_matrix.loc[
                        col, self.spop.processed_df_columns[nan_col_index]
                    ] = 1
    ...
    def cont_na_validator(self, step=None):
        ...
        if step == PROCESSOR_STEP:
            if self.spop.cont_na is None:
                ...
                assert all(
                    self.spop.df_dtypes[col] in NUM_COLS_DTYPES
                    for col in self.spop.cont_na
                )
                self.spop.cont_na = {
                    col: col_cont_na
                    for col, col_cont_na in self.spop.cont_na.items()
                    if self.spop.method[col] in NA_METHODS
                }
    ...
    def smoothing_validator(self, step=None):
        ...
        if step == PROCESSOR_STEP:
            if self.spop.smoothing is False:
                ...
                self.spop.smoothing = {
                    col: self.spop.df_dtypes[col] in NUM_COLS_DTYPES
                    for col in self.spop.df_columns
                }
            else:
                ...
                assert all(
                    (
                        smoothing_method == DENSITY
                        and self.spop.df_dtypes[col] in NUM_COLS_DTYPES
                    )
                    or smoothing_method is False
                    for col, smoothing_method in self.spop.smoothing.items()
                )
                self.spop.smoothing = {
                    col: (
                        self.spop.smoothing.get(col, False) == DENSITY
                        and self.spop.df_dtypes[col] in NUM_COLS_DTYPES
                    )
                    for col in self.spop.df_columns
                }
        ...
        if step == FIT_STEP:
            for col in self.spop.processed_df_columns:
                if col in self.spop.numtocat:
                    ...
                    self.spop.smoothing[
                        self.spop.processor.processing_dict[NAN_KEY][col][
                            "col_nan_name"
                        ]
                    ] = False
    ...
    def catgroups_validator(self, step=None):
        if step == INIT_STEP:
            ...
            if isinstance(catgroups_type, int):
                ...
                assert all(
                    (isinstance(col_groups, int) and col_groups > 1)
                    for col_groups in self.spop.catgroups.values()
                )
        ...
        if step == PROCESSOR_STEP:
            if self.spop.catgroups is None:
                ...
                self.spop.catgroups = {
                    col: self.spop.catgroups for col in self.spop.numtocat
                }
