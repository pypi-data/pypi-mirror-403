...
class Scene:
    ...
    def _create_frame(self, t: float, width: int, height: int) -> np.ndarray:
        ...
        return self._create_frame_for_elements(t, width, height, self._elements)
    ...
    def _create_frame_for_elements(
        ...
        """Render a frame from the provided elements.

        Args:
            t (float): Current time in seconds for visibility checks.
            width (int): Frame width in pixels.
            height (int): Frame height in pixels.
            elements (list[FrameElement]): Elements to render.
    ...
    def export_frame(
        ...
        """Export the frame at time t as a PIL image.

        Args:
            t (float): Time in seconds on the scene timeline.
            width (int): Frame width in pixels.
            height (int): Frame height in pixels.
            output_path (Optional[str]): File path to write the image into.

        Returns:
            Image.Image: Rendered frame image.
        """

        elements = self._get_elements()
        frame_data = self._create_frame_for_elements(t, width, height, elements)
        frame_image = Image.fromarray(frame_data, mode="RGBA")
        if output_path:
