...
file `htmx_forms/generic_form_inner.html`:
...
<div class="row">
    <div class="col-sm-12">
        <form method="post"{% if action %} action="{{ action }}"{% endif %} class="form{% if form_class %} {{form.form_class}}{% endif %}" novalidate autocomplete="off"{% if form.form_enctype %} enctype="{{form.form_enctype}}"{% endif %}>
            {% csrf_token %}
            {{ form }}
...
You can use various features for rendering a form by injecting `HTMXFormMixin` into the Form class:
...
A `render_form_field` template tag is available for normalize the rendering of a generic form field, as specified in the `htmx_forms/form/form_field.html` template (which can be overridden in several ways as described later):
...
where `htmx_forms/form/form_field.html` is:

```html 
<div class="form-field {% if field.errors %}is-invalid{% endif %}">
    {{ field.label_tag }}
...
Example usages in a template:

```html
{% load htmx_forms_tags %}
...
- The caller **can provide a custom value for `ccs_class`** and optionally **an HTML fragment** that will be included in the rendered field
- **One or more attributes may follow**:
  - data-* attributes must be passed as data_xyz → they become data-xyz
  - Boolean attributes work (required=True); that is, the key alone is included only if and only if the corresponding value is True



### Overridding the template used for rendering fields

The template used is determined according to the following rules:

- by consulting the mapping of the form `form_field_template_overrides` (field-specific) or `form_field_template_name` if evaluated; example:

```python
form_field_template_name = "my_form_render_field.html"
form_field_template_overrides = {"username": "render_username.html", "password": "render_password.html", }
```

- consulting the project settings:

```python
HTMX_FORMS = {
    "FORM_FIELD_TEMPLATE_NAME": 'frontend/form/form_field.html',
}
```

- using the default provided by the library: `htmx_forms/form/form_field.html`
- the latter can also be overridden in the project as usual



### Adding attributes and/or appending HTML fragments to each field

**Adding HTMX attributes to form fields gives several benefits:**
...
In general, it is preferable to **define these HTMX attributes directly in the Django Form**, which already describes field semantics (Field type, Validation rules, Required/optional, Help text, Labels, Widget, ...); HTMX behavior is semantic UI behavior, so it belongs here.

To this purpose,`HTMXFormMixin` injects into your Form class who mappings, later read by  the template tags to customize the field rendering:

- an `htmx_field_attributes` dictionary of attributes for HTMX to be injected into specific fields (actually, it can also be used to inject generic attributes)
- an `htmx_field_fragments` dictionary of HTML fragments to be injected into specific fields; the fragments will be submitted to the template engine for the necessary interpolation before rendering
...
| Key                      | Default                         | Meaning                                                      |
| ------------------------ | ------------------------------- | ------------------------------------------------------------ |
| FORM_FIELD_TEMPLATE_NAME | htmx_forms/form/form_field.html | Template used for form fields rendering (unless overridden in the Form) |
| ...                      |                                 |                                                              |
