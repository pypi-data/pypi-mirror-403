...
from qblox_scheduler.helpers.diagnostics_report import _generate_diagnostics_report
...
class ScheduleGettable:
    ...
    schedule_function
        A function which returns a :class:`qblox_scheduler.schedule.Schedule` or
    ...
    def __init__(
        ...
        schedule_function: Callable[..., Schedule | TimeableSchedule],
        ...
        if self.real_imag:
            ...
            if data_labels:
                ...
                logger.info("Auto-generating labels. %s", self.label)
    ...
    def compile(self) -> CompiledSchedule:
        ...
        if isinstance(schedule, Schedule):
            ...
            if timeable_schedule is None:
                raise RuntimeError("Can not compile schedule with near-time steps")
            assert isinstance(timeable_schedule, TimeableSchedule)
        else:
            timeable_schedule = schedule
        ...
        self._compiled_schedule = self._backend.compile(
            schedule=timeable_schedule, config=compilation_config
        )
    ...
    def initialize(self) -> None:
        ...
        instr_coordinator = self.quantum_device.instr_instrument_coordinator
        if not instr_coordinator:
            raise ValueError(f"{self.quantum_device} has no instrument coordinator")
        if not self._compiled_schedule:
            raise ValueError(f"{self.quantum_device} has no compiled schedule")
    ...
    def get(self) -> tuple[np.ndarray, ...]:
        ...
        instr_coordinator = self.quantum_device.instr_instrument_coordinator
        if not instr_coordinator:
            raise ValueError(f"{self.quantum_device} has no instrument coordinator assigned")
        ...
        if self._return_xarray:
            return acquired_data  # type: ignore[reportReturnType]
    ...
    def initialize_and_get_with_report(self) -> str:
        ...
        instrument_coordinator = self.quantum_device.instr_instrument_coordinator
