...
from qblox_scheduler.analysis import fitting_models as fm
from qblox_scheduler.analysis.calibration import (
...
class SingleQubitTimedomainAnalysis(ba.BaseAnalysis):
    ...
    def process_data(self):
        ...
        if self.calibration_points == "auto":
            self.calibration_points = has_calibration_points(self.dataset_processed.S21.values)
    ...
    def analyze_fit_results(self):
        ...
        if fit_warning is None:
            ...
            if not self.calibration_points:
                ...
                text_msg += format_value_string("offset", fit_result.params["offset"], unit=unit)
        else:
            text_msg = ba.wrap_text(fit_warning)
            self.quantities_of_interest["fit_success"] = False

        self.quantities_of_interest["T1"] = ba.lmfit_par_to_ufloat(fit_result.params["tau"])
...
class EchoAnalysis(SingleQubitTimedomainAnalysis, _DecayFigMixin):
    ...
    def analyze_fit_results(self):
        ...
        self.quantities_of_interest["t2_echo"] = ba.lmfit_par_to_ufloat(fit_result.params["tau"])
        self.quantities_of_interest["fit_msg"] = text_msg
...
class RamseyAnalysis(SingleQubitTimedomainAnalysis, _DecayFigMixin):
    ...
    def analyze_fit_results(self):
        ...
        self.quantities_of_interest["T2*"] = ba.lmfit_par_to_ufloat(fit_parameters["tau"])
...
class AllXYAnalysis(SingleQubitTimedomainAnalysis):
    ...
    def process_data(self):
        ...
        repeats = round(number_points / 21)
...
class RabiAnalysis(SingleQubitTimedomainAnalysis):
    ...
    def run(self, calibration_points: bool = True):
        ...
        if not isinstance(calibration_points, bool):
            raise TypeError(
                f"Incorrect input. calibration_points={calibration_points} must be a bool."
            )
        return super().run(calibration_points=calibration_points)
    ...
    def _rotate_to_calibrated_axis(self):
        ...
        self.dataset_processed.S21_rot.attrs["long_name"] = "Rotated transmission $S_{21}^R$"
