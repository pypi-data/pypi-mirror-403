...
def get_model_common_doc() -> str:
    ...
    """
    return (
        lmfit.models.COMMON_INIT_DOC.replace(":class:`Model`", ":class:`~lmfit.model.Model`")
        .replace("\n    ", "\n")
        .replace(" : str", " : :obj:`str`")
        .replace("['x']", ":code:`['x']`")
        .replace(", optional", "")
...
class CosineModel(lmfit.model.Model):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.set_param_hint("phase", vary=True, min=-np.pi, max=np.pi)  # enforce phase range
    ...
    # Same design pattern is used in lmfit.models to inherit common docstrings.
...
class ResonatorModel(lmfit.model.Model):
    ...
    # Same design pattern is used in lmfit.models
...
class RabiModel(lmfit.model.Model):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Fix the phase at pi so that the output is at a minimum when x=0
...
class DecayOscillationModel(lmfit.model.Model):
    ...
    def guess(self, data, **kws) -> lmfit.parameter.Parameters:
        t = kws.get("t")
        if t is None:
            raise ValueError('Time variable "t" must be specified in order to guess parameters')
...
def fft_freq_phase_guess(data: np.ndarray, t: np.ndarray) -> tuple[float, float]:
    ...
    # negative frequency components, and we want a positive frequency.
