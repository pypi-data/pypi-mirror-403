...
from qblox_scheduler.operations.gate_library import X90, Measure, Reset, Rxy, X, Y
...
def rabi_sched(
    ...
    schedule = TimeableSchedule("Rabi", repetitions)
    schedule.add_resource(ClockResource(name=clock, freq=frequency))

    for i, (amp, duration) in enumerate(zip(amps, durations, strict=False)):
...
def t1_sched(
    ...
    schedule = TimeableSchedule("T1", repetitions)
...
def ramsey_sched(
    ...
    schedule = TimeableSchedule("Ramsey", repetitions)
...
def echo_sched(
    ...
    schedule = TimeableSchedule("Echo", repetitions)
    for i, tau in enumerate(times):
        schedule.add(Reset(device_element), label=f"Reset {i}")
        schedule.add(X90(device_element))
...
def cpmg_sched(
    ...
    repetitions
        The amount of times the TimeableSchedule will be repeated, default is 1.
    ...
    schedule = TimeableSchedule("CPMG", repetitions)
    for i, tau in enumerate(times):
...
def allxy_sched(
    ...
    schedule = TimeableSchedule("AllXY", repetitions)
    ...
    for i, elt_idx in enumerate(element_select_idx):
        ...
        schedule.add(Measure(device_element, bin_mode=BinMode.AVERAGE), label=f"Measurement {i}")
    return schedule
...
def readout_calibration_sched(
    ...
    schedule = TimeableSchedule(
        f"Readout calibration {device_element}, {prepared_states}", repetitions
    )
...
def rabi_pulse_sched(
    ...
    """
    schedule = TimeableSchedule("Rabi schedule (pulse)", repetitions)
