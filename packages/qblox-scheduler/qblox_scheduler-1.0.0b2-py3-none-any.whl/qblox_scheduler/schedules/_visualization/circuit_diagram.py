...
import qblox_scheduler.schedules._visualization.pulse_scheme as ps
...
from qblox_scheduler.schedules._visualization import constants
...
def _walk_schedule(
    sched_or_op: TimeableSchedule | Operation, time_offset: int = 0
) -> Iterator[tuple[int, Operation | TimeableSchedule | _ControlFlowEnd]]:
    if isinstance(sched_or_op, TimeableScheduleBase):
        ...
        yield time_offset, sched_or_op
        if isinstance(sched_or_op.body, TimeableScheduleBase):
...
def _walk_schedule_only_operations(
    sched_or_op: TimeableSchedule | Operation,
) -> Iterator[Operation]:
    if isinstance(sched_or_op, TimeableScheduleBase):
...
def _draw_operation(
    ...
    if operation.valid_gate:
        ...
        idxs = [port_map[pulse_info["port"]]] if pulse_info["port"] else []
        ...
        idxs = [port_map[operation["acquisition_info"]["port"]]]
...
def _get_indices(
    ...
    def add_index_from_operation(operation: Operation, index_set: set[int]) -> None:
        ...
        if operation["pulse_info"]:
            index_set.update({port_map[operation["pulse_info"]["port"]]})
        if operation["acquisition_info"]:
            index_set.update({port_map[operation["acquisition_info"]["port"]]})
...
def _draw_conditional(
    ...
    body: Operation | TimeableSchedule,
    body_start: int,
    body_end: int,
    device_element_map: dict[str, int],
    port_map: dict[str, int],
    ...
    def draw_rectangle_with_arrow(bottom_device_element: int, top_device_element: int) -> None:
        p1 = mpl.patches.Rectangle(  # type: ignore
...
def _get_device_element_and_port_map_from_schedule(
    ...
    for operation in _walk_schedule_only_operations(schedule):
        if operation.valid_gate:
            ...
            continue
        if operation["pulse_info"] and (port := operation["pulse_info"]["port"]) is not None:
            # Can be None e.g. in case of NCO operations.
            ports.add(port)
        if (
            operation["acquisition_info"]
            and (port := operation["acquisition_info"]["port"]) is not None
        ):
            ports.add(port)
...
def _get_feedback_label_and_device_element_idx(
    ...
    if (
        operation["acquisition_info"]
        and (feedback_label := operation["acquisition_info"].get("feedback_trigger_label", None))
        is not None
    ):
        return feedback_label, port_map[operation["acquisition_info"]["port"]]
    if (
...
def circuit_diagram_matplotlib(
    ...
    for abs_time, operation in _walk_schedule(schedule):
        ...
        elif isinstance(operation, TimeableScheduleBase):
            pass
