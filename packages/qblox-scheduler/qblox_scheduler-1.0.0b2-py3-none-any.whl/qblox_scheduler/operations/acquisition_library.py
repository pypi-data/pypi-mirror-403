...
from qblox_scheduler.enums import (
    BinMode,
...
from qblox_scheduler.resources import DigitalClockResource
...
class WeightedIntegratedSeparated(Acquisition):
    ...
    def __init__(
        ...
        coords: dict | None = None,
        acq_index: int | None = None,
        bin_mode: BinMode | str = BinMode.AVERAGE_APPEND,
        phase: float = 0,
        t0: float = 0,
    ) -> None:
        _warn_deprecated_acq_index(acq_index)
        ...
        if "acq_return_type" not in self.data["acquisition_info"]:
            self.data["acquisition_info"]["acq_return_type"] = complex
            self.data["acquisition_info"]["protocol"] = "WeightedIntegratedSeparated"
...
class SSBIntegrationComplex(Acquisition):
    ...
    def __init__(
        ...
        if "acq_return_type" not in self.data["acquisition_info"]:
            self.data["acquisition_info"]["acq_return_type"] = complex
            self.data["acquisition_info"]["protocol"] = "SSBIntegrationComplex"
...
class NumericalSeparatedWeightedIntegration(WeightedIntegratedSeparated):
    ...
    def __init__(
        ...
        self.data["acquisition_info"]["protocol"] = "NumericalSeparatedWeightedIntegration"
...
class NumericalWeightedIntegration(NumericalSeparatedWeightedIntegration):
    ...
    def __init__(
        ...
        self.data["acquisition_info"]["protocol"] = "NumericalWeightedIntegration"
...
class WeightedThresholdedAcquisition(NumericalWeightedIntegration):
    ...
    def __init__(
        ...
        self.data["acquisition_info"]["protocol"] = "WeightedThresholdedAcquisition"
        self.data["acquisition_info"]["feedback_trigger_label"] = feedback_trigger_label
        self.data["acquisition_info"]["acq_threshold"] = acq_threshold
        self.data["acquisition_info"]["acq_rotation"] = 360 - 45
        self.data["acquisition_info"]["acq_return_type"] = (
...
class TriggerCount(Acquisition):
    ...
    def __init__(
        ...
        if bin_mode == BinMode.AVERAGE:
            raise ValueError(
                f"{bin_mode} is unsupported for the TriggerCount acquisition protocol. "
...
class ThresholdedTriggerCount(Acquisition):
    ...
    def __str__(self) -> str:
        acq_info = copy(self.data["acquisition_info"])
...
class DualThresholdedTriggerCount(Acquisition):
    ...
    def __str__(self) -> str:
        ...
        for suffix in DualThresholdedTriggerCountLabels:  # type: ignore
            acq_info[f"label_{suffix}"] = acq_info["feedback_trigger_labels"][suffix]
        return self._get_signature(acq_info)
