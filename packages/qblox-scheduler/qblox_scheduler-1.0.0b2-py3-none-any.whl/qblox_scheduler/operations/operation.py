...
from qblox_scheduler.json_utils import JSONSchemaValMixin, lru_cache
from qblox_scheduler.operations.expressions import substitute_value_in_arbitrary_container
...
class Operation(JSONSchemaValMixin, UserDict):
    ...
    **JSON schema of a valid Operation**
    ...
    def duration(self) -> float:
        ...
        have zero duration.
        """
        if self.data["pulse_info"] and self.data["acquisition_info"]:
            raise ValueError(
                "This operation has both pulse and acquisition info. It is not clear whether this "
                "is a pulse or an acquisition."
            )

        if self.data["pulse_info"]:
            return self.data["pulse_info"]["t0"] + self.data["pulse_info"]["duration"]

        if self.data["acquisition_info"]:
            return self.data["acquisition_info"]["t0"] + self.data["acquisition_info"]["duration"]

        return 0  # Maybe not entirely correct, but it is legacy behaviour.
    ...
    def add_device_representation(self, device_operation: Operation) -> None:
        ...
        """
        if self.data["pulse_info"] or self.data["acquisition_info"]:
            raise ValueError(
                "Cannot add another device representation to an operation that already has a "
                "device representation."
            )
        if device_operation.data["pulse_info"]:
            self.data["pulse_info"] = device_operation.data["pulse_info"]
        elif device_operation.data["acquisition_info"]:
            self.data["acquisition_info"] = device_operation.data["acquisition_info"]
    ...
    def get_used_port_clocks(self) -> set[tuple[str, str]]:
        ...
        if self.valid_pulse:
            info = "pulse_info"
        elif self.valid_acquisition:
            info = "acquisition_info"
        else:
            raise RuntimeError(
        ...
        port = self.data[info].get("port")
        clock = self.data[info].get("clock")
    ...
    def is_conditional_acquisition(self) -> bool:
        ...
        """
        acq_info = self.data["acquisition_info"]
        return (
            acq_info.get("feedback_trigger_label") is not None
            or len(acq_info.get("feedback_trigger_labels", {})) > 0
        )
    ...
    def has_voltage_offset(self) -> bool:
        ...
        return (
            "offset_path_I" in self.data["pulse_info"] or "offset_path_Q" in self.data["pulse_info"]
        )
