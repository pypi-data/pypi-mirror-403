...
# If the operation is a TimeableSchedule type, we always return a TimeableSchedule.
# Otherwise, we can return an Operation or a TimeableSchedule.
@overload
...
def _extract_clock_freqs(
    operation: Operation | TimeableSchedule, all_clock_freqs: dict[str, float]
) -> None:
    if isinstance(operation, TimeableScheduleBase):
...
def _extract_clocks_used(operation: Operation) -> set[str]:
    ...
    elif operation["pulse_info"]:
        return {operation["pulse_info"]["clock"]}
    elif operation["acquisition_info"]:
        return {operation["acquisition_info"]["clock"]}
    return set()
...
def _set_pulse_and_acquisition_clock(
    ...
    ----------
    schedule
        The resources from ``operation`` are added to `.TimeableSchedule``
...
def _valid_clock_in_schedule(
    ...
    all_clock_freqs
        All clock frequencies
    schedule
        TimeableSchedule that potentially has the clock in its resources
    operation
...
def _compile_multiplexed(
    ...
    for mux_idx, device_element in enumerate(device_elements):
        ...
        device_op: Operation | TimeableSchedule = _get_device_repr_from_cfg_multiplexed(
            operation,
    ...
    if len(inner_subschedules) != 0:
        inner_schedule: TimeableSchedule = TimeableSchedule(f"Inner schedule for {operation!s}")
...
def _compile_single_device_element(
    ...
    device_op: Operation | TimeableSchedule = _get_device_repr_from_cfg(
        operation=operation,
    ...
    if isinstance(device_op, TimeableScheduleBase):
        return _compile_circuit_to_device(
            operation=device_op,
            device_cfg=device_cfg,
            device_overrides=new_device_overrides,
...
def _compile_two_device_elements(
    ...
    Note: it updates the `operation`, if it can directly add pulse representation.
    """
    parent_element, child_element = device_elements
    edge = f"{parent_element}_{child_element}"
