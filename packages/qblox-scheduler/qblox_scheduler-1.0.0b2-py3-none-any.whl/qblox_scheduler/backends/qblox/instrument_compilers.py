...
from qblox_scheduler.backends.qblox import compiler_abc
...
from qblox_scheduler.backends.qblox.timetag import TimetagSequencerCompiler
...
from qblox_scheduler.enums import TimeRef

if TYPE_CHECKING:
    from qblox_scheduler.backends.qblox.compiler_abc import _SequencerT_co
...
class QCMCompiler(BasebandModuleCompiler):
    ...
    static_hw_properties: StaticAnalogModuleProperties = StaticAnalogModuleProperties(
        instrument_type="QCM",
        max_acquisition_bin_count=MAX_NUMBER_OF_BINS,
    ...
    def _configure_filter(
        ...
        filt.config = FilterConfig.ENABLED
        if marker_debug_mode_enable:
            filt.marker_delay = FilterMarkerDelay.DELAY_COMP
...
class QRCCompiler(RFModuleCompiler):
    ...
    static_hw_properties = StaticAnalogModuleProperties(  # type: ignore
        instrument_type="QRC",
        max_acquisition_bin_count=MAX_NUMBER_OF_BINS_QRC,
        max_awg_output_voltage=None,
        mixer_dc_offset_range=BoundedParameter(min_val=-50, max_val=50, units="mV"),
        channel_name_to_digital_marker={
    ...
    def _set_time_ref_channel(
        ...
        for acq in all_acqs_with_time_ref_port:
            ...
            if (
                ...
                and len({portclock_to_path[pc] for pc in matching_port_clocks}) > 1
            ):
                raise ValueError(
...
class QSMCompiler(compiler_abc.ClusterModuleCompiler):
    """QSM specific implementation of the qblox compiler."""

    _settings: DCModuleSettings  # type: ignore
    supports_acquisition = False  # type: ignore
    max_number_of_instructions = MAX_NUMBER_OF_INSTRUCTIONS_QCM  # type: ignore
    static_hw_properties = StaticDCModuleProperties(
        instrument_type="QSM",
    ...
    def __init__(
        ...
        super().__init__(name, total_play_time, instrument_cfg)
        self._settings = DCModuleSettings.extract_settings_from_mapping(instrument_cfg)  # type: ignore
    ...
    def compile(
        self,
        debug_mode: bool,  # noqa: ARG002
        repetitions: int = 1,  # noqa: ARG002
        sequence_to_file: bool | None = None,  # noqa: ARG002
    ) -> dict[str, Any]:
...
class ClusterCompiler(compiler_abc.InstrumentCompiler):
    ...
    compiler_classes: ClassVar[dict[str, type]] = {
        "QCM": QCMCompiler,
        "QRM": QRMCompiler,
