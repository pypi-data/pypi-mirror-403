...
from qblox_scheduler.backends.qblox.compiler_abc import (
...
from qblox_scheduler.backends.qblox.operation_handling.factory_analog import (
...
from qblox_scheduler.operations.pulse_library import SetClockFrequency
...
class AnalogSequencerCompiler(SequencerCompiler):
    ...
    def __init__(
        ...
        self._associated_lo_frequency: float | None = None
    ...
    def _prepare_acq_settings(
        ...
        acquisition_infos: list[OpInfo] = [acq.operation_info for acq in acquisitions]
    ...
    def prepare(self) -> None:
        ...
        is called.
        """
        self._update_set_clock_frequency_operations_and_frequency_sweep_domains()
        self._assert_enough_time_between_freq_phase_updates(self._get_ordered_operations())
        super().prepare()

    def _update_set_clock_frequency_operations_and_frequency_sweep_domains(self) -> None:
        vars_in_set_freq = set()
        for op_strat in self.op_strategies:
            if op_strat.operation_info.name == SetClockFrequency.__name__:
                ...
                if isinstance(op_strat.operation_info.data["clock_freq_new"], Variable):
                    vars_in_set_freq.add(op_strat.operation_info.data["clock_freq_new"])

        for op_strat in self.op_strategies:
            if isinstance(op_strat, LoopStrategy):
                for var in vars_in_set_freq:
                    if var in op_strat.operation_info.data["domain"]:
                        if self._associated_lo_frequency is None:
                            raise RuntimeError(
                                f"The frequency of the LO associated with sequencer {self.name} is "
                                "not known yet."
                            )
                        old_domain: LinearDomain = op_strat.operation_info.data["domain"][var]
                        # We know the data type must be Hz here because the variable is used in a
                        # SetClockFrequency operation.
                        op_strat.operation_info.data["domain"][var] = LinearDomain(
                            dtype=DType.FREQUENCY,
                            start=old_domain.start - self._associated_lo_frequency,
                            stop=old_domain.stop - self._associated_lo_frequency,
                            num=old_domain.num,
                        )
    ...
    def _write_pre_wait_sync_instructions(self, qasm: QASMProgram) -> None:
        ...
        if (
            ...
            and module_options.rf_output_on is True
            ...
            if portclock in input_gains:
                ...
                if init_portclock and (
                    ...
                    raise ValueError(
                        ...
                        f"https://docs.qblox.com/en/main/products/qblox_scheduler/user_guide/hardware_config.html#gain-and-attenuation"
                    )
    ...
    def _configure_dc_latency_comp_for_output(self, output: int, dc_comp: int) -> None:
        for i, mask in enumerate(
            ...
            getattr(self._settings.distortion_corrections[output], f"exp{i}").config = (
                FilterConfig.DELAY_COMP if dc_comp & mask else FilterConfig.BYPASSED
            )
        self._settings.distortion_corrections[output].fir.config = (
            FilterConfig.DELAY_COMP
            if dc_comp & DistortionCorrectionLatencyEnum.FIR
            else FilterConfig.BYPASSED
        )
    ...
    def _configure_dc_latency_comp_for_marker(self, output: int, dc_comp: int) -> None:
        for i, mask in enumerate(
            ...
            getattr(self._settings.distortion_corrections[output], f"exp{i}").marker_delay = (
                FilterMarkerDelay.DELAY_COMP if dc_comp & mask else FilterMarkerDelay.BYPASSED
            )
        self._settings.distortion_corrections[output].fir.marker_delay = (
            FilterMarkerDelay.DELAY_COMP
            if dc_comp & DistortionCorrectionLatencyEnum.FIR
            else FilterMarkerDelay.BYPASSED
        )
...
class BasebandModuleCompiler(AnalogModuleCompiler, ABC):
    ...
    def assign_frequencies(
        ...
        if external_lo is None:
            ...
            sequencer.set_associated_lo_frequency(0)
        ...
        except Exception as error:  # noqa: BLE001 Adding sequencer info to exception message
        ...
        sequencer.set_associated_lo_frequency(freqs.LO)
...
class RFModuleCompiler(AnalogModuleCompiler, ABC):
    ...
    def assign_attenuation(self) -> None:
        ...
        if input_att_cfg is not None:
            # Gather all channel names with a portclock in input_att_cfg
            channel_names_with_input_att: set[str] = set()
            for portclock, path in self.instrument_cfg.portclock_to_path.items():
                if portclock in input_att_cfg and path.channel_name in (
                    "complex_input_0",
                    "complex_input_1",
                    "complex_output_0",
                    "complex_output_1",
                ):
                    channel_names_with_input_att.add(path.channel_name)

            # Check whether the prohibited input, output pairs are present
            for input_ch, output_ch in (
                ("complex_input_0", "complex_output_0"),
                ("complex_input_1", "complex_output_1"),
            ):
                if {input_ch, output_ch}.issubset(channel_names_with_input_att):
                    raise ValueError(
                        f"'input_att' is defined for both '{output_ch}' and "
                        f"'{input_ch}' on module '{self.name}', which is prohibited. "
        ...
        for (
            ...
            output_to_out_att_name = {
                "complex_output_0": "out0_att",
                "complex_output_1": "out1_att",
                "complex_output_2": "out2_att",
                "complex_output_3": "out3_att",
                "complex_output_4": "out4_att",
                "complex_output_5": "out5_att",
            }
            if (
                (out_att_name := output_to_out_att_name.get(path.channel_name)) is not None
                and output_att_cfg is not None
                and (out_att := output_att_cfg.get(portclock)) is not None
            ):
                setattr(self._settings, out_att_name, _convert_to_int(out_att, label=out_att_name))
            # Note, the keys are intentionally outputs:
            # the inputs are handled differently (sometimes as outputs) in the compiler,
            # for compatibility reasons.
            input_to_in_att_name = {
                "complex_output_0": "in0_att",
                "complex_output_1": "in1_att",
            }
            if (
                (in_att_name := input_to_in_att_name.get(path.channel_name)) is not None
                and input_att_cfg is not None
                and (in_att := input_att_cfg.get(portclock)) is not None
            ):
                setattr(self._settings, in_att_name, _convert_to_int(in_att, label=in_att_name))
