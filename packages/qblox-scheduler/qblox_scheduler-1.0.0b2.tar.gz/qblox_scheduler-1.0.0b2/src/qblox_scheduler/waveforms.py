...
def ramp(t: np.ndarray, amp: float, offset: float, duration: float) -> np.ndarray:
...
def drag(
    ...
    All inputs are measured in units of s and Hz, while phase angles are specified in degrees.
    ...
    sigma
        Width of the Gaussian envelope. If None, it is calculated as
        duration / (2 * nr_sigma).
    phase
...
def interpolated_complex_waveform(
    ...
    **kwargs,
    ...
    if isinstance(gain, complex):
        return real_interpolator(t) * gain.real + 1.0j * imag_interpolator(t) * gain.imag
    else:
        return (real_interpolator(t) + 1.0j * imag_interpolator(t)) * gain
