...
from qblox_scheduler.helpers.waveforms import area_pulse
...
def _determine_sum_and_end_of_all_pulses(
    operation: TimeableSchedule | Operation,
    sampling_rate: float,
    time_offset: float,
    ports: set[str],
    is_conditional: bool,
...
def _determine_compensation_pulse(
    ...
    * the compensation pulse starts just after the previous pulse.
    ...
    operation_with_abs_times = (
        operation
        if not isinstance(operation, TimeableSchedule)
...
def process_compensation_pulses(
    ...
    """
    operation: Operation | TimeableSchedule = schedule
    if isinstance(operation, TimeableScheduleBase):
        ...
        pulse_compensated_schedule = TimeableSchedule("pulse_compensated_schedule")
