...
def _dispersive_measurement(  # noqa: C901, PLR0915
    pulse_amp: float,
    ...
    reset_clock_phase
        Whether to reset the NCO clock phase. Default is True.
    reference_magnitude
    ...
    """
    kwargs: dict[str, Any] = {} if bin_mode is None else {"bin_mode": bin_mode}

    subschedule = TimeableSchedule("dispersive_measurement")
    ...
    if acq_protocol == "SSBIntegrationComplex":
        ...
        loop_subschedule = TimeableSchedule("loop_SSB")
    ...
    if freq is not None:
        subschedule_with_freq = TimeableSchedule("dispersive_measurement_with_freq")
...
def dispersive_measurement_transmon(
    ...
    pulse_type
        The type of pulse to use. Default is "SquarePulse".
    bin_mode
        The binning mode for the acquisition. Default is the protocol default.
    acq_protocol_default
        The default acquisition protocol to use. Default is "SSBIntegrationComplex".
    reset_clock_phase
        Whether to reset the NCO clock phase. Default is True.
...
def optical_measurement(
    ...
    coords
        Coords.
    acq_index
        Acquisition index as defined in the TimeableSchedule
    bin_mode
    ...
    device_op = TimeableSchedule("OpticalMeasurement")

    if pulse_type == "SquarePulse":
        settings = zip(pulse_amplitudes, pulse_durations, pulse_ports, pulse_clocks, strict=False)
    ...
    if acq_protocol == "TriggerCount":
        ...
        if acq_time_source is not None:
            kwargs["time_source"] = acq_time_source
        if acq_time_ref is not None:
            kwargs["time_ref"] = acq_time_ref
        device_op.add(
            Timetag(
                port=acq_port,
                clock=acq_clock,
