...
class ReadoutCalibrationAnalysis(ba.BaseAnalysis):
    ...
    def run_fitting(self) -> None:
        ...
        except Exception as lda_error:  # noqa: BLE001
    ...
    def create_figures(self) -> None:
        ...
        ax.scatter(x1_fp[:, 0], x1_fp[:, 1], marker="x", s=20, color="#000099")  # dark blue
        ...
        y_at_xlim = (-self._lda.coef_[0][0] * xlim - self._lda.intercept_[0]) / self._lda.coef_[0][
            1
        ]
        ...
        if abs(self.quantities_of_interest["acq_rotation_rad"] % (2 * np.pi) - np.pi) < np.pi / 2:
