...
from qblox_scheduler import enums, resources
...
OperationReferencePoint = Literal["start", "center", "end"]
"""Type hint for operation reference points."""


class TimeableScheduleBase(JSONSchemaValMixin, JSONSerializable, UserDict, ABC):
    """
    ...
    **JSON schema of a valid Schedule**
    ...
    @abstractmethod
    def clone(self) -> TimeableScheduleBase:
    ...
    def plot_circuit_diagram(
        ...
        if plot_backend == "mpl":
            import qblox_scheduler.schedules._visualization.circuit_diagram as cd
    ...
    def plot_pulse_diagram(
        ...
        **backend_kwargs,
        ...
        from qblox_scheduler.schedules._visualization.pulse_diagram import sample_schedule
        ...
        if plot_backend == "mpl":
            ...
            from qblox_scheduler.schedules._visualization.pulse_diagram import (
    ...
    def _generate_timing_table_list(
        cls,
        operation: Operation | TimeableScheduleBase,
        time_offset: float,
        ...
        if isinstance(operation, TimeableScheduleBase):
            ...
            if operation["pulse_info"]:
                op_info = operation["pulse_info"]
            elif operation["acquisition_info"]:
                op_info = operation["acquisition_info"]
            else:
                return
            t0 = time_offset + op_info["t0"]
            df_row = {
                "waveform_op_id": str(operation),
                "port": op_info["port"],
                "clock": op_info["clock"],
                "abs_time": t0,
    ...
    def __setstate__(self, state: dict[str, Any]) -> None:
        ...
        if isinstance(data["schedulables"], list):
            ...
            data["schedulables"] = dict(data["schedulables"])
        self.data = data

    @classmethod
    def is_valid(cls, schedule: TimeableScheduleBase) -> bool:
        """Check if schedule adheres to JSON schema."""
        super().is_valid(schedule)  # type: ignore
        return True


class TimeableSchedule(TimeableScheduleBase):
    """
    ...
    def add_resource(self, resource: Resource) -> None:
        ...
        if self._scope_stack:
            return self._scope_stack[-1].add_resource(resource)
    ...
    def add(
        ...
        """
        if self._scope_stack:
            return self._scope_stack[-1].add(
                operation,
                rel_time,
                ref_op,
                ref_pt,
    ...
    def _validate_add_arguments(
        ...
        if not isinstance(operation, (Operation, TimeableSchedule)):
            raise ValueError(
                f"Attempting to add operation to schedule. "
                f"The provided object '{operation=}' is not"
                " an instance of Operation or TimeableSchedule"
            )
    ...
    def declare(self, dtype: DType) -> Variable:
        ...
        """
        if self._scope_stack:
            return self._scope_stack[-1].declare(dtype)
        var = Variable(dtype)
        self.define(var)
        return var
    ...
    def define(self, var: Variable) -> None:
        ...
        """
        if self._scope_stack:
            return self._scope_stack[-1].define(var)
        self["variables"][var.id_] = var

    def clone(self) -> TimeableSchedule:
        """Clone this schedule into a separate independent schedule."""
        new_data = deepcopy(self.data)
        return self.__class__(self.name, self.repetitions, new_data)
    ...
    def substitute(
        ...
        for k, op in self.operations.items():
            new_operations[k] = op.substitute(substitutions)
            if new_operations[k] is not op:
                changed = True
            else:
                new_operations[k] = new_operations[k].clone()

        new_schedulables = {}
        for k, sched in self.schedulables.items():
            new_schedulables[k] = sched.substitute(substitutions)
            if new_schedulables[k] is not sched:
                changed = True
            else:
                new_schedulables[k] = new_schedulables[k].clone()

        if changed:
            new_schedule = self.clone()
            new_schedule.data["operation_dict"] = new_operations
            new_schedule.data["schedulables"] = new_schedulables
            return new_schedule
        else:
            return self
    ...
    def loop(
        self,
        domain: dict[Variable, LinearDomain],
        rel_time: float = 0,
        ref_op: Schedulable | str | None = None,
        ref_pt: OperationReferencePoint | None = None,
        ref_pt_new: OperationReferencePoint | None = None,
    ...
    def loop(
        ...
        *domains: LinearDomain,
        ...
        """
        self._scope_stack.append(TimeableSchedule())
        if isinstance(domain, dict):
            if domains:
                raise TypeError(
                    "can not supply domain as a dict and extra domains at the same time"
                )
            variables = list(domain.keys())
            all_domains = list(domain.values())
            ...
            for dom in all_domains:
                variables.append(self.declare(dom.dtype))
        for dom in all_domains:
            if strategy is None and dom.dtype.is_timing_sensitive():
        ...
        body = self._scope_stack.pop()
        enclosing_scope = self._scope_stack[-1] if self._scope_stack else self
        enclosing_scope.add(
            LoopOperation(
                body,
                domain=dict(zip(variables, all_domains, strict=False)),
                strategy=strategy,
            ),
            rel_time=rel_time,
            ref_op=ref_op,
...
class Schedulable(JSONSchemaValMixin, UserDict):
    ...
    def clone(self) -> Schedulable:
        ...
        sched = self.__class__(self["name"], self["operation_id"])
        sched.data = deepcopy(self.data)
        return sched
    ...
    def substitute(
        ...
        for constraint in self["timing_constraints"]:
            if isinstance(constraint.rel_time, Expression):
                sub_rel_time = constraint.rel_time.substitute(substitutions)
                sub_constraint = constraint.copy()
                sub_constraint.rel_time = sub_rel_time
                sub_constraints.append(sub_constraint)
                if sub_rel_time is not constraint.rel_time:
                    constraints_changed = True

        if constraints_changed:
            new_sched = self.__class__(self["name"], self["operation_id"])
            for constraint in sub_constraints:
                new_sched.add_timing_constraint(
                    constraint.rel_time,
                    constraint.ref_schedulable,
                    constraint.ref_pt_new,
                    constraint.ref_pt,
                )
            return new_sched
        else:
...
class TimingConstraint:
    ...
    ref_pt_new: Literal["start", "center", "end"] | None
    ...
    def copy(self) -> TimingConstraint:
        """Copy this TimingConstraint to a fresh new instance."""
        return TimingConstraint(**self.data)
...
class CompiledSchedule(TimeableScheduleBase):
    """
    ...
    def __init__(self, schedule: TimeableSchedule) -> None:
        # validate the input data to ensure it is valid schedule data
        super().__init__()
    ...
    def clone(self) -> CompiledSchedule:
        """Clone this schedule into a separate independent schedule."""
        schedule = TimeableSchedule(self.data["name"], self.data["repetitions"], self.data)
        new_schedule = schedule.clone()
        return self.__class__(new_schedule)
    ...
    def substitute(
        ...
        schedule = TimeableSchedule(
            self.data["name"], self.data["repetitions"], deepcopy(self.data)
        )
        sub_schedule = schedule.substitute(substitutions)
        if schedule is not sub_schedule:
            return self.__class__(sub_schedule)
        else:
            return self
