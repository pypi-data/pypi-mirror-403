...
from qblox_scheduler.backends.qblox import compiler_container, constants
from qblox_scheduler.backends.qblox.crosstalk_compensation import (
...
from qblox_scheduler.backends.qblox.schedule import CompiledInstructions
from qblox_scheduler.backends.qblox.stack_pulses import stack_pulses
from qblox_scheduler.backends.qblox.switch_outputs import switch_outputs
...
from qblox_scheduler.compilation import _determine_absolute_timing, _normalize_absolute_timing
from qblox_scheduler.enums import DualThresholdedTriggerCountLabels, TriggerCondition
...
from qblox_scheduler.operations.hardware_operations import long_ramp_pulse, long_square_pulse
...
def _all_pulses_with_abs_times(
    ...
    if isinstance(operation, TimeableScheduleBase):
        for schedulable in operation.schedulables.values():
            ...
            _all_pulses_with_abs_times(inner_operation, time_offset + abs_time, accumulator)
...
def _validate_non_overlapping_long_pulses(
    schedule: TimeableSchedule, specs: list[LongPulseReplacementSpec]
) -> None:
    """
    ...
    """
    abs_times_and_operations: list[tuple[float, Operation]] = []
    _all_pulses_with_abs_times(schedule, 0, abs_times_and_operations)
    ...
    for i, (abs_time_op, op) in enumerate(abs_times_and_operations):
        if op.has_voltage_offset or any(spec.match(op.data["pulse_info"]) for spec in specs):
...
def _replace_long_pulses_recursively(
    ...
    elif any(spec.match(operation.data["pulse_info"]) for spec in specs):
        replacing_schedule = TimeableSchedule(operation.name)
        pulse_info = operation.data["pulse_info"]
        for spec in specs:
            if spec.match(pulse_info):
                new_pulse = spec.pulse_factory(
                    ...
                    **spec.extra_kwargs(pulse_info),
                )
                replacing_schedule.add(new_pulse, ref_pt="start")
            ...
            # No matches
            return None
        return _normalize_absolute_timing(_determine_absolute_timing(replacing_schedule))
...
def compile_long_pulses_to_awg_offsets(  # noqa: D417
    ...
    _validate_non_overlapping_long_pulses(schedule, specs)
...
def _get_module_cfg(
    ...
    for device_cfg in device_cfgs.values():
        if (
            ...
            return device_cfg.hardware_description.modules[module_idx]
    raise KeyError(f"Could not find module associated with port-clock {port_clock_str}")


def _get_module_type(
    port: str, clock: str, compilation_config: CompilationConfig
) -> InstrumentType:
    module_cfg = _get_module_cfg(port, clock, compilation_config)
    return InstrumentType(module_cfg.instrument_type)
...
def _insert_latch_reset(
    ...
    if isinstance(operation, TimeableScheduleBase):
        ...
        if acq_info["protocol"] in (
            ...
            if (feedback_trigger_label := acq_info.get("feedback_trigger_label")) is not None:
                at = (
...
def compile_conditional_playback(  # noqa: D417
    ...
    all_conditional_acqs_and_control_flows: list[tuple[float, Operation]] = []
    _all_conditional_acqs_and_control_flows_and_latch_reset(
...
def hardware_compile(
    ...
    # compilation function is called directly instead of through a `ScheduleCompiler`.
...
class QbloxHardwareCompilationConfig(HardwareCompilationConfig):
    ...
    config_type: Literal["QbloxHardwareCompilationConfig"] = "QbloxHardwareCompilationConfig"  # type: ignore[reportIncompatibleVariableOverride]
    """
    A reference to the
    ...
    compilation_passes: list[SimpleNodeConfig] = Field(
        default=[
            SimpleNodeConfig(
                name="switch_outputs",
                compilation_func=switch_outputs,
            ),
            SimpleNodeConfig(
    ...
    def _validate_connectivity_channel_names(self) -> QbloxHardwareCompilationConfig:
        ...
        for (
            ...
            module_index = int(module_idx_str.group(1))
            ...
            try:
                cluster_descr.modules[module_index].validate_channel_names(channel_names)
            except ValueError as exc:
                instrument_type = cluster_descr.modules[module_index].instrument_type
                valid_channels = cluster_descr.modules[module_index].get_valid_channels()
                ...
                raise QbloxModuleNotFoundError(
                    f"Module '{module_index}' of cluster '{cluster_name}' "
                    f"not found in the hardware description. "
                    f"Please ensure all modules mentioned in "
                    f"the connectivity are present in the hardware description. "
                ) from exception
        return self
    ...
    def _extract_instrument_compilation_configs(  # noqa: C901
        ...
        if self.hardware_options.modulation_frequencies is not None:
            for portclock, frequencies in self.hardware_options.modulation_frequencies.items():
        ...
        for option, pc_to_value_map in self.hardware_options.model_dump(exclude_unset=True).items():
            if pc_to_value_map is None:
                continue
            for pc, value in pc_to_value_map.items():
                # NOTE: Here we handle the QSM case in which hardware options are mapped
                #  by "clusterX.moduleY.io_channelZ", so `pc` is not really a port-clock.
                try:
                    cp = PartialChannelPath.from_path(pc)
                    if cp.cluster_name in cluster_configs:
                        clusters_hardware_options[cp.cluster_name][option][pc] = value
        ...
        for cluster_name, options in clusters_hardware_options.items():
            cluster_configs[cluster_name].hardware_options = QbloxHardwareOptions.model_validate(
                # Convert back to dict because otherwise unset keys will be filled with `{}`
                # rather than being left to `None`, which in turn breaks `exclude_unset=True`
                # behaviour in the tests (pydantic >= 2.12.0).
                dict(options)
            )
        ...
        for instrument_name, cfg in cluster_configs.items():
            ...
            for module_idx in cfg.hardware_description.modules:
                if isinstance(cfg.hardware_description.modules[module_idx], QSMDescription):
                    continue
    ...
    def _get_all_portclock_to_path_and_lo_name_to_path(  # noqa: C901, PLR0915
        self,
...
class _ClusterModuleCompilationConfig(ABC, DataStructure):
    ...
    portclock_to_path: dict[str, ChannelPath] = Field(default_factory=dict)
    """Mapping between portclocks and their associated channel name paths
    (e.g. cluster0.module1.complex_output_0)."""
    lo_to_path: dict[str, ChannelPath] = Field(default_factory=dict)
    ...
    sequencer_allowed_channels: ClassVar[dict[SequencerIndex, AllowedChannels]]
    ...
    def _sequencer_to_portclock(self) -> dict[SequencerIndex, str]:
        ...
        # restriction on qblox-scheduler, but not in the hardware.
        #
...
class _QSMCompilationConfig(_ClusterModuleCompilationConfig):
...
class _ClusterCompilationConfig(DataStructure):
    ...
    module_config_classes: ClassVar[dict[str, type[_ClusterModuleCompilationConfig]]] = {
    ...
    def _extract_module_compilation_configs(
        ...
        for option, values in self.hardware_options.model_dump(exclude_unset=True).items():
            for pc, option_value in values.items():
                ...
                try:
                    cp = PartialChannelPath.from_path(pc)
                    if cp.module_name is not None:
                        module_idx = cp.module_idx
                    else:
                        continue
                except ValueError:
...
def _add_support_input_channel_names(  # noqa: C901
...
def _add_clock_freqs_to_set_clock_frequency(
    schedule: TimeableSchedule, operation: Operation | TimeableSchedule | None = None
) -> None:
    ...
    elif pulse_info := operation["pulse_info"]:
...
def _get_pulse_start_ends(abs_time: float, operation: Operation) -> dict[str, tuple[float, float]]:
    ...
    pulse_info = operation["pulse_info"]
    prev_start, prev_end = pulse_start_ends_per_port[f"{pulse_info['port']}_{pulse_info['clock']}"]
