...
from qblox_scheduler.backends.qblox.helpers import is_square_pulse, to_grid_time
...
def stack_pulses(schedule: TimeableSchedule, config) -> TimeableSchedule:  # noqa: D417, ARG001, ANN001
    """
...
def _stack_pulses_by_interval(
    schedule: TimeableSchedule, pulses_by_interval: list[PulseInterval]
) -> TimeableSchedule:
...
def _stack_arbitrary_pulses(
    ...
    for key in interval.pulse_keys:
        ...
        check_unsupported_expression(*pulse_info.values(), operation_name=pulse.name)
        waveform = helpers.generate_waveform_data(pulse_info, sampling_rate=constants.SAMPLING_RATE)
        start_idx = round(
...
def _stack_square_pulses(
    ...
    for key in interval.pulse_keys:
        ...
        if combined_pulse is None:
            ...
            combined_pulse.data["pulse_info"]["t0"] = 0
            combined_pulse.data["pulse_info"]["duration"] = round(
                to_grid_time(interval.end_time - interval.start_time)
                * (1 / constants.SAMPLING_RATE),
                9,
            )
        else:
            combined_pulse.data["pulse_info"]["amp"] += pulse_info["amp"]
    _create_schedulable(schedule, interval.start_time, combined_pulse)


def _create_schedulable(
    schedule: TimeableSchedule, start_time: float, pulse: Operation | TimeableSchedule | None
) -> None:
