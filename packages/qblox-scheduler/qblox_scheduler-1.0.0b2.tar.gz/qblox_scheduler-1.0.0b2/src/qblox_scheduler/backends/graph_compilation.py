...
from qblox_scheduler.structure.model import (
    DataStructure,
    deserialize_class,
...
ConcreteHardwareCompilationConfig = TypeVar(
    "ConcreteHardwareCompilationConfig", bound=HardwareCompilationConfig
)
...
class SimpleNodeConfig(DataStructure):
    ...
    compilation_func: Callable[[TimeableSchedule, CompilationConfig], TimeableScheduleBase]
    """
    ...
    def _import_compilation_func_if_str(
        cls,
        fun: Callable[[TimeableSchedule, Any], TimeableSchedule],
    ) -> Callable[[TimeableSchedule, Any], TimeableSchedule]:
...
class OperationCompilationConfig(DataStructure):
    ...
    factory_func: Callable[..., Operation | TimeableSchedule] | None
    """
...
class DeviceCompilationConfig(DataStructure):
    ...
    compilation_passes: list[SimpleNodeConfig] = Field(  # type: ignore
        default=[
            {
                "name": "unroll_loops",
                "compilation_func": "qblox_scheduler.compilation._unroll_loops",
            },
            {
                "name": "merge_rz_gates",
                "compilation_func": "qblox_scheduler.compilation._merge_rz_gates",
            },
            {
                "name": "circuit_to_device",
                "compilation_func": "qblox_scheduler.backends.circuit_to_device."
            ...
            },
            {
                ...
                "compilation_func": "qblox_scheduler.pulse_compensation."
                + "process_compensation_pulses",
            },
            {
                "name": "determine_absolute_timing",
                "compilation_func": "qblox_scheduler.compilation._determine_absolute_timing",
            },
            {
                "name": "normalize_absolute_timing",
                "compilation_func": "qblox_scheduler.compilation._normalize_absolute_timing",
            },
        ],
...
class CompilationConfig(DataStructure):
    ...
    """
    hardware_compilation_config: ConcreteHardwareCompilationConfig | None = None
    """
    ...
    def _import_backend_if_str(
        cls,
        class_: type[ScheduleCompiler] | str,
    ) -> type[ScheduleCompiler]:
...
class CompilationNode:
    ...
    def _compilation_func(
        self, schedule: TimeableSchedule | DataStructure, config: DataStructure
    ) -> TimeableSchedule | DataStructure:
        """
        ...
        # a TimeableSchedule class but for more advanced compilers, a graph might want to do
...
class SimpleNode(CompilationNode):
    ...
    def _compilation_func(
        self, schedule: TimeableSchedule, config: DataStructure | dict
    ) -> TimeableSchedule:
...
class ScheduleCompiler(CompilationNode):
    """
    ...
    def compile(
        self,
        schedule: Schedule | TimeableSchedule,
        ...
        """
        # Imported here to prevent import cycles, necessary to minimize API breakage
        # with the old Schedule API.
        from qblox_scheduler.schedule import Schedule

        if isinstance(schedule, Schedule):
            if not schedule._experiment.steps:
                ...
                if timeable_schedule is None:
                    raise RuntimeError(
                        "Can not compile a schedule with untimed sections, "
                        "consider using the methods on Schedule instead."
                    )
                schedule = timeable_schedule
    ...
    def draw(self, ax: Axes = None, figsize: tuple[float, float] = (20, 10), **options) -> Axes:
        ...
        if self._task_graph is None:
            raise RuntimeError(
                "Task graph has not been initialized. Consider compiling a TimeableSchedule "
...
class SerialCompiler(ScheduleCompiler):
    ...
    def _compilation_func(
        self, schedule: TimeableSchedule, config: SerialCompilationConfig
    ) -> CompiledSchedule:
        """
        ...
        # single TimeableSchedule class, see
