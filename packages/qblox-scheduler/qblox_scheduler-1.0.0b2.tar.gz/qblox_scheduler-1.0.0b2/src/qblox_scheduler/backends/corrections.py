...
from qblox_scheduler.operations.pulse_library import NumericalPulse
...
def determine_relative_latency_corrections(
    ...
    schedule: TimeableSchedule | None = None,
) -> dict[str, float]:
    """
    ...
    if schedule is None:
        raise ValueError(
            ...
            f"requires the `TimeableSchedule` argument if "
            f"`hardware_cfg` is a `HardwareCompilationConfig`."
        )

    port_clocks = ["-".join(map(str, port_clock)) for port_clock in schedule.get_used_port_clocks()]
...
@overload
def apply_software_distortion_corrections(
    operation: TimeableSchedule, distortion_corrections: dict
) -> None: ...
def apply_software_distortion_corrections(
    operation: Operation | TimeableSchedule, distortion_corrections: dict
) -> Operation | TimeableSchedule | None:
    """
    ...
    **only** replacing the dict in ``"pulse_info"`` associated to that specific
    ...
    if isinstance(operation, TimeableScheduleBase):
        ...
        pulse_data = operation.data["pulse_info"]
        ...
        if portclock_key in distortion_corrections:
            ...
            corrected_pulse = distortion_correct_pulse(
                ...
                distortion_correction=SoftwareDistortionCorrection.model_validate(correction_cfg),
            )

            # TODO we have operation, corrected_pulse and substitute_operation. Which is not needed?
            operation.data["pulse_info"] = corrected_pulse.data["pulse_info"]
