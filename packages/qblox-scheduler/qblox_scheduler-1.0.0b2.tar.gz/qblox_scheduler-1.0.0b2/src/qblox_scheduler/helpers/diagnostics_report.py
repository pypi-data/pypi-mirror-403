...
from qblox_scheduler._version import __version__ as __scheduler_version__
...
def _generate_diagnostics_report(  # noqa: C901, PLR0915
    ...
    def _get_dependency_versions() -> list:
        dependencies = [
            dependency
            for dependency in distribution("qblox-scheduler").requires  # pyright: ignore[reportOptionalIterable]
        ...
        all_dependency_versions.append(f"qblox-scheduler: {__scheduler_version__}")
    ...
    report_zipfile = os.path.join(
        OutputDirectoryManager.get_datadir(), f"diagnostics_report_{report_type}_{uuid4()}.zip"
    )
