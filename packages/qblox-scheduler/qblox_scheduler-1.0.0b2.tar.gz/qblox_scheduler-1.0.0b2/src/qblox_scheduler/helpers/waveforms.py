...
from qblox_scheduler import math as math_helpers
from qblox_scheduler.helpers import schedule as schedule_helpers
...
def get_waveform_by_pulseid(
    ...
    for schedulable in schedule.schedulables.values():
        ...
        if operation["pulse_info"]:
            ...
            for weight in acq_info["waveforms"]:
                pulse_id = schedule_helpers.get_pulse_uuid(weight)
                pulseid_waveformfn_dict[pulse_id] = partial(get_waveform, pulse_info=weight)
...
def exec_waveform_function(wf_func: str, t: np.ndarray, pulse_info: dict) -> np.ndarray:
    ...
    """
    pulses_with_amp_parameter: list[str] = [
        "square",
        "ramp",
        "soft_square",
    ...
    if wf_func.startswith("qblox_scheduler.waveforms") and fn_name in pulses_with_amp_parameter:
        amp_param = "gain" if "interpolated_complex_waveform" in fn_name else "amp"
        if isinstance(pulse_info[amp_param], Sequence):
            amp_i, amp_q = pulse_info[amp_param]
            amp = amp_i + 1j * amp_q
        else:
            amp = pulse_info[amp_param]

        pulse_info_cpy = copy(pulse_info)
        pulse_info_cpy[amp_param] = amp

        if fn_name == "ramp":
            pulse_info_cpy["offset"] = pulse_info.get("offset", 0)
        waveform = exec_custom_waveform_function(wf_func, t, pulse_info_cpy)
    else:
...
def area_pulse(pulse: dict[str, Any], sampling_rate: float) -> float:
    ...
    if pulse["wf_func"] == "qblox_scheduler.waveforms.square":
