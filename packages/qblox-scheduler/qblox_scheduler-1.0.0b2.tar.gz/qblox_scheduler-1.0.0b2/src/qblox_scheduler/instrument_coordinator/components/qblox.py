...
@dataclass(frozen=True)
class _StaticDCModuleProperties(_StaticHardwareProperties):
    """Dataclass for storing configuration differences across Qblox devices."""

    settings_type: type[DCModuleSettings]
    """The settings dataclass to use that the hardware needs to configure to."""
...
_QSM_PROPERTIES = _StaticDCModuleProperties(
    settings_type=DCModuleSettings,
    number_of_sequencers=0,
    number_of_output_channels=8,
    number_of_input_channels=8,
    number_of_scope_acq_channels=0,
)
...
class _AnalogModuleComponent(_ModuleComponentBase):
    ...
    def _configure_sequencer_settings(self, seq_idx: int, settings: SequencerSettings) -> None:
        ...
        if settings.modulation_freq is not None:
...
class _AnalogReadoutComponent(_AnalogModuleComponent):
    ...
    def _configure_global_settings(self, settings: AnalogModuleSettings) -> None:  # type: ignore
        """
        Configures all settings that are set globally for the whole instrument.
...
class _QSMComponent(_ModuleComponentBase):
    """QSM specific InstrumentCoordinator component."""

    _hardware_properties = _QSM_PROPERTIES
    ...
    def _configure_global_settings(self, settings: DCModuleSettings) -> None:  # type: ignore[reportIncompatibleMethodOverride]
        """
        Configures all settings that are set globally for the whole instrument.
        ...
        """
        for param_name in ("source_mode", "measure_mode", "slew_rate", "integration_time"):
            setting_dict = getattr(settings, param_name)
            for channel_idx, setting_value in setting_dict.items():
                self._set_parameter(
                    self.instrument.io_channels[channel_idx], param_name, setting_value
                )

        if settings.safe_voltage_range:
            for channel_idx, (min_v, max_v) in settings.safe_voltage_range.items():
                self.instrument.io_channels[channel_idx].set_safe_voltage_range(min_v, max_v)
...
class _AcquisitionManagerBase(ABC):
    ...
    def _retrieve_acquisition_fully_append_recursive(
        ...
        if isinstance(node, AcqFullyAppendLoopNode):
            ...
            average_repetitions = (
                node.repetitions
                if ((node.repetitions is not None) and (node.bin_mode is BinMode.AVERAGE))
                else 1
            )
            for i in range(append_repetitions):
                for child in node.children:
                    new_dataset, qblox_acq_bin = self._retrieve_acquisition_fully_append_recursive(
                        ...
                        acq_index_offset=append_repetitions * acq_index_offset + i,
                        total_average_repetitions=total_average_repetitions * average_repetitions,
                    )
            ...
            data = self._protocol_to_bin_function(protocol)(
                ...
                total_average_repetitions=total_average_repetitions,
            )
    ...
    def retrieve_acquisition(self) -> Dataset:
        ...
        for sequencer_name, seq_hardware_mapping in self._acq_hardware_mapping.items():
            if (
                len(seq_hardware_mapping.non_fully_append) == 0
                and len(seq_hardware_mapping.fully_append) == 0
            ):
                continue
    ...
    def delete_acquisition_data(self) -> None:
        ...
        than once.
        """
        for seq_name, mapping in self._acq_hardware_mapping.items():
            if len(mapping.non_fully_append) or len(mapping.fully_append):
    ...
    def _get_trigger_count_bin(
        ...
        if np.isnan(counts[qblox_acq_bin]):
            ...
            return (
                self._repetitions * total_average_repetitions * counts[qblox_acq_bin]
                if (
                    sum_multiply_repetitions
                    and (
                        self._acq_channels_data[acq_channel].bin_mode
                        in (BinMode.SUM, BinMode.AVERAGE_APPEND)
                    )
                )
                else round(counts[qblox_acq_bin])
            )
    ...
    def _get_trigger_count_data(
        ...
        *,
        ...
        sequencer_name
            Sequencer.
        sum_multiply_repetitions
            Multiplies data by repetitions for the SUM and AVERAGE_APPEND bin mode.
        ...
        if self._acq_channels_data[acq_channel].bin_mode == BinMode.DISTRIBUTION:
            ...
            return self._get_binned_data(
                bin_data_function=partial(
                    self._get_trigger_count_bin, sum_multiply_repetitions=sum_multiply_repetitions
                ),
                hardware_retrieved_acquisitions=hardware_retrieved_acquisitions,
                acq_channel=acq_channel,
                seq_channel_hardware_mapping=seq_channel_hardware_mapping,
    ...
    def _get_thresholded_trigger_count_bin(
        self,
        qblox_acq_index: int,
        qblox_acq_bin: int,
        ...
        assert isinstance(thresholded_trigger_count_metadata, ThresholdedTriggerCountMetadata)
        count = self._get_trigger_count_bin(
            ...
            acq_duration=acq_duration,
            acq_channel=acq_channel,
            sum_multiply_repetitions=sum_multiply_repetitions,
            total_average_repetitions=total_average_repetitions,
        )
        count = round(count)
        ...
        if count == -1:
            return -1
        elif thresholded_trigger_count_metadata.condition == TriggerCondition.GREATER_THAN_EQUAL_TO:
            return int(count >= thresholded_trigger_count_metadata.threshold)
    ...
    def _get_thresholded_trigger_count_data(
        self,
        *,
        hardware_retrieved_acquisitions: dict,
        acq_channel: str,
        ...
        return self._get_binned_data(
            bin_data_function=partial(
                self._get_thresholded_trigger_count_bin,
                sum_multiply_repetitions=sum_multiply_repetitions,
            ),
            hardware_retrieved_acquisitions=hardware_retrieved_acquisitions,
            acq_channel=acq_channel,
...
class _QRMAcquisitionManager(_AcquisitionManagerBase):
    ...
    def _protocol_to_bin_function(self, protocol: str) -> Callable:
        match protocol:
            ...
            case "TriggerCount":
                return partial(self._get_trigger_count_bin, sum_multiply_repetitions=False)
            case "ThresholdedTriggerCount":
                return partial(
                    self._get_thresholded_trigger_count_bin, sum_multiply_repetitions=False
                )
            case _:
    ...
    def _protocol_to_acq_function_map(self, protocol: str) -> Callable:
        match protocol:
            ...
            case "ThresholdedTriggerCount":
                return partial(
                    self._get_thresholded_trigger_count_data, sum_multiply_repetitions=True
                )
            case "Trace":
    ...
    def _get_integration_weighted_separated_bin(
        ...
        total_average_repetitions: int,  # noqa: ARG002, unused parameter
    ) -> complex:
        bin_data = self._get_bin_data(hardware_retrieved_acquisitions, qblox_acq_index)
...
class _QTMAcquisitionManager(_AcquisitionManagerBase):
    ...
    def _protocol_to_bin_function(self, protocol: str) -> Callable:
        match protocol:
            ...
            case "TriggerCount":
                return partial(self._get_trigger_count_bin, sum_multiply_repetitions=True)
            case "ThresholdedTriggerCount":
                return partial(
                    self._get_thresholded_trigger_count_bin, sum_multiply_repetitions=True
                )
    ...
    def _split_timetag_trace_data_per_window(
        ...
        *first* pulse recorded in each window. This data is used to calculate the
        ...
        timetag_traces = []
        for ref_timetag, timetags_in_window in zip(timetags, parsed_scope_data, strict=False):
...
class ClusterComponent(base.InstrumentCoordinatorComponentBase):
    ...
    def __init__(self, instrument: Cluster) -> None:
        ...
        # is_qcm_type, is_qrm_type, is_rf_type, is_qtm_type, is_qrc_type, is_qsm_type
        module_type_map: dict[ComponentTypeProperties, type[_ModuleComponentBase]] = {
            ComponentTypeProperties(True, False, False, False, False, False): _QCMComponent,
            ComponentTypeProperties(True, False, True, False, False, False): _QCMRFComponent,
            ComponentTypeProperties(False, True, False, False, False, False): _QRMComponent,
            ComponentTypeProperties(False, True, True, False, False, False): _QRMRFComponent,
            ComponentTypeProperties(False, False, True, False, True, False): _QRCComponent,
            ComponentTypeProperties(False, False, False, True, False, False): _QTMComponent,
            ComponentTypeProperties(False, False, False, False, False, True): _QSMComponent,
        }
            ...
            try:
                icc_class = module_type_map[
                    ComponentTypeProperties(
                        ...
                        getattr(instrument_module, "is_qsm_type", False),
                    )
                ]
            except KeyError:
                continue
    ...
    def get_module_descriptions(self) -> dict[int, ClusterModuleDescription]:
        ...
        for module in self._cluster_modules.values():
            ...
            elif type(module) is _QSMComponent:
                module_types[index] = QSMDescription()
        return module_types
