...
def get_static_version_info(version_file=STATIC_VERSION_FILE):
    ...
    with open(os.path.join(package_root, version_file), "rb") as f:
        exec(f.read(), {}, version_info)  # noqa: S102
    return version_info
...
def get_version_from_git():
    ...
    for opts in (["--first-parent"], []):
        try:
            p = subprocess.Popen(  # noqa: S603
                [git_path, "describe", "--long", "--always"] + opts,
                cwd=package_root,
    ...
    try:
        p = subprocess.Popen([git_path, "diff", "--quiet"], cwd=package_root)  # noqa: S603
    except OSError:
...
def get_version_from_git_archive(version_info):
    ...
    version_tags = {r[len(version_tag) :] for r in refs if r.startswith(version_tag)}
    if version_tags:
...
def _write_version(fname) -> None:
    ...
    with open(fname, "w") as f:
        f.write(f"# This file has been created by setup.py.\nversion = '{__version__}'\n")
