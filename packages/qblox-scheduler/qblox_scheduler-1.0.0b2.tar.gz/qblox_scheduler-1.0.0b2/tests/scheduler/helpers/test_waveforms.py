...
from qblox_scheduler.helpers.schedule import get_pulse_uuid
...
@pytest.mark.parametrize(
    ...
    [
        ("qblox_scheduler.waveforms.square", 2.4e9),
        ("qblox_scheduler.waveforms.ramp", 2.4e9),
        ("qblox_scheduler.waveforms.soft_square", 2.4e9),
        ("qblox_scheduler.waveforms.drag", 2.4e9),
    ],
)
def test_get_waveform(mocker: MockerFixture, wf_func: str, sampling_rate: float) -> None:
    # Arrange
    mock = mocker.patch("qblox_scheduler.helpers.waveforms.exec_waveform_function", return_value=[])
...
def test_get_waveform_by_pulseid(
    schedule_with_pulse_info: TimeableSchedule,
) -> None:
    # Arrange
    operation_id = list(schedule_with_pulse_info.schedulables.values())[0]["operation_id"]
    pulse_info_0 = schedule_with_pulse_info.operations[operation_id]["pulse_info"]
...
def test_get_waveform_by_pulseid_are_unique(
    ...
    # Arrange
    schedule = TimeableSchedule("my-schedule")
    schedule.add(X90("q0"))
    schedule.add(X90("q0"))
    ...
    pulse_info_0 = schedule.operations[operation_id]["pulse_info"]
    pulse_id = get_pulse_uuid(pulse_info_0)
    expected_keys: list[int] = [pulse_id]
...
def test_get_waveform_by_pulseid_empty(empty_schedule: TimeableSchedule) -> None:
    # Arrange
    # Act
...
def test_exec_custom_waveform_function(mocker: MockerFixture) -> None:
    ...
    mocker.patch(
        ...
        # but ...
        "qblox_scheduler.helpers.waveforms.import_python_object_from_string",
        return_value=mock,
    )
