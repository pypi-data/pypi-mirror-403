...
from qblox_scheduler.operations.gate_library import X90, X
...
def test_operation_duration_single_pulse() -> None:
    ...
    gauss = GaussPulse(amplitude=0.2, phase=27, duration=200e-9, clock="cl:01", port="p.01")
...
def test_operation_add_pulse() -> None:
    ...
    assert len(dgp1["pulse_info"]) > 0
    with pytest.raises(ValueError, match="device representation"):
        dgp1.add_device_representation(dgp1)
    ...
    x90.add_device_representation(dgp)
    assert len(x90["pulse_info"]) > 0

    gauss = GaussPulse(amplitude=0.2, phase=27, duration=200e-9, clock="cl:01", port="p.01")
    with pytest.raises(ValueError, match="device representation"):
        x90.add_device_representation(gauss)
...
def test_decompose_long_square_pulse() -> None:
    ...
    for single_duration, sum_ in zip([False, True], sums, strict=False):
        ...
        assert sum(pulse["pulse_info"]["duration"] for pulse in pulses) == pytest.approx(sum_)
    ...
    for single_duration in [False, True]:
        ...
        assert len(pulses) == int(duration // duration_sq_max) + bool(duration % duration_sq_max)
        assert sum(pulse["pulse_info"]["duration"] for pulse in pulses) == pytest.approx(duration)
...
def test_complex_square_pulse(mock_setup_basic_transmon_with_standard_params):
    ...
    assert pulse0["pulse_info"]["amp"] == 1 + 1j
    ...
    operations = list(pulse1.operations.values())
    assert operations[0]["pulse_info"]["offset_path_I"] == 1
    assert operations[0]["pulse_info"]["offset_path_Q"] == 1
    assert operations[1]["pulse_info"]["offset_path_I"] == 0
    assert operations[1]["pulse_info"]["offset_path_Q"] == 0
    assert operations[2]["pulse_info"]["amp"] == 1 + 1j
...
def test_dc_compensation_pulse_amp() -> None:
    ...
    TestCase().assertAlmostEqual(pulse2.data["pulse_info"]["amp"], -1.5)
...
def test_dc_compensation_pulse_modulated() -> None:
    ...
    assert pulse.data["pulse_info"]["duration"] == 0.0
...
def test_dc_compensation_pulse_duration() -> None:
    ...
    TestCase().assertAlmostEqual(pulse2.data["pulse_info"]["duration"], 1.5e-8)
    TestCase().assertAlmostEqual(pulse2.data["pulse_info"]["amp"], -1.0)
...
def test_dragpulse_beta(mock_setup_basic_transmon_with_standard_params):
    mock_setup_basic_transmon_with_standard_params["q0"].rxy.amp180 = 0.2
    mock_setup_basic_transmon_with_standard_params["q0"].rxy.beta = 5e-11

    sched = TimeableSchedule("Test DRAG Pulse")
    sched.add(X("q0"))
    ...
    beta = list(compiled_sched.operations.values())[0].data["pulse_info"].get("beta")
    assert beta == mock_setup_basic_transmon_with_standard_params["q0"].rxy.beta, (
        "The amplification of the derivative DRAG pulse is not equal to the beta parameter"
    )
