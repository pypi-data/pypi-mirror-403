...
from qblox_scheduler.operations.conditional_reset import ConditionalReset
...
from qblox_scheduler.operations.pulse_compensation_library import (
    PulseCompensation,
)
...
from qblox_scheduler.operations.spin_library import SpinInit
...
def test_compile_all_gates_example_transmon_cfg(device_cfg_transmon_example):
    ...
    sched = TimeableSchedule("Test schedule")
...
def test_compile_symmetric_gate_distinguished_qubits(
    mock_setup_basic_transmon, get_subschedule_operation
):
    """
    ...
    edge_q3_q2 = CompositeSquareEdge(
        parent_element=mock_setup_basic_transmon[q3], child_element=mock_setup_basic_transmon[q2]
    )
    ...
    flux_pulse = get_subschedule_operation(compiled_schedule_q2_q3, [0, 0])
    assert flux_pulse["pulse_info"]["port"] == "q2:fl"

    compiled_schedule_q3_q2 = compile_schedule(q3, q2)
    flux_pulse = get_subschedule_operation(compiled_schedule_q3_q2, [0, 0])
    assert flux_pulse["pulse_info"]["port"] == "q3:fl"
...
def test_measurement_compile(device_cfg_transmon_example, get_subschedule_operation):
    ...
    assert m0_q0_acq["acq_channel"] == 0
    assert m0_q0_acq["acq_index"] is None

    m0_q1_acq = get_subschedule_operation(new_dev_sched, [0, 1, 2])["acquisition_info"]
    assert m0_q1_acq["acq_channel"] == 1
    assert m0_q1_acq["acq_index"] is None

    m1_acq = get_subschedule_operation(new_dev_sched, [1, 2])["acquisition_info"]
    assert m1_acq["acq_channel"] == 0
    assert m1_acq["acq_index"] is None

    m2_acq = get_subschedule_operation(new_dev_sched, [2, 2])["acquisition_info"]
    assert m2_acq["acq_channel"] == 1
    assert m2_acq["acq_index"] is None

    m3_acq = get_subschedule_operation(new_dev_sched, [3, 2])["acquisition_info"]
    assert m3_acq["acq_channel"] == 2
    assert m3_acq["acq_index"] is None

    m4_q0_acq = get_subschedule_operation(new_dev_sched, [4, 0, 2])["acquisition_info"]
    assert m4_q0_acq["acq_channel"] == 0
    assert m4_q0_acq["acq_index"] is None

    m4_q1_acq = get_subschedule_operation(new_dev_sched, [4, 1, 2])["acquisition_info"]
    assert m4_q1_acq["acq_channel"] == 1
    assert m4_q1_acq["acq_index"] is None
...
def test_clock_not_defined_raises():
    simple_config = DeviceCompilationConfig(
        ...
        elements={
            "q0": {
                "measure": {
                    "factory_func": "qblox_scheduler.operations."
...
def test_operation_not_in_config_raises_custom():
    ...
    sched = TimeableSchedule("Test missing single q op")
    sched.add(Reset("q0"))
    with pytest.raises(ConfigKeyError):
        _ = compile_circuit_to_device_with_config_validation(
            sched,
    ...
    sched = TimeableSchedule("Test schedule mux missing op")
    ...
    sched = TimeableSchedule("Test missing 2Q op")
    sched.add(CZ("q0", "q1"))
    with pytest.raises(ConfigKeyError):
        _ = compile_circuit_to_device_with_config_validation(
            sched,
...
def test_config_with_callables():
    ...
    sched = TimeableSchedule("Test callable op")
    sched.add(Reset("q0", "q1"))
    _ = compile_circuit_to_device_with_config_validation(
        sched,
        config=SerialCompilationConfig(name="test", device_compilation_config=simple_config),
...
def test_valid_clock_in_schedule():
    ...
    # Arrange
    schedule, _device_cfg, operation = schedule_for_clock_tests()
...
def test_set_reference_magnitude(mock_setup_basic_transmon, get_subschedule_operation):
    ...
    assert operations_dict_with_repr_keys["Rxy(theta=90, phi=0, qubit='q2')"]["pulse_info"][
        "reference_magnitude"
    ] == ReferenceMagnitude(0.5, "V")
    assert operations_dict_with_repr_keys["Rxy(theta=12, phi=0, qubit='q3')"]["pulse_info"][
        "reference_magnitude"
    ...
    assert measure_q2_operations_dict_with_repr_keys[
        (
            ...
            "port='q2:res',"
            "clock='q2.ro',"
            "reference_magnitude=ReferenceMagnitude(value=20.0, unit='dBm'),t0=0)"
        )
    ]["pulse_info"]["reference_magnitude"] == ReferenceMagnitude(20, "dBm")
    ...
    assert (
        ...
        ]["pulse_info"]["reference_magnitude"]
        is None
    )
...
def test_clock_resources_and_subschedules_compiles():
    schedule = TimeableSchedule("test clock resource subschedule")
...
def test_long_time_trace_protocol(
    ...
    schedule = TimeableSchedule("LongTimeTrace")
...
def test_device_overrides_dispersive_measure(
    ...
    duration_0 = acq_operation_0["acquisition_info"]["duration"]
    acq_operation_1 = get_subschedule_operation(compiled_schedule, [1, 2])
    duration_1 = acq_operation_1["acquisition_info"]["duration"]
...
def test_device_overrides_multiple_levels_hamilton(
    ...
    duration_0 = operation_0["pulse_info"]["duration"]
    operation_1 = get_subschedule_operation(compiled_schedule, [1, 1])
    duration_1 = operation_1["pulse_info"]["duration"]
...
def test_compile_spin_init():
    ...
    edge_q2_q3 = SpinEdge(parent_element=q2, child_element=q3)
