...
def test_trigger_count_bin_mode_average_append_qrm(
    mock_setup_basic_nv,
    make_cluster_component,
    mocker,
):
    ...
    dataarray = xr.DataArray(
        ...
        coords={
            "acq_index_0": [0, 1, 2, 3],
            "index": ("acq_index_0", [0, 0, 0, 1]),
            "loop_repetition_0": ("acq_index_0", [0, 1, 2, 0]),
            "rep": ("acq_index_0", [0, 1, 2, np.nan]),
        },
        attrs={"acq_protocol": "TriggerCount", "acq_index_dim_name": "acq_index_0"},
    )
    expected_dataset = xr.Dataset({0: dataarray})
    ...
    with sched.loop(arange(0, 3, 1, dtype=DType.NUMBER)) as rep:
        sched.add(
            TriggerCount(
                duration=1e-6,
                port="qe0:optical_readout",
...
def test_trigger_count_bin_mode_average_append_qtm(
    mock_setup_basic_nv,
    make_cluster_component,
    mocker,
):
        ...
        "0": {
            ...
            "acquisition": {
                "bins": {
                    ...
                    "avg_cnt": [0.0, 0.0, 0.0, 0.0],
                }
            },
        }
    }

    schedule_repetitions = 10
    dataarray = xr.DataArray(
        [60, 50, 40, 30],
        dims=["acq_index_0"],
        coords={
    ...
    sched = TimeableSchedule("digital_pulse_and_acq", repetitions=schedule_repetitions)
    with sched.loop(arange(0, 3, 1, dtype=DType.NUMBER)) as rep:
        sched.add(
            TriggerCount(
                duration=1e-6,
