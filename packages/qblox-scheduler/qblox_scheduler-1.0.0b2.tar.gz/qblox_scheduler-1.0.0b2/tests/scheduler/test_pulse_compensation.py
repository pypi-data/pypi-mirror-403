...
from qblox_scheduler.pulse_compensation import (
...
class UnsupportedControlFlowOperation(ControlFlowOperation):
    ...
    @body.setter
    def body(self, value: Union[Operation, TimeableSchedule]) -> None:
        pass
...
def test_insert_compensation_pulses(  # noqa: PLR0915
    ...
    assert isinstance(compensated_subschedule, TimeableSchedule)
    ...
    if compensation_pulse_q0["pulse_info"]["port"] == "q1:mw":
    ...
    assert len(compensation_pulse_q0["pulse_info"]) > 0
    assert compensation_pulse_q0["pulse_info"]["wf_func"] == "qblox_scheduler.waveforms.square"
    assert compensation_pulse_q0["pulse_info"]["reference_magnitude"] is None
    assert compensation_pulse_q0["pulse_info"]["t0"] == 0
    assert compensation_pulse_q0["pulse_info"]["port"] == "q0:mw"
    assert math.isclose(compensation_pulse_q0["pulse_info"]["amp"], -0.5910714285714285)
    assert math.isclose(compensation_pulse_q0["pulse_info"]["duration"], 2.8e-8)

    assert len(compensation_pulse_q1["pulse_info"]) > 0
    assert compensation_pulse_q1["pulse_info"]["wf_func"] == "qblox_scheduler.waveforms.square"
    assert compensation_pulse_q1["pulse_info"]["reference_magnitude"] is None
    assert compensation_pulse_q1["pulse_info"]["t0"] == 0
    assert compensation_pulse_q1["pulse_info"]["port"] == "q1:mw"
    assert math.isclose(compensation_pulse_q1["pulse_info"]["amp"], -0.5625)
    assert math.isclose(compensation_pulse_q1["pulse_info"]["duration"], 4e-9)
    ...
    assert len(compiled_x_op["pulse_info"]) > 0
    assert compiled_x_op["pulse_info"]["wf_func"] == "qblox_scheduler.waveforms.drag"
