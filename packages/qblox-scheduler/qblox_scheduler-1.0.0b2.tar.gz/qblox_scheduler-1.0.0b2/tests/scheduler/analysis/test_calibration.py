...
def test_rotate_to_calibrated_axis():
    ...
    np.testing.assert_array_almost_equal(corrected_data, np.array([0.0, 1.0]), decimal=15)
...
def test_has_calibration_points_outside_cal(angles):
    ...
    for points, has_cal in zip(
        [data, np.concatenate((data, [center_0, center_1]))], [False, True], strict=False
    ):
        for angle in angles:
...
def test_has_calibration_points_points_only_single_cluster(angles):
    ...
    for points, has_cal in zip(
        [data, np.concatenate((data, [center_1]))], [False, True], strict=False
    ):
        for angle in angles:
            assert has_calibration_points(points * np.exp(1j * angle)) == has_cal
...
def test_has_calibration_points_datasets(tmp_analysis_test_data_dir, angles, tuid, has_cal):
