...
from qblox_scheduler.analysis import spectroscopy_analysis as sa
...
def test_raw_data_not_in_processed_dataset(analysis_and_ref, tmp_analysis_test_data_dir):
    OutputDirectoryManager.set_datadir(tmp_analysis_test_data_dir)
    analysis, _ = analysis_and_ref

    container = Path(AnalysisDataContainer.locate_experiment_container(analysis.tuid))
    file_path = container / "analysis_ResonatorSpectroscopyAnalysis" / "dataset_processed.hdf5"
