...
def test_t1_quantities_of_interest(t1_analysis_no_cal_points):
    ...
    assert t1_analysis_no_cal_points.quantities_of_interest["T1"].nominal_value == approx(
        exp_t1, abs=5 * t1_analysis_no_cal_points.quantities_of_interest["T1"].std_dev
    )


def test_t1_analysis_with_cal_points(tmp_analysis_test_data_dir):
    """
    Used to run the analysis a single time and run unit tests against the created
    analysis object.
    """
...
def test_echo_analysis_no_cal(tmp_analysis_test_data_dir):
    OutputDirectoryManager.set_datadir(tmp_analysis_test_data_dir)
    ...
    for fit_name, fit_result in analysis_obj.fit_results.items():
        loaded_fit_result = EchoAnalysis.load_fit_result(tuid=analysis_obj.tuid, fit_name=fit_name)

        assert loaded_fit_result.params == fit_result.params


def test_echo_analysis_with_cal(tmp_analysis_test_data_dir):
    OutputDirectoryManager.set_datadir(tmp_analysis_test_data_dir)
...
def test_ramsey_no_cal_generated(tmp_analysis_test_data_dir):
    """test that the right figures get created"""
    OutputDirectoryManager.set_datadir(tmp_analysis_test_data_dir)
    analysis = RamseyAnalysis(tuid="20210422-104958-297-7d6034").run(artificial_detuning=250e3)
    ...
    assert analysis.quantities_of_interest["fitted_detuning"].nominal_value == pytest.approx(
...
@pytest.fixture(scope="module", autouse=True)
def ramsey_analysis_qubit_freq(tmp_analysis_test_data_dir):
    OutputDirectoryManager.set_datadir(tmp_analysis_test_data_dir)
...
def test_ramsey_load_fit_results(ramsey_analysis_qubit_freq, tmp_analysis_test_data_dir):
    OutputDirectoryManager.set_datadir(tmp_analysis_test_data_dir)
...
def test_quantities_of_interest_qubit_freq(ramsey_analysis_qubit_freq):
    ...
    assert isinstance(ramsey_analysis_qubit_freq.quantities_of_interest["T2*"], Variable)
    assert isinstance(
    ...
    # Tests that the fitted values are correct (to within 5 standard deviations)
    assert ramsey_analysis_qubit_freq.quantities_of_interest["T2*"].nominal_value == pytest.approx(
        values["T2*"],
        rel=0.01,
    )
...
def test_ramsey_analysis_with_cal(tmp_analysis_test_data_dir):
    OutputDirectoryManager.set_datadir(tmp_analysis_test_data_dir)
...
def test_ramsey_analysis_with_cal_qubit_freq_reporting(tmp_analysis_test_data_dir):
    OutputDirectoryManager.set_datadir(tmp_analysis_test_data_dir)
...
def allxy_analysis_obj(tmp_analysis_test_data_dir):
    OutputDirectoryManager.set_datadir(tmp_analysis_test_data_dir)
...
def test_allxy_analysis_invalid_data(caplog, tmp_analysis_test_data_dir):
    OutputDirectoryManager.set_datadir(tmp_analysis_test_data_dir)
    ...
    assert (
        ...
        f"{analysis!r}" in record.getMessage()
    )
    exception = record.exc_info[1]
    assert isinstance(exception, ValueError)
    assert exception.args[0].startswith("Invalid dataset. The number of calibration points")


def test_allxy_load_fit_results_missing(tmp_analysis_test_data_dir):
    OutputDirectoryManager.set_datadir(tmp_analysis_test_data_dir)

    with pytest.raises(FileNotFoundError, match="No fit results found for this analysis."):
        AllXYAnalysis.load_fit_result(tuid="20210419-173649-456-23c5f3", fit_name="fit_name")


@pytest.fixture(scope="session", autouse=True)
def rabi_analysis_obj(tmp_analysis_test_data_dir):
    OutputDirectoryManager.set_datadir(tmp_analysis_test_data_dir)
    rabi_analysis_obj = RabiAnalysis(tuid="20210419-153127-883-fa4508").run()
    return rabi_analysis_obj


def test_rabi_load_fit_results(rabi_analysis_obj, tmp_analysis_test_data_dir):
    OutputDirectoryManager.set_datadir(tmp_analysis_test_data_dir)
...
def test_quantities_of_interest(rabi_analysis_obj):
    ...
    assert isinstance(rabi_analysis_obj.quantities_of_interest["Pi-pulse amplitude"], Variable)
    # Tests that the fitted values are correct (to within 5 standard deviations)
    assert rabi_analysis_obj.quantities_of_interest["Pi-pulse amplitude"].nominal_value == approx(
        498.8e-3,
        abs=5e-3,
    )
...
def test_quantities_of_interest_negative_amp(tmp_analysis_test_data_dir):
    OutputDirectoryManager.set_datadir(tmp_analysis_test_data_dir)
    ...
    assert analysis.quantities_of_interest["Pi-pulse amplitude"].nominal_value == approx(
        250e-3,
        abs=10e-3,
    )
...
@pytest.fixture(scope="session", autouse=True)
def rabi_analysis_obj_bad_fit(tmp_analysis_test_data_dir):
    """
    ...
    # TODO: No reliable indicator for bad fits (https://gitlab.com/quantify-os/quantify-core/-/issues/245).
    analysis = RabiAnalysis(dataset=dataset, settings_overwrite={"mpl_fig_formats": []}).run()
