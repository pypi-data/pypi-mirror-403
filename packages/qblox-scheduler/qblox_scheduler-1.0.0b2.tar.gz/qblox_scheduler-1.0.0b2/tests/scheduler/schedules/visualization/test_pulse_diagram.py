...
from qblox_scheduler.operations.gate_library import CZ, Measure, Reset, Rxy
...
def test_pulse_diagram_large_amplitude_warns() -> None:
    schedule = TimeableSchedule("test")
    schedule.add(SquarePulse(amp=1.2, duration=4e-6, port="SDP"))
    schedule = _determine_absolute_timing(schedule=schedule)

    _, ax = plt.subplots()
    with pytest.warns(UserWarning, match="Waveform amplitude exceeds maximum of 1"):
        schedule.plot_pulse_diagram(combine_waveforms_on_same_port=True, ax=ax)
...
def test_sample_schedule_sweeps() -> None:
    schedule = TimeableSchedule()
    with schedule.loop(linspace(1.0, -1.0, 3, dtype=DType.AMPLITUDE)) as amp:
        schedule.add(SquarePulse(amp=amp, duration=10e-9, port="qrm:in0"))
    schedule.add(IdlePulse(4e-9))

    schedule = _determine_absolute_timing(schedule=schedule)

    waveforms = sample_schedule(schedule)

    np.testing.assert_array_almost_equal(
        waveforms["qrm:in0"][0][0].time,
        np.array(
            [
                -1e-11,
                0.0,
    ...
    np.testing.assert_array_almost_equal(
        waveforms["qrm:in0"][0][0].signal,
        np.array([0.0, 1.0, 1.0, 0.0]),
        decimal=9,
    )

    np.testing.assert_array_almost_equal(
        waveforms["qrm:in0"][0][1].time,
        np.array(
            [
                9.99e-9,
                1.0e-8,
    ...
    np.testing.assert_array_almost_equal(
        waveforms["qrm:in0"][0][1].signal,
        np.array([0.0, 0.0, 0.0, 0.0]),
        decimal=9,
    )

    np.testing.assert_array_almost_equal(
        waveforms["qrm:in0"][0][2].time,
        np.array(
            [
                1.999e-8,
                2.0e-8,
...
def test_schedule_repetitions_are_not_sampled():
    schedule = TimeableSchedule(repetitions=1024)
