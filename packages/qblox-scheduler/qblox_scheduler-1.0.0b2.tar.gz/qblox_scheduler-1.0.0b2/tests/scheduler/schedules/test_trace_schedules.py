...
from qblox_scheduler.schedules import trace_schedules
...
def test_trace_schedule() -> None:
    ...
    assert idle_pulse_op["pulse_info"]["duration"] == init_duration

    # SquarePulse
    square_pulse_op = schedule.operations[list(schedule.schedulables.values())[1]["operation_id"]]
    assert square_pulse_op["pulse_info"]["duration"] == pulse_duration
    assert (
    ...
    assert trace_acq_op["acquisition_info"]["duration"] == integration_time
    assert (
...
def test_two_tone_trace_schedule() -> None:
    ...
    pulse_info = square_pulse_op["pulse_info"]
    ...
    acq_info = trace_op["acquisition_info"]
