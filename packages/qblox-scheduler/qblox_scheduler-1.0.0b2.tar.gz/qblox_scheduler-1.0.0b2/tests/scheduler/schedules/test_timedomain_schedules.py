...
from qblox_scheduler.schedules import timedomain_schedules as ts
...
class TestRabiSched(_CompilesAllBackends):
    ...
    def test_rabi_pulse_ops(self):
        ...
        rabi_pulse = self.uncomp_sched.operations[rabi_op_hash]["pulse_info"]
        assert rabi_pulse["amplitude"] == 0.2
        assert rabi_pulse["beta"] == 0
    ...
    def test_batched_variant_single_val(self, device_compile_config_basic_transmon):
        ...
        rabi_pulse = sched.operations[rabi_op_hash]["pulse_info"]
        assert rabi_pulse["amplitude"] == 0.5
        assert rabi_pulse["beta"] == 0
        assert rabi_pulse["duration"] == 20e-9
    ...
    def test_batched_variant_amps(self, device_compile_config_basic_transmon):
        ...
        for i, exp_amp in enumerate(amps):
            ...
            rabi_pulse = sched.operations[rabi_op_hash]["pulse_info"]
            assert rabi_pulse["amplitude"] == exp_amp
            assert rabi_pulse["beta"] == 0
            assert rabi_pulse["duration"] == 20e-9
    ...
    def test_correct_inference_of_port_clock(self):
        ...
        assert rabi_op["pulse_info"]["port"] == "q0:mw"
        assert rabi_op["pulse_info"]["clock"] == "q0.01"
...
class TestCPMGSched(_CompilesAllBackends):
    ...
    def test_timing(self):
        ...
        for key in self.uncomp_sched.operations:
            if isinstance(self.uncomp_sched.operations[key], TimeableSchedule):
                ...
                for sub_schedulable in sub_sched.schedulables.values():
                    ...
                    if operation["name"] == "IdlePulse":
                        sub_sched_duration += operation["pulse_info"]["duration"]
