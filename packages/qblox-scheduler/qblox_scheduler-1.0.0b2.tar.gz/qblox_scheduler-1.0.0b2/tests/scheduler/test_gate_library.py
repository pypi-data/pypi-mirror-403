...
from qblox_scheduler.operations.nv_native_library import ChargeReset
...
from qblox_scheduler.schedules.schedule import Schedulable, TimeableSchedule
from qblox_scheduler.schemas.examples import utils


def test_schedule_add_schedulables() -> None:
    sched = TimeableSchedule("my exp")
    test_lab = "test label"
...
def test_schedule_add_schedulables_one_missing() -> None:
    ...
    with pytest.raises(ValueError):
        _validate_schedulable_references(sched, graph)
        TimeableSchedule.is_valid(sched)
...
def test_schedule_add_schedulables_missing_reference() -> None:
    ...
    # specifying a Schedulable that is not in the TimeableSchedule should work
    new_sched = TimeableSchedule("redundant")
...
class TestGateLevelOperation:
    ...
    def test__str__(self, operation: Operation) -> None:
        ...
        is an instance of the operation type.
        """
    ...
    def test__repr__modify_not_equal(self, operation: Operation) -> None:
        ...
        # Act
        obj.data["pulse_info"].update({"clock": "q0.01"})
