# coding: utf-8

"""
    Arthur Scope

    No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

    The version of the OpenAPI document: 0.1.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from arthur_client.api_bindings.models.resource_list_custom_aggregation_test_spec import ResourceListCustomAggregationTestSpec

class TestResourceListCustomAggregationTestSpec(unittest.TestCase):
    """ResourceListCustomAggregationTestSpec unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> ResourceListCustomAggregationTestSpec:
        """Test ResourceListCustomAggregationTestSpec
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `ResourceListCustomAggregationTestSpec`
        """
        model = ResourceListCustomAggregationTestSpec()
        if include_optional:
            return ResourceListCustomAggregationTestSpec(
                records = [
                    arthur_client.api_bindings.models.custom_aggregation_test_spec.CustomAggregationTestSpec(
                        reported_aggregations = [
                            arthur_client.api_bindings.models.reported_custom_aggregation.ReportedCustomAggregation(
                                metric_name = '', 
                                description = '', 
                                value_column = '', 
                                timestamp_column = '', 
                                metric_kind = 'sketch', 
                                dimension_columns = [
                                    ''
                                    ], )
                            ], 
                        aggregate_args = [
                            null
                            ], 
                        sql = '', 
                        name = '', 
                        description = '', 
                        aggregation_arg_configuration = [
                            arthur_client.api_bindings.models.metrics_arg_spec.MetricsArgSpec(
                                arg_key = '', 
                                arg_value = null, )
                            ], 
                        start_timestamp = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                        end_timestamp = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                        dataset_id = '', 
                        id = '', 
                        authored_by = arthur_client.api_bindings.models.user.User(
                            created_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                            updated_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                            organization_id = '', 
                            id = '', 
                            first_name = '', 
                            last_name = '', 
                            email = '', 
                            picture = '', 
                            user_type = 'user', 
                            data_plane_id = '', 
                            client_id = '', 
                            organization_name = '', ), 
                        workspace_id = '', 
                        test_status = 'completed', 
                        created_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                        job_id = '', )
                    ],
                pagination = arthur_client.api_bindings.models.pagination.Pagination(
                    page = 56, 
                    page_size = 56, 
                    total_pages = 56, 
                    total_records = 56, )
            )
        else:
            return ResourceListCustomAggregationTestSpec(
                records = [
                    arthur_client.api_bindings.models.custom_aggregation_test_spec.CustomAggregationTestSpec(
                        reported_aggregations = [
                            arthur_client.api_bindings.models.reported_custom_aggregation.ReportedCustomAggregation(
                                metric_name = '', 
                                description = '', 
                                value_column = '', 
                                timestamp_column = '', 
                                metric_kind = 'sketch', 
                                dimension_columns = [
                                    ''
                                    ], )
                            ], 
                        aggregate_args = [
                            null
                            ], 
                        sql = '', 
                        name = '', 
                        description = '', 
                        aggregation_arg_configuration = [
                            arthur_client.api_bindings.models.metrics_arg_spec.MetricsArgSpec(
                                arg_key = '', 
                                arg_value = null, )
                            ], 
                        start_timestamp = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                        end_timestamp = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                        dataset_id = '', 
                        id = '', 
                        authored_by = arthur_client.api_bindings.models.user.User(
                            created_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                            updated_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                            organization_id = '', 
                            id = '', 
                            first_name = '', 
                            last_name = '', 
                            email = '', 
                            picture = '', 
                            user_type = 'user', 
                            data_plane_id = '', 
                            client_id = '', 
                            organization_name = '', ), 
                        workspace_id = '', 
                        test_status = 'completed', 
                        created_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                        job_id = '', )
                    ],
                pagination = arthur_client.api_bindings.models.pagination.Pagination(
                    page = 56, 
                    page_size = 56, 
                    total_pages = 56, 
                    total_records = 56, ),
        )
        """

    def testResourceListCustomAggregationTestSpec(self):
        """Test ResourceListCustomAggregationTestSpec"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
