...
class Listen(object):
    ...
    def __init__(
        ...
        entity_id: str,
        code: list[str],
        form_pattern: Optional[list[str]] = None,
        dispatcher_pattern: Optional[list[str]] = None,
        receive_value: bool = False,
    ) -> None:
        """
        Регистрация метода в шине событий.

        :param entity_id: ID сущности META.
        :param code: Коды ObjectLogService META - ADD, SET, REMOVE.
        :param form_pattern: Формы, на которые должен реагировать хендлер, может быть пустым.
            Если пустой - значит form в event-е может быть заполненным или незаполненным,
            а если * - форма должна быть заполнена, но заполнена любым значением.
        :param dispatcher_pattern: Генераторы событий - используются для шины событий как клиент,
            который продуцирует полезную работу: запускает фоновые процессы, записывает логи, генерирует события в шину.
        ...
        :param receive_value: Добавлять ли value в событие.
        """
...
class EventBus(object):
    ...
    def __init__(self, app) -> None:
        ...
        """
        self.__app = app
        self.__listeners_map: dict[str, list[Listener]] = {}
