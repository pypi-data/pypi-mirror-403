...
class HTTPClient:
    ...
    def request(
        ...
        # Use form-urlencoded content type for form_data
        if form_data:
        ...
        response = self.client.request(
            ...
            params=params,
            json=json_data,
            data=form_data,  # httpx uses 'data' for form-urlencoded
    ...
    def stream_post(
        ...
        logger.info(f"[HTTP] stream_post to {full_url}")
        logger.info(f"[HTTP] stream_post json_data: {json_data}")
        logger.info(f"[HTTP] stream_post headers: Authorization={'***' if 'Authorization' in request_headers else 'None'}")
        ...
        with httpx.Client(base_url=self.base_url, timeout=STREAM_TIMEOUT) as client:
            with client.stream(
                ...
                logger.info(f"[HTTP] stream_post response status: {response.status_code}")
                if not response.is_success:
                    # Read full response for error handling
                    response.read()
                    logger.error(f"[HTTP] stream_post error response: {response.text[:500]}")
