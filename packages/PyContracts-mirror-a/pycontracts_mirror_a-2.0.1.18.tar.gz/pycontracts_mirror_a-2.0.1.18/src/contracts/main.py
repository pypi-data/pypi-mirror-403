...
def check(contract, value, desc=None, context={}):  # @ReservedAssignment
    """
    Checks that ``value`` satisfies the contract
    ...
    if is_param_string(contract):
        try:
            return check_contracts([contract], [value], context)
        except ContractNotRespected as e:
            if desc is not None:
                e.error = "%s\n%s" % (desc, e.error)
            raise e
        ...
        try:
            ...
            raise ContractNotRespected(contract, desc, value, dict())
    else:
        raise ValueError(
            "I expect a string (contract spec) or Contract as the first "
