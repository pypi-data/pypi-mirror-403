...
if sys.version_info[:2] == (2, 7):
    ...
    def getcallargs(func, *positional, **named):
        ...
        if ismethod(func) and im_self is not None:
            ...
            and (
                not inspect.isroutine(positional[0])
                or "self" not in inspect.signature(positional[0]).parameters()
            )
            and (
                not hasattr(positional[0], "__init__")
                or not inspect.signature(positional[0].__init__).parameters
            )
        ):
            args = args[1:]
