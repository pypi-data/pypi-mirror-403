...
try:
    ...
    PID_FILE = f"/home/{_username}/GW/GN/vmhost/data/server.pid"
...
class App():
    ...
    def runGNServerVMHostEnvironment(self):
        base = Path(f'/home/{_username}/GW/GN')
        ...
        self.run(
            domain=_domain_from_web2_to_web5(cast(str, vm['domain'])),
            port=cast(dict, vm['config']).get('port', 40001),
            gn_server_crt=cast(dict, vm['config']).get('gn_server_crt', open(str(cast(Path, vm['p']) / 'gnscrt.bin'), 'rb').read()),
            host=cast(dict, vm['config']).get('host', '0.0.0.0')
        )
        ...
        if vm_host:
            ...
            if server['git_repo_auto_update'] is not False:
                if server['git_repo_auto_update'] is True:
                    __path = f'/home/{_username}/GW/GN/servers/{_domain_from_web5_to_web2(domain)}'
                    if not os.path.exists(__path):
                        __path = f'/home/{_username}/GW/GN/servers/gns-{_domain_from_web5_to_web2(domain)}'
                else:
                    __path = server['git_repo_auto_update']

                subprocess.run(
                    [f"/home/{_username}/GW/GN/vmhost/update_git_repo.sh", __path],
                    check=True
                )
    ...
    async def _send_message_to_server(self, domain: str, path: str, payload: Optional[dict] = None, timeout: float = 1.0, res: list = []) -> Optional[GNResponse]:
        ...
        c = self._app.client.request(GNRequest('POST', Url(f'gn://[127.0.0.1]:{port}/!gn-vm-host/{path}'), payload=payload), reconnect_wait=2)
    ...
    def __add_routes(self):
        ...
        async def ping_handler(request: GNRequest, domain: str, timeout: float = 3.0):
            try:
