...
class DoCollinearity:
    ...
    def _format_output(self, total_res_ls: list[CollRes]) -> None:
        ...
        dginfo = open(f'{self.savefile}.digest', 'w+')
        for collres in total_res_ls:
            for i in range(len(collres)):
                ...
                # Write bkinfo.
                fout.write(f'# Alignment {counter}: score={score} pvalue={pvalue:.4f} '
                           f'dence1={dence1:.2f} dence2={dence2:.2f} N={n} {chr1}&{chr2} {direct}\n')

                # Write digest info.
                dginfo.write(f'{counter}\t{chr1}\t{chr2}\t{direct}\t{score}\t{pvalue:4f}\t{dence1:.2f}\t{dence2:.2f}'
