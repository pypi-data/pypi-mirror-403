...
class TangoAttrIORController(IORegisterController):
    ...
    +) Calibration - String of the triples min, pos,max
        ...
        That means: set position of the IOR 83, will write 4.5 to
                    the TangoAttr. when the reading of the TangoAttr
                    is between [1.4,1.6] the IOR value will be 33.
    ...
    axis_attributes = {
        ...
        # type hackish until arrays supported
        CALIBRATION: {
            Type: str,
        ...
        VALUE: {Type: int, Description: "Value", Access: DataAccess.ReadWrite},
    }
    MaxDevice = 1024
    ...
    def StateOne(self, axis):
        ...
        try:
            dev_proxy = self.devsExtraAttributes[axis][
                DPROXY
            ] or self._buildProxy(axis)
        ...
        if not lcalibration == 0 and not llabels == lcalibration:
            ...
            if state in (DevState.FAULT, DevState.UNKNOWN):
                status = (
                    "Masked ALARM state, tango device is in %s "
    ...
    def ReadOne(self, axis):
        ...
        dev_proxy = self.devsExtraAttributes[axis][DPROXY] or self._buildProxy(
            axis
        )
        llabels = len(self.devsExtraAttributes[axis][LABELS])
        ...
        try:
            ...
            # only round about integer the attribute value
            if llabels == 0:
                return int(value)
            ...
            #      defined ranges, and the IOR position is defined in labels
                ...
                msg = (
                    "Value ({}) out of calibration bounds. "
    ...
    def WriteOne(self, axis, value):
        ...
        try:
            ...
            # case 0:
            # nothing to translate, what is written goes to the attribute
            if llabels == 0:
            ...
            #              central position of the calibration.
                ...
                try:
                    ...
                    raise ValueError("Invalid position.")
                self._log.debug(
                    "%s ior_destination = %s" % (dev, ior_destination)
                )
                # central element
                calibrated_position = calibration[ior_destination][1]
                self._log.debug(
                    "%s calibrated_position = %s" % (dev, calibrated_position)
                )
                dev_proxy.write_attribute(attr, calibrated_position)
        except Exception as e:
            self._log.error(
                "Exception writing attribute:%s.%s"
                % (self.devsExtraAttributes[axis][DEVICE], attr)
            )
            raise e
    ...
    def _buildProxy(self, axis):
        """Try to create a device proxy, or return None to mark it
        ...
        try:
            ...
            msg = "Cannot create the proxy for the device %s (axis %d)" % (
                self.devsExtraAttributes[axis][DEVICE],
                axis,
            )
            self._log.warn(msg)
            raise DevFailed(msg)
