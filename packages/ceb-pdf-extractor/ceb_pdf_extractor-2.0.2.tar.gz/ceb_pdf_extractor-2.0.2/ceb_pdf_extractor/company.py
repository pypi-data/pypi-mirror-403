...
class PDFTableExtractor_Company:
    """光大银行公司版PDF表格数据提取器"""
    ...
    COMPANY_CONFIG = {
        ...
        'report_title': '光大银行PDF对账单提取报告（公司版）',
        'default_params': {
            'target_size': 6.0,
            'size_tolerance': 0.2,
            'row_tolerance': 5.0,
    ...
    def _merge_single_cell_rows_in_matrix(self, matrix):
        """
        新的合并策略：如果某行第1列为空，则将该行所有数据接到上一行数据后
        """
...
def main():
    ...
    # 创建公司版提取器并处理（传入选择的导出类型）
    extractor = PDFTableExtractor_Company(pdf_path, export_type)
