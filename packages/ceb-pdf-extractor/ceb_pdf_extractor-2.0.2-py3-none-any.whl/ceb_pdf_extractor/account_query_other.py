"""账户交易明细查询模板 - 专用于账户交易明细查询PDF（从交易对方名开始提取）"""
...
class PDFTableExtractor_AccountQuery_other:
    """账户交易明细查询PDF表格数据提取器"""
    ...
    def __init__(self, pdf_path, target_font="FangSong", target_size=10.0,
        ...
        self.all_data_pages = []  # 新增：按原始顺序存储所有页面数据
    ...
    def _extract_table_cells(self, page):
        """提取页面中的表格单元格数据 - 从"交易对方名"开始提取"""
        text_dict = page.get_text("dict")
        cells = []

        # 先找到"交易对方名"的位置
        transaction_name_y_start = None
        for block in text_dict.get("blocks", []):
            if block["type"] == 0:  # 文本块
                for line in block.get("lines", []):
                    for span in line.get("spans", []):
                        if "交易对方名" in span["text"]:
                            transaction_name_y_start = span["bbox"][1]  # y_start坐标
                            print(f"    找到'交易对方名'位置，y_start: {transaction_name_y_start}")
                            break
                    if transaction_name_y_start is not None:
                        break
                if transaction_name_y_start is not None:
                    break

        # 如果没有找到"交易对方名"，则从页面开始提取
        if transaction_name_y_start is None:
            transaction_name_y_start = 0
            print(f"    未找到'交易对方名'，从页面开始提取")
        ...
        for block in text_dict.get("blocks", []):
            if block["type"] == 0:  # 文本块
                for line in block.get("lines", []):
                    for span in line.get("spans", []):
                        # 匹配指定字体和大小的文本，且位置在"交易对方名"之后（包含"交易对方名"本身）
                        ...
                        if font_match and size_match:
                            ...
                            # 只提取y_start大于等于"交易对方名"位置的文本
                            if text and span["bbox"][1] >= transaction_name_y_start:
                                cells.append({
                                    "text": text,
                                    "bbox": span["bbox"],
        ...
        for page_num in range(len(self.doc)):
            ...
            if customer_info:
                ...
                self.customer_data[customer_key]["pages"].append({
                    "page_num": page_num + 1,
                    "customer_info": customer_info,  # 存储该页的customer_info
                    "matrix": matrix,
                    "has_header": True
                })
                ...
                if customer_key in self.customer_data:
                    ...
                    self.customer_data[customer_key]["pages"].append({
                        "page_num": page_num + 1,
                        "customer_info": current_customer_info,  # 存储当前页的customer_info
                        "matrix": matrix,
                        "has_header": False
                    })
                    self.customer_data[customer_key]["total_rows"] += len(matrix)
    ...
    def _clean_excel_file(self, excel_file):
        """清理单个Excel文件中的空格和不可见字符"""
        try:
    ...
    def _generate_report(self, mode, excel_files_info=None, merged_filepath=None):
        """生成处理报告"""
        ...
        for customer_key, account_data in self.customer_data.items():
            customer_names.add(customer_key)
            total_data_rows += account_data.get("total_rows", 0)
        ...
        if mode == "separate":
            ...
            report_filename = f"单独文件清洗报告（{self.pdf_filename}）.txt"
        else:  # merged
            report_content = f"合并文件 - 共{len(customer_names)}个客户，{total_data_wan}万条数据"
            report_filename = f"合并文件清洗报告（{self.pdf_filename}）.txt"
    ...
    def process_separate_files(self):
        """生成多个Excel文件（一个账户一个文件）"""
        try:
            print(f"\n{'='*60}")
            print("📁 模式：生成单独的Excel文件（一个客户一个文件）")
            print(f"{'='*60}")
            ...
            print(f"\n✅ 扫描完成，找到 {len(self.customer_data)} 个客户账户")
            ...
            for customer_key, account_data in self.customer_data.items():
                ...
                if all_rows and len(all_rows) > 0:
                    ...
                    # 清理文件
                    self._clean_excel_file(filepath)
            ...
            # 生成处理报告
            self._generate_report("separate", excel_files_info)
    ...
    def process_single_file(self):
        """生成单个总Excel文件（按原始PDF顺序）"""
        try:
            print(f"\n{'='*60}")
            print("📊 模式：生成单个总Excel文件（按原PDF顺序）")
            print(f"{'='*60}")
            ...
            # 创建总Excel文件
            merged_filename = f"{self.pdf_filename}_总数据.xlsx"
            merged_filepath = os.path.join(self.output_dir, merged_filename)

            try:
                merged_wb = Workbook()
                merged_ws = merged_wb.active
                merged_ws.title = "所有数据"
                print(f"✅ 创建总Excel文件: {merged_filename}")
            except Exception as e:
                print(f"❌ 创建Excel文件时出错: {e}")
                return None
            ...
            for page_data in self.all_data_pages:
                ...
                for i in range(data_start, len(matrix)):
                    ...
                    merged_ws.append(new_row)
                    total_data_rows += 1

            # 应用格式
            if header_written:
                # 获取所有行数据以应用格式
                all_rows = list(merged_ws.iter_rows(values_only=True))
                if all_rows:
                    self._apply_excel_format(merged_ws, all_rows)

            # 保存Excel文件
            merged_wb.save(merged_filepath)

            # 清理文件
            self._clean_excel_file(merged_filepath)

            # 记录文件信息
            self.extracted_files.append(merged_filepath)

            # 生成处理报告
            self._generate_report("merged", merged_filepath=merged_filepath)
    ...
    def process(self, mode="separate"):
        """执行完整的处理流程（提供模式选择）"""
        if mode == "separate":
            return self.process_separate_files()
        elif mode == "single":
            return self.process_single_file()
        else:
            print(f"❌ 未知模式: {mode}，请选择 'separate' 或 'single'")
            return None
...
def process_pdf_to_multiple_excel(pdf_path):
    ...
    extractor = PDFTableExtractor_AccountQuery_other(pdf_path)
    return extractor.process_separate_files()


def process_pdf_to_single_excel(pdf_path):
    """处理PDF生成单个总Excel文件（按原PDF顺序）"""
    extractor = PDFTableExtractor_AccountQuery_other(pdf_path)
    return extractor.process_single_file()


def main():
    """主函数"""
    print("=" * 60)
    print("光大银行PDF对账单提取工具 - 账户交易明细查询版（从交易对方名开始）")
    print("专用于：账户交易明细查询PDF")
    print("=" * 60)

    # 选择模式
    print("\n请选择处理模式：")
    print("1. 生成单独的Excel文件（一个客户一个文件）")
    print("2. 生成单个总Excel文件（所有数据在一个文件，按原PDF顺序）")
    ...
    if os.path.isfile(input_path):
        ...
        extractor = PDFTableExtractor_AccountQuery_other(input_path)
        result = extractor.process(mode=mode)
