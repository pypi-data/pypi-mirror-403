...
class PDFTableExtractor_NoWatermark:
    """光大银行无水印版PDF表格数据提取器"""
    ...
    NO_WATERMARK_CONFIG = {
        ...
        'supplementary_columns': [
            '账户名称', '账户类别', '系统账号', '客户账号', '查询起止日期',
            '交易总笔数', '转出笔数', '转出金额', '存入笔数', '存入金额'
        ],
        'report_title': '光大银行PDF对账单提取报告（无水印版）',
        'default_params': {
            'target_size': 6.0,
            'size_tolerance': 0.2,
            'row_tolerance': 5.0,
    ...
    def __init__(self, pdf_path, export_type="split"):
        ...
        # 使用无水印版固定配置
        config = self.NO_WATERMARK_CONFIG
    ...
    def _extract_customer_info(self, page):
        ...
        # 检查关键字段是否都存在（只需检查账户名称）
        if '账户名称' in info and info['账户名称'].strip():
            return info
        else:
            return None
    ...
    def _detect_columns_from_header(self, rows):
        ...
        if self.debug_mode:
            print(f"    检测到列位置: {column_positions}")
            print(f"    列数量: {len(column_positions)}")
    ...
    def _assign_cells_with_header_constraint(self, rows, column_positions):
        ...
        for row_idx, row_cells in enumerate(rows):
            ...
            if self.debug_mode and row_idx == 0:
                print(f"    表头行单元格: {[cell['text'] for cell in sorted_cells]}")
                print(f"    表头行单元格位置: {[cell['x_center'] for cell in sorted_cells]}")

            for cell in sorted_cells:
                closest_col_idx = self._find_closest_column(cell["x_center"], column_positions)

                distance = abs(cell["x_center"] - column_positions[closest_col_idx])
                threshold = self.col_tolerance * 2

                if distance > threshold:
                    if self.debug_mode:
                        print(f"    单元格 '{cell['text']}' 距离列{closest_col_idx}位置{column_positions[closest_col_idx]}的距离{distance}超过阈值{threshold}，跳过")
                    continue
            ...
            # 调试信息：打印每行的数据
            if self.debug_mode:
                non_empty_cells = [(i, cell) for i, cell in enumerate(matrix_row) if cell]
                if non_empty_cells:
                    print(f"    第{row_idx+1}行数据: {non_empty_cells}")
    ...
    def _place_cell_in_column(self, cell, target_col_idx, matrix_row, max_cols):
        if not matrix_row[target_col_idx]:
            ...
            if self.debug_mode:
                print(f"      单元格 '{cell['text']}' 放入列 {target_col_idx}")
            return
        ...
        for i in range(target_col_idx + 1, max_cols):
            if not matrix_row[i]:
                ...
                if self.debug_mode:
                    print(f"      单元格 '{cell['text']}' 放入列 {i}（原列{target_col_idx}已有数据）")
                return
        ...
        for i in range(target_col_idx - 1, -1, -1):
            if not matrix_row[i]:
                matrix_row[i] = cell["text"]
                if self.debug_mode:
                    print(f"      单元格 '{cell['text']}' 放入列 {i}（原列{target_col_idx}已有数据，后无空列）")
                return

        # 如果前后都没有空列，合并到目标列
        matrix_row[target_col_idx] += " " + cell["text"]
        if self.debug_mode:
            print(f"      单元格 '{cell['text']}' 合并到列 {target_col_idx}（前后都无空列）")
    ...
    def _ensure_header_completeness(self, header_cells, matrix_row, column_positions):
        ...
        for header_text in unassigned_texts:
            ...
            if not matrix_row[closest_col_idx]:
                ...
                if self.debug_mode:
                    print(f"      未分配的表头 '{header_text}' 放入列 {closest_col_idx}")
            else:
                ...
                if empty_cols:
                    ...
                    if self.debug_mode:
                        print(f"      未分配的表头 '{header_text}' 放入最近空列 {nearest_empty_idx}")
    ...
    def _merge_single_cell_rows_in_matrix(self, matrix):
        ...
        for row_idx in range(len(matrix) - 1, 0, -1):
            ...
            if len(row) >= 1:
                # 检查第1列是否为空或只有空白
                col_1_value = row[0] if 0 < len(row) else ''
                if not col_1_value or str(col_1_value).strip() == '':
                    # 获取上一行
        ...
        if self.debug_mode and rows_to_delete:
            print(f"    合并了 {len(rows_to_delete)} 行数据")

        return matrix

    """改进的合并策略：检查是否是备注/摘要续行"""
    def _merge_continuation_rows(self, matrix):
        """
        更智能的合并策略：识别常见的续行模式
        """
        if not matrix or len(matrix) < 2:
            ...
            if non_empty_count <= 2:
                ...
                for col_idx in range(len(row)):
                    ...
                    if cell_value and str(cell_value).strip():
                        ...
                        if row_idx > 0:
                            prev_row = matrix[row_idx - 1]
                            prev_value = prev_row[col_idx] if col_idx < len(prev_row) else ''

                            # 如果上一行对应列为空，但其他列有数据，可能是续行
                            if (not prev_value or str(prev_value).strip() == '') and any(prev_row[i] for i in range(len(prev_row)) if i != col_idx and prev_row[i] and str(prev_row[i]).strip()):
                                # 将当前单元格追加到上一行对应列
                                prev_row[col_idx] = cell_value

                                if self.debug_mode:
                                    print(f"      第{row_idx+1}行列{col_idx}数据合并到第{row_idx}行列{col_idx}: '{cell_value}'")
    ...
    def scan_pages(self):
        ...
        for page_num in range(len(self.doc)):
            ...
            if not customer_info:
                print(f"  第{page_num + 1}页: 未找到账户名称，跳过")
                continue

            # 获取账户名称
            customer_name = customer_info.get('账户名称', '未知')
        ...
        for customer_key, page_data_list in self.customer_data.items():
            ...
            for i, page_data in enumerate(page_data_list):
                ...
                if len(matrix) > 1:
                    ...
                    # 再应用更智能的续行合并逻辑
                    merged_data_rows = self._merge_continuation_rows(merged_data_rows)
    ...
    def process_customer_data_merged(self):
        ...
        if all_rows:
            ...
            print(f"   包含 {len(self.customer_data)} 个账户的数据")
    ...
    def _generate_clean_report(self, excel_files_info):
        ...
        if self.export_type == "merged":
            ...
            # 生成报告内容
            report_content = f"共{total_customers}个账户，{total_data_wan}万条数据，已合并到总表"
        else:
            # 分表报告
            customer_names = set()
            total_data_rows = 0
            ...
            # 生成报告内容
            report_content = f"共{len(customer_names)}个账户，{total_data_wan}万条数据"
...
def main():
    """主函数 - 直接运行无水印版提取器"""
    print("=" * 70)
    print("光大银行无水印版PDF对账单提取工具")
    print("=" * 70)

    # 首先选择导出模式
    print("\n📋 请选择导出类型:")
    print("1. 按账户名称导出分表（每个账户一个Excel文件）")
    print("2. 导出总表（所有账户数据合并到一个Excel文件）")
    ...
    # 获取PDF文件路径
    print(f"\n📋 您选择的导出类型: {export_type_name}")
    pdf_path = input("请输入PDF文件位置：").strip()
    ...
    # 创建无水印版提取器并处理
    extractor = PDFTableExtractor_NoWatermark(pdf_path, export_type)
