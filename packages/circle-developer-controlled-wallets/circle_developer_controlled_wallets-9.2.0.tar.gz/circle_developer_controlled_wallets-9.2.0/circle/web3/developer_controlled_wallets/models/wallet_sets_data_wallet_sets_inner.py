...
class WalletSetsDataWalletSetsInner(BaseModel):
    ...
    actual_instance: Optional[Union[DeveloperWalletSet, EndUserWalletSet]] = None
    one_of_schemas: Set[str] = { "DeveloperWalletSet", "EndUserWalletSet" }
    ...
    def __get_pydantic_core_schema__(cls, source_type, handler):
        ...
        def convert_child_types(v):
            ...
            if isinstance(v, DeveloperWalletSet):
                return cls(actual_instance=v)
            if isinstance(v, EndUserWalletSet):
                return cls(actual_instance=v)
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = WalletSetsDataWalletSetsInner.model_construct()
        error_messages = []
        match = 0
    ...
    def from_dict(cls, obj: Union[str, Dict[str, Any]]) -> Self:
        """Create an instance of WalletSetsDataWalletSetsInner from dict"""
        ...
        try:
            instance.actual_instance = DeveloperWalletSet.model_validate(obj)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into EndUserWalletSet
        try:
            instance.actual_instance = EndUserWalletSet.model_validate(obj)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when creating `actual instance` in WalletSetsDataWalletSetsInner with oneOf schemas: DeveloperWalletSet, EndUserWalletSet. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when creating `actual instance` in WalletSetsDataWalletSetsInner with oneOf schemas: DeveloperWalletSet, EndUserWalletSet. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], DeveloperWalletSet, EndUserWalletSet]]:
