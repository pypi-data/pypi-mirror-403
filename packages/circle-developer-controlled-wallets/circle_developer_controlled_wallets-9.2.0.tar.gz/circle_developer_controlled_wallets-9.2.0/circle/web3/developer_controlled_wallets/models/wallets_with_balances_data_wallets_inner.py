...
class WalletsWithBalancesDataWalletsInner(BaseModel):
    ...
    actual_instance: Optional[Union[EOAWalletWithBalances, SCAWalletWithBalances]] = None
    one_of_schemas: Set[str] = { "EOAWalletWithBalances", "SCAWalletWithBalances" }
    ...
    def __get_pydantic_core_schema__(cls, source_type, handler):
        ...
        def convert_child_types(v):
            ...
            if isinstance(v, EOAWalletWithBalances):
                return cls(actual_instance=v)
            if isinstance(v, SCAWalletWithBalances):
                return cls(actual_instance=v)
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = WalletsWithBalancesDataWalletsInner.model_construct()
        error_messages = []
        match = 0
    ...
    def from_dict(cls, obj: Union[str, Dict[str, Any]]) -> Self:
        """Create an instance of WalletsWithBalancesDataWalletsInner from dict"""
        ...
        try:
            instance.actual_instance = EOAWalletWithBalances.model_validate(obj)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into SCAWalletWithBalances
        try:
            instance.actual_instance = SCAWalletWithBalances.model_validate(obj)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when creating `actual instance` in WalletsWithBalancesDataWalletsInner with oneOf schemas: EOAWalletWithBalances, SCAWalletWithBalances. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when creating `actual instance` in WalletsWithBalancesDataWalletsInner with oneOf schemas: EOAWalletWithBalances, SCAWalletWithBalances. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], EOAWalletWithBalances, SCAWalletWithBalances]]:
