#!/usr/bin/env python3
"""
MCP AI Jerry 服务器主要模块

此模块提供 MCP (Model Context Protocol) 的智能交互反馈功能，
...
def save_feedback_to_file(feedback_data: dict, file_path: str | None = None) -> str:
    ...
    debug_log(f"反馈资料已保存至: {file_path}")
    return file_path
...
def process_images(images_data: list[dict]) -> list:
    """
    处理图片资料，转换为 MCP 图片内容对象

    Args:
        images_data: 图片资料列表

    Returns:
        list: 包含图片信息的 ImageContent 对象列表
    """
    mcp_images = []
    ...
    for i, img in enumerate(images_data, 1):
        try:
            ...
            if isinstance(img["data"], bytes):
                ...
                debug_log(f"图片 {i} 已是 base64 字符串")
            else:
            ...
            # 创建 ImageContent 对象（MCP 标准类型，可正确序列化）
            ...
            debug_log(f"图片 {i} ({file_name}) 处理成功，MIME 类型: {mime_type}")
...
async def interactive_feedback(
    ...
    try:
        ...
        debug_log(f"授权验证异常: {e}")
        network_error = True
        # 严格模式：网络错误时不回退到本地缓存，除非本地缓存仍在有效验证期内
        # 这里只允许打开 Web UI 让用户看到错误提示
        is_authorized = False
    
    # 即使未授权，也允许打开 Web UI 以便激活
    # 但如果未授权，会在 Web UI 中显示激活提示
    ...
    debug_log(f"授权状态: {'已授权' if is_authorized else '未授权'}, 网络错误: {network_error}")
    ...
    try:
        ...
        if network_error and not is_authorized:
            debug_log("网络错误导致授权验证失败")
            return [TextContent(
                type="text", 
                text="⚠️ 授权验证失败\n\n无法连接到授权服务器，请检查网络连接后重试。\n\n如果网络正常但仍无法验证，请联系技术支持。"
            )]
        
        # 如果仍未授权（非网络错误），返回激活提示
        if not is_authorized:
            debug_log("用户未完成激活，返回激活提示")
            return [TextContent(
                type="text", 
                text="⚠️ 请先完成激活\n\n您尚未激活软件，请在 Web UI 界面中输入激活码完成激活。\n\n激活后即可正常使用所有功能。"
            )]
