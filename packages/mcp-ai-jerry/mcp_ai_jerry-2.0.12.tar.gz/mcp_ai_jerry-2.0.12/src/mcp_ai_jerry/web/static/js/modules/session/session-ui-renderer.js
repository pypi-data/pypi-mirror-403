...
(function() {
    ...
    SessionUIRenderer.prototype.initializeProjectPathDisplay = function() {
        ...
        if (projectPathElement) {
            ...
            if (fullPath) {
                // 僅提取專案名稱（最後一個目錄名）
                const projectName = this.extractProjectName(fullPath);
                if (DEBUG_MODE) console.log('🎨 初始化時專案名稱:', projectName);

                // 更新顯示文字
                DOMUtils.safeSetTextContent(projectPathElement, projectName);
            }
        }
    };
     ...
     * 從完整路徑提取專案名稱（最後一個目錄名）
     */
    SessionUIRenderer.prototype.extractProjectName = function(fullPath) {
        if (!fullPath) return '';
        // 處理 Windows 和 Unix 路徑分隔符
        const separator = fullPath.includes('\\') ? '\\' : '/';
        const parts = fullPath.split(separator).filter(function(p) { return p.length > 0; });
    ...
    SessionUIRenderer.prototype.updateTopProjectPathDisplay = function(sessionData) {
        ...
        if (projectPathElement && sessionData.project_directory) {
            ...
            // 僅提取專案名稱（最後一個目錄名）
            const projectName = this.extractProjectName(fullPath);
            if (DEBUG_MODE) console.log('🎨 專案名稱:', projectName);
