...
async def stop_training(run_id: int) -> Tuple[bool, str]:
    ...
    # 고아 컨테이너 정리 (algo 컨테이너 등)
    try:
        await cleanup_orphaned_containers()
    except Exception as e:
        print(f"[stop_training] Orphan cleanup failed: {e}")
...
async def stop_evaluation(run_id: int) -> Tuple[bool, str]:
    ...
    try:
        ...
        print(f"[stop_evaluation] Orphan cleanup failed: {e}")
...
# 정리 간격 (초)
CLEANUP_INTERVAL_SECONDS = 10  # 10초
...
async def cleanup_orphaned_containers() -> int:
    ...
    Note: asyncio.Lock으로 이중 실행 방지
    """
    # 이중 실행 방지 - 이미 실행 중이면 스킵
    if _cleanup_lock.locked():
        return 0
    
    async with _cleanup_lock:
        return await _cleanup_orphaned_containers_impl()


async def _cleanup_orphaned_containers_impl() -> int:
    """cleanup 실제 구현 (lock 내부에서 호출됨)"""
    try:
