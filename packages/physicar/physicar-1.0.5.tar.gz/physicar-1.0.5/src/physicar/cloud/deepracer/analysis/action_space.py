...
def plot_action_histogram(
    ...
    if iteration is not None:
        plot_df = df[df["iteration"] == iteration].copy()
        title_suffix = f"(Iteration {iteration})"
    elif episode is not None:
        plot_df = df[df["episode"] == episode].copy()
        title_suffix = f"(Episode {episode})"
    else:
        plot_df = df.copy()
        title_suffix = "(All)"
    
    # Ensure numeric types for histogram
    plot_df["steering_angle"] = pd.to_numeric(plot_df["steering_angle"], errors="coerce")
    plot_df["speed"] = pd.to_numeric(plot_df["speed"], errors="coerce")
    plot_df = plot_df.dropna(subset=["steering_angle", "speed"])
