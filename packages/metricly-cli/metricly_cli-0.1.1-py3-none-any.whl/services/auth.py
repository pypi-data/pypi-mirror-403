...
def get_user_by_email(email: str) -> UserContext:
    ...
    member_doc = db.collection(ORGANIZATIONS_COLLECTION).document(current_org) \
        .collection(MEMBERS_SUBCOLLECTION).document(user_id).get()
...
def get_user_orgs(email: str) -> list[dict]:
    ...
    # Query all 'members' subcollections where document ID is user_id
    memberships = db.collection_group(MEMBERS_SUBCOLLECTION) \
        .where("__name__", ">=", db.document(f"_/{user_id}")) \
        .where("__name__", "<=", db.document(f"_/{user_id}\uf8ff")) \
        .stream()
...
def switch_org(email: str, org_id: str) -> UserContext:
    ...
    member_doc = db.collection(ORGANIZATIONS_COLLECTION).document(org_id) \
        .collection(MEMBERS_SUBCOLLECTION).document(user_id).get()
