...
async def add_widget(
    ...
    - pivot: Optional pivot configuration for table widgets (cross-tabulation):
        - pivot.on: Dimension name to pivot on (transforms dimension values into columns)
        - pivot.show_totals: Whether to show row and column totals (default: false)
        - Note: pivot.on must be a dimension included in query.dimensions

    Table Pivot Examples:
    - Revenue by region and product with regions as columns:
        type: "table"
        query: {metrics: ["revenue"], dimensions: ["product", "region"]}
        pivot: {on: "region", show_totals: true}
    - Monthly sales by category with months as columns:
        type: "table"
        query: {metrics: ["sales"], dimensions: ["category"], grain: "month"}
        pivot: {on: "metric_time", show_totals: true}

    When to use pivot vs regular table:
    - Use pivot when comparing one dimension across another (cross-tabulation)
    - Use regular table for detailed row-by-row data or when you need many columns
    - Pivot works best with a small number of pivot column values (<10)
...
async def update_widget(
    ...
    Args:
        ...
        updates: Fields to update (title, query, format, type, pivot, etc.)
        expected_version: For optimistic locking - if provided, update will fail
            if the current version doesn't match.
    ...
    - title: Widget title
    - type: Widget type (kpi, area_chart, bar_chart, line_chart, table, donut, heatmap)
    - query: Query configuration (metrics, dimensions, grain, etc.)
    - format: Value formatting options
    - width: Column span (1-10)
    - time_scope: Date scope ("range", "latest", "latest_complete")
    - pivot: For table widgets only - cross-tabulation config:
        - pivot.on: Dimension to pivot on (creates columns from values)
        - pivot.show_totals: Show row/column totals (default: false)
        - Set pivot to null/empty to remove pivot and show as regular table
