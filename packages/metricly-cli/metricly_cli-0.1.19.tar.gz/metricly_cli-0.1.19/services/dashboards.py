...
async def add_widget(
    ...
    # Check for common schema mistakes and provide helpful errors
    query = widget.get("query", {})
    if "time_scope" in query:
        raise ValueError(
            "time_scope must be at widget level, not inside query. "
            "Move 'time_scope' from query to the widget object."
        )
...
async def update_widget(
    ...
    # Check for common schema mistakes and provide helpful errors
    query = updated_widget.get("query", {})
    if "time_scope" in query:
        raise ValueError(
            "time_scope must be at widget level, not inside query. "
            "Move 'time_scope' from query to the widget object."
