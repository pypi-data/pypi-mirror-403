...
class PivotConfig(BaseModel):
    """Configuration for pivot table transformation."""
    on: str = Field(description="Dimension name to pivot on - creates columns from dimension values")
    show_totals: bool = Field(
        default=False,
        description="Whether to show row and column totals"
    )
...
class VisualizationConfig(BaseModel):
    ...
    pivot: PivotConfig | None = Field(
        default=None,
        description="For table type: pivot configuration to transform rows into columns for cross-tabulation"
    )
...
class WidgetUpdate(BaseModel):
    ...
    pivot: PivotConfig | None = Field(
        default=None,
        description="For table type: pivot config to create cross-tabulation (pivot.on must be in query.dimensions)"
    )
...
Use pivot tables when the user wants to compare one dimension across another (cross-tabulation).
Pivot transforms dimension values into columns for easy side-by-side comparison.

When to suggest pivot table:
- "Revenue by product AND region" → pivot on region, shows products as rows, regions as columns
- "Compare monthly sales across categories" → pivot on category or metric_time
- "Cross-tabulation of X by Y" → explicit request for pivot
- Any request for a "matrix view" or "comparison grid"

When NOT to use pivot:
- Simple lists or detailed row data
- Too many pivot column values (>10 creates unwieldy tables)
- Time-series visualizations (use line/area charts instead)

Example pivot configuration:
```
type: "table"
query: {metrics: ["revenue"], dimensions: ["product", "region"]}
pivot: {on: "region", show_totals: true}
```
This shows products as rows, regions as columns, with totals.

IMPORTANT: pivot.on must be a dimension in query.dimensions
...
4. Common modifications:
   ...
   - "Pivot on region" → add pivot: {on: "region"} (for tables)
   - "Show as cross-tab" → convert to table with pivot config
   - "Remove pivot" → set pivot to null
...
- Changing a single widget's type, title, query, format, orientation, or pivot config
