...
def get_user_display_name(uid: str) -> str | None:
    """Look up user's display name by UID.

    Returns displayName if set, otherwise email, or None if user not found.

    Args:
        uid: User's Firestore document ID

    Returns:
        Display name, email, or None if not found
    """
    db = _get_firestore_client()
