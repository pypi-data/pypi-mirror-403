...
from auth import get_current_user, require_org_role, AuthenticatedUser
...
# Mount MCP server at /mcp for Claude.ai integration
...
async def render_dashboard_endpoint(
    ...
    Requires authentication. Returns binary file data.
    """
...
async def render_widget_endpoint(
    ...
    Requires authentication. Returns PNG image.
    """
    # Verify user has access to the dashboard
    try:
        dashboard = storage.get_dashboard(user.org_id, dashboard_id, user.uid)
