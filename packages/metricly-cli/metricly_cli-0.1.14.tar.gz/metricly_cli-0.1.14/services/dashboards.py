...
def _repair_dashboard_data(data: dict) -> dict:
    """Repair dashboard data by ensuring all widgets have IDs.

    This handles corrupted dashboards created by older CLI versions
    that may have widgets without IDs. Auto-generates UUIDs for any
    missing widget IDs to allow Pydantic validation to succeed.

    Args:
        data: Raw dashboard data from storage

    Returns:
        Repaired dashboard data with all widget IDs populated
    """
