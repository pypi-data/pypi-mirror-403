...
def dashboards_render(
    ...
    """
    from services.render import render_dashboard as render_dashboard_service
    from services.render import RenderError, ChromeConnectionError, RenderTimeoutError
    ...
    try:
        ...
        print_error("Cannot connect to Chrome", hint="Ensure headless Chrome is running. See docs for setup.")
        raise typer.Exit(1)
    except RenderTimeoutError as e:
        print_error("Render timed out", hint="The dashboard may be too large or complex.")
        raise typer.Exit(1)
        ...
        if "not found" in error_msg.lower():
            ...
            print_error(f"Render failed: {error_msg}")
        raise typer.Exit(1)
...
def dashboards_render_widget(
    ...
    """
    from services.render import render_widget as render_widget_service
    from services.render import RenderError, ChromeConnectionError, RenderTimeoutError
    ...
    try:
        ...
        print_error("Render timed out", hint="The widget may be too complex.")
        raise typer.Exit(1)
    except RenderError as e:
        print_error(f"Render failed: {e}")
        raise typer.Exit(1)
