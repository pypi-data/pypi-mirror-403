...
def dashboards_list(
    ...
    from services.auth import get_user_display_name

    def resolve_owner(owner_uid: str) -> str:
        """Resolve owner UID to display name."""
        if owner_uid == user.uid:
            return "you"
        name = get_user_display_name(owner_uid)
        return name or owner_uid[:8] + "..."
    ...
    try:
        ...
        if format == "json":
            ...
            summary = {
                ...
                "owner": resolve_owner(dashboard.owner),
        ...
        print_error(f"Dashboard is corrupted: {ve.error_count()} validation error(s)")
        print_info(f"To delete this corrupted dashboard: metricly dashboards delete {dashboard_id} --force --yes")
        raise typer.Exit(1)
    except ValueError as e:
        print_error(str(e))
        raise typer.Exit(1)
...
def dashboards_delete(
    ...
    force: Annotated[
        bool,
        typer.Option("--force", "-f", help="Delete even if dashboard is corrupted"),
    ] = False,
):
    """Delete a dashboard.
    ...
    If a dashboard is corrupted and cannot be loaded, use --force --yes to delete it anyway.
    """
    user = require_auth()
    ...
    try:
        # Try to get the dashboard to show its title
        dashboard = None
        ...
        try:
            ...
            if force:
                print_warning(f"Dashboard is corrupted: {ve.error_count()} validation error(s)")
                dashboard_title = f"[corrupted] {dashboard_id}"
            else:
                print_error("Dashboard is corrupted and cannot be loaded")
                print_info("Use --force --yes to delete corrupted dashboards")
                raise typer.Exit(1)

        # Confirm deletion
        if not yes:
            confirm = typer.confirm(
                f"Delete dashboard '{dashboard_title}' ({dashboard_id})?"
            )
            if not confirm:
                print_info("Cancelled")
                raise typer.Exit(0)
        ...
        print_success(f"Deleted dashboard: {dashboard_title}")
