...
from easy_sgr.sgr_agent_core.server.__main__ import load_config

# Verify we're importing the real function
assert (
    load_config.__module__ == "easy_sgr.sgr_agent_core.server.__main__"
), "Must import load_config from easy_sgr.sgr_agent_core.server.__main__, not a mock!"
        ...
        agents_data = {
            "agents": {
                "test_agent_from_agents_yaml": {
                    "base_class": "easy_sgr.sgr_agent_core.agents.sgr_agent.SGRAgent",
...
class TestAgentsLoadingOrder:
    ...
    def test_agents_yaml_error_logging(self, temp_dir, reset_global_config):
        ...
        with patch("easy_sgr.sgr_agent_core.server.__main__.logger") as mock_logger:
            # Use actual load_config() from __main__.py
            with pytest.raises(ValueError, match="must contain 'agents' key"):
                load_config(str(config_yaml), str(agents_yaml))
