...
from easy_sgr.sgr_agent_core.agents import SGRAgent
from easy_sgr.sgr_agent_core.models import AgentStatesEnum
from easy_sgr.sgr_agent_core.server.endpoints import (
    _is_agent_id,
    agents_storage,
...
from easy_sgr.sgr_agent_core.server.models import ChatCompletionRequest, ClarificationRequest
...
class TestChatCompletionEndpoint:
    ...
    @patch("easy_sgr.sgr_agent_core.server.endpoints.AgentFactory")
    @pytest.mark.asyncio
    async def test_create_new_agent_success(self, mock_factory):
        ...
        with patch("easy_sgr.sgr_agent_core.server.endpoints.asyncio.create_task") as mock_create_task:
    ...
    async def test_create_agent_with_multiple_messages(self):
        ...
        with patch("easy_sgr.sgr_agent_core.server.endpoints.AgentFactory") as mock_factory:
            mock_agent_def = Mock()
            mock_factory.get_definitions_list.return_value = [mock_agent_def]
            mock_agent_def.name = "sgr_agent"
            mock_factory.create = AsyncMock(return_value=mock_agent)
...
class TestAgentStateEndpoint:
    ...
    async def test_get_agent_state_success(self):
        ...
        from easy_sgr.sgr_agent_core.models import SourceData
