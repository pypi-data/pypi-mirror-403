...
def _pivot_multilabel_predictions(
    ...
    values: "np.ndarray[Any, np.dtype[np.float32]]" = np.empty(
        (frame.shape[0], len(labels)), np.float32
    )
    thresholds: "np.ndarray[Any, np.dtype[np.float32]]" = np.empty(
        (frame.shape[0], len(labels)), np.float32
    )
    predictions: "np.ndarray[Any, np.dtype[np.int8]]" = np.empty(
        (frame.shape[0], len(labels)), np.int8
    )
