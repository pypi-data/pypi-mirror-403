...
class Rule(KLCRule):
    ...
    def defaultOffset(self) -> float:
        ...
        self.module_dir = f"{os.path.splitext(module_dir)}"
    ...
    def check(self) -> bool:
        ...
        if self.bad_width:
            self.error(f"Courtyard width error (expected width = {KLC_CRTYD_WIDTH}mm)")
        ...
        if self.bad_grid:
            self.error(f"Courtyard lines are not on {KLC_CRTYD_GRID}mm grid")
