...
class Rule(KLCRule):
    ...
    def checkPins(self, pins: List[Pin], is_power: bool) -> bool:
        ...
        if self.type_errors:
            ...
            for pin in self.type_errors:
                self.errorExtra(
                    f"{pinString(pin)} should be of type NOT CONNECTED, but is of type {pin.etype}"
                )
        ...
        if self.invisible_errors:
            ...
            for pin in self.invisible_errors:
                self.warningExtra(f"{pinString(pin)} should be INVISIBLE")
        ...
        if self.power_invisible_errors:
            ...
            for pin in self.power_invisible_errors:
                self.errorExtra(f"{pinString(pin)} is of type power_in and invisible")
    ...
    def fix(self) -> None:
        ...
        for pin in self.invisible_errors:
            if not pin.is_hidden:
                pin.is_hidden = True
                self.info(f"Setting pin {pin.number} to INVISIBLE")

        for pin in self.type_errors:
            if not pin.etype == "no_connect":
                pin.etype = "no_connect"
                self.info(f"Changing pin {pin.number} type to NO_CONNECT")
