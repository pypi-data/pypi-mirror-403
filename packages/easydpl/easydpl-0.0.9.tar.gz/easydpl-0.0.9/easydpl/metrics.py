...
__all__ = ['rmse', 'exp_rmspe', 'perplexity', 'AccumMetric', 'skm_to_fastai', 'optim_metric', 'accuracy', 'error_rate',
           ...
           'msle', 'mape', 'mspe', 'smape', 'r2', 'ExplainedVariance', 'R2Score', 'PearsonCorrCoef', 'SpearmanCorrCoef', 'foreground_acc', 'Dice',
...
rmse = AccumMetric(lambda o, t: _rmse(o, t))
...
def mape(inp, targ):
    ...
    inp,targ = flatten_check(inp,targ)
    return torch.mean(torch.abs((inp - targ) / targ))

# %% ../nbs/13b_metrics.ipynb 93
def mspe(inp, targ):
    "Mean squared percentage error between `inp` and `targ`."
    inp,targ = flatten_check(inp,targ)
    return torch.mean(torch.square((inp - targ) / targ))

# %% ../nbs/13b_metrics.ipynb 94
def smape(inp, targ):
    "Symmetric mean absolute percentage error between `inp` and `targ`."
    inp,targ = flatten_check(inp,targ)
    denominator = torch.abs(inp) + torch.abs(targ)
    # 避免除以零
    mask = denominator == 0
    denominator = torch.where(mask, torch.ones_like(denominator), denominator)
    smape_vals = torch.where(mask, torch.zeros_like(inp), 2 * torch.abs(inp - targ) / denominator)
    return torch.mean(smape_vals) * 100

# %% ../nbs/13b_metrics.ipynb 95
def r2(inp, targ):
    "R-squared (coefficient of determination) between `inp` and `targ`."
    inp,targ = flatten_check(inp,targ)
