...
class Learner(GetAttr):
    ...
    @delegates(GatherPredsCallback.__init__)
    def get_n_step_preds(self,
        n_step:int, # Time step to extract (1-indexed, e.g., 1 for first step, 6 for sixth step)
        ds_idx:int=1, # `DataLoader` to use for predictions if `dl` is None. 0: train. 1: valid
        dl=None, # `DataLoader` to use for predictions, defaults to `ds_idx=1` if None
        with_input:bool=False, # Return inputs with predictions
        with_decoded:bool=False, # Return decoded predictions
        ...
        act=None, # Apply activation to predictions, defaults to `self.loss_func`'s activation
        inner:bool=False, # If False, create progress bar, show logger, use temporary `cbs`
        reorder:bool=False, # Disable reorder to preserve sliding window order
        cbs:Callback|MutableSequence|None=None, # Temporary `Callback`s to apply during prediction
        **kwargs
    )->tuple:
        # reorder=False preserves sliding window order for temporal continuity
        res = self.get_preds(
            ds_idx=ds_idx, dl=dl, with_input=with_input, with_decoded=with_decoded,
            with_loss=with_loss, act=act, inner=inner, reorder=reorder, cbs=cbs, **kwargs
        )
        
        # Convert to 0-indexed
        step_idx = n_step - 1
        # Determine prediction index in result tuple
        pred_i = 1 if with_input else 0
        res = list(res)
        
        # Extract n-th step predictions
        if res[pred_i] is not None and len(res[pred_i].shape) >= 2:
            pred_len = res[pred_i].shape[1]
            if step_idx >= pred_len:
                raise ValueError(f"n_step={n_step} exceeds pred_len={pred_len}")
            
            # Slice n-th step from each window: [num_windows, pred_len, ...] -> [num_windows, ...]
            res[pred_i] = res[pred_i][:, step_idx]
            # Also slice targets
            targ_i = pred_i + 1
            if targ_i < len(res) and res[targ_i] is not None and len(res[targ_i].shape) >= 2:
                if step_idx < res[targ_i].shape[1]:
                    res[targ_i] = res[targ_i][:, step_idx]
            # Handle decoded predictions
            if with_decoded:
                dec_i = pred_i + 2 if with_input else pred_i + 1
                if dec_i < len(res) and res[dec_i] is not None and len(res[dec_i].shape) >= 2:
                    if step_idx < res[dec_i].shape[1]:
                        res[dec_i] = res[dec_i][:, step_idx]
        return tuple(res)
...
add_docs(Learner, "Group together a `model`, some `dls` and a `loss_func` to handle training",
    ...
    get_n_step_preds="Get predictions for the n-th time step from multi-step forecasts, slicing [batch, time_steps, ...] to [batch, ...]",
