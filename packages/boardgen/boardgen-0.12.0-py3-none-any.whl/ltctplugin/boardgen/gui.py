...
class BoardgenPanel(BasePanel):
    ...
    def __init__(self, parent: wx.Window, frame):
        ...
        self.ShowAll = self.BindCheckBox("checkbox_show_all")
        ...
        self.TextEditItem = self.FindStaticText("text_edit_item")
        ...
        self.lvm = LVM.get()
        self.core = Core()
        self.core.set_base_dir(
            dir_base=join(self.lvm.path(), "boards", "boardgen"),
        )
    ...
    def ScanEditItems(self) -> None:
        ...
        for type, base_only in res_types:
            items = sorted(self.core.list_json_with_path(type, recursive=base_only))
            if base_only:
                items = [(k, v) for k, v in items if k.startswith("_base")]
            for name, file in items:
                self.core.load_json(type, name)
        _ = self.core.presets
        _ = self.core.flash
    ...
    def UpdateEditItem(self) -> None:
        ...
        self.TextEditItem.SetLabel(self.file_map.get(item_name))
        if "pcb" in obj:
            obj = obj["pcb"]
        self.vars = obj["vars"] if "vars" in obj else {}
