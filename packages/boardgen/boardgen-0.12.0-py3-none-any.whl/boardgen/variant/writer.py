...
class VariantWriter(VariantParts):
    ...
    def generate(self, board: Board):
        ...
        ports: dict[RoleType, dict[int, dict[str, set[int]]]] = {}
            ...
            for role_type, roles in pin.items():
                ...
                for role in roles:
                    ...
                    if port not in ports[role_type][index]:
                        ports[role_type][index][port] = set()
                    ports[role_type][index][port].add(pin_number)
            ...
            for index, signals in interfaces.items():
                ...
                for signal, pin_numbers in signals.items():
                    ...
                    if len(pin_numbers) > 1:
                        for i, pin in enumerate(sorted(pin_numbers)):
                            c_name = self.pins[self.gpio_map[pin]][0]
                            self.add_item(section, f"{key}_{i}", pin, c_name)
                    else:
                        pin_number = sorted(pin_numbers)[0]
                        c_name = self.pins[self.gpio_map[pin_number]][0]
                        self.add_item(section, key, pin_number, c_name)
                    key = f"PINS_{section.name}{index}_{signal}"
                    pins_array = ", ".join(f"{pin}u" for pin in sorted(pin_numbers))
                    self.add_item(section, key, f"{{{pins_array}}}")
