...
class ReadmeParts(ABC):
    ...
    def add_text(self, *text: str) -> "ReadmeParts":
        self.items.append(" ".join(filter(None, text)))
        return self

    def add_styled(self, style: str, *text: str) -> "ReadmeParts":
        self.items.append(style + " ".join(filter(None, text)) + style)
        return self

    def add_list(self, *items: str) -> "ReadmeParts":
        self.items.append("- " + "\n- ".join(items))
        return self

    def add_ordered_list(self, *items: str) -> "ReadmeParts":
        self.items.append("\n".join(f"{i}. {v}" for i, v in enumerate(items)))
        return self

    def add_details(self, summary: str = None) -> "ReadmeParts":
        self.items.append("<details>")
        if summary:
            self.items.append(f"<summary>{summary}</summary>")
        return self

    def end_details(self) -> "ReadmeParts":
        self.items.append("</details>")
        return self
