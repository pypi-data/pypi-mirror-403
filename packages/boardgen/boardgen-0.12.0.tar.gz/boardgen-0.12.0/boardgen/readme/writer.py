...
class ReadmeWriter(ReadmeParts):
    ...
    def write(self, board: Board):
        ...
        self.add_styled(
            ...
            board.vendor,
            board.url and "-",
            board.url and self.get_link("product page", board.url),
        )
        if board.doc.links:
            self.add_list(
        ...
        if board.doc.params.manufacturer:
            mcu = f"{board.doc.params.manufacturer} {mcu}"
        ...
        if board.doc.params.layout:
            rows.append(["Partition layout", board.doc.params.layout])
        if board.doc.params.keys:
            rows.append(["Encryption keys", board.doc.params.keys])
        ...
        if board.doc.flashing:
            self.add_heading("Quick flashing guide", 2)
            flashing = board.doc.flashing
            if flashing.links:
                self.add_list(
                    *[
                        self.get_link(text, href)
                        for text, href in flashing.links.items()
                    ]
                )
            if flashing.pins:
                self.add_table(
                    ["Pin", "Connection"],
                    *[[k, v] for k, v in flashing.pins.items()],
                )
            if flashing.guide:
                self.add_ordered_list(*flashing.guide)
        ...
        if board.flash:
            ...
            header = ["Name", "Code", "Start", "Length", "End"]
            rows = []
            prev_end = 0
            for reg in flash:
                reg.hex_size_len = 0
                ...
                name = names.get(reg.name, reg.name)
                code = f"`{reg.name}`"
                rows.append([name, code] + reg.lst)
                prev_end = reg.end
            self.add_table(header, *rows)
