...
class CoreCache(ABC):
    ...
    def list_json_with_path(
        self, type: str, recursive: bool = False
    ) -> set[tuple[str, str]]:
        dirs = self.get_dirs(type)
        files = set()
        for dir in dirs:
            for file in glob(
                ...
                name = relpath(file, dir).replace("\\", "/").rpartition(".")[0]
                files.add((name, file))
        return files

    def load_json(self, type: str, name: str, path: str = None) -> dict | None:
        ...
        if not path:
            ...
            self._cache[type][name] = data
            if self.json_hook:
                self.json_hook(type, name, data, path)
            return data
        print("JSON file", type, name, "doesn't exist")
        return None
