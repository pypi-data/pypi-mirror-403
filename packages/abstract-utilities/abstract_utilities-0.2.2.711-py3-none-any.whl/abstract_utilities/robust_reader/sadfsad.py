...
def _should_skip_dir(dir_name: str, exclude_dirs: set[str]) -> bool:
    ...
    """
    return dir_name in exclude_dir
...
class shoudSkipManager(metaclass=SingletonMeta):
    def __init__(self,exclude_types=None,exclude_file_patterns=None,exclude_dir=None):
        if not hasattr(self, 'initialized') or self.initialized == False:
            ...
            self.exclude_dir = exclude_dir.copy()
            self.exclude_file_patterns = exclude_file_patterns.copy()
            self.exclude_types = exclude_types.copy()
    def should_skip(self,exclude_item=None,exclude_types=None,exclude_file_patterns=None,exclude_dir=None):
        if (exclude_dir==None and exclude_file_patterns == None and exclude_types == None) and exclude_item:
            if isinstance(item,str):
               if _should_skip_dir(dir_name=exclude_item, exclude_dir=self.exclude_dir):
                   return True
        ...
        if exclude_dir:
            if _should_skip_dir(filename=exclude_item, exclude_patterns=self.exclude_dir):
                return True
        return False
...
def collect_filepaths(
    ...
    - If an input is a directory, walk it recursively:
        • Skip any subdirectory named in `exclude_dir`
...
'''
from abstract_utilities import *
text= text.replace('exclude_dirs','exclude_dir').replace('exclude_dir','exclude_dirs').split('\n')
write_to_file(contents='\n'.join(text),file_path='file_reader.py2')
