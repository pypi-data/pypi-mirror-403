...
Run with: python examples/streaming_matcher_demo.py
"""
...
"""
    ...
    if result1:
        matched_text = matcher1.get_text(result1).split("\n")[0]
    ...
    if result2:
        matched_text = matcher2.get_text(result2).split("\n")[0]
        print(f"✅ Found: {matched_text}")
    else:
        print("❌ No match")
...
"""
    ...
    for part in query_parts:
        ...
        if result:
            preview = matcher.get_text(result)[:50].replace("\n", "\\n")
...
"""
    ...
    for chunk in typing_sequence:
        print(f"⌨️  Typing: {chunk!r}")
        result = matcher.push(chunk)

        if result:
            # Show the matched function signature
            lines = matcher.get_text(result).split("\n")
    ...
    if final_matches:
        for match in final_matches:
            matched_lines = matcher.get_text(match).split("\n")[:3]
