...
class ShapeNet64Vox(nn.Module):

    def __init__(self, hidden_dim=256):
        super(ShapeNet64Vox, self).__init__()
        # accepts 128**3 res input
        self.conv_in = nn.Conv3d(1, 32, 3, padding=1)  # out: 64
        ...
        feature_size = (1 + 32 + 64 + 128 + 128 ) * 7
        ...
        self.conv_in_bn = nn.BatchNorm3d(32)
        self.conv1_1_bn = nn.BatchNorm3d(64)
        self.conv2_1_bn = nn.BatchNorm3d(128)
        self.conv3_1_bn = nn.BatchNorm3d(128)
    ...
    def forward(self, p, x):
        ...
        #random_name = np.random.random(1)[0]
        #torch.save(p,'E:/debug_ifnet/point_'+str(random_name)+'.pt')
        #torch.save(x,'E:/debug_ifnet/img_'+str(random_name)+'.pt')
        ...
        features = torch.cat((feature_0, feature_1, feature_3, feature_4, feature_5),
                             dim=1)  # (B, features, 1,7,sample_num)
        shape = features.shape
        features = torch.reshape(features,
                                 (shape[0], shape[1] * shape[3], shape[4]))  # (B, featues_per_sample, samples_num)
...
class ShapeNet128Vox(nn.Module):
    ...
    def __init__(self, hidden_dim=256):
        ...
        self.fc_out = nn.Conv1d(hidden_dim, 1, 1)
        self.actvn = nn.ReLU()
        self.fc_out2 = nn.Conv1d(hidden_dim, 3, 1)
        self.actvn2 = nn.Softmax()
    ...
    def forward(self, p, x):
        ...
        net = self.actvn(self.fc_1(net))
        net = self.actvn(self.fc_2(net))
        net1 = self.fc_out(net)
        out = net1.squeeze(1)
        
        net2 = self.fc_out2(net)
        out2 = net2.squeeze(1)

        return out, out2
    
class ShapeNet256Vox(nn.Module):

    def __init__(self, hidden_dim=256):
        super(ShapeNet256Vox, self).__init__()
        self.dropout_rate = 0.2
        
        # accepts 128**3 res input
        self.conv_in = nn.Conv3d(1, 8, 3, padding=1)  # out: 256
        self.conv_a = nn.Conv3d(8, 16, 3, padding=1)  # out: 128
        self.conv_a_1 = nn.Conv3d(16, 16, 3, padding=1)  # out: 128
        self.conv_0 = nn.Conv3d(16, 32, 3, padding=1)  # out: 64
        self.conv_0_1 = nn.Conv3d(32, 32, 3, padding=1)  # out: 64
        self.conv_1 = nn.Conv3d(32, 64, 3, padding=1)  # out: 32
        self.conv_1_1 = nn.Conv3d(64, 64, 3, padding=1)  # out: 32
        ...
        self.drop = nn.Dropout3d(p=self.dropout_rate)

        feature_size = (1 + 8 + 16 + 32 + 64 + 128 + 128 ) * 7
        self.fc_0 = nn.Conv1d(feature_size, hidden_dim, 1)
        self.fc_1 = nn.Conv1d(hidden_dim, hidden_dim, 1)
        self.fc_2 = nn.Conv1d(hidden_dim, hidden_dim, 1)
        self.fc_out = nn.Conv1d(hidden_dim, 1, 1)
        ...
        self.conv_in_bn = nn.BatchNorm3d(8)
        self.conva_1_bn = nn.BatchNorm3d(16)
        self.conv0_1_bn = nn.BatchNorm3d(32)
        self.conv1_1_bn = nn.BatchNorm3d(64)
        self.conv2_1_bn = nn.BatchNorm3d(128)
        self.conv3_1_bn = nn.BatchNorm3d(128)
    ...
    def forward(self, p, x):
        ...
        net = self.actvn(self.conv_a(net))
        net = self.actvn(self.conv_a_1(net))
        net = self.conva_1_bn(net)
        feature_a = F.grid_sample(net, p,align_corners=True)  # out : (B,C (of x), 1,1,sample_num)
        net = self.drop(net)
        net = self.maxpool(net)
        ...
        features = torch.cat((feature_0, feature_a, feature_1, feature_2, feature_3, feature_4, feature_5),
                             dim=1)  # (B, features, 1,7,sample_num)
        shape = features.shape
        features = torch.reshape(features,
                                 (shape[0], shape[1] * shape[3], shape[4]))  # (B, featues_per_sample, samples_num)
...
class ShapeNet512Vox_true(nn.Module):

    def __init__(self, hidden_dim=256):
        super(ShapeNet512Vox, self).__init__()
        #self.dropout_rate = 0
        
        # accepts 128**3 res input
        self.conv_in = nn.Conv3d(1, 4, 3, padding=1)  # out: 512
        self.conv_b = nn.Conv3d(4, 8, 3, padding=1)  # out: 256
        self.conv_b_1 = nn.Conv3d(8, 8, 3, padding=1)  # out: 256
        self.conv_a = nn.Conv3d(8, 16, 3, padding=1)  # out: 128
        self.conv_a_1 = nn.Conv3d(16, 16, 3, padding=1)  # out: 128
        self.conv_0 = nn.Conv3d(16, 32, 3, padding=1)  # out: 64
        self.conv_0_1 = nn.Conv3d(32, 32, 3, padding=1)  # out: 64
        ...
        feature_size = (1 + 4 + 8 + 16 + 32 + 64 + 128 + 128 ) * 7
        self.fc_0 = nn.Conv1d(feature_size, hidden_dim, 1)
        self.fc_1 = nn.Conv1d(hidden_dim, hidden_dim, 1)
        self.fc_2 = nn.Conv1d(hidden_dim, hidden_dim, 1)
        self.fc_out = nn.Conv1d(hidden_dim, 1, 1)
        ...
        self.conv_in_bn = nn.BatchNorm3d(4)
        self.convb_1_bn = nn.BatchNorm3d(8)
        self.conva_1_bn = nn.BatchNorm3d(16)
        self.conv0_1_bn = nn.BatchNorm3d(32)
        self.conv1_1_bn = nn.BatchNorm3d(64)
        self.conv2_1_bn = nn.BatchNorm3d(128)
    ...
    def forward(self, p, x):
        ...
        net = self.actvn(self.conv_b(net))
        net = self.actvn(self.conv_b_1(net))
        #net = self.convb_1_bn(net)
        feature_b = F.grid_sample(net, p,align_corners=True)  # out : (B,C (of x), 1,1,sample_num)
        #net = self.drop(net)
        net = self.maxpool(net)
        ...
        features = torch.cat((feature_0, feature_b, feature_a, feature_1, feature_2, feature_3, feature_4, feature_5),
                             dim=1)  # (B, features, 1,7,sample_num)
        shape = features.shape
        features = torch.reshape(features,
                                 (shape[0], shape[1] * shape[3], shape[4]))  # (B, featues_per_sample, samples_num)
...
class ShapeNet512Vox(nn.Module):
    ...
    def __init__(self, hidden_dim=256):
        ...
        #self.conv_b = nn.Conv3d(4, 8, 3, padding=1)  # out: 256
        #self.conv_b_1 = nn.Conv3d(8, 8, 3, padding=1)  # out: 256
        self.conv_a = nn.Conv3d(4, 16, 3, padding=1)  # out: 128
        self.conv_a_1 = nn.Conv3d(16, 16, 3, padding=1)  # out: 128
        self.conv_0 = nn.Conv3d(16, 32, 3, padding=1)  # out: 64
        self.conv_0_1 = nn.Conv3d(32, 32, 3, padding=1)  # out: 64
        self.conv_1 = nn.Conv3d(32, 64, 3, padding=1)  # out: 32
        ...
        feature_size = (1 + 4 + 16 + 32 + 64 + 128 + 128 ) * 7
        self.fc_0 = nn.Conv1d(feature_size, hidden_dim, 1)
        self.fc_1 = nn.Conv1d(hidden_dim, hidden_dim, 1)
        self.fc_2 = nn.Conv1d(hidden_dim, hidden_dim, 1)
        self.fc_out = nn.Conv1d(hidden_dim, 1, 1)
