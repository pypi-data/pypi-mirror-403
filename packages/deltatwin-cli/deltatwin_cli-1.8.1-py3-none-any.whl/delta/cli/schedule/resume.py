...
@click.command(
    name='resume',
    short_help='resume a scheduled DeltaTwin® component execution')
@click.option(
    '--conf',
    '-c',
    type=str,
...
def resume_deltatwin_schedule(conf, schedule_id, format_output: str):
    """
    Resume a specific DeltaTwin® schedule depending
    on the DeltaTwin® component schedule ID.
    """
    schedules = API.resume_scheduled_run(conf, schedule_id)
