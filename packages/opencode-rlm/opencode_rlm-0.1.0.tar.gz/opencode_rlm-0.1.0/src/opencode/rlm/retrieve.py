...
from opencode.config.storage import ContextBlock, Message, PartType
...
from opencode.rlm.context import TokenCounter
...
class ContextRetriever:
    ...
    def retrieve_for_query(
        ...
        # Filter by threshold
        threshold = config.rlm.semantic_search.get("proactive_threshold", 0.4)
...
def get_by_session(session_id: str) -> list[dict]:
    ...
    searcher = get_searcher()
    blocks = searcher.get_recent(session_id=session_id, limit=1000)
    return [b.to_dict() for b in blocks]
...
def list_tasks() -> list[dict]:
    ...
    searcher = get_searcher()
    blocks = searcher.get_recent(limit=1000)
    tasks = []
    for block in blocks:
        if block.task_id:
            tasks.append(
