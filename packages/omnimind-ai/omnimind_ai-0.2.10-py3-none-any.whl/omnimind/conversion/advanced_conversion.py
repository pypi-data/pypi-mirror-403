...
def full_model_conversion(
    ...
    # 🔥 CRITICAL: Explicit embedding and lm_head copying FIRST
    print("   🔍 Copying embedding and lm_head weights...")
    embed_copied = False
    head_copied = False
    
    # 1. Embeddings - explicit copy with shape handling
    # Handle various naming conventions: embed_tokens, embedding, wte, word_embeddings
    for src_name, src_param in source_params.items():
        src_is_embed = any(x in src_name.lower() for x in ['embed_tokens', 'embedding', 'wte', 'word_embed'])
        if src_is_embed and 'weight' in src_name:
            for tgt_name, tgt_param in target_params.items():
                tgt_is_embed = any(x in tgt_name.lower() for x in ['embed_tokens', 'embedding', 'wte', 'word_embed'])
                if tgt_is_embed and 'weight' in tgt_name:
                    print(f"      📍 Embedding: {src_name} {src_param.shape} → {tgt_name}")
                    if src_param.shape == tgt_param.shape:
                        tgt_param.data.copy_(src_param.data)
                    else:
                        # Handle shape mismatch
                        min_vocab = min(src_param.shape[0], tgt_param.shape[0])
                        min_dim = min(src_param.shape[1], tgt_param.shape[1])
                        tgt_param.data[:min_vocab, :min_dim] = src_param.data[:min_vocab, :min_dim]
                        print(f"      ⚠️ Shape mismatch, copied {min_vocab}x{min_dim}")
                    stats["embeddings"] += 1
                    embed_copied = True
                    print(f"      ✅ Embedding copied!")
                    break
            if embed_copied:
                break
    
    # 1b. LM Head - explicit copy
    for src_name, src_param in source_params.items():
        if 'lm_head' in src_name and 'weight' in src_name:
            for tgt_name, tgt_param in target_params.items():
                if 'lm_head' in tgt_name:
                    print(f"      📍 LM Head: {src_name} {src_param.shape} → {tgt_name}")
                    if src_param.shape == tgt_param.shape:
                        tgt_param.data.copy_(src_param.data)
                    else:
                        min_vocab = min(src_param.shape[0], tgt_param.shape[0])
                    ...
                    stats["lm_head"] += 1
                    head_copied = True
                    print(f"      ✅ LM Head copied!")
                    break
            if head_copied:
                break
    
    if not embed_copied:
        print(f"      ⚠️ WARNING: Embedding not copied! Check source model structure.")
    if not head_copied:
        print(f"      ⚠️ WARNING: LM Head not copied! Check source model structure.")
    ...
    # 3. Norms (lm_head already copied above)
    for src_name, src_param in source_params.items():
...
def convert_from_native_format(
    ...
    # 🔥 CRITICAL: First pass - find and copy embedding & lm_head explicitly
    embed_copied = False
    head_copied = False
    
    print(f"   🔍 Scanning for embedding and lm_head weights...")
    
    for sf_file in safetensors_files:
        with safe_open(str(sf_file), framework="pt", device=device) as st:
            for name in st.keys():
                # 🔥 EXPLICIT embedding detection (model.embed_tokens.weight, model.embedding.weight)
                src_is_embed = any(x in name.lower() for x in ['embed_tokens', 'embedding', 'wte', 'word_embed'])
                if not embed_copied and src_is_embed and 'weight' in name:
                        tensor = st.get_tensor(name).to(torch.bfloat16)
                        print(f"      📍 Found embedding: {name} {tensor.shape}")
                        
                        # Find target embedding parameter
                        for tgt_name, tgt_param in target_params.items():
                            if 'embed_tokens' in tgt_name or 'embed' in tgt_name.lower():
                                # Copy with shape handling
                                if tensor.shape == tgt_param.shape:
                                    tgt_param.data.copy_(tensor)
                                else:
                                    # Handle vocab size mismatch
                                    min_vocab = min(tensor.shape[0], tgt_param.shape[0])
                                    min_dim = min(tensor.shape[1], tgt_param.shape[1])
                                    tgt_param.data[:min_vocab, :min_dim] = tensor[:min_vocab, :min_dim]
                                    print(f"      ⚠️ Shape mismatch, copied {min_vocab}x{min_dim}")
                                stats["embeddings"] += 1
                                embed_copied = True
                                print(f"      ✅ Copied embedding to {tgt_name}")
                                break
                        del tensor
                
                # 🔥 EXPLICIT lm_head detection
                if not head_copied and 'lm_head' in name:
                    if 'weight' in name:
                        tensor = st.get_tensor(name).to(torch.bfloat16)
                        print(f"      📍 Found lm_head: {name} {tensor.shape}")
                        ...
                        for tgt_name, tgt_param in target_params.items():
                            if 'lm_head' in tgt_name:
                                ...
                                stats["lm_head"] += 1
                                head_copied = True
                                print(f"      ✅ Copied lm_head to {tgt_name}")
                                break
                        del tensor
    
    if not embed_copied:
        print(f"      ⚠️ WARNING: Embedding not found or not copied!")
    if not head_copied:
        print(f"      ⚠️ WARNING: LM Head not found or not copied!")
    
    # Second pass - process other weights
    for sf_file in safetensors_files:
        with safe_open(str(sf_file), framework="pt", device=device) as st:
            for name in st.keys():
                ...
                # Handle embeddings (fallback for other embedding formats)
                if is_embed and not embed_copied:
                    for tgt_name, tgt_param in target_params.items():
