...
class SQLiteWeightStorage:
    ...
    def _tensor_to_bytes(self, tensor: torch.Tensor) -> bytes:
        ...
        by using view() to reinterpret as int16/uint8 WITHOUT copying data.
        This is memory-efficient (no RAM doubling) and the original dtype 
        is preserved in the database and restored on load.
        """
        buffer = io.BytesIO()
        ...
        if original_dtype in self._SPECIAL_DTYPES:
            # Use view() trick: reinterpret bits as int16 (same 16-bit size)
            # This does NOT copy data - just changes how bytes are interpreted
            # bfloat16 (2 bytes) -> view as int16 (2 bytes) = NO RAM increase!
            if original_dtype == torch.bfloat16:
                tensor_cpu = tensor_cpu.view(torch.int16)
            elif original_dtype in {torch.float8_e4m3fn, torch.float8_e4m3fnuz, 
                                    torch.float8_e5m2, torch.float8_e5m2fnuz}:
                # Float8 (1 byte) -> view as uint8 (1 byte)
    ...
    def _bytes_to_tensor(
        ...
        Uses view() to restore from int16/uint8 without copying data.
        """
        ...
        try:
            if dtype.startswith("torch."):
                ...
                if hasattr(torch, dtype_name):
                    ...
                    # Use view() for special dtypes (reverse of save operation)
                    if target_dtype == torch.bfloat16:
                        # int16 -> view as bfloat16 (no copy)
                        tensor = tensor.view(torch.bfloat16)
                    elif target_dtype in {torch.float8_e4m3fn, torch.float8_e4m3fnuz,
                                          torch.float8_e5m2, torch.float8_e5m2fnuz}:
                        # uint8 -> view as float8 (no copy)
