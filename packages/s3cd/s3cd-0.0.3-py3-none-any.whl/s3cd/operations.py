...
from s3cd.config import S3Client, S3ClientError
...
class S3Operations:
    ...
    def _log(self, message: str, only_verbose: bool = False, err: bool = False) -> None:
        ...
        elif not only_verbose:
            typer.echo(message, err=err)

    def validate_source_directory(self, source_path: Path) -> None:
        if not source_path.exists():
            self._log(f'Error: Source directory does not exist: {source_path}', err=True)
            raise typer.Exit(1)

        if not source_path.is_dir():
            self._log(f'Error: Source path is not a directory: {source_path}', err=True)
            raise typer.Exit(1)

    def validate_release_directory(self, source_path: Path, release_id: UUID) -> Path:
        release_path = source_path / str(release_id)

        if not release_path.exists():
            self._log(f'Error: Directory {release_id} not found in {source_path}', err=True)
            raise typer.Exit(1)

        if not release_path.is_dir():
            self._log(f'Error: {release_id} is not a directory in {source_path}', err=True)
            raise typer.Exit(1)
    ...
    def validate_index_html(self, release_path: Path) -> None:
        ...
        if not index_html_path.exists():
            self._log(f'Error: index.html not found in {release_path.name} directory', err=True)
            raise typer.Exit(1)
    ...
    def upload(self, source_dir: str) -> None:
        ...
        self.validate_source_directory(source_path)
        self.validate_source_directory(source_release_path)
    ...
    def _upload_info_json(self) -> None:
        info_generator = ReleaseInfoGenerator()
        info_json = info_generator.generate()

        info_key = '.cds/release.json'
        self._log(f'Uploading release.json to s3://{self.config.cds.s3.bucket}/{info_key}')

        try:
            self.s3_client.put_object(self.config.cds.s3.bucket, info_key, info_json.encode('utf-8'))
