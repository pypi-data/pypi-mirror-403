...
class ListStdDevTransformer(
    ...
    """
    Calculate the listwise standard deviation across the query id column.
    - If inputCol is set, the transformer calculates the stddev of the input column
    based on all the items in the same query id column.
    - If inputCols is set, the transformer calculates the stddev of the first column
    ...
    Example: calculate the stddev price in the same query, based on the top N
    items sorted by descending production.

    :param inputCol: Value column, on which to calculate the stddev.
    :param inputCols: Input column name.
    - The first is the value column, on which to calculate the stddev.
