...
class Request1C:
    ...
    def authenticate(self, url, only_headers=False):
        ...
        if ('ddos' in resp.headers.get('Server').lower()):
            raise DDOSProtectionError('DDOS protection during authentication')
        ...
        if ('ddos' in resp.headers.get('Server').lower()):
            raise DDOSProtectionError('DDOS protection during authentication after login')
