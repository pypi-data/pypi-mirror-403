...
def get_download_url_by_filetype(**kwargs):
    ...
    __data = get_data(releasesURL+"/version_files?nick={product}&ver={version}".format(**kwargs),kwargs.get("flush_cookie"))

    __file_list = [x for x in __data if x.get('filetype')==kwargs.get("filetype")]
    if len(__file_list) == 0:
        __file_list_wo_filter = [x for x in __data]
        raise WrongFiletype(kwargs.get("filetype"),__file_list_wo_filter)
