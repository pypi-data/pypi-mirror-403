...
from unrealircd_rpc_py.modules.tosql.models import Channel, ChannelMembers, Client, NameBan, User, Server, ClientServer
    ...
    def channel_tosql(self) -> bool:
        ...
        for rpc_channel in rpc_channels:
            ...
            if not isinstance(_c['creation_time'], datetime):
                _c['creation_time'] = datetime.strptime(_c['creation_time'], self._date_format) if _c['creation_time'] is not None else None

            keys_pop = ['bans', 'ban_exemptions', 'invite_exceptions', 'members', 'error']
        ...
        if sql.insert_multiple_objs_to_db(dbmodels):
            sql.logs.debug('Channels and Channel Members inserted into database!')
            return True
    ...
    def client_tosql(self) -> bool:
        ...
        for rpc_client in rpc_clients:
            ...
            if not isinstance(_c['connected_since'], datetime):
                _c['connected_since'] = datetime.strptime(_c['connected_since'], self._date_format) if _c['connected_since'] is not None else None
                _c['idle_since'] = datetime.strptime(_c['idle_since'], self._date_format) if _c['idle_since'] is not None else None
    ...
    def nameban_tosql(self) -> bool:
        ...
        for rpc_nb in rpc_nbs:
            ...
            if not isinstance(_c['set_at'], datetime):
                _c['set_at'] = datetime.strptime(_c['set_at'], self._date_format) if _c['set_at'] is not None else None
                _c['expire_at'] = datetime.strptime(_c['expire_at'], self._date_format) if _c['expire_at'] is not None else None
    ...
    def client_server_tosql(self) -> bool:
        ...
        for rpc_cserv in rpc_servs:
            ...
            if not isinstance(_cs['connected_since'], datetime):
                _cs['connected_since'] = datetime.strptime(_cs['connected_since'], self._date_format) if _cs['connected_since'] is not None else None
                _cs['idle_since'] = datetime.strptime(_cs['idle_since'], self._date_format) if _cs['idle_since'] is not None else None

            _server = rpc_cserv.server.to_dict()
            if not isinstance(_server['boot_time'], datetime):
                _server['boot_time'] = datetime.strptime(_server['boot_time'], self._date_format) if _server['boot_time'] is not None else None
        ...
        if sql.insert_multiple_objs_to_db(db_clientserver_models):
            sql.logs.debug('Client Server & Server have been inserted into database!')
            return True
