...
#let conf(
  ...
  set page(
    paper: "a5",
    flipped: false,
    margin: (left: 2.0cm, top: 1.5cm, right: 2cm, bottom: 1.5cm),
    header: none,
...
//  set list(block: true)
  ...
  show heading.where(level: 1): it => {
    ...
    align(left, block(above: 3.0em, below: 1.0em, width: 100% )[
      #set text(font: font, weight: "bold", size: 16pt, fill:rgb("#00008B"))
      #block(it.body)
    ])
  }

  show heading.where(level: 2
    ): it => align(left, block(above: 2.0em, below: 1.0em, width: 100%)[
        #set text(font: font, weight: "semibold", size: 11pt, fill: rgb("#00008B"))
        #block(it.body) 
      ])

  show heading.where(level: 3
    ): it => align(left, block(above: 2.5em, below: 1.0em, width: 100%)[
        #set text(font: font, weight: "semibold", size: 10pt, fill: rgb("#00008B"))
        #block(it.body) 
      ])
  ...
  show regex("https://coinmarketcap.com/academy/glossary/.*"): it => {
    v(1fr)
    text(size: 6pt, fill: rgb("#444444"),it)
  }
