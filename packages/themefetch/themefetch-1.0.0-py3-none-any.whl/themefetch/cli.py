...
def batch_command(
    ...
    def process_show(show: dict) -> dict:
        ...
        Returns:
            dict with keys: show, status (success/failed/skipped), reason
        """
        show_name = show["show_name"]
        year = show["year"]
        show_path = show["path"]
    ...
    logger.info(f"Successful: {success_count}/{len(shows)}")
    logger.info(f"Skipped: {skipped_count}")
    logger.info(f"Failed: {failed_count}")

    # Show detailed failure list if any failures occurred
    if failed_count > 0:
        logger.info("")
        ...
        for result in results:
            if result["status"] == "failed":
                ...
                logger.info(f"  - {show['show_name']} ({show['year']}): {reason}")
