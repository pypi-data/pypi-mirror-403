...
- Видалено sync wait() - використовуйте wait_async() для async контексту
- Всі затримки через asyncio.sleep() (NON-BLOCKING)
- Збережено sync версію тільки для зворотної сумісності з попередженням
...
class AdaptiveThrottler:
    ...
    def __init__(
        ...
        logger.info(
            f"✅ AdaptiveThrottler initialized (async-only): "
    ...
    def _adjust_delay(self) -> None:
        ...
        if error_rate > self.error_threshold:
            ...
            logger.warning(
                f"⚠️ High error rate {error_rate:.1f}% → "
