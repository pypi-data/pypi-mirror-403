...
class Project:
    def __init__(self, project: str, name: str, token: Optional[str] = None) -> None:
        if not project:
    ...
    def list(self, status: Optional[str] = None, *, return_issue: bool = False) -> List[Any]:
        ...
        matches = [item for item in items if (item.status or "").lower() == status_name.lower()]
        if return_issue:
            return matches
    ...
    def take(
        ...
        *,
        ...
        if issue_id is None:
            ...
            raise TakeError(f"No available issues to take in status '{status_name}'")
    ...
    def _ensure_label(self, repo: str, label: str) -> None:
        ...
        try:
            self.client.rest(
                ...
                json_body={"name": label, "color": "ededed", "description": "Task owner"},
            )
        except ApiError as exc:
