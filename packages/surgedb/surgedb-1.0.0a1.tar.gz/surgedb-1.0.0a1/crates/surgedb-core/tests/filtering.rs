use surgedb_core::{Config, VectorDb, VectorId};
...
fn test_metadata_filtering() {
    ...
    // Insert data
    // vec1: "books"
    db.insert("vec1", &[1.0, 0.0, 0.0, 0.0], Some(json!({"category": "books"}))).unwrap();
    // vec2: "movies" (similar to vec1)
    db.insert("vec2", &[1.0, 0.1, 0.0, 0.0], Some(json!({"category": "movies"}))).unwrap();
    // vec3: "books" (orthogonal)
    db.insert("vec3", &[0.0, 1.0, 0.0, 0.0], Some(json!({"category": "books"}))).unwrap();
    ...
    assert!(results_filtered.iter().all(|(id, _, _)| id.as_str() != "vec2"));
}
...
fn test_complex_filtering() {
    ...
    db.insert("v1", &[1.0, 0.0], Some(json!({"tag": "A", "val": 10}))).unwrap();
    db.insert("v2", &[1.0, 0.0], Some(json!({"tag": "B", "val": 10}))).unwrap();
    db.insert("v3", &[1.0, 0.0], Some(json!({"tag": "A", "val": 20}))).unwrap();
