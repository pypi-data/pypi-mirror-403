...
class SQLiteDatabase(DatabaseInterface):
    ...
    def get_resource_database_path(self) -> str:
        ...
        resource_db_path = os.path.join(resources_dir, "webui-0.7.2.db")

        # Normalize the path
        resource_db_path = os.path.normpath(resource_db_path)

        if not os.path.exists(resource_db_path):
            logger.error(f"Resource database not found: {resource_db_path}")
            raise DatabaseReplacementError(
                f"Resource database not found: {resource_db_path}"
            )
    ...
    def validate_and_repair_database(self, reset_active: bool) -> None:
        ...
        """
        logger.info("Starting database validation and repair process")
