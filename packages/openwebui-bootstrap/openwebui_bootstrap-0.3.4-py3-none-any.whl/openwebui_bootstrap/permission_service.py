...
from .models import UserEntity, GroupEntity, GroupMemberEntity, PermissionSystemConfig
...
class PermissionService:
    ...
    def get_effective_permissions(self, user: UserEntity) -> PermissionSystemConfig:
        ...
        # Merge group permissions (groups are applied in order, later groups override earlier ones)
        ...
        # Apply user-specific permission overrides (user permissions take highest precedence)
        if user.permissions:
            effective_permissions = self._merge_permissions(
    ...
    def has_permission(
        ...
        Args:
            ...
            permission_path: Dot-separated path to the permission (e.g., 'workspace.models')
    ...
    def _get_base_permissions_for_role(self, role: str) -> PermissionSystemConfig:
        ...
        if role.lower() == "admin":
            ...
            return PermissionSystemConfig(
                workspace={k: True for k in PermissionSystemConfig().workspace.keys()},
                chat={k: True for k in PermissionSystemConfig().chat.keys()},
                features={k: True for k in PermissionSystemConfig().features.keys()},
                sharing={k: True for k in PermissionSystemConfig().sharing.keys()},
                settings={k: True for k in PermissionSystemConfig().settings.keys()},
            )
            ...
            return PermissionSystemConfig(
                workspace={k: False for k in PermissionSystemConfig().workspace.keys()},
                chat={k: False for k in PermissionSystemConfig().chat.keys()},
                features={k: False for k in PermissionSystemConfig().features.keys()},
                sharing={k: False for k in PermissionSystemConfig().sharing.keys()},
                settings={k: False for k in PermissionSystemConfig().settings.keys()},
            )
