...
class ConfigManager:
    ...
    def apply_config(self, config: OpenWebUIConfig) -> None:
        """Apply configuration to the database.
    ...
    def _create_group_entity(self, group_config: GroupConfig) -> GroupEntity:
        ...
        """
        # Find the first user as group owner
        # For now, use a default admin user ID
        owner_id = uuid4()  # This would be replaced with actual logic
    ...
    def _create_model_entity(
        ...
        first_user_email = config.users[0].email if config.users else None
        first_user_id = None

        if first_user_email:
            try:
                first_user = self.database.get_user_by_email(first_user_email)
                if first_user:
                    first_user_id = first_user.id
            except Exception:
                # If we can't get the user, use a placeholder UUID
                first_user_id = uuid4()
