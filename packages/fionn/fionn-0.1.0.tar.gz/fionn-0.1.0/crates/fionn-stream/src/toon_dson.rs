...
use crate::skiptape::toon_batch::SimdToonBatchProcessor;
use fionn_core::Result;
use fionn_core::format::FormatKind;
use fionn_ops::DsonOperation;

/// TOON-DSON integration processor
pub struct ToonDsonProcessor {
    inner: FormatDsonProcessor<SimdToonBatchProcessor>,
}

impl Default for ToonDsonProcessor {
    fn default() -> Self {
        Self::new()
    }
}

impl ToonDsonProcessor {
    /// Create a new TOON-DSON processor
    #[must_use]
    pub fn new() -> Self {
        Self {
            inner: FormatDsonProcessor::new(SimdToonBatchProcessor::new()),
        }
    }

    /// Process TOON data with schema filtering and DSON operations
    ///
    /// # Errors
    ...
    /// Process TOON data with schema filtering only
    ///
    /// # Errors
    /// Returns an error if processing fails
    ...
    /// Process TOON data without filtering
    ///
    /// # Errors
    /// Returns an error if processing fails
    ...
    /// Process TOON data with DSON operations only
    ///
    /// # Errors
    /// Returns an error if processing fails
...
mod tests {
    ...
    fn test_processor_creation() {
        let processor = ToonDsonProcessor::new();
        assert_eq!(processor.format_kind(), FormatKind::Toon);
    }
    ...
    fn test_process_with_operations() {
        ...
        let operations = vec![DsonOperation::FieldAdd {
            ...
            value: OperationValue::StringRef("toon".to_string()),
        }];
