...
class BoundaryType:
    """Base class for handling Boundary Type data i.e. the individual entries in the bc tables check file.
...
class BoundaryTypeBC(BoundaryType):
    """Base class for handling :code:`BC` boundary data coming from BC check files.
...
class BoundaryTypeQT(BoundaryTypeBC):
    """Class for handling :code:`QT` boundary data coming from BC check files.
...
class BoundaryTypeHT(BoundaryTypeBC):
    """Class for handling :code:`HT` boundary data coming from BC check files.
    ...
    def __init__(self, line: str) -> None:
        super().__init__(line)
        self.type = 'HT'
        self.data_type = 'water level'


class BoundaryTypeHQ(BoundaryTypeBC):
    """Class for handling :code:`HQ` boundary data coming from BC check files.
    ...
    def __init__(self, line: str) -> None:
        ...
        if not self.name:
            self.name = self.id
            self.valid = True
        self.data_type = 'rating curve'


class BoundaryTypeSA(BoundaryTypeBC):
    """Class for handling :code:`SA` boundary data coming from BC check files.
...
class BoundaryTypeRF(BoundaryTypeBC):
    """Class for handling :code:`RF` boundary data coming from BC check files.
