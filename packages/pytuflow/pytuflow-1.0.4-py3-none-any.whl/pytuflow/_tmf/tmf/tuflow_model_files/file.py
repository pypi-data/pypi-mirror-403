...
class TuflowPath(Path):
    ...
    def lyrname(self):
        ...
        if str(self).lower().startswith('netcdf:'):
            p = str(self)[len('netcdf:'):].strip('"').strip("'")
            if os.path.splitext(p)[1].lower() == '.nc':
                return None
    ...
    def open_gis(self, mode='r'):
        ...
        OGROpen
            ...
            these attributes.
        """
        from .gis import has_gdal, has_geopandas, _prefer_gdal

        if _prefer_gdal and has_gdal:
            return OGROpen(self, mode)
        elif has_geopandas:
            return PyOGRIOOpen(self, mode)
    ...
    def open_grid(self, mode: str = 'r', band: int = 1):
        from .gis import has_gdal, has_rasterio, _prefer_gdal
        ...
        if self.suffix.lower() == '.gpkg':
            ...
            try:
                ...
                with Dataset(self.dbpath, 'r') as nc:
                    lyrname = self.lyrname.lower()
                    vars = [var.lower() for var in nc.variables.keys()]
                    return lyrname in vars
            except ImportError:
                pass
            ...
            try:
                ...
                with File(self.dbpath, 'r') as h5:
                    lyrname = self.lyrname.lower()
                    vars = [var.lower() for var in h5.keys()]
                    return lyrname in vars
            except ImportError:
                pass
