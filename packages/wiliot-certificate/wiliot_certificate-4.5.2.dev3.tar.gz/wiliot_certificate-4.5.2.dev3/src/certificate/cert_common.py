...
def get_pkts_data_frame(test, gw_data=False, brg_data=False, per_pkt_type=False):
    ...
    for p in pkts:
        ...
        if type(p[DECODED_DATA][PACKET_TYPE]) == str or p[DECODED_DATA][PACKET_TYPE] == None:
            cert_prints.wlt_print(f"Skipped packet {p}", log_level=cert_prints.DEBUG)
            continue
        if per_pkt_type:
...
def get_gw_type(mqttc):
    ...
    for msg in messages:
        if GW_STATUS in msg: # protobuf
            return msg[GW_STATUS].get(GW_TYPE, None), msg
        if GW_CONF in msg: # JSON
            return msg.get(GW_TYPE), msg
    return None, None
...
def get_gw_api_version(mqttc):
    ...
    for msg in messages:
        if GW_CONF in msg: # JSON
            return msg[GW_CONF].get(GW_API_VERSION)
        if GW_STATUS in msg: # protobuf
            return msg[GW_STATUS].get(GW_API_VERSION)
    return None
...
def exp_sig_ind_pkts(tx_brg, rx_brg, cycles):
    if tx_brg.single_polarization_antenna:
        tx_brg_ant_polarization_num = 1
    else:
        tx_brg_ant_polarization_num = 2
    if rx_brg.single_polarization_antenna:
...
def rx_tx_antenna_check(test, received_signal_ind_pkts, tx_brg_, rx_brg_, cycles):
    ...
    if not tx_brg_.single_polarization_antenna:
    ...
    if not rx_brg_.single_polarization_antenna:
    ...
    if not rx_brg_.single_polarization_antenna and not tx_brg_.single_polarization_antenna:
...
def output_power_supported(module, output_power):
    return ag.reverse_mapping(module.field_mapping['outputPower'], output_power) in module.field_supported_values['outputPower']
