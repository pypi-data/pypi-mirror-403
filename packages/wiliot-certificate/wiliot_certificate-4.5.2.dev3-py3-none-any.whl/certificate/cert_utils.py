...
SINGLE_POLARIZATION_ANTENNA_PER_TESTER_BOARD_TYPE = {ag.BOARD_TYPE_FANSTEL_WIFI_V0: True, ag.BOARD_TYPE_FANSTEL_LAN_V0: True, ag.BOARD_TYPE_MINEW_POE_V0: False}
    ...
    def __init__(self, line, tester, dut, brg1=None, exit_on_param_failure=False,
        ...
        if line:
            ...
            for s in self.name.split('/'):
                module_py_name = module_name_py2id(s)
                if module_py_name:
...
class Bridge:
    def __init__(self, id_str="", board_type=0, cfg_hash=0, api_version=ag.API_VERSION_LATEST, interface_pkt=None, max_output_power_dbm=None, single_polarization_antenna=False, rel_path=".", validation_schema=None):
        """
        ...
        Args:
            ...
            single_polarization_antenna: Whether the bridge has a single polarization antenna (default: False)
                    ...
                    for attribute in self.validation_schema[module]["properties"][field].keys():
                        if attribute not in module_class.field_metadata[field]:
                            ...
                            if hasattr(module_class, "field_supported_values") and field in module_class.field_supported_values:
                                ...
                                # copy the supported enum values from the schema
                                module_class.field_supported_values[field] = copy.deepcopy(self.validation_schema[module]["properties"][field][attribute])
                            else:
        ...
        self.single_polarization_antenna = single_polarization_antenna
        self.sup_caps = []
        self.modules = []
        if interface_pkt:
...
def ut_prep_brg(args, start_time, tester, brg_id, validation_schema, is_tester_brg=False):
    ...
    single_polarization_antenna = SINGLE_POLARIZATION_ANTENNA_PER_TESTER_BOARD_TYPE[board_type] if is_tester_brg else args.single_polarization_antenna
    test.active_brg = Bridge(brg.id_str, interface_pkt=interface, max_output_power_dbm=max_output_power, single_polarization_antenna=single_polarization_antenna, validation_schema=validation_schema)
    ...
    return Bridge(brg.id_str, interface_pkt=interface, max_output_power_dbm=max_output_power, single_polarization_antenna=single_polarization_antenna, validation_schema=validation_schema)
