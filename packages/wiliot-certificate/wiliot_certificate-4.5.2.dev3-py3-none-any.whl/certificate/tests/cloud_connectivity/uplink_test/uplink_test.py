...
import common.api_if.api_validation as api_if
...
def geolocation(test, all_messages_in_test):
    ...
    if test.get_phase_rc(GEOLOCATION) == TEST_SKIPPED:
        ...
        test.add_phase_reason(GEOLOCATION, 'Accuracy uploaded' if accuracy_uploaded else 'No accuracy')
...
def run(test):
    ...
    for param in test.params:
        ...
        valid, reason = api_if.validate_received_packets(recieved_pkts)
        if valid is False:
    ...
    # Other Validations
    # Api Validation
    test = api_if.api_validation(test, msg_type=api_if.MESSAGE_TYPES.DATA, msgs=[msg.body for msg in all_data_messages_in_test])
