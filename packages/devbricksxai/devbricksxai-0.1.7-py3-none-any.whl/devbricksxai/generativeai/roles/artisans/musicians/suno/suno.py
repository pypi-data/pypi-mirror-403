...
class Suno():
    ...
    def __init__(self, cookie: Optional[str] = None, model_version: str = ModelVersions.CHIRP_V3_5) -> None:
        ...
        if cookie is None:
            cookie = COOKIE
        if cookie == "":
            logger.error("Suno cookie not provided. API may not work.")
            return
    ...
    def _wait_for_audio(self, song_ids):
        ...
        while time.time() - start_time < 100:  # Timeout after 100 seconds
            try:
                ...
                all_completed = all(
                    audio.status in ['complete'] for audio in clips)
                if all_completed:
                    logger.info("Generated Audio Successfully ✅")
                    return clips
                last_clips = clips
    ...
    def get_songs(self, song_ids: Optional[Union[List[str], str]] = None) -> List[Clip]:
        """
    ...
    def get_lyrics(self, id: str):
        """
        Retrieve lyrics of a song by its ID.
        ...
        """
        self._keep_alive()  # Ensure session is active
        logger.info(f"Getting Song Lyrics...{id}")
        response = self.client.get(
            f"{Suno.BASE_URL}/api/generate/lyrics/{id}")  # Call API
        logger.debug(response.text)
    ...
    def download(self, song: Optional[Union[str, Clip]], path: str = "./downloads",) -> str:
        """
