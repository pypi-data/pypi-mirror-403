...
class SortedMapLibrary(Library):
    ...
    def servicers(self):
        if self.authorizer is None:
            ...
            # Capture locally because `self` gets re-defined.
            overriding_authorizer = self.authorizer

            class SortedMapServicerWithAuth(SortedMapServicer):
                """Subclass SortedMapServicer but with a new authorizer."""

                def authorizer(self):
                    return overriding_authorizer

            return [SortedMapServicerWithAuth]
