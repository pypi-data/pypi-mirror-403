...
class Projection3D:
    def __init__(
        ...
        val_raw_dir="val_raw/",
        val_projection_dir="val_projection/",
        n_channel_in=1,
        downsample_factor=1,
        generate_npz=True,
    ...
    def Train(self):
        ...
        model = ProjectionCARE(
            config, name="projection_" + self.model_name, basedir=self.model_dir
        )

        if os.path.exists(
            self.model_dir + "projection_" + self.model_name + "/" + "weights_now.h5"
        ):
        ...
        if os.path.exists(
            self.model_dir + "projection_" + self.model_name + "/" + "weights_last.h5"
        ):
            print("Loading checkpoint model")
            model.load_weights(
                self.model_dir
        ...
        if os.path.exists(
            self.model_dir + "projection_" + self.model_name + "/" + "weights_best.h5"
        ):
            print("Loading checkpoint model")
            model.load_weights(
                self.model_dir
