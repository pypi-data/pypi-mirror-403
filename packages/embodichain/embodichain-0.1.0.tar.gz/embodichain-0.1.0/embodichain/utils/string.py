...
def is_regular_expression(pattern: str) -> bool:
    """Checks if the input string is a valid regular expression.
    Args:
        pattern: The input string to check.

    Returns:
        bool: True if the input string is a valid regular expression, False otherwise.
    """
    try:
        re.compile(pattern)
        return True
...
def remove_regex_chars(pattern: str) -> str:
    """Remove common regex metacharacters from the input pattern.
    Args:
        pattern: The input string pattern.
    ...
    regex_chars = r"[\.\*\+\?\[\]\(\)\{\}\^\$\|\\]"
    return re.sub(regex_chars, "", pattern)
...
def resolve_matching_names_values(
    ...
    Reference:
    https://github.com/isaac-sim/IsaacLab/blob/main/source/isaaclab/isaaclab/utils/string.py#L178-L271
    """
    # check valid input
