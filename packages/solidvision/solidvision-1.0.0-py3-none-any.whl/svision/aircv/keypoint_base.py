...
class KeypointMatching(object):
    ...
    def find_best_result(self):
        ...
        if len(self.good) in [0, 1]:
            ...
            middle_point, pypts, w_h_range = self._many_good_pts(
                self.kp_sch, self.kp_src, self.good
            )
    ...
    def show_match_image(self):
        ...
        for m in self.good:
            ...
            cv2.line(
                matching_info_img,
                (int(self.kp_sch[m.queryIdx].pt[0]), int(self.kp_sch[m.queryIdx].pt[1])),
                (int(self.kp_src[m.trainIdx].pt[0] + w_sch), int(self.kp_src[m.trainIdx].pt[1])),
                color,
            )
    ...
    def _handle_three_good_points(self, kp_sch, kp_src, good):
        ...
        pts_sch1 = int(kp_sch[good[0].queryIdx].pt[0]), int(kp_sch[good[0].queryIdx].pt[1])
        pts_sch2 = (
            int((kp_sch[good[1].queryIdx].pt[0] + kp_sch[good[2].queryIdx].pt[0]) / 2),
            int((kp_sch[good[1].queryIdx].pt[1] + kp_sch[good[2].queryIdx].pt[1]) / 2),
        )
        pts_src1 = int(kp_src[good[0].trainIdx].pt[0]), int(kp_src[good[0].trainIdx].pt[1])
        pts_src2 = (
            int((kp_src[good[1].trainIdx].pt[0] + kp_src[good[2].trainIdx].pt[0]) / 2),
    ...
    def _many_good_pts(self, kp_sch, kp_src, good):
        ...
        sch_pts, img_pts = (
            np.float32([kp_sch[m.queryIdx].pt for m in good]).reshape(-1, 1, 2),
            np.float32([kp_src[m.trainIdx].pt for m in good]).reshape(-1, 1, 2),
        )
        # M是转化矩阵
        ...
        sch_pts, img_pts = (
            np.float32([kp_sch[m.queryIdx].pt for m in selected]).reshape(-1, 1, 2),
            np.float32([kp_src[m.trainIdx].pt for m in selected]).reshape(-1, 1, 2),
        )
        M, mask = self._find_homography(sch_pts, img_pts)
        ...
        pts = np.float32([[x_min, y_min], [x_min, y_max], [x_max, y_max], [x_max, y_min]]).reshape(
            -1, 1, 2
        )
    ...
    def _get_origin_result_with_two_points(self, pts_sch1, pts_sch2, pts_src1, pts_src2):
        ...
        if (
            pts_sch1[0] == pts_sch2[0]
            or pts_sch1[1] == pts_sch2[1]
            or pts_src1[0] == pts_src2[0]
            or pts_src1[1] == pts_src2[1]
        ):
            return None
        ...
        sch_middle_point = (
            int((pts_sch1[0] + pts_sch2[0]) / 2),
            int((pts_sch1[1] + pts_sch2[1]) / 2),
        )
        ...
        # 超出左边界取0, 超出右边界取w_s-1, 超出下边界取0, 超出上边界取h_s-1
        x_min, x_max = (
            int(max(middle_point[0] - (w * x_scale) / 2, 0)),
            int(min(middle_point[0] + (w * x_scale) / 2, w_s - 1)),
        )
        y_min, y_max = (
            int(max(middle_point[1] - (h * y_scale) / 2, 0)),
            int(min(middle_point[1] + (h * y_scale) / 2, h_s - 1)),
        )
