...
def cocos_min_strategy(w, h, sch_resolution, src_resolution, design_resolution=(960, 640)):
    ...
    scale_sch = min(
        1.0 * sch_resolution[0] / design_resolution[0],
        1.0 * sch_resolution[1] / design_resolution[1],
    )
    scale_src = min(
        1.0 * src_resolution[0] / design_resolution[0],
        1.0 * src_resolution[1] / design_resolution[1],
    )
...
def predict_area(im_source, op_pos, radius_x, radius_y, src_resolution=None):
    ...
    if start_x == end_x or start_y == end_y:
        ...
        # 输出调试信息.
        log_info = "predict rect:  X (%(start_x)s:%(end_x)s)   Y (%(start_y)s:%(end_y)s)" % {
            "start_x": start_x,
            "end_x": end_x,
            "start_y": start_y,
            "end_y": end_y,
