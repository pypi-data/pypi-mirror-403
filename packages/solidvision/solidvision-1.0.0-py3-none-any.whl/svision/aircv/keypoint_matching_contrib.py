...
from svision.aircv.error import *  # noqa
from svision.aircv.keypoint_base import KeypointMatching
    ...
    def init_detector(self):
        ...
        if check_cv_version_is_new():
            ...
            try:
                ...
                raise NoModuleError(
                    "There is no %s module in your OpenCV environment !" % self.METHOD_NAME
                )
        else:
            # OpenCV2.x
...
class SIFTMatching(KeypointMatching):
    ...
    def init_detector(self):
        ...
        if check_cv_version_is_new():
            try:
                ...
                try:
                    ...
                    raise NoModuleError(
                        "There is no %s module in your OpenCV environment, need contrib module!"
                        % self.METHOD_NAME
                    )
        else:
            # OpenCV2.x
        ...
        self.matcher = cv2.FlannBasedMatcher(
            {"algorithm": self.FLANN_INDEX_KDTREE, "trees": 5}, dict(checks=50)
        )
        ...
        if check_cv_version_is_new():
            ...
            try:
                self.detector = cv2.xfeatures2d.SURF_create(
                    self.HESSIAN_THRESHOLD, upright=self.UPRIGHT
                )
            except:
                raise NoModuleError(
                    "There is no %s module in your OpenCV environment, need contribmodule!"
                    % self.METHOD_NAME
                )
        else:
            # OpenCV2.x
