...
from svision.aircv.aircv import crop_image
from svision.aircv.cal_confidence import cal_rgb_confidence, cal_ccoeff_confidence
from svision.aircv.error import TemplateInputError
from svision.aircv.utils import (
    generate_result,
...
class MultiScaleTemplateMatchingPre(MultiScaleTemplateMatching):
    ...
    def find_best_result(self):
        ...
        if self.resolution != ():
            ...
            check_source_larger_than_search(self.im_source, self.im_search)
            if (
                self.resolution[0] < self.im_search.shape[1]
                or self.resolution[1] < self.im_search.shape[0]
            ):
                raise TemplateInputError("error: resolution is too small.")
            # 第二步：计算模板匹配的结果矩阵res
            if self.record_pos is not None:
                area, self.resolution = self._get_area_scope(
                    self.im_source, self.im_search, self.record_pos, self.resolution
                )
                self.im_source = crop_image(self.im_source, area)
                check_source_larger_than_search(self.im_source, self.im_search)
            r_min, r_max = self._get_ratio_scope(self.im_source, self.im_search, self.resolution)
            s_gray, i_gray = img_mat_rgb_2_gray(self.im_search), img_mat_rgb_2_gray(self.im_source)
            confidence, max_loc, w, h, _ = self.multi_scale_search(
                i_gray,
                s_gray,
