...
def new(
    db_client: SnowflakeCursor,
    table_id: DbTableId,
    table: Table,
    if_not_exist: bool,
) -> Cmd[ResultE[None]]:
    enum_primary_keys = tuple(enumerate(table.primary_keys))
    enum_columns = tuple(enumerate(tuple((i, table.columns[i]) for i in table.order)))
    ...
    values = FrozenDict({f"default_{index}": c.default for index, (_, c) in enum_columns})
