...
# Extract vertices from the RDF data graph
~/.local/jena/bin/sparql \
  --data ./resilience/uc_kg_extended.ttl \
  --query ./rq_kg2pg_vertices.sparql \
  --results csv > out/kg2pg_vertices.csv

# Extract edges from the RDF data graph
~/.local/jena/bin/sparql \
  --data ./resilience/uc_kg_extended.ttl \
...
sed -e '/MY_NODES/{r out/kg2pg_vertices_only.json' -e 'd}' networkx_template.json > out/nx_data_graph.json
sed -i -e '/MY_LINKS/{r out/kg2pg_edges_only.json' -e 'd}' out/nx_data_graph.json
