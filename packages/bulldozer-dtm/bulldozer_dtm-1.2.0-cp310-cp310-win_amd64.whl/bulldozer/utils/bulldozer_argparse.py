...
class BulldozerHelpFormatter(argparse.RawTextHelpFormatter):
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        """
        argparse.RawTextHelpFormatter.__init__(self, max_help_position=55, **kwargs)

    def bulldozer_add_usage(
        self,
        positionals: Union[list, None] = None,
        optionals: Union[list, None] = None,
        commons: Union[list, None] = None,
    ...
    def _bulldozer_format_usage(self, positionals: list, optionals: list, commons: list, prefix: str) -> str:
        """
        ...
        if optionals:
            ...
            for it, param in enumerate([arg + "] " for arg in format_usage(optionals, []).split("] ") if arg]):
            ...
            usage += (
                "\n"
                + (len(prog) + len(prefix) + 1) * " "
                + " ".join([param for param in [format_usage(positionals, [])] if param])
            )
        else:
            # Formatter used for CLI with config file usage
            usage += " " + " ".join([param for param in [format_usage(positionals, [])] if param])
...
class BulldozerArgumentParser(argparse.ArgumentParser):
    ...
    def get_bulldozer_groups(self) -> tuple:
        """
        This method returns the two groups of arguments containing
        the Bulldozer parameters (required and optional).
    ...
    def format_help(
        ...
        if add_epilog:
            formatter.add_text(
                "If extra arguments are provided, these will override the original "
