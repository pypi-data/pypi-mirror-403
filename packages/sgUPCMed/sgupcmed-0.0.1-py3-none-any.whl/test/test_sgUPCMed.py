...
from sgUPCMed.sgUPCMed import SGUPCMed
from unittest.mock import patch
import sgUPCMed.sgUPCMed as sgUPcmed
...
def test_initialization():
    ...
    """
    model = SGUPCMed(C=2, m=2.0, max_iter=5, tol=0.1,
...
def test_modified_median_refine_all_violated(data):
    ...
    with patch.object(model, "_violates_hard_uniqueness", return_value=True), \
         patch.object(sgUPcmed, "candidate_score_weighted", fake_candidate_score_weighted):
...
def test_pipeline(data):
    ...
    It checks the integrity of the final prototypes, membership matrices (U_), 
    parameter (m,beta) and the objective function value (J).
    """
    model = SGUPCMed(C=2, max_iter=5)
    model.fit(data)
...
def test_compute_beta(data):
    ...
    with multiprocessing.Pool(multiprocessing.cpu_count()) as mp_pool:
        model = SGUPCMed(C=2)
        beta = model._compute_beta(data, mp_pool)
...
def test_more_clusters_than_samples(data):
    ...
    """
    model = SGUPCMed(C=30, max_iter=1)
    with pytest.raises(ValueError):
        model.fit(data)
...
def test_enforce_unique_prototypes(data):
    ...
    """
    model = SGUPCMed(C=2, enforce_unique=True)
    model.fit(data)
