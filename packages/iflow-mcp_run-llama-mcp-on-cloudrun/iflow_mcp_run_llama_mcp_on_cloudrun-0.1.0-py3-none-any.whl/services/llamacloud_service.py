...
class LlamaCloudService:
    ...
    def __init__(self):
        ...
        if not LLAMA_INDEX_AVAILABLE:
            logger.warning("llama-index package not available, using mock data")
            self.use_mock_data = True
            return
        
        if not LLAMA_CLOUD_API_KEY or LLAMA_CLOUD_API_KEY in ["llx-your-api-key-here", "llx-test-key-for-testing"]:
            logger.warning("LLAMA_CLOUD_API_KEY is not configured, using mock data")
            self.use_mock_data = True
            return
        
        if not LLAMA_CLOUD_INDEX_NAME:
            logger.warning("LLAMA_CLOUD_INDEX_NAME is not configured, using mock data")
            self.use_mock_data = True
            return
    ...
    def _get_mock_candidates(self, top_k: int) -> List[CandidateMatch]:
        """Generate mock candidate data for testing."""
        mock_candidates = [
            CandidateMatch(
                node_id="mock_1",
                score=0.95,
                content="John Doe - Senior Python Developer with 8 years experience in Django, FastAPI, and AWS. Experienced in building scalable microservices and RESTful APIs.",
                metadata={"file_name": "john_doe_resume.pdf", "experience_years": 8},
            ...
            CandidateMatch(
                node_id="mock_2",
                score=0.88,
                content="Jane Smith - Machine Learning Engineer with 5 years experience in Python, TensorFlow, and PyTorch. Expertise in NLP and computer vision projects.",
                metadata={"file_name": "jane_smith_resume.pdf", "experience_years": 5},
            ...
            CandidateMatch(
                node_id="mock_3",
                score=0.82,
                content="Bob Johnson - Full Stack Developer with 6 years experience in React, Node.js, and PostgreSQL. Strong background in both frontend and backend development.",
                metadata={"file_name": "bob_johnson_resume.pdf", "experience_years": 6},
                candidate_name="Bob Johnson",
                file_name="bob_johnson_resume.pdf"
            ),
        ]
    ...
    def _get_index(self):
        ...
        if self._index is None:
            try:
                ...
                logger.warning(f"Failed to connect to LlamaCloud index, using mock data: {e}")
                self.use_mock_data = True
    ...
    async def retrieve_candidates(
        ...
        try:
            ...
            if not index:
                logger.warning("Index not available, using mock data")
                self.use_mock_data = True
                return self._get_mock_candidates(top_k)
            ...
            logger.warning(f"Error retrieving candidates, using mock data: {e}")
            self.use_mock_data = True
            return self._get_mock_candidates(top_k)
    ...
    async def retrieve_candidates_by_qualifications(
        ...
        try:
            ...
            logger.warning(f"Error retrieving candidates by qualifications, using mock data: {e}")
            self.use_mock_data = True
            return self._get_mock_candidates(top_k)
    ...
    async def search_by_skills(
        ...
        try:
            ...
            logger.warning(f"Error searching candidates by skills, using mock data: {e}")
            self.use_mock_data = True
            return self._get_mock_candidates(top_k)