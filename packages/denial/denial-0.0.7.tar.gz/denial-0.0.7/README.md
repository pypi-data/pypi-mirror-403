...
A: Indeed, we already have one source of unique IDs for objects: their addresses in memory. Checks of the type x is sentinel can be identical in meaning to those used in this library. However, this option has two significant drawbacks. The first is that within a single process, the same memory address can be assigned to two different objects. Sometimes this can lead to hard-to-track errors when you need to verify the uniqueness of your sentinel object. The second is related to the fact that we may want to synchronize sentinels across multiple processes or even programs, we may want to transfer them over the network, and so on. Unfortunately, this is not possible with memory addresses. Since this library claims to be universal, I had to abandon this option.
