...
from rediskit.redis import client
from rediskit.redis.client import h_get_cache_from_redis, h_set_cache_to_redis
from rediskit.redis_lock import get_redis_mutex_lock
...
def maybe_data_in_cache(
    ...
    if storage_type == "string":
        cached = client.load_blob_from_redis(
            tenant_id,
...
def dump_data(
    ...
    if storage_type == "string":
        client.dump_blob_to_redis(tenant_id, computed_memoize_key, payload=payload, ttl=computed_ttl, connection=connection)
    elif storage_type == "hash":
