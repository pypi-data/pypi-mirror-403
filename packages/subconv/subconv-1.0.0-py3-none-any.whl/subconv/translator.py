...
def build_translation_prompt(blocks: list[SubtitleBlock]) -> str:
    numbered_texts = "\n".join(f"{b.number}. {b.text}" for b in blocks)
    return f"""
            Traduza as seguintes legendas para portugues brasileiro.
            Mantenha uma linguagem natural e conversacional.

            Regras:
            - Mantenha o formato numerado (numero. traducao)
            - Preserve o significado e o tom original
            - Use expressoes naturais do portugues brasileiro
            - Retorne APENAS as traducoes numeradas, sem explicacoes adicionais
...
async def translate_batch(
    ...
    async def _translate() -> list[SubtitleBlock]:
        prompt = build_translation_prompt(blocks)
...
async def translate_all(
    ...
    tasks = [translate_batch(client, list(batch)) for batch in batches]
