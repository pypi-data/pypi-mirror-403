...
=======
            if field_value and not isinstance(field_value, dict):
            ...
            if concurrency > 1:
                with ThreadPoolExecutor(max_workers=concurrency) as executor:
                    futures = [
                        executor.submit(
                            self.run_sim,
                            software, sim_name, sim_number, simFileType, timeout_s, verbose, concurrency_mode='local'
                        )
...
=======
            ...
            for sim_name, sim_number in zip(sim_names, sim_numbers):
                # Run simulation
                self.run_sim(software, sim_name, sim_number, simFileType, timeout_s, verbose, concurrency_mode='htcondor')
>>>>>>> Stashed changes
        else:
...
=======
                timeout_s: int =None, verbose: bool = False, concurrency_mode: str = 'none'):
>>>>>>> Stashed changes
        """
        Run selected simulation.
        The function applies a different logic for each simulation software.
        : param concurrency_mode: determines if and where concurrent simulations are run: 'none', 'local', or'htcondor'
        : type concurrency_mode: str
        """
        if verbose:
            if concurrency_mode == 'local':
        ...
        if software == 'COSIM':
            if concurrency_mode == 'htcondor':
                raise Exception('Concurrent execution of COSIM simulations on htcondor is not supported.')
            local_COSIM_folder = self.settings.local_COSIM_folder
            local_model_folder = os.path.join(local_COSIM_folder)
...
=======
            if concurrency_mode == 'htcondor':
                ...
                dFiQuS = DriverFiQuS(FiQuS_path=self.settings.FiQuS_path,
                                    path_folder_FiQuS_output=path_folder_FiQuS, path_folder_FiQuS_input=path_folder_FiQuS,
                                    verbose=verbose, GetDP_path=self.settings.GetDP_path, htcondor_settings=self.settings.htcondor)
            ...
            if concurrency_mode == 'htcondor':
                raise Exception('Concurrent execution of LEDET simulations on htcondor is not supported.')
            #local_LEDET_folder = self._get_local_folder('local_LEDET_folder')
            dLEDET = DriverLEDET(path_exe=self.settings.LEDET_path, path_folder_LEDET=self.settings.local_LEDET_folder, verbose=verbose)
            self.summary = dLEDET.run_LEDET(simulation_name, str(sim_number), simFileType=simFileType)
        elif software == 'PSPICE':
            if concurrency_mode == 'htcondor':
                raise Exception(f'Concurrent execution of PSPICE simulations on htcondor is not supported.')
            #local_PSPICE_folder = self._get_local_folder('local_PSPICE_folder')
            ...
            if concurrency_mode == 'htcondor':
                raise Exception('Concurrent execution of PyBBQ simulations on htcondor is not supported')
            #local_PyBBQ_folder = self._get_local_folder('local_PyBBQ_folder')
>>>>>>> Stashed changes
...
=======
            if concurrency_mode == 'htcondor':
                raise Exception('Concurrent execution of PyCoSim simulations on htcondor is not supported.')
            #local_PyCoSim_folder = self._get_local_folder('local_PyCoSim_folder')
...
=======
            if concurrency_mode == 'htcondor':
                raise Exception('Concurrent execution of SIGMA simulations on htcondor is not supported.')
            #local_SIGMA_folder = self._get_local_folder('local_SIGMA_folder')
            ...
            if concurrency_mode == 'htcondor':
                raise Exception(f'Concurrent execution of XYCE simulations on htcondor is not supported.')
            #local_XYCE_folder = self._get_local_folder('local_XYCE_folder')
