...
def extract_output(
    portf: Portfolio, op: OptimProblem, res: Results, prices: dict = None
) -> dict:
    ...
    if isinstance(res, str):  # not successful
        ...
        duals = pd.DataFrame(
            index=times
        )  # duals of problem -  correspond to nodal prices
        special = pd.DataFrame(
            columns=["asset", "variable", "name", "value", "costs"]
        )  # one line per asset / parameter
        ...
        for a in portf.assets:
            variable_names = op.mapping[
                (op.mapping["asset"] == a.name) & (op.mapping["type"] == "i")
            ]["var_name"].unique()
...
def output_to_file(output, file_name: str, csv_ger: bool = False):
    ...
    Args:
        output (output dict): Results to be written into file. Dict of pd.DataFrames
    ...
    if file_extension in (".xlsx", ".xls"):
        ...
        raise NotImplementedError(
            "output format - " + file_extension + " - not implemented"
        )
...
def set_param(obj, path, value):
    ...
    try:
        ...
        if "name" in o:
            ...
            n = "NA"
        raise ValueError(
            "Error. Object could not be created. Parameter issue? Object: "
            + n
            + " | parameter "
            + str(path)
        )
...
def optimize(
    ...
    if split_interval_size is None:
        ...
        if not isinstance(split_interval_size, str):
            raise ValueError("split_interval_size must be a string")
