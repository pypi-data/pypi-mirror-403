...
def json_serialize_objects(obj) -> dict:
    ...
    if isinstance(obj, dt.datetime) or isinstance(obj, pd.Timestamp):
        ...
        res = {
            "__class__": dt.datetime.__name__,
            "__tz__": mytz,
            "__value__": obj.strftime("%Y-%m-%d %H:%M:%S"),
        }
    elif isinstance(obj, dt.date):
        res = {"__class__": dt.date.__name__, "__value__": str(obj)}
...
def to_json(obj, file_name=None):
    ...
    if file_name is None:
        ...
        with open(file_name, "w") as file:
            json.dump(
                obj, file, indent=4, sort_keys=True, default=json_serialize_objects
            )
