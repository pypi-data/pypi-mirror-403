...
class Asset:
    ...
    def __init__(
        ...
        if isinstance(nodes, Node):
            ...
            self.nodes = nodes
        assert isinstance(self.nodes, (tuple, list)), (
            "nodes must be of type Node or list of nodes in  " + self.name
        )
        for n in self.nodes:
            assert isinstance(n, Node), (
                "noses mus be of type Node or list of nodes in " + self.name
            )
        self.wacc = wacc
    ...
    def set_timegrid(self, timegrid: Timegrid):
        ...
        self.timegrid.set_wacc(
            self.wacc
        )  # create discount factors for timegrid and asset's wacc
        if self.freq is not None:
            ...
            assert freq_a >= freq_p, (
                "Asset timegrid must have less/equal granular frequency than portfolios. Asset:"
                + str(self.name)
            )
        self.timegrid.set_restricted_grid(
            self.start, self.end, self.freq
        )  # restricted timegrid for asset lifetime and own freq

    @abc.abstractmethod
    def setup_optim_problem(
        self,
        prices: Union[dict, pd.DataFrame, pd.Series, None],
...
class Storage(Asset):
    ...
    def __init__(
        ...
        if isinstance(
            size, float
        ):  # do not check if time dependent, otherwise not possible to check
            assert start_level <= size, (
                "Storage --" + self.name + "--: start level must be <=  storage size"
            )
        self.cap_in = cap_in
        self.cap_out = cap_out
        # check if constant value - otherwise not possible to check
        if isinstance(cap_in, float):
            assert cap_in >= 0, (
                "Storage --" + self.name + "--: cap_in must not be negative"
            )
        if isinstance(cap_out, float):
            assert cap_out >= 0, (
                "Storage --" + self.name + "--: cap_out must not be negative"
            )
        self.eff_in = eff_in
        ...
        #### periodicity
        assert not (
            (periodicity_duration is not None) and (periodicity is None)
    ...
    def setup_optim_problem(
        ...
        # check: timegrid set?
        assert hasattr(self, "timegrid"), (
            "Set timegrid of asset before creating optim problem. Asset: " + self.name
        )
        ...
        if self.price is not None:
            ...
            if (
                (not self.no_simult_in_out)
                and ((self.eff_in < 1) or (self.eff_out < 1))
                and (not (price >= 0).all())
            ):
                print(
                    "Storage --"
                    + self.name
                    + "--: no_simult_in_out is set to False, but there are neg. prices. Likely storage will load & unload simultaneously. You want that?"
                )
        ...
        sep_needed = (
            ...
            or (self.cost_out != 0)
            or (len(self.nodes) == 2)
            or (self.max_cycles_no is not None)
        )
        ...
        if self.max_cycles_no is not None:
            ...
            try:
                ...
                extra_time = pd.Timedelta(1, self.max_cycles_freq)
...
class SimpleContract(Asset):
    ...
    def setup_optim_problem(
        ...
        self._no_separate_disp_vars = (
            (all(extra_costs == 0.0)) or (all(max_cap <= 0.0)) or (all(min_cap >= 0.0))
        )
        if self._no_separate_disp_vars:
            # in this case no need for two variables per time step
            u = max_cap  # upper bound
            l = min_cap  # lower
            if any(extra_costs != 0):
                if all(max_cap <= 0.0):  # dispatch always negative
                    price = price - extra_costs
                if all(min_cap >= 0.0):  # dispatch always negative
                    price = price + extra_costs
            c = price * discount_factors  # set price and discount
            mapping["time_step"] = I
            mapping["var_name"] = "disp"  # name variables for use e.g. in RI
        else:
            u = np.hstack((np.minimum(0.0, max_cap), np.maximum(0.0, max_cap)))
            l = np.hstack((np.minimum(0.0, min_cap), np.maximum(0.0, min_cap)))
        ...
        mapping["node"] = self.nodes[0].name
        mapping["type"] = (
            "d"  # only dispatch variables (needed to impose nodal restrictions in portfolio)
        )
        # if we're using a less granular asset timegrid, add dispatch for every minor grid point
        # Effectively we concat the mapping for each minor point (one row each)
        if hasattr(self.timegrid.restricted, "I_minor_in_major"):
...
class Transport(Asset):
    ...
    def __init__(
        ...
        nodes: List[Node] = [
            Node(name="default_node_from"),
            Node(name="default_node_to"),
        ],
        start: dt.datetime = None,
        end: dt.datetime = None,
        wacc: float = 0,
        ...
        assert min_cap <= max_cap, (
            "Transport with min_cap >= max_cap leads to ill-posed optimization problem. Asset name: "
            + name
        )
        self.min_cap = min_cap
        self.max_cap = max_cap
    ...
    def setup_optim_problem(
        ...
        min_cap = self.make_vector(
            self.min_cap, prices, default_value=0.0, convert=True
        )
...
def define_restr(my_take, my_type, my_n, map, timegrid, node=None):
    ...
    for s, e, v in zip(my_take["start"], my_take["end"], my_take["values"]):
        ...
        for i, t in enumerate(timegrid.restricted.timepoints):
            if (s <= t) and (e > t):
                if node is None:
                    I.extend(
                        map.index[
                            map["time_step"] == timegrid.restricted.I[i]
                        ].to_list()
                    )
                else:
                    I.extend(
        ...
        if not len(I) == 0:  # interval could be outside timegrid, then omit restriction
            ...
            my_v = (
                v
                / ((e - s) / pd.Timedelta(1, timegrid.main_time_unit))
                * timegrid.dt[map.loc[I, "time_step"].unique()].sum()
            )
...
class Contract(SimpleContract):
    ...
    def __init__(
        ...
        if not min_take is None:
            assert isinstance(min_take, dict), (
                "min_take must be dict with keys (start, end, value). Asset: "
                + self.name
            )
            assert (
                "values" in min_take
            ), "min_take must be of dict type with start, end & values (values missing)"
            assert (
                "start" in min_take
            ), "min_take must be of dict type with start, end & values (start missing)"
            assert (
                "end" in min_take
            ), "min_take must be of dict type with start, end & values (end missing)"
        ...
        if not max_take is None:
            assert isinstance(max_take, dict), (
                "max_take must be dict with keys (start, end, value). Asset: "
                + self.name
            )
            assert (
                "values" in max_take
    ...
    def setup_optim_problem(
        ...
        if not max_take is None:
            # assert right sizes
            assert (len(max_take["start"]) == (len(max_take["end"]))) and (
                len(max_take["start"]) == (len(max_take["values"]))
            )
        ...
        if not min_take is None:
            # assert right sizes
            assert (len(min_take["start"]) == (len(min_take["end"]))) and (
                len(min_take["start"]) == (len(min_take["values"]))
            )
        ...
        if self.periodicity is not None:
            op.__make_periodic__(
                freq_period=self.periodicity,
                freq_duration=self.periodicity_duration,
                timegrid=timegrid,
            )
...
class MultiCommodityContract(Contract):
    ...
    def __init__(
        ...
        nodes: Union[Node, List[Node]] = [
            Node(name="default_node_1"),
            Node(name="default_node_2"),
        ],
        start: dt.datetime = None,
        end: dt.datetime = None,
        wacc: float = 0,
        ...
        """
        super(MultiCommodityContract, self).__init__(
            name=name,
            nodes=nodes,
            start=start,
            end=end,
        ...
        if not factors_commodities is None:
            assert isinstance(
                factors_commodities, (list, np.array)
            ), "factors_commodities must be given as list"
            assert len(factors_commodities) == len(
                self.nodes
            ), "number of factors_commodities must equal number of nodes"
...
class ExtendedTransport(Transport):
    ...
    def setup_optim_problem(
        ...
        if not max_take is None:
            ...
            A1, b1, c1 = define_restr(
                my_take, "L", n, op.mapping, timegrid, node=self.node_names[0]
            )
            A = sp.vstack((A, A1))
            b = np.hstack((b, b1))
            cType = cType + c1
        if not min_take is None:
            ...
            my_take["values"] = -np.asarray(my_take["values"])
            A1, b1, c1 = define_restr(
                my_take, "U", n, op.mapping, timegrid, node=self.node_names[0]
            )
            A = sp.vstack((A, A1))
            b = np.hstack((b, b1))
            cType = cType + c1
...
class ScaledAsset(Asset):
    ...
    def __init__(
        ...
        Args:
            ...
            fix_costs (float, optional):  Costs in currency per norm scale and per main_time_unit (in timegrid).
                                          Defaults to 0.
        """
        super(ScaledAsset, self).__init__(
            name=name, nodes=base_asset.nodes, start=start, end=end, wacc=wacc
        )
        self.base_asset = base_asset
        assert (
            min_scale <= max_scale
        ), "Problem not well defined. min_scale must be <= max_scale"
        self.min_scale = min_scale
        assert min_scale >= 0.0, "Minimum asset scale must be >=0"
        self.max_scale = max_scale
        self.norm_scale = norm_scale
        assert (
            norm_scale > 0.0
        ), "normalization of asset size must not be smaller / equal to zero"
    ...
    def setup_optim_problem(
        ...
        op = self.base_asset.setup_optim_problem(
            prices=prices, timegrid=timegrid, costs_only=costs_only
        )
        # set timegrid if given as optional argument
        ...
        mymap = {
            "time_step": 0,  ## assign fix costs to first time step
...
class OrderBook(Asset):
    ...
    def __init__(
        ...
        if isinstance(orders, Dict):
            ...
            assert len(orders["start"]) == len(
                orders["end"]
            ), "order must have same number of start and end values   " + str(name)
            assert len(orders["start"]) == len(
                orders["end"]
            ), "order must have same number of start and end values   " + str(name)
            assert len(orders["start"]) == len(
                orders["capa"]
            ), "order must have same number of start and capa values   " + str(name)
            assert len(orders["start"]) == len(
                orders["price"]
            ), "order must have same number of start and price values   " + str(name)
        else:
            raise ValueError(
                "Order Book bust be Dictionary (start, end, capa, price) or pd.Dataframe   -  "
                + str(name)
            )
    ...
    def setup_optim_problem(
        ...
        mapping = pd.DataFrame(columns=["time_step", "var_name", "disp_factor"])
        ...
        for iO in range(0, len(self.orders["start"])):
            ...
            c[iO] = (
                myc * sum(dt[myI] * discount_factors[myI]) * myp
            )  # sign (buy/sell in price as exec [0,1])
            mymap = pd.DataFrame()
            mymap["time_step"] = I[myI]  # order valid here
            mymap["disp_factor"] = (
                myc * dt[myI]
            )  # order exec [0,1], effect capa incl. sign buy/sell
            mymap["var_name"] = iO
        ...
        return OptimProblem(c=c, l=l, u=u, mapping=mapping, timegrid=self.timegrid)
