...
class SimpleContractTest(unittest.TestCase):
    def test_optimization(self):
        ...
        a = eao.assets.SimpleContract(
            name="SC", price="rand_price", nodes=node, min_cap=-10.0, max_cap=10.0
        )
...
class StorageTest(unittest.TestCase):
    ...
    def test_trivial_storage_with_inflow(self):
        ...
        node = eao.assets.Node("testNode")
        timegrid = eao.assets.Timegrid(
            dt.date(2021, 1, 1), dt.date(2021, 2, 2), freq="d", main_time_unit="d"
        )
    ...
    def test_store_cost_with_cap_costs(self):
        ...
        self.assertAlmostEqual(res.value, 1e6 - (5 + 10 + 5) * 0.5 - 10 * 1 - 10 * 2, 3)
        print(res)
...
class TransportTest(unittest.TestCase):
    ...
    def test_extended_transport(self):
        ...
        transa = eao.assets.ExtendedTransport(
            name="TrA",
            nodes=[node1a, node2],
            min_cap=0.0,
            max_cap=10.0,
        ...
        transb = eao.assets.ExtendedTransport(
            name="TrB",
            nodes=[node1b, node2],
            min_cap=0.0,
            max_cap=10.0,
        ...
        buya = eao.assets.SimpleContract(
            name="buya", price="buy", max_cap=2, nodes=node1a
        )
        buyb = eao.assets.SimpleContract(
            name="buyb", price="buy", max_cap=2, nodes=node1b
        )
        sell = eao.assets.SimpleContract(
            name="sell", price="sell", min_cap=-1, nodes=node2
        )
    ...
    def test_extended_transport_max_min_take(self):
        ...
        prices = {"buy": np.zeros(timegrid.T), "sell": 1 * np.ones(timegrid.T)}
        max_take = {
            "start": [dt.date(2021, 1, 1), dt.date(2021, 1, 2), dt.date(2021, 1, 11)],
            "end": [dt.date(2021, 1, 2), dt.date(2021, 1, 11), dt.date(2021, 2, 11)],
            "values": [0, 2.2, 0],
        }
        trans = eao.assets.ExtendedTransport(
            name="TrB",
            nodes=[node1, node2],
            min_cap=0.0,
            max_cap=10.0,
        ...
        buy = eao.assets.SimpleContract(
            name="buya", price="buy", max_cap=2, nodes=node1
        )
        sell = eao.assets.SimpleContract(
            name="sell", price="sell", min_cap=-1, nodes=node2
        )
        ...
        self.assertAlmostEqual(
            out["dispatch"]
            .loc[pd.Timestamp(2021, 1, 1) : pd.Timestamp(2021, 1, 1), "buya (N1)"]
            .sum(),
            0.0,
            5,
        )
        self.assertAlmostEqual(
            out["dispatch"]
            .loc[pd.Timestamp(2021, 1, 2) : pd.Timestamp(2021, 1, 10), "buya (N1)"]
            .sum(),
            2.2,
            5,
        )
        self.assertAlmostEqual(
            out["dispatch"]
            .loc[pd.Timestamp(2021, 1, 11) : pd.Timestamp(2021, 2, 10), "buya (N1)"]
            .sum(),
            0,
            5,
        )
    ...
    def test_transport_efficiency(self):
        ...
        buy = eao.assets.SimpleContract(
            name="buya", price="buy", max_cap=1, nodes=node1a
        )
        sell = eao.assets.SimpleContract(
            name="sell", price="sell", min_cap=-1, nodes=node2
        )
    ...
    def test_transport_vector(self):
        ...
        buy = eao.assets.SimpleContract(
            name="buya", price="buy", max_cap=100, nodes=node1a
        )
        sell = eao.assets.SimpleContract(
            name="sell", price="sell", min_cap=-100, nodes=node2
        )
    ...
    def test_transport_vector_eff(self):
        ...
        trans = eao.assets.Transport(
            name="TrA", nodes=[node1a, node2], min_cap=0, max_cap=1, efficiency="eff"
        )
        buy = eao.assets.SimpleContract(
            name="buya", price="buy", max_cap=100, nodes=node1a
        )
        ...
        np.testing.assert_almost_equal(
            out["dispatch"]["TrA (N1a)"], -np.ones(timegrid.T), 2
        )
...
class ContractTest(unittest.TestCase):
    def test_max_cap_vector(self):
        ...
        a = eao.assets.Contract(
            name="SC", price="rand_price", nodes=node, min_cap=min_cap, max_cap=min_cap
        )
    ...
    def test_contract_min_take(self):
        ...
        max_cap = min_cap.copy()
        max_cap["values"] = np.ones(len(min_cap["start"])) * 10.0
        # min and max take
    ...
    def test_max_cap_vector_from_pricesDF(self):
        ...
        a = eao.assets.Contract(
            name="SC", price="rand_price", nodes=node, min_cap="cap", max_cap="cap"
        )
    ...
    def test_predefined_multicommodity(self):
        ...
        prices = {
            "price": portf.timegrid.values_to_grid(
                {
                    "start": pd.to_datetime(prices["start"].values),
...
class ScaledAsset(unittest.TestCase):
    def test_scaled_asset(self):
        ...
        a = eao.assets.SimpleContract(
            name="market", price="spot", min_cap=-10, max_cap=10, nodes=node
        )
        b = eao.assets.Storage(name="battery", nodes=node, cap_in=1, cap_out=1, size=4)

        scaled_b = eao.assets.ScaledAsset(
            name="scaled_b", base_asset=b, max_scale=1, fix_costs=0.0
        )
    ...
    def test_scaled_transport(self):
        ...
        input_ts = {
            ...
            "cons": -2
            * np.asanyarray(
                [0, 0, 1, 1, 1, 1, 1, 4, 1, 2, 1, 1, 1, 1, 2, 1, 2, 1, 2, 2, 2, 5, 7, 3]
            ),
        }
        behind_meter = eao.assets.Node("behind meter")
        front_of_meter = eao.assets.Node("front of meter")

        consumption = eao.assets.SimpleContract(
            name="consumption", nodes=behind_meter, min_cap="cons", max_cap="cons"
        )

        supply = eao.assets.SimpleContract(
            name="supply", nodes=front_of_meter, price="price", min_cap=0, max_cap=100
        )
        grid_consumption_normed = eao.assets.Transport(
            name="grid_in_normed",  #
            nodes=[front_of_meter, behind_meter],
            # costs_const = grid_fees['var'], # variable grid fees
            min_cap=0,
            max_cap=1,
        )  # normed capacity (to 1 MW)
        grid_consumption = eao.assets.ScaledAsset(
            name="grid_in",
            base_asset=grid_consumption_normed,
            max_scale=1000,
            fix_costs=100,
        )  # yearly fix capacity costs (here scaled to hourly as main time unit)
        battery = eao.assets.Storage(
            name="battery", nodes=behind_meter, cap_in=1, cap_out=1, size=2
        )
        portf = eao.portfolio.Portfolio(
            [supply, consumption, grid_consumption, battery]
        )
...
class DiscountRate(unittest.TestCase):
    ...
    def test_store_cost_with_cap_costs_discount(self):
        ...
        for ii in range(36, 48):
            self.assertAlmostEqual(res.x[ii], 0, 3)
        # total value: earnung 10+1e5, costs for storage .5 per MWh in storage
        self.assertAlmostEqual(res.value, 1e4 - (5 + 10 + 5) * 0.5 - 10 * 1 - 10 * 2, 3)
...
class TestOrderOrderBooks(unittest.TestCase):
    def test_order(self):
        ...
        ob = dict(
            ...
            capa=[-1.1, -2.2, 3.3, 4.4],
            price=[8, 12, 13, 9],
        )

        a = eao.assets.SimpleContract(
            name="SC", nodes=node, price="market", min_cap=-100.0, max_cap=100.0
        )
        ...
        try:
            ob3 = eao.assets.OrderBook(orders=ob_missing)
        except:
            pass  # should fail
        else:
            raise ValueError("Should have raised an arror")
    ...
    def test_order_enforce_allatonce(self):
        ...
        node = eao.assets.Node("testNode")
        timegrid = eao.assets.Timegrid(
            dt.date(2021, 1, 1), dt.date(2021, 2, 1), freq="h"
        )

        ## define order book
        ob = dict(
            start=[
                pd.Timestamp(2021, 1, 1, 10),
                pd.Timestamp(2021, 1, 2),
                pd.Timestamp(2021, 1, 3),
                pd.Timestamp(2021, 1, 7),
            ],
            end=[
                pd.Timestamp(2021, 1, 6),
        ...
        a = eao.assets.SimpleContract(
            name="SC", nodes=node, price="market", min_cap=-10.0, max_cap=0
        )  # restricted sale
        ...
        self.assertAlmostEqual(
            out["dispatch"].max().iloc[1], 10, 3
        )  # given by market, orders partly executed
    ...
    def test_order_book_battery(self):
        ...
        ob = pd.DataFrame(
            columns=["start", "end", "capa", "price"]
        )  # alternative to dict is DataFrame ... converted in asset
        r = dict()  # row
        ### orders
        ...
        for ii in timegrid.I:
            ...
            for i in range(0, 2):
                ...
                ob.loc[len(ob)] = r
        # battery
        b = eao.assets.Storage("battery", node, cap_in=10, cap_out=10, size=40)
        ...
        order_book = eao.assets.OrderBook(
            "orders", node, orders=ob, full_exec=True
...
class TestVarious(unittest.TestCase):
    def test_availability_CHP(self):
        ...
        data = {
            "price": 50.0 * np.ones(timegrid.T),
            "priceGas": 0.1 * np.ones(timegrid.T),
