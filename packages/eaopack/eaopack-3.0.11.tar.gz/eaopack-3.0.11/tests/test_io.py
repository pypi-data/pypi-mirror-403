...
class IOTests(unittest.TestCase):
    ...
    def test_serialize_assets(self):
        ...
        check = (
            check
            and (n.commodity == nn.commodity)
            and (n.name == nn.name)
            and (n.unit.factor == nn.unit.factor)
            and (n.unit.flow == nn.unit.flow)
            and (n.unit.volume == nn.unit.volume)
        )
    ...
    def test_serialize_portfolio(self):
        ...
        prices = {
            "rand_price_1": np.ones(timegrid.T) * 1.0,
            "rand_price_2": np.ones(timegrid.T) * 10.0,
            "rand_price_3": np.random.randn(timegrid.T) * 10.0,
        }
        ...
        mys = json.dumps(
            portf, indent=4, default=eao.serialization.json_serialize_objects
        )
        ...
        run_from_json(
            file_name_in=myf, prices=prices, file_name_out="test_results.xlsx"
        )
        run_from_json(
            file_name_in=myf,
            prices=prices,
            file_name_out="test_results.csv",
            csv_ger=True,
        )
        for output_file in (
            "test_results.xlsx",
            "test_results_DCF.csv",
            "test_results_dispatch.csv",
            "test_results_internal_variables.csv",
            "test_results_prices.csv",
            "test_results_special.csv",
    ...
    def test_dates_serialize(self):
        ...
        asset_file = os.path.join(
            os.path.join(os.path.dirname(__file__)), "test_result_asset.JSON"
        )
        ...
        charge = eao.assets.ExtendedTransport(
            name="charge",
            min_cap=0.0,
            max_cap=12.0,
        ...
        discharge = eao.assets.ExtendedTransport(
            name="discharge",
            min_cap=0.0,
            max_cap=23.0,
        ...
        market = eao.assets.SimpleContract(
            max_cap=10, min_cap=-10, price="price", nodes=node_main, name="market"
        )
        portf = eao.portfolio.Portfolio([battery, charge, discharge])
        struct_battery = eao.assets.StructuredAsset(
            name="xxxxx", portfolio=portf, nodes=node_main
        )
        ## write to JSON
...
class OptimizeShortcutTests(unittest.TestCase):
    ...
    def test_optimize_shortcut(self):
        ...
        # a1.set_timegrid(timegrid)
        a2 = eao.assets.SimpleContract(
            name="SC_2", price="rand_price_2", nodes=node1, min_cap=-5.0, max_cap=10.0
        )  # , extra_costs= 1.)
        # a2.set_timegrid(timegrid)
        a3 = eao.assets.SimpleContract(
            name="SC_3",
        ...
        # a3.set_timegrid(timegrid)
        prices = {
            "rand_price_1": np.random.rand(timegrid.T) - 0.5,
            "rand_price_2": 5.0 * (np.random.rand(timegrid.T) - 0.5),
        }
        ...
        eao.io.output_to_file(out, "test_results_output.xlsx")
        eao.io.output_to_file(out, "test_results_output.csv")

    def test_not_enough_data(self):
        """Unit test: cast data into timegrid - check error catching etc"""

        tg_data = eao.assets.Timegrid(
            dt.date(2021, 1, 1), dt.date(2021, 4, 1), freq="15min"
        )
        # random data
        data = pd.DataFrame(
        ...
        tg = eao.assets.Timegrid(
            dt.date(2020, 12, 31), dt.date(2021, 3, 1), freq="15min"
        )
        with self.assertRaises(Exception):
            df = tg.prices_to_grid(data)
