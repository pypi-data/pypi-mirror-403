...
class various(unittest.TestCase):
    def test_battery_efficiency(self):
        ...
        portf.assets[0].no_simult_in_out = (
            True  # !!! we sum up in and out in output. Computing the storage level won't work without enforcing
        )
        ...
        self.assertAlmostEqual(
            out["dispatch"].loc[:, "battery"].sum(), -71.66666667, 3
        )  # reversion test
    ...
    def test_opt_shortcut(self):
        ...
        a2 = eao.assets.SimpleContract(
            name="SC_2", price="rand_price_1", nodes=node1, min_cap=-5.0, max_cap=10.0
        )  # , extra_costs= 1.)
