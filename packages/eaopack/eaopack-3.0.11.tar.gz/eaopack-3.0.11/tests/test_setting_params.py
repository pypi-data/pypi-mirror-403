...
class test_setting_params(unittest.TestCase):
    ...
    def test_setting_portfolio(self):
        ...
        a2 = eao.assets.SimpleContract(
            name="SC_2", price="rand_price_1", nodes=node1, min_cap=-15.0, max_cap=-10.0
        )  # , extra_costs= 1.)
        ...
        # original unchanged
        self.assertEqual(
            eao.io.get_param(portf, ["assets", 0, "nodes", 0, "unit", "flow"]), "MW"
        )
        # new changed
        self.assertEqual(
            eao.io.get_param(p, ["assets", 0, "nodes", 0, "unit", "flow"]), "kW"
        )
        # new changed
        pass
