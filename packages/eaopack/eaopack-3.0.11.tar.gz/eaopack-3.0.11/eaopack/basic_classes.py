...
class Unit:
    def __init__(
        ...
        Args:
            ...
            commodity (str, optional): Name of the commodity covered - e.g. power, heat, gas, CO2, coal. Defaults to "none"
        """
        self.volume = volume
        self.flow = flow
        self.factor = factor
        ...
        assert (
            factor == 1.0
        ), "conversion of volume/flow units using factor not implemented. please choose units with conversion factor 1"


class Node:
    def __init__(
        self, name: str, commodity: Union[None, str] = None, unit: Unit = Unit()
    ):
...
class Timegrid:
    def __init__(
        self,
        start: Union[None, dt.date, dt.datetime, pd.Timestamp],
        ...
        if ref_timegrid is not None:
            ...
            if freq == ref_timegrid.freq:
                I = (ref_timegrid.timepoints >= self.start) & (
                    ref_timegrid.timepoints < self.end
                )
                ...
                assert (
                    freq_a >= freq_p
                ), "timegrid must have less/equal granular frequency than reference"
                # generate more granular timegrid - all minor grid points in "right interval"  |->|->|->|
                pts = pd.date_range(
                    start=self.start, end=self.end, freq=freq, tz=self.tz
                )
                self.I = []
                self.I_minor_in_major = (
                    []
                )  # to contain the list of minor grid points in major grid
                self.timepoints = []
                self.dt = []
                self.Dt = []
                if hasattr(ref_timegrid, "discount_factors"):
        ...
        # no reference TG given
            ...
            timepoints = pd.date_range(
                start=self.start, end=self.end, freq=self.freq, tz=self.tz
            )
            self.timepoints = timepoints[0:-1]  # specific time points of mesh
            self.dt = (timepoints[1:] - timepoints[0:-1]) / pd.Timedelta(
                1, self.main_time_unit
            )  # time steps in mesh
            self.Dt = np.cumsum(
                self.dt
            )  # total duration since start (in main time unit)
    ...
    def prices_to_grid(self, prices: dict):
        ...
        if prices.index[0] > self.timepoints[0]:
            raise ValueError(
                "Error: Data provided starts later than timegrid -- check input data"
            )
        if prices.index[-1] < self.timepoints[-1]:
            raise ValueError(
    ...
    def set_restricted_grid(
        self, start: dt.datetime = None, end: dt.datetime = None, freq: bool = None
    ):
        ...
        if freq is None:  # standard case
            self.restricted = Timegrid(
                start,
                end,
                freq=self.freq,
                main_time_unit=self.main_time_unit,
                ref_timegrid=self,
            )
    ...
    def values_to_grid(self, inp: StartEndValueDict) -> np.array:
        ...
        if not "end" in inp:
            if len(inp["start"]) > 1:
                ...
                inp["end"][-1] = inp["end"][-1] + 2 * (
                    inp["start"][-1] - inp["start"][-2]
                )  # generously extend validity
        ...
        for s, e, v in zip(inp["start"], inp["end"], inp["values"]):
            I = (self.timepoints >= pd.to_datetime(s)) & (
                self.timepoints < pd.to_datetime(e)
            )
