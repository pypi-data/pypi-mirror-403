...
class OptimProblem:
    ...
    def __init__(
        ...
        assert not np.isnan(
            c.sum()
        ), "nan value in optim problem. Check input data -- c"
        assert not np.isnan(
            l.sum()
        ), "nan value in optim problem. Check input data -- l"
        assert not np.isnan(
            u.sum()
        ), "nan value in optim problem. Check input data -- u"
        if not b is None:
            assert not np.isnan(
                b.sum()
            ), "nan value in optim problem. Check input data -- b"

        # make periodic
        if periodic_period_length is not None:
            assert timegrid is not None, "for periodic optim problem need timegrid"
            self.__make_periodic__(
                freq_period=periodic_period_length,
                freq_duration=periodic_duration,
                timegrid=timegrid,
            )

    def __make_periodic__(
        self, freq_period: str, freq_duration: str, timegrid: Timegrid
    ):
        ...
        if "disp_factor" not in self.mapping.columns:
            self.mapping["disp_factor"] = 1.0
        ...
        self.mapping["disp_factor"] = self.mapping["disp_factor"].fillna(
            1.0
        )  # ensure there's a one where not assigned yet
        tp = timegrid.timepoints
        T = timegrid.T
        try:
            periods = pd.date_range(
                tp[0] - pd.Timedelta(1, freq_period),
                tp[-1] + pd.Timedelta(1, freq_period),
                freq=freq_period,
                tz=timegrid.tz,
            )
        except:
            periods = pd.date_range(
                tp[0] - pd.Timedelta(freq_period),
                tp[-1] + pd.Timedelta(freq_period),
                freq=freq_period,
                tz=timegrid.tz,
            )
        if freq_duration is None:
            durations = [
                tp[0],
                tp[-1] + (tp[-1] - tp[0]),
            ]  # whole interval - generously extending end
        else:
            try:
                durations = pd.date_range(
                    tp[0] - pd.Timedelta(1, freq_duration),
                    tp[-1] + pd.Timedelta(1, freq_duration),
                    freq=freq_duration,
                    tz=timegrid.tz,
                )
            except:
                durations = pd.date_range(
                    tp[0] - pd.Timedelta(freq_duration),
                    tp[-1] + pd.Timedelta(freq_duration),
                    freq=freq_duration,
                    tz=timegrid.tz,
                )
        # gave a bit space in case date ranges do not have the same start - deleting now superfluous (early) start
        if periods[1] <= tp[0]:
            periods = periods.drop(periods[0])
        if durations[1] <= tp[0]:
            durations = durations.drop(durations[0])
        # assertions - ensure that all periods are of same length
        d = periods[1:] - periods[0:-1]
        assert all(d == d[0]), (
            "Error. All periods must have same length. Not given for chosen frequency "
            + periods
        )
        ...
        self.mapping = pd.merge(
            self.mapping, df, left_on="time_step", right_index=True, how="left"
        )
        self.mapping["new_idx"] = self.mapping.index
        idx = np.asarray(
            self.mapping.index
        ).copy()  # get mapping index to change it later on
            ...
            for mynode in self.mapping["node"].unique():
                for mytype in self.mapping["type"].unique():
                    for myvar in list(self.mapping["var_name"].unique()):
                        I = (
                            (self.mapping["asset"] == myasset)
                            & (self.mapping["node"] == mynode)
                            & (self.mapping["var_name"] == myvar)
                            & (self.mapping["type"] == mytype)
                        )
                        # loop through durations
                        for dur in self.mapping.loc[I].dur.unique():
                            # loop through period steps
                            for sub_per in self.mapping.loc[
                                I & (self.mapping["dur"] == dur)
                            ].sub_per.unique():
                                II = (
                                    I
                                    & (self.mapping["dur"] == dur)
                                    & (self.mapping["sub_per"] == sub_per)
                                )
                                if II.sum() <= 1:
                                    pass  ##  Nothing to do. There is only one variable
                                else:
                                    vars = self.mapping.index[
                                        II
                                    ]  # variables to be joined
                                    ...
                                    # Adjust mapping.
                                    assert all(
                                        self.mapping.loc[II, "disp_factor"]
                                        == self.mapping.loc[II, "disp_factor"].iloc[0]
                                    ), "periodicity cannot be imposed where disp factors are not identical"
                                    idx[out] = leading
    ...
    def optimize(
        ...
        assert np.all(
            self.l <= self.u
        ), "Error. Lower bounds must be smaller or equal to upper bounds"
        ...
        if "bool" in map:
            my_bools = map.loc[
                (~map.index.duplicated(keep="first")) & (map["bool"])
            ].index.values.tolist()
            if len(my_bools) == 0:
                my_bools = False
            else:
        ...
        if interface == "cvxpy":
            ...
            if not self.A is None:
                ...
                if any(myRows):
                    ...
                    constraints = constraints + [
                        AU @ x <= bU
                    ]  # matrix/vector multiplication in CVXPY notation
                # LOWER limit
                my_type = "L"
                myRows = [mya == my_type for mya in self.cType]
                if any(myRows):
            ...
            if target.lower() == "value":
                objective = (
                    -self.c.T @ x
                )  # @ is the matrix/vector multiplication in CVXPY notation
            elif target.lower() == "robust":
                assert not samples is None  # need samples
                if isinstance(samples[0], (float, int)):
                    raise ValueError(
                        "For robust optimization, samples must be list of arrays "
                    )
                ...
                for myc in samples:
                    constraints = constraints + [
                        -myc.T @ x >= DCF_min
                    ]  # sign: maximize negative costs
                objective = DCF_min
            else:
                raise NotImplementedError(
                    "Target function -- " + target + " -- not implemented"
                )
            ...
            if prob.status == "optimal":
                ...
                if not isMIP:
                    ...
                    myduals = None
                results = Results(value=prob.value, x=x.value, duals=myduals)
            ...
            if not self.A is None:
                ...
                for i in range(len(self.A.rows)):
                    if self.cType[i] == "U":
                        ...
                        raise ValueError(
                            "Unknown ctype " + self.cType[i] + "at position " + str(i)
                        )
                    for j in range(len(self.A.rows[i])):
                        constraint.SetCoefficient(
                            x[self.A.rows[i][j]], self.A.data[i][j]
                        )
            ...
            if status == pywraplp.Solver.OPTIMAL:
                ...
                results = Results(
                    value=solver_.Objective().Value(), x=x_value, duals=None
                )
            ...
            raise NotImplementedError(
                "Interface - " + str(interface) + " -not implemented"
            )
