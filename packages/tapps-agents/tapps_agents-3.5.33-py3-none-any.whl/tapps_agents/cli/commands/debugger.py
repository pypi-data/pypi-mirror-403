...
def handle_debugger_command(args: object) -> None:
    ...
    try:
        asyncio.run(debugger.activate(offline_mode=offline_mode))
        if command == "debug":
            result = asyncio.run(
                debugger.run(
                    "debug",
