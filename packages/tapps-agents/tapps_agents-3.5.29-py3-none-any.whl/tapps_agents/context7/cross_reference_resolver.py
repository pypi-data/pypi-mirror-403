...
class CrossReferenceResolver:
    ...
    def get_related_documentation(
        self, library: str, topic: str | None = None, max_results: int = 5
    ) -> list[dict[str, Any]]:
        """
        ...
        for ref_info in cross_refs.get("available_references", []):
            if ref_info.get("available"):
                entry = self.kb_cache.get(ref_info["library"], ref_info["topic"])
                if entry:
                    related_docs.append(
                        {
...
def create_cross_reference_resolver(
    cache_structure: CacheStructure, kb_cache: KBCache
