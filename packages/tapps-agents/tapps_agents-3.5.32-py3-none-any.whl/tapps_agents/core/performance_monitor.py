...
class PerformanceMonitor:
    ...
    def __init__(self, task_id: str, output_dir: Path):
        ...
        self.metrics: dict[str, Any] = {
            ...
            "throughput": {"agents_per_second": 0, "total_agents": 0},
            "efficiency": {
    ...
    def record_agent(
        ...
        self.agent_timings.append(
            {"agent_id": agent_id, "duration": duration, "success": success}
        )
    ...
    def finish(self, max_parallel: int):
        ...
        if self.agent_timings:
            ...
            if total_duration > 0:
                self.metrics["parallelism"]["average_parallel"] = (
                    total_agent_time / total_duration
                )

        # Calculate throughput
        if total_duration > 0:
            self.metrics["throughput"]["agents_per_second"] = (
                len(self.agent_timings) / total_duration
            )
    ...
    def get_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "successful_agents": sum(
                1 for a in self.metrics["agents"].values() if a.get("success")
            ),
            "failed_agents": sum(
                1 for a in self.metrics["agents"].values() if not a.get("success")
            ),
