...
class ResourceMetrics:
    ...
    def is_high_usage(
        self, cpu_threshold: float = 50.0, memory_threshold: float = 80.0
    ) -> bool:
        """
...
class ResourceMonitor:
    ...
    def get_average_metrics(self, duration_seconds: int = 60) -> ResourceMetrics | None:
        ...
        avg_memory_available = sum(m.memory_available_mb for m in recent_metrics) / len(
            recent_metrics
        )
        avg_disk = sum(m.disk_percent for m in recent_metrics) / len(recent_metrics)
        avg_disk_used = sum(m.disk_used_gb for m in recent_metrics) / len(
            recent_metrics
        )
        avg_disk_free = sum(m.disk_free_gb for m in recent_metrics) / len(
            recent_metrics
        )
    ...
    def should_use_background_agent(self) -> bool:
        ...
        return metrics.is_high_usage(
            cpu_threshold=self.cpu_threshold, memory_threshold=self.memory_threshold
        )
