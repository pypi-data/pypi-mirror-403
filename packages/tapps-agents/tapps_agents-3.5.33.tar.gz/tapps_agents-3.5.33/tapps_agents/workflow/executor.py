...
class WorkflowExecutor:
        ...
        try:
            ...
            if "_beads_issue_id" not in state_vars:
                bid = create_workflow_issue(
                    ...
                    config,
                    self.workflow.name,
                    self.user_prompt or state_vars.get("target_file", "") or "",
                )
                if bid:
                    state_vars["_beads_issue_id"] = bid
                    self.state.variables = state_vars
    ...
    async def execute(
        ...
        try:
            ...
            _vars = getattr(self.state, "variables", None) if self.state else None
            beads_issue_id = (_vars or {}).get("_beads_issue_id")
