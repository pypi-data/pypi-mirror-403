...
class FailureData(BaseModel):
    ...
    """ # noqa: E501
    attribute_name: Optional[StrictStr] = Field(default=None, description="The name of the attribute that failed the measurement", alias="attributeName")
    description: Optional[StrictStr] = Field(default=None, description="Describing the reason that a specific measurement failed to be written to the DB")
    code: Optional[StrictStr] = Field(default=None, description="The code of the exception that failed the measurement to be written to the DB")
    entire_message_failure: Optional[StrictStr] = Field(default=None, description="Specifies if some data from the message was registered", alias="entireMessageFailure")
    # BioT Code Start: Store custom attributes in a dedicated dict (safe default)
    additional_properties: Dict[str, Any] = Field(default_factory=dict)
    # BioT Code End
    __properties: ClassVar[List[str]] = ["attributeName", "description", "code", "entireMessageFailure"]

    @field_validator('entire_message_failure')
    def entire_message_failure_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['ENTIRE_MESSAGE_FAILURE', 'PARTIAL_MESSAGE_FAILURE', 'UNKNOWN']):
            raise ValueError("must be one of enum values ('ENTIRE_MESSAGE_FAILURE', 'PARTIAL_MESSAGE_FAILURE', 'UNKNOWN')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of FailureData from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of FailureData from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "code": obj.get("code"),
            "entireMessageFailure": obj.get("entireMessageFailure")
        })
