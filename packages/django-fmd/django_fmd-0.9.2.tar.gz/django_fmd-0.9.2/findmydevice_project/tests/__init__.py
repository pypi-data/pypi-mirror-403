...
install_import_hook(packages=('findmydevice', 'findmydevice_project'))


PROJECT_ROOT = Path(findmydevice.__file__).parent.parent
...
def pre_configure_tests() -> None:
    ...
    # We use LiveServerTestCase that makes real requests, so we can't use:
