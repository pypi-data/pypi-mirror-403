...
class AlwaysLoggedInAsSuperUser:
    ...
    def _auto_login(self, request: HttpRequest):
        ...
        _print_and_message(
            request, f'Autologin applied. Your logged in as {settings.DEFAULT_USERNAME!r}'
        )
        user = authenticate(
            request=request, username=settings.DEFAULT_USERNAME, password=settings.DEFAULT_USERPASS
        )
        login(request, user)
