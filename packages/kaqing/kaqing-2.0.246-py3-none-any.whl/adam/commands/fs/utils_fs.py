...
def show_last_results_for_restart_nodes(state: ReplState, cmd: CommandInfo):
        ...
        # ls = NodeRestarter.pending()
        for k, v in sorted(list(NodeRestarter.pending().items()), key=lambda kv: kv[1]):
            lines.append(f'{k[0]}\t{k[1]}\tPending\t{datetime.fromtimestamp(v)}')

        tabulize(lines, header='POD\tNAMESPACE\tSTATUS\tSCHEDULED/COMPLETED', separator='\t')
