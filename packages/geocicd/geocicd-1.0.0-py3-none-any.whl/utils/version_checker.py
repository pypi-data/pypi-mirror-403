...
class VersionChecker:
    ...
    CACHE_FILE = Path.home() / '.geopipeline' / 'version_check_cache.json'
    CACHE_DURATION = timedelta(hours=1)
    DEFAULT_REGISTRY = 'https://pypi.org'
    PACKAGE_NAME = 'geopipeline'
    ...
    def get_registry_url(self) -> str:
        ...
        Checks in order:
        1. GEOPIPELINE_REGISTRY_URL environment variable
        2. ~/.geopipeline/config.yaml
        3. Default PyPI
        ...
        env_url = os.environ.get('GEOPIPELINE_REGISTRY_URL')
        if env_url:
            logger.debug(f"Using registry URL from environment: {env_url}")
            return env_url
        
        # Check config file
        config_file = Path.home() / '.geopipeline' / 'config.yaml'
