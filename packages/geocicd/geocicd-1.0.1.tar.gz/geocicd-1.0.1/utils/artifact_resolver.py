...
components, supporting Nexus, AWS ECR, and DockerHub registries.
"""
...
from utils.nexus_registry_client import NexusRegistryClient
...
class ArtifactResolver:
    ...
    unchanged components, supporting Nexus, AWS ECR, and DockerHub.
    ...
    def get_last_successful_artifact(
        ...
        registry_config = self._get_registry_config(config)
        if not registry_config:
            raise ArtifactResolverError(
                "No artifact registry configured for change detection",
                component=component,
            )
    ...
    def _get_registry_config(self, config: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
        ...
        return change_detection.get('artifactRegistry')
    ...
    def _get_image_name(self, component_config: Dict[str, Any], config: Dict[str, Any]) -> str:
        ...
        """
        # Try to get from build.destination
        build_config = component_config.get('build', {})
        destinations = build_config.get('destination', [])
        
        for dest in destinations:
            if dest.get('type') == 'dockerRegistry':
                image_name = dest.get('imageName')
                if image_name:
                    return image_name
        
        # Fallback: construct from project and component name
    ...
    def _create_registry_client(self, registry_config: Dict[str, Any]) -> RegistryClient:
        ...
        Raises:
            ArtifactResolverError: If registry type is unsupported
        """
        registry_type = registry_config.get('type', '').lower()
        registry_url = registry_config.get('url', '')
        ...
        if registry_type == 'nexus':
            ...
            return NexusRegistryClient(registry_url, username, password)
            ...
            raise ArtifactResolverError(
                f"Unsupported registry type: {registry_type}. Supported types: nexus, ecr, dockerhub",
