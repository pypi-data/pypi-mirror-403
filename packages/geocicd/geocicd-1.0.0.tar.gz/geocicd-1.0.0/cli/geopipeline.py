"""
GeoPipeline CLI - Main entry point
        ...
        if result.update_required:
            ...
            console.print(Panel(
                Text.from_markup(
                    ...
                    f"[cyan]pip install --upgrade geopipeline[/cyan]\n\n"
...
def cli(ctx: click.Context, skip_version_check: bool, version: bool) -> None:
    """
    GeoPipeline - GitLab CI/CD Pipeline Generator
    ...
    if version:
        console.print(f"GeoPipeline version {__version__}")
        sys.exit(0)
...
def version_info() -> None:
    ...
    console.print(f"\n[bold]GeoPipeline Version Information[/bold]\n")
    ...
    try:
        ...
        if result.update_required:
            console.print("[yellow]⚠️  Update available! Run:[/yellow] [cyan]pip install --upgrade geopipeline[/cyan]\n")
        else:
...
def main() -> None:
    ...
    This function is called when the geopipeline command is executed.
