"""
Nexus Docker registry client implementation.
"""
...
class NexusRegistryClient(RegistryClient):
    """Docker registry client for Nexus repositories."""
    
    def __init__(self, registry_url: str, username: Optional[str] = None, password: Optional[str] = None):
        """
        Initialize Nexus registry client.
        
        Args:
            registry_url: Nexus registry URL (e.g., nexus.example.com:8082)
        ...
        logger.debug(f"Initialized NexusRegistryClient for {self.registry_url}")
    
    def list_tags(self, image_name: str) -> List[str]:
        """
        List all tags for an image in Nexus registry.
        ...
        try:
            ...
            raise ArtifactResolverError(
                f"Failed to connect to Nexus registry: {str(e)}",
                registry_type="nexus",
                registry_url=self.registry_url,
            )
        except (KeyError, json.JSONDecodeError) as e:
            raise ArtifactResolverError(
                f"Invalid response from Nexus registry: {str(e)}",
                registry_type="nexus",
                registry_url=self.registry_url,
            )
    ...
    def get_tag_metadata(self, image_name: str, tag: str) -> Dict[str, Any]:
        """
        Get metadata for a specific image tag in Nexus.
