import ctypes
from ctypes import c_char_p, c_double, c_int, c_void_p, CFUNCTYPE, POINTER
import os
import threading
import sys
...
class OptQuestBase:
        ...
        if cls._dll is None:
            ...
            with cls._dll_lock:
                ...
                if cls._dll is None:
                    ...
                    mode = ctypes.RTLD_GLOBAL if hasattr(ctypes, 'RTLD_GLOBAL') else 0 | ctypes.RTLD_NOW if hasattr(ctypes, 'RTLD_NOW') else 0
    ...
    def _set_function_prototypes(cls):
        ...
        cls._dll.OQAddDiscreteVariable.argtypes = [HOptQuestModel, c_char_p, c_double, c_double, c_double]
        ...
        cls._dll.OQAddTupleVariable.argtypes = [HOptQuestModel, c_char_p, DoubleArray2D, c_int, c_int]
        cls._dll.OQAddGeolocationVariable.restype = c_int
        cls._dll.OQAddGeolocationVariable.argtypes = [HOptQuestModel, c_char_p, DoubleArray2D, c_int]
        ...
        cls._dll.OQAddExchangeableGroup.argtypes = [HOptQuestModel, c_char_p, StringArray2D, c_int, c_int]
        cls._dll.OQAddVariableExclusiveRange.restype = c_int
        cls._dll.OQAddVariableExclusiveRange.argtypes = [HOptQuestModel, c_char_p, c_double, c_double]
        ...
        cls._dll.OQSetObjectiveConfidence.argtypes = [HOptQuestModel, c_char_p, c_int, c_int, c_double]
        ...
        cls._dll.OQSetSampleRDSParams.argtypes = [HOptQuestModel, c_char_p, c_char_p, DoubleArray, c_int]
        ...
        cls._dll.OQParallelOptimize.argtypes = [HOptQuestModel, c_int, c_int, OQEvaluator, OQMonitor, c_void_p]
        cls._dll.OQParallelSample.restype = c_int
        cls._dll.OQParallelSample.argtypes = [HOptQuestModel, c_int, c_int, OQEvaluator, OQMonitor, c_void_p]
...
class OptQuestModel(OptQuestBase):
    ...
    def add_continuous_variable(self, name, min_val, max_val):
        return int(self._dll.OQAddContinuousVariable(self.model, c_char_p(name.encode('utf-8')), c_double(min_val), c_double(max_val))) != 0

    def add_integer_variable(self, name, min_val, max_val):
        return int(self._dll.OQAddIntegerVariable(self.model, c_char_p(name.encode('utf-8')), c_double(min_val), c_double(max_val))) != 0

    def add_discrete_variable(self, name, min_val, max_val, step):
        return int(self._dll.OQAddDiscreteVariable(self.model, c_char_p(name.encode('utf-8')), c_double(min_val), c_double(max_val), c_double(step))) != 0
    ...
    def add_design_variable(self, name, num):
        return int(self._dll.OQAddDesignVariable(self.model, c_char_p(name.encode('utf-8')), c_int(num))) != 0
    ...
    def add_enumeration_variable(self, name, values):
        ...
        return int(self._dll.OQAddEnumerationVariable(self.model, c_char_p(name.encode('utf-8')), c_values, c_int(n))) != 0
    ...
    def add_permutation_variable(self, group, names):
        ...
        return int(self._dll.OQAddPermutationVariable(self.model, c_char_p(group.encode('utf-8')), c_names, c_int(n))) != 0
    ...
    def add_tuple_variable(self, name, tuples):
        ...
        return int(self._dll.OQAddTupleVariable(self.model, c_char_p(name.encode('utf-8')), c_tuples, c_int(nt), c_int(nv))) != 0
    ...
    def add_geolocation_variable(self, name, locations):
        ...
        return int(self._dll.OQAddGeolocationVariable(self.model, c_char_p(name.encode('utf-8')), c_locations, c_int(nl))) != 0
    ...
    def add_selection_variable(self, name, selVars):
        ...
        c_names = (c_char_p * n)(*keep_alive)
        return int(self._dll.OQAddSelectionVariable(self.model, c_char_p(name.encode('utf-8')), c_names, c_int(n))) != 0
    ...
    def add_exchangeable_group(self, name, group):
        ...
        return int(self._dll.OQAddExchangeableGroup(self.model, c_char_p(name.encode('utf-8')), c_group, c_int(ng), c_int(nv))) != 0

    def add_variable_exclusive_range(self, name, min_val, max_val):
        return int(self._dll.OQAddVariableExclusiveRange(self.model, c_char_p(name.encode('utf-8')), c_double(min_val), c_double(max_val))) != 0
    ...
    def set_output_statistic(self, name, statistic, statistic_value=None):
        if statistic_value is None:
            statistic_value = 0
        return int(self._dll.OQSetOutputStatistic(self.model, c_char_p(name.encode('utf-8')), c_int(statistic), c_double(statistic_value))) != 0

    def add_expression_variable(self, name, expression):
        return int(self._dll.OQAddExpressionVariable(self.model, c_char_p(name.encode('utf-8')), c_char_p(expression.encode('utf-8')))) != 0

    def add_objective(self, name, direction, expression):
        return int(self._dll.OQAddObjective(self.model, c_char_p(name.encode('utf-8')), c_char_p(direction.encode('utf-8')), c_char_p(expression.encode('utf-8')))) != 0

    def add_minimize_objective(self, name, expression):
        return int(self._dll.OQAddMinimizeObjective(self.model, c_char_p(name.encode('utf-8')), c_char_p(expression.encode('utf-8')))) != 0

    def add_maximize_objective(self, name, expression):
        return int(self._dll.OQAddMaximizeObjective(self.model, c_char_p(name.encode('utf-8')), c_char_p(expression.encode('utf-8')))) != 0

    def set_objective_goal(self, name, min_val, max_val):
        return int(self._dll.OQSetObjectiveGoal(self.model, c_char_p(name.encode('utf-8')), c_double(min_val), c_double(max_val))) != 0

    def set_objective_confidence(self, name, ctype, clevel, epct):
        return int(self._dll.OQSetObjectiveConfidence(self.model, c_char_p(name.encode('utf-8')), c_int(ctype), c_int(clevel), c_double(epct))) != 0

    def set_objective_statistic(self, name, statistic, statistic_value):
        return int(self._dll.OQSetObjectiveStatistic(self.model, c_char_p(name.encode('utf-8')), c_int(statistic), c_double(statistic_value))) != 0

    def add_constraint(self, name, expression):
        return int(self._dll.OQAddConstraint(self.model, c_char_p(name.encode('utf-8')), c_char_p(expression.encode('utf-8')))) != 0
    ...
    def add_sample_metric(self, name, response, error):
        return int(self._dll.OQAddSampleMetric(self.model, c_char_p(name.encode('utf-8')), c_char_p(response.encode('utf-8')), c_char_p(error.encode('utf-8')))) != 0
    ...
    def set_sample_variogram(self, variogram):
        return int(self._dll.OQSetSampleVariogram(self.model, c_char_p(variogram.encode('utf-8')))) != 0
    ...
    def set_sample_rds_params(self, name, distribution, rdsParams):
        ...
        return int(self._dll.OQSetSampleRDSParams(self.model, c_char_p(name.encode('utf-8')), c_char_p(distribution.encode('utf-8')), c_values, n))
    ...
    def set_sample_rds_poisson(self, name, nBins, l):
        return self.set_sample_rds_params(name, 'POISSON', [nBins, l])
    ...
    def optimize(self, num_solutions, evaluator=None, monitor=None, npar=None):
        ...
        if npar is None:
            result = self._dll.OQOptimize(self.model, c_int(num_solutions), eval_func, mon_func, None)
        else:
            result = self._dll.OQParallelOptimize(self.model, c_int(npar), c_int(num_solutions), eval_func, mon_func, None)
        return int(result) != 0
    ...
    def sample(self, num_solutions, evaluator=None, monitor=None, npar=None):
        ...
        mon_func = Monitor(monitor) if monitor else ctypes.cast(None, OQMonitor)
        if npar is None:
            result = self._dll.OQSample(self.model, c_int(num_solutions), eval_func, mon_func, None)
        else:
            result = self._dll.OQParallelSample(self.model, c_int(npar), c_int(num_solutions), eval_func, mon_func, None)
        return int(result) != 0
...
class OptQuestSolution(OptQuestBase):
    ...
    def set_value(self, name, value):
        return int(self._dll.OQSetValue(self.solution, c_char_p(name.encode('utf-8')), c_double(value)))
