...
'''
        finding = Finding(title=title, test=test, description=description, cwe=798, severity=severity, file_path=findings['file_path'], line=findings['line_start'], dynamic_finding=False, static_finding=True, date=findings['commit_date'])
        key = hashlib.md5((((title + findings['match']) + str(findings['line_start'])) + str(findings['line_end'])).encode('utf-8'), usedforsecurity=False).hexdigest()
        if (key not in dupes):
            dupes[key] = finding
