...
from norm_findings.stubs.django.core.files.uploadedfile import TemporaryUploadedFile
...
'''
            ...
            try:
                ...
                cwe = 0
            dupe_key = hashlib.md5(str((references + title)).encode('utf-8'), usedforsecurity=False).hexdigest()
            if (dupe_key in dupes):
                ...
                dupes[dupe_key] = True
                finding = Finding(title=title, unique_id_from_tool=dupe_key, date=date, test=test, active=active, is_mitigated=is_mitigated, mitigated=mitigated, description=description, severity=severity, mitigation='See description', impact='No impact provided', references=references, cwe=cwe, dynamic_finding=False, cvssv3=cvssv3_vector, cvssv3_score=cvssv3_score)
...
'''
        if (triaged_date is not None):
            triaged_date = datetime.strftime(datetime.strptime(triaged_date, '%Y-%m-%dT%H:%M:%S.%fZ'), '%Y-%m-%d')
            description += f'''Triaged: {triaged_date}
'''
        description += '##Report: \n{}\n'.format(content['attributes']['vulnerability_information'])
        structured_scope_fields_to_label: dict[(str, str)] = {'asset_identifier': 'Asset Identifier', 'asset_type': 'Asset Type', 'confidentiality_requirement': 'Confidentiality Requirement', 'integrity_requirement': 'Integrity Requirement', 'availability_requirement': 'Availability Requirement', 'max_severity': 'Max Severity', 'instruction': 'Instruction', 'eligible_for_bounty': 'Eligible for Bounty', 'eligible_for_submission': 'Eligible for Submission', 'reference': 'Reference'}
...
class HackerOneBugBountyProgram():
    ...
    fields_to_label: ClassVar[dict[(str, str)]] = {'id': 'ID', 'weakness': 'Weakness Category', 'substate': 'Substate', 'reporter': 'Reporter', 'assigned': 'Assigned To', 'public': 'Public', 'triageted_at': 'Triaged On', 'closed_at': 'Closed On', 'awarded_at': 'Awarded On', 'bounty': 'Bounty Price', 'bonus': 'Bonus', 'first_response_at': 'First Response On', 'source': 'Source', 'reference': 'Reference', 'reference_url': 'Reference URL', 'structured_scope': 'Structured Scope', 'structured_scope_reference': 'Structured Scope Reference', 'original_report_id': 'Original Report ID', 'collaborating_users': 'Collaboration Users', 'duplicate_report_ids': 'Duplicate Report IDs'}
    ...
    def parse_findings(self, dict_list: list[dict], test: Test) -> list[Finding]:
        ...
        for entry in dict_list:
            ...
            finding = Finding(title=entry.get('title'), severity=self.convert_severity(entry), description=self.parse_description(entry), date=date_parser.parse(entry.get('reported_at')), dynamic_finding=True, test=test, **status_dict)
    ...
    def determine_status(self, row) -> dict:
        ...
        if ((status := row.get('state')) is not None):
            ...
            if (status == 'closed'):
                return {'is_mitigated': True, 'active': False, 'mitigated': date_parser.parse(row.get('closed_at'))}
        return default_status
...
class H1Parser(HackerOneVulnerabilityDisclosureProgram, HackerOneBugBountyProgram):
    ...
    def get_findings(self, scan_file: TemporaryUploadedFile, test: Test) -> list[Finding]:
        'Return the list of findings generated from the uploaded report.'
        file_name = scan_file.name
        if str(file_name).endswith('.json'):
            return self.determine_json_format(scan_file, test)
        if str(file_name).endswith('.csv'):
            return self.determine_csv_format(scan_file, test)
