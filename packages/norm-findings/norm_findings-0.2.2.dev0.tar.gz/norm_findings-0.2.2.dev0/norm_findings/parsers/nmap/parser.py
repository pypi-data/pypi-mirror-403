...
'''
            ...
            for port_element in host.findall('ports/port'):
                ...
                endpoint = Endpoint(host=(fqdn or ip), protocol=protocol)
...
'''
                ...
                if (dupe_key in dupes):
                    ...
                    find = Finding(title=title, test=test, description=description, severity=severity, mitigation='N/A', impact='No impact provided', vuln_id_from_tool=script_id)
    ...
    def manage_vulner_script(self, test, dupes, script_element, endpoint, report_date=None):
        for component_element in script_element.findall('table'):
            ...
            for vuln in component_element.findall('table'):
                ...
                finding = Finding(title=vuln_id, test=test, description=description, severity=severity, component_name=(component_cpe.get_product()[0] if (len(component_cpe.get_product()) > 0) else ''), component_version=(component_cpe.get_version()[0] if (len(component_cpe.get_version()) > 0) else ''), vuln_id_from_tool=vuln_id, nb_occurences=1)
                finding.unsaved_endpoints = [endpoint]
                if (('type' in vuln_attributes) and (vuln_attributes['type'] == 'cve')):
