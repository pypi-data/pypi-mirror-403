...
from norm_findings.parsers.reversinglabs_spectraassure.rlJsonInfo import RlJsonInfo
from norm_findings.parsers.reversinglabs_spectraassure.rlJsonInfo.cve_info_node import CveInfoNode
...
class ReversinglabsSpectraassureParser():
    ...
    def _find_duplicate(self, key: str) -> typing.Union[(Finding, None)]:
        logger.debug('')
    ...
    def _one_finding(self, *, node: CveInfoNode, test: Any) -> Finding:
        logger.debug('%s', node)
        key = self._make_hash(((node.title + ' ') + node.component_file_path))
        cve = node.cve
        finding = Finding(date=node.scan_date, title=node.title, description=(((node.title + ' ') + node.description) + '\n'), cve=cve, cvssv3_score=node.score, severity=node.score_severity, vuln_id_from_tool=node.vuln_id_from_tool, file_path=node.component_file_path, component_name=node.component_name, component_version=node.component_version, nb_occurences=1, hash_code=key, references=None, active=True, test=test, static_finding=True, dynamic_finding=False)
    ...
    def get_findings(self, scan_file: Any, test: Any) -> list[Finding]:
        rl_json_info_instance = RlJsonInfo(file_handle=scan_file)
        ...
        for cve_info_node_instance in rl_json_info_instance.get_results_list():
            finding = self._one_finding(node=cve_info_node_instance, test=test)
            if (finding is None):
                continue
