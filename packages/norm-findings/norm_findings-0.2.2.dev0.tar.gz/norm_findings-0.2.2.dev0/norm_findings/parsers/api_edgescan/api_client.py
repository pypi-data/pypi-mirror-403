...
class EdgescanAPI():
    ...
    def get_findings(self, asset_ids):
        ...
        response = requests.get(url=url, headers=self.get_headers(), proxies=self.get_proxies(), timeout=settings.REQUESTS_TIMEOUT)
        response.raise_for_status()
        return response.json()

    def get_headers(self):
        return {'X-API-TOKEN': self.api_key, 'Content-Type': 'application/json', 'User-Agent': 'DefectDojo'}
