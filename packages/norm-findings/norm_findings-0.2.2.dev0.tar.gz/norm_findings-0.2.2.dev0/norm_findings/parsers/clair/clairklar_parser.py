...
class ClairKlarScan():
    ...
    def get_item_clairklar(self, item_node, test):
        ...
        if ('Link' in item_node):
            link = item_node['Link']
        return Finding(title=((((((item_node['Name'] + ' - ') + '(') + item_node['FeatureName']) + ', ') + item_node['FeatureVersion']) + ')'), test=test, severity=severity, description=description, mitigation=mitigation, references=link, false_p=False, duplicate=False, out_of_scope=False, mitigated=None, cwe=1035, static_finding=True, dynamic_finding=False, impact='No impact provided')
