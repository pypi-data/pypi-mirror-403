...
class Compliance():
    ...
    def get_item(self, finding: dict, test):
        ...
        resource_arns = [arn for resource in finding.get('Resources', []) if (arn := resource.get('Id'))]
        mitigation = ''
        impact = []
        references = []
...
'''
        ...
        for resource in finding.get('Resources', []):
            ...
            if (component_name == 'AwsEcrContainerImage'):
                ...
                arn = resource.get('Id')
                if details:
                    impact.extend((f'Image ARN: {arn}', f"Registry: {details.get('RegistryId')}", f"Repository: {details.get('RepositoryName')}", f"Image digest: {details.get('ImageDigest')}", f"Image tags: {','.join(details.get('ImageTags', []))}"))
                title_suffix = f" - Image: {arn.split('/', 1)[1]}"
            else:
        ...
        false_p = False
        result = Finding(title=f'{title}{title_suffix}', test=test, description=description, mitigation=mitigation, references='\n'.join(references), severity=severity, impact='\n'.join(impact), active=active, verified=False, false_p=false_p, unique_id_from_tool=finding_id, mitigated=mitigated, is_mitigated=is_Mitigated, static_finding=True, dynamic_finding=False, component_name=component_name)
