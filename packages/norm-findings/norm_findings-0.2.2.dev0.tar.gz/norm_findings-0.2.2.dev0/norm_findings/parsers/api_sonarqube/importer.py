...
import typing
import datetime
# Required stubs: Sonarqube_Issue, Finding
...
from norm_findings.stubs.models import Finding, Sonarqube_Issue
from norm_findings.stubs.utils import create_notification
...
class SonarQubeApiImporter():
    ...
    def is_confirmed(state):
        return (state.lower() in {'confirmed', 'accepted', 'detected'})

    @staticmethod
    def is_closed(state):
        return (state.lower() in {'resolved', 'falsepositive', 'wontfix', 'closed', 'dismissed', 'rejected'})
    ...
    def prepare_client(test):
        ...
        if test.api_scan_configuration:
            ...
            if (config.product != product):
                msg = f'Product API Scan Configuration and Product do not match. Product: "{product.name}" ({product.id}), config.product: "{config.product.name}" ({config.product.id})'
            ...
            if (sqqs.count() == 1):
                ...
                msg = f'More than one Product API Scan Configuration has been configured, but none of them has been chosen. Please specify which one should be used. Product: "{product.name}" ({product.id})'
                raise ValidationError(msg)
            else:
                msg = f'''There are no API Scan Configurations for this Product.
Please add at least one API Scan Configuration for SonarQube to this Product. Product: "{product.name}" ({product.id})'''
                raise ValidationError(msg)
    ...
    def import_issues(self, test):
        ...
        try:
            ...
            logger.info(f"Found {len(issues)} issues for component {component['key']}")
            sonarUrl = client.sonar_api_url[:(- 3)]
            for issue in issues:
                status = issue['status']
...
'''
                ...
                (sonarqube_issue, _) = Sonarqube_Issue.objects.update_or_create(key=issue['key'], defaults={'status': status, 'type': issue_type})
                if Finding.objects.filter(sonarqube_issue=sonarqube_issue).exists():
                    sonarqube_issue = None
                find = Finding(title=title, cwe=cwe, description=description, test=test, severity=severity, references=references, file_path=component_key, line=line, verified=self.is_confirmed(status), false_p=False, duplicate=False, out_of_scope=False, mitigated=None, mitigation='No mitigation provided', impact='No impact provided', static_finding=True, sonarqube_issue=sonarqube_issue, unique_id_from_tool=issue.get('key'))
                items.append(find)
        except Exception as e:
            logger.exception('SonarQube API import issue')
            create_notification(event='sonarqube_failed', title='SonarQube API import issue', description=e, icon='exclamation-triangle', source='SonarQube API', obj=test.engagement.product)
        return items
    ...
    def import_hotspots(self, test):
        try:
            ...
            logger.info(f"Found {len(hotspots)} hotspots for project {component['key']}")
            sonarUrl = client.sonar_api_url[:(- 3)]
...
'''
                    ...
                    sonarqube_permalink = 'No permalink \n'
                references = ((sonarqube_permalink + self.get_references(rule.get('riskDescription', ''))) + self.get_references(rule.get('fixRecommendations', '')))
                (sonarqube_issue, _) = Sonarqube_Issue.objects.update_or_create(key=hotspot['key'], defaults={'status': status, 'type': issue_type})
                if Finding.objects.filter(sonarqube_issue=sonarqube_issue).exists():
                    sonarqube_issue = None
                find = Finding(title=title, cwe=cwe, description=description, test=test, severity=severity, references=references, file_path=component_key, line=line, active=True, verified=self.is_confirmed(status), false_p=False, duplicate=False, out_of_scope=False, static_finding=True, scanner_confidence=scanner_confidence, sonarqube_issue=sonarqube_issue, unique_id_from_tool=f"hotspot:{hotspot.get('key')}")
                items.append(find)
        except Exception as e:
            logger.exception('SonarQube API import issue')
            create_notification(event='sonarqube_failed', title='SonarQube API import issue', description=e, icon='exclamation-triangle', source='SonarQube API', obj=test.engagement.product)
    ...
    def clean_rule_description_html(raw_html):
        search = re.search('^(.*?)(?:(<h2>See</h2>)|(<b>References</b>))', raw_html, re.DOTALL)
        if search:
    ...
    def clean_cwe(raw_html):
        search = re.search('CWE-(\\d+)', raw_html)
        if search:
