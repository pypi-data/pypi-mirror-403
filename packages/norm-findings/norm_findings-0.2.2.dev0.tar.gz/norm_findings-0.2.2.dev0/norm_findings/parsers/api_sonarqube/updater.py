...
import typing
import datetime
# Required stubs: Sonarqube_Issue_Transition

import logging
from collections import deque
from norm_findings.stubs.models import Sonarqube_Issue_Transition
...
class SonarQubeApiUpdater():
    ...
    def get_sonarqube_required_transitions_for(self, current_status, target_status, is_hotspot):
        if (current_status == target_status):
            return None
        mapping = (self.MAPPING_SONARQUBE_HOTSPOT_STATUS_TRANSITION if is_hotspot else self.MAPPING_SONARQUBE_STATUS_TRANSITION)
        if (not [x for x in mapping if (current_status in x.get('from'))]):
            return None
        transitions = [x for x in mapping if (target_status == x.get('to'))]
        if transitions:
            ...
            if (not is_hotspot):
                ...
                for transition in transitions:
                    for t_from in transition.get('from'):
                        possible_transition = self.get_sonarqube_required_transitions_for(current_status, t_from, is_hotspot)
                        if possible_transition:
                            transitions_result.extendleft(possible_transition)
                            return list(transitions_result)
            else:
                transitions_result = deque()
                transitions_result.appendleft({'status': transitions[0].get('transition'), 'resolution': transitions[0].get('resolution')})
                return list(transitions_result)
        return None
    ...
    def update_sonarqube_finding(self, finding):
        ...
        if (not sonarqube_issue):
            return
        logger.debug("Checking if finding '%s' needs to be updated in SonarQube", finding)
        ...
        if is_hotspot:
            ...
            current_status = issue.get('status')
        transitions = self.get_sonarqube_required_transitions_for(current_status, target_status, is_hotspot=is_hotspot)
        if (not transitions):
            logger.debug('--> SQ Current status: %s. Current target status: %s', current_status, target_status)
            return
        logger.debug(f"Updating finding '{finding}' transition {current_status} -> {target_status} in SonarQube")
        for transition in transitions:
            if is_hotspot:
                client.transition_hotspot(sonarqube_issue.key, status=transition['status'], resolution=transition['resolution'])
            else:
                client.transition_issue(sonarqube_issue.key, transition)
        Sonarqube_Issue_Transition.objects.create(sonarqube_issue=finding.sonarqube_issue, finding_status=(finding.status().replace('Risk Accepted', 'Accepted') if finding.status() else finding.status()), sonarqube_status=current_status, transitions=','.join(((transition['status'] if is_hotspot else transition) for transition in transitions)))
