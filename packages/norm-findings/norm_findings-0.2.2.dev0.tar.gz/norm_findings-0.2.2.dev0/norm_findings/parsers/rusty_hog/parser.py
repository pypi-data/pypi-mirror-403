...
class RustyhogParser():
    ...
    def get_tests(self, scan_type, handle):
        ...
        test = ParserTest(name=parsername, parser_type=parsername, version='')
    ...
    def __getitem(self, vulnerabilities, scanner):
        ...
        for vulnerability in vulnerabilities:
            ...
            if (vulnerability.get('reason') is not None):
                description += '\n**Reason:** {}'.format(vulnerability.get('reason'))
...
**This string was found:** {found_secret_string}'''
                ...
                if ((vulnerability.get('old_file_id') is not None) and (vulnerability.get('new_file_id') is not None)):
                    description += '\n**Old and new file IDs:** {} - {}'.format(vulnerability.get('old_file_id'), vulnerability.get('new_file_id'))
                if ((vulnerability.get('old_line_num') is not None) and (vulnerability.get('new_line_num') is not None)):
                    description += '\n**Old and new line numbers:** {} - {}'.format(vulnerability.get('old_line_num'), vulnerability.get('new_line_num'))
...
**This string was found:** {found_secret_string}'''
            ...
            if (scanner == 'Choctaw Hog'):
                title = '{} found in Git path {} ({})'.format(vulnerability.get('reason'), vulnerability.get('path'), vulnerability.get('commitHash'))
            elif (scanner == 'Duroc Hog'):
                title = '{} found in path {}'.format(vulnerability.get('reason'), vulnerability.get('path'))
            elif (scanner == 'Gottingen Hog'):
                title = '{} found in Jira ID {} ({})'.format(vulnerability.get('reason'), vulnerability.get('issue_id'), vulnerability.get('location'))
                if (not file_path):
                    file_path = vulnerability.get('url')
            elif (scanner == 'Essex Hog'):
                title = '{} found in Confluence Page ID {}'.format(vulnerability.get('reason'), vulnerability.get('page_id'))
                if (not file_path):
                    file_path = vulnerability.get('url')
            finding = Finding(title=title, severity='High', cwe=cwe, description=description, file_path=file_path, static_finding=True, dynamic_finding=False, payload=found_secret_string)
