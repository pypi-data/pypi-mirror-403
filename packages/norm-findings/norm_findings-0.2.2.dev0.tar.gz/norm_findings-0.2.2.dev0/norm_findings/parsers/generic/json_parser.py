...
import typing
import datetime
# Required stubs: Finding, Endpoint, FileUpload

import base64
import dateutil
from norm_findings.stubs.django.core.files.base import ContentFile
from norm_findings.stubs.models import Endpoint, FileUpload, Finding
...
class GenericJSONParser():
    ...
    def _get_test_json(self, data):
        test_internal = ParserTest(name=data.get('name', self.ID), parser_type=data.get('type', self.ID), version=data.get('version'), description=data.get('description'), dynamic_tool=data.get('dynamic_tool'), static_tool=data.get('static_tool'), soc=data.get('soc'))
