...
class CoverityApiParser():
    ...
    def get_findings(self, scan_file, test):
        ...
        if ('viewContentsV1' not in tree):
            ...
            raise ValueError(msg, scan_file.name)
        items = []
        for issue in tree['viewContentsV1']['rows']:
            if (issue.get('displayIssueKind') != 'Security'):
                continue
            description_formated = '\n'.join([f"**CID:** `{issue.get('cid')}`", f"**Type:** `{issue.get('displayType')}`", f"**Status:** `{issue.get('status')}`", f"**Classification:** `{issue.get('classification')}`"])
            finding = Finding()
            finding.test = test
