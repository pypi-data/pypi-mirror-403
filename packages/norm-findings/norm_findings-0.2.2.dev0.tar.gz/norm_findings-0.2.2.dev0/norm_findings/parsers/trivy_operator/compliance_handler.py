        ...
        for result in benchmarkreport.get('results'):
            for check in result.get('checks'):
                ...
                if (check.get('success') is False):
                    ...
                    finding = Finding(test=test, title=title, severity=severity, mitigation=check_remediation, description=description, static_finding=False, dynamic_finding=True, fix_available=True)
                    if check_checkID:
