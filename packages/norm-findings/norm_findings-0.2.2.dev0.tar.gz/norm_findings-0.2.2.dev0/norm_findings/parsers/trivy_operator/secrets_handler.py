...
class TrivySecretsHandler():
    ...
    def handle_secrets(self, labels, secrets, test):
        ...
        for secret in secrets:
            ...
            secret_description = SECRET_DESCRIPTION_TEMPLATE.format(title=secret_title, category=secret_category, match=secret_match)
            secret_description += ('\n**container.name:** ' + container_name)
            secret_description += ('\n**resource.kind:** ' + resource_kind)
            secret_description += ('\n**resource.name:** ' + resource_name)
            secret_description += ('\n**resource.namespace:** ' + resource_namespace)
            secret_description += ('\n**ruleID:** ' + secret_rule_id)
            finding = Finding(test=test, title=title, severity=secret_severity, references=secret_references, description=secret_description, file_path=secret_target, static_finding=True, dynamic_finding=False, service=service, fix_available=True)
