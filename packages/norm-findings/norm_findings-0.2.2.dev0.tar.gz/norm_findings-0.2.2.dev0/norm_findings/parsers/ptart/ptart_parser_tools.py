...
def parse_ptart_fix_effort(effort):
    effort_mapping = {1: 'High', 2: 'Medium', 3: 'Low'}
    return effort_mapping.get(effort)

def parse_title_from_hit(hit):
    hit_title = hit.get('title', None)
    hit_id = hit.get('id', None)
    return (f'{hit_id}: {hit_title}' if (hit_title and hit_id) else (hit_title or hit_id or 'Unknown Hit'))
...
def parse_retest_status(status):
    fix_status_mapping = {'F': 'Fixed', 'NF': 'Not Fixed', 'PF': 'Partially Fixed', 'NA': 'Not Applicable', 'NT': 'Not Tested'}
...
def get_file_suffix_from_screenshot(screenshot):
    return (pathlib.Path(screenshot['screenshot']['filename']).suffix if (('screenshot' in screenshot) and ('filename' in screenshot['screenshot'])) else '')
