...
class NiktoXMLParser():
    ...
    def process_scandetail(self, scan, test, dupes):
        for item in scan.findall('item'):
            ...
            sentences = re.split('(?<!\\w\\.\\w.)(?<![A-Z][a-z]\\.)(?<=\\.|\\?)\\s', description)
            titleText = (sentences[0][:900] if (len(sentences) > 0) else description[:900])
            description = '\n'.join([f"**Host:** `{item.findtext('iplink')}`", f"**Description:** `{item.findtext('description')}`", f"**HTTP Method:** `{item.attrib.get('method')}`"])
            severity = 'Info'
            if ((item.get('osvdbid') is not None) and (item.get('osvdbid') != '0')):
                severity = 'Medium'
            finding = Finding(title=titleText, test=test, description=description, severity=severity, dynamic_finding=True, static_finding=False, vuln_id_from_tool=item.attrib.get('id'), nb_occurences=1)
            try:
