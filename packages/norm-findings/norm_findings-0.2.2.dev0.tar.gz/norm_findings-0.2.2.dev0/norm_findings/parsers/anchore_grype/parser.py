...
class AnchoreGrypeParser():
    ...
    def get_description_for_scan_types(self, scan_type):
        return "A vulnerability scanner for container images, filesystems, and SBOMs. JSON report generated with '--output=json' format."

    def get_findings(self, scan_file, test):
        logger.debug('file: %s', scan_file)
...
'''
            ...
            if (dupe_key in dupes):
                ...
                dupes[dupe_key] = Finding(title=finding_title.replace('\x00', ''), description=finding_description.replace('\x00', ''), cvssv3=finding_cvss3, epss_score=finding_epss_score, epss_percentile=finding_epss_percentile, severity=vuln_severity, mitigation=finding_mitigation, references=finding_references, component_name=artifact_name, component_version=artifact_version.replace('\x00', ''), vuln_id_from_tool=vuln_id, tags=finding_tags, static_finding=True, dynamic_finding=False, nb_occurences=1, file_path=file_path, fix_available=fix_available, fix_version=fix_version)
    ...
    def get_cvss(self, cvss):
        if cvss:
            for cvss_item in cvss:
                ...
                if ((len(cvss_objects) > 0) and isinstance(cvss_objects[0], CVSS3)):
                    return vector
        return None
    ...
    def get_vulnerability_ids(self, vuln_id, related_vulnerabilities):
        ...
        if related_vulnerabilities:
            vulnerability_ids.extend((related_vulnerability_id for related_vulnerability in related_vulnerabilities if (related_vulnerability_id := related_vulnerability.get('id'))))
