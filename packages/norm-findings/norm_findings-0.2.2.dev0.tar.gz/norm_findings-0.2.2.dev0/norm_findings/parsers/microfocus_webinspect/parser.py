                ...
                if (classifications is not None):
                    for content in classifications.findall('Classification'):
                        if (('kind' in content.attrib) and (content.attrib['kind'] == 'CWE')):
                            ...
                            description += (('\n\n' + content.text) + '\n')
                finding = Finding(title=issue.findtext('Name'), test=test, cwe=cwe, description=description, mitigation=mitigation, severity=severity, references=reference, static_finding=False, dynamic_finding=True, nb_occurences=1)
                if ('id' in issue.attrib):
                    finding.unique_id_from_tool = issue.attrib.get('id')
                finding.unsaved_endpoints = [endpoint]
                dupe_key = hashlib.sha256(f'{finding.description}|{finding.title}|{finding.severity}'.encode()).hexdigest()
