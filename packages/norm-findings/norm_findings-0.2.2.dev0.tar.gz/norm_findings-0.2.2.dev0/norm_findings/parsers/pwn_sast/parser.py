    ...
    def get_findings(self, scan_file, test):
        ...
        if (results is not None):
            ...
            for data_hash in data_arr:
                ...
                steps_to_reproduce = '\n'.join(['Install pwn_sast Driver via: https://github.com/0dayinc/pwn#installation', 'Execute the pwn_sast Driver via:', f'```pwn_sast --dir-path . --uri-source-root {git_repo_root_uri} -s```'])
                for line in line_no_and_contents:
                    ...
                    severity = 'Info'
                    description = '\n'.join([f'SAST Module: {sast_module}', f'Offending URI: {offending_uri}', f'Line: {line_no}', f'Committed By: {author}', 'Line Contents:', f'```{contents}```'])
                    impact = '\n'.join([f'Security Control Impacted: {section}', f'NIST 800-53 Security Control Details: {nist_800_53_uri}', f'CWE Details: {cwe_uri}'])
                    mitigation = '\n'.join([f'NIST 800-53 Security Control Details / Mitigation Strategy: {nist_800_53_uri}'])
                    unique_finding_key = hashlib.sha256((offending_uri + contents).encode('utf-8')).hexdigest()
                    if (unique_finding_key in findings):
                        finding = findings[unique_finding_key]
                        finding.nb_occurences += 1
                    else:
                        finding = Finding(title=f'{sast_module} Entry in {offending_file} Line: {line_no}', test=test, severity=severity, description=description, impact=impact, mitigation=mitigation, static_finding=True, dynamic_finding=False, cwe=cwe_id, nb_occurences=1, steps_to_reproduce=steps_to_reproduce, file_path=offending_file, unique_id_from_tool=unique_finding_key)
                        if mitigation:
