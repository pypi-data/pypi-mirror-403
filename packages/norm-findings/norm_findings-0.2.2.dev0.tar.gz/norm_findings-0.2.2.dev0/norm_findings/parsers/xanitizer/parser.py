...
class XanitizerParser():
    ...
    def get_findings_internal(self, root, test):
        ...
        for finding in root.findall('finding'):
            ...
            dojofinding = Finding(test=test, title=self.generate_title(finding, line), description=description, cwe=self.resolve_cwe(finding), severity=self.resolve_severity(finding), file_path=self.generate_file_path(finding), line=line, date=date, static_finding=True)
...
**Code Excerpt:** '''
    ...
    def generate_file_path(self, finding):
        ...
        if ((finding.find('node') is not None) and finding.find('node').get('relativePath')):
