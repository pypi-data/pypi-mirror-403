                ...
                for severity in severity_results:
                    ...
                    for vuln in vulns:
                        ...
                        if (vuln['severity'] not in {'High', 'Medium', 'Low'}):
                            continue
                        unique_key = hashlib.md5(str(vuln).encode('utf-8'), usedforsecurity=False).hexdigest()
...
def get_item(item_node, test):
    ...
    return Finding(title=item_node.get('title'), test=test, severity=item_node.get('severity'), description=description, mitigation='N/A', references='', false_p=False, duplicate=False, out_of_scope=False, mitigated=None, file_path=item_node.get('file'), line=item_node.get('line'), static_finding=True, dynamic_finding=False, impact='No impact provided')
