...
'''
        ...
        dupe_key = hashlib.md5((((title + secret) + severity) + str(line)).encode('utf-8'), usedforsecurity=False).hexdigest()
        if (dupe_key in dupes):
            finding = dupes[dupe_key]
            ...
            finding = Finding(title=title, test=test, cwe=798, description=description.replace('\x00', '�'), severity=severity, mitigation='Secrets and passwords should be stored in a secure vault or secure storage.', impact='This weakness can lead to the exposure of resources or functionality to unintended actors, possibly providing attackers with sensitive information or even execute arbitrary code.', file_path=file, line=line, dynamic_finding=False, static_finding=True, nb_occurences=1)
            dupes[dupe_key] = finding
