...
import html2text
from norm_findings.stubs.django.utils.encoding import force_str
...
class ArachniParser():
    ...
    def get_item(self, item_node, report_date):
        ...
        remediation = item_node.get('remedy_guidance', 'n/a')
        if remediation:
        ...
        if (severity == 'Informational'):
            severity = 'Info'
        finding = Finding(title=item_node['name'], date=report_date, severity=severity, description=description, mitigation=remediation, references=references, impact='No impact provided', cwe=item_node.get('cwe'), vuln_id_from_tool=item_node.get('digest'))
