...
import logging
from norm_findings.parsers.sonarqube.soprasteria_helper import SonarQubeSoprasteriaHelper
    ...
    def get_items(self, tree, test, mode):
        ...
        if (len(detailTbody) == 2):
            ...
            for vuln in vulnerabilities_table:
                ...
                vuln_severity = SonarQubeSoprasteriaHelper().convert_sonar_severity((vuln_properties[1].text and vuln_properties[1].text.strip()))
                ...
                try:
                    ...
                    vuln_references = SonarQubeSoprasteriaHelper().get_references(vuln_rule_name, vuln_details)
                    ...
                    vuln_cwe = 0
                if (mode is None):
                    SonarQubeSoprasteriaHelper().process_result_file_name_aggregated(test, dupes, vuln_title, vuln_cwe, vuln_description, vuln_file_path, vuln_line, vuln_severity, vuln_mitigation, vuln_references)
                else:
                    SonarQubeSoprasteriaHelper().process_result_detailed(test, dupes, vuln_title, vuln_cwe, vuln_description, vuln_file_path, vuln_line, vuln_severity, vuln_mitigation, vuln_references, vuln_key)
