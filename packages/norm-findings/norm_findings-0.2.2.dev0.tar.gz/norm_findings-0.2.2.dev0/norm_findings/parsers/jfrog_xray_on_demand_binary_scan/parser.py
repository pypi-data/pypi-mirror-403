...
def get_component_name_version(name):
    match = re.match('([a-z]+://[a-z\\d\\.:]+):([a-z\\d\\.\\-]+)', name, re.IGNORECASE)
    if (match is None):
        return (name, '')
...
def get_item_set(vulnerability):
    ...
    for (component_name_with_version, component) in vulnerability.get('components', {}).items():
        ...
        finding = Finding(title=title, severity_justification=(severity_justification or None), severity=severity, description=vulnerability['summary'].strip(), mitigation=((mitigation + remediation) or None), component_name=component_name, component_version=component_version, impact=(impact or None), references=(references or None), static_finding=True, dynamic_finding=False, cvssv3=cvssv3, vuln_id_from_tool=vuln_id_from_tool, fix_available=fix_available)
