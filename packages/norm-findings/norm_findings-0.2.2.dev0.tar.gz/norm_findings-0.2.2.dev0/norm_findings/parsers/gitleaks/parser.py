...
class GitleaksParser():
    ...
    def get_fields(self) -> list[str]:
        ...
        return ['title', 'description', 'severity', 'file_path', 'line', 'dynamic_finding', 'static_finding', 'nb_occurences']
...
'''
        ...
        if (('Github' in reason) or ('AWS' in reason) or ('Heroku' in reason)):
            severity = 'Critical'
        finding = Finding(title=titleText, test=test, cwe=798, description=description, severity=severity, file_path=file_path, line=line, dynamic_finding=False, static_finding=True)
...
'''
        ...
        severity = 'High'
        dupe_key = hashlib.md5(((title + secret) + str(line)).encode('utf-8'), usedforsecurity=False).hexdigest()
        if (dupe_key in dupes):
            ...
            finding = Finding(title=title, test=test, cwe=798, description=description, severity=severity, file_path=file_path, line=line, dynamic_finding=False, static_finding=True, nb_occurences=1)
            if tags:
