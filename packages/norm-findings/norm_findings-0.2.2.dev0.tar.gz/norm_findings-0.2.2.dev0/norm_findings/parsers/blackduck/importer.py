...
class BlackduckImporter(Importer):
    ...
    def _process_project_findings(self, project_ids, security_issues, files=None):
        ...
        for project_id in project_ids:
            ...
            for issue in security_issues[project_id]:
                ...
                (yield BlackduckFinding(cve, security_issue_dict.get('Description'), security_issue_dict.get('Security Risk'), security_issue_dict.get('Impact'), (security_issue_dict.get('Component name') or security_issue_dict.get('Project name')), (security_issue_dict.get('Component version name') or security_issue_dict.get('Version')), security_issue_dict.get('Vulnerability source'), security_issue_dict.get('URL'), security_issue_dict.get('Channel version origin id'), security_issue_dict.get('Component origin id'), security_issue_dict.get('Published on'), security_issue_dict.get('Updated on'), security_issue_dict.get('Base score'), security_issue_dict.get('Exploitability'), security_issue_dict.get('Remediation status'), security_issue_dict.get('Remediation target date'), security_issue_dict.get('Remediation actual date'), security_issue_dict.get('Remediation comment'), location))
    ...
    def get_cve(self, vuln_id):
        cve_search = re.search('CVE-\\d{4}-\\d{4,9}', vuln_id, re.IGNORECASE)
