...
TRIVY_SEVERITIES = {'CRITICAL': 'Critical', 'HIGH': 'High', 'MEDIUM': 'Medium', 'LOW': 'Low', 'UNKNOWN': 'Info'}
CVSS_SEVERITY_SOURCES = ['nvd', 'ghsa', 'redhat', 'bitnami']
...
class TrivyParser():
    ...
    def convert_trivy_status(self, trivy_status: str) -> dict:
        ...
        status_mapping = {'unknown': {'verified': False}, 'not_affected': {'active': False, 'verified': True, 'is_mitigated': True}, 'affected': {'active': True, 'verified': True}, 'fixed': {'active': True, 'verified': True}, 'under_investigation': {'active': True, 'verified': False}, 'will_not_fix': {'active': True, 'verified': True}, 'fix_deferred': {'active': True, 'verified': True}, 'end_of_life': {'active': True, 'verified': True}}
    ...
    def get_findings(self, scan_file, test):
        ...
        if (cluster_name is not None):
            ...
            for resource in resources:
                ...
                findings += self.get_result_items(test, resource.get('Results', []), resource_name)
            return findings
    ...
    def get_result_items(self, test, results, service_name=None, artifact_name=''):
        ...
        for target_data in results:
            ...
            for vuln in vulnerabilities:
                ...
                description = DESCRIPTION_TEMPLATE.format(title=vuln.get('Title', ''), target=target, type=vul_type, fixed_version=mitigation, description_text=vuln.get('Description', ''))
                finding = Finding(test=test, title=title, cwe=cwe, severity=severity, file_path=file_path, references=references, description=description, impact=impact, mitigation=mitigation, component_name=package_name, component_version=package_version, cvssv3=cvssv3, cvssv3_score=cvssv3_score, static_finding=True, dynamic_finding=False, fix_available=fix_available, service=service_name, **status_fields)
            ...
            for misconfiguration in misconfigurations:
                ...
                description = MISC_DESCRIPTION_TEMPLATE.format(target=target_target, type=misc_type, description=misc_description, message=misc_message)
...
'''
                ...
                if references:
                    ...
                    references = '\n'.join(misc_references)
                finding = Finding(test=test, title=title, severity=severity, references=references, description=description, mitigation=misc_resolution, fix_available=True, static_finding=True, dynamic_finding=False, service=service_name)
            ...
            for secret in secrets:
                ...
                description = SECRET_DESCRIPTION_TEMPLATE.format(title=secret_title, category=secret_category, match=secret_match)
                severity = TRIVY_SEVERITIES[secret_severity]
                finding = Finding(test=test, title=title, severity=severity, description=description, file_path=target_target, line=secret_start_line, static_finding=True, dynamic_finding=False, fix_available=True, service=service_name)
            ...
            for lic in licenses:
                ...
                description = LICENSE_DESCRIPTION_TEMPLATE.format(title=license_name, category=license_category, package=license_pkgname)
                severity = TRIVY_SEVERITIES[license_severity]
                finding = Finding(test=test, title=title, severity=severity, description=description, file_path=license_filepath, scanner_confidence=license_confidence, url=license_link, static_finding=True, dynamic_finding=False, fix_available=True, service=service_name)
                finding.unsaved_tags = [target_class]
                items.append(finding)
        return items
