    ...
    def __parse_disa(self, test, items, sheet):
        ...
        for row in sheet.iter_rows(min_row=1, values_only=True):
            if first:
                ...
                tags = 'disa'
                finding = Finding(title=title, date=date, severity=severity, description=description, impact=impact, references=references, test=test, unique_id_from_tool=unique_id, static_finding=True, dynamic_finding=False)
    ...
    def __parse_oval(self, test, items, sheet):
        ...
        for row in sheet.iter_rows(min_row=1, values_only=True):
            if first:
                ...
                tags = 'oval'
                finding = Finding(title=title, severity=severity, unique_id_from_tool=unique_id, test=test, static_finding=True, dynamic_finding=False)
                if row[headers['ref']]:
    ...
    def __parse_twistlock(self, test, items, sheet):
        ...
        for row in sheet.iter_rows(min_row=1, values_only=True):
            if first:
                ...
                tags = 'twistlock'
                finding = Finding(title=title, url=url, severity=severity, description=description, component_name=component_name, component_version=component_version, severity_justification=severity_justification, test=test, static_finding=True, dynamic_finding=False)
                if row[headers['cve']]:
    ...
    def __parse_anchore_compliance(self, test, items, sheet):
        ...
        for row in sheet.iter_rows(min_row=1, values_only=True):
            if first:
                ...
                finding = Finding(title=title, severity=severity, mitigation=mitigation, description=description, test=test, static_finding=True, dynamic_finding=False)
                finding.unsaved_tags = tags
                items.append(finding)
