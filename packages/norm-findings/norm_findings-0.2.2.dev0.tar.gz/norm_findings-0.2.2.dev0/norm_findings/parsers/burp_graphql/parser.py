...
class BurpGraphQLParser():
    ...
    def create_findings(self, scan_data, test):
        ...
        for issue in finding_data:
            find = Finding(title=issue.get('Title'), description=issue.get('Description'), test=test, severity=issue.get('Severity'), mitigation=issue.get('Mitigation'), references=issue.get('References'), impact=issue.get('Impact'), cwe=int(issue.get('CWE')), false_p=False, duplicate=False, out_of_scope=False, static_finding=False, dynamic_finding=True, nb_occurences=1)
    ...
    def parse_evidence(self, evidence):
        ...
        while (i < evidence_len):
            ...
            if (((i + 1) < evidence_len) and evidence[(i + 1)].get('response_segments') and (evidence[(i + 1)].get('response_index') == request_dict.get('request_index'))):
                response = ''
