...
class SnykIssueApiParser():
    ...
    def extract_if_fix_is_available(self, finding_type, coordinates):
        ...
        for coordinate in coordinates:
            if (finding_type == 'code'):
                if (coordinate.get('is_fixable_snyk') or coordinate.get('is_fixable_upstream') or coordinate.get('is_fixable_manually')):
                    return True
            if (finding_type == 'package_vulnerability'):
                if (coordinate.get('is_fixable_snyk') or coordinate.get('is_fixable_upstream') or coordinate.get('is_fixable_manually') or coordinate.get('is_patchable') or coordinate.get('is_pinnable') or coordinate.get('is_upgradeable')):
                    return True
        return False
        ...
        for coordinate in coordinates:
            ...
            for representation in coordinate.get('representations', []):
                if (not is_type_code):
                    ...
                    impact_locations.append(['Source Location:', f"File: {location.get('file', 'Unknown')}", f"Commit: {location.get('commit_id', 'Unknown')}", f"Lines: {start.get('line', '?')}-{end.get('line', '?')}", f"Columns: {start.get('column', '?')}-{end.get('column', '?')}", ''])
                    if (not file_path):
    ...
    def extract_problems(self, problems):
        if problems:
            problem = problems[0]
            return [f"id: {problem.get('id', 'Unknown')}", f"Source: {problem.get('source', 'Unknown')}", f"Type: {problem.get('type', 'Unknown')}", (f"URL: {problem.get('url', 'Unknown')}" if problem.get('url') else ''), f"Last Updated: {problem.get('updated_at', 'Unknown')}", '']
        return None
    ...
    def extract_risk_score(self, risk):
        if (risk and ('score' in risk)):
            ...
            if isinstance(score, dict):
                return f"Risk Score: {score.get('value', 'N/A')} (Model: {score.get('model', 'N/A')})"
        return None
    ...
    def get_finding(self, issue, test):
        ...
        if component_name:
            impact_details.extend(['Package Details:', f'Package: {component_name}', f"Version: {(component_version or 'Unknown')}", ''])
        impact_details.append(f"Reachable: {('Yes' if reachable else 'No')}")
        finding = Finding(title=attributes.get('title', ''), test=test, severity=severity, description=attributes.get('description', ''), static_finding=True, dynamic_finding=False, unique_id_from_tool=issue.get('id'), file_path=file_path, line=line, out_of_scope=is_out_of_scope, active=((attributes.get('status') == 'open') and (not attributes.get('ignored', False))), verified=(attributes.get('ignored', True) or (attributes.get('status') == 'resolved')), false_p=attributes.get('ignored'), is_mitigated=(attributes.get('status') == 'resolved'), cwe=(cwes[0] if cwes else None), date=created, component_name=component_name, component_version=component_version, risk_accepted=False)
