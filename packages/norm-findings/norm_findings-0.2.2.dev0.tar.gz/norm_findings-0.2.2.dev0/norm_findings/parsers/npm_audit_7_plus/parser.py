    ...
    def parse_json(self, json_output):
        ...
        if tree.get('audit'):
            if (not tree.get('audit').get('auditReportVersion')):
                msg = 'This parser only supports output from npm audit version 7 and above.'
                raise ValueError(msg)
...
def get_item(item_node, tree, test):
    ...
    if (item_node['via'] and isinstance(item_node['via'][0], dict) and (len(item_node['via']) > 1)):
        references.extend((vuln['url'] for vuln in item_node['via'][1:] if isinstance(vuln, dict)))
    dojo_finding = Finding(title=title, test=test, severity=severity, description=description, mitigation=mitigation, references=', '.join(references), component_name=component_name, false_p=False, duplicate=False, out_of_scope=False, mitigated=None, impact='No impact provided', static_finding=static_finding, dynamic_finding=False, vuln_id_from_tool=unique_id_from_tool)
    if (cwe is not None):
        cwe = int(cwe.split('-')[1])
...
def get_vuln_description(item_node, tree):
    ...
    effects_handled = []
    description = ''
    description += (((item_node['name'] + ' ') + item_node['range']) + '\n')
    description += (('Severity: ' + item_node['severity']) + '\n')
    for via in item_node['via']:
        if isinstance(via, str):
            description += (('Depends on vulnerable versions of ' + via) + '\n')
        else:
