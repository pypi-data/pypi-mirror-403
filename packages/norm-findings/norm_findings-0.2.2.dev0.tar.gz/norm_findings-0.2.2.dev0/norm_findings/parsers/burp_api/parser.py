...
class BurpApiParser():
    ...
    def get_findings(self, scan_file, test):
        ...
        for issue_event in tree.get('issue_events', []):
            if ((issue_event.get('type') == 'issue_found') and ('issue' in issue_event)):
                issue = issue_event.get('issue')
                title = issue.get('name', 'Burp issue')
                severity = convert_severity(issue)
                description_formated = DESCRIPTION_TEMPLATE.format(title=title, serial_number=issue.get('serial_number', '<None>'), type_index=issue.get('type_index', '<None>'), confidence=issue.get('confidence', '<None>'), description_text=issue.get('description', '<None>'))
                false_p = False
                if (issue.get('severity', 'undefined') == 'false_positive'):
                    false_p = True
                finding = Finding(title=title, severity=severity, description=description_formated, mitigation='No mitigation provided', references='No references provided', false_p=false_p, impact='No impact provided', static_finding=False, dynamic_finding=True, unique_id_from_tool=str(issue.get('serial_number', '')), vuln_id_from_tool=str(issue.get('type_index', '')))
                ...
                for evidence in issue.get('evidence', []):
                    if (evidence.get('type') not in {'InformationListEvidence', 'FirstOrderEvidence'}):
                        continue
