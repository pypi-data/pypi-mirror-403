            ...
            if ((errors + warns) > 0):
                for issue in report['messages']:
                    ...
                    if (dupe_key in dupes):
                        ...
                        dupes[dupe_key] = True
                        find = Finding(title=title, test=test, description=findingdetail, severity=sev.title(), file_path=filepath, line=issue['line'], static_finding=True, dynamic_finding=False)
                        dupes[dupe_key] = find
                        findingdetail = ''
