...
class ApiBlackduckParser():
    ...
    def get_findings(self, scan_file, test):
        data = (BlackduckApiImporter().get_findings(test) if (scan_file is None) else json.load(scan_file))
        findings = []
        for entry in data:
            ...
            finding = Finding(test=test, title=f'{vulnerability_id} in {component_name}:{component_version}', description=entry['vulnerabilityWithRemediation'].get('description'), severity=entry['vulnerabilityWithRemediation']['severity'].title(), component_name=component_name, component_version=component_version, static_finding=True, dynamic_finding=False, unique_id_from_tool=entry['vulnerabilityWithRemediation'].get('vulnerabilityName'))
