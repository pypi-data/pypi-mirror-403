...
class VeracodeScaParser():
    ...
    def get_findings(self, scan_file, test):
        ...
        return self.get_findings_csv(scan_file, test)
    ...
    def _get_findings_json(self, file, test):
        ...
        for issue in embedded.get('issues', []):
            ...
            if (vuln_id and (not vuln_id.startswith(('cve', 'CVE')))):
                vuln_id = ('CVE-' + vuln_id)
            ...
            description = 'Project name: {}\nTitle: \n>{}\n\n-----\n\n'.format(issue.get('project_name'), vulnerability.get('title'))
            finding = Finding(test=test, title=f'{component_name}:{component_version} | {vuln_id}', description=description, severity=severity, component_name=component_name, component_version=component_version, static_finding=True, dynamic_finding=False, unique_id_from_tool=issue.get('id'), date=date, nb_occurences=1)
            if vuln_id:
            ...
            if ((issue.get('Ignored') and (issue.get('Ignored').capitalize() == 'True')) or (status and ((status.capitalize() == 'Resolved') or (status.capitalize() == 'Fixed')))):
    ...
    def get_findings_csv(self, file, test):
        ...
        for row in csvarray:
            ...
            finding = Finding(test=test, title=f'{library}:{version} | {vuln_id}', description=description, severity=severity, component_name=library, component_version=version, static_finding=True, dynamic_finding=False, unique_id_from_tool=issueId, date=date, nb_occurences=1)
            finding.unsaved_vulnerability_ids = [vuln_id]
            if cvss_score:
                finding.cvssv3_score = cvss_score
            if ((row.get('Ignored') and (row.get('Ignored').capitalize() == 'True')) or (row.get('Status') and (row.get('Status').capitalize() == 'Resolved'))):
                finding.is_mitigated = True
                finding.mitigated = timezone.now()
                finding.active = False
            findings.append(finding)
