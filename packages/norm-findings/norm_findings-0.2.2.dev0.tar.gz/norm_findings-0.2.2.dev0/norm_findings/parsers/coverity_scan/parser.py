...
class CoverityScanParser():
    ...
    SEVERITY_MAPPING = {'Audit': 'Info', 'Low': 'Low', 'Medium': 'Medium', 'High': 'High'}
...
{event_description}'''
            ...
            finding = Finding(test=test, title=checker_properties.get('subcategoryShortDescription'), severity=self.SEVERITY_MAPPING[checker_properties.get('impact')], description=description, file_path=issue.get('strippedMainEventFilePathname'), line=issue.get('mainEventLineNumber'), static_finding=True, dynamic_finding=False, nb_occurences=issue.get('occurrenceCountForMK'), cwe=int(checker_properties.get('cweCategory')), mitigation=mitigation, impact=checker_properties.get('subcategoryLocalEffect'), vuln_id_from_tool=vuln_id)
