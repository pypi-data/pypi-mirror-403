...
class CycognitoParser():
    ...
    def get_findings(self, scan_file, test):
        ...
        for vulnerability in data:
            ...
            finding = Finding(title=title, test=test, description=description, severity=base_severity.capitalize(), references=str('\n'.join(references)), date=datetime.strptime(first_detected, '%Y-%m-%dT%H:%M:%S.%fZ').strftime('%Y-%m-%d'), dynamic_finding=True, mitigation=mitigation, impact=impact)
