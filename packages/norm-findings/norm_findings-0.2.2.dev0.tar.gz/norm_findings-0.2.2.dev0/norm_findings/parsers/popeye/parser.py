                ...
                for (issue_group, issue_list) in issues.items():
                    for issue in issue_list:
                        if (issue['level'] != 0):
                            ...
                            vuln_id_from_tool = re.search('\\[(POP-\\d+)\\].+', issue['message']).group(1)
                            finding = Finding(title=title, test=test, description=description, severity=severity, static_finding=False, dynamic_finding=True, vuln_id_from_tool=vuln_id_from_tool)
