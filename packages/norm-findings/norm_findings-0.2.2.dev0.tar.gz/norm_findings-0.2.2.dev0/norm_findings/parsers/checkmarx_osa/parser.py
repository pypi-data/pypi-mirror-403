...
class CheckmarxOsaParser():
    ...
    def get_findings(self, scan_file, test):
        ...
        for item in vulnerabilities:
            ...
            finding_item = Finding(title='{} {} | {}'.format(library['name'], library['version'], vulnerability_id), severity=item['severity']['name'], description=item.get('description', 'NC'), unique_id_from_tool=item.get('id', None), references=item.get('url', None), mitigation=item.get('recommendations', None), component_name=library['name'], component_version=library['version'], cwe=1035, cvssv3_score=item.get('score', None), publish_date=(datetime.strptime(item['publishDate'], '%Y-%m-%dT%H:%M:%S') if ('publishDate' in item) else None), static_finding=True, dynamic_finding=False, scanner_confidence=(self.checkmarx_confidence_to_defectdojo_confidence(library['confidenceLevel']) if ('confidenceLevel' in library) else None), active=(status != 'NOT_EXPLOITABLE'), false_p=(status == 'NOT_EXPLOITABLE'), verified=(status not in {'TO_VERIFY', 'NOT_EXPLOITABLE', 'PROPOSED_NOT_EXPLOITABLE'}), test=test)
