...
class SysdigReportsParser():
    ...
    def get_findings(self, scan_file, test):
        ...
        if scan_file.name.lower().endswith('.csv'):
            arr_data = self.load_csv(filename=scan_file)
    ...
    def parse_json(self, data, test):
        ...
        for item in vulnerability:
            ...
            packageSuggestedFix = (item.get('packageSuggestedFix', '') or '')
            k8sPodCount = (item.get('k8sPodCount', '') or '')
            description = ''
            ...
            find = Finding(title=((vulnName + '_') + vulnFixVersion), test=test, description=description, severity=vulnSeverity, mitigation=mitigation, static_finding=True, references=vulnLink, component_name=packageName, component_version=packageVersion)
            if vulnName:
    ...
    def load_csv(self, filename) -> SysdigData:
        ...
        for row in csvarray:
            ...
            if ('vulnerability id' in reader.fieldnames):
                ...
                csv_data_record.image_id = row.get('image id', '')
                csv_data_record.k8s_pod_count = ''
                csv_data_record.package_suggested_fix = ''
                csv_data_record.in_use = (row.get('package in use', '').lower() == 'true')
                csv_data_record.risk_accepted = (row.get('risk accepted', '').lower() == 'true')
                csv_data_record.registry_name = ''
                csv_data_record.registry_image_repository = ''
                csv_data_record.cloud_provider_name = ''
                csv_data_record.cloud_provider_account_id = ''
                csv_data_record.cloud_provider_region = ''
                csv_data_record.registry_vendor = ''
