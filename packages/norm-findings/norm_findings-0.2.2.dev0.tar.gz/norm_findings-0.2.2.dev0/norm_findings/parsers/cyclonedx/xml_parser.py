...
from norm_findings.parsers.cyclonedx.helpers import Cyclonedxhelper
...
class CycloneDXXMLParser():
    ...
    def _get_findings_xml(self, file, test):
        ...
        ns = {'b': namespace.replace('{', '').replace('}', ''), 'v': 'http://cyclonedx.org/schema/ext/vulnerability/1.0'}
        report_date = None
        ...
        for component in root.findall('b:components/b:component', namespaces=ns):
            ...
            if ('bom-ref' in component.attrib):
                bom_refs[component.attrib['bom-ref']] = {'name': component_name, 'version': component_version}
            for vulnerability in component.findall('v:vulnerabilities/v:vulnerability', namespaces=ns):
                finding_vuln = self.manage_vulnerability_legacy(vulnerability, ns, bom_refs, report_date=report_date, component_name=component_name, component_version=component_version)
    ...
    def manage_vulnerability_legacy(self, vulnerability, ns, bom_refs, report_date, component_name=None, component_version=None):
        ...
        if (not description):
            description = '\n'.join([f'**Ref:** {ref}', f'**Id:** {vuln_id}', f'**Severity:** {severity}'])
...
'''
        finding = Finding(title=f'{component_name}:{component_version} | {vuln_id}', description=description, severity=severity, references=references, component_name=component_name, component_version=component_version, vuln_id_from_tool=vuln_id, nb_occurences=1)
        if report_date:
...
'''
    ...
    def get_cwes(self, node, prefix, namespaces):
        return [int(cwe.text) for cwe in node.findall((((prefix + ':cwes/') + prefix) + ':cwe'), namespaces) if cwe.text.isdigit()]

    def _manage_vulnerability_xml(self, vulnerability, ns, bom_refs, report_date, component_name=None, component_version=None):
...
{detail}'''
        ...
        severity = Cyclonedxhelper().fix_severity(severity)
        if (not description):
            description = '\n'.join([f'**Id:** {vuln_id}', f'**Severity:** {severity}'])
        references = ''
...
'''
        ...
        for target in vulnerability.findall('b:affects/b:target', namespaces=ns):
            ...
            (component_name, component_version) = Cyclonedxhelper()._get_component(bom_refs, ref.text)
            finding = Finding(title=f'{component_name}:{component_version} | {vuln_id}', description=description, severity=severity, mitigation=vulnerability.findtext('b:recommendation', namespaces=ns), references=references, component_name=component_name, component_version=component_version, static_finding=True, dynamic_finding=False, vuln_id_from_tool=vuln_id, nb_occurences=1)
            if vulnerability_ids:
                finding.unsaved_vulnerability_ids = vulnerability_ids
            if report_date:
                finding.date = report_date
