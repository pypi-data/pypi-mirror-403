...
class KiuwanSCAParser():
    SEVERITY = {'-': 'Low', 'LOW': 'Low', 'MEDIUM': 'Medium', 'HIGH': 'High', 'CRITICAL': 'Critical', 'Low': 'Low', 'Medium': 'Medium', 'High': 'High', 'Critical': 'Critical'}
    ...
    def get_findings(self, scan_file, test):
        ...
        for row in data:
            ...
            for component in components:
                ...
                key = hashlib.sha256(((((((((finding.description + '|') + finding.severity) + '|') + finding.component_name) + '|') + finding.component_version) + '|') + str((finding.cwe or ''))).encode('utf-8')).hexdigest()
                if (key not in dupes):
                    dupes[key] = finding
