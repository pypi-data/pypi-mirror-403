...
def get_item(vulnerability, service, test, artifact_name, artifact_version, artifact_sha256):
    ...
    finding = Finding(vuln_id_from_tool=vuln_id_from_tool, service=service, title=vulnerability['summary'], cwe=cwe, cvssv3=cvssv3, severity=severity, description=((((impact_path.name + ':') + impact_path.version) + ' -> ') + vulnerability['description']), test=test, file_path=impact_paths[0], component_name=artifact_name, component_version=artifact_version, static_finding=True, dynamic_finding=False, unique_id_from_tool=unique_id_from_tool)
...
def decode_service(name):
    match = re.match('.*/(.*):', name, re.IGNORECASE)
    if (match is None):
        return ''
    return match[1]

def decode_cwe_number(value):
    match = re.match('CWE-\\d+', value, re.IGNORECASE)
    if (match is None):
        return 0
...
def decode_artifact(artifact_general):
    ...
    match = re.match('(.*):(.*)', artifact_general['name'], re.IGNORECASE)
    if match:
...
def decode_impact_path(path):
    impact_path = ImpactPath('', '', '')
    match = re.match('.*\\/(.*)$', str(path), re.IGNORECASE)
    if (match is None):
        return impact_path
    fullname = match[1]
    match = re.match('.*sha256__(.*).tar', path, re.IGNORECASE)
    if match:
        impact_path.sha = (match[1][:64] if (len(match[1]) > 64) else match[1])
    if ('.jar' in fullname):
        match = re.match('(.*)-', fullname, re.IGNORECASE)
        if match:
            impact_path.name = match[1]
        match = re.match('.*-(.*).jar', fullname, re.IGNORECASE)
        if match:
        ...
        if match:
            impact_path.name = match[1]
        match = re.match('.*-(.*).js', fullname, re.IGNORECASE)
        if match:
            impact_path.version = match[1]
    else:
