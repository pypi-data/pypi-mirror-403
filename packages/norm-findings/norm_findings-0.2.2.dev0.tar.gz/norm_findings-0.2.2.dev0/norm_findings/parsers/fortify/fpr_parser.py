...
from norm_findings.parsers.fortify.fortify_data import DescriptionData, RuleData, SnippetData, VulnerabilityData
...
class FortifyFPRParser():
    ...
    def identify_root(self, zip_data: dict, filename_suffix: str, msg_if_not_found: typing.Union[(str, None)]=None) -> tuple[(Element, dict[(str, str)])]:
    ...
    def get_source_location(self, analysis_info: Element) -> typing.Union[(Element, None)]:
        'Return the SourceLocation element if we are able to reach it.'
        key_path = ['Unified', 'Trace', 'Primary', 'Entry', 'Node', 'SourceLocation']
        current_element = analysis_info
        for key in key_path[:(- 3)]:
