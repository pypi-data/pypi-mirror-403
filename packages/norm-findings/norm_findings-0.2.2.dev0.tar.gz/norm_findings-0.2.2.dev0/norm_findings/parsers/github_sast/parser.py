    ...
    def get_findings(self, scan_file, test):
        ...
        for vuln in data:
            ...
            if (owner and repo and commit_sha and loc.get('path') and loc.get('start_line')):
                file_link = f"{parsed.scheme}://{parsed.netloc}/{owner}/{repo}/blob/{commit_sha}/{loc['path']}#L{loc['start_line']}"
            ...
            description = '\n'.join(desc_lines)
            finding = Finding(title=title, test=test, description=description, severity=severity, active=active, static_finding=True, dynamic_finding=False, vuln_id_from_tool=rule_id)
