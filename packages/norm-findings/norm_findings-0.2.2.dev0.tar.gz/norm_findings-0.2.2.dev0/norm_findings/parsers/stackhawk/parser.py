...
import json
from norm_findings.stubs.django.utils.dateparse import parse_datetime
...
class StackHawkParser():
    ...
    def get_findings(self, scan_file, test):
        completed_scan = self.__parse_json(scan_file)
    ...
    def __extract_finding(self, raw_finding, metadata: StackHawkScanMetadata, test) -> Finding:
        ...
        for path in paths:
            steps_to_reproduce += (((((('**' + path['path']) + '**') + self.__endpoint_status(path['status'])) + '\n') + self.__hyperlink(path['pathURL'])) + '\n')
    ...
    def __parse_json(json_output):
        ...
        if (('scanCompleted' not in report) or ('service' not in report) or (report['service'] != 'StackHawk')):
            msg = ' Unexpected JSON format provided. Need help? Check out the StackHawk Docs at https://docs.stackhawk.com/workflow-integrations/defect-dojo.html'
