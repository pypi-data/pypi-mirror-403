    ...
    def ingest_findings(self, sorted_findings, test):
        ...
        for i in sorted_findings:
            ...
            if str(i.cvss_vector_v3):
                cvss_vectors = '{}{}'.format('CVSS:3.1/', i.cvss_vector_v3)
            ...
            unique_finding_key = hashlib.sha256(f'{((file_path + object_sha1) + title)}'.encode()).hexdigest()
            if (unique_finding_key in findings):
                finding = findings[unique_finding_key]
                finding.nb_occurences += 1
            else:
                finding = Finding(title=title, test=test, cvssv3_score=cvss_score, severity=severity, description=description, mitigation=mitigation, impact=impact, static_finding=True, dynamic_finding=True, cwe=int(cwe), nb_occurences=1, references=references, file_path=file_path, url=i.vulnerability_url, vuln_id_from_tool=str(cve), severity_justification=cvss_vectors, component_name=i.component, component_version=i.version, unique_id_from_tool=unique_finding_key)
                if cvss_v3:
