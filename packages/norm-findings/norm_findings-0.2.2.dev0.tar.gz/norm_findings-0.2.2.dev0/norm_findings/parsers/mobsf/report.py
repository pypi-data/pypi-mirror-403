...
class MobSFjsonreport():
    ...
    def get_findings(self, scan_file, test):
        dupes = {}
        for (key, item) in scan_file.get('results').items():
            metadata = item.get('metadata')
            cwe = int(re.match('(cwe|CWE)-([0-9]+)', metadata.get('cwe')).group(2))
            masvs = metadata.get('masvs')
            owasp_mobile = metadata.get('owasp-mobile')
            description = '\n'.join([f"**Description:** `{metadata.get('description')}`", f'**OWASP MASVS:** `{masvs}`', f'**OWASP Mobile:** `{owasp_mobile}`'])
            ...
            for (file_path, line, snippet) in files:
                finding = Finding(title=f'{key}', test=test, severity=severity, nb_occurences=1, cwe=cwe, description=description, references=references)
                if file_path:
                    finding.file_path = file_path
                    finding.line = line
                    finding.description = f'''{description}
**Snippet:** `{snippet}`'''
                dupe_key = hashlib.sha256(((((key + str(cwe)) + masvs) + owasp_mobile) + file_path).encode('utf-8')).hexdigest()
