...
class NancyParser():
    ...
    def get_description_for_scan_types(self, scan_type):
        return 'Nancy output file (go list -json -deps ./... | nancy sleuth >  nancy.json) can be imported in JSON format.'
        ...
        for vuln in vulnerable:
            ...
            if vuln['Vulnerabilities']:
                ...
                for associated_vuln in vuln['Vulnerabilities']:
                    ...
                    finding = Finding(title=associated_vuln['Title'], description=associated_vuln['Description'], test=test, severity=self.convert_cvss_score(associated_vuln['CvssScore']), component_name=comp_name, component_version=comp_version, false_p=False, duplicate=False, out_of_scope=False, static_finding=True, dynamic_finding=False, vuln_id_from_tool=associated_vuln.get('Id', associated_vuln.get('ID')), references='\n'.join(references))
                    finding.unsaved_vulnerability_ids = vulnerability_ids
                    cvss_vector = associated_vuln['CvssVector']
                    if (cvss_vector and cvss_vector.startswith('CVSS:3.')):
                        finding.cvssv3 = CVSS3(associated_vuln['CvssVector']).clean_vector()
                    elif (cvss_vector and cvss_vector.startswith('CVSS:4.')):
                        finding.cvssv4 = CVSS4(associated_vuln['CvssVector']).clean_vector()
                    if associated_vuln['Title'].startswith('CWE-'):
                        cwe = associated_vuln['Title'].split(':')[0].split('-')[1]
                        finding.cwe = int(cwe)
