...
def get_item(item_node, test):
    ...
    for npm_finding in item_node['findings']:
        component_version = (component_version or npm_finding['version'])
    ...
    try:
        ...
        filepath = ''
    dojo_finding = Finding(title=((((((item_node['title'] + ' - ') + '(') + item_node['module_name']) + ', ') + item_node['vulnerable_versions']) + ')'), test=test, severity=severity, file_path=filepath, description=((((((((((((((item_node['url'] + '\n') + item_node['overview']) + '\n Vulnerable Module: ') + item_node['module_name']) + '\n Vulnerable Versions: ') + str(item_node['vulnerable_versions'])) + '\n Patched Version: ') + str(item_node['patched_versions'])) + '\n Vulnerable Paths: ') + str(paths)) + '\n CWE: ') + str(item_node['cwe'])) + '\n Access: ') + str(item_node['access'])), cwe=cwe, mitigation=item_node['recommendation'], references=item_node['url'], component_name=item_node['module_name'], component_version=component_version, false_p=False, duplicate=False, out_of_scope=False, mitigated=None, impact='No impact provided', static_finding=True, dynamic_finding=False)
