...
def get_item(vulnerability, test):
    ...
    finding = Finding(title=title, test=test, severity=severity, description=((vulnerability.get('description', vulnerability.get('summary')) + '\n\n') + extra_desc).strip(), mitigation=mitigation, component_name=component_name, component_version=component_version, file_path=vulnerability['path'], severity_justification=f'''CVSS v3 base score: {cvss_v3}
CVSS v2 base score: {cvss_v2}''', static_finding=True, dynamic_finding=False, references=references, impact=severity, date=scan_time, unique_id_from_tool=vulnerability['issue_id'], tags=tags, fix_available=fix_available)
