...
class OrtParser():
    ...
    def get_findings(self, scan_file, test):
        if (scan_file is None):
            return []
        evaluated_model = self.parse_json(scan_file)
        if evaluated_model:
    ...
    def get_items(self, evaluated_model, test):
        ...
        rule_violations_models = get_rule_violation_models(rule_violations_unresolved, packages, licenses, dependency_trees)
        for model in rule_violations_models:
            item = get_item(model, test)
            unique_key = hashlib.md5((item.title + item.references).encode(), usedforsecurity=False).hexdigest()
...
def get_rule_violation_models(rule_violations_unresolved, packages, licenses, dependency_trees):
    return [get_rule_violation_model(violation, packages, licenses, dependency_trees) for violation in rule_violations_unresolved]
...
how to fix : {model.rule_violation['how_to_fix']}'''
    ...
    return Finding(title=model.rule_violation['rule'], test=test, references=model.rule_violation['message'], description=desc, severity=severity, static_finding=True)
