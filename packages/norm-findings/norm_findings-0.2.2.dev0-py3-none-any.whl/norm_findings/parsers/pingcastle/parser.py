...
class PingCastleParser():
    CVE_REGEX = re.compile('(CVE-\\d{4}-\\d{4,7})', re.IGNORECASE)
    ...
    def get_findings(self, scan_file, test):
        ...
        for rr in root.findall('RiskRules/HealthcheckRiskRule'):
            ...
            severity = self._apply_contextual_bump(severity=severity, category=category, model=model, risk_id=risk_id, rationale=rationale)
            if ((not severity) or (severity not in self._SEVERITY_ORDER)):
                severity = 'Info'
            title = f'[PingCastle] {risk_id} ({category}/{model})'
            description = self._compose_risk_rule_description(domain_fqdn=domain_fqdn, risk_id=risk_id, points=points, category=category, model=model, rationale=rationale, dc_infos=dc_infos, root=root)
            finding = Finding(title=title, test=test, description=description, severity=severity, mitigation='Review and remediate according to PingCastle recommendations.', impact='Risk identified by PingCastle HealthCheck.', vuln_id_from_tool=risk_id)
            if report_date:
    ...
    def _compose_risk_rule_description(self, domain_fqdn, risk_id, points, category, model, rationale, dc_infos, root):
        lines = []
        lines.append('### PingCastle Risk Rule')
        ...
        if (risk_id.startswith('A-DC-') or ('DomainControllers' in root.tag)):
            if dc_infos:
                ...
                for dc in dc_infos:
                    ips = ', '.join(dc.get('ips', []))
                    lines.append(f"- **{dc['name']}** (OS: {dc.get('os', '?')}, IPs: {ips}, SpoolerRemote: {dc.get('remote_spooler', 'false')})")
        return '\n'.join(lines)
    ...
    def _collect_domain_controllers(self, root):
        ...
        for dc in root.findall('DomainControllers/HealthcheckDomainController'):
            ...
            dc_info = {'name': name, 'os': os, 'remote_spooler': (remote_spooler.lower() == 'true'), 'ips': ips, 'rpc_interfaces': []}
            for rpc in dc.findall('RPCInterfacesOpen/HealthcheckDCRPCInterface'):
                dc_info['rpc_interfaces'].append({'ip': rpc.attrib.get('IP', ''), 'interface': rpc.attrib.get('Interface', ''), 'opnum': rpc.attrib.get('OpNum', ''), 'function': rpc.attrib.get('Function', '')})
...
'''
                for ri in dc['rpc_interfaces']:
                    finding.description += f'''- IP: `{ri['ip']}` | Interface: `{ri['interface']}` | OpNum: `{ri['opnum']}` | Function: `{ri['function']}`
'''
...
'''
    ...
    def _is_dc_specific_risk(risk_id: str, model: str='', rationale: str='') -> bool:
        ...
        if rid.startswith(dc_prefixes):
            return True
        dc_specific_ids = {'A-DC-Spooler', 'A-DC-Coerce', 'A-AuditDC', 'S-DC-SubnetMissing'}
    ...
    def _apply_contextual_bump(self, severity: str, category: str='', model: str='', risk_id: str='', rationale: str='') -> str:
