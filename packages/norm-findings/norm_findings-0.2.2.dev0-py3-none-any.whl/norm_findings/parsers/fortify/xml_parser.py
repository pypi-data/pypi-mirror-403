...
class FortifyXMLParser():
    ...
    def xml_structure_24_2(self, root, test):
        ...
        for issues in root.findall('Issues'):
            for issue in issues.iter('Issue'):
                ...
                finding = Finding(title=name, severity=self.severity_translator(severity=int(severity)), static_finding=True, test=test, description=description)
    ...
    def xml_structure_before_24_2(self, root, test):
            ...
            if (ReportSection.findtext('Title') in {'Results Outline', 'Issue Count by Category'}):
                ...
                for group in ReportSection.iter('GroupingSection'):
                    ...
                    if (maj_attr_summary is not None):
                        ...
                        meta_pair[place][title] = {x.findtext('Name'): x.findtext('Value') for x in meta_info}
        ...
        for issue in issues:
            issue.attrib['iid']
            details = {'Category': issue.find('Category').text, 'Folder': issue.find('Folder').text, 'Kingdom': issue.find('Kingdom').text, 'Abstract': issue.find('Abstract').text, 'Friority': issue.find('Friority').text, 'FileName': issue.find('Primary').find('FileName').text, 'FilePath': issue.find('Primary').find('FilePath').text, 'LineStart': issue.find('Primary').find('LineStart').text}
            ...
            if (issue.find('Source') is not None):
                source = {'FileName': issue.find('Source').find('FileName').text, 'FilePath': issue.find('Source').find('FilePath').text, 'LineStart': issue.find('Source').find('LineStart').text, 'Snippet': issue.find('Source').find('Snippet').text}
        ...
        for (issue_key, issue) in issue_map.items():
            ...
            if (title not in dupes):
                items.append(Finding(title=title, severity=issue['Friority'], file_path=issue['FilePath'], line=int(issue['LineStart']), static_finding=True, test=test, description=self.format_description(issue, cat_meta), mitigation=self.format_mitigation(issue, cat_meta), unique_id_from_tool=issue_key))
                dupes.add(title)
        return items
