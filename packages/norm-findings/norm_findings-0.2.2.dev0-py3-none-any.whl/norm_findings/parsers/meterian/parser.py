...
class MeterianParser():
    ...
    def get_findings(self, scan_file, test):
        findings = []
        report_json = json.load(scan_file)
    ...
    def do_get_findings(self, single_security_report, scan_date, test):
        ...
        for dependency_report in single_security_report['reports']:
            ...
            for advisory in dependency_report['advices']:
                ...
                finding = Finding(title=finding_title, date=scan_date, test=test, severity=severity, severity_justification=((((('Issue severity of: **' + severity) + '** from a base ') + 'CVSS score of: **') + str(advisory.get('cvss'))) + '**'), description=advisory['description'], component_name=lib_name, component_version=lib_ver, false_p=False, duplicate=False, out_of_scope=False, impact=severity, static_finding=True, dynamic_finding=False, file_path='Manifest file', unique_id_from_tool=advisory['id'])
    ...
    def get_severity(self, advisory):
        if ('cvss' in advisory):
            if (advisory['cvss'] <= 3.9):
                ...
                severity = 'Critical'
        elif ((advisory['severity'] == 'SUGGEST') or (advisory['severity'] == 'NA') or (advisory['severity'] == 'NONE')):
            severity = 'Info'
        else:
