...
class AppSpiderParser():
    ...
    def get_findings(self, scan_file, test):
        if (scan_file is None):
            return None
        vscan = ElementTree.parse(scan_file)
        root = vscan.getroot()
        if ('VulnSummary' not in str(root.tag)):
            msg = "Please ensure that you are uploading AppSpider's VulnerabilitiesSummary.xml file.At this time it is the only file that is consumable by DefectDojo."
        ...
        for finding in root.iter('Vuln'):
            ...
            if (dupe_key in dupes):
                ...
                find = Finding(title=title, test=test, description=html2text.html2text(description), severity=severity, mitigation=html2text.html2text(mitigation), impact='N/A', references=None, cwe=cwe)
