    ...
    def prepare_client(self, test):
        ...
        if test.api_scan_configuration:
            ...
            if (config.product != product):
                msg = f'API Scan Configuration for Vulners API and Product do not match. Product: "{product.name}" ({product.id}), config.product: "{config.product.name}" ({config.product.id})'
            ...
            if (configs.count() == 1):
                ...
                msg = f'There are no API Scan Configurations for this Product. Please add at least one API Scan Configuration for Vulners to this Product. Product: "{product.name}" ({product.id})'
                raise ValidationError(msg)
