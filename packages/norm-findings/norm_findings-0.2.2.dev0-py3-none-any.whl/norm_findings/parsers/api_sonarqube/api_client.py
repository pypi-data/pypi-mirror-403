...
from norm_findings.stubs.utils import prepare_for_view
...
class SonarQubeAPI():
    ...
    def find_project(self, project_name, organization=None, branch=None):
        ...
        response = self.session.get(url=f'{self.sonar_api_url}/components/search', params=parameters, headers=self.default_headers, timeout=settings.REQUESTS_TIMEOUT)
        if (not response.ok):
            msg = f"Unable to find the project {project_name} due to {response.status_code} - {response.content.decode('utf-8')}"
            raise Exception(msg)
...
'
    ...
    def get_project(self, project_key, organization=None, branch=None):
        ...
        parameters = {'component': project_key}
        if branch:
            parameters['branch'] = branch
        if organization:
            parameters['organization'] = organization
            ...
            parameters['organization'] = self.org_id
        response = self.session.get(url=f'{self.sonar_api_url}/components/show', params=parameters, headers=self.default_headers, timeout=settings.REQUESTS_TIMEOUT)
        if (not response.ok):
            msg = f"Unable to find the project {project_key} due to {response.status_code} - {response.content.decode('utf-8')}"
            raise Exception(msg)
    ...
    def find_issues(self, component_key, types='VULNERABILITY', organization=None, branch=None):
        ...
        while (page <= max_page):
            ...
            response = self.session.get(url=f'{self.sonar_api_url}/issues/search', params=request_filter, headers=self.default_headers, timeout=settings.REQUESTS_TIMEOUT)
            if (not response.ok):
                msg = f"Unable to find the issues for component {component_key} due to {response.status_code} - {response.content.decode('utf-8')}"
                raise Exception(msg)
    ...
    def find_hotspots(self, project_key, organization=None, branch=None):
        ...
        while (page <= max_page):
            ...
            if organization:
                ...
                request_filter['organization'] = self.org_id
            response = self.session.get(url=f'{self.sonar_api_url}/hotspots/search', params=request_filter, headers=self.default_headers, timeout=settings.REQUESTS_TIMEOUT)
            if (not response.ok):
                msg = f'Unable to find the hotspots for project {project_key} due to {response.status_code} - {response.content}'
                raise Exception(msg)
    ...
    def get_issue(self, issue_key):
        ...
        response = self.session.get(url=f'{self.sonar_api_url}/issues/search', params=request_filter, headers=self.default_headers, timeout=settings.REQUESTS_TIMEOUT)
        if (not response.ok):
            msg = f"Unable to get issue {issue_key} due to {response.status_code} - {response.content.decode('utf-8')}"
            raise Exception(msg)
        issues = response.json().get('issues', [])
        for issue in issues:
            if (issue['key'] == issue_key):
                return issue
        msg = f"""Expected Issue "{issue_key}", but it returned{[x.get('key') for x in response.json().get('issues')]}. Full response: {response.json()}"""
        raise Exception(msg)
    ...
    def get_rule(self, rule_id, organization=None):
        ...
        if (not rule):
            request_filter = {'key': rule_id}
            if organization:
                request_filter['organization'] = organization
            elif self.org_id:
                request_filter['organization'] = self.org_id
            response = self.session.get(url=f'{self.sonar_api_url}/rules/show', params=request_filter, headers=self.default_headers, timeout=settings.REQUESTS_TIMEOUT)
            if (not response.ok):
                msg = f"Unable to get the rule {rule_id} due to {response.status_code} - {response.content.decode('utf-8')}"
                raise Exception(msg)
    ...
    def get_hotspot_rule(self, rule_id):
        ...
        rule = self.rules_cache.get(rule_id)
        if (not rule):
            response = self.session.get(url=f'{self.sonar_api_url}/hotspots/show', params={'hotspot': rule_id}, headers=self.default_headers, timeout=settings.REQUESTS_TIMEOUT)
            if (not response.ok):
                msg = f'Unable to get the hotspot rule {rule_id} due to {response.status_code} - {response.content}'
                raise Exception(msg)
            rule = response.json()['rule']
            self.rules_cache.update({rule_id: rule})
        return rule
    ...
    def get_hotspot(self, rule_id):
        ...
        rule = self.rules_cache.get(rule_id)
        if (not rule):
            response = self.session.get(url=f'{self.sonar_api_url}/hotspots/search', params={'hotspots': rule_id}, headers=self.default_headers, timeout=settings.REQUESTS_TIMEOUT)
            if (not response.ok):
                msg = f'Unable to get the hotspot rule {rule_id} due to {response.status_code} - {response.content}'
                raise Exception(msg)
    ...
    def transition_issue(self, issue_key, transition):
        ...
        response = self.session.post(url=f'{self.sonar_api_url}/issues/do_transition', data={'issue': issue_key, 'transition': transition}, headers=self.default_headers, timeout=settings.REQUESTS_TIMEOUT)
        if (not response.ok):
            msg = f"Unable to transition {transition} the issue {issue_key} due to {response.status_code} - {response.content.decode('utf-8')}"
            raise Exception(msg)
    ...
    def transition_hotspot(self, issue_key, status, resolution=None):
        ...
        response = self.session.post(url=f'{self.sonar_api_url}/hotspots/change_status', data=data, headers=self.default_headers, timeout=settings.REQUESTS_TIMEOUT)
        if (not response.ok):
            msg = f"Unable to change status {status} / resolution {resolution} the issue {issue_key} due to {response.status_code} - {response.content.decode('utf-8')}"
            raise Exception(msg)
    ...
    def add_comment(self, issue_key, text):
        ...
        response = self.session.post(url=f'{self.sonar_api_url}/issues/add_comment', data={'issue': issue_key, 'text': text}, headers=self.default_headers, timeout=settings.REQUESTS_TIMEOUT)
        if (not response.ok):
            msg = f"Unable to add a comment into issue {issue_key} due to {response.status_code} - {response.content.decode('utf-8')}"
            raise Exception(msg)
    ...
    def test_connection(self):
        ...
        response = self.session.get(url=f'{self.sonar_api_url}/components/search', params=parameters, headers=self.default_headers, timeout=settings.REQUESTS_TIMEOUT)
        if (not response.ok):
            msg = f"Unable to connect and search in SonarQube due to {response.status_code} - {response.content.decode('utf-8')}"
            raise Exception(msg)
        try:
            num_projects = response.json()['paging']['total']
        except RequestsJSONDecodeError:
            msg = f''' Test request was successful (there was no HTTP-4xx or HTTP-5xx) but response doesn't contain
