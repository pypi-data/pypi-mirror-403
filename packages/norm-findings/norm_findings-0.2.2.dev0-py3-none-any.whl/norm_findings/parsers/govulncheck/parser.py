...
class GovulncheckParser():
    ...
    def get_location(data, node):
        ...
        return [f"{x['Pos']['Filename']}:{x['Pos']['Line']}:{x['Pos']['Column']}" for x in data['Calls']['Functions'][str(node)]['CallSites']]
    ...
    def get_findings(self, scan_file, test):
            ...
            if isinstance(data, dict):
                if data['Vulns']:
                    ...
                    for (cve, elems) in groupby(list_vulns, key=(lambda vuln: vuln['OSV']['aliases'][0])):
                        ...
                        d = {'cve': cve, 'severity': SEVERITY, 'title': first_elem[0]['OSV']['id'], 'component_name': first_elem[0]['OSV']['affected'][0]['package']['name'], 'component_version': self.get_version(data, first_elem[0]['RequireSink'])}
		...
		Introduced in {introduced}, fixed in {fixed}''')
                        ...
                        vuln_functions = ', '.join(set(osv_data['affected'][0]['ecosystem_specific']['imports'][0].get('symbols', [])))
                        description = f'''**Summary:** {summary}
**Vulnerable functions:** {vuln_functions}
**Affected Ecosystem:** {affected_ecosystem}
**Affected Versions:** {range_info}
**Vulnerable Package:** {affected_package['name']}
**Traces found :**
{self.get_finding_trace_info(data, osv_data['id'])}'''
                        ...
                        d = {'cve': cve, 'severity': severity, 'title': title, 'component_name': component_name, 'component_version': affected_version, 'description': description, 'impact': impact, 'references': references, 'file_path': path, 'url': db_specific_url, 'unique_id_from_tool': osv_id}
                        findings.append(Finding(**d))
            return findings
