...
class N0s1Parser():
    ...
    def get_tests(self, scan_type, handle):
        ...
        test = ParserTest(name=subscanner, parser_type=subscanner, version=data.get('tool', {}).get('version', ''), description=f'Scan from {subscanner}')
    ...
    def get_findings_from_data(self, data):
        ...
        for (finding_id, finding_data) in data.get('findings', {}).items():
            ...
            merged_regex = {'id': regex_id, 'description': regex_ref.get('description', regex_info.get('description', 'N/A')), 'regex': regex_ref.get('regex', regex_info.get('regex', 'N/A')), 'keywords': regex_info.get('keywords', []), 'tags': regex_info.get('tags', [])}
...
'''
            ...
            if (dupe_key in dupes):
                continue
            finding = Finding(title=title, description=description, severity='High', dynamic_finding=True, static_finding=False, unique_id_from_tool=dupe_key)
