    ...
    def get_findings(self, scan_file, test):
        ...
        if (data != {}):
            ...
            for cve in data['cves']:
                cvename = cve['name']
                description = [f'**CVE**: {cvename}', f"**Description**: {cve['description']}", f'**Banner**: {title}']
                severity = self.convert_cvss_score(raw_value=cve['cvssv2'])
                finding = Finding(title=((str(title) + '_') + str(cvename)), test=test, description='\n'.join(description), severity=severity, static_finding=False)
            ...
            for kex in data['kex']:
                if (('fail' in kex['notes']) and ('warn' in kex['notes'])):
                    ...
                    severity = 'High'
                    finding = Finding(title=((str(title) + '_') + str(kexname)), test=test, description='\n'.join(description), severity=severity, static_finding=False)
                    items.append(finding)
                    finding.unsaved_endpoints = []
                    endpoint = Endpoint(host=data['target'].split(':')[0], port=data['target'].split(':')[1])
                    finding.unsaved_endpoints.append(endpoint)
            ...
            for key in data['key']:
                if (('fail' in key['notes']) and ('warn' in key['notes'])):
                    ...
                    severity = 'High'
                    finding = Finding(title=((str(title) + '_') + str(keyname)), test=test, description='\n'.join(description), severity=severity, static_finding=False)
                    items.append(finding)
                    finding.unsaved_endpoints = []
                    endpoint = Endpoint(host=data['target'].split(':')[0], port=data['target'].split(':')[1])
                    finding.unsaved_endpoints.append(endpoint)
            ...
            for mac in data['mac']:
                if (('fail' in mac['notes']) and ('warn' in mac['notes'])):
                    ...
                    severity = 'High'
                    finding = Finding(title=((str(title) + '_') + str(macname)), test=test, description='\n'.join(description), severity=severity, static_finding=False)
                    items.append(finding)
                    finding.unsaved_endpoints = []
                    endpoint = Endpoint(host=data['target'].split(':')[0], port=data['target'].split(':')[1])
                    finding.unsaved_endpoints.append(endpoint)
