...
class TwistlockCSVParser():
    ...
    def parse_issue(self, row, test):
        ...
        finding = Finding(title=textwrap.shorten(title, width=255, placeholder='...'), test=test, date=finding_date, severity=convert_severity(data_severity), description=((((data_description + '\n\nVulnerable Package: ') + data_package_name) + '\n\nCurrent Version: ') + str(data_package_version)), mitigation=data_fix_status, component_name=textwrap.shorten(data_package_name, width=200, placeholder='...'), component_version=data_package_version, severity_justification=f'(CVSS v3 base score: {data_cvss})', impact=impact_text)
    ...
    def parse(self, filename, test):
        ...
        for row in reader:
            ...
            if (finding is not None):
                key = hashlib.md5(((((finding.severity + '|') + finding.title) + '|') + finding.description).encode('utf-8'), usedforsecurity=False).hexdigest()
                if (key not in dupes):
                    dupes[key] = finding
    ...
    def get_items(self, tree, test):
        ...
        if results:
            ...
            for node in vulnerabilityTree:
                ...
                unique_key = (node['id'] + str(((node['packageName'] + str(node['packageVersion'])) + str(node['severity']))))
            ...
            for node in complianceTree:
                ...
                if node.get('id'):
                    ...
                    unique_key = ('compliance_' + hashlib.md5((node.get('title', '') + node.get('description', '')).encode('utf-8'), usedforsecurity=False).hexdigest())
                items[unique_key] = item
...
def get_item(vulnerability, test, image_metadata=''):
    ...
    impact_text = '\n'.join(impact_parts)
    finding = Finding(title=((((vulnerability.get('id', 'Unknown Vulnerability') + ': ') + vulnerability.get('packageName', 'Unknown Package')) + ' - ') + str(vulnerability.get('packageVersion', ''))), test=test, severity=severity, description=((((vulnerability.get('description', '') + '\n\nVulnerable Package: ') + vulnerability.get('packageName', '')) + '\n\nCurrent Version: ') + str(vulnerability.get('packageVersion', ''))), mitigation=(status.title() if isinstance(status, str) else ''), references=vulnerability.get('link'), component_name=vulnerability.get('packageName', ''), component_version=vulnerability.get('packageVersion', ''), severity_justification=f'''Vector: {cvssv3} (CVSS v3 base score: {cvssv3_score})

{riskFactors}''', cvssv3=cvssv3, cvssv3_score=cvssv3_score, impact=impact_text, date=date)
...
def get_compliance_item(compliance, test, image_metadata=''):
    ...
    severity = (convert_severity(compliance['severity']) if ('severity' in compliance) else 'Info')
...
class TwistlockParser():
    ...
    def get_findings(self, scan_file, test):
        ...
        if scan_file.name.lower().endswith('.json'):
            return TwistlockJsonParser().parse(scan_file, test)
        if scan_file.name.lower().endswith('.csv'):
            return TwistlockCSVParser().parse(scan_file, test)
