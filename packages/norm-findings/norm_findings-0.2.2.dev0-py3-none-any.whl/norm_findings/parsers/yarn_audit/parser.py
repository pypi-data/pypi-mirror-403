...
from norm_findings.stubs.utils import get_npm_cwe
...
class YarnAuditParser():
    ...
    def get_items_yarn2(self, tree, test):
        ...
        for element in tree:
            ...
            dojo_finding = Finding(title=str(childid), test=test, severity=self.severitytranslator(severity=childseverity), description=description, component_version=str(child_tree_versions), false_p=False, duplicate=False, out_of_scope=False, mitigated=None, static_finding=True, dynamic_finding=False)
            items.append(dojo_finding)
            if (value is not None):
                dojo_finding.component_name = value
        return items

    def get_items_auditci(self, tree, test):
        items = []
        for element in tree.get('advisories'):
            ...
            dojo_finding = Finding(title=((tree.get('advisories').get(element).get('cves')[0] + '_') + tree.get('advisories').get(element).get('module_name')), test=test, severity=self.severitytranslator(severity=tree.get('advisories').get(element).get('severity')), description=description, mitigation=tree.get('advisories').get(element).get('recommendation'), references=((url + '\n') + references), component_name=tree.get('advisories').get(element).get('module_name'), component_version=tree.get('advisories').get(element).get('findings')[0]['version'], false_p=False, duplicate=False, out_of_scope=False, mitigated=None, impact='No impact provided', static_finding=True, dynamic_finding=False)
    ...
    def get_item_yarn(self, item_node, test):
        ...
        dojo_finding = Finding(title=((((((item_node['title'] + ' - ') + '(') + item_node['module_name']) + ', ') + item_node['vulnerable_versions']) + ')'), test=test, severity=severity, file_path=item_node['findings'][0]['paths'][0], description=((((((((((((((item_node['url'] + '\n') + item_node['overview']) + '\n Vulnerable Module: ') + item_node['module_name']) + '\n Vulnerable Versions: ') + str(item_node['vulnerable_versions'])) + '\n Patched Version: ') + str(item_node['patched_versions'])) + '\n Vulnerable Paths: ') + str(paths)) + '\n CWE: ') + str(item_node['cwe'])) + '\n Access: ') + str(item_node['access'])), cwe=cwe, mitigation=item_node['recommendation'], references=item_node['url'], component_name=item_node['module_name'], component_version=item_node['findings'][0]['version'], false_p=False, duplicate=False, out_of_scope=False, mitigated=None, impact='No impact provided', static_finding=True, dynamic_finding=False)
