    ...
    def get_findings(self, scan_file, test):
        ...
        try:
            if ('UTC' in data['Generated']):
                scan_date = datetime.datetime.strptime(data['Generated'].split(' ')[0], '%d/%m/%Y').date()
            else:
        ...
        for item in data['Vulnerabilities']:
            ...
            finding = Finding(title=title, test=test, description=findingdetail, severity=sev.title(), mitigation=mitigation, impact=impact, date=scan_date, references=references, cwe=cwe, static_finding=True)
            ...
            if (item['Classification'] is not None):
                if ((item['Classification'].get('Cvss') is not None) and (item['Classification'].get('Cvss').get('Vector') is not None)):
                    ...
                    cvss_objects = cvss_parser.parse_cvss_from_text(item['Classification']['Cvss31']['Vector'])
                    if (len(cvss_objects) > 0):
                        finding.cvssv3 = cvss_objects[0].clean_vector()
