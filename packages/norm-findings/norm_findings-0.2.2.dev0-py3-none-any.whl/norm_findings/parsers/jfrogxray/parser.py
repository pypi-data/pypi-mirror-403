    ...
    def get_items(self, tree, test):
        ...
        if ('data' in tree):
            ...
            for node in vulnerabilityTree:
                ...
                unique_key = ((((node.get('id') + node.get('summary')) + node.get('provider')) + node.get('source_comp_id')) + title_cve)
...
**Provider:** {provider}'''
    ...
    finding = Finding(title=title, cwe=cwe, test=test, severity=severity, description=(vulnerability['summary'] + extra_desc).strip(), mitigation=mitigation, component_name=component_name, component_version=component_version, file_path=vulnerability.get('source_comp_id'), static_finding=True, dynamic_finding=False, cvssv3=cvssv3, fix_available=fix_available)
    if vulnerability_ids:
