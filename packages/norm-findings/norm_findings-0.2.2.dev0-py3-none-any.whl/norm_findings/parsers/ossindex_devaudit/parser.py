        ...
        for package in results.get('Packages', []):
            ...
            if (len(package.get('Vulnerabilities', [])) > 0):
                for vulnerability in package.get('Vulnerabilities', []):
                    item = get_item(dependency_name=package_data['name'], dependency_version=package_data['version'], dependency_source=package_data['pm'], vulnerability=vulnerability, test=test)
...
def get_item(dependency_name, dependency_version, dependency_source, vulnerability, test):
    ...
    try:
        ...
        raise ValueError(msg)
    return Finding(title=((((((((dependency_source + ':') + dependency_name) + ' - ') + '(') + dependency_version) + ', ') + cwe_data) + ')'), test=test, severity=get_severity(vulnerability.get('cvssScore', '')), description=vulnerability['title'], cwe=cwe, cvssv3=vulnerability['cvssVector'].replace('CVSS:3.0', ''), mitigation='Upgrade the component to the latest non-vulnerable version, or remove the package if it is not in use.', references=vulnerability.get('reference', ''), false_p=False, duplicate=False, out_of_scope=False, mitigated=None, static_finding=False, dynamic_finding=False, impact='No impact provided by scan')

def get_severity(cvss_score):
    result = 'Info'
    if cvss_score:
        ratings = [('Critical', 9.0, 10.0), ('High', 7.0, 8.9), ('Medium', 4.0, 6.9), ('Low', 0.1, 3.9)]
