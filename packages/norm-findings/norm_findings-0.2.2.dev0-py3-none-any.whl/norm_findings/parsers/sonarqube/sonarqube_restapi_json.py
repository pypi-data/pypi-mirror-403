    ...
    def get_json_items(self, json_content, test, mode):
        ...
        if json_content.get('issues'):
            for issue in json_content.get('issues'):
                if (issue.get('type') == 'BUG'):
                    ...
                    item = Finding(title=((rule + '_') + key), description=description, test=test, severity=self.severitytranslator(issue.get('severity')), static_finding=True, dynamic_finding=False, line=line, date=date)
                    ...
                    if (tags != []):
                        description += (('**tags:** ' + ', '.join(tags)) + '\n')
                    description += self.returncomponent(json_content, component)
                    item = Finding(title=((rule + '_') + key), description=description, test=test, severity=self.severitytranslator(issue.get('severity')), static_finding=True, dynamic_finding=False, component_name=component_name, component_version=component_version, cwe=cwe, cvssv3_score=cvss, file_path=component, line=line, date=date)
                    item.unsaved_tags = ['vulnerability']
                    vulnids = []
                    if ('Reference: CVE' in message):
                        cve_pattern = 'Reference: CVE-\\d{4}-\\d{4,7}'
                        cves = re.findall(cve_pattern, message)
                        vulnids.extend((cve.split('Reference: ')[1] for cve in cves))
                    if ('References: CVE' in message):
                        cve_pattern = 'References: CVE-\\d{4}-\\d{4,7}'
                        cves = re.findall(cve_pattern, message)
                        vulnids.extend((cve.split('References: ')[1] for cve in cves))
                    if ('Reference: GHSA' in message):
                        cve_pattern = 'Reference: GHSA-[23456789cfghjmpqrvwx]{4}-[23456789cfghjmpqrvwx]{4}-[23456789cfghjmpqrvwx]{4}'
                        cves = re.findall(cve_pattern, message)
                        vulnids.extend((cve.split('Reference: ')[1] for cve in cves))
                    if ('References: GHSA' in message):
                        cve_pattern = 'References: GHSA-[23456789cfghjmpqrvwx]{4}-[23456789cfghjmpqrvwx]{4}-[23456789cfghjmpqrvwx]{4}'
                        cves = re.findall(cve_pattern, message)
                        vulnids.extend((cve.split('References: ')[1] for cve in cves))
                    ...
                    if (codeVariants != []):
                        description += (('**codeVariants:** ' + ', '.join(codeVariants)) + '\n')
                    description += self.returncomponent(json_content, component)
                    item = Finding(title=((rule + '_') + key), description=description, test=test, severity=self.severitytranslator(issue.get('severity')), static_finding=True, dynamic_finding=False, file_path=component, line=line, date=date)
        ...
        if json_content.get('hotspots'):
            for hotspot in json_content.get('hotspots'):
                ...
                description += self.returncomponent(json_content, component)
                item = Finding(title=((ruleKey + '_') + key), description=description, test=test, severity=self.severitytranslator(hotspot.get('vulnerabilityProbability')), static_finding=True, dynamic_finding=False, file_path=component, line=line, date=date)
