    ...
    def get_json_items(self, json_content, test, mode):
        ...
        for issue in issues:
            ...
            if ((title is None) or (mitigation is None)):
                raise ValueError(("Parser ValueError: can't find a title or a mitigation for vulnerability of name " + rule_id))
            try:
                ...
                issue_references = SonarQubeSoprasteriaHelper().get_references(rule_id, html_desc_as_e_tree)
                ...
                issue_cwe = 0
            if (mode is None):
                SonarQubeSoprasteriaHelper().process_result_file_name_aggregated(test, dupes, title, issue_cwe, issue_description, file_path, line, severity, mitigation, issue_references)
            else:
                SonarQubeSoprasteriaHelper().process_result_detailed(test, dupes, title, issue_cwe, issue_description, file_path, line, severity, mitigation, issue_references, key)
