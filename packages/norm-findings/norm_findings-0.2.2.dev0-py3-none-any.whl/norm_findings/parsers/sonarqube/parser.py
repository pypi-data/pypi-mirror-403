...
from norm_findings.parsers.sonarqube.sonarqube_restapi_zip import SonarQubeRESTAPIZIP
from norm_findings.parsers.sonarqube.soprasteria_html import SonarQubeSoprasteriaHTML
from norm_findings.parsers.sonarqube.soprasteria_json import SonarQubeSoprasteriaJSON
...
class SonarQubeParser():
    ...
    def get_findings(self, scan_file, test):
        if scan_file.name.endswith('.json'):
            json_content = json.load(scan_file)
            ...
            return []
        if scan_file.name.endswith('.zip'):
            if (str(scan_file.__class__) == "<class '_io.TextIOWrapper'>"):
                input_zip = zipfile.ZipFile(scan_file.name, 'r')
            else:
                input_zip = zipfile.ZipFile(scan_file, 'r')
        ...
        tree = etree.parse(scan_file, parser)
