...
class Crunch42Parser():
        ...
        if (('report' in tree) and tree['report'].get('security')):
            ...
            for (key, node) in results.items():
                for issue in node['issues']:
                    item = self.get_item(issue, key, test)
                    items[iterator] = item
                    iterator += 1
    ...
    def get_item(self, issue, title, test):
        ...
        if (criticality == 1):
            ...
            severity = 'Critical'
        return Finding(unique_id_from_tool=fingerprint, title=title, test=test, severity=severity, description=((((((((((('**fingerprint**: ' + str(fingerprint)) + '\n') + '**pointer**: ') + str(pointer)) + '\n') + '**message**: ') + str(message)) + '\n') + '**score**: ') + str(score)) + '\n'), false_p=False, duplicate=False, out_of_scope=False, static_finding=True, dynamic_finding=False)
