...
class BundlerAuditParser():
    ...
    def get_findings(self, scan_file, test):
        ...
        for warning in warnings:
            ...
            title = (((((('Gem ' + gem_name) + ': ') + advisory_title) + ' [') + advisory_id) + ']')
            ...
            fingerprint = (((('bundler-audit' + gem_name) + gem_version) + advisory_id) + sev)
            dupe_key = hashlib.md5(fingerprint.encode('utf-8'), usedforsecurity=False).hexdigest()
            if (dupe_key in dupes):
                find = dupes[dupe_key]
            else:
                dupes[dupe_key] = True
                find = Finding(title=title, test=test, description=findingdetail, severity=sev, mitigation=mitigation, references=references, date=find_date, static_finding=True, dynamic_finding=False, component_name=gem_name, component_version=gem_version)
                if advisory_id:
