...
class AWSProwlerParser():
    ...
    def get_findings(self, scan_file, test):
        if scan_file.name.lower().endswith('.csv'):
            return self.process_csv(scan_file, test)
        if scan_file.name.lower().endswith('.json'):
            return self.process_json(scan_file, test)
    ...
    def process_csv(self, file, test):
        ...
        for row in reader:
            ...
            prowler_check_number = re.search('\\[(.*?)\\]', title_text).group(1)
            control = re.sub('\\[.*\\]\\s', '', title_text)
            ...
            if (dupe_key in dupes):
                ...
                find = Finding(active=active, title=textwrap.shorten(result_extended, 150), cwe=1032, test=test, description=description, severity=sev, references=documentation, static_finding=True, dynamic_finding=False, nb_occurences=1, mitigation=mitigation, impact=impact)
                dupes[dupe_key] = find
    ...
    def process_json(self, file, test):
        ...
        for issue in data:
            ...
            if (dupe_key in dupes):
                ...
                find = Finding(title=textwrap.shorten(result_extended, 150), cwe=1032, test=test, description=description, severity=sev, references=documentation, date=date.fromisoformat(timestamp[:10]), static_finding=True, dynamic_finding=False, nb_occurences=1, mitigation=mitigation, impact=impact)
                dupes[dupe_key] = find
        return list(dupes.values())
