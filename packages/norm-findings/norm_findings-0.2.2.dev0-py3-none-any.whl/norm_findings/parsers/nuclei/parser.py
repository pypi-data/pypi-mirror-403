    ...
    def get_findings(self, scan_file, test):
        ...
        for item in data:
            ...
            if (severity not in Finding.SEVERITIES):
                logger.debug('Unsupported severity value "%s", change to "%s"', severity, self.DEFAULT_SEVERITY)
            ...
            finding = Finding(title=f'{name}', test=test, severity=severity, nb_occurences=1, vuln_id_from_tool=template_id)
            ...
            dupe_key = hashlib.sha256((((template_id + item_type) + matcher) + str(endpoint.host)).encode('utf-8')).hexdigest()
