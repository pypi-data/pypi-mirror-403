...
class ApiCobaltParser():
    ...
    def get_findings(self, scan_file, test):
        data = (CobaltApiImporter().get_findings(test) if (scan_file is None) else json.load(scan_file))
        findings = []
        for entry in data['data']:
            ...
            description = '\n'.join([resource['description'], '', 'Cobalt.io details:', f"- Impact: {resource['impact']}", f"- Likelihood: {resource['likelihood']}", '', 'Cobalt.io link:', links['ui']['url']])
    ...
    def include_finding(self, resource):
        ...
        allowed_states = ['carried_over', 'check_fix', 'duplicate', 'invalid', 'need_fix', 'new', 'out_of_scope', 'triaging', 'valid_fix', 'wont_fix']
    ...
    def is_active(self, cobalt_state):
        return ((not self.is_mitigated(cobalt_state)) and (not self.is_false_p(cobalt_state)) and (not self.is_out_of_scope(cobalt_state)))
