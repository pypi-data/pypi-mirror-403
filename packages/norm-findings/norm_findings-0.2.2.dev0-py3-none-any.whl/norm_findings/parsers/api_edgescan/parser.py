...
class ApiEdgescanParser():
    ...
    def get_findings(self, scan_file, test):
        data = (json.load(scan_file) if scan_file else EdgescanImporter().get_findings(test))
    ...
    def make_finding(self, test, vulnerability):
        ...
        finding.unsaved_endpoints = [(Endpoint.from_uri(vulnerability['location']) if ('://' in vulnerability['location']) else Endpoint.from_uri(('//' + vulnerability['location'])))]
        return finding
