...
class BurpDastardlyParser():
    ...
    def get_description_for_scan_types(self, scan_type):
        return 'Import Burp Dastardly XML files.'
            ...
            if (int(node.attrib['failures']) != 0):
                ...
                for case in testcase:
                    for fail in case.findall('failure'):
                        ...
                        finding = Finding(title=title, url=name, test=test, severity=severity, description=description, false_p=False, duplicate=False, out_of_scope=False, mitigated=None, dynamic_finding=True)
