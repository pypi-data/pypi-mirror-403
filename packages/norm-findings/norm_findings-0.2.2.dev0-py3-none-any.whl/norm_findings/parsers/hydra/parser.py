...
class HydraParser():
    ...
    def get_findings(self, scan_file, test):
        report = self.__parse_json(scan_file)
    ...
    def __extract_finding(self, raw_finding, metadata: HydraScanMetadata, test) -> Finding:
        ...
        finding = Finding(test=test, title=('Weak username / password combination found for ' + host), date=(parse_datetime(metadata.date) if metadata.date else date.today()), severity='High', description=((((((host + ' on port ') + str(port)) + ' is allowing logins with easy to guess username ') + username) + ' and password ') + password), static_finding=False, dynamic_finding=True, service=metadata.service_type)
