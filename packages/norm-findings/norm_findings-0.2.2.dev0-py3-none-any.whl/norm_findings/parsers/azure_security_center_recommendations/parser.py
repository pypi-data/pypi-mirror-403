    ...
    def process_csv(self, file, test):
        ...
        for row in reader:
            if (row.get('state').lower() == 'unhealthy'):
                ...
                findings.append(Finding(title=title, cwe=1032, test=test, description=description, severity=severity, references=azure_portal_recommendation_link, mitigation=remediation_steps, date=datetime.strptime(status_change_date[0:10], '%Y-%m-%d').date(), vuln_id_from_tool=recommendation_name, unique_id_from_tool=recommendation_id, static_finding=True, dynamic_finding=False))
        return findings
