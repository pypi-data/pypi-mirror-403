...
class NexposeParser():
    ...
    def parse_tests_type(self, node, vulns_definitions):
        ...
        for tests in node.findall('tests'):
            for test in tests.findall('test'):
                if ((test.get('id') in vulns_definitions) and (test.get('status') in {'vulnerable-exploited', 'vulnerable-version', 'vulnerable-potential'})):
        ...
        for vulnsDef in tree.findall('VulnerabilityDefinitions'):
            for vulnDef in vulnsDef.findall('vulnerability'):
                ...
                if (severity_chk >= 9):
                    ...
                    sev = 'Info'
                vuln = {'desc': '', 'name': vulnDef.get('title'), 'vector': vulnDef.get('cvssVector'), 'refs': {}, 'resolution': '', 'severity': sev, 'tags': []}
    ...
    def get_items(self, tree, vulns, test):
        ...
        for nodes in tree.findall('nodes'):
            for node in nodes.findall('node'):
                ...
                host['vulns'].append({'name': 'Host Up', 'desc': 'Host is up because it replied on ICMP request or some TCP/UDP port is up', 'severity': 'Info'})
                ...
                for endpoints in node.findall('endpoints'):
                    for endpoint in endpoints.findall('endpoint'):
                        svc = {'protocol': endpoint.get('protocol'), 'port': int(endpoint.get('port')), 'status': endpoint.get('status')}
                        for services in endpoint.findall('services'):
                            for service in services.findall('service'):
                                ...
                                svc['vulns'].append({'name': 'Open port {}/{}'.format(svc['protocol'].upper(), svc['port']), 'desc': '{}/{} port is open with "{}" service'.format(svc['protocol'], svc['port'], service.get('name')), 'severity': 'Info', 'tags': ([re.sub('[^A-Za-z0-9]+', '-', service.get('name').lower()).rstrip('-')] if (service.get('name') != '<unknown>') else [])})
        ...
        for host in hosts:
            ...
            for service in host['services']:
                for vuln in service['vulns']:
                    dupe_key = (vuln['severity'] + vuln['name'])
                    find = self.findings(dupe_key, dupes, vuln)
                    endpoint = Endpoint(host=host['name'], port=service['port'], protocol=(service['name'] if (service['name'] in SCHEME_PORT_MAP) else service['protocol']), fragment=(service['protocol'].lower() if (service['name'] == 'dns') else None))
                    find.unsaved_endpoints.append(endpoint)
                    find.unsaved_tags = vuln.get('tags', [])
    ...
    def findings(dupe_key, dupes, vuln):
        ...
        if (dupe_key in dupes):
            ...
            find = Finding(title=vuln['name'], description=((html2text.html2text(vuln['desc'].strip()) + '\n\n') + html2text.html2text(vuln.get('pluginOutput', '').strip())), severity=vuln['severity'], mitigation=(html2text.html2text(vuln.get('resolution')) if vuln.get('resolution') else None), impact=(vuln.get('vector') or None), false_p=False, duplicate=False, out_of_scope=False, dynamic_finding=True, date=vuln.get('vulnerableSince'))
            refs = ''
