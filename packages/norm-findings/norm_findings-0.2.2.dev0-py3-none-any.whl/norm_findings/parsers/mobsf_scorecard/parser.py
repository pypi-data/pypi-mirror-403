...
class MobSFScorecardParser():
    ...
    def get_findings(self, scan_file, test):
        ...
        if ('timestamp' in data):
            ...
            find_date = datetime.now()
        appsec_fields_for_test_desc = ['file_name', 'hash', 'security_score', 'app_name', 'version_name']
        main_fields_for_test_desc = ['app_type', 'package_name', 'bundle_id', 'sdk_name', 'platform']
        test_description = ''
...
'''
        ...
        finding_severities = {'high': 'High', 'warning': 'Medium', 'info': 'Info', 'secure': 'Info', 'hotspot': 'Low'}
        dd_findings = {}
        for (mobsf_severity, defectdojo_severity) in finding_severities.items():
            if (mobsf_severity in data.get('appsec', {})):
                for mobsf_finding in data['appsec'][mobsf_severity]:
                    ...
                    finding = Finding(title=title, cwe=919, test=test, description=f'''**Category:** {section}

{description}''', severity=defectdojo_severity, references=None, date=find_date, static_finding=True, dynamic_finding=False, nb_occurences=1)
