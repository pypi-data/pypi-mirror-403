...
'''
        test_description = '{}**Provider:** {}\n'.format(test_description, data['provider_name'])
        test_description = '{}**Ruleset:** `{}`\n'.format(test_description, last_run['ruleset_name'])
        test_description = '{}**Ruleset Description:** {}\n'.format(test_description, last_run['ruleset_about'])
...
:---|---:|---:|---:|---:|---:'''
        for (service, items) in list(last_run['summary'].items()):
            test_description += '\n'
            test_description += '|'.join([service, str(items['checked_items']), str(items['flagged_items']), str(items['max_level']), str(items['resources_count']), str(items['rules_count'])])
        tests = []
        test = ParserTest(name=self.ID, parser_type=data['provider_name'], version=last_run.get('version'))
        ...
        for service_name in data['services']:
            ...
            for finding_name in service_item.get('findings', []):
                ...
                for name in finding.get('items', []):
                    ...
                    self.item_data = ''
                    find = Finding(title=textwrap.shorten(finding['description'], 150), date=last_run_date, cwe=1032, description=description_text, severity=self.getCriticalityRating(finding['level']), mitigation=finding.get('remediation'), file_path=name, dynamic_finding=False, static_finding=True, vuln_id_from_tool=':'.join([data['provider_code'], finding_name]))
        ...
        if isinstance(src, dict):
            for (dictkey, value) in src.items():
                if isinstance(src, str):
                    self.item_data = ((self.item_data + dictkey) + '\n')
                self.recursive_print(value, (depth + 1), dictkey)
            ...
            if (self.pdepth != depth):
                self.item_data += '\n'
            if key:
                self.item_data = ((self.item_data + self.formatview(depth)) + f'''**{key.title()}:** {src}

''')
            else:
                self.item_data = ((self.item_data + self.formatview(depth)) + f'''{src}
''')
            self.pdepth = depth
