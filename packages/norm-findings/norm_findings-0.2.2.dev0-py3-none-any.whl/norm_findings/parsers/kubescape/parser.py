            ...
            for control in controls:
                for rule in control['rules']:
                    ...
                    if (retrocompatibility_condition or (control['status']['status'] == 'failed')):
                        ...
                        find = Finding(title=textwrap.shorten(title, 150), test=test, description=description, mitigation=mitigation, steps_to_reproduce=steps_to_reproduce, references=f'https://hub.armosec.io/docs/{controlID.lower()}', severity=severity, component_name=resourceid, static_finding=True, dynamic_finding=False)
