...
class AnchoreEngineParser():
    ...
    def get_findings(self, scan_file, test):
        try:
            ...
            with Path(scan_file).open(encoding='utf-8') as file:
                data = json.load(file)
...
'''
        for item in data.get('securityEvaluation', []):
            ...
            title = (((((vulnerability_id + '-') + item.get('package', 'Unknown')) + '(') + item.get('packageType', 'Unknown')) + ')')
...
'''
            ...
            dupe_key = '|'.join([item.get('cves', 'None'), item.get('package', 'None'), item.get('packageType', 'None'), item.get('path', 'None'), item.get('severity', 'None')])
            if (dupe_key in dupes):
                find = dupes[dupe_key]
            else:
                find = Finding(title=title, test=test, cve=item.get('cves'), cvssv3_score=cvssv3_base_score, description=details, severity=severity, mitigation=mitigation, references=references, file_path=item.get('path'), component_name=item.get('package'), url=item.get('link'), static_finding=True, dynamic_finding=False, vuln_id_from_tool=vulnerability_id, fix_available=fix_available)
                if vulnerability_id:
    ...
    def get_findings_without_metadata(self, data, test):
        ...
        for item in data['vulnerabilities']:
            ...
            if (cvssv3_base_score and ((float(cvssv3_base_score) < 0) or (float(cvssv3_base_score) > 10))):
                cvssv3_base_score = None
            references = item['url']
            dupe_key = '|'.join([item.get('image_digest', item.get('imageDigest', 'None')), item['feed'], item['feed_group'], item['package_name'], item['package_version'], item['package_path'], item['vuln']])
            if (dupe_key in dupes):
                find = dupes[dupe_key]
            else:
                dupes[dupe_key] = True
                find = Finding(title=title, test=test, cvssv3_score=cvssv3_base_score, description=findingdetail, severity=sev, mitigation=mitigation, references=references, file_path=item['package_path'], component_name=item['package_name'], component_version=item['package_version'], url=item.get('url'), static_finding=True, dynamic_finding=False, vuln_id_from_tool=item.get('vuln'), fix_available=fix_available)
                if vulnerability_id:
                    find.unsaved_vulnerability_ids = [vulnerability_id]
                dupes[dupe_key] = find
        return list(dupes.values())
