...
class TenableXMLParser():
    ...
    def get_fields(self) -> list[str]:
        ...
        return ['title', 'description', 'severity', 'mitigation', 'impact', 'cwe', 'cvssv3']
    ...
    def get_dedupe_fields(self) -> list[str]:
        ...
        return ['title', 'severity', 'cwe', 'description']
    ...
    def get_findings(self, scan_file: str, test: Test) -> list:
        nscan = ElementTree.parse(scan_file)
        root = nscan.getroot()
        if ('NessusClientData_v2' not in root.tag):
            msg = 'This version of Nessus report is not supported. Please make sure the export is formatted using the NessusClientData_v2 schema.'
...
'''
                    ...
                    if (cwe_element_text is not None):
                        match = re.search('\\d+', cwe_element_text)
                        if match:
                            cwe = int(match.group())
                    cvssv3 = None
...
{plugin_output}'''
                    ...
                    if ((fqdn is not None) and ('://' in fqdn)):
                        ...
                        endpoint = Endpoint(protocol=protocol, host=(fqdn or ip), port=port)
