    ...
    def get_findings(self, scan_file, test):
        ...
        for detect_file in data.get('results'):
            for item in data.get('results').get(detect_file):
                ...
                dupe_key = hashlib.sha256((((item_type + file) + str(line)) + hashed_secret).encode('utf-8')).hexdigest()
                if (dupe_key in dupes):
                    finding = dupes[dupe_key]
                    finding.nb_occurences += 1
                else:
                    finding = Finding(title=item_type, test=test, description=description, cwe=798, date=find_date, severity='High', verified=is_verified, active=((('is_secret' in item) and (item['is_secret'] is True)) or ('is_secret' not in item)), file_path=file, line=line, nb_occurences=1, false_p=(('is_secret' in item) and (item['is_secret'] is False)))
