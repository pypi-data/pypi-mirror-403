        ...
        for item in vulnerabilities:
            if ((item['condition'] != 'Package unfixed') and (item['severity'] != 'Untriaged')):
                ...
                if (dupe_key in dupes):
                    ...
                    dupes[dupe_key] = True
                    find = Finding(title=title, test=test, description=description, severity=severity, references=references, static_finding=True, component_name=package_name, component_version=package_version, cvssv3_score=cvssv3_score, publish_date=publish_date, unique_id_from_tool=dupe_key, date=detection_time)
                    if agent_name:
