...
class RetireJsParser():
            ...
            for result in node['results']:
                if ('vulnerabilities' in result):
                    for vulnerability in result['vulnerabilities']:
                        ...
                        unique_key = hashlib.md5(((item.title + item.references) + encrypted_file).encode(), usedforsecurity=False).hexdigest()
    ...
    def get_item(self, item_node, test, file):
        ...
        return Finding(title=title, test=test, cwe=1035, severity=item_node['severity'].title(), description=((title + '\n\n Affected File - ') + file), file_path=file, references='\n'.join(item_node['info']), false_p=False, duplicate=False, out_of_scope=False)
