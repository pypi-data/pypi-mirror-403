...
class Severityfilter():
    ...
    def __init__(self):
        self.severity_mapping = {'Very Low': 'Info', 'Low': 'Low', 'Normal': 'Medium', 'High': 'High', 'Very High': 'Critical'}
        self.severity = None
        ...
        for row in csvarray:
            ...
            if (finding is not None):
                ...
                if (finding.description is None):
                    finding.description = ''
                key = hashlib.md5(((((((finding.severity + '|') + finding.title) + '|') + finding.description) + '|') + str(finding.cwe)).encode('utf-8'), usedforsecurity=False).hexdigest()
                if (key not in dupes):
                    dupes[key] = finding
