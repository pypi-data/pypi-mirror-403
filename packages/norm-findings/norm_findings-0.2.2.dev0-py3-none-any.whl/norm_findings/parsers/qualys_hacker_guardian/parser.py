...
class QualysHackerGuardianParser():
    ...
    def get_fields(self) -> list[str]:
        ...
        return ['title', 'severity', 'description', 'date', 'impact', 'mitigation', 'unique_id_from_tool', 'dynamic_finding', 'active', 'nb_occurences']
    ...
    def get_findings(self, scan_file, test):
        ...
        for row in reader:
            ...
            finding = Finding(title=row.get('VULN TITLE'), severity=self.qualys_severity_lookup[row.get('Q_SEVERITY', 1)], description=f'''**Category**: {row.get('CATEGORY', 'Unknown')}
**Threat**: {row.get('THREAT', 'No threat detected')}
**Result**: {row.get('RESULT', 'No threat detected')}
''', date=date_parser.parse(row.get('LAST SCAN')), impact=row.get('IMPACT'), mitigation=row.get('SOLUTION'), unique_id_from_tool=row.get('QID'), dynamic_finding=True, active=True, nb_occurences=1)
