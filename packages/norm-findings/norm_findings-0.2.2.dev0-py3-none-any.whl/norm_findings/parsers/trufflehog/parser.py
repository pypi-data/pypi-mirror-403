    ...
    def get_findings_v2(self, data, test):
        ...
        for line in data:
            ...
            if (dupe_key in dupes):
                ...
                dupes[dupe_key] = True
                finding = Finding(title=titleText, test=test, cwe=798, description=description, severity=severity, mitigation='Secrets and passwords should be stored in a secure vault and/or secure storage.', impact='This weakness can lead to the exposure of resources or functionality to unintended actors, possibly providing attackers with sensitive information or even execute arbitrary code.', references='N/A', file_path=file, line=0, url='N/A', dynamic_finding=False, static_finding=True, nb_occurences=1)
                dupes[dupe_key] = finding
...
'''
            ...
            if (not verified):
                if (('Oauth' in detector_name) or ('AWS' in detector_name) or ('Heroku' in detector_name)):
                    ...
                    severity = 'Medium'
            dupe_key = hashlib.md5(((((file + detector_name) + str(line_number)) + commit) + (raw + rawV2)).encode('utf-8'), usedforsecurity=False).hexdigest()
            if (dupe_key in dupes):
                finding = dupes[dupe_key]
                finding.description += description
                finding.nb_occurences += 1
                ...
                dupes[dupe_key] = True
                finding = Finding(title=titleText, test=test, cwe=798, description=description, severity=severity, mitigation=mitigation, impact='This weakness can lead to the exposure of resources or functionality to unintended actors, possibly providing attackers with sensitive information or even execute arbitrary code.', references='N/A', file_path=file, line=line_number, url='N/A', dynamic_finding=False, static_finding=True, nb_occurences=1)
                dupes[dupe_key] = finding
        return list(dupes.values())
