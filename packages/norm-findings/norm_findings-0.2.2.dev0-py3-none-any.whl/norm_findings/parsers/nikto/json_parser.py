...
class NiktoJSONParser():
    ...
    def process_json(self, file, test):
        ...
        for vulnerability in data.get('vulnerabilities', []):
            description = '\n'.join([f"**id:** `{vulnerability.get('id')}`", f"**msg:** `{vulnerability.get('msg')}`", f"**HTTP Method:** `{vulnerability.get('method')}`"])
            if (vulnerability.get('OSVDB') is not None):
                description += ('\n' + f"**OSVDB:** `{vulnerability.get('OSVDB')}`")
            finding = Finding(title=vulnerability.get('msg'), severity='Info', description=description, vuln_id_from_tool=vulnerability.get('id'), nb_occurences=1, references=vulnerability.get('references'))
            ...
            endpoint = Endpoint(host=host, port=port, path=vulnerability.get('url'))
            ...
            if (dupe_key in dupes):
                ...
                find.unique_id_from_tool = None
                find.vuln_id_from_tool = None
