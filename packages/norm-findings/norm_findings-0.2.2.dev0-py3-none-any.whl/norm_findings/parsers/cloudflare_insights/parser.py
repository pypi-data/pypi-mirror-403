...
class CloudflareInsightsParser():
    ...
    def _map_severity(self, value):
        ...
        mapping = {'low': 'Low', 'moderate': 'Medium', 'critical': 'Critical', 'high': 'High'}
        return mapping.get(normalized, 'Info')

    def _extract_host_from_subject(self, subject: str) -> typing.Union[(str, None)]:
        if (not subject):
            return None
        s = subject.strip()
        if (not s):
    ...
    def _parse_csv(self, content: str, test):
        ...
        for row in reader:
            ...
            finding = Finding(test=test, title=title, severity=mapped_severity, description=description, mitigation=recommended_action, references='Not provided!', static_finding=False, dynamic_finding=True)
