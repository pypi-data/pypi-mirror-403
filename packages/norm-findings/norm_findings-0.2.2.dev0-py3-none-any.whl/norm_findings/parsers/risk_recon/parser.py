...
from norm_findings.parsers.risk_recon.api import RiskReconAPI
...
class RiskReconParser():
    ...
    def get_findings(self, scan_file, test):
        if scan_file:
            ...
            if (not data.get('test', None)):
                api = RiskReconAPI(data.get('api_key', None), data.get('url_endpoint', None), data.get('companies', data.get('filters', [])))
    ...
    def _get_findings_internal(self, findings, test):
        ...
        for item in findings:
            ...
            finding = Finding(title=title, test=test, description=findingdetail, severity=sev, static_finding=False, dynamic_finding=True, date=date, unique_id_from_tool=item.get('finding_id'), nb_occurences=1)
