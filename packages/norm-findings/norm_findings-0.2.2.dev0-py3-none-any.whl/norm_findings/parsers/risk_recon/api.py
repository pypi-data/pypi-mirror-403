...
class RiskReconAPI():
    ...
    def map_toes(self):
        response = self.session.get(url=f'{self.url}/toes', headers={'accept': 'application/json', 'Authorization': self.key}, timeout=settings.REQUESTS_TIMEOUT)
        if response.ok:
            comps = {}
            data = response.json()
    ...
    def get_findings(self):
        for toe in self.toe_map:
            response = self.session.get(url=f'{self.url}/findings/{toe}', headers={'accept': 'application/json', 'Authorization': self.key}, timeout=settings.REQUESTS_TIMEOUT)
            if response.ok:
                data = response.json()
                for finding in data:
