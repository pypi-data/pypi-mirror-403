...
import typing
import datetime
# Required stubs: Endpoint, Endpoint_Status, Finding
...
from norm_findings.stubs.models import Endpoint, Endpoint_Status, Finding
...
class CyberwatchGaleaxParser():
    ...
    def get_findings(self, scan_file, test):
        logger.debug('Starting get_findings with filename: %s', scan_file)
        try:
            file_content = self.read_file_content(scan_file)
    ...
    def collect_cve_data(self, json_data, cve_data):
        ...
        description = f'''CVSS Base vector: {cvssv3}
CVSS Base score: {cve_score}
CVE Published At: {cve_published_at}
Exploit Code Maturity: {exploit_code_maturity}
EPSS: {cve_epss}
Parent CWE:{','.join([f'CWE-{cwe}' for cwe in additional_cwes])}
CAPECS: {cwe_capecs}
TTP: {cwe_ttp}
'''
        references = f'Updated At: {updated_at_str}'
        if (cve_code not in cve_data):
            cve_data[cve_code] = {'cwe_num': primary_cwe, 'additional_cwes': additional_cwes, 'epss': (cve_epss if (cve_epss != 'N/A') else None), 'severity': severity, 'description': description, 'impact': impact, 'references': references, 'cvssv3': cvssv3, 'cvssv3_score': cvssv3_score, 'products': {}, 'no_product_endpoints': set()}
    ...
    def collect_products_data(self, updates_assets, server_lookup, c_data):
        ...
        for (product, data) in updates_assets.items():
            ...
            if (product not in c_data['products']):
                c_data['products'][product] = {'endpoints': set(), 'versions': set(), 'active_mitigated_data': []}
    ...
    def build_findings_for_cve(self, cve_code, c_data, test):
        ...
        if (not products):
            ...
            findings.append(self.create_finding(title=cve_code, test=test, description=description, severity=severity, mitigation=mitigation, impact=impact, references=references, active=False, mitigated=mitigated_date, cvssv3=cvssv3, cvssv3_score=cvssv3_score, endpoints=endpoints, cwe_num=cwe_num, epss=epss, cve_code=cve_code, additional_cwes=additional_cwes))
            return findings
        for (product, p_data) in products.items():
            ...
            findings.append(self.create_finding(title=f'{cve_code} on {product}', test=test, description=description, severity=severity, mitigation=mitigation, impact=impact, references=references, active=active_status, mitigated=mitigated_date, cvssv3=cvssv3, cvssv3_score=cvssv3_score, endpoints=endpoints, cwe_num=cwe_num, epss=epss, cve_code=cve_code, component_name=product, component_version=component_version_str, additional_cwes=additional_cwes))
        return findings
    ...
    def determine_product_finding_state(self, p_data):
        ...
        mitigated_date = (max((am[1] for am in p_data['active_mitigated_data'] if am[1])) if ([am[1] for am in p_data['active_mitigated_data'] if am[1]] and (not active_status)) else (timezone.now() if (not active_status) else None))
        return (component_version_str, active_status, mitigated_date)

    def create_finding(self, title, test, description, severity, mitigation, impact, references, active, mitigated, cvssv3, cvssv3_score, endpoints, cwe_num=None, epss=None, cve_code=None, component_name=None, component_version=None, additional_cwes=None):
        'Helper to create a Finding object with all the common attributes.'
        finding = Finding(title=title, test=test, description=description, severity=severity, mitigation=mitigation, impact=impact, references=references, active=active, verified=False, false_p=False, duplicate=False, out_of_scope=False, mitigated=mitigated, numerical_severity=Finding.get_numerical_severity(severity), cvssv3=cvssv3, cvssv3_score=cvssv3_score, static_finding=False, dynamic_finding=True, component_name=component_name, component_version=component_version)
    ...
    def global_version_dedup(self, findings):
        ...
        severity_priority_map = {'Critical': 4, 'High': 3, 'Medium': 2, 'Low': 1, 'Info': 0}
    ...
    def process_security_issue(self, json_data, test):
        ...
        impact = ''
        finding = Finding(title=f'Security Issue - {security_issue_title}', test=test, description=description, severity=severity, mitigation=mitigation, impact=impact, references=references, active=active_status, verified=False, false_p=False, duplicate=False, out_of_scope=False, mitigated=mitigated_date, numerical_severity=Finding.get_numerical_severity(severity), static_finding=False, dynamic_finding=True)
        if issue_id:
    ...
    def process_servers_for_security_issue(self, servers):
        ...
        for server in servers:
            ...
            endpoint_status = Endpoint_Status(endpoint=endpoint, finding=None, mitigated=mitigated_date, last_modified=detected_at, false_positive=False, out_of_scope=False, mitigated_by=None)
    ...
    def convert_severity(self, severity):
        severity_mapping = {'CRITICAL': 'Critical', 'HIGH': 'High', 'MEDIUM': 'Medium', 'LOW': 'Low', 'INFO': 'Info', 'LEVEL_CRITICAL': 'Critical', 'LEVEL_HIGH': 'High', 'LEVEL_MEDIUM': 'Medium', 'LEVEL_LOW': 'Low', 'LEVEL_INFO': 'Info'}
