"""
PostgreSQL TrustStore Implementation using DataFlow.

Provides persistent storage for EATP Trust Lineage Chains with:
- Efficient storage using JSONB for complex nested structures
- Caching for <10ms get_chain() performance
- Soft delete capability
- Advanced filtering by agent_id, authority_id, capability_type, active status
- Pagination support for list operations

Architecture:
- Uses DataFlow for zero-config database operations
- Automatically generates 11 workflow nodes per model
- Built-in caching via DataFlow cache integration
