...
from kaizen.core.specialist_types import SpecialistDefinition, SkillDefinition
...
class TestSpecialistLoaderFilesystem:
    ...
    def test_load_specialist_from_project_dir(self, tmp_path: Path):
        ...
        specialist_file.write_text("""# Code Reviewer Specialist
...
""")
    ...
    def test_load_multiple_specialists(self, tmp_path: Path):
        ...
        (specialists_dir / "python-expert.md").write_text("""# Python Expert
**Description**: Python programming specialist
**System Prompt**: You are a Python expert.
""")

        (specialists_dir / "data-analyst.md").write_text("""# Data Analyst
**Description**: Data analysis and visualization
**System Prompt**: You analyze data and create visualizations.
**Temperature**: 0.5
""")

        (specialists_dir / "security-auditor.md").write_text("""# Security Auditor
...
""")
        ...
        assert set(registry.list()) == {"python-expert", "data-analyst", "security-auditor"}

        # Verify individual specialists
        assert registry.get("python-expert").description == "Python programming specialist"
    ...
    def test_load_specialists_priority_order(self, tmp_path: Path):
        ...
        (project_dir / "reviewer.md").write_text("""# Reviewer
...
""")
        ...
        (local_dir / "reviewer.md").write_text("""# Reviewer
...
""")
    ...
    def test_load_specialists_with_programmatic_base(self, tmp_path: Path):
        ...
        specialists_dir = tmp_path / ".kaizen" / "specialists"
        specialists_dir.mkdir(parents=True)
        (specialists_dir / "helper.md").write_text("""# Helper
...
""")
    ...
    def test_load_specialists_missing_required_fields(self, tmp_path: Path):
        ...
        # Missing Description
        (specialists_dir / "no-description.md").write_text("""# No Description
**System Prompt**: Missing description field.
""")

        # Missing System Prompt
        (specialists_dir / "no-prompt.md").write_text("""# No Prompt
**Description**: Has description but no system prompt.
""")

        # Valid specialist
        (specialists_dir / "valid.md").write_text("""# Valid
...
""")
    ...
    def test_isolated_mode_skips_filesystem(self, tmp_path: Path):
        ...
        specialists_dir = tmp_path / ".kaizen" / "specialists"
        specialists_dir.mkdir(parents=True)
        (specialists_dir / "should-not-load.md").write_text("""# Should Not Load
...
""")
        ...
        (skill_dir / "pytest-tips.md").write_text("""# Pytest Tips
Use fixtures effectively.
""")

        (skill_dir / "coverage.md").write_text("""# Coverage
...
""")
    ...
    def test_load_skill_without_frontmatter(self, tmp_path: Path):
        ...
        (skill_dir / "SKILL.md").write_text("""# Quick Tips
...
"""
    ...
    def test_load_context_file_priority(self, tmp_path: Path):
        ...
        (project_kaizen / "KAIZEN.md").write_text("# Project Context\nDefault project info.")

        # Create local context
        local_kaizen = tmp_path / ".kaizen-local"
        local_kaizen.mkdir()
        (local_kaizen / "KAIZEN.md").write_text("# Local Context\nCustom local overrides.")
...
class TestLoadAll:
    ...
    def test_load_all_full_structure(self, tmp_path: Path):
        ...
        specialists_dir.mkdir(parents=True)
        (specialists_dir / "analyst.md").write_text("""# Analyst
...
""")
    ...
    def test_load_all_with_all_sources(self, tmp_path: Path):
        ...
        (user_specialists / "global-helper.md").write_text("""# Global Helper
...
""")
        ...
        (project_specialists / "project-helper.md").write_text("""# Project Helper
...
""")
        ...
        (local_specialists / "local-helper.md").write_text("""# Local Helper
...
class TestEdgeCases:
    ...
    def test_malformed_temperature_value(self, tmp_path: Path):
        ...
        specialists_dir = tmp_path / ".kaizen" / "specialists"
        specialists_dir.mkdir(parents=True)
        (specialists_dir / "bad-temp.md").write_text("""# Bad Temp
...
""")
    ...
    def test_malformed_max_tokens_value(self, tmp_path: Path):
        ...
        specialists_dir = tmp_path / ".kaizen" / "specialists"
        specialists_dir.mkdir(parents=True)
        (specialists_dir / "bad-tokens.md").write_text("""# Bad Tokens
...
""")
    ...
    def test_special_characters_in_names(self, tmp_path: Path):
        ...
        (specialists_dir / "code-review_v2.md").write_text("""# Code Review V2
...
""")
    ...
    def test_unicode_content(self, tmp_path: Path):
        ...
        specialists_dir = tmp_path / ".kaizen" / "specialists"
        specialists_dir.mkdir(parents=True)
        (specialists_dir / "multilingual.md").write_text("""# Multilingual Specialist
...
""")
    ...
    def test_symlink_handling(self, tmp_path: Path):
        ...
        actual_dir.mkdir()
        (actual_dir / "linked.md").write_text("""# Linked Specialist
