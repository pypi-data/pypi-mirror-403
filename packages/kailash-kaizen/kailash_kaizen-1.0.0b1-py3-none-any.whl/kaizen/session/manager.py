...
Provides session management for Agentic-OS integration:
...
from .state import Message, SessionState, SessionStatus, SessionSummary, SubagentCall, ToolInvocation
...
class KaizenSessionManager:
    """
    Session manager for Agentic-OS integration.
    ...
    async def resume_session(self, session_id: str) -> SessionState:
        ...
        if state.status != SessionStatus.PAUSED:
            raise ValueError(f"Session {session_id} is not paused (status: {state.status})")
