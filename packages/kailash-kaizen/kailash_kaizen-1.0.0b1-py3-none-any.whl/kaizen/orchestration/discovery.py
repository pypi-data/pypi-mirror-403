"""Agent discovery extensions for Agentic-OS integration.
...
class AgentWithAccess:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "name": getattr(self.metadata.agent, "name", self.metadata.agent.__class__.__name__),
...
class AgentSkillMetadata:
    """Metadata for agent as skill in Agentic-OS UI.

    Provides all information needed to display an agent/skill
    in the Agentic-OS platform UI.
    ...
    def from_agent(
        ...
        input_schema = None
        signature = getattr(agent, "_signature", None) or getattr(agent, "signature", None)
        if signature:
    ...
    def from_specialist_definition(
        ...
        return cls(
            ...
            description=getattr(definition, "description", f"{specialist_name} specialist"),
            ...
            for name, field_value in signature.__class__.__dict__.items():
                if hasattr(field_value, "__class__"):
                    ...
                    if field_class == "InputField":
                        desc = getattr(field_value, "desc", "") or getattr(field_value, "description", "")
                        schema["properties"][name] = {
                            "type": "string",
                            "description": desc,
                        }
                        # Check if required (no default)
                        if not hasattr(field_value, "default") or field_value.default is None:
...
class UserFilteredAgentDiscovery:
    ...
    async def find_agents_for_user(
        ...
        for agent_metadata in agents:
            ...
            if has_access:
                results.append(AgentWithAccess(
            ...
            try:
                ...
                if hasattr(result, "constraints"):
                    if "max_daily_invocations" in result.constraints:
                        constraints.max_daily_invocations = result.constraints["max_daily_invocations"]
                    if "max_tokens" in result.constraints:
                        constraints.max_tokens_per_session = result.constraints["max_tokens"]
    ...
    async def list_skill_metadata(
        ...
        if user_id and organization_id:
            ...
            return [
                AgentSkillMetadata.from_agent(a.agent, a.agent_id)
                for a in agents
            ]
