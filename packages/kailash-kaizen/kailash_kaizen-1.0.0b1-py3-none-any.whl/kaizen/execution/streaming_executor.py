...
Provides the core execution bridge between Kaizen agents and Agentic-OS,
...
class StreamingExecutor:
    ...
    - Progress visualization (TaskGraph in Agentic-OS)
    - Cost attribution and tracking
    - Tool invocation monitoring
    - Subagent spawning events
    - Session state management

    All 10 Agentic-OS event types are emitted at appropriate lifecycle points:
    ...
    async def execute_with_events(
        ...
        *,
        ...
        This is the primary interface for Agentic-OS integration.
    ...
    def _build_inputs(self, agent: Any, task: str) -> Dict[str, Any]:
        ...
        if signature:
            ...
            for name, field_value in signature.__class__.__dict__.items():
                if hasattr(field_value, "__class__") and field_value.__class__.__name__ == "InputField":
    ...
    async def _process_result(
        ...
        for subagent_call in subagent_calls:
            ...
            yield self._emit(
                SubagentSpawnEvent(
                    session_id=session_id,
                    subagent_id=subagent_call.get("subagent_id", f"subagent-{uuid.uuid4().hex[:8]}"),
    ...
    def _extract_output(self, result: Dict[str, Any]) -> str:
        ...
        for field in ["answer", "response", "output", "text", "content", "result"]:
            if field in result:
                value = result[field]
