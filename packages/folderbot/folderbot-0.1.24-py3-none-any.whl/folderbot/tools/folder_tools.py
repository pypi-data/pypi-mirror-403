...
class FolderTools:
    ...
    async def execute_async(
        ...
        """Execute a tool, supporting both sync and async tools."""
        start_time = time.time()

        # Try scheduler tools first (async)
        if self._scheduler_tools:
            try:
                ...
                if result is not None:
                    self._log_tool_call(
                        tool_name, tool_input, result, start_time, user_id
                    )
                    return result
            except Exception as e:
                result = ToolResult(content=f"Scheduler tool error: {e}", is_error=True)
                self._log_tool_call(tool_name, tool_input, result, start_time, user_id)
                return result

        # Fall back to synchronous execution for all other tools
        return self.execute(tool_name, tool_input, user_id=user_id)
