...
class IndexManager:
    ...
    def update(self, memory_id: str, file_path: str, metadata: Dict, content: str, auto_commit: bool = True):
        ...
        # 處理日期欄位：優先使用 captured_at，fallback 到 date
        captured_at = metadata.get('captured_at') or metadata.get('date') or ''
        updated = metadata.get('updated') or metadata.get('date') or ''
