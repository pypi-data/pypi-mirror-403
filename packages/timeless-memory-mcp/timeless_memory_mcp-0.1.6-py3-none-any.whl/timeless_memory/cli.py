...
def cmd_memory_create(args):
    """建立記憶"""
    memory_manager, _, _, _, relation_manager, _ = get_managers()
    ...
    # 自動關聯實體
    if args.entity_ids:
        entity_ids = args.entity_ids.split(',')
        linked = []
        for eid in entity_ids:
            ...
            try:
                ...
                if link_result.get('success'):
                    ...
                    print(f"   ⚠️ 關聯失敗: {eid} - {link_result.get('message', '')}")
            except Exception as e:
                print(f"   ⚠️ 關聯失敗: {eid} - {e}")
        if linked:
            print(f"   🔗 已關聯實體: {', '.join(linked)}")
...
def main():
    ...
    create_parser.add_argument('--title', help='明確標題（選填）')
    ...
    create_parser.add_argument('--entity-ids', dest='entity_ids', help='關聯實體 ID（逗號分隔）')
