...
def memory(
    ...
    # create/update 參數
    content: str = None,
    title: str = None,  # 明確標題（選填，若不提供則自動生成）
    speaker: str = "user",
    authority: int = 3,
    quality: int = 3,
    tags: List[str] = None,
    ...
    category: str = "情節記憶",
    context: str = "",
    entity_ids: List[str] = None,  # 建立時自動關聯的實體 ID 列表
    ...
    Args:
        action: 操作類型
        content: 記憶內容
        title: 明確標題（選填，若不提供則自動從 content 生成）
        ...
        category: 分類
        context: 情境說明
        entity_ids: 建立時自動關聯的實體 ID 列表（如 ["project-教育雲", "person-張三"]）
        memory_id: 記憶 ID
        query: 搜尋關鍵字
        authority_min: 最低權重
        quality_min: 最低品質
    ...
    if action == "create":
        ...
        linked_entities = []
        link_errors = []
        if entity_ids and result.get("id"):
            _, _, _, _, relation_manager, _ = _get_managers()
            for eid in entity_ids:
                try:
                    link_result = relation_manager.link_memory(result["id"], eid, "mentions")
                    if link_result.get("success"):
                        linked_entities.append(eid)
                    else:
                        link_errors.append({"entity_id": eid, "error": link_result.get("message", "連結失敗")})
                except Exception as e:
                    link_errors.append({"entity_id": eid, "error": str(e)})
...
def chat(
    ...
    1. chat(action="list_mentions") → 取得被 @ 提及的中文名/英文名
    2. chat(action="search_mention", space_name="謝承緯") → 取得上下文，判斷對應的 user_id
    3. chat(action="get_user_context", user_id="xxx") → 確認 user_id 的發言內容
    4. entity(action="batch_create", entities=[...]) → 建立實體（包含 chat_id 和別名）
    ...
    try:
        ...
        if action == "sync":
            ...
            return json.dumps({
                "success": False,
                "error": f"未知的 action: {action}",
                "available_actions": ["sync", "download", "convert", "list_spaces", "status", "init_auth", "analyze", "get_user_context", "list_months", "get_month_data", "save_summary", "list_mentions", "search_mention"]
            }, ensure_ascii=False, indent=2)
