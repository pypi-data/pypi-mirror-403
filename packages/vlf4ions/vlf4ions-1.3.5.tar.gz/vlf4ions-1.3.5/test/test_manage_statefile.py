...
from vlf4ions.class_definition import station, receiver
...
def test_write_df():
    ...
    NRK = station("NRK", 56.2, -7.6, 37.5, 22.02, ":10", df=1.4136e-4)
    NAA = station("NAA", 51.6, -33.5, 24.0, 32.39, ":18", df=3.1128e-5)
    stations = [NRK, NAA]
...
def test_manage_bp():
    # This is also where we test the computation of DA and DP
    ...
    # Test computation of DA and DP
    this_receiver = receiver("Test_receiver", 0, 0, threshold=1)
    this_station = station(la_station, 0, 0, 0, 10, "")

    # Update of DA and DP in normal times
    this_station.compute_DA_DP(
        path, la_date, amp=3, phase=[5, 60], this_receiver=this_receiver
    )
    DP2 = this_station.DP
    DA2 = this_station.DA

    # First condition, first return to quiet

    # Write new breakpoint
    ms.write_newbreakpoint(la_station, la_date, path, 3.5, 3, 3, 3, 3, 3)
    this_station.compute_DA_DP(
        path, la_date, amp=3, phase=[5, 60], this_receiver=this_receiver
    )
    DP3 = this_station.DP
