...
class station:
    ...
    def __init__(
        ...
        self.orientation = (
            orientation  # 1 to look for the E/W phase, 0 for the N/S (default 1)
        )
    ...
    def update_breakpoints(
        ...
        try:
            ...
            amp_NS, amp_EW, phase_NS, phase_EW, the_time, nb_error = (
                rf.read_Narrowband_real_time(self, path_mau, this_receiver)
            )
            ...
            flag, nb_data_points, amp = fl.flags(
                amp_NS, amp_EW, self.nb_data_points, today, path_sunrise, this_receiver
            )
            self.nb_data_points = nb_data_points
            self.flag = flag
        ...
        if (
            this_receiver.nb_error == 5
        ):  # it's been five minutes since the antenna stopped or had problems
