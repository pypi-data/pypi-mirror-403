...
def test_flags():
    ...
    flag_1, data_length_1, data_1 = flags(
        amp_NS, amp_EW, previous_nb_points, today, path_to_files, Nancay, phase
    )

    # Test random phase
    phase_disturbed = np.arange(1, 400, 30)
    flag_phase, data_length_phase, data_phase = flags(
        amp_NS, amp_EW, previous_nb_points, today, path_to_files, Nancay, phase_disturbed
    )
    ...
    flag_2, data_length_2, data_2 = flags(
        amp_NS, amp_EW, data_length_1, today, path_to_files, Nancay, phase
    )
    ...
    flag_3, data_length_3, data_3 = flags(
        amp_NS, amp_EW, previous_nb_points, night, path_to_files, Nancay, phase
    )
