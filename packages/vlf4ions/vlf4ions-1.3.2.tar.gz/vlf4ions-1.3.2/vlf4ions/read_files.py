...
def read_Narrowband_postprocessing(la_date, la_station, path): # For the moment, only takes the calibration after 2023_05_16
    ...
    :param la_date: Date when the files need to be read ()"""
...
def read_Narrowband_real_time(station, path): 
    ...
    # amp_NS
    list_of_files = glob.glob(path+'*'+station.name+'_100A.mau')
    latest_file = max(list_of_files, key=os.path.getctime)
    ...
    # phase_EW
    list_of_files = glob.glob(path+'*'+station.name+'_101B.mau')
    latest_file = max(list_of_files, key=os.path.getctime)
    ...
    try :
        ...
        # If there is a pb with the phase_EW files, a solution is to take the phase_NS ones
        # phase_NS
        list_of_files = glob.glob(path+'*'+station.name+'_100B.mau')
        latest_file = max(list_of_files, key=os.path.getctime)
            #fname = shutil.copy2(f, pathdest)
        try :
            data = read_mat(latest_file)
    ...
    # amp_EW
    list_of_files = glob.glob(path+'*'+station.name+'_101A.mau')
    latest_file = max(list_of_files, key=os.path.getctime)
        #fname = shutil.copy2(f, pathdest)
    try :
        data = read_mat(latest_file)
