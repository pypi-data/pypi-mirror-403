...
def test_analyse_bp():
    ...
    quiet_now, prev_timedecay, DP_now, DA = acp.analyse_breakpoint(current_time, current_p1, amp, phase, the_time, quiet, quiet_phase, quiet_amp, last_time, quiet_p1, detection_threshold, quiet_p1, quiet_breakpoint)
    ...
    current_p1 = 5
    current_time = the_time[-1]
    quiet_now, prev_timedecay, DP_now, DA = acp.analyse_breakpoint(current_time, current_p1, amp, phase, the_time, quiet, quiet_phase, quiet_amp, last_time, last_p1, detection_threshold, quiet_p1, quiet_breakpoint)
    if quiet_now == 1 or DP_now != 1:
    ...
    quiet_now, prev_timedecay, DP_now, DA = acp.analyse_breakpoint(current_time, current_p1, amp, phase, the_time, quiet, quiet_phase, quiet_amp, last_time, last_p1, detection_threshold, quiet_p1, quiet_breakpoint)
    if quiet_now == 1:
        errors.append('Problem with breakpoint analysis for flare decay: declared as quiet')
   ...
   # Prepare for test 4
    ...
    if quiet_now == 1:
        errors.append('Problem with breakpoint analysis for flare decay update: declared as quiet')
    if prev_timedecay != 8.5:
        print(prev_timedecay)
        errors.append('Problem with breakpoint analysis for flare decay update: decay time')
   ...
   # Prepare for test 5
    ...
    quiet_now, prev_timedecay, DP_now, DA = acp.analyse_breakpoint(current_time, current_p1, amp, phase, the_time, quiet, quiet_phase, quiet_amp, last_time, last_p1, detection_threshold, quiet_p1, quiet_breakpoint)
    if DP_now != 0:
        errors.append('Problem with bp analysis - quiet after quiet, DP not zero if slope is negative')
    if quiet_now == 0:
        errors.append('Problem with bp analysis - does not go stay quiet if new slope is negative')
