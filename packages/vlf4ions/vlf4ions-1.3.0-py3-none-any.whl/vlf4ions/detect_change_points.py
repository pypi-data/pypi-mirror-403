...
def find_break_point_realtime(the_time, data, delta = 0.1):
    ...
    delta: in percent/100 (default : 0.1). This is the threshold over which we 
    ...
    if L >  9: # At least 9 points are necessary for this
        ...
        likelihood_with_bp, p1 = _compute_likelihood_criteria_several_change_points(the_time, data, np.array([0, L-6, L-1]))
        
        # Determine whether there is a new breakpoint
        detec = (likelihood_without_bp - likelihood_with_bp)/likelihood_without_bp > delta
...
def detect_change_points(the_time, data):
    """Based on Guralnik, V. & Srivastava, J. (1999)
    Detect change_points in the 'data' array """
    
    # Initialisation
    change_points = np.array([0, len(data)])
    likelihood, dummy = _compute_likelihood_criteria(change_points[0], change_points[1], the_time, data)
    last_likelihood = likelihood
    stopping_criterion = 1
    threshold = 0.03 # Threshold can be changed. A lower threshold will give more change_points, but will also take longer
    ...
    while stopping_criterion > threshold and loop_counter < 1e3:
        ...
        for s in range(0, N):
            ...
            if change_points[s + 1] - change_points[s] > 5:
                new_candidate = _find_candidate_detection(change_points[s], change_points[s + 1], the_time, data)
                temp_change_points = np.insert(change_points, s + 1, new_candidate)
                new_likelihood_criterion = _compute_likelihood_criteria_several_change_points(the_time, data, temp_change_points)
...
def _compute_likelihood_criteria_several_change_points(the_time, data, change_points):
    ...
    Computes the total likelihood criterion for data, knowing that there are change_points at specified locations
    
    change_points is a list of indices of the changepoints already computed. 
    The first index is 0 (for the beggining of the 'data' array), the last is the number of elements in the 'data' array."""
    
    N = len(change_points)-1
    total_L = 0
    
    for k in range(0, N):
        likelihood_criteria, a = _compute_likelihood_criteria(change_points[k], change_points[k+1], the_time, data)
...
def _find_candidate_detection(i, j, the_time, data):
    ...
    'the_time' is the corresponding time array and must have the same length as 'data'
    
    Returns the indice in the array data (! not data[i:j]!) of this best candidate.
    """
    ...
    for k in range(i+2, j-2):
        ...
        likelihood_first_segment, dummy = _compute_likelihood_criteria(i, k, the_time, data)
        likelihood_second_segment, dummy = _compute_likelihood_criteria(k+1, j, the_time, data)
...
def _compute_likelihood_criteria(i, j, the_time, data):
    ...
    i is the index of the start of the segment of interest in the 'the_time' and 'data' arrays. 
    j is the index of the end of this segment.
    'the_time' and 'data' must have the same length"""
    
    m = j - i + 1 #Number of elements in the the_time & data arrays
    
    a, b = np.polyfit(the_time[i:j+1], data[i:j+1], 1) # Use a linear model to fit the segment. THIS CAN BE CHANGED !
