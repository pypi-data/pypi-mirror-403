...
def compute_df(date_inf, date_sup, station, path_to_files, file_endings, epsilon):
    ...
    while eps>epsilon: # This is the bissection method
        ...
        eps = _loop_for_compute_df(nb_days, date_inf, station, path_to_files, file_endings)
...
def _loop_for_compute_df(nb_days, date_inf, station, path_to_files, file_endings):
    ...
    for i in range(nb_days):
        ...
        try:
            amp_NS, amp_EW, phase_NS, phase_EW, the_time = rf.read_Narrowband_postprocessing(la_date, station.name, path_to_files, file_endings)
    ...
    all_jumps = all_jumps/(days_with_data - 1) # There are nb_days - 1 midnight crossings
