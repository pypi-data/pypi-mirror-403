...
class receiver:
    ...
    def __init__(self, name, lat, lon, threshold=0.5, nb_error=0, file_endings = ['_100A', '_100B', '_101A', '_101B']):
        self.name = name
        self.lat = lat
        self.lon = lon
    ...
    def craft_newsletter(self, filename, completenamegiven=False):
        ...
        for i in range(1, len(flux)):
            ...
            if quiet[i] < 1 and quiet[i-1] == 1: # Flare beginning
                ...
                i_start = i
            elif (quiet[i] == 1 and quiet[i-1] < 1) or (quiet[i] < 1 and i == len(flux)): # Flare ending
        ...
        body += "\n"
        body += 'From ' + str(the_time[0]) + ' to ' + str(the_time[-1]) + ' UT \n'

        for f in range(nb_flares):
            body += 'Flare ' + str(f) + ' : \n'
            body += '-------------------- \n'
            body += 'Start time : ' + str(starts[f]) + ' UT \n'
            body += 'Max flux : ' + str(max_flux[f]) + ' W/m^2 at ' + str(max_times[f]) + ' UT \n'
            body += 'End time : ' + str(ends[f]) + ' UT \n \n'
...
class station:
    ...
    '''
    def __init__(self, name, lat, lon, freq, detection_threshold, reading_time, Cal_NS = 0.14, Cal_EW = 0.08, df = 0, time_average = 60, delta = 0.1, nb_data_points = 0, nb_error = 0, Ne=1e8, DP=0, DA=0, p1=0, quiet=1, flag=0, mu=-6, sigma = 1, orientation=1):
        ...
        self.orientation = orientation # 1 to look for the E/W phase, 0 for the N/S (default 1)
    ...
    def update_df(self, this_receiver, path_to_data, epsilon=5, nb_days=30):
        ...
        # Compute the new df
        df = correct_phase.compute_df(date_inf, date_sup, self, path_to_data, this_receiver.file_endings, epsilon)

        # Update the df value
        self.df = df

    def update_breakpoints(self, path_breakpoint, path_mau, path_sunrise, alerts_antenna, alerts_detection, this_receiver):
        ...
        try:
            ...
            amp_NS, amp_EW, phase_NS, phase_EW, the_time, nb_error = rf.read_Narrowband_real_time(self, path_mau, this_receiver)
            ...
            flag, nb_data_points, amp = fl.flags(amp_NS, amp_EW, self.nb_data_points, today, path_sunrise, this_receiver)
            self.nb_data_points = nb_data_points
            self.flag = flag
            ...
            if flag == 1: # The antenna has stopped
                ...
                if the_time[-1] - the_time[0] > 2:
                    two_hours = int(np.floor(7200/self.time_average)) # Number of indices in those two hours
                    ...
                    msf.write_newbreakpoint(self.name, la_date, path_breakpoint, the_time[-1], p1, amp, phase[-1], 1, 0)
                    ...
                    if detec:
                        ...
                        if self.quiet == 1 and p1 > 1000:
                            print('Breakpoint due to bad correction of phase')
                            self.flag = 5
                            quiet_now = 1
                            DP = 0
                            DA = 0
        ...
        if this_receiver.nb_error == 5: # it's been five minutes since the antenna stopped or had problems
    ...
    def get_pdf_coeff(self, path_to_probas):
        ...
        if self.DP > 5: # Only do it for high DP, the other ones may be due to small noise
