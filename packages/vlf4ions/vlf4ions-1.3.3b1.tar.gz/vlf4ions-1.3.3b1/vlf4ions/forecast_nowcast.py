...
class nowcast:
    ...
    def __init__(self, stations, receiver, alerts, reading_time=':00', ten=6.6e-06, fifty=2.2e-06, seventyfive=1.4e-06, debug=0):
    ...
    def run(self, path_to_probas, path_to_results, path_to_CSVfiles): # Compute flux estimate
        ...
        ten, fifty, seventyfive, best_guess = plot_probas(probas, flux, path_to_results, today, stations_on)
        pm.plot_map(self.stations, self.receiver, today, path_to_results)

        if nb_stations_on > 1: # If there is no station, or only one station working, no alerts are sent
            ...
            ms.write_fluxestimate(best_guess, quiet, today, path_to_results, 'nowcast', nb_stations_on)
            if self.debug == 2:
                print(quiet)
                print(best_guess)
            ...
            for alert in self.alerts:
                ...
                if alert.threshold > 0:
                    ...
                    elif alert.send_each_change and (np.abs(ten - self.ten)/self.ten > 0.1 or np.abs(fifty - self.fifty)/self.fifty > 0.1 or np.abs(seventyfive - self.seventyfive)/self.seventyfive > 0.1) and self.nb_stations_on == nb_stations_on and quiet < 1 and ten > alert.threshold:
