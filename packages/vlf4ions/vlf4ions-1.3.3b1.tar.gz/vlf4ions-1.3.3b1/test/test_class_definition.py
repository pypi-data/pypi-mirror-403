...
def test_craft_newsletter():
    ...
    # Define alert
    test = cd.email_alerts('Test', 'Placeholder', 'fakesender', 'fakepassword', 'fakerecipients')

    path = os.path.join(dirname, 'Test_flux.csv')
    flux, times, starts, ends = test.craft_newsletter(path, True)

    assert(test.body.count('\n') == 15 and flux == [2, 5] and times == [2, 6] and starts == [1, 5] and ends == [3, 7])
