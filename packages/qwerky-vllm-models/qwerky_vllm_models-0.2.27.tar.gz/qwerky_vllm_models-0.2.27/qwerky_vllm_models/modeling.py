...
# =============================================================================
# MAMBAINLLAMA MAMBA MIXER (Native vLLM ops with proper CustomOp integration)
# =============================================================================

def _create_mamba_mixer_class():
    """Factory function to create MambaInLlamaMambaMixer with proper vLLM integration.

    Uses @CustomOp.register decorator + MambaBase inheritance for:
    1. Proper state management (vLLM allocates and binds kv_cache)
    2. CUDA graph compatibility (state persists across graph executions)
    3. Correct callability (CustomOp handles __call__ dispatch)
    """

    if _MambaBase is not None and _CustomOp is not None:
        # Full vLLM integration with MambaBase + CustomOp
        @_CustomOp.register("mambainllama_mixer")
        class MambaInLlamaMambaMixerVLLM(_MambaBase, _CustomOp):
            """MambaInLlama Mamba mixer with full vLLM V1 integration.

            This class properly integrates with vLLM's state management:
            - Inherits from MambaBase (discoverable by vLLM for state allocation)
            - Inherits from CustomOp (proper callability with forward dispatch)
            - Uses @CustomOp.register for the op registry
            ...
            def __init__(
                ...
                # Placeholder for vLLM-managed state (will be populated by bind_kv_cache)
            ...
            def get_state_shape(self) -> tuple[tuple[int, ...], tuple[int, ...]]:
                ...
                # vLLM expects: conv_state (dim, conv_kernel-1), ssm_state (dim, d_state)
            ...
            # =================================================================
            # CustomOp interface methods (required for proper forward dispatch)
            # =================================================================

            def forward(self, hidden_states: torch.Tensor, output: torch.Tensor):
                """Forward dispatch - called by CustomOp.__call__."""
            ...
            def forward_native(self, hidden_states: torch.Tensor, output: torch.Tensor):
                """Native forward - fallback when CUDA not available."""
                self.forward_cuda(hidden_states, output)

            def forward_cuda(self, hidden_states: torch.Tensor, output: torch.Tensor):
                """CUDA forward with vLLM native ops and state management."""
                # Get state and metadata from ForwardContext
                forward_context = get_forward_context()
                attn_metadata = None
                state_indices = None

                if forward_context is not None and hasattr(forward_context, 'attn_metadata'):
                    fc_metadata = forward_context.attn_metadata
                    if isinstance(fc_metadata, dict) and self.prefix in fc_metadata:
                        attn_metadata = fc_metadata[self.prefix]
                    elif fc_metadata is not None:
                        attn_metadata = fc_metadata
                ...
                # Get conv_state and ssm_state from kv_cache (populated by vLLM)
                conv_state = self.kv_cache[0] if len(self.kv_cache) > 0 else None
                ssm_state = self.kv_cache[1] if len(self.kv_cache) > 1 else None

                # Run the actual computation
                result = self._forward_impl(hidden_states, conv_state, ssm_state, state_indices, attn_metadata)
                output.copy_(result)
            ...
            def _forward_impl(
                ...
                if _mamba_ops_available and conv_state is not None and ssm_state is not None:
                    return self._forward_with_vllm_ops(
                        x, z, B, C, dt, conv_state, ssm_state, state_indices
                    )
                else:
            ...
            def _forward_with_vllm_ops(
                ...
                if is_decode and state_indices is not None:
                    ...
                    x_t = rearrange(x, "t d -> 1 d t") if x.dim() == 2 else rearrange(x, "b t d -> b d t")
                    ...
                    dt_t = rearrange(dt, "... d -> ... d 1") if dt.dim() == 2 else dt
                    if dt_t.dim() == 2:
                        dt_t = dt_t.unsqueeze(0)
                    dt_t = rearrange(dt_t, "b t d -> b d t")

                    B_t = rearrange(B, "t h d -> 1 h d t") if B.dim() == 3 else rearrange(B, "b t h d -> b h d t")
                    C_t = rearrange(C, "t h d -> 1 h d t") if C.dim() == 3 else rearrange(C, "b t h d -> b h d t")
                    z_t = rearrange(z, "t d -> 1 d t") if z.dim() == 2 else rearrange(z, "b t d -> b d t")
                    ...
                    y = rearrange(y, "b d t -> b t d") if y.dim() == 3 else y
                    if y.shape[0] == 1:
                        y = y.squeeze(0)
