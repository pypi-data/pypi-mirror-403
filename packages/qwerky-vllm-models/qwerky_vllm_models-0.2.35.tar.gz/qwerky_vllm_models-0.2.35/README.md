...
- Added extensive V1-specific debug logging to diagnose state retrieval
- This matches how vLLM's native MambaMixer retrieves state in V1 architecture

### 0.2.34
- **DEBUG**: Added logging to diagnose gibberish output issue
- Logs: attn_metadata type, mamba_metadata availability, state_indices
- Logs: conv_state/ssm_state shapes, which forward path is used
- Fix: Look for state_indices in attn_metadata.mamba_metadata (vLLM location)
