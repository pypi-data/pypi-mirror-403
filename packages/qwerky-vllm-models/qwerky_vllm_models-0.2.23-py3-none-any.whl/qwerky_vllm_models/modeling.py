...
class MambaInLlamaMambaMixer(nn.Module):
    ...
    while using vLLM's native Triton-accelerated ops for performance.
    ...
    def forward(
        ...
        Args:
            ...
            conv_state: (batch, conv_dim, d_conv) - conv state cache
            ssm_state: (batch, num_C_head, d_state, d_inner//num_C_head) - SSM state
        ...
        if is_decode:
            ...
            return self._decode_step(x, z, B, C, dt, A, conv_state, ssm_state)
        else:
            # Prefill path - full sequence
            return self._prefill(x, z, B, C, dt, A, conv_state, ssm_state, seqlen)
    ...
    def _prefill(
        ...
        if conv_state is not None:
            conv_state.copy_(F.pad(x, (self.d_conv - x.shape[-1], 0))[:, :, -self.d_conv:])
        ...
        y = self._ssm_scan(x, dt, A, B, C, z, ssm_state)
    ...
    def _decode_step(
        ...
        # SSM state update
        y = self._ssm_state_update(x, dt, A, B, C, z, ssm_state)
    ...
    def _ssm_scan(
        ...
        Args:
            ...
            ssm_state: (batch, num_C_head, d_state, head_dim)
        """
        batch, dim, seqlen = u.shape
        orig_dtype = u.dtype

        # Apply softplus to delta (bias already applied in dt_proj)
        ...
        if ssm_state is not None:
            ssm_state.copy_(rearrange(state, "b h d n -> b h n d"))
    ...
    def _ssm_state_update(
        ...
        # Apply softplus to dt (bias already applied in dt_proj)
        dt = F.softplus(dt)
...
class MambaDecoderLayer(nn.Module):
    ...
    def __init__(self, config: MambaInLlamaMambaConfig, layer_idx: int):
        ...
        self.mamba = MambaInLlamaMambaMixer(config, layer_idx)
    ...
    def forward(
        ...
        residual = hidden_states
        hidden_states = self.input_layernorm(hidden_states)
        hidden_states = self.mamba(hidden_states, conv_state, ssm_state, cache_position)
        hidden_states = residual + hidden_states
...
class MambaInLlamaMambaModel(nn.Module):
    ...
    def __init__(self, config: MambaInLlamaMambaConfig):
        super().__init__()
        self.config = config
        ...
        for layer_idx in range(config.num_hidden_layers):
            if layer_idx in config.attn_layers:
                ...
                self.layers.append(MambaDecoderLayer(config, layer_idx))
    ...
    def forward(
        ...
        for i, layer in enumerate(self.layers):
            if isinstance(layer, MambaDecoderLayer):
                ...
                hidden_states = layer(hidden_states, conv_state, ssm_state, cache_position)
            else:
