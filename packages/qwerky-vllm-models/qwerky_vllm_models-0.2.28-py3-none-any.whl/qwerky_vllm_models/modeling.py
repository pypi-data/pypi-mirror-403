...
def _create_mamba_mixer_class():
    ...
    Uses MambaBase inheritance for:
    ...
    3. Discovery by vLLM's state allocation system (via AttentionLayerBase)
    """

    if _MambaBase is not None:
        # Full vLLM integration with MambaBase (for state allocation discovery)
        class MambaInLlamaMambaMixerVLLM(_MambaBase):
            ...
            - vLLM allocates state and binds via bind_kv_cache() -> self.kv_cache
            - State persists across CUDA graph executions
            ...
            def forward(
                ...
                **kwargs,
                ...
                3. ForwardContext.attn_metadata
                """
                state_indices = None
                ...
                # Also check ForwardContext if state still not available
                if (conv_state is None or ssm_state is None) and get_forward_context is not None:
                    forward_context = get_forward_context()
                    if forward_context is not None:
                        # Try to get from ForwardContext's attn_metadata
                        if hasattr(forward_context, 'attn_metadata'):
                            fc_metadata = forward_context.attn_metadata
                            if fc_metadata is not None:
                                if state_indices is None and hasattr(fc_metadata, 'state_indices_tensor'):
                                    state_indices = fc_metadata.state_indices_tensor
