...
class OpenAIProvider(AIProvider):
    ...
    def __init__(self, api_key: str, model: str = "gpt-4"):
        ...
        if not OPENAI_AVAILABLE:
            raise AIServiceError("OpenAI package not installed. Install with: pip install openai")
    ...
    def _get_system_prompt(self, request: AIRequest) -> str:
        ...
        system_prompt = f"""You are an expert software developer and AI assistant for DevOS.
...
- Be concise but thorough"""
    ...
    def _get_chat_system_prompt(self, request: AIRequest) -> str:
        ...
        return f"""You are DevOS AI, a helpful development assistant.
...
Be thorough and provide working solutions."""
    ...
    def _handle_error(self, error: Exception) -> None:
        ...
        if "authentication" in error_str or "unauthorized" in error_str:
            raise AuthenticationError(f"OpenAI authentication failed: {error}")
        elif "quota" in error_str or "limit" in error_str:
            raise QuotaExceededError(f"OpenAI quota exceeded: {error}")
        elif "rate" in error_str:
            raise AIServiceError(f"OpenAI rate limit exceeded: {error}")
        else:
            raise ProviderError(f"OpenAI provider error: {error}")
