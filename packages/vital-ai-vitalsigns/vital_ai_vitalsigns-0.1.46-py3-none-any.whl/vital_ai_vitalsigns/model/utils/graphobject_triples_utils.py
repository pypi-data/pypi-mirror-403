...
class GraphObjectTriplesUtils:
    ...
    def from_triples_impl(cls, triples: Generator[Tuple, None, None], *, modified=False) -> G:
        ...
        # Group triples by predicate to handle multi-value properties
        ...
        for subject, predicate, obj_value in generated_triples:
            ...
            if predicate_str == VitalConstants.uri_prop_uri:
                continue
            predicate_groups[predicate_str].append(obj_value)
        
        # Process each predicate group
        for predicate, obj_values in predicate_groups.items():
            ...
            if multiple_values:
                # Multi-value property - collect all values (even if just one)
                value_list = []
                for obj_value in obj_values:
                    if isinstance(obj_value, Literal):
                        value_list.append(obj_value.toPython())
                    elif isinstance(obj_value, URIRef):
                        value_list.append(str(obj_value))
                    else:
                        value_list.append(obj_value)
                setattr(graph_object, predicate, value_list)
            else:
                # Single-value property - use the first/only value
