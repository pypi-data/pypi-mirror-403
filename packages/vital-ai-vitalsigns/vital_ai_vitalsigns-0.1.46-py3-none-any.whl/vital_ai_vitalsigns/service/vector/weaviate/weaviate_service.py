...
from vital_ai_vitalsigns.config.vitalsigns_config import EmbeddingModelConfig, CollectionConfig, VectorDatabaseConfig
...
class WeaviateVectorService(VitalVectorService):
    ...
    def __init__(self, config: VectorDatabaseConfig, **kwargs):
        # Extract configuration values from the vector database config
        self.rest_endpoint = config.rest_endpoint
        self.rest_port = config.rest_port
        self.rest_ssl = config.rest_ssl
        self.rest_api_key = config.rest_api_key
        self.grpc_endpoint = config.grpc_endpoint
        self.grpc_port = config.grpc_port
        self.vector_endpoint = config.vector_endpoint
        self.vector_port = config.vector_port
        self.vector_api_key = config.vector_api_key
        
        # Build endpoint URL from components
        if self.rest_endpoint:
            protocol = "https" if self.rest_ssl else "http"
            port_suffix = f":{self.rest_port}" if self.rest_port not in [80, 443] else ""
            self.endpoint = f"{protocol}://{self.rest_endpoint}{port_suffix}"
        else:
            self.endpoint = None
            
        # Use rest_api_key as the primary API key for backward compatibility
        self.api_key = self.rest_api_key
        
        # Extract additional config properties
        self.schema_list = config.vector_database_schema_list
