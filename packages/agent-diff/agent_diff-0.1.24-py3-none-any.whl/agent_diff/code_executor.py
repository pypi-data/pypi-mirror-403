...
def process(data, expr):
    ...
    if '|' in expr:
        ...
        agg = ('length', 'keys', 'type', 'first', 'last', 'min', 'max', 'add', 'unique', 'reverse', 'sort')
        is_agg = any(right_stripped == f or right_stripped.startswith(f + '(') for f in agg)
        # Don't auto-iterate for array access, aggregate functions, or iteration syntax
    ...
    # Handle select(condition) - basic support
...
def create_openai_tool(executor: BaseExecutorProxy):
    ...
    if isinstance(executor, PythonExecutorProxy):
        ...
        def execute_bash(code: str) -> str:
            ...
            Available: curl (auto-proxied), jq (built-in), standard unix tools.
            Example: curl -s https://api.example.com | jq '.data[0].name'

            $WORKSPACE is a persistent directory - files survive between calls.
            """
