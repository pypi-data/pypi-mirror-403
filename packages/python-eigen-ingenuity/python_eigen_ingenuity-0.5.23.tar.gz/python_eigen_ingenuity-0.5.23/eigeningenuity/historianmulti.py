...
class HistorianMulti(object):
    ...
    def getAggregates(
        ...
        """Get aggregates/statistics for one or more tags over a time range

        Args:
            tags: A datatag, or list of datatags
            start: Start of the time range. Accepts epoch milliseconds, python datetime objects, or strings of format dd-mm-YYYYTHH:MM:DD
            end: End of the time range. Accepts epoch milliseconds, python datetime objects, or strings of format dd-mm-YYYYTHH:MM:DD
            intervals (Optional): Number of equal time intervals to split the time range into. One aggregate will be returned per interval. Defaults to 1
            aggregates (Optional): A string, or list of strings of the aggregates to return. Allowed values are: "FIRST", "AVG", "NUMGOOD", "MAX", "LAST", "VAR", "MEDIAN", "NUMBAD", "MIN", "COUNT", "STDDEV". Defaults to all aggregates.
            output (Optional): The format in which to return the data. Accepts one of:
                ...
                - "df" - A formatted pandas dataframe object. Defaults to "json".
        Returns:
    ...
    def getAggregateIntervals(
        ...
        """Get aggregates/statistics for one or more tags, split into specified time intervals, over a time range

        Args:
            tags: A datatag, or list of datatags
            start: Start of the time range. Accepts epoch milliseconds, python datetime objects, or strings of format dd-mm-YYYYTHH:MM:DD
            end: End of the time range. Accepts epoch milliseconds, python datetime objects, or strings of format dd-mm-YYYYTHH:MM:DD
            interval_size (Optional): Duration of each sub time interval to calculate aggregates for. Format like e.g. ["1s", "1m", "1h", "1w", "1y"]. Defaults to the entire time range
            final_interval_behavior (Optional): Behavior for the final interval if it would extend beyond the total window. Options include "INCLUDE", "EXCLUDE", etc. Defaults to "EXCLUDE".
            aggregates (Optional): A string, or list of strings of the aggregates to return. Allowed values are: "FIRST", "AVG", "NUMGOOD", "MAX", "LAST", "VAR", "MEDIAN", "NUMBAD", "MIN", "COUNT", "STDDEV". Defaults to all aggregates.
            output (Optional): The format in which to return the data. Accepts one of:
                - "raw" - The raw json returned by the API
                - "json" - A processed version of the json response
