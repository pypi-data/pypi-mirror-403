...
class Client:
    ...
    def __init__(self, project, credentials=None):
        ...
        self.bigquery_client = bigquery.Client(project=project, credentials=credentials)
    ...
    def download_table(self, table, local_folder='.', separator=',', print_header=True):
        ...
        transfer_client = transfer.Client(project=self.project, credentials=self.credentials)
        ...
        for blob in blobs:
            ...
            full_local_path = os.path.join(local_folder, blob.name)
    ...
    def run_query(self, query=None, query_file_name=None, params=(), destination=None, write_preference='empty',
                  partition_date=None, partition_field=None, clustering_fields=None, suppress_query_output=False):
        ...
        Args:
            ...
            clustering_fields (List[str], Optional): The field(s) on which the destination table is clustered. The field must be a top-level field. Can cluster on at most 4 fields.
            suppress_query_output (bool, Optional): True to suppress the query output otherwise False. Defaults to :data:`False`.
        ...
        if not suppress_query_output:
            ...
            logs.client.logger.info('Writing query results into {}'.format(destination))
