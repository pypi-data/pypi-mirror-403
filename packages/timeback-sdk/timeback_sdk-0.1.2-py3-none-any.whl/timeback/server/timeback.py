...
def _validate_config(app_config: AppConfig) -> None:
    ...
    for i, course in enumerate(courses):
        ...
        if not course_sensor and not top_sensor:
            identifier = (
...
async def create_server(config: TimebackConfig) -> TimebackInstance:
    ...
    # Resolve env-specific overrides for each course
    resolved_courses = [
        _resolve_course_for_env(course, config.env) for course in raw_app_config.courses
    ]
    app_config = AppConfig(
