...
class OIDCClient:
    ...
    async def get_authorization_url(
        ...
        *,
        ...
        Note: state is always base64url-encoded JSON to match TS SDK behavior.
        If state is None, defaults to empty dict {} so callback always decodes
        to an object (never None).
        """
        config = await self._get_config()
        auth_endpoint = config["authorization_endpoint"]

        # Always encode state as base64url JSON to match TS SDK behavior.
    ...
    def decode_state(self, state: str) -> Any:
        ...
        Returns decoded object, or None if decoding fails (matching TS SDK behavior
        where tryDecodeState returns undefined on failure).
        """
        try:
