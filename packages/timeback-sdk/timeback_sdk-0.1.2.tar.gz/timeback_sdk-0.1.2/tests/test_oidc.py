"""Tests for OIDC client (matching TS SDK behavior)."""
...
class TestOIDCStateHandling:
    """Tests for OIDC state encoding/decoding (matching TS SDK behavior).

    Key behavior (matching TS SDK tryDecodeState):
    - Valid base64url JSON decodes to the original object
    - Invalid/corrupt state returns None (TS returns undefined)
    - Sign-in handler defaults state to {} before encoding

    See TS SDK identity-full.ts:72-79 for reference:
        function tryDecodeState<TState>(stateParam: string): TState | undefined {
            try { return decodeBase64Url(stateParam) as TState }
            catch { ssoLog.warn('Failed to decode state'); return undefined }
        }
    """
    ...
    def test_decode_state_with_invalid_string_returns_none(self) -> None:
        """Invalid state should return None (matches TS tryDecodeState returning undefined)."""
        oidc = OIDCClient(issuer="https://example.com", client_id="test", client_secret="secret")
    ...
    def test_decode_state_with_random_token_returns_none(self) -> None:
        """Random CSRF token should return None (matches TS undefined behavior)."""
        oidc = OIDCClient(issuer="https://example.com", client_id="test", client_secret="secret")
