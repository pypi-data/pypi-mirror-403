...
Course Resolution:
- resolve_activity_course — Resolves a course selector to a config entry.
...
def resolve_activity_course(
    ...
    - **Code-based**: CourseCodeRef(code) — matches course_code

    Args:
        courses: Configured courses from timeback.config.json
        course_ref: Course selector from the client payload

    Returns:
        Matched course config entry

    Raises:
        ActivityCourseResolutionError: When selector is unknown or ambiguous
    """
    from ...shared.types import CourseCodeRef, SubjectGradeCourseRef
    ...
    if isinstance(course_ref, SubjectGradeCourseRef):
        ...
        # Code-based: match by course_code
