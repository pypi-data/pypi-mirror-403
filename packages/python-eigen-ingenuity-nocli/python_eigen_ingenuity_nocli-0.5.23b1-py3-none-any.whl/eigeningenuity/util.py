...
def _reformat_multi_response(original_dict, is_aggregate=False):
    if is_aggregate:
    ...
    for key, value in original_dict.items():
        if type(value) == list:
            ...
            data_points = (
                value.get("dataPoints", None) or value.get("dataPoint", None) or value
            )
...
def format_output(
    ...
    if is_aggregate:
        items = {}
        aggregates_lower = [agg.lower() for agg in aggregates]
        for tag, data in response["results"].items():
            items[tag] = []
            for agg in data["aggregates"]:
                filtered_agg = {
                    k: v
                    for k, v in agg.items()
                    if k.lower() in ["from", "to", "tag", *aggregates_lower]
                }
                items[tag].append(filtered_agg)
    elif not timestamps:
    ...
    if output == "raw":
        ...
        if len(tags) == 1 and (not timestamps or len(timestamps) == 1):
            return _reformat_multi_response(items[tags[0]], is_aggregate)
        else:
            return _reformat_multi_response(items, is_aggregate)
    elif output == "string":
        return json.dumps(items, indent=4)
    elif output == "df":
        if is_aggregate:
            return aggregate_response_to_df(items)
        else:
...
def aggregate_response_to_df(items):
    rows = []
    for tag, data in items.items():
        data = force_list(data)
        for entry in data:
            rows.append({"tag": tag, **entry})
