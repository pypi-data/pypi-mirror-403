...
def _build_ravel_component(in_xyz, limitsize=None):
    ...
    if limitsize is not None:
        ...
        if skipper > 1:
            print(
                "Warning, source data too big ({0}/{1}),\n "
                "sub sampling every {2}th element.".format(
                    input_size, limitsize, skipper
                )
            )
    out_xyz = np.stack(
        (
            np.ravel(np.take(in_xyz, 0, axis=-1))[::skipper],
...
def _interpolate_var(in_val, index, inv_dist):
    """
    interpolate nP_array of source intour a np.arry of target
    """

    estimate = np.sum(
        inv_dist * np.reshape(in_val[np.ravel(index)], np.shape(inv_dist)), axis=1
    )
...
def cloud2cloud(
    source_xyz, source_val, target_xyz, stencil=3, limitsource=None, power=1.0, tol=None
):
    """
    ...
    index, dists, inv_dist, skipper = _compute_dists(
        source_xyz, target_xyz, limitsource, stencil, power
    )
    target_val = {}
    for key in source_val:
        source = source_val[key]

        if stencil > 1:
            estimate = _interpolate_var(
                np.ravel(source)[::skipper],
                index,
                inv_dist,
            )
        else:
            estimate = np.ravel(source)[::skipper][index]
