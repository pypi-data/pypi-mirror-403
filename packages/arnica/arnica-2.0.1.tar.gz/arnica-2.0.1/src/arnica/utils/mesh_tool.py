...
def dilate_center(x_coor, y_coor, perturbation=0.1):
    ...
    adim_rad = np.sqrt(
        ((x_coor - x_center) / (0.5 * x_width)) ** 2
        + ((y_coor - y_center) / (0.5 * y_width)) ** 2
    )

    rad_dir_x = (x_coor - x_center) / np.maximum(adim_rad, 1.0e-16)
    rad_dir_y = (y_coor - y_center) / np.maximum(adim_rad, 1.0e-16)
...
def get_mesh(params_mesh):
    ...
    if params_mesh["kind"] == "rect":
        ...
        x_coor, y_coor = gen_cyl_grid_2d(
            r_min, r_max, n_r_pts, theta_min, theta_max, int(angle * n_r_pts + 1)
        )
    else:
