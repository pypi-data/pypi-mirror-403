...
def get_neighbor(kdtree, point, list_points):
    ...
    *Find the closiest neighbor that has not already been found*
    ...
    From the kdtree, find the two closiest points of 'point', the 1st being itself. If
    the second closiest point has already been found, then the number of researched
    points is increased until finding a point that has not already been found.
...
def sort_points_by_dist(points_coor, starting_pt):
    ...
    *Reorder a point cloud by distances* .
    From a starting fictive point, the first
    point 1 of the spline iget_neighbors found, as the closiest one. From that point,
    the following points are obtained with get_neighbor() until having sorted all
    points.
    ...
    while len(list_points) < len(points_coor):
        ...
        if int(new_progress / 10) > int(progress / 10):
            ...
            print(
                f"Sorting process... {progress:d}% : {delta_time:.3f}s",
                end="\r",
            )
...
def sample_arrays_by_dist_interval(dists, samples_res, *args):
    ...
    *Sample data and optional args at each sample_res along data*
    ...
    From an array containing the distance between sorted points, and from a sample
    resolution defined by the distance between two samples, the function picks up the
    indexes corresponding to a sample, and returns an array of sampled distances and
...
def sample_points_from_cloud(
    points_coor, starting_pt, n_samples=None, samples_res=None
):
    """
    *Sort and sample unsorted curve*
    ...
    _, (sampled_ordered_indexes,) = sample_arrays_by_dist_interval(
        ordered_dists, samples_res, ordered_indexes
    )
