...
def projection_kdtree(points, directions, point_surface, normal_surface, **kwargs):
    ...
    Algorithm :
        ...
        - If project bool is True, first compute [p] projection from [p] points along
          the [p,1] spherical closest node's normal. If False, projected_points =
          points.
        - Reduce computation to the [p,k] spherical closest nodes of the [p]
          projected_points
        - Compute the [p,k] cylindrical distances from the [p,k] closest nodes to the
          [p] lines defined by the [p] projected points and the [p] direction vectors.
    """
    opts = {"neigbors": 1000, "tol": 1000.0, "project": True}
    if "proj_axis" in kwargs:
        warnings.warn(
            "'proj_axis' kwarg is deprecated. Please use boolean 'project' "
    ...
    if opts["project"]:
        ...
        projected_pts = project_points(
            points, normal_surface[index], point_surface[index][:, np.newaxis, :]
        ).proj_pts
    ...
    Proj_KDTree = namedtuple(
        "Proj_KDTree", ("moved_pts", "indexes", "axi_dists", "cyl_dists")
    )
...
def compute_dists(points, directions, points_surf, normals_surf, tol):
    ...
    *Compute cylindrical distances*
    ...
    For a i-number of points coordinates, compute the cylindrical distances between the
    i,p-number of nodes with the i-number of axis.

    The array is then clipped according to the node normals and the direction of the
    drills.
    ...
    cyl_dists = np.where(
        np.absolute(axi_dists) < tol, cyl_dists, BIG * np.ones(cyl_dists.shape)
    )
...
def project_points(points_source, normals, points_target):
    ...
    *Compute the projected points, radial dists and axial dists*
    ...
    Compute projection from source points S of dim (k,) or (i,k),\ on a plan defined by
    normals N of dim (k,), (i,k), (p,k) or (i,p,k),\ and points T of dim (k,), (i,k),
    (p,k), (i,p,k). With :
    ...
    axi_dists = np.sum(np.multiply((points_target - points_source), normals), axis=-1)
    ...
    *Compute intersection coordinates of a line and a plan*
    ...
    ::
        nml_plan   xyz_line
           ...
           |        /
           |       /  vec_line
        ___x______I________sample_curve_from_cloud
        xyz_plan   \
                    Intersection point
