...
The `AxiPointCloud` class handles axysymmetric 1D meshes with associated variables. The
class also provides utility functions for calculating radius (`rad`), theta angle
(`theta`), and recompute theta range from coordinates
(`recompute_theta_range_from_coords`). Additionally, it offers methods for rotating
(`rotate`) and duplicating (`dupli_rotate`) the point cloud, as well as saving to an
XDMF file (`dump`).
...
class AxiPointCloud:
    ...
    def __init__(
        self, xcoor, ycoor, zcoor, name="Unnamed", vars=None, theta_range=None
    ):
        """
        startup class
    ...
    def rotate(self, shift_angle):
        ...
        :param shift_angle: angle in radians float
        """
    ...
    def dupli_rotate(self, repeat=1):
        ...
        :param repeat: number or repetitions integer
        """
