"""
This module converts dictionaries of 2D NumPy arrays into XMF files for visualization or
storage.
"""
...
def dump_dico_2d_nparrays(data_dict, filename, x_coords, y_coords, z_coords, **kw_args):
    ...
    data_dict : dictionnary holding the  2d arrays, on the format
                data_dict[key] = array(n1, n2) where (n1, n_2) is a subset of (n_x, n_y,
                n_z)
    ...
    visu_ = NpArray2Xmf(
        filename, time=time, mesh_name=grid_name, domain_name=domain_name
    )
...
def dump_dico_2d_time_nparrays(
    data_dict, root_path, prefix, x_coords, y_coords, z_coords, **kw_args
):
    ...
    steps : a list of integer time series steps
            that will be used to generate xmf files on the format : <prefix>_<step>.xmf
            if None steps will be generated as the range of time dimension of data
            arrays

    times : a list of float physical times that will
            be used in xmf files to describe the time of each step. if None will be
            generated as the range of time dimension of data arrays
    """
    ...
    for step in range(n_time):
        ...
        filename = f"{path}/{prefix}_{steps[step]:08d}.xmf"
        grid_name = f"{prefix}_{steps[step]:08d}"
        ...
        collection.append(f"{prefix}_{steps[step]:08d}.xmf")

    filename = f"{path}/{prefix}_collection.xmf"
...
def dump_dico_1d_nparrays(filename, data_dict):
    ...
    if format_ not in ["csv", "xlsx"]:
        ...
        try:
            ...
            msg = (
                "dump_dico_1d_nparrays cannot find 'pandas' neither/nor "
...
def plot_dict_data_as_file(data_dict, filename, x_data, y_data, **kw_args):
    ...
    filename : the file to which the plot is written it contains
                the extension that defines the format e.g: 'plot_toto.png' Supported
                formats/extensions : png, pdf, ps, eps and svg If not provided, by
                default "pdf" extension is used.
    data_dict : a dictionnary holding the data arrays
    ...
    fig.subplots_adjust(left=0.15, right=0.97, bottom=0.15, top=0.9, wspace=0.27)
    ...
    axe.grid(which="major", color="gray", linestyle="--", dashes=(8, 12), linewidth=0.5)
