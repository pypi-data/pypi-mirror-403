...
def renormalize(np_ar_vect):
    ...
    result = np_ar_vect / np.maximum(
        np.expand_dims(np.linalg.norm(np_ar_vect, axis=-1), axis=-1), SMALL
    )
...
def angle_btw_vects(np_ar_vect1, np_ar_vect2, convert_to_degree=False):
    ...
    sdot = np.sum(renormalize(np_ar_vect1) * renormalize(np_ar_vect2), axis=-1)
...
def rotate_vect_around_axis(xyz, *tuples_rot):
    ...
    *Rotate vector around vector or series of vector*
    ...
    :param xyz: Array of xyz-coordinates of shape (n,3) :param tuples_rot: List of tuple
...
def vect_to_quat(vect_targ, vect_source):
    ...
    *Generate a quaternion from two vectors*
    ...
    A quaternion is a rotation object. From two vectors, the rotation angle and the
    rotation axis are computed. The rotation vector generates then a quaternion for each
    serie of vectors.
...
def make_radial_vect(coord, vects):
    ...
    *Recalibrate vectors to make them radial.*
    ...
    The vectors are readjusted to cross x-axis. It is mainly done for nodes on the limit
    of the boundary for axi-cylindrical geometries.
...
def mask_cloud(np_ar_xyz, axis, support):
    """
    mask a cloud of n 3D points in in xyz axis among x,y,z,theta,r support a 2 value
    tuple : (0,3), (-12,float(inf))
    ...
    def true_between(np_arr, low, high):
        """
        return a boolean array of the same shape, true if value are btw low and high
        """
    ...
    if axis not in possible_axes:
        raise IOError(
            "axis " + axis + "not among possible axes " + ";".join(possible_axes)
        )
    ...
    if axis == "r":
        mask = true_between(
            np.hypot(np.take(np_ar_xyz, 1, axis=-1), np.take(np_ar_xyz, 2, axis=-1)),
            support[0],
            support[1],
        )
    if axis == "theta":
