...
class ProcessDefinition:
    ...
    def __init__(self, tf: ctrl.TransferFunction, delay_time: float, dt: float = 0.01, Total_time: float = 10):
        assert delay_time >= 0, "delay_time must be non-negative"
...
class PIDController:
    ...
    def IIMC_tuning(self, process: ProcessDefinition, theta=0.01, l=0.5):
        ...
        if order == 1:
            ...
            raise ValueError(
                "IIMC tuning is only implemented for first and second order systems."
            )
    ...
    def IDA2_tuning(self, rho=0):
        psi = rho * (self.Kp / (2 * self.Kd) + self.Kp / (2 * self.Kd)) - self.Kp / (
            2 * self.Kd
        )
        Kp = self.Kp - 2 * self.Kd * psi
        Ki = self.Ki + self.Kd * psi**2 - self.Kp * psi
        Kd = self.Kd
        self.Kp = Kp
        self.Ki = Ki
        self.Kd = Kd
...
__all__ = ["ProcessDefinition", "PIDController"]
__version__ = "0.1.0"
__author__ = "Dr. Bharat Verma"
