...
def _get_test_modules(
    ...
    for setup_py in all_setup_pys:
        ...
        if setup_py.name == PYPROJECT_TOML:
            ptr_params = parse_pyproject_toml(setup_py)
        elif setup_py.name == SETUP_CFG:
...
def _recursive_find_files(
    ...
    # Look for setup.py, pyproject.toml, or setup.cfg files
    files.update(
        {
            x
            for x in base_dir.iterdir()
            if x.is_file() and x.name in {SETUP_PY, PYPROJECT_TOML, SETUP_CFG}
        }
    )
    for directory in dirs:
...
def _select_preferred_config_per_directory(config_files: set[Path]) -> set[Path]:
    """
    Select the preferred configuration file for each directory.
    When a directory contains multiple config file types, prefers:
    pyproject.toml > setup.py > setup.cfg

    Args:
        config_files: Set of paths to configuration files

    Returns:
        Set with one config file per directory, selecting the highest priority file
    """
    deduplicated: set[Path] = set()
    dirs_seen: set[Path] = set()

    # Sort to ensure consistent ordering: pyproject.toml before setup.py before setup.cfg
    sorted_files = sorted(
        config_files,
        key=lambda p: (
            p.parent,
            0 if p.name == PYPROJECT_TOML else 1 if p.name == SETUP_PY else 2,
        ),
    )

    for file_path in sorted_files:
        if file_path.parent not in dirs_seen:
            deduplicated.add(file_path)
            dirs_seen.add(file_path.parent)
...
def find_setup_pys(
    ...
    """Find setup.py, pyproject.toml, or setup.cfg files"""
    setup_pys: set[Path] = set()
    _recursive_find_files(setup_pys, base_path, exclude_patterns, follow_symlinks)
    return _select_preferred_config_per_directory(setup_pys)
...
def parse_pyproject_toml(
    ...
    # If setup_py is already a pyproject.toml, use it directly
    if setup_py.name == PYPROJECT_TOML:
        pyproject_toml_path = setup_py
    else:
...
def parse_setup_cfg(setup_py: Path) -> dict[str, Any]:
    ...
    ptr_params: dict[str, Any] = {}
    # If setup_py is already a setup.cfg, use it directly
    if setup_py.name == SETUP_CFG:
        setup_cfg = setup_py
    else:
        setup_cfg = setup_py.parent / SETUP_CFG
...
async def async_main(
    ...
    if not tests_to_run:
        LOG.error(
            f"{str(base_path)} has no setup.py, pyproject.toml, or setup.cfg files with unit tests defined. Exiting"
        )
        return 1
