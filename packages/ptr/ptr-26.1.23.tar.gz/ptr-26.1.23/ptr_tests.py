...
class TestPtr(unittest.TestCase):
    ...
    def test_analyze_coverage(self, mock_log: Mock, mock_time: Mock) -> None:
        ...
        fake_setup_py = Path("unittest") / ptr.SETUP_PY
    ...
    def test_find_setup_py(self) -> None:
        ...
        # Now that setup.py is removed, we should find pyproject.toml
        self.assertEqual(str(found_setup_py.relative_to(base_path)), ptr.PYPROJECT_TOML)
    ...
    def test_find_setup_py_exclude_default(self) -> None:
        with TemporaryDirectory() as td:
            ...
            touch_files(*(adir / ptr.SETUP_PY for adir in (build_path, cooper_path)))

            setup_pys = ptr.find_setup_pys(td_path, {"build*"})
            self.assertEqual(len(setup_pys), 1)
            self.assertEqual(
                setup_pys.pop().relative_to(td_path), Path("cooper") / ptr.SETUP_PY
            )

    def test_select_preferred_config_per_directory(self) -> None:
        """Test that _select_preferred_config_per_directory prefers pyproject.toml > setup.py > setup.cfg"""
        with TemporaryDirectory() as td:
            td_path = Path(td)

            # Test 1: Directory with all three config files - should prefer pyproject.toml
            dir1 = td_path / "dir1"
            dir1.mkdir()
            setup_py = dir1 / ptr.SETUP_PY
            pyproject_toml = dir1 / ptr.PYPROJECT_TOML
            setup_cfg = dir1 / ptr.SETUP_CFG
            touch_files(setup_py, pyproject_toml, setup_cfg)

            result = ptr._select_preferred_config_per_directory(
                {setup_py, pyproject_toml, setup_cfg}
            )
            self.assertEqual(len(result), 1)
            self.assertEqual(result.pop(), pyproject_toml)

            # Test 2: Directory with only setup.py and setup.cfg - should prefer setup.py
            dir2 = td_path / "dir2"
            dir2.mkdir()
            setup_py2 = dir2 / ptr.SETUP_PY
            setup_cfg2 = dir2 / ptr.SETUP_CFG
            touch_files(setup_py2, setup_cfg2)

            result = ptr._select_preferred_config_per_directory({setup_py2, setup_cfg2})
            self.assertEqual(len(result), 1)
            self.assertEqual(result.pop(), setup_py2)

            # Test 3: Directory with only setup.cfg
            dir3 = td_path / "dir3"
            dir3.mkdir()
            setup_cfg3 = dir3 / ptr.SETUP_CFG
            touch_files(setup_cfg3)

            result = ptr._select_preferred_config_per_directory({setup_cfg3})
            self.assertEqual(len(result), 1)
            self.assertEqual(result.pop(), setup_cfg3)

            # Test 4: Multiple directories with different config files
            dir4 = td_path / "dir4"
            dir5 = td_path / "dir5"
            dir4.mkdir()
            dir5.mkdir()
            setup_py4 = dir4 / ptr.SETUP_PY
            pyproject_toml5 = dir5 / ptr.PYPROJECT_TOML
            touch_files(setup_py4, pyproject_toml5)

            result = ptr._select_preferred_config_per_directory(
                {setup_py4, pyproject_toml5}
            )
            self.assertEqual(len(result), 2)
            self.assertIn(setup_py4, result)
            self.assertIn(pyproject_toml5, result)
    ...
    # Since we now find pyproject.toml directly, we need to let it parse normally
    # Only patch setup.cfg parsing
    @patch("ptr.parse_setup_cfg")
    @patch("ptr.print")  # noqa
    def test_get_test_modules(self, mock_print: Mock, mock_setup_cfg: Mock) -> None:
        ...
        # Now that setup.py is removed, we should find pyproject.toml
        self.assertEqual(
            test_modules[base_path / ptr.PYPROJECT_TOML],
    ...
    def test_parse_setup_cfg(self) -> None:
        tmp_dir = Path(gettempdir())
        setup_cfg = tmp_dir / ptr.SETUP_CFG
        setup_py = tmp_dir / ptr.SETUP_PY
    ...
    def test_test_runner(self) -> None:
        ...
        with TemporaryDirectory() as td:
            td_path = Path(td)
            setup_py_path = td_path / ptr.SETUP_PY
    ...
    def test_test_steps_runner(self, mock_print: Mock) -> None:
        with TemporaryDirectory() as td:
            td_path = Path(td)
            fake_setup_py = td_path / ptr.SETUP_PY
