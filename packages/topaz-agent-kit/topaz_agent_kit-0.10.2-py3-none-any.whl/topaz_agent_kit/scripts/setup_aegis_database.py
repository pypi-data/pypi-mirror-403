...
def get_font_for_language(language: str) -> str:
    ...
    if language == "zh":
        ...
        if _REGISTERED_CJK_FONT:
            ...
            # User can install a CJK font and it will be auto-detected
            return "Helvetica"
...
def generate_mock_sows(cursor, vendors: List[Dict[str, Any]],
                       ...
                       invoice_type_dist: Optional[Dict[str, float]] = None) -> List[Dict[str, Any]]:
    ...
    for i in range(count):
        ...
        # Classify SOW by invoice type using distribution
    ...
    print(f"  ✓ Generated {len(sows)} Statements of Work")
    return sows


def generate_mock_milestones(cursor, sows: List[Dict[str, Any]], projects: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for sow in sows:
        ...
        for i in range(num_milestones):
            ...
            milestone_code = f"MS-{sow['sow_number']}-{i+1:02d}"
...
def generate_current_invoice_pdfs(cursor, vendors: List[Dict[str, Any]], pos: List[Dict[str, Any]],
    ...
    pos_with_sows = [po for po in pos if po.get("sow_id") and any(s["sow_id"] == po.get("sow_id") for s in sows)]
    ...
    for idx, i in enumerate(all_indices[:straight_through_count]):
        ...
        if max_invoice_amount is not None and total_amount > max_invoice_amount:
            # Reduce the last line item to fit within the cap
            if line_items:
        ...
        if hist_row and hist_row[2] and hist_row[2] > 0:  # count is at index 2
            ...
            if total_amount > max_normal_amount:
                # Reduce amount to stay within normal range
        ...
        # Assign language from pre-calculated distribution list
    ...
    for idx, i in enumerate(exception_indices):
        ...
        for po_candidate in pos_with_sows:
            # Match SOW by sow_id and invoice_type (POs now reference SOWs via sow_id)
            matching_sows_candidate = [s for s in sows if s["sow_id"] == po_candidate.get("sow_id") and s.get("invoice_type") == required_invoice_type]
            # If no exact match, try without invoice_type filter
            if not matching_sows_candidate:
                matching_sows_candidate = [s for s in sows if s["sow_id"] == po_candidate.get("sow_id")]
...
def main():
    ...
    print("\n3. Generating contractual data...")
    # Generate SOWs first (before POs, so POs can link to SOWs)
    sows = generate_mock_sows(cursor, vendors, projects, args.sow_count, invoice_type_dist)
    # Generate POs linked to SOWs
    pos = generate_mock_pos(cursor, vendors, projects, wbs_list, sows, args.po_count)
    milestones = generate_mock_milestones(cursor, sows, projects)
    ...
    print("\n6. Generating current invoice PDFs...")
