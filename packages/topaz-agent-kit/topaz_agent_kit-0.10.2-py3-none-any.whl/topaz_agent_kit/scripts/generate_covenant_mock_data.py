...
# Contract ID prefix uses current year
CONTRACT_ID_PREFIX = f"CONTRACT-{datetime.now().year}"
...
def generate_contract_id(seed: Optional[int] = None) -> str:
    """Generate a unique contract ID."""
    if seed is not None:
        # Use seed to generate consistent ID
        rng = random.Random(seed)
        contract_num = rng.randint(1, 999)
    else:
        contract_num = random.randint(1, 999)
    return f"{CONTRACT_ID_PREFIX}-{contract_num:03d}"
...
def find_existing_contract_ids(base_dir: Path) -> List[str]:
    """Find all existing contract IDs from pre_contract directory."""
    ...
    for item in pre_contract_base.iterdir():
        if item.is_dir() and any(item.iterdir()):
            contract_ids.append(item.name)
    
    return sorted(contract_ids)


def find_existing_change_orders(base_dir: Path, contract_id: str) -> List[Path]:
    """Find all existing change orders for a contract."""
    change_orders_dir = base_dir / "change_orders"
    if not change_orders_dir.exists():
        return []
    
    change_orders = []
    for file in change_orders_dir.glob(f"{contract_id}_change_order_*.pdf"):
        change_orders.append(file)
    
    return sorted(change_orders)


def get_next_change_order_version(base_dir: Path, contract_id: str) -> str:
    """Get the next change order version number."""
    existing = find_existing_change_orders(base_dir, contract_id)
    if not existing:
        return "v1"
    ...
    for file in existing:
        # Extract version from filename like CONTRACT-2025-318_change_order_v1.pdf
...
def check_existing_data(base_dir: Path, contract_id: str) -> Dict[str, bool]:
    """Check what files already exist for a contract."""
    pre_contract_dir = base_dir / "pre_contract" / contract_id
    draft_dir = base_dir / "draft_contracts"
    signed_dir = base_dir / "signed_contracts"
    change_orders_dir = base_dir / "change_orders"
    artifacts_dir = base_dir / "artifacts" / contract_id
    ...
    return {
        ...
        "pre_contract_report_exists": (reports_dir / f"{contract_id}_pre_contract_synthesis_report.md").exists(),
        "draft_contract_exists": (draft_dir / f"{contract_id}_draft_v1.pdf").exists(),
        "draft_validation_report_exists": (artifacts_dir / "draft_contract_validation_report.json").exists(),
        "signed_contract_exists": (signed_dir / f"{contract_id}_signed.pdf").exists(),
...
def generate_email_thread(
    ...
    subjects = [
        f"Initial Discussion - {contract_id} Project Scope",
        f"Commercial Terms Discussion - {contract_id}",
        f"Follow-up: {contract_id} Requirements",
    ]
...
I wanted to follow up on our discussion about the {contract_id} project. Based on our meeting last week, here are the key points we discussed:
...
4. Schedule follow-up meeting in 2 weeks""")
    ...
    # Build final content
    content = f"""# Meeting Notes - {contract_id} Project Discussion
**Date:** {date}
...
def generate_informal_scope_doc(
    ...
    if not REPORTLAB_AVAILABLE:
        ...
        if contract_value >= 1_000_000_000:
            ...
            value_str = f"£{contract_value:,}"
        scope_content = get_project_scope_content(contract_type, contract_value)
        return f"""Informal Scope Document - {contract_id}
...
"""
    ...
    temp_path = Path("/tmp") / f"{contract_id}_informal_scope_temp.pdf"
    ...
    # Title
    story.append(Paragraph(f'Informal Scope Document - {contract_id}', title_style))
    ...
    story.append(Paragraph(f"Project ID: {contract_id}", styles['Normal']))
...
def generate_pre_contract_data(
    ...
    email1_path.write_text(generate_email_thread(
        contract_id, 0, contract_type, communication_style, contract_value, seed
    ))
    ...
    email2_path.write_text(generate_email_thread(
        contract_id, 1, contract_type, communication_style, contract_value, seed
    ))
    ...
    meeting_notes_path.write_text(generate_meeting_notes(
        contract_id, contract_type, completeness_level, communication_style, contract_value, seed
    ))
    generated_files.append(meeting_notes_path)
    
    # Informal scope document
    if REPORTLAB_AVAILABLE:
        scope_doc_temp = generate_informal_scope_doc(contract_id, contract_type, contract_value, seed)
        ...
        scope_doc_path.write_text(generate_informal_scope_doc(contract_id, contract_type, contract_value, seed))
        generated_files.append(scope_doc_path)
...
def generate_draft_contract_pdf(
    ...
    filepath = draft_dir / f"{contract_id}_draft_v1.pdf"
    ...
    story.append(Paragraph(f"Project: {contract_id}", title_style))
    story.append(Spacer(1, 0.3 * inch))
...
def generate_signed_contract_pdf(
    ...
    filepath = signed_dir / f"{contract_id}_signed.pdf"
...
def generate_change_order_pdf(
    ...
    filepath = change_orders_dir / f"{contract_id}_change_order_{version}.pdf"
...
"""
    ...
    simple_path.write_text(simple_wbs)
...
"""
    ...
    medium_path.write_text(medium_wbs)
...
"""
    ...
    complex_path.write_text(complex_wbs)
...
def option_1_pre_contract(
    ...
    print(f"Contract ID: {contract_id} (auto-generated)")
    ...
    files = generate_pre_contract_data(
        base_dir, contract_id, contract_type, completeness_level, communication_style, contract_value, seed
    )
...
def option_2_draft_validation(
    ...
    if keep_existing:
        existing_contract_ids = find_existing_contract_ids(base_dir)
        if existing_contract_ids:
            # Use the most recent existing contract ID
            contract_id = existing_contract_ids[-1]
            print(f"Contract ID: {contract_id} (using existing)")
        else:
            print(f"Contract ID: {contract_id} (auto-generated - no existing contracts found)")
    else:
        print(f"Contract ID: {contract_id} (auto-generated)")
    ...
    existing = check_existing_data(base_dir, contract_id)
    ...
    if keep_existing and existing["pre_contract_exists"]:
        ...
        if existing["pre_contract_summary_exists"]:
            ...
            summary_path.write_text(json.dumps(
                generate_pre_contract_summary(contract_id, contract_type, completeness_level, contract_value, seed),
                indent=2
            ))
    ...
    draft_path = generate_draft_contract_pdf(
        contract_id, base_dir, deviation_pattern, contract_type, contract_value, seed
    )
...
def option_3_signed_contract(
    ...
    if existing["signed_contract_exists"] and keep_existing:
        ...
        print(f"  ✓ Using existing: signed_contracts/{contract_id}_signed.pdf")
        signed_path = base_dir / "signed_contracts" / f"{contract_id}_signed.pdf"
    else:
        print("\nGenerating signed contract...")
        contract_value = get_contract_value(contract_type, seed)
        signed_path = generate_signed_contract_pdf(contract_id, base_dir, contract_type, contract_value, seed)
    ...
    if existing["pre_contract_exists"] or not keep_existing:
        files_summary.append(f"  • pre_contract/{contract_id}/ (from Option 1)")
    if existing["pre_contract_summary_exists"] or not keep_existing:
        files_summary.append(f"  • artifacts/{contract_id}/pre_contract_summary_v1.json (from Option 1)")
    if existing["draft_contract_exists"] or not keep_existing:
        files_summary.append(f"  • draft_contracts/{contract_id}_draft_v1.pdf (from Option 2)")
    if existing["draft_validation_report_exists"]:
        files_summary.append(f"  • artifacts/{contract_id}/draft_contract_validation_report.json (from Option 2)")
...
def option_4_change_order(
    ...
    if not existing["signed_contract_exists"]:
        ...
        print(f"   Expected file: signed_contracts/{contract_id}_signed.pdf")
    ...
    existing_change_orders = find_existing_change_orders(base_dir, contract_id)
    ...
    version = get_next_change_order_version(base_dir, contract_id)
    ...
    # Show what will be used by the pipeline
    files_summary = []
    files_summary.append(f"  • signed_contracts/{contract_id}_signed.pdf (required)")
    if existing["signed_contract_summary_exists"]:
        files_summary.append(f"  • artifacts/{contract_id}/signed_contract_structured_summary.json (if available)")
    ...
    while True:
        ...
        try:
            ...
            if choice == "0":
                ...
                existing_contract_ids = find_existing_contract_ids(base_dir)
                if existing_contract_ids:
                    print(f"\n✓ Found {len(existing_contract_ids)} existing contract(s): {', '.join(existing_contract_ids)}")
                    contract_id = existing_contract_ids[-1]
                ...
                option_4_change_order(base_dir, contract_id, keep_existing=True, contract_type=contract_type, seed=seed)
                ...
                contract_id = generate_contract_id(seed)
                ...
                if option == 1:
                    ...
                    print("  • pre_contract/{contract_id}/email_thread_1.txt")
                    print("  • pre_contract/{contract_id}/email_thread_2.txt")
                    print("  • pre_contract/{contract_id}/meeting_notes_2025-01-10.md")
                    print("  • pre_contract/{contract_id}/informal_scope_doc.pdf")
                    ...
                    if existing_contract_ids:
                        ...
                        print(f"    - Will use existing contract: {existing_contract_ids[-1]}")
                        print("    - Will reuse pre-contract artifacts from Option 1")
                        print("    - Will generate: draft_contracts/{contract_id}_draft_v1.pdf")
                        ...
                        print("\n⚠️  No existing contracts found.")
                        print("\nWhat will be generated:")
                        print("  • pre_contract/{contract_id}/ (4 files)")
                        print("  • artifacts/{contract_id}/pre_contract_summary_v1.json")
                        print("  • draft_contracts/{contract_id}_draft_v1.pdf")
                        print("\n⚠️  WARNING: If you choose 'N' (clear existing), ALL existing mock data")
                        print("   will be deleted (except historical_wbs/).")
                    ...
                    if existing_contract_ids:
                        ...
                        print("    - Will generate: signed_contracts/{contract_id}_signed.pdf")
                        print("  • If you choose 'N' (clear existing):")
                        print("    - Will clear ALL existing data (except historical_wbs/)")
                        ...
                        print("\n⚠️  No existing contracts found.")
                        print("\nWhat will be generated:")
                        print("  • signed_contracts/{contract_id}_signed.pdf")
                ...
                if option == 1:
                    option_1_pre_contract(base_dir, contract_id, keep_existing, contract_type, completeness_level, communication_style, seed)
                elif option == 2:
                    option_2_draft_validation(base_dir, contract_id, keep_existing, contract_type, deviation_pattern, completeness_level, communication_style, seed)
                elif option == 3:
                    option_3_signed_contract(base_dir, contract_id, keep_existing, contract_type, completeness_level, seed)
...
def main():
    ...
    contract_id = generate_contract_id(args.seed)
    
    # Execute selected option with scenarios
    if args.option == 1:
        option_1_pre_contract(base_dir, contract_id, args.keep_existing, contract_type, completeness_level, communication_style, args.seed)
    elif args.option == 2:
        option_2_draft_validation(base_dir, contract_id, args.keep_existing, contract_type, deviation_pattern, completeness_level, communication_style, args.seed)
    elif args.option == 3:
        option_3_signed_contract(base_dir, contract_id, args.keep_existing, contract_type, completeness_level, args.seed)
    elif args.option == 4:
        # For option 4, try to find existing contract ID
        existing_contract_ids = find_existing_contract_ids(base_dir)
        if existing_contract_ids:
            contract_id = existing_contract_ids[-1]
        option_4_change_order(base_dir, contract_id, args.keep_existing, contract_type, args.seed)
