...
def main():
    ...
    # If output_dir is explicitly provided (not None and not empty), use it
    # Otherwise, fall back to project_dir + BASE_DIR
    if args.output_dir and args.output_dir.strip():
        # Use explicitly provided output directory
        print(f"Using provided output-dir: {args.output_dir}")
        base_dir = resolve_script_path(args.output_dir, project_name=project_name)
    else:
        # Use project_dir + BASE_DIR
        if args.output_dir:
            print(f"Warning: output-dir provided but is empty/whitespace, falling back to default")
        else:
            print(f"Using default output directory (project_dir + BASE_DIR)")
        project_dir = resolve_script_path(args.project_dir, project_name=project_name)
        base_dir = project_dir / BASE_DIR
    
    print(f"Final output directory: {base_dir}")
