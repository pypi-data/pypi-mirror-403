...
class ADKBaseAgent(BaseAgent):
    ...
    def _create_agent(self) -> None:
        ...
        try:
            ...
            # Normalize instruction prompt for Windows compatibility (same as inputs)
            # This ensures the instruction is clean UTF-8 before being passed to ADK/LiteLLM
            instruction = self.prompt.get("instruction", "")
            if instruction and isinstance(instruction, str):
                try:
                    # Normalize encoding: encode to bytes then decode back to ensure clean UTF-8
                    instruction = instruction.encode('utf-8', errors='replace').decode('utf-8', errors='replace')
                    self.logger.debug("Normalized ADK instruction prompt ({} chars)", len(instruction))
                except Exception as e:
                    self.logger.warning("Failed to normalize UTF-8 encoding for instruction: {}", e)
            
            self.logger.debug("ADK agent instruction: {}", instruction)
    ...
    def _build_multimodal_content(self, rendered_inputs: str, context: Dict[str, Any]) -> types.Content:
        ...
        # This prevents encoding issues that could trigger Azure content filter violations
        if rendered_inputs and isinstance(rendered_inputs, str):
            # Ensure the string is properly encoded as UTF-8
            # On Windows, strings might have different encoding that could cause issues
            try:
                # Normalize encoding: encode to bytes then decode back to ensure clean UTF-8
                rendered_inputs = rendered_inputs.encode('utf-8', errors='replace').decode('utf-8', errors='replace')
            except Exception as e:
                self.logger.warning("Failed to normalize UTF-8 encoding for rendered inputs: {}", e)
        
        # Log prompt details for debugging content filter issues
        # This helps identify if prompt length or content is triggering filters
        if rendered_inputs:
            prompt_length = len(rendered_inputs)
            # Count occurrences of potential trigger patterns
            has_backslashes = '\\' in rendered_inputs
            has_single_quotes = "'" in rendered_inputs and rendered_inputs.count("'") > 10
            json_count = rendered_inputs.count('{') + rendered_inputs.count('[')
            self.logger.debug(
                "ADK prompt stats - Length: {} chars, Backslashes: {}, Single quotes (many): {}, JSON structures: {}",
                prompt_length, has_backslashes, has_single_quotes, json_count
            )
            # Log first 200 chars and last 200 chars for debugging (without sensitive data)
            if prompt_length > 400:
                self.logger.debug("Prompt preview (first 200): {}...", rendered_inputs[:200])
                self.logger.debug("Prompt preview (last 200): ...{}", rendered_inputs[-200:])
    ...
    async def _execute_agent(self, context: Dict[str, Any], variables: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Log the full content being sent for debugging
            # This helps identify what ADK/LiteLLM is actually sending to Azure
            if hasattr(content, 'parts') and content.parts:
                text_parts = [p.text for p in content.parts if hasattr(p, 'text') and p.text]
                if text_parts:
                    total_text = ' '.join(text_parts)
                    self.logger.debug("ADK sending text content ({} chars): {}...", len(total_text), total_text[:500])
            ...
            try:
                ...
                self.logger.error("ADK/LiteLLM execution failed: {}", e)
                self.logger.error("Error type: {}", type(e).__name__)
                if hasattr(e, 'response'):
                    self.logger.error("Error response: {}", e.response)
                if hasattr(e, 'status_code'):
                    self.logger.error("HTTP status code: {}", e.status_code)
                raise
