"""
The code in this file is adapted from the Twist2D project (https://github.com/kYangLi/Twist2D).

Used in accordance with the original project's license.
"""
import numpy as np

FLOAT_PREC = 1e-6
DEFAULT_IN_POSCAR = 'POSCAR'
DEFAULT_OUT_POSCAR = 'POSCAR-Twisted'
...
class Twist2D:
    ...
    def _get_poscar_prim_vecs(self, lines):
        ...
        if not (
            self.float_eq(a1_z, 0.0) and self.float_eq(a2_z, 0.0) and
            self.float_eq(a3_x, 0.0) and self.float_eq(a3_y, 0.0)
        ):
            raise ValueError(
                "Primitive POSCAR: The c axis must in z direction!"
            )
    ...
    def _get_poscar_atoms_info(self, lines):
        ...
        atom_coord_list = self._get_poscar_atoms_coord(
            lines, sum(quantities_list)
        )
    ...
    def read_primcell_of_layers(self, filename):
        ...
        if not self._poscar_in_direct_mode(lines):
            raise ValueError(
                "Primitive POSCAR: the atoms' coordinates MUST in 'Direct' mode!"
            )
    ...
    def _get_supercell_vecs(self, supercell_matrix, primitive_vecs, super_a3_z):
        ...
        supercell_vecs = np.array(
            [
                [svs_2d[0,0], svs_2d[0,1], 0],
    ...
    def _get_atoms_cell_shifts(self,
        a1_boder, a2_boder, supercell_a1p, supercell_a2p
    ):
    ...
    def _get_supercell_atoms_coord(self, 
        supercell_matrix_inv, super_a3_z, a3_z, atoms_cell_shifts,
        atom_coord_list, scell_shift_x, scell_shift_y, supercell_vecs,
        supercell_shift_z
    ):
    ...
    def _check_supercell_cellnum(self, atoms_cell_shifts, supercell_num):
        ...
        if check_supercell_num != supercell_num:
            raise ValueError(f"Supercell generation: expect {supercell_num} primitive cell in supercell, but find {check_supercell_num}...")


    def gen_supercell(self,
        super_mult_vec1, super_mult_vec2, primitive_vecs, a3_z, quantities_list,
        atom_coord_list, super_a3_z=20.0, scell_shift_x=0.0, scell_shift_y=0.0,
        supercell_shift_z=0.0
    ):
        """Generate the supercell"""
        # Supercell number
        supercell_num = self._get_atoms_num_in_supercell(
            super_mult_vec1, super_mult_vec2
        )
        # Supercell number list
        supercell_quantities_list = [
            val*supercell_num for val in quantities_list
        ]
        ...
        # Supercell vectors 
        supercell_vecs = self._get_supercell_vecs(
            supercell_matrix, primitive_vecs, super_a3_z
        )
        # Calculate supercell shifts according to supercell vector a1',a2'
        #--> Get the boder of the atomic index searching
        a1_boder, a2_boder = self._get_index_boder_of_atoms(
            super_mult_vec1, super_mult_vec2
        )
        #--> Find all atoms' shift vector in the supercell
        atoms_cell_shifts = self._get_atoms_cell_shifts(
            a1_boder, a2_boder, super_mult_vec1,  super_mult_vec2
        )
        ...
        self._get_supercell_atoms_coord(
            supercell_matrix_inv, super_a3_z, a3_z, atoms_cell_shifts,
            atom_coord_list, scell_shift_x, scell_shift_y, supercell_vecs,
            supercell_shift_z
        )
    ...
    # +--------------+
    def add_layer(self, 
        super_a1_mult, super_a2_mult, next_layer_dis=2, scs_x=0.0, scs_y=0,
        ...
        curr_supercell_info = {
            "super_mult_a1"  : np.array(super_a1_mult),
            "super_mult_a2"  : np.array(super_a2_mult),
            "layer_distance" : next_layer_dis,
            "scell_shift_x"  : scs_x,
            "scell_shift_y"  : scs_y,
        }
    ...
    def twist_layers(self, start_z=0.0, super_a3_z=20.0):
        ...
        for i, supercell_info in enumerate(self.supercell_info_list):
            ...
            supercell_vecs, supercell_quantities_list, \
                supercell_atom_coord_list, frac_z_range = \
                    self.gen_supercell(
                        super_mult_a1, super_mult_a2, primitive_vecs, 
            ...
            if curr_max_atom_z > 1.0: # Not use supercell_shift_z to avoid mistake near the bound.
                raise ValueError("Twisting layers: coordinate z is out of range, pls reduce the layer distance or increase the cell length of z.")
    ...
    @property
    def layers_strain(self):
        """The added strain (volume changes) in each layer."""
        layers_vol_changes = []
    ...
    def write_res_to_poscar(self, filename=DEFAULT_OUT_POSCAR):
        ...
        for vec in supercell_vecs:
            out_poscar_lines.append(
                "    %13s    %13s    %13s"
                %("%4.8f"%vec[0], "%4.8f"%vec[1], "%4.8f"%vec[2])
            )
        # Atoms symbol list
...
class TwistBilayerGraphene(Twist2D):
    def _check_angle(self, angle):
        ...
        for i, primcell_info in enumerate(self.primcell_info_list):
            ...
            if not self.float_eq(phi, angle, prec=1E-3):
                raise ValueError(
                    f"Layer {layer_i}: the primitive cell vectors' angle must be 60 degree, current is {phi}."
                )


    def add_layers(self, m, n, layer_dis, scs_x, scs_y, prim_poscar):
    ...
    def gen(self,
        m, n, prim_poscar=DEFAULT_IN_POSCAR, poscar_out=DEFAULT_OUT_POSCAR,
        start_z=0.1, super_a3_z=20.0, layer_dis=2.0, scs_x=0.0, scs_y=0.0
    ):
        """Generate the twisted bilayer graphene (TBG)."""
        self.add_layers(m, n, layer_dis, scs_x, scs_y, prim_poscar)
