"""
Viewsets for channel_integrations/v1/logs
"""
import logging
...
def sort_by_date_aggregation(objects, date_fields, filter, order):  # pylint: disable=redefined-builtin
    # Arbitrarily old date to make sure null dates are considered lower in the
    # `Greatest` annotation than any existing or to be
...
class ContentSyncStatusViewSet(PermissionRequiredForIntegratedChannelMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        content_metadata_transmission_audit_filter = \
            {field: self.kwargs.get(field) for field in content_metadata_fields}
        ...
        for query_filter in self.request.GET.keys():
            if query_filter in table_fields:
                content_metadata_transmission_audit_filter[f'{query_filter}__contains'] = \
                    self.request.GET.get(query_filter)
...
class LearnerSyncStatusViewSet(PermissionRequiredForIntegratedChannelMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if self.request.GET.get('sort_by'):
            ...
            # The serialized `sync_last_attempted_at` value is the representation
            # of both the `modified` and `created` values
            if "sync_last_attempted_at" in content_sort_by[0]:
                order = "-" if "-" in content_sort_by[0] else ""
                return sort_by_date_aggregation(
