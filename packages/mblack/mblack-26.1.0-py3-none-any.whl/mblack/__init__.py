...
@click.option(
    ...
    type=str,
    help=(
        "Require a specific version of Black to be running (useful for unifying"
...
@click.option(
    ...
    help=(
        ...
        " on all platforms (Windows, too). Exclusions are calculated first,"
        " inclusions later."
    ),
    show_default=True,
)
...
@click.option(
    ...
    help=(
        ...
        " ignored due to exclusion patterns."
    ),
)
...
def main(
    ...
    if (
        ...
        err(
            f"{error_msg} The required version `{required_version}` does not"
            f" match the running version `{__version__}`!"
        )
        ctx.exit(1)
    if ipynb and pyi:
...
def get_features_used(
    ...
    for n in node.pre_order():
        if is_string_token(n):
            ...
            n.type in {syms.tname_star, syms.tname}
...
def assert_equivalent(src: str, dst: str) -> None:
    ...
    try:
        ...
        raise AssertionError(
            ...
            f" AST: {exc}\nThis could be caused by running Black with an older"
            " Python version that does not support new syntax used in your"
            " source file."
        ) from exc
    ...
    if src_ast_str != dst_ast_str:
        ...
        raise AssertionError(
            ...
            " https://github.com/psf/black/issues.  This diff might be"
            f" helpful: {log}"
        ) from None
...
def assert_stable(src: str, dst: str, mode: Mode) -> None:
    ...
    if dst != newdst:
        ...
        raise AssertionError(
            ...
            " of the formatter.  Please report a bug on"
            " https://github.com/psf/black/issues.  This diff might be"
            f" helpful: {log}"
        ) from None
