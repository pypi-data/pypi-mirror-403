    ...
    def __call__(
        ...
        if isinstance(match_result, Err):
            ...
            raise CannotTransform(
                f"The string transformer {self.__class__.__name__} does not"
                " recognize this line as one that it can transform."
            ) from cant_transform
...
class CustomSplitMapMixin:
    ...
    _CUSTOM_SPLIT_MAP: ClassVar[dict[_Key, tuple[CustomSplit, ...]]] = (  # type: ignore
        defaultdict(tuple)
    )
        ...
        for inc in [1, -1]:
            ...
            while is_valid_index(i) and line.leaves[i].type in [
                ...
                if line.leaves[i].type == STANDALONE_COMMENT:
                    ...
                    return TErr(
                        "StringMerger does NOT merge string groups which"
                        " contain stand-alone comments."
                    )
...
class BaseStringSplitter(StringTransformer):
    ...
    def _validate(self, line: Line, string_idx: int) -> TResult[None]:
        ...
        if not string_leaf.parent or [
            ...
            return TErr(
                f"This string ({string_leaf.value}) appears to be pointless"
                " (i.e. has no parent)."
            )
        ...
        if id(
            ...
            return TErr(
                "Line appears to end with an inline pragma comment. Splitting"
                " the line could modify the pragma's behavior."
            )
...
class StringSplitter(BaseStringSplitter, CustomSplitMapMixin):
    ...
    def do_splitter_match(self, line: Line) -> TMatchResult:
        ...
        # Else the first leaf MAY be a string operator symbol or the 'in' keyword...
            ...
            or (LL[idx].type == token.NAME and str(LL[idx]) == "in")
        ):
            idx += 1
    ...
    def do_transform(
        ...
        if max_break_idx < 0:
            yield TErr(
                f"Unable to split {LL[string_idx].value} at such high of a line"
                f" depth: {line.depth}"
            )
            return
...
class StringParenWrapper(BaseStringSplitter, CustomSplitMapMixin):
            ...
            if " " not in string_value:
                ...
                if len(string_value) > max_string_length:
                    ...
                    if not self.has_custom_splits(string_value):
                        ...
                        return TErr(
                            "We do not wrap long strings in parentheses when"
                            " the resultant line would still be over the"
                            " specified line length and can't be split further"
                            " by StringSplitter."
                        )
            return Ok(string_idx)
    ...
    @staticmethod
    def _dict_match(LL: list[Leaf]) -> int | None:
        """
        Returns:
            string_idx such that @LL[string_idx] is equal to our target (i.e.
