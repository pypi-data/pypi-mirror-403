"""
Make Labels Module (v3 - Polars Backend, Vectorized)
=====================================================
...
def _process_column_labels_df(
    ...
    """
    Process column labels DataFrame into dictionary (vectorized).
    ...
    # Build dictionary (vectorized - zip on columns)
    result: ColumnLabelsDict = dict(zip(
        df_cleaned.get_column(var_col).to_list(),
        df_cleaned.get_column(label_col).to_list()
    ))
...
def _process_value_labels_df(
    ...
    """
    Process value labels DataFrame into nested dictionary (vectorized).
    ...
    # Build nested dictionary using group_by (vectorized per group)
    result: ValueLabelsDict = {}
    
    # Get unique variables
    variables = df_cleaned.get_column(var_col).unique().to_list()
    
    for variable in variables:
        group = df_cleaned.filter(pl.col(var_col) == variable)
        
        # Extract typed values vectorized within group
        val_ints = group.get_column("_val_int").to_list()
        val_floats = group.get_column("_val_float").to_list()
        val_strs = group.get_column("_val_str").to_list()
        labels = group.get_column(label_col).to_list()
        
        # Build inner dict with proper types
        inner_dict: dict[int | float | str, str] = {}
        for i, label in enumerate(labels):
            if val_ints[i] is not None:
                inner_dict[int(val_ints[i])] = label
            elif val_floats[i] is not None:
                inner_dict[float(val_floats[i])] = label
            elif val_strs[i] is not None and val_strs[i] != "":
                inner_dict[str(val_strs[i])] = label
        
        if inner_dict:
            result[variable] = inner_dict
...
def make_labels(
    ...
    This v3 version uses Polars with fully vectorized column operations.
    ...
    try:
        _print("=" * 60)
        _print("COMBINED LABEL MAKER (v3 - Polars Vectorized) - Starting Processing")
        _print("=" * 60)
