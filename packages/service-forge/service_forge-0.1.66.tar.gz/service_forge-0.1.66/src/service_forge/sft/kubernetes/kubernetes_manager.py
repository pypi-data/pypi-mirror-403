                ...
                try:
                    ...
                    if e.status == 404:
                        ...
                        try:
                            ...
                            error_msg = f"Failed to create {kind}/{name}"
                            error_details = []

                            if create_error.status:
                                error_details.append(f"HTTP Status: {create_error.status}")

                            if create_error.reason:
                                error_details.append(f"Reason: {create_error.reason}")

                            if create_error.body:
                                try:
                                    import json
                                    body = json.loads(create_error.body)
                                    if isinstance(body, dict):
                                        if body.get("message"):
                                            error_details.append(f"Message: {body['message']}")
                                        if body.get("reason"):
                                            error_details.append(f"Reason: {body['reason']}")
                                except:
                                    error_details.append(f"Details: {create_error.body}")
