...
class DummyEvaluator(ModelEvaluator):
    ...
    def evaluate(
        ...
        if model_type == "classifier":
            ...
            raise AzureMLMLFlowValueException(f"Unsupported model type {model_type}")
...
def test_dataset_metadata():
    ...
    assert d1._metadata == {
        "hash": "01b68cad26d85014aa2275910cea9e20fcadb7688dc80817ba85b925cc35e662",
        "name": "a1",
        "path": "/path/to/a1",
    }
...
def test_dataset_with_array_data():
    ...
    with pytest.raises(AzureMLMLFlowValueException, match="all element must has the same length"):
...
def test_dataset_autogen_feature_names():
    ...
    with pytest.raises(
            AzureMLMLFlowValueException, match="features example rows must be the same length with labels array"
    ):
...
def test_normalize_evaluators_and_evaluator_config_args():
    ...
    assert _normalize_config(None, None) == (["default", "azureml"], {})
    with pytest.raises(
            AzureMLMLFlowValueException, match="`evaluator_config` argument must be a dictionary mapping each evaluator"
    ):
    ...
    with pytest.raises(
            AzureMLMLFlowValueException, match="evaluator_config must be a dict contains mapping from evaluator name to"
    ):
