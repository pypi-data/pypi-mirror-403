...
from mlflow.models.evaluation.base import EvaluationArtifact, EvaluationResult
...
def _hash_array_like_obj_as_bytes(data):
    ...
    if isinstance(data, pd.DataFrame):
        ...
        raise AzureMLMLFlowValueException(ErrorStrings.UnsupportedDataType)
...
class EvaluationDataset:
    ...
    def __init__(self, data, *, targets, name=None, path=None, feature_names=None):
        ...
        if name is not None and '"' in name:
            raise AzureMLMLFlowValueException(ErrorStrings.InvalidDatasetTemplate.format(type="name", value=name))
        if path is not None and '"' in path:
            raise AzureMLMLFlowValueException(ErrorStrings.InvalidDatasetTemplate.format(type="path", value=path))
        ...
        if feature_names is not None and len(set(feature_names)) < len(list(feature_names)):
            raise AzureMLMLFlowValueException(
        ...
        if isinstance(data, (np.ndarray, list)):
            ...
            if feature_names is not None:
                ...
                if num_features != len(feature_names):
                    raise AzureMLMLFlowValueException("feature name list must be the same length with feature data.")
            ...
            raise AzureMLMLFlowValueException(ErrorStrings.InvalidData)
...
class ModelEvaluator(metaclass=ABCMeta):
    ...
    def load_model(self, model, model_type):
        ...
        if isinstance(model, str):
            ...
            raise AzureMLMLFlowValueException(ErrorStrings.InvalidModelURI.format(instance="mlflow.pyfunc.PyFuncModel"))
        return model
...
def _normalize_evaluators_and_evaluator_config_args(
    ...
    if evaluators is None:
        ...
        if evaluator_config is not None:
            conf_dict_value_error = AzureMLMLFlowValueException(
        ...
        raise AzureMLMLFlowValueException(ErrorStrings.InvalidEvaluatorName)
...
def _evaluate(
    ...
    if len(eval_results) == 0:
        raise AzureMLMLFlowValueException(ErrorStrings.InvalidModelEvaluator)
