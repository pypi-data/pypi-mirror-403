...
from azureml.evaluate.mlflow.exceptions import AzureMLMLFlowUserException, AzureMLMLFlowValueException
...
def _evaluate_custom_metric(custom_metric_tuple, eval_df, builtin_metrics):
    ...
    if result is None:
        raise AzureMLMLFlowUserException(f"{exception_header} returned None.")
    ...
    def _evaluate_classifier(self):
        ...
        if self.is_binomial:
            if list(self.label_list) not in [[0, 1], [-1, 1]]:
                raise AzureMLMLFlowValueException(ErrorStrings.InvalidBinaryClassifierLabels)
            _logger.info(
        ...
        with TempDir() as temp_dir, matplotlib.rc_context(_matplotlib_config):
            ...
            with mlflow.utils.autologging_utils.disable_autologging():
                if model_type == "classifier":
                    ...
                    raise AzureMLMLFlowValueException(ErrorStrings.UnsupportedModelType.format(type=model_type))
