...
from devices_utils import get_hardware_identifier
...
class SystemUtils:
    ...
    def system_get_cpu_model() -> str:
        ...
        """
        try:
            cpu_info = SystemUtils.system_get_cpu_info()
    ...
    @staticmethod
    def get_devices_md5(uppercase: bool = False) -> str:
        ...
        md5_obj = HashUtils.hash_md5_string(formatted_str)

        # 4. 可选：转为大写
        if uppercase:
            return md5_obj.upper()
            ...
            for process in psutil.process_iter(['pid', 'name']):
                try:
                    ...
                    if process.info['name'].lower() == name.lower():
                        # 结束进程
                        if SystemUtils.process_kill_by_pid(process.info['pid'], force):
                            killed_count += 1
    ...
    def port_get_processes(port: int) -> list:
        ...
        try:
            # 检查TCP连接
            tcp_result = SystemUtils.port_check_process(port, 'tcp')
            if tcp_result:
                processes.append(tcp_result)

            # 检查UDP连接
            udp_result = SystemUtils.port_check_process(port, 'udp')
            if udp_result:
