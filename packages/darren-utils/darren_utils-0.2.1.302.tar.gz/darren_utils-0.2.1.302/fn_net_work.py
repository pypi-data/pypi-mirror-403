...
from .darren_ret import DarrenRet
...
class FNNetWork:
    def __init__(self, config_data):
        ...
        self.Appid = darren.utils.url_get_param(self.app_web, "AppId")
    ...
    def conmonsend(self, data) -> DarrenRet:
        data["Time"] = darren.time.get_timestamp(is_10_digits=True)
        data["Status"] = random.randint(10000, 99999)
        data_txt = darren.utils.json_parse_safe(data)
        headers = {
        ...
        if self.crypto_type == 3:
            aes_key = darren.string.random_string(24, uppercase=True, digits=True, lowercase=True)
            # print("随机aes_key", aes_key)
            encrypt = darren.aes.aes_encrypt_string(json.dumps(data_txt), aes_key, mode='CBC', padding_mode='PKCS7',
                                         iv=b'\x00' * 16)
            sign = darren.rsa.rsa_encrypt_string(aes_key, self.public_key, padding_scheme='PKCS1')
            send_data = {
                "a": encrypt,
                "b": sign
            }
        elif self.crypto_type == 2:
            encrypt = darren.aes.aes_encrypt_string(data_txt, self.crypto_key_aes)
            send_data = {
                "a": encrypt
            }
        else:
        ...
        json_data = darren.utils.json_parse_safe(response.text)
        ...
        if self.crypto_type == 3:
            ...
            if data.get('Api') in rsa_api_list:
                # 强制走rsa加密的接口
                tmp_aes_key = darren.rsa.rsa_decrypt_with_public_key(b, self.public_key, padding_scheme='PKCS1')
                if tmp_aes_key is None:
                    # print("RSA解密失败")
                    return DarrenRet.error(message="RSA解密失败")
                # print("tmp_aes_key", tmp_aes_key)
                data_txt = darren.aes.aes_decrypt_string(a, tmp_aes_key, mode='CBC', padding_mode='PKCS7', iv=b'\x00' * 16)
            ...
            a=json_data.get("a")
            data_txt = darren.aes.aes_decrypt_string(a, self.crypto_key_aes, mode='CBC', padding_mode='PKCS7', iv=b'\x00' * 16)

        else:
            data_txt = json.dumps(json_data)
        # print("请求返回", data_txt)
        json_data = darren.utils.json_parse_safe(data_txt)
        server_time = json_data.get("Time")
        current_time = darren.time.get_timestamp(is_10_digits=True)
