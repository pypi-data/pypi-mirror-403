...
def get_devices_md5(uppercase: bool = False) -> tuple[str, str]:

    """
    对硬件信息字典排序并生成MD5值
    :param hardware_info: get_hardware_identifier返回的字典
    :param uppercase: MD5结果是否大写（默认小写）
    :return: 32位MD5字符串
    """
    # 1. 按key升序排序，生成有序键值对列表
    hardware_info = get_hardware_identifier()
    sorted_items = sorted(hardware_info.items(), key=lambda x: x[0])

    # 2. 格式化：将None转为"未知"，拼接为"key:value|key:value"格式
    formatted_str = "|".join([f"{k}:{v if v is not None else '未知'}" for k, v in sorted_items])

    # 3. 生成MD5（统一使用utf-8编码，避免跨平台编码问题）
    md5_obj = hashlib.md5(formatted_str.encode("utf-8"))
    md5_result = md5_obj.hexdigest()

    # 4. 可选：转为大写
    if uppercase:
        md5_result = md5_result.upper()

    return md5_result, formatted_str  # 返回MD5和格式化字符串（便于调试）
...
#         print(f"{key}: {value}")
#     hardware_md5, formatted_str = get_sorted_hardware_md5(hardware_info, uppercase=False)
#     print(f"设备唯一标识（MD5）：{hardware_md5}")
#     print(f"格式化字符串：{formatted_str}")
#     #设备唯一标识（MD5）：B42A4A9137D367C13D944EAD4B30ECE7