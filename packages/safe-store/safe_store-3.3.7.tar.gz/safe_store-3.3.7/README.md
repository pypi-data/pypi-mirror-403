...
# 2. Add a document.
with store:
    # returns {'num_chunks_added': int, 'num_chunks_ignored': int}
    stats = store.add_document("path/to/document.pdf")
    print(f"Added {stats['num_chunks_added']} chunks, ignored {stats['num_chunks_ignored']} empty/invalid chunks.")
...
`safe_store` now includes enhanced parsing and chunking capabilities for structured data and code:

*   **CSV & Excel**: Rows are parsed with their headers included in each line (e.g., `Row 1: Header=Value | Header2=Value2`). This ensures that even if a row is chunked separately, the context of what the values represent is never lost.
*   **JSON**: Files are pretty-printed before chunking to ensure that structure is preserved as much as possible.
*   **Code**: Use the new `recursive` chunking strategy for code files. It intelligently splits text based on hierarchy (e.g., classes, functions, blocks) rather than blindly cutting at token limits.

```python
# Use recursive chunking for code or structured text
store = safe_store.SafeStore(
    db_path="code_base.db", 
    chunking_strategy="recursive"
)
store.add_document("src/main.py")
```
...
# Get a list of all available vectorizers
...
*   `add_document(path, ..., skip_chunking=False)`: Parses, optionally chunks, vectorizes, and stores a document. **Returns** `{'num_chunks_added': int, 'num_chunks_ignored': int}`. Handles unsupported formats, empty files, or encrypted content gracefully by logging a warning and returning 0 added chunks.
*   `add_text(unique_id, text, ..., skip_chunking=False)`: Vectorizes and stores raw text. **Returns** `{'num_chunks_added': int, 'num_chunks_ignored': int}`.
