...
try:
    __version__ = importlib.metadata.version("terra_browse_variables")
except importlib.metadata.PackageNotFoundError:
    __version__ = "unknown"


class TerraBrowseVariables(TerraBaseWidget):
    _esm = TerraBaseWidget.get_autoloader() + """
    function render({ model, el }) {
        // create an instance of the component
        let component = document.createElement('terra-browse-variables')
         ...
         */
        component.catalog = model.get('catalog')
    ...
    catalog = traitlets.Unicode('').tag(sync=True)
