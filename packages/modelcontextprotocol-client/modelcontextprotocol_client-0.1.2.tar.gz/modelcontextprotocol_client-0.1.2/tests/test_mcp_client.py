...
async def test_mcp():
    ...
    try:
        ...
        result = await session.call_tool("get_user_info", {"user_id": "123"})
        print(f"   ✓ get_user_info('123') = {result.content[0].text}")
        
        # Call search_items tool
        print("\n   Testing search_items tool...")
        result = await session.call_tool("search_items", {"query": "test", "limit": 3})
        ...
        result = await session.get_prompt("code_review_prompt", {
            "code": "def hello():\n    print('world')",
            "language": "python"
        })
        ...
        result = await session.read_resource("docs://getting-started")
        print(f"   ✓ Read docs (first 100 chars): {result.contents[0].text[:100]}...")
        
        # Read data resource
        print("\n   Testing data://sample resource...")
        result = await session.read_resource("data://sample")
