...
from .preprocess.base import TextGenerationPreprocessArgs
# from .preprocess.preprocess_for_inference import TextGenerationForInfer
...
class TextGenerationRunner(BaseRunner):
    ...
    def run_preprocess_for_finetune(self, component_args: Namespace, unknown_args: List[str]) -> None:
        ...
        from .preprocess.preprocess_for_finetune import TextGenerationPreprocessForFinetune
        ...
        preprocess_arg_parser = HfArgumentParser([TextGenerationPreprocessArgs])
        output_args = preprocess_arg_parser.parse_args_into_dataclasses(unknown_args, return_remaining_strings=True)
        preprocess_args: TextGenerationPreprocessArgs = output_args[0]

        logger.info(f"unused args - {output_args[1]}")

        preprocess_obj = TextGenerationPreprocessForFinetune(component_args, preprocess_args)
    ...
    def run_finetune(self, component_plus_preprocess_args: Namespace) -> None:
        ...
        from .finetune.finetune import TextGenerationFinetune

        finetune_obj = TextGenerationFinetune(vars(component_plus_preprocess_args))
