...
QWEN_32B_DEEPSPEED_CONFIG = str(Path(__file__).parent.resolve() / "zero3_qwen3_32b.json")
...
# Models that should auto-enable Liger Kernel for memory efficiency
# Limiting to specific model names instead of model types
# To add a new model: Simply add the model name pattern to this list
LIGER_KERNEL_SUPPORTED_MODELS = [
    "Qwen3-32B",
...
FORCE_FLASH_ATTENTION_2_MODEL_TYPES = [
    ...
    HfModelTypes.PHI_LONGROPE,
    PHI3_MINI_4K_INSTRUCT_MODEL_TYPE,
    HfModelTypes.QWEN3,  # Added for Qwen3 Flash Attention support
]
...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=str2bool,
        default="true",
        help="eval on start of the training loop",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str2bool,
        default="false",
        help="If set to true, will use Liger Kernel for memory-efficient training",
    )
...
def resolve_deepspeed_config(args: Namespace, model_type: str) -> str:
    ...
    if model_type == HfModelTypes.QWEN3:
        logger.info(f"Using deepspeed config for qwen3 model - {QWEN_32B_DEEPSPEED_CONFIG}.")
        # Force deepspeed stage 3
        args.deepspeed_stage = 3
        return QWEN_32B_DEEPSPEED_CONFIG
...
def setup_and_validate_deepspeed(args: Namespace, do_validate: bool = True):
    ...
    args.deepspeed = resolve_deepspeed_config(args, model_type)
...
def set_gradient_checkpointing(args: Namespace):
    ...
    # Disable gradient checkpointing for QWEN3 when batch size is 1
    if (
        hasattr(args, "model_type")
        and args.model_type == HfModelTypes.QWEN3
        and hasattr(args, "per_device_train_batch_size")
        and args.per_device_train_batch_size == 1
    ):
        logger.info(
            f"Identified model type: {args.model_type} with per_device_train_batch_size=1. "
            "Disabling gradient checkpointing for better performance."
        )
...
def get_model_type(args: Namespace) -> str:
    """Get model type from model config"""
    config = AutoConfig.from_pretrained(args.model_name_or_path)
    return config.model_type    

def should_enable_liger_kernel(model_name: str) -> bool:
    """
    Check if Liger Kernel should be auto-enabled for this model.

    Args:
        model_name: The model name or path

    Returns:
        True if model matches any pattern in LIGER_KERNEL_SUPPORTED_MODELS
    """
    if not model_name:
        return False

    for supported_model in LIGER_KERNEL_SUPPORTED_MODELS:
        # Use regex to match exact model names or patterns with word boundaries
        # This matches "Qwen3-32B" but not "fake-qwen3-32b-custom"
        pattern = rf"\b{re.escape(supported_model)}\b"
        if re.search(pattern, model_name, re.IGNORECASE):
            logger.info(f"Liger Kernel auto-enabled: matched '{supported_model}' in '{model_name}'")
            return True
...
def finetune(args: Namespace):
    ...
    # Map evaluation_strategy to eval_strategy
    if hasattr(args, 'evaluation_strategy'):
        setattr(args, 'eval_strategy', args.evaluation_strategy)
    ...
    logger.info(f"Evaluation Strategy: {getattr(args, 'eval_strategy', None)}")

    # Auto-enable Liger Kernel for supported models
    model_name = getattr(args, 'model_name', None)
    current_use_liger = getattr(args, 'use_liger_kernel', False)

    if not current_use_liger and should_enable_liger_kernel(model_name):
        logger.info(f"Auto-enabling Liger Kernel for model: {model_name}")
        setattr(args, 'use_liger_kernel', True)
    ...
    # Override the save strategy to 'no' to avoid issue with deepspeed
