...
class FramesSection(CollapsibleSection):
    ...
    def configure_signals(self):
        ...
        self.chunk_size_component.field.valueChanged.connect(self.on_chunk_size_change)
    ...
    def on_custom_range_change(self):
        self.dialog.store.set_custom_range(self.custom_range_component.field.text())
        self._set_sequences()
    ...
    def on_scout_frames_change(self):
        self.dialog.store.set_scout_frames(self.scout_frames_component.field.text())
        self._set_sequences()
    ...
    def _set_sequences(self):
        ...
        if self.main_sequence:
            self.main_sequence.cap_chunk_count(k.MAX_TASKS)
        ...
        if self.dialog.configuration_tab:
            self.dialog.configuration_tab.section("InfoSection").update(
                self.main_sequence, self.scout_sequence
            )

    def populate_from_store(self):
        super(FramesSection, self).populate_from_store()
    ...
    def resolve(self, expander, **kwargs):
        ...
        for i, chunk in enumerate(chunks):
            ...
            tasks.append(
                {"command": this_expander.evaluate(template), "frames": str(chunk)}
            )
...
def scene_frame_spec():
    ...
    if rend_time_type == REND_TIME_TYPE_SINGLE:
        ...
        return "{:d}-{:d}".format(int(frame_range.start), int(frame_range.end))
