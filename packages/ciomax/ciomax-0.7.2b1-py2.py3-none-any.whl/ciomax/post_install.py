...
ADDTLMAXPLUGINS = os.path.expanduser(os.path.join(os.environ.get("PROGRAMFILES"), "AutoDesk"))
...
""".format(
    ...
    path = ADDTLMAXPLUGINS
    if not os.path.exists(path):
        # sys.stderr.write(
        #     "No 3ds startup directory in your local app data: {}\n".format(PLATFORM)
        # )
        sys.stderr.write(
            "No Autodesk 3dsMax directory in your Progam Files: {}\n".format(PLATFORM)
        )
        manual_instructions(content)
        sys.exit(0)

    # print("All paths we're going to look at:", os.listdir(path))
    # for folder in [f for f in os.listdir(path) if f.startswith("20")]:
    print([f for f in os.listdir(path) if f.startswith("3ds Max 20")])
    for folder in [f for f in os.listdir(path) if f.startswith("3ds Max 20")]:
        # path = os.path.join(
        #     LOCALAPPDATA, "Autodesk", "3dsMax", folder, "ENU", "scripts", "startup"
        # )

        
        path = os.path.join(
            ADDTLMAXPLUGINS, folder, "Plugins"
        )
        # print("Going to install to this folder:", path)
        ...
        try:
            ...
            sys.stdout.write(
                "Failed to write a Conductor startup file to: {}\n".format(stub_file)
            )
            pass
