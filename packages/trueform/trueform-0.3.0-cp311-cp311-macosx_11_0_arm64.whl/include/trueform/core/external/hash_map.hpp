//          Copyright Malte ska2rupke 2017.
...
#ifdef _MSC_VER
#define ska2_NOINLINE(...) __declspec(noinline) __VA_ARGS__
#else
#define ska2_NOINLINE(...) __VA_ARGS__ __attribute__((noinline))
#endif

namespace ska2 {
// this is modified from original
// so it works on emscripten
...
private:
  ...
  template <typename Key, typename... Args>
  ska2_NOINLINE(std::pair<iterator, bool>)
