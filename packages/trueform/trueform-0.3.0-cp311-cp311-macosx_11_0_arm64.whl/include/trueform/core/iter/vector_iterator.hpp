...
namespace tf::iter {
template <typename Iterator, std::size_t BlockSize> struct vector_api_handle {
public:
  ...
  vector_api_handle() = default;
  vector_api_handle(Iterator iter) : iter{std::move(iter)} {}
  ...
  auto dereference() const -> reference {
    return tf::make_vector_view<BlockSize>(&(*iter));
  }
...
struct vector_iterator : stride_api<vector_iterator<Iterator, BlockSize>,
                                    vector_api_handle<Iterator, BlockSize>> {
private:
  using base_t = stride_api<vector_iterator<Iterator, BlockSize>,
                            vector_api_handle<Iterator, BlockSize>>;
  using handle_t = vector_api_handle<Iterator, BlockSize>;
...
auto make_vector_iterator(Iterator iter) {
  return vector_iterator<Iterator, BlockSize>{std::move(iter)};
}
