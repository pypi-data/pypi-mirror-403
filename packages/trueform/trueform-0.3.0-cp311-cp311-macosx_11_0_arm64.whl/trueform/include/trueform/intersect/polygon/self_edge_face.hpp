...
auto self_edge_face(Record &&record, const Handle0 &handle0,
  ...
  for (decltype(poly0_size) j = 0; j < poly0_size; j++) {
    ...
    if (std::find(indices1.begin(), indices1.end(), indices0[j]) !=
            indices1.end() ||
        std::find(indices1.begin(), indices1.end(), indices0[next_j]) !=
            indices1.end())
      continue;
  ...
  for (decltype(poly1_size) j = 0; j < poly1_size; j++) {
    ...
    if (std::find(indices0.begin(), indices0.end(), indices1[j]) !=
            indices0.end() ||
        std::find(indices0.begin(), indices0.end(), indices1[next_j]) !=
            indices0.end())
      continue;
