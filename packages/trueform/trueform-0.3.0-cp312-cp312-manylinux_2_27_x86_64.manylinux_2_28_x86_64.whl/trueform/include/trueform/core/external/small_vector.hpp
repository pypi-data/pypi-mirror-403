...
public:
  ...
  /*
    ...
    explicit SmallVector(const
    tf::external::llvm_vecsmall::iterator_range<RangeTy> &R) :
    SmallVectorImpl<T>(N) { this->append(R.begin(), R.end());
    }
  */
...
template <typename T>
inline void swap(tf::external::llvm_vecsmall::SmallVectorImpl<T> &LHS,
                 tf::external::llvm_vecsmall::SmallVectorImpl<T> &RHS) {
  LHS.swap(RHS);
}

/// Implement std::swap in terms of SmallVector swap.
template <typename T, unsigned N>
inline void swap(tf::external::llvm_vecsmall::SmallVector<T, N> &LHS,
                 tf::external::llvm_vecsmall::SmallVector<T, N> &RHS) {
  LHS.swap(RHS);
}
} // namespace std
