...
namespace tf::core {
template <std::size_t Dims, typename Policy0, typename Policy1> struct ray {
  ...
  ray() = default;
  ray(const tf::point_like<Dims, Policy0> &origin,
      const tf::vector_like<Dims, Policy1> &direction)
      : origin{origin}, direction{direction} {}

  template <typename Policy2, typename Policy3>
  auto operator=(const ray<Dims, Policy2, Policy3> &other) -> std::enable_if_t<
...
auto make_ray(const point_like<N, T0> &origin,
              const vector_like<N, T1> &direction) -> ray<N, T0, T1> {
  return ray<N, T0, T1>(origin, direction);
}
} // namespace tf::core
