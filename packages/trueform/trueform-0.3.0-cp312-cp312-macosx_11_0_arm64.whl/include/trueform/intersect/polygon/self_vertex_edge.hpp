...
auto self_vertex_edge(Record &&record, const Handle0 &handle0,
    ...
    for (decltype(poly1_size) j = 0; j < poly1_size; j++) {
      if (handle0.representation.vertex[i] && handle1.representation.edge[j]) {
        ...
        if (!(indices0[i] == indices1[j] || indices0[i] == indices1[next_j]) &&
            tf::intersects(handle0.polygon[i],
      ...
      if (handle1.representation.vertex[j] && handle0.representation.edge[i]) {
        if (!(indices1[j] == indices0[i] || indices1[j] == indices0[next_i]) &&
            tf::intersects(handle1.polygon[j],
                           tf::make_segment_between_points(
