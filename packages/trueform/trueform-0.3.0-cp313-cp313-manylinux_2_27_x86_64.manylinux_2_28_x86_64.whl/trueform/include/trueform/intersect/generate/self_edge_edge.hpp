...
#include "../polygon/self_edge_edge.hpp"
...
template <typename Handle0, typename Handle1, typename Index, typename T,
          std::size_t Dims>
auto self_edge_edge(const Handle0 &handle0, const Handle1 &handle1,
  ...
  tf::intersect::polygon::self_edge_edge(
      [&](Index e0_0, Index e0_1, Index e1_0, Index e1_1, const auto &pt) {
        ...
        intersections.push_back(make_canonical_intersection<Index>(
            Index(handle0.id), Index(handle1.id), {e0_0, tf::topo_type::edge},
            {e1_0, tf::topo_type::edge}, id));
      },
      handle0, handle1);
}
} // namespace tf::intersect::generate
