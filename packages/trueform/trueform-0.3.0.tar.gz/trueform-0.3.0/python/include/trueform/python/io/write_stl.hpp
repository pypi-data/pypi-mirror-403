...
/// @brief Template implementation for write_stl
/// @tparam Index The index type (int or int64_t)
/// @param faces Numpy array of face indices (N, 3) with dtype Index
/// @param points Numpy array of points (M, 3) with dtype float32
...
auto write_stl_impl(
    nanobind::ndarray<nanobind::numpy, Index, nanobind::shape<-1, 3>>
        faces_array,
  ...
  float *data_pts = static_cast<float *>(points_array.data());
  ...
  std::size_t count_fcs = faces_array.shape(0) * 3;
  auto faces = tf::make_blocked_range<3>(tf::make_range(data_fcs, count_fcs));
  ...
  if (transformation_opt.has_value()) {
    ...
    auto transformed_polygons = polygons | tf::tag(tf::make_frame(transformation_view));
    return tf::write_stl(transformed_polygons, filename);
  } else {
