...
class ConfigPropagationManager:
    ...
    def collect_dependencies_needing_configs(
        self, sub_agents: list[SubAgentType], dependency_type: ItemType
    ) -> list[Any]:
        """Collect dependencies (tools or MCPs) that don't have configs in any sub-agent.

        This method collects only the dependencies that would need configuration
        from the parent, excluding dependencies that already have configs defined
        in at least one sub-agent.
        ...
        Returns:
            list[Any]: List of unique dependencies that need parent configs.
        """
        ...
        # Collect all dependencies that have configs in any sub-agent
        configured_deps = self._collect_configured_dependency_names(sub_agents, dependency_type)

        # Now collect all dependencies, filtering out those with configs
        ...
        # Filter out dependencies that already have configs
        return [dep for dep in context.dependencies if name_extractor(dep) not in configured_deps]

    def _collect_configured_dependency_names(
        self, sub_agents: list[SubAgentType], dependency_type: ItemType
    ) -> set[str]:
        """Collect names of all dependencies that have configs in any sub-agent.
        ...
        Returns:
            set[str]: Set of dependency names that have configs.
        """
        configured_deps = set()

        def collect_configured(agent: Agent) -> None:
            """Recursively collect configured dependencies from agent hierarchy.

            Args:
                agent (Agent): The agent to collect from.
            """
            # Get the appropriate configs based on type
            configs = agent.tool_configs if dependency_type == ItemType.TOOL else agent.mcp_configs
            if configs:
                configured_deps.update(configs.keys())

            # Recursively check nested agents
            for nested_agent in agent.agents or []:
                if isinstance(nested_agent, Agent):
                    collect_configured(nested_agent)

        # Collect all configured dependencies
        for sub_agent in sub_agents or []:
            if isinstance(sub_agent, Agent):
                collect_configured(sub_agent)

        return configured_deps
