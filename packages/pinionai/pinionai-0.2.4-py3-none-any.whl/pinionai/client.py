_Bl="Twilio 'from' number not configured. Cannot send SMS."
_Bk='completedAt'
_Bj='Error: URL template evaluation failed.'
_Bi='contentType'
_Bh='token_type'
_Bg='client_credentials'
_Bf='function_call'
_Be='Error: AI client is not configured.'
_Bd='Cannot get Gemini response: AI client is not initialized.'
_Bc='Using environment credentials/IAM role for S3.'
_Bb='boto3 library is required for S3 operations.'
_Ba='Using Application Default Credentials for GCS.'
_BZ='google-cloud-storage library is required for GCS operations.'
_BY='action_action_key'
_BX='__builtins__'
_BW='sourceVariable'
_BV='promptName'
_BU='unknown_intent'
_BT='Error: AI response was malformed (function call had no name).'
_BS='resp_mime_type'
_BR='gemini-3-pro-preview'
_BQ='safetySettings'
_BP='mime_type'
_BO='Cannot update session: session not properly initialized.'
_BN='variables'
_BM='accountId'
_BL='DEEPSEEK_API_KEY'
_BK='ANTHROPIC_API_KEY'
_BJ='OPENAI_API_KEY'
_BI='EMAIL_FROM_ADDRESS'
_BH='EMAIL_API_KEY'
_BG='GEMINI_API_KEY'
_BF='GCP_REGION'
_BE='GCP_PROJECT'
_BD='WEBSOCKET_URL'
_BC='user_input'
_BB='Journey token not available.'
_BA='Accept-Encoding'
_B9='client_secret'
_B8='client_id'
_B7='grant_type'
...
_B2='delivery_method'
_B1='format'
_B0='destinationVariable'
_A_='function'
_Az='header'
_Ay='gemini-2.0-flash'
_Ax='model_provider'
_Aw='agentDefaultIntent'
_Av='intentContainsPairsList'
_Au='intentNameList'
_At='value'
_As='action_flow'
_Ar='action_type'
...
_Ai='privacy'
_Ah='prompt_type'
_Ag='gcs_service_account'
_Af='ragStoreVectorDistanceThreshold'
_Ae='ragStoreTopK'
_Ad='ragStoreResourceId'
_Ac='transferPasskeyFlag'
_Ab='transferAllowed'
_Aa='access_token'
_AZ='Content-Encoding'
_AY='CONCURRENT_OPERATIONS_LIMIT'
_AX='TWILIO_NUMBER'
...
class AsyncPinionAIClient:
	...
	def __init__(self,create_key,agent_id,host_url,client_id,client_secret=_A,version=_A,token=_A,session_id=_A,raw_session_data=_A,initial_engagement_data=_A,grpc_server_address=_A,gcp_project_id=_A,gcp_region=_A,gemini_api_key=_A,twilio_account_sid=_A,twilio_auth_token=_A,twilio_number=_A,email_api_key=_A,email_from_address=_A,openai_api_key=_A,anthropic_api_key=_A,deepseek_api_key=_A,concurrent_operations_limit=_A):
		...
		self._agent_id=agent_id;self._host_url=host_url.rstrip('/');self._client_id=client_id;self._client_secret=client_secret;self._version=version;self._grpc_server_address=grpc_server_address or os.environ.get(_BD,'localhost:50051');self._encryption_manager=EncryptionManager(client_secret=agent_id);self._gcp_project_id=gcp_project_id or os.environ.get(_BE);self._gcp_region=gcp_region or os.environ.get(_BF);self._gemini_api_key=gemini_api_key or os.environ.get(_BG);self._twilio_account_sid=twilio_account_sid or os.environ.get(_AV);self._twilio_auth_token=twilio_auth_token or os.environ.get(_AW);self._twilio_number=twilio_number or os.environ.get(_AX);self._email_api_key=email_api_key or os.environ.get(_BH);self._email_from_address=email_from_address or os.environ.get(_BI);self._openai_api_key=openai_api_key or os.environ.get(_BJ);self._anthropic_api_key=anthropic_api_key or os.environ.get(_BK);self._deepseek_api_key=deepseek_api_key or os.environ.get(_BL);self._concurrent_operations_limit=int(concurrent_operations_limit or os.environ.get(_AY,'50'));self._semaphore=asyncio.Semaphore(self._concurrent_operations_limit);self._http_session=httpx.AsyncClient(base_url=self._host_url,timeout=12e1,follow_redirects=_D);self._token=token;self._session_id=session_id;self._raw_session_data=raw_session_data;self.var={};self.engagement=''
	...
	async def create_from_stream(cls,file_stream,host_url,key_secret=_A):
		...
		async with httpx.AsyncClient(base_url=host_url.rstrip('/'),timeout=12e1)as http_session:
			try:
				...
				compressed_data=gzip.compress(json.dumps(api_payload,default=_json_datetime_serializer).encode(_M));response=await http_session.post('/filesession',headers={_O:_K,_AZ:_l},content=compressed_data);response.raise_for_status();resp_json=response.json()
		...
		decryption_key=resp_json.get(A);access_token=resp_json.get(_Aa);version=resp_json.get('version_type')
		...
		agent_id=resp_json.get(_B,{}).get(_E,{}).get('agentId');session_id=resp_json.get(_B,{}).get(_E,{}).get('uid');client_id=resp_json.get(_B,{}).get(_E,{}).get(_BM)
	...
	def _configure_client_from_session_data(self):
		...
		if _B in self._raw_session_data and self._raw_session_data[_B]:
			...
			if self._rag_stores_by_name:first_store_cfg=next(iter(self._rag_stores_by_name.values()));self.var[_Ad]=first_store_cfg.get(_u,'');self.var[_Ae]=first_store_cfg.get(_v,10);self.var[_Af]=first_store_cfg.get(_w,.5)
			else:self.var[_Ad]='';self.var[_Ae]=10;self.var[_Af]=.5
			self._account_id=self._raw_session_data[_B][_E].get(_BM);self._customer_id=self._raw_session_data[_B][_E].get('customerId');self._agent_id_from_api=agent_data.get('agentId');self._agent_description=self._clean_text(agent_data.get(_N,''))
	...
	def _initialize_genai_client(self):
		...
		if self.var[_m]:
			...
			if conn_config and conn_config.get(_n)==_Ag:
	...
	async def process_user_input(self,user_input='',sender=_b):
		...
		if not self.transfer_requested:
			...
			if self.next_intent:self.var['prompt_resp_var']=self.next_intent;prompt_type=self.var[_Ah]=_A9;user_input=self.next_intent;intent_data=self._get_intent_details(self.next_intent);self.next_intent=''
			else:
				...
				prompt_type=self.var.get(_Ah);intent_data=self._get_intent_details(self.var[prompt_resp_var_name])
			self.var[_c]=''
			...
			intent_type=intent_data[_F];self.var[A]=intent_data[_C];self._privacy_level=self.var['privacy_resp']=intent_data[_Ai];self.current_pipeline=self.var['pipeline_resp']=intent_data[_x];delivery_method=intent_data[_Aj];language=intent_data[_d];intent_input_vars=self._normalize_intent_vars(intent_data.get(B,{}));context_flow=intent_data.get(E,{});intent_subprompt=context_flow.get(C,{});intent_finalprompt=context_flow.get(D,{});intent_preprocess=context_flow.get(F,{});intent_subprocess=context_flow.get(G,{});intent_finalprocess=context_flow.get(H,{});intent_actionprocess=intent_data.get(_Ak,{})
			...
			if intent_type!=_y:
				if intent_input_vars:
					agent_vars_config=self._raw_session_data.get(_B,{}).get(_E,{}).get(_B,{}).get(_I,{}).get(_BN,[])
				...
				if self._class_intent_data:intent_data=self._class_intent_data;user_input=self._class_message;intent_type=intent_data[_F];self.var[A]=intent_data[_C];delivery_method=intent_data[_Aj];language=intent_data[_d];intent_input_vars=self._normalize_intent_vars(intent_data.get(B,{}));context_flow=intent_data.get(E,{});intent_subprompt=context_flow.get(C,{});intent_finalprompt=context_flow.get(D,{});intent_preprocess=context_flow.get(F,{});intent_subprocess=context_flow.get(G,{});intent_finalprocess=context_flow.get(H,{});intent_actionprocess=intent_data.get(_Ak,{});self._privacy_level=intent_data[_Ai];self.current_pipeline=intent_data[_x]
			if intent_input_vars:
				...
				if waiting_for_inputs:
					...
					if final_response:self.chat_messages.append({_S:_Al,_R:final_response})
					...
					if not self.authorized and'private'in self._privacy_level or J in self._privacy_level:
						...
						if intent_data[K]:
							...
							if customer_id and enrolled:
								pipeline_key=self.var[L]if J in self._privacy_level else self.var['authentication_pipeline'];json_payload={_Am:pipeline_key,_p:{_f:delivery_method,_g:self.phone_number},_An:{_Ao:self.unique_id,_g:self.phone_number},_E:{_Ap:self._session_id},_d:language}
								if self.var.get(_q):json_payload[_Aq]=self.var[_q]
								...
								auth_response_msg=f"You are not enrolled. We are sending a link to {self.phone_number} so that you can enroll your device.";json_payload={_Am:self.var['enroll_pipeline'],_p:{_f:delivery_method,_g:self.phone_number},_An:{_Ao:self.unique_id,_g:self.phone_number},_E:{_Ap:self._session_id},_d:language}
								if self.var.get(_q):json_payload[_Aq]=self.var[_q]
			...
			if not final_response:
				if intent_type=='fixed':fixed_message_template=self._clean_text(intent_data['fixedResponseMessage']);final_response=self._evaluate_f_string(fixed_message_template,self.var)
					...
					action_details_from_intent=self._map_to_standard_action_details(intent_data);action_type=action_details_from_intent.get(_Ar)
					if action_type=='journey':self.var,final_response=await self._run_journey_action(self.var,action_details_from_intent)
						...
						action_flow=action_details_from_intent.get(_As)
						if action_flow:
	...
	async def update_pinion_session(self):
		...
		if not self._session_id or not self._token or not self._raw_session_data:logger.error(_BO);return
	...
	async def start_grpc_client_listener(self,sender_id=_b):
		...
		try:credentials=grpc.ssl_channel_credentials();self._grpc_channel=agrpc.secure_channel(self._grpc_server_address,credentials);self._grpc_stub=ChatServiceStub(self._grpc_channel);logger.info(f"gRPC client connected to {self._grpc_server_address} for session {self._session_id} as {sender_id}");self._grpc_listener_task=asyncio.create_task(self._grpc_read_handler(self.grpc_sender_id,self._session_id,self._grpc_stub));return _D
			...
			logger.error(f"gRPC connection failed: {e.code()} - {e.details()}",exc_info=_D)
			if self._grpc_channel:await self._grpc_channel.close();self._grpc_channel=_A
			raise PinionAIGrpcError(f"Failed to connect to gRPC server: {e.details()}")from e
		except Exception as e:
			logger.error(f"An unexpected error occurred while starting gRPC client: {e}",exc_info=_D)
			if self._grpc_channel:await self._grpc_channel.close();self._grpc_channel=_A
			raise PinionAIGrpcError(f"Failed to start gRPC client listener: {e}")from e
	...
	async def send_grpc_message(self,message_text):
		...
		try:recipient_id=_Al if self.grpc_sender_id==_b else _b;request=ChatMessageRequest(thread_id=1,message=message_text,sender_id=self.grpc_sender_id,recipient_id=recipient_id,session_id=str(self._session_id));await self._grpc_stub.SendMessage(request);logger.info(f"gRPC message sent by {self.grpc_sender_id}: {message_text}")
	...
	def _extract_data_from_session(self,session_data_root):
		'Extracts variables and agent configuration into the var dictionary.';var_dict={};agent_config=session_data_root.get(_E,{}).get(_B,{}).get(_I,{});global_to_self_attr_map={_BD:'_grpc_server_address',_BE:'_gcp_project_id',_BF:'_gcp_region',_BG:'_gemini_api_key',_AV:'_twilio_account_sid',_AW:'_twilio_auth_token',_AX:'_twilio_number',_BH:'_email_api_key',_BI:'_email_from_address',_BJ:'_openai_api_key',_BK:'_anthropic_api_key',_BL:'_deepseek_api_key',_AY:'_concurrent_operations_limit'}
		for variable_def in agent_config.get(_BN,[]):
			name=variable_def[_C];value=variable_def[_At];var_type=variable_def.get(_F)
			if variable_def.get(_AC)=='global':
				os.environ[name]=str(value)
				if name in global_to_self_attr_map:
					attr_name=global_to_self_attr_map[name]
					if name==_AY:self._semaphore=asyncio.Semaphore(int(value));self._concurrent_operations_limit=int(value);logger.info(f"Updated concurrent operations limit to {value} from session configuration.")
					else:setattr(self,attr_name,str(value))
		...
		for intent in agent_config.get('intents',[]):
			...
			intent_privacy_pairs.append((intent_name,intent[_Ai]));intent_type_pairs.append((intent_name,intent[_F]));intent_contains_word_pairs.append((intent_name,intent.get('containsWord','')));intent_action_key_pairs_raw.append((intent_name,intent.get(_x,'')))
		var_dict[_Au]=intent_names;var_dict['noInputIntentNameList']=no_input_intent_names;var_dict['inputIntentNameList']=input_intent_names;var_dict['intentPrivacyPairsList']=intent_privacy_pairs;var_dict['intentTypePairsList']=intent_type_pairs;var_dict[_Av]=intent_contains_word_pairs;resolved_action_key_pairs=[]
		...
		var_dict['intentActionPairsList']=resolved_action_key_pairs;var_dict[_AD]=agent_config.get(_AD);var_dict[_Aw]=agent_config.get('defaultIntent');return var_dict
	...
	async def _prompt_response(self,target_prompt,user_input=''):
		...
		self.var[_Ah]=prompt_config[_F];prompt_resp_var_name=prompt_config[_AE];prompt_body_template=self._clean_text(prompt_config[_W]);current_prompt_text=self._evaluate_f_string(prompt_body_template,self.var,user_input=user_input);model_provider=prompt_config.get(_Ax,_e).lower();model_name=prompt_config.get(_X);llm_response_text=_A;function_calls=_A;system_instruction=self._evaluate_f_string(self._agent_description or'',self.var)
		if model_provider==_e:
			...
			if prompt_config.get(_AF):
				...
				if file_uri:content_parts.append(Part.from_uri(file_uri=file_uri,mime_type=prompt_config[_BP]))
			content_parts.append(current_prompt_text);gen_config=google_genai_types.GenerateContentConfig(system_instruction=system_instruction if system_instruction else _A,temperature=prompt_config[_AG],top_p=prompt_config[_Y],top_k=prompt_config[_AH],candidate_count=prompt_config[_AI],max_output_tokens=prompt_config[_AJ],stop_sequences=prompt_config.get('stop')or _A,safety_settings=prompt_config.get(_BQ))
			if model_name in[_BR]and prompt_config.get(_i)is not _A:gen_config.thinking_config=google_genai_types.ThinkingConfig(thinking_level=prompt_config.get(_i))
			if prompt_config.get(_AK):gen_config.response_schema=prompt_config[_AK];gen_config.response_mime_type=prompt_config.get(_BS,_K)
		...
		if function_calls:
			...
			if not function_name:logger.error(f"LLM response contained a function call with no name: {function_call}");return self.var,_BT
		...
		if llm_response_text:
			if prompt_config[_F]==_A9:
				...
				matched_intent_key=self._match_intent_from_text(raw_response,self.var.get(_Av,[]),self.var.get(_Aw,_BU),self.var.get(_Au,[]));self.var[prompt_resp_var_name]=matched_intent_key
	...
	async def _async_prompt_response(self,target_prompt,user_input='',var_snapshot=_A):
		...
		if function_calls:
			function_call=function_calls[0];function_name=function_call.get(_C);function_args=function_call.get(_J,{})
			if not function_name:logger.error(f"LLM async response contained a function call with no name: {function_call}");return prompt_resp_var_name,_BT
		...
		if llm_response_text:
			if prompt_config[_F]==_A9:
				...
				final_value_for_var=self._match_intent_from_text(raw_response,current_var.get(_Av,[]),current_var.get(_Aw,_BU),current_var.get(_Au,[]))
	...
	async def _select_mcp_config(self,mcp_group_names,current_var,user_input,mcp_group_config):
		...
		select_mcp_declaration=FunctionDeclaration(name=B,description="Select the most appropriate MCP from the list based on the user's request.",parameters={_F:_s,_AM:{A:{_F:_AP,_N:'The name of the selected MCP.',_AO:[mcp.get(_C)for mcp in mcp_options]}},_AN:[A]});prompt_name=mcp_group_config.get(_BV,'');prompt_config=_A;prompt_resp_var_name=_A;current_prompt_text=user_input or''
		if prompt_name:
			...
			prompt_model_provider=prompt_config.get('modelProvider',_e);prompt_model=_Ay
	...
	async def _run_mcp_action(self,current_var,mcp_name,user_input=_A):
		...
		try:
			...
			prompt_name=mcp_config.get(_BV,'');prompt_config=_A;prompt_resp_var_name=_A;current_prompt_text=user_input or''
			if prompt_name:
				logger.info(f"MCP '{mcp_name}' is using prompt '{prompt_name}'.");prompt_config,_,_=self._get_prompt_details(prompt_name,current_var)
				if not prompt_config:error_msg=f"Error: Prompt '{prompt_name}' for MCP '{mcp_name}' not found.";logger.error(error_msg);return current_var,error_msg
			...
			if is_http_call:
				eval_mcp_url=mcp_url;http_headers=self._evaluate_vars_in_structure(mcp_config.get(_Az,{}),current_var);mcp_connector=mcp_config.get(_U,'')
				if mcp_connector:
			...
			if session_context:
				async with session_context as session:
					...
					logger.info(f"Asking LLM to choose a mcp activity: '{current_prompt_text[:100]}...'");model_provider=prompt_config.get(_Ax,_e)if prompt_config else _e;model_to_use=_Ay
	...
	async def _run_agent_loop(self,prompt,client,session,model_id,max_tool_turns,initial_temperature,tool_call_temperature,mcp_description=_A):
		...
		while has_function_calls and turn_count<max_tool_turns:
			turn_count+=1;function_calls_to_execute=[part.function_call for part in latest_content.parts if part.function_call];tool_response_parts=await self._execute_tool_calls(function_calls_to_execute,session);contents.append(google_genai_types.Content(role=_A_,parts=tool_response_parts));subsequent_config=google_genai_types.GenerateContentConfig(temperature=tool_call_temperature,**base_gen_config_dict);response=await client.aio.models.generate_content(model=model_id,contents=contents,config=subsequent_config)
	...
	async def _run_api_action(self,current_var,api_name):
		...
		headers={}
		if api_config.get(_Az):
			try:headers.update(json.loads(api_config[_Az]))
	...
	def _run_parser_action(self,current_var,parser_name):
		...
		source_var_name=parser_config.get(_BW)
	...
	def _run_merger_action(self,current_var,merger_name):
		...
		dest_var_name=merger_config.get(_B0)
	...
	async def _run_form_action(self,current_var,form_name):
		...
		if not self._session_id or not self._token:logger.error(_BO);return current_var,_A
		...
		if response_obj and response_obj.status_code in[200,201]:
			try:
				form_response_url=response_data['success_url'];result_variable=form_config.get(_B0,_A)
	...
	async def _run_script_action(self,var,script_name):
		...
		try:
			if script_type=='javascript':
				...
				local_script_vars=var.copy();logger.warning('Executing Python scripts from the database is a security risk and is currently disabled.');exec(script_code,{_BX:{}},local_script_vars)
	...
	async def _evaluate_rule(self,expr,var):
		...
		try:
			...
			try:return eval(formatted_expr,{_BX:{}},{_z:var})
	...
	async def _execute_outcome(self,outcome_statement,outcome_type,user_input):
		...
		if outcome_type=='Set a variable'and outcome_statement.get(A):
			set_config=outcome_statement.get(A);variable_to_set=set_config.get('variable');value_to_set=set_config.get(_At)
			...
			auth_value=outcome_statement.get(_At)
	...
	async def _run_element_action(self,current_var,element_name):
		...
		if element_type=='date':generated_date=self._generate_date(element_config.get(_B1,'%Y-%m-%d'));variable_out=element_config.get(A);current_var[variable_out]=generated_date;return current_var,_A
		elif element_type=='datetime':generated_datetime=self._generate_datetime(element_config.get(_B1,'%Y-%m-%d %H:%M:%S'));variable_out=element_config.get(A);current_var[variable_out]=generated_datetime;return current_var,_A
	...
	async def _run_journey_action(self,current_var,action_details):
		"Handles a 'journey' type action, from either an intent or an action entity.";B='configuration';A='payment_description';action_key_var_name=action_details.get(_BY);action_key=current_var.get(action_key_var_name)if action_key_var_name else _A
		...
		delivery_method=action_details.get(_B2);language=action_details.get(_d);_,self.unique_id,self.phone_number=format_phone(self.phone_number);json_payload={_Am:action_key,_p:{_f:delivery_method,_g:self.phone_number},_An:{_Ao:self.unique_id,_g:self.phone_number},_E:{_Ap:self._session_id},_d:language}
		if current_var.get(_q):json_payload[_Aq]=current_var[_q]
		if action_details.get(_B3)=='credit card payment'and current_var.get(A):json_payload[B]={'credit-card-payment':{'currency':current_var.get('payment_currency'),'lineItems':[{'title':current_var.get(A),'amount':current_var.get('payment_amount'),'quantity':current_var.get('payment_quantity')}]}}
		if action_details.get(_B3)=='one time passcode':json_payload[B]={'random-code':{'code':'6'}}
		if action_details.get(_B2)=='voice':json_payload[_p]={_f:delivery_method,_g:self.phone_number,'callOperator':'twilio'}
	...
	def _run_transfer_action(self,current_var,action_details):
		...
		if current_var.get(_Ab):
			...
			if current_var.get(_Ac):passkey=self._generate_password(6);current_var['transferPasskey']=passkey;sms_message=f"Your agent will validate themselves with the following shared passkey: {passkey}"
		...
		return current_var,final_response
	def _map_to_standard_action_details(self,data):'Maps fields from an intent or action entity to a standard action_details dictionary.';return{_C:data.get(_C),_Ar:data.get('actionType'),_BY:data.get(_x),_B2:data.get(_Aj),_d:data.get(_d),_B3:data.get('actionEvent'),_h:data.get('actionResponseMessage'),_As:data.get(_Ak,{})}
	async def _run_action_action(self,current_var,action_name,final_response_from_caller=_A,user_input=_A):
		...
		action_details=self._map_to_standard_action_details(action_entity_data);action_type=action_details.get(_Ar,'general')
		...
		action_flow=action_details.get(_As,{});action_produced_final_response=_A
		if action_flow:
	...
	async def _gcs_write(self,bucket_name,file_path,content_bytes,connector_name):
		...
		if not storage:raise ImportError(_BZ)
		gcs_client=_A
		if connector_name:
			conn_config=self._get_connector_details(connector_name)
			if conn_config and conn_config.get(_n)==_Ag:sa_info=json.loads(conn_config.get(_o,'{}'));gcs_client=storage.Client.from_service_account_info(sa_info);logger.info(f"Using GCS service account from connector '{connector_name}'.")
		if not gcs_client:gcs_client=storage.Client();logger.info(_Ba)
	...
	async def _s3_write(self,bucket_name,file_path,content_bytes,connector_name):
		...
		if not boto3:raise ImportError(_Bb)
		s3_client=_A
		if connector_name:
			conn_config=self._get_connector_details(connector_name)
			if conn_config and conn_config.get(_n)=='aws_iam':s3_client=boto3.client('s3',aws_access_key_id=conn_config.get(_B4),aws_secret_access_key=conn_config.get(_o));logger.info(f"Using AWS IAM credentials from connector '{connector_name}'.")
		if not s3_client:s3_client=boto3.client('s3');logger.info(_Bc)
	...
	def _configure_tools_for_request(self,direct_tool_configs,processed_fd_dicts,model_name):
		...
		RESTRICTED_MODELS_FOR_MIXED_TOOLS={_Ay};has_functions=bool(function_declarations);has_search=any(t.google_search for t in final_tools_for_config);model_is_restricted=model_name in RESTRICTED_MODELS_FOR_MIXED_TOOLS
	...
	def _get_gemini_response_sync(self,model_name,contents,config):
		...
		if not self._genai_client:logger.error(_Bd);return _Be,[],_A
		try:
			...
			if response.candidates and len(response.candidates)>0:
				...
				if candidate.content and candidate.content.parts:
					fc_parts=[part.function_call for part in candidate.content.parts if hasattr(part,_Bf)and part.function_call]
					if fc_parts:
						function_calls=[]
	...
	async def _get_openai_response_async(self,model_name,messages,prompt_config,tools=_A):
		...
		client=AsyncOpenAI(api_key=self._openai_api_key);openai_params={_X:model_name,_A0:messages,_A8:prompt_config.get(_AG),_B5:prompt_config.get(_AJ),_Y:prompt_config.get(_Y),'frequency_penalty':prompt_config.get('frequencyPenalty'),'presence_penalty':prompt_config.get('presencePenalty')}
		if prompt_config.get(_i):openai_params[_i]=prompt_config.get(_i)
		if tools:openai_params[_P]=tools;openai_params[_B6]='auto'
	...
	async def _get_anthropic_response_async(self,model_name,system_prompt,messages,prompt_config,tools=_A):
		...
		client=AsyncAnthropic(api_key=self._anthropic_api_key);anthropic_params={_X:model_name,_A2:system_prompt,_A0:messages,_A8:prompt_config.get(_AG),_B5:prompt_config.get(_AJ,4096),_Y:prompt_config.get(_Y),_AH:prompt_config.get(_AH)}
		if tools:anthropic_params[_P]=tools;anthropic_params[_B6]={_F:'auto'}
	...
	async def _get_token_api(self,host,client_id,client_secret):
		url='/token';headers={_O:_K};data={_B7:_Bg,_B8:client_id,_B9:client_secret}
		try:response=await self._http_session.post(url,headers=headers,json=data);response.raise_for_status();auth_response=response.json();return f"{auth_response[_Bh]} {auth_response[_Aa]}"
	...
	async def _start_session_api(self,host,agent_id_to_start,token):
		url=f"/agent/{agent_id_to_start}";headers={_BA:_l,_t:_A6,_L:token}
		try:
	...
	async def _start_version_api(self,host,agent_id_to_start,token,version_str):
		url=f"/version/{agent_id_to_start}/{version_str}";headers={_BA:_l,_t:_A6,_L:token}
		try:
			response=await self._http_session.get(url,headers=headers);response.raise_for_status();data=response.json();session_uid=data.get(_B,{}).get(_E,{}).get('uid')
			if session_uid:return session_uid,data
	...
	async def _get_session_api(self,host,session_uid,token):
		url=f"/session/{session_uid}";headers={_t:_A6,_L:token,_BA:_l}
		try:
	...
	async def _post_session_api(self,host,token,session_uid,data_payload,transfer_req,transfer_acc):
		url=f"/session";headers={_O:_K,_AZ:_l,_L:token};full_payload={'sessionUid':session_uid,_B:data_payload,'transferRequested':transfer_req,'transferAccepted':transfer_acc}
		try:
	...
	async def _post_form_api(self,host,token,full_payload):
		url=f"/transaction";headers={_O:_K,_AZ:_l,_L:token}
		try:
			json_string_payload=json.dumps(full_payload,default=_json_datetime_serializer);compressed_data=gzip.compress(json_string_payload.encode(_M));response=await self._http_session.post(url,headers=headers,content=compressed_data)
	...
	async def _get_token_for_connector(self,connector_config):
		'Gets auth token for a given connector configuration.';A='refresh_token';url=connector_config.get(_Z);client_id=connector_config.get(_B4);client_secret=connector_config.get(_o);grant_type=connector_config.get(_n,_Bg);content_type=connector_config.get(_Bi,_K);use_header_payload=connector_config.get('headerPayload',_G);scope=connector_config.get(_AC,_A);refresh_token=connector_config.get('refreshToken',_A)
		...
		if use_header_payload:client_keys=f"{client_id}:{client_secret}";client_keys_b64=base64.b64encode(client_keys.encode()).decode();headers[_L]=f"Basic {client_keys_b64}";headers[_O]='application/x-www-form-urlencoded';data_to_send=f"grant_type={grant_type}"
			...
			if _A7 in content_type.lower():
				data_to_send={_B7:grant_type,_B8:client_id,_B9:client_secret}
				...
				form_data=[(_B7,grant_type),(_B8,client_id),(_B9,client_secret)]
		...
		try:
			...
			async with self._semaphore:response=await self._http_session.post(url,headers=headers,content=payload);response.raise_for_status();auth_response=response.json();token_type=auth_response.get(_Bh,'Bearer').capitalize();return f"{token_type} {auth_response[_Aa]}"
	...
	async def _generic_api_post_put(self,current_var,api_config,headers,method):
		...
		if not url:logger.error(f"URL could not be evaluated for API '{api_config.get(_C)}'. Template: {url_template}");return current_var,_A,_A,_Bj
		...
		try:
			async with self._semaphore:
				if method=='POST':response=await self._http_session.post(url,headers=headers,content=final_data_bytes,timeout=120)
				elif method=='PUT':response=await self._http_session.put(url,headers=headers,content=final_data_bytes,timeout=120)
	...
	async def _generic_api_get(self,current_var,api_config,headers):
		...
		if not url:logger.error(f"URL could not be evaluated for API GET '{api_config.get(_C)}'. Template: {url_template}");return current_var,_A,_A,_Bj
		logger.debug(f"API GET Request URL: {url}");logger.debug(f"API GET Request Headers: {headers}")
		try:
			async with self._semaphore:response=await self._http_session.get(url,headers=headers,timeout=120)
			parsed_response:0
			try:parsed_response=response.json()
			except json.JSONDecodeError:parsed_response=response.text
			if api_config.get(_a):current_var[api_config[_a]]=parsed_response
	...
	async def _journey_send_pipeline(self,current_var,json_payload,delivery_method,user_input=_A):
		...
		if not bearer_token:return _A,_BB
		post_url='https://app.journeyid.io/api/system/executions';headers={_t:_K,_O:_K,_L:bearer_token}
		try:
			async with self._semaphore:response=await self._http_session.post(post_url,headers=headers,content=json.dumps(json_payload),timeout=120)
	...
	async def _journey_execution_status_check(self,execution_id,current_var,max_retries=12,delay_seconds=5,user_input=_A):
		...
		if not bearer_token:return _G,_BB
		url=f"https://app.journeyid.io/api/system/executions/{execution_id}";headers={_AT:_K,_L:bearer_token}
		for attempt in range(max_retries):
			try:
				async with self._semaphore:response=await self._http_session.get(url,headers=headers,timeout=30)
				response.raise_for_status();status_data=response.json()
				if status_data.get(_Bk):return _D,status_data.get('outcome',{}).get(_k,'Task completed successfully.')
			...
			try:
				...
				for exec_item in executions:
					if exec_item.get(A,{}).get('key')==pipeline_key_to_check or exec_item.get(A,{}).get('id')==pipeline_key_to_check:
						if exec_item.get(_Bk):return _D,exec_item.get('outcome',{}).get(_k,'Pipeline completed.')
						break
	...
	async def _twilio_sms_message(self,sms_body,to_phone,from_phone_twilio=_A):
		...
		if not twilio_from_number:logger.error(_Bl);return
	...
	async def _infobip_send_email(self,plain_text_body,html_body_content,to_address,subject,from_address=_A,token=_A,cc_address=_A,bcc_address=_A,reply_to_address=_A):
		...
		try:
			async with self._semaphore:response=await self._http_session.post(infobip_url,headers=headers,json=payload);response.raise_for_status();logger.info(f"Email sent to {to_address} via Infobip. Response: {response.text}")
	...
	async def _ms_graph_send_email(self,plain_text_body,html_body_content,to_address,subject,from_address=_A,token=_A,cc_address=_A,bcc_address=_A,reply_to_address=_A):
		...
		graph_api_endpoint='https://graph.microsoft.com/v1.0';url=f"{graph_api_endpoint}/users/{from_address}/sendMail";headers={_L:token,_O:_K};email_payload={_k:{_AR:subject,_W:{_Bi:'HTML',_R:html_body_content}},'saveToSentItems':'true'}
		...
		try:
			async with self._semaphore:response=await self._http_session.post(url,headers=headers,json=email_payload);response.raise_for_status();logger.info(f"Email sent to {to_address} via Microsoft Graph API. Response status: {response.status_code}")
	...
	async def _gmail_send_email(self,plain_text_body,html_body_content,to_address,subject,from_address=_A,token=_A,cc_address=_A,bcc_address=_A,reply_to_address=_A):
		...
		try:
			async with self._semaphore:response=await self._http_session.post(url,headers=headers,json=payload);response.raise_for_status();data=response.json();logger.info(f"Email sent to {to_address} via Gmail API. Message ID: {data.get('id')}")
			...
			for(key,value)in list(data_structure.items()):
				if isinstance(value,str):
					if A in value or B in value:
						try:eval_globals={_z:context_vars,C:context_vars,D:context_vars,_BC:user_input};data_structure[key]=eval(f"f'''{value}'''",eval_globals,{})
			...
			for(i,item)in enumerate(data_structure):
				if isinstance(item,str):
					if A in item or B in item or user_input is not _A and'{user_input}'in item:
						try:eval_globals={_z:context_vars,C:context_vars,D:context_vars,_BC:user_input};data_structure[i]=eval(f"f'''{item}'''",eval_globals,{})
	...
	def _evaluate_f_string(self,template_string,context_vars,user_input=_A):
		...
		try:eval_namespace={_z:context_vars,_BC:user_input or''};return eval(f"f'''{template_string}'''",eval_namespace,{})
	...
	def _translate_tools_for_provider(self,provider,direct_tool_configs,processed_fd_dicts):
		...
		translated_tools=[]
		if provider==_j:
			for fd in processed_fd_dicts:translated_tools.append({_F:_A_,_A_:{_C:fd.get(_C),_N:fd.get(_N),_AU:fd.get(_AU)}})
		elif provider==_A1:
