...
def apply_patterns_memref_alloc_to_alloca(*, size_limit=None, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(ApplyAllocToAllocaOp(size_limit=size_limit, loc=loc, ip=ip))
...
def apply_patterns_memref_expand_ops(*, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(ApplyExpandOpsPatternsOp(loc=loc, ip=ip))
...
def apply_patterns_memref_expand_strided_metadata(*, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(ApplyExpandStridedMetadataPatternsOp(loc=loc, ip=ip))
...
def apply_patterns_memref_extract_address_computations(*, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(ApplyExtractAddressComputationsPatternsOp(loc=loc, ip=ip))
...
def apply_patterns_memref_fold_memref_alias_ops(*, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(ApplyFoldMemrefAliasOpsPatternsOp(loc=loc, ip=ip))
...
def apply_patterns_memref_resolve_ranked_shaped_type_result_dims(*, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(ApplyResolveRankedShapedTypeResultDimsPatternsOp(loc=loc, ip=ip))
...
def memref_alloca_to_global(get_global, global_, alloca, *, loc=None, ip=None) -> _Sequence[_ods_ir.Value]:
  return _get_op_result_or_op_results(MemRefAllocaToGlobalOp(getGlobal=get_global, global_=global_, alloca=alloca, loc=loc, ip=ip))
...
def memref_erase_dead_alloc_and_stores(target, *, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(MemRefEraseDeadAllocAndStoresOp(target=target, loc=loc, ip=ip))
...
def memref_make_loop_independent(transformed, target, num_loops, *, loc=None, ip=None) -> _ods_ir.Value:
  return _get_op_result_or_op_results(MemRefMakeLoopIndependentOp(transformed=transformed, target=target, num_loops=num_loops, loc=loc, ip=ip))
...
def memref_multibuffer(transformed, target, factor, *, skip_analysis=None, loc=None, ip=None) -> _ods_ir.Value:
  return _get_op_result_or_op_results(MemRefMultiBufferOp(transformed=transformed, target=target, factor=factor, skip_analysis=skip_analysis, loc=loc, ip=ip))
...
def apply_conversion_patterns_memref_memref_to_llvm_type_converter(*, use_aligned_alloc=None, index_bitwidth=None, use_generic_functions=None, use_bare_ptr_call_conv=None, data_layout=None, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(MemrefToLLVMTypeConverterOp(use_aligned_alloc=use_aligned_alloc, index_bitwidth=index_bitwidth, use_generic_functions=use_generic_functions, use_bare_ptr_call_conv=use_bare_ptr_call_conv, data_layout=data_layout, loc=loc, ip=ip))
