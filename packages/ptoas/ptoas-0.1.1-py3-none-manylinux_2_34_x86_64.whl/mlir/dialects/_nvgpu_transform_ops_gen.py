...
def apply_conversion_patterns_nvgpu_nvgpu_to_nvvm(*, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(ApplyNVGPUToNVVMConversionPatternsOp(loc=loc, ip=ip))
...
def nvgpu_create_async_groups(result, target, *, bypass_l1=None, loc=None, ip=None) -> _ods_ir.Value:
  return _get_op_result_or_op_results(CreateAsyncGroupsOp(result=result, target=target, bypass_l1=bypass_l1, loc=loc, ip=ip))
...
def nvgpu_pipeline_shared_memory_copies(result, for_op, depth, *, peel_epilogue=None, failure_propagation_mode=None, loc=None, ip=None) -> _ods_ir.Value:
  return _get_op_result_or_op_results(PipelineSharedMemoryCopiesOp(result=result, for_op=for_op, depth=depth, peel_epilogue=peel_epilogue, failure_propagation_mode=failure_propagation_mode, loc=loc, ip=ip))
...
def nvgpu_rewrite_copy_as_tma(target, *, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(RewriteCopyAsTmaOp(target=target, loc=loc, ip=ip))
...
def nvgpu_rewrite_matmul_as_mma_sync(target, *, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(RewriteMatmulAsMmaSyncOp(target=target, loc=loc, ip=ip))
