...
@app.command("install")
def license_install(
    license_file: Path = typer.Argument(..., help="Path to license file to install", exists=False),
    ...
    if new_license:
        ...
        console.print(f"[green]✓[/green] License installed to {dest_file}")
        console.print(f"  Tier: {tier_display}")
        console.print(f"  Email: {new_license.payload.email}")
    else:
        console.print(f"[green]✓[/green] License file copied to {dest_file}")
        console.print("[yellow]Warning:[/yellow] Could not verify the installed license.")
    console.print()
