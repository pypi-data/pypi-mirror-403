...
class TestInitBundledCli:
    ...
    def test_main_skips_config_yaml(self, tmp_path, monkeypatch):
        """Should skip config.yaml files containing cookiecutter templates."""
        # Create template with config.yaml
        template_dir = tmp_path / "template"
        template_dir.mkdir()
        (template_dir / ".paircoder").mkdir()
        (template_dir / ".paircoder" / "config.yaml").write_text(
        ...
        assert result == 0
        # config.yaml should NOT be copied (it's in SKIP_FILES)
        assert not (output_dir / ".paircoder" / "config.yaml").exists()
        # other.txt should be copied
