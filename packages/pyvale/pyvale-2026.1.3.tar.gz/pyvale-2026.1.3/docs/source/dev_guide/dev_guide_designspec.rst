...
----------------------------------------
A simulated sensor will produce a simulated measurement M which is defined to be: M = T + E(S) + E(R) where T is the ground truth value taken from the given input multi-physics simulation, E(S) includes any sources of systematic measurement error (e.g. averaging, digitisation, calibration), and E(R) includes any sources of random errors (e.g. noise).
