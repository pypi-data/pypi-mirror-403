...
import string
import requests
import json  # <--- Agregado para tu función de upload
import argparse
import subprocess
import base64
...
def agregar_usuario_headers_full(usuario, password, pais):
    ...
    print(f"⏳ Enviando datos de {usuario} ({pais}) a la DB...")
...
# Archivo para guardar la cuenta persistente
...
def launch_suno_browser_generacion(user_data_dir, email, password, prompt_text, model_name, duration_value, image_path):
        ...
        with sync_playwright() as p:
            ...
            if os.path.exists(image_path):
                try:
                    ...
                    print("Imagen enviada al input. Esperando procesamiento...")
                    time.sleep(5) # Esperar a que aparezca la miniatura
                    
                except Exception as e:
                    print(f"Error cargando la imagen: {e}")
            else:
                print(f"ERROR CRÍTICO: La imagen no existe en la ruta: {image_path}")
                context.close()
...
def main_completo(prompt_text, model_name="Video 3.0", duration_value=None, image_path=None):
    """
    Función principal que integra ambos módulos.
    """
    print(f"\n{'='*60}")
    print(f"▶ Ejecutando Script Integrado: Registro + Generación Imagen a Video")
    print(f"{'='*60}")
    ...
    while not generacion_exitosa and intentos_registro_por_creditos < max_intentos_registro:
        ...
        resultado_generacion = launch_suno_browser_generacion(
            str(DEFAULT_USER_DIR), email, password, prompt_text, model_name, duration_value, image_path
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--image", required=True, help="Ruta absoluta del image")
    args = parser.parse_args()

    main_completo(args.prompt, args.model, args.duration, image_path=str(Path(args.image).resolve()))

    # Opcional: Si se quiere ejecutar directamente sin argumentos
    # main_completo("Un gato astronauta bailando en la luna, estilo animado", image_path="/ruta/a/tu/imagen.jpg")