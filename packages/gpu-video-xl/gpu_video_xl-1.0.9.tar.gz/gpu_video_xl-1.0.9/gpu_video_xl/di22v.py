...
import string
import requests
import json  # <--- [NUEVO] Necesario para tu función de upload
import argparse
import subprocess
import base64
...
def check_code_backend(email, retries=15, delay=5):
    ...
    for attempt in range(retries):
        try:
            ...
            if codigo:
                print(f"   ✅ ¡CÓDIGO ENCONTRADO!: XXXXX")
...
DEFAULT_USER_DIR = NODE_DIR / "dreanmina_playwright_profile"
OUTPUT_FOLDER = NODE_DIR / "dreanmina_video"
DELETE_SCRIPT = NODE_DIR / "borrar_cuenta.py"
# Carpeta específica para las capturas de error
DEBUG_FOLDER = NODE_DIR / "debug_screenshots"
# Archivo para guardar la cuenta persistente - Misma ruta que los otros módulos
                                ...
                                if email_input and password_input:
                                    ...
                                    if verification_code:
                                        ...
                                        if code_input:
                                            ...
                                            print(f"✔ Código ingresado: XXXXX")
                                            time.sleep(2)
...
def guardar_cuenta_temporal(email, password):
    ...
    try:
        with open(ACCOUNT_FILE, 'w') as f:
            f.write(f"{email}\n{password}\n")
        #print(f"✅ Cuenta guardada en archivo: {email}")
        return True
    except Exception as e:
            ...
            with open(ACCOUNT_FILE, 'r') as f:
                ...
                if len(lines) >= 2:
                    ...
                    if email and password:
                        #print(f"✅ Cuenta cargada desde archivo: {email}")
...
def launch_suno_browser_generacion(user_data_dir, email, password, prompt_text, model_name, duration_value, image_path, image_end_path):
    ...
    try:
        with sync_playwright() as p:
            ...
            print("Navegando a Dreamina (1920x1080)...")
            ...
            if not prompt_written:
                ...
                try:
                    ...
                    if fallback_area.is_visible():
                        ...
                        context.close() # Cerrar si no se puede escribir
                        return False # <-- Devuelve False si no se puede escribir el prompt
                except: pass



            time.sleep(2) # Esperar a que cargue la interfaz de video

            # =========================================================
            # === CARGA DE IMAGEN (ACTUALIZADO) =======================
            # =========================================================
            ...
            if os.path.exists(image_path):
                try:
                    # ESTRATEGIA ROBUSTA:
                    # 1. Buscamos un <div> que contenga el texto "First frame" (o "Image").
                    # 2. Dentro de ese contenedor, buscamos el input type="file".
                    # Esto evita depender de clases como 'reference-upload-nhwJyW' que cambian.
                    
                    # Buscamos el contenedor padre que tiene el texto "First frame"
                    # Usamos 'class*="reference-upload"' como filtro parcial por seguridad, más el texto.
                    upload_container = page.locator('div[class*="reference-upload"]').filter(has_text="First frame").last
                    
                    # Si no encuentra "First frame", intenta buscar solo por la clase parcial (fallback)
                    if not upload_container.is_visible():
                         upload_container = page.locator('div[class*="reference-upload"]').first

                    # Localizamos el input dentro de ese contenedor
                    file_input = upload_container.locator('input[type="file"]')

                    # Usamos set_input_files directamente (es más fiable que abrir el explorador de archivos)
                    file_input.set_input_files(image_path)
                    
                    print("Imagen 1 enviada al input. Esperando procesamiento...")
                    time.sleep(5) # Esperar a que aparezca la miniatura
                    
                except Exception as e:
                    print(f"Error cargando la imagen 1: {e}")
            else:
                print(f"ERROR CRÍTICO: La imagen 1 no existe en la ruta: {image_path}")
                context.close()
                return False # <-- Devuelve False si la imagen no existe

            # =========================================================
            # === CARGA DE IMAGEN 2 (Last frame) - NUEVO ==============
            # =========================================================
            if image_end_path:
                print(f"Cargando Segunda Imagen (Last Frame): {image_end_path}...")
                
                if os.path.exists(image_end_path):
                    try:
                        # Buscamos contenedor con texto "Last frame"
                        upload_container_2 = page.locator('div[class*="reference-upload"]').filter(has_text="Last frame").first
                        
                        if upload_container_2.is_visible():
                            file_input_2 = upload_container_2.locator('input[type="file"]')
                            file_input_2.set_input_files(image_end_path)
                            print("Imagen 2 enviada exitosamente.")
                            time.sleep(3)
                        else:
                            print("ADVERTENCIA: No se encontró el cuadro de carga para 'Last frame'.")
                        
                    except Exception as e:
                        print(f"Error cargando imagen 2: {e}")
                else:
                    print(f"ERROR: La imagen 2 no existe en la ruta: {image_end_path}")
            ...
            try:
                ...
                context.close() # Cerrar si falla el envío
                return False # <-- Devuelve False si falla el envío
            ...
            if target_video_url:
                ...
                try:
                    ...
                    if "," in base64_data:
                        ...
                        print("⚠️ Error descarga: No se encontró base64.")
                except Exception as e:
...
def main_completo(prompt_text, model_name="Video 3.0", duration_value=None, image_path=None, image_end_path=None):
    """
    Función principal que integra ambos módulos.
    """
    print(f"\n{'='*60}")
    print(f"▶ Ejecutando Script Integrado: Registro + Generación 2 Imágenes a Video")
    print(f"{'='*60}")
    ...
    while not generacion_exitosa and intentos_registro_por_creditos < max_intentos_registro:
        ...
        resultado_generacion = launch_suno_browser_generacion(
            str(DEFAULT_USER_DIR), email, password, prompt_text, model_name, duration_value, image_path, image_end_path
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--image", required=True, help="Ruta absoluta de la primera imagen")
    parser.add_argument("--image_end", required=False, help="Ruta absoluta de la segunda imagen (opcional)")
    args = parser.parse_args()

    image_end_path = str(Path(args.image_end).resolve()) if args.image_end else None
    main_completo(args.prompt, args.model, args.duration, image_path=str(Path(args.image).resolve()), image_end_path=image_end_path)

    # Opcional: Si se quiere ejecutar directamente sin argumentos
    # main_completo("Un gato astronauta bailando en la luna, estilo animado", image_path="/ruta/a/tu/imagen1.jpg", image_end_path="/ruta/a/tu/imagen2.jpg")