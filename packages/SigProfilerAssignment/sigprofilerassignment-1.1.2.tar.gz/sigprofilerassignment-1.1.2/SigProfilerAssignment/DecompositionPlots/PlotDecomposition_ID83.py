...
BRACKET_PATH = os.path.join(
    PATHS, "DecompositionPlots/ReferenceFiles/Accolade_fermante.png"
)
...
LAYOUT_5_TEXT = [
    (TEXT_X_COORD, (TEXT_Y_COORD - HEIGHT_GRAPH * 1) + 40),
    (TEXT_X_COORD, (TEXT_Y_COORD - HEIGHT_GRAPH * 2) + 35),
    (TEXT_X_COORD, (TEXT_Y_COORD - HEIGHT_GRAPH * 3) + 30),
    (TEXT_X_COORD, (TEXT_Y_COORD - HEIGHT_GRAPH * 4) + 25),
    (TEXT_X_COORD, (TEXT_Y_COORD - HEIGHT_GRAPH * 5) + 20),
]


LAYOUT_4_GRAPH = [
    (
        GRAPH_X_COORD - WIDTH_GAP,
        (GRAPH_Y_COORD - HEIGHT_GRAPH * 1) - HEIGHT_GRAPH / 2 + 10,
    ),
    (
        GRAPH_X_COORD - WIDTH_GAP,
        (GRAPH_Y_COORD - HEIGHT_GRAPH * 2) - HEIGHT_GRAPH / 2 + 5,
    ),
    (
        GRAPH_X_COORD - WIDTH_GAP,
        (GRAPH_Y_COORD - HEIGHT_GRAPH * 3) - HEIGHT_GRAPH / 2 - 5,
    ),
    (
        GRAPH_X_COORD - WIDTH_GAP,
        (GRAPH_Y_COORD - HEIGHT_GRAPH * 4) - HEIGHT_GRAPH / 2 - 10,
    ),
]

LAYOUT_4_TEXT = [
    (TEXT_X_COORD, (TEXT_Y_COORD - HEIGHT_GRAPH * 1) - 7),
    (TEXT_X_COORD, (TEXT_Y_COORD - HEIGHT_GRAPH * 2) - 12),
    (TEXT_X_COORD, (TEXT_Y_COORD - HEIGHT_GRAPH * 3) - 22),
...
LAYOUT_2_TEXT = [
    (TEXT_X_COORD, (TEXT_Y_COORD - HEIGHT_GRAPH * 2) - 11),
    (TEXT_X_COORD, (TEXT_Y_COORD - HEIGHT_GRAPH * 3) - 21),
]
...
def plot_1(bases, project, c_draw, denovo_plots_dict, basis_plots_dict):
    for i in range(0, 1):
        ...
        c_draw.drawImage(
            image,
            LAYOUT_1_GRAPH[i][X_COORD],
            LAYOUT_1_GRAPH[i][Y_COORD],
            width=WIDTH_GRAPH,
            height=HEIGHT_GRAPH,
        )
        c_draw.drawString(
            LAYOUT_1_TEXT[i][X_COORD], LAYOUT_1_TEXT[i][Y_COORD], str(bases[i][1]) + "%"
        )
...
def plot_2(bases, project, c_draw, denovo_plots_dict, basis_plots_dict):
    for i in range(0, 2):
        ...
        c_draw.drawImage(
            image,
            LAYOUT_2_GRAPH[i][X_COORD],
            LAYOUT_2_GRAPH[i][Y_COORD],
            width=WIDTH_GRAPH,
            height=HEIGHT_GRAPH,
        )
        c_draw.drawString(
            LAYOUT_2_TEXT[i][X_COORD], LAYOUT_2_TEXT[i][Y_COORD], str(bases[i][1]) + "%"
        )
...
def plot_3(bases, project, c_draw, denovo_plots_dict, basis_plots_dict):
    for i in range(0, 3):
        ...
        c_draw.drawImage(
            image,
            LAYOUT_3_GRAPH[i][X_COORD],
            LAYOUT_3_GRAPH[i][Y_COORD],
            width=WIDTH_GRAPH,
            height=HEIGHT_GRAPH,
        )
        c_draw.drawString(
            LAYOUT_3_TEXT[i][X_COORD], LAYOUT_3_TEXT[i][Y_COORD], str(bases[i][1]) + "%"
        )
...
def plot_4(bases, project, c_draw, denovo_plots_dict, basis_plots_dict):
    for i in range(0, 4):
        ...
        c_draw.drawImage(
            image,
            LAYOUT_4_GRAPH[i][X_COORD],
            LAYOUT_4_GRAPH[i][Y_COORD],
            width=WIDTH_GRAPH,
            height=HEIGHT_GRAPH,
        )
        c_draw.drawString(
            LAYOUT_4_TEXT[i][X_COORD], LAYOUT_4_TEXT[i][Y_COORD], str(bases[i][1]) + "%"
        )
...
def plot_5(bases, project, c_draw, denovo_plots_dict, basis_plots_dict):
    for i in range(0, 5):
        ...
        c_draw.drawImage(
            image,
            LAYOUT_5_GRAPH[i][X_COORD],
            LAYOUT_5_GRAPH[i][Y_COORD],
            width=WIDTH_GRAPH,
            height=HEIGHT_GRAPH,
        )
        c_draw.drawString(
            LAYOUT_5_TEXT[i][X_COORD], LAYOUT_5_TEXT[i][Y_COORD], str(bases[i][1]) + "%"
        )
...
def draw_statistics(c_draw, statistics, sig_version, custom_text):
    ...
    if custom_text is not None:
        c_draw.drawString(
            WIDTH_GAP + 15, LAYOUT_2_TEXT[1][Y_COORD] - 120, str(custom_text)
        )
...
def draw_bracket(num_bases, c_draw):
    ...
    c_draw.drawImage(
        BRACKET_PATH,
        MID_WIDTH_LETTER - 15,
        BRACKET_SIZES[num_plts][0],
        width=15,
        height=BRACKET_SIZES[num_plts][1],
        mask="auto",
    )
...
def gen_plot(
    ...
    if reconstruction:
        ...
        c.drawImage(
            recon_image,
            WIDTH_GAP,
            LAYOUT_2_GRAPH[1][Y_COORD],
            width=WIDTH_GRAPH,
            height=HEIGHT_GRAPH,
        )
        c.drawString(
            WIDTH_GRAPH - WIDTH_GAP - 35, LAYOUT_2_TEXT[0][Y_COORD], "Original"
        )
        c.drawString(
            WIDTH_GRAPH - WIDTH_GAP - 55, LAYOUT_2_TEXT[1][Y_COORD], "Reconstructed"
        )
        ...
        c.drawImage(
            denovo_image,
            WIDTH_GAP,
            MID_HEIGHT_LETTER - HEIGHT_GRAPH / 2,
            width=WIDTH_GRAPH,
            height=HEIGHT_GRAPH,
        )
