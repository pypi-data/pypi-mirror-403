...
# SV32 plots have a dimension of 16" width, 10" height
WIDTH_GRAPH = 290
HEIGHT_GRAPH = 214.89
...
LAYOUT_6_TEXT = [(600, 700), (900, 700), (600, 480), (900, 480), (600, 260), (900, 260)]
...
LAYOUT_5_TEXT = [(600, 700), (900, 700), (600, 480), (900, 480), (600, 260)]
...
def plot_6(bases, project, c_draw, denovo_plots_dict, basis_plots_dict):
    for i in range(0, 6):
        ...
        c_draw.drawImage(
            image,
            LAYOUT_6_GRAPH[i][X_COORD],
            LAYOUT_6_GRAPH[i][Y_COORD],
            width=WIDTH_GRAPH,
            height=HEIGHT_GRAPH,
        )
        c_draw.drawString(
            LAYOUT_6_TEXT[i][X_COORD], LAYOUT_6_TEXT[i][Y_COORD], str(bases[i][1]) + "%"
        )
...
def plot_7_plus(bases, project, c_draw, denovo_plots_dict, basis_plots_dict):
    ...
    c_draw.drawString(
        GRAPH_X_COORD - 330, (TEXT_Y_COORD - HEIGHT_GRAPH * 6) + 557, extra_sigs
    )
...
def draw_statistics(c_draw, statistics, sig_version, custom_text):
    ...
    c_draw.drawString(
        WIDTH_GAP + 15,
        LAYOUT_2_TEXT_RECON[1][Y_COORD] - 205,
    ...
    if custom_text is not None:
        c_draw.drawString(
            GRAPH_X_COORD - 330, LAYOUT_2_TEXT_RECON[1][Y_COORD] - 205, str(custom_text)
        )
...
def gen_plot(
    ...
    if reconstruction:
        c.drawImage(
            denovo_image,
            WIDTH_GAP,
            LAYOUT_2_GRAPH_RECON[0][Y_COORD],
            width=WIDTH_GRAPH,
            height=HEIGHT_GRAPH,
        )
        c.drawString(
            WIDTH_GRAPH - WIDTH_GAP - 25, LAYOUT_2_TEXT_RECON[0][Y_COORD], "Original"
        )
        c.drawImage(
            recon_image,
            WIDTH_GAP,
            LAYOUT_2_GRAPH_RECON[1][Y_COORD],
            width=WIDTH_GRAPH,
            height=HEIGHT_GRAPH,
        )
        c.drawString(
            WIDTH_GRAPH - WIDTH_GAP - 45,
            LAYOUT_2_TEXT_RECON[1][Y_COORD],
            "Reconstructed",
        )
