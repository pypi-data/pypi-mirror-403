...
LAYOUTS = [
    (LAYOUT_1_GRAPH, LAYOUT_1_TEXT),
    (LAYOUT_2_GRAPH, LAYOUT_2_TEXT),
    (LAYOUT_3_GRAPH, LAYOUT_3_TEXT),
    (LAYOUT_4_GRAPH, LAYOUT_4_TEXT),
    (LAYOUT_5_GRAPH, LAYOUT_5_TEXT),
]
...
def plot_1(bases, project, c_draw, denovo_plots_dict, basis_plots_dict):
    for i in range(0, 1):
        ...
        c_draw.drawString(
            LAYOUT_1_TEXT[i][X_COORD],
            LAYOUT_1_TEXT[i][Y_COORD],
            str(float("{0:.2f}".format(bases[i][1]))) + "%",
        )
...
def plot_2(bases, project, c_draw, denovo_plots_dict, basis_plots_dict):
    for i in range(0, 2):
        ...
        c_draw.drawString(
            LAYOUT_2_TEXT[i][X_COORD],
            LAYOUT_2_TEXT[i][Y_COORD],
            str(float("{0:.2f}".format(bases[i][1]))) + "%",
        )
...
def plot_3(bases, project, c_draw, denovo_plots_dict, basis_plots_dict):
    for i in range(0, 3):
        ...
        c_draw.drawString(
            LAYOUT_3_TEXT[i][X_COORD],
            LAYOUT_3_TEXT[i][Y_COORD],
            str(float("{0:.2f}".format(bases[i][1]))) + "%",
        )
...
def plot_4(bases, project, c_draw, denovo_plots_dict, basis_plots_dict):
    for i in range(0, 4):
        ...
        c_draw.drawString(
            LAYOUT_4_TEXT[i][X_COORD],
            LAYOUT_4_TEXT[i][Y_COORD],
            str(float("{0:.2f}".format(bases[i][1]))) + "%",
        )
...
def plot_5(bases, project, c_draw, denovo_plots_dict, basis_plots_dict):
    for i in range(0, 5):
        ...
        c_draw.drawString(
            LAYOUT_5_TEXT[i][X_COORD],
            LAYOUT_5_TEXT[i][Y_COORD],
            str(float("{0:.2f}".format(bases[i][1]))) + "%",
        )
...
def plot_6_plus(bases, project, c_draw, denovo_plots_dict, basis_plots_dict):
    ...
    for i in range(5, len(bases) - 1):
        extra_sigs += (
            str(bases[i][0]) + " (" + str(float("{0:.2f}".format(bases[i][1]))) + "%), "
        )

    extra_sigs += (
        bases[len(bases) - 1][0]
        + " ("
        + str(float("{0:.2f}".format(bases[len(bases) - 1][1])))
        + "%)"
    )
...
def gen_plot(
    ...
    if reconstruction:
        ...
        c.drawString(
            WIDTH_GRAPH - WIDTH_GAP - 25, LAYOUT_2_TEXT[0][Y_COORD], "Original"
        )
        c.drawImage(
            recon_image,
            WIDTH_GAP,
        ...
        c.drawString(
            WIDTH_GRAPH - WIDTH_GAP - 45, LAYOUT_2_TEXT[1][Y_COORD], "Reconstructed"
        )
