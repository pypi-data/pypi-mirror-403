...
class 语句访问者(AST访问者基类):
    ...
    def visit_Assign(self, node: ast.Assign):
        ...
        from .工具 import _ArrayCallResult
        if isinstance(value, _ArrayCallResult):
            ...
            if isinstance(初始化列表, 列表初始化列表):
                ...
                if len(大小列表) == 1:
                    # 一维数组
                    init_code += ", ".join(str(self.transpiler.获取值(v)) for v in 初始化列表)
                else:
                    # 多维数组
                    inner_size = 大小列表[-1]
                    for i in range(0, len(初始化列表), inner_size):
                        inner_list = 初始化列表[i:i+inner_size]
                        init_code += "{" + ", ".join(str(self.transpiler.获取值(v)) for v in inner_list) + "}"
                        if i + inner_size < len(初始化列表):
                            init_code += ", "
                init_code += "}"
                init_list = 列表初始化列表(init_code, 元素类型名, len(初始化列表))
            else:
                init_list = 列表初始化列表(str(初始化列表), 元素类型名, 0)

            # 创建数组变量
            from .std_array import 标准数组
            target_var = 标准数组(元素类型名, 大小列表, init_list, node.targets[0].id, False)
    ...
    def visit_AnnAssign(self, node: ast.AnnAssign):
        ...
        # 特殊处理：Array[T, N] 类型（C++ 原生数组）
        from .工具 import _ArrayInstance
        if isinstance(target_py_type, _ArrayInstance):
            if not isinstance(node.target, ast.Name):
                self.抛出错误("Array assignment target must be a name", node)

            # 获取数组信息
            元素类型 = target_py_type.元素类型
            大小列表 = target_py_type.大小列表
            c_elem_type_str = target_py_type.获取C类型名()
            # 从完整的类型名中提取元素类型
            # 例如 "int64_t[3]" -> "int64_t"
            元素类型名 = str(self.解析类型(元素类型))
            ...
            target_var = 标准数组(元素类型名, 大小列表, init_list, node.target.id, False)
