...
class 表达式访问者(错误处理混入, 类型处理混入, 参数处理混入):
    ...
    def 获取下标(self, node: ast.Subscript):
        ...
        # 特殊处理：Array[T, N] 类型注解
        # 检查 obj 是否是 Array 类
        from .工具 import Array, _ArrayInstance
        from .cpp类型 import CInt常量
        if isinstance(obj, type(Array)) and obj.__name__ == 'Array':
            # Array 类型注解，需要 Array[类型, 大小] 语法
            if not isinstance(slice_node, ast.Tuple) or len(slice_node.elts) != 2:
                self.抛出错误("Array type annotation requires exactly 2 parameters: Array[类型, 大小]", node)
            elem_type = self.获取值(slice_node.elts[0])
            size = self.获取值(slice_node.elts[1])
            # 大小必须是整数常量（Python int 或 CInt常量）
            if isinstance(size, CInt常量):
                size = size.v
            if not isinstance(size, int):
                self.抛出错误("Array size must be an integer constant", node)
    ...
    def 处理调用(self, node: ast.Call):
        ...
        # 0. 处理 Array 调用 - Array(int, 3, [1, 2, 3])
        if isinstance(node.func, ast.Name) and node.func.id == 'Array':
            from .工具 import Array, _ArrayCallResult
            # 解析参数
            ...
            初始化列表 = self.获取值(node.args[2]) if len(node.args) >= 3 else []
            # 大小必须是整数常量
            from .cpp类型 import CInt常量
