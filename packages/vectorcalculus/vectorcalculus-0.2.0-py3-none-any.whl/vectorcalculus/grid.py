...
"""
from typing import Sequence, Tuple, Callable, Optional, Any, cast
...
def gradient_on_grid(scalar_field: Callable[..., np.ndarray], *axes: Sequence[Any], indexing: str = "xy", spacing: Optional[Sequence[Any]] = None, edge_order: int = 2) -> Tuple[np.ndarray, ...]:
    ...
    if spacing is None:
        ...
        if indexing == "ij" or len(axes) < 2:
            ...
            spacing_arrs = [np.asarray(axes[1]), np.asarray(axes[0])] + [np.asarray(a) for a in axes[2:]]
        spacing = spacing_arrs
    # np.gradient typing is picky; treat result as Any then convert
    grads_any: Any = np.gradient(values, *spacing, edge_order=edge_order)  # type: ignore[arg-type]
...
def divergence_on_grid(vector_field: Callable[..., Sequence[np.ndarray]], *axes: Sequence[Any], indexing: str = "xy", spacing: Optional[Sequence[Any]] = None, edge_order: int = 2) -> np.ndarray:
    ...
    for i, comp in enumerate(components):
        derivs_any: Any = np.gradient(comp, *spacing, edge_order=edge_order)  # type: ignore[arg-type]
...
def curl_on_grid(vector_field: Callable[..., Sequence[np.ndarray]], *axes: Sequence[Any], indexing: str = "xy", spacing: Optional[Sequence[Any]] = None, edge_order: int = 2) -> np.ndarray:
    ...
    dFx_any: Any = np.gradient(Fx, *spacing, edge_order=edge_order)  # type: ignore[arg-type]
    dFy_any: Any = np.gradient(Fy, *spacing, edge_order=edge_order)  # type: ignore[arg-type]
    dFz_any: Any = np.gradient(Fz, *spacing, edge_order=edge_order)  # type: ignore[arg-type]
    dFx = tuple(dFx_any)
    dFy = tuple(dFy_any)
    dFz = tuple(dFz_any)
