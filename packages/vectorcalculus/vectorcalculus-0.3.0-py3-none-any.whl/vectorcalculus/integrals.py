    ...
    if _np is None:
        ...
        for i, u in enumerate(us):
            for j, v in enumerate(vs):
                ...
                vm = max(0, j - 1)
                ru = tuple(
                    (param(us[up], v)[k] - param(us[um], v)[k]) / (us[up] - us[um])
                    for k in range(len(param(u, v)))
                )
                rv = tuple(
                    (param(u, vs[vp])[k] - param(u, vs[vm])[k]) / (vs[vp] - vs[vm])
                    for k in range(len(param(u, v)))
                )
                # cross product
                ...
                if vector_field:
                    ...
                    total += (
                        fval * (nx * nx + ny * ny + nz * nz) ** 0.5 * du * dv * weight
                    )
        return float(total)
...
def stokes_verifier(
    ...
    surf = surface_integral(
        surface_param, curl_at_point, u0, u1, v0, v1, mu=mu, mv=mv, vector_field=True
    )

    return {
        "circulation": float(circ),
        "surface_integral": float(surf),
        "difference": float(circ - surf),
    }


def divergence_verifier_box(
    vector_field: Callable[[Sequence[float]], Sequence[float]],
    x0: float,
    x1: float,
    y0: float,
    ...
    return {
        "flux": float(flux),
        "volume_integral": float(vol),
        "difference": float(flux - vol),
    }
