...
if sys.platform == "win32":
    pass  # stdout/stderr handling is now done in Cli.py
...
class ConnectionManager:
    ...
    def _start_log_reader(self, process, name="Process"):
        """Reads stdout from a process and sends it to the log callback."""
        def reader():
            try:
    ...
    def connect_proxy_gui(self, config: dict) -> str:
        ...
        if not server_ip or not domain:
            raise Exception("Invalid configuration: server_ip and domain are required")
    ...
    def connect_vpn_gui(self, config: dict) -> str:
        if not is_admin():
        ...
        if self.tun2proxy_process.poll() is not None:
            output = ""
            # Since we're consuming stdout in a thread, we might miss the immediate error.
            # But the log callback should have captured it.
            self.disconnect()
