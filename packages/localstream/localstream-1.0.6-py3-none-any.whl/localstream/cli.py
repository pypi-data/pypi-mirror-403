...
# ==========================================
# CLI Output Fix for Windows GUI Builds
# ==========================================
if sys.platform == "win32":
    ...
    def is_stdout_working():
        try:
            if sys.stdout is None or sys.stdout.closed:
                return False
            # Try writing nothing to check if the stream is valid
            sys.stdout.write('')
            return True
        except Exception:
            return False
    ...
    def is_stdin_working():
        try:
            if sys.stdin is None or sys.stdin.closed:
                return False
            # We can't easily 'read' to test without blocking, 
            # but we can check fileno checking if it's a TTY
            return sys.stdin.isatty()
        except Exception:
            return False

    def attach_console_if_needed():
        # optimized: check if we already have working streams
        ...
        # If already attached, this will fail (return 0), which is fine.
        kernel32.AttachConsole(-1)

        try:
            # Re-open stdout/stderr if needed
            if not stdout_ok:
                sys.stdout = open("CONOUT$", "w", encoding="utf-8", errors="replace")
                sys.stderr = open("CONOUT$", "w", encoding="utf-8", errors="replace")
            
            # Re-open stdin if needed
            if not stdin_ok:
                sys.stdin = open("CONIN$", "r", encoding="utf-8", errors="replace")
        except Exception:
            pass
        
        # Fallbacks for stdout/stderr to devnull
    ...
    # Re-wrap for UTF-8 support if we have a valid buffer now
    # (CONOUT$ opens as text, so buffer might not exist or be needed)
    if sys.stdout and not sys.stdout.closed and hasattr(sys.stdout, 'buffer'):
        try:
            # Check if it's already encoding utf-8 to avoid double wrap
...
# Final sanity check: ensure we never have None streams
...
try:
    import colorama
    # Only init colorama if we have a real terminal and open stream
    if sys.stdout and not sys.stdout.closed and sys.stdout.isatty():
        colorama.init()
    elif sys.stdout and not sys.stdout.closed:
        # Strip ANSI if not a tty but colorama is present
