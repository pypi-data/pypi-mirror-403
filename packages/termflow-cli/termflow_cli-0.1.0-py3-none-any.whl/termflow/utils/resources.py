...
def get_resource_path(filename: str) -> Path:
    ...
    - Installed mode: uses importlib.resources to access files from the package
    ...
    try:
        resource = files("termflow").parent / filename
        ...
        with files("termflow").parent.joinpath(filename).open('r'):
            pass
        return Path(str(resource))
    except (FileNotFoundError, TypeError, AttributeError, IsADirectoryError):
        pass
    
    # Fallback for development mode: relative to project root
...
def get_ui_resource_path(filename: str) -> Path:
    ...
    Returns:
        Path to the resource file in the ui module
    """
    try:
        resource = files("termflow.ui") / filename
        if hasattr(resource, '__fspath__'):
            path = Path(str(resource))
            if path.exists():
                return path
        # For packaged contexts, return the resource object as Path
        with files("termflow.ui").joinpath(filename).open('r'):
            pass
        return Path(str(resource))
    except (FileNotFoundError, TypeError, AttributeError, IsADirectoryError):
        pass
