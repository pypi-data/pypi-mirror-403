from collections.abc import Mapping
from typing import Any, TypeVar, Union, cast

from attrs import define as _attrs_define
from attrs import field as _attrs_field

from ..models.windows_indexing_settings_indexing_type import WindowsIndexingSettingsIndexingType
from ..types import UNSET, Unset

T = TypeVar("T", bound="WindowsIndexingSettings")


@_attrs_define
class WindowsIndexingSettings:
    """
    Attributes:
        indexing_type (WindowsIndexingSettingsIndexingType): Indexing mode.
        included_folders (Union[Unset, list[str]]): Array of paths to the indexed folders.
            > Required for the `SpecifiedFolders` indexing mode.
        excluded_folders (Union[Unset, list[str]]): Array of paths to folders that are excluded from the indexing scope.
            > Required for the `ExceptSpecifiedFolders` indexing mode.
    """

    indexing_type: WindowsIndexingSettingsIndexingType
    included_folders: Union[Unset, list[str]] = UNSET
    excluded_folders: Union[Unset, list[str]] = UNSET
    additional_properties: dict[str, Any] = _attrs_field(init=False, factory=dict)

    def to_dict(self) -> dict[str, Any]:
        indexing_type = self.indexing_type.value

        included_folders: Union[Unset, list[str]] = UNSET
        if not isinstance(self.included_folders, Unset):
            included_folders = self.included_folders

        excluded_folders: Union[Unset, list[str]] = UNSET
        if not isinstance(self.excluded_folders, Unset):
            excluded_folders = self.excluded_folders

        field_dict: dict[str, Any] = {}
        field_dict.update(self.additional_properties)
        field_dict.update(
            {
                "indexingType": indexing_type,
            }
        )
        if included_folders is not UNSET:
            field_dict["includedFolders"] = included_folders
        if excluded_folders is not UNSET:
            field_dict["excludedFolders"] = excluded_folders

        return field_dict

    @classmethod
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        d = dict(src_dict)
        indexing_type = WindowsIndexingSettingsIndexingType(d.pop("indexingType"))

        included_folders = cast(list[str], d.pop("includedFolders", UNSET))

        excluded_folders = cast(list[str], d.pop("excludedFolders", UNSET))

        windows_indexing_settings = cls(
            indexing_type=indexing_type,
            included_folders=included_folders,
            excluded_folders=excluded_folders,
        )

        windows_indexing_settings.additional_properties = d
        return windows_indexing_settings

    @property
    def additional_keys(self) -> list[str]:
        return list(self.additional_properties.keys())

    def __getitem__(self, key: str) -> Any:
        return self.additional_properties[key]

    def __setitem__(self, key: str, value: Any) -> None:
        self.additional_properties[key] = value

    def __delitem__(self, key: str) -> None:
        del self.additional_properties[key]

    def __contains__(self, key: str) -> bool:
        return key in self.additional_properties
