...
class Wav2VecAlignment:
    ...
    def __init__(self, device='cuda' if not torch.backends.mps.is_available() else 'mps'):
        self.model = Wav2Vec2ForCTC.from_pretrained(os.environ.get("TTSDB_TORTOISE_W2V_ROBUST", "jbetker/wav2vec2-large-robust-ft-libritts-voxpopuli")).cpu()
        self.feature_extractor = Wav2Vec2FeatureExtractor.from_pretrained(os.environ.get("TTSDB_TORTOISE_W2V_960H", "facebook/wav2vec2-large-960h"))
        self.tokenizer = Wav2Vec2CTCTokenizer.from_pretrained(os.environ.get("TTSDB_TORTOISE_TACOTRON_SYMBOLS", "jbetker/tacotron-symbols"))
        self.device = device
