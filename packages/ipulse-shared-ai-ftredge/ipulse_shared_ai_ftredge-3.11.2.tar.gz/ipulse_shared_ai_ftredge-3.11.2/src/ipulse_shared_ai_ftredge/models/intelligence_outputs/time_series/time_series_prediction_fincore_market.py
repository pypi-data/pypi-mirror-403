...
class FincoreMarketInvestmentThesisWithRating(BaseModel):
    """
    ...
    prediction_response_generated_at_utc: Optional[datetime] = Field(None, description="When this analysis was generated")
    
    # --- Target Context ---
    subject_id: str = Field(..., description="ID of the asset being analyzed")
    subject_name: Optional[str] = Field(None, description="Name of the asset being analyzed")
    subject_category: Optional[str] = Field(None, description="Asset category: equity, crypto, fund")
    subject_category_detailed: Optional[str] = Field(None, description="Detailed category: common_stock, etf, bitcoin, mutual_fund")
    contract_or_ownership_type: Optional[str] = Field(None, description="Type: spot, adr, gdr, futures")
    predicted_records_type: Optional[str] = Field(None, description="Type of records predicted (e.g., 'eod_adjc')")
    unit: Optional[str] = Field(None, description="Unit: USD, EUR, BTC, etc.")
    ...
    # --- Horizon Context ---
    prediction_horizon_end_timestamp_utc: Optional[datetime] = Field(None, description="End of prediction horizon")
    prediction_horizon_timeframe: Optional[str] = Field(None, description="Horizon timeframe: 5Y, 3Y, etc.")
    ...
    forecasted_return_percent_bear_case_horizon: Optional[float] = Field(None, description="Bear case return %")
    forecasted_return_percent_bull_case_horizon: Optional[float] = Field(None, description="Bull case return %")
    forecasted_return_percent_reasonable_horizon: Optional[float] = Field(None, description="Reasonable case return %")
    ...
    investment_rating_explanation: Optional[str] = Field(None, description="Rating explanation")
    
    investment_rating_confidence_subjective_by_model: Optional[float] = Field(None, ge=0.0, le=1.0, description="Confidence (0-1)")
    advised_to_reevaluate_rating_by_timestamp_utc: Optional[datetime] = Field(None, description="When to reevaluate")
    ...
    forecast_horizon_anchor_value: Optional[float] = Field(None, description="Forecast horizon anchor value")
    forecast_horizon_anchor_value_timestamp_utc: Optional[datetime] = Field(None, description="Forecast horizon anchor timestamp")
    ...
    changelog_registry: Optional[Any] = Field(None, description="JSON changelog with timestamp keys and 'updater_id:: description' values")
    ...
    @classmethod
    def from_bigquery_row(cls, row: Dict[str, Any]) -> "FincoreMarketInvestmentThesisWithRating":
