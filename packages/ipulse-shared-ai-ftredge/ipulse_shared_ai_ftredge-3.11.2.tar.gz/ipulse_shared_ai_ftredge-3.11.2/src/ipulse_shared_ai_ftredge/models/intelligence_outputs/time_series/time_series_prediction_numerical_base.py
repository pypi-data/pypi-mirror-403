...
class TimeSeriesPredictionNumericalBase(BaseModel):
    ...
    values_mode: Optional[str] = Field(
        None, 
        description=(
            "Indicates how step-by-step percentage changes were derived. "
            "'original' = Model produced absolute prices(non-s-tationary) directly. "
            "'calculated' = Model produced step by step change (stationary), then absolute prices were calculated and populated post-processing."
        )
    )
