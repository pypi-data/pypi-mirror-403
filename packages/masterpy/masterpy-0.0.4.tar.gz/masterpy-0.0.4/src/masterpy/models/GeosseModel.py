...
from masterpy import sort_binary_vectors,States,Event
...
class GeosseModel(BaseModel):
    ...
    def make_states(self):
        ...
        states = {}
        states['S'] = (self.num_ranges) #, 1)  # exant species with ranges
        states['G'] = (self.num_char) #, 1)    # gain events in region
        states['L'] = (self.num_char) #, 1)    # loss events in region
        states['X'] = (self.num_char) #, 1)    # extinction in region
    ...
    def make_start_state(self, params):
        # get starting state for lineage
        start_state = {}
        start_state['S'] = str( self.params['StartState'][0] )
        return start_state

    def make_start_sizes(self, params):
        start_sizes = {}
        # get starting regions of lineage
        start_range_vec = self.statespace.int2vec[ self.params['StartState'][0] ]
        #print('start_range_vec', start_range_vec)
        #start_sizes['G'] = np.array(start_range_vec)
        #print(start_sizes['G'])
    ...
    def make_params(self):
        ...
        params['StartState'] = sp.stats.randint.rvs(low=0,
    ...
    def make_events(self):
        ...
        events_x = self.make_events_x()
        
        # regional extinction (extirpation) events
        events_e = self.make_events_e()
        
        # dispersal events
        events_d = self.make_events_d()
        
        # within-region speciation events
        events_w = self.make_events_w()
        
        # between-region speciation events
        events_b = self.make_events_b(normalize_rates=True)
        
        core_events = events_w + events_d + events_b + events_x + events_e

        extra_events = []
        if self.model_variant == 'density_effect':
             extra_events_x_DE = self.make_events_x_DE()
             extra_events_e_DE = self.make_events_e_DE()
    ...
    def make_events_x(self):  
        group = 'Extinction'
        rates = self.params['x']
        statespace = self.statespace
        events = []
        for range_state,range_vector in enumerate(statespace.int2vec):
    ...
    def make_events_x_DE(self):  
        group = 'Extinction_DE'
        rates = self.params['xd']
        statespace = self.statespace
        events = []
        for range_state,range_vector in enumerate(statespace.int2vec):
            if sum(range_vector) == 1:
        ...
        for range_state,range_vector in enumerate(statespace.int2vec):
            for region_idx,region_state in enumerate(range_vector):
                if sum(range_vector) >= 2 and region_state == 1:
                    ...
                    new_state = statespace.vec2int[ new_bits ]
    ...
    def make_events_e_DE(self):
        group = 'Extirpation_DE'
        rates = self.params['ed']
        statespace = self.statespace
        events = []
        for range_state,range_vector in enumerate(statespace.int2vec):
            for region_idx,region_state in enumerate(range_vector):
    ...
    def make_events_d(self):
        group = 'Dispersal'
        rates = self.params['d']
        statespace = self.statespace
        # how many compound states
        num_states = len(statespace.int2int)
        ...
        for range_state in range(num_states):
            ...
            for a,new_region_idx in enumerate(off[range_state]):
                ...
                new_bits = list(statespace.int2vec[range_state]).copy()
                new_bits[new_region_idx] = 1
                new_state = statespace.vec2int[ tuple(new_bits) ]
    ...
    def make_events_b(self,normalize_rates=False):
        ...
        events = []
        all_rates = []
        for range_state,anc_region_set in enumerate(statespace.int2set):
            ...
            for left_region_set in splits:
                ...
                left_state  = statespace.set2int[left_region_set] 
                right_state = statespace.set2int[right_region_set]
                # build event
