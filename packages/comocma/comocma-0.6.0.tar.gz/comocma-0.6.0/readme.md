![Deploy to GitHub Pages](https://github.com/CMA-ES/pycomocma/actions/workflows/tests.yml/badge.svg)
# Introduction
...
- Documentation in
  ...
  - [epydocs format (outdated)](https://cma-es.github.io/pycomocma/comocma-epydocs/index.html)
...
runs the doc-tests from `comocma.como` and `comocma.test` (by calling ``comocma.test.main()``).
...
### Initialize
list_of_solvers = comocma.get_cmas(
        num_kernels * [dimension * [0]], sigma0) # return num_kernels cma instances
...
```
    ...
    {0: {'tolx': 0.0001}, 1: {'tolx': 0.0001}, 2: {'tolx': 0.0001}, 3: {'tolx': 0.0001}, 4: {'tolx': 0.0001}}
...
plots the optimization from "the first" kernel (the order of kernels is arbitrary) and
