...
class Scheduler:
    ...
    def __init__(
        ...
        Args:
            ...
            signal_handler (Callable[[str, dict], None], optional):
                信号回调函数，用于接收调度器的事件通知。
                如果为 None，则使用默认的 default_signal_handler (打印到 logger)。
        ...
        self.signal_handler = signal_handler if signal_handler is not None else self._default_signal_handler
        self._stop_requested = False
            
        self.logger.debug("Scheduler initialized. Name: %s, Stop on error: %s", name, stop_on_error)

    def _default_signal_handler(self, event: str, data: dict) -> None:
        """默认信号处理器：将事件记录到日志"""
        if event == "start":
            self.logger.info(f"Signal [start]: Total tasks={data.get('total_tasks')}")
        elif event == "task_start":
            idx = data.get('index', -1) + 1
            total = data.get('total', '?')
            self.logger.info(f"Signal [task_start]: {data.get('task_name')} ({idx}/{total})")
        elif event == "task_success":
            self.logger.debug(f"Signal [task_success]: {data.get('task_name')}")
        elif event == "task_error":
            self.logger.error(f"Signal [task_error]: {data.get('task_name')} - {data.get('error')}")
        elif event == "finish":
            self.logger.info(f"Signal [finish]: Status={data.get('status')}")
    ...
    def add(self, func: Callable[..., Any], *args: Any, task_name: str | None = None, **kwargs: Any) -> None:
        """
        向调度器添加一个待执行的任务。
        ...
        """
        # 智能处理 'name' 参数冲突：
        # 如果用户传递了 'name' 但没有传递 'task_name'，且目标函数看起来不接受 'name'，
        # 则假设用户意图是设置任务名称。
        if task_name is None and "name" in kwargs:
            try:
                sig = inspect.signature(func)
                # 检查函数签名中是否有 'name' 参数，或者是接受 **kwargs
                has_name_param = "name" in sig.parameters
                has_kwargs = any(p.kind == inspect.Parameter.VAR_KEYWORD for p in sig.parameters.values())
                
                if not has_name_param and not has_kwargs:
                    # 目标函数不接受 name 也不接受 **kwargs，说明这个 name 是给调度器的
                    task_name = kwargs.pop("name")
                    self.logger.warning(
                        "Using 'name' kwarg as task name is deprecated and may conflict. "
                        "Please use 'task_name' argument instead."
                    )
            except ValueError:
                # 某些内置函数可能无法获取签名，保守处理，不做操作
                pass
    ...
    def set_attribute(self, name: str, value: Any) -> None:
        ...
        self.logger.debug("Attribute '%s' set on Scheduler instance.", name)
