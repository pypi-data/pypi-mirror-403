"""Dart plugin with semantic search support."""
...
class DartPluginSemantic(PluginWithSemanticSearch):
    """Dart plugin with semantic search capabilities."""
    ...
    def _preindex(self) -> None:
        """Pre-index Dart files in the current directory."""
    ...
    def supports(self, path: str | Path) -> bool:
        """Return True if file extension matches Dart."""
        return Path(path).suffix in self._get_extensions()

    def indexFile(self, path: str | Path, content: str) -> IndexShard:
        """Index a Dart file with optional semantic embeddings."""
        ...
        return IndexShard(file=str(path), symbols=symbols, language="dart")

    def _extract_symbols(self, content: str, file_id: Optional[int] = None) -> List[Dict]:
        """Extract symbols from Dart code."""
        symbols = []
        lines = content.split("\n")

        # Basic symbol extraction - override in actual implementation
        for i, line in enumerate(lines):
            if "class" in line or "void" in line or "Future" in line or "Stream" in line:
                stripped = line.strip()
