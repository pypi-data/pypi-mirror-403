...
class ResponseSubmitScreenerEvent(BaseModel):
    """
    ResponseSubmitScreenerEvent
    """ # noqa: E501
    type: StrictStr
    screener_questions_responses: List[ScreenerQuestionResponse] = Field(alias="screenerQuestionsResponses")
    __properties: ClassVar[List[str]] = ["type", "screenerQuestionsResponses"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['SUBMIT_SCREENER']):
            raise ValueError("must be one of enum values ('SUBMIT_SCREENER')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of ResponseSubmitScreenerEvent from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of ResponseSubmitScreenerEvent from a dict"""
        if obj is None:
            return None
