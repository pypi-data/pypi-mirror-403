...
def run_build_conan(args, project_dir: Path, toolchain_name: Optional[str], build_type: str, project_config: dict = None, for_test: bool = False) -> bool:
    ...
    # 生成 conanbase.py（支持依赖注入，避免硬编码版本）
    _ensure_conanbase_in_project(project_dir, logger, project_config, toolchain.name)

    # 输出构建配置
    logger.info(f"构建配置: toolchain={toolchain.name}, target={target}, build_type={build_type}")

    # 版本号和依赖将通过模板引擎注入到 conanbase.py
    logger.debug(f"项目版本: {project_config.get('version', '0.0.0') if project_config else '0.0.0'}")
...
def run_build(args) -> bool:
    ...
    return run_build_conan(args, project_dir, toolchain_name, build_type, project_config)


def _ensure_conanbase_in_project(project_dir: Path, logger, project_config: dict, toolchain_name: str = "default") -> None:
    ...
    Args:
        project_dir: 项目目录
        logger: 日志记录器
        project_config: service.json 配置
    ...
    template_file = get_mcli_root() / "templates" / "conanbase.py.mct"

    # 准备模板上下文
    project_version = project_config.get("version", "0.0.0") if project_config else "0.0.0"
    ...
    # 解析 dependencies.build 中的 conan 依赖
    dependencies = project_config.get("dependencies", {}) if project_config else {}
