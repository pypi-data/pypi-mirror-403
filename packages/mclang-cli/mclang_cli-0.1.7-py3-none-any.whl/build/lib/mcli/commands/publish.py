...
def run_publish_conan(args, project_dir: Path, toolchain_name: Optional[str]) -> bool:
    ...
    # 重新生成 conanbase.py，确保使用正确的工具链名称
    # 这很重要，因为 conan export-pkg 会读取 conanbase.py 的 layout() 来确定构建目录
    if toolchain:
        logger.info("  更新 conanbase.py 以匹配工具链...")
        try:
            from mcli.paths import get_mcli_root
            ...
            if template_file.exists():
                ...
                context = {
                    "project_version": pkg_version,  # 直接使用已解析的版本
                    "conan_requires": [],
                    "conan_test_requires": [],
                ...
                logger.debug(f"✓ 已更新 conanbase.py: version={pkg_version}, toolchain={toolchain.name}")
        except Exception as e:
            logger.warning(f"更新 conanbase.py 失败: {e}")
    ...
    if not run_build_conan(args, project_dir, toolchain_name, requested_bt, project_config):
