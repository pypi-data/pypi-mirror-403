...
def _get_project_templates_dirs() -> List[tuple[str, Path]]:
    """获取所有 project 模板目录（包括第三方模板）

    Returns:
        [(namespace, templates_dir), ...] 列表
        - namespace: 命名空间（"mcli" 或公司名）
        - templates_dir: 模板目录路径
    """
    from ..paths import get_installed_templates_dir

    dirs = []

    # 1. mcli 自带模板
    mcli_templates = get_templates_dir() / "project"
    if mcli_templates.is_dir():
        dirs.append(("mcli", mcli_templates))

    # 2. 第三方模板 (thirdparty/company-name/)
    # 第三方模板直接放在 company-name/ 下，无 project/ 层
    thirdparty_dir = get_installed_templates_dir() / "thirdparty"
    if thirdparty_dir.is_dir():
        for company_dir in thirdparty_dir.iterdir():
            if company_dir.is_dir():
                dirs.append((company_dir.name, company_dir))

    return dirs


def _parse_template_spec(template_spec: str) -> tuple[Optional[str], str]:
    """解析模板规范

    Args:
        template_spec: 模板规范，如 "company/template" 或 "template"

    Returns:
        (namespace, template_name) 元组
        - namespace: 公司/组织名，如果未指定则为 None
        - template_name: 模板名称
    """
    if "/" in template_spec:
        namespace, template_name = template_spec.split("/", 1)
        return namespace, template_name
    else:
        return None, template_spec
...
def _list_available_templates() -> List[str]:
    ...
    for namespace, templates_dir in _get_project_templates_dirs():
...
def _get_template(template_spec: str) -> Optional[ProjectTemplate]:
    """查找指定模板
    ...
    Returns:
        模板对象，如果未找到则返回 None
    """
    namespace, template_name = _parse_template_spec(template_spec)

    # 如果指定了命名空间，精确查找
    if namespace:
        for ns, templates_dir in _get_project_templates_dirs():
            if ns == namespace:
    ...
    # 未指定命名空间，遍历所有目录查找
    # 反向遍历，让第三方模板优先
    for ns, templates_dir in reversed(_get_project_templates_dirs()):
        template_dir = templates_dir / template_name
        if template_dir.exists() and template_dir.is_dir():
            return ProjectTemplate(template_dir)
...
def show_template_help(template_spec: str) -> bool:
    """显示模板的可配置变量"""
    logger = get_logger("create", Path.cwd(), "debug")
    template = _get_template(template_spec)

    if template is None:
        logger.error(f"模板 '{template_spec}' 不存在")
        logger.info(f"所有模板: {', '.join(_list_available_templates())}")
        return False

    print(f"模板: {template_spec}")
...
def get_template_descriptions() -> Dict[str, str]:
    """获取所有模板的描述"""
    templates = {}
    for spec in _list_available_templates():
        template = _get_template(spec)
        if template:
            templates[spec] = template.description
    return templates
...
def setup_parser(subparsers):
    ...
    create_parser.add_argument(
        ...
        "--template",
        default="app",
        help="项目模板 (app=应用程序, module=模块, 或 company/template 指定第三方模板)",
    )
    create_parser.add_argument(
        "-o",
        "--output",
