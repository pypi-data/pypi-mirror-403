...
def _fwd_kernel(
    ...
    q = tl.load(Q_block_ptr + offs_m[:, None] * stride_qm + offs_d[None, :] * stride_qk)
    ...
    for start_n in range(0, N_CTX, BLOCK_N):
        ...
        k = tl.load(K_block_ptr + (start_n + offs_n)[:, None] * stride_kn + offs_d[None, :] * stride_kk)
        ...
        v = tl.load(V_block_ptr + (start_n + offs_n)[:, None] * stride_vn + offs_d[None, :] * stride_vk)
        acc += tl.dot(p.to(v.dtype), v)
        
        m_i = m_ij
    
    acc = acc / l_i[:, None]
    tl.store(O_block_ptr + offs_m[:, None] * stride_om + offs_d[None, :] * stride_ok, acc)
...
def triton_attention(q, k, v, scale, causal=True):
    ...
    grid = (triton.cdiv(total_seq, BLOCK), num_heads)
    ...
    _fwd_kernel[grid](
        ...
        BLOCK_M=BLOCK, BLOCK_DMODEL=BLOCK_DMODEL, BLOCK_N=BLOCK,
    )
