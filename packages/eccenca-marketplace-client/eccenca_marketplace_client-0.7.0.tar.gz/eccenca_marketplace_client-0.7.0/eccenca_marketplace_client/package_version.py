...
class PackageVersion:
    ...
    def from_archive(cls, archive: Path, validate_files: bool = True) -> "PackageVersion":
        ...
        with zipfile.ZipFile(archive, "r") as zipf, zipf.open(MANIFEST_NAME) as file:
    ...
    def from_directory(cls, directory: Path, validate_files: bool = True) -> "PackageVersion":
        ...
        json_text = (directory / MANIFEST_NAME).read_text()
        manifest = PackageVersion.load_manifest(json_text)
        if validate_files:
            ...
            PackageVersion.validate_files(manifest, directory)
    ...
    def build_archive(self, archive: Path | None = None) -> Path:
        ...
        JSON and add all files which are described as file spec to the ZIP.
        ...
        with ZipFile(archive_path, "w", zipfile.ZIP_DEFLATED, compresslevel=9) as zipf:
            ...
            zipf.writestr(MANIFEST_NAME, manifest_json)
    ...
    @staticmethod
    def validate_files(manifest: ValidManifest, content: Path) -> bool:
        """Additional checks for all files"""
        if not content.is_dir():
            raise ValueError("Cannot validate: content is not a directory")
        for file_spec in manifest.files:
            ...
            if file_stat.st_size == 0:
                raise PackageContentError("Empty files are not allowed.")
        return True
    ...
    def validate_file_paths(manifest: ValidManifest, content: Path) -> bool:
        ...
        if content.is_dir():
            content_list = {
                ...
                if _.is_file() and _.name != MANIFEST_NAME
            }
        if content.is_file():
            with ZipFile(content) as zip_archive:
                content_list = {
                    ...
                    if not _.is_dir() and _.filename != MANIFEST_NAME
                }
