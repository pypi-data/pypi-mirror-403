...
class TestImageParamsV2:
    ...
    def test_default_initialization(self):
        ...
        assert params.quality == DEFAULTS.IMAGE_QUALITY  # medium
    ...
    def test_to_query_bool_conversion(self):
        ...
        params = ImageParamsV2(enhance=True, safe=True)
        query = params.to_query()

        assert "enhance=true" in query.lower()
        assert "safe=true" in query.lower()
...
class TestChatParamsV2:
    ...
    def test_default_initialization(self):
        ...
        # ✅ ИСПРАВЛЕНО: ожидаем 1.0 вместо None
    ...
    def test_to_body_excludes_default_temperature(self):
        """Test to_body excludes temperature if default (1.0)."""
        # ✅ ИСПРАВЛЕНО: теперь проверяем, что temperature=1.0 НЕ включается
...
class TestConstants:
    ...
    def test_defaults_exist(self):
        ...
        # ✅ ИСПРАВЛЕНО: проверяем новый дефолт
        assert DEFAULTS.IMAGE_NEGATIVE_PROMPT == "worst quality, blurry"
