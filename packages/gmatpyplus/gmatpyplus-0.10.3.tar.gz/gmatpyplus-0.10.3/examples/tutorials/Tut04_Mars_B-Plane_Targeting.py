...
near_earth_fm = gp.ForceModel('NearEarthFM', primary_body='Earth',
                              gravity_field=gp.ForceModel.GravityField(degree=8, order=8),
...
near_mars_fm = gp.ForceModel('NearMarsFM', central_body='Mars', primary_body='Mars',
                             gravity_field=gp.ForceModel.GravityField(body='Mars', model='Mars-50C', degree=8,
...
moi = gp.ImpulsiveBurn('MOI', coord_sys={'CoordinateSystem': 'Local', 'Origin': 'Mars', 'Axes': 'VNB'},
                       decrement_mass=True, tanks='MainTank')
...
mcs = [
    # Target Mars' B-plane
    gp.Target('Target desire B-plane coordinates', dc1, exit_mode='SaveAndContinue', command_sequence=[
        gp.Propagate('Prop 3 days', sat, near_earth, (f'{sat.name}.ElapsedDays', 3)),
        gp.Propagate('Prop 12 days to TCM', sat, deep_space, (f'{sat.name}.ElapsedDays', 12)),
        # Vary the Trajectory Correction Maneuver (TCM) elements
        gp.Vary('Vary TCM.V', dc1, variable=f'{tcm.name}.Element1', initial_value=0.003937696373137754,
                perturbation=0.00001, lower=-10e300, upper=10e300, max_step=0.002),
        gp.Vary('Vary TCM.N', dc1, variable=f'{tcm.name}.Element2', initial_value=0.006042317048292264,
                perturbation=0.00001, lower=-10e300, upper=10e300, max_step=0.002),
        gp.Vary('Vary TCM.B', dc1, variable=f'{tcm.name}.Element3', initial_value=-0.0006747125433520692,
                perturbation=0.00001, lower=-10e300, upper=10e300, max_step=0.002),
        gp.Maneuver('Apply TCM', tcm, sat),
        gp.Propagate('Prop 280 days', sat, deep_space, (f'{sat.name}.ElapsedDays', 280)),
        gp.Propagate('Prop to Mars Periapsis', sat, near_mars, f'{sat.name}.Mars.Periapsis'),
        gp.Achieve('Achieve BdotT', dc1, f'{sat.name}.{mars_inertial.name}.BdotT', 0, tolerance=0.00001),
        gp.Achieve('Achieve BdotR', dc1, f'{sat.name}.{mars_inertial.name}.BdotR', -7000, tolerance=0.00001)
    ]),
    # Capture into Mars orbit
    gp.Target('Mars capture', dc1, exit_mode='SaveAndContinue', command_sequence=[
        # Vary the burn duration to achieve an altitude of 12000 km
        gp.Vary('Vary MOI.V', dc1, variable=f'{moi.name}.Element1', initial_value=-1.603439847094663,
                perturbation=0.00001, lower=-10e300, upper=10e300, max_step=0.1),
        gp.Maneuver('Apply MOI', moi, sat),
        gp.Propagate('Prop to Mars Apoapsis', sat, near_mars, f'{sat.name}.Mars.Apoapsis'),
        gp.Achieve('Achieve RMAG', dc1, f'{sat.name}.Mars.RMAG', 12000),
    ]),
    gp.Propagate('Prop for 1 day', sat, near_mars, (f'{sat.name}.ElapsedDays', 1)),
]
