...
fb1 = gp.FiniteBurn('FiniteBurn1', thruster_to_fire)
...
mcs = [
    ...
    gp.Target('Raise Apoapsis', dc1, exit_mode='SaveAndContinue', command_sequence=[
        # Vary the FiniteBurn duration to achieve an apoapsis with RMAG = 12000 km
        gp.Vary('Vary Burn Duration', dc1, burn_dur.name, initial_value=200, upper=10000, max_step=100),
        gp.BeginFiniteBurn(fb1, sat, 'Turn Thruster On'),
        gp.Propagate('Prop BurnDuration', sat, prop, (f'{sat.name}.ElapsedSecs', burn_dur.name)),
        gp.EndFiniteBurn(fb1, 'Turn Thruster Off'),
        gp.Propagate('Prop To Apoapsis', sat, prop, f'{sat.name}.Earth.Apoapsis'),
        gp.Achieve('Achieve Apoapsis Radius = 12000', dc1, f'{sat.name}.Earth.RMAG', 12000)
    ])
]
