...
class SidebarPanel(QFrame):
    ...
    def _refresh_contacts(self) -> None:
        ...
        # Clear existing contact cards (keep empty_label and stretch)
        widgets_to_delete = []
        for i in range(self._contacts_layout.count()):
            item = self._contacts_layout.itemAt(i)
            if item and item.widget() and item.widget() != self._empty_label:
                widgets_to_delete.append(item.widget())
        
        for widget in widgets_to_delete:
            self._contacts_layout.removeWidget(widget)
            widget.deleteLater()
