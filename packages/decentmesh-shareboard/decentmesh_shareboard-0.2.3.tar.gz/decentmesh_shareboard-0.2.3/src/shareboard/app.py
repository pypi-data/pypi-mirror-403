...
class ShareBoardWindow(QMainWindow):
    ...
    def __init__(self, parent: Optional[QWidget] = None):
        ...
        self._is_connected = False  # Track if network is ready
    ...
    def _on_status_changed(self, status: str) -> None:
        ...
        # Once connected, stay connected (ignore further connecting messages)
        if self._is_connected:
            # Only react to errors/disconnects
            status_lower = status.lower()
            if "error" in status_lower or "failed" in status_lower or "disconnected" in status_lower:
                self._is_connected = False
                self._sidebar.set_status("offline")
            return
        
        # Parse status for badge display
        status_lower = status.lower()
