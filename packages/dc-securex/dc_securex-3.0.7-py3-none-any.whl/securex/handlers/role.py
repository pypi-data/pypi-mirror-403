...
class RoleHandler:
    ...
    async def on_role_create(self, role: discord.Role):
        ...
        try:
            ...
            # Check audit log for who created the role
            ...
            async for entry in guild.audit_logs(limit=50, action=discord.AuditLogAction.role_create):
                ...
                if entry.target.id == role.id:
                    if not await self._is_authorized(guild, entry.user.id):
                        # Unauthorized role creation - delete it
                        await role.delete(reason="SecureX: Unauthorized role creation")
                        print(f"🗑️ Deleted unauthorized role: {role.name} created by {entry.user.name}")
            ...
            print(f"Error in on_role_create: {e}")
