...
class GuildWorker:
    ...
    async def _restoration_loop(self):
        ...
        while True:
            try:
                ...
                await self._restore_guild_settings(guild, entry)
    ...
    async def _restore_guild_settings(self, guild, entry):
        """Restore guild settings using audit log before/after changes"""
        try:
            print(f"🔧 Starting restoration for guild: {guild.name}")
            start_time = datetime.now()
            
            restore_params = {}
            restored_items = []
            
            # Discord.py's AuditLogChanges object - access specific attributes
            changes = entry.changes
            
            # Check for name change
            if hasattr(changes, 'name') and changes.name:
                if changes.name.before:
                    restore_params["name"] = changes.name.before
                    restored_items.append(f"name ({changes.name.before})")
                    print(f"📝 Name change: '{changes.name.after}' → '{changes.name.before}'")
            
            # Check for vanity URL change
            if hasattr(changes, 'vanity_url_code') and changes.vanity_url_code:
                if changes.vanity_url_code.before:
                    vanity_success = await self._restore_vanity_via_api(guild.id, changes.vanity_url_code.before)
                    if vanity_success:
                        restored_items.append(f"vanity (discord.gg/{changes.vanity_url_code.before})")
                        print(f"📝 Vanity change: '{changes.vanity_url_code.after}' → '{changes.vanity_url_code.before}'")
            
            # Apply restoration if there are changes
            if restore_params:
            ...
            if restored_items:
                ...
                print(f"ℹ️ No restorable changes found")
