...
class GuildWorker:
    ...
    async def _restore_guild_settings(self, guild, entry):
        ...
        try:
            ...
            # Parse audit log changes to get "before" values
            if entry.changes:
                for change in entry.changes:
                    if change.key == "name" and change.before:
                        restore_params["name"] = change.before
                        restored_items.append(f"name ({change.before})")
                        print(f"📝 Name change detected: '{change.after}' → '{change.before}'")
                    
                    elif change.key == "vanity_url_code" and change.before:
                        # Restore vanity URL via API
                        vanity_success = await self._restore_vanity_via_api(guild.id, change.before)
                        if vanity_success:
                            restored_items.append(f"vanity (discord.gg/{change.before})")
                            print(f"📝 Vanity change detected: '{change.after}' → '{change.before}'")
            ...
            if restored_items:
                ...
                print(f"ℹ️ No restorable changes found in audit log")
