    ...
    async def _restore_guild_settings(self, guild):
        ...
        try:
            ...
            if restored_items:
                ...
                # CRITICAL: Refresh backup with current state after restoration
                # This ensures the next restoration uses the correct data
                await asyncio.sleep(1)  # Wait for Discord to apply changes
                updated_guild = await self.bot.fetch_guild(guild.id)
                await self.sdk.backup_manager.save_guild_settings(
                    updated_guild.id,
                    name=updated_guild.name,
                    icon=str(updated_guild.icon.url) if updated_guild.icon else None,
                    banner=str(updated_guild.banner.url) if updated_guild.banner else None,
                    description=updated_guild.description,
                    vanity_url_code=backup_data.get("vanity_url_code")  # Keep same vanity
                )
                print(f"💾 Updated backup with current guild state")
