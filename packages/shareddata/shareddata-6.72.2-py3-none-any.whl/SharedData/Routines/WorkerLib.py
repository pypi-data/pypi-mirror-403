...
def promote_to_master():
    """Configure nginx as master (direct backend proxy)."""
    Logger.log.info('Configuring nginx as master...')
    return update_nginx_config(NGINX_MASTER_CONFIG)

def demote_to_slave(master_ip: str):
    """Configure nginx as slave (proxy to master)."""
    Logger.log.info(f'Configuring nginx as slave, proxying to master at {master_ip}...')
    config = NGINX_SLAVE_CONFIG_TEMPLATE.format(master_ip=master_ip)
    return update_nginx_config(config)
...
def is_nginx_slave_config(master_ip: str) -> bool:
    ...
    if current_config is None:
        return False
    expected_config = NGINX_SLAVE_CONFIG_TEMPLATE.format(master_ip=master_ip)
    # Normalize whitespace for comparison
    current_normalized = ' '.join(current_config.split())
    expected_normalized = ' '.join(expected_config.split())
    return current_normalized == expected_normalized

def ensure_nginx_master_config() -> bool:
    """Ensure nginx is configured as master, update if necessary."""
    if not is_nginx_master_config():
        Logger.log.info('Nginx configuration mismatch detected. Updating to master config...')
        return promote_to_master()
    return True

def ensure_nginx_slave_config(master_ip: str) -> bool:
    ...
    if not is_nginx_slave_config(master_ip):
        ...
        return demote_to_slave(master_ip)
    return True
