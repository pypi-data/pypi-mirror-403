...
BTMINERV3_DATA_LOC = DataLocations(
    **{
        ...
        str(DataOptions.POOLS): DataFunction(
            "_get_pools",
            [RPCAPICommand("get_miner_status_pools", "get.miner.status:pools")],
        ),
        str(DataOptions.UPTIME): DataFunction(
            "_get_uptime",
            [RPCAPICommand("rpc_get_miner_status_summary", "get.miner.status:summary")],
...
class BTMinerV3(StockFirmware):
        ...
        for idx in range(board_count):
            ...
            boards.append(
                HashBoard(
                    ...
                    chip_temp=board_data.get("chip-temp-avg"),
    ...
    async def _get_pools(
        self, get_miner_status_pools: dict | None = None) -> list[PoolMetrics]:
        return await self.get_pools()

    async def get_pools(self) -> list[PoolMetrics]:
        search_pools = await CGMinerRPCAPI(ip=self.ip).pools()
        if not search_pools:
            return []
        resurt = []
        for index, data in enumerate(search_pools.get("POOLS", [])):
            url = data.get("URL")
            pool_url = PoolUrl.from_str(url) if url else None
            pool = PoolMetrics(
                accepted=data.get("Accepted"),
                rejected=data.get("Rejected"),
                get_failures=data.get("Get Failures"),
                remote_failures=data.get("Remote Failures"),
                active=data.get("Stratum Active"),
    ...
    async def _get_expected_hashrate(
        ...
        if self.expected_hashboards is not None and res == (
            ...
            return None
        rate_unit = "TH"  # В whatsminers вместе с expected_hashrate не приходят единицы измерения, полагаем что это TH
        # атрибут .expected_hashrate должен храниться в виде экземпляра метакласса MinerAlgoMeta (можно кастомный от этого)
        return self.algo.hashrate(rate=float(res), unit=self.algo.unit.from_str(rate_unit)).into(self.algo.unit.default)
