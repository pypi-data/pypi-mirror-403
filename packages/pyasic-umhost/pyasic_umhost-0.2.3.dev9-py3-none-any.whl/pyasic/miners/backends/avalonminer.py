...
class AvalonMiner(CGMiner):
    ...
    async def _is_sleep(self, rpc_estats: dict = None) -> bool | None:
        if rpc_estats is None:
            try:
                rpc_estats = await self.rpc.estats()
            except APIError:
    ...
    async def _is_mining(self, rpc_estats: dict = None) -> bool:
        if rpc_estats is None:
            try:
                rpc_estats = await self.rpc.estats()
            except APIError:
        ...
        try:
            mm = self.parse_estats(rpc_estats)["STATS"][0]["MM ID0"]

            # 1. SoftOFF: если > 0 — устройство в софтовом оффе, не считаем это майнингом
            softoff = int(mm.get("SoftOFF", 0))
            if softoff > 0:
                return False

            # 2. Статус работы
            status = str(mm.get("SYSTEMSTATU", "")).lower()
            # если статус не "in work" — это не майнинг (idle, sleep, ошибка и т.п.)
            if "in work" not in status:
                return False

            # 3. Хешрейт
            ghs_avg = float(mm.get("GHSavg", 0) or 0.0)
            # если хешрейт нулевой — тоже не майнинг
            if ghs_avg <= 0.0:
                return False

            # Если прошли все проверки — реально майнит
            return True
