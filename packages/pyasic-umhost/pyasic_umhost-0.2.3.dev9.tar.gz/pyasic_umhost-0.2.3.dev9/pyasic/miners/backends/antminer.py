...
class AntminerModern(BMMiner):
        ...
        if rpc_stats is not None:
            try:
                for board in rpc_stats["STATS"][0]["chain"]:
                    unit = getattr(self, "default_unit", "GH")
                    hashboards[board["index"]].hashrate = self.algo.hashrate(
                        rate=board["rate_real"],
                        unit=self.algo.unit.from_str(unit),  # type: ignore[attr-defined]
                    ).into(
    ...
    async def _get_expected_hashrate(
        ...
        if rpc_stats is not None:
            try:
                ...
                # Проверка на наличие единицы измерения, Если нет берем из атрибута класса модели
                if "rate_unit" in rpc_stats["STATS"][1]:
                    rate_unit = rpc_stats["STATS"][1]["rate_unit"]
                else:
                    rate_unit = getattr(self, "default_unit", "GH")
    ...
    async def _is_sleep(self, web_get_conf: dict = None) -> bool | None:
        if web_get_conf is None:
            try:
                web_get_conf = await self.web.get_miner_conf()
            except APIError:
