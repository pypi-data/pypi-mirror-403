...
class PopbillBase(__with_metaclass(Singleton, object)):
    ...
    def _getToken(self, CorpNum):
        ...
        try:
            token = self.__tokenCache["TEST_"+CorpNum if self.IsTest else "PROD_"+CorpNum]
        except KeyError:
            token = None
        ...
        if refreshToken:
            try:
                ...
                try:
                    del self.__tokenCache["TEST_"+CorpNum if self.IsTest else "PROD_"+CorpNum]
                except KeyError:
                    pass

                self.__tokenCache["TEST_"+CorpNum if self.IsTest else "PROD_"+CorpNum] = token
