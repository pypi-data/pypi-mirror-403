...
class TaxinvoiceService(PopbillBase):
    ...
    def attachFileBinary(self, CorpNum, MgtKeyType, MgtKey, BinaryFile, UserID=None):
        ...
        if BinaryFile == None:
            raise PopbillException(-99999999, "파일이 입력되지 않았습니다.")

        if BinaryFile.fileName == None or BinaryFile.fileName == "":
            raise PopbillException(-99999999, "파일명이 입력되지 않았습니다.")

        if BinaryFile.fileData == None:
            raise PopbillException(-99999999, "파일 데이터가 입력되지 않았습니다.")

        files = []
        try:
            files = [File(fieldName="Filedata", fileName=BinaryFile.fileName , fileData=BinaryFile.fileData)]
        except IOError:
            raise PopbillException(-99999999, "파일이 읽을 수 없습니다.")
