...
class TestPlayerToolsIntegration:
    ...
    async def test_fpl_search_players_markdown(self, session_store):
        """Test searching players with markdown output."""
        # Arrange
        params = SearchPlayersInput(name_query="Salah", limit=10)

        # Act
        result = await fpl_search_players(params)

        # Assert
        assert "Player Search Results: 'Salah'" in result
        assert "Salah" in result or "M.Salah" in result  # Could be web_name
        assert "Liverpool" in result
        # Price can be £13.0m or £14.0m depending on mock data

    async def test_fpl_search_players_json(self, session_store):
        """Test searching players with JSON output."""
        # Arrange
        params = SearchPlayersInput(
            name_query="Salah", limit=10, response_format=ResponseFormat.JSON
        )
        ...
        # Assert
        assert '"query": "Salah"' in result
        assert '"count":' in result
        assert '"players":' in result
        # Name could be "Salah" or "M.Salah" (web_name)
        assert '"name":' in result

    async def test_fpl_search_players_no_results(self, session_store):
        """Test searching with no matches."""
        # Arrange
        params = SearchPlayersInput(name_query="NonexistentPlayer123")
    ...
    async def test_fpl_search_players_by_team_success(self, session_store):
        """Test getting team squad."""
        # Arrange
        params = SearchPlayersByTeamInput(team_name="Liverpool")

        # Act
        result = await fpl_search_players_by_team(params)

        # Assert
        assert "Liverpool" in result
        assert "Salah" in result
        assert "MID:" in result or "**MID:**" in result  # Position grouping

    async def test_fpl_search_players_by_team_json(self, session_store):
        """Test getting team squad in JSON format."""
        # Arrange
        params = SearchPlayersByTeamInput(team_name="Arsenal", response_format=ResponseFormat.JSON)
        ...
        assert '"name": "Arsenal"' in result
        assert '"player_count":' in result

    async def test_fpl_search_players_by_team_not_found(self, session_store):
        """Test team not found error."""
        # Arrange
        params = SearchPlayersByTeamInput(team_name="NonexistentTeam")
        ...
        assert "NonexistentTeam" in result
    ...
    async def test_fpl_get_top_performers_markdown(self, session_store, mock_fixtures_data):
        ...
        assert "GW" in result or "error" in result.lower() or "No finished fixtures" in result
...
class TestTeamToolsIntegration:
    ...
    async def test_fpl_get_team_info_success(self, session_store):
        """Test getting team information."""
        from src.tools.teams import GetTeamInfoInput, fpl_get_team_info

        # Arrange
        params = GetTeamInfoInput(team_name="Arsenal")

        # Act
        result = await fpl_get_team_info(params)

        # Assert
        assert "Arsenal" in result
        assert "ARS" in result  # Short name

    async def test_fpl_get_team_info_not_found(self, session_store):
        """Test team not found error."""
        from src.tools.teams import GetTeamInfoInput, fpl_get_team_info

        # Arrange
        params = GetTeamInfoInput(team_name="NonexistentTeam")

        # Act
        result = await fpl_get_team_info(params)

        # Assert
        assert "no team found" in result.lower() or "no teams found" in result.lower()

    async def test_fpl_list_all_teams_markdown(self, session_store):
        """Test listing all teams in markdown format."""
        from src.tools.teams import ListAllTeamsInput, fpl_list_all_teams

        # Arrange
        params = ListAllTeamsInput()

        # Act
        result = await fpl_list_all_teams(params)
    ...
    async def test_fpl_list_all_teams_json(self, session_store):
        """Test listing all teams in JSON format."""
        from src.tools.teams import ListAllTeamsInput, fpl_list_all_teams

        # Arrange
        params = ListAllTeamsInput(response_format=ResponseFormat.JSON)
...
class TestGameweekToolsIntegration:
    ...
    async def test_fpl_get_gameweek_info_success(self, session_store):
        """Test getting specific gameweek info."""
        from src.tools.gameweeks import GetGameweekInfoInput, fpl_get_gameweek_info

        # Arrange
        params = GetGameweekInfoInput(gameweek_number=1)

        # Act
        result = await fpl_get_gameweek_info(params)

        # Assert
        assert "Gameweek 1" in result or "GW 1" in result or "GW1" in result

    async def test_fpl_list_all_gameweeks_markdown(self, session_store):
        """Test listing all gameweeks."""
        from src.tools.gameweeks import ListAllGameweeksInput, fpl_list_all_gameweeks

        # Arrange
        params = ListAllGameweeksInput()

        # Act
        result = await fpl_list_all_gameweeks(params)

        # Assert
        assert "Gameweek" in result
        # Should list multiple gameweeks

    async def test_fpl_list_all_gameweeks_json(self, session_store):
        """Test listing all gameweeks in JSON format."""
        from src.tools.gameweeks import ListAllGameweeksInput, fpl_list_all_gameweeks

        # Arrange
        params = ListAllGameweeksInput(response_format=ResponseFormat.JSON)

        # Act
        result = await fpl_list_all_gameweeks(params)

        # Assert
        assert '"gameweeks":' in result or '"events":' in result
...
class TestLeagueManagerToolsIntegration:
    ...
    async def test_fpl_get_manager_squad_success(self, session_store, mock_manager_picks):
        """Test getting manager squad."""
        from src.tools.leagues import GetManagerSquadInput, fpl_get_manager_squad

        # Arrange
        params = GetManagerSquadInput(team_id=12345, gameweek=2)
        ...
        # Act
        result = await fpl_get_manager_squad(params)

        # Assert
        assert "team" in result.lower() or "squad" in result.lower()

    async def test_fpl_get_manager_squad_current_gameweek(self, session_store, mock_manager_picks):
        """Test getting manager squad for current gameweek (no gameweek param)."""
        from src.tools.leagues import GetManagerSquadInput, fpl_get_manager_squad

        # Arrange
        params = GetManagerSquadInput(team_id=12345)
    ...
    async def test_fpl_get_manager_by_team_id_json_format(
        self, session_store, mock_manager_picks
    ):
