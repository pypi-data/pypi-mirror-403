...
class Playwright(DriverInterface):
    ...
    def __init__(self, config: dict, event_sdk: Optional[EventSDK] = None):
        ...
        self._is_remote_connection = False
    ...
    async def _launch_app_async(self, app_identifier, event_name):
        try:
            ...
            # Check if connecting to remote browser or launching locally
            if remote_url and remote_url.strip():
                # Connect to remote browser
                self._is_remote_connection = True
                remote_url = remote_url.strip()
                browser_type = getattr(self._pw, browser)
                
                # Determine connection type
                connection_type = self.config.get("connection_type", "").lower()
                if connection_type == "cdp" or (not connection_type and not self._is_websocket_url(remote_url)):
                    # Use CDP connection (http:// or https://)
                    internal_logger.debug("[Playwright] Connecting to remote browser via CDP: %s", remote_url)
                    try:
                        self._browser = await browser_type.connect_over_cdp(remote_url)
                    except Exception as e:
                        internal_logger.error("[Playwright] Failed to connect via CDP: %s", e)
                        raise OpticsError(
                            Code.E0102,
                            f"Failed to connect to remote browser via CDP at {remote_url}: {e}",
                            cause=e
                        )
                else:
                    # Use WebSocket connection (ws:// or wss://)
                    internal_logger.debug("[Playwright] Connecting to remote browser via WebSocket: %s", remote_url)
                    try:
                        self._browser = await browser_type.connect(remote_url)
                    except Exception as e:
                        internal_logger.error("[Playwright] Failed to connect via WebSocket: %s", e)
                        raise OpticsError(
                            Code.E0102,
                            f"Failed to connect to remote browser via WebSocket at {remote_url}: {e}",
                            cause=e
                        )
                
                # For remote connections, get existing contexts or create a new one
                ...
                if contexts:
                    ...
                    # Create new context for remote browser
                    self._context = await self._browser.new_context(viewport=viewport)
                    self.page = await self._context.new_page()
            else:
                # Launch browser locally (existing behavior)
                self._is_remote_connection = False
                internal_logger.debug("[Playwright] Launching browser locally")
            ...
            connection_mode = "remote" if self._is_remote_connection else "local"
            internal_logger.debug("[Playwright] Application launched (%s connection)", connection_mode)
    ...
    def _is_websocket_url(self, url: str) -> bool:
        """
        Check if the URL is a WebSocket endpoint.
        
        Args:
            url: The URL to check
            
        Returns:
            True if URL is a WebSocket endpoint (ws:// or wss://), False otherwise
        """
        if not url:
            return False
        url_lower = url.lower().strip()
        return url_lower.startswith("ws://") or url_lower.startswith("wss://")
        ...
        try:
            if self.page:
                try:
                    ...
                    internal_logger.debug(f"[Playwright] Error closing page during cleanup: {e}")
                finally:
                    self.page = None
            if self._context:
                try:
                    ...
                    internal_logger.debug(f"[Playwright] Error closing context during cleanup: {e}")
                finally:
                    self._context = None
            if self._browser:
                try:
                    ...
                    internal_logger.debug(f"[Playwright] Error closing browser during cleanup: {e}")
                finally:
                    self._browser = None
            # Only stop Playwright instance for local connections
            # Remote connections manage their own Playwright instance
            if self._pw and not self._is_remote_connection:
                try:
                    await self._pw.stop()
                except Exception as e:
                    internal_logger.debug(f"[Playwright] Error stopping Playwright during cleanup: {e}")
                finally:
                    self._pw = None
            elif self._pw and self._is_remote_connection:
                # For remote connections, just clear the reference
                # Don't stop the remote Playwright instance
                self._pw = None
    ...
    async def _terminate_async(self):
        ...
        try:
            ...
            # Stop Playwright (only for local connections)
            # Remote connections manage their own Playwright instance
            if self._pw and not self._is_remote_connection:
                try:
                    await self._pw.stop()
