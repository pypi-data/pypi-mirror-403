...
class TerminalRunner(BaseRunner):
    ...
    def _safe_console_print(
        ...
        try:
            ...
            logger.warning(
                f"{error_prefix} error, falling back to plain text: {display_error}"
            )

            # 如果没有提供fallback_content，尝试从renderable中提取
            if fallback_content is None:
                if hasattr(renderable, "renderable") and hasattr(
    ...
    def run(self, request: AgenticEditRequest) -> str:
        ...
        console.print(
            Panel(
                ...
                title=get_message("/agent/edit/objective"),
                border_style="blue",
            )
        )
        ...
        try:
            ...
            for event in event_stream:
                if isinstance(event, ConversationIdEvent):
                    console.print(
                        f"[dim]Conversation ID: {event.conversation_id}[/dim]"
                    )
                    continue
                if isinstance(event, TokenUsageEvent):
                    ...
                    model_info = (
                        llm_utils.get_model_info(model_name, self.args.product_mode)
                        or {}
                    )
                    input_price = (
                        model_info.get("input_price", 0.0) if model_info else 0.0
                    )
                    output_price = (
                    ...
                    # 添加日志记录
                    logger.info(
                        f"Token Usage: Model={model_name}, Input Tokens={last_meta.input_tokens_count}, Output Tokens={last_meta.generated_tokens_count}, Input Cost=${input_cost:.6f}, Output Cost=${output_cost:.6f}"
                    )
                    ...
                    if event.tokens_used > event.pruned_tokens_used:
                        console.print(
                            f"[dim]conversation tokens: {event.tokens_used} -> {event.pruned_tokens_used} (conversation round: {event.conversation_round})[/dim]"
                        )
                    else:
                        console.print(
                            f"[dim]conversation tokens: {event.tokens_used} (conversation round: {event.conversation_round})[/dim]"
                        )

                elif isinstance(event, LLMReasoningEvent):
                    # Display reasoning content only if enable_agentic_reasoning_content is True
                    # The <inner_thinking> tags are already added in agentic_edit.py
                    if self.args.enable_agentic_reasoning_content:
                    ...
                    # Use different border styles and icons for different tool types
                    if isinstance(event.tool, (TodoReadTool, TodoWriteTool)):
                        border_style = "cyan"
                        icon = "📝"
                    elif isinstance(event.tool, ReadImageTool):
                        border_style = "magenta"
                        icon = "🖼️"
                    elif isinstance(event.tool, ExecuteExternalToolTool):
                        border_style = "yellow"
                        icon = "🔧"
                    else:
                        border_style = "blue"
                    ...
                    result_title = get_tool_result_title(
                        event.tool_name, result.success
                    )
                    ...
                    # Special handling for ReadImageTool - display as markdown
                    if event.tool_name == "ReadImageTool":
                        # Use magenta border for image analysis results
                        border_style = "magenta" if result.success else "red"
                        title_icon = "🖼️" if result.success else "❌"
                        title = f"{title_icon} {result_title}"
                        ...
                        if result.content:
                            ...
                            self._safe_console_print(
                                ...
                                fallback_content=display_content,
                                error_prefix="Image analysis content display",
                            )
                        else:
                            status_message = f"[bold]Status:[/bold] {'Success' if result.success else 'Failure'}\n[bold]Message:[/bold] {result.message}"
                            fallback_message = f"Status: {'Success' if result.success else 'Failure'}\nMessage: {result.message}"
                            self._safe_console_print(
                                ...
                                fallback_content=fallback_message,
                                error_prefix="Image analysis message display",
                            )
                        continue  # Skip the rest of the processing for ReadImageTool

                    # Special handling for ExecuteExternalToolTool
                    if event.tool_name == "ExecuteExternalToolTool":
                        # Use yellow border for external tool results
                        border_style = "yellow" if result.success else "red"
                        title_icon = "🔧" if result.success else "❌"
                        title = f"{title_icon} {result_title}"
                        ...
                        if result.content:
                            ...
                            if isinstance(content_data, dict):
                                ...
                                error_type = content_data.get("error_type", "")
                                ...
                                if error_type:
                                    display_parts.append(
                                        f"[bold]Error Type:[/bold] {error_type}"
                                    )
                                if error:
                                    display_parts.append(f"[bold]Error:[/bold] {error}")
                                if output_text:
                                    # Truncate long output
                                    if len(output_text) > 1000:
                                        output_display = f"{output_text[:500]}\n\n[dim]... (truncated, {len(output_text)} chars total) ...[/dim]\n\n{output_text[-300:]}"
                                    else:
                                ...
                                display_parts.append(str(content_data))

                        # Always show message if available and not already covered by content
                        if result.message and not result.success:
                            # For failures, show the message which contains detailed error info
                            if not display_parts or not any(
                                "Output:" in p for p in display_parts
                            ):
                                # Truncate long message
                                message_text = str(result.message)
                                if len(message_text) > 1500:
                                    message_text = f"{message_text[:1200]}\n\n[dim]... (truncated) ...[/dim]\n\n{message_text[-200:]}"
                                display_parts.append(
                                    f"[bold]Message:[/bold]\n{message_text}"
                                )
                        ...
                        if display_parts:
                            ...
                            display_content = f"[bold]Status:[/bold] {'Success' if result.success else 'Failure'}"
                        ...
                        self._safe_console_print(
                            ...
                            fallback_content=display_content,
                            error_prefix="External tool content display",
                        )
                        continue  # Skip the rest of the processing for ExecuteExternalToolTool
                    ...
                    if result.content is not None:
                        ...
                        try:
                            ...
                            if isinstance(result.content, str) and has_hint_in_text(
                                result.content
                            ):
                                processed_content = extract_content_from_text(
                                    result.content
                                )
                            ...
                            if isinstance(processed_content, (dict, list)):
                                ...
                                if event.tool_name == "ReadFileTool" and isinstance(
                                    event.result.message, str
                                ):
                                ...
                                syntax_content = Syntax(
                                    _format_content(processed_content),
                                    lexer,
                                    theme="default",
                                    line_numbers=True,
                                )
                    ...
                    self._safe_console_print(
                        console,
                        Panel(
                            Markdown(event.completion.response),
