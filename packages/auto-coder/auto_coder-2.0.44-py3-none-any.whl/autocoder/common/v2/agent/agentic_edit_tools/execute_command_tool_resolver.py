...
class ExecuteCommandToolResolver(BaseToolResolver):
    ...
    def _parse_env(self, env_str: Optional[str]) -> Optional[Dict[str, str]]:
        ...
        Args:
            env_str: JSON 格式的环境变量字符串，如 '{"KEY": "value"}'

        Returns:
            Optional[Dict[str, str]]: 解析后的环境变量字典，如果解析失败返回 None
        """
        if not env_str:
            return None
        try:
    ...
    def _get_external_tool_names(self) -> Set[str]:
        ...
        Returns:
            Set[str]: 外部工具名称集合（包含带扩展名和不带扩展名的版本）
        """
    ...
    def _check_external_tool_command(self, command: str) -> Tuple[bool, Optional[str]]:
        ...
        Returns:
            Tuple[bool, Optional[str]]: (是否是外部工具, 工具名称)
        """
        if not command:
            return False, None
    ...
    def _build_external_tool_redirect_result(
        ...
        """
        构建引导用户使用 execute_external_tool 的错误结果
        ...
        error_message = f"""⚠️ EXTERNAL TOOL DETECTED - Please use `execute_external_tool` instead
   ...
   <query>your search term</query>
    ...
    def _execute_background_command(
        ...
        try:
            ...
            # 转换 start_time 为字符串格式 (从 float timestamp 转换为 ISO 格式)
    ...
    def _execute_batch_commands(
        ...
        for cmd in cmd_list:
            if isinstance(cmd, str):
                ...
                # 如果是字典类型，很可能是 YAML 解析问题
                if isinstance(cmd, dict):
                    error_msg += (
                        "⚠️  YAML Parsing Issue Detected:\n"
                        "Commands containing colons (:) are being parsed as dictionaries.\n\n"
                        "🔧 Solution - Wrap commands with quotes:\n"
                        "  cmds:\n"
        ...
        if external_tools_found:
            # 构建错误信息，列出所有检测到的外部工具
            error_parts = [
                "⚠️ EXTERNAL TOOLS DETECTED IN BATCH COMMANDS",
                "",
    ...
    def resolve(self) -> ToolResult:
        ...
        # 检查是否是外部工具命令，如果是则引导使用 execute_external_tool
