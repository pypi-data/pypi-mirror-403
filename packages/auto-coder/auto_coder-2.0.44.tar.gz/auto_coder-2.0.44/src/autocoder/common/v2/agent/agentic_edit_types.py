...
class ExecuteCommandTool(BaseTool):
    ...
    env: Optional[str] = None  # JSON 格式环境变量，如 '{"KEY": "value"}'
...
class ReadImageTool(BaseTool):
    ...
    支持读取本地图片文件并使用 vl_model 进行图片内容理解和分析。
    多个图片路径可用逗号分割。

    例如：
    <read_image>
        <path>./images/screenshot.png</path>
        <query>请描述这张图片的内容</query>
    </read_image>
    """

    path: str  # 图片路径，多个路径用逗号分割
    query: str  # 关于图片的查询问题


class ExecuteExternalToolTool(BaseTool):
    """执行外部工具命令，自动补充敏感信息

    从 .autocodertools 目录加载的外部工具可能需要账号、密码或 API key 等敏感信息。
    此工具会自动从工具所在目录读取 `工具名.key` 文件，并将其中的参数自动添加到命令行中。

    .key 文件格式（YAML），支持多层级子命令:
    ```yaml
    # 全局参数（对所有子命令生效）
    global:
      api_key: "sk-xxxx"
    ...
    # 多层级子命令参数（使用 _params 存储当前层级参数）
    subcommands:
      config:
        _params:              # config 子命令的参数
          verbose: "true"
        set:
          _params:            # config,set 子命令的参数
            format: "json"
          proxy:
            _params:          # config,set,proxy 子命令的参数
              timeout: "30"
      upload:
        _params:
          token: "yyy"
    ...
    参数自动添加规则：
    1. global 中的参数会添加到所有命令中
    2. 沿子命令路径收集所有层级的 _params 参数
    3. 参数格式: --key value 或 --key=value（取决于 param_style 配置）
    ...
    tool_name: str  # 外部工具名称（从 .autocodertools 加载的工具）
    subcommands: Optional[str] = (
        None  # 子命令名称，逗号分隔支持多层级，如 "config,set,proxy"
    )
    args: Optional[str] = None  # 额外的命令行参数
    timeout: Optional[int] = 60  # 超时时间（秒），默认60秒
    background: Optional[bool] = False  # 是否后台运行，默认为False
    requires_approval: bool = True  # 是否需要用户确认，默认需要
...
TOOL_MODEL_MAP: Dict[str, Type[BaseTool]] = {
    ...
    "execute_external_tool": ExecuteExternalToolTool,
}
