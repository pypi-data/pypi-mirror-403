...
class WriteToFileToolResolver(LinterEnabledToolResolver):
    def __init__(
        self, agent: Optional["AgenticEdit"], tool: WriteToFileTool, args: AutoCoderArgs
    ):
        super().__init__(agent, tool, args)
        self.tool: WriteToFileTool = tool  # For type hinting
        self.args = args

    def write_file_normal(
        self,
        file_path: str,
        content: str,
        source_dir: str,
        ...
        try:
            ...
            if self.agent:
                ...
                self.agent.record_file_change(
                    rel_path, change_type, diff=None, content=new_content_to_write
                )
            ...
            action_text = (
                "appended to" if (mode == "append" and not is_new) else "wrote to"
            )
            ...
            return ToolResult(
                success=False,
                message=f"An error occurred while writing to the file: {str(e)}",
            )
    ...
    def resolve(self) -> ToolResult:
        ...
        if (
            not self.args.allow_files_outside_workspace
            and not abs_file_path.startswith(abs_project_dir)
        ):
            return ToolResult(
                success=False,
                message=f"Error: Access denied. Attempted to write file outside the project directory: {file_path}",
            )

        return self.write_file_normal(
            file_path,
            content,
            source_dir,
            abs_project_dir,
