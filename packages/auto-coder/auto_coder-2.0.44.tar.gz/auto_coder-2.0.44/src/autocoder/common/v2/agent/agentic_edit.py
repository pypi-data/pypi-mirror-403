...
class AgenticEdit:
    ...
    def _analyze(self, request: AgenticEditRequest) -> str:
         ...
         **WARNING - Do NOT use `&` or `nohup` for background processes**: Using `&` in the command string does NOT work properly. The subprocess mechanism waits for the entire shell process to finish, and although `&` puts the command in the shell's background internally, the process's stdout/stderr are still connected to the parent shell, causing subprocess to continue waiting. Always use `<background>true</background>` parameter instead.
         **Sub Agent Execution**: You can also delegate tasks to sub-agents using pipe syntax like `echo 'task description' | auto-coder.run --model {{ current_model }} --is-sub-agent` or `cat task_file.txt | auto-coder.run --model {{ current_model }} --is-sub-agent`. The --is-sub-agent flag indicates this agent is called by another agent rather than directly by a human.
         ...
         - env: (optional) Environment variables for the command execution in JSON format. Example: {"API_KEY": "xxx", "DEBUG": "true"}
         ...
         ## read_image
         Description: Analyze image content using a vision model (vl_model). Supports multiple image formats.
         Parameters:
         - path: (required) Image file path. For multiple images, use comma-separated list: "a.png, b.png"
         - query: (required) The question or description of what you want to understand about the image(s)

         Supported formats: JPEG/JPG, PNG, GIF, WebP, BMP

         Usage:
         <read_image>
         <path>./screenshots/error.png</path>
         <query>What error is shown in this screenshot?</query>
         </read_image>

         Multiple images:
         <read_image>
         <path>./design/before.png, ./design/after.png</path>
         <query>Compare these two UI designs and describe the differences</query>
         </read_image>

         Note: Requires vl_model to be configured in args to use this tool.
        ...
        Description: Execute an external tool loaded from .autocodertools directories. External tools may require sensitive information such as accounts, passwords, or API keys. This tool automatically reads the `tool_name.key` file from the tool's directory and adds the parameters to the command line automatically.

        The .key file format (YAML, supports multi-level subcommands):
        ```yaml
        # Global parameters (apply to all subcommands)
        global:
          api_key: "sk-xxxx"
          username: "user"
        ...
        param_style: "space"      # "space" for --key value, "equals" for --key=value
        param_prefix: "--"        # Parameter prefix, default is "--"
        ```

        Parameter auto-addition rules:
        1. Parameters in `global` are added to all commands
        2. Parameters are collected along the subcommand path (all `_params` from each level)
        3. Parameter format: --key value or --key=value (depending on param_style configuration)

        Parameters:
        - tool_name: (required) The name of the external tool (loaded from .autocodertools)
        - subcommands: (optional) Comma-separated subcommand names for multi-level, e.g., "config,set,proxy"
        - args: (optional) Additional command line arguments
        - timeout: (optional) Timeout in seconds. Default is 60.
        - background: (optional) Whether to run in background. Default is false.
        - requires_approval: (optional) Whether user confirmation is needed. Default is true.

        Usage:
        <execute_external_tool>
        <tool_name>my_cli_tool</tool_name>
        <subcommands>config,set,proxy</subcommands>
        <args>--url http://example.com</args>
        <timeout>120</timeout>
        <background>false</background>
         ...
         **1. Web Page to HTML**: Generate complete HTML from webpage screenshots
         <read_image>
         <path>./webpage.png</path>
         <query>Generate a complete HTML file replicating this design with inline CSS and JavaScript</query>
         </read_image>

         **2. OCR & Format Conversion**: Extract text or convert to JSON/Markdown/CSV
         <read_image>
         <path>./table.png</path>
         <query>Extract this table as JSON</query>
         </read_image>

         **3. Multi-Round Analysis**: For complex images, query multiple times for details
         - Round 1: Overall layout and structure
         - Round 2: Colors, fonts, and styling
         - Round 3: Specific component details
