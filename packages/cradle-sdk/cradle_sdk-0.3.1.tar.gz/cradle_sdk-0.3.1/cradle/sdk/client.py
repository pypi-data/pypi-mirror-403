...
from cradle.sdk.types.workspace import (
    CustomPredictorContainerCreate,
    CustomPredictorContainerCreateResponse,
    CustomPredictorContainerUpdate,
    CustomPredictorCreateResponse,
    CustomPredictorResponse,
    ListCustomPredictorResponse,
    ListProjectResponse,
    ListRoundResponse,
    ProjectCreate,
    ProjectResponse,
...
class Client:
    ...
    @property
    def custom_predictor(self) -> "CustomPredictorClient":
        return CustomPredictorClient(self._client)
...
class CustomPredictorClient:
    """Custom predictors can be used instead of or in addition to predictors trained on the Cradle platform.

    A custom predictor is a function that, given a sequence, provides one or more scores for that sequence.
    These APIs allow to upload a Docker image that contains a custom predictor to the Cradle platform.

    If you would like to use custom predictors, please get in touch with your customer success representative.
    """

    def __init__(self, client: HttpClient):
        self._client = client.with_prefix("/customPredictor")

    def get_current(self, name: str) -> CustomPredictorResponse:
        """Return the custom predictor with the most recent version"""
        return self._client.get(":get", CustomPredictorResponse, params={"name": name})

    def get_version(self, version_id: int) -> CustomPredictorResponse:
        """Return the custom predictor with the given version ID"""
        return self._client.get(":getVersion", CustomPredictorResponse, params={"version_id": version_id})

    def list(self) -> Generator[CustomPredictorResponse, None, None]:
        """List the active and being-provisioned versions of every predictor in the workspace."""
        opts = ListOptions()
        while True:
            resp = self._client.get(":list", ListCustomPredictorResponse, params=opts.model_dump())
            yield from resp.items
            if resp.cursor is None:
                return
            opts.cursor = resp.cursor
    ...
    def create(
        self, cp: CustomPredictorContainerCreate, container_image: Path, upload_timeout: float = 300
    ) -> CustomPredictorContainerCreateResponse:
        """Create a new custom predictor"""
        with container_image.open("rb") as f:
            predictor = self._client.post(":create", CustomPredictorCreateResponse, json=cp)

            response = self._client.http_client.put(
                predictor.root.upload_url,
                headers=predictor.root.upload_headers,
                content=f,
    ...
    def update(
        self, name: str, cp: CustomPredictorContainerUpdate, container_image: Path, upload_timeout: float = 300
    ) -> CustomPredictorContainerCreateResponse:
        """Create a new version for a custom predictor.

        The response contains a signed URL that should be used to upload the
        container image for the predictor. The version will not be considered
        active (usable) until the container image has been provided.
        """
        with container_image.open("rb") as f:
            predictor = self._client.post(":update", CustomPredictorCreateResponse, params={"name": name}, json=cp)
    ...
    def delete(self, name: str) -> None:
        """Delete the custom predictor with the given name"""
        self._client.post(":delete", response_type=None, params={"name": name})
