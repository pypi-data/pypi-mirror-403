...
class MonomerHomologsData(BaseModel):
    kind: Literal["MONOMER"] = Field(default="MONOMER")
    homologs: (
        list | Annotated[TableSourceTable | TableSourceQuery | TableSourceTaskResult, Field(discriminator="kind")]
    ) = Field(
        description="Homologous sequences used for training. These sequences should be evolutionarily related to the target protein family."
    )
...
class MonomerSamplerConditioningData(BaseModel):
    kind: Literal["MONOMER"] = Field(default="MONOMER")
    train_dataset: (
        list | Annotated[TableSourceTable | TableSourceQuery | TableSourceTaskResult, Field(discriminator="kind")]
    ) = Field(description="The training dataset containing assayed assemblies.")
    val_dataset: (
        list
        | Annotated[TableSourceTable | TableSourceQuery | TableSourceTaskResult, Field(discriminator="kind")]
        | None
        ...
        default=None,
        description="The validation dataset containing assayed assemblies. If not set, the training dataset will be split to create a validation dataset.",
    )
    sampler: ArtifactParam = Field(description="The pre-trained sampler model to condition.")


class MonomerSamplerConditioningResult(BaseModel):
    kind: Literal["MONOMER"] = Field(default="MONOMER")
    conditioned_sampler: ArtifactParam = Field(description="The conditioned sampler model.")
...
class MonomerTrainBaseModelResult(BaseModel):
    kind: Literal["MONOMER"] = Field(default="MONOMER")
    sampler: ArtifactParam = Field(description="The trained sampler model that can be used for sequence generation.")
...
class SamplerConditioningParameters(ParametersBase):
    task_type: ClassVar[str] = "condition_sampler/v2"

    data: MonomerSamplerConditioningData | VhVlSamplerConditioningData = Field(
        description="The training and validation data along with the pre-trained sampler model.", discriminator="kind"
    )
    primary_objective: Measure = Field(
        description="The assay on which to condition the sampler model. Must correspond to an assay present in the datasets."
    )


class SamplerConditioningResult(ResultBase):
    models: MonomerSamplerConditioningResult | VhVlSamplerConditioningResult = Field(
        description="The conditioned sampler.", discriminator="kind"
    )
...
class TrainBaseModelParameters(ParametersBase):
    task_type: ClassVar[str] = "train_base_model/v2"

    data: MonomerHomologsData | VhVlHomologsData = Field(
        description="Homologous sequences for training the base model. The model learns the sequence distribution of the provided protein family.",
        discriminator="kind",
    )


class TrainBaseModelResult(ResultBase):
    models: MonomerTrainBaseModelResult | VhVlTrainBaseModelResult = Field(
        description="The trained sampler model.", discriminator="kind"
    )
    report: ArtifactParam | None = Field(default=None, description="Training report with metrics and diagnostics.")
...
class VhVlHomologsData(BaseModel):
    kind: Literal["VHVL"] = Field(default="VHVL")
    homologs: (
        list | Annotated[TableSourceTable | TableSourceQuery | TableSourceTaskResult, Field(discriminator="kind")]
    ) = Field(
        description="Homologous antibody sequences used for training. These sequences should be evolutionarily related to the target antibody family."
    )
...
class VhVlSamplerConditioningData(BaseModel):
    kind: Literal["VH_VL"] = Field(default="VH_VL")
    train_dataset: (
        list | Annotated[TableSourceTable | TableSourceQuery | TableSourceTaskResult, Field(discriminator="kind")]
    ) = Field(description="The training dataset containing assayed assemblies.")
...
class VhVlSamplerConditioningResult(BaseModel):
    kind: Literal["VH_VL"] = Field(default="VH_VL")
    conditioned_sampler: ArtifactParam = Field(description="The conditioned sampler model.")
...
class VhVlTrainBaseModelResult(BaseModel):
    kind: Literal["VHVL"] = Field(default="VHVL")
    sampler: ArtifactParam = Field(
        description="The trained sampler model that can be used for antibody sequence generation."
    )
