/**
 * DeepSeek Provider Example for SecureShell TypeScript
 * 
 * Demonstrates a DeepSeek Agent (OpenAI-compatible) using SecureShell.
 * 
 * Usage:
 *   export DEEPSEEK_API_KEY=sk-...
 *   npx tsx examples/secureshell-ts/providers/deepseek.ts
 */

import 'dotenv/config';
import OpenAI from 'openai';
import { SecureShell, OpenAITools, DeepSeekProvider } from '@secureshell/ts';

async function main() {
    console.log('🤖 DeepSeek Agent + SecureShell Tools Demo\n');
    ...
    // 2. Initialize DeepSeek Client (via OpenAI SDK)
    ...
    const messages: OpenAI.Chat.Completions.ChatCompletionMessageParam[] = [
        { role: 'system', content: 'You are a helpful assistant. Use shell commands when needed.' },
        { role: 'user', content: 'List files in current directory and show me package.json' }
    ];
