# TheFuck-AI [![Version][version-badge]][version-link] [![Build Status][workflow-badge]][workflow-link] [![Coverage][coverage-badge]][coverage-link] [![MIT License][license-badge]](LICENSE.md)

*TheFuck-AI* is an AI-enhanced CLI tool that not only corrects errors in previous console commands, but also **generates commands from natural language**.

Based on the original [thefuck](https://github.com/nvbn/thefuck) project, with powerful AI capabilities added.

## ✨ New: AI Command Generation

Use natural language to generate shell commands:

```bash
➜ 列出当前目录所有文件
zsh: command not found: 列出当前目录所有文件
...
```bash
➜ 找到所有大于100MB的文件
zsh: command not found: 找到所有大于100MB的文件

➜ ai
[AI] 执行: find . -size +100M -type f
```

```bash
➜ 把当前目录压缩成zip
zsh: command not found: 把当前目录压缩成zip

➜ ai
[AI] 执行: zip -r archive.zip .
```
...
Is it too slow? [Try the experimental instant mode!](#experimental-instant-mode)
...
On other systems, install *TheFuck-AI* by using `pip`:

```bash
pip install thefuck-ai
```
...
After installation, run the setup wizard to configure AI features:
...
1. **Choose AI provider** - DeepSeek (default) or custom OpenAI-compatible API
2. **Enter API Key** - The wizard will validate your key automatically
3. **Save configuration** - Settings are saved to `~/.config/thefuck/settings.py`

### Manual Configuration

You can also configure AI settings manually in `~/.config/thefuck/settings.py`:

```python
# AI API Key (required)
ai_api_key = 'your-api-key-here'

# API endpoint (optional, default: DeepSeek)
...
```bash
export THEFUCK_AI_API_KEY='your-api-key-here'
```

### Usage

After a command error, simply type `ai` to let AI generate the correct command:
