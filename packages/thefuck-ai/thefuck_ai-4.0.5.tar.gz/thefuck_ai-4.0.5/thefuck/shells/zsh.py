...
class Zsh(Generic):
    ...
    def ai_alias(self, alias_name='ai'):
        """Generate AI command alias for zsh.

        This alias allows users to run 'ai <request>' to generate
        commands using AI. Handles risk levels: SAFE/LOW auto-execute,
        ...
        return '''
            {name} () {{
                ...
                export TF_HISTORY;
                export PYTHONIOENCODING=utf-8;
                TF_RESULT=$(thefuck --ai $@);
                if [[ -n "$TF_RESULT" ]]; then
                    TF_RISK="${{TF_RESULT%%:*}}";
                    TF_CMD="${{TF_RESULT#*:}}";
                    if [[ "$TF_RISK" == "SAFE" ]] || [[ "$TF_RISK" == "LOW" ]]; then
                        echo "[AI] 执行: $TF_CMD";
                        eval $TF_CMD;
                        {alter_history}
                    elif [[ "$TF_RISK" == "MEDIUM" ]]; then
                        echo "[AI] 建议执行: $TF_CMD";
                        read "confirm?[警告] 此命令需要确认，是否执行？[y/N] ";
