...
class Destination:
    ...
    def _validate_configuration(self, config: dict) -> None:
        ...
        """
        proxy_type = self._config.get("destinationConfiguration", {}).get("ProxyType")
        auth_type = self._config.get("destinationConfiguration", {}).get(
            "Authentication"
        )
    ...
    async def _get_configuration(self) -> dict:
        ...
        """
        config = await self.instance.get_configuration(self.name)
    ...
    async def _build_parameters(self, jwt: str) -> None:
        ...
        """
        if (
            self.auth_type != AuthType.OAUTH2_CLIENT_CREDENTIALS.value
            and self.config.parameters is not None
        ):
            return self.config.parameters
        elif (
            self.auth_type == AuthType.OAUTH2_CLIENT_CREDENTIALS.value
            and self.config.expires_in is not None
            and self.config.expires_in > datetime.now()
        ):
            return self.config.parameters
        else:
            ...
            if (
                ...
                self.config["expires_in"] = datetime.now() + timedelta(
                    seconds=config["authTokens"][0].get(
    ...
    def get_client(self, jwt: str = None) -> httpx.Client:
        ...
        return httpx.Client(**self.config["parameters"])
    ...
    async def get_aclient(self, jwt: str = None) -> httpx.AsyncClient:
        ...
        return httpx.AsyncClient(**self.config["parameters"])
