...
from PyQt5.QtWidgets import QFileDialog, QHBoxLayout, QGridLayout, QLabel, QComboBox, QCheckBox, QSpinBox
from PyQt5.QtCore import Qt, pyqtSignal
import os
from PyQt5 import QtWidgets, QtCore
from PyQt5.QtWidgets import QFileDialog, QHBoxLayout, QVBoxLayout, QGridLayout, QLabel, QComboBox, QCheckBox, QSpinBox
from PyQt5.QtCore import Qt, pyqtSignal

import os
import re
from PyQt5 import QtWidgets, QtCore
from PyQt5.QtWidgets import QFileDialog, QHBoxLayout, QLabel, QVBoxLayout, QCheckBox
from PyQt5.QtCore import Qt
from PyQt5.QtGui import QPixmap, QImage
...
from PyQt5.QtWidgets import (QFileDialog, QVBoxLayout, QHBoxLayout, QLabel,
                             QCheckBox, QSpinBox, QGridLayout, QComboBox)
from PyQt5.QtCore import Qt, pyqtSignal
import os
import re
from PyQt5 import QtWidgets, QtCore
...
class QFileSaveDialogPreview(QFileDialog):
    ...
    def _update_ui_visibility(self, path_or_filter):
        """
        Determines which UI elements to show based on the selected file/filter.
        """
        # Convert input to string and lower case
        path_str = str(path_or_filter).lower()

        # Combine input with any currently selected files to be thorough
        current_files = self.selectedFiles()
        if current_files:
            path_str += " " + current_files[0].lower()

        # 1. Check Types
        is_video = any(ext in path_str for ext in self.ext_video) or "video" in path_str or "movie" in path_str
        is_nifti = any(ext in path_str for ext in self.ext_nifti) or "nii" in path_str
        is_photo = any(ext in path_str for ext in self.ext_photo)
        ...
        # Photo -> Segmented Only
        self.checkBox_seg_only.setVisible(is_photo)

        # Force the check ON if it is visible (as requested)
        if is_photo:
