...
class TestPDFAttentionPaper:
    ...
    def test_get_base64_pdf(self, doc):
        """Test PDF to base64 conversion."""
        import base64
        result = doc.get_base64(page_index=0, dpi=72)
    ...
    def test_get_base64_pdf_different_page(self, doc):
        """Test getting base64 for different pages."""
        import base64
        page0 = doc.get_base64(page_index=0, dpi=72, mime_prefix=False)
        page1 = doc.get_base64(page_index=1, dpi=72, mime_prefix=False)
        # Different pages should produce different output
        assert page0 != page1

    def test_to_chat_content_pdf(self, doc):
        """Test PDF to chat content format."""
        result = doc.to_chat_content(page_index=0, dpi=72)
...
class TestImage:
    ...
    def test_get_base64_image(self, doc):
        """Test image to base64 conversion."""
        import base64
        result = doc.get_base64()
    ...
    def test_to_chat_content_image(self, doc):
        """Test image to chat content format."""
        result = doc.to_chat_content()
        assert result['type'] == 'image_url'
        assert result['image_url']['url'].startswith('data:image/jpeg;base64,')

    def test_get_base64_with_resize(self, doc):
        """Test image resize for base64."""
        try:
        ...
        import base64
        # Original is 1600x900, resize to max 100x100
        result = doc.get_base64(max_width=100, max_height=100, mime_prefix=False)
