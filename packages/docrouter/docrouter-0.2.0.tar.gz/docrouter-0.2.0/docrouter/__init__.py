...
# === Utilities ===
def _resize_image(image_bytes: bytes, max_size: tuple[int, int], output_format: str = 'PNG') -> bytes:
    """Resize image to fit within max_size while preserving aspect ratio."""
    try:
        from PIL import Image
    except ImportError:
        raise ImportError("Pillow required for image resizing: pip install Pillow")
...
class DocumentHandle:
    ...
    def get_base64(
        self,
        *,
        mime_prefix: bool = True,
        page_index: int = 0,
        dpi: int = 150,
        max_size: tuple[int, int] | None = None
    ) -> str:
        """
        Get base64-encoded image data for multimodal LLM APIs.

        For images: encodes the original image bytes.
        For PDFs: renders the specified page as PNG.
        Other formats raise UnsupportedOperationError.

        Args:
            mime_prefix: If True, returns "data:{mime};base64,{data}". If False, returns raw base64.
            page_index: For PDFs, which page to render (default: 0).
            dpi: For PDFs, rendering resolution (default: 150).
            max_size: Optional (width, height) tuple to resize/thumbnail the image.

        Returns:
            Base64-encoded string, optionally with data URI prefix.
        """
        if self._doc.fmt == 'image':
            image_bytes = self._raw
            ext = self._path.suffix.lower()
            mime_type = _IMAGE_MIMES.get(ext, 'image/png')
        elif self._doc.fmt == 'pdf':
            image_bytes = self._doc.render(page_index, dpi)
            mime_type = 'image/png'
        else:
            raise UnsupportedOperationError(
                f"get_base64 only supported for images and PDFs, not {self._doc.fmt}"
            )

        # Resize if requested
        if max_size is not None:
            output_format = 'PNG' if mime_type == 'image/png' else mime_type.split('/')[-1].upper()
            if output_format == 'JPEG':
                output_format = 'JPEG'
            image_bytes = _resize_image(image_bytes, max_size, output_format)
    ...
    def to_chat_content(
        ...
        *,
        ...
        """
        Get image content in OpenAI/Anthropic chat API format.

        Returns a dict suitable for use in multimodal chat messages:
        ...
        Returns:
            Dict in OpenAI/Anthropic-compatible image format.
        """
        data_uri = self.get_base64(
            mime_prefix=True,
            page_index=page_index,
            dpi=dpi,
            max_size=max_size
