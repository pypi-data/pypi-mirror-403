...
# RB 2022: included explicit in jscatter from original source
# changes to get again toctree with maxdepth
...
def html_page_context(app, pagename, templatename, context, doctree):
    ...
    Modifies the context directly.
     ...
     - Replaces the 'toctree' function with one that uses the entire
       document structure, and uses only prune and collapse.
...
def get_rendered_toctree(builder, docname, prune=True, collapse=True, maxdeepth=2):
...
def build_full_toctree(builder, docname, prune, collapse, maxdeepth):
    ...
    for toctreenode in doctree.traverse(addnodes.toctree):
        # RB: To limit the depth of the toctree we need prune=True and maxdepth
        toctree = env.resolve_toctree(docname, builder, toctreenode, collapse=collapse,
                                      prune=prune, maxdepth=maxdeepth)
