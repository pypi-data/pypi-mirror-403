...
class NASBench201Wrapper(BenchmarkInterface):
    ...
    def evaluate(self, arch: Dict) -> BenchmarkResult:
        ...
        # Deterministic RNG based on architecture
        rng = np.random.RandomState(hash(arch['arch_str']) % 2**32)
        
        acc = 0.70 + (complexity / 20.0) + rng.normal(0, 0.02)
        acc = np.clip(acc, 0.1, 0.96)
        train_time = complexity * 5.0
