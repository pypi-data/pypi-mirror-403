function Z = Linkage_JP(Y, method, pdistArg, varargin)
...
% Modified by Jesus Perez-Ortega, Sep 2023 (lines commented 184:186)
...
if nargin > 2
    pdistArg = convertStringsToChars(pdistArg);
end
...
if nargs>=3 % should be data input
    ...
    if ischar(pdistArg) || isa(pdistArg,'function_handle')
        pdistArg = {pdistArg};
...
if any(strcmp(method,{'ce' 'me' 'wa'}))
    ...
    if ~passX % The distance matrix is passed
        % Commented lines because iseuclidean(Y) takes too much time!
...
% The memory efficient option can be valid only when the following three
...
if passX
    if (memEff)
        ...
        Z = internal.stats.linkagemex(Y',method,{'euc'},memEff); %note that Y is transposed here
    else
        Z = internal.stats.linkagemex(Y,method,pdistArg, memEff);
    end
else
    Z = internal.stats.linkagemex(Y,method);
end
...
if any(zdiff<0)
    ...
    if any(abs(zdiff(negLocs)) > eps(Z(negLocs,3))*10) % eps(the larger of the two values)
