...
from ..rest_api import RetryStrategy  # noqa: F401
from ..rest_api import TGUDFStorage  # noqa: F401
from ..rest_api import models  # noqa: F401
from ..sql import _execution as _sql_exec
from ..taskgraphs import _results as _tg_results
from ..taskgraphs.registration import TaskGraphRegistrar
...
class DAGError(tiledb.TileDBError):
...
class Node(futures.FutureLike[_T]):
    ...
    *args : sequence
    ...
    mode : Mode, optional
        Mode the Node is to run in. Default: realtime.
    resource_class : str, optional
        Name of a UDF resource class.
    resources : dict, optional
        A UDF resource description.
    storage : list of TGUDFStorage, optional
        Storage and filesystem mount points for the Node. A realtime
        mode Node does not support storage.
    expand_node_output : Node, optional
        Node to expand processes upon.
    _download_results : bool, optional
        An optional boolean to override default result-downloading
        behavior. If True, will always download the results of the
        function immediately upon completion.  If False, will not
        download the results of the function immediately, but will be
        downloaded when ``.result()`` is called.
    _internal_prewrapped_func : callable, optional
        For internal use only. A function that returns.  something that
        is already a Result, which does not require wrapping.  We assume
        that all prewrapped functions make server calls.
    _internal_accepts_stored_params : bool, optional
        For internal use only.  Applies only when ``_prewrapped_func``
        is used. ``True`` if ``_prewrapped_func`` can accept stored
        parameters. ``False`` if it cannot, and all parameters must be
        serialized.
    **kwargs : dict
    ...
    ------
    TypeError
        When ``func`` and ``storage`` are invalid or incompatible with
        the Node's mode.
    ValueError
        When resources and/or resource class are invalid.
    ...
    dag : DAG
        The DAG that the Node is attached to.
    mode : Mode
        The Node's mode (realtime or batch).
    parents : dict
        A mapping of parent node ids to Node instances.
    children : dict
        A mapping of child node ids to Node instances.
    args : tuple
        Positional arguments for the Node's function.
    kwargs : dict
        Keyword arguments for the Node's function.
    ...
    def __init__(
        ...
        *args: Any,
        ...
        mode: Optional[Mode] = Mode.REALTIME,
        ...
        _internal_prewrapped_func: Optional[Callable[..., "results.Result[_T]"]] = None,
        _internal_accepts_stored_params: Optional[bool] = True,
        **kwargs: Any,
    ) -> None:
        ...
        if storage and self.mode == Mode.REALTIME:
            raise TypeError("Realtime nodes do not support storage.")
        else:
...
class DAG:
    ...
    use_processes : bool, optional
        If True the DAG will use processes instead of threads.  Default:
        False.
    done_callback : callable, optional
        Callback function to register for when dag is completed.
        Function will be passed reference to this DAG.
    update_callback : callable, optional
        Callback function to register for when dag status is updated.
        Function will be passed reference to this DAG.
    name : str, optional
        Human-readable name for DAG to be shown in task graph logs.
    retry_strategy : RetryStrategy, optional
        K8S retry policy to be applied to each Node.
    workflow_retry_strategy : RetryStrategy, optional
    ...
    completed_nodes : dict
        Mapping of completed nodes.
    failed_nodes : dict
        Mapping of failed nodes.
    running_nodes : dict
        Mapping of running nodes.
    not_started_nodes : dict
        Mapping of queued nodes.
    cancelled_nodes : dict
        Mapping of cancelled nodes.
    name : str
        Human-readable name for DAG to be shown in task graph logs.
    server_graph_uuid : UUID
        The server-generated UUID of this graph, used for logging.  Will
        be ``None`` until :meth:`initial_setup` is called. If submitting
    ...
    def __init__(
        ...
        *,
        ...
        workflow_retry_strategy: Optional[RetryStrategy] = None,
        ...
        self.workspace = workspace or client.get_workspace_id()
    ...
    def _add_prewrapped_node(
        ...
        **kwargs,
        ...
        with self._lifecycle_condition:
            ...
            if self.mode == Mode.BATCH:
                if mode and mode != Mode.BATCH:
                    raise ValueError(
                        "BATCH mode DAG can only execute BATCH mode Nodes."
                    )
                node_mode = Mode.BATCH
            elif local_mode:
                # Check for deprecated local_mode parameter
                node_mode = Mode.LOCAL
            else:
                node_mode = Mode.REALTIME
    ...
    def submit_array_udf(
        ...
        **kwargs: Any,
        ...
        _download_results : bool, optional
            ...
            behavior. If True, will always download the results of the
            function immediately upon completion.  If False, will not
            download the results of the function immediately, but will
            be downloaded when ``.result()`` is called.
        _internal_prewrapped_func : callable, optional
            For internal use only. A function that returns.  something
            that is already a Result, which does not require wrapping.
            We assume that all prewrapped functions make server calls.
        _internal_accepts_stored_params : bool, optional
            For internal use only.  Applies only when
            ``_prewrapped_func`` is used. ``True`` if
            ``_prewrapped_func`` can accept stored parameters. ``False``
            if it cannot, and all parameters must be serialized.
        kwargs : dict
    ...
    def register(
        ...
        *,
        ...
        ----------
        path : str or object
            The TileDB path at which the task graph is to be registered.
            May be a path relative to a teamspace, a `Folder` or `Asset`
            instance, or an absolute "tiledb" URI. If the path to
            a folder is provided, the name or id of the task graph will
            be appended to form a full asset path.
        teamspace : Teamspace or str, optional
            The teamspace to which the task graph will be registered,
            specified by object or id. If not provided, the `path`
            parameter is queried for a teamspace id.

        Raises
        ------
        DAGError:
            When this task graph can not be registered.
        ...
        This creates a task graph asset at path "dags/example1" in the
        teamspace named "teamspace".

        >>> dag = DAG(name="example2")
        >>> dag.register(folder)

        This creates a DAG asset in "teamspace" at the path
        "dags/example2". The DAG's name has been appended to the folder
        path.

        A DAG can also be registered to a specific absolute "tiledb" URI
        ...
        dag_name = self.name or self.id
        registrar = TaskGraphRegistrar()
        try:
            registrar.create(
                path_id,
                teamspace_id,
                self._tdb_to_json(dag_name),
                dag_name,
            )
        except assets.AssetCreatorError as exc:
            raise DAGError("Failed to register DAG.") from exc
