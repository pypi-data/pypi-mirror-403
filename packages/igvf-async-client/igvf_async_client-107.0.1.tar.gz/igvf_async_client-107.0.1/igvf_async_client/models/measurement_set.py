...
class MeasurementSet(BaseModel):
    ...
    preferred_assay_titles: Optional[List[StrictStr]] = Field(default=None, description="The custom lab preferred label for the experiment performed.")
    ...
    multiome_size: Optional[Annotated[int, Field(strict=True, ge=1)]] = Field(default=None, description="The number of datasets included in the multiome experiment this measurement set is a part of.")
    control_types: Optional[List[StrictStr]] = Field(default=None, description="The types of control this measurement set represents.")
    ...
    enrichment_designs: Optional[List[StrictStr]] = Field(default=None, description="Files describing the assay-specific enrichment strategy (e.g., primer designs for PCR-based methods or probe/bait designs for hybrid capture) that define which regions are preferentially sequenced in this measurement set.")
    strand_specificity: Optional[StrictStr] = Field(default=None, description="Indicates whether a transcriptomic library is forward, reverse, or unstranded based on the strand-specific protocol used.")
    ...
    external_image_urls: Optional[List[Annotated[str, Field(strict=True)]]] = Field(default=None, description="Links to the external site where images produced by this measurement are stored.")
    ...
    barcode_replacement_file: Optional[StrictStr] = Field(default=None, description="A file containing original barcodes and the new barcodes used to replace the original barcodes. One common application is to use in preprocessing Parse SPLiT-seq data with the single cell uniform pipeline.")
    library_preparation_kit: Optional[StrictStr] = Field(default=None, description="A kit utilized in the library preparation procedure.")
    ...
    def preferred_assay_titles_validate_enum(cls, value):
        ...
        for i in value:
            if i not in set(['RNA-seq', 'scRNA-seq', 'snRNA-seq', 'scNT-seq', 'scNT-seq2', 'scNT-seq3', 'scMultiome-NT-seq', 'Parse SPLiT-seq', 'ATAC-seq', 'varACCESS', 'ACCESS-ATAC', 'scATAC-seq', 'scACCESS-ATAC', 'snATAC-seq', 'mtscMultiome', 'DOGMA-seq', 'DUAL-IPA', '10x multiome', '10x multiome with MULTI-seq', '10x snATAC-seq with Scale pre-indexing', 'snRNA-seq with Scale pre-indexing', 'MULTI-seq', 'SHARE-seq', 'Histone ChIP-seq', 'TF ChIP-seq', 'CUT&RUN', 'MPRA', 'MPRA (scQer)', 'electroporated MPRA', 'AAV-MPRA', 'lentiMPRA', 'STARR-seq', 'Cell painting', 'Variant painting via fluorescence', 'Variant painting via immunostaining', 'Pooled Y2H', 'Arrayed semi-qY2H v1', 'Arrayed semi-qY2H v2', 'Arrayed semi-qY2H v3', 'Arrayed yN2H', 'Arrayed mN2H', 'smFISH', 'MERFISH', 'Proliferation CRISPR screen', 'Migration CRISPR screen', 'CRISPR FlowFISH screen', 'CRISPR FACS screen', 'CRISPR MACS screen', 'CRISPR mCherry screen', 'HCR-FlowFISH screen', 'Variant-EFFECTS', 'scCRISPR screen', 'Perturb-seq', 'Parse Perturb-seq', 'CC-Perturb-seq', 'TAP-seq', 'CROP-seq', 'Multiome Perturb-seq', 'SGE', 'Immune-SGE', 'MAVE', 'MIAA', 'snmC-Seq2', 'snMCT-seq', 'snM3C-seq', 'VAMP-seq', 'VAMP-seq (MultiSTEP)', 'LABEL-seq', 'Hi-C', 'HiCAR', 'Spatial transcriptomics', 'HT-recruit', 'ONT dRNA', 'ONT Fiber-seq', 'ONT direct WGS', 'Bisulfite-seq', 'WGS', 'DNase-seq']):
                raise ValueError("each list item must be one of ('RNA-seq', 'scRNA-seq', 'snRNA-seq', 'scNT-seq', 'scNT-seq2', 'scNT-seq3', 'scMultiome-NT-seq', 'Parse SPLiT-seq', 'ATAC-seq', 'varACCESS', 'ACCESS-ATAC', 'scATAC-seq', 'scACCESS-ATAC', 'snATAC-seq', 'mtscMultiome', 'DOGMA-seq', 'DUAL-IPA', '10x multiome', '10x multiome with MULTI-seq', '10x snATAC-seq with Scale pre-indexing', 'snRNA-seq with Scale pre-indexing', 'MULTI-seq', 'SHARE-seq', 'Histone ChIP-seq', 'TF ChIP-seq', 'CUT&RUN', 'MPRA', 'MPRA (scQer)', 'electroporated MPRA', 'AAV-MPRA', 'lentiMPRA', 'STARR-seq', 'Cell painting', 'Variant painting via fluorescence', 'Variant painting via immunostaining', 'Pooled Y2H', 'Arrayed semi-qY2H v1', 'Arrayed semi-qY2H v2', 'Arrayed semi-qY2H v3', 'Arrayed yN2H', 'Arrayed mN2H', 'smFISH', 'MERFISH', 'Proliferation CRISPR screen', 'Migration CRISPR screen', 'CRISPR FlowFISH screen', 'CRISPR FACS screen', 'CRISPR MACS screen', 'CRISPR mCherry screen', 'HCR-FlowFISH screen', 'Variant-EFFECTS', 'scCRISPR screen', 'Perturb-seq', 'Parse Perturb-seq', 'CC-Perturb-seq', 'TAP-seq', 'CROP-seq', 'Multiome Perturb-seq', 'SGE', 'Immune-SGE', 'MAVE', 'MIAA', 'snmC-Seq2', 'snMCT-seq', 'snM3C-seq', 'VAMP-seq', 'VAMP-seq (MultiSTEP)', 'LABEL-seq', 'Hi-C', 'HiCAR', 'Spatial transcriptomics', 'HT-recruit', 'ONT dRNA', 'ONT Fiber-seq', 'ONT direct WGS', 'Bisulfite-seq', 'WGS', 'DNase-seq')")
        return value
    ...
    def control_types_validate_enum(cls, value):
        ...
        for i in value:
            if i not in set(['baseline', 'reference transduction', 'low FACS signal', 'non-targeting', 'unsorted FACS input', 'untransfected', 'untreated', 'wildtype']):
                raise ValueError("each list item must be one of ('baseline', 'reference transduction', 'low FACS signal', 'non-targeting', 'unsorted FACS input', 'untransfected', 'untreated', 'wildtype')")
        return value
    ...
    @field_validator('library_preparation_kit')
    def library_preparation_kit_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['10X Chromium Single Cell 3 prime v1', '10X Chromium Single Cell 3 prime v2', '10X Chromium Single Cell 3 prime v3', '10X Chromium Single Cell 3 prime v3.1', '10X Chromium Single Cell 3 prime v4', '10X Chromium Single Cell 5 prime v1', '10X Chromium Single Cell 5 prime v2', '10X Chromium Single Cell 5 prime v3']):
            raise ValueError("must be one of enum values ('10X Chromium Single Cell 3 prime v1', '10X Chromium Single Cell 3 prime v2', '10X Chromium Single Cell 3 prime v3', '10X Chromium Single Cell 3 prime v3.1', '10X Chromium Single Cell 3 prime v4', '10X Chromium Single Cell 5 prime v1', '10X Chromium Single Cell 5 prime v2', '10X Chromium Single Cell 5 prime v3')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in related_measurement_sets (list)
        _items = []
        if self.related_measurement_sets:
            for _item in self.related_measurement_sets:
                if _item:
                    _items.append(_item.to_dict())
            _dict['related_measurement_sets'] = _items
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "enrichment_designs": obj.get("enrichment_designs"),
            "strand_specificity": obj.get("strand_specificity"),
            "auxiliary_sets": obj.get("auxiliary_sets"),
            "external_image_urls": obj.get("external_image_urls"),
            ...
            "barcode_replacement_file": obj.get("barcode_replacement_file"),
            "library_preparation_kit": obj.get("library_preparation_kit"),
            "@id": obj.get("@id"),
            "@type": obj.get("@type"),
            ...
            "related_measurement_sets": [RelatedMeasurementSetGroup.from_dict(_item) for _item in obj["related_measurement_sets"]] if obj.get("related_measurement_sets") is not None else None,
