...
class PythonDocstringTest(unittest.TestCase):

    def testAbsoluteDateDocstringClass(self):
        assert "This class represents a specific instant in time." in AbsoluteDate.__doc__

    def testAbsoluteDateDocstringInstance(self):
        date = AbsoluteDate.J2000_EPOCH
        print(date.__doc__)

        assert "This class represents a specific instant in time." in date.__doc__
    ...
    def testGroundFieldOfViewDetector(self):
        ...
        assert "Frame associated to a position near the surface of a body shape." in TopocentricFrame.__doc__

        assert "unit vector in the east direction" in TopocentricFrame.getEast.__doc__
...
if __name__ == '__main__':
    suite = unittest.TestLoader().loadTestsFromTestCase(PythonDocstringTest)
