...
class TestNomadicMLClient:
    ...
    def test_create_folder_proxy(self):
        client = NomadicML(api_key="valid_api_key")
        mock_video = MagicMock()
        mock_video.create_folder.return_value = {"id": "folder1"}
        client.__dict__["video"] = mock_video

        result = client.create_folder("Marketing")

        assert result == {"id": "folder1"}
        mock_video.create_folder.assert_called_once_with("Marketing")

    def test_get_folder_proxy(self):
        client = NomadicML(api_key="valid_api_key")
        mock_video = MagicMock()
        mock_video.get_folder.return_value = {"id": "folder1"}
        client.__dict__["video"] = mock_video

        result = client.get_folder("Marketing")

        assert result == {"id": "folder1"}
        mock_video.get_folder.assert_called_once_with("Marketing")
