#!/usr/bin/env python3
"""
Python equivalent of 'single mosgap.in' for PADRE simulation.

This script generates the same PADRE input deck as 'single mosgap.in'
...
def create_single_mosgap_simulation():
    """Create the single MOS gap simulation equivalent to 'single mosgap.in'"""

    sim = Simulation()

    # Mesh Specification
    sim.mesh = Mesh(nx=3, ny=60)
    sim.mesh.add_y_mesh(1, 0, ratio=1)
    sim.mesh.add_y_mesh(10, 0.002, ratio=0.8)
    sim.mesh.add_y_mesh(60, 0.6, ratio=1.25)
    ...
    sim.add_region(Region(2, ix_low=1, ix_high=3, iy_low=10, iy_high=60,
    ...
    sim.add_electrode(Electrode(2, ix_low=1, ix_high=3, iy_low=60, iy_high=60))
    ...
    sim.add_command(Plot1D(potential=True, y_start=0, y_end=0.6, x_start=0.5, x_end=0.5,
                           ascii=True, outfile="pot"))
    sim.add_command(Plot1D(qfn=True, y_start=0, y_end=0.6, x_start=0.5, x_end=0.5,
                           ascii=True, outfile="qfn"))
    sim.add_command(Plot1D(qfp=True, y_start=0, y_end=0.6, x_start=0.5, x_end=0.5,
                           ascii=True, outfile="qfp"))
    sim.add_command(Plot1D(band_val=True, y_start=0, y_end=0.6, x_start=0.5, x_end=0.5,
                           ascii=True, outfile="val"))
    sim.add_command(Plot1D(band_con=True, y_start=0, y_end=0.6, x_start=0.5, x_end=0.5,
                           ascii=True, outfile="con"))
    sim.add_command(Plot1D(electrons=True, y_start=0, y_end=0.6, x_start=0.5, x_end=0.5,
                           ascii=True, outfile="ele"))
    sim.add_command(Plot1D(holes=True, y_start=0, y_end=0.6, x_start=0.5, x_end=0.5,
                           ascii=True, outfile="hole"))
    sim.add_command(Plot1D(net_charge=True, y_start=0, y_end=0.6, x_start=0.5, x_end=0.5,
                           ascii=True, outfile="ro"))
    sim.add_command(Plot1D(e_field=True, y_start=0, y_end=0.6, x_start=0.5, x_end=0.5,
    ...
    sim.add_solve(Solve(v1=-2.0, vstep=0.2, nsteps=20, electrode=1,
...
if __name__ == "__main__":
    sim = create_single_mosgap_simulation()
