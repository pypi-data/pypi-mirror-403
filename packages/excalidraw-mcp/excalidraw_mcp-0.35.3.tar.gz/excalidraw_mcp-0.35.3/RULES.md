...
- **Use UV for Tool Execution**
  ...
  - Execute tools like pyright, ruff, and excalidraw-mcp through UV: `uv run pyright`
...
- **Code Quality Validation**
  ...
  - Code should pass all quality checks when run through excalidraw-mcp
  - The ultimate goal is to run `python -m excalidraw-mcp -x -t` without any errors
...
- **Test Coverage Improvement (MANDATORY)**
  ...
  - **Target 85% milestone coverage**: Work toward 85% minimum coverage requirement (current: 77.1%)
...
```mermaid
flowchart TD
    Start[Start AI Agent Mode] --> FastHooks[Run Fast Hooks]
    FastHooks --> HooksPass{Hooks Pass?}

    HooksPass -->|No| RetryOnce[Retry Once]
    RetryOnce --> FastHooks

    HooksPass -->|Yes| CollectTests[Collect ALL Test Failures]
    CollectTests --> NoStopTests{More Failures?}
    NoStopTests -->|Yes| CollectTests
    NoStopTests -->|No| CollectHooks[Collect ALL Hook Issues]

    CollectHooks --> NoStopHooks{More Issues?}
    NoStopHooks -->|Yes| CollectHooks
    NoStopHooks -->|No| ApplyFixes[Apply AI Fixes<br/>Batch All Issues]

    ApplyFixes --> Increment[Increment Iteration Count]
    Increment --> CheckMax{< 10 Iterations?}

    CheckMax -->|Yes| FastHooks
    CheckMax -->|No| Complete[Complete with Errors]

    Complete --> CheckSuccess{All Fixed?}
    CheckSuccess -->|Yes| Success[Success: All Pass]
    CheckSuccess -->|No| Fail[Fail: Max Iterations]

    style Start fill:#e3f2fd
    style FastHooks fill:#fff3e0
    style ApplyFixes fill:#f3e5f5
...
- **MCP Server Integration**
  ...
  - MCP tools: `execute_excalidraw-mcp`, `get_job_progress`, `get_comprehensive_status`
...
- **Quality Standards Maintenance**
  ...
  - Test all documentation updates by running `python -m excalidraw-mcp --comprehensive`
