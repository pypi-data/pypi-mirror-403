# Wood Wide Python API library

<!-- prettier-ignore -->
[![PyPI version](https://img.shields.io/pypi/v/woodwide.svg?label=pypi%20(stable))](https://pypi.org/project/woodwide/)

The Wood Wide Python library provides convenient access to the Wood Wide REST API from any Python 3.9+
...
to add `WOOD_WIDE_AI_API_KEY="My API Key"` to your `.env` file
so that your API Key is not stored in source control.

## Async usage

Simply import `AsyncWoodWide` instead of `WoodWide` and use `await` with each API call:
...
async def main() -> None:
    response = await client.auth.retrieve_me()
...
import os
import asyncio
from woodwide import DefaultAioHttpClient
from woodwide import AsyncWoodWide
...
response), a subclass of `woodwide.APIStatusError` is raised, containing `status_code` and `response` properties.
...
except woodwide.APIConnectionError as e:
    print("The server could not be reached")
    print(e.__cause__)  # an underlying Exception, likely raised within httpx.
except woodwide.RateLimitError as e:
...
client.with_options(max_retries=5).auth.retrieve_me()
```
...
client.with_options(timeout=5.0).auth.retrieve_me()
```
...
You can enable logging by setting the environment variable `WOOD_WIDE_LOG` to `info`.

```shell
$ export WOOD_WIDE_LOG=info
```
...
auth = response.parse()  # get the object that `auth.retrieve_me()` would have returned
print(auth)
```
...
client = WoodWide(
    # Or use the `WOOD_WIDE_BASE_URL` env var
