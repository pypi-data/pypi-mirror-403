...
def test_login(name, method, usr, pwd, exp_except, request):
    ...
    context.api = EliteCloudApi(usr, pwd)
    ...
    if exp_except is None:
        ...
        match method:
            ...
            case 'Auth-Api': context.api._login_auth_api()
...
def test_login_seq(name, usr, pwd, exp_except, request):
    ...
    context.api = EliteCloudApi(usr, "wrong_pwd")
    assert context.api.closed == False
    assert context.api._login_method is None
...
def test_fetch_status(name, method, loop, exp_except, request):
    context = request.getfixturevalue("context")
    context.api = EliteCloudApi(TEST_USERNAME, TEST_PASSWORD)
    assert context.api.closed == False

    # Login
    match method:
        case 'Auto':                 context.api.login()
        case LoginMethod.AUTH_API:   context.api._login_auth_api()
    ...
    for site in sites:
        ...
        resources = context.api.fetch_site_resources(site_uuid)
        ...
        status = context.api.fetch_site_status(site_uuid)
...
def test_subscribe_status(name, method, loop, exp_except, request):
    context = request.getfixturevalue("context")
    flags = {
