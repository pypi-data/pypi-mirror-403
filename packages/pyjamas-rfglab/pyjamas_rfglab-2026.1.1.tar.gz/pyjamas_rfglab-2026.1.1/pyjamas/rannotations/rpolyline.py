...
class RPolyline(rannotation.RAnnotation):
    ...
    def bresenham_singlepix_line3d(p1: numpy.ndarray, p2: numpy.ndarray, precision: int = 0) -> (
        ...
        :param precision: numpy.int=0. Although according to Bresenham's line
                algorithm, point coordinates x1 y1 z1 and x2 y2 z2 should
                be integer numbers, this program extends its limit to all
                real numbers. If any of them are floating numbers, you
                should specify how many digits of decimal that you would
    ...
    def tomask(self, imshape: numpy.ndarray, brushsz: numpy.uint8) -> (numpy.ndarray, numpy.ndarray):
        ...
        binary_image = numpy.zeros(imshape, dtype=bool)
         ...
         # appending to a Python list is O(1), to a numpy array is O(n), as a new array is created with each append.
        x_segments = []
        y_segments = []
        ...
        for ii in range(len(self.points) - 1):
            ...
            x_segments.append(thex)
            y_segments.append(they)

        # Concatenate once (much faster than repeated append)
        x = numpy.concatenate(x_segments) if x_segments else numpy.array([])
        y = numpy.concatenate(y_segments) if y_segments else numpy.array([])

        # Get rid of any pixels not on the image.
        ind_good = (x >= 0) & (x < imshape[1]) & (y >= 0) & (y < imshape[0])
        x = x[ind_good]
        y = y[ind_good]

        # Those points form an edge mask (we remove repeated points) after inverting coordinates
        # to go from (x, y) to (row, col).
        themask_perimeter = numpy.unique(numpy.vstack([y.astype(int), x.astype(int)]), axis=1)

        # Set all the pixels under the polyline to True in the mask.
        binary_image[themask_perimeter[0], themask_perimeter[1]] = True
        ...
        perimeter_pixel_indices = themask_perimeter.T
        inside_pixel_indices = numpy.column_stack(themask_inside)
