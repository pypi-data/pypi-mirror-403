...
class rimutils:
    ...
    @classmethod
    def extract_contours(cls, labeled_image: Optional[numpy.ndarray] = None, border_objects: bool = False, extract_spots: bool = False, fully_connected: Optional[str] = 'low') -> list:
        """
        ...
        for aLabel in thelabels:
            ...
            aContour = skimsr.find_contours(bin_mask, 0, fully_connected)
    ...
    @classmethod
    def mask_from_polylines(cls, imsize: Tuple[int, int], polylines: List, brushsz: int = 3, labeled: Optional[bool] = False) -> numpy.ndarray:
        """
        Generates a binary or labelled image from a list of polylines.

        :param imsize: (row, col) dimensions of the image to generate
        :param polylines: list of polylines (each polyline is an nx2 array of points)
        :param brushsz: brushsz width use to "paint" the polyline edge.
        :param labeled: True if the output must be a labeled image, in which each mask object carries a label corresponding to its position in the list of polylines.
        :return:
        """
        imtype = int if labeled else bool

        mask_image = numpy.zeros(imsize, dtype=imtype)

        for index, apoly in enumerate(polylines):
            # Create a polyline and measure it:
            thepolyline = pjsannpol.RPolyline(apoly)
            _, inside_mask = thepolyline.tomask(imsize, brushsz)
            mask_image[inside_mask[::, 0], inside_mask[::, 1]] = index+1 if labeled else True
