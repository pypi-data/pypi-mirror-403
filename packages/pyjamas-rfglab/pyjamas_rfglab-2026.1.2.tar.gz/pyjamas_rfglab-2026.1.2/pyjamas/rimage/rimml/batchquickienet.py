    ...
    def predict(self, slices: numpy.ndarray, indices: numpy.ndarray, progress_signal: Optional[ThreadSignals] = None,
        ...
        for i, index in enumerate(indices):
            ...
            if annotations is not None:
                ...
                if len(this_object_array) > 0 and len(this_object_array) == len(theids):  # the second condition ensures the same number of objects in each frame. We should be able to remove that.
