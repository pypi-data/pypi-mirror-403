...
from bibble.files.path_reader import PathReader
from bibble.files.path_writer import PathWriter
...
class BidiPaths(IdenBidiMiddleware):
    """ A simple wrapper around PathReader and PathWriter,
    to make a bidirectional paths middleware
    """

    def __init__(self, *args, lib_root:pl.Path, reader:Maybe[PathReader]=None, writer:Maybe[PathWriter]=None, **kwargs) -> None:
        kwargs.setdefault(API.ALLOW_INPLACE_MOD_K, False)
        super().__init__(*args, **kwargs)
        self._reader = reader or PathReader(lib_root=lib_root)
        self._writer = writer or PathWriter(lib_root=lib_root)
