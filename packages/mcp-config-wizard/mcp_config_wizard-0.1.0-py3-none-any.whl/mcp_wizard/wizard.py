...
def select_client() -> tuple[MCPClient, Path] | None:
    """Select an MCP client, scope, and get its config path."""
    ...
    for client in clients:
        if client.name == "custom":
            choices.append(Choice(title="📁 Custom Location", value=client))
        else:
            status = "✓" if client.exists() else "○"
            choices.append(
                Choice(title=f"{status} {client.display_name}", value=client)
            )
    ...
    if selected.name == "custom":
        ...
        if not custom_path:
            return None
        return selected, Path(custom_path)
    ...
    if len(scoped_paths) > 1:
        ...
        for config_path in scoped_paths:
            ...
            scope_choices.append(
                Choice(
                    title=f"{'🌍' if config_path.scope == MCPScope.GLOBAL else '📂'} {scope_label}: {config_path.path}",
                    value=config_path
                )
            )
