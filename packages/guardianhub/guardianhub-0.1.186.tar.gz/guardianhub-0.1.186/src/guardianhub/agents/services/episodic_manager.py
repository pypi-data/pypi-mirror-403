...
class EpisodicManager:
    ...
    async def get_infrastructure_ground_truth(self, search_term: str, env: str) -> List[Dict[str, Any]]:
        """Retrieves structural truth from Neo4j."""
        query = """
        MATCH (ci:ConfigurationItem)-[:DEPENDS_ON*1..2]->(dep)
        WHERE (ci.name CONTAINS $search OR ci.tags CONTAINS $search) AND ci.env = $env
        RETURN ci.name as name, ci.status as status, ci.kind as kind, collect(dep.name) as impacted_services
        LIMIT 25
        """
        return await self.graph_client._execute_read_query(query, {"search": search_term, "env": env})
