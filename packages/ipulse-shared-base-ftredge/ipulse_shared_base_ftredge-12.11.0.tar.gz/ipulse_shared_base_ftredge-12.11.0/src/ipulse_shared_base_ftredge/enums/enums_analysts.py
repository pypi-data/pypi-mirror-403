...
class PredictionTaskType(AutoLower):
    ...
    GENERATE_INVESTMENT_THESIS_AND_TIMESERIES_CLOSE_PRICE_CHANGE = auto()  # Combined qualitative and quantitative
    GENERATE_INVESTMENT_THESIS_AND_TIMESERIES_CLOSE_PRICE_PCT_CHANGE = auto()  # Combined qualitative and quantitative
...
class PredictionTarget(AutoLower):
    ...
    Defines the analytical goal of the prediction pipeline.
    """
    CLOSE_PRICE_CHANGE = auto()
    CLOSE_PRICE_PCT_CHANGE = auto()
...
class AnalystMode(AutoLower):
    ...
    FAST_THINKER = auto()  # thinking_level=FAST, no web, no RAG
    THINKER = auto()  # thinking_level=NORMAL, no web, no RAG
    DEEP_THINKER = auto()  # thinking_level=DEEP, no web, no RAG
    DEEP_CREATIVE_THINKER = auto()  # thinking_level=DEEP, creativity_level=CREATIVE, no web, no RAG
    ULTRA_DEEP_THINKER = auto()  # thinking_level=ULTRA_DEEP, no web, no RAG
    CREATIVE_THINKER = auto()  # creativity_level=CREATIVE, thinking_level=DEEP

    RESEARCHER = auto()  # thinking_level=NORMAL, web_search=True, no RAG
    THINKING_RESEARCHER = auto()  # thinking_level=NORMAL, web_search=True, web_mode=STANDARD
    DEEP_THINKING_RESEARCHER = auto()  # thinking_level=DEEP, web_search=True, web_mode=STANDARD
    DEEP_THINKING_DEEP_RESEARCHER = auto()  # thinking_level=DEEP, web_search=True, WebSearchMode=DEEP , no RAG

    # Scholar Modes - RAG-enabled
    SCHOLAR = auto()  # thinking_level=NORMAL, rag_search=True, rag_mode=STANDARD
    DEEP_SCHOLAR = auto()  # thinking_level=DEEP, rag_search=True, rag_mode=PRECISE
    CREATIVE_SCHOLAR = auto()  # creativity_level=CREATIVE, rag_search=True
        
    # Hybrid Modes - RAG + Web
    SCHOLAR_RESEARCHER = auto()  # Both rag_search=True and web_search=True, thinking_level=NORMAL
    DEEP_SCHOLAR_RESEARCHER = auto()  # Both enabled, thinking_level=DEEP, web_search_mode=DEEP
    CREATIVE_SCHOLAR_RESEARCHER = auto()  # creativity_level=CREATIVE, both search types enabled
...
class ThinkingLevel(AutoLower):
    ...
    NORMAL = auto()  # Standard reasoning (default)
    DEEP = auto()  # Deep, thorough reasoning (default)
    ULTRA_DEEP = auto()  # Maximum reasoning depth (when available)
...
class CreativityLevel(AutoLower):
    ...
    """
    DETERMINISTIC = auto()  # Low temperature, highly consistent
    BALANCED = auto()  # Moderate creativity (default)
