...
class PartialUpdateSubscriptionDto(BaseModel):
    """
    Subscriptions configuration. Omit subscriptions (or set to null) to leave all subscriptions unchanged. An empty object ({}) also makes no changes. When subscriptions is provided, interceptions and notifications are full replacements: [] removes all, a list replaces existing (unmentioned items removed). Omit a collection to keep its current value.
    ...
    """ # noqa: E501
    interception_order: StrictInt = Field(description="The order in which all interceptions for this plugin will be run in their respective APIs. This can be overridden in each interception's data. Must be a positive number. Default is 1", alias="interceptionOrder")
    interceptions: JsonNullableListInterceptionSubscriptionDto
    notifications: JsonNullableListNotificationSubscriptionDto
    # BioT Code Start: Store custom attributes in a dedicated dict (safe default)
    additional_properties: Dict[str, Any] = Field(default_factory=dict)
    # BioT Code End
    __properties: ClassVar[List[str]] = ["interceptionOrder", "interceptions", "notifications"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of PartialUpdateSubscriptionDto from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # override the default output from pydantic by calling `to_dict()` of interceptions
        if self.interceptions:
            _dict['interceptions'] = self.interceptions.to_dict()
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of PartialUpdateSubscriptionDto from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "interceptionOrder": obj.get("interceptionOrder"),
            "interceptions": JsonNullableListInterceptionSubscriptionDto.from_dict(obj["interceptions"]) if obj.get("interceptions") is not None else None,
            "notifications": JsonNullableListNotificationSubscriptionDto.from_dict(obj["notifications"]) if obj.get("notifications") is not None else None
        })
