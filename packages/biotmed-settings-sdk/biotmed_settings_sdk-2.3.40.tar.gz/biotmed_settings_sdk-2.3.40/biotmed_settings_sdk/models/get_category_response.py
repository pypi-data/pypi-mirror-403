...
class GetCategoryResponse(BaseModel):
    ...
    built_in_supported_types: Optional[List[StrictStr]] = Field(default=None, alias="builtInSupportedTypes")
    custom_supported_types: Optional[List[StrictStr]] = Field(default=None, alias="customSupportedTypes")
    allowed_categories_to_link_to: Optional[List[StrictStr]] = Field(default=None, alias="allowedCategoriesToLinkTo")
    support_mandatory_validation: Optional[StrictBool] = Field(default=None, alias="supportMandatoryValidation")
    custom_base_path: Optional[StrictStr] = Field(default=None, alias="customBasePath")
    # BioT Code Start: Store custom attributes in a dedicated dict (safe default)
    additional_properties: Dict[str, Any] = Field(default_factory=dict)
    # BioT Code End
    __properties: ClassVar[List[str]] = ["name", "displayName", "builtInSupportedTypes", "customSupportedTypes", "allowedCategoriesToLinkTo", "supportMandatoryValidation", "customBasePath"]

    @field_validator('built_in_supported_types')
    def built_in_supported_types_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        for i in value:
            if i not in set(['LABEL', 'PARAGRAPH', 'INTEGER', 'DECIMAL', 'BOOLEAN', 'SINGLE_SELECT', 'MULTI_SELECT', 'ADDRESS', 'NAME', 'PHONE', 'EMAIL', 'DATE_TIME', 'DATE', 'REFERENCE', 'TIMEZONE', 'LOCALE', 'FILE', 'IMAGE', 'LINK', 'WAVEFORM']):
                raise ValueError("each list item must be one of ('LABEL', 'PARAGRAPH', 'INTEGER', 'DECIMAL', 'BOOLEAN', 'SINGLE_SELECT', 'MULTI_SELECT', 'ADDRESS', 'NAME', 'PHONE', 'EMAIL', 'DATE_TIME', 'DATE', 'REFERENCE', 'TIMEZONE', 'LOCALE', 'FILE', 'IMAGE', 'LINK', 'WAVEFORM')")
        return value

    @field_validator('custom_supported_types')
    def custom_supported_types_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "builtInSupportedTypes": obj.get("builtInSupportedTypes"),
            "customSupportedTypes": obj.get("customSupportedTypes"),
            "allowedCategoriesToLinkTo": obj.get("allowedCategoriesToLinkTo"),
            "supportMandatoryValidation": obj.get("supportMandatoryValidation"),
            "customBasePath": obj.get("customBasePath")
        })
