...
class BaseLocale(BaseModel):
    ...
    """ # noqa: E501
    code: StrictStr = Field(description="Locale in the following format: {language-ISO639}-{countryCode-ISO3166} lowercase.")
    hidden: Optional[StrictBool] = None
    translation_fallback_types: Optional[List[StrictStr]] = Field(default=None, description="Valid array values: DEFAULT_LOCALE,SYSTEM_LOCALE. Allow to control the fallback mechanism. If not provided(null) then the default behavior is the following fallback order: -> DEFAULT_LOCALE -> SYSTEM_LOCALE. The recommended value is null. When all of the fallback types return an empty value, no translation will be made, i.e. the original value will be retained", alias="translationFallbackTypes")
    # BioT Code Start: Store custom attributes in a dedicated dict (safe default)
    additional_properties: Dict[str, Any] = Field(default_factory=dict)
    # BioT Code End
    __properties: ClassVar[List[str]] = ["code", "hidden", "translationFallbackTypes"]

    @field_validator('translation_fallback_types')
    def translation_fallback_types_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        for i in value:
            if i not in set(['DEFAULT_LOCALE', 'SYSTEM_LOCALE']):
                raise ValueError("each list item must be one of ('DEFAULT_LOCALE', 'SYSTEM_LOCALE')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of BaseLocale from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of BaseLocale from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "translationFallbackTypes": obj.get("translationFallbackTypes")
        })
