...
class MongoCustomerCRUD:
    def __init__(
        ...
        self.collection = self.db.get_collection("customers")
    ...
    def execute_bulk_operations(self, operations: list):
        try:
            if operations:
    ...
    def process_records(self, records: list, operation: str):
        operations = self.validate_and_prepare_operations(records, operation)
        self.execute_bulk_operations(operations)
...
mongo_crud.process_records(message["records"], message["metadata"]["operation"])
