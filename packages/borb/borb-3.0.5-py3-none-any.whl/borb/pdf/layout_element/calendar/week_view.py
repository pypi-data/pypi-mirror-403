...
# -*- coding: utf-8 -*-
"""
A calendar view that represents a full week of scheduled events.

This class renders a horizontal grid of days, each divided into
30-minute intervals from a defined start to end hour. Events are placed
into non-overlapping lanes within their respective time ranges, allowing
...
class WeekView(CalendarView):
    ...
    @staticmethod
    def __calculate_number_of_characters_that_fit(
        font_size: int,
        height: int,
        s: str,
        width: int,
        ...
        return max(
            [
                ...
                if Paragraph(text=s, font_size=font_size).get_size(
                    available_space=(width, 2**64)
                )[1]
                < height
            ]
        )
    ...
    def paint(
        ...
        for weekday in range(0, 7):
            weekday_x: int = (
                background_x + hour_label_width + (weekday * self.__lane_width)
            )
            for lane_index, lane in enumerate(self.__grid[weekday]):
                ...
                while i < len(lane):
                    ...
                    event_top: int = (
                        background_y + h - day_label_height - i * self.__item_height
                    )
                    event_bottom: int = (
                        event_top
