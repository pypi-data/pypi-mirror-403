...
class Agent(BaseModel):
    ...
    async def turn(
        ...
        *,
        ...
        goal = f"""Respond to the user's message and help them with their request.
...
Use your available tools and skills as needed. When you've fully addressed the user's request, say DONE."""
    ...
    def _build_system_prompt(
        ...
        if self._skills:
            ...
            parts.append(
                "## Your Skills\n"
                f"{skills_text}\n\n"
                "To use a skill as a tool, call `skill.invoke` with:\n"
                '- `skill_id`: one of the skill IDs above\n'
                "- `inputs`: an object matching that skill's input schema"
            )

        parts.append(
            """## Instructions
- Use your available tools strategically to help the user
- Remember information that might be useful later
...
- When done, include "DONE" in your response"""
    ...
    def _gather_tools(self, ctx: "Context") -> list[str]:
        ...
        if self._skills:
            tools.extend(["skill.invoke", "skill.list"])

        # Standard tools
        tools.extend(["llm.*", "memory.*"])
        return tools
