...
class AlbusServer:
    ...
    async def start(
        ...
        print("\nKey Endpoints:")
        print("  GET  /api/v1/health          - Health check with dependency status")
        print("  GET  /api/v1/config/validate - Validate configuration")
        print("  GET  /api/v1/tools           - List all tools")
        print("  GET  /api/v1/pathways        - List all pathways")
        print("  POST /api/v1/pathways        - Create a pathway")
        print("  GET  /api/v1/agents          - List all agents")
        print("  POST /api/v1/agents/{id}/turn - Run an agent turn")
        print("  GET  /api/v1/ws              - WebSocket: events + JSON-RPC")
        print("  GET  /api/v1/help            - Comprehensive API help")
        print()
...
async def _async_main(host: str, port: int, debug: bool):
    ...
    # Deploy packs from config
    deployed_packs = []
    if deploy_config.packs:
        from packs.registry import resolve_pack_ids

        packs_to_deploy, missing = resolve_pack_ids(deploy_config.packs)
        if missing:
            logger.warning("Unknown pack IDs in config: %s", missing)

        service = server.runtime.pathway_service
        for p in packs_to_deploy:
            try:
                service.deploy_pack(p)
                deployed_packs.append(p)
            except Exception as e:
                logger.warning("Failed to deploy pack %s: %s", p.id, e)

        if deployed_packs:
            logger.info(
                "Deployed %d packs: %s",
                len(deployed_packs),
                [p.id for p in deployed_packs],
            )
    else:
        logger.info("No packs configured (use albus.yaml or ALBUS_CONFIG)")

    # =========================================================================
    # AGENT SERVICE - Deploy agents from config
    # =========================================================================
    from albus.application.agents import AgentService

    agent_service = AgentService(
        store=ctx.extras.get("studio_store"),
        pathway_service=server.runtime.pathway_service,
    )
    ctx.extras["agent_service"] = agent_service

    # Deploy agents from config
    deployed_agents = []
    agent_ids = getattr(deploy_config, "agents", None) or []
    if agent_ids:
        from agents.registry import resolve_agent_ids, list_available_agents

        # Handle "*" wildcard (deploy all agents)
        if agent_ids == ["*"] or agent_ids == "*":
            agents_to_deploy = list_available_agents()
            missing = []
        else:
            agents_to_deploy, missing = resolve_agent_ids(agent_ids)

        if missing:
            logger.warning("Unknown agent IDs in config: %s", missing)

        for a in agents_to_deploy:
            try:
                agent_service.register(a, source=f"code:agents.{a.id}")
                deployed_agents.append(a)
            except Exception as e:
                logger.warning("Failed to deploy agent %s: %s", a.id, e)

        if deployed_agents:
            logger.info(
                "Deployed %d agents: %s",
                len(deployed_agents),
                [a.id for a in deployed_agents],
            )
    else:
        # Auto-deploy all registered agents by default (developer-friendly)
        try:
            ...
            for a in all_agents:
                try:
                    ...
                    logger.debug("Failed to auto-deploy agent %s: %s", a.id, e)

            if deployed_agents:
                logger.info(
                    "Auto-deployed %d agents: %s",
                    len(deployed_agents),
                    [a.id for a in deployed_agents],
                )
        except ImportError:
            logger.debug("No agents module found")

    # Setup triggers for all deployed packs
    trigger_count = 0
    for pack in deployed_packs:
        try:
            subs = await trigger_manager.setup_pack(pack)
            trigger_count += len(subs)
        except Exception as e:
            logger.warning("Failed to setup triggers for pack %s: %s", pack.id, e)

    if trigger_count > 0:
        logger.info(
            "Activated %d triggers from %d packs", trigger_count, len(deployed_packs)
        )
