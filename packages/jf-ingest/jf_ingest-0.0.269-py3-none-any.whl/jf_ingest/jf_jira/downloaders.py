...
def _detect_project_rekeys_and_update_metadata(
    ...
    for project in projects:
        ...
        if (
            ...
            and project['key'] != jellyfish_project_ids_to_keys[project['id']]
        ):
            logging_helper.send_to_agent_log_file(
                f'Project (project_id={project["id"]}) {project["key"]} was detected as being rekeyed (it was previously {jellyfish_project_ids_to_keys[project["id"]]}. Attempting to re-download all related jira issue data'
            )
...
def _get_project_filters(
    ...
    if include_categories:
        ...
        def _include_filter(proj):
            ...
            if not 'projectCategory' in proj:
                return False
...
def _get_projects(
    jira_connection: JIRA, search_str: Optional[str] = None, use_pagination: bool = True
) -> list[dict]:
    """Fetch Projects, which involves some nuance!
    Some customers have problems using the /project API
    endpoint because too many projects exist and we time out
    with 504 errors. To avoid this, we should use the paginated
    /project/search API endpoint which should be more performant.

    NOTE: This API endpoint exists on both cloud and server endpoints,
    HOWEVER, many of our server customers likely have not allow listed
    this API endpoint yet. Instead of getting them all to retroactively
    allow list this API endpoint, we will:
      1. Attempt to fetch projects via the /project/search API endpoint
        a. If we can do that, great
        b. If we encounter a 404, it is likely due to firewall restrictions.
           In that case, fall back to the /project API endpoint


    Args:
        jira_connection (JIRA): A valid Jira Connection
        search_str (Optional[str], optional): A search string to filter projects. Defaults to None.
    ...
    projects = []
    if use_pagination:
        base_url = jira_connection._get_url("project/search")

        start_at = 0
        while True:
            page_url = f'{base_url}?startAt={start_at}&maxResults=50'
            if search_str:
                page_url += f'&query={search_str}'

            try:
                response = retry_for_status(jira_connection._session.get, page_url)
                response.raise_for_status()
            except JIRAError as e:
                #
                # NOTE: If we can't access the /project/search endpoint it could be that
                # this server customer hasn't allow listed the API endpoint. Fall back
                # to the "old" way of fetching projects
                #
                if e.status_code == 404:
                    logger.warning(
                        "The Jira instance does not support the /project/search API endpoint. Falling back to /project API endpoint."
                    )
                    return [
                        proj.raw
                        for proj in retry_for_status(
                            jira_connection.projects,
                        )
                    ]
                raise

            json_str = response.content.decode()
            page_result = json.loads(json_str) or {}
...
def download_projects_and_versions_and_components(
    ...
    include_categories: list[str],
    exclude_categories: list[str],
    use_projects_pagination: bool = False,
) -> list[dict]:
    ...
    with record_span('download_jira_projects'):
        ...
        all_project_dicts: list[dict] = _get_projects(
            jira_connection, search_str=None, use_pagination=use_projects_pagination
        )

        project_dicts = [proj for proj in all_project_dicts if all(filt(proj) for filt in filters)]
        ...
        add_telemetry_fields({'jira_project_count': len(project_dicts)})
    ...
    with record_span('download_jira_components'):
        ...
        for p in project_dicts:
            ...
            p.update({"components": components})
    ...
    with record_span('download_jira_versions'):
        ...
        for p in project_dicts:
            if p.get("archived", False):
                logging_helper.send_to_agent_log_file(
                    f'Skipping version download for archived project {p.get("key", "(Unknown Key)")} because archived projects hide version history',
                    level=logging.DEBUG,
                )
                versions = []
            else:
            ...
            p.update({"versions": [v.raw for v in versions]})
            version_count += len(versions)
        add_telemetry_fields({'jira_project_version_count': version_count})
        logger.info("Done downloading Jira Versions!")

    logger.info(
        f"Done downloading Jira Project, Components, and Version. Found {len(project_dicts)} projects"
    )
