...
from django.utils.html import SafeString, mark_safe, format_html
...
from lino.utils.html import table2html, table2htmls

from lino.utils.jsgen import VisibleComponent
from lino.utils.html import E, tostring, forcetext, html2text, html_attrs
...
class LayoutElement(VisibleComponent):
    ...
    def as_plain_htmls(self, ar, obj):
        return format_html("<p>cannot handle {}</p>", self.__class__)
...
class ConstantElement(LayoutElement):
    ...
    def as_plain_htmls(self, ar, obj):
        return format_html("{}", self.value.get("html"))
...
class FieldElement(LayoutElement):
    ...
    def as_plain_htmls(self, ar, obj):
        ...
        if not text:
            text = " "
        yield format_html("<label>{}</label>", self.field.verbose_name)
        if self.layout_handle.layout.label_align == layouts.LABEL_ALIGN_TOP:
            yield mark_safe("<br>")
        input_classes = (
            "form-control " + self.input_classes
        ...
        yield format_html(
            '<input type="text" value="{}" class="{}"/>', text, input_classes
        )
...
class TextFieldElement(FieldElement):
    ...
    def as_plain_htmls(self, ar, obj):
        ...
        if not text:
            text = " "
        yield format_html(
            '<div class="form-group"><label>{}</label><textarea rows="{}" class="form-control">{}</textarea></div>',
            self.get_label(),
...
class DisplayElement(FieldElement):
    ...
    def as_plain_htmls(self, ar, obj):
        ...
        if etree.iselement(value):
            ...
            for x in super().as_plain_htmls(ar, obj):
                yield x
...
class HtmlBoxElement(DisplayElement):
    ...
    def as_plain_htmls(self, ar, obj):
        value = self.value_from_object(obj, ar)
        if value is NOT_PROVIDED:
            value = mark_safe(ar.no_data_text)
...
class Wrapper(VisibleComponent):
    ...
    def as_plain_htmls(self, ar, obj):
        for chunk in self.wrapped.as_plain_htmls(ar, obj):
            yield chunk
...
class Container(LayoutElement):
    ...
    def as_plain_htmls(self, ar, obj):
        if self.vertical:
            children = []
            for e in self.elements:
                for chunk in e.as_plain_htmls(ar, obj):
                    children.append(chunk)
            yield format_html("<fieldset>{}</fieldset>", "".join(children))
        else:
            tr = []
            for e in self.elements:
                cell_chunks = []
                ...
                cell = format_html(
                    '<td style="vertical-align: top;">{}</td>', cell_html
                )
                tr.append(cell)
            yield format_html("<table><tr>{}</tr></table>", "".join(tr))
...
class GridElement(Container):
    ...
    def headers2htmls(self, ar, columns, headers, header_links, widths, **cellattrs):
        assert len(headers) == len(columns)
        for i, e in enumerate(columns):
            txt = headers[i]
            # print 20131015, txt
            txt = "".join(join_elems(txt.split("\n"), sep="<br>"))
            ...
            classes = " ".join(e.get_cell_classes())
            if classes:
                cellattrs["class"] = classes
            if widths:
                cellattrs["width"] = widths[i]
            yield mark_safe("<th %s>%s</th>" % (html_attrs(**cellattrs), txt))
    ...
    def as_plain_htmls(self, ar, obj):
        sar = ar.spawn(self.actor.default_action, master_instance=obj)
        yield table2htmls(sar, as_main=(self.name == "main"))
...
class TabPanel(Panel):
    ...
    def as_plain_htmls(self, ar, obj):
        ...
        nav_html = format_html(
            '<ul class="nav nav-tabs">{}</ul>', mark_safe("".join(nav_items))
        )
        yield nav_html
        ...
        for i, e in enumerate(self.elements):
            ...
            for chunk in e.as_plain_htmls(ar, obj):
                cell_chunks.append(chunk)
            cell_html = "".join(cell_chunks)
            extra_classes = " in active" if i == 0 else ""
            main_item = format_html(
        ...
        main_html = format_html(
            '<div class="tab-content">{}</div>', mark_safe("".join(main_items))
        )
        yield main_html
