...
CLONEABLE_ATTRS = frozenset(
    """ah request user subst_user
...
class unused_CallbackManager(object):
    ...
    def run_callback(self, request, thread_id, button_id):
        ...
        if cb is None:
            logger.debug(
                "No callback %r in %r" % (thread_id, list(self.pending_threads.keys()))
            )
        ...
        for c in cb.choices:
            if c.name == button_id:
                ...
                if settings.SITE.log_each_action_request and not a.readonly:
                    logger.info(
                        "run_callback {0} {1} {2}".format(thread_id, cb.message, c.name)
                    )
                    # ar.debug(cb.message)
                func = c.func
                try:
...
def set_callback(ar, cb):
    ...
    if answer is None:
        ...
        rqdata = {
            ...
            for k, v in (
                ar.rqdata.lists() if getattr(ar, "rqdata", None) is not None else {}
            )
        }
        rqdata.pop("_dc", None)
        ...
        for c in cb.choices:
            ...
            xcallback[c.name + "_resendEvalJs"] = ar.renderer.ar2js(
                ...
                ar.selected_rows,
                rqdata=rqdata,
                xcallback={"xcallback_id": cb_id, "choice": c.name},
            )
        # print(buttons)
