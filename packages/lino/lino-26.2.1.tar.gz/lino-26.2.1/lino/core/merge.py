...
class MergePlan(object):
    ...
    def analyze(self):
        ...
        for m, fk in traverse_ddh_fklist(self.obj.__class__):
            ...
            if fk.name in m.allow_cascaded_delete and not self.keep_volatiles.get(
                full_model_name(m, "_")
            ):
    ...
    def build_confirmation_message(self):
        ...
        def collect_summary(prefix, fk_qs):
            ...
            for fld, qs in fk_qs:
                if qs.count() > 0:
                    parts.append(
                        "%d %s" % (qs.count(), str(fld.model._meta.verbose_name_plural))
                    )
            if len(parts) != 0:
        ...
        collect_summary(
            gettext("will get reassigned."), self.related + self.generic_related
        )
        items.append(E.li(gettext("%s will be deleted") % self.obj))
        msg = gettext(
            "Are you sure you want to merge " "%(this)s into %(merge_to)s?"
        ) % dict(this=self.obj, merge_to=self.merge_to)
        if len(items) != 0:
    ...
    def execute(self, **kw):
        ...
        ) % dict(this=fmt(self.obj), merge_to=fmt(self.merge_to), updated=update_count)
