...
name_prefixes2 = set(("VAN DEN", "VAN DER", "VAN DE", "IN HET", "VON DER", "DE LA"))
    ...
    if len(a) == 0:
        ...
        if last_name_first:
            ...
            if a01.upper() in name_prefixes2:
                return dict(last_name=a01 + " " + a[2], first_name=" ".join(a[3:]))
            elif a[0].upper() in name_prefixes1:
                return dict(last_name=a[0] + " " + a[1], first_name=" ".join(a[2:]))
            else:
            ...
            if len(a) >= 4:
                ...
                if pc.upper() in name_prefixes2:
                    return dict(last_name=pc + " " + a[-1], first_name=" ".join(a[:-3]))
            pc = a[-2]  # prefix candidate
            if pc.upper() in name_prefixes1:
                return dict(last_name=pc + " " + a[-1], first_name=" ".join(a[:-2]))
...
class Human(model.Model):
    ...
    .. attribute:: title
        ...
        An optional professional position or academic qualification like "Dr."
        or "PhD".
    ...
    .. attribute:: middle_name
        ...
        <https://en.wikipedia.org/wiki/Middle_name>`_.

    .. attribute:: gender

      The sex of this person (male, female, nonbinary).

      Possible values are defined in :class:`lino.modlib.system.Genders`.
    """
    ...
    title = models.CharField(
        pgettext("(of a human)", "Title"), max_length=200, blank=True
    )
    ...
    def get_full_name(self, salutation=True, upper=None, last_name_first=None, **salutation_options):
        """
        ...
        """
        words = []
        if last_name_first is None:
            last_name_first = settings.SITE.last_name_first
        if salutation:
        ...
        rv = join_words(*words)
        if last_name_first and self.first_name:
            rv += ", " + self.first_name
        return rv
...
class Born(model.Model):
    ...
    birth_date = fields.IncompleteDateField(blank=True, verbose_name=_("Birth date"))
    ...
    @vfields.displayfield(
        _("Age"), wildcard_data_elem=True, sortable_by=['birth_date'])
    ...
    def setup_parameters(cls, fields):
        fields.update(
            aged_from=models.IntegerField(
                ...
                null=True,
                help_text=_(
                    "Select only persons aged at least " "the given number of years."
                ),
            ),
            aged_to=models.IntegerField(
                ...
                null=True,
                help_text=_(
                    "Select only persons aged at most " "the given number of years."
                ),
            ),
        )
    ...
    def get_request_queryset(cls, ar, **filter):
        ...
        if pv.aged_from:
            min_date = today - datetime.timedelta(days=pv.aged_from * 365)
            qs = qs.filter(birth_date__lte=min_date.strftime("%Y-%m-%d"))

        if pv.aged_to:
            max_date = today - datetime.timedelta(days=pv.aged_to * 365)
