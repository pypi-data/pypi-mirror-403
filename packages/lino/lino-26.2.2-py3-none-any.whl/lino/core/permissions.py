...
def make_static_permission_handler_(actor, readonly, debug_permissions, required_roles):
    ...
    if debug_permissions:  # False:
        ...
        def allow(action, user_type):
            ...
            if True:  # not v:
                logger.info(
                    "debug_permissions (view) %r " "required(%s), allow(%s)--> %s",
                    action,
...
def make_permission_handler_(
    ...
    if debug_permissions:
        ...
        def allow(action, user, obj, state):
            ...
            logger.info(
                "debug_permissions %r required(%s), " "allow(%s, %s, %s) --> %s",
                action,
                required_roles,
                user.username,
                repr(obj),
