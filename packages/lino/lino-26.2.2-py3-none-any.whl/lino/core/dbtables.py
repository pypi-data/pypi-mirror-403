...
class Table(AbstractTable):
    ...
    unique_keys = ['pk']  # Deprecated feature. Do not override.
    ...
    def get_row_by_pk(self, ar, pk):
        ...
        # else:
        # msg = None
        for fldname in self.unique_keys:
            # todo: use type mapping instead of making a db access
            try:
                return qs.get(**{fldname: pk})
            # except Exception as e:  # self.model.DoesNotExist:
            except (self.model.DoesNotExist, ValueError) as e:
                logger.debug("%s %s %s", fldname, pk, e)
                # msg = str(e)
        if False:
            sql = qs.query
            import sqlparse
            sql = str(sql).replace('"', '')
            msg = sqlparse.format(sql, reindent=True, keyword_case='upper')
            raise self.model.DoesNotExist(f"No row {pk} in {self} ({msg})") from None
        raise self.model.DoesNotExist(f"No row {pk} in {self}") from None
    ...
    def wildcard_data_elems(self):
        return vfields.wildcard_data_elems(self.model)
    ...
    def get_request_queryset(self, ar, **filter):
        ...
        def apply(qs):
            ...
            if ar.param_values is not None:
                ...
                # print(f"20251017 {ar.known_values} {qs.query}")

                for lookup, k in self.model._range_filters.items():
                    if (v := ar.param_values[k]):
            ...
            # A distinct() is necessary  to avoid duplicate rows in for #6484
            # (Filter nach Kompetenzen), where we  added a simple params field
            # 'exam__challenge__skill'. And as far as I imagine it doesn't
            # increase server load when not needed. Until now I don't know of
            # any table where the app developer would want non-distinct rows.
            qs = qs.distinct()
            return qs
