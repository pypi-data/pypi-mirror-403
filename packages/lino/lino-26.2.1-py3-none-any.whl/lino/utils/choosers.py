...
from lino.core.utils import getrqdata, resolve_model
from lino.core.vfields import VirtualField
from lino.core.vfields import DummyField
from lino.core.utils import set_default_verbose_name
...
class Chooser(FieldChooser):
    ...
    def get_request_choices(self, ar, tbl):
        ...
        # print(f"20251124c {tbl.master}")
    # kw = {}

        # ba = tbl.get_url_action(tbl.default_elem_action_name)
        # 20120202

        NEW_6492 = False  # 20251125 20251126 20251127 20251128 20251129
        if NEW_6492:
            # print(f"20251129 {tbl.master_field}")
            from lino.core.actors import Actor
            if isinstance(tbl, type) and issubclass(tbl, Actor):
                ar = tbl.create_request(request=ar.request)
        else:
        ...
        # print(f"20251124d {self.meth} {kw}")
...
def check_for_chooser(holder, field, name):
    # holder is either a Model, an Actor or an Action.
    # name is normally same as field.name except for remot fields
    if isinstance(field, DummyField):
        return
    ...
    methname = name + "_choices"
    m = getattr(holder, methname, None)
    if m is not None:
        # if field.name == "exam__challenge__skill":
        # print(f"20250511 found chooser for {name} in {holder} is {m}")
...
def discover_choosers():
    ...
    for model in apps.get_models():
        for field in model._meta.fields:
            check_for_chooser(model, field, field.name)
