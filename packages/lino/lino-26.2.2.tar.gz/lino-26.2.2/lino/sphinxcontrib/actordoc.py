...
class ddrefRole(XRefRole):
    ...
    def __call__(self, typ, rawtext, text, lineno, inliner, options={}, content=[]):
        ...
        self._lineno = lineno
        return XRefRole.__call__(
            self, typ, rawtext, text, lineno, inliner, options, content
        )
...
class ActorDirective(Lino2rstDirective):
    ...
    def get_rst(self):
        ...
        # ~ from django.conf import settings
        # ~ from djangosite. import set_language
        with translation.override(self.language):
            level, cls = resolve_name(self.content[0])
            ...
            if issubclass(cls, models.Model):
                ...
                model_reports = [r for r in kernel.master_tables if r.model is cls]
                model_reports += [r for r in kernel.slave_tables if r.model is cls]
