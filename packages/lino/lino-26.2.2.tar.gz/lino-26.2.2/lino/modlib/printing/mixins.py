...
NEEDSPDF_MSG = _("Please set a PDF print method for {}")
...
class CachedPrintable(Clonable, Printable):
    ...
    def get_cache_mtime(self):
        ...
        if not filename.exists():
            return None
        mtime = datetime.datetime.fromtimestamp(filename.lstat().st_mtime)
        if settings.USE_TZ:
    ...
    def build_target(self, ar):
        bm = self.get_build_method()
        t = bm.build(ar, self.__class__.do_print, self)
        if t is None:
            raise Exception(f"{self} : build() returned None?!")
        self.touch_from_timestamp(t)

    def touch_from_timestamp(self, t):
...
class MultiCachedPrintable(CachedPrintable):

    class Meta:
        abstract = True

    ensure_even_pages = False
    clear_cache_of_children = False

    def get_printable_children(self):
        raise NotImplementedError

    def clear_cache(self):
        super().clear_cache()
        if self.clear_cache_of_children:
            for obj in self.get_printable_children():
                obj.clear_cache()
    ...
    def build_target(self, ar):
        ...
        for obj in self.get_printable_children():
            ...
            if bm.target_ext != ".pdf":
                raise Warning(NEEDSPDF_MSG.format(obj))
            if obj.must_build_printable(bm):
                obj.build_target(ar)
            children.append(obj.get_target_file().path)
        if len(children) == 0:
            raise Warning(_("No content to print for {}").format(self))
        merger = PdfWriter()
        for pdf in children:
            merger.append(pdf)
            if self.ensure_even_pages and len(merger.flattened_pages) % 2:
                merger.add_blank_page()
        merger.write(target.path)
        merger.close()
        self.touch_from_timestamp(target.path.stat().st_mtime)
...
class CachedPrintableChecker(Checker):
    ...
    def get_checkdata_problems(self, ar, obj, fix=False):
        if obj.build_method is None:
            return
        if (mtime := obj.get_cache_mtime()) != obj.build_time:
            msg = _("Build time is {btime} but should be {mtime}.")
            params = dict(btime=obj.build_time, mtime=mtime)
            yield (True, msg.format(**params))
            if fix:
                obj.build_time = mtime
                obj.full_clean()
                obj.save()
