        ...
        for name, cl in kw.items():
            ...
            for i, a in enumerate(cls.workflow_actions):
                if a.action_name == name:
                    new = cl(a.target_state, a.required_roles, sort_index=a.sort_index)
                    new.attach_to_workflow(cls, name)
                    cls.workflow_actions[i] = new
                    found = True
                    break
            if not found:
                raise Exception("There is no workflow action named {0}".format(name))
...
class ChangeStateAction(actions.Action):
    ...
    def __init__(self, target_state, required_roles=None, help_text=None, **kw):
    ...
    def execute(self, ar, obj):
        def doit(ar):
            ...
            obj.set_workflow_state(ar, ar.actor.workflow_state_field, self.target_state)
