...
from lino.utils.choosers import discover_choosers, check_for_chooser
...
from .atable import TableHandle
...
class Kernel(object):
    ...
    def kernel_startup(self, site):
        ...
        for model in models_list:
            ...
            for k in ('hidden_columns', 'active_fields',
                ...
                if (v := getattr(model, k, None)) is not None:
                    setattr(model, k, fields.fields_list(model, v))
        ...
        for a in actors.actors_list:
            for name, field in a.virtual_fields.items():
                ...
                check_for_chooser(a, field, name)
        ...
        for res in actors.actors_list:
            ...
            for ba in res.get_actions():
                ...
                if ba.action.full_params_layout is not None:
                    ba.action.full_params_layout.get_layout_handle()
        ...
        # Loop over all actors, get their handle created and only then we
        # initialize the stores. This is here to fix #6404 (Inconsistent dataKey
        # in LightWeightContainer values). which happened because the virtual
        # fields for slave panels were getting created multiple times
        # (CommentsByRFC1 and CommentsByRFC2)
        for a in actors.actors_list:
            # if not a.abstract and a.model and a.get_handle_name is None:
            if a.get_handle_name is None and not a.is_abstract():
                # print("20251007", a)
                h = a.get_handle()
                h.store.init_store()
            ...
            for m, fk in model._lino_ddh.fklist:
                ...
                if fk.remote_field.on_delete == models.CASCADE:
                    if must_protect(m, fk, model):
                        ...
                        # msg = f"Set {fmn(m)}.{fk.name}.on_delete "
                        # msg += "to PROTECT because field is not specified in "
                        # msg += f"allow_cascaded_delete. {m} {model}"
                        # logger.info(msg)
                    ...
                    if fk.remote_field.on_delete == models.SET_NULL:
                        ...
                        msg = ("{0}.{1} has custom on_delete {2}").format(
    ...
    def run_action(self, ar):
        ...
        try:
            ...
            next_url = ar.rqdata.get("next_url", None)
            if next_url is not None:
                ar.set_response(replace_url=next_url)
    ...
    def setup_handle(self, h, ar):
        ...
        if h.actor.full_params_layout:
            h.full_params_layout_handle = h.actor.full_params_layout.get_layout_handle()
        h.store = Store(h)
