...
import pickle
import os
from .utils import _transform_space, MyProgressBar, load_model, StandardScalerSimplified
from scipy import interpolate
from .matter_powerspectrum import load_smeared_bao_emu

trapz = np.trapz if np.__version__[0] == '1' else np.trapezoid
...
class Lbias_expansion(object):
    ...
    By default, the nonlinear Lagrangian bias expansion terms emulator
    (described in Zennaro et al, 2021) and LPT lagrangian bias expansion terms
    emulator (described in Aricò et al, 2021) are loaded.
    Another emulator loaded by deafult is the IR-resummed linear power
    spectrum.

    :param lpt: whether to load the LPT emulator, defaults to True
    :type lpt: boolean, optional
    :param compute_sigma8: whether to load the sigma8 emulator, defaults to
                           True
    ...
    def __init__(self, lpt=True, smeared_bao=True, nonlinear_boost=True,
                 ...
                 nonlinear_emu_field_name='lbias_emu_field',
                 nonlinear_emu_read_rotation=True,
        ...
        if self.compute_nonlinear_boost:
            self.emulator['nonlinear'] = load_nonlinear_lbias_emu(
                ...
                field_name=nonlinear_emu_field_name,
                read_rotation=nonlinear_emu_read_rotation
            )
    ...
    def get_galaxy_real_pk(self, bias=None, omega_cold=None, omega_matter=None,
    ...
    def get_nonlinear_pnn(self, omega_cold=None, omega_matter=None,
        ...
        :param k: a vector of wavemodes in h/Mpc at which the nonlinear boost
                  ...
                  the default wavemodes of the nonlinear emulator will be
        ...
        for n in range(15):
            ...
            if n in n_switch:
                ...
                lpt_term = pk_lpt[n]
            if n in n_log:
                _this_P_nn = np.squeeze(
                    np.exp(emulator['scaler'][n].inverse_transform(prediction))
                    * lpt_term)
            else:
                _this_P_nn = np.squeeze(
                    emulator['scaler'][n].inverse_transform(prediction)
                    * lpt_term)
            _this_P_nn = np.array(_this_P_nn)
            if n in n_switch:
                if len(pk_lpt.shape) == 2:
                    _this_P_nn[:25] = _this_P_nn[:25] * \
                        pk_lpt[n][:25] / lpt_term[:25]
                else:
                    _this_P_nn[:, :25] = _this_P_nn[:, :25] * \
                        pk_lpt[n, :, :25] / lpt_term[:, :25]
        ...
        if k is not None:
            ...
            mink_msg = "WARNING: the nonlinear emulator is extrapolating" \
                + "to k < 0.01 h/Mpc!"
            ...
            for n in range(15):
                ...
                unexpected_negative = np.any(P_nn[n] <= 0.0)
                if (n in n_log) & (unexpected_negative is False):
                    new_P_nn.append(np.exp(interpolate.interp1d(
                        np.log(emulator['k']),
                        np.log(P_nn[n]),
                        kind='cubic',
                    ...
                    new_P_nn.append(interpolate.interp1d(
                        np.log(emulator['k']),
                        P_nn[n], kind='cubic',
                        axis=0 if grid is None else 1,
                        fill_value='extrapolate')(np.log(k)))
    ...
    def get_lpt_pk(self, omega_cold=None, omega_matter=None, omega_baryon=None,
        ...
        :type omega_cold: float or array
        :param omega_matter: omega total matter (cdm + baryons + neutrinos),
                             either omega_cold or omega_matter should
        ...
        :type sigma8_cold: float or array
        :param A_s: primordial scalar amplitude at k=0.05 1/Mpc, either
                    sigma8_cold or A_s should be specified,
                    if both are specified they should be
                    consistent
        :type A_s: float or array
        :param hubble: adimensional Hubble parameters, h=H0/(100 km/s/Mpc)
        ...
        P_nn = []
        for n in range(15):
            pred = emulator['model'][n](pp.reshape(-1, 9))
        ...
        if k is not None:
            ...
            the current value is {max(k)} h/Mpc""")
            mink_msg = "WARNING: the l-bias lpt emulator is extrapolating" + \
                "to k < 0.01 h/Mpc!"
            if (min(k) <= 1e-2) & (self.verbose):
                print(mink_msg)
    ...
    def get_smeared_bao_pk(self, omega_cold=None, omega_matter=None,
        ...
        if k is not None:
            if (max(k) > 30.) | (min(k) < 1e-3):
                ...
                pk_bao = np.exp(interpolate.interp1d(
                    np.log(emulator['k']),
                    np.log(pk_bao),
                    kind='cubic',
                    axis=0 if grid is None else 1,
    ...
    #     Note: no checks are performed to increase speed, be aware
    #     of what you do
    ...
    #     Note: no checks are performed to increase speed, be aware of
    #     what you do
    ...
    #     _pp = _transform_space(
    #         coordinates, space_rotation=False, bounds=emulator['bounds'])

    #     pk_lpt = self._vector_eval_lpt_pk(coordinates)
    #     pk_bao = self._vector_eval_smeared_bao_pk(coordinates)
    #     pk_bao_interp = np.exp(interp1d(
    #         np.log(self.smeared_bao_emulator['k']),
    #         np.log(pk_bao),
    #         kind='cubic')(np.log(emulator['k'])))
    ...
    #                 * lpt_term)
    #         else:
    #             _this_P_nn = emulator['scaler'][n].inverse_transform(
    #                 prediction) * lpt_term
    #         if n in n_switch:
    #             _this_P_nn[:, :25] = _this_P_nn[:, :25] * \
    #                 pk_lpt[n, :, :25] / lpt_term[:, :25]
    ...
    def Hubble(self, **kwargs):
        """Deprecated, use Hubble_latetime_LCDM
        """
        raise RuntimeError("This function is deprecated, use Hubble_latetime_LCDM")


    def Hubble_latetime_LCDM(self, omega_cold=None, omega_matter=None, omega_baryon=None,
                             sigma8_cold=None, A_s=None, hubble=None, ns=None,
                             neutrino_mass=None, w0=None, wa=None, expfactor=None,
                             k=None, **kwargs):
        ...
        :param expfactor: expansion factor a = 1 / (1 + z)
        :type expfactor: float or array
        :return: Hubble function in km / s / Mpc
        :rtype: float or array
        """
        _kwargs = locals()
        kwargs = {key: _kwargs[key]
                  for key in set(list(_kwargs.keys())) - set(['self'])}
        O_m = (kwargs['omega_matter'] if kwargs['omega_matter'] is not None
               else kwargs['omega_cold'])
        O_Lambda = 1 - O_m
        return kwargs['hubble'] * 100 * np.sqrt(O_m / expfactor**3 + O_Lambda)

    def comoving_radial_distance(self, omega_cold=None, omega_matter=None,
                                 omega_baryon=None, sigma8_cold=None, A_s=None,
                                 hubble=None, ns=None, neutrino_mass=None,
        ...
        H_vector = self.Hubble_latetime_LCDM(expfactor=a_vector, **hpars)
        return 3e3 * hpars['hubble']*100 * trapz(1 / H_vector, x=z_vector)
...
def load_lpt_emu(verbose=True):
    ...
    old_names = [(basefold + '/' + "lpt_emulator"),
                 (basefold + '/' + "lpt_emulator_v2.0.0"),
                 (basefold + '/' + "lpt_emulator_v2.0.1")]
    ...
    if (not os.path.exists(emulator_name)):
        ...
        urllib.request.urlretrieve(
            'https://bacco.dipc.org/lpt_emulator_v2.0.2.tar',
            emulator_name + '.tar',
            MyProgressBar())
    ...
    for n in range(15):
        ...
        emulator['model'].append(load_model(
            i_emulator_name+f'/lpt_emu_field{n}.h5'))
        emulator['scaler'].append(StandardScalerSimplified(np.array(nn_details['scaler_mean'])))
...
def load_nonlinear_lbias_emu(emu_type='nn', fold_name=None, detail_name=None,
                             field_name=None, read_rotation=True,
                             verbose=True):
    ...
    if fold_name is None:
        ...
        old_names = [(basefold + '/' + "lbias_emulator"),
                     (basefold + '/' + "lbias_real_1.0.0")]
        for old_name in old_names:
            if os.path.exists(old_name):
                import shutil
                shutil.rmtree(old_name)
        ...
        assert os.path.exists(emulator_name), f'{emulator_name} does not exist'
        msg = f'{os.path.join(emulator_name, detail_name)} does not exist'
        assert os.path.exists(os.path.join(emulator_name, detail_name)), msg
    ...
    if (not os.path.exists(emulator_name)):
        ...
        urllib.request.urlretrieve(
            'https://bacco.dipc.org/lbias_real_1.0.1.tar',
            emulator_name + '.tar',
            MyProgressBar())
        tf = tarfile.open(emulator_name+'.tar', 'r')
        tf.extractall(path=basefold, filter='tar')
    ...
    emulator['model'] = []
    for n in range(15):
        i_emulator_name = f'{emulator_name}/{field_name}{n}'
        emulator['model'].append(load_model((i_emulator_name
                                             + f'/{field_name}{n}.h5')))

    with open(os.path.join(emulator_name, detail_name), 'rb') as f:
        details = pickle.load(f)
    emulator['scaler'] = [StandardScalerSimplified(np.array(details['scaler_mean'][n])) for n in range(15)]
