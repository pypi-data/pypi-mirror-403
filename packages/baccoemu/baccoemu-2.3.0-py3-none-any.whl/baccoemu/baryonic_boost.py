from .utils import MyProgressBar, load_model, StandardScalerSimplified
import numpy as np
import copy
import pickle
...
def load_baryonic_emu_Arico2021(fold_name=None, detail_name=None, verbose=True):
    """Loads in memory the baryonic boost emulator, described in Aricò et al. 2021.
    ...
    import os
    if verbose:
        print('Loading Baryonic Emulator Arico et al. (2021)...')
    ...
    if fold_name is not None:
        ...
        if (not os.path.exists(emulator_name)):
            ...
            urllib.request.urlretrieve(
                'https://bacco.dipc.org/baryon_emu_1.0.2.tar',
                emulator_name + '.tar',
                MyProgressBar())
    ...
    emulator['model'] = load_model(emulator_name + '/baryon_emu_1.0.2.h5')
    detail_name = 'details.pickle' if detail_name is None else detail_name
    ...
    if fold_name is None:
        ...
        emulator['keys'] = ['omega_cold', 'sigma8_cold', 'omega_baryon',
                            ...
                            'theta_inn', 'M_inn', 'theta_out', 'expfactor']
    if verbose:
        print('Baryonic Emulator Arico et al. (2025) loaded in memory.')
    return emulator


def load_baryonic_emu_Burger2025(fold_name=None, verbose=True):
    """Loads in memory the baryonic boost emulator, described in Burger et al. 2025.
    ...
    import os
    if verbose:
        print('Loading Baryonic Emulator Burger et al. (2025)...')
    basefold = os.path.dirname(os.path.abspath(__file__))
    ...
    if fold_name is not None:
        ...
        if (not os.path.exists(emulator_name)):
            ...
            print('Downloading Emulator data (5 Mb)...')
            urllib.request.urlretrieve(
                'https://bacco.dipc.org/bispectrum_baryonic_boost.zip',
                emulator_name + '.zip',
                MyProgressBar())

            with zipfile.ZipFile(emulator_name + '.zip', 'r') as zf:
                zf.extractall(path=basefold)

            os.remove(emulator_name + '.zip')

        emulator_name = emulator_name + "/powerspectra_ratio_bcm_emu_folded.pkl"

    emulator = {}
    emulator['emu_type'] = 'nn'

    emu = EmulatorJAX(filepath=emulator_name,verbose=verbose)
    emulator['model'] = emu.rescaled_predict
    emulator['trained_keys'] = list(set(emu.parameters) - {'k'})
    bounds = []
    for _key in emulator['trained_keys']:
        bounds.append(emu.parameter_ranges[_key])
    emulator['bounds'] = np.array(bounds)


    emulator['keys'] = [ tp.replace('Omega_m','omega_cold')
                             .replace('Omega_b','omega_baryon')
                               .replace('sigma8','sigma8_cold')
                                        for tp in emulator['trained_keys']]

    emulator['k_range'] = emu.parameter_ranges['k']

    if verbose:
        print('Baryonic Emulator for matter powerspectrum Burger et al. (2025) loaded in memory.')
    return emulator



def load_bispecrum_baryonic_emu(fold_name=None, verbose=True):
    """Loads in memory the baryonic boost emulator, described in Burger et al. 2025.
    ...
    if fold_name is not None:
        ...
        if (not os.path.exists(emulator_name)):
            ...
            # print('Downloading Emulator data (5 Mb)...')
            # urllib.request.urlretrieve(
            #     'https://bacco.dipc.org/bispectrum_baryonic_boost.tar',
            #     emulator_name + '.tar',
            #     MyProgressBar())
            # tf = tarfile.open(emulator_name+'.tar', 'r')
        ...
        emulator_name = emulator_name + "/bispectra_ratio_bcm_emu_folded.pkl"
    ...
    emu = EmulatorJAX(filepath=emulator_name,verbose=verbose)
    emulator['model'] = emu.rescaled_predict
    emulator['trained_keys'] = list(set(emu.parameters) - {'k1','k2','k3'})
    bounds = []
    for _key in emulator['trained_keys']:
        bounds.append(emu.parameter_ranges[_key])
    emulator['bounds'] = np.array(bounds)
    ...
    if verbose:
        print('Baryonic Emulator for matter bispectrum Burger et al. (2025) loaded in memory.')
    return emulator
...
def get_baryon_fractions(M_200c, omega_cold=None, omega_matter=None,
    ...
    :return: a dictionary containing the baryonic components mass fractions,
             with the following keywords: 'ej_gas' -> ejected gas,
             'cen_galaxy' -> central galaxy, 'sat_galaxy' -> satellite galaxy,
             'bo_gas' -> bound gas, 're_gas' -> reaccreted gas,
             'dark_matter' -> dark matter, 'gas' -> total gas,
             'stellar' -> total stars, 'baryon' -> total baryons
