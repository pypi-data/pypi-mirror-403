...
def test_run_job_global_log_prefix(tmp_path):
    script = _make_script(tmp_path, "printf 'line1\\nline2\\n'")
    cfg = _make_cfg(tmp_path, script)
    ...
    global_log = tmp_path / "logs" / "all.log"
    lines = [line for line in global_log.read_text(encoding="utf-8").splitlines() if "line" in line]
    assert len(lines) == 2
    assert all("test_job | line" in line for line in lines)
