...
def run_instance(
    ...
    try:
        ...
        if run_perf:
            ...
            if (not run_perf_profiling) and (
                process_isolation and instance_id not in ISOLATION_CHECK_EXCEPTIONS
            ):
...
def get_dataset_from_preds(
    ...
    for instance in dataset:
        ...
        perf_profiling_postedit_file = report_dir / "perf_profiling_output_postedit.txt"
        ...
        if (
            run_perf_profiling
            or prediction[KEY_INSTANCE_ID] in ISOLATION_CHECK_EXCEPTIONS
        ):
        ...
        conditions = [
            (run_correctness, covering_tests_file.exists()),
            (run_perf, perf_summary_file.exists()),
            (run_perf_profiling, perf_profiling_postedit_file.exists()),
        ]
        if all(not run_cond or exist_cond for run_cond, exist_cond in conditions):
...
def main(
    ...
    dataset = get_dataset_from_preds(
        ...
        run_perf=run_perf,
        run_perf_profiling=run_perf_profiling,
    )
