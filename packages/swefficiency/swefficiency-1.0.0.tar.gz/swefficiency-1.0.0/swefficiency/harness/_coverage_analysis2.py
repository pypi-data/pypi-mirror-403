            ...
            if line.line_type == LINE_TYPE_REMOVED:
                ...
                if remove_chunk_first_surviving_line is not None:
                    ...
                    if start_line_range is None or end_line_range is None:
                        print(
                            "Warning: start_line_range or end_line_range is None, skipping this range."
                        )
                        print(start_line_range, end_line_range)
                        continue

                    post_edit_lines_of_interest.extend(
                        list(range(start_line_range, end_line_range + 1))
                    )
                    remove_chunk_first_surviving_line = None
...
def get_lines_to_ignore(patched_file_text):
    ...
    for node in ast.walk(atok.tree):
        ...
        # CASE 2: Ignore changes to private assignments like "__all__" etc.
            ...
            isinstance(node, t) for t in (ast.Assign, ast.AnnAssign, ast.AugAssign)
        ):
...
def get_definition_nodes(patch_set: PatchSet):
    ...
    for patched_file in patch_set:
        ...
        for hunk_lines_of_interest, hunk in zip(
            post_edit_lines_of_interest, patched_file
        ):
            filtered_lines = [
                line for line in hunk_lines_of_interest if line not in ignore_lines
            ]
            ...
            for node in ast.walk(atok.tree):
                ...
                if not any(
                    isinstance(node, t) for t in (ast.FunctionDef, ast.ClassDef)
                ):
                    continue
            ...
            if latest_node is not None:
                ...
                patched_file_to_relevant_hunks[abs_path_patched_file].append(
                    (latest_node, latest_node_span)
                )

    return (
        patched_file_to_relevant_hunks,
        original_lines_of_interest,
        patched_file_to_lines_to_ignore,
    )


def get_lines_of_interest(
    definition_nodes, original_lines_of_interest, lines_to_ignore
):
...
definition_nodes, original_lines_of_interest, lines_to_ignore = get_definition_nodes(
    patch_set
)
lines_of_interest = get_lines_of_interest(
    definition_nodes, original_lines_of_interest, lines_to_ignore
)
...
def handle_coverage_file(coverage_filepath):
    ...
    for file_path, relevant_lines in lines_of_interest.items():
        try:
            ...
            intersection = lines_executed_in_covered_file_set.intersection(
                lines_of_interest_set
            )
