...
def compare_trees(tree1, tree2, node_ignore_fn=None) -> bool:
    ...
    tree1_children = [
        n for n in tree1.children if node_ignore_fn is None or not node_ignore_fn(n)
    ]
    tree2_children = [
        n for n in tree2.children if node_ignore_fn is None or not node_ignore_fn(n)
    ]
...
argparser.add_argument(
    "--postedit_file", type=str, required=True, help="Post-edit file."
)
...
trees_equal = compare_trees(
    preedit_tree.root_node, postedit_tree.root_node, NODE_IGNORE_FN
)
