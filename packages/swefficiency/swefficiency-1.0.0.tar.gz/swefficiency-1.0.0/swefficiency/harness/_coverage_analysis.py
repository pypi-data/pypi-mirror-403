        ...
        for patched_file in patch_set:
            ...
            for hunk in patched_file:
                ...
                for l in lines_executed_in_covered_file:
                    ...
                    if any(
                        file_lines[l - 1].strip().startswith(kw)
                        for kw in ["class", "def", "#"]
                    ):
                        continue
                ...
                actual_lines_executed_in_covered_file = set(
                    actual_lines_executed_in_covered_file
                )

                # Check for intersection.
                hunk_intersects_with_coverage = (
                    actual_lines_executed_in_covered_file.intersection(
                        post_edit_lines_of_interest
                    )
                )
                if hunk_intersects_with_coverage:
                    ...
                    covered_lines = [
                        lines[i - 1] for i in hunk_intersects_with_coverage
                    ]
