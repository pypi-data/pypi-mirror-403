...
def parse_patch(patch_text):
    ...
    for raw in patch_text.splitlines():
        if raw.startswith("--- "):
            ...
            if (
                new_path
                and os.path.splitext(new_path)[1] in PY_EXTS
...
class IntrospectionScanner(ast.NodeVisitor):
    def __init__(self, src_lines, path):
        self.src, self.path, self.findings, self.imps = (
            src_lines,
            path,
            [],
            ImportResolver(),
        ...
        self.imported_modules = (
            set()
        )  # collected imports in this file (for reference graph)
    ...
    def visit_Import(self, node):
        ...
        for a in node.names:
            ...
            root = a.name.split(".")[0]
            if root in ("inspect", "traceback") or (
                FLAG_NATIVE_IMPORTS and root in ("ctypes", "cffi")
            ):
    ...
    def _resolve_callee(self, f):
        ...
        if isinstance(f, ast.Attribute):
            ...
            if isinstance(left, ast.Name):
                base = left.id
                mod = self.imps.mod_alias.get(base, base)
    ...
    def _suppressed(self, line_no):
        try:
            return SUPPRESS_PRAGMA in self.src[line_no - 1]
        except Exception:
            return False
    ...
    def _add(self, node, kind, qual, lineno, col):
        ...
        snippet = ""
        try:
            snippet = self.src[lineno - 1].rstrip()
        except Exception:
            pass
    ...
    if source == "worktree":
        ...
        try:
            return subprocess.check_output(
                ["git", "show", ":" + path], universal_newlines=True
            )
...
def main():
    ap = argparse.ArgumentParser(
        description="Report stack-introspection on ADDED lines from a patch (Py3.5+)."
    )
    ap.add_argument(
        "--patch-file", required=True, help="Unified diff; use '-' for stdin."
    )
    ap.add_argument(
        "--postimage-from",
    ...
    patch_text = (
        sys.stdin.read()
        if args.patch_file == "-"
        else open(args.patch_file, "r", encoding="utf-8").read()
    )
    ...
    if not args.include_standalone_added:
        unref_added = determine_unreferenced_added(
            new_files, touched_files, args.postimage_from
        )
    ...
    for f in filtered:
        print(
            "{path}:{line}:{col}: [{kind} {qual}]".format(
