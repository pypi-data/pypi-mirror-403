...
def get_releases_and_oldest_commits(repo_owner, repo_name, github_token=None):
    ...
    per_page_count = 100
    print(
        f"Fetching releases for {repo_owner}/{repo_name} (up to {per_page_count} per page)..."
    )
    ...
    while True:
        ...
        try:
            ...
            if page == 1:  # If the first page fails, likely a repo-level or auth issue
                return [
                    {
                        "error": f"Could not fetch releases for {repo_owner}/{repo_name}: {e.response.text if e.response else e}"
                    }
                ]
            break  # Stop if a subsequent page fails, assuming prior pages were processed.
        except (
            requests.exceptions.RequestException
        ) as e:  # Catch other request exceptions like network errors
            error_msg = f"Network or request error fetching releases (page {page}): {e}"
            print(error_msg)
            if page == 1:
                return [
                    {
                        "error": f"Could not fetch releases for {repo_owner}/{repo_name}: {e}"
                    }
                ]
            break
        ...
        for release in current_page_releases:
            ...
            published_at_display = (
                published_at_raw if published_at_raw else "Draft or Not Published"
            )
            ...
            if tag_name == "N/A" or not tag_name:  # Ensure tag_name is valid
                print(
                    f"Skipping release '{release_name}' due to missing or invalid tag_name."
                )
            ...
            try:
                ...
                commit_details = get_commit_details_for_tag(
                    repo_owner, repo_name, tag_name, headers
                )
                ...
                requests.exceptions.RequestException
            ) as e:  # Catch other request exceptions
                ...
                error_message = (
                    f"An unexpected error occurred for tag '{tag_name}': {e}"
                )
                print(error_message)
                release_info["error"] = error_message
    ...
    if (
        not all_releases_info
        and page == 1
        and not any(item.get("error") for item in all_releases_info)
    ):
...
if __name__ == "__main__":
    ...
    if not github_token:
        ...
        print(
            "This may lead to lower rate limits or inability to access private repositories."
        )
        if (
            input("Do you want to provide a token now? (yes/no): ").strip().lower()
            == "yes"
        ):
    ...
    if not repo_owner or not repo_name:
        ...
        release_commit_data = get_releases_and_oldest_commits(
        ...
        if release_commit_data:
            ...
            if (
                len(release_commit_data) == 1
                and "error" in release_commit_data[0]
                and release_commit_data[0].get("release_name") is None
            ):
                ...
                for info in release_commit_data:
                    ...
                    if info.get("commit_author_date"):
                        print(
                            f"Commit Date:  {info['commit_author_date']} (Author Date)"
                        )
