...
/// \brief A non-recoverable panic from Rust
///
/// We throw this error when a Rust plugin panics. Due to the way
/// the exception propagation works from Rust currently, there is
/// no way to keep the program alive and we have to terminate when
/// we catch this error.
struct RustError : public LowlevelError {
  /// Initialize the error with an explanatory string
  RustError(const string &s) : LowlevelError(s) {}
};

/// \brief A non-recoverable panic caused by a Pipeline change
///
/// We throw this error when the pipeline has changed in the manager
/// application, causing the user to refresh the decompiler.
struct PipelineChangedError : public LowlevelError {
  /// Initialize the error with an explanatory string
  PipelineChangedError(const string &s) : LowlevelError(s) {}
};
