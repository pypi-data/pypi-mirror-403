...
/// \brief Hook for changing the behavior of the universal action creation
///
/// This is meant to implement a plugin interface to allow external tools to
/// override the behavior for creating the universal action. Currently this
/// has to be done at the very beginning when everything is initialized and
/// changing the universal action while everything is running is currently
/// not allowed.
class UniversalActionHook {
public:
  virtual void universalAction(Architecture *conf, ActionGroup* root) = 0;
};

extern UniversalActionHook *universalact_hook;
