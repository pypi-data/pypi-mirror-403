...
class RNN(nn.Module):
    def __init__(self, num_series, hidden, nonlinearity):
        ...
        # Set up network.
        self.rnn = nn.RNN(
            num_series, hidden, nonlinearity=nonlinearity, batch_first=True
        )
    ...
    for it in range(max_iter):
        ...
        if (it + 1) % check_every == 0:
            ...
            if verbose > 0:
                ...
                print("Variable usage = %.2f%%" % (100 * torch.mean(crnn.GC().float())))
...
def train_model_adam(
    ...
    for it in range(max_iter):
        ...
        if lam_ridge > 0:
            loss = loss + sum(
                [ridge_regularize(net, lam_ridge) for net in crnn.networks]
            )
...
def train_unregularized(
    crnn, X, context, lr, max_iter, lookback=5, check_every=50, verbose=1
):
    """Train model with Adam."""
    p = X.shape[-1]
    loss_fn = nn.MSELoss(reduction="mean")
