...
def cleanup_duplicate_env_entries(
    env_var: EnvVariable,
    profile_path: Path,
    current_content: str,
) -> str:
    """
    Remove duplicate/old entries for an environment variable.

    Args:
        env_var: Environment variable to clean up.
        profile_path: Path to shell profile.
        current_content: Current profile content.

    Returns:
        Cleaned content with duplicates removed.
    """
    # Remove all existing exports for this variable (to prevent duplicates)
    if env_var.append_to_path:
        # Remove AiCippy PATH markers and their exports
        marker_pattern = re.compile(
            r'# AiCippy PATH\n[^\n]*\n?',
            re.MULTILINE
        )
        current_content = marker_pattern.sub('', current_content)
    else:
        # Remove all export lines for this variable
        export_pattern = re.compile(
            rf'^export\s+{re.escape(env_var.name)}=.*$\n?',
            re.MULTILINE
        )
        current_content = export_pattern.sub('', current_content)
...
def set_env_variable_unix(
    ...
    Automatically cleans up any existing/duplicate entries before setting.
    ...
    try:
        ...
        # CLEANUP: Remove any duplicate/old entries first (auto-cleanup feature)
        current_content = cleanup_duplicate_env_entries(env_var, profile_path, current_content)

        # Write cleaned content back
        if profile_path.exists():
            profile_path.write_text(current_content)

        # Now proceed with setting the new value
        export_pattern = re.compile(
            rf'^export\s+{re.escape(env_var.name)}=',
            re.MULTILINE
        )
