...
def install_python_packages(
    ...
    base_flags = ["--quiet", "--disable-pip-version-check"] if silent else []
    ...
    if upgrade_pip:
        ...
        try:
            cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "pip"] + base_flags
            result = subprocess.run(
                cmd,
                capture_output=True,
                text=True,
                timeout=120,
                stdin=subprocess.DEVNULL,  # Auto-accept by not waiting for input
            )
            if result.returncode != 0:
    ...
    if packages:
        ...
        if progress_callback:
            progress_callback(f"Installing {len(packages)} packages...", int(current / total * 50))

        try:
            cmd = [
                sys.executable, "-m", "pip", "install", "--upgrade",
                *base_flags,
                "--no-warn-script-location",  # Suppress script warnings
                *package_specs
            ]
            result = subprocess.run(
                cmd,
                ...
                stdin=subprocess.DEVNULL,  # Auto-accept
            )
            if result.returncode != 0:
            ...
            errors.append(f"Error during bulk install: {e}")
