...
### Fixed
- **Multi-tenancy Support**: Refactored `IRISExecutor` to support per-session namespaces. The `database` parameter in the PostgreSQL `StartupMessage` is now correctly honored, allowing clients to specify their target IRIS namespace.
- **Embedded Namespace Context**: Added missing `SetNamespace` calls to the batch execution path (`executemany`) in embedded mode, ensuring consistent namespace context for bulk operations.
- **Namespace Isolation**: Mitigated potential race conditions in embedded mode by ensuring explicit namespace switching before every query execution.
