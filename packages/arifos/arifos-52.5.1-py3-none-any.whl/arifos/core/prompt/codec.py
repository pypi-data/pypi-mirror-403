"""
@PROMPT Codec Layer (arifos/core/prompt/codec.py)
...
class SignalExtractor:
    ...
    def _route_to_engine(self, intent: IntentType, risk: RiskLevel, reversible: bool) -> EngineRoute:
...
if __name__ == "__main__":
    ...
    # Extract signal from user input
    extractor = SignalExtractor()
    
    user_input = "Can I delete all old logs from the database?"
    signal = extractor.extract(user_input)
    
    print("=" * 60)
    print("USER INPUT (Natural Language)")
    print("=" * 60)
    print(signal.raw_input)
    print()
    
    print("=" * 60)
    print("EXTRACTED SIGNAL (@PROMPT Decode)")
    print("=" * 60)
    print(f"Intent: {signal.intent}")
    print(f"Risk Level: {signal.risk_level}")
    print(f"Reversible: {signal.reversible}")
    print(f"Route to: {signal.engine_route}")
    print(f"Hidden Assumptions: {signal.hidden_assumptions}")
    print()
    
    print("=" * 60)
    print("ROUTING DECISION")
    print("=" * 60)
    print(f"Engine(s) to invoke: {signal.engine_route}")
    print(f"(Would call: {signal.engine_route}_tool)")
    print()
    
    print("=" * 60)
    print("VERDICT (from engine) → Response")
    print("=" * 60)
    formatter = ResponseFormatter()
    response = formatter.encode_response(
        verdict="SABAR",
        floor="F1 Amanah (Reversibility)",
        reason="Deletion is irreversible without backups",
        engine=EngineRoute.APEX
    )
    print(response.human_readable)
    print(f"Next Step: {response.suggested_action}")
    print()
    print("=" * 60)
    print("JSON Response (for machine processing)")
    print("=" * 60)
