...
class Register(Command):
    ...
    async def preferences(self, user: GatewayUser) -> Form:
        return Form(
            ...
            timeout_handler=functools.partial(self._preferences_timeout, user=user),  # type:ignore
        )

    def _preferences_timeout(self, user: GatewayUser) -> None:
        self.xmpp.event("user_register", Iq(sfrom=user.jid.bare))
        self.xmpp.store.users.update(user)
        self.xmpp.send_message(
            mfrom=self.xmpp.boundjid.bare,
            mto=user.jid,
            mbody="You did not choose your preferences in time, falling back to defaults. "
            "You can change preferences later with the 'preferences' command.\n"
