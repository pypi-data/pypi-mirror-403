...
from .storage import (
    ...
    set_api_key, get_api_key, load_registry,
    WeekState, Task, TaskEstimation, TaskSignals, WORKPLAN_DIR
)
...
def plan(goal_description: str = typer.Argument(..., help="The goal for this week")):
    ...
    with console.status("[bold blue]Stage 1-9: Expanding intent and grounding context...", spinner="dots"):
        ...
        # Calculate planned points immediately
        total_points = sum(t.estimation.points for t in week.tasks)
        week.metrics.planned_points = total_points
...
@app.command()
def restore(backup_id: Optional[str] = typer.Argument(None, help="Backup ID (timestamp) to restore")):
    """Restores a previous weekly plan from backup."""
    workplan_path = Path.cwd() / WORKPLAN_DIR
    if not workplan_path.exists():
        console.print("[red].workplan directory not found.[/red]")
        return

    backups = sorted(list(workplan_path.glob("week_backup_*.json")), reverse=True)
    ...
    if backup_id:
        # Check if user provided a simple index (e.g. "1")
        if backup_id.isdigit() and len(backup_id) < 5:
            try:
                idx = int(backup_id) - 1
                if 0 <= idx < len(backups):
                    selected_backup = backups[idx]
                else:
                    console.print(f"[red]ID {backup_id} is out of range (1-{len(backups)}).[/red]")
                    return
            except ValueError:
                pass
        
        # If not matched by index, try to find specific backup ID
        if not selected_backup:
            for bk in backups:
                if backup_id in bk.name:
                    selected_backup = bk
                    break
            if not selected_backup:
                console.print(f"[red]Backup ID '{backup_id}' not found.[/red]")
                return
    else:
        # Interactive Mode
        ...
        for i, bk in enumerate(backups[:10]):  # Show last 10
            try:
                with open(bk, 'r') as f:
                    data = json.load(f)
                    ts = bk.name.replace("week_backup_", "").replace(".json", "")
                    date_str = datetime.strptime(ts, "%Y%m%d_%H%M%S").strftime("%Y-%m-%d %H:%M")
                    goal = data.get("goal", "Unknown")[:40] + "..."
                    task_count = str(len(data.get("tasks", [])))
                    
                    # Use simple index for selection
                    idx = str(i + 1)
                    backup_map[idx] = bk
                    table.add_row(idx, date_str, goal, task_count)
            except Exception:
                continue

        console.print(table)
        choice = typer.prompt("Select backup to restore (ID)", default="1")
        selected_backup = backup_map.get(choice)
    ...
    # Confirmation
    console.print(f"\n[bold yellow]WARNING:[/bold yellow] This will overwrite your current 'week.json' with contents from {selected_backup.name}.")
    if not typer.confirm("Are you sure?"):
        console.print("Restore cancelled.")
        return

    try:
        with open(selected_backup, 'r') as src, open(workplan_path / "week.json", 'w') as dst:
            dst.write(src.read())
        console.print(f"[bold green]✓ Successfully restored plan from {selected_backup.name}[/bold green]")
        details(project=None)
    except Exception as e:
        console.print(f"[red]Error restoring backup: {e}[/red]")

@app.command()
def export(filename: Optional[str] = typer.Argument(None, help="Output filename (e.g. status.csv)"), format: str = typer.Option("csv", help="Export format (csv or json)")):
...
def task(task_id: str, project: Optional[str] = typer.Option(None, help="Project name to view task from")):
    ...
    if selected_task.microtasks:
        table = Table(title=f"Daily Microtasks", box=None)
        table.add_column("ID", style="cyan", width=8)
        table.add_column("Step", style="white")
        table.add_column("Status", style="dim", width=10)
        
        for m in selected_task.microtasks:
            status_icon = "✅" if m.status == "done" else "⭕"
            table.add_row(m.id, m.title, f"{status_icon} {m.status}")
            
        console.print(table)

@app.command()
def decompose(task_id: str):
    """Decomposes a complex task into actionable daily microtasks using AI."""
    week = load_week()
    if not week:
    ...
    if selected_task.microtasks:
        console.print(f"[yellow]Task {selected_task.task_id} already has {len(selected_task.microtasks)} microtasks.[/yellow]")
        if not typer.confirm("Do you want to regenerate them?"):
            return

    with console.status(f"[bold blue]Decomposing {selected_task.task_id} into daily steps...", spinner="dots"):
        context = get_project_context()
        ai_client = WorkplanAIClient()
        microtasks_data = ai_client.decompose_task(selected_task.title, selected_task.description, context)
    
    if not microtasks_data:
        console.print("[red]Failed to generate microtasks.[/red]")
        return

    from .storage import Microtask # Ensure Microtask is imported available
    new_microtasks = []
    for m in microtasks_data:
        new_microtasks.append(Microtask(id=m.get("id"), title=m.get("title"), status="todo"))
    
    selected_task.microtasks = new_microtasks
    save_week(week)
    
    table = Table(title=f"Microtasks for {selected_task.task_id}")
    table.add_column("ID", style="cyan")
    table.add_column("Step", style="white")
    table.add_column("Status", style="dim")
    
    for m in new_microtasks:
        table.add_row(m.id, m.title, m.status)
        
    console.print(table)
    console.print("[green]Microtasks saved! Future check-off command coming soon.[/green]")

@app.command()
def update(task_id: str, status: str = typer.Option(..., "--status", "-s", help="New status: todo, in_progress, done, blocked"), mt: Optional[str] = typer.Option(None, "--microtask", "-m", help="Microtask ID to update")):
    """Manually updates the status of a task or microtask."""
    week = load_week()
    if not week:
        console.print("[yellow]No goal set for this week.[/yellow]")
        return
    ...
    # Handle Microtask Update
    if mt:
        if not selected_task.microtasks:
            console.print(f"[yellow]Task {task_id} has no microtasks. Run `workplan decompose {task_id}` first.[/yellow]")
            return
            
        selected_mt = next((m for m in selected_task.microtasks if m.id.lower() == mt.lower()), None)
        # Fallback for just numeric ID
        if not selected_mt:
            selected_mt = next((m for m in selected_task.microtasks if m.id.split("-")[-1] == mt), None)
            
        if not selected_mt:
            console.print(f"[red]Microtask {mt} not found in {task_id}.[/red]")
            return
            
        selected_mt.status = status.lower()
        save_week(week)
        console.print(f"[bold green]Updated Microtask {selected_mt.id} to {status.lower()}[/bold green]")
        return

    # Handle Parent Task Update
    selected_task.status = status.lower()
    save_week(week)
    ...
    console.print(f"[bold green]Updated {selected_task.task_id} to[/bold green] [{status_color}]{selected_task.status}[/{status_color}]")
...
@app.command(name="ls", help="Alias for 'details' (List tasks)")
def ls(project: Optional[str] = typer.Option(None, help="Project name to load details from")):
    if isinstance(project, typer.models.OptionInfo):
        project = None
    details(project)

@app.command(name="ps", help="Alias for 'dashboard' (List active projects)")
def ps():
    dashboard()

@app.command(name="inspect", help="Alias for 'task' (Inspect task details)")
def inspect(task_id: str, project: Optional[str] = typer.Option(None, help="Project name")):
    task(task_id, project)

@app.command(name="p", help="Alias for 'plan'")
def p(goal: str = typer.Argument(..., help="The one-line goal for the week")):
    plan(goal)

@app.command(name="u", help="Alias for 'update'")
def u(task_id: str, status: str = typer.Option(..., "--status", "-s", help="New status"), mt: Optional[str] = typer.Option(None, "--microtask", "-m", help="Microtask ID")):
    update(task_id, status, mt)

@app.command(name="s", help="Alias for 'sync'")
def s():
    sync()
