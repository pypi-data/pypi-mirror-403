...
class StashStore:
    ...
    def get_raw(self, key: str) -> Optional[str]:
        """Retrieve a value by key without updating access stats."""
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute(
                "SELECT value FROM entries WHERE key = ?", (key,)
            )
    ...
    def list(self, entry_type: Optional[str] = None, 
        ...
        with sqlite3.connect(self.db_path) as conn:
            if entry_type:
                ...
                """
                if limit is None:
                    cursor = conn.execute(base_sql, (entry_type,))
                else:
                    cursor = conn.execute(base_sql + " LIMIT ?", (entry_type, limit))
            else:
                base_sql = """
                    SELECT key, entry_type,
                           LENGTH(value) as size,
    ...
    def search(self, pattern: str, limit: int = 50) -> List[Tuple[str, str]]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            try:
                ...
                raise ValueError(f"Invalid search pattern for FTS: {exc}") from exc
    ...
    def export_entries(self) -> List[Dict[str, Any]]:
        """Export all entries with metadata for backup/migration."""
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute("""
                SELECT key, value, entry_type, metadata,
                       created_at, updated_at, access_count
