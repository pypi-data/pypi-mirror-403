...
def cmd_peek(args, store: StashStore) -> int:
    ...
    if args.range:
        try:
            ...
            print("✗ Invalid range. Use 'start:end' or 'start'.", file=sys.stderr)
            return 1
...
def cmd_version(args, store: StashStore) -> int:
    """Print version."""
    print(__version__)
    return 0


def cmd_export(args, store: StashStore) -> int:
    ...
    if args.output:
        ...
        print(f"✓ Exported {len(data['entries'])} entries to {args.output}")
    else:
        print(output)
    return 0


def cmd_import(args, store: StashStore) -> int:
    ...
    for e in entries:
        key = e.get("key")
        value = e.get("value")
        entry_type = e.get("type", "kv")
        metadata = e.get("metadata", {})
        if key is None or value is None:
            continue
        store.set(key, value, entry_type=entry_type, metadata=metadata)
        imported += 1
...
def cmd_ctx_list(args, store: StashStore) -> int:
    """List contexts."""
    ctx = ContextManager(store)
    contexts = ctx.list_contexts()
    if not contexts:
    ...
    for c in contexts:
        print(f"{c['context_id']:<30} {format_size(c['size']):>8}  (accessed {c['access_count']}x)")
    return 0


def cmd_ctx_info(args, store: StashStore) -> int:
    """Show context info."""
    ctx = ContextManager(store)
    info = ctx.info(args.context_id)
    if not info:
        print(f"✗ Context not found: {args.context_id}", file=sys.stderr)
        return 1
    ...
    print(f"Context: {info['context_id']}")
    for k, v in meta.items():
        print(f"  {k}: {v}")
    return 0


def cmd_ctx_rm(args, store: StashStore) -> int:
    """Delete a context."""
    ctx = ContextManager(store)
    if ctx.forget(args.context_id):
        print(f"✓ Removed context '{args.context_id}'")
        return 0
    print(f"✗ Context not found: {args.context_id}", file=sys.stderr)
    return 1
...
Examples:
  ...
  stash forget logs                  Delete an entry
    ...
    parser.add_argument("--version", action="version", version=f"stash {__version__}")
    ...
    # export
    p_export = subparsers.add_parser("export", help="Export stash to JSON")
    p_export.add_argument("--output", "-o", help="Output file (default: stdout)")

    # import
    p_import = subparsers.add_parser("import", help="Import stash from JSON")
    p_import.add_argument("file", help="JSON file to import")

    # ctx-list
    p_ctx_list = subparsers.add_parser("ctx-list", help="List contexts")
    p_ctx_list.add_argument("--json", action="store_true", help="Output as JSON")

    # ctx-info
    p_ctx_info = subparsers.add_parser("ctx-info", help="Show context info")
    p_ctx_info.add_argument("context_id", help="Context ID")
    p_ctx_info.add_argument("--json", action="store_true", help="Output as JSON")

    p_ctx_rm = subparsers.add_parser("ctx-rm", help="Remove a context")
    p_ctx_rm.add_argument("context_id", help="Context ID")
    ...
    commands = {
        ...
        "export": cmd_export,
        "import": cmd_import,
        "ctx-list": cmd_ctx_list,
        "ctx-info": cmd_ctx_info,
        "ctx-rm": cmd_ctx_rm,
    }
