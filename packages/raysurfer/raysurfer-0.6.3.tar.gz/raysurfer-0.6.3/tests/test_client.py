...
class TestAuthenticationErrors:
    ...
    async def test_async_auth_error_401(self, httpx_mock):
        ...
        async with AsyncRaySurfer(
            api_key="invalid-key", base_url="http://test.local"
        ) as client:
    ...
    async def test_async_missing_api_key(self, httpx_mock):
        ...
        httpx_mock.add_response(
            json={"code_blocks": [], "total_found": 0}, status_code=200
        )
...
class TestAPIErrors:
    ...
    async def test_async_api_error_400(self, httpx_mock):
        ...
        async with AsyncRaySurfer(
            api_key="test-key", base_url="http://test.local"
        ) as client:
...
class TestTimeouts:
    ...
    async def test_async_timeout_error(self, httpx_mock):
        ...
        async with AsyncRaySurfer(
            api_key="test-key", base_url="http://test.local", timeout=0.001
        ) as client:
...
class TestRequestHeaders:
    ...
    async def test_async_auth_header(self, httpx_mock):
        ...
        async with AsyncRaySurfer(
            api_key="my-secret-key", base_url="http://test.local"
        ) as client:
            await client.get_code_snips(task="test task")
