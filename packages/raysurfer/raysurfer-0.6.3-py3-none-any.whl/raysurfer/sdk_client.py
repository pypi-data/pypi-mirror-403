...
class RaysurferClient:
    ...
    def __init__(
        ...
        Args:
            ...
            namespace: Custom namespace for code storage/retrieval
            ...
            if response.files:
                self._debug.table(
                    [
                        {
                            "filename": f.filename,
                            "similarity": f"{f.similarity_score * 100:.0f}%",
                            "verdict": f"{f.verdict_score * 100:.0f}%",
                            "combined": f"{f.combined_score * 100:.0f}%",
    ...
    async def _upload_to_cache(self) -> None:
        ...
        try:
            ...
            result = await self._raysurfer.upload_new_code_snips(
            ...
            if result.code_blocks_stored > 0:
                self._debug.log(f"Cached {result.code_blocks_stored} code blocks")
