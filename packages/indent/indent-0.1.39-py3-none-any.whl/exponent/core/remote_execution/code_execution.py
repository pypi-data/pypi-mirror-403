...
def _truncate_shell_output(content: str) -> tuple[str, bool]:
    """Truncate shell output to BASH_CHARACTER_LIMIT, writing full output to a temp file."""
    if len(content) <= BASH_CHARACTER_LIMIT:
        return content, False

    file_path = _write_full_output_to_file(content)
    ...
    return truncation_msg + truncated_value, True
    ...
    if request.language == "python":
        ...
        async for shell_output in execute_shell_streaming(
            ...
            if isinstance(shell_output, StreamedOutputPiece):
                ...
                truncated_content, was_truncated = _truncate_shell_output(content)
                yield StreamingCodeExecutionResponse(
                    correlation_id=request.correlation_id,
