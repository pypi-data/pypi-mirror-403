...
describe("Tool Notifications", () => {
  ...
  test("sends tools/list_changed notification after restart", async () => {
    const serverPath = createDummyServer(tempDir);
    const watchedFile = join(tempDir, "watched.py");
    writeFileSync(watchedFile, "# initial");
    ...
    const logContent = await Bun.file(logFile).text();
    expect(logContent).toContain("Sent tools/list_changed notification");
  });

  test("help mentions tools notification feature", async () => {
    const proc = spawn({
      cmd: ["bun", "mcpmon.ts", "--help"],
      stdout: "pipe",
      stderr: "pipe",
...
describe("Gateway Mode", () => {
  ...
  test("help shows --config option", async () => {
    const proc = spawn({
      cmd: ["bun", "mcpmon.ts", "--help"],
      stdout: "pipe",
      stderr: "pipe",
  ...
  test("help shows both usage modes", async () => {
    const proc = spawn({
      cmd: ["bun", "mcpmon.ts", "--help"],
      stdout: "pipe",
      stderr: "pipe",
    ...
    expect(output).toContain("Single server:");
    expect(output).toContain("Multi gateway:");
  });
});
