...
class TestSendToolsListChanged:
    """Test send_tools_list_changed function."""

    def test_outputs_correct_json(self, capsys, monkeypatch):
        """send_tools_list_changed should output correct JSON-RPC notification."""
        import json
        from mcpmon import log, LogLevel
    ...
    def test_outputs_single_line(self, capsys, monkeypatch):
        """Notification should be on a single line."""
        from mcpmon import log, LogLevel
        ...
        try:
            ...
            # Should be exactly one line with content (may have trailing newline)
...
""")
    ...
    def test_mcpmon_sends_notification_on_restart(self, temp_dir, dummy_server):
        """mcpmon should send tools/list_changed notification after restart."""
        import json
        ...
        try:
            ...
            # Wait for restart and notification
            time.sleep(2)

            # Check log file for notification message
            log_content = log_file.read_text()
            assert "Sent tools/list_changed notification" in log_content

            # Check stdout for JSON-RPC notification
            # Note: We can't easily capture stdout in this test since it goes through a pipe
            # But we verify the log message which confirms the function was called

        finally:
            proc.terminate()
            proc.wait(timeout=5)

    def test_mcpmon_no_notification_when_disabled(self, temp_dir, dummy_server):
        """mcpmon should not send notification when --no-notify-tools-changed is used."""
        # Create a watched file
        watched_file = temp_dir / "watched.py"
        watched_file.write_text("# initial")
        ...
        try:
            ...
            # Check that restart happened but no notification
            log_content = log_file.read_text()
            assert "Restart #1" in log_content
            assert "Sent tools/list_changed notification" not in log_content
...
class TestCLI:
    ...
    def test_notify_tools_changed_in_help(self):
        """--notify-tools-changed should be documented in help."""
        result = subprocess.run(
            [sys.executable, "-m", "mcpmon", "--help"],
            capture_output=True,
            text=True,
        ...
        assert result.returncode == 0
        assert "--notify-tools-changed" in result.stdout
        assert "--no-notify-tools-changed" in result.stdout
