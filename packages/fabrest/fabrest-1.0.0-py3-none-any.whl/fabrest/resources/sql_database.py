...
class SQLDatabasesResource(ItemTypeResource):
    ...
    def start_mirroring(
        ...
        response = self._require_transport().request(
            method="POST",
            url=routes.sql_database_start_mirroring(self.workspace_id, database_id),
            options=opts,
            session=session,
        )
    ...
    def stop_mirroring(
        ...
        response = self._require_transport().request(
            method="POST",
            url=routes.sql_database_stop_mirroring(self.workspace_id, database_id),
            options=opts,
            session=session,
        )
        return self._handle_response(response, opts)
