...
# Uses word-boundary regex to prevent false positives like "key" matching "key_name" or "monkey".
# Pattern format: (^|_)word(_|$|[0-9]) - matches at start, after underscore, or before underscore/end/digit
# NOTE: "key" is intentionally excluded as too broad - use specific patterns like "api_key"
SUSPICIOUS_KEY_PATTERNS: tuple[re.Pattern[str], ...] = (
    re.compile(r"(^|_)password(_|$|[0-9])", re.IGNORECASE),
    re.compile(r"(^|_)passwd(_|$|[0-9])", re.IGNORECASE),
    re.compile(r"(^|_)secret(_|$|[0-9])", re.IGNORECASE),
    re.compile(r"(^|_)token(_|$|[0-9])", re.IGNORECASE),
    re.compile(r"(^|_)credential(_|$|[0-9])", re.IGNORECASE),
    re.compile(r"(^|_)auth(_|$|[0-9])", re.IGNORECASE),
    re.compile(r"(^|_)private(_|$|[0-9])", re.IGNORECASE),
    re.compile(r"(^|_)api_?key(_|$|[0-9])", re.IGNORECASE),  # Matches api_key, apikey
    re.compile(r"(^|_)access_key(_|$|[0-9])", re.IGNORECASE),
    re.compile(r"(^|_)secret_key(_|$|[0-9])", re.IGNORECASE),
)
...
class Sanitizer:
    ...
    def _is_suspicious_key(self, key: str) -> bool:
        ...
        - "password" does NOT match: reset_password_link (link is not boundary)
        - "key" is excluded - too broad (matches key_name, monkey, etc.)
        """
        return any(pattern.search(key) for pattern in SUSPICIOUS_KEY_PATTERNS)
