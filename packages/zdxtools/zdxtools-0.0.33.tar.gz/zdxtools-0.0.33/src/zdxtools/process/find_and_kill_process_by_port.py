...
def find_and_kill_process_by_port(port):
    ...
    try:
        ...
        if os_type == 'windows':
            ...
            icmp_out = subprocess.Popen(commands[os_type][0],encoding="utf8", shell=True,stdout=subprocess.PIPE)
            stdout, stderr = icmp_out.communicate(timeout=5)
            if len(stdout) > 0:
                pid = stdout.split('LISTENING')[1].replace(' ','').replace('\n','')
            else:pid = None
        else:
            # Linux/Mac上，我们使用subprocess来调用'lsof'（注意：可能是'lsof'，但更常见的是'lsof -i :{port}'且需要'sudo'权限）
            # 注意：下面的命令可能需要sudo权限来执行
            result = subprocess.run(commands[os_type][0], capture_output=True, text=True)
            ...
            for line in lines:
                if 'PID' in line:
                    ...
                    break
            if pid is None:
                print(f"没有找到监听{port}端口的进程")
                return
                # 结束进程
        if pid:
            command = commands[os_type][1].format(pid)
...
def find_and_kill_process_by_portlist(portlist):
    '''
