...
class ImgbbImagehostConfig(base.ImagehostConfigBase):
    """User configuration for :class:`ImgbbImagehost`"""
...
class ImgbbImagehost(base.ImagehostBase):
    """Upload images to imgbb.com"""

    name = 'imgbb'

    Config = ImgbbImagehostConfig
    ...
    description = (
        ...
        f'  3. Store your API key in {utils.fs.tildify_path(constants.IMGHOSTS_FILEPATH)}:\n'
        '\n'
    ...
    async def _upload_image(self, image_path):
        try:
            response = await utils.http.post(
                url=f'{self.config["base_url"]}/1/upload',
                cache=False,
                data={
