...
def open_iterable(
    ...
    if is_cloud_uri:
        try:
            ...
            backend_pkg = CLOUD_STORAGE_SCHEMES.get(backend or "", "fsspec")
            raise ImportError(
                f"Cloud storage support requires 'fsspec'. Install it with: pip install fsspec"
            ) from None
        ...
        if backend:
            try:
                ...
                raise ImportError(
                    f"Cloud storage URI '{filename}' requires '{backend}'. "
    ...
    try:
        ...
        if is_cloud_uri and cloud_stream is not None:
            if result["codec"] is not None and engine != "duckdb":
                ...
                codec = result["codec"](
                    filename=filename, fileobj=cloud_stream, mode=mode, options=codecargs
                )
                iterable = result["datatype"](
                    codec=codec, mode=normalized_mode, options=iterableargs
                )
            elif engine == "duckdb":
                # DuckDB engine does not support cloud storage directly
                raise ValueError(
                    f"DuckDB engine does not support cloud storage URIs. "
                    f"Use engine='internal' for cloud storage files."
                )
            else:
                # Pass the stream directly to the datatype
                iterable = result["datatype"](
                    stream=cloud_stream, mode=normalized_mode, options=iterableargs
                )
