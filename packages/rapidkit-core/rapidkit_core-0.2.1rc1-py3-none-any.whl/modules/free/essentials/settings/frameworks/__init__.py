...
"""Settings module plugin registry integration."""
...
PLUGIN_ENTRYPOINT_GROUP = "rapidkit.settings.plugins"
...
def register_plugin(plugin_class: Type[FrameworkPlugin]) -> None:
    """Register a settings framework plugin."""

    _REGISTRY.register(plugin_class)


def refresh_plugin_registry(*, auto_discover: bool = True) -> None:
    """Reset registry contents and repopulate with built-ins and entry points."""
...
def get_plugin_info(framework_name: str) -> Optional[Dict[str, object]]:
    """Return detailed metadata for a registered plugin."""
