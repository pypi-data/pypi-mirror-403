#!/usr/bin/env python3
"""Unified module generator for Storage."""
...
MODULE_NAME = "storage"
MODULE_CLASS = "FileStorage"
MODULE_TITLE = "Storage"
MODULE_TIER = "free"
MODULE_BASE = "src/modules/free/business/storage"
PYTHON_OUTPUT_REL = f"{MODULE_BASE}/storage.py"
NEST_OUTPUT_REL = f"{MODULE_BASE}/storage.service.ts"
...
def _load_yaml(relative_path: str) -> Mapping[str, Any]:
    ...
    with target.open(encoding=DEFAULT_ENCODING) as handle:
...
def build_base_context(config: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    storage_section = dict(base_config.get("storage", {}))
    ...
    context = {
        ...
        "storage_defaults": storage_section,
        "storage_logging": dict(base_config.get("logging", {})),
        "storage_performance": dict(base_config.get("performance", {})),
        "storage_retry": dict(base_config.get("retry", {})),
    }

    context["storage_defaults"].setdefault("local", {})
    return context
...
def generate_variant_files(
    ...
    # Materialise canonical vendor-backed health shim and ensure the shared
    # health package exists for the generated project.
...
def main() -> None:
    ...
    if len(sys.argv) != EXPECTED_ARG_COUNT:
        guidance = (
            "Usage: python -m modules.free.business.storage.generate <variant> <target_dir>\n"
            "Example: python -m modules.free.business.storage.generate fastapi ../../examples/storage\n"
