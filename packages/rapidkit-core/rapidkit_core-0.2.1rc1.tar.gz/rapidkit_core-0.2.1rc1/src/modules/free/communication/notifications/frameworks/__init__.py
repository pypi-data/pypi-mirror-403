"""Framework plugins for notifications module."""
...
_REGISTRY: PluginRegistry = PluginRegistry(
    entry_point_group="rapidkit.modules.free.communication.notifications.plugins"
)
...
def get_plugin(framework_name: str) -> Optional[FrameworkPlugin]:
    ...
    base_name = framework_name.split(".", 1)[0]
    if not _REGISTRY.is_available(framework_name) and base_name != framework_name:
        if not _REGISTRY.is_available(base_name):
            refresh_plugin_registry(auto_discover=False)
        if _REGISTRY.is_available(base_name):
