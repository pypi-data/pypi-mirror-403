...
from . import VERSION, TRandom, documentor, exceptions
...
def print_doc(
    ...
    if output is None:
        output = sys.stdout
    fake = TRandom(locale=lang, includes=includes)
    fake.seed_instance(seed)

    from trandom.providers import BaseProvider
    ...
    if provider_or_field:
        if "." in provider_or_field:
            ...
            fake = TRandom(locale, providers=[provider_or_field], includes=includes)
            fake.seed_instance(seed)
