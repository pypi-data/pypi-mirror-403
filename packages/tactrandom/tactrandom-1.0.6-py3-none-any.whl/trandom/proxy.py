...
class TRandom:
    ...
    def __init__(
        ...
        **config: Any,
        ...
        self._factory_map: OrderedDict[str, Generator | TRandom] = OrderedDict()
        self._weights = None
        ...
        if len(locales) == 1:
            ...
            for locale in locales:
                self._factory_map[locale] = TRandom(
                    locale,
                    providers,
                    generator,
                    includes,
    ...
    def __getattribute__(self, attr: str) -> Any:
        ...
        """
        if attr == "seed":
            msg = "Calling `.seed()` on instances is deprecated. Use the class method `TRandom.seed()` instead."
            raise TypeError(msg)
        else:
    ...
    @property
    def factories(self) -> list[Generator | TRandom]:
        return self._factories

    def items(self) -> list[tuple[str, Generator | TRandom]]:
        return list(self._factory_map.items())


class UniqueProxy:
    def __init__(self, proxy: TRandom, excluded_types: tuple[type, ...] = ()):
        self._proxy = proxy
        self._seen: dict = {}
    ...
    def exclude_types(self, types: list[type]) -> UniqueProxy:
        ...
        Example:
            ...
            >>> proxy = rand.unique.exclude_types([bool])
...
class OptionalProxy:
    ...
    def __init__(self, proxy: TRandom):
        self._proxy = proxy
