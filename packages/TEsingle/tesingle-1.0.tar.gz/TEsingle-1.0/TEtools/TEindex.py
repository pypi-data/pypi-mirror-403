...
class TEfeatures(object):
    def __init__(self):
        ...
        self.fullShortDict = {}
        self.subfIdxDict = {}
        self.fullShortIdxDict = {}

    def getElements(self):
        return self.subfList

    def getStrand(self, idx):
        return self._nameIDmap[idx][-1]

    def getLengths(self):
        return self.subfLengthList
    ...
    def TE_annotation(self, iv_seq):
        ...
        TEs_translated = set()
        for idx in TEs:
            TEs_translated.add(self.fullShortIdxDict[idx])
    ...
    def build(self, filename):
        ...
        for line in f:
            ...
            if ele_id == "" or transc_id == "" or family_id == "" or class_id == "":
                ...
                sys.stderr.write("TE GTF format error! There is missing annotation at line %s.\n" % linenum)
                raise
            ...
            if subf_name not in self.subfLengthDict:
                self.subfLengthDict[subf_name] = 0
            self.subfLengthDict[subf_name] += tlen
            ...
            if chrom not in self.indexlist:
                self.indexlist[chrom] = BinaryTree()
            ...
            while bin_startID <= bin_endID:
                ...
                self.indexlist[chrom].insert(start_pos, end_pos, name_idx)
                bin_startID += 1
        ...
        self.subfList = sorted(self.subfLengthDict.keys())
        for n, subf in enumerate(self.subfList):
            self.subfLengthList.append(self.subfLengthDict[subf])
            self.subfIdxDict[subf] = n

        for n, annot in enumerate(self._nameIDmap):
            self.fullShortIdxDict[n] = self.subfIdxDict[self.fullShortDict[annot]]
