...
//    def __init__(self, name):
    ...
    {
      static const char* kwargs[] = { "filename", "errorOnPublishGap", "blocksPerRealloc", "blockSize", NULL };
      char* filename;
      PyObject* value = NULL;
      unsigned long blocksPerRealloc = 10000;
      unsigned long blockSize = 2048;
      if (!PyArg_ParseTupleAndKeywords(args, kwds, "s|O!kk", (char**)kwargs, &filename, &PyBool_Type, &value, &blocksPerRealloc, &blockSize))
      {
        return NULL;
      }
      ...
      {
        UNLOCKGIL;
        self->impl = new PublishStore(fileName, blocksPerRealloc, blockSize, errorOnPublishGap);
        self->impl->addRef();
      } DISPATCH_EXCEPTION;
    ...
    void add_types(PyObject* module_)
    {
      ...
      .addMethod("truncate_on_close", truncate_on_close, truncate_on_close_doc)
