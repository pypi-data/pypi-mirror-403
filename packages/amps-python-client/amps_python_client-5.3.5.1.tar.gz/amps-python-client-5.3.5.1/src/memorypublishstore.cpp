...
//    def __init__(self, name):
    ...
    {
      static const char* kwargs[] = { "blocksPerRealloc", "errorOnPublishGap", "blockSize", NULL };
      unsigned long numBlocks = 10000;
      PyObject* value = NULL;
      unsigned long blockSize = 2048; // DEFAULT_BLOCK_SIZE
      if (!PyArg_ParseTupleAndKeywords(args, kwds, "|kO!k", (char**)kwargs, &numBlocks, &PyBool_Type, &value, &blockSize))
      {
        return NULL;
      }
      ...
      {
        UNLOCKGIL;
        self->impl = new Store(new MemoryPublishStore((size_t)numBlocks, (size_t)blockSize, errorOnPublishGap));
      } DISPATCH_EXCEPTION;
