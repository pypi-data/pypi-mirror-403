...
class BaseTestCase:
    ...
    def execute_preconditions(self):
        ...
        if self.case_data.get("preconditions"):
            for api in self.case_data.get("preconditions"):
                ...
                self._execute_setup_script(api_info)
                api_info = self._replace_variables(api_info)
                response = self._request_api(api_info)
                self._execute_teardown_script(response)
                self._extract_data(api, response)
    ...
    def _execute_setup_script(self, api_info):
        """执行用例前置脚本"""
        # 获取用例中的前置脚本
        # 创建一个脚本执行器
    ...
    def _execute_teardown_script(self, response):
        """执行用例后置脚本"""
        try:
            # 执行后置脚本
    ...
    def _replace_variables(self, api_info: dict) -> dict:
        """替换用例参数中的变量引用"""
    ...
    def _release_assertions(self, assertions: dict) -> dict:
        self.result.add_info_log("开始替换用例参数中的变量引用")
        pattern = r"\${{(.+?)}}"
        data = str(assertions)
        while re.search(pattern, data):
    ...
    def _request_api(self, api_info):
        """请求接口"""
    ...
    def _assert_result(self, assertions, response):
        """断言用例结果"""
    ...
    def _extract_data(self, api_info, response):
        """提取数据"""
    ...
    def _re_extract(self, string, pattern):
        """
        使用正则表达式提取数据
    ...
    def _run(self):
        ...
        self._execute_setup_script(case_api_info)
        case_api_info = self._replace_variables(case_api_info)
        response = self._request_api(case_api_info)
        self._execute_teardown_script(response)
        assertions = self._release_assertions(self.case_data.get("assertions", {}))
        self._assert_result(assertions, response)
