...
Agents Resource - Unified AI Agents access.

Provides access to specialized AI agents:
...
@dataclass
class AgentInfo(DataclassMixin):
    """Agent information."""
    ...
    description: str
    description_pt: str
    status: str  # active, beta, coming_soon, deprecated
    capabilities: list[str]
    languages: list[str]


@dataclass
class AgentChatResponse(DataclassMixin):
...
@dataclass
class AgentStreamChunk(DataclassMixin):
    ...
    agent: str | None = None
    session_id: str | None = None
    # Final metadata (only in "end" chunk)
    intent: str | None = None
    confidence: float | None = None
...
class AgentsResource:
    ...
    Provides unified access to specialized AI agents for different products.
    ...
    Usage:
        ...
        dedalo = client.agents.get("dedalo")

        # Chat with an agent
        response = client.agents.chat(
            agent="dedalo",
            messages=[{"role": "user", "content": "Olá, quero saber sobre seus produtos"}],
        )
    ...
    def list(self, include_coming_soon: bool = False) -> list[AgentInfo]:
        """
    ...
    def chat(
        ...
        response = self._client.post("/v1/agents/chat", json=payload)
