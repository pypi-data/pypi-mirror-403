...
class FVHDConfig(BaseModel):
    ...
    n_components: int = Field(
        default=2, description="Number of dimensions for the embedding."
    )
    ...
    c: float = Field(
        default=0.2, description="Attraction/Repulsion balance coefficient."
    )
    ...
    autoadapt: bool = Field(
        default=True, description="Enable auto-adaptation of learning rate."
    )
