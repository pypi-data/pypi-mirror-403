...
try:
    ...
    warnings.warn("datasets library not found. Install it with: pip install datasets", stacklevel=2)
...
class OrcaDPODataLoader:
    ...
    def load_data(
        self, split: str = "train", cache: bool = True, streaming: bool = False
    ) -> List[Dict]:
        """
    ...
    def iter_examples(
        self, split: str = "train", batch_size: Optional[int] = None
    ) -> Iterator[Dict]:
        """
    ...
    def sample_examples(
        self, n: int = 5, split: str = "train", seed: Optional[int] = None
    ) -> List[Dict]:
        """
    ...
    def export_for_training(
        ...
        formatted_examples = [self.format_for_dpo(ex, include_system) for ex in examples]
