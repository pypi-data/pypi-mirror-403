...
class MarginManager:
    ...
    def get_balance(self, account: AccountId, token: str) -> int:
        ...
        if self._client.has_backend:
            try:
                collateral = self._client.backend.get_collateral(
                    account.owner,
                    account.account_id,
                    account.isolated_margin_token,
                )
                for c in collateral:
    ...
    def get_all_balances(self, account: AccountId) -> dict[str, int]:
        ...
        if self._client.has_backend:
            try:
                ...
                return {c.token: int(c.amount) for c in collateral}
            except Exception:
                pass
        return {}
    ...
    def get_total_value_usd(self, account: AccountId) -> Decimal:
        ...
        Returns:
            Total value in USD as Decimal (scaled by 10^18).
            
        Example:
            >>> value = client.margin.get_total_value_usd(account)
            >>> print(f"Total margin: ${float(value) / 1e18:.2f}")
        """
        if self._client.has_backend:
            try:
                collateral = self._client.backend.get_collateral(
                ...
                total = sum(c.amount_usd for c in collateral)
