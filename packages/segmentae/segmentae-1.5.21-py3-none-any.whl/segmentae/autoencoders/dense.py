...
class DenseAutoencoder:
    ...
    def _get_optimizer(self):
        ...
        if self.optimizer in optimizers:
            ...
            raise ValueError(
                f"Unsupported optimizer: {self.optimizer}. Supported optimizers are: {list(optimizers.keys())}"
            )
    ...
    def fit(self, input_data: pd.DataFrame):
        ...
        if np.max(self.hidden_dims) > 3 * self.input_dim:
            print(
                "Layers neurons exceeds considerably the number input features risking overfitting, "
        ...
        # Compile the model
        self.autoencoder.compile(optimizer=self._get_optimizer(), loss="mean_squared_error")
