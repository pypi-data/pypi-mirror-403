 ...
 *
 * "current_player" bitboard can be transformed into a compact and non ambiguous
 * key by adding an extra bit on top of the last non empty cell of each column.
 ...
 * must be defined outside of the class definition to be available at compile
 * time for bottom_mask
 */
...
public:
   ...
   *
   * This key is a base 3 representation of the sequence of played moves column
   * per column, from bottom to top. The 3 digits are top_of_colum(0),
   * current_player(1), opponent(2).
   *
   * example: game "45" where player one played colum 4, then player two played
   * column 5 has a representation in base 3 digits : 0 0 0 1 0 2 0 0 0 or :
   * 3*3^3 + 1*3^5
   *
   * The symetric key is the mimimum key of the two keys built iterating columns
   * from left to righ or right to left.
   *
   ...
   * A losing move is a move leaving the possibility for the opponent to win
   * directly.
   *
   * Warning this function is intended to test position where you cannot win in
   * one turn If you have a winning move, this function can miss it and prefer
   * to prevent the opponent to make an alignment.
   */
