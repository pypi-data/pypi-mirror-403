...
def _run_flutter_build() -> None:
    ...
    if sys.platform.startswith("win"):
        ...
        # On Windows, Flutter is often `flutter.bat`. .bat/.cmd cannot be invoked
        # directly without going through cmd.exe.
        if flutter_cmd.lower().endswith((".bat", ".cmd")):
            cmd = ["cmd", "/c", flutter_cmd, "build", "windows", "--release", "--no-tree-shake-icons"]
        else:
            cmd = [flutter_cmd, "build", "windows", "--release", "--no-tree-shake-icons"]
    elif sys.platform == "darwin":
        # Keep Material Icons available.
...
def _flutter_build_output_is_usable() -> bool:
    flutter_dir = os.path.join(_repo_root(), "flut", ".flutter")
    if sys.platform.startswith("win"):
        base = os.path.join(flutter_dir, "build", "windows", "x64", "runner", "Release")
        return (
    ...
    if sys.platform.startswith("linux"):
        base = os.path.join(flutter_dir, "build", "linux", "x64", "release", "bundle")
        return os.path.isdir(base)
    return False
...
def _copy_flutter_build_output_into_wheel(build_lib: str) -> None:
    ...
    if sys.platform.startswith("win"):
        src = os.path.join(flutter_dir, "build", "windows", "x64", "runner", "Release")
        dst = os.path.join(
            build_lib,
            "flut",
            ".flutter",
        ...
        src = os.path.join(flutter_dir, "build", "linux", "x64", "release", "bundle")
        dst = os.path.join(
            build_lib, "flut", ".flutter", "build", "linux", "x64", "release", "bundle"
        )
    else:
        raise RuntimeError(f"Unsupported platform for Flutter build: {sys.platform}")
...
class CustomBuildPy(build_py):
    def run(self):
        ...
        # Ensure the Flutter host app is built, then bundle its build output
        # into the wheel so `flut` can run after install.
...
def _get_platform_tag() -> str:
    """Return the appropriate wheel platform tag for the current OS."""
    if sys.platform.startswith("win"):
        return "win_amd64"
    elif sys.platform == "darwin":
