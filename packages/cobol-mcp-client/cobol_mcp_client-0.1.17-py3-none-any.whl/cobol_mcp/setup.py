...
def _resolve_cursor_linux_path(home: Path) -> Path:
    """Resolve Cursor config path on Linux. Prefers existing file, defaults to ~/.cursor/."""
    dotcursor = home / ".cursor" / "mcp.json"
    xdg = home / ".config" / "cursor" / "mcp.json"
    if dotcursor.exists():
        return dotcursor
    if xdg.exists():
        return xdg
...
def find_mcp_config_path(ide: str) -> Path | None:
    ...
    config_paths: dict[str, dict[str, Path]] = {
        "cursor": {
            ...
            "Linux": _resolve_cursor_linux_path(home),
        },
        "claude-desktop": {
        ...
        "roo-code": {
            ...
            "Windows": Path(os.environ.get("APPDATA", "")) / "Code" / "User" / "globalStorage" / "rooveterinaryinc.roo-cline" / "settings" / "cline_mcp_settings.json",
...
def setup_amp(api_key: str) -> bool:
    ...
    try:
        ...
        subprocess.run([
            ...
            "--env", f"COBOL_MCP_API_KEY={api_key}",
