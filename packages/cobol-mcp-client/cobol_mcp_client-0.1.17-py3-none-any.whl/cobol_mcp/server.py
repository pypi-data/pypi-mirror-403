...
from .tools.search_migration import search_migration as run_search_migration
from .tools.translate import lookup_translation

mcp = FastMCP("cobol-mcp", instructions=(
    "Before calling `search`, `search_migration`, or `check`, read the cobol://context "
    "resource first. It contains dialect differences, effective query patterns, and "
    "context that significantly improves result quality."
))
...
@mcp.tool()
async def search_migration(query: str, top_k: int = 8) -> str:
    """
    Search COBOL-to-Java migration documentation for strategies, patterns, and tools.

    Uses hybrid search (semantic + keyword) over migration-specific sources:
    - Academic research on COBOL-to-Java migration (CSMR 2013)
    - Language migration FAQ and best practices
    - COBOL-to-Java conversion guides
    - Ispirer Java Framework guide for COBOL conversion

    Examples:
    - "automated COBOL to Java conversion tools"
    - "migration testing strategies"
    - "batch program conversion approach"

    Read cobol://context for guidance on when to use each tool.
    """
    return await run_search_migration(query, top_k=top_k)
