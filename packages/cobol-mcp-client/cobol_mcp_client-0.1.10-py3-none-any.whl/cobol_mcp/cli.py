    ...
    try:
        ...
        if ch == '\x1b':
            if select.select([sys.stdin], [], [], 0.3)[0]:
                ...
                if ch2 == '[' and select.select([sys.stdin], [], [], 0.3)[0]:
...
def _interactive_select(ides: list[str]) -> list[str]:
    ...
    num_lines = len(ides) + 2  # items + hint line + blank

    def render():
        # Move up to overwrite previous render (except first call)
        ...
        sys.stdout.write(f"\033[2K  {CYAN}\u2191/\u2193{NC} navigate  {CYAN}\u2423{NC} toggle  {CYAN}\u23ce{NC} confirm  {CYAN}a{NC} all\n")
        sys.stdout.flush()
