    ...
    def __init__(self, data, threshold=0, color=None, threshold_line_color='blue', threshold_line_width=1,
        ...
        if isinstance(data, dict):
            ...
            for col in range(len(columns)):
                data.iloc[0, col] = np.add(data.iloc[0, col], x_max, dtype=np.int64)
                x_ticks.append((max(data.iloc[0, col]) - x_max) / 2 + x_max)
                x_max = max(data.iloc[0, col])
                if log_base != 0:
                    data.iloc[1, col] = np.log(data.iloc[1, col]) / np.log(log_base)
                if reverse:
                    data.iloc[1, col] = -data.iloc[1, col]
                tmp_y.extend(data.iloc[1, col])
