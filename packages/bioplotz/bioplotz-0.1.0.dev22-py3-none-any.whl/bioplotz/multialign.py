...
from matplotlib.offsetbox import TextArea, AnnotationBbox, HPacker
...
class _MultiAlign(object):
    ...
    @staticmethod
    def __rainbow_text(x, y, strings, colors, background_colors, ax=None, char_width_px=6, **kwargs):
        """
        ...
        **kwargs
        ...
        #     props.update(kwargs)
        #     if bc:
        #         t = TextArea(s, textprops=dict(color=c, bbox=dict(facecolor=bc, edgecolor="none", pad=0), **kwargs))
        #     else:
        #         t = TextArea(s, textprops=dict(color=c, **kwargs))
        #     texts.append(t)
        #
        # packer = HPacker(children=texts, align="baseline", pad=0, sep=0)
        # ab = AnnotationBbox(packer, (x, y), xycoords="data", box_alignment=(0.0, 0.5), frameon=False, pad=0)
        # ax.add_artist(ab)
        # return ab
        if ax is None:
            ax = plt.gca()
        ...
        t_base = ax.transData
        fig = ax.figure
        current_offset_px = 0.0
        artists = []
        for s, c, bc in zip(strings, colors, background_colors):
            bbox_props = dict(facecolor=bc, linewidth=0, pad=0) if bc else None
            text = ax.text(x, y, s, color=c, bbox=bbox_props, transform=t_base, ha="center", va="center", **kwargs)

            if current_offset_px > 0.:
                text.set_transform(offset_copy(t_base, fig=fig, x=current_offset_px, units="dots"))
            current_offset_px += len(s) * char_width_px
            artists.append(text)
        return artists

    def plot(self, ax, fontsize=10, **kwargs):
        aln_len = 0
        ...
        temp_text = ax.text(0, 0, "X", **kwargs)
        ...
        for i in range(row_cnt):
            ...
            if len(divide_and_pos) > x_max:
                x_max = len(divide_and_pos)
            colors = ['black' for _ in range(len(divide_and_pos))]
            background_colors = ['white' for _ in range(len(divide_and_pos))]
            self.__rainbow_text(0, i * (seq_cnt + 2), list(divide_and_pos), colors, background_colors, ax=ax,
                                char_width_px=char_width, **kwargs)
            ...
            if self.__color_mode == "match":
                ...
                for j in range(seq_cnt):
                    ...
                    self.__rainbow_text(0, i * (seq_cnt + 2) + j + 1, self.__data[gid][sp: ep],
                                        colors, background_colors, ax=ax, char_width_px=char_width, **kwargs)
            elif self.__color_mode == "base":
                for j in range(seq_cnt):
                    gid = seq_list[j]
                    y_ticks.append(i * (seq_cnt + 2) + j + 1)
