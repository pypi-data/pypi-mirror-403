...
class HistoryManager:
    ...
    def pop_entries(self, count: int) -> list[HistoryEntry]:
        """Remove and return the most recent N history entries.
        
        Args:
            count: Number of entries to pop. If greater than available,
                   pops all available entries.
        ...
        # Pop entries from the end (most recent first)
        entries = []
        for _ in range(actual_count):
            entries.append(self._entries.pop())
        
        self._save()
        return entries
    
    def get_entry_summaries(self) -> list[dict]:
        """Get summary information for all history entries.
        
        Returns:
            List of dicts with entry summaries (index, timestamp, directory, file_count),
            ordered from most recent to oldest.
        """
        summaries = []
        for i, entry in enumerate(reversed(self._entries)):
            summaries.append({
                "index": len(self._entries) - 1 - i,
                "display_index": i + 1,  # 1-based for user display
                "timestamp": entry.timestamp,
                "target_directory": entry.target_directory,
                "file_count": len(entry.moves),
                "folders_created": len(entry.folders_created),
            })
        return summaries
...
def compute_file_hash(
    ...
    Args:
        ...
        quick_hash: If True, only hash the first chunk for speed.
                   Use for initial grouping, then verify with full hash.
    ...
    with open(path, "rb") as f:
        if quick_hash:
            # Only hash first chunk for quick comparison
