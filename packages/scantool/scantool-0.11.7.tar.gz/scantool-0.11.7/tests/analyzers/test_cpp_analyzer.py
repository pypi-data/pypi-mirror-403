...
import pytest
from scantool.analyzers.cpp_analyzer import CppAnalyzer
from scantool.analyzers.models import ImportInfo, EntryPointInfo


@pytest.fixture
def analyzer():
    """Create C++ analyzer instance."""
    return CppAnalyzer()
...
def test_extract_imports_local_headers(analyzer):
...
"""
    imports = analyzer.extract_imports("test.cpp", content)
...
def test_extract_imports_system_headers(analyzer):
...
def test_extract_imports_with_spaces(analyzer):
...
#   include <stdlib.h>
"""
    imports = analyzer.extract_imports("test.c", content)
...
def test_extract_imports_line_numbers(analyzer):
...
def test_find_entry_points_main_no_args(analyzer):
...
"""
    entry_points = analyzer.find_entry_points("main.cpp", content)
...
def test_find_entry_points_main_with_args(analyzer):
...
def test_find_entry_points_main_with_array_syntax(analyzer):
...
def test_find_entry_points_void_main(analyzer):
...
"""
    entry_points = analyzer.find_entry_points("main.c", content)
...
def test_find_entry_points_extern_main(analyzer):
...
def test_find_entry_points_winmain(analyzer):
...
"""
    entry_points = analyzer.find_entry_points("winmain.cpp", content)
...
def test_find_entry_points_wwinmain(analyzer):
...
def test_find_entry_points_dllmain(analyzer):
...
"""
    entry_points = analyzer.find_entry_points("dllmain.cpp", content)
...
def test_find_entry_points_gtest(analyzer):
...
"""
    entry_points = analyzer.find_entry_points("test.cpp", content)
...
def test_find_entry_points_catch2(analyzer):
...
def test_should_analyze_normal_files(analyzer):
    ...
    assert analyzer.should_analyze("src/main.cpp") == True
    assert analyzer.should_analyze("include/utils.h") == True
    assert analyzer.should_analyze("lib/parser.cc") == True
    assert analyzer.should_analyze("core/engine.cxx") == True


def test_should_analyze_skip_protobuf(analyzer):
    """Test that protobuf generated files should be skipped."""
    assert analyzer.should_analyze("proto/messages.pb.h") == False
    assert analyzer.should_analyze("proto/api.pb.cc") == False
    assert analyzer.should_analyze("proto/service.pb.cpp") == False


def test_should_analyze_skip_qt_moc(analyzer):
    """Test that Qt moc generated files should be skipped."""
    assert analyzer.should_analyze("moc_widget.cpp") == False
    assert analyzer.should_analyze("moc_mainwindow.h") == False
    assert analyzer.should_analyze("generated/moc_dialog.cpp") == False


def test_should_analyze_skip_qt_ui(analyzer):
    """Test that Qt UI generated files should be skipped."""
    assert analyzer.should_analyze("ui_mainwindow.h") == False
    assert analyzer.should_analyze("ui_dialog.h") == False


def test_should_analyze_skip_qt_qrc(analyzer):
    """Test that Qt resource generated files should be skipped."""
    assert analyzer.should_analyze("qrc_resources.cpp") == False


def test_should_analyze_skip_generated(analyzer):
    """Test that generated files should be skipped."""
    assert analyzer.should_analyze("parser.gen.h") == False
    assert analyzer.should_analyze("lexer.gen.cpp") == False
    assert analyzer.should_analyze("generated_code.h") == False


def test_should_analyze_keep_generator(analyzer):
    ...
    assert analyzer.should_analyze("code_generator.cpp") == True


def test_should_analyze_skip_build_dirs(analyzer):
    ...
    assert analyzer.should_analyze("/build/main.cpp") == False
    assert analyzer.should_analyze("path/build/main.cpp") == False
    assert analyzer.should_analyze("/cmake-build-debug/foo.cpp") == False
    assert analyzer.should_analyze("project/cmake-build-release/bar.h") == False


def test_classify_file_main_cpp(analyzer):
...
"""
    cluster = analyzer.classify_file("src/main.cpp", content)
    assert cluster == "entry_points"


def test_classify_file_main_c(analyzer):
...
"""
    cluster = analyzer.classify_file("main.c", content)
    assert cluster == "entry_points"
...
"""
    cluster = analyzer.classify_file("src/app.cpp", content)
    assert cluster == "entry_points"


def test_classify_file_header_in_include(analyzer):
...
"""
    ...
    cluster = analyzer.classify_file("project/include/api.h", content)
    assert cluster == "infrastructure"


def test_classify_file_header_in_api(analyzer):
...
"""
    ...
    cluster = analyzer.classify_file("src/api/client.hpp", content)
    assert cluster == "core_logic"


def test_classify_file_config_header(analyzer):
    """Test classification of config headers."""
    content = """
#define VERSION "1.0.0"
#define MAX_CONNECTIONS 100
"""
    cluster = analyzer.classify_file("include/config.h", content)
...
"""
    cluster = analyzer.classify_file("test_utils.cpp", content)
    assert cluster == "tests"


def test_classify_file_with_test_macro(analyzer):
...
"""
    cluster = analyzer.classify_file("math_tests.cpp", content)
    assert cluster == "tests"


def test_classify_file_in_src(analyzer):
...
"""
    cluster = analyzer.classify_file("src/processor.cpp", content)
    assert cluster == "core_logic"


def test_classify_file_third_party(analyzer):
...
"""
    ...
    cluster = analyzer.classify_file("libs/third_party/json/json.cpp", content)
    assert cluster == "infrastructure"


def test_extract_imports_complex_real_world(analyzer):
...
"""
    imports = analyzer.extract_imports("src/main.cpp", content)
...
"""
    entry_points = analyzer.find_entry_points("test_main.cpp", content)
