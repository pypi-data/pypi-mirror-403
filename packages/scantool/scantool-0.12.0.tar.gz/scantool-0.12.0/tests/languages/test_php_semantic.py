...
class TestPHPAnalyzerMetadata:
    ...
    def test_language_name(self, language):
        """Test language name."""
        assert language.get_language_name() == "PHP"
...
class TestShouldAnalyze:
    ...
    def test_should_analyze_normal_file(self, language):
        ...
        assert language.should_analyze("src/Controller/UserController.php") is True
        assert language.should_analyze("index.php") is True
        assert language.should_analyze("app/Models/User.php") is True

    def test_should_analyze_skip_blade_cache(self, language):
        """Test that Laravel Blade cache files are skipped."""
        assert language.should_analyze("storage/framework/views/abc123.php") is False
        assert language.should_analyze("storage/framework/views/compiled/test.php") is False

    def test_should_analyze_phtml(self, language):
        """Test that .phtml files are analyzed."""
        assert language.should_analyze("template.phtml") is True


class TestExtractImports:
    """Test import extraction."""

    def test_extract_imports_use_statements(self, language):
...
"""
    ...
    def test_extract_imports_use_with_alias(self, language):
...
"""
    ...
    def test_extract_imports_grouped_use(self, language):
...
"""
    ...
    def test_extract_imports_relative_paths(self, language):
        """Test extraction of relative path imports."""
        content = """<?php
require './config/app.php';
include '../helpers/functions.php';
"""
        imports = language.extract_imports("src/app.php", content)
    ...
    def test_extract_imports_use_function(self, language):
...
class TestFindEntryPoints:
    ...
    def test_find_entry_points_index_php(self, language):
        """Test detection of index.php as entry point."""
        content = """<?php
require 'vendor/autoload.php';
echo "Hello World";
"""
        entry_points = language.find_entry_points("public/index.php", content)
    ...
    def test_find_entry_points_not_index(self, language):
        """Test that non-index files are not marked as entry files."""
        content = """<?php
echo "Hello";
"""
        entry_points = language.find_entry_points("app/test.php", content)

        index_entries = [ep for ep in entry_points if ep.type == "entry_file"]
        assert len(index_entries) == 0

    def test_find_entry_points_laravel_routes(self, language):
...
"""
        entry_points = language.find_entry_points("routes/api.php", content)
    ...
    def test_find_entry_points_route_resource(self, language):
        """Test detection of Laravel resource routes."""
        content = """<?php
Route::resource('photos', 'PhotoController');
"""
        entry_points = language.find_entry_points("routes/web.php", content)

        route_entries = [ep for ep in entry_points if ep.type == "route"]
        assert len(route_entries) == 1
        assert route_entries[0].name == "RESOURCE photos"

    def test_find_entry_points_controllers(self, language):
...
"""
        entry_points = language.find_entry_points("app/Controllers/UserController.php", content)
    ...
    def test_find_entry_points_final_controller(self, language):
...
"""
        entry_points = language.find_entry_points("app/Controllers/ApiController.php", content)

        controller_entries = [ep for ep in entry_points if ep.type == "controller"]
        assert len(controller_entries) == 1
        assert controller_entries[0].name == "ApiController"

    def test_find_entry_points_invokable(self, language):
...
"""
        entry_points = language.find_entry_points("app/Controllers/ShowUserController.php", content)
    ...
    def test_find_entry_points_mixed(self, language):
...
class TestClassifyFile:
    ...
    def test_classify_file_index_php(self, language):
        """Test classification of index.php as entry point."""
        content = """<?php
require 'vendor/autoload.php';
"""
        cluster = language.classify_file("public/index.php", content)
...
"""
        # Test by directory
        cluster = language.classify_file("app/Controllers/UserController.php", content)
        assert cluster == "entry_points"

        # Test by filename
        cluster = language.classify_file("UserController.php", content)
        assert cluster == "entry_points"

    def test_classify_file_routes(self, language):
        """Test classification of route files."""
        content = """<?php
Route::get('/', function() {});
"""
        cluster = language.classify_file("routes/web.php", content)
        assert cluster == "entry_points"

        cluster = language.classify_file("routes/api.php", content)
        assert cluster == "entry_points"
...
"""
        cluster = language.classify_file("app/Models/User.php", content)
...
class EmailService {}
"""
        cluster = language.classify_file("app/Services/EmailService.php", content)
        assert cluster == "core_logic"

    def test_classify_file_middleware(self, language):
        """Test classification of middleware files."""
        content = """<?php
class Authenticate {}
"""
        cluster = language.classify_file("app/Middleware/Authenticate.php", content)
        assert cluster == "core_logic"
...
"""
        cluster = language.classify_file("config/app.php", content)
        assert cluster == "config"

        cluster = language.classify_file("config.production.php", content)
        assert cluster == "config"

    def test_classify_file_migrations(self, language):
        """Test classification of migration files."""
        content = """<?php
class CreateUsersTable {}
"""
        cluster = language.classify_file("database/migrations/2024_create_users_table.php", content)
        assert cluster == "database"

    def test_classify_file_seeders(self, language):
        """Test classification of seeder files."""
        content = """<?php
class UserSeeder {}
"""
        cluster = language.classify_file("database/seeders/UserSeeder.php", content)
        assert cluster == "database"

    def test_classify_file_views(self, language):
...
"""
        cluster = language.classify_file("resources/views/home.blade.php", content)
        assert cluster == "presentation"

        cluster = language.classify_file("templates/index.php", content)
        assert cluster == "presentation"
...
"""
        cluster = language.classify_file("tests/Unit/UserTest.php", content)
...
class TestRealWorldExamples:
    ...
    def test_laravel_controller_example(self, language):
...
"""
    ...
    def test_symfony_controller_example(self, language):
...
"""
        imports = language.extract_imports("src/Controller/HomeController.php", content)
        entry_points = language.find_entry_points("src/Controller/HomeController.php", content)
