...
class TestPythonAnalyzerSkip:
    """Test PythonLanguage.should_analyze()."""

    def test_skip_pyc_files(self):
        """Should skip .pyc files."""
        language = PythonLanguage()
        assert language.should_analyze("module.pyc") is False
        assert language.should_analyze("module.pyo") is False
        assert language.should_analyze("module.pyd") is False

    def test_analyze_normal_py(self):
        """Should analyze normal .py files."""
        language = PythonLanguage()
        assert language.should_analyze("main.py") is True
        assert language.should_analyze("src/utils.py") is True
        assert language.should_analyze("__init__.py") is True  # Empty or not, we analyze


class TestTypeScriptAnalyzerSkip:
    """Test TypeScriptLanguage.should_analyze()."""

    def test_skip_minified_js(self):
        """Should skip minified JS files."""
        language = TypeScriptLanguage()
        assert language.should_analyze("bundle.min.js") is False
        assert language.should_analyze("app.min.mjs") is False
        assert language.should_analyze("vendor.min.cjs") is False

    def test_skip_d_ts_files(self):
        """Should skip TypeScript declaration files."""
        language = TypeScriptLanguage()
        assert language.should_analyze("types.d.ts") is False
        assert language.should_analyze("globals.d.ts") is False

    def test_skip_bundles(self):
        """Should skip webpack/rollup bundles."""
        language = TypeScriptLanguage()
        assert language.should_analyze("main.bundle.js") is False
        assert language.should_analyze("chunk-vendor.js") is False
        assert language.should_analyze("chunk-123abc.js") is False

    def test_analyze_normal_ts(self):
        """Should analyze normal TypeScript files."""
        language = TypeScriptLanguage()
        assert language.should_analyze("main.ts") is True
        assert language.should_analyze("component.tsx") is True
        assert language.should_analyze("app.js") is True
        assert language.should_analyze("utils.mjs") is True


class TestGoAnalyzerSkip:
    """Test GoLanguage.should_analyze()."""

    def test_skip_generated_pb_go(self):
        """Should skip protobuf generated files."""
        language = GoLanguage()
        assert language.should_analyze("api.pb.go") is False
        assert language.should_analyze("messages.pb.go") is False

    def test_skip_generated_gen_go(self):
        """Should skip other generated files."""
        language = GoLanguage()
        assert language.should_analyze("models.gen.go") is False
        assert language.should_analyze("generated_models.go") is False

    def test_analyze_test_files(self):
        """Should analyze test files (they're kept, classified separately)."""
        language = GoLanguage()
        assert language.should_analyze("main_test.go") is True

    def test_analyze_normal_go(self):
        """Should analyze normal Go files."""
        language = GoLanguage()
        assert language.should_analyze("main.go") is True
        assert language.should_analyze("server.go") is True
        assert language.should_analyze("internal/utils.go") is True
...
class TestCodeMapDiscovery:
    ...
    def test_skip_combined_with_analyzer_should_analyze(self):
        """Should combine directory skip AND language.should_analyze()."""
        with tempfile.TemporaryDirectory() as tmpdir:
            project_dir = Path(tmpdir)
            ...
            # Should NOT find app.min.js (TypeScriptLanguage.should_analyze() = False)
            # Should NOT find module.pyc (PythonLanguage.should_analyze() = False)
            file_paths = [f.path for f in result.files]
