...
class TestMarkdownAnalyzer:
    """Test suite for Markdown language."""
    ...
    def test_language_name(self, language):
        """Test language name."""
        assert language.get_language_name() == "Markdown"

    def test_extract_imports_markdown_links(self, language):
...
"""
        imports = language.extract_imports("README.md", content)
    ...
    def test_extract_imports_skip_urls(self, language):
...
"""
        imports = language.extract_imports("test.md", content)

        # Should find no imports (all are URLs)
        assert len(imports) == 0

    def test_extract_imports_skip_anchors_only(self, language):
...
"""
    ...
    def test_extract_imports_strip_anchors(self, language):
...
"""
    ...
    def test_extract_imports_html_img_tags(self, language):
...
"""
    ...
    def test_extract_imports_skip_data_uris(self, language):
...
"""
    ...
    def test_extract_imports_include_directives(self, language):
...
"""
    ...
    def test_extract_imports_mixed_types(self, language):
...
"""
        imports = language.extract_imports("docs/current/page.md", content)
    ...
    def test_find_entry_points_readme(self, language):
        """Test detection of README.md as entry point."""
        content = "# My Project\n\nWelcome!"
        entry_points = language.find_entry_points("README.md", content)
    ...
    def test_find_entry_points_index(self, language):
        """Test detection of INDEX.md as entry point."""
        content = "# Index\n\nTable of contents"
        entry_points = language.find_entry_points("INDEX.md", content)
    ...
    def test_find_entry_points_home(self, language):
        """Test detection of HOME.md as entry point."""
        content = "# Home Page"
        entry_points = language.find_entry_points("HOME.md", content)
    ...
    def test_find_entry_points_main_heading(self, language):
...
"""
        entry_points = language.find_entry_points("docs.md", content)
    ...
    def test_find_entry_points_getting_started(self, language):
...
Follow these steps...
"""
        entry_points = language.find_entry_points("guide.md", content)

        heading_entries = [ep for ep in entry_points if ep.type == "entry_heading"]
        assert len(heading_entries) == 1
        assert heading_entries[0].name == "Getting Started"

    def test_find_entry_points_multiple(self, language):
...
"""
    ...
    def test_find_entry_points_case_insensitive(self, language):
...
"""
    ...
    def test_should_analyze_normal_file(self, language):
        ...
        assert language.should_analyze("README.md") is True
        assert language.should_analyze("docs/api.md") is True
        assert language.should_analyze("guide.markdown") is True

    def test_should_analyze_skip_generated(self, language):
        """Test that generated files are skipped."""
        assert language.should_analyze("api.generated.md") is False
        assert language.should_analyze("docs.auto.md") is False

    def test_classify_file_readme(self, language):
        """Test that README files are classified as entry_points."""
        content = "# README"
        cluster = language.classify_file("README.md", content)
        assert cluster == "entry_points"

        cluster = language.classify_file("README.markdown", content)
        assert cluster == "entry_points"

    def test_classify_file_docs_directory(self, language):
        """Test that files in docs/ are classified as documentation."""
        content = "# API Reference"
        cluster = language.classify_file("docs/api.md", content)
        assert cluster == "documentation"

        cluster = language.classify_file("documentation/guide.md", content)
        assert cluster == "documentation"

    def test_classify_file_other(self, language):
        """Test fallback classification for other files."""
        content = "# Notes"
        # Should use BaseAnalyzer's classify_file
        cluster = language.classify_file("notes.md", content)
        # BaseAnalyzer returns "other" for unrecognized patterns
        assert cluster == "other"

    def test_extract_imports_image_extensions(self, language):
...
"""
    ...
    def test_extract_imports_empty_file(self, language):
        ...
        content = ""
        imports = language.extract_imports("empty.md", content)
        assert len(imports) == 0

    def test_extract_imports_no_links(self, language):
...
More text here.
"""
        imports = language.extract_imports("plain.md", content)
        assert len(imports) == 0

    def test_find_entry_points_no_entry_points(self, language):
...
More content.
"""
        entry_points = language.find_entry_points("regular.md", content)
        # Should find no entry points (no Main/Home/Getting Started headings)
        assert len(entry_points) == 0

    def test_extract_imports_complex_paths(self, language):
...
"""
        imports = language.extract_imports("docs/api/current/page.md", content)
    ...
    def test_extract_imports_whitespace_handling(self, language):
