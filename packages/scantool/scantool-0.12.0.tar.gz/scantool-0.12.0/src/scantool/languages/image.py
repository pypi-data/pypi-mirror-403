"""Image language support - metadata extraction for image files.

This module provides structure scanning for image files (PNG, JPG, GIF, etc.)
by extracting metadata like dimensions, colors, and optimization hints.
...
class ImageLanguage(BaseLanguage):
    """Language handler for image files.

    Provides structure scanning that extracts:
    - Format and color mode
    - Dimensions and aspect ratio
    - Content type inference (icon, logo, photo, etc.)
    - Dominant colors (for RGB/RGBA images)
    - Animation frames (for GIF)
    - Optimization suggestions

    Note: Images don't have imports or entry points.
    """
    ...
    def extract_imports(self, file_path: str, content: str) -> list[ImportInfo]:
        """Images don't have imports."""
        return []

    def find_entry_points(self, file_path: str, content: str) -> list[EntryPointInfo]:
        """Images don't have entry points."""
        return []
    ...
    def _get_dominant_colors(self, img, n: int = 3) -> list[str]:
    ...
    def _get_optimization_hints(self, img, file_size: int) -> list[str]:
