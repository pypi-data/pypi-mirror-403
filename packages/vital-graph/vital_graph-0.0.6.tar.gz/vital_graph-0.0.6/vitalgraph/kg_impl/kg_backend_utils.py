...
class FusekiPostgreSQLBackendAdapter(KGBackendInterface):
    ...
    async def store_objects(self, space_id: str, graph_id: str, objects: List[GraphObject]) -> BackendOperationResult:
        ...
        try:
            ...
            # Check if hasKGGraphURI triples are present in the RDF graph
            kg_graph_uri_triples = [t for t in rdf_graph if 'hasKGGraphURI' in str(t[1])]
            frame_graph_uri_triples = [t for t in rdf_graph if 'hasFrameGraphURI' in str(t[1])]
            self.logger.info(f"🔥 Found {len(kg_graph_uri_triples)} hasKGGraphURI triples in RDF graph")
            self.logger.info(f"🔥 Found {len(frame_graph_uri_triples)} hasFrameGraphURI triples in RDF graph")
