...
class FusekiDatasetManager:
    ...
    async def add_quads_to_dataset(self, space_id: str, quads: List[tuple], 
        ...
        try:
            ...
            # Check for hasKGGraphURI quads
            kg_graph_uri_quads = [q for q in quads if 'hasKGGraphURI' in str(q[1])]
            if kg_graph_uri_quads:
                logger.info(f"🔥 Fuseki: Found {len(kg_graph_uri_quads)} hasKGGraphURI quads to write")
                for quad in kg_graph_uri_quads[:5]:  # Log first 5
                    logger.info(f"🔥 Fuseki: hasKGGraphURI quad: {quad[0]} -> {quad[2]}")
