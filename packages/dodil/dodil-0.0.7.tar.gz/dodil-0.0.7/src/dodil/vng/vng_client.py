...
class VngInput:
    ...
    def from_any(cls, item: Union[str, "VngInput"]) -> "VngInput":
        ...
        if isinstance(item, str):
            # Determine kind hint from extension
            kind = None
            lower = item.lower()
            if lower.endswith(('.png', '.jpg', '.jpeg', '.webp', '.gif', '.bmp', '.tiff')):
                kind = "IMAGE"
            elif lower.endswith(('.mp4', '.avi', '.mov', '.mkv', '.webm', '.flv', '.wmv', '.mpeg', '.mpg')):
                kind = "VIDEO"
            elif lower.endswith(('.mp3', '.wav', '.flac', '.aac', '.ogg', '.m4a')):
                kind = "AUDIO"
            elif lower.endswith(('.pdf', '.docx', '.doc', '.txt', '.md', '.markdown')):
                kind = "DOCUMENT"
...
class VngClient:
    ...
    def _parse_rest_results(self, job_data, num_inputs) -> List[List[float]]:
        ...
        for out in outputs:
            ...
            if chunks:
                # Take first chunk's embedding
                emb = chunks[0].get("embedding", [])
                if emb:
                    print(f"[VngClient] Received embedding for input {inp_id}, len={len(emb)}")
                    trunc = emb[:5] + ["..."] if len(emb) > 5 else emb
                    print(f"  First 5 dims: {trunc}")
                else:
                    print(f"[VngClient] Warning: Empty embedding for input {inp_id}")

                results_map[inp_id] = emb
            else:
        ...
        for i in range(num_inputs):
            ordered_embeddings.append(results_map.get(str(i), []))
