...
The REST API documentation can be found on [docs.memv.ai](https://docs.memv.ai). The full API of this library can be found in [api.md](api.md).
...
import os
import asyncio
from memvai import DefaultAioHttpClient
from memvai import AsyncMemv
...
When the library is unable to connect to the API (for example, due to network connection problems or a timeout), a subclass of `memvai.APIConnectionError` is raised.

When the API returns a non-success status code (that is, 4xx or 5xx
response), a subclass of `memvai.APIStatusError` is raised, containing `status_code` and `response` properties.

All errors inherit from `memvai.APIError`.
...
try:
    client.spaces.list()
except memvai.APIConnectionError as e:
...
These methods return an [`APIResponse`](https://github.com/mem-v/memv-python/tree/main/src/memvai/_response.py) object.

The async client returns an [`AsyncAPIResponse`](https://github.com/mem-v/memv-python/tree/main/src/memvai/_response.py) with the same structure, the only difference being `await`able methods for reading the response content.
...
We are keen for your feedback; please open an [issue](https://www.github.com/mem-v/memv-python/issues) with questions, bugs, or suggestions.
