...
@pytest.fixture(scope="function")
def assistant_factory(assistant_utils, conversation_utils):
    """Create assistants for tests with automatic cleanup.

    This fixture tracks all assistants created during a test and cleans them up
    automatically, including their conversations.

    Usage:
        def test_something(assistant_factory):
            assistant1 = assistant_factory.create(model_type="gpt-4", temperature=0.5)
            assistant2 = assistant_factory.create(model_type="claude-3")
            # Test logic...
    ...
    class AssistantFactory:
        ...
        def create(*args, **kwargs):
            """Create an assistant and track it for cleanup."""
            _assistant = assistant_utils.create_assistant(*args, **kwargs)
            created_assistants.append(_assistant)
            return _assistant

    yield AssistantFactory()

    # Cleanup all created assistants and their conversations
    for assistant in created_assistants:
        try:
            ...
            try:
                ...
                logger.error(
                    f"Failed to list conversations for assistant {assistant.id}: {e}"
                )
        except Exception as e:
            logger.error(f"Failed to delete assistant {assistant.name}: {e}")
...
def pytest_sessionfinish(session):
    ...
    This function attempts to clean up all test data created during the session.
    All cleanup operations are wrapped in try-except blocks to ensure that
    failures in one cleanup operation don't prevent other cleanups from running.
    """
    ...
    if CredentialsManager.get_parameter("CLEANUP_DATA", "true").lower() == "true":
        ...
        # Integrations cleanup (PROJECT scope)
        logger.info("Cleaning up project integrations...")
        try:
            ...
            for integration in integrations:
                if prefix in integration.alias:
                    try:
                        ...
                        logger.info(f"Deleted project integration: {integration.alias}")
                    except Exception as e:
                        logger.error(
                            f"Failed to delete project integration {integration.alias}: {e}"
                        )
        except Exception as e:
            logger.error(f"Failed to list project integrations: {e}")

        # Integrations cleanup (USER scope)
        logger.info("Cleaning up user integrations...")
        try:
            ...
            for integration in integrations:
                if prefix in integration.alias:
                    try:
                        ...
                        sleep(clean_up_timeout)
                        logger.info(f"Deleted user integration: {integration.alias}")
                    except Exception as e:
                        logger.error(
                            f"Failed to delete user integration {integration.alias}: {e}"
                        )
        except Exception as e:
            logger.error(f"Failed to list user integrations: {e}")

        # Assistants cleanup
        logger.info("Cleaning up assistants...")
        try:
            ...
            for assistant in assistants:
                if prefix in assistant.name:
                    try:
                        ...
                        # Clean up conversations for this assistant
                        try:
                            conversations = client.conversations.list_by_assistant_id(
                                assistant.id
                            )
                            for conversation in conversations:
                                try:
                        ...
                        logger.error(
                            f"Failed to delete assistant {assistant.name}: {e}"
                        )
        except Exception as e:
            logger.error(f"Failed to list assistants: {e}")

        # Datasources cleanup
        logger.info("Cleaning up datasources...")
        try:
            filters.pop("search")
            filters["name"] = prefix
            ...
            for datasource in datasources:
                if prefix in datasource.name:
                    try:
                        ...
                        logger.error(
                            f"Failed to delete datasource {datasource.name}: {e}"
                        )
        except Exception as e:
            ...
            for workflow in workflows:
                if prefix in workflow.name:
                    try:
                        ...
                        logger.error(f"Failed to delete workflow {workflow.name}: {e}")
        except Exception as e:
        ...
        try:
            ...
            logger.error(f"Failed to cleanup providers: {e}")

        # Jira test issues cleanup
        logger.info("Cleaning up Jira test issues...")
        try:
            jira_utils = JiraUtils(is_cloud=False)
            jira_utils.cleanup_jira_project(summary_prefix=prefix)
        except Exception as e:
            logger.error(f"Failed to cleanup Jira Server issues: {e}")

        try:
            jira_utils = JiraUtils(is_cloud=True)
            jira_utils.cleanup_jira_project(summary_prefix=prefix)
        except Exception as e:
            logger.error(f"Failed to cleanup Jira Cloud issues: {e}")

        # Confluence test pages cleanup
        logger.info("Cleaning up Confluence test pages...")
        try:
            confluence_utils = ConfluenceUtils(is_cloud=False)
            confluence_utils.cleanup_confluence_space(title_prefix=prefix)
        except Exception as e:
            logger.error(f"Failed to cleanup Confluence Server pages: {e}")

        try:
            confluence_utils = ConfluenceUtils(is_cloud=True)
            confluence_utils.cleanup_confluence_space(title_prefix=prefix)
        except Exception as e:
            logger.error(f"Failed to cleanup Confluence Cloud pages: {e}")
