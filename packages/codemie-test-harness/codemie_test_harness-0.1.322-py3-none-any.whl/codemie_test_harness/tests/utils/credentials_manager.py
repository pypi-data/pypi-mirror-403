...
class CredentialsManager:
    ...
    CREDENTIAL_MAPPINGS = {
        ...
        # === LITE LLM CREDENTIALS (Environment-based) ===
        # Preview LiteLLM
        "PREVIEW_LITE_LLM_URL": {
            "env": "LITE_LLM_URL",
            "aws_param": f"{INTEGRATIONS_PARAMETER_PATH}litellm",
            "aws_path": "preview.url",
        },
        "PREVIEW_LITE_LLM_API_KEY": {
            "env": "LITE_LLM_API_KEY",
            "aws_param": f"{INTEGRATIONS_PARAMETER_PATH}litellm",
            "aws_path": "preview.api_key",
        },
        # Production LiteLLM
        "PROD_LITE_LLM_URL": {
            "env": "LITE_LLM_URL",
        ...
        "PROD_LITE_LLM_API_KEY": {
            "env": "LITE_LLM_API_KEY",
            "aws_param": f"{INTEGRATIONS_PARAMETER_PATH}litellm",
            "aws_path": "prod.api_key",
        },
    ...
    def lite_llm_credentials() -> List[CredentialValues]:
        """Create LiteLLM credentials using environment-aware configuration-driven approach.

        Uses different JSON paths based on environment:
        - Preview environment: preview.api_key
        - Production environment: prod.api_key
        """

        if EnvironmentResolver.is_production():
            # For production environment, use prod.api_key path
            api_key = CredentialsManager.get_parameter("PROD_LITE_LLM_API_KEY")
        else:
            # For preview/other environments, use preview.api_key path
            api_key = CredentialsManager.get_parameter("PREVIEW_LITE_LLM_API_KEY")

        return [
            CredentialValues(key="api_key", value=api_key),
        ]
