...
from codemie_test_harness.tests.test_data.direct_tools.ado_work_item_tools_test_data import (
...
def test_workflow_with_ado_work_item_get_tools_direct(
    ado_integration,
    workflow_utils,
    similarity_check,
    workflow_with_tool,
...
@pytest.mark.parametrize(
    "toolkit,tool_name,prompt,expected_response",
    ado_work_item_get_test_data,
    ids=[f"{row[1]}_hardcoded" for row in ado_work_item_get_test_data],
)
def test_workflow_with_ado_work_item_get_tools_with_hardcoded_args(
    ado_integration,
    workflow_utils,
    similarity_check,
    workflow_with_tool,
...
@pytest.mark.parametrize(
    "toolkit,tool_name,prompt,expected_response",
    ado_work_item_get_test_data,
    ids=[f"{row[1]}_overriding" for row in ado_work_item_get_test_data],
)
def test_workflow_with_ado_work_item_get_tools_with_overriding_args(
    ado_integration,
    workflow_utils,
    similarity_check,
    workflow_with_tool,
