...
class FormDecl(ASTNode):
    ...
    def __init__(self, name: str, params: List[Tuple[str, str, Optional[str]]],  # (name, kind, type)
                 body: List[ASTNode]):
        self.name = name
        self.params = params  # 'USING' or 'CHANGING' with optional type
        self.body = body
...
# NEW: Inline DATA declaration node
class InlineData(ASTNode):
    """Inline DATA declaration: @DATA(var_name)"""
    
    def __init__(self, name: str):
        self.name = name
    
    def __repr__(self):
        return f"InlineData(@DATA({self.name}))"
...
# Token patterns (order matters!) - ADDED @ symbol
TOKEN_PATTERNS = [
...
class Parser:
    ...
    def parse_primary(self) -> ASTNode:
        ...
        # Inline DATA declaration: @DATA(var)
        if tok.value == "@":
            self.next()  # Consume @
            if self.accept("DATA"):
                self.expect("(")
                var_name = self.expect("ID").value.lower()
                self.expect(")")
                return InlineData(var_name)
...
class FullParser(Parser):
    ...
    def parse_data_stmt(self) -> Optional[DataDecl]:
        """DATA: lv_a TYPE i [LENGTH n] VALUE 10 [, ...]."""
        # This method handles multiple DATA declarations separated by commas
        self.expect("DATA")
        self.accept(":")
        ...
        if self.accept("TYPE"):
            # Check for TABLE OF syntax
            if self.accept("TABLE"):
                self.expect("OF")
                base_type = self.expect("ID").value.upper()
                type_spec = f"TABLE OF {base_type}"
            else:
        ...
        # Check if there's a comma for another declaration
        if self.accept(","):
            # Return this declaration, parser will continue with next DATA
    ...
    def parse_form(self) -> FormDecl:
        ...
        if self.accept("USING"):
            while not self.accept("."):
                ...
                # Check for TYPE specification (optional)
                param_type = None
                if self.accept("TYPE"):
                    param_type = self.expect("ID").value.upper()
                
                params.append((param_name, "USING", param_type))
                if self.accept(","):
                    continue
                break
        ...
        if self.accept("CHANGING"):
            while not self.accept("."):
                ...
                params.append((param_name, "CHANGING", param_type))
                if self.accept(","):
                    continue
                break
