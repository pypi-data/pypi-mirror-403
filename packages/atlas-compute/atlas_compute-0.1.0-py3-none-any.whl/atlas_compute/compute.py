...
class ComputeClient:
    ...
    def __init__(self, username, password, warehouse_id):
        """Authenticates the current user to get a token"""
        self.warehouse_id = warehouse_id
        self.requests_repo = RequestsRepo(
            base_url="https://atlas-orchestrator.atlas.coreoutline.com"
        )
        try:
            self.token = self.requests_repo.authenticate(username, password)
    ...
    def get_credentials(self):
        ...
        """
        try:
            response = self.requests_repo.get(f"api/warehouses/{self.warehouse_id}/credentials")
    ...
    def create_client(self, client_type: str):
        ...
        assert client_type in ["warehouse", "search_and_analytics", "storage"]
        try:
            if client_type == "warehouse":
                ...
                return self.client
            elif client_type == "search_and_analytics":
                self.client = OpenSearch(
                    host=self.credentials["opensearch"]["host"],
                    port=int(self.credentials["opensearch"]["port"]),
                    user=self.credentials["opensearch"]["username"],
                    password=self.credentials["opensearch"]["password"],
                    secure=True,
                    verify=False
                )
                return self.client
                ...
                self.client = Minio(
                    endpoint=self.credentials["minio"]["host"]+":"+self.credentials["minio"]["port"],
