...
from aibanking import Jocall3, AsyncJocall3, DefaultAioHttpClient
from aibanking._utils import is_dict
...
logging.getLogger("aibanking").setLevel(logging.DEBUG)
...
def client(request: FixtureRequest) -> Iterator[Jocall3]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with Jocall3(base_url=base_url, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncJocall3]:
    ...
    async with AsyncJocall3(base_url=base_url, _strict_response_validation=strict, http_client=http_client) as client:
        yield client
