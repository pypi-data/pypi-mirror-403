...
# MoviePy import compatibility (v2 + v1)
try:
    # MoviePy v2: prefer top-level, but VideoClip may not be exported
    from moviepy import AudioFileClip  # usually exported in v2

    try:
        from moviepy import VideoClip  # sometimes NOT exported
    except Exception:
        from moviepy.video.VideoClip import VideoClip  # stable path
except Exception:
    # MoviePy v1 fallback
...
def _decode_audio_mono_ffmpeg(audio_path: str | Path, sr: int) -> np.ndarray:
    ...
    snd = np.frombuffer(p.stdout, dtype=np.float32)
    return snd


def _audio_to_rms_frames(
    audio_path: str | Path, fps: int, cfg: RenderConfig
) -> tuple[np.ndarray, float]:
    sr = getattr(cfg, "audio_sr", 22050)

    # Prefer decoding with ffmpeg for broad codec + MoviePy version compatibility.
    # (Some MoviePy versions on Py3.12 can error inside `to_soundarray`.)
    try:
        snd = _decode_audio_mono_ffmpeg(audio_path, sr=sr)
        duration = float(len(snd) / sr) if len(snd) else 0.0
    except Exception:
...
def render_call_video(
    ...
    def render_pixel_frame(amp: float) -> np.ndarray:
        ...
        if cfg.inner_accent and amp > 0.25:
            ...
            w2 = max(1, width // 2)
            d.ellipse(
                (cx - r2, cy - r2, cx + r2, cy + r2), outline=(180, 180, 180), width=w2
            )
    ...
    # Render silent video first, then mux original audio with ffmpeg.
    ...
    try:
        # MoviePy v1 uses `make_frame` + `set_fps`; v2 uses `frame_function` + `with_fps`.
        try:
            clip = VideoClip(make_frame=make_frame, duration=duration).set_fps(cfg.fps)
        except TypeError:
            clip = VideoClip(frame_function=make_frame, duration=duration).with_fps(
                cfg.fps
            )
