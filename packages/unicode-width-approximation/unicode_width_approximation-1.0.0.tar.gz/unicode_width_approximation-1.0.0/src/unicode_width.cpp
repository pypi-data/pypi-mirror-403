...
namespace unicode_width {
    ...
    static constexpr int NUM_WIDE_CHAR_RANGES = 125;

    static const std::int32_t WIDE_CHAR_RANGES[] = {
        ...
        0x2FF0, 0x2FFF, 0x3000, 0x3000, 0x3001, 0x303E, 0x3041, 0x3096, 0x3099, 0x30FF,
        0x3105, 0x312F, 0x3131, 0x318E, 0x3190, 0x31E5, 0x31EF, 0x321E, 0x3220, 0x3247,
        0x3250, 0xA48C, 0xA490, 0xA4C6, 0xA960, 0xA97C, 0xAC00, 0xD7A3, 0xF900, 0xFAFF,
        0xFE10, 0xFE19, 0xFE30, 0xFE52, 0xFE54, 0xFE66, 0xFE68, 0xFE6B, 0xFF01, 0xFF60,
        0xFFE0, 0xFFE6, 0x16FE0, 0x16FE4, 0x16FF0, 0x16FF6, 0x17000, 0x18CD5, 0x18CFF, 0x18D1E,
    ...
    static constexpr int NUM_ZERO_WIDTH_RANGES = 395;

    static const std::int32_t ZERO_WIDTH_RANGES[] = {
        0x0000, 0x001F, 0x007F, 0x009F, 0x00AD, 0x00AD, 0x0300, 0x036F, 0x0483, 0x0487,
        0x0488, 0x0489, 0x0591, 0x05BD, 0x05BF, 0x05BF, 0x05C1, 0x05C2, 0x05C4, 0x05C5,
        0x05C7, 0x05C7, 0x0600, 0x0605, 0x0610, 0x061A, 0x061C, 0x061C, 0x064B, 0x065F,
        0x0670, 0x0670, 0x06D6, 0x06DC, 0x06DD, 0x06DD, 0x06DF, 0x06E4, 0x06E7, 0x06E8,
        ...
        0x0829, 0x082D, 0x0859, 0x085B, 0x0890, 0x0891, 0x0897, 0x089F, 0x08CA, 0x08E1,
        0x08E2, 0x08E2, 0x08E3, 0x0902, 0x093A, 0x093A, 0x093C, 0x093C, 0x0941, 0x0948,
        ...
        0x180E, 0x180E, 0x180F, 0x180F, 0x1885, 0x1886, 0x18A9, 0x18A9, 0x1920, 0x1922,
        ...
        0x1A73, 0x1A7C, 0x1A7F, 0x1A7F, 0x1AB0, 0x1ABD, 0x1ABE, 0x1ABE, 0x1ABF, 0x1ADD,
        ...
        0x2028, 0x2028, 0x2029, 0x2029, 0x202A, 0x202E, 0x2060, 0x2064, 0x2066, 0x206F,
        0x20D0, 0x20DC, 0x20DD, 0x20E0, 0x20E1, 0x20E1, 0x20E2, 0x20E4, 0x20E5, 0x20F0,
        0x2CEF, 0x2CF1, 0x2D7F, 0x2D7F, 0x2DE0, 0x2DFF, 0x302A, 0x302D, 0x3099, 0x309A,
        0xA66F, 0xA66F, 0xA670, 0xA672, 0xA674, 0xA67D, 0xA69E, 0xA69F, 0xA6F0, 0xA6F1,
        ...
        0xABED, 0xABED, 0xFB1E, 0xFB1E, 0xFE00, 0xFE0F, 0xFE20, 0xFE2F, 0xFEFF, 0xFEFF,
        ...
        0x11F36, 0x11F3A, 0x11F40, 0x11F40, 0x11F42, 0x11F42, 0x11F5A, 0x11F5A, 0x13430, 0x1343F,
        ...
        0x1BCA0, 0x1BCA3, 0x1CF00, 0x1CF2D, 0x1CF30, 0x1CF46, 0x1D167, 0x1D169, 0x1D173, 0x1D17A,
    ...
    static constexpr int NUM_EMOJI_PRESENTATION_RANGES = 81;

    static const std::int32_t EMOJI_PRESENTATION_RANGES[] = {
    ...
    bool isWideChar(const std::int32_t code) {
        int l = 0, r = NUM_WIDE_CHAR_RANGES - 1;
        while (l <= r) {
            const int mid = l + (r - l) / 2;
            const auto start = WIDE_CHAR_RANGES[mid * 2];
            const auto end = WIDE_CHAR_RANGES[mid * 2 + 1];
    ...
    bool isZeroWidth(const std::int32_t code) {
        int l = 0, r = NUM_ZERO_WIDTH_RANGES - 1;
        while (l <= r) {
            const int mid = l + (r - l) / 2;
            const auto start = ZERO_WIDTH_RANGES[mid * 2];
            const auto end = ZERO_WIDTH_RANGES[mid * 2 + 1];
            if (start <= code && code <= end) {
                return true;
            }
            if (code > end) {
    ...
    static bool isEmojiPresentation(const std::int32_t code) {
        int l = 0, r = NUM_EMOJI_PRESENTATION_RANGES - 1;
        while (l <= r) {
            const int mid = l + (r - l) / 2;
            const auto start = EMOJI_PRESENTATION_RANGES[mid * 2];
            const auto end = EMOJI_PRESENTATION_RANGES[mid * 2 + 1];
            if (start <= code && code <= end) {
                return true;
            }
            if (code > end) {
    ...
    bool isWideCharBruteForce(const std::int32_t code) {
        for (int i = 0; i < NUM_WIDE_CHAR_RANGES; i++) {
            const auto start = WIDE_CHAR_RANGES[i * 2];
            const auto end = WIDE_CHAR_RANGES[i * 2 + 1];
            if (start <= code && code <= end) {
                return true;
            }
            if (code < start) {
    ...
    bool isZeroWidthBruteForce(const std::int32_t code) {
        for (int i = 0; i < NUM_ZERO_WIDTH_RANGES; i++) {
            const auto start = ZERO_WIDTH_RANGES[i * 2];
            const auto end = ZERO_WIDTH_RANGES[i * 2 + 1];
            if (start <= code && code <= end) {
                return true;
            }
            if (code < start) {
    ...
    int getCodepointWidth(const std::int32_t code) {
        if (isZeroWidth(code)) {
            return 0;
        }
        if (isWideChar(code)) {
    ...
    int getStringWidth(const std::string& s) {
        ...
        const auto clusters = grapheme_break::segmentGraphemeClusters(codepoints);
        int totalWidth = 0;
        for (const auto& cluster : clusters) {
            totalWidth += getCodepointWidth(cluster[0]);
        }
        return totalWidth;
    }
