...
from nancal_langchain_base.global_conf.context import Context
from nancal_langchain_base.log.common import is_prod, get_execute_mode
from nancal_langchain_base.log.config import LOG_DIR
from nancal_langchain_base.log.parser import LangGraphParser
...
try:
    ...
    LOG_FILE = os.path.join(FALLBACK_LOG_DIR, 'app.logs')
    print(f"Warning: Using fallback logs directory: {FALLBACK_LOG_DIR}, due to error: {e}", flush=True)
...
def write_log(log_entry):
    ...
    try:
        ...
        # 如果写入失败，打印到标准错误
        print(f"Failed to write logs: {e}", flush=True)
        # 尝试再次写入作为备选
        try:
            log_json = json.dumps(log_entry, ensure_ascii=False)
            ...
            print(f"Fallback logs write failed: {fallback_e}", flush=True)
...
def log_workflow_start(project_id, commit_id, log_id=None, execute_id="", input_data="", method=""):
    ...
    """
    event_type = "测试run开始" if not is_prod() else "开始运行"
    execute_mode = "测试run" if not is_prod() else "run"

    message = f"工作流开始执行 - {'测试运行' if is_prod() else '运行'}"
...
class Logger(BaseCallbackHandler):
    ...
    def on_chain_start_graph(
        ...
        if node_info is None:
            ...
            if node_name in self.parser.condition_funcs:
                ...
                write_log(log_entry)
                return
            logger.debug(f"节点 {node_name} 在图中不存在")
            return
        log_entry = create_log_entry(
            level="info",
            message=f"节点 '{node_info.name}' 已开始执行",
            input_data=_serialize_data(inputs),
    ...
    def on_chain_end_graph(
        ...
        if parent_run_id is None:  # 根节点
            ...
            log_entry = create_log_entry(
                level="info",
                message=f"节点 '{node_info.name}' 运行结束",
                output_data=_serialize_data(outputs),
