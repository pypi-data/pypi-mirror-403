"""Sessions API server for Azure Dynamic Sessions Custom Container.

This FastAPI server handles code execution requests from Azure Dynamic Sessions.
It receives Python code via POST /code/execute and returns execution results.

The code is executed in the same Python process with access to all installed
packages including the triagent module.
"""
...
app = FastAPI(
    title="Triagent Sessions API",
    description="Code execution endpoint for Azure Dynamic Sessions",
)


class CodeProperties(BaseModel):
    """Properties for code execution request."""
    codeInputType: str = "inline"
...
class ExecuteResult(BaseModel):
    ...
    stderr: str = ""
    result: Any = None
    executionTimeInMilliseconds: int = 0


class ExecuteResponse(BaseModel):
    """Code execution response."""
    properties: ExecuteResult
...
async def health():
    ...
    Returns:
        dict: Health check result with status, auth state, and session info
...
async def execute_code(request: ExecuteRequest):
    ...
    This endpoint mirrors Azure Dynamic Sessions code execution API.
