...
class SessionSummaryUpdateRequest(BaseModel):
    ...
    """ # noqa: E501
    stop_reason: Optional[Annotated[str, Field(min_length=0, strict=True, max_length=128)]] = Field(default=None, description="The reason the session was stopped", alias="_stopReason")
    stop_reason_code: Optional[StrictStr] = Field(default=None, description="The code of the stop reason", alias="_stopReasonCode")
    # BioT Code Start: Store custom attributes in a dedicated dict (safe default)
    additional_properties: Dict[str, Any] = Field(default_factory=dict)
    # BioT Code End
    __properties: ClassVar[List[str]] = ["_stopReason", "_stopReasonCode"]

    @field_validator('stop_reason_code')
    def stop_reason_code_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['COMPLETION', 'GENERAL_DEVICE_ERROR', 'TIMEOUT']):
            raise ValueError("must be one of enum values ('COMPLETION', 'GENERAL_DEVICE_ERROR', 'TIMEOUT')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of SessionSummaryUpdateRequest from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of SessionSummaryUpdateRequest from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "_stopReason": obj.get("_stopReason"),
            "_stopReasonCode": obj.get("_stopReasonCode")
        })
