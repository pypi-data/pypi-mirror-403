...
def _resample_categorical_agg(df: pd.DataFrame, n_out: int, agg_func: dict) -> pd.DataFrame:
    ...
    groups = pd.cut(indices, bins, right=False, labels=False)
    ...
    # 修复最后一个点可能的越界问题
    groups[groups == -1] = n_out - 1
    groups[np.isnan(groups)] = n_out - 1
    # 确保最后一个 bin 不溢出
    groups = np.minimum(groups, n_out - 1)
        
    # 执行聚合并重置索引
    resampled = df.groupby(groups).agg(agg_func)
    if len(resampled) != n_out:
        resampled = resampled.reindex(range(n_out))
