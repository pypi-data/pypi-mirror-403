"""This function calculates the F ndarray used in the AMA algorithm."""
...
def makeF(phi,cof,q,nlag,nlead,neq):
    ...
    F = np.zeros(shape=(nlead*neq,nlead*neq))
    ...
    # Construct the B_R ndarray used in the last column of F 
    newB_R = np.zeros(shape=(neq*nlead,neq))

    # Insert Identity ndarray into first element of newB_R
    ...
    for k in range(nlead,0,-1):
        # Calculate the ndarray to be entered into F and enter it
        newEntry = -phi @ H_plus @ newB_R
        F[neq*(nlead-1):neq*nlead,neq*(k-1):neq*k] = newEntry
        if k > 1:
            # Update newB_R for the next ndarray to be inserted by shifting down 
            # each element L rows
            ...
            newB_R[0:neq,0:neq] = np.zeros(shape=(neq,neq))
