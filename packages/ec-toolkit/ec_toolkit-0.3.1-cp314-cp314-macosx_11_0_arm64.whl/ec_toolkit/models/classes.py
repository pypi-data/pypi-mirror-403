...
class Compound:
    ...
    logger: logging.Logger = logging.getLogger("ec_toolkit.models.Compound")
    ...
    def enable_logging(cls, level: int = logging.INFO) -> None:
        ...
        if not cls.logger.handlers:
            ...
            h.setFormatter(logging.Formatter("[Compound] %(levelname)s: %(message)s"))
    ...
    def __init__(
        ...
        self.logger.debug(
            "Compound created: %s refs=%s converged=%s",
            self.formula,
            list(refs.keys()),
            converged,
        )
...
class ElementaryStep:
    ...
    apply_correction : bool
        If True, this is the correction step; properties `.d_e` and `.d_corr` raise AttributeError to intentionally hide the values for the correction step.
    """

    stoich: dict = field(default_factory=dict)
    label: str = ""
    is_electrochemical: bool = False
    apply_correction: bool = False

    logger: logging.Logger = logging.getLogger("ec_toolkit.models.ElementaryStep")

    def __post_init__(self) -> None:
        # validate stoich keys/values and coerce
        if not isinstance(self.stoich, dict):
            raise TypeError("stoich must be a dict[Compound, number]")
        # ensure keys are Compound-like (duck type) and values numeric
        new_stoich: dict = {}
        for k, v in self.stoich.items():
            # accept objects with .energy method (duck-typing) — this keeps it flexible
            if not hasattr(k, "energy"):
                raise TypeError(
                    "stoich keys must be Compound-like objects with an .energy() method"
                )
            try:
                new_stoich[k] = float(v)
            except Exception as exc:
                raise TypeError(
                    f"stoich coefficient for {k!r} is not numeric: {exc}"
                ) from exc
        self.stoich = new_stoich
        self.logger.debug(
            "ElementaryStep %r created: %d species, apply_correction=%s",
            self.label,
            len(self.stoich),
    ...
    def enable_logging(cls, level: int = logging.INFO) -> None:
        ...
        if not cls.logger.handlers:
            h = logging.StreamHandler()
            h.setFormatter(
                logging.Formatter("[ElementaryStep] %(levelname)s: %(message)s")
            )
            cls.logger.addHandler(h)
        cls.logger.propagate = False
    ...
    def d_corr(self) -> float:
        """Combined vibrational correction contribution (ZPE + E_th - T*S)."""
        if self.apply_correction:
            raise AttributeError(
...
class Mechanism:
    ...
    Constructor:
        Mechanism(steps: list[ElementaryStep], eq_pot: float, sym_fac: int = 1, ref_el: str = "RHE", *, is_oxidation_reaction: bool)
    ...
    logger: logging.Logger = logging.getLogger("ec_toolkit.models.Mechanism")
    ...
    def enable_logging(cls, level: int = logging.INFO) -> None:
        ...
        if not cls.logger.handlers:
            h = logging.StreamHandler()
            h.setFormatter(logging.Formatter("[Mechanism] %(levelname)s: %(message)s"))
            cls.logger.addHandler(h)
        cls.logger.propagate = False
    ...
    def __init__(
        ...
        *,
        ...
        # [snip — keep the same validation and caching logic you already have]
        self.logger.debug(
            "Mechanism created: %d steps, eq_pot=%.3f", len(steps), eq_pot
        )
